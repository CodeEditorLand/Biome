...
use biome_service::{settings::WorkspaceSettings, workspace::DocumentFileSource};
...
impl FromStr for CodeBlockTest {
	...
	fn from_str(input:&str) -> anyhow::Result<Self> {
		// This is based on the parsing logic for code block languages in
		// `rustdoc`: https://github.com/rust-lang/rust/blob/6ac8adad1f7d733b5b97d1df4e7f96e73a46db42/src/librustdoc/html/markdown.rs#L873
		let tokens = input.split([',', ' ', '\t']).map(str::trim).filter(|token| !token.is_empty());

		let mut test = CodeBlockTest { tag:String::new(), expect_diagnostic:false, ignore:false };
	...
	match test.document_file_source() {
		...
		DocumentFileSource::Json(file_source) => {
			...
			if parse.has_errors() {
				...
				let options =
					AnalyzerOptions { file_path:PathBuf::from(&file_path), ..Default::default() };
		...
		DocumentFileSource::Html(..) => {
			todo!("HTML analysis is not yet supported")
		},
		DocumentFileSource::Grit(..) => {
			todo!("Grit analysis is not yet supported")
		},
	...
	for event in parser {
		match event {
			...
			Event::Start(Tag::CodeBlock(CodeBlockKind::Fenced(meta))) => {
				// Track the content of code blocks to pass them through the
				// analyzer
