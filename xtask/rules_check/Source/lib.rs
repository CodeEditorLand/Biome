...
use biome_analyze::{
	...
	GroupCategory, Queryable, RegistryVisitor, Rule, RuleCategory, RuleFilter,
...
pub fn check_rules() -> anyhow::Result<()> {
	...
	impl RegistryVisitor<JsonLanguage> for LintRulesVisitor {
		fn record_category<C: GroupCategory<Language = JsonLanguage>>(
			&mut self,
		) {
			if matches!(C::CATEGORY, RuleCategory::Lint) {
				C::record_groups(self);
	...
	impl RegistryVisitor<CssLanguage> for LintRulesVisitor {
		fn record_category<C: GroupCategory<Language = CssLanguage>>(
			&mut self,
		) {
			if matches!(C::CATEGORY, RuleCategory::Lint) {
				C::record_groups(self);
...
fn assert_lint(
	...
	match test.document_file_source() {
				...
				biome_js_analyze::analyze(
					...
					|signal| {
						if let Some(mut diag) = signal.diagnostic() {
							let category = diag
								.category()
								.expect("linter diagnostic has no code");
		...
		DocumentFileSource::Css(..) => {
			let parse =
				biome_css_parser::parse_css(code, CssParserOptions::default());
