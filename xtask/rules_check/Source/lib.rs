...
use biome_service::{
	settings::{ServiceLanguage, WorkspaceSettings},
...
impl FromStr for CodeBlockTest {
	...
	fn from_str(input:&str) -> anyhow::Result<Self> {
		...
		let tokens = input.split([',', ' ', '\t']).map(str::trim).filter(|token| !token.is_empty());
	...
	pub fn write_diagnostic(&mut self, diag:biome_diagnostics::Error) -> anyhow::Result<()> {
		...
		if self.test.expect_diagnostic {
			if self.all_diagnostics.len() > 1 {
				...
				bail!(
					"Analysis of '{group}/{rule}' on the following code block returned multiple \
					 diagnostics.\n\n{code}"
				);
			}
		} else {
			// ...or if the analysis returns a diagnostic when it is expected to not report
			// one.
			self.print_all_diagnostics();
			...
			bail!(
				"Analysis of '{group}/{rule}' on the following code block returned an unexpected \
				 diagnostic.\n\n{code}"
			);
		}
...
/// exactly zero or one diagnostic depending on the value of
/// `expect_diagnostic`. That diagnostic is then emitted as text into the
/// `content` buffer
fn assert_lint(
	group:&'static str,
	rule:&'static str,
	...
	if test.use_options {
		let Some(partial_config) = config else {
			bail!(
				"Code blocks tagged with 'use_options' must be preceded by a valid 'json,options' \
				 code block."
			);
		};

		settings.get_current_settings_mut().merge_with_configuration(
				...
				biome_js_analyze::analyze(&root, filter, &options, file_source, None, |signal| {
					if let Some(mut diag) = signal.diagnostic() {
						...
						let severity = settings
							...
							.expect(
								"If you see this error, it means you need to run cargo \
	...
	if test.expect_diagnostic {
		// Fail the test if the analysis didn't emit any diagnostic
			...
			"Analysis of '{group}/{rule}' on the following code block returned no \
			 diagnostics.\n\n{code}",
		);
	}
	...
	if diagnostics.has_error {
		bail!(
			"A code snippet must emit one single diagnostic, but it seems multiple diagnostics \
			 were emitted. Make sure that all the snippets inside the code block \
			 'expect_diagnostic' emit only one diagnostic."
		)
	}
...
fn get_first_member<V:Into<AnyJsonValue>>(parent:V, expected_name:&str) -> Option<AnyJsonValue> {
	...
	if member_name.as_str() == expected_name { member.value().ok() } else { None }
}
	...
	match test.document_file_source() {
		DocumentFileSource::Json(file_source) => {
			...
			let Some(result) = partial_configuration else {
				bail!(
					"Failed to deserialize configuration options for '{group}/{rule}' from the \
					 following code block due to unknown error.\n\n{code}"
				);
			};
		...
		_ => {
			...
			bail!(
				"The following non-JSON code block for '{group}/{rule}' was marked as containing \
				 configuration options. Only JSON code blocks can used to provide configuration \
				 options.\n\n{code}"
			);
		},
	}
...
/// Parse the documentation fragment for a lint rule (in markdown) and lint the
/// code blocks.
