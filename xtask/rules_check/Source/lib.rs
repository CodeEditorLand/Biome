...
use biome_analyze::{
	AnalysisFilter, AnalyzerOptions, ControlFlow, GroupCategory, Queryable, RegistryVisitor, Rule, RuleCategory,
...
pub fn check_rules() -> anyhow::Result<()> {
	...
	impl RegistryVisitor<JsonLanguage> for LintRulesVisitor {
		...
		fn record_rule<R>(&mut self)
		where
			R: Rule<Options: Default, Query: Queryable<Language = JsonLanguage, Output: Clone>> + 'static,
		{
			self.push_rule::<R, <R::Query as Queryable>::Language>()
		}
	}
	...
	impl RegistryVisitor<CssLanguage> for LintRulesVisitor {
		...
		fn record_rule<R>(&mut self)
		where
			R: Rule<Options: Default, Query: Queryable<Language = CssLanguage, Output: Clone>> + 'static,
		{
			self.push_rule::<R, <R::Query as Queryable>::Language>()
		}
	}
	...
	impl RegistryVisitor<GraphqlLanguage> for LintRulesVisitor {
		...
		fn record_rule<R>(&mut self)
		where
			R: Rule<Options: Default, Query: Queryable<Language = GraphqlLanguage, Output: Clone>> + 'static,
		{
			self.push_rule::<R, <R::Query as Queryable>::Language>()
		}
	}
...
impl<'a> DiagnosticWriter<'a> {
	pub fn new(group: &'a str, rule: &'a str, test: &'a CodeBlockTest, code: &'a str) -> DiagnosticWriter<'a> {
		DiagnosticWriter {
			group,
			rule,
			test,
	...
	match test.document_file_source() {
		DocumentFileSource::Js(file_source) => {
			...
			let (code, file_source) = match file_source.as_embedding_kind() {
				EmbeddingKind::Astro => {
					(biome_service::file_handlers::AstroFileHandler::input(code), JsFileSource::ts())
				},
			...
			if parse.has_errors() {
				...
				let options = create_analyzer_options::<JsLanguage>(&workspace_settings, project_key, &file_path, test);

				let services = JsAnalyzerServices::from((Default::default(), Default::default(), file_source));
		...
		DocumentFileSource::Json(file_source) => {
			...
			if parse.has_errors() {
				...
				let options =
					create_analyzer_options::<JsonLanguage>(&workspace_settings, project_key, &file_path, test);
...
fn make_json_object_with_single_member<V: Into<AnyJsonValue>>(name: &str, value: V) -> JsonObjectValue {
...
fn parse_documentation(group: &'static str, rule: &'static str, docs: &'static str) -> anyhow::Result<()> {
