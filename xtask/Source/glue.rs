...
pub fn run_process(
	...
	run_process_inner(&cmd, echo, stdin)
		.with_context(|| format!("process `{cmd}` failed"))
}
...
fn shelx(cmd: &str) -> Vec<String> {
	let mut res = Vec::new();
	for (string_piece, in_quotes) in
		cmd.split('\'').zip([false, true].iter().copied().cycle())
	{
		if in_quotes {
			res.push(string_piece.to_string())
		} else if !string_piece.is_empty() {
			res.extend(
				string_piece.split_ascii_whitespace().map(|it| it.to_string()),
			)
		}
	}
	res
