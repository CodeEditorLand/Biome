...
fn main() {
	let result = Command::new("cargo").args(["bench", "-p", "xtask_libs_bench"]).output();
	...
	for (k, v_setup) in tests.iter() {
		if let Some(name) = k.strip_suffix("_setup") {
			...
			println!("{name}");
			println!("\tdiff inst: {} is {} - {}", v_all.0 - v_setup.0, v_all.0, v_setup.0);
			println!("\tdiff l1: {} is {} - {}", v_all.1 - v_setup.1, v_all.1, v_setup.1);
			println!("\tdiff l2: {} is {} - {}", v_all.2 - v_setup.2, v_all.2, v_setup.2);
			println!("\tdiff ram: {} is {} - {}", v_all.3 - v_setup.3, v_all.3, v_setup.3);
			println!("\tdiff cycles: {} is {} - {}", v_all.4 - v_setup.4, v_all.4, v_setup.4);
		}
	}
}
