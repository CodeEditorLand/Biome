...
impl From<TestRunOutcome> for Outcome {
	fn from(run_outcome: TestRunOutcome) -> Self {
		match run_outcome {
			...
			TestRunOutcome::IncorrectlyPassed(_) | TestRunOutcome::IncorrectlyErrored { .. } => Outcome::Failed,
...
impl TestCaseFiles {
	pub(crate) fn single(name: String, code: String, source_type: JsFileSource, options: JsParserOptions) -> Self {
	...
	pub(crate) fn add(&mut self, name: String, code: String, source_type: JsFileSource, options: JsParserOptions) {
...
pub(crate) fn run_test_suite(test_suite: &dyn TestSuite, context: &mut TestRunContext) -> TestResults {
	...
	context.pool.scoped(|scope| {
		scope.execute(|| {
			...
			for result in rx {
				...
				results.push(TestResult { test_case: result.test_case, outcome: result.outcome.into() });
			}
				...
				let outcome = match std::panic::catch_unwind(|| test_ref.run()) {
					...
					Err(panic) => {
						let error = panic
							...
							.unwrap_or_default();
						tracing::error!(panic = error.as_str(), name = test.name(), "Test panicked");
	...
	context.reporter.test_suite_completed(&instance, &test_results);
