...
pub fn coverage_compare(base_result_path: Option<&str>, new_result_path: Option<&str>, markdown: bool) {
...
fn read_test_results(path: &Path, name: &'static str) -> HashMap<String, TestResults> {
	let buffer = fs::read(path).unwrap_or_else(|err| panic!("Can't read the file of the {name} results: {err:?}"));
