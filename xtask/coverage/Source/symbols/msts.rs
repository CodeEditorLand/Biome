	...
	fn run(&self) -> TestRunOutcome {
		...
		let mut actual:Vec<_> = biome_js_semantic::semantic_events(r.syntax())
			...
			.filter(|x| {
				...
				// We do the same below because TS classifies some string
				// literals as symbols and we also filter them below.
				match x {
		...
		if expected.symbols.len() != actual.len() {
			TestRunOutcome::IncorrectlyErrored { files:t, errors:vec![] }
		} else {
			for (expected, actual) in expected.symbols.iter().zip(actual) {
				let are_names_eq = expected.name == code[actual.range()].trim();
				if !are_names_eq {
					return TestRunOutcome::IncorrectlyErrored { files:t, errors:vec![] };
				}
			}
...
impl TestSuite for SymbolsMicrosoftTestSuite {
	fn name(&self) -> &str { "symbols/microsoft" }
...
fn parse_symbol(input:&str) -> Option<Symbol> {
	...
	Some(Symbol { name:name.to_string(), path:path.to_string(), decls })
}
...
/// and if the line contains any symbols, one line per symbol as described by
/// the method [parse_symbol];
fn load_symbols_file(txt:&str) -> SymbolsFile {
	...
	SymbolsFile { code_file:PathBuf::from(code_file), symbols }
}
