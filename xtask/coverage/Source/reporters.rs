...
use biome_diagnostics::{termcolor::Buffer, DiagnosticExt, Error};
use colored::Colorize;
use indicatif::ProgressBar;
use serde_json::Value;

use crate::{
	runner::{TestCaseFiles, TestRunOutcome, TestRunResult, TestSuite, TestSuiteInstance},
	Summary,
	TestResults,
};
...
pub(crate) trait TestReporter: Send + Sync {
	...
	/// Called after all tests for a test suite have been loaded, right before
	/// the runner executes the tests
...
impl Default for DefaultReporter {
	fn default() -> Self { Self { pb:ProgressBar::hidden(), start:Instant::now() } }
}

impl TestReporter for DefaultReporter {
	fn test_suite_started(&mut self, _suite:&dyn TestSuite) { self.start = Instant::now(); }
	...
	fn test_loaded(&mut self) { self.pb.inc(1); }
	...
	fn test_completed(&mut self, result:&TestRunResult) {
		...
		self.pb.println(format!("{} {}", label, result.test_case.blue()));
	}
...
pub enum SummaryDetailLevel {
	...
	Coverage,
	/// Prints the coverage table as well as all failing tests with their
	/// diagnostics
	Failing,

	/// Prints the RAST of the parsed syntax and the diagnostics for all tests
	/// (including tests that pass with expected diagnostics).
	Debug,
}
		...
		Ok(match s {
			...
			_ => {
				return Err(String::from(
					"Unknown summary detail level. Valid values are: 'coverage', 'failing, and \
					 'rast'.",
				));
			},
		})
...
impl SummaryDetailLevel {
	fn is_coverage_only(&self) -> bool { matches!(self, SummaryDetailLevel::Coverage) }

	fn is_debug(&self) -> bool { matches!(self, SummaryDetailLevel::Debug) }
}

/// Reporter that prints a summary for each phase (tests loaded, test suite
/// completed) to the console output
...
impl OutputTarget {
	pub fn stdout() -> Self { OutputTarget::Stdout(std::io::stdout()) }

	pub fn stderr() -> Self { OutputTarget::Stderr(std::io::stderr()) }
}
...
impl SummaryReporter {
	pub fn new(detail_level:SummaryDetailLevel, output_target:OutputTarget) -> Self {
		...
		Self { results:HashMap::default(), buffer, output_target, detail_level }
	}

	fn writeln(&mut self, msg:&str) { writeln!(self.buffer, "{msg}").unwrap(); }
	...
	fn summary_table(results:HashMap<String, Summary>) -> String {
		...
		let offset = if has_multiple_test_suites {
			table.column(0).set_header("Test suite").set_align(Align::Left);
			1
		} else {
			0
		};

		table.column(offset).set_header("Tests ran".to_string()).set_align(Align::Right);
		table.column(offset + 1).set_header("Passed").set_align(Align::Right);
		table.column(offset + 2).set_header("Failed").set_align(Align::Right);
		table.column(offset + 3).set_header("Panics").set_align(Align::Right);
		table.column(offset + 4).set_header("Coverage").set_align(Align::Right);
		...
		let rows = results.into_iter().map(|(suite, summary)| {
			...
			let mut values = if has_multiple_test_suites { vec![suite] } else { Vec::default() };
...
impl TestReporter for SummaryReporter {
	fn test_completed(&mut self, result:&TestRunResult) {
		...
		match &result.outcome {
					...
					for file in files {
						if let Some(errors) = file.parse().ok().err() {
							all_errors.extend(errors.into_iter().map(|error| {
								error.with_file_path(file.name()).with_file_source_code(file.code())
							}));
						}
					}
			...
			TestRunOutcome::IncorrectlyErrored { errors, files } => {
				...
				let errors:Vec<_> =
					errors.iter().map(|diagnostic| Error::from(diagnostic.clone())).collect();
	...
	fn test_suite_completed(&mut self, suite:&TestSuiteInstance, results:&TestResults) {
		self.results.insert(suite.name().to_string(), results.summary.clone());
	}
	...
	fn run_completed(&mut self) {
		...
		self.output_target.write_all(self.buffer.as_slice()).unwrap();
...
impl TestReporter for JsonReporter {
	fn test_suite_completed(&mut self, suite:&TestSuiteInstance, result:&TestResults) {
		self.results
			.insert(suite.name().to_string(), serde_json::to_value(result).unwrap());
	}
...
impl MulticastTestReporter {
	pub fn new(reporter:Box<dyn TestReporter>) -> Self { Self(vec![reporter]) }

	pub fn add(&mut self, reporter:Box<dyn TestReporter>) { self.0.push(reporter); }
}
