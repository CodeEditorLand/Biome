...
impl Default for DefaultReporter {
	fn default() -> Self {
		Self { pb: ProgressBar::hidden(), start: Instant::now() }
	}
}
...
impl TestReporter for DefaultReporter {
	...
	fn test_suite_run_started(&mut self, suite: &TestSuiteInstance) {
		...
		self.pb = ProgressBar::new(suite.len() as u64).with_message(format!("{} tests", "Running".bold().cyan()));
...
impl TestReporter for SummaryReporter {
	fn test_completed(&mut self, result: &TestRunResult) {
		...
		match &result.outcome {
					...
					for file in files {
						if let Some(errors) = file.parse().ok().err() {
							all_errors.extend(
								errors
									.into_iter()
									.map(|error| error.with_file_path(file.name()).with_file_source_code(file.code())),
							);
						}
					}
			...
			TestRunOutcome::IncorrectlyPassed(_) => {
				self.writeln(&format!("{} {}: Incorrectly passed", "[FAIL]".bold().red(), result.test_case));
			},
			TestRunOutcome::IncorrectlyErrored { errors, files } => {
				self.writeln(&format!("{} {}: Incorrectly errored:", "[FAIL]".bold().red(), result.test_case));
				let errors: Vec<_> = errors.iter().map(|diagnostic| Error::from(diagnostic.clone())).collect();
		...
		if self.detail_level.is_debug() {
			if let Some(files) = result.outcome.files() {
				for file in files {
					...
					self.writeln(&format!("RAST Output for {}:\n{:#?}\n", &file.name().bold(), program.syntax()));
				}
			}
		}
	}
