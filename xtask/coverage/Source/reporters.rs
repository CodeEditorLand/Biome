use crate::runner::{
	TestCaseFiles, TestRunOutcome, TestRunResult, TestSuite, TestSuiteInstance,
};
...
pub(crate) trait TestReporter: Send + Sync {
	...
	fn test_suite_completed(
		&mut self,
		_suite: &TestSuiteInstance,
		_result: &TestResults,
	) {
	}
				...
				if self.detail_level.is_debug() {
					...
					for file in files {
						if let Some(errors) = file.parse().ok().err() {
							all_errors.extend(errors.into_iter().map(
								|error| {
									error
