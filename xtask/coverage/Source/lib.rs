...
use ts::{ts_babel::BabelTypescriptTestSuite, ts_microsoft::MicrosoftTypescriptTestSuite};
use util::decode_maybe_utf16_string;

pub use crate::reporters::SummaryDetailLevel;
use crate::{
	js::test262::Test262TestSuite,
	reporters::{
		DefaultReporter,
		JsonReporter,
		MulticastTestReporter,
		OutputTarget,
		SummaryReporter,
		TestReporter,
	},
	runner::{run_test_suite, TestRunContext, TestSuite},
};
...
impl Default for TestResults {
	fn default() -> Self {
		Self {
			summary:Summary { tests_ran:0, passed:0, failed:0, panics:0, coverage:0.0 },
			details:vec![],
		}
	}
}
...
impl TestResults {
	...
	pub fn errored_tests(&self) -> usize {
		self.details.iter().filter(|res| matches!(res.outcome, Outcome::Failed)).count()
	}

	pub fn passed_tests(&self) -> usize {
		self.details.iter().filter(|res| res.outcome == Outcome::Passed).count()
	}
}

pub fn run(suites:Option<&str>, filter:Option<&str>, json:bool, detail_level:SummaryDetailLevel) {
	...
	let mut context = TestRunContext {
		...
		pool:&yastl::Pool::new(std::thread::available_parallelism().map_or(2, usize::from).max(2)),
	};
...
fn check_file_encoding(path:&std::path::Path) -> Option<String> {
	...
	decode_maybe_utf16_string(&buffer).ok().map(|decoded| decoded.to_string())
}
