...
use crate::reporters::{
	DefaultReporter, JsonReporter, MulticastTestReporter, OutputTarget,
...
pub fn run(
	...
	let mut reporters =
		MulticastTestReporter::new(Box::<DefaultReporter>::default());
...
fn get_test_suites(suites: Option<&str>) -> Vec<Box<dyn TestSuite>> {
	...
	while let Some(id) = ids.pop() {
		match id {
			...
			ALL_TS_SUITES | "typescript" => {
				ids.extend(["ts/microsoft", "ts/babel"])
			},
