		...
		match (base_result, new_result) {
			...
			(Some(base), Some(new)) => {
				match (&base.outcome, &new.outcome) {
					...
					(Outcome::Failed, Outcome::Panicked) => {
						report_diff.failed_to_panic.push(new)
					},
					(Outcome::Panicked, Outcome::Failed) => {
						report_diff.panic_to_failed.push(new)
					},
