...
pub fn emit_compare(
	...
	if markdown {
		/// Generates a proper diff format, with some bold text if things
		/// change.
		...
		match (base_result, new_result) {
			...
			(Some(base), Some(new)) => {
				match (&base.outcome, &new.outcome) {
					...
					// we want to ignore cases where both results yield the same
					// enum this means that their status hasn't changed,
					// not worth tracking
					_ => {},
				}
			},
