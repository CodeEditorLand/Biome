...
pub fn emit_compare(base_results: &TestResults, new_results: &TestResults, test_suite: &str, markdown: bool) {
	...
	if markdown {
		...
		println!(
			...
			format_args!(
				"{}{}{:.2}%{}",
				if coverage_diff.abs() > f64::EPSILON { "**" } else { "" },
		...
		fn summary(title: &str, tests: &[&TestResult]) {
			if !tests.is_empty() {
				println!();
				println!("<details><summary><b>{} ({}):</b></summary>", title, tests.len());
				println!("\n```");
		...
		summary(":interrobang: Panic To Failed", &report_diff.panic_to_failed);
		summary(":heavy_plus_sign: Added Tests", &report_diff.added_tests);
		summary(":heavy_minus_sign: Removed Tests", &report_diff.removed_tests);
	} else {
		let mut table = AsciiTable::default();

		println!("{test_suite} conformance changes:");

		table.column(0).set_header("Tests result").set_align(Align::Left);
		table.column(1).set_header("main branch").set_align(Align::Right);
		table.column(2).set_header("PR").set_align(Align::Right);
		table.column(3).set_header("Difference").set_align(Align::Right);

		let passed_diff = base_passed - new_passed;
		let failed_diff = base_failed - new_failed;
		let panics_diff = base_panics - new_panics;
		let passed_row: Vec<&dyn std::fmt::Display> = vec![&"Passed", &base_passed, &new_passed, &passed_diff];
		let failed_row: Vec<&dyn std::fmt::Display> = vec![&"Failed", &base_failed, &new_failed, &failed_diff];
		let panics_row: Vec<&dyn std::fmt::Display> = vec![&"Panics", &base_panics, &new_panics, &panics_diff];
...
fn compare_diffs<'a>(base_results: &'a TestResults, new_results: &'a TestResults) -> ReportDiff<'a> {
		...
		match (base_result, new_result) {
			...
			(Some(base), Some(new)) => {
				match (&base.outcome, &new.outcome) {
					(Outcome::Passed, Outcome::Failed | Outcome::Panicked) => report_diff.regression.push(new),
					(Outcome::Failed | Outcome::Panicked, Outcome::Passed) => report_diff.fixed.push(new),
