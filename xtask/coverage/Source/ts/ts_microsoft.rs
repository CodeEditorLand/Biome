use std::{borrow::Cow, convert::TryFrom, fmt::Write, io, path::Path, process::Command};
...
use crate::{
	check_file_encoding,
	runner::{
		create_bogus_node_in_tree_diagnostic,
		TestCase,
		TestCaseFiles,
		TestRunOutcome,
		TestSuite,
	...
	fn run(&self) -> TestRunOutcome {
		...
		for file in &files {
			match file.parse().ok() {
				Ok(root) => {
					if let Some(bogus) =
						root.syntax().descendants().find(|descendant| descendant.kind().is_bogus())
					{
		...
		if all_errors.is_empty() && expected_errors {
			...
			TestRunOutcome::IncorrectlyErrored { errors:all_errors, files }
		} else if !bogus_errors.is_empty() {
			TestRunOutcome::IncorrectlyErrored { errors:bogus_errors, files }
		} else {
...
impl TestSuite for MicrosoftTypescriptTestSuite {
	fn name(&self) -> &str { "ts/microsoft" }

	fn base_path(&self) -> &str { CASES_PATH }
...
/// These files start with `// @<option-name>: <option-value>` and are followed
/// by the file's content. This function extracts the individual files with
/// their content and drops unsupported files.
fn extract_metadata(code:&str, path:&str) -> TestCaseMetadata {
	// Returns a match for a test option. Test options have the form `// @name:
	// value`
	let options_regex =
...
fn add_file_if_supported(files:&mut TestCaseFiles, name:String, content:String) {
	...
	if let Ok(mut source_type) = JsFileSource::try_from(path) {
		...
		// A very basic heuristic to determine if a module is a `Script` or a
		// `Module`. The TypeScript parser automatically detects whatever a
		// file is a module or a script by the presence of any module syntax.
		// Rome's parser doesn't support this today because it would require
		// moving any "strict mode" or "module" specific checks into a second
		// compiler pass. The reason this is needed is that the module syntax
		// may appear at the very end of the file after the parser has already
		// processed some syntax that is invalid in strict mode (for example,
		// an "arguments" variable).
...
fn should_error(name:&str, run_options:&[String]) -> bool {
	let error_reference_file = Path::new(REFERENCE_PATH)
		.join(Path::new(name).with_extension("errors.txt").file_name().unwrap());
