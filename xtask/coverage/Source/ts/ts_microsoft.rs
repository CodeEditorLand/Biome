...
fn add_file_if_supported(files:&mut TestCaseFiles, name:String, content:String) {
	...
	if let Ok(mut source_type) = JsFileSource::try_from(path) {
		...
		// The TypeScript parser automatically detects whatever a file is a module or a
		// script by the presence of any module syntax. Rome's parser doesn't support
		// this today because it would require moving any "strict mode" or "module"
		// specific checks into a second compiler pass. The reason this is needed is
		// that the module syntax may appear at the very end of the file after the
		// parser has already processed some syntax that is invalid in strict mode
		// (for example, an "arguments" variable).
