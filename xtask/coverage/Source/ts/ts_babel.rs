...
impl BabelTypescriptTestCase {
	fn new(path:&Path, code:String, expected_to_fail:bool, variant:LanguageVariant) -> Self {
		let name = path.parent().unwrap().strip_prefix(CASES_PATH).unwrap().display().to_string();

		Self { name, code, expected_to_fail, variant }
	}
}
...
impl TestCase for BabelTypescriptTestCase {
	...
	fn run(&self) -> TestRunOutcome {
		...
		if self.expected_to_fail && result.diagnostics().is_empty() {
			...
			TestRunOutcome::IncorrectlyErrored { files, errors:result.diagnostics().to_vec() }
		} else if let Some(bogus) =
			result.syntax().descendants().find(|descendant| descendant.kind().is_bogus())
		{
...
impl TestSuite for BabelTypescriptTestSuite {
	...
	fn load_test(&self, path:&std::path::Path) -> Option<Box<dyn crate::runner::TestCase>> {
		...
		Some(Box::new(BabelTypescriptTestCase::new(path, code, should_fail, variant)))
	}
}
