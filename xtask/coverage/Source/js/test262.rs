...
use biome_rowan::{syntax::SyntaxKind, AstNode};
use regex::Regex;
...
impl Test262TestCase {
	...
	fn execute_test(&self, append_use_strict:bool, source_type:JsFileSource) -> TestRunOutcome {
		...
		let should_fail =
			self.meta.negative.as_ref().filter(|neg| neg.phase == Phase::Parse).is_some();
...
impl TestSuite for Test262TestSuite {
	fn name(&self) -> &str { "js/262" }

	fn base_path(&self) -> &str { BASE_PATH }
	...
	fn load_test(&self, path:&Path) -> Option<Box<dyn TestCase>> {
		...
		if !meta.negative.as_ref().map_or(true, |negative| negative.phase == Phase::Parse) {
			None
		} else {
