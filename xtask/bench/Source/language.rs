...
use biome_analyze::{
	AnalysisFilter, AnalyzerConfiguration, AnalyzerOptions, ControlFlow, Never, RuleCategoriesBuilder,
};
...
impl Parse<'_> {
	...
	pub fn parse(&self) -> Parsed {
		match self {
			...
			Parse::Json(code) => Parsed::Json(biome_json_parser::parse_json(code, JsonParserOptions::default())),
	...
	pub fn parse_with_cache(&self, cache: &mut NodeCache) -> Parsed {
		match self {
			Parse::JavaScript(source_type, code) => Parsed::JavaScript(
				biome_js_parser::parse_js_with_cache(code, *source_type, JsParserOptions::default(), cache),
				*source_type,
			),
			...
			Parse::Graphql(code) => Parsed::Graphql(biome_graphql_parser::parse_graphql_with_cache(code, cache)),
			Parse::Html(code) => Parsed::Html(biome_html_parser::parse_html_with_cache(code, cache)),
		}
	}
}
...
impl Parsed {
	pub fn format_node(&self) -> Option<FormatNode> {
		match self {
			Parsed::JavaScript(parse, source_type) => Some(FormatNode::JavaScript(parse.syntax(), *source_type)),
	...
	pub fn format_node(&self) -> FormatResult<FormattedNode> {
		match self {
			Self::JavaScript(root, source_type) => {
				biome_js_formatter::format_node(JsFormatOptions::new(*source_type), root).map(FormattedNode::JavaScript)
			},
			Self::Json(root) => {
				biome_json_formatter::format_node(JsonFormatOptions::default(), root).map(FormattedNode::Json)
			},
			Self::Css(root) => {
				biome_css_formatter::format_node(CssFormatOptions::default(), root).map(FormattedNode::Css)
			},
			FormatNode::Graphql(root) => {
				biome_graphql_formatter::format_node(GraphqlFormatOptions::default(), root).map(FormattedNode::Graphql)
			},
			FormatNode::Html(root) => {
				biome_html_formatter::format_node(HtmlFormatOptions::default(), root).map(FormattedNode::Html)
			},
		}
	}
}
	...
	pub fn analyze(&self) {
		match self {
			Analyze::JavaScript(root) => {
				...
				biome_js_analyze::analyze(root, filter, &options, &[], Default::default(), |event| {
