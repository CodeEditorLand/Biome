...
impl TestCase {
	pub fn try_from(file_url: &str) -> Result<TestCase, String> {
		...
		let path =
			Utf8Path::new(&env::var("CARGO_MANIFEST_DIR").unwrap_or_else(|_| env!("CARGO_MANIFEST_DIR").to_owned()))
				.ancestors()
				.nth(2)
				.unwrap()
				.join("target")
		...
		let content = std::fs::read_to_string(&path).map_err(err_to_string).or_else(|_| {
		...
		content.map(|code| {
			...
			TestCase { id: filename.to_string(), code, path }
		})
	}
	...
	pub fn extension(&self) -> &str {
		self.path.extension().expect("Expected test case to have extension")
	}
}
