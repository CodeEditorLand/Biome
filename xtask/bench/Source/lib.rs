...
pub use crate::{language::Parse, test_case::TestCase};
...
pub fn err_to_string<E:std::fmt::Debug>(e:E) -> String { format!("{e:?}") }
...
pub fn bench_parser_group(group:&mut BenchmarkGroup<WallTime>, test_case:TestCase) {
	...
	group.bench_with_input(BenchmarkId::new(test_case.filename(), "cached"), &code, |b, _| {
		b.iter_batched(
			|| {
...
pub fn bench_formatter_group(group:&mut BenchmarkGroup<WallTime>, test_case:TestCase) {
	...
	group.throughput(Throughput::Bytes(code.len() as u64));
	group.bench_with_input(BenchmarkId::from_parameter(test_case.filename()), code, |b, _| {
