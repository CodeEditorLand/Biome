...
use biome_js_syntax::{
	AnyJsBinding, AnyJsBindingPattern, AnyJsCallArgument, AnyJsDeclaration,
	AnyJsDeclarationClause, AnyJsExportClause, AnyJsExpression,
	AnyJsFormalParameter, AnyJsImportClause, AnyJsLiteralExpression,
	AnyJsModuleItem, AnyJsName, AnyJsNamedImportSpecifier, AnyJsObjectMember,
	AnyJsObjectMemberName, AnyJsParameter, AnyJsStatement, AnyTsName,
	AnyTsReturnType, AnyTsType, AnyTsTypeMember, JsFileSource, TriviaPieceKind,
	T,
};
...
pub(crate) fn generate_workspace_bindings(mode: Mode) -> Result<()> {
	let bindings_path = project_root()
		.join("packages/@biomejs/backend-jsonrpc/src/workspace.ts");
	let methods = methods();
	...
	for method in &methods {
		let params =
			generate_type(&mut declarations, &mut queue, &method.params);
		let result =
			generate_type(&mut declarations, &mut queue, &method.result);
		...
		member_definitions.push(AnyTsTypeMember::TsMethodSignatureTypeMember(
			make::ts_method_signature_type_member(
				AnyJsObjectMemberName::JsLiteralMemberName(
					make::js_literal_member_name(make::ident(&camel_case)),
				),
				make::js_parameters(
					make::token(T!['(']),
	...
	member_definitions.push(AnyTsTypeMember::TsMethodSignatureTypeMember(
		make::ts_method_signature_type_member(
			AnyJsObjectMemberName::JsLiteralMemberName(
				make::js_literal_member_name(make::ident("destroy")),
			),
			make::js_parameters(
				make::token(T!['(']),
		...
		.with_return_type_annotation(make::ts_return_type_annotation(
			make::token(T![:]),
			AnyTsReturnType::AnyTsType(AnyTsType::TsVoidType(
				make::ts_void_type(make::token(T![void])),
			)),
		))
		.build(),
	));
				...
				make::ts_type_alias_declaration(
					...
					AnyTsType::TsReferenceType(
						make::ts_reference_type(
							AnyTsName::JsReferenceIdentifier(
								make::js_reference_identifier(make::ident(
	...
	items.push(AnyJsModuleItem::JsExport(make::js_export(
		...
		make::token(T![export]),
		AnyJsExportClause::AnyJsDeclarationClause(
			AnyJsDeclarationClause::TsInterfaceDeclaration(
	...
	let member_separators =
		(0..member_declarations.len()).map(|_| make::token(T![,]));
