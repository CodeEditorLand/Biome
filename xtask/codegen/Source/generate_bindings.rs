...
use biome_js_syntax::{
	AnyJsBinding, AnyJsBindingPattern, AnyJsCallArgument, AnyJsDeclaration, AnyJsDeclarationClause, AnyJsExportClause,
	AnyJsExpression, AnyJsFormalParameter, AnyJsImportClause, AnyJsLiteralExpression, AnyJsModuleItem, AnyJsName,
	AnyJsNamedImportSpecifier, AnyJsObjectMember, AnyJsObjectMemberName, AnyJsParameter, AnyJsStatement, AnyTsName,
...
pub(crate) fn generate_workspace_bindings(mode: Mode) -> Result<()> {
	...
	for method in &methods {
		...
		member_definitions.push(AnyTsTypeMember::TsMethodSignatureTypeMember(
			make::ts_method_signature_type_member(
				AnyJsObjectMemberName::JsLiteralMemberName(make::js_literal_member_name(make::ident(&camel_case))),
				make::js_parameters(
					make::token(T!['(']),
					make::js_parameter_list(
						Some(AnyJsParameter::AnyJsFormalParameter(AnyJsFormalParameter::JsFormalParameter(
							...
							.with_type_annotation(make::ts_type_annotation(make::token(T![:]), params))
							.build(),
						))),
						None,
					),
							...
							.with_argument(AnyJsExpression::JsCallExpression(
								make::js_call_expression(
									AnyJsExpression::JsStaticMemberExpression(make::js_static_member_expression(
										...
										make::token(T![.]),
										AnyJsName::JsName(make::js_name(make::ident("request"))),
									)),
	...
	let support_kind_schema =
		SchemaGenerator::from(SchemaSettings::openapi3()).root_schema_for::<biome_service::workspace::SupportKind>();
	generate_type(&mut declarations, &mut queue, &support_kind_schema);
	let rule_domain_schema =
		SchemaGenerator::from(SchemaSettings::openapi3()).root_schema_for::<biome_analyze::RuleDomain>();
					...
					make::js_named_import_specifiers(
						...
						make::js_named_import_specifier_list(
							Some(AnyJsNamedImportSpecifier::JsShorthandNamedImportSpecifier(
								make::js_shorthand_named_import_specifier(AnyJsBinding::JsIdentifierBinding(
	...
	items.extend(declarations.into_iter().map(|(decl, description)| {
		...
		AnyJsModuleItem::JsExport(make::js_export(
			...
			AnyJsExportClause::AnyJsDeclarationClause(match decl {
				AnyJsDeclaration::JsClassDeclaration(decl) => AnyJsDeclarationClause::JsClassDeclaration(decl),
				AnyJsDeclaration::JsFunctionDeclaration(decl) => AnyJsDeclarationClause::JsFunctionDeclaration(decl),
				AnyJsDeclaration::JsVariableDeclaration(decl) => AnyJsDeclarationClause::JsVariableDeclarationClause(
				...
				AnyJsDeclaration::TsEnumDeclaration(decl) => AnyJsDeclarationClause::TsEnumDeclaration(decl),
				AnyJsDeclaration::TsExternalModuleDeclaration(decl) => {
					AnyJsDeclarationClause::TsExternalModuleDeclaration(decl)
				},
				AnyJsDeclaration::TsGlobalDeclaration(decl) => AnyJsDeclarationClause::TsGlobalDeclaration(decl),
				AnyJsDeclaration::TsImportEqualsDeclaration(decl) => {
					AnyJsDeclarationClause::TsImportEqualsDeclaration(decl)
				},
				AnyJsDeclaration::TsInterfaceDeclaration(decl) => AnyJsDeclarationClause::TsInterfaceDeclaration(decl),
				AnyJsDeclaration::TsModuleDeclaration(decl) => AnyJsDeclarationClause::TsModuleDeclaration(decl),
				AnyJsDeclaration::TsTypeAliasDeclaration(decl) => AnyJsDeclarationClause::TsTypeAliasDeclaration(decl),
			}),
		))
	}));
	...
	member_definitions.push(AnyTsTypeMember::TsMethodSignatureTypeMember(
		make::ts_method_signature_type_member(
			AnyJsObjectMemberName::JsLiteralMemberName(make::js_literal_member_name(make::ident("destroy"))),
			make::js_parameters(make::token(T!['(']), make::js_parameter_list(None, None), make::token(T![')'])),
		)
		.with_return_type_annotation(make::ts_return_type_annotation(
			make::token(T![:]),
			AnyTsReturnType::AnyTsType(AnyTsType::TsVoidType(make::ts_void_type(make::token(T![void])))),
		))
		.build(),
	));
	...
	items.push(AnyJsModuleItem::JsExport(make::js_export(
		...
		AnyJsExportClause::AnyJsDeclarationClause(AnyJsDeclarationClause::JsFunctionDeclaration(
			make::js_function_declaration(
				...
				AnyJsBinding::JsIdentifierBinding(make::js_identifier_binding(make::ident("createWorkspace"))),
				make::js_parameters(
					make::token(T!['(']),
					make::js_parameter_list(
						Some(AnyJsParameter::AnyJsFormalParameter(AnyJsFormalParameter::JsFormalParameter(
				...
				make::js_function_body(
					...
					make::js_statement_list(Some(AnyJsStatement::JsReturnStatement(
						make::js_return_statement(make::token(T![return]))
							.with_argument(AnyJsExpression::JsObjectExpression(make::js_object_expression(
								make::token(T!['{']),
								make::js_object_member_list(member_declarations, member_separators),
								make::token(T!['}']),
							)))
							.build(),
					))),
	...
	let module = make::js_module(make::js_directive_list(None), make::js_module_item_list(items), make::eof()).build();
