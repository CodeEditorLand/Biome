...
pub fn generate_syntax_factory(ast:&AstSrc, language_kind:LanguageKind) -> Result<String> {
	...
	let normal_node_arms = ast.nodes.iter().map(|node| {
		...
		let fields = if node.dynamic {
			// Chunk the fields of the node into groups of unordered nodes that
			// need to be checked in parallel and ordered nodes that get
			// checked one by one.
			...
			field_groups
				...
				.map(|group| {
					match group.len() {
						0 => {
							unreachable!("Somehow encountered a group of fields with no entries")
						},
						// Single-field groups are assumed to act like ordered
						// fields, so they can just check the kind and
						// move on if there's no match.
						1 => {
							let field = group[0];
