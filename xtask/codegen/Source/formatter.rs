...
impl GitRepo {
	fn open() -> Self {
		...
		let statuses = repo.statuses(Some(&mut repo_opts)).expect("failed to read repository status");
...
impl ModuleIndex {
	fn new(root: PathBuf) -> Self {
		...
		let mut queue: VecDeque<_> = NodeDialect::all().iter().map(|dialect| root.join(dialect.as_str())).collect();
		...
		while let Some(dir) = queue.pop_front() {
			...
			let iter = read_dir(&dir).unwrap_or_else(|err| panic!("failed to read '{}': {}", dir.display(), err));
			...
			for entry in iter {
				...
				let file_type = entry
					.file_type()
					.unwrap_or_else(|err| panic!("failed to read file type of '{}': {}", path.display(), err));
	...
	fn print(mut self, stage: &mut Vec<PathBuf>) {
		...
		for file in self.unused_files {
			remove_file(&file).unwrap_or_else(|err| panic!("failed to delete '{}': {}", file.display(), err));
			stage.push(file);
		}
	}
}
...
fn generate_formatter(repo: &GitRepo, language_kind: LanguageKind) {
	...
	let formatter_crate_path = project_root().join("crates").join(language_kind.formatter_crate_name());
	...
	let mut format_impls = BoilerplateImpls::new(formatter_crate_path.join("src/generated.rs"), language_kind);
...
fn get_node_concept(kind: &NodeKind, dialect: &NodeDialect, language: &LanguageKind, name: &str) -> NodeConcept {
	if matches!(kind, NodeKind::Bogus) {
		...
		match language {
			LanguageKind::Js => match name {
				...
				_ if name.ends_with("Expression") || name.ends_with("Argument") || name.ends_with("Arguments") => {
				...
				// TypeScript
				"Assertion" | "ConstAssertion" | "NonNull" | "TypeArgs" | "ExprWithTypeArgs" => NodeConcept::Expression,
				...
				_ if dialect.is_jsx()
					&& (name.ends_with("Element") || name.ends_with("Tag") || name.ends_with("Fragment")) =>
				{
					NodeConcept::Tag
				},
			...
			LanguageKind::Css => match name {
				...
				_ if matches!(
					name,
					"Number" | "Percentage" | "Ratio" | "String" | "Color" | "Length" | "UrlValueRaw"
