...
pub fn promote_rule(rule_name: &str, new_group: &str) {
	...
	{
		// rule found!
		let new_group_src_path =
			analyzers_path.join("src/lint").join(new_group);
		let new_rule_path =
			new_group_src_path.join(format!("{rule_name_snake}.rs"));
		let new_group_test_path =
			analyzers_path.join("tests/specs").join(new_group);
		...
		if !new_group_src_path.exists() {
			fs::create_dir(&new_group_src_path)
				.expect("To create the group source folder");
		}
		...
		if !new_group_test_path.exists() {
			fs::create_dir(&new_group_test_path)
				.expect("To create the group test folder");
		}
		let old_test_path =
			analyzers_path.join("tests/specs/nursery").join(rule_name);
		let new_test_path = new_group_test_path.join(rule_name);
		fs::rename(old_test_path, new_test_path)
			.expect("To move rule test folder");
	} else {
