...
pub(crate) fn generate_migrate_eslint(mode: Mode) -> Result<()> {
	...
	for (eslint_name, (group_name, rule_metadata)) in visitor.0 {
		...
		let is_inspuired = rule_metadata.source_kind.is_some_and(|source_kind| source_kind.is_inspired());
	...
	let file_path = project_root().join("crates/biome_cli/src/execute/migrate/eslint_any_rule_to_biome.rs");
		...
		<R::Query as Queryable>::Output: Clone,
	{
		for source in R::METADATA.sources {
			if source.is_eslint() || source.is_eslint_plugin() {
				self.0
					.insert(source.to_namespaced_rule_name(), (<R::Group as RuleGroup>::NAME, R::METADATA));
			}
		}
	}
}
