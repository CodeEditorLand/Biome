...
use crate::{
	...
	generate_target_language_constants::generate_target_language_constants,
	js_kinds_src::{AstEnumSrc, AstListSeparatorConfiguration, AstListSrc, AstNodeSrc, TokenKind},
	language_kind::{LanguageKind, ALL_LANGUAGE_KIND},
	termcolorful::{println_string_with_fg_color, Color},
	update,
};
		...
		while let Some(variant) = union_queue.pop_front() {
			if union_map.contains_key(variant) {
				...
				if union_set.insert(&current_union.name) {
					...
					// We either have a circular dependency or 2 variants
	...
	for node in grammar.iter() {
		...
		match classify_node_rule(grammar, rule, &name) {
			NodeRuleClassification::Union(variants) => {
				// TODO: This is CSS-specific and would be better handled with a
				// per-language method for classifying or modifying rules
				// before generation.
				...
				ast.unions.push(AstEnumSrc { documentation:vec![], name, variants })
			},
			...
			NodeRuleClassification::DynamicNode => {
				...
				ast.nodes.push(AstNodeSrc { documentation:vec![], name, fields, dynamic:true })
			},
			NodeRuleClassification::Bogus => ast.bogus.push(name),
			NodeRuleClassification::List { separator, element_name } => {
				ast.push_list(name.as_str(), AstListSrc { element_name, separator });
			},
		}
	}
...
enum NodeRuleClassification {
	...
	/// children of a node are unordered, the entire node becomes dynamically
	/// ordered
	DynamicNode,
	...
	/// A list node of the form `A = B*` or `A = (B (',' B)*)` or `A = (B (','
	/// B)* ','?)`
	List {
...
fn classify_node_rule(grammar:&Grammar, rule:&Rule, name:&str) -> NodeRuleClassification {
	match rule {
		...
		Rule::Node(node) if name.starts_with("AnyCss") && name.ends_with("PropertyValue") => {
			// TODO: This is CSS-specific and would be better handled with a
			// per-language method for classifying or modifying rules before
			// generation.
			//
			// We use the convention `AnyCss*PropertyValue` to automatically
			// inject additional implicit variants. If there is only one
			// normal production for the node, then it won't be a
			// `Rule::Alt`, and needs to be handled
...
fn clean_token_name(grammar:&Grammar, token:&Token) -> String {
	...
	// These tokens, when parsed to proc_macro2::TokenStream, generates a stream
	// of bytes that can't be recognized by [quote].
...
fn handle_rule(
	...
	match rule {
		Rule::Labeled { label, rule } => {
			// Some methods need to be manually implemented because they need
			// some custom logic; we use the prefix "manual__" to exclude
			// labelled nodes.
		...
		Rule::Node(node) => {
			...
			let field = Field::Node { name, ty, optional, unordered };
			fields.push(field);
		},
		Rule::Token(token) => {
		...
		Rule::Alt(rules) => {
			...
			if optional {
				panic!(
					"Alternates cannot be nested within an optional Rule. Use a new Node to \
					 contain the alternate {label:?}"
				);
			}
			for rule in rules {
		...
		Rule::Seq(rules) => {
			for rule in rules {
				// Sequences can be optional if they are wrapped by an Opt rule,
				// so it is inherited
		...
		Rule::UnorderedAll(rules) => {
			for rule in rules {
				// UnorderedAll only implies each contained rule is unordered,
				// while optionality is inherited from the parent.
				handle_rule(fields, grammar, rule, label, optional, true);
			}
		},
		Rule::UnorderedSome(rules) => {
			for rule in rules {
				// UnorderedSome implies each contained rule is unordered _and_
				// optional.
