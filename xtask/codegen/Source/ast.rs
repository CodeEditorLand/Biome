...
use crate::js_kinds_src::{
	AstEnumSrc, AstListSeparatorConfiguration, AstListSrc, AstNodeSrc,
	TokenKind,
};
...
pub(crate) fn generate_syntax(
	...
	if language_kind.supports_grit() {
		let target_language_constants_file =
			target_language_path.join("constants.rs");
...
fn check_unions(unions: &[AstEnumSrc]) {
	...
	let union_map: HashMap<_, _> =
		unions.iter().map(|en| (&en.name, en)).collect();
		...
		while let Some(variant) = union_queue.pop_front() {
			if union_map.contains_key(variant) {
				...
				if union_set.insert(&current_union.name) {
					...
					panic!(
						"Variant '{variant}' used twice or circular dependency"
					);
				}
			} else {
...
pub(crate) fn append_css_property_value_implied_alternatives(
	variants: Vec<String>,
) -> Vec<String> {
	...
	for node in grammar.iter() {
		...
		match classify_node_rule(grammar, rule, &name) {
			...
			NodeRuleClassification::Node => {
				...
				let is_dynamic =
					fields.iter().any(|field| field.is_unordered());
			...
			NodeRuleClassification::List { separator, element_name } => {
				ast.push_list(
					name.as_str(),
					AstListSrc { element_name, separator },
				);
			},
		}
	}
...
fn classify_node_rule(
	grammar: &Grammar,
	rule: &Rule,
	name: &str,
) -> NodeRuleClassification {
	match rule {
		// this is for enums
		Rule::Alt(alternatives) => {
			...
			for alternative in alternatives {
				match alternative {
					Rule::Node(it) => {
						all_alternatives.push(grammar[*it].name.clone())
					},
		...
		Rule::Seq(rules) => {
			// (T (',' T)* ','?)
			// (T (',' T)*)
			if let Some(comma_list) =
				handle_comma_list(grammar, rules.as_slice())
			{
				NodeRuleClassification::List {
		...
		Rule::UnorderedAll(_) | Rule::UnorderedSome(_) => {
			NodeRuleClassification::DynamicNode
		},
		Rule::Node(node)
			if name.starts_with("AnyCss")
				&& name.ends_with("PropertyValue") =>
		{
			// TODO: This is CSS-specific and would be better handled with a per-language
			// method for classifying or modifying rules before generation.
			//
...
fn handle_rule(
	...
	match rule {
		Rule::Labeled { label, rule } => {
			...
			if handle_tokens_in_unions(
				fields, grammar, rule, label, optional, unordered,
			) {
				return;
			}
		...
		Rule::Node(node) => {
			...
			let name =
				label.map_or_else(|| Case::Snake.convert(&ty), String::from);
...
fn handle_comma_list<'a>(
	...
	let comma = match repeat.as_slice() {
		[comma, Rule::Node(n)] => {
			let separator_matches_trailing =
...
fn handle_tokens_in_unions(
	...
	for rule in rule.iter() {
		match rule {
			Rule::Token(token) => {
				token_kinds.push(clean_token_name(grammar, token))
			},
			_ => return false,
		}
	}
