...
pub fn generate_nodes(ast: &AstSrc, language_kind: LanguageKind) -> Result<String> {
	let (node_defs, node_boilerplate_impls): (Vec<_>, Vec<_>) = ast
		...
		.map(|node| {
			...
			let methods = node.fields.iter().enumerate().map(|(slot_index, field)| match field {
				Field::Token { name, kind, .. } => {
					let many = matches!(kind, TokenKind::Many(_));

					let method_name = if many { format_ident!("{}", name) } else { field.method_name(language_kind) };
				...
				Field::Node { ty, optional, .. } => {
					...
					if is_list {
						if *optional {
							panic!(
								"Lists cannot be optional. Instead, the grammar should handle the situation where the list is empty."
							);
						}
			...
			let fields = node.fields.iter().map(|field| {
				let name = match field {
					Field::Token { name, kind: TokenKind::Many(_), .. } => format_ident!("{}", name),
			...
			let (slot_fields, slot_constructors): (Vec<_>, Vec<_>) = node
				...
				.map(|field| match field {
					Field::Token { name, kind, .. } => {
						...
						let method_name =
							if many { format_ident!("{}", name) } else { field.method_name(language_kind) };
	...
	let (union_defs, union_boilerplate_impls): (Vec<_>, Vec<_>) = ast
		...
		.map(|union| {
			...
			let variants: Vec<_> = simple_variants.iter().map(|var| format_ident!("{}", var)).collect();
			...
			let all_variant_names: Vec<_> = union.variants.iter().map(|variant| format_ident!("{}", variant)).collect();
	...
	match field {
		...
		Field::Token { kind, .. } => match kind {
			...
			TokenKind::Many(expected) => {
				let expected_kinds = expected.iter().map(|kind| token_kind_to_code(kind, language_kind));
				quote! {
