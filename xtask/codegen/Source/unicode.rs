...
impl Properties {
	...
	pub fn extract(&self, property: &str) -> Result<Vec<(char, char)>> {
		fn parse_code_point(input: &str) -> Result<char> {
			char::from_u32(u32::from_str_radix(input, 16).with_context(
				|| format!("unable to parse `{}` as a code point", &input),
			)?)
		...
		self.raw
			...
			.try_fold(
				...
				|mut buffer, code_point_or_range| {
					let range @ (start, end) = match code_point_or_range
						.split_once("..")
					{
						Some((start, end)) => {
							(parse_code_point(start)?, parse_code_point(end)?)
						},
						None => {
							let code_point =
								parse_code_point(code_point_or_range)?;
					...
					if let Some((_, previous_end)) = buffer.last_mut() {
						assert!(
							*previous_end < start,
							"need sorted table for binary search"
						);
