...
impl Properties {
	/// Retrieve properties from cache, or from the unicode website if cache is
	/// missing. # Errors
	/// Return an error if reading cache or fetching fresh data from the unicode
	/// website fails.
	...
	fn path() -> PathBuf { xtask::project_root().join(paths::DERIVED_CORE_PROPERTIES) }
	...
	pub fn extract(&self, property:&str) -> Result<Vec<(char, char)>> {
		...
		self.raw
			...
			.filter_map(|line| {
				...
				// Check if the remaining fields contains the property we are
				// looking for.
			...
			.try_fold(Vec::<(char, char)>::new(), |mut buffer, code_point_or_range| {
