...
impl ProjectDiagnostic {
	...
	// Vec<Error>) -> Self {     let mut diagnostic = Self::new(markup!("Failed
	// to deserialize the manifest"));     diagnostic.verbose_advice = Advice {
...
impl ProjectAnalyzeDiagnostic {
	fn new(message:impl Display) -> Self {
		Self { message:MessageAndDescription::from(markup! {{message}}.to_owned()), range:None }
	}
