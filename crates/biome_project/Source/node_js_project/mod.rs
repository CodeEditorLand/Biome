...
use crate::{
	node_js_project::tsconfig_json::TsConfigJson,
	Manifest,
	Project,
	ProjectAnalyzeDiagnostic,
...
impl Project for NodeJsProject {
	...
	fn project_path(&self) -> &Path { self.manifest_path.as_path() }

	fn manifest(&self) -> Option<&Self::Manifest> { Some(&self.manifest) }
	...
	fn has_errors(&self) -> bool { !self.diagnostics.is_empty() }
}
