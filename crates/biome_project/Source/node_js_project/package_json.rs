...
impl Dependencies {
	pub fn to_keys(&self) -> Vec<String> { self.0.keys().cloned().collect() }

	pub fn contains(&self, specifier:&str) -> bool { self.0.contains_key(specifier) }
...
impl From<&str> for Version {
	fn from(value:&str) -> Self { Self::Literal(value.to_string()) }
}

impl From<String> for Version {
	fn from(value:String) -> Self { Self::Literal(value) }
}
		...
		for (key, value) in members.flatten() {
			...
			match key_text.text() {
				...
				"license" => {
					...
					// TODO: add proper parsing of license, e.g. support for AND
					// keywords
				...
				_ => {
					// each package can add their own field, so we should ignore
					// any extraneous key and only deserialize the ones
					// that Biome deems important
				},
			}
		}
		Some(result)
...
impl PackageType {
	pub const fn is_commonjs(&self) -> bool { matches!(self, Self::Commonjs) }

	pub const fn is_module(&self) -> bool { matches!(self, Self::Module) }
}
