		...
		match byte {
			...
			b'\\' => {
				if let Some((escaped_index, escaped)) = bytes.next() {
					// If we encounter an alternate quote that is escaped, we
					// have to remove the escape from it.
					// This is done because of how the enclosed strings can
					// change. Check `computed_preferred_quote` for more
					// details.
					let should_unescape = match escaped {
						...
						0xE2 => {
							// Prserve escaping of Unicode characters U+2028 and
							// U+2029
			...
			_ => {
				// If we encounter a preferred quote and it's not escaped, we
				// have to replace it with an escaped version.
