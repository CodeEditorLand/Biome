...
///
/// The function allocates a new string only if at least one change is
/// performed.
///
/// In the followinf example `"` is escaped, `\'` and `\l` are unescaped, and
/// the newline is normalized.
///
/// ```
...
/// );
/// ```
pub fn normalize_string(raw_content:&str, preferred_quote:Quote, is_escape_preserved:bool) -> Cow<str> {
	...
	while let Some((byte_index, byte)) = bytes.next() {
		match byte {
			...
			b'\\' => {
				if let Some((escaped_index, escaped)) = bytes.next() {
					...
					let should_unescape = match escaped {
						...
						b'^' | b'\n' | b'0'..=b'7' | b'\\' | b'b' | b'f' | b'n' | b'r' | b't' | b'u' | b'v' | b'x' => {
							false
						},
						b'\r' => {
						...
						0xE2 => {
							...
							!(matches!(bytes.next(), Some((_, 0x80))) && matches!(bytes.next(), Some((_, 0xA8 | 0xA9))))
						},
						...
						_ => {
							...
							//
							// We always unescape alternate quots regardless of
							// `is_escape_preserved`.
							escaped == alternate_quote || (escaped != preferred_quote && !is_escape_preserved)
						},
					};
			...
			_ => {
				// If we encounter a preferred quote and it's not escaped, we have to replace it
...
mod tests {
	...
	fn normalize_escapes() {
		...
		assert_eq!(normalize_string("\\\u{2028}", Quote::Double, false), "\\\u{2028}");

		assert_eq!(normalize_string("\\\u{2029}", Quote::Double, false), "\\\u{2029}");

		assert_eq!(normalize_string("a\\a", Quote::Double, false), "aa");

		assert_eq!(normalize_string("👍\\👍", Quote::Single, false), "👍👍");

		assert_eq!(normalize_string("\\\u{2027}", Quote::Double, false), "\u{2027}");

		assert_eq!(normalize_string("\\\u{2030}", Quote::Double, false), "\u{2030}");
