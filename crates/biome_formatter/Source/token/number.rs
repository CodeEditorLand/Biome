...
use crate::{CstFormatContext, Format, prelude::*};
...
fn format_trimmed_number(text:&str) -> Cow<str> {
	...
	// Look at only the start of the text, ignore any sign, and make sure numbers
	// always start with a digit. Add 0 if missing.
	...
	loop {
		// We use a None pseudo-char at the end of the string to simplify the match
		// cases that follow
		let curr_or_none_terminator_char = match curr {
			Some((curr_index, curr_char)) => (curr_index, Some(curr_char)),
			None => (text.len(), None),
		};
		// Look for termination of the decimal part or exponent and see if we need to
		// print it differently.
		match (&state, curr_or_none_terminator_char) {
			(
				DecimalPart(FormatNumberLiteralDecimalPart { dot_index, last_non_zero_index: None }),
				(curr_index, Some(b'e') | None),
			) => {
				// The decimal part equals zero, ignore it completely.
				// Caveat: Prettier still prints a single `.0` unless there was *only* a
				// trailing dot.
			...
			(
				DecimalPart(FormatNumberLiteralDecimalPart { last_non_zero_index: Some(last_non_zero_index), .. }),
				(curr_index, Some(b'e') | None),
			) if last_non_zero_index.get() < curr_index - 1 => {
				// The decimal part ends with at least one zero, ignore them but copy the part
				// from the dot until the last non-zero.
				cleaned_text.push_str(&text[copied_or_ignored_chars..=last_non_zero_index.get()]);

				copied_or_ignored_chars = curr_index;
			},
			(Exponent(FormatNumberLiteralExponent { e_index, first_non_zero_index: None, .. }), (curr_index, None)) => {
			...
			{
				// The exponent begins with a plus or at least one zero, ignore them but copy
				// the part from the first non-zero until the end.
				cleaned_text.push_str(&text[copied_or_ignored_chars..=*e_index]);
		...
		match (&state, curr) {
			...
			(_, Some((curr_index, b'.'))) => {
				state = DecimalPart(FormatNumberLiteralDecimalPart { dot_index:curr_index, last_non_zero_index:None });
			},
...
mod tests {
	...
	fn keeps_the_input_string_if_no_change_needed() {
		assert!(matches!(format_trimmed_number("0.1e2"), Cow::Borrowed("0.1e2")));
	}
}
