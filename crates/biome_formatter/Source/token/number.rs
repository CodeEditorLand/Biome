...
use crate::{prelude::*, CstFormatContext, Format};
...
fn format_trimmed_number(text:&str) -> Cow<str> {
	...
	// Will be filled only if and when the first place that needs reformatting
	// is detected.
	let mut cleaned_text = String::new();

	// Look at only the start of the text, ignore any sign, and make sure
	// numbers always start with a digit. Add 0 if missing.
	...
	loop {
		// We use a None pseudo-char at the end of the string to simplify the
		// match cases that follow
		let curr_or_none_terminator_char = match curr {
			Some((curr_index, curr_char)) => (curr_index, Some(curr_char)),
			None => (text.len(), None),
		};
		// Look for termination of the decimal part or exponent and see if we
		// need to print it differently.
		match (&state, curr_or_none_terminator_char) {
			(
				...
				// Caveat: Prettier still prints a single `.0` unless there was
				// *only* a trailing dot.
			...
			(
				...
				// The decimal part ends with at least one zero, ignore them but
				// copy the part from the dot until the last non-zero.
			...
			{
				// The exponent begins with a plus or at least one zero, ignore
				// them but copy the part from the first non-zero until the end.
				cleaned_text.push_str(&text[copied_or_ignored_chars..=*e_index]);
				if *is_negative {
					cleaned_text.push('-');
				}
			...
			(DecimalPart(decimal_part), Some((curr_index, b'1'..=b'9'))) => {
				state = DecimalPart(FormatNumberLiteralDecimalPart {
					last_non_zero_index:Some(unsafe {
						// We've already entered InDecimalPart, so curr_index
						// must be >0
			...
			(
				...
				state = Exponent(FormatNumberLiteralExponent {
					first_digit_index:Some(unsafe {
						// We've already entered InExponent, so curr_index must
						// be >0
						NonZeroUsize::new_unchecked(curr_index)
					}),
					first_non_zero_index:if curr_char != b'0' {
						Some(unsafe {
							// We've already entered InExponent, so curr_index
							// must be >0
							NonZeroUsize::new_unchecked(curr_index)
						})
					} else {
