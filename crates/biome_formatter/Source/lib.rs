...
use biome_rowan::{
	Language, NodeOrToken, SyntaxElement, SyntaxNode, SyntaxResult,
	SyntaxToken, SyntaxTriviaPiece, TextLen, TextRange, TextSize,
	TokenAtOffset,
};
pub use buffer::{
	Buffer, BufferExtensions, BufferSnapshot, Inspect, PreambleBuffer,
...
#[derive(
	Debug, Default, Clone, Copy, Deserializable, Eq, Hash, Merge, PartialEq,
)]
#[cfg_attr(
	feature = "serde",
...
impl<'de> serde::Deserialize<'de> for IndentWidth {
		...
		D: serde::Deserializer<'de>,
	{
		...
		let indent_width =
			IndentWidth::try_from(value).map_err(serde::de::Error::custom)?;
		Ok(indent_width)
	}
}
...
impl FromStr for IndentWidth {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		let value =
			u8::from_str(s).map_err(ParseFormatNumberError::ParseError)?;
		let value = Self::try_from(value)
			.map_err(ParseFormatNumberError::TryFromU8Error)?;
		Ok(value)
	}
}
...
impl std::fmt::Display for ParseFormatNumberError {
	fn fmt(&self, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		match self {
			ParseFormatNumberError::ParseError(err) => {
				std::fmt::Display::fmt(err, fmt)
			},
			ParseFormatNumberError::TryFromU16Error(err) => {
				std::fmt::Display::fmt(err, fmt)
			},
			ParseFormatNumberError::TryFromU8Error(err) => {
				std::fmt::Display::fmt(err, fmt)
			},
		}
	}
...
impl FromStr for LineWidth {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		let value =
			u16::from_str(s).map_err(ParseFormatNumberError::ParseError)?;
		let value = Self::try_from(value)
			.map_err(ParseFormatNumberError::TryFromU16Error)?;
		Ok(value)
	}
}
...
#[derive(
	Clone, Copy, Debug, Default, Deserializable, Eq, Hash, Merge, PartialEq,
)]
#[cfg_attr(
	feature = "serde",
	derive(serde::Serialize, serde::Deserialize, schemars::JsonSchema),
...
pub trait CstFormatContext: FormatContext {
	...
	type CommentRule: FormatRule<SourceComment<Self::Language>, Context = Self>
		+ Default;
	...
	Context: FormatContext,
{
	...
	pub fn print_with_indent(&self, indent: u16) -> PrintResult<Printed> {
		let print_options = self.context.options().as_print_options();
		let printed = Printer::new(print_options)
	...
	C::Language: 'static,
{
	...
	fn fmt(
		...
		crate::write!(
			f,
			[format_skipped_token_trivia(token), format_trimmed_token(token),]
		)
	}
}
	...
	Context: FormatContext,
{
	...
	let mut buffer =
		VecBuffer::with_capacity(arguments.items().len(), &mut state);
...
pub trait FormatLanguage {
	...
	type FormatRule: FormatRule<SyntaxNode<Self::SyntaxLanguage>, Context = Self::Context>
		+ Default;
	...
	fn is_range_formatting_node(
		&self,
		_node: &SyntaxNode<Self::SyntaxLanguage>,
	) -> bool {
		true
	}
...
pub fn format_range<Language: FormatLanguage>(
	...
	if start_token_trimmed_start >= range.start()
		&& start_token_trimmed_start <= range.end()
	{
		...
		if let Some(next_token) = start_token.next_token() {
			let next_token_start =
				text_non_whitespace_range(&next_token).start();
	...
	let common_root = if start_node == end_node {
		...
		let result_end_node = end_node
			...
			.take_while(|end_parent| {
				end_parent.text_range().start() >= start_node_start
			})
			.last()
			.unwrap_or(end_node);
		...
		let result_start_node = start_node
			.ancestors()
			.take_while(|start_parent| {
				start_parent.text_range().end() <= end_node_end
			})
			.last()
	...
	let (end_source, end_dest) = match range_end {
		...
		None => (
			...
			TextSize::try_from(printed.as_code().len())
				.expect("code length out of bounds"),
		),
	};
...
pub fn format_sub_tree<L: FormatLanguage>(
	...
	let mut tokens =
		std::iter::successors(root.first_token(), |token| token.prev_token());
	...
	let trivias =
		first_token_trivias.chain(next_tokens_trivias).filter(|piece| {
	...
	let last_whitespace =
		trivias.map_while(|piece| piece.as_whitespace()).last();
...
impl<Context> FormatState<Context> {
	...
	#[inline]
	pub fn track_token<L: Language>(
		&mut self,
		#[allow(unused_variables)] token: &SyntaxToken<L>,
	) {
		cfg_if::cfg_if! {
...
mod tests {
	...
	fn test_out_of_range_line_width() {
		...
		let deserialized = deserialize_from_json_str::<TestConfig>(
			source,
			Default::default(),
			"",
		);
