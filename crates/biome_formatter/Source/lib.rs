...
///
/// ```
/// use biome_formatter::{IndentStyle, LineWidth, format, prelude::*, write};
...
///
/// ```
/// use biome_formatter::{FormatState, Formatted, VecBuffer, format_args, prelude::*, write};
///
/// # fn main() -> FormatResult<()> {
				...
				Some(prev_marker) => {
					if marker.source > prev_marker.source {
						if prev_marker.dest == marker.dest {
							...
							// can get an incorrect substring in the source text e.g
							// ...
							// SourceMarker {
							//     source: 94, <----- but we need to use this source position to get
							// correct substring in the source     dest: 99,
							// },
							// SourceMarker {
							//     source: 96,
			...
			range_end = match range_end {
				Some(prev_marker) => {
					if marker.source <= prev_marker.source || marker.dest == prev_marker.dest {
						...
						// but we need to check if the marker has the same dest, otherwise we can
						// get an incorrect substring in the source text e.g
						// ...
						// SourceMarker {
						//     source: 94,
						//     dest: 99, <----- both markers have the same dest.
						// },
						// SourceMarker {
						//     source: 96, <----- but we need to use this source position to get
						// correct substring in the source     dest: 99,
						// },
						// ...
						Some(marker)
...
pub fn format_sub_tree<L:FormatLanguage>(root:&SyntaxNode<L::SyntaxLanguage>, language:L) -> FormatResult<Printed> {
	...
	// From the iterator of tokens, build an iterator of trivia pieces (once again
	// the iterator is reversed, starting from the last trailing trivia towards the
	// first leading trivia). The first token is handled specially as we only wan
	...
	// Finally run the iterator until a newline trivia is found, and get the last
	// whitespace trivia before it
