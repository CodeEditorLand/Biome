...
//!
//! This module defines [FormatElement], an IR to format code documents and
//! provides a mean to print such a document to a string. Objects that know how
//! to format themselves implement the [Format] trait.
//!
//! ## Formatting Traits
//!
//! * [Format]: Implemented by objects that can be formatted.
//! * [FormatRule]: Rule that knows how to format an object of another type.
//!   Necessary in the situation where it's necessary to implement [Format] on
//!   an object from another crate. This module defines the [FormatRefWithRule]
//!   and [FormatOwnedWithRule] structs to pass an item with its corresponding
//!   rule.
//! * [FormatWithRule] implemented by objects that know how to format another
//!   type. Useful for implementing some reusable formatting logic inside of
//!   this module if the type itself doesn't implement [Format]
//!
//! ## Formatting Macros
//!
//! This crate defines two macros to construct the IR. These are inspired by
//! Rust's `fmt` macros
...
use crate::{
	builders::syntax_token_cow_slice,
	comments::{CommentStyle, Comments, SourceComment},
	format_element::document::Document,
	formatter::Formatter,
	group_id::UniqueGroupIdBuilder,
	prelude::TagKind,
	printer::{Printer, PrinterOptions},
	trivia::{format_skipped_token_trivia, format_trimmed_token},
};
...
impl IndentStyle {
	...
	pub const fn is_tab(&self) -> bool { matches!(self, IndentStyle::Tab) }

	/// Returns `true` if this is an [IndentStyle::Space].
	pub const fn is_space(&self) -> bool { matches!(self, IndentStyle::Space) }
}
...
#[derive(Default)]
pub enum LineEnding {
	///  Line Feed only (\n), common on Linux and macOS as well as inside git
	/// repos
	#[default]
	Lf,
...
impl LineEnding {
	...
	pub const fn is_line_feed(&self) -> bool { matches!(self, LineEnding::Lf) }

	/// Returns `true` if this is a [LineEnding::Crlf].
	pub const fn is_carriage_return_line_feed(&self) -> bool { matches!(self, LineEnding::Crlf) }

	/// Returns `true` if this is a [LineEnding::Cr].
	pub const fn is_carriage_return(&self) -> bool { matches!(self, LineEnding::Cr) }
}
...
impl From<IndentWidthFromIntError> for ParseFormatNumberError {
	fn from(value:IndentWidthFromIntError) -> Self { Self::TryFromU8Error(value) }
}

impl From<LineWidthFromIntError> for ParseFormatNumberError {
	fn from(value:LineWidthFromIntError) -> Self { Self::TryFromU16Error(value) }
}

impl From<ParseIntError> for ParseFormatNumberError {
	fn from(value:ParseIntError) -> Self { Self::ParseError(value) }
}
...
impl From<LineWidth> for u16 {
	fn from(value:LineWidth) -> Self { value.0 }
}
...
impl QuoteStyle {
	...
	pub fn as_byte(&self) -> u8 { self.as_char() as u8 }
	...
	pub const fn is_double(&self) -> bool { matches!(self, Self::Double) }
}
...
pub trait FormatContext {
	...
	/// The source map can be used to map positions of the formatted nodes back
	/// to their original source locations or to resolve the source text.
...
pub trait FormatOptions {
	...
	/// Whether to insert spaces around brackets in object literals. Defaults to
	/// true.
...
/// The [CstFormatContext] is an extension of the CST unaware [FormatContext]
/// and must be implemented by every language.
///
/// The context customizes the comments formatting and stores the comments of
/// the CST.
...
impl SimpleFormatContext {
	pub fn new(options:SimpleFormatOptions) -> Self { Self { options } }
}
...
impl FormatOptions for SimpleFormatOptions {
	...
	fn attribute_position(&self) -> AttributePosition { self.attribute_position }

	fn bracket_spacing(&self) -> BracketSpacing { self.bracket_spacing }
...
impl Display for SimpleFormatOptions {
	fn fmt(&self, f:&mut std::fmt::Formatter<'_>) -> std::fmt::Result { fmt::Debug::fmt(self, f) }
}
...
impl<Context> Formatted<Context> {
	pub fn new(document:Document, context:Context) -> Self { Self { document, context } }
...
impl Printed {
	pub fn new(
		...
		Self { code, range, sourcemap, verbatim_ranges:verbatim_source }
	}

	/// Construct an empty formatter result
	pub fn new_empty() -> Self {
		Self { code:String::new(), range:None, sourcemap:Vec::new(), verbatim_ranges:Vec::new() }
	}

	/// Range of the input source file covered by this formatted code,
	/// or None if the entire file is covered in this instance
	pub fn range(&self) -> Option<TextRange> { self.range }
	...
	pub fn sourcemap(&self) -> &[SourceMarker] { &self.sourcemap }

	/// Returns a list of [SourceMarker] mapping byte positions
	/// in the output string to the input source code, consuming the result
	pub fn into_sourcemap(self) -> Vec<SourceMarker> { self.sourcemap }

	/// Takes the list of [SourceMarker] mapping byte positions in the output
	/// string to the input source code.
	pub fn take_sourcemap(&mut self) -> Vec<SourceMarker> { std::mem::take(&mut self.sourcemap) }

	/// Access the resulting code, borrowing the result
	pub fn as_code(&self) -> &str { &self.code }

	/// Access the resulting code, consuming the result
	pub fn into_code(self) -> String { self.code }
	...
	pub fn verbatim_ranges(&self) -> &[TextRange] { &self.verbatim_ranges }
...
/// `biome_formatter` equivalent to [std::fmt::Display].
///
/// ## Example
/// Implementing `Format` for a custom struct
///
/// ```
/// use biome_formatter::{format, prelude::*, write, IndentStyle, LineWidth};
...
/// 		write!(
/// 			f,
/// 			[hard_line_break(), dynamic_text(&self.0, TextSize::from(0)), hard_line_break(),]
/// 		)
/// 	}
/// }
///
...
{
	#[inline(always)]
	fn fmt(&self, f:&mut Formatter<Context>) -> FormatResult<()> { Format::fmt(&**self, f) }
}
...
///
/// Implementing [Format] on the object itself is preferred over implementing
/// [FormatRule] but this isn't possible inside of a dependent crate for
/// external type.
///
/// For example, the `biome_js_formatter` crate isn't able to implement [Format]
/// on `JsIfStatement` because both the [Format] trait and `JsIfStatement` are
/// external types (Rust's orphan rule).
///
/// That's why the `biome_js_formatter` crate must define a new-type that
/// implements the formatting of `JsIfStatement`.
...
impl<C> Default for FormatToken<C> {
	fn default() -> Self { Self { context:PhantomData } }
}
...
///
/// Useful in situation where a type itself doesn't implement [Format] (e.g.
/// because of Rust's orphan rule) but you want to implement some common
/// formatting logic.
///
/// ## Examples
///
/// This can be useful if you want to format a `SyntaxNode` inside
/// biome_formatter.. `SyntaxNode` doesn't implement [Format] itself but the
/// language specific crate implements `AsFormat` and `IntoFormat` for it and
/// the returned [Format] implement [FormatWithRule].
///
/// ```ignore
...
where
	R: FormatRule<T>,
{
	pub fn new(item:&'a T, rule:R) -> Self { Self { item, rule } }
}
	...
	R: FormatRule<T>,
{
	...
	fn item(&self) -> &Self::Item { self.item }
}
...
{
	#[inline(always)]
	fn fmt(&self, f:&mut Formatter<R::Context>) -> FormatResult<()> { self.rule.fmt(self.item, f) }
}
...
where
	R: FormatRule<T>,
{
	pub fn new(item:T, rule:R) -> Self { Self { item, rule } }
...
/// The `write` function takes a target buffer and an `Arguments` struct that
/// can be precompiled with the `format_args!` macro.
///
...
///
/// ```
/// use biome_formatter::{format_args, prelude::*, write, FormatState, Formatted, VecBuffer};
///
/// # fn main() -> FormatResult<()> {
...
///
/// let formatted =
/// 	Formatted::new(Document::from(buffer.into_vec()), SimpleFormatContext::default());
///
...
pub trait FormatLanguage {
	...
	/// Performs an optional pre-processing of the tree. This can be useful to
	/// remove nodes that otherwise complicate formatting.
	///
	/// Return [None] if the tree shouldn't be processed. Return [Some] with the
	/// transformed tree and the source map otherwise.
	fn transform(
		&self,
	...
	/// range formatting process: for instance in JavaScript the function
	/// returns true for statement and declaration nodes, to ensure the entire
	/// statement gets formatted instead of the smallest sub-expression that
	/// fits the range
	fn is_range_formatting_node(&self, _node:&SyntaxNode<Self::SyntaxLanguage>) -> bool { true }
			...
			Some((transformed, source_map)) => {
				...
				if &transformed == root {
					...
					match root
                        ...
                        .last()
					{
						// current root node is the topmost node we don't need to insert the
						// transformed node back
...
pub fn format_range<Language:FormatLanguage>(
	...
	if range.is_empty() {
		return Ok(Printed::new(String::new(), Some(range), Vec::new(), Vec::new()));
	}

	let root_range = root.text_range();
	if range.start() < root_range.start() || range.end() > root_range.end() {
		return Err(FormatError::RangeError { input:range, tree:root_range });
	}
				...
				Some(prev_marker) => {
					if marker.source > prev_marker.source {
						if prev_marker.dest == marker.dest {
							// we found a marker that is closer to the start
							// range than we have but we need to check if
							// the marker has the same dest, otherwise we can
							// get an incorrect substring in the source text
							// e.g
							// ...
							// SourceMarker {
							//     source: 94, <----- but we need to use this
							// source position to get correct substring in the
							// source     dest: 99,
							// },
							// SourceMarker {
							//     source: 96,
							//     dest: 99, <----- both markers have the same
							// dest. },
							// ...
							Some(prev_marker)
						} else {
			...
			range_end = match range_end {
				Some(prev_marker) => {
					if marker.source <= prev_marker.source || marker.dest == prev_marker.dest {
						...
						// but we need to check if the marker has the same dest,
						// otherwise we can get an incorrect substring in the
						// source text e.g
						// ...
						// SourceMarker {
						//     source: 94,
						...
						// },
						// SourceMarker {
						//     source: 96, <----- but we need to use this source
						// position to get correct substring in the source
						//     dest: 99,
						// },
						// ...
						Some(marker)
	...
	Ok(Printed::new(code.into(), Some(input_range), sourcemap, verbatim_ranges))
}
...
pub fn format_sub_tree<L:FormatLanguage>(
	...
	// trivia pieces of each token from the first token of the common root
	// towards the start of the file
	let mut tokens = std::iter::successors(root.first_token(), |token| token.prev_token());

	// From the iterator of tokens, build an iterator of trivia pieces (once
	// again the iterator is reversed, starting from the last trailing trivia
	// towards the first leading trivia). The first token is handled specially
	// as we only wan to consider its leading trivia pieces
	...
	let trivias = first_token_trivias.chain(next_tokens_trivias).filter(|piece| {
		// We're only interested in newline and whitespace trivias, skip
		// over comments
	...
	// Finally run the iterator until a newline trivia is found, and get the
	// last whitespace trivia before it
...
/// This structure stores the state that is relevant for the formatting of the
/// whole document.
///
/// This structure is different from [crate::Formatter] in that the formatting
/// infrastructure creates a new [crate::Formatter] for every [crate::write!]
/// call, whereas this structure stays alive for the whole process of formatting
/// a root with [crate::format!].
...
impl<Context> FormatState<Context> {
	...
	#[cfg(not(debug_assertions))]
	#[inline]
	pub fn is_token_tracking_disabled(&self) -> bool { false }

	/// Returns `true` if token tracking is currently disabled.
	#[cfg(debug_assertions)]
	pub fn is_token_tracking_disabled(&self) -> bool { self.printed_tokens.is_disabled() }
...
mod tests {
	...
	fn test_out_of_range_line_width() {
		...
		impl<'a> DiagnosticPrinter<'a> {
			fn new(diagnostics:&'a [Error]) -> Self { Self { diagnostics } }
		}
