use crate::{printer::call_stack::PrintElementArgs, FormatElement};
...
impl<'a> LineSuffixes<'a> {
	/// Extends the line suffixes with `elements`, storing their call stack
	/// arguments with them.
	...
	pub(super) fn has_pending(&self) -> bool { !self.suffixes.is_empty() }
}
...
pub(super) enum LineSuffixEntry<'a> {
	...
	/// Potentially changed call arguments that should be used to format any
	/// following items.
