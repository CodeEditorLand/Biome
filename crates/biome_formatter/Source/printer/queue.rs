...
pub(super) trait Queue<'a> {
	...
	fn pop(&mut self) -> Option<&'a FormatElement> {
		match self.stack().top() {
			Some(top_slice) => {
				// SAFETY: Safe because queue ensures that slices inside `slices` are never
				// empty.
	...
	/// Returns the next element, recursively resolving the first element of
	...
	/// Queues a single element to process before the other elements in this
	/// queue.
	fn push(&mut self, element:&'a FormatElement) { self.extend_back(std::slice::from_ref(element)) }

	/// Queues a slice of elements to process before the other elements in this
	/// queue.
	...
	/// Skips all content until it finds the corresponding end tag with the
	/// given kind.
	...
	/// Iterates over all elements until it finds the matching end tag of the
	/// specified kind.
...
impl<'a> PrintQueue<'a> {
	...
	pub(super) fn is_empty(&self) -> bool { self.slices.is_empty() }
}

impl<'a> Queue<'a> for PrintQueue<'a> {
	type Stack = Vec<&'a [FormatElement]>;

	fn stack(&self) -> &Self::Stack { &self.slices }

	fn stack_mut(&mut self) -> &mut Self::Stack { &mut self.slices }

	fn next_index(&self) -> usize { self.next_index }

	fn set_next_index(&mut self, index:usize) { self.next_index = index }
}

/// Queue for measuring if an element fits on the line.
///
/// The queue is a view on top of the [PrintQueue] because no elements should be
/// removed from the [PrintQueue] while measuring.
#[must_use]
#[derive(Debug)]
...
impl<'a, 'print> FitsQueue<'a, 'print> {
	pub(super) fn new(print_queue:&'print PrintQueue<'a>, saved:Vec<&'a [FormatElement]>) -> Self {
		let stack = StackedStack::with_vec(&print_queue.slices, saved);

		Self { stack, next_index:print_queue.next_index }
	}

	pub(super) fn finish(self) -> Vec<&'a [FormatElement]> { self.stack.into_vec() }
}
...
where
	Q: Queue<'a>,
{
	fn new(queue:&'q mut Q, kind:TagKind) -> Self { Self { queue, kind, depth:1, lifetime:PhantomData } }
}
...
/// A predicate determining when to end measuring if some content fits on the
/// line.
///
/// Called for every [`element`](FormatElement) in the [FitsQueue] when
/// measuring if a content fits on the line. The measuring of the content ends
/// after the first element [`element`](FormatElement) for which this
/// predicate returns `true` (similar to a take while iterator except that it
/// takes while the predicate returns `false`).
...
impl FitsEndPredicate for AllPredicate {
	fn is_end(&mut self, _element:&FormatElement) -> PrintResult<bool> { Ok(false) }
}

/// Filter that takes all elements between two matching [Tag::StartEntry] and
/// [Tag::EndEntry] tags.
#[derive(Debug)]
...
impl SingleEntryPredicate {
	pub(super) const fn is_done(&self) -> bool { matches!(self, SingleEntryPredicate::Done) }
}

impl Default for SingleEntryPredicate {
	fn default() -> Self { SingleEntryPredicate::Entry { depth:0 } }
}
...
impl FitsEndPredicate for SingleEntryPredicate {
	fn is_end(&mut self, element:&FormatElement) -> PrintResult<bool> {
		let result = match self {
			...
			SingleEntryPredicate::Entry { depth } => {
				match element {
...
mod tests {
	use crate::{
		...
		prelude::Tag,
		printer::queue::{PrintQueue, Queue},
	};

	#[test]
	fn extend_back_pop_last() {
		let mut queue = PrintQueue::new(&[FormatElement::Tag(Tag::StartEntry), FormatElement::Space]);
