...
/// Stores arguments passed to `print_element` call, holding the state specific
/// to printing an element. E.g. the `indent` depends on the token the Printer's
/// currently processing. That's why it must be stored outside of the
/// [PrinterState] that stores the state common to all elements.
///
/// The state is passed by value, which is why it's important that it isn't
/// storing any heavy data structures. Such structures should be stored on the
/// [PrinterState] instead.
...
impl PrintElementArgs {
	pub fn new() -> Self { Self { ..Self::default() } }

	pub(super) fn mode(&self) -> PrintMode { self.mode }
...
impl Default for PrintElementArgs {
	fn default() -> Self { Self { mode:PrintMode::Expanded } }
}

/// Call stack that stores the [PrintElementCallArgs].
///
/// New [PrintElementCallArgs] are pushed onto the stack for every
/// [`start`](Tag::is_start) [`Tag`](FormatElement::Tag) and popped when
/// reaching the corresponding [`end`](Tag::is_end) [`Tag`](FormatElement::Tag).
pub(super) trait CallStack {
	...
	/// Pops the call arguments at the top and asserts that they correspond to a
	/// start tag of `kind`.
	///
	/// Returns `Ok` with the arguments if the kind of the top stack frame
	/// matches `kind`, otherwise returns `Err`.
	fn pop(&mut self, kind:TagKind) -> PrintResult<PrintElementArgs> {
		let last = self.stack_mut().pop();

		match last {
			Some(StackFrame { kind: StackFrameKind::Tag(actual_kind), args }) if actual_kind == kind => Ok(args),
			...
			// This should be unreachable but having it for completeness. Happens if the stack is
			// empty.
	...
	#[cold]
	fn invalid_document_error(end_kind:TagKind, start_kind:Option<TagKind>) -> InvalidDocumentError {
		match start_kind {
	...
	fn top(&self) -> PrintElementArgs { self.stack().top().expect("Expected `stack` to never be empty.").args }

	/// Returns the [TagKind] of the current stack frame or [None] if this is
	...
	/// Creates a new stack frame for a [FormatElement::Tag] of `kind` with
	/// `args` as the call arguments.
...
impl PrintCallStack {
	pub(super) fn new(args:PrintElementArgs) -> Self { Self(vec![StackFrame { kind:StackFrameKind::Root, args }]) }
}

impl CallStack for PrintCallStack {
	type Stack = Vec<StackFrame>;

	fn stack(&self) -> &Self::Stack { &self.0 }

	fn stack_mut(&mut self) -> &mut Self::Stack { &mut self.0 }
}

/// Call stack used for measuring if some content fits on the line.
///
/// The stack is a view on top of the [PrintCallStack] because the stack frames
/// are still necessary for printing.
#[must_use]
...
impl<'print> FitsCallStack<'print> {
	...
	pub(super) fn finish(self) -> Vec<StackFrame> { self.stack.into_vec() }
}

impl<'a> CallStack for FitsCallStack<'a> {
	type Stack = StackedStack<'a, StackFrame>;

	fn stack(&self) -> &Self::Stack { &self.stack }

	fn stack_mut(&mut self) -> &mut Self::Stack { &mut self.stack }
}

/// Suffix stack that stores the indention.
///
/// When ElementKind is [suffix], push the current indention onto the
/// SuffixStack.
pub(super) trait SuffixStack {
	...
	fn push_suffix(&mut self, indention:Indention) { self.suffix_stack_mut().push(indention); }
}

/// Indent stack that stores the history of indention.
///
/// When the element kind is [indent] or [align], push the current indentation
/// onto the stack of indentations. When the element kind is [dedent], pop the
/// last item from the indentations stack and push it onto the temp_indentations
/// stack. When the element kind is [end_dedent], pop the last item from the
/// temp_indentations stack and push it onto the indentations stack.
pub(super) trait IndentStack {
	...
	fn pop(&mut self) { self.current_stack_mut().pop(); }

	fn indention(&self) -> Indention { self.current_stack().top().copied().unwrap_or_default() }

	fn reset_indent(&mut self) { self.current_stack_mut().push(Indention::default()); }
...
/// Indent stack used for storing indetion history when printing the
/// [FormatElement]s
#[derive(Debug, Clone)]
...
impl PrintIndentStack {
	...
	pub fn flush_suffixes(&mut self) { self.indentions.extend(self.suffix_indentions.drain(..).rev()); }
}
impl IndentStack for PrintIndentStack {
	type HistoryStack = Vec<Indention>;
	type Stack = Vec<Indention>;

	fn current_stack(&self) -> &Self::Stack { &self.indentions }

	fn current_stack_mut(&mut self) -> &mut Self::Stack { &mut self.indentions }

	fn history_stack_mut(&mut self) -> &mut Self::HistoryStack { &mut self.history_indentions }
}
impl SuffixStack for PrintIndentStack {
	type SuffixStack = Vec<Indention>;

	fn suffix_stack_mut(&mut self) -> &mut Self::SuffixStack { &mut self.suffix_indentions }
}
/// Indent stack used for storing the history of indention when measuring fits
/// on the line.
///
/// The stack is a view on top of the [PrintIndentStack] because the stack
/// frames are still necessary when printing.
...
impl<'print> FitsIndentStack<'print> {
	pub(super) fn new(
		...
		let history_indentions = StackedStack::with_vec(&print.history_indentions, saved_history_indent_stack);
