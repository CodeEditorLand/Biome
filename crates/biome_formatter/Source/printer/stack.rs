/// A school book stack. Allows adding, removing, and inspecting elements at the
/// back.
...
impl<T> Stack<T> for Vec<T> {
	...
	fn top(&self) -> Option<&T> { self.last() }
}

/// A Stack that is stacked on top of another stack. Guarantees that the
/// underlying stack remains unchanged.
#[derive(Debug, Clone)]
pub(super) struct StackedStack<'a, T> {
	/// The content of the original stack.
	original:&'a [T],

	/// Items that have been pushed since the creation of this stack and aren't
...
impl<'a, T> StackedStack<'a, T> {
	#[cfg(test)]
	pub(super) fn new(original:&'a [T]) -> Self { Self::with_vec(original, Vec::new()) }

	/// Creates a new stack that uses `stack` for storing its elements.
	pub(super) fn with_vec(original:&'a [T], stack:Vec<T>) -> Self { Self { original, stack } }

	/// Returns the underlying `stack` vector.
	pub(super) fn into_vec(self) -> Vec<T> { self.stack }
}
	...
	T: Copy,
{
	...
	fn push(&mut self, value:T) { self.stack.push(value); }

	fn top(&self) -> Option<&T> { self.stack.last().or_else(|| self.original.last()) }
}
