...
use crate::{
	...
	prelude::{
		Tag::EndFill,
		tag::{DedentMode, Tag, TagKind, VerbatimKind},
	},
	printer::{
		call_stack::{
			CallStack,
		...
		queue::{AllPredicate, FitsEndPredicate, FitsQueue, PrintQueue, Queue, SingleEntryPredicate},
	},
};
...
impl<'a> Printer<'a> {
	pub fn new(options:PrinterOptions) -> Self { Self { options, state:PrinterState::default() } }

	/// Prints the passed in element as well as all its content
	pub fn print(self, document:&'a Document) -> PrintResult<Printed> { self.print_with_indent(document, 0) }

	/// Prints the passed in element as well as all its content,
	/// starting at the specified indentation level
	pub fn print_with_indent(mut self, document:&'a Document, indent:u16) -> PrintResult<Printed> {
	...
	fn print_element(
		...
		match element {
			...
			FormatElement::DynamicText { text, source_position } => self.print_text(text, Some(*source_position)),
				...
				let group_mode = if !group.mode().is_flat() {
					...
					match args.mode() {
						PrintMode::Flat if self.state.measured_group_fits => {
							// A parent group has already verified that this group fits on a single
							// line Thus, just continue in flat mode
							PrintMode::Flat
						},
						// The printer is either in expanded mode or it's necessary to re-measure if
						// the group fits because the printer printed a line break
						_ => {
							...
							// Measure to see if the group fits up on a single line. If that's the
							// case, print the group in "flat" mode, otherwise continue in
							// expanded mode
			...
			FormatElement::Tag(StartVerbatim(kind)) => {
				if let VerbatimKind::Verbatim { length } = kind {
					self.state
						.verbatim_markers
						.push(TextRange::at(TextSize::from(self.state.buffer.len() as u32), *length));
				}
			...
			FormatElement::Tag(
				tag @ (EndLabelled | EndEntry | EndGroup | EndConditionalContent | EndVerbatim | EndFill),
			) => {
				stack.pop(tag.kind())?;
			},

			FormatElement::Tag(tag @ EndIndentIfGroupBreaks(group_id)) => {
				if let PrintMode::Expanded = self.state.group_modes.unwrap_print_mode(*group_id, element) {
					indent_stack.pop();
				}
	...
	fn print_text(&mut self, text:&str, source_position:Option<TextSize>) {
		...
		self.push_marker(SourceMarker { source:self.state.source_position, dest:self.state.buffer.text_len() });
	...
	fn flush_line_suffixes(
		...
		if suffixes.len() > 0 {
			// Print this line break element again once all the line suffixes have been
			// flushed
			...
			for entry in suffixes.rev() {
				match entry {
					...
					LineSuffixEntry::Args(args) => {
						...
						const LINE_SUFFIX_END:&FormatElement = &FormatElement::Tag(Tag::EndLineSuffix);
	...
	fn print_best_fitting(
		...
		if args.mode().is_flat() && self.state.measured_group_fits {
			...
			for variant in normal_variants.iter() {
				...
				// Skip the first element because we want to override the args for the entry and
				// the args must be popped from the stack as soon as it sees the matching
				// end entry.
				...
				// Remove the content slice because printing needs the variant WITH the start
				// entry
	...
	///
	/// `Fill` is a sequence of *item*, *separator*, *item*, *separator*,
	/// *item*, ... entries. The goal is to fit as many items (with their
	/// separators) on a single line as possible and first expand the
	/// *separator* if the content exceeds the print width and only fallback to
	/// expanding the *item*s if the *item* or the *item* and the expanded
	/// *separator* don't fit on the line.
	///
	...
	/// * The *item* and *separator* fit on the line but there's not enough
	///   space for the *next item*. Print the *item* in flat mode and the
	///   *separator* in expanded mode.
	...
	/// * The *item* fits on the line but the *separator* does not in flat
	///   **NOR** expanded mode. Print the *item* and *separator* in expanded
	///   mode.
	/// * The *item* does not fit on the line. Print the *item* and *separator*
	///   in expanded mode.
	fn print_fill_entries(
		...
		while matches!(queue.top(), Some(FormatElement::Tag(Tag::StartEntry))) {
			...
			let last_pair_layout = if item_fits {
				// Measure the remaining pairs until the first item or separator that does not
				// fit (or the end of the fill element). Optimisation to avoid re-measuring
				// the next-item twice:
				// * Once when measuring if the *item*, *separator*, *next-item* fit
				// * A second time when measuring if *next-item*, *separator*, *next-next-item*
				//   fit.
				loop {
			...
			for _ in 0..flat_pairs {
				self.print_fill_item(queue, stack, indent_stack, args.with_print_mode(PrintMode::Flat))?;

				self.print_fill_separator(queue, stack, indent_stack, args.with_print_mode(PrintMode::Flat))?;
			}
			...
			let item_mode = match last_pair_layout {
				...
				FillPairLayout::ItemMaybeFlat => {
					...
					// Re-measuring is required to get the measurer in the correct state for
					// measuring the separator.
			...
			if matches!(queue.top(), Some(FormatElement::Tag(Tag::StartEntry))) {
				...
				// Push a new stack frame with print mode `Flat` for the case where the
				// separator gets printed in expanded mode but does contain a group to
				// ensure that the group will measure "fits" with the "flat" versions of the
				// next item/separator.
				stack.push(TagKind::Fill, args.with_print_mode(PrintMode::Flat));

				self.print_fill_separator(queue, stack, indent_stack, args.with_print_mode(separator_mode))?;
	...
	/// Fully print an element (print the element itself and all its
	/// descendants)
	///
...
enum FillPairLayout {
	/// The item, separator, and next item fit. Print the first item and the
	/// separator in flat mode.
	Flat,

	/// The item and separator fit but the next element does not. Print the item
	/// in flat mode and the separator in expanded mode.
	ItemFlatSeparatorExpanded,

	/// The item does not fit. Print the item and any potential separator in
	/// expanded mode.
	Expanded,

	/// The item fits but the separator does not in flat mode. If the separator
	/// fits in expanded mode then print the item in flat and the separator in
	/// expanded mode, otherwise print both in expanded mode.
	ItemMaybeFlat,
}
...
/// Tracks the mode in which groups with ids are printed. Stores the groups at
/// `group.id()` index. This is based on the assumption that the group ids for a
/// single document are dense.
#[derive(Debug, Default)]
...
impl GroupModes {
	...
	fn unwrap_print_mode(&self, group_id:GroupId, next_element:&FormatElement) -> PrintMode {
		self.get_print_mode(group_id).unwrap_or_else(|| {
			panic!(
				"Expected group with id {group_id:?} to exist but it wasn't present in the document. Ensure that a \
				 group with such a document appears in the document before the element {next_element:?}."
			)
		})
	}
}
...
enum Indention {
	/// Indent the content by `count` levels by using the indention sequence
	/// specified by the printer options.
	Level(u16),

	/// Indent the content by n-`level`s using the indention sequence specified
	/// by the printer options and `align` spaces.
	Align { level:u16, align:NonZeroU8, align_count:u16 },
}

impl Indention {
	const fn is_empty(&self) -> bool { matches!(self, Indention::Level(0)) }

	/// Creates a new indention level with a zero-indent.
	const fn new() -> Self { Indention::Level(0) }
	...
	///
	/// The behaviour depends on the [`indent_style`][IndentStyle] if this is an
	/// [Indent::Align]:
	/// * **Tabs**: `align` is converted into an indent. This results in `level`
	///   increasing by two: once for the align, once for the level increment
	/// * **Spaces**: Increments the `level` by one and keeps the `align`
	///   unchanged.
	///
	/// Keeps any  the current value is [Indent::Align] and increments the level
	/// by one.
	...
	///
	/// It increments the `level` value if the current value is
	/// [Indent::IndentAlign].
	fn set_align(self, count:NonZeroU8) -> Self {
		match self {
			Indention::Level(indent_count) => Indention::Align { level:indent_count, align:count, align_count:1 },
...
struct FitsMeasurer<'a, 'print> {
	...
	/// Bomb that enforces that finish is explicitly called to restore the
	/// `fits_stack` and `fits_queue` vectors.
	bomb:DebugDropBomb,
}
...
impl<'a, 'print> FitsMeasurer<'a, 'print> {
	...
	fn new(
		...
		let fits_indent_stack = FitsIndentStack::new(print_indent_stack, saved_indent_stack, saved_stack_tem_indent);
		...
		Self {
			...
			must_be_flat:false,
			printer,
			bomb:DebugDropBomb::new("MeasurerFits must be `finished` to restore the `fits_queue` and `fits_stack`."),
		}
	}

	/// Tests if it's possible to print the content of the queue up to the first
	/// hard line break or the end of the document on a single line without
	...
	fn fill_item_fits(&mut self) -> PrintResult<bool> { self.fill_entry_fits(PrintMode::Flat) }

	/// Tests if the content of a `Fill` separator fits with `mode`.
	///
	/// Returns `Err` if the top element of the queue is not a [Tag::StartEntry]
	/// or if the document has any mismatching start/end tags.
	fn fill_separator_fits(&mut self, mode:PrintMode) -> PrintResult<bool> { self.fill_entry_fits(mode) }
	...
	fn fits_element(&mut self, element:&'a FormatElement) -> PrintResult<Fits> {
		...
		match element {
				...
				if args.mode().is_flat() {
					match line_mode {
						...
						LineMode::Hard | LineMode::Empty => {
							...
							//     -4, -5, -6
							//   ]
							// Here, `-4` contains a hardline because of the leading comment, but
							// that doesn't cause the element (`-4`) nor the separator (`,`)
							// to print in expanded mode, allowing the rest of the elements
							// to fill in. If this _did_ respect `must_be_flat` and returned
							// `Fits::No` instead, the result would put the `-4` on its own
							// line, which is not preferable (at least, it doesn't match
							// Prettier):   [
							//     -1, -2, 3
							//     // leading comment
							//     -4,
							//     -5, -6
							//   ]
							// The perception here is that most comments inline for fills are used
							// to separate _groups_ rather than to single out an individual
							// element.
							//
							...
							// expand _because_ of this hard line, and so the fill entry and
							// separator are automatically printed in expanded mode anyway,
							// and this fit check has no bearing on that (so it doesn't need
							// to care about flat or not):   <div>
							//     <span a b>
							//       <Foo />
							//     </span>{" "}
							//     ({variable})
							//   </div>
							// Here the `<span>...</span>` _is_ grouped and contains a hardline, so
							// it is known to break and _not_ fit already because the check
							// is performed on the group. But within the group itself, the
							// content with hardlines (the `\n<Foo />\n`) _does_ fit, for the
							// same logic in the first case.
							return Ok(Fits::Yes);
						},
					}
				} else {
					// Reachable if the restQueue contains an element with mode expanded because
					// Expanded is what the mode's initialized to by default
					// This means, the printer is outside of the current element at this point and
					// any line break should be printed as regular line break -> Fits
					return Ok(Fits::Yes);
				}
			},
			...
			FormatElement::Tag(StartGroup(group)) => {
				...
				let group_mode = if !group.mode().is_flat() { PrintMode::Expanded } else { args.mode() };
			...
			FormatElement::Tag(StartConditionalContent(condition)) => {
				let group_mode = match condition.group_id {
					None => args.mode(),
					Some(group_id) => self.group_modes().get_print_mode(group_id).unwrap_or_else(|| args.mode()),
				};
			...
			FormatElement::Tag(StartIndentIfGroupBreaks(id)) => {
				let group_mode = self.group_modes().get_print_mode(*id).unwrap_or_else(|| args.mode());
			...
			FormatElement::Tag(tag @ EndIndentIfGroupBreaks(group_id)) => {
				let group_mode = self.group_modes().get_print_mode(*group_id).unwrap_or_else(|| args.mode());
	...
	fn fits_text(&mut self, text:&str) -> Fits {
		...
		self.state.line_width +=
			indent.level() as usize * self.options().indent_width().value() as usize + indent.align() as usize;
		...
		for c in text.chars() {
			let char_width = match c {
				'\t' => self.options().indent_width.value() as usize,
				'\n' => {
					return if self.must_be_flat || self.state.line_width > self.options().print_width.into() {
						Fits::No
					} else {
						Fits::Yes
					};
	...
	fn options(&self) -> &PrinterOptions { &self.printer.options }

	fn group_modes(&self) -> &GroupModes { &self.printer.state.group_modes }

	fn group_modes_mut(&mut self) -> &mut GroupModes { &mut self.printer.state.group_modes }
}
...
fn invalid_end_tag<R>(end_tag:TagKind, start_tag:Option<TagKind>) -> PrintResult<R> {
	Err(PrintError::InvalidDocument(match start_tag {
		...
		Some(kind) => InvalidDocumentError::StartEndTagMismatch { start_kind:end_tag, end_kind:kind },
	}))
}
...
mod tests {
	use crate::{
		...
		format_args,
		prelude::*,
		printer::{PrintWidth, Printer, PrinterOptions},
		write,
	};
	...
	fn format_with_options(root:&dyn Format<SimpleFormatContext>, options:PrinterOptions) -> Printed {
	...
	fn it_prints_a_group_on_a_single_line_if_it_fits() {
		let result =
			format(&FormatArrayElements { items:vec![&text("\"a\""), &text("\"b\""), &text("\"c\""), &text("\"d\"")] });
...
two lines`,
  ...
  ["0123456789", "0123456789", "0123456789", "0123456789", "0123456789"],
	...
	fn it_use_the_indent_character_specified_in_the_options() {
		...
		let result = format_with_options(
			&FormatArrayElements { items:vec![&text("'a'"), &text("'b'"), &text("'c'"), &text("'d'")] },
			options,
		);
	...
	fn it_prints_consecutive_empty_lines_as_one() {
		let result = format(&format_args![text("a"), empty_line(), empty_line(), empty_line(), text("b"),]);
	...
	fn conditional_with_group_id_in_fits() {
		let content = format_with(|f| {
			...
			write!(
				f,
				[
					group(&format_args![text("The referenced group breaks."), hard_line_break()])
						.with_group_id(Some(group_id)),
					group(&format_args![
						text("This group breaks because:"),
						soft_line_break_or_space(),
						if_group_fits_on_line(&text("This content fits but should not be printed."))
							.with_group_id(Some(group_id)),
						if_group_breaks(&text(
							"It measures with the 'if_group_breaks' variant because the referenced group breaks and \
		...
		assert_eq!(
			printed.as_code(),
			"The referenced group breaks.\nThis group breaks because:\nIt measures with the 'if_group_breaks' variant \
			 because the referenced group breaks and that's just way too much text."
		);
	}
	...
	fn out_of_order_group_ids() {
		let content = format_with(|f| {
			...
			write!(
				...
				[
					group(&text(
						"Group with id-1 does not fit on the line because it exceeds the line width of 80 characters \
						 by"
					))
					.with_group_id(Some(id_1)),
					hard_line_break()
				]
			)?;
