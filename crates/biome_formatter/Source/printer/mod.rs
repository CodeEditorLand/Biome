...
use crate::{
	format_element::{document::Document, tag::Condition, BestFittingElement, LineMode, PrintMode},
	prelude::{
...
impl<'a> Printer<'a> {
				...
				let group_mode = if !group.mode().is_flat() {
					...
					match args.mode() {
						PrintMode::Flat if self.state.measured_group_fits => {
							// A parent group has already verified that this
							// group fits on a single line Thus, just
							// continue in flat mode
							PrintMode::Flat
						},
						// The printer is either in expanded mode or it's
						// necessary to re-measure if the group fits
						...
						_ => {
							...
							// Measure to see if the group fits up on a single
							// line. If that's the case, print the group
							// in "flat" mode, otherwise continue in expanded
							// mode
	...
	fn flush_line_suffixes(
		...
		if suffixes.len() > 0 {
			// Print this line break element again once all the line suffixes
			// have been flushed
	...
	fn print_best_fitting(
		...
		if args.mode().is_flat() && self.state.measured_group_fits {
			...
			for variant in normal_variants.iter() {
				// Test if this variant fits and if so, use it. Otherwise try
				// the next variant.
				...
				// Skip the first element because we want to override the args
				// for the entry and the args must be popped from the stack
				// as soon as it sees the matching end entry.
				...
				// Remove the content slice because printing needs the variant
				// WITH the start entry
	...
	fn print_fill_entries(
		...
		// It's already known that the content fit, print all items in flat
		// mode.
		...
		while matches!(queue.top(), Some(FormatElement::Tag(Tag::StartEntry))) {
			...
			let last_pair_layout = if item_fits {
				// Measure the remaining pairs until the first item or separator
				// that does not fit (or the end of the fill element).
				...
				loop {
					...
					if item_fits {
						...
						// Item and separator both fit, but the next element
						// doesn't. Print the separator in expanded mode
						// and then re-measure if the item now fits in the
			...
			let item_mode = match last_pair_layout {
				...
				FillPairLayout::ItemMaybeFlat => {
					let mut measurer = FitsMeasurer::new_flat(queue, stack, indent_stack, self);
					// SAFETY: That the item fits is guaranteed by
					// `ItemMaybeFlat`. Re-measuring is required to get the
					// measurer in the correct state for measuring the
					// separator.
			...
			if matches!(queue.top(), Some(FormatElement::Tag(Tag::StartEntry))) {
				...
				// Push a new stack frame with print mode `Flat` for the case
				// where the separator gets printed in expanded mode
				// but does contain a group to ensure that the group will
				// measure "fits" with the "flat" versions of the next
				// item/separator.
				stack.push(TagKind::Fill, args.with_print_mode(PrintMode::Flat));
				self.print_fill_separator(
					queue,
		...
		while let Some(element) = queue.pop() {
			match element {
				FormatElement::Tag(Tag::StartEntry) => {
					// Handle the start of the first element by pushing the args
					// on the stack.
					if depth == 0 {
						depth = 1;
				...
				FormatElement::Tag(Tag::EndEntry) => {
					depth -= 1;
					// Reached the end entry, pop the entry from the stack and
					// return.
					if depth == 0 {
						stack.pop(TagKind::Entry)?;
						return Ok(());
...
struct PrinterState<'a> {
	...
	// Re-used queue to measure if a group fits. Optimisation to avoid
	// re-allocating a new vec everytime a group gets measured
...
impl GroupModes {
	...
	fn unwrap_print_mode(&self, group_id:GroupId, next_element:&FormatElement) -> PrintMode {
		self.get_print_mode(group_id).unwrap_or_else(|| {
			panic!(
				"Expected group with id {group_id:?} to exist but it wasn't present in the \
				 document. Ensure that a group with such a document appears in the document \
				 before the element {next_element:?}."
			)
		})
	}
}
	...
	fn fits_element(&mut self, element:&'a FormatElement) -> PrintResult<Fits> {
		...
		match element {
			...
			FormatElement::Line(line_mode) => {
				if args.mode().is_flat() {
					match line_mode {
						...
						LineMode::Soft => {},
						LineMode::Hard | LineMode::Empty => {
							// Even in flat mode, content that _directly_
							// contains a hard or empty
							// line is considered to fit when a hard break is
							// reached, since that break is always going
							// to exist, regardless of the print mode.
							// This is particularly important for `Fill`
							// entries, where _ungrouped_ content that
							// contains hard breaks shouldn't force the
							// surrounding elements to also expand.
							// Example:   [
							//     -1, -2, 3
							//     // leading comment
							//     -4, -5, -6
							//   ]
							// Here, `-4` contains a hardline because of the
							// leading comment, but that doesn't cause
							// the element (`-4`) nor the separator (`,`) to
							// print in expanded mode, allowing the rest
							// of the elements to fill in. If this
							// _did_ respect `must_be_flat` and returned
							// `Fits::No` instead, the result
							// would put the `-4` on its own line, which is not
							// preferable (at least, it doesn't match
							// Prettier):   [
							//     -1, -2, 3
							//     // leading comment
							//     -4,
							//     -5, -6
							//   ]
							// The perception here is that most comments inline
							// for fills are used to separate _groups_
							// rather than to single out an individual element.
							//
							// The alternative case is when the fill entry is
							// grouped, in which case this fit returns
							// `Yes`, but the parent group is already known to
							// expand _because_ of this hard line, and so the
							// fill entry and separator are automatically
							// printed in expanded mode anyway, and this fit
							// check has no bearing on that (so it
							// doesn't need to care about flat or not):
							//   <div>
							//     <span a b>
							//       <Foo />
							//     </span>{" "}
							//     ({variable})
							//   </div>
							// Here the `<span>...</span>` _is_ grouped and
							// contains a hardline, so it is known to
							// break and _not_ fit already because the check is
							// performed on the group. But within the
							// group itself, the content with hardlines
							// (the `\n<Foo />\n`) _does_ fit, for the same
							// logic in the first case.
							return Ok(Fits::Yes);
						},
					}
				} else {
					// Reachable if the restQueue contains an element with mode
					// expanded because Expanded is what the mode's
					// initialized to by default This means, the printer is
					// outside of the current element at this point and any
			...
			FormatElement::LocatedTokenText { slice, .. } => {
				return Ok(self.fits_text(slice));
			},
	...
	fn fits_text(&mut self, text:&str) -> Fits {
		...
		for c in text.chars() {
			let char_width = match c {
				'\t' => self.options().indent_width.value() as usize,
				'\n' => {
					return if self.must_be_flat { Fits::No } else { Fits::Yes };
				},
	...
	fn conditional_with_group_id_in_fits() {
		...
		assert_eq!(
			...
			"The referenced group breaks.\nThis group breaks because:\nIt measures with the \
			 'if_group_breaks' variant because the referenced group breaks and that's just way \
			 too much text."
		);
	}
	...
	fn out_of_order_group_ids() {
		let content = format_with(|f| {
			...
			write!(
				...
				[
					group(&text(
						"Group with id-1 does not fit on the line because it exceeds the line \
						 width of 80 characters by"
					))
