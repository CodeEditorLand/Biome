	...
	fn fmt(&self, fmt: &mut std::fmt::Formatter) -> std::fmt::Result {
		match self {
			...
			FormatElement::Line(mode) => {
				fmt.debug_tuple("Line").field(mode).finish()
			},
			...
			FormatElement::Tag(tag) => {
				fmt.debug_tuple("Tag").field(tag).finish()
			},
		}
	}
}
...
pub const LINE_TERMINATORS: [char; 3] =
...
impl FormatElements for FormatElement {
	...
	fn has_label(&self, label_id: LabelId) -> bool {
		match self {
			...
			FormatElement::Interned(interned) => {
				interned.deref().has_label(label_id)
			},
			_ => false,
		}
	}
	...
	fn end_tag(&self, kind: TagKind) -> Option<&Tag> {
		match self {
			FormatElement::Tag(tag) if tag.kind() == kind && tag.is_end() => {
				Some(tag)
			},
			_ => None,
		}
...
impl BestFittingElement {
	...
	pub unsafe fn from_vec_unchecked(
		variants: Vec<Box<[FormatElement]>>,
	) -> Self {
		debug_assert!(
			variants.len() >= 2,
...
#[cfg(target_pointer_width = "64")]
static_assert!(
	std::mem::size_of::<crate::format_element::tag::VerbatimKind>() == 8usize
);
