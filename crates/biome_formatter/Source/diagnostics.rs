...
pub enum FormatError {
	/// In case a node can't be formatted because it either misses a require
	/// child element or a child is present that should not (e.g. a trailing
	/// comma after a rest element).
	SyntaxError,
	...
	/// In case printing the document failed because it has an invalid
	/// structure.
	InvalidDocument(InvalidDocumentError),

	/// Formatting failed because some content encountered a situation where a
	/// layout choice by an enclosing [crate::Format] resulted in a poor layout
	/// for a child [crate::Format].
	///
	/// It's up to an enclosing [crate::Format] to handle the error and pick
	/// another layout. This error should not be raised if there's no outer
	/// [crate::Format] handling the poor layout error, avoiding that
	/// formatting of the whole document fails.
	PoorLayout,
}
...
impl From<SyntaxError> for FormatError {
	fn from(error:SyntaxError) -> Self { FormatError::from(&error) }
}
...
impl From<PrintError> for FormatError {
	fn from(error:PrintError) -> Self { FormatError::from(&error) }
}
...
impl Diagnostic for FormatError {
	...
	fn message(
		...
		match self {
			...
			FormatError::InvalidDocument(error) => {
				std::write!(fmt, "Invalid document: {error}")
			},
			FormatError::PoorLayout => {
				std::write!(
					fmt,
...
mod test {
	...
	use crate::{
		diagnostics::{ActualStart, InvalidDocumentError},
		prelude::{FormatError, TagKind},
	};
