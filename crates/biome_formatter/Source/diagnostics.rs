...
use biome_diagnostics::{
	category, Category, Diagnostic, DiagnosticTags, Location, Severity,
};
...
impl From<&PrintError> for FormatError {
	fn from(error: &PrintError) -> Self {
		match error {
			PrintError::InvalidDocument(reason) => {
				FormatError::InvalidDocument(*reason)
			},
		}
	}
}
	...
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		match self {
			...
			InvalidDocumentError::StartTagMissing { kind } => {
				std::write!(
					f,
					"End tag of kind {kind:?} without matching start tag."
				)
			},
