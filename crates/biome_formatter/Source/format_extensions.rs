...
pub trait MemoizeFormat<Context> {
	/// Returns a formattable object that memoizes the result of `Format` by
	/// cloning. Mainly useful if the same sub-tree can appear twice in the
	/// formatted output because it's used inside of `if_group_breaks` or
	/// `if_group_fits_single_line`.
	///
	/// ```
	/// use std::cell::Cell;
	///
	/// use biome_formatter::{format, prelude::*, write};
	...
	///
	/// impl MyFormat {
	/// 	pub fn new() -> Self { Self { value:Cell::new(1) } }
	/// }
	///
	...
	/// 	format!(SimpleFormatContext::default(), [normal, space(), normal])?
	/// 		.print()?
	/// 		.as_code()
	/// );
	///
	...
	/// 	format![SimpleFormatContext::default(), [memoized, space(), memoized]]?
	/// 		.print()?
	/// 		.as_code()
	/// );
	/// # Ok(())
...
/// Memoizes the output of its inner [Format] to avoid re-formatting a potential
/// expensive object.
#[derive(Debug)]
	...
	F: Format<Context>,
{
	...
	///
	/// Performs the formatting if the content hasn't been formatted at this
	/// point.
	///
	/// # Example
	///
