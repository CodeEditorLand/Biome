use std::{borrow::Cow, cell::Cell, marker::PhantomData, num::NonZeroU8};
...
/// A line break that only gets printed if the enclosing `Group` doesn't fit on
/// a single line. It's omitted if the enclosing `Group` fits on a single line.
/// A soft line break is identical to a hard line break when not enclosed inside
/// of a `Group`.
///
/// # Examples
///
...
///
/// ```
/// use biome_formatter::{format, format_args, prelude::*};
///
...
/// # }
/// ```
/// See [soft_line_break_or_space] if you want to insert a space between the
/// elements if the enclosing `Group` fits on a single line.
///
...
/// ```
#[inline]
pub const fn soft_line_break() -> Line { Line::new(LineMode::Soft) }

/// A forced line break that are always printed. A hard line break forces any
/// enclosing `Group` to be printed over multiple lines.
///
/// # Examples
///
/// It forces a line break, even if the enclosing `Group` would otherwise fit on
/// a single line. ```
/// use biome_formatter::{format, format_args, prelude::*};
///
/// # fn main() -> FormatResult<()> {
...
/// ```
#[inline]
pub const fn hard_line_break() -> Line { Line::new(LineMode::Hard) }

/// A forced empty line. An empty line inserts enough line breaks in the output
/// for the previous and next element to be separated by an empty line.
///
/// # Examples
///
/// ```
...
/// ```
#[inline]
pub const fn empty_line() -> Line { Line::new(LineMode::Empty) }

/// A line break if the enclosing `Group` doesn't fit on a single line, a space
/// otherwise.
///
/// # Examples
///
/// The line breaks are emitted as spaces if the enclosing `Group` fits on a a
/// single line: ```
/// use biome_formatter::{format, format_args, prelude::*};
///
/// # fn main() -> FormatResult<()> {
...
/// ```
#[inline]
pub const fn soft_line_break_or_space() -> Line { Line::new(LineMode::SoftOrSpace) }
...
impl Line {
	const fn new(mode:LineMode) -> Self { Self { mode } }
}
...
/// Creates a token that gets written as is to the output. Make sure to properly
/// escape the text if it's user generated (e.g. a string and not a language
/// keyword).
///
/// # Line feeds
/// Tokens may contain line breaks but they must use the line feeds (`\n`).
/// The [crate::Printer] converts the line feed characters to the character
/// specified in the [crate::PrinterOptions].
///
/// # Examples
///
/// ```
/// use biome_formatter::{format, prelude::*};
///
/// # fn main() -> FormatResult<()> {
/// let elements = format!(SimpleFormatContext::default(), [text("Hello World")])?;
///
/// assert_eq!("Hello World", elements.print()?.as_code());
/// # Ok(())
/// # }
/// ```
///
/// Printing a string literal as a literal requires that the string literal is
/// properly escaped and enclosed in quotes (depending on the target language).
///
/// ```
...
/// [`Cow::Borrowed`] or some replaced content if `text` is [`Cow::Owned`].
...
pub fn located_token_text<L:Language>(token:&SyntaxToken<L>, range:TextRange) -> LocatedTokenText {
...
/// Inserts a boundary for line suffixes that forces the printer to print all
/// pending line suffixes. Helpful if a line sufix shouldn't pass a certain
/// point.
///
/// ## Examples
///
...
/// let elements = format!(
/// 	SimpleFormatContext::default(),
/// 	[text("a"), line_suffix(&text("c")), text("b"), line_suffix_boundary(), text("d")]
/// )?;
///
/// assert_eq!("abc\nd", elements.print()?.as_code());
/// # Ok(())
/// # }
/// ```
pub const fn line_suffix_boundary() -> LineSuffixBoundary { LineSuffixBoundary }
...
/// parts of the formatter may inspect the [labelled
/// element](Tag::StartLabelled) using [FormatElements::has_label].
///
/// ## Examples
///
/// ```rust
...
/// 		write!(recording, [labelled(LabelId::of(MyLabels::Main), &text("'I have a label'"))])?;
///
/// 		let recorded = recording.stop();
///
/// 		let is_labelled = recorded
/// 			.first()
/// 			.map_or(false, |element| element.has_label(LabelId::of(MyLabels::Main)));
///
/// 		if is_labelled {
...
/// ## Alternatives
///
/// Use `Memoized.inspect(f)?.has_label(LabelId::of(MyLabels::Main)` if you need
/// to know if some content breaks that should only be written later.
#[inline]
...
/// ```
#[inline]
pub const fn space() -> Space { Space }
...
///
/// ```
/// use biome_formatter::{format, format_args, prelude::*, LineWidth, SimpleFormatOptions};
///
/// # fn main() -> FormatResult<()> {
/// let context = SimpleFormatContext::new(SimpleFormatOptions {
...
/// 	])]
/// )?;
/// assert_eq!("nineteen_characters\n1", elements.print()?.as_code());
/// # Ok(())
/// # }
/// ```
/// # Examples
...
/// 	])]
/// )?;
/// assert_eq!("nineteen_characters1", elements.print()?.as_code());
/// # Ok(())
/// # }
/// ```

#[inline]
pub const fn hard_space() -> HardSpace { HardSpace }
...
/// # fn main() -> FormatResult<()> {
/// let elements =
/// 	format!(SimpleFormatContext::default(), [text("a"), maybe_space(true), text("b")])?;
/// let nospace =
/// 	format!(SimpleFormatContext::default(), [text("a"), maybe_space(false), text("b")])?;
///
/// assert_eq!("a b", elements.print()?.as_code());
...
///
/// This helper should be used only in rare cases, instead you should rely more
/// on [block_indent] and [soft_block_indent]
///
/// # Examples
///
/// ```
...
/// ```
///
/// When the indent_style is tab, [indent] convert the preceding alignments to
/// indents ```
/// use biome_formatter::{format, format_args, prelude::*};
///
/// # fn main() -> FormatResult<()> {
...
/// )?;
///
/// assert_eq!("root\n\t\t\tshoud be 3 tabs", block.print()?.as_code());
/// #    Ok(())
/// # }
/// ```
...
/// It reduces the indention for the given content depending on the closest
/// [indent] or [align] parent element.
...
/// 				hard_line_break(),
/// 				text("aligned"),
/// 				dedent(&format_args![hard_line_break(), text("not aligned"),]),
...
/// 			]
/// 		),
/// 		dedent(&format_args![hard_line_break(), text("Dedent on root level is a no-op.")])
/// 	]
/// )?;
///
/// assert_eq!(
...
/// 					dedent(&format_args![hard_line_break(), text("Indented, not aligned"),]),
/// 				]
/// 			),
/// 		]),
/// 		align(
...
/// 					dedent(&format_args![hard_line_break(), text("aligned, not Intended"),]),
/// 				])
/// 			]
/// 		),
...
/// It resets the indent document so that the content will be printed at the
/// start of the line.
///
/// # Examples
///
...
/// ## Prettier
///
/// This resembles the behaviour of Prettier's `align(Number.NEGATIVE_INFINITY,
/// content)` IR element.
#[inline]
...
///
/// [align] is a variant of `[indent]` that indents its content by a specified
/// number of spaces rather than using the configured indent character (tab or a
/// specified number of spaces).
///
/// You should use [align] when you want to indent a content by a specific
/// number of spaces. Using [indent] is preferred in all other situations as it
/// respects the users preferred indent character.
///
/// # Examples
///
/// ## Tab indention
...
/// 		text("?"),
/// 		space(),
/// 		align(2, &format_args![text("function () {"), hard_line_break(), text("}"),]),
/// 		hard_line_break(),
/// 		text(":"),
/// 		space(),
/// 		align(
...
/// You can see that:
///
/// * the printer indents the function's `}` by two spaces because it is inside
///   of an `align`.
/// * the block `console.log` gets indented by two tabs. This is because `align`
///   increases the indention level by one (same as `indent`) if you nest an
///   `indent` inside an `align`. Meaning that, `align > ... > indent` results
...
/// use std::num::NonZeroU8;
///
/// use biome_formatter::{format, format_args, prelude::*, IndentStyle, SimpleFormatOptions};
///
/// # fn main() -> FormatResult<()> {
/// let context = SimpleFormatContext::new(SimpleFormatOptions {
/// 	indent_style:IndentStyle::Space,
...
/// ```
///
/// The printing of `align` differs if using spaces as indention sequence *and*
/// it contains an `indent`. You can see the difference when comparing the
/// indention of the `console.log(...)` expression to the previous example:
///
/// * tab indention: Printer indents the expression with two tabs because the
///   `align` increases the indention level.
/// * space indention: Printer indents the expression by 4 spaces (one indention
///   level) **and** 2 spaces for the align.
...
/// Inserts a hard line break before and after the content and increases the
/// indention level for the content by one.
///
/// Block indents indent a block of code, such as in a function body, and
/// therefore insert a line break before and after the content.
///
...
/// Indents the content by inserting a line break before and after the content
/// and increasing the indention level for the content by one if the enclosing
/// group doesn't fit on a single line. Doesn't change the formatting if the
/// enclosing group fits on a single line.
///
/// # Examples
///
/// Indents the content by one level and puts in new lines if the enclosing
/// `Group` doesn't fit on a single line
///
/// ```
/// use biome_formatter::{format, format_args, prelude::*, LineWidth, SimpleFormatOptions};
///
...
/// 	[group(&format_args![
/// 		text("["),
/// 		soft_block_indent(&format_args![text("5,"), soft_line_break_or_space(), text("10"),]),
/// 		text("]"),
/// 	])]
/// )?;
///
/// assert_eq!("[5, 10]", elements.print()?.as_code());
/// # Ok(())
/// # }
/// ```
#[inline]
...
/// # Examples
///
/// Adds line breaks and indents the content if the enclosing group doesn't fit
/// on the line.
///
/// ```
/// use biome_formatter::{format, format_args, prelude::*, LineWidth, SimpleFormatOptions};
...
/// ```
///
/// Adds spaces around the content if the caller requests it and the group fits
/// on the line.
///
/// ```
/// use biome_formatter::{format, format_args, prelude::*};
...
/// 		text("{"),
/// 		soft_block_indent_with_maybe_space(
/// 			&format_args![text("a"), text(":"), space(), text("5"),],
/// 			true
/// 		),
/// 		text("}")
/// 	])]
/// )?;
///
/// assert_eq!("{ a: 5 }", elements.print()?.as_code());
/// # Ok(())
/// # }
/// ```
///
/// Does not add spaces around the content if the caller denies it and the group
/// fits on the line. ```
/// use biome_formatter::{format, format_args, prelude::*};
///
/// # fn main() -> FormatResult<()> {
...
/// and indent. Otherwise, just inserts a space.
///
/// Line indents are used to break a single line of code, and therefore only
/// insert a line break before the content and not after the content.
///
/// # Examples
///
/// Indents the content by one level and puts in new lines if the enclosing
/// `Group` doesn't fit on a single line. Otherwise, just inserts a space.
///
/// ```
/// use biome_formatter::{format, format_args, prelude::*, LineWidth, SimpleFormatOptions};
///
...
/// )?;
///
/// assert_eq!("name =\n\tfirstName + lastName", elements.print()?.as_code());
/// # Ok(())
/// # }
/// ```
///
...
/// )?;
///
/// assert_eq!("a = 10", elements.print()?.as_code());
/// # Ok(())
/// # }
/// ```
#[inline]
...
/// It functions similarly to soft_line_indent_or_space, but instead of a
/// regular space, it inserts a hard space.
///
/// # Examples
///
/// Indents the content by one level and puts in new lines if the enclosing
...
/// 		soft_line_break_or_space(),
/// 		text("="),
/// 		soft_line_indent_or_hard_space(&format_args![text("10"),]),
/// 	])]
/// )?;
///
/// assert_eq!("value\n=\n\t10", elements.print()?.as_code());
/// # Ok(())
/// # }
/// ```
...
/// Adds spaces around the content if its enclosing group fits on a line,
/// otherwise indents the content and separates it by line breaks.
///
/// # Examples
///
...
/// 	[group(&format_args![
/// 		text("{"),
/// 		soft_space_or_block_indent(&format_args![text("a"), text(":"), space(), text("5"),]),
/// 		text("}")
/// 	])]
/// )?;
///
...
/// Creates a logical `Group` around the content that should either consistently
/// be printed on a single line or broken across multiple lines.
///
/// The printer will try to print the content of the `Group` on a single line,
/// ignoring all soft line breaks and emitting spaces for soft line breaks or
/// spaces. The printer tracks back if it isn't successful either because it
/// encountered a hard line break, or because printing the `Group` on a single
/// line exceeds the configured line width, and thus it must print all its
/// content on multiple lines, emitting line breaks for all line break kinds.
///
/// # Examples
///
...
/// )?;
///
/// assert_eq!("[1, 2, 3]", elements.print()?.as_code());
/// # Ok(())
/// # }
/// ```
///
...
impl<Context> Group<'_, Context> {
	...
	/// Changes the [PrintMode] of the group from [`Flat`](PrintMode::Flat) to
	/// [`Expanded`](PrintMode::Expanded). The result is that any soft-line
	/// break gets printed as a regular line break.
	///
	/// This is useful for content rendered inside of a
	/// [FormatElement::BestFitting] that prints each variant
	/// in [PrintMode::Flat] to change some content to be printed in
	/// [`Expanded`](PrintMode::Expanded) regardless. See the documentation of
	/// the [`best_fitting`] macro for an example.
...
///
/// Has no effect if used outside of a group or element that introduce implicit
/// groups (fill element).
///
/// ## Examples
///
/// ```
/// use biome_formatter::{format, format_args, prelude::*, LineWidth};
///
/// # fn main() -> FormatResult<()> {
/// let elements = format!(
/// 	SimpleFormatContext::default(),
...
/// enclosing `Group` that is printed on a single line. The element allows, for
/// example, to insert a trailing comma after the last array element only if the
/// array doesn't fit on a single line.
///
/// The element has no special meaning if used outside of a `Group`. In that
/// case, the content is always emitted.
///
/// If you're looking for a way to only print something if the `Group` fits on a
/// single line see [self::if_group_fits_on_line].
///
/// # Examples
///
/// Omits the trailing comma for the last array element if the `Group` fits on a
/// single line ```
/// use biome_formatter::{format, format_args, prelude::*};
///
/// # fn main() -> FormatResult<()> {
...
/// The content isn't emitted for `Group`s spanning multiple lines.
///
/// See [if_group_breaks] if you're looking for a way to print content only for
/// groups spanning multiple lines.
///
/// # Examples
///
/// Adds the trailing comma for the last array element if the `Group` fits on a
/// single line ```
/// use biome_formatter::{format, format_args, prelude::*};
///
/// # fn main() -> FormatResult<()> {
...
/// )?;
///
/// assert_eq!("[1, 2, 3,]", formatted.print()?.as_code());
/// # Ok(())
/// # }
/// ```
/// 
...
impl<Context> IfGroupBreaks<'_, Context> {
	/// Inserts some content that the printer only prints if the group with the
	/// specified `group_id` is printed in multiline mode. The referred group
	/// must appear before this element in the document but doesn't have to one
	/// of its ancestors.
	///
	/// # Examples
	///
	/// Prints the trailing comma if the array group doesn't fit. The `group_id`
	/// is necessary because `fill` creates an implicit group around each item
	/// and tries to print the item in flat mode. The item `[4]` in this
	/// example fits on a single line but the trailing comma should still be
	/// printed
	///
	/// ```
	/// use biome_formatter::{format, format_args, prelude::*, write, LineWidth, SimpleFormatOptions};
	///
	/// # fn main() -> FormatResult<()> {
	/// let context = SimpleFormatContext::new(SimpleFormatOptions {
	/// 	line_width:LineWidth::try_from(20).unwrap(),
...
///
/// This IR has the same semantics as using [if_group_breaks] and
/// [if_group_fits_on_line] together.
///
/// ```
/// # use biome_formatter::prelude::*;
...
/// ```
///
/// If you want to indent some content if the enclosing group breaks, use
/// [`indent`].
///
/// Use [if_group_breaks] or [if_group_fits_on_line] if the fitting and breaking
/// content differs more than just the indention level.
///
/// # Examples
///
...
/// 				&format_args![hard_line_break(), text("a => b")],
/// 				group_id
/// 			)
/// 		]
/// 	)
...
{
	#[inline(always)]
	fn fmt(&self, f:&mut Formatter<Context>) -> FormatResult<()> { (self.formatter)(f) }
}
...
/// Creates an object implementing `Format` that calls the passed closure to
/// perform the formatting.
///
/// # Examples
///
/// ```
/// use biome_formatter::{format, prelude::*, write, SimpleFormatContext};
...
/// 							write!(f, [dynamic_text(item, TextSize::default())])
/// 						}));
/// 					}
/// 					join.finish()
/// 				})),
...
/// # fn main() -> FormatResult<()> {
/// let formatted =
/// 	format!(SimpleFormatContext::default(), [MyFormat { items:vec!["a", "b", "c"] }])?;
///
/// assert_eq!("(\n\ta b c\n)", formatted.print()?.as_code());
/// # Ok(())
/// # }
...
///
/// This can be useful in situation where the borrow checker doesn't allow you
/// to use [`format_with`] because the code formatting the content consumes the
/// value and cloning the value is too expensive. An example of this is if you
/// want to nest a `FormatElement` or non-cloneable `Iterator` inside of a
...
///
/// ```
/// use biome_formatter::{format, prelude::*, write, Buffer, SimpleFormatContext};
///
/// struct MyFormat;
///
	...
	Separator: Format<Context>,
{
	...
	/// Creates a new instance that prints the passed separator between every
	/// two entries.
	...
	pub fn finish(&mut self) -> FormatResult<()> { self.result }
}

/// Builder to join together nodes that ensures that nodes separated by empty
/// lines continue to be separated by empty lines in the formatted output.
#[must_use = "must eventually call `finish()` on Format builders"]
	...
	Separator: Format<Context>,
{
	...
	/// Adds a new node with the specified formatted content to the output,
	/// respecting any new lines that appear before the node in the input
	/// source.
	...
	/// Adds an iterator of entries to the output. Each entry is a `(node,
	/// content)` tuple.
...
/// Get the number of line breaks between two consecutive SyntaxNodes in the
/// tree
...
impl<'a, 'buf, Context> FillBuilder<'a, 'buf, Context> {
	...
	/// Adds an iterator of entries to the fill output. Uses the passed
	/// `separator` to separate any two items.
	...
	pub fn finish(&mut self) -> FormatResult<()> {
		self.result.and_then(|_| self.fmt.write_element(FormatElement::Tag(EndFill)))
	}
}

/// The first variant is the most flat, and the last is the most expanded
/// variant. See [`best_fitting!`] macro for a more in-detail documentation
#[derive(Copy, Clone)]
...
impl<'a, Context> BestFitting<'a, Context> {
	/// Creates a new best fitting IR with the given variants. The method itself
	/// isn't unsafe but it is to discourage people from using it because the
	/// printer will panic if the slice doesn't contain at least the least and
	/// most expanded variants.
	///
	/// You're looking for a way to create a `BestFitting` object, use the
	/// `best_fitting![least_expanded, most_expanded]` macro.
	///
	/// ## Safety
...
impl<Context> Format<Context> for BestFitting<'_, Context> {
	fn fmt(&self, f:&mut Formatter<Context>) -> FormatResult<()> {
		...
		// SAFETY: The constructor guarantees that there are always at least two
