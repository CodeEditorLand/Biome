...
use crate::prelude::*;
use crate::{
	format_element, write, Argument, Arguments, GroupId, TextRange, TextSize,
};
...
{
	Align {
		count: NonZeroU8::new(count)
			.expect("Alignment count must be a non-zero number."),
...
/// ```
#[inline]
pub fn soft_block_indent<Context>(
	content: &impl Format<Context>,
) -> BlockIndent<Context> {
...
impl<Context> Format<Context> for BlockIndent<'_, Context> {
	fn fmt(&self, f: &mut Formatter<Context>) -> FormatResult<()> {
		...
		match self.mode {
			...
			IndentMode::HardSpace => {
				write!(f, [hard_space(), soft_line_break()])?
			},
		}
...
/// ```
#[inline]
pub fn if_group_breaks<Content, Context>(
	content: &Content,
...
/// ```
#[inline]
pub fn if_group_fits_on_line<Content, Context>(
...
impl<Context> Format<Context> for IndentIfGroupBreaks<'_, Context> {
	fn fmt(&self, f: &mut Formatter<Context>) -> FormatResult<()> {
		f.write_element(FormatElement::Tag(StartIndentIfGroupBreaks(
			self.group_id,
		)))?;
		Arguments::from(&self.content).fmt(f)?;
...
impl<'fmt, 'buf, Separator, Context>
	JoinNodesBuilder<'fmt, 'buf, Separator, Context>
where
	Separator: Format<Context>,
{
	pub(super) fn new(
...
impl<'a, Context> BestFitting<'a, Context> {
	...
	pub unsafe fn from_arguments_unchecked(
		variants: Arguments<'a, Context>,
	) -> Self {
		assert!(
			variants.0.len() >= 2,
	...
	fn fmt(&self, f: &mut Formatter<Context>) -> FormatResult<()> {
		...
		let element = unsafe {
			FormatElement::BestFitting(
				format_element::BestFittingElement::from_vec_unchecked(
					formatted_variants,
				),
			)
		};
