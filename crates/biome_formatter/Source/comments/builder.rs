	...
	Style: CommentStyle,
{
	...
	fn visit_node(&mut self, event:WalkEvent<SyntaxNode<Style::Language>>) {
		match event {
			WalkEvent::Enter(node) => {
				// Lists cannot have comments attached. They either belong to
				// the entire parent or to the first child. So we ignore
				// lists all together
				...
				// Set following here because it is the "following node" of the
			...
			WalkEvent::Leave(node) => {
				...
				// We're passed this node, flush any pending comments for its
				// children
				self.following_node_index = None;
				self.flush_comments(None);

				// We're passed this node, so it must precede the sibling that
				// comes next.
	...
	fn visit_token(&mut self, token:SyntaxToken<Style::Language>) {
		...
		// Process the leading trivia of the current token. the trailing trivia
		// is handled as part of the next token
		...
		// Set following node to `None` because it now becomes the enclosing
		// node.
		if let Some(following_node) = self.following_node() {
			...
			// That means, following node is only set for the first token of a
			// node. Unset preceding node if this is the first token because
			// the preceding node belongs to the parent.
	...
	fn update_comments(
		...
		// Update the lines after of all comments as well as the positioning of
		// end of line comments.
		while let Some((index, comment)) = comments.next() {
			// Update the position of all trailing comments to be end of line as
			// we've seen a line break since.
	...
	fn visit_trailing_comments(
		...
		// Number of lines before the next comment, token, or skipped token
		// trivia
		let mut lines_before = 0;

		// Trailing comments are all `SameLine` comments EXCEPT if any is
		// followed by a line break, a leading comment (that always have line
		// breaks), or there's a line break before the token.
	...
	fn flush_before_r_paren_comments(
		...
		// Using the `enclosing` as default but it's mainly to satisfy Rust. The
		// only case where it is used is if someone formats a Parenthesized
		// expression as the root. Something we explicitly disallow
				...
				match comment.text_position {
					CommentTextPosition::EndOfLine => {
						match (comment.take_preceding_node(), comment.take_following_node()) {
							(Some(preceding), Some(_)) => {
								// Attach comments with both preceding and
								// following node to the preceding
								// because there's a line break separating it
								// from the following node.
								// ```javascript
								// a; // comment
								// b
								// ```
					...
					CommentTextPosition::OwnLine => {
						match (comment.take_preceding_node(), comment.take_following_node()) {
							...
							// b
							// ```
							// attach the comment to the `b` expression
							// statement
							(_, Some(following)) => {
								self.push_leading_comment(&following, comment);
							},
					...
					CommentTextPosition::SameLine => {
						match (comment.take_preceding_node(), comment.take_following_node()) {
							(Some(preceding), Some(following)) => {
								// Only make it a trailing comment if it
								// directly follows the preceding node but not
								// if it is separated by one or more
								// tokens ```javascript
								// a /* comment */ b;   //  Comment is a
								// trailing comment a, /* comment */ b;
								// // Comment should be a leading comment
								// ```
			...
			SourceParentheses::SourceMap { next, tail, .. } => {
				while let Some(range) = next {
					...
					if range.transformed == offset {
						// A deleted range can contain multiple tokens. See if
						// there's any `)` in the deleted
	...
	fn outer_most_parenthesized_node<L:Language>(
		...
		match self {
			...
			SourceParentheses::SourceMap { map, .. } => {
				...
				// How this works: We search the outer most node that, in the
				// source document ends right after the `)`. The issue is,
				// it is possible that multiple nodes end right after the `)`
				//
				// ```javascript
				// !(
				//     a
				...
				//  )
				// ```
				// The issue is, that in the transformed document, the
				// `ReferenceIdentifier`, `IdentifierExpression`,
				// `UnaryExpression`, and `ExpressionStatement` all end at
				// the end position of `)`. However, not all the nodes start
				// at the same position. That's why this code also tracks the
				// start. We first find the closest node that directly ends
				// at the position of the right paren. We then continue
				// upwards to find the most outer node that starts at the same
				// position as that node. (In this case,
				// `ReferenceIdentifier` -> `IdentifierExpression`.
				...
				let ancestors = token.ancestors().take_while(|node| {
					...
					// Greater than to guarantee that we always return at least
					// one node AND handle the case where a node is wrapped
					// in multiple parentheses. Take the first node that
					// fully encloses the parentheses
...
b;"#;
		...
		// A lot of code that simply removes the parenthesized expression and
		// moves the parens trivia to the identifiers leading / trailing
		// trivia.
