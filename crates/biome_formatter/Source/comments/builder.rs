...
use crate::{
	TextRange,
	TextSize,
	source_map::{DeletedRangeEntry, DeletedRanges},
};
	...
	Style: CommentStyle,
{
	...
	pub(super) fn visit(
		...
		for event in root.preorder_with_tokens(Direction::Next) {
			match event {
				WalkEvent::Enter(SyntaxElement::Node(node)) => self.visit_node(WalkEvent::Enter(node)),

				WalkEvent::Leave(SyntaxElement::Node(node)) => self.visit_node(WalkEvent::Leave(node)),
		...
		if let Some(last_token) = self.last_token.take() {
			...
			let (comments_start, lines_before, position, trailing_end) = self.visit_trailing_comments(last_token, None);
	...
	fn visit_node(&mut self, event:WalkEvent<SyntaxNode<Style::Language>>) {
		match event {
			WalkEvent::Enter(node) => {
				// Lists cannot have comments attached. They either belong to the entire parent
				// or to the first child. So we ignore lists all together
				...
				// Set following here because it is the "following node" of the next token's
				// leading trivia.
	...
	fn visit_token(&mut self, token:SyntaxToken<Style::Language>) {
		...
		// Process the leading trivia of the current token. the trailing trivia is
		// handled as part of the next token
		...
		if let Some(following_node) = self.following_node() {
			...
			// Unset preceding node if this is the first token because the preceding node
	...
	fn queue_comment(&mut self, comment:DecoratedComment<Style::Language>) { self.pending_comments.push(comment); }
	...
	fn update_comments(
		...
		// Update the lines after of all comments as well as the positioning of end of
		// line comments.
		while let Some((index, comment)) = comments.next() {
			// Update the position of all trailing comments to be end of line as we've seen
			// a line break since.
			if index < trailing_end && position.is_own_line() {
				comment.text_position = CommentTextPosition::EndOfLine;
			}

			comment.lines_after = comments.peek().map_or(lines_before, |(_, next)| next.lines_before);
		}
	}
	...
	fn visit_trailing_comments(
		...
		// Trailing comments are all `SameLine` comments EXCEPT if any is followed by a
		// line break, a leading comment (that always have line breaks), or there's a
		// line break before the token.
		...
		for piece in token.trailing_trivia().pieces() {
			...
			if let Some(parens_source_range) = self.parentheses.r_paren_source_range(piece.text_range().end()) {
	...
	/// Processes comments appearing right before a `)` of a parenthesized
	/// expressions.
	#[cold]
	fn flush_before_r_paren_comments(
		...
		let parenthesized_node = self.parentheses.outer_most_parenthesized_node(last_token, parens_source_range);

		let preceding = parenthesized_node;

		// Using the `enclosing` as default but it's mainly to satisfy Rust. The only
		// case where it is used is if someone formats a Parenthesized expression as
		// the root. Something we explicitly disallow in biome_js_formatter
...
impl<L:Language> CommentsBuilder<L> {
	fn add_comment(&mut self, placement:CommentPlacement<L>) {
		match placement {
			...
			CommentPlacement::Dangling { node, comment } => self.push_dangling_comment(&node, comment),
					...
					CommentTextPosition::EndOfLine => {
						match (comment.take_preceding_node(), comment.take_following_node()) {
							(Some(preceding), Some(_)) => {
								// Attach comments with both preceding and following node to the
								// preceding because there's a line break separating it from
								// the following node. ```javascript
								// a; // comment
								// b
								// ```
							...
							(None, None) => {
								self.push_dangling_comment(&comment.enclosing_node().clone(), comment);
							},
						}
					},
					...
					CommentTextPosition::SameLine => {
						match (comment.take_preceding_node(), comment.take_following_node()) {
							(Some(preceding), Some(following)) => {
								// Only make it a trailing comment if it directly follows the
								// preceding node but not if it is separated by one or more
								// tokens ```javascript
								// a /* comment */ b;   //  Comment is a trailing comment
								// a, /* comment */ b;  // Comment should be a leading comment
								// ```
								if preceding.text_range().end() == comment.piece().as_piece().token().text_range().end()
								{
									self.push_trailing_comment(&preceding, comment);
								} else {
									self.push_leading_comment(&following, comment);
	...
	fn mark_has_skipped(&mut self, token:&SyntaxToken<L>) { self.skipped.insert(token.key()); }
	...
	fn push_dangling_comment(&mut self, node:&SyntaxNode<L>, comment:impl Into<SourceComment<L>>) {
		self.comments.push_dangling(node.key(), comment.into());
	}

	fn push_trailing_comment(&mut self, node:&SyntaxNode<L>, comment:impl Into<SourceComment<L>>) {
		self.comments.push_trailing(node.key(), comment.into());
	}

	fn finish(self) -> (CommentsMap<SyntaxElementKey, SourceComment<L>>, FxHashSet<SyntaxElementKey>) {
		(self.comments, self.skipped)
	}
}

impl<L:Language> Default for CommentsBuilder<L> {
	fn default() -> Self { Self { comments:CommentsMap::new(), skipped:FxHashSet::default() } }
}
...
impl<'a> SourceParentheses<'a> {
	fn from_source_map(source_map:Option<&'a TransformSourceMap>) -> Self {
		match source_map {
			...
			Some(source_map) => {
				...
				SourceParentheses::SourceMap { map:source_map, next:deleted.next(), tail:deleted }
			},
		}
	}

	/// Returns the range of `node` including its parentheses if any. Otherwise
	...
	/// Tests if the next offset is at a position where the original source
	/// document used to have an `)`.
	///
	/// Must be called with offsets in increasing order.
	///
	/// Returns the source range of the `)` if there's any `)` in the deleted
	/// range at this offset. Returns `None` otherwise
			...
			SourceParentheses::SourceMap { next, tail, .. } => {
				while let Some(range) = next {
					...
					if range.transformed == offset {
						// A deleted range can contain multiple tokens. See if there's any `)` in
						// the deleted range and compute its source range.
	...
	/// Searches the outer most node that still is inside of the parentheses
	/// specified by the `parentheses_source_range`.
	fn outer_most_parenthesized_node<L:Language>(
		...
		match self {
			...
			SourceParentheses::SourceMap { map, .. } => {
				...
				// How this works: We search the outer most node that, in the source document
				// ends right after the `)`. The issue is, it is possible that multiple
				// nodes end right after the `)`
				//
				// ```javascript
				// !(
				//     a
				...
				//  )
				// ```
				// The issue is, that in the transformed document, the `ReferenceIdentifier`,
				// `IdentifierExpression`, `UnaryExpression`, and `ExpressionStatement`
				// all end at the end position of `)`.
				// However, not all the nodes start at the same position. That's why this code
				// also tracks the start. We first find the closest node that directly ends
				// at the position of the right paren. We then continue upwards to find the
				// most outer node that starts at the same position as that node. (In this case,
				...
				// SAFETY:
				// * The builder starts with a node which guarantees that every token has a
				//   parent node.
				// * The above `take_while` guarantees to return `true` for the parent of the
				//   token.
...
b;"#;
		...
		// A lot of code that simply removes the parenthesized expression and moves the
		// parens trivia to the identifiers leading / trailing trivia.
		let parenthesized = root.descendants().find_map(JsParenthesizedExpression::cast).unwrap();

		let reference_identifier = root.descendants().find_map(JsReferenceIdentifier::cast).unwrap();

		let mut mutation = BatchMutation::new(root);

		let identifier_expression =
			JsIdentifierExpression::cast(parenthesized.expression().unwrap().into_syntax()).unwrap();
...
/* test */"#,
	...
	impl CommentStyle for TestCommentStyle {
		...
		fn is_suppression(_:&str) -> bool { false }

		fn get_comment_kind(_:&SyntaxTriviaPieceComments<Self::Language>) -> CommentKind { CommentKind::Block }
	...
	impl TestCommentStyle {
		fn finish(self) -> Vec<DecoratedComment<JsLanguage>> { self.recorded_comments.take() }
	}
}
