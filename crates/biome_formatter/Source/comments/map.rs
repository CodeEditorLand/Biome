...
/// An optimized multi-map implementation for storing leading, dangling, and
/// trailing parts for a key.
///
/// A naive implementation using three multimaps, one to store the leading,
/// dangling, and trailing parts, requires between `keys < allocations < keys *
/// 3` vec allocations.
///
/// This map implementation optimises for the use case where:
/// * Parts belonging to the same key are inserted together. For example, all
///   parts for the key `a` are inserted before inserting any parts for the key
///   `b`.
/// * The parts per key are inserted in the following order: leading, dangling,
///   and then trailing parts.
///
/// Parts inserted in the above mentioned order are stored in a `Vec` shared by
/// all keys to reduce the number of allocations and increased cache locality.
/// The implementation falls back to storing the leading, dangling, and trailing
/// parts of a key in dedicated `Vec`s if the parts aren't inserted in the above
/// described order. However, this comes with a slight performance penalty due
/// to:
/// * Requiring up to three [Vec] allocations, one for the leading, dangling,
///   and trailing parts.
/// * Requires copying already inserted parts for that key (by cloning) into the
///   newly allocated [Vec]s.
/// * Resolving the slices for every part requires an extra level of
///   indirection.
///
/// ## Limitations
///
/// The map supports storing up to `u32::MAX - 1` parts. Inserting the
/// `u32::MAX`nth part panics.
///
/// ## Comments
///
/// Storing the leading, dangling, and trailing comments is an exemplary use
/// case for this map implementation because it is generally desired to keep the
/// comments in the same order as in the source document. This translates to
/// inserting the comments per node and for every node in leading, dangling,
/// trailing order (same order as this map optimises for).
///
/// Running Biome formatter on real world use cases showed that more than 99.99%
/// of comments get inserted in the described order.
///
/// The size limitation isn't a concern for comments because Biome supports
/// source documents with a size up to 4GB (`u32::MAX`) and every comment has at
/// least a size of 2 bytes:
/// * 1 byte for the start sequence, e.g. `#`
/// * 1 byte for the end sequence, e.g. `\n`
///
/// Meaning, the upper bound for comments parts in a document are `u32::MAX /
/// 2`.
pub(super) struct CommentsMap<K, V> {
	...
	/// Vector containing the leading, dangling, and trailing vectors for out of
	/// order entries.
	///
...
impl<K:std::hash::Hash + Eq, V> CommentsMap<K, V> {
	pub fn new() -> Self { Self { index:FxHashMap::default(), parts:Vec::new(), out_of_order:Vec::new() } }
	...
	where
		...
		match self.index.get_mut(&key) {
			...
			Some(Entry::InOrder(entry)) if entry.trailing_start.is_none() && self.parts.len() == entry.range().end => {
				self.parts.push(part);
			...
			Some(entry) => {
				let out_of_order = Self::entry_to_out_of_order(entry, &self.parts, &mut self.out_of_order);
	...
	where
		...
		match self.index.get_mut(&key) {
			...
			Some(Entry::InOrder(entry)) if entry.trailing_end.is_none() && self.parts.len() == entry.range().end => {
				self.parts.push(part);
	...
	where
		...
		match entry {
			Entry::InOrder(in_order) => {
				...
				*entry = Entry::OutOfOrder(OutOfOrderEntry { leading_index:index, _count:Count::new() });
	...
	pub fn has(&self, key:&K) -> bool { self.index.contains_key(key) }

	/// Returns an iterator over all leading, dangling, and trailing parts of
	/// `key`.
...
pub(super) enum PartsIterator<'a, V> {
	/// The slice into the [CommentsMap::parts] [Vec] if this is an in-order
	/// entry or the trailing parts of an out-of-order entry.
	Slice(std::slice::Iter<'a, V>),

	/// Iterator over the leading parts of an out-of-order entry. Returns the
	/// dangling parts, and then the trailing parts once the leading iterator
	/// is fully consumed.
	Leading { leading:std::slice::Iter<'a, V>, dangling:&'a [V], trailing:&'a [V] },

	/// Iterator over the dangling parts of an out-of-order entry. Returns the
...
impl<'a, V> Iterator for PartsIterator<'a, V> {
	...
	where
		...
		match self {
			...
			PartsIterator::Dangling { dangling, trailing } => trailing.last().or_else(|| dangling.last()),
		}
	}
}
...
#[derive(Debug)]
struct InOrderEntry {
	/// Index into the [CommentsMap::parts] vector where the leading parts of
	/// this entry start
	leading_start:PartIndex,

	/// Index into the [CommentsMap::parts] vector where the dangling parts
	/// (and, thus, the leading parts end) start.
	dangling_start:PartIndex,

	/// Index into the [CommentsMap::parts] vector where the trailing parts
	/// (and, thus, the dangling parts end) of this entry start
	trailing_start:Option<PartIndex>,

	/// Index into the [CommentsMap::parts] vector where the trailing parts of
	/// this entry end
...
impl InOrderEntry {
	...
	fn leading_range(&self) -> Range<usize> { self.leading_start.value()..self.dangling_start.value() }
	...
	fn trailing_range(&self) -> Range<usize> {
		match (self.trailing_start, self.trailing_end) {
			(Some(trailing_start), Some(trailing_end)) => trailing_start.value()..trailing_end.value(),
	...
	fn range(&self) -> Range<usize> {
		self.leading_start.value()..self.trailing_end.or(self.trailing_start).unwrap_or(self.dangling_start).value()
	}
}

#[derive(Debug)]
struct OutOfOrderEntry {
	/// Index into the [CommentsMap::out_of_order] vector at which offset the
	/// leaading vec is stored.
	leading_index:usize,
	_count:Count<OutOfOrderEntry>,
}

impl OutOfOrderEntry {
	const fn leading_index(&self) -> usize { self.leading_index }

	const fn dangling_index(&self) -> usize { self.leading_index + 1 }

	const fn trailing_index(&self) -> usize { self.leading_index + 2 }
}
...
///
/// This means, that only `u32 - 1` parts can be stored. This should be
/// sufficient for storing comments because: Comments have length of two or more
/// bytes because they consist of a start and end character sequence (`#` + new
/// line, `/*` and `*/`). Thus, a document with length `u32` can have at most
/// `u32::MAX / 2` comment-parts.
#[repr(transparent)]
#[derive(Debug, Copy, Clone, Eq, PartialEq, Hash)]
struct PartIndex(NonZeroU32);

impl PartIndex {
	fn from_len(value:usize) -> Self { Self(NonZeroU32::try_from(value as u32 + 1).unwrap()) }

	fn value(&self) -> usize { (u32::from(self.0) - 1) as usize }

	fn increment(&mut self) { *self = self.incremented(); }

	fn incremented(&self) -> PartIndex { PartIndex(NonZeroU32::new(self.0.get() + 1).unwrap()) }
}
