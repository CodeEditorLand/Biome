...
pub(super) struct CommentsMap<K, V> {
	...
	/// Vector containing the leading, dangling, and trailing vectors for out
	/// of order entries.
	///
...
impl<K:std::hash::Hash + Eq, V> CommentsMap<K, V> {
	...
	where
		...
		match self.index.get_mut(&key) {
			...
			// Has leading and dangling comments and its comments are at the end
			// of parts
			Some(Entry::InOrder(entry))
	...
	where
		...
		match entry {
			Entry::InOrder(in_order) => {
				...
				*entry =
					Entry::OutOfOrder(OutOfOrderEntry { leading_index:index, _count:Count::new() });
