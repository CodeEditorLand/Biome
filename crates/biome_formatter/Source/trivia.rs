...
use crate::{
	...
	write, Argument, Arguments, CstFormatContext, FormatRefWithRule, GroupId,
...
fn should_nestle_adjacent_doc_comments<L: Language>(
	...
	first.has_newline()
		&& second.has_newline()
		&& (second.text_range().start()).sub(first.text_range().end())
			== TextSize::from(0)
			...
			match comment.kind() {
				CommentKind::Block | CommentKind::InlineBlock => {
					match comment.lines_after() {
						0 => {
							let should_nestle = leading_comments_iter
								.peek()
	...
	Context: CstFormatContext,
{
	fn fmt(&self, f: &mut Formatter<Context>) -> FormatResult<()> {
		...
		let trailing_comments = match self {
			FormatTrailingComments::Node(node) => {
				comments.trailing_comments(node)
			},
			FormatTrailingComments::Comments(comments) => comments,
		};

		let mut total_lines_before = 0;
		let mut previous_comment: Option<&SourceComment<Context::Language>> =
			None;
		...
		for comment in trailing_comments {
			...
			let should_nestle =
				previous_comment.map_or(false, |previous_comment| {
					should_nestle_adjacent_doc_comments(
						previous_comment,
						comment,
					)
			...
			if total_lines_before > 0 {
				...
				let content = format_with(|f| {
					write!(f, [maybe_space(!should_nestle), format_comment])
				});
	...
	Context: CstFormatContext,
{
	fn fmt(&self, f: &mut Formatter<Context>) -> FormatResult<()> {
		...
		let dangling_comments = match self {
			FormatDanglingComments::Node { node, .. } => {
				comments.dangling_comments(node)
			},
		...
		let format_dangling_comments = format_with(|f| {
			let mut previous_comment: Option<
				&SourceComment<Context::Language>,
			> = None;
		...
		match self.indent() {
			...
			DanglingIndentMode::Soft => {
				write!(
					f,
					[group(&soft_block_indent(&format_dangling_comments))]
				)
			},
...
pub const fn format_trimmed_token<L: Language>(
	token: &SyntaxToken<L>,
) -> FormatTrimmedToken<L> {
	...
	C: CstFormatContext<Language = L>,
{
	fn fmt(&self, f: &mut Formatter<C>) -> FormatResult<()> {
		...
		if f.comments().has_skipped(self.token) {
			...
			write!(
				f,
				[if_group_fits_on_line(&format_skipped_token_trivia(
					self.token
				))
				.with_group_id(self.group_id)]
			)?;
...
impl<L: Language> FormatSkippedTokenTrivia<'_, L> {
		...
		Context: CstFormatContext<Language = L>,
	{
		...
		let skipped_range = skipped_range.unwrap_or_else(|| {
			TextRange::empty(self.token.text_range().start())
		});

		f.write_element(FormatElement::Tag(Tag::StartVerbatim(
			VerbatimKind::Verbatim { length: skipped_range.len() },
		)))?;
...
impl<Context> Format<Context>
	for FormatSkippedTokenTrivia<'_, Context::Language>
where
	Context: CstFormatContext,
{
	fn fmt(&self, f: &mut Formatter<Context>) -> FormatResult<()> {
