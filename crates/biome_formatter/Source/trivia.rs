					...
					match comment.lines_after() {
						0 => {
							let should_nestle = leading_comments_iter
								.peek()
								.is_some_and(|next_comment| should_nestle_adjacent_doc_comments(comment, next_comment));
	...
	fn fmt(&self, f: &mut Formatter<Context>) -> FormatResult<()> {
		...
		for comment in trailing_comments {
			...
			let should_nestle = previous_comment
				.is_some_and(|previous_comment| should_nestle_adjacent_doc_comments(previous_comment, comment));
						...
						line_suffix(&format_with(|f| {
							match comment.lines_before() {
								...
								0 => {
									...
									if previous_comment
										.is_some_and(|previous_comment| previous_comment.kind().is_line())
									{
										write!(f, [hard_line_break()])?;
									} else {
										write!(f, [space()])?;
