...
use crate::{
	comments::{is_doc_comment, CommentKind, CommentStyle},
...
/// - the two comments are immediately adjacent to each other, with no
///   characters between them.
///
/// In this case, the comments are considered "nestled" - a pattern that JSDoc
/// uses to represent overloaded types, which get merged together to create the
/// final type for the subject. The comments must be kept immediately adjacent
/// after formatting to preserve this behavior.
///
/// There isn't much documentation about this behavior, but it is mentioned on
/// the JSDoc repo for documentation: https://github.com/jsdoc/jsdoc.github.io/issues/40. Prettier also
...
pub const fn format_leading_comments<L:Language>(node:&SyntaxNode<L>) -> FormatLeadingComments<L> {
					...
					[
						line_suffix(&format_with(|f| {
							match comment.lines_before() {
								_ if should_nestle => {},
								0 => {
									// If the comment is immediately following a
									// block-like comment, then it can
									// stay on the same line with just a space
									// between. Otherwise, it gets a
									// hard break.
									//
									//   /** hello */ // hi
									//   /**
...
pub enum DanglingIndentMode {
	/// Writes every comment on its own line and indents them with a block
	/// indent.
	///
	/// # Examples
	/// ```ignore
	...
	/// Writes every comment on its own line and indents them with a soft line
	/// indent. Guarantees to write a line break if the last formatted comment
	/// is a [line](CommentKind::Line) comment.
	///
	/// # Examples
	///
	/// ```ignore
...
impl<L:Language> FormatDanglingComments<'_, L> {
	...
	pub fn with_block_indent(self) -> Self { self.with_indent_mode(DanglingIndentMode::Block) }

	/// Indents the comments with a [soft block](DanglingIndentMode::Soft)
	/// indent.
	pub fn with_soft_block_indent(self) -> Self { self.with_indent_mode(DanglingIndentMode::Soft) }
...
/// Formats the skipped token trivia of a removed token and marks the token as
/// tracked.
...
/// Formats the trivia of a token that is present in the source text but should
/// be omitted in the formatted output.
...
///
/// This will print the skipped token trivia that belong to `token` to
/// `content`; `token` is then marked as consumed by the formatter.
...
/// Formats the given token only if the group does break and otherwise retains
/// the token's skipped token trivia.
...
/// Formats a token with its skipped token trivia that only gets printed if its
/// enclosing group does break but otherwise gets omitted from the formatted
/// output.
...
impl<L:Language> FormatSkippedTokenTrivia<'_, L> {
	...
	where
		...
		// Iterate over the remaining pieces to find the full range from the
		// first to the last skipped token trivia. Extract the comments
		// between the last skipped token trivia and the token.
				...
				skipped_range = Some(match skipped_range {
					...
					None => {
						if dangling_comments.is_empty() {
							match lines {
								0 if spaces == 0 => {
									// Token had no space to previous token nor
									// any preceding comment. Keep it that way
								},
								0 => write!(f, [space()])?,
		...
		if dangling_comments.is_empty() {
			match lines {
				0 if spaces == 0 => {
					// Don't write a space if there was non in the source
					// document
					Ok(())
				},
				0 => write!(f, [space()]),
