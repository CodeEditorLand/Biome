...
use crate::{
	...
	SourceComment,
	TextRange,
	comments::{CommentKind, CommentStyle, is_doc_comment},
...
impl<L:Language> FormatSkippedTokenTrivia<'_, L> {
	...
	where
		...
		// Iterate over the remaining pieces to find the full range from the first to
		// the last skipped token trivia. Extract the comments between the last
		// skipped token trivia and the token.
