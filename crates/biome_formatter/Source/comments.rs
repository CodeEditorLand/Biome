...
use crate::{
	CstFormatContext, FormatResult, FormatRule, TextSize, TransformSourceMap,
};
use biome_rowan::syntax::SyntaxElementKey;
use biome_rowan::{
	Language, SyntaxNode, SyntaxToken, SyntaxTriviaPieceComments,
};
	...
	pub(crate) fn assert_formatted_all_comments(&self) {
		...
		if has_unformatted_comments {
			...
			for node in self
				...
				.descendants()
			{
				unformatted_comments.extend(
					self.leading_comments(&node).iter().filter_map(|comment| {
						(!comment.formatted.get()).then_some(
							DebugComment::Leading {
								node: node.clone(),
								comment,
							},
						)
				...
				unformatted_comments.extend(
					self.dangling_comments(&node).iter().filter_map(
						|comment| {
							(!comment.formatted.get()).then_some(
								DebugComment::Dangling {
									node: node.clone(),
									comment,
								},
							)
				...
				unformatted_comments.extend(
					self.trailing_comments(&node).iter().filter_map(
						|comment| {
							(!comment.formatted.get()).then_some(
								DebugComment::Trailing {
									node: node.clone(),
									comment,
								},
							)
...
/// "#)));
/// ```
pub fn is_alignable_comment<L: Language>(
	comment: &SyntaxTriviaPieceComments<L>,
) -> bool {
...
/// "#)));
/// ```
pub fn is_doc_comment<L: Language>(
	comment: &SyntaxTriviaPieceComments<L>,
) -> bool {
	if !comment.has_newline() {
		return false;
