...
pub struct Comments<L:Language> {
	...
	/// lifetimes is necessary to support the use case where a (formattable
	/// object)[crate::Format] iterates over all comments, and writes them into
	/// the [crate::Formatter] (mutably borrowing the [crate::Formatter] and in
	/// turn its context).
	///
	/// ```block
...
struct CommentsData<L:Language> {
	...
	/// a child node without previously checking if the child has a suppression
	/// comment.
	///
	/// The implementation refrains from snapshotting the checked nodes because
	/// a node gets formatted as verbatim if its formatting fails which has the
	/// same result as formatting it as suppressed node (thus, guarantees that
	/// the formatting isn't changed).
	#[cfg(debug_assertions)]
