...
//!
//! Most programming languages support comments allowing programmers to document
//! their programs. Comments are different from other syntaxes  because
//! programming languages allow comments in almost any position, giving
//! programmers great flexibility on where they can write comments:
//!
//! ```ignore
//! /**
...
//! * The formatter must consistently place comments so that re-formatting the
//!   output yields the same result and does not create invalid syntax (line
//!   comments).
//! * It is essential that formatters place comments close to the syntax the
//!   programmer intended to document. However, the lack of rules regarding
//!   where comments are allowed and what syntax they document requires the use
//!   of heuristics to infer the documented syntax.
//!
//! This module strikes a balance between placing comments as closely as
//! possible to their source location and reducing the complexity of formatting
//! comments. It does so by associating comments per node rather than a token.
//! This greatly reduces the combinations of possible comment positions but
//! turns out to be, in practice, sufficiently precise to keep comments close to
//! their source location.
//!
//! ## Node comments
//!
//! Comments are associated per node but get further distinguished on their
//! location related to that node:
//!
//! ### Leading Comments
//!
...
//!
//! ## Limitations
//! Limiting the placement of comments to leading, dangling, or trailing node
//! comments reduces complexity inside the formatter but means, that the
//! formatter's possibility of where comments can be formatted depends on the
//! AST structure.
//!
...
//! ```
//!
//! Because all children of the `continue` statement are tokens, it is only
//! possible to make the comments leading, dangling, or trailing comments of the
//! `continue` statement. But this results in a loss of information as the
//! formatting code can no longer distinguish if a comment appeared before or
//! after the label and, thus, has to format them the same way.
//!
//! This hasn't shown to be a significant limitation today but the
//! infrastructure could be extended to support a `label` on [`SourceComment`]
//! that allows to further categorise comments.
...
pub enum CommentKind {
	/// An inline comment that can appear between any two tokens and doesn't
	...
	/// A block comment that can appear between any two tokens and contains at
...
impl CommentKind {
	pub const fn is_line(&self) -> bool { matches!(self, CommentKind::Line) }

	pub const fn is_block(&self) -> bool { matches!(self, CommentKind::Block) }

	pub const fn is_inline_block(&self) -> bool { matches!(self, CommentKind::InlineBlock) }

	/// Returns `true` for comments that can appear inline between any two
	/// tokens.
	///
	/// ## Examples
	///
...
impl<L:Language> SourceComment<L> {
	...
	pub fn piece(&self) -> &SyntaxTriviaPieceComments<L> { &self.piece }

	/// The number of lines between this comment and the **previous** token or
	/// comment.
	///
	/// # Examples
	///
	...
	/// ```
	///
	/// Returns `0` because there's no line break between the token `a` and the
	/// comment.
	///
	/// ## Own Line
	///
	...
	/// ```
	///
	/// Returns `2` because there are two line breaks between the token `a` and
	/// the comment.
	pub fn lines_before(&self) -> u32 { self.lines_before }
	...
	/// ```
	///
	/// Returns `2` because there are two line breaks between the comment and
	/// the token `b`.
	///
	/// ## Same line
	///
	...
	/// ```
	///
	/// Returns `0` because there are no line breaks between the comment and the
	/// token `b`.
	pub fn lines_after(&self) -> u32 { self.lines_after }

	/// The kind of the comment
	pub fn kind(&self) -> CommentKind { self.kind }
	...
	#[cfg(debug_assertions)]
	pub fn mark_formatted(&self) { self.formatted.set(true) }
}

/// A comment decorated with additional information about its surrounding
/// context in the source document.
///
/// Used by [CommentStyle::place_comment] to determine if this should become a
/// [leading](self#leading-comments), [dangling](self#dangling-comments), or
/// [trailing](self#trailing-comments) comment.
#[derive(Debug, Clone)]
...
impl<L:Language> DecoratedComment<L> {
	...
	///
	/// A node encloses a comment when the comment is between two of its direct
	/// children (ignoring lists).
	///
	/// # Examples
	///
	/// ```ignore
	...
	/// ```
	///
	/// The enclosing node is the array expression and not the identifier `b`
	/// because `a` and `b` are children of the array expression and `comment`
	/// is a comment between the two nodes.
	pub fn enclosing_node(&self) -> &SyntaxNode<L> { &self.enclosing }

	/// Returns the comment piece.
	pub fn piece(&self) -> &SyntaxTriviaPieceComments<L> { &self.comment }

	/// Returns the node preceding the comment.
	///
	/// The direct child node (ignoring lists) of the
	/// [`enclosing_node`](DecoratedComment::enclosing_node) that precedes this
	/// comment.
	///
	/// Returns [None] if the
	/// [`enclosing_node`](DecoratedComment::enclosing_node) only consists of
	/// tokens or if all preceding children of the
	/// [`enclosing_node`](DecoratedComment::enclosing_node) have been tokens.
	///
	/// The Preceding node is guaranteed to be a sibling of
	...
	/// [/* comment */]
	/// ```
	/// Returns [None] because the comment has no preceding node, only a
	/// preceding `[` token.
	///
	/// ## Preceding node
	///
	...
	/// ```
	///
	///  Returns `Some(a)` because `a` is the preceding node of `comment`. The
	/// presence of the `,` token doesn't change that.
	pub fn preceding_node(&self) -> Option<&SyntaxNode<L>> { self.preceding.as_ref() }

	/// Takes the [`preceding_node`](DecoratedComment::preceding_node) and
	/// replaces it with [None].
	fn take_preceding_node(&mut self) -> Option<SyntaxNode<L>> { self.preceding.take() }

	/// Returns the node following the comment.
	///
	/// The direct child node (ignoring lists) of the
	/// [`enclosing_node`](DecoratedComment::enclosing_node) that follows this
	/// comment.
	///
	/// Returns [None] if the
	/// [`enclosing_node`](DecoratedComment::enclosing_node) only consists of
	/// tokens or if all children children of the
	/// [`enclosing_node`](DecoratedComment::enclosing_node) following this
	/// comment are tokens.
	///
	/// The following node is guaranteed to be a sibling of
	...
	/// ```
	///
	/// Returns [None] because there's no node following the comment, only the
	/// `]` token.
	///
	/// ## Following node
	///
	...
	/// ```
	///
	/// Returns `Some(a)` because `a` is the node directly following the
	/// comment.
	///
	...
	/// ```
	///
	/// Returns `Some(test)` because the `test` identifier is the first node
	/// following `comment`.
	///
	...
	/// ```
	///
	/// Returns `None` because `comment` is enclosed inside the parenthesized
	/// expression and it has no children following `/* comment */.
	pub fn following_node(&self) -> Option<&SyntaxNode<L>> { self.following.as_ref() }

	/// Takes the [`following_node`](DecoratedComment::following_node) and
	/// replaces it with [None].
	fn take_following_node(&mut self) -> Option<SyntaxNode<L>> { self.following.take() }

	/// The number of line breaks between this comment and the **previous**
	/// token or comment.
	///
	/// # Examples
	///
	...
	pub fn text_position(&self) -> CommentTextPosition { self.text_position }

	/// The next token that comes after this comment. It is possible that other
	/// comments are between this comment and the token.
	///
	/// ```ignore
	/// a /* comment */ /* other */ b
	/// ```
	///
	/// The `following_token` for both comments is `b` because it's the token
	/// coming after the comments.
	pub fn following_token(&self) -> Option<&SyntaxToken<L>> { self.following_token.as_ref() }
}
...
pub enum CommentTextPosition {
	/// A comment that is on the same line as the preceding token and is
	/// separated by at least one line break from the following token.
	///
	/// # Examples
	///
	/// ## End of line
	...
	/// ```
	///
	/// Both `/* this */` and `// or this` are end of line comments because both
	/// comments are separated by at least one line break from the following
	/// token `b`.
	///
	/// ## Own line
	///
	...
	/// ```
	///
	/// This is not an end of line comment because it isn't on the same line as
	/// the preceding token `a`.
	EndOfLine,
	...
	/// ```
	///
	/// Both comments are own line comments because they are separated by one
	/// line break from the preceding token `a`.
	OwnLine,

	/// A comment that is placed on the same line as the preceding and following
	/// token.
	///
	/// # Examples
	///
...
impl CommentTextPosition {
	pub const fn is_same_line(&self) -> bool { matches!(self, CommentTextPosition::SameLine) }

	pub const fn is_own_line(&self) -> bool { matches!(self, CommentTextPosition::OwnLine) }

	pub const fn is_end_of_line(&self) -> bool { matches!(self, CommentTextPosition::EndOfLine) }
}

#[derive(Debug)]
pub enum CommentPlacement<L:Language> {
	/// Makes `comment` a [leading comment](self#leading-comments) of `node`.
	Leading { node:SyntaxNode<L>, comment:SourceComment<L> },
	/// Makes `comment` a [trailing comment](self#trailing-comments) of `node`.
	Trailing { node:SyntaxNode<L>, comment:SourceComment<L> },

	/// Makes `comment` a [dangling comment](self#dangling-comments) of `node`.
	Dangling { node:SyntaxNode<L>, comment:SourceComment<L> },
	...
	/// Makes the comment a...
	///
	/// * [trailing comment] of the [`preceding_node`] if both the
	///   [`following_node`] and [`preceding_node`] are not [None] and the
	///   comment and [`preceding_node`] are only separated by a space (there's
	///   no token between the comment and [`preceding_node`]).
	/// * [leading comment] of the [`following_node`] if the [`following_node`]
	///   is not [None]
	/// * [trailing comment] of the [`preceding_node`] if the [`preceding_node`]
	///   is not [None]
	...
	/// ```
	///
	/// The comment becomes a [dangling comment] of the enclosing array
	/// expression because both the [`preceding_node`] and [`following_node`]
	/// are [None].
	///
	/// # Own line comments
	///
	...
	/// * [leading comment] of the [`following_node`] if the [`following_node`]
	///   is not [None]
	/// * or a [trailing comment] of the [`preceding_node`] if the
	...
	/// ```
	///
	/// The comment becomes a [dangling comment] of the array expression because
	/// both [`preceding_node`] and [`following_node`] are [None].
	///
	///
	...
	/// * [trailing comment] of the [`preceding_node`] if the [`preceding_node`]
	///   is not [None]
	/// * or a [leading comment] of the [`following_node`] if the
	...
	/// ```
	///
	/// The comment becomes a [trailing comment] of the node `a` because there's
	/// no token between the node `a` and the `comment`.
	///
	/// ```ignore
	/// [a, /* comment */ b]
	/// ```
	///
	/// The comment becomes a [leading comment] of the node `b` because the node
	/// `a` and the comment are separated by a `,` token.
	///
	/// ### Comment with preceding node only
	///
	/// ```ignore
	...
	/// ```
	///
	/// The comment becomes a [trailing comment] of the node `a` because the
	/// [`following_node`] is [None].
	///
	/// ### Comment with following node only
	///
	/// ```ignore
	...
	/// ```
	///
	/// The comment becomes a [leading comment] of the node `b` because the
...
impl<L:Language> CommentPlacement<L> {
	...
	/// Returns the placement if it isn't [CommentPlacement::Default], otherwise
...
pub trait CommentStyle: Default {
	...
	/// Returns `true` if a comment with the given `text` is a `biome-ignore
	/// format:` suppression comment.
	fn is_suppression(_text:&str) -> bool { false }
...
pub struct Comments<L:Language> {
	/// The use of a [Rc] is necessary to achieve that [Comments] has a
	/// lifetime that is independent from the [crate::Formatter].
	/// Having independent lifetimes is necessary to support the use case where
	/// a (formattable object)[crate::Format] iterates over all comments, and
	/// writes them into the [crate::Formatter] (mutably borrowing the
	/// [crate::Formatter] and in turn its context).
	///
	/// ```block
	...
	/// ```
	///
	/// Using an `Rc` here allows to cheaply clone [Comments] for these use
	/// cases.
...
impl<L:Language> Comments<L> {
	...
	/// Returns `true` if the given `node` has any
	/// [leading](self#leading-comments) or [trailing](self#trailing-comments)
	/// comments.
	#[inline]
	pub fn has_comments(&self, node:&SyntaxNode<L>) -> bool { self.data.comments.has(&node.key()) }
	...
	/// Returns `true` if the node has any [trailing](self#trailing-comments)
	/// [line](CommentKind::Line) comment.
	...
	/// Returns an iterator over the [leading](self#leading-comments),
	/// [dangling](self#dangling-comments), and [trailing](self#trailing)
	/// comments of `node`.
	...
	/// [trailing](self#trailing-comments) suppression comment.
	///
	/// # Examples
	///
	/// ```javascript
	...
	/// ```
	///
	/// Returns `true` for the expression statement but `false` for the call
	/// expression because the call expression is nested inside of the
	...
	/// Marks that it isn't necessary for the given node to check if it has been
	/// suppressed or not.
	#[cfg(debug_assertions)]
	...
	/// Verifies that [NodeSuppressions::is_suppressed] has been called for
	/// every node of `root`. This is a no-op in builds that have the feature
	/// `debug_assertions` disabled.
	///
	/// # Panics
	/// If theres any node for which the formatting didn't very if it has a
	/// suppression comment.
	#[cfg(debug_assertions)]
...
struct CommentsData<L:Language> {
	...
	/// This index of nodes that have been checked if they have a suppression
	/// comments is used to detect format implementations that manually format
	/// a child node without previously checking if the child has a
	/// suppression comment.
	///
	/// The implementation refrains from snapshotting the checked nodes because
	/// a node gets formatted as verbatim if its formatting fails which has
	/// the same result as formatting it as suppressed node (thus, guarantees
	/// that the formatting isn't changed).
	#[cfg(debug_assertions)]
...
enum DebugComment<'a, L:Language> {
	Leading { comment:&'a SourceComment<L>, node:SyntaxNode<L> },
	Trailing { comment:&'a SourceComment<L>, node:SyntaxNode<L> },
	Dangling { comment:&'a SourceComment<L>, node:SyntaxNode<L> },
}
	...
	fn fmt(&self, f:&mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		match self {
			...
			DebugComment::Dangling { node, comment } => {
				f.debug_struct("Dangling")
					.field("node", node)
					.field("comment", comment)
					.finish()
			},
			DebugComment::Trailing { node, comment } => {
				f.debug_struct("Trailing")
					.field("node", node)
					.field("comment", comment)
					.finish()
			},
...
impl<C> Default for FormatPlainComment<C> {
	fn default() -> Self { FormatPlainComment { context:PhantomData } }
}
