				...
				let element_expands = match element {
					...
					FormatElement::Interned(interned) => {
						match checked_interned.get(interned) {
							...
							None => {
								let interned_expands = propagate_expands(interned, enclosing, checked_interned);
					...
					FormatElement::ExpandParent | FormatElement::Line(LineMode::Hard | LineMode::Empty) => true,
					_ => false,
				};
		...
		while let Some(element) = iter.next() {
			...
			match element {
				...
				FormatElement::BestFitting(best_fitting) => {
					...
					f.write_elements([FormatElement::Tag(StartIndent), FormatElement::Line(LineMode::Hard)])?;

					for variant in best_fitting.variants() {
						write!(f, [variant.deref(), hard_line_break()])?;
					}

					f.write_elements([FormatElement::Tag(EndIndent), FormatElement::Line(LineMode::Hard)])?;
				...
				FormatElement::Interned(interned) => {
					...
					match interned_elements.get(interned).copied() {
						...
						Some(reference) => {
							write!(
								f,
								[dynamic_text(&std::format!("<ref interned *{reference}>"), TextSize::default())]
							)?;
						},
					}
				},
				...
				FormatElement::Tag(tag) => {
					...
					// Handle documents with mismatching start/end or superfluous end tags
						...
						match tag_stack.pop() {
							None => {
								// Only write the end tag without any indent to ensure the output
								// document is valid.
								write!(
									f,
									[
										text("<END_TAG_WITHOUT_START<"),
										dynamic_text(&std::format!("{:?}", tag.kind()), TextSize::default()),
										text(">>"),
									]
								)?;
							...
							Some(start_kind) if start_kind != tag.kind() => {
								write!(
									...
									[
										...
										dynamic_text(&std::format!("{start_kind:?}"), TextSize::default()),
										text(", end: "),
										dynamic_text(&std::format!("{:?}", tag.kind()), TextSize::default()),
										text(">>")
									]
						...
						StartGroup(group) => {
							...
							if let Some(group_id) = group.id() {
								write!(
									f,
									[
										dynamic_text(&std::format!("\"{group_id:?}\""), TextSize::default()),
										text(","),
										space(),
									]
								)?;
		...
		while let Some(top) = tag_stack.pop() {
			write!(
				...
				[
					...
					text(")"),
					soft_line_break_or_space(),
					dynamic_text(&std::format!("<START_WITHOUT_END<{top:?}>>"), TextSize::default()),
				]
			)?;
		}
...
impl FormatElements for [FormatElement] {
	...
	fn has_label(&self, expected:LabelId) -> bool { self.first().map_or(false, |element| element.has_label(expected)) }
	...
	fn start_tag(&self, kind:TagKind) -> Option<&Tag> {
		...
		fn traverse_slice<'a>(slice:&'a [FormatElement], kind:TagKind, depth:&mut usize) -> Option<&'a Tag> {
	...
	fn end_tag(&self, kind:TagKind) -> Option<&Tag> { self.last().and_then(|element| element.end_tag(kind)) }
}
...
mod tests {
	...
	use crate::{SimpleFormatContext, format, format_args, prelude::*, write};
  ...
  group(expand: propagated, [<ref interned *0>])
	...
	fn escapes_quotes() {
		...
		let token_text =
			FormatElement::LocatedTokenText { source_position:TextSize::default(), slice:token.token_text() };

		let mut document = Document::from(vec![
			FormatElement::DynamicText { text:"\"foo\"".into(), source_position:TextSize::default() },
			token_text,
		]);
