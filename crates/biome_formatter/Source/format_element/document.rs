...
impl Document {
	/// Sets [`expand`](tag::Group::expand) to [`GroupMode::Propagated`] if the
	/// group contains any of:
	/// * a group with [`expand`](tag::Group::expand) set to
	///   [GroupMode::Propagated] or [GroupMode::Expand].
	/// * a non-soft [line break](FormatElement::Line) with mode
	///   [LineMode::Hard], [LineMode::Empty], or [LineMode::Literal].
	/// * a [FormatElement::ExpandParent]
	///
	/// [`BestFitting`] elements act as expand boundaries, meaning that the fact
	/// that a [`BestFitting`]'s content expands is not propagated past the
	/// [`BestFitting`] element.
	///
			...
			for element in elements {
				let element_expands = match element {
					...
					FormatElement::Interned(interned) => {
						match checked_interned.get(interned) {
					...
					FormatElement::BestFitting(best_fitting) => {
						...
						enclosing.pop();
						// BestFitting acts as a boundary, meaning there is no
						// need to continue processing this element and we
						// can move onto the next. However, we _don't_ set
						// `expands = false`, because that ends up negating the
						// expansion when processing `Interned` elements.
						//
						// Only interned lists are affected, because they cache
						// the expansion value based on the value of
						// `expands` at the end of iterating the children. If
						// a `best_fitting` element occurs after the last
						// expanding element, and we end up setting
						// `expands = false` here, then the interned element
						// ends up thinking that its content doesn't
						// expand, even though it might. Example:
						//   group(1,
						//     interned 1 [
						//       expand_parent,
						//       best_fitting,
						...
						//     [ref interned 1]
						//   )
						// Here, `group(1)` gets expanded directly by the
						// `expand_parent` element. This happens
						// immediately, and then `expands = true` is set. The
						// interned element continues processing, and
						// encounters the `best_fitting`. If
						// we set `expands = false` there, then the interned
						// element's result ends up being `false`, even
						// though it does actually expand. Then, when
						// `group(2)` checks for expansion, it looks at the ref
						// to `interned 1`, which thinks it doesn't
						// expand, and so `group(2)` stays flat.
						//
						// By _not_ setting `expands = false`, even though
						// `best_fitting` is a boundary for expansion, we
						// ensure that any references to the interned
						// element will get the correct value for whether the
						// contained content actually expands, regardless
						// of the order of elements within it.
						//
						// Instead, just returning false here enforces that
						// `best_fitting` doesn't think it expands
						// _itself_, but allows other sibling elements to still
...
impl From<Vec<FormatElement>> for Document {
	fn from(elements:Vec<FormatElement>) -> Self { Self { elements } }
}

impl Deref for Document {
	type Target = [FormatElement];

	fn deref(&self) -> &Self::Target { self.elements.as_slice() }
}
...
impl FormatContext for IrFormatContext {
	...
	fn options(&self) -> &Self::Options { &IrFormatOptions }
...
impl FormatOptions for IrFormatOptions {
	fn indent_style(&self) -> IndentStyle { IndentStyle::Space }

	fn indent_width(&self) -> IndentWidth { IndentWidth::default() }

	fn line_width(&self) -> LineWidth { LineWidth::default() }

	fn line_ending(&self) -> LineEnding { LineEnding::Lf }
		...
		while let Some(element) = iter.next() {
			...
			match element {
					...
					match element {
						...
						element if element.is_text() => {
							...
							let new_element = match element {
								// except for static text because
								// source_position is unknown
				...
				FormatElement::Tag(tag) => {
					...
					// Handle documents with mismatching start/end or
					// superfluous end tags
					else {
						match tag_stack.pop() {
							None => {
								// Only write the end tag without any indent to
								// ensure the output document is valid.
								write!(
									f,
									[
...
impl FormatElements for [FormatElement] {
	...
	fn has_label(&self, expected:LabelId) -> bool {
		self.first().map_or(false, |element| element.has_label(expected))
	}
...
mod tests {
	...
	use crate::{format, format_args, prelude::*, write, SimpleFormatContext};
