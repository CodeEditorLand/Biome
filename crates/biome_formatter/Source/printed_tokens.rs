...
/// Tracks the ranges of the formatted (including replaced or tokens formatted
/// as verbatim) tokens.
///
/// This implementation uses the fact that no two tokens can have an overlapping
/// range to avoid the need for an interval tree. Thus, testing if a token has
/// already been formatted only requires testing if a token starting at the same
/// offset has been formatted.
...
impl PrintedTokens {
	...
	pub(crate) fn set_disabled(&mut self, disabled:bool) { self.disabled = disabled; }

	pub(crate) fn is_disabled(&self) -> bool { self.disabled }

	pub(crate) fn snapshot(&self) -> PrintedTokensSnapshot {
		PrintedTokensSnapshot { len:self.offsets.len(), disabled:self.disabled }
	}
	...
	pub fn assert_all_tracked<L:Language>(&self, root:&SyntaxNode<L>) {
			...
			if !offsets.shift_remove(&token.text_trimmed_range().start()) {
				panic!(
					"token has not been seen by the formatter: {token:#?}.\nUse `format_replaced` \
					 if you want to replace a token from the formatted output.\nUse \
					 `format_removed` if you want to remove a token from the formatted \
					 output.\nparent: {:#?}",
					token.parent()
				)
			}
		...
		if let Some(offset) = offsets.into_iter().next() {
			panic!(
				"tracked offset {offset:?} doesn't match any token of {root:#?}. Have you passed \
				 a token from another tree?"
			);
		}
	}
}
