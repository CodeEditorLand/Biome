...
pub trait Buffer {
	...
	fn write_fmt(
		mut self: &mut Self,
		arguments: Arguments<Self::Context>,
	) -> FormatResult<()> {
...
fn clean_interned(
	...
	match interned_cache.get(interned) {
		...
		None => {
			...
			let result =
				interned.iter().enumerate().find_map(|(index, element)| {
					match element {
						FormatElement::Line(
						...
						FormatElement::Interned(inner) => {
							...
							if &cleaned_inner != inner {
								let mut cleaned =
									Vec::with_capacity(interned.len());
								cleaned.extend_from_slice(&interned[..index]);
								cleaned.push(FormatElement::Interned(
									cleaned_inner,
								));
						...
						match element {
							...
							_ if condition_content_stack
								...
								.is_some_and(|condition| {
									condition.mode == PrintMode::Expanded
								}) =>
							{
								continue
							},
							...
							FormatElement::Interned(interned) => cleaned.push(
								FormatElement::Interned(clean_interned(
									interned,
									interned_cache,
									condition_content_stack,
								)),
							...
							FormatElement::BestFitting(best_fitting) => {
								...
								most_flat.iter().rev().for_each(|element| {
		...
		while let Some(element) = element_statck.pop() {
			match element {
				...
				FormatElement::Interned(interned) => {
					...
					self.inner
						.write_element(FormatElement::Interned(cleaned))?
				},
				// Since this buffer aims to simulate infinite print width, we don't need to retain the best fitting.
				// Just extract the flattest variant and then handle elements within it.
				FormatElement::BestFitting(best_fitting) => {
					let most_flat = best_fitting.most_flat();
					most_flat.iter().rev().for_each(|element| {
						element_statck.push(element.clone())
					});
				},
