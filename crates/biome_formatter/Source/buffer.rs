...
fn clean_interned(
	...
	match interned_cache.get(interned) {
		...
		None => {
			// Find the first soft line break element, interned element, or conditional
			// expanded content that must be changed.
			...
			let result = match result {
				// Copy the whole interned buffer so that becomes possible to change the necessary
				// elements.
				Some((mut cleaned, rest)) => {
					...
					while let Some(element) = element_stack.pop() {
						match element {
							...
							// Since this buffer aims to simulate infinite print width, we don't
							// need to retain the best fitting. Just extract the flattest
							// variant and then handle elements within it.
	...
	fn write_element(&mut self, element:FormatElement) -> FormatResult<()> {
		...
		while let Some(element) = element_statck.pop() {
			match element {
				...
				// Since this buffer aims to simulate infinite print width, we don't need to retain
				// the best fitting. Just extract the flattest variant and then handle elements
				// within it.
				FormatElement::BestFitting(best_fitting) => {
					let most_flat = best_fitting.most_flat();
