...
use crate::{
	...
	prelude::{tag::Condition, LineMode, PrintMode, Tag},
	Format,
	FormatResult,
	FormatState,
};

/// A trait for writing or formatting into [FormatElement]-accepting buffers or
/// streams.
pub trait Buffer {
	/// The context used during formatting
	type Context;

	/// Writes a [crate::FormatElement] into this buffer, returning whether the
	/// write succeeded.
	///
	/// # Errors
	...
	///
	/// Prefer using [BufferExtensions::start_recording] over accessing
	/// [Buffer::elements] directly.
	#[doc(hidden)]
	...
	/// Returns the mutable formatting state relevant for this formatting
	/// session.
...
///
/// Used in cases where the formatting of an object fails but a parent formatter
/// knows an alternative strategy on how to format the object that might
/// succeed.
#[derive(Debug)]
pub enum BufferSnapshot {
	/// Stores an absolute position of a buffers state, for example, the offset
	/// of the last written element.
	Position(usize),

	/// Generic structure for custom buffers that need to store more complex
	/// data. Slightly more expensive because it requires allocating the buffer
	/// state on the heap.
	Any(Box<dyn Any>),
}
...
impl BufferSnapshot {
	...
	pub fn unwrap_position(&self) -> usize {
		match self {
			...
			BufferSnapshot::Any(_) => {
				panic!("Tried to unwrap Any snapshot as a position.")
			},
		}
	}
...
/// Implements the `[Buffer]` trait for all mutable references of objects
/// implementing [Buffer].
impl<W:Buffer<Context = Context> + ?Sized, Context> Buffer for &mut W {
	...
	fn state_mut(&mut self) -> &mut FormatState<Self::Context> { (**self).state_mut() }

	fn snapshot(&self) -> BufferSnapshot { (**self).snapshot() }

	fn restore_snapshot(&mut self, snapshot:BufferSnapshot) { (**self).restore_snapshot(snapshot) }
}
...
impl<'a, Context> VecBuffer<'a, Context> {
	pub fn new(state:&'a mut FormatState<Context>) -> Self { Self::new_with_vec(state, Vec::new()) }
	...
	/// Consumes the buffer and returns the written [`FormatElement]`s as a
	/// vector.
	pub fn into_vec(self) -> Vec<FormatElement> { self.elements }

	/// Takes the elements without consuming self
	pub fn take_vec(&mut self) -> Vec<FormatElement> { std::mem::take(&mut self.elements) }
}
...
impl<Context> Buffer for VecBuffer<'_, Context> {
	...
	fn elements(&self) -> &[FormatElement] { self }

	fn state(&self) -> &FormatState<Self::Context> { self.state }

	fn state_mut(&mut self) -> &mut FormatState<Self::Context> { self.state }

	fn snapshot(&self) -> BufferSnapshot { BufferSnapshot::position(self.elements.len()) }
...
/// This struct wraps an existing buffer and emits a preamble text when the
/// first text is written.
///
/// This can be useful if you, for example, want to write some content if what
/// gets written next isn't empty.
///
/// # Examples
///
/// ```
...
pub struct PreambleBuffer<'buf, Preamble, Context> {
	...
	/// The pre-amble to write once the first content gets written to this
	/// buffer.
	preamble:Preamble,

	/// Whether some content (including the pre-amble) has been written at this
	/// point.
	empty:bool,
}
...
impl<'buf, Preamble, Context> PreambleBuffer<'buf, Preamble, Context> {
	...
	pub fn did_write_preamble(&self) -> bool { !self.empty }
}
	...
	Preamble: Format<Context>,
{
	...
	fn elements(&self) -> &[FormatElement] { self.inner.elements() }

	fn state(&self) -> &FormatState<Self::Context> { self.inner.state() }

	fn state_mut(&mut self) -> &mut FormatState<Self::Context> { self.inner.state_mut() }
...
/// Buffer that allows you inspecting elements as they get written to the
/// formatter.
...
///
/// * Removes [`lines`](FormatElement::Line) with the mode
///   [`Soft`](LineMode::Soft).
/// * Replaces [`lines`](FormatElement::Line) with the mode
///   [`Soft`](LineMode::SoftOrSpace) with a [`Space`](FormatElement::Space)
///
/// # Examples
///
/// ```
...
pub struct RemoveSoftLinesBuffer<'a, Context> {
	...
	/// Caches the interned elements after the soft line breaks have been
	/// removed.
	///
	/// The `key` is the [Interned] element as it has been passed to
	/// [Self::write_element] or the child of another [Interned] element. The
	/// `value` is the matching document of the key where all soft line breaks
	/// have been removed.
	///
	/// It's fine to not snapshot the cache. The worst that can happen is that
	/// it holds on interned elements that are now unused. But there's little
	/// harm in that and the cache is cleaned when dropping the buffer.
	interned_cache:FxHashMap<Interned, Interned>,

	/// Store the conditional content stack to help determine if the current
	/// element is within expanded conditional content.
	conditional_content_stack:Vec<Condition>,
}

impl<'a, Context> RemoveSoftLinesBuffer<'a, Context> {
	/// Creates a new buffer that removes the soft line breaks before writing
	/// them into `buffer`.
...
fn clean_interned(
	...
	match interned_cache.get(interned) {
		...
		None => {
			// Find the first soft line break element, interned element, or
			// conditional expanded content that must be changed.
			...
			let result = match result {
				// Copy the whole interned buffer so that becomes possible to
				...
				Some((mut cleaned, rest)) => {
					...
					while let Some(element) = element_stack.pop() {
						match element {
							...
							// All content within an expanded conditional gets
							// dropped. If there's a matching flat
							// variant, that will still get kept.
							...
							// Since this buffer aims to simulate infinite print
							// width, we don't need to retain the best fitting.
							// Just extract the flattest variant and then handle
							// elements within it.
	...
	fn write_element(&mut self, element:FormatElement) -> FormatResult<()> {
		...
		while let Some(element) = element_statck.pop() {
			match element {
				...
				// All content within an expanded conditional gets dropped. If
				// there's a matching flat variant, that will still get
				// kept.
				...
				// Since this buffer aims to simulate infinite print width, we
				// don't need to retain the best fitting. Just extract the
				// flattest variant and then handle elements within it.
				FormatElement::BestFitting(best_fitting) => {
					let most_flat = best_fitting.most_flat();
...
pub trait BufferExtensions: Buffer + Sized {
	/// Returns a new buffer that calls the passed inspector for every element
	/// that gets written to the output
	#[must_use]
	...
	/// Starts a recording that gives you access to all elements that have been
	/// written between the start and end of the recording
	///
	/// #Examples
	///
	/// ```
	...
	/// ```
	#[must_use]
	fn start_recording(&mut self) -> Recording<Self> { Recording::new(self) }
...
where
	B: Buffer,
{
	fn new(buffer:&'buf mut B) -> Self { Self { start:buffer.elements().len(), buffer } }
