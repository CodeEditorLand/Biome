...
use crate::prelude::*;
use crate::{
	Arguments, Buffer, Comments, CstFormatContext, FormatContext, FormatState,
	FormatStateSnapshot, GroupId, VecBuffer,
};
...
impl<'buf, Context> Formatter<'buf, Context> {
	...
	pub fn new(
		buffer: &'buf mut (dyn Buffer<Context = Context> + 'buf),
	) -> Self {
		Self { buffer }
	}
	...
	pub fn join_nodes_with_space<'a>(
		&'a mut self,
	) -> JoinNodesBuilder<'a, 'buf, Space, Context> {
	...
	pub fn intern_vec(
		&mut self,
		mut elements: Vec<FormatElement>,
