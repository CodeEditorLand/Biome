...
/// Handles the formatting of a CST and stores the context how the CST should be
/// formatted (user preferences). The formatter is passed to the [Format]
...
impl<'buf, Context> Formatter<'buf, Context> {
	...
	/// )?;
	///
	/// assert_eq!("1, 2, 3, 4", formatted.print()?.as_code());
	/// # Ok(())
	/// # }
	/// ```
	...
	/// Specialized version of [crate::Formatter::join_with] for joining
	/// SyntaxNodes separated by a space, soft line break or empty line
	/// depending on the input file.
	///
	/// This function inspects the input source and separates consecutive
	...
	/// Specialized version of [crate::Formatter::join_with] for joining
	/// SyntaxNodes separated by one or more line breaks depending on the input
	/// file.
	///
	/// This function inspects the input source and separates consecutive
	/// elements with either a [crate::builders::hard_line_break] or
	/// [crate::builders::empty_line] depending on how many line breaks were
	/// separating the elements in the original file.
	...
	/// Specialized version of [crate::Formatter::join_with] for joining
	/// SyntaxNodes separated by a simple space.
	///
	/// This function *disregards* the input source and always separates
	/// consecutive elements with a plain [crate::builders::space], forcing a
	/// flat layout regardless of any line breaks or spaces were separating the
	/// elements in the original file.
	///
	/// This function should likely only be used in a `best_fitting!` context,
	/// where one variant attempts to force a list of nodes onto a single line
	/// without any possible breaks, then falls back to a broken out variant if
	/// the content does not fit.
	pub fn join_nodes_with_space<'a>(&'a mut self) -> JoinNodesBuilder<'a, 'buf, Space, Context> {
		JoinNodesBuilder::new(space(), self)
	}

	/// Concatenates a list of [crate::Format] objects with spaces and line
	/// breaks to fit them on as few lines as possible. Each element introduces
	/// a conceptual group. The printer first tries to print the item in flat
	/// mode but then prints it in expanded mode if it doesn't fit.
	///
	/// ## Examples
	///
	/// ```rust
	...
	/// 	text("<b>Important: </b>"),
	/// 	text(
	/// 		"Please do not commit memory bugs such as segfaults, buffer overflows, etc. otherwise \
	/// 		 you ",
	/// 	),
	/// 	text("<em>will</em>"),
	/// 	text(" be reprimanded"),
	...
	/// assert_eq!(
	/// 	&std::format!(
	/// 		"<b>Important: </b>\nPlease do not commit memory bugs such as segfaults, buffer \
	/// 		 overflows, etc. otherwise you \n<em>will</em> be reprimanded"
	/// 	),
	/// 	formatted.print()?.as_code()
	/// );
	/// # Ok(())
