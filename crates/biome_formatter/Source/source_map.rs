...
use biome_rowan::{Language, SyntaxNode, TextLen, TextSize};
use rustc_hash::FxHashMap;

use crate::{Printed, SourceMarker, TextRange};

/// A source map for mapping positions of a pre-processed tree back to the
/// locations in the source tree.
///
/// This is not a generic purpose source map but instead focused on supporting
/// the case where a language removes or re-orders nodes that would otherwise
/// complicate the formatting logic. A common use case for pre-processing is the
/// removal of all parenthesized nodes. Removing parenthesized nodes simplifies
/// the formatting logic when it has different behaviour depending if a child or
/// parent is of a specific node kind. Performing such a test with parenthesized
/// nodes present in the source code means that the formatting logic has to skip
/// over all parenthesized nodes until it finds the first non-parenthesized node
/// and then test if that node is of the expected kind.
///
/// This source map implementation supports removing tokens or re-structuring
/// nodes without changing the order of the tokens in the tree (requires no
/// source map).
///
/// The following section uses parentheses as a concrete example to explain the
/// functionality of the source map. However, the source map implementation
/// isn't restricted to removing parentheses only, it supports mapping
/// transformed to source position for any use case where a transform deletes
/// text from the source tree.
///
/// ## Position Mapping
///
/// The source map internally tracks all the ranges that have been deleted from
/// the source code sorted by the start of the deleted range. It further stores
/// the absolute count of deleted bytes preceding a range. The deleted range
/// together with the absolute count allows to re-compute the source location
/// for every transformed location and has the benefit that it requires
/// significantly fewer memory than source maps that use a source to destination
/// position marker for every token.
///
/// ## Map Node Ranges
///
/// Only having the deleted ranges to resolve the original text of a node isn't
/// sufficient. Resolving the original text of a node is needed when formatting
/// a node as verbatim, either because formatting the node failed because of a
/// syntax error, or formatting is suppressed with a `biome-ignore format:`
/// comment.
///
/// ```text
/// // Source           // Transformed
...
/// ```
///
/// Using the above example, the following source ranges should be returned when
/// quering with the transformed ranges:
///
/// * `a` -> `a`: Should not include the leading `(`
/// * `b` -> `b`: Should not include the trailing `)`
/// * `a + b` -> `(a + b)`: Should include the leading `(` and trailing `)`.
/// * `a + b + c + d` -> `(a + b) + (c + d)`: Should include the fist `(` token
///   and the last `)` token because the expression statement fully encloses the
///   `a + b` and `c + d` nodes.
///
/// This is why the source map also tracks the mapped trimmed ranges for every
/// node.
#[derive(Debug, Clone)]
pub struct TransformSourceMap {
	...
	/// Key: Start or end position of node for which the trimmed range should
	/// be extended Value: The trimmed range.
	mapped_node_ranges:FxHashMap<TextSize, TrimmedNodeRangeMapping>,
}

impl TransformSourceMap {
	/// Returns the text of the source document as it was before the
	/// transformation.
	pub fn source(&self) -> &SourceText { &self.source_text }

	/// Maps a range of the transformed document to a range in the source
	/// document.
	///
	/// Complexity: `O(log(n))`
	...
	/// Maps the trimmed range of the transformed node to the trimmed range in
	/// the source document.
	///
	...
	fn resolve_trimmed_range(&self, mut source_range:TextRange) -> TextRange {
		...
		if let Some(mapping) = start_mapping {
			// If the queried node fully encloses the original range of the
			// node, then extend the range
	...
	/// Returns an iterator over all deleted ranges in increasing order by their
	/// start position.
	pub fn deleted_ranges(&self) -> DeletedRanges {
		DeletedRanges { source_text:self.source(), deleted_ranges:self.deleted_ranges.iter() }
	}
	...
	fn source_offset_with_range(
		...
		match deleted_range {
			Some(range) => {
				...
				// Transformed position directly falls onto a position where a
				// deleted range starts or ends (depending on the position)
				...
				if range.transformed_start() == transformed_offset {
					match position {
						...
						// `a)`, deleted range is right after the token. That's
						// why `source_start` is the offset that truncates
						// the `)` and `source_end` includes it
						RangePosition::End => range.source_start(),
					}
				}
				// The position falls outside of a position that has a
				// leading/trailing deleted range. For example, if you get
				// the position of `+` in `(a + b)`. That means, the trimmed
				// and non-trimmed offsets are the same
				else {
	...
	/// Maps the source code positions relative to the transformed tree of
	/// `printed` to the location in the original, untransformed source code.
	///
	/// The printer creates a source map that allows mapping positions from the
	/// newly formatted document back to the locations of the tree. However,
	/// the source positions stored in [crate::FormatElement::DynamicText]
	/// and [crate::FormatElement::LocatedTokenText] are relative to the
	/// transformed tree and not the original tree passed to
	/// [crate::format_node].
	///
	/// This function re-maps the positions from the positions in the
	/// transformed tree back to the positions in the original, untransformed
	/// tree.
	...
	fn map_markers(&self, markers:&mut [SourceMarker]) {
		...
		for marker in markers {
			// It's not guaranteed that markers are sorted by source location
			// (line suffix comments). It can, therefore, be necessary to
			// navigate backwards again. In this case, do a binary search for
			// the index of the next deleted range (`O(log(n)`).
			...
			if out_of_order_marker {
				...
				// Find the range for this mapping. In most cases this is a
				// no-op or only involves a single step because markers are
				// most of the time in increasing source order.
...
/// The transform function builds the source map by iterating over all tokens
/// and pushing source text in the builder. This correctly builds up the source
/// text for the sub-tree, but the offsets are incorrect if the root isn't at
/// the start of the document. The struct wraps a String and the offset in the
/// document of the formatting node to get correct text slice.
...
impl SourceText {
	pub fn with_source(text:String) -> Self { Self { text, ..Default::default() } }

	pub fn with_offset(offset:TextSize) -> Self { Self { offset, ..Default::default() } }
	...
	pub fn push_str(&mut self, string:&str) { self.text.push_str(string) }
}
...
struct TrimmedNodeRangeMapping {
	...
	/// ```
	///
	/// `0..7` for `a + b` if its range should also include the parenthesized
	/// range.
...
/// Stores the information about a range in the source document that isn't
/// present in the transformed document and provides means to map the
/// transformed position back to the source position.
///
/// # Examples
///
/// ```javascript
...
/// ```
///
/// A transform that removes the parentheses from the above expression removes
/// the ranges `0..1` (`(` token) and `6..7` (`)` token) and the source map
/// creates one [DeletedRange] for each:
///
/// ```text
/// DeletedRange {
///     source_range: 0..1,
...
/// ```
///
/// The first range indicates that the range `0..1` for the `(` token has been
/// removed. The second range indicates that the range `6..7` for the `)` token
/// has been removed and it stores that, up to this point, but not including, 1
/// more byte has been removed.
#[derive(Debug, Copy, Clone, Eq, PartialEq)]
struct DeletedRange {
	/// The range in the source document of the bytes that have been omitted
	/// from the transformed document.
	source_range:TextRange,

	/// The accumulated count of all removed bytes up to (but not including)
...
impl DeletedRange {
	fn new(source_range:TextRange, total_length_preceding_deleted_ranges:TextSize) -> Self {
		...
		Self { source_range, total_length_preceding_deleted_ranges }
	}

	/// The number of deleted characters starting from [source
	/// offset](DeletedRange::source_start).
	fn len(&self) -> TextSize { self.source_range.len() }

	/// The start position in bytes in the source document of the omitted
	/// sequence in the transformed document.
	fn source_start(&self) -> TextSize { self.source_range.start() }

	/// The end position in bytes in the source document of the omitted sequence
	/// in the transformed document.
	fn source_end(&self) -> TextSize { self.source_range.end() }
...
pub struct TransformSourceMapBuilder {
	...
	/// The keys are a position in the source map where a trimmed node starts
	/// or ends. The values are the metadata about a trimmed node range
	mapped_node_ranges:FxHashMap<TextSize, TrimmedNodeRangeMapping>,
}
...
impl TransformSourceMapBuilder {
	...
	pub fn with_offset(offset:TextSize) -> Self {
		Self { source_text:SourceText::with_offset(offset), ..Default::default() }
	}

	/// Creates a new builder for a document with the given source.
	pub fn with_source(source:String) -> Self {
		Self { source_text:SourceText::with_source(source), ..Default::default() }
	}
	...
	///
	/// The formatter uses the trimmed range when formatting a node in verbatim
	/// either because the node failed to format because of a syntax error or
	/// because it's formatting is suppressed with a `biome-ignore format:`
	/// comment.
	///
	/// This method adds a mapping to widen a nodes trimmed range to enclose
	/// another range instead. This is e.g. useful when removing parentheses
	/// around expressions where `(/* comment */ a /* comment */)` because
	...
	pub fn extend_trimmed_node_range(
		...
		self.mapped_node_ranges.insert(original_range.end(), mapping);
	}

	/// Creates a source map that performs single position lookups in
	/// `O(log(n))`.
	pub fn finish(mut self) -> TransformSourceMap {
		let mut merged_mappings = Vec::with_capacity(self.deleted_ranges.len());

		if !self.deleted_ranges.is_empty() {
			self.deleted_ranges.sort_by(|a, b| {
			...
			for range in self.deleted_ranges.drain(1..) {
				// Merge adjacent ranges to ensure there's only ever a single
				// mapping starting at the same transformed offset.
...
pub struct DeletedRangeEntry<'a> {
	...
	/// The position in the transformed document where the removed range would
	/// have been (but is not, because it was removed)
...
impl<'a> Iterator for DeletedRanges<'a> {
	...
	fn size_hint(&self) -> (usize, Option<usize>) { self.deleted_ranges.size_hint() }
...
mod tests {
	...
	use crate::{source_map::DeletedRangeEntry, TextRange, TextSize, TransformSourceMapBuilder};
	...
	fn range_mapping() {
		...
		// The following mapping assume the tranformed string to be (including
		// whitespace): "a + b + c + d + e";
	...
	fn trimmed_range() {
		...
		// Don't mind the unknown nodes, it doesn't really matter what the nodes
		// are.
		let mut cst_builder = RawSyntaxTreeBuilder::new();
		cst_builder.start_node(RawLanguageKind::ROOT);
		...
		let mut bogus = root.descendants().filter(|node| node.kind() == RawLanguageKind::BOGUS);
