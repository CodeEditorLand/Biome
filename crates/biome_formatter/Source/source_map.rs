...
pub struct TransformSourceMap {
	...
	/// Key: Start or end position of node for which the trimmed range should be
	/// extended Value: The trimmed range.
...
impl TransformSourceMap {
	...
	fn resolve_trimmed_range(&self, mut source_range:TextRange) -> TextRange {
		...
		if let Some(mapping) = start_mapping {
			// If the queried node fully encloses the original range of the node, then
			// extend the range
	...
	fn source_offset_with_range(
		...
		match deleted_range {
			Some(range) => {
				...
				// Transformed position directly falls onto a position where a deleted range
				// starts or ends (depending on the position) For example when querying: `a`
				// in `(a)` or (a + b)`, or `b`
				if range.transformed_start() == transformed_offset {
					match position {
						RangePosition::Start => range.source_end(),
						// `a)`, deleted range is right after the token. That's why `source_start`
						// is the offset that truncates the `)` and `source_end` includes it
						RangePosition::End => range.source_start(),
					}
				}
				// The position falls outside of a position that has a leading/trailing deleted
				// range. For example, if you get the position of `+` in `(a + b)`.
	...
	fn map_markers(&self, markers:&mut [SourceMarker]) {
		...
		for marker in markers {
			// It's not guaranteed that markers are sorted by source location (line suffix
			...
			if out_of_order_marker {
				...
				// Find the range for this mapping. In most cases this is a no-op or only
				// involves a single step because markers are most of the time in increasing
				// source order.
...
struct DeletedRange {
	...
	/// The accumulated count of all removed bytes up to (but not including) the
	/// start of this range.
...
pub struct TransformSourceMapBuilder {
	...
	/// The keys are a position in the source map where a trimmed node starts or
	/// ends. The values are the metadata about a trimmed node range
	mapped_node_ranges:FxHashMap<TextSize, TrimmedNodeRangeMapping>,
}
	...
	pub fn finish(mut self) -> TransformSourceMap {
		...
		if !self.deleted_ranges.is_empty() {
			...
			for range in self.deleted_ranges.drain(1..) {
				// Merge adjacent ranges to ensure there's only ever a single mapping starting
				// at the same transformed offset.
...
mod tests {
	...
	use crate::{TextRange, TextSize, TransformSourceMapBuilder, source_map::DeletedRangeEntry};
