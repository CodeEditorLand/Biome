...
impl TransformSourceMap {
	...
	pub fn trimmed_source_range<L: Language>(
		&self,
		node: &SyntaxNode<L>,
	) -> TextRange {
		self.trimmed_source_range_from_transformed_range(
	...
	pub fn trimmed_source_text<L: Language>(
		&self,
		node: &SyntaxNode<L>,
	) -> &str {
	...
	#[cfg(test)]
	fn trimmed_source_text_from_transformed_range(
		&self,
		range: TextRange,
	) -> &str {
	...
	fn source_offset(
		...
		let index = self
			.deleted_ranges
			.binary_search_by_key(&transformed_offset, |range| {
			...
			Some(range) => {
				...
				// That means, the trimmed and non-trimmed offsets are the same
					...
					let transformed_delta =
						transformed_offset - range.transformed_start();
	...
	fn map_markers(&self, markers: &mut [SourceMarker]) {
		...
		for marker in markers {
			...
			let out_of_order_marker = previous_marker
				.map_or(false, |previous| previous.source > marker.source);

			if out_of_order_marker {
				let index = self
					.deleted_ranges
					.binary_search_by_key(&marker.source, |range| {
						range.transformed_start()
					});
...
impl TransformSourceMapBuilder {
	...
	pub fn extend_trimmed_node_range(
		...
		let mapping =
			TrimmedNodeRangeMapping { original_range, extended_range };
		...
		if !self.deleted_ranges.is_empty() {
			...
			for range in self.deleted_ranges.drain(1..) {
				...
				if last_mapping.source_range.end() == range.start() {
					last_mapping.source_range =
						last_mapping.source_range.cover(range);
				} else {
...
mod tests {
	...
	fn range_mapping() {
		...
		cst_builder
			.token(RawLanguageKind::STRING_TOKEN, "(a + (((b + c)) + d)) + e");
		...
		// `(`
		builder.add_deleted_range(TextRange::new(
			TextSize::from(0),
			TextSize::from(1),
		));
		...
		// `a`
		assert_eq!(
			source_map.source_range(TextRange::new(
				TextSize::from(0),
				TextSize::from(1)
			)),
	...
	fn trimmed_range() {
		...
		builder.extend_trimmed_node_range(
			expression.text_trimmed_range(),
			inner.text_trimmed_range(),
		);
		// Extend `(a)` to the range of `((a))`
		builder.extend_trimmed_node_range(
			inner.text_trimmed_range(),
			outer.text_trimmed_range(),
		);
		...
		// Query `a`
		assert_eq!(
			source_map.trimmed_source_text_from_transformed_range(
