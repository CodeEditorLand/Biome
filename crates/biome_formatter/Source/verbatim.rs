...
use biome_rowan::{
	AstNode, Direction, Language, SyntaxElement, SyntaxNode, TextRange,
};
...
pub fn format_verbatim_node<L: Language>(
	node: &SyntaxNode<L>,
) -> FormatVerbatimNode<L> {
	FormatVerbatimNode {
		node,
	...
	fn fmt(&self, f: &mut Formatter<Context>) -> FormatResult<()> {
		for element in self.node.descendants_with_tokens(Direction::Next) {
			match element {
				...
				SyntaxElement::Node(node) => {
					...
					for comment in
						comments.leading_dangling_trailing_comments(&node)
					{
			...
			Context: CstFormatContext,
		{
			f.context().source_map().map_or_else(
				|| range,
				|source_map| source_map.source_range(range),
			)
		}
		...
		if self.format_comments {
			...
			let outside_trimmed_range =
				leading_comments.partition_point(|comment| {
					comment.piece().text_range().end()
						<= trimmed_source_range.start()
				});
			...
			write!(
				f,
				[FormatLeadingComments::Comments(outside_trimmed_range)]
			)?;
		...
		if self.format_comments {
			...
			let outside_trimmed_range_start = trailing_comments
				.partition_point(|comment| {
			...
			write!(
				f,
				[FormatTrailingComments::Comments(outside_trimmed_range)]
			)?;
		}
...
pub fn format_bogus_node<L: Language>(
	node: &SyntaxNode<L>,
) -> FormatVerbatimNode<L> {
	FormatVerbatimNode {
		node,
...
pub fn format_suppressed_node<L: Language>(
	node: &SyntaxNode<L>,
) -> FormatVerbatimNode<L> {
	FormatVerbatimNode {
		node,
