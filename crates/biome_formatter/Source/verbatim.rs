...
/// "Formats" a node according to its original formatting in the source text.
/// It's functionally equal to [`format_verbatim_node`], but it doesn't track
/// the node as [VerbatimKind::Verbatim].
pub fn format_verbatim_skipped<L:Language>(node:&SyntaxNode<L>) -> FormatVerbatimNode<L> {
	FormatVerbatimNode { node, kind:VerbatimKind::Skipped, format_comments:true }
}
	...
	Context: CstFormatContext,
{
	fn fmt(&self, f:&mut Formatter<Context>) -> FormatResult<()> {
		...
		// The trimmed range of a node is its range without any of its leading or
		// trailing trivia. Except for nodes that used to be parenthesized, the range
		// than covers the source from the `(` to the `)` (the trimmed range of the
		// parenthesized expression, not the inner expression)
		...
		// Find the first skipped token trivia, if any, and include it in the verbatim
		// range because the comments only format **up to** but not including skipped
		// token trivia.
