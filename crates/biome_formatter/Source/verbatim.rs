...
/// Being able to format a node "as is" is useful if a node contains syntax
/// errors. Formatting a node with syntax errors has the risk that Biome
/// misinterprets the structure of the code and formatting it could
/// "mess up" the developers, yet incomplete, work or accidentally introduce new
/// syntax errors.
///
/// You may be inclined to call `node.text` directly. However, using `text`
/// doesn't track the nodes nor its children source mapping information,
/// resulting in incorrect source maps for this subtree.
///
/// These nodes and tokens get tracked as [VerbatimKind::Verbatim], useful to
/// understand if these nodes still need to have their own implementation.
pub fn format_verbatim_node<L:Language>(node:&SyntaxNode<L>) -> FormatVerbatimNode<L> {
	FormatVerbatimNode {
		node,
		kind:VerbatimKind::Verbatim { length:node.text_range().len() },
	...
	Context: CstFormatContext,
{
	fn fmt(&self, f:&mut Formatter<Context>) -> FormatResult<()> {
		...
		// The trimmed range of a node is its range without any of its leading
		// or trailing trivia. Except for nodes that used to be parenthesized,
		// the range than covers the source from the `(` to the `)` (the
		// trimmed range of the parenthesized expression, not the inner
		// expression)
		...
		// Format all leading comments that are outside of the node's source
		// range.
		...
		// Find the first skipped token trivia, if any, and include it in the
		// verbatim range because the comments only format **up to** but not
		// including skipped token trivia.
...
/// Formats bogus nodes. The difference between this method  and
/// `format_verbatim` is that this method doesn't track nodes/tokens as
/// [VerbatimKind::Verbatim]. They are just printed as they are.
pub fn format_bogus_node<L:Language>(node:&SyntaxNode<L>) -> FormatVerbatimNode<L> {
	FormatVerbatimNode { node, kind:VerbatimKind::Bogus, format_comments:true }
}
...
pub fn format_suppressed_node<L:Language>(node:&SyntaxNode<L>) -> FormatVerbatimNode<L> {
	FormatVerbatimNode { node, kind:VerbatimKind::Suppressed, format_comments:true }
}
	...
	fn fmt(&self, f:&mut Formatter<Context>) -> FormatResult<()> {
		...
		match self.inner.fmt(f) {
			...
			Err(FormatError::SyntaxError) => {
				...
				// Lists that yield errors are formatted as they were suppressed
				// nodes. Doing so, the formatter formats the nodes/tokens
				// as is.
