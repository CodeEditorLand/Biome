		...
		for attr in attrs {
			if attr.path.is_ident("diagnostic") {
				...
				let attrs = match DiagnosticAttrs::parse.parse(tokens) {
					Ok(attrs) => attrs,
					Err(err) => abort!(err.span(), "failed to parse \"diagnostic\" attribute: {}", err),
				};
			...
			for attr in field.attrs {
				...
				if attr.path.is_ident("location") {
					...
					let attr = match LocationAttr::parse.parse(tokens) {
						Ok(attr) => attr,
						Err(err) => abort!(err.span(), "failed to parse \"location\" attribute: {}", err),
					};
