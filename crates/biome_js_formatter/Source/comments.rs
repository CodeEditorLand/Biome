...
impl FormatRule<SourceComment<JsLanguage>> for FormatJsLeadingComment {
	...
	fn fmt(
		...
		if is_alignable_comment(comment.piece()) {
			...
			// SAFETY: Safe, `is_alignable_comment` only returns `true` for multiline
			// comments
...
fn handle_call_expression_comment(
	...
	// Make comments between the callee and the arguments leading comments of the
	// first argument. ```javascript
	// a /* comment */ (call)
	// ```
...
fn handle_continue_break_comment(
	...
	// Make comments between `continue`/`break` and the label, leading comments of
	// the label. ```javascript
	// continue /* comment */ a;
	// ```
	...
	// continue /* comment */ a;
	// ```
	// This differs from Prettier because other ASTs use an identifier for the label
	// whereas Biome uses a token.
	match enclosing.kind() {
		JsSyntaxKind::JS_CONTINUE_STATEMENT | JsSyntaxKind::JS_BREAK_STATEMENT => {
			match enclosing.parent() {
				...
				// to prevent that the comment becomes a trailing comment of the parent when
				// re-formatting ```javascript
...
fn handle_class_comment(comment:DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
	// Make comments following after the `extends` or `implements` keyword trailing
	// comments of the preceding extends, type parameters, or id.
	// ```javascript
	// class a9 extends
	//   /* comment */
	// b {
	...
	if let Some(following) = comment.following_node() {
		...
		if let Some(preceding) = comment.preceding_node() {
			// Make all comments between the id/type parameters and the extends clause
			// trailing comments of the id/type parameters
			//
			// ```javascript
			// class Test
			//
		...
		// Handle the case where there are no members, attach the comments as dangling
		// comments. ```javascript
		// class Test // comment
		// {
		// }
...
fn handle_conditional_comment(
	...
	// Make end of line comments that come after the operator leading comments of
	// the consequent / alternate. ```javascript
	// a
...
fn handle_if_statement_comment(
	...
	fn handle_else_clause(
		...
		// Make all comments trailing comments of the `consequent` if the `consequent`
		// is a `JsBlockStatement` ```javascript
		// if (test) {
		//
		// } /* comment */ else if (b) {
		//     test
	...
	match (comment.enclosing_node().kind(), comment.following_node()) {
		(JsSyntaxKind::JS_IF_STATEMENT, Some(following)) => {
			...
			// Move comments coming before an if chain inside the body of the first non
			// chain if.
			//
			// ```javascript
...
fn handle_variable_declarator_comment(
	...
	match enclosing.kind() {
		JsSyntaxKind::JS_ASSIGNMENT_EXPRESSION | JsSyntaxKind::TS_TYPE_ALIAS_DECLARATION => {
			// Makes all comments preceding objects/arrays/templates or block comments
			// leading comments of these nodes. ```javascript
			// let a = // comment
			// { };
			// ```
		...
		JsSyntaxKind::JS_INITIALIZER_CLAUSE => {
			...
			if matches!(
				...
				// Keep trailing comments with the id for variable declarators. Necessary
				// because the value is wrapped inside of an initializer clause.
				// ```javascript
				// let a = // comment
				//      b;
				// ```
...
fn handle_import_export_specifier_comment(
	...
	match enclosing_node.kind() {
		// Make end of line or own line comments in the middle of an import specifier leading
		// comments of that specifier ```javascript
		// import { a //comment1
		// //comment2
		// //comment3
		// as b} from "";
		...
		// Make end of line or own line comments in the middle of an import assertion a leading
		// comment of the assertion
	...
	match enclosing_node.kind() {
		JsSyntaxKind::JS_IMPORT_NAMED_CLAUSE => {
				...
				comment.preceding_node().and_then(JsNamedImportSpecifiers::cast_ref)
			{
				...
				// attach comments to the last specifier as trailing comments if comments are at
				// the end of the line ```javascript
					...
					&& comment.text_position() == CommentTextPosition::EndOfLine
				{
					...
					// attach comments to the import specifier as leading comments if comments are
					// placed after the from keyword ```javascript
					// import {} from // comment
					// "foo"
					// ```
