...
use biome_formatter::{
	comments::{
		CommentKind, CommentPlacement, CommentStyle, CommentTextPosition,
...
use biome_js_syntax::{
	AnyJsClass, AnyJsName, AnyJsRoot, AnyJsStatement, JsArrayHole,
	JsArrowFunctionExpression, JsBlockStatement, JsCallArguments,
	JsCatchClause, JsEmptyStatement, JsFinallyClause, JsFormalParameter,
	JsFunctionBody, JsIdentifierBinding, JsIdentifierExpression, JsIfStatement,
	JsLanguage, JsNamedImportSpecifiers, JsParameters, JsSyntaxKind,
	JsSyntaxNode, JsVariableDeclarator, JsWhileStatement,
	TsInterfaceDeclaration, TsMappedType,
};
use biome_rowan::{
	AstNode, SyntaxNodeOptionExt, SyntaxTriviaPieceComments, TextLen,
};
...
impl CommentStyle for JsCommentStyle {
	...
	fn get_comment_kind(
		comment: &SyntaxTriviaPieceComments<JsLanguage>,
	) -> CommentKind {
...
fn handle_after_arrow_fat_arrow_comment(
	...
	if JsArrowFunctionExpression::can_cast(comment.enclosing_node().kind()) {
		...
		//  ) => c;
		// ```
		if let Some(js_parameters) =
			comment.preceding_node().and_then(JsParameters::cast_ref)
		{
...
fn handle_array_hole_comment(
	...
	if let Some(array_hole) =
		comment.preceding_node().and_then(JsArrayHole::cast_ref)
	{
...
fn handle_call_expression_comment(
	...
	if let Some(arguments) =
		comment.following_node().and_then(JsCallArguments::cast_ref)
	{
...
fn handle_switch_default_case_comment(
	...
	let Some(block) =
		comment.following_node().and_then(JsBlockStatement::cast_ref)
	else {
...
fn handle_class_comment(
	...
	let first_member =
		if let Some(class) = AnyJsClass::cast_ref(comment.enclosing_node()) {
			class.members().first().map(AstNode::into_syntax)
		} else if let Some(interface) =
			TsInterfaceDeclaration::cast_ref(comment.enclosing_node())
		{
	...
	if let Some(following) = comment.following_node() {
		if let Some(body) = JsFunctionBody::cast_ref(following) {
			if let Some(directive) = body.directives().first() {
				return CommentPlacement::leading(
					directive.into_syntax(),
					comment,
				);
			}

			let first_non_empty = body.statements().iter().find(|statement| {
			...
			return match first_non_empty {
				...
				Some(statement) => {
					CommentPlacement::leading(statement.into_syntax(), comment)
				},
			};
		}
	}
...
fn handle_member_expression_comment(
	...
	// /* comment */[b]
	// ```
	if AnyJsName::can_cast(following.kind())
		|| JsIdentifierExpression::can_cast(following.kind())
	{
		CommentPlacement::leading(comment.enclosing_node().clone(), comment)
	} else {
		CommentPlacement::Default(comment)
...
fn handle_function_comment(
	...
	let Some(body) =
		comment.following_node().and_then(JsFunctionBody::cast_ref)
	else {
		return CommentPlacement::Default(comment);
	};
...
fn handle_if_statement_comment(
	...
	match (comment.enclosing_node().kind(), comment.following_node()) {
		(JsSyntaxKind::JS_IF_STATEMENT, Some(following)) => {
			let if_statement =
				JsIfStatement::unwrap_cast(comment.enclosing_node().clone());

			if let Some(preceding) = comment.preceding_node() {
				// Test if this is a comment right before the condition's `)`
				if comment.following_token().map_or(false, |token| {
					token.kind() == JsSyntaxKind::R_PAREN
				}) {
					return CommentPlacement::trailing(
						preceding.clone(),
						comment,
			...
			if let Some(if_statement) = JsIfStatement::cast_ref(following) {
				if let Ok(nested_consequent) = if_statement.consequent() {
					return place_leading_statement_comment(
						nested_consequent,
						comment,
					);
				}
...
fn handle_while_comment(
	...
	let (while_statement, following) = match (
		...
		(Some(while_statement), Some(following)) => {
			(while_statement, following)
		},
		_ => return CommentPlacement::Default(comment),
	};
...
fn handle_for_comment(
	...
	if !is_for_in_or_of
		&& !matches!(enclosing.kind(), JsSyntaxKind::JS_FOR_STATEMENT)
	{
		return CommentPlacement::Default(comment);
	}
	...
	match enclosing.kind() {
		...
		JsSyntaxKind::JS_VARIABLE_DECLARATOR => {
			let variable_declarator =
				JsVariableDeclarator::unwrap_cast(enclosing.clone());
					...
					if initializer.syntax() == following
						&& initializer
							...
							.map_or(false, |expression| {
								is_complex_value(expression.syntax())
							}) =>
				{
					return CommentPlacement::leading(
						initializer.into_syntax(),
						comment,
					);
				},
				_ => {
		...
		JsSyntaxKind::JS_INITIALIZER_CLAUSE => {
			...
			if matches!(
				...
				let not_complex = matches!(
					parent_kind,
					Some(JsSyntaxKind::JS_PROPERTY_CLASS_MEMBER)
	...
	match comment.enclosing_node().kind() {
		...
		| JsSyntaxKind::TS_PROPERTY_PARAMETER => {
			...
			if let Some(preceding_node) = comment.preceding_node() {
				if comment.following_node().kind()
					!= Some(JsSyntaxKind::JS_DECORATOR)
				{
					return CommentPlacement::trailing(
...
fn handle_mapped_type_comment(
	...
	if matches!(
		...
		if let Some(enclosing) =
			TsMappedType::cast_ref(comment.enclosing_node())
		{
			if let Ok(keys_type) = enclosing.keys_type() {
				return CommentPlacement::trailing(
					keys_type.into_syntax(),
					comment,
				);
			}
		}
		...
		JsSyntaxKind::JS_IMPORT_NAMED_CLAUSE => {
				...
				.and_then(JsNamedImportSpecifiers::cast_ref)
			{
					...
					&& comment.text_position() == CommentTextPosition::EndOfLine
				{
					...
					// "foo"
					// ```
					let is_after_from_keyword =
						comment.following_token().map_or(true, |token| {
							token.kind() != JsSyntaxKind::FROM_KW
						});
					if is_after_from_keyword {
						if let Some(following_node) = comment.following_node() {
							return CommentPlacement::leading(
								following_node.clone(),
								comment,
							);
						}
					}
...
fn place_block_statement_comment(
	...
	let first_non_empty =
		block_statement.statements().iter().find(|statement| {
			!matches!(statement, AnyJsStatement::JsEmptyStatement(_))
		});

	match first_non_empty {
		None => {
			CommentPlacement::dangling(block_statement.into_syntax(), comment)
		},
		Some(statement) => {
			CommentPlacement::leading(statement.into_syntax(), comment)
		},
