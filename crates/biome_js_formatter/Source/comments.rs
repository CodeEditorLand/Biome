...
use crate::{prelude::*, utils::AnyJsConditional};
	...
	fn place_comment(
		...
		match comment.text_position() {
			CommentTextPosition::EndOfLine => {
				handle_typecast_comment(comment)
			...
			CommentTextPosition::OwnLine => {
				handle_member_expression_comment(comment)
					.or_else(handle_function_comment)
					.or_else(handle_if_statement_comment)
					.or_else(handle_while_comment)
					.or_else(handle_try_comment)
			...
			CommentTextPosition::SameLine => {
				handle_if_statement_comment(comment)
					.or_else(handle_while_comment)
					.or_else(handle_for_comment)
					.or_else(handle_root_comments)
...
/// Force end of line type cast comments to remain leading comments of the next
/// node, if any
...
fn handle_after_arrow_fat_arrow_comment(
	...
	if JsArrowFunctionExpression::can_cast(comment.enclosing_node().kind()) {
		...
		// ) => c;
		// ```
		if let Some(js_ident_binding) =
			comment.preceding_node().and_then(JsIdentifierBinding::cast_ref)
		{
...
/// become trailing comments by default. Override it that all comments are
/// leading comments.
...
fn handle_call_expression_comment(
	...
	// Make comments between the callee and the arguments leading comments of
	// the first argument. ```javascript
	// a /* comment */ (call)
	// ```
...
fn handle_continue_break_comment(
	...
	// Make comments between `continue`/`break` and the label, leading comments
	// of the label. ```javascript
	// continue /* comment */ a;
	// ```
	...
	// continue /* comment */ a;
	// ```
	// This differs from Prettier because other ASTs use an identifier for the
	// label whereas Biome uses a token.
	match enclosing.kind() {
		JsSyntaxKind::JS_CONTINUE_STATEMENT | JsSyntaxKind::JS_BREAK_STATEMENT => {
			match enclosing.parent() {
				// Make it the trailing of the parent if this is a
				// single-statement body to prevent that the comment becomes
				// a trailing comment of the parent when re-formatting
				// ```javascript
...
/// ```
///
/// All other same line comments will use `Default` placement if they have a
/// preceding node. ```javascript
/// switch(x) {
///     default:
///         a(); // asd
///         break;
		...
		(JsSyntaxKind::TS_DECLARE_STATEMENT, Some(following)) => {
			match following.kind() {
				JsSyntaxKind::TS_GLOBAL_DECLARATION => {
					// Global declarations have no identifier, so keep at
					// default
					CommentPlacement::Default(comment)
				},
				...
				| JsSyntaxKind::TS_TYPE_ALIAS_DECLARATION => {
					...
					// This is the first child of the module declaration which
					// is the identifier
...
fn handle_class_comment(comment:DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
	...
	// trailing comments of the preceding extends, type parameters, or id.
	// ```javascript
	// class a9 extends
	//   /* comment */
	// b {
	...
	if let Some(following) = comment.following_node() {
		// Make comments preceding the first member leading comments of the
		// member ```javascript
		// class Test { /* comment */
		//      prop;
		// }
		...
		if let Some(preceding) = comment.preceding_node() {
			// Make all comments between the id/type parameters and the extends
			// clause trailing comments of the id/type parameters
			//
			// ```javascript
			// class Test
			//
		...
		// Handle the case where there are no members, attach the comments as
		// dangling comments. ```javascript
		// class Test // comment
		// {
		// }
		// ```
...
fn handle_method_comment(comment:DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
	...
	// Move end of line and own line comments before the method body into the
	// function ```javascript
	// class Test {
	//   method() /* test */
	//  {}
...
fn handle_function_comment(comment:DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
	...
	// Make line comments between the `)` token and the function body leading
	// comments of the first statement or dangling comments of the body.
	// ```javascript
...
fn handle_conditional_comment(
	...
	let (conditional, following) =
		match (AnyJsConditional::cast_ref(enclosing), comment.following_node()) {
	...
	// Make end of line comments that come after the operator leading comments
	// of the consequent / alternate. ```javascript
	// a
...
fn handle_if_statement_comment(
	...
	fn handle_else_clause(
		...
		// Make all comments trailing comments of the `consequent` if the
		// `consequent` is a `JsBlockStatement` ```javascript
		// if (test) {
		//
		// } /* comment */ else if (b) {
		//     test
		...
		// Handle end of line comments that aren't stretching over multiple
		// lines. Make them dangling comments of the consequent expression
		//
		// ```javascript
	...
	match (comment.enclosing_node().kind(), comment.following_node()) {
		(JsSyntaxKind::JS_IF_STATEMENT, Some(following)) => {
			...
			// Move comments coming before an if chain inside the body of the
			// first non chain if.
			//
			// ```javascript
		...
		(JsSyntaxKind::JS_ELSE_CLAUSE, _) => {
			if let Some(if_statement) =
				comment.enclosing_node().parent().and_then(JsIfStatement::cast)
			{
				if let Ok(consequent) = if_statement.consequent() {
...
fn handle_for_comment(comment:DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
	...
	// ```
		...
		.following_node()
		.map_or(false, |following| JsEmptyStatement::can_cast(following.kind()))
	{
		if let Some(preceding) = comment.preceding_node() {
...
fn handle_variable_declarator_comment(
	...
	match enclosing.kind() {
		JsSyntaxKind::JS_ASSIGNMENT_EXPRESSION | JsSyntaxKind::TS_TYPE_ALIAS_DECLARATION => {
			// Makes all comments preceding objects/arrays/templates or block
			// comments leading comments of these nodes. ```javascript
			// let a = // comment
			// { };
			// ```
		...
		JsSyntaxKind::JS_INITIALIZER_CLAUSE => {
			...
			if matches!(
				...
				// Keep trailing comments with the id for variable declarators.
				// Necessary because the value is wrapped inside of an
				// initializer clause. ```javascript
				// let a = // comment
				//      b;
				// ```
				if not_complex
	...
	match comment.enclosing_node().kind() {
		...
		JsSyntaxKind::JS_INITIALIZER_CLAUSE => {
			if let Some(parameter) =
				comment.enclosing_node().parent().and_then(JsFormalParameter::cast)
			{
				// Keep end of line comments after the `=` trailing comments of
				// the id ```javascript
				// function a(
				//   b = // test
				//     c
...
/// Format comments preceding the type parameter name in mapped types on the
/// line above.
///
...
fn handle_import_export_specifier_comment(
	...
	match enclosing_node.kind() {
		// Make end of line or own line comments in the middle of an import
		// specifier leading comments of that specifier ```javascript
		// import { a //comment1
		// //comment2
		// //comment3
		// as b} from "";
		...
		// Make end of line or own line comments in the middle of an import
		// assertion a leading comment of the assertion
...
/// Attach comments before the `async` keyword or `*` token to the preceding
/// node if it exists to ensure these comments are placed before the `async`
/// keyword or `*` token. ```javascript
/// class Foo {
///    @decorator()
///    // comment
///    async method() {}
...
fn handle_import_named_clause_comments(
	...
	match enclosing_node.kind() {
		JsSyntaxKind::JS_IMPORT_NAMED_CLAUSE => {
			if let Some(import_specifiers) =
				comment.preceding_node().and_then(JsNamedImportSpecifiers::cast_ref)
			{
				let specifier_list = import_specifiers.specifiers();
				// attach comments to the last specifier as trailing comments if
				// comments are at the end of the line ```javascript
					...
					&& comment.text_position() == CommentTextPosition::EndOfLine
				{
					...
					// attach comments to the import specifier as leading
					// comments if comments are placed after the from keyword
					// ```javascript
					// import {} from // comment
					// "foo"
					// ```
...
pub(crate) fn is_type_comment(comment:&SyntaxTriviaPieceComments<JsLanguage>) -> bool {
	...
	text.trim_start_matches("/**")
		...
		.split_whitespace()
		.any(|word| {
			match word.strip_prefix("@type").or_else(|| word.strip_prefix("@satisfies")) {
