...
use biome_formatter::{
	comments::{
		CommentKind, CommentPlacement, CommentStyle, CommentTextPosition, Comments, DecoratedComment, SourceComment,
	},
	write,
};
...
use biome_js_syntax::{
	...
	JsCallArguments, JsCatchClause, JsEmptyStatement, JsFinallyClause, JsFormalParameter, JsFunctionBody,
	JsIdentifierBinding, JsIdentifierExpression, JsIfStatement, JsLanguage, JsNamedImportSpecifiers, JsParameters,
	JsSyntaxKind, JsSyntaxNode, JsVariableDeclarator, JsWhileStatement, TsInterfaceDeclaration, TsMappedType,
};
...
impl FormatRule<SourceComment<JsLanguage>> for FormatJsLeadingComment {
	...
	fn fmt(&self, comment: &SourceComment<JsLanguage>, f: &mut Formatter<Self::Context>) -> FormatResult<()> {
		if is_alignable_comment(comment.piece()) {
			...
			write!(
				...
				[&format_once(|f| {
					for line in lines {
						write!(f, [hard_line_break(), text(" "), dynamic_text(line.trim(), source_offset)])?;
...
fn handle_after_arrow_fat_arrow_comment(comment: DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
...
fn handle_after_arrow_param_comment(comment: DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
...
fn handle_array_hole_comment(comment: DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
...
fn handle_call_expression_comment(comment: DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
...
fn handle_continue_break_comment(comment: DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
	...
	if let (Some(preceding), Some(parent)) = (comment.preceding_node(), comment.enclosing_node().parent()) {
		if preceding.kind() == JsSyntaxKind::JS_LABEL && parent.kind() != JsSyntaxKind::JS_FOR_STATEMENT {
...
/// formatting.
fn handle_switch_default_case_comment(comment: DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
	...
	let Some(block) = comment.following_node().and_then(JsBlockStatement::cast_ref) else {
...
fn handle_labelled_statement_comment(comment: DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
	match comment.enclosing_node().kind() {
		JsSyntaxKind::JS_LABELED_STATEMENT => CommentPlacement::leading(comment.enclosing_node().clone(), comment),
		_ => CommentPlacement::Default(comment),
	}
}
...
fn handle_class_comment(comment: DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
	...
	if matches!(
		...
		JsSyntaxKind::JS_EXTENDS_CLAUSE | JsSyntaxKind::TS_IMPLEMENTS_CLAUSE | JsSyntaxKind::TS_EXTENDS_CLAUSE
	) {
...
fn handle_root_comments(comment: DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
	if let Some(root) = AnyJsRoot::cast_ref(comment.enclosing_node()) {
		let is_blank = match &root {
			...
			AnyJsRoot::JsModule(module) => module.directives().is_empty() && module.items().is_empty(),
			AnyJsRoot::JsScript(script) => script.directives().is_empty() && script.statements().is_empty(),
			AnyJsRoot::TsDeclarationModule(module) => module.directives().is_empty() && module.items().is_empty(),
		};
...
fn handle_member_expression_comment(comment: DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
...
fn handle_conditional_comment(comment: DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
	let enclosing = comment.enclosing_node();

	let (conditional, following) = match (AnyJsConditional::cast_ref(enclosing), comment.following_node()) {
	...
	let is_after_operator =
		conditional.colon_token().as_ref() == Ok(&token) || conditional.question_mark_token().as_ref() == Ok(&token);
...
fn handle_if_statement_comment(comment: DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
	fn handle_else_clause(
		...
		// else expr3; /* b*/
		// ```
		if !comment.kind().is_block() && !comment.text_position().is_own_line() && comment.preceding_node().is_some() {
			return CommentPlacement::trailing(consequent, comment);
		}
	...
	match (comment.enclosing_node().kind(), comment.following_node()) {
		...
		(JsSyntaxKind::JS_ELSE_CLAUSE, _) => {
			if let Some(if_statement) = comment.enclosing_node().parent().and_then(JsIfStatement::cast) {
				if let Ok(consequent) = if_statement.consequent() {
					return handle_else_clause(comment, consequent.into_syntax(), if_statement.into_syntax());
				}
			}
		},
		_ => {
...
fn handle_while_comment(comment: DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
	let (while_statement, following) =
		match (JsWhileStatement::cast_ref(comment.enclosing_node()), comment.following_node()) {
...
fn handle_variable_declarator_comment(comment: DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
	let following = match comment.following_node() {
	...
	match enclosing.kind() {
		...
		JsSyntaxKind::JS_INITIALIZER_CLAUSE => {
			...
			if matches!(
				...
				let not_complex =
					matches!(parent_kind, Some(JsSyntaxKind::JS_PROPERTY_CLASS_MEMBER)) || !is_complex_value(following);
...
fn handle_parameter_comment(comment: DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
	...
	match comment.enclosing_node().kind() {
		...
		JsSyntaxKind::JS_INITIALIZER_CLAUSE => {
			if let Some(parameter) = comment.enclosing_node().parent().and_then(JsFormalParameter::cast) {
...
/// }
/// ```
fn handle_mapped_type_comment(comment: DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
	if !matches!(comment.enclosing_node().kind(), JsSyntaxKind::TS_MAPPED_TYPE) {
		return CommentPlacement::Default(comment);
	}

	if matches!(
		comment.following_node().kind(),
		Some(JsSyntaxKind::TS_TYPE_PARAMETER_NAME | JsSyntaxKind::TS_MAPPED_TYPE_READONLY_MODIFIER_CLAUSE,)
	) {
		return CommentPlacement::dangling(comment.enclosing_node().clone(), comment);
	}

	if matches!(comment.preceding_node().kind(), Some(JsSyntaxKind::TS_TYPE_PARAMETER_NAME)) {
...
fn handle_union_type_comment(comment: DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
	match (comment.enclosing_node().kind(), comment.preceding_node()) {
		(JsSyntaxKind::TS_UNION_TYPE, Some(preceding)) => CommentPlacement::trailing(preceding.clone(), comment),
		_ => CommentPlacement::Default(comment),
	}
}

fn handle_import_export_specifier_comment(comment: DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
	...
	match enclosing_node.kind() {
		...
		| JsSyntaxKind::JS_EXPORT_NAMED_FROM_SPECIFIER => CommentPlacement::leading(enclosing_node.clone(), comment),
		// Make end of line or own line comments in the middle of an import assertion a leading comment of the assertion
		JsSyntaxKind::JS_IMPORT_ASSERTION_ENTRY => CommentPlacement::leading(enclosing_node.clone(), comment),
...
///    async method() {}
/// }
fn handle_class_method_comment(comment: DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
	let enclosing_node = comment.enclosing_node();
	match enclosing_node.kind() {
		JsSyntaxKind::JS_METHOD_CLASS_MEMBER => {
			if let Some(following_token) = comment.following_token() {
				if matches!(following_token.kind(), JsSyntaxKind::ASYNC_KW | JsSyntaxKind::STAR) {
					if let Some(preceding) = comment.preceding_node() {
						return CommentPlacement::trailing(preceding.clone(), comment);
					}
				}
...
fn handle_import_named_clause_comments(comment: DecoratedComment<JsLanguage>) -> CommentPlacement<JsLanguage> {
	let enclosing_node = comment.enclosing_node();
	match enclosing_node.kind() {
		JsSyntaxKind::JS_IMPORT_NAMED_CLAUSE => {
			if let Some(import_specifiers) = comment.preceding_node().and_then(JsNamedImportSpecifiers::cast_ref) {
...
pub(crate) fn is_type_comment(comment: &SyntaxTriviaPieceComments<JsLanguage>) -> bool {
	...
	text.trim_start_matches("/**")
		...
		.split_whitespace()
		.any(
			|word| match word.strip_prefix("@type").or_else(|| word.strip_prefix("@satisfies")) {
