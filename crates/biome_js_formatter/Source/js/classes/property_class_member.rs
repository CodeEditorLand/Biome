...
use crate::{
	context::Semicolons,
	prelude::*,
	utils::{AnyJsAssignmentLike, FormatSemicolon},
};
...
impl Format<JsFormatContext> for FormatClassPropertySemicolon<'_> {
	fn fmt(&self, f:&mut Formatter<JsFormatContext>) -> FormatResult<()> {
		match f.options().semicolons() {
			...
			Semicolons::AsNeeded => {
				match (self.semicolon, needs_semicolon(self.property)?) {
					(None, false) => Ok(()),
...
fn needs_semicolon(property:&AnyJsPropertyClassMember) -> SyntaxResult<bool> {
	...
	let Some(next_member) = property.syntax().next_sibling().and_then(AnyJsClassMember::cast)
	else {
		return Ok(false);
	};
