...
use biome_js_syntax::{
	AnyJsClassMember, AnyJsClassMemberName, JsInitializerClause, JsPropertyClassMember, JsSyntaxToken,
	TsInitializedPropertySignatureClassMember, TsPropertySignatureClassMember,
};
...
impl AnyJsPropertyClassMember {
	fn name(&self) -> SyntaxResult<AnyJsClassMemberName> {
		match self {
			...
			AnyJsPropertyClassMember::TsInitializedPropertySignatureClassMember(property) => property.name(),
		}
	}
	...
	fn value(&self) -> Option<JsInitializerClause> {
		match self {
			...
			AnyJsPropertyClassMember::TsInitializedPropertySignatureClassMember(property) => property.value().ok(),
		}
	}

	fn has_property_annotation(&self) -> bool {
		match self {
			AnyJsPropertyClassMember::JsPropertyClassMember(property) => property.property_annotation().is_some(),
...
impl<'a> FormatClassPropertySemicolon<'a> {
	pub fn new(property: &'a AnyJsPropertyClassMember, semicolon: Option<&'a JsSyntaxToken>) -> Self {
		Self { property, semicolon }
	}
}
...
fn needs_semicolon(property: &AnyJsPropertyClassMember) -> SyntaxResult<bool> {
	...
	let Some(next_member) = property.syntax().next_sibling().and_then(AnyJsClassMember::cast) else {
		return Ok(false);
	};
	...
	// a = b;
	// instanceof;
	if property.value().is_some() && (next_member.has_name("instanceof")? || next_member.has_name("in")?) {
		return Ok(true);
	}
...
fn has_modifiers(member: &AnyJsClassMember) -> bool {
	let is_empty = match member {
		AnyJsClassMember::JsConstructorClassMember(constructor) => constructor.modifiers().is_empty(),
		...
		AnyJsClassMember::TsConstructorSignatureClassMember(constructor) => constructor.modifiers().is_empty(),
		...
		AnyJsClassMember::TsPropertySignatureClassMember(property) => property.modifiers().is_empty(),
		AnyJsClassMember::TsInitializedPropertySignatureClassMember(property) => property.modifiers().is_empty(),
