...
use biome_js_syntax::{
	AnyJsClassMemberName, JsConstructorClassMember, JsConstructorParameters, JsFunctionBody, JsParameters,
	TsMethodSignatureClassMember, TsMethodSignatureTypeMember, TsReturnTypeAnnotation, TsTypeParameters,
};
...
impl FormatAnyJsMethodMember {
	fn async_token(&self) -> Option<JsSyntaxToken> {
		match self {
			...
			FormatAnyJsMethodMember::TsMethodSignatureClassMember(signature) => signature.async_token(),
	...
	fn name(&self) -> SyntaxResult<AnyJsMemberName> {
		Ok(match self {
			...
			FormatAnyJsMethodMember::TsMethodSignatureClassMember(signature) => signature.name()?.into(),
	...
	fn type_parameters(&self) -> Option<TsTypeParameters> {
		match self {
			...
			FormatAnyJsMethodMember::JsConstructorClassMember(_) => None,
			FormatAnyJsMethodMember::TsMethodSignatureClassMember(signature) => signature.type_parameters(),
			FormatAnyJsMethodMember::TsMethodSignatureTypeMember(member) => member.type_parameters(),
		}
	}
	...
	fn parameters(&self) -> SyntaxResult<MethodParameters> {
		Ok(match self {
			...
			FormatAnyJsMethodMember::JsConstructorClassMember(member) => member.parameters()?.into(),
			FormatAnyJsMethodMember::TsMethodSignatureClassMember(signature) => signature.parameters()?.into(),
			FormatAnyJsMethodMember::TsMethodSignatureTypeMember(member) => member.parameters()?.into(),
		})
	}
	...
	fn return_type_annotation(&self) -> Option<TsReturnTypeAnnotation> {
		match self {
			...
			FormatAnyJsMethodMember::JsMethodObjectMember(member) => member.return_type_annotation(),
			FormatAnyJsMethodMember::JsConstructorClassMember(_) => None,
			FormatAnyJsMethodMember::TsMethodSignatureClassMember(signature) => signature.return_type_annotation(),
			FormatAnyJsMethodMember::TsMethodSignatureTypeMember(member) => member.return_type_annotation(),
		}
	}
	...
	fn question_mark_token(&self) -> Option<JsSyntaxToken> {
		match self {
			...
			FormatAnyJsMethodMember::JsConstructorClassMember(_) => None,
			FormatAnyJsMethodMember::TsMethodSignatureClassMember(signature) => signature.question_mark_token(),
