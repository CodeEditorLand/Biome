...
use biome_js_syntax::{JsImportAssertion, JsImportAssertionFields};
...
impl FormatNodeRule<JsImportAssertion> for FormatJsImportAssertion {
	fn fmt_fields(&self, node:&JsImportAssertion, f:&mut JsFormatter) -> FormatResult<()> {
		let JsImportAssertionFields { l_curly_token, assertions, r_curly_token, assertion_kind } =
			node.as_fields();
	...
	fn fmt_dangling_comments(&self, _:&JsImportAssertion, _:&mut JsFormatter) -> FormatResult<()> {
