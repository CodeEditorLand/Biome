...
impl FormatNodeRule<JsImportNamedClause> for FormatJsImportNamedClause {
	fn fmt_fields(&self, node:&JsImportNamedClause, f:&mut JsFormatter) -> FormatResult<()> {
		...
		// can_break implementation, return `format_element` instead of boolean to
		// reduce enum conversion overhead. if `can_break` is true we just use the
		// previous format strategy, otherwise we use the new format strategy. reference https://github.com/prettier/prettier/blob/5b113e71b1808d6916f446c3aa49c3c53e3bdb98/src/language-js/print/module.js#L173
		...
		// `standaloneSpecifiers` corresponding our `JsDefaultImportSpecifier` + part of
		// `JsNamespaceImportSpecifier`, `groupedSpecifiers` corresponding our
		// `JsNamedImportSpecifiers`

		//  Here we use an opposite way of thinking, we only thinking about the way that
		// can not break That's to say
		...
		{
			// SAFETY: we know that the `specifiers.specifiers().len() == 1`, so unwrap
			// `iter().next()` is safe.
