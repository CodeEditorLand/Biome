...
impl FormatNodeRule<JsImportNamedClause> for FormatJsImportNamedClause {
	fn fmt_fields(&self, node:&JsImportNamedClause, f:&mut JsFormatter) -> FormatResult<()> {
		...
		// can_break implementation, return `format_element` instead of boolean
		// to reduce enum conversion overhead. if `can_break` is true we just
		// use the previous format strategy, otherwise we use the new format
		// strategy. reference https://github.com/prettier/prettier/blob/5b113e71b1808d6916f446c3aa49c3c53e3bdb98/src/language-js/print/module.js#L173
		...
		// `standaloneSpecifiers` corresponding our `JsDefaultImportSpecifier` +
		// part of `JsNamespaceImportSpecifier`, `groupedSpecifiers`
		// corresponding our `JsNamedImportSpecifiers`

		//  Here we use an opposite way of thinking, we only thinking about the
		// way that can not break That's to say
		// 2. length of `JsNamedImportSpecifiers` at least is one
		// 3. Surrounding of the only `JsNamedImportSpecifiers` should not have any
		//    comments
		if specifiers.len() == 1 && !f.context().comments().is_suppressed(named_specifiers.syntax())
		{
			// SAFETY: we know that the `specifiers.specifiers().len() == 1`, so
			// unwrap `iter().next()` is safe.
				...
				(first_specifier.node(), first_specifier.trailing_separator())
			{
				if f.comments().has_comments(specifier.syntax()) {
					...
					write!(
						f,
						[maybe_space(should_insert_space_around_brackets), r_curly_token.format()]
					)
				}
			} else {
				write![f, [named_specifiers.format()]]
