...
impl FormatNodeRule<JsImportNamedClause> for FormatJsImportNamedClause {
	fn fmt_fields(&self, node: &JsImportNamedClause, f: &mut JsFormatter) -> FormatResult<()> {
		let JsImportNamedClauseFields { type_token, named_specifiers, from_token, source, assertion } =
			node.as_fields();
		...
		if specifiers.len() == 1 && !f.context().comments().is_suppressed(named_specifiers.syntax()) {
			...
			if let (Ok(specifier), Ok(separator)) = (first_specifier.node(), first_specifier.trailing_separator()) {
				if f.comments().has_comments(specifier.syntax()) {
					write!(f, [named_specifiers.format()])
				} else {
					let JsNamedImportSpecifiersFields { l_curly_token, specifiers: _, r_curly_token } =
					...
					write!(f, [maybe_space(should_insert_space_around_brackets), r_curly_token.format()])
				}
			} else {
				write![f, [named_specifiers.format()]]
			}
