...
use biome_js_syntax::{JsExportNamedFromClause, JsExportNamedFromClauseFields};
...
impl FormatNodeRule<JsExportNamedFromClause> for FormatJsExportNamedFromClause {
	fn fmt_fields(&self, node:&JsExportNamedFromClause, f:&mut JsFormatter) -> FormatResult<()> {
		...
		match specifiers.elements().next() {
			Some(AstSeparatedElement { node: Ok(node), trailing_separator: Ok(separator) })
				if specifiers.len() == 1 && !f.comments().has_comments(node.syntax()) =>
			{
				write!(f, [maybe_space(should_insert_space_around_brackets), node.format()])?;
		...
		write![
			f,
			[r_curly_token.format(), space(), from_token.format(), space(), source.format(),]
		]?;
