...
impl FormatRule<AnyJsExportDefaultDeclaration> for FormatAnyJsExportDefaultDeclaration {
	...
	fn fmt(&self, node: &AnyJsExportDefaultDeclaration, f: &mut JsFormatter) -> FormatResult<()> {
		match node {
			AnyJsExportDefaultDeclaration::JsClassExportDefaultDeclaration(node) => node.format().fmt(f),

			AnyJsExportDefaultDeclaration::JsFunctionExportDefaultDeclaration(node) => node.format().fmt(f),

			AnyJsExportDefaultDeclaration::TsDeclareFunctionExportDefaultDeclaration(node) => node.format().fmt(f),
