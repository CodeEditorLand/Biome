...
use biome_js_syntax::{AnyJsArrayElement, AnyJsExpression, JsArrayElementList, JsArrayExpressionFields};
...
impl FormatNodeRule<JsArrayExpression> for FormatJsArrayExpression {
	fn fmt_fields(&self, node: &JsArrayExpression, f: &mut JsFormatter) -> FormatResult<()> {
		let JsArrayExpressionFields { l_brack_token, elements, r_brack_token } = node.as_fields();
	...
	fn fmt_dangling_comments(&self, _: &JsArrayExpression, _: &mut JsFormatter) -> FormatResult<()> {
			...
			match element? {
				AnyJsArrayElement::AnyJsExpression(AnyJsExpression::JsArrayExpression(array)) => {
					let next_is_array_or_end = matches!(
						elements.peek(),
						None | Some(Ok(AnyJsArrayElement::AnyJsExpression(AnyJsExpression::JsArrayExpression(_))))
					);
				...
				AnyJsArrayElement::AnyJsExpression(AnyJsExpression::JsObjectExpression(object)) => {
					let next_is_object_or_empty = matches!(
						elements.peek(),
						None | Some(Ok(AnyJsArrayElement::AnyJsExpression(AnyJsExpression::JsObjectExpression(_))))
					);
