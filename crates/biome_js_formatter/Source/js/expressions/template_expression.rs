...
use crate::{js::lists::template_element_list::FormatJsTemplateElementListOptions, prelude::*};
...
impl FormatNodeRule<JsTemplateExpression> for FormatJsTemplateExpression {
	...
	fn needs_parentheses(&self, item:&JsTemplateExpression) -> bool { item.needs_parentheses() }
}
	...
	fn write_elements(&self, f:&mut JsFormatter) -> FormatResult<()> {
		match self {
			AnyJsTemplate::JsTemplateExpression(template) => {
				...
				let options = FormatJsTemplateElementListOptions { is_test_each_pattern };
