...
impl FormatNodeRule<JsStaticMemberExpression> for FormatJsStaticMemberExpression {
	...
	fn needs_parentheses(&self, item:&JsStaticMemberExpression) -> bool { item.needs_parentheses() }
}

#[derive(Debug, Copy, Clone)]
enum StaticMemberLikeLayout {
	/// Forces that there's no line break between the object, operator, and
	/// member
	NoBreak,

	/// Breaks the static member expression after the object if the whole
