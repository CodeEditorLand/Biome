...
use biome_js_syntax::{
	AnyJsAssignment, AnyJsAssignmentPattern, AnyJsComputedMember, AnyJsExpression, AnyJsName, JsAssignmentExpression,
	...
	fn fmt(&self, f: &mut Formatter<JsFormatContext>) -> FormatResult<()> {
		...
		match layout {
			StaticMemberLikeLayout::NoBreak => {
				let format_no_break =
					format_with(|f| write!(f, [self.operator_token().format(), self.member().format()]));
...
impl AnyJsStaticMemberLike {
	...
	fn operator_token(&self) -> SyntaxResult<JsSyntaxToken> {
		match self {
			AnyJsStaticMemberLike::JsStaticMemberExpression(expression) => expression.operator_token(),
	...
	fn layout(&self, is_member_chain: bool) -> SyntaxResult<StaticMemberLikeLayout> {
		...
		let is_nested = match &parent {
			Some(parent) => {
				if JsAssignmentExpression::can_cast(parent.kind()) || JsInitializerClause::can_cast(parent.kind()) {
				...
				AnyJsStaticMemberLike::can_cast(parent.kind()) || AnyJsComputedMember::can_cast(parent.kind())
			},
			None => false,
		};
		...
		let layout = match first_non_static_member_ancestor.and_then(AnyJsExpression::cast) {
			...
			Some(AnyJsExpression::JsAssignmentExpression(assignment)) => {
				if matches!(
					assignment.left()?,
					AnyJsAssignmentPattern::AnyJsAssignment(AnyJsAssignment::JsIdentifierAssignment(_))
				) {
					StaticMemberLikeLayout::BreakAfterObject
				} else {
					StaticMemberLikeLayout::NoBreak
