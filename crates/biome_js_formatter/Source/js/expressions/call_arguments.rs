...
use biome_js_syntax::{
	AnyJsCallArgument, AnyJsExpression, AnyJsFunctionBody, AnyJsLiteralExpression, AnyJsStatement, AnyTsReturnType,
	AnyTsType, JsBinaryExpressionFields, JsCallArgumentList, JsCallArguments, JsCallArgumentsFields, JsCallExpression,
	JsExpressionStatement, JsFunctionExpression, JsImportCallExpression, JsLanguage, JsLogicalExpressionFields,
	JsSyntaxKind, TsAsExpressionFields, TsSatisfiesExpressionFields,
};
...
impl FormatNodeRule<JsCallArguments> for FormatJsCallArguments {
	fn fmt_fields(&self, node: &JsCallArguments, f: &mut JsFormatter) -> FormatResult<()> {
		...
		let (is_commonjs_or_amd_call, is_test_call) = call_expression.as_ref().map_or((Ok(false), Ok(false)), |call| {
			...
			|| (is_test_call? && is_first_arg_string_literal_or_template)
		{
			return write!(
				...
				[
					...
					format_with(|f| {
						f.join_with(space())
							.entries(args.format_separated(",").with_trailing_separator(TrailingSeparator::Omit))
							.finish()
					}),
					r_paren_token.format()
				]
		...
		let arguments: Vec<_> = args
			...
			.enumerate()
			.map(|(index, element)| {
				let leading_lines = element.node().map_or(0, |node| get_lines_before(node.syntax()));
		...
		if let Some(group_layout) = arguments_grouped_layout(&args, f.comments()) {
			...
			write!(
				...
				[
					l_paren_token.format(),
					soft_block_indent(&format_once(|f| { write_arguments_multi_line(arguments.iter(), f) })),
					r_paren_token.format(),
				]
			)
		} else {
...
impl FormatCallArgument {
	...
	fn fmt_with_cache_mode(&self, cache_mode: FunctionBodyCacheMode, f: &mut JsFormatter) -> FormatResult<()> {
		match self {
			...
			FormatCallArgument::Default { element, is_last, .. } => {
				match element.node()? {
					...
					AnyJsCallArgument::AnyJsExpression(AnyJsExpression::JsArrowFunctionExpression(arrow)) => {
						write!(
							f,
...
fn write_grouped_arguments(
	...
	let grouped = arguments
		...
		.map(|(index, argument)| {
			...
			FormatGroupedArgument { argument, single_argument_list: last_index == 0, layout }.memoized()
		})
		.collect::<Vec<_>>();
	...
	let most_flat = {
		...
		let result = write!(
			...
			[
				l_paren,
				format_with(|f| { f.join_with(soft_line_break_or_space()).entries(grouped.iter()).finish() }),
				r_paren
			]
		);
...
fn with_token_tracking_disabled<F: FnOnce(&mut JsFormatter) -> R, R>(f: &mut JsFormatter, callback: F) -> R {
...
impl Format<JsFormatContext> for FormatGroupedArgument {
	fn fmt(&self, f: &mut Formatter<JsFormatContext>) -> FormatResult<()> {
		match self.layout {
			Some(GroupedCallArgumentLayout::GroupedFirstArgument) => {
				FormatGroupedFirstArgument { argument: &self.argument, is_only: self.single_argument_list }.fmt(f)
			},
			Some(GroupedCallArgumentLayout::GroupedLastArgument) => {
				FormatGroupedLastArgument { argument: &self.argument, is_only: self.single_argument_list }.fmt(f)
			},
...
fn arguments_grouped_layout(args: &JsCallArgumentList, comments: &JsComments) -> Option<GroupedCallArgumentLayout> {
...
fn should_group_first_argument(list: &JsCallArgumentList, comments: &JsComments) -> SyntaxResult<bool> {
	use AnyJsExpression::*;

	let mut iter = list.iter();
	match (iter.next(), iter.next()) {
		(Some(Ok(AnyJsCallArgument::AnyJsExpression(first))), Some(Ok(AnyJsCallArgument::AnyJsExpression(second))))
...
fn should_group_last_argument(list: &JsCallArgumentList, comments: &JsComments) -> SyntaxResult<bool> {
	use AnyJsExpression::*;
			...
			match last {
				JsArrayExpression(array) if list.len() > 1 => {
					...
					if list.len() == 2
						&& matches!(
							penultimate,
							Some(Ok(AnyJsCallArgument::AnyJsExpression(JsArrowFunctionExpression(_))))
						) {
						return Ok(false);
					}
...
fn is_relatively_short_argument(argument: AnyJsExpression) -> bool {
	match argument {
		AnyJsExpression::JsBinaryExpression(binary_expression) => {
			if let JsBinaryExpressionFields { left: Ok(left), operator_token: _, right: Ok(right) } =
		...
		AnyJsExpression::JsLogicalExpression(logical_expression) => {
			if let JsLogicalExpressionFields { left: Ok(left), operator_token: _, right: Ok(right) } =
		...
		AnyJsExpression::TsAsExpression(as_expression) => {
			if let TsAsExpressionFields { expression: Ok(expression), as_token: _, ty: Ok(annotation) } =
		...
		AnyJsExpression::TsSatisfiesExpression(as_expression) => {
			if let TsSatisfiesExpressionFields { expression: Ok(expression), satisfies_token: _, ty: Ok(annotation) } =
				as_expression.as_fields()
			{
				is_simple_ts_type(&annotation) && SimpleArgument::from(expression).is_simple()
			} else {
		...
		AnyJsExpression::AnyJsLiteralExpression(AnyJsLiteralExpression::JsRegexLiteralExpression(_)) => true,
...
fn can_group_expression_argument(
	...
	let result = match argument {
		JsObjectExpression(object_expression) => {
			!object_expression.members().is_empty() || comments.has_comments(object_expression.syntax())
		},

		JsArrayExpression(array_expression) => {
			!array_expression.elements().is_empty() || comments.has_comments(array_expression.syntax())
		},
		...
		TsAsExpression(as_expression) => can_group_expression_argument(&as_expression.expression()?, false, comments)?,
		...
		JsArrowFunctionExpression(arrow_function) => {
			...
			let can_group_body = match &body {
				...
				AnyJsFunctionBody::AnyJsExpression(JsCallExpression(_) | JsConditionalExpression(_))
					if !is_arrow_recursion =>
				{
					true
				},
				_ => false,
...
fn is_commonjs_or_amd_call(arguments: &JsCallArguments, call: &JsCallExpression) -> SyntaxResult<bool> {
	...
	let result = match reference.name()?.text() {
		"require" => {
			...
			match args.len() {
				...
				1 => matches!(
					args.first(),
					Some(Ok(AnyJsCallArgument::AnyJsExpression(AnyJsExpression::AnyJsLiteralExpression(
		...
		"define" => {
			...
			if in_statement {
				...
				match args.len() {
					...
					2 => matches!(
						args.first(),
						Some(Ok(AnyJsCallArgument::AnyJsExpression(AnyJsExpression::JsArrayExpression(_))))
					),
					3 => {
...
fn is_multiline_template_only_args(arguments: &JsCallArguments) -> bool {
	...
	match args.first() {
		Some(Ok(AnyJsCallArgument::AnyJsExpression(AnyJsExpression::JsTemplateExpression(template))))
			if args.len() == 1 =>
		{
...
fn is_react_hook_with_deps_array(arguments: &JsCallArguments, comments: &JsComments) -> bool {
	...
	match (args.next(), args.next()) {
		(
			...
			if !callback.parameters().is_ok_and(|parameters| parameters.is_empty()) {
				return false;
			}
...
fn is_function_composition_args(arguments: &JsCallArguments) -> bool {
	...
	for arg in args.iter().flatten() {
		use AnyJsExpression::*;
		match arg {
			AnyJsCallArgument::AnyJsExpression(JsFunctionExpression(_) | JsArrowFunctionExpression(_)) => {
