...
use crate::{
	...
	js::{
		bindings::parameters::has_only_simple_parameters,
		declarations::function_declaration::FormatFunctionOptions,
		expressions::arrow_function_expression::{
			is_multiline_template_starting_on_same_line,
			FormatJsArrowFunctionExpressionOptions,
		},
		lists::array_element_list::can_concisely_print_array_list,
	},
	prelude::*,
	utils::{
		function_body::FunctionBodyCacheMode,
		is_long_curried_call,
		member_chain::SimpleArgument,
...
impl FormatNodeRule<JsCallArguments> for FormatJsCallArguments {
	fn fmt_fields(&self, node:&JsCallArguments, f:&mut JsFormatter) -> FormatResult<()> {
		let JsCallArgumentsFields { l_paren_token, args, r_paren_token } = node.as_fields();
		...
		let arguments:Vec<_> = args
			...
			.map(|(index, element)| {
				let leading_lines =
					element.node().map_or(0, |node| get_lines_before(node.syntax()));
...
enum FormatCallArgument {
	...
	/// The argument has been formatted because a caller inspected if it
	/// [Self::will_break].
	///
	/// Allows to re-use the formatted output rather than having to call into
...
impl FormatCallArgument {
	/// Returns `true` if this argument contains any content that forces a group
	/// to [`break`](FormatElements::will_break).
	fn will_break(&mut self, f:&mut JsFormatter) -> bool {
		match &self {
			FormatCallArgument::Default { element, leading_lines, .. } => {
			...
			FormatCallArgument::Inspected { content: Ok(Some(result)), .. } => result.will_break(),
	...
	/// # Panics
	///
	/// If [`cache_function_body`](Self::cache_function_body) or
	/// [`will_break`](Self::will_break) has been called on this argument
	/// before.
	...
	fn fmt_with_cache_mode(
		...
		match self {
			...
			FormatCallArgument::Inspected { content, .. } => {
				match content.clone()? {
					Some(element) => {
			...
			FormatCallArgument::Default { element, is_last, .. } => {
				match element.node()? {
...
/// Writes the function arguments, and groups the first or last argument
/// depending on `group_layout`.
fn write_grouped_arguments(
	...
	let grouped_breaks = {
		...
		// if any of the not grouped elements break, then fall back to the
		// variant where all arguments are printed in expanded mode.
		...
		match grouped_arg.element().node()? {
			...
			_ => {
				// Node doesn't have a function body or its a function that
				// doesn't get re-formatted.
			},
		}
	...
	// We now cache them the delimiters tokens. This is needed because
	// `[biome_formatter::best_fitting]` will try to print each version first
	// tokens on the left
	...
	// Now reformat the first or last argument if they happen to be a function
	// or arrow function expression. Function and arrow function expression
	// apply a custom formatting that removes soft line breaks from the
	// parameters, type parameters, and return type annotation.
	//
	// This implementation caches the function body of the "normal" formatted
	// function or arrow function expression to avoid quadratic complexity if
	// the functions' body contains another call expression with an arrow or
	// function expression as first or last argument.
	...
	let most_flat = {
		...
		// Turns out, using the grouped layout isn't a good fit because some
		// parameters of the grouped function or arrow expression break. In
		// that case, fall back to the all args expanded formatting.
		// This back tracking is required because testing if the grouped
		// argument breaks would also return `true` if any content of the
		// function body breaks. But, as far as this is concerned, it's only
		// interested if any content in the signature breaks.
	...
	// Write the second variant that forces the group of the first/last argument
	// to expand.
	let middle_variant = {
		let mut buffer = VecBuffer::new(f.state_mut());
...
/// Helper for formatting the first grouped argument (see
/// [should_group_first_argument]).
...
impl Format<JsFormatContext> for FormatGroupedFirstArgument<'_> {
	fn fmt(&self, f:&mut Formatter<JsFormatContext>) -> FormatResult<()> {
		...
		match element.node()? {
			// Call the arrow function formatting but explicitly passes the call
			// argument layout down so that the arrow function formatting
			// removes any soft line breaks between parameters and the return
			// type.
			AnyJsCallArgument::AnyJsExpression(JsArrowFunctionExpression(arrow)) => {
				with_token_tracking_disabled(f, |f| {
					...
					match element.trailing_separator()? {
						...
						// The separator is added inside of the arrow function
						// formatting
						Some(separator) => {
							if self.is_only {
								write!(f, [format_removed(separator)])?;
			...
			// For all other nodes, use the normal formatting (which already has
			// been cached)
...
/// Helper for formatting the last grouped argument (see
...
impl Format<JsFormatContext> for FormatGroupedLastArgument<'_> {
	fn fmt(&self, f:&mut Formatter<JsFormatContext>) -> FormatResult<()> {
		...
		// For function and arrow expressions, re-format the node and pass the
		// argument that it is the last grouped argument. This changes the
		// formatting of parameters, type parameters, and return types
...
/// Disable the token tracking because it is necessary to format function/arrow
...
fn function_has_only_simple_parameters(expression:&JsFunctionExpression) -> bool {
	expression
		.parameters()
		.map_or(true, |parameters| has_only_simple_parameters(&parameters, false))
}

/// Helper for formatting a grouped call argument (see
/// [should_group_first_argument] and [should_group_last_argument]).
...
fn can_group_expression_argument(
	...
	let result = match argument {
		...
		JsArrowFunctionExpression(arrow_function) => {
			...
			// }
			let can_group_type =
				return_type_annotation.and_then(|rty| rty.ty().ok()).map_or(true, |any_type| {
					match any_type {
						AnyTsReturnType::AnyTsType(AnyTsType::TsReferenceType(_)) => {
							match &body {
								AnyJsFunctionBody::JsFunctionBody(body) => {
									body.statements().iter().any(|statement| {
										match statement {
											AnyJsStatement::JsEmptyStatement(s) => {
												// When the body contains an empty statement,
												// comments in the body will get
												// attached to that statement rather than
												// the body itself, so they need to be checked for
												// comments as well to ensure that the
												// body is still considered
												// groupable when those empty statements are removed
												// by the printer.
...
fn is_commonjs_or_amd_call(
	...
	let result = match reference.name()?.text() {
		"require" => {
			...
			match args.len() {
				0 => false,
				// `require` can be called with any expression that resolves to
				// a string. This check is only an escape hatch to allow a
				// complex expression to break rather than group onto the
				// previous line.
				//
...
/// Returns `true` if `arguments` contains a single [multiline template literal
/// argument that starts on its own
/// ](is_multiline_template_starting_on_same_line).
...
fn is_react_hook_with_deps_array(arguments:&JsCallArguments, comments:&JsComments) -> bool {
	...
	match (args.next(), args.next()) {
		(
			...
			if !callback.parameters().map_or(false, |parameters| parameters.is_empty()) {
				return false;
			}
...
/// Tests if a call has multiple anonymous function like (arrow or function
