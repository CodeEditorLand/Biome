...
fn write_grouped_arguments(
	...
	// Now reformat the first or last argument if they happen to be a function or
	// arrow function expression. Function and arrow function expression apply a
	// custom formatting that removes soft line breaks from the parameters,
	// type parameters, and return type annotation.
	//
	// This implementation caches the function body of the "normal" formatted
	// function or arrow function expression to avoid quadratic complexity if the
	// functions' body contains another call expression with an arrow or function
	// expression as first or last argument.
	...
	let most_flat = {
		...
		// Turns out, using the grouped layout isn't a good fit because some parameters
		// of the grouped function or arrow expression break. In that case, fall back
		// to the all args expanded formatting.
		// This back tracking is required because testing if the grouped argument breaks
		// would also return `true` if any content of the function body breaks. But,
		// as far as this is concerned, it's only interested if any content in the
		// signature breaks.
	...
	// Write the second variant that forces the group of the first/last argument to
	// expand.
	let middle_variant = {
		let mut buffer = VecBuffer::new(f.state_mut());
...
impl Format<JsFormatContext> for FormatGroupedFirstArgument<'_> {
	fn fmt(&self, f:&mut Formatter<JsFormatContext>) -> FormatResult<()> {
		...
		match element.node()? {
			// Call the arrow function formatting but explicitly passes the call argument layout
			// down so that the arrow function formatting removes any soft line breaks between
...
impl Format<JsFormatContext> for FormatGroupedLastArgument<'_> {
	fn fmt(&self, f:&mut Formatter<JsFormatContext>) -> FormatResult<()> {
		...
		// For function and arrow expressions, re-format the node and pass the argument
		// that it is the last grouped argument. This changes the formatting of
		// parameters, type parameters, and return types to remove any soft line
		// breaks.
		match element.node()? {
