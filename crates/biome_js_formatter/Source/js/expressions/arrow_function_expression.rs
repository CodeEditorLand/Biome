...
use crate::{
	...
	js::{
		...
		expressions::call_arguments::GroupedCallArgumentLayout,
	},
	prelude::*,
	utils::{
		function_body::{FormatMaybeCachedFunctionBody, FunctionBodyCacheMode},
...
impl FormatNodeRule<JsArrowFunctionExpression> for FormatJsArrowFunctionExpression {
	fn fmt_fields(&self, node:&JsArrowFunctionExpression, f:&mut JsFormatter) -> FormatResult<()> {
		let layout =
		...
		match layout {
			...
			ArrowFunctionLayout::Single(arrow) => {
				...
				let format_body =
					FormatMaybeCachedFunctionBody { body:&body, mode:self.options.body_cache_mode };

				// With arrays, arrow selfs and objects, they have a natural
				// line breaking strategy: Arrays and objects become blocks:
				//
				//    [
				//      100000,
				//      200000,
				...
				// do not have a soft line break after the arrow because the
				// body is going to get broken anyways.
...
/// Writes the arrow function type parameters, parameters, and return type
/// annotation.
///
/// Formats the parameters and return type annotation without any soft line
/// breaks if `is_first_or_last_call_argument` is `true` so that the parameters
/// and return type are kept on the same line.
///
/// # Errors
///
/// Returns [`FormatError::PoorLayout`] if `is_first_or_last_call_argument` is
/// `true` but the parameters or return type annotation contain any content that
/// forces a [*group to break](FormatElements::will_break).
///
...
fn should_add_parens(body:&AnyJsFunctionBody) -> bool {
	// Add parentheses to avoid confusion between `a => b ? c : d` and `a <= b ?
	// c : d` but only if the body isn't an object/function or class expression
	// because parentheses are always required in that case and added by the
...
fn has_rest_object_or_array_parameter(parameters:&AnyJsArrowFunctionParameters) -> bool {
	match parameters {
		...
		AnyJsArrowFunctionParameters::JsParameters(parameters) => {
			parameters.items().iter().flatten().any(|parameter| {
				match parameter {
...
/// Returns `true` if parentheses can be safely avoided and the
/// `arrow_parentheses` formatter option allows it
...
enum ArrowFunctionLayout {
	...
	///
	/// An arrow function is part of the chain when it is the body of the parent
	/// arrow function.
	///
...
struct ArrowChain {
	...
	/// The arrow functions in the chain that are neither the first nor the
	/// last. Empty for chains consisting only of two arrow functions.
...
impl Format<JsFormatContext> for ArrowChain {
	fn fmt(&self, f:&mut Formatter<JsFormatContext>) -> FormatResult<()> {
		let ArrowChain { head, tail, expand_signatures, .. } = self;
		...
		// With arrays, objects, sequence expressions, and block function
		// bodies, the opening brace gives a convenient boundary to insert a
		// line break, allowing that token to live immediately after the last
		// arrow token and save a line from being printed with just the
		// punctuation.
		//
		// (foo) => (bar) => [a, b]
		//
				...
				for arrow in self.arrows() {
					...
					let formatted_signature = format_with(|f:&mut JsFormatter| {
						if should_format_comments {
							// A grouped layout implies that the arrow chain
							// is trying to be rendered
							// in a condensend, single-line format (at least
							// the signatures, not necessarily the
							// body). In that case, we _need_ to prevent the
							// leading comments from inserting line
							// breaks. But if it's _not_ a grouped layout,
							// then we want to _force_ the line break so
							// that the leading comments
							// don't inadvertently end up on the previous
							// line after the fat arrow.
					...
					// Arrow chains indent a second level for every item other
					// than the first:   (a) =>
					//     (b) =>
					//     (c) =>
					//       0
					// Because the chain is printed as a flat list, each entry
					// needs to set its own indention. This ensures that
					// the first item keeps the same level as the
					// surrounding content, and then each subsequent item has
					...
					// The arrow of the tail is formatted outside of the group
					// to ensure it never breaks from the body
					if arrow != tail {
		...
		let format_tail_body_inner = format_with(|f| {
			...
			// Ensure that the parens of sequence expressions end up on their
			// own line if the body breaks
			if matches!(
				tail_body,
			...
			// Format the trailing comments of all arrow function EXCEPT the
			// first one because the comments of the head get formatted as
			// part of the `FormatJsArrowFunctionExpression` call.
		...
		let format_tail_body = format_with(|f| {
			// if it's inside a JSXExpression (e.g. an attribute) we should
			// align the expression's closing } with the line with the opening
			// {.
...
impl ArrowFunctionLayout {
	/// Determines the layout for the passed arrow function. See
	/// [ArrowFunctionLayout] for a description of the different layouts.
	fn for_arrow(
		arrow:JsArrowFunctionExpression,
		comments:&JsComments,
		options:&FormatJsArrowFunctionExpressionOptions,
			...
			match current.body()? {
				...
				_ => {
					break match head {
						...
						Some(head) => {
							ArrowFunctionLayout::Chain(ArrowChain {
								head,
								middle,
								tail:current,
...
/// Returns `true` if the template contains any new lines inside of its text
/// chunks.
fn template_literal_contains_new_line(template:&JsTemplateExpression) -> bool {
	template.elements().iter().any(|element| {
		match element {
			AnyJsTemplateElement::JsTemplateChunkElement(chunk) => {
				chunk.template_chunk_token().map_or(false, |chunk| chunk.text().contains('\n'))
			},
...
/// Returns `true` for a template that starts on the same line as the previous
/// token and contains a line break.
///
///
/// # Examples
///
...
/// ```
///
/// Returns `true` because the template starts on the same line as the `+` token
/// and its text contains a line break.
///
/// ```javascript
/// "test" + `no line break`
/// ```
...
/// ```
///
/// Returns `false` because the template isn't on the same line as the '+'
/// token.
