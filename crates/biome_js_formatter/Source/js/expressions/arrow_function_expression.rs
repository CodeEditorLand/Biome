...
use biome_formatter::{CstFormatContext, FormatRuleWithOptions, RemoveSoftLinesBuffer, format_args, write};
use biome_js_syntax::expression_left_side::AnyJsExpressionLeftSide;
use biome_js_syntax::parentheses::NeedsParentheses;
use biome_js_syntax::{
	AnyJsArrowFunctionParameters, AnyJsBindingPattern, AnyJsExpression, AnyJsFormalParameter, AnyJsFunctionBody,
	AnyJsParameter, AnyJsTemplateElement, JsArrowFunctionExpression, JsFormalParameter, JsSyntaxKind,
	JsTemplateExpression, is_test_call_argument,
};
...
impl FormatNodeRule<JsArrowFunctionExpression> for FormatJsArrowFunctionExpression {
	fn fmt_fields(&self, node: &JsArrowFunctionExpression, f: &mut JsFormatter) -> FormatResult<()> {
		let layout = ArrowFunctionLayout::for_arrow(node.clone(), f.context().comments(), &self.options)?;
		...
		match layout {
			...
			ArrowFunctionLayout::Single(arrow) => {
				...
				let format_body = FormatMaybeCachedFunctionBody { body: &body, mode: self.options.body_cache_mode };
				...
				let body_has_soft_line_break = match &body {
					JsFunctionBody(_)
					| AnyJsExpression(JsArrowFunctionExpression(_) | JsArrayExpression(_) | JsObjectExpression(_)) => {
					...
					AnyJsExpression(JsSequenceExpression(sequence)) => {
						...
						return write!(
							...
							[group(&format_args![
								formatted_signature,
								group(&format_args![space(), text("("), soft_block_indent(&format_body), text(")")])
							])]
						);
					},
					_ => false,
				};
				let body_is_condition_type = matches!(body, AnyJsExpression(JsConditionalExpression(_)));
						...
						write!(
							...
							[
								...
								group(&format_args![
									...
									is_last_call_arg.then_some(format_args![FormatTrailingCommas::All,]),
	...
	fn fmt_dangling_comments(&self, _: &JsArrowFunctionExpression, _: &mut JsFormatter) -> FormatResult<()> {
		...
		let formatted_parameters = format_with(|f: &mut JsFormatter| {
			...
			match arrow.parameters()? {
				AnyJsArrowFunctionParameters::AnyJsBinding(binding) => {
					let should_hug = is_test_call_argument(arrow.syntax())? || is_first_or_last_call_argument;
					...
					if should_hug || parentheses_not_needed {
						...
						write!(
							f,
							[&soft_block_indent(&format_args![binding.format(), FormatTrailingCommas::All])]
						)?
					}
...
fn should_add_parens(body: &AnyJsFunctionBody) -> bool {
	...
	match &body {
		AnyJsFunctionBody::AnyJsExpression(expression @ AnyJsExpression::JsConditionalExpression(_)) => {
...
fn has_rest_object_or_array_parameter(parameters: &AnyJsArrowFunctionParameters) -> bool {
	match parameters {
		...
		AnyJsArrowFunctionParameters::JsParameters(parameters) => {
			parameters.items().iter().flatten().any(|parameter| match parameter {
				AnyJsParameter::AnyJsFormalParameter(AnyJsFormalParameter::JsFormalParameter(parameter)) => {
					matches!(
						parameter.binding(),
...
impl Format<JsFormatContext> for ArrowChain {
	fn fmt(&self, f: &mut Formatter<JsFormatContext>) -> FormatResult<()> {
		...
		let format_arrow_signatures = format_with(|f| {
			let join_signatures = format_with(|f: &mut JsFormatter| {
				...
				for arrow in self.arrows() {
					...
					let should_format_comments =
						!is_first_in_chain && f.context().comments().has_leading_comments(arrow.syntax());
					...
					let formatted_signature = format_with(|f: &mut JsFormatter| {
						if should_format_comments {
							...
							if is_grouped_call_arg_layout {
								...
								write!(f, [soft_line_break_or_space(), format_leading_comments(arrow.syntax())])?;
							}
						}
		...
		let format_tail_body_inner = format_with(|f| {
			let format_tail_body =
				FormatMaybeCachedFunctionBody { body: &tail_body, mode: self.options.body_cache_mode };
			...
			if matches!(
				...
				if has_comment {
					...
					write!(
						f,
						[group(&format_args![text("("), soft_block_indent(&format_tail_body), text(")")])]
					)?;
				}
			} else {
		...
		let format_tail_body = format_with(|f| {
			...
			let should_add_soft_line = matches!(
				head_parent.kind(),
				Some(JsSyntaxKind::JSX_EXPRESSION_CHILD | JsSyntaxKind::JSX_EXPRESSION_ATTRIBUTE_VALUE)
			);
...
impl ArrowFunctionLayout {
	...
	fn for_arrow(
		...
		let result = loop {
			match current.body()? {
				AnyJsFunctionBody::AnyJsExpression(AnyJsExpression::JsArrowFunctionExpression(next))
					if matches!(
...
fn template_literal_contains_new_line(template: &JsTemplateExpression) -> bool {
	template.elements().iter().any(|element| match element {
		AnyJsTemplateElement::JsTemplateChunkElement(chunk) => {
			chunk.template_chunk_token().is_ok_and(|chunk| chunk.text().contains('\n'))
		},
...
mod tests {
	...
	fn needs_parentheses() {
		...
		assert_needs_parentheses!("<Function>(a => a)", JsArrowFunctionExpression, JsFileSource::ts());
