	...
	fn fmt_fields(&self, node:&JsArrowFunctionExpression, f:&mut JsFormatter) -> FormatResult<()> {
		...
		match layout {
			...
			ArrowFunctionLayout::Single(arrow) => {
				...
				// With arrays, arrow selfs and objects, they have a natural line breaking
				// strategy: Arrays and objects become blocks:
				//
				//    [
				//      100000,
				//      200000,
...
fn should_add_parens(body:&AnyJsFunctionBody) -> bool {
	// Add parentheses to avoid confusion between `a => b ? c : d` and `a <= b ? c :
	// d` but only if the body isn't an object/function or class expression because
	// parentheses are always required in that case and added by the object
	// expression itself
	match &body {
...
impl Format<JsFormatContext> for ArrowChain {
	fn fmt(&self, f:&mut Formatter<JsFormatContext>) -> FormatResult<()> {
				...
				for arrow in self.arrows() {
					...
					let formatted_signature = format_with(|f:&mut JsFormatter| {
						if should_format_comments {
							// A grouped layout implies that the arrow chain is trying to be
							// rendered in a condensend, single-line format (at least the
							// signatures, not necessarily the body). In that case, we _need_
							// to prevent the leading comments from inserting line breaks.
							// But if it's _not_ a grouped layout, then we want to _force_
							// the line break so that the leading comments
		...
		let format_tail_body_inner = format_with(|f| {
			...
			// Ensure that the parens of sequence expressions end up on their own line if
			// the body breaks
			if matches!(
				tail_body,
			...
			// Format the trailing comments of all arrow function EXCEPT the first one
			// because the comments of the head get formatted as part of the
			// `FormatJsArrowFunctionExpression` call.
		...
		let format_tail_body = format_with(|f| {
			// if it's inside a JSXExpression (e.g. an attribute) we should align the
			// expression's closing } with the line with the opening {.
