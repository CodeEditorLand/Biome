...
use biome_js_syntax::{parentheses::NeedsParentheses, JsYieldExpression, JsYieldExpressionFields};
...
impl FormatNodeRule<JsYieldExpression> for FormatJsYieldExpression {
	fn fmt_fields(&self, node:&JsYieldExpression, f:&mut JsFormatter) -> FormatResult<()> {
		let JsYieldExpressionFields { yield_token, argument } = node.as_fields();

		write![f, [yield_token.format(), argument.format()]]
	}

	fn needs_parentheses(&self, item:&JsYieldExpression) -> bool { item.needs_parentheses() }
}
...
mod tests {
	...
	fn needs_parentheses() {
		assert_needs_parentheses!("function* test() { (yield a)`template` }", JsYieldExpression);
		...
		assert_needs_parentheses!("function* test() { (yield a) && b }", JsYieldExpression);
		assert_needs_parentheses!("function* test() { (yield a) + b }", JsYieldExpression);
		assert_needs_parentheses!("function* test() { (yield a) instanceof b }", JsYieldExpression);
		...
		assert_needs_parentheses!("function* test() { (yield b) as number }", JsYieldExpression);
		assert_needs_parentheses!("function* test() { (yield b)! }", JsYieldExpression);
