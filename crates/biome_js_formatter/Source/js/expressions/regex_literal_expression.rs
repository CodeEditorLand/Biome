...
impl FormatNodeRule<JsRegexLiteralExpression> for FormatJsRegexLiteralExpression {
	fn fmt_fields(&self, node:&JsRegexLiteralExpression, f:&mut JsFormatter) -> FormatResult<()> {
		...
		// find end slash, so we could split our regex literal to two part `body`:
		// raw_string[0..end_slash_pos + 1] and `flags`: raw_string[end_slash_pos + 1..] reference https://tc39.es/ecma262/#prod-RegularExpressionLiteral
	...
	fn needs_parentheses(&self, item:&JsRegexLiteralExpression) -> bool { item.needs_parentheses() }
}
