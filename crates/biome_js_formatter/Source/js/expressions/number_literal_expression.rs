...
impl FormatNodeRule<JsNumberLiteralExpression> for FormatJsNumberLiteralExpression {
	fn fmt_fields(&self, node: &JsNumberLiteralExpression, f: &mut JsFormatter) -> FormatResult<()> {
		let JsNumberLiteralExpressionFields { value_token } = node.as_fields();
		format_number_token(&value_token?, NumberFormatOptions::default().keep_one_trailing_decimal_zero()).fmt(f)
	}
