...
use biome_js_syntax::{JsForStatement, JsForStatementFields};
...
impl FormatNodeRule<JsForStatement> for FormatJsForStatement {
	fn fmt_fields(&self, node:&JsForStatement, f:&mut JsFormatter) -> FormatResult<()> {
		...
		// Move dangling trivia between the `for /* this */ (` to the top of the
		// `for` and add a line break after.
		let comments = f.context().comments();
		let dangling_comments = comments.dangling_comments(node.syntax());
		if !dangling_comments.is_empty() {
			write!(f, [format_dangling_comments(node.syntax()), soft_line_break_or_space()])?;
		}
