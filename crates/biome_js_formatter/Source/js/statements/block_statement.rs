...
impl FormatNodeRule<JsBlockStatement> for FormatJsBlockStatement {
	fn fmt_fields(&self, node:&JsBlockStatement, f:&mut JsFormatter) -> FormatResult<()> {
		let JsBlockStatementFields { l_curly_token, statements, r_curly_token } = node.as_fields();
		...
		if is_empty_block(node, comments) {
			...
			for stmt in
				statements.iter().filter_map(|stmt| JsEmptyStatement::cast(stmt.into_syntax()))
			{
...
fn is_empty_block(block:&JsBlockStatement, comments:&JsComments) -> bool {
	// add extra branch to avoid formatting the same code twice and generating
	// different code, here is an example:
	// ```js
	//     try
	// /* missing comment */
	// {;}
	// finally {}
	// ```
	// if we don't add the extra branch, this function will return false,
	// because  `block.statement` has one empty statement, and would be
	// formatted as : ```js
	//     try
	// /* missing comment */
	// {}
	// finally {}
	// ```
	// for the second time, the function would return true, because the block is
	// empty and `parent.syntax.kind` is  `JS_TRY_FINALLY_STATEMENT`, which
	// would hit the branch `Some(_) => true`, finally the code would be
	// formatted as: ```js
	// try
	// missing comment
	// {
...
// * non empty block with comments (trailing comments on {, or leading comments
//   on })
fn is_non_collapsible(block:&JsBlockStatement) -> bool {
	...
	match parent.kind() {
		...
		// prettier collapse the catch block when it don't have `finalizer`,
		// insert a new line when it has `finalizer`
		Some(JsSyntaxKind::JS_CATCH_CLAUSE) => {
			// SAFETY: since parent node have `Some(kind)`, this must not be
			// `None`
