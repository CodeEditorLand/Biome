...
fn is_empty_block(block:&JsBlockStatement, comments:&JsComments) -> bool {
	...
	// finally {}
	// ```
	// if we don't add the extra branch, this function will return false, because
	// `block.statement` has one empty statement, and would be formatted as :
	// ```js
	//     try
	// /* missing comment */
	// {}
	...
	// empty and `parent.syntax.kind` is  `JS_TRY_FINALLY_STATEMENT`, which would
	// hit the branch `Some(_) => true`, finally the code would be formatted as:
	// ```js
	// try
	// missing comment
	// {
...
fn is_non_collapsible(block:&JsBlockStatement) -> bool {
	...
	match parent.kind() {
		...
		// prettier collapse the catch block when it don't have `finalizer`, insert a new line when
		// it has `finalizer`
