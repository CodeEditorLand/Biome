...
impl FormatNodeRule<JsExpressionStatement> for FormatJsExpressionStatement {
	fn fmt_node(&self, node:&JsExpressionStatement, f:&mut JsFormatter) -> FormatResult<()> {
		...
		let is_after_bogus = f.elements().start_tag(TagKind::Verbatim).map_or(false, |signal| {
			match signal {
	...
	fn fmt_fields(&self, node:&JsExpressionStatement, f:&mut JsFormatter) -> FormatResult<()> {
		let JsExpressionStatementFields { expression, semicolon_token } = node.as_fields();
...
/// Returns `true` if a semicolon is required to keep the semantics of the
/// program.
///
/// Semicolons are optional in most places in JavaScript, but they are sometimes
/// required. Generally, semicolons are necessary if an identifier + start of a
/// statement may form a valid expression. For example:
///
/// ```javascript
/// a
/// ["b"]
/// ```
///
/// The above can either be the computed member expression `a["b"]` or the
/// identifier `a` followed by an expression statement `["b"]`.
///
		...
		let needs_semi = match &current {
			AnyJsExpressionLeftSide::AnyJsExpression(expression) => {
				match expression {
					...
					| AnyJsLiteralExpression(
						self::AnyJsLiteralExpression::JsRegexLiteralExpression(_),
					)
