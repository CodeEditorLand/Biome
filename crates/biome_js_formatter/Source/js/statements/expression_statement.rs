...
use biome_js_syntax::{
	AnyJsAssignment, AnyJsAssignmentPattern, AnyJsExpression, JsExpressionStatement, JsSyntaxKind, JsUnaryOperator,
};
...
impl FormatNodeRule<JsExpressionStatement> for FormatJsExpressionStatement {
	...
	fn fmt_dangling_comments(&self, _: &JsExpressionStatement, _: &mut JsFormatter) -> FormatResult<()> {
	...
	while let Some(current) = expression.take() {
		let needs_semi = match &current {
			AnyJsExpressionLeftSide::AnyJsExpression(expression) => match expression {
				...
				| AnyJsLiteralExpression(self::AnyJsLiteralExpression::JsRegexLiteralExpression(_))
				...
				JsUnaryExpression(unary) => {
					matches!(unary.operator(), Ok(JsUnaryOperator::Plus | JsUnaryOperator::Minus))
				},
			...
			AnyJsExpressionLeftSide::AnyJsAssignmentPattern(assignment) => matches!(
				...
				AnyJsAssignmentPattern::JsArrayAssignmentPattern(_)
					| AnyJsAssignmentPattern::AnyJsAssignment(
						AnyJsAssignment::JsParenthesizedAssignment(_) | AnyJsAssignment::TsTypeAssertionAssignment(_)
					)
			),
		};
