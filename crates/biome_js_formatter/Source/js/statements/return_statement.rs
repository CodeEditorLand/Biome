...
use crate::{
	prelude::*,
	utils::{FormatOptionalSemicolon, FormatStatementSemicolon},
};
...
impl FormatNodeRule<JsReturnStatement> for FormatJsReturnStatement {
	...
	fn fmt_dangling_comments(&self, _:&JsReturnStatement, _:&mut JsFormatter) -> FormatResult<()> {
...
/// Tests if the passed in argument has any leading comments. This is the case
/// if
/// * the argument has any leading comment
/// * the argument's left side has any leading comment (see
///   [get_expression_left_side]).
///
/// Traversing the left nodes is necessary in case the first node is
/// parenthesized because parentheses will be removed (and be re-added by the
/// return statement, but only if the argument breaks)
