...
use biome_js_syntax::{AnyJsStatement, JsDoWhileStatement, JsDoWhileStatementFields};

use crate::{
	prelude::*,
	utils::{FormatStatementBody, FormatStatementSemicolon},
};
...
impl FormatNodeRule<JsDoWhileStatement> for FormatJsDoWhileStatement {
	fn fmt_fields(&self, node:&JsDoWhileStatement, f:&mut JsFormatter) -> FormatResult<()> {
		...
		write!(f, [group(&format_args![do_token.format(), FormatStatementBody::new(&body)])])?;
