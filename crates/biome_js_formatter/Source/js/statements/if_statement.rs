...
impl FormatNodeRule<JsIfStatement> for FormatJsIfStatement {
	fn fmt_fields(&self, node: &JsIfStatement, f: &mut JsFormatter) -> FormatResult<()> {
		...
		let JsIfStatementFields { if_token, l_paren_token, test, r_paren_token, consequent, else_clause } =
			node.as_fields();
		...
		if let Some(else_clause) = else_clause {
			...
			let dangling_line_comment = dangling_comments.last().is_some_and(|comment| comment.kind().is_line());
			...
			let else_on_same_line =
				matches!(consequent, JsBlockStatement(_)) && !trailing_line_comment && !dangling_line_comment;
