...
use biome_js_syntax::{JsGetterObjectMember, JsGetterObjectMemberFields};
...
impl FormatNodeRule<JsGetterObjectMember> for FormatJsGetterObjectMember {
	fn fmt_fields(&self, node: &JsGetterObjectMember, f: &mut JsFormatter) -> FormatResult<()> {
		let JsGetterObjectMemberFields { get_token, name, l_paren_token, r_paren_token, return_type, body } =
			node.as_fields();
