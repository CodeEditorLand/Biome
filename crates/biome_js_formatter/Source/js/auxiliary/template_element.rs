...
use crate::{
	context::TabWidth,
	js::{
		expressions::array_expression::FormatJsArrayExpressionOptions,
...
enum TemplateElementLayout {
	/// Tries to format the expression on a single line regardless of the print
	/// width.
	SingleLine,

	/// Tries to format the expression on a single line but may break the
	/// expression if the line otherwise exceeds the print width.
	Fit,
}
...
impl FormatNodeRule<JsTemplateElement> for FormatJsTemplateElement {
	fn fmt_fields(&self, node:&JsTemplateElement, formatter:&mut JsFormatter) -> FormatResult<()> {
...
impl Format<JsFormatContext> for FormatTemplateElement {
	fn fmt(&self, f:&mut JsFormatter) -> FormatResult<()> {
				...
				AnyTemplateElement::JsTemplateElement(template) => {
					match template.expression()? {
						AnyJsExpression::JsArrayExpression(expression) => {
							let option = FormatJsArrayExpressionOptions { is_force_flat_mode:true };
		...
		let format_inner = format_with(|f:&mut JsFormatter| {
			match layout {
				TemplateElementLayout::SingleLine => {
				...
				TemplateElementLayout::Fit => {
					...
					// It's preferred to break after/before `${` and `}` rather
					// than breaking in the middle of some expressions.
					let indent = f.context().comments().has_comments(&self.element.inner_syntax()?)
						|| matches!(
							expression,
							Some(
								JsStaticMemberExpression(_)
									...
									| JsSequenceExpression(_) | TsAsExpression(_)
									| TsSatisfiesExpression(_) | JsBinaryExpression(_)
									| JsLogicalExpression(_) | JsInstanceofExpression(_)
									| JsInExpression(_) | JsIdentifierExpression(_)
							)
						);
...
where
	...
	// Adds as many nested `indent` elements until it reaches the desired
	// indention level.
