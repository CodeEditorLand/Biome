...
use biome_js_syntax::{AnyJsExpression, JsSyntaxNode, JsSyntaxToken, JsTemplateElement, TsTemplateElement};
	...
	fn fmt(&self, f: &mut JsFormatter) -> FormatResult<()> {
			...
			TemplateElementLayout::Fit => {
					...
					|| matches!(
						...
						Some(
							JsStaticMemberExpression(_)
								...
								| TsAsExpression(_) | TsSatisfiesExpression(_)
								| JsBinaryExpression(_) | JsLogicalExpression(_)
				...
				match &interned_expression {
					Some(element) if indent => {
						write!(f, [soft_block_indent(&format_with(|f| f.write_element(element.clone())))])
					},
		...
		let format_indented = format_with(|f: &mut JsFormatter| {
			if self.options.after_new_line {
				...
				write_with_indention(&format_inner, self.options.indention, f.options().tab_width(), f)
			}
		});
...
impl AnyTemplateElement {
	...
	fn inner_syntax(&self) -> SyntaxResult<JsSyntaxNode> {
		match self {
			AnyTemplateElement::JsTemplateElement(template) => template.expression().map(AstNode::into_syntax),
			AnyTemplateElement::TsTemplateElement(template) => template.ty().map(AstNode::into_syntax),
		}
	}
