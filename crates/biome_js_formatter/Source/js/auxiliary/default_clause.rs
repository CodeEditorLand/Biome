...
use biome_js_syntax::{AnyJsStatement, JsDefaultClause, JsDefaultClauseFields};
...
impl FormatNodeRule<JsDefaultClause> for FormatJsDefaultClause {
	fn fmt_fields(&self, node: &JsDefaultClause, f: &mut JsFormatter) -> FormatResult<()> {
		...
		if is_single_block_statement {
			...
			// no line break needed after because it is added by the indent in the switch
			// statement
			write!(f, [indent(&format_args!(hard_line_break(), consequent.format()))])
		}
	}
