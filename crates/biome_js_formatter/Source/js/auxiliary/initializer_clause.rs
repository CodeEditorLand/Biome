...
use biome_js_syntax::{JsInitializerClause, JsInitializerClauseFields};

use crate::{
	prelude::*,
	utils::{with_assignment_layout, AssignmentLikeLayout},
};
...
impl FormatNodeRule<JsInitializerClause> for FormatJsInitializerClause {
	fn fmt_fields(&self, node:&JsInitializerClause, f:&mut JsFormatter) -> FormatResult<()> {
		let JsInitializerClauseFields { eq_token, expression } = node.as_fields();
