...
use biome_js_syntax::{
	AnyJsDeclarationClause, AnyJsExportClause, AnyJsExportDefaultDeclaration, JsDecoratorList, JsExport,
};
	...
	fn fmt(&self, node: &JsDecoratorList, f: &mut JsFormatter) -> FormatResult<()> {
		...
		if let Some(export) = node.parent::<JsExport>() {
			...
			let class_decorators = match export.export_clause()? {
				AnyJsExportClause::AnyJsDeclarationClause(AnyJsDeclarationClause::JsClassDeclaration(class)) => {
			...
			if is_parameter_decorators {
				...
				let export = node.syntax().grand_parent().and_then(|grand_parent| {
					JsExport::cast_ref(&grand_parent).or_else(|| grand_parent.parent().and_then(JsExport::cast))
				});
				let is_export = export.is_some();

				let has_decorators_before_export = export.is_some_and(|export| !export.decorators().is_empty());
