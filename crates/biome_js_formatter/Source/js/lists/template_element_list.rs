...
use crate::{
	context::TabWidth,
	js::auxiliary::{
		template_chunk_element::AnyTemplateChunkElement,
		template_element::{AnyTemplateElement, TemplateElementOptions},
	},
	prelude::*,
	utils::test_each_template::EachTemplateTable,
};
		...
		for element in self.elements() {
			match element {
				AnyTemplateElementOrChunk::AnyTemplateElement(element) => {
					let options = TemplateElementOptions { after_new_line, indention };
...
/// The indention derived from a position in the source document. Consists of
/// indention level and spaces
#[derive(Debug, Copy, Clone, Default)]
		...
		let size = match by_new_line {
			...
			Some((_, after_new_line)) => {
				...
				for byte in after_new_line.bytes() {
					match byte {
						b'\t' => {
							// Tabs behave in a way that they are aligned to the
							// nearest multiple of tab_width:
							// number of spaces -> added size
							// 0 -> 4, 1 -> 4, 2 -> 4, 3 -> 4
							// 4 -> 8, 5 -> 8, 6 -> 8, 7 -> 8 ..
							// Or in other words, it clips the size to the next
