...
impl FormatRule<JsVariableDeclaratorList> for FormatJsVariableDeclaratorList {
	...
	fn fmt(&self, node: &JsVariableDeclaratorList, f: &mut JsFormatter) -> FormatResult<()> {
		...
		let is_parent_for_loop = node.syntax().grand_parent().is_some_and(|grand_parent| {
			matches!(
				grand_parent.kind(),
				JsSyntaxKind::JS_FOR_STATEMENT | JsSyntaxKind::JS_FOR_OF_STATEMENT | JsSyntaxKind::JS_FOR_IN_STATEMENT
			)
		});

		let has_any_initializer = node
			.iter()
			.any(|declarator| declarator.is_ok_and(|declarator| declarator.initializer().is_some()));
