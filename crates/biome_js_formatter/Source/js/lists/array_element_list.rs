use crate::prelude::*;
use biome_formatter::{CstFormatContext, Expand, FormatContext, FormatRuleWithOptions, GroupId, write};
...
/// unary expression (+, -, ~ or !)
pub(crate) fn can_concisely_print_array_list(list: &JsArrayElementList, comments: &JsComments) -> bool {
	...
	list.elements().all(|item| {
		let syntax = match item.into_node() {
			...
			Ok(AnyJsExpression(JsUnaryExpression(expr))) => {
				...
				match argument {
					Ok(AnyJsLiteralExpression(biome_js_syntax::AnyJsLiteralExpression::JsNumberLiteralExpression(
						literal,
					))) => {
