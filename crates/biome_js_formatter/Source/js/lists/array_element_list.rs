	...
	fn fmt(&self, node:&JsArrayElementList, f:&mut JsFormatter) -> FormatResult<()> {
		...
		match layout {
			ArrayLayout::Fill => {
				...
				// Using format_separated is valid in this case as
				// can_print_fill does not allow holes
...
enum ArrayLayout {
	...
	/// Prints every element on a single line if the whole array expression
	/// exceeds the line width, or any of its elements gets printed in
	/// *expanded* mode. ```javascript
	/// [
	///     a.b(),
	///     4,
	///     3,
...
pub(crate) fn can_concisely_print_array_list(
	...
	use biome_js_syntax::{AnyJsArrayElement::*, AnyJsExpression::*, JsUnaryOperator::*};
