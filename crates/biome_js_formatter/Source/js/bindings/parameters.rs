...
use biome_js_syntax::{
	AnyJsBinding, AnyJsBindingPattern, AnyJsConstructorParameter, AnyJsExpression, AnyJsFormalParameter,
	AnyJsParameter, AnyTsType, JsArrowFunctionExpression, JsConstructorParameters, JsParameters, JsSyntaxNode,
	JsSyntaxToken, is_test_call_argument,
};
...
impl FormatAnyJsParameters {
	fn l_paren_token(&self) -> SyntaxResult<JsSyntaxToken> {
		match self {
			...
			FormatAnyJsParameters::JsConstructorParameters(parameters) => parameters.l_paren_token(),
		}
	}

	fn list(&self) -> AnyJsParameterList {
		match self {
			FormatAnyJsParameters::JsParameters(parameters) => AnyJsParameterList::from(parameters.items()),
	...
	fn r_paren_token(&self) -> SyntaxResult<JsSyntaxToken> {
		match self {
			...
			FormatAnyJsParameters::JsConstructorParameters(parameters) => parameters.r_paren_token(),
		}
	}
...
pub(crate) fn should_hug_function_parameters(
	...
	let result = match only_parameter {
		AnyParameter::AnyJsParameter(parameter) => match parameter {
			...
			AnyJsParameter::TsThisParameter(this) => this
				.type_annotation()
				.is_some_and(|type_annotation| matches!(type_annotation.ty(), Ok(AnyTsType::TsObjectType(_)))),
		},
		AnyParameter::AnyJsConstructorParameter(constructor_parameter) => match constructor_parameter {
			AnyJsConstructorParameter::AnyJsFormalParameter(formal_parameter) => {
				hug_formal_parameter(&formal_parameter, should_hug_formal_parameter)?
			},
			AnyJsConstructorParameter::JsRestParameter(_) | AnyJsConstructorParameter::TsPropertyParameter(_) => false,
		},
	};
...
/// a function to group.
pub(crate) fn has_only_simple_parameters(parameters: &JsParameters, allow_type_annotations: bool) -> bool {
	parameters
		.items()
		.into_iter()
		.flatten()
