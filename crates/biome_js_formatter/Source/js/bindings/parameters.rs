...
use crate::{
	js::{
		expressions::arrow_function_expression::can_avoid_parentheses,
		lists::parameter_list::FormatJsAnyParameterList,
	},
	prelude::*,
};
	...
	fn fmt(&self, f:&mut Formatter<JsFormatContext>) -> FormatResult<()> {
		...
		match layout {
			...
			ParameterLayout::Hug => {
				...
				write!(f, [FormatJsAnyParameterList::with_layout(&list, ParameterLayout::Hug)])?;
...
impl FormatAnyJsParameters {
	...
	/// Returns `true` for function parameters if the function is an argument of
	/// a [test `CallExpression`](is_test_call_expression).
	fn is_in_test_call(&self) -> SyntaxResult<bool> {
		let result = match self {
			FormatAnyJsParameters::JsParameters(parameters) => {
				match parameters.syntax().parent() {
	...
	fn as_arrow_function_expression(&self) -> Option<JsArrowFunctionExpression> {
		match self {
			FormatAnyJsParameters::JsParameters(parameters) => {
				parameters.syntax().parent().and_then(JsArrowFunctionExpression::cast)
			},
...
pub enum ParameterLayout {
	...
	/// Enforce that the opening and closing parentheses aren't separated from
	/// the first token of the parameter. For example, to enforce that the `{`
	/// and `}` of an object expression are formatted on the same line
	/// as the `(` and `)` tokens even IF the object expression itself breaks
	...
	/// The default layout formats all parameters on the same line if they fit
	/// or breaks after the `(` and before the `)`.
	///
	/// ```javascript
	/// function test(
	///     firstParameter,
...
pub(crate) fn should_hug_function_parameters(
	...
	/// Returns true if the first parameter should be forced onto the same line
	/// as the `(` and `)` parentheses. See the `[ParameterLayout::Hug]
	/// documentation.
	///
	/// parameter `should_hug_formal_parameter` is a bool value used to
	/// determine whether the parenthesized arrow function parameters should be
	/// on the same line as the arrow function after removing the parentheses.
				...
				match parameter.initializer() {
					None => {
						match parameter.binding()? {
							...
							| AnyJsBindingPattern::AnyJsBinding(AnyJsBinding::JsMetavariable(_)) => true,
							...
							AnyJsBindingPattern::AnyJsBinding(
								...
								if should_hug_formal_parameter {
									...
									parameter.type_annotation().map_or(false, |type_annotation| {
										matches!(
											type_annotation.ty(),
