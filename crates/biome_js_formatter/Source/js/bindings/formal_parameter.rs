...
impl FormatNodeRule<JsFormalParameter> for FormatJsFormalParameter {
	fn fmt_fields(&self, node: &JsFormalParameter, f: &mut JsFormatter) -> FormatResult<()> {
		let JsFormalParameterFields { decorators, binding, question_mark_token, type_annotation, initializer } =
			node.as_fields();

		let content =
			format_with(|f| write![f, [binding.format(), question_mark_token.format(), type_annotation.format()]]);
