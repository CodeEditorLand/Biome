...
use biome_js_syntax::{JsFormalParameter, JsFormalParameterFields};

use crate::{
	js::bindings::parameters::{should_hug_function_parameters, FormatAnyJsParameters},
	prelude::*,
	utils::FormatInitializerClause,
};
...
impl FormatNodeRule<JsFormalParameter> for FormatJsFormalParameter {
	fn fmt_fields(&self, node:&JsFormalParameter, f:&mut JsFormatter) -> FormatResult<()> {
		...
		let content = format_with(|f| {
			write![f, [binding.format(), question_mark_token.format(), type_annotation.format()]]
		});
