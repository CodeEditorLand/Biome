...
use biome_js_syntax::{
	...
	TsDeclareFunctionDeclaration, TsDeclareFunctionExportDefaultDeclaration, TsReturnTypeAnnotation, TsTypeParameters,
};
...
impl FormatFunction {
	fn async_token(&self) -> Option<JsSyntaxToken> {
		match self {
			...
			FormatFunction::JsFunctionExportDefaultDeclaration(declaration) => declaration.async_token(),
			FormatFunction::TsDeclareFunctionDeclaration(member) => member.async_token(),
			FormatFunction::TsDeclareFunctionExportDefaultDeclaration(member) => member.async_token(),
		}
	}
	...
	fn function_token(&self) -> SyntaxResult<JsSyntaxToken> {
		match self {
			...
			FormatFunction::JsFunctionExportDefaultDeclaration(declaration) => declaration.function_token(),
			FormatFunction::TsDeclareFunctionDeclaration(declaration) => declaration.function_token(),
			FormatFunction::TsDeclareFunctionExportDefaultDeclaration(declaration) => declaration.function_token(),
		}
	}
	...
	fn star_token(&self) -> Option<JsSyntaxToken> {
		match self {
			...
			FormatFunction::JsFunctionExportDefaultDeclaration(declaration) => declaration.star_token(),
	...
	fn id(&self) -> SyntaxResult<Option<AnyJsBinding>> {
		match self {
			...
			FormatFunction::TsDeclareFunctionExportDefaultDeclaration(declaration) => Ok(declaration.id()),
		}
	}
	...
	fn type_parameters(&self) -> Option<TsTypeParameters> {
		match self {
			...
			FormatFunction::JsFunctionExportDefaultDeclaration(declaration) => declaration.type_parameters(),
			FormatFunction::TsDeclareFunctionDeclaration(declaration) => declaration.type_parameters(),
			FormatFunction::TsDeclareFunctionExportDefaultDeclaration(declaration) => declaration.type_parameters(),
		}
	}
	...
	fn parameters(&self) -> SyntaxResult<JsParameters> {
		match self {
			...
			FormatFunction::JsFunctionExportDefaultDeclaration(declaration) => declaration.parameters(),
			FormatFunction::TsDeclareFunctionDeclaration(declaration) => declaration.parameters(),
			FormatFunction::TsDeclareFunctionExportDefaultDeclaration(declaration) => declaration.parameters(),
		}
	}

	fn return_type_annotation(&self) -> Option<TsReturnTypeAnnotation> {
		match self {
			FormatFunction::JsFunctionDeclaration(declaration) => declaration.return_type_annotation(),
			FormatFunction::JsFunctionExpression(expression) => expression.return_type_annotation(),
			FormatFunction::JsFunctionExportDefaultDeclaration(declaration) => declaration.return_type_annotation(),
			FormatFunction::TsDeclareFunctionDeclaration(declaration) => declaration.return_type_annotation(),
	...
	fn body(&self) -> SyntaxResult<Option<JsFunctionBody>> {
		Ok(match self {
			...
			FormatFunction::JsFunctionExportDefaultDeclaration(declaration) => Some(declaration.body()?),
			FormatFunction::TsDeclareFunctionDeclaration(_) => None,
			FormatFunction::TsDeclareFunctionExportDefaultDeclaration(_) => None,
		})
	}
	...
	pub(crate) fn fmt_with_options(&self, f: &mut JsFormatter, options: &FormatFunctionOptions) -> FormatResult<()> {
		if let Some(async_token) = self.async_token() {
			write!(f, [async_token.format(), space()])?;
		}

		write!(f, [self.function_token().format(), self.star_token().format()])?;
		...
		write!(
			...
			[group(&format_with(|f| {
				...
				let group_parameters = should_group_function_parameters(
					...
					return_type_annotation.as_ref().map(|annotation| annotation.ty()),
		...
		if let Some(body) = self.body()? {
			write!(
				...
				[
					space(),
					FormatMaybeCachedFunctionBody { body: &body.into(), mode: options.body_cache_mode }
				]
			)?;
		}
