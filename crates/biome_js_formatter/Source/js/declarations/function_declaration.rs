...
use crate::{
	js::expressions::call_arguments::GroupedCallArgumentLayout,
	prelude::*,
	utils::function_body::{FormatMaybeCachedFunctionBody, FunctionBodyCacheMode},
};
...
impl FormatFunction {
	...
	/// # Errors
	///
	/// Returns [`FormatError::PoorLayout`] if
	/// [`call_argument_layout`](FormatFunctionOptions::call_argument_layout] is
	/// `Some` and the function parameters contain some content that [*force a
	/// group to break*](FormatElements::will_break).
	///
...
/// Grouping the parameters has the effect that the return type will break
/// first.
