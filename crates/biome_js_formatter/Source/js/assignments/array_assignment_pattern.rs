...
impl FormatNodeRule<JsArrayAssignmentPattern> for FormatJsArrayAssignmentPattern {
	fn fmt_fields(&self, node: &JsArrayAssignmentPattern, f: &mut JsFormatter) -> FormatResult<()> {
		let JsArrayAssignmentPatternFields { l_brack_token, elements, r_brack_token } = node.as_fields();
	...
	fn fmt_dangling_comments(&self, _: &JsArrayAssignmentPattern, _: &mut JsFormatter) -> FormatResult<()> {
