...
use biome_js_syntax::{
	AnyJsAssignment, AnyJsExpression, AnyJsOptionalChainExpression, AnyTsType, JsLanguage, JsLogicalExpression,
	JsSyntaxKind, JsSyntaxNode, JsSyntaxTrivia, misc_ext::AnyJsParenthesized,
};
use biome_rowan::{AstNode, SyntaxKind, SyntaxRewriter, SyntaxToken, TextSize, VisitNodeSignal, chain_trivia_pieces};
...
impl JsFormatSyntaxRewriter {
	pub fn with_offset(offset: TextSize) -> Self {
		JsFormatSyntaxRewriter { source_map: TransformSourceMapBuilder::with_offset(offset), ..Default::default() }
	}
}
...
impl JsFormatSyntaxRewriter {
	...
	fn visit_parenthesized(&mut self, parenthesized: AnyJsParenthesized) -> VisitNodeSignal<JsLanguage> {
		...
		match inner.first_token() {
			...
			None => {
				let updated = match parenthesized {
					AnyJsParenthesized::JsParenthesizedExpression(expression) => {
						...
						expression.with_expression(AnyJsExpression::unwrap_cast(inner)).into_syntax()
					},
					AnyJsParenthesized::JsParenthesizedAssignment(assignment) => {
						...
						assignment.with_assignment(AnyJsAssignment::unwrap_cast(inner)).into_syntax()
					},
			...
			Some(first_token) => {
				self.source_map
					.extend_trimmed_node_range(inner_trimmed_range, parenthesized.syntax().text_trimmed_range());
				...
				let l_paren_trivia = chain_trivia_pieces(l_paren.leading_trivia().pieces(), l_paren_trailing);
				...
				while let Some(trivia) = leading_trivia.peek() {
					...
					if trivia.is_newline() && first_new_line.is_none() {
						first_new_line = Some((trivia.text_range() + inner_offset, leading_trivia.next().unwrap()));
				...
				let leading_trivia =
					chain_trivia_pieces(first_new_line.map(|(_, trivia)| trivia).into_iter(), leading_trivia);
				...
				let r_paren_trivia =
					chain_trivia_pieces(r_paren.leading_trivia().pieces(), r_paren.trailing_trivia().pieces());
				...
				VisitNodeSignal::Replace(updated.replace_child(last_token.into(), new_last.into()).unwrap())
			},
		}
	}
	...
	fn visit_logical_expression(&mut self, logical: JsLogicalExpression) -> VisitNodeSignal<JsLanguage> {
			...
			(Ok(left), Ok(operator), Ok(right)) => {
				...
				let updated = match right {
					AnyJsExpression::JsLogicalExpression(right_logical) => {
						match (right_logical.left(), right_logical.operator_token(), right_logical.right()) {
							(Ok(right_left), Ok(right_operator), Ok(right_right))
								if right_operator.kind() == operator.kind() =>
							{
								logical
									.with_left(
										biome_js_factory::make::js_logical_expression(left, operator, right_left)
											.into(),
									)
					...
					right => {
						...
						{
							logical.with_left(left).with_operator_token_token(operator).with_right(right)
						} else {
							logical
						}
					},
	...
	match AnyJsExpression::cast_ref(inner) {
		...
		Some(AnyJsExpression::JsStaticMemberExpression(static_expr)) => {
			static_expr.is_optional()
				|| static_expr
					.object()
					.map_or(true, |object| object.syntax().kind() != JsSyntaxKind::JS_IDENTIFIER_EXPRESSION)
		},
...
mod tests {
	...
	use biome_js_syntax::{
		JsArrayExpression, JsBinaryExpression, JsExpressionStatement, JsFileSource, JsIdentifierExpression,
		JsLogicalExpression, JsSequenceExpression, JsStringLiteralExpression, JsSyntaxNode, JsUnaryExpression,
		JsxTagExpression,
	};
	...
	fn rebalances_logical_expressions() {
		...
		let mut logical_expressions: Vec<_> = transformed.descendants().filter_map(JsLogicalExpression::cast).collect();
	...
	fn only_rebalances_logical_expressions_with_same_operator() {
		...
		let logical_expressions: Vec<_> = transformed.descendants().filter_map(JsLogicalExpression::cast).collect();
	...
	fn adjacent_nodes() {
		...
		let identifiers: Vec<_> = transformed.descendants().filter_map(JsIdentifierExpression::cast).collect();
	...
	fn intersecting_ranges() {
		...
		let string_literal = transformed.descendants().find_map(JsStringLiteralExpression::cast).unwrap();
	...
	fn enclosing_node() {
		...
		let expression_statement = transformed.descendants().find_map(JsExpressionStatement::cast).unwrap();
	...
	#[test]
	fn comments() {
		let (transformed, source_map) = source_map_test("/* outer */ (/* left */ a + b /* right */) /* outer end */;");
...
!(
  ...
  x + y // foo
	...
	fn mappings() {
		...
		let formatted = format_node(JsFormatOptions::new(JsFileSource::default()), &transformed).unwrap();
