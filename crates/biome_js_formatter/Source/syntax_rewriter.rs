	...
	fn visit_parenthesized(
		...
		match inner.first_token() {
			...
			None => {
				let updated = match parenthesized {
					AnyJsParenthesized::JsParenthesizedExpression(expression) => {
						// SAFETY: Safe because the rewriter never rewrites an expression to a non
						// expression.
						expression
					...
					AnyJsParenthesized::JsParenthesizedAssignment(assignment) => {
						// SAFETY: Safe because the rewriter never rewrites an assignment to a non
						// assignment.
						assignment
			...
			Some(first_token) => {
				...
				// The leading whitespace before the opening parens replaces the whitespace
				// before the node.
				...
				// Remove all leading new lines directly in front of the token but keep the
				// leading new-line if it precedes a skipped token trivia or a comment.
				...
				// SAFETY: Calling `unwrap` is safe because we know that `inner_first` is part
				// of the `inner` subtree.
				...
				// SAFETY: Calling `unwrap` is safe because `last_token` only returns `None` if
				// a node's subtree doesn't contain ANY token, but we know that the subtree
				// contains at least the first token.
				...
				// SAFETY: Calling `unwrap` is safe because we know that `last_token` is part of
				// the `updated` subtree.
