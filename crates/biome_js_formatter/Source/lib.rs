...
use biome_formatter::{
	CstFormatContext, Format, FormatLanguage, FormatToken, TransformSourceMap, comments::Comments, write,
};
use biome_js_syntax::{AnyJsDeclaration, AnyJsStatement, JsLanguage, JsSyntaxKind, JsSyntaxNode, JsSyntaxToken};
...
impl FormatLanguage for JsFormatLanguage {
	...
	fn create_context(self, root: &JsSyntaxNode, source_map: Option<TransformSourceMap>) -> Self::Context {
...
pub fn format_range(options: JsFormatOptions, root: &JsSyntaxNode, range: TextRange) -> FormatResult<Printed> {
...
pub fn format_node(options: JsFormatOptions, root: &JsSyntaxNode) -> FormatResult<Formatted<JsFormatContext>> {
...
";
		...
		assert_eq!(result.range(), Some(TextRange::new(range_start, range_end + TextSize::from(1))));
	}
