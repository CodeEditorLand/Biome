...
use biome_formatter::{
	comments::Comments, write, CstFormatContext, Format, FormatLanguage,
	FormatToken, TransformSourceMap,
};
use biome_formatter::{
	Buffer, FormatOwnedWithRule, FormatRefWithRule, Formatted, Printed,
};
use biome_js_syntax::{
...
impl FormatLanguage for JsFormatLanguage {
	...
	fn create_context(
		...
		let comments =
			Comments::from_node(root, &JsCommentStyle, source_map.as_ref());
...
";
		...
		let range_start =
			TextSize::try_from(input.find("let").unwrap() - 2).unwrap();
		let range_end =
			TextSize::try_from(input.find("const").unwrap()).unwrap();
...
"#;
	...
	fn format_range_out_of_bounds() {
		...
		let result = format_range(
			...
			TextRange::new(
				TextSize::from(0),
				TextSize::of(src) + TextSize::from(5),
			),
		);
