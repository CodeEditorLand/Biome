...
use crate::prelude::*;
use crate::utils::jsx::{
	JsxChild, JsxChildrenIterator, JsxRawSpace, JsxSpace, is_meaningful_jsx_text, is_whitespace_jsx_expression,
	jsx_split_children,
};
...
impl FormatJsxChildList {
	pub(crate) fn fmt_children(&self, list: &JsxChildList, f: &mut JsFormatter) -> FormatResult<FormatChildrenResult> {
				...
				JsxChild::Word(word) => {
					let separator = match children_iter.peek() {
						...
						Some(JsxChild::NonText(next_child)) => Some(WordSeparator::EndOfText {
							is_soft_line_break: !matches!(next_child, AnyJsxChild::JsxSelfClosingElement(_))
								|| word.is_single_character(),
						}),

						Some(JsxChild::Newline | JsxChild::Whitespace | JsxChild::EmptyLine) => None,
				...
				JsxChild::Newline => {
					let is_soft_break = {
						...
						// ```
							...
							let is_next_next_element_new_line = matches!(next_next_element, Some(JsxChild::Newline));
				...
				JsxChild::NonText(non_text) => {
					let line_mode = match children_iter.peek() {
						Some(JsxChild::Word(word)) => {
							...
							// adefg
							// ```
							if matches!(non_text, AnyJsxChild::JsxSelfClosingElement(_)) && !word.is_single_character()
							{
								Some(LineMode::Hard)
							} else {
								Some(LineMode::Soft)
	...
	fn disarm_debug_assertions(&self, node: &JsxChildList, f: &mut JsFormatter) {
		...
		for child in node {
			match child {
				JsxExpressionChild(expression) if is_whitespace_jsx_expression(&expression, f.context().comments()) => {
					...
					match expression.expression().unwrap() {
						AnyJsExpression::AnyJsLiteralExpression(AnyJsLiteralExpression::JsStringLiteralExpression(
							...
							f.context().comments().mark_suppression_checked(string_literal.syntax());
			...
			match child {
				...
				JsxText(text) => {
					meta.meaningful_text = meta.meaningful_text
						|| text.value_token().is_ok_and(|token| is_meaningful_jsx_text(token.text()));
				},
				_ => {},
			}
		}
			...
			if let Some(elements) = f.intern_vec(self.elements.take()) {
				match self.layout {
					...
					MultilineLayout::NoFill => f.write_elements([
						FormatElement::Tag(Tag::StartGroup(tag::Group::new().with_mode(GroupMode::Expand))),
						elements,
