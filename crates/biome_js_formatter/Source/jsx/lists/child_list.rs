...
use biome_formatter::{
	format_args,
	format_element::tag::{GroupMode, Tag},
	write,
	CstFormatContext,
...
/// The result of formatting the children of an [JsxChildList]. This is
/// ultimately determined by [FormatJsxChildList::layout].
#[derive(Debug)]
pub(crate) enum FormatChildrenResult {
	...
	/// ```
	///
	/// This usually occurs when the children are already formatted over
	/// multiple lines, or when the children contains another tag.
	ForceMultiline(FormatMultilineChildren),

	/// Let the formatter determine whether the children should be formatted
	/// over multiple lines or if they can be kept on a single line.
	BestFitting { flat_children:FormatFlatChildren, expanded_children:FormatMultilineChildren },
}
...
impl FormatJsxChildList {
	pub(crate) fn fmt_children(
		...
		while let Some(child) = children_iter.next() {
			...
			match &child {
				// A single word: Both `a` and `b` are a word in `a b` because
				// they're separated by JSX Whitespace.
				JsxChild::Word(word) => {
					let separator = match children_iter.peek() {
						Some(JsxChild::Word(_)) => {
							// Separate words by a space or line break in
							// extended mode
							Some(WordSeparator::BetweenWords)
						},

						// Last word or last word before an element without any
						// whitespace in between
						Some(JsxChild::NonText(next_child)) => {
					...
					if let Some(separator) = separator {
						...
						// it's safe to write without a separator because None
						// means that next element is a separator or end of the
						// iterator
				...
				JsxChild::Whitespace => {
					...
					// Leading whitespace. Only possible if used together with a
					// expression child
					//
					// ```
					// <div>
				...
				JsxChild::Newline => {
					let is_soft_break = {
						// Here we handle the case when we have a newline
						// between a single-character word and a jsx element
						// We need to use the previous and the next element
						// [JsxChild::Word, JsxChild::Newline,
						// JsxChild::NonText] ```
						// <div>
						//   <div>First</div>,
						//   <div>Second</div>
						...
						// Here we handle the case when we have a
						// single-character word between a new line and a jsx
						// element Here we need to look ahead two elements
						// [JsxChild::Newline, JsxChild::Word,
						// JsxChild::NonText] ```
						// <div>
						//   <div>First</div>
						//   ,<div>Second</div>
				...
				JsxChild::NonText(non_text) => {
					let line_mode = match children_iter.peek() {
						Some(JsxChild::Word(word)) => {
							// Break if the current or next element is a self
							// closing element ```javascript
						...
						// Add a hard line break if what comes after the element
						// is not a text or is all whitespace
					...
					if force_multiline {
						if let Some(format_separator) = format_separator {
							...
							// it's safe to write without a separator because
							// None means that next element is a separator or
							// end of the iterator
	...
	/// Tracks the tokens of [JsxText] and [JsxExpressionChild] nodes to be
	/// formatted and asserts that the suppression comments are checked (they
	/// get ignored).
	///
	/// This is necessary because the formatting of [JsxChildList] bypasses the
	/// node formatting for [JsxText] and [JsxExpressionChild] and instead,
	/// formats the nodes itself.
	...
	fn disarm_debug_assertions(&self, node:&JsxChildList, f:&mut JsFormatter) {
		...
		for child in node {
			match child {
				...
				{
					f.context().comments().mark_suppression_checked(expression.syntax());
					...
					match expression.expression().unwrap() {
						AnyJsExpression::AnyJsLiteralExpression(
							...
							f.state_mut().track_token(&string_literal.value_token().unwrap());

							f.state_mut().track_token(&expression.l_curly_token().unwrap());
							f.state_mut().track_token(&expression.r_curly_token().unwrap());
						},
						_ => unreachable!(),
					}
				},
				JsxText(text) => {
					...
					f.context().comments().mark_suppression_checked(text.syntax());
				},
				_ => {
					continue;
				},
	...
	/// Computes additional meta data about the children by iterating once over
	/// all children.
...
impl JsxChildListLayout {
	const fn is_multiline(&self) -> bool { matches!(self, JsxChildListLayout::Multiline) }
}
...
struct ChildrenMeta {
	...
	/// `true` if any child contains meaningful a [JsxText] with meaningful
	/// text.
...
enum WordSeparator {
	...
	/// A separator of a word at the end of a [JsxText] element. Either because
	/// it is the last child in its parent OR it is right before the start of
	/// another child (element, expression, ...).
	///
	/// ```javascript
	...
	/// ```
	///
	/// Creates a soft line break EXCEPT if the next element is a self closing
	/// element or the previous word was an ascii punctuation, which results in
	/// a hard line break:
	///
	/// ```javascript
	/// a = <div>ab<br/></div>;
...
impl WordSeparator {
	...
	fn will_break(&self) -> bool {
		matches!(self, WordSeparator::EndOfText { is_soft_line_break:false })
	}
}
...
///
/// The multiline layout may use [FormatElement::Fill] element that requires
/// that its children are an alternating sequence of `[element, separator,
/// element, separator, ...]`.
///
/// This requires that each element is wrapped inside of a list if it emits more
/// than one element to uphold the constraints of [FormatElement::Fill].
///
/// However, the wrapping is only necessary for [MultilineLayout::Fill] for when
/// the [FormatElement::Fill] element is used.
///
/// This builder takes care of doing the least amount of work necessary for the
/// chosen layout while also guaranteeing that the written element is valid
#[derive(Debug, Clone)]
...
impl MultilineBuilder {
	fn new(layout:MultilineLayout) -> Self { Self { layout, result:Ok(Vec::new()) } }

	/// Formats an element that does not require a separator
	/// It is safe to omit the separator because at the call side we must
	/// guarantee that we have reached the end of the iterator or the next
	/// element is a space/newline that should be written into the separator
	/// "slot".
	fn write_content(&mut self, content:&dyn Format<JsFormatContext>, f:&mut JsFormatter) {
		self.write(content, None, f);
	}

	/// Formatting a separator does not require any element in the separator
	/// slot
	...
	fn write(
		...
		self.result = result.and_then(|elements| {
			let elements = {
				...
				match self.layout {
					MultilineLayout::Fill => {
						// Make sure that the separator and content only ever
...
impl FlatBuilder {
	fn new() -> Self { Self { result:Ok(Vec::new()), disabled:false } }
	...
	fn finish(self) -> FormatResult<FormatFlatChildren> {
		assert!(
			!self.disabled,
			"The flat builder has been disabled and thus, does no longer store any elements. Make \
			 sure you don't call disable if you later intend to format the flat content."
		);
