...
impl FormatJsxChildList {
	pub(crate) fn fmt_children(
		...
		while let Some(child) = children_iter.next() {
			...
			match &child {
				// A single word: Both `a` and `b` are a word in `a b` because they're separated by
				// JSX Whitespace.
				JsxChild::Word(word) => {
					let separator = match children_iter.peek() {
						...
						// Last word or last word before an element without any whitespace in
						// between
					...
					if let Some(separator) = separator {
						...
						// it's safe to write without a separator because None means that next
						// element is a separator or end of the iterator
				...
				JsxChild::Newline => {
					let is_soft_break = {
						// Here we handle the case when we have a newline between a single-character
						// word and a jsx element We need to use the previous and the next
						// element [JsxChild::Word, JsxChild::Newline, JsxChild::NonText]
						// ```
						// <div>
						//   <div>First</div>,
						//   <div>Second</div>
						...
						// Here we handle the case when we have a single-character word between a
						// new line and a jsx element Here we need to look ahead two elements
				...
				JsxChild::NonText(non_text) => {
					let line_mode = match children_iter.peek() {
						...
						// Add a hard line break if what comes after the element is not a text or is
						// all whitespace
