...
enum OpeningElementLayout {
	...
	///
	/// Applied for elements that have no attributes nor any comment attached to
	/// their name.
	///
	/// ```javascript
	...
	/// Opening element with a single attribute that contains no line breaks,
	/// nor has comments.
	///
	/// ```javascript
	...
	/// Default layout that indents the attributes and formats each attribute on
	/// its own line.
	///
	/// ```javascript
	/// <div
...
/// Returns `true` if this is an attribute with a [JsxString] initializer that
/// does not contain any new line characters.
...
/// Returns `true` if this is an attribute with a [JsxString] initializer that
/// contains at least one new line character.
...
fn as_string_literal_attribute_value(attribute:&AnyJsxAttribute) -> Option<JsxString> {
	...
	match attribute {
		JsxAttribute(attribute) => {
			attribute.initializer().and_then(|initializer| {
