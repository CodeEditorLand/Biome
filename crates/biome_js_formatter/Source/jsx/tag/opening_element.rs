...
use biome_js_syntax::{
	AnyJsxAttribute, AnyJsxAttributeValue, AnyJsxElementName, JsSyntaxToken, JsxAttributeList, JsxOpeningElement,
	JsxSelfClosingElement, JsxString, TsTypeArguments,
};
...
impl Format<JsFormatContext> for AnyJsxOpeningElement {
	fn fmt(&self, f: &mut Formatter<JsFormatContext>) -> FormatResult<()> {
		...
		match layout {
			...
			OpeningElementLayout::IndentAttributes { name_has_comments, last_attribute_has_comments } => {
	...
	fn compute_layout(&self, comments: &JsComments) -> SyntaxResult<OpeningElementLayout> {
		...
		let layout = if self.is_self_closing() && attributes.is_empty() && !name_has_comments {
			...
			&& attributes.iter().all(|attribute| {
				is_single_line_string_literal_attribute(&attribute) && !comments.has_comments(attribute.syntax())
			}) && !name_has_comments
		{
			OpeningElementLayout::SingleStringAttribute
		} else {
...
fn is_single_line_string_literal_attribute(attribute: &AnyJsxAttribute) -> bool {
	as_string_literal_attribute_value(attribute)
		.is_some_and(|string| string.value_token().is_ok_and(|text| !text.text_trimmed().contains('\n')))
}
...
fn as_string_literal_attribute_value(attribute: &AnyJsxAttribute) -> Option<JsxString> {
	...
	match attribute {
		JsxAttribute(attribute) => attribute.initializer().and_then(|initializer| match initializer.value() {
