...
use biome_js_syntax::{AnyJsExpression, AnyJsxChild, JsxChildList, JsxElement, JsxExpressionChild, JsxFragment};
		...
		match layout {
			...
			ElementLayout::Default => {
				...
				let multiple_attributes = match self {
					AnyJsxTagWithChildren::JsxElement(element) => element.opening_element()?.attributes().len() > 1,
