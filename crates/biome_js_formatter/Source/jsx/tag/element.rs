...
use crate::{
	jsx::lists::child_list::{FormatChildrenResult, FormatJsxChildList, JsxChildListLayout},
	prelude::*,
	utils::jsx::{is_jsx_suppressed, is_meaningful_jsx_text},
};
				...
				match child {
					JsxText(text) => {
						...
						if !is_meaningful_jsx_text(value_token.text()) {
							...
							f.context_mut().comments().mark_suppression_checked(text.syntax());
							// It's safe to ignore the tokens here because JSX text tokens can't
							// have comments (nor whitespace) attached.
...
enum ElementLayout {
	...
	/// Default layout used for all elements that have children and
	/// [ElementLayout::Template] does not apply.
	///
	/// ```javascript
	/// <Element2>
	///   Some more content
