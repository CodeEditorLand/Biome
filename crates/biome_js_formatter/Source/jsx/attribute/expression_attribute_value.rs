...
impl FormatNodeRule<JsxExpressionAttributeValue> for FormatJsxExpressionAttributeValue {
	fn fmt_fields(
		...
		let JsxExpressionAttributeValueFields { l_curly_token, expression, r_curly_token } =
			node.as_fields();
...
/// Tests if an expression inside of a [JsxExpressionChild] or
/// [JsxExpressionAttributeValue] should be inlined. Good:
/// ```jsx
///  <ColorPickerPage
///     colors={[
///        "blue",
...
pub(crate) fn should_inline_jsx_expression(
	...
	match expression {
		...
		JsAwaitExpression(await_expression) => {
			match await_expression.argument() {
