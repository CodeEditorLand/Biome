...
use biome_js_syntax::{AnyJsExpression, AnyJsxTag, JsxExpressionAttributeValue, JsxExpressionAttributeValueFields};
...
impl FormatNodeRule<JsxExpressionAttributeValue> for FormatJsxExpressionAttributeValue {
	fn fmt_fields(&self, node: &JsxExpressionAttributeValue, f: &mut JsFormatter) -> FormatResult<()> {
		let JsxExpressionAttributeValueFields { l_curly_token, expression, r_curly_token } = node.as_fields();
...
///     } />
/// ```
pub(crate) fn should_inline_jsx_expression(expression: &AnyJsExpression, comments: &JsComments) -> bool {
	use AnyJsExpression::*;
