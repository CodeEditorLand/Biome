...
pub(crate) mod tests {
	...
	use biome_js_syntax::{parentheses::NeedsParentheses, JsFileSource, JsLanguage};
	...
	>(
		...
		let node = if let Some(index) = index {
			...
			match matching_nodes.len() {
				...
				_ => {
					panic!(
						"Expected to find a single node matching '{}' in '{input}' but found \
						 multiple ones:\n {matching_nodes:#?}",
						core::any::type_name::<T>()
					);
				},
	...
	/// ```
	///
	/// Asserts that [NeedsParentheses.needs_parentheses()] returns true for the
	/// only [JsStaticMemberExpression] in the program.
	///
	/// ```
	/// # use biome_js_syntax::JsStaticMemberExpression;
	/// use biome_js_formatter::assert_needs_parentheses;
	...
	///
	/// Asserts that [NeedsParentheses.needs_parentheses()] returns true for the
	/// second (in pre-order) [JsStaticMemberExpression] in the program.
	#[macro_export]
	...
	///
	/// Asserts that [NeedsParentheses.needs_parentheses()] returns true for the
	/// first (in pre-order) [JsStaticMemberExpression] in the program.
	#[macro_export]
