...
impl FormatNodeRule<TsInterfaceDeclaration> for FormatTsInterfaceDeclaration {
				...
				if should_indent_extends_only {
					write!(
						...
						[
							...
							if_group_fits_on_line(&soft_line_break_or_space()).with_group_id(type_parameter_group),
						]
					)?;
				} else {
	...
	fn fmt_dangling_comments(&self, _: &TsInterfaceDeclaration, _: &mut JsFormatter) -> FormatResult<()> {
