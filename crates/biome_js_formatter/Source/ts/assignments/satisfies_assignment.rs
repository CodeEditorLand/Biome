use biome_js_syntax::{parentheses::NeedsParentheses, TsSatisfiesAssignment};
...
impl FormatNodeRule<TsSatisfiesAssignment> for FormatTsSatisfiesAssignment {
	...
	fn needs_parentheses(&self, item:&TsSatisfiesAssignment) -> bool { item.needs_parentheses() }
}
...
mod tests {
	...
	fn needs_parentheses() {
		...
		assert_needs_parentheses!("(a satisfies number)! = 'test'", TsSatisfiesAssignment);
		assert_needs_parentheses!("(<number>(a satisfies number)) = 'test'", TsSatisfiesAssignment);
		assert_needs_parentheses!("++(a satisfies number)", TsSatisfiesAssignment);
		assert_needs_parentheses!("(a satisfies number)--", TsSatisfiesAssignment);
		assert_needs_parentheses!("({ a: a satisfies number } = { a: 5 })", TsSatisfiesAssignment);
	}
}
