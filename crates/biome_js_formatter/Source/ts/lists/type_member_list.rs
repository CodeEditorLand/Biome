...
use crate::{context::Semicolons, prelude::*};
...
impl FormatRule<TsTypeMemberList> for FormatTsTypeMemberList {
	...
	fn fmt(&self, node:&TsTypeMemberList, f:&mut JsFormatter) -> FormatResult<()> {
		...
		for (index, member) in items.enumerate() {
			joiner.entry(
				member.syntax(),
				&TsTypeMemberItem { last:index == last_index, member:&member },
			)
		}
...
impl Format<JsFormatContext> for TsTypeMemberItem<'_> {
	fn fmt(&self, f:&mut JsFormatter) -> FormatResult<()> {
		...
		if !is_verbatim {
			// Children don't format the separator on purpose, so it's up to the
			// parent - this node, to decide to print their separator
