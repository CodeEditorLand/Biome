...
use crate::{
	js::auxiliary::metavariable::FormatJsMetavariable,
	prelude::*,
	ts::{
		bogus::bogus_type::FormatTsBogusType,
		module::import_type::FormatTsImportType,
		types::{
			any_type::FormatTsAnyType,
			array_type::FormatTsArrayType,
			bigint_literal_type::FormatTsBigintLiteralType,
			bigint_type::FormatTsBigintType,
			boolean_literal_type::FormatTsBooleanLiteralType,
			boolean_type::FormatTsBooleanType,
			conditional_type::FormatTsConditionalType,
			constructor_type::FormatTsConstructorType,
			function_type::FormatTsFunctionType,
			indexed_access_type::FormatTsIndexedAccessType,
			infer_type::FormatTsInferType,
			intersection_type::FormatTsIntersectionType,
			mapped_type::FormatTsMappedType,
			never_type::FormatTsNeverType,
			non_primitive_type::FormatTsNonPrimitiveType,
			null_literal_type::FormatTsNullLiteralType,
			number_literal_type::FormatTsNumberLiteralType,
			number_type::FormatTsNumberType,
			object_type::FormatTsObjectType,
			parenthesized_type::FormatTsParenthesizedType,
			reference_type::FormatTsReferenceType,
			string_literal_type::FormatTsStringLiteralType,
			string_type::FormatTsStringType,
			symbol_type::FormatTsSymbolType,
			template_literal_type::FormatTsTemplateLiteralType,
			this_type::FormatTsThisType,
			tuple_type::FormatTsTupleType,
			type_operator_type::FormatTsTypeOperatorType,
			typeof_type::FormatTsTypeofType,
			undefined_type::FormatTsUndefinedType,
			union_type::FormatTsUnionType,
			unknown_type::FormatTsUnknownType,
			void_type::FormatTsVoidType,
		},
	},
	JsCommentStyle,
};
...
impl FormatRule<TsUnionTypeVariantList> for FormatTsUnionTypeVariantList {
	...
	fn fmt(&self, node:&TsUnionTypeVariantList, f:&mut JsFormatter) -> FormatResult<()> {
		...
		f.join_with(space())
			.entries(node.elements().enumerate().map(|(index, item)| {
				FormatTypeVariant {
...
impl Format<JsFormatContext> for FormatTypeVariant<'_> {
	fn fmt(&self, f:&mut JsFormatter) -> FormatResult<()> {
		...
		// This is a hack: It by passes the regular format node to only format
		...
		//     }
		// - Empty tuple types also format their own dangling comments [ // This is a
		//   dangling comment, formatted inside TsTupleType ]
...
fn is_type_suppressed(ty:&AnyTsType, list:&TsUnionTypeVariantList, comments:&JsComments) -> bool {
	comments.mark_suppression_checked(ty.syntax());

	if let AnyTsType::TsUnionType(union) = ty {
		// If the union isn't empty, than the suppression applies to the first
		// variant
	...
	// Otherwise check if the node has a suppression in its leading or dangling
	// comments before then checking the previous variants trailing "OwnLine"
	// comments (comments that are on their own line)
	...
	// Test if the preceding node as a trailing own line comment that is a
	// suppression
	...
	// If this is the first variant, then see if the union has a leading
	// suppression comment.
