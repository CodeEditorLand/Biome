...
impl FormatRule<TsUnionTypeVariantList> for FormatTsUnionTypeVariantList {
	...
	fn fmt(&self, node: &TsUnionTypeVariantList, f: &mut JsFormatter) -> FormatResult<()> {
		...
		f.join_with(space())
			.entries(node.elements().enumerate().map(|(index, item)| FormatTypeVariant {
		...
		let format_node = format_with(|f: &mut JsFormatter| {
			if is_suppressed {
				...
				match node {
					...
					AnyTsType::TsBooleanLiteralType(ty) => FormatTsBooleanLiteralType.fmt_node(ty, f),
					...
					AnyTsType::TsTemplateLiteralType(ty) => FormatTsTemplateLiteralType.fmt_node(ty, f),
