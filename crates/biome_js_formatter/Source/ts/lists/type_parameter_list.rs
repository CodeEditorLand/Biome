...
use crate::{context::trailing_commas::FormatTrailingCommas, prelude::*};
...
impl FormatRule<TsTypeParameterList> for FormatTsTypeParameterList {
	...
	fn fmt(&self, node:&TsTypeParameterList, f:&mut JsFormatter) -> FormatResult<()> {
		// Type parameter lists of arrow function expressions have to include at
		// least one comma to avoid any ambiguity with JSX elements.
		// Thus, we have to add a trailing comma when there is a single type
		// parameter. The comma can be omitted in the case where the single
		// parameter has a constraint, i.i. an `extends` clause.
		...
		f.join_with(&soft_line_break_or_space())
			.entries(node.format_separated(",").with_trailing_separator(trailing_separator))
			.finish()
	}
}
