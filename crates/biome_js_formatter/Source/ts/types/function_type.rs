...
use biome_js_syntax::{TsFunctionType, TsFunctionTypeFields, parentheses::NeedsParentheses};

use crate::{js::declarations::function_declaration::should_group_function_parameters, prelude::*};
...
impl FormatNodeRule<TsFunctionType> for FormatTsFunctionType {
	fn fmt_fields(&self, node:&TsFunctionType, f:&mut JsFormatter) -> FormatResult<()> {
		let TsFunctionTypeFields { parameters, fat_arrow_token, type_parameters, return_type } =
			node.as_fields();
		...
		let format_inner = format_with(|f| {
			...
			write![f, [space(), fat_arrow_token.format(), space(), format_return_type]]
		});

		write!(f, [group(&format_inner)])
	}

	fn needs_parentheses(&self, item:&TsFunctionType) -> bool { item.needs_parentheses() }
}
...
mod tests {
	...
	fn needs_parentheses() {
		...
		// This does require parentheses but the formatter will strip the leading `&`,
		// leaving only the inner type thus, no parentheses are required
