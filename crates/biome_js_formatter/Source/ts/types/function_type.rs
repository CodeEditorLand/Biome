...
impl FormatNodeRule<TsFunctionType> for FormatTsFunctionType {
	fn fmt_fields(&self, node: &TsFunctionType, f: &mut JsFormatter) -> FormatResult<()> {
		let TsFunctionTypeFields { parameters, fat_arrow_token, type_parameters, return_type } = node.as_fields();
...
mod tests {
	...
	fn needs_parentheses() {
		...
		assert_needs_parentheses!("type s = (() => string) extends string ? string : number", TsFunctionType);
		assert_not_needs_parentheses!("type s = A extends string ? (() => string) : number", TsFunctionType);
		assert_not_needs_parentheses!("type s = A extends string ? string : (() => string)", TsFunctionType)
	}
}
