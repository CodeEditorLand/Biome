...
use biome_js_syntax::{parentheses::NeedsParentheses, TsTypeofType, TsTypeofTypeFields};
...
mod tests {
	...
	fn needs_parentheses() {
		...
		// Disambiguates to `keyof ((typeof obj)[number])`, so the outer `keyof`
		// doesn't need parentheses as it's written here, but the inner
		// `typeof obj` does, to clarify precedence.
		assert_not_needs_parentheses!("let s: keyof (typeof obj[number]);", TsTypeOperatorType);
		assert_needs_parentheses!("let s: keyof (typeof obj[number]);", TsTypeofType);

		// Forced precedence change with added parentheses, the `typeof` is no
		// longer an indexed access type, so no parentheses are needed on it
		// directly.
