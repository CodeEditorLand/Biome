use biome_formatter::{format_args, trivia::FormatLeadingComments, write};
...
impl FormatNodeRule<TsMappedType> for FormatTsMappedType {
	fn fmt_fields(&self, node:&TsMappedType, f:&mut JsFormatter) -> FormatResult<()> {
		...
		let type_annotation_has_leading_comment = mapped_type
			.as_ref()
			.map_or(false, |annotation| comments.has_leading_comments(annotation.syntax()));

		let format_inner = format_with(|f| {
			write!(
				f,
				[FormatLeadingComments::Comments(comments.dangling_comments(node.syntax()))]
			)?;
