...
use crate::{
	prelude::*,
	utils::{should_hug_type, FormatTypeMemberSeparator},
};
...
impl FormatNodeRule<TsUnionType> for FormatTsUnionType {
	...
	fn fmt_fields(&self, node:&TsUnionType, f:&mut JsFormatter) -> FormatResult<()> {
		let TsUnionTypeFields { leading_separator_token, types } = node.as_fields();
		...
		while let Some(grand_parent) =
			union_type_at_top.syntax().grand_parent().and_then(TsUnionType::cast)
		{
		...
		let should_indent = {
			...
			// These parents have indent for their content, so we don't need to
			// indent here
			!match parent_kind {
		...
		let content = format_with(|f| {
			...
			let is_inside_complex_tuple_type =
				node.parent::<TsTupleTypeElementList>().map_or(false, |tuple| tuple.len() > 1);
	...
	fn needs_parentheses(&self, item:&TsUnionType) -> bool { item.needs_parentheses() }
