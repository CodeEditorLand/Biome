...
impl FormatNodeRule<TsUnionType> for FormatTsUnionType {
	...
	fn fmt_fields(&self, node: &TsUnionType, f: &mut JsFormatter) -> FormatResult<()> {
		...
		while let Some(grand_parent) = union_type_at_top.syntax().grand_parent().and_then(TsUnionType::cast) {
		...
		let content = format_with(|f| {
			...
			let is_inside_complex_tuple_type =
				node.parent::<TsTupleTypeElementList>().is_some_and(|tuple| tuple.len() > 1);
