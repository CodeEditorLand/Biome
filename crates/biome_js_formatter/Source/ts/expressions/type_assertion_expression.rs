...
impl FormatNodeRule<TsTypeAssertionExpression> for FormatTsTypeAssertionExpression {
	fn fmt_fields(&self, node: &TsTypeAssertionExpression, f: &mut JsFormatter) -> FormatResult<()> {
		let TsTypeAssertionExpressionFields { l_angle_token, ty, r_angle_token, expression } = node.as_fields();
		...
		if break_after_cast {
			...
			write!(
				...
				[best_fitting![
					...
					format_args![
						format_cast,
						group(&format_args![text("("), block_indent(&format_expression), text(")")])
					],
					format_args![format_cast, format_expression]
				]]
			)
