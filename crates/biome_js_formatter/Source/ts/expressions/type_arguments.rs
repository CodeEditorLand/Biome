use biome_formatter::{format_args, write, FormatError::SyntaxError};
use biome_js_syntax::{
	AnyJsExpression,
	AnyTsType,
	JsSyntaxKind,
...
use crate::{
	prelude::*,
	utils::{is_object_like_type, should_hug_type},
};
	...
	fn fmt_fields(&self, node:&TsTypeArguments, f:&mut JsFormatter) -> FormatResult<()> {
		...
		let is_arrow_function_variables = {
			match ts_type_argument_list.first() {
				...
				Some(Ok(ty)) => {
					// we then go up until we can find a potential type
					// annotation, meaning four levels up
		...
		if should_inline {
			write!(
				f,
				[l_angle_token.format(), ts_type_argument_list.format(), r_angle_token.format()]
			)
		} else {
			write!(
				f,
