...
use crate::{AsFormat, IntoFormat, prelude::*, utils::sort_modifiers_by_precedence};
	...
	Modifier: for<'a> From<&'a Node>,
{
	fn fmt(&self, f:&mut Formatter<JsFormatContext>) -> FormatResult<()> {
		...
		// need to use peek the iterator to check if the current node is a decorator and
...
where
	...
	for node in list.iter().skip(1) {
		match node.syntax().kind() {
			...
			_ => {
				// if we encounter a non-decorator with a leading newline after a decorator and
				// the next modifier
