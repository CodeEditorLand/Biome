...
use biome_js_syntax::{JsLanguage, JsSyntaxKind::JS_DECORATOR, Modifier};
use biome_rowan::{AstNode, AstNodeList, NodeOrToken};

use crate::{prelude::*, utils::sort_modifiers_by_precedence, AsFormat, IntoFormat};
...
impl<List> FormatModifiers<List> {
	pub(crate) fn from(list:List) -> Self { Self { list } }
}
	...
	Modifier: for<'a> From<&'a Node>,
{
	fn fmt(&self, f:&mut Formatter<JsFormatContext>) -> FormatResult<()> {
		...
		// need to use peek the iterator to check if the current node is a
		// decorator and don't advance the iterator
		...
		let decorators = format_once(|f| {
			...
			while let Some(node) = iter.peek() {
				...
				match node.syntax().kind() {
					...
					_ => {
						// if we encounter a non-decorator we break out of the
						// loop
						break;
					},
				}
...
/// This function expands decorators enclosing a group if there is a newline
/// between decorators or after the last decorator.
pub(crate) fn should_expand_decorators<List, Node>(list:&List) -> bool
where
	Node: AstNode<Language = JsLanguage>,
	List: AstNodeList<Language = JsLanguage, Node = Node>, {
	// we need to skip the first node because we look for newlines between
	// decorators or after the last decorator
	for node in list.iter().skip(1) {
		match node.syntax().kind() {
			...
			_ => {
				// if we encounter a non-decorator with a leading newline after
				// a decorator and the next modifier
				return node.syntax().has_leading_newline();
			},
		}
	}

	// if we encounter a non-decorator with a leading newline after a decorator
	// and the next node or token
	list.syntax_list().node().next_sibling_or_token().map_or(false, |node| {
		match node {
