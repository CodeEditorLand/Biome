use biome_formatter::{formatter::Formatter, write, Format, FormatResult};
use biome_js_syntax::{
...
use crate::{
	js::bindings::parameters::{should_hug_function_parameters, FormatAnyJsParameters},
	prelude::*,
	JsFormatContext,
};
...
impl JsObjectPatternLike {
	...
	fn is_inline(&self, comments:&JsComments) -> FormatResult<bool> {
		...
		Ok((matches!(parent_kind, Some(JsSyntaxKind::JS_FORMAL_PARAMETER))
	...
	fn is_hug_parameter(&self, comments:&JsComments) -> bool {
		match self {
			...
			JsObjectPatternLike::JsObjectBindingPattern(binding) => {
				binding
					...
					.map_or(false, |parameters| {
						should_hug_function_parameters(&parameters, comments, false)
							.unwrap_or(false)
					})
			},
		}
	...
	fn fmt(&self, f:&mut Formatter<JsFormatContext>) -> FormatResult<()> {
		...
		match self.layout(f.comments())? {
			ObjectPatternLayout::Empty => {
				write!(f, [format_dangling_comments(self.syntax()).with_soft_block_indent()])?;
			},
			ObjectPatternLayout::Inline => {
...
enum ObjectPatternLayout {
	/// Wrap the properties in a group with
	/// [`should_expand`](Group::should_expand) equal to `expand`.
	///
	...
	/// Don't wrap the properties in a group and instead "inline" them in the
	/// parent.
	///
	/// Desired if the pattern is a parameter of a function that [should
	/// hug](should_hug_function_parameters) OR if the pattern is the left side
	/// of an assignment.
	Inline,
}
