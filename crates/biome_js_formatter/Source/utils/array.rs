...
use crate::{context::trailing_commas::FormatTrailingCommas, prelude::*, AsFormat};

/// Utility function to print array-like nodes (array expressions, array
/// bindings and assignment patterns)
	...
	for (index, element) in node.elements().enumerate() {
		...
		join.entry(
			...
			&format_with(|f| {
				...
				if is_disallow {
					// Trailing separators are disallowed, replace it with an
					// empty element
					if let Some(separator) = element.trailing_separator()? {
						write!(f, [format_removed(separator)])?;
					}
				} else if is_force || index != last_index {
					// In forced separator mode or if this element is not the
					// last in the list, print the separator
...
pub(crate) enum TrailingSeparatorMode {
	/// Trailing separators are not allowed after this element (eg. rest
	/// elements)
	Disallow,
	/// Trailing separators are inserted after this element except if its the
	/// last element and the group is not breaking
	Auto,
	/// Trailing separators will always be inserted after this element (eg. hole
	/// elements)
	Force,
}
