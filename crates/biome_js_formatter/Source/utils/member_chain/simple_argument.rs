...
use biome_js_syntax::{
	AnyJsArrayElement, AnyJsAssignment, AnyJsCallArgument, AnyJsExpression, AnyJsLiteralExpression, AnyJsName,
	AnyJsObjectMember, AnyJsObjectMemberName, AnyJsTemplateElement, JsComputedMemberExpressionFields,
	JsPostUpdateOperator, JsPreUpdateOperator, JsSpread, JsStaticMemberExpressionFields, JsTemplateExpression,
	JsUnaryOperator,
};
...
impl SimpleArgument {
				...
				if let Some(arguments) = arguments {
					...
					arguments.args().len() + usize::from(depth) <= 2
						&& arguments.args().iter().all(|argument| {
							argument.map_or(true, |argument| SimpleArgument::from(argument).is_simple_impl(depth + 1))
						})
				} else {
					true
				}
		...
		if let SimpleArgument::Expression(expression) = self {
			match expression {
				AnyJsExpression::JsStaticMemberExpression(static_expression) => {
					let JsStaticMemberExpressionFields { member, object, .. } = static_expression.as_fields();
				...
				AnyJsExpression::JsComputedMemberExpression(computed_expression) => {
					let JsComputedMemberExpressionFields { member, object, .. } = computed_expression.as_fields();
	...
	fn is_simple_non_null_assertion_expression(&self, depth: u8) -> SyntaxResult<bool> {
		if let SimpleArgument::Expression(AnyJsExpression::TsNonNullAssertionExpression(assertion)) = self {
	...
	fn is_simple_unary_expression(&self, depth: u8) -> SyntaxResult<bool> {
		if let SimpleArgument::Expression(AnyJsExpression::JsUnaryExpression(unary_expression)) = self {
			if matches!(
	...
	fn is_simple_update_expression(&self, depth: u8) -> SyntaxResult<bool> {
		...
		match self {
			SimpleArgument::Expression(AnyJsExpression::JsPreUpdateExpression(update_expression)) => {
				if matches!(
			...
			SimpleArgument::Expression(AnyJsExpression::JsPostUpdateExpression(update_expression)) => {
				if matches!(
					update_expression.operator()?,
	...
	fn is_simple_array_expression(&self, depth: u8) -> bool {
		if let SimpleArgument::Expression(AnyJsExpression::JsArrayExpression(array_expression)) = self {
			array_expression
				.elements()
				.iter()
	...
	fn is_simple_object_expression(&self, depth: u8) -> bool {
		if let SimpleArgument::Expression(AnyJsExpression::JsObjectExpression(object_expression)) = self {
			object_expression
				.members()
				.iter()
				...
				.all(|member| {
					...
					match member {
						...
						JsPropertyObjectMember(property) => {
							let is_computed =
								matches!(property.name(), Ok(AnyJsObjectMemberName::JsComputedMemberName(_)));

							let is_simple = property
								.value()
								.is_ok_and(|value| SimpleArgument::from(value).is_simple_impl(depth + 1));
