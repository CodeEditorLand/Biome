...
/// This enum tracks the arguments inside a call expressions and checks if they
/// are simple or not.
///
/// The heuristic changes based on its type and the depth of the expressions.
/// For example if we have expressions as arguments, having 2 or more them tags
/// the first argument as "not simple".
///
/// Criteria are different:
/// - *complex*: if the chain of simple arguments exceeds the depth 2 or higher
/// - *complex*: if the argument is a [JsRegexLiteralExpression] with len()
///   greater than 5
/// - *simple*: the argument is a literal
/// - *simple*: the argument is a [JsRegexLiteralExpression] with len() less
///   than 5
...
/// - *simple*: the argument is a [JsUnaryExpression], has a trivial operator
///   (`!`, `-`, `~`, or `+`), and the argument is simple
/// - *simple*: the argument is a [JsPreUpdateExpression] or
///   [JsPostUpdateExpression], with a trivial operator (`++` or `--`), and the
///   argument is simple.
/// - *simple*: the argument is a [TsNonNullAssertionExpression] and the
///   argument is simple
/// - if the argument is a template literal, check [is_simple_template_literal]
/// - if the argument is an object expression, all its members are checked if
///   they are simple or not. Check [is_simple_object_expression]
/// - if the argument is an array expression, all its elements are checked if
///   they are simple or not. Check [is_simple_array_expression]
///
...
impl SimpleArgument {
	...
	pub fn is_simple(&self) -> bool { self.is_simple_impl(0) }
	...
	fn is_simple_impl(&self, depth:u8) -> bool {
		...
		self.is_simple_template(depth)
			...
			|| self.is_simple_non_null_assertion_expression(depth).unwrap_or(false)
		...
		let result = if let SimpleArgument::Expression(any_expression) = self {
			if is_call_like_expression(any_expression) {
				...
				let arguments = match any_expression {
					...
					_ => {
						unreachable!("The check is done inside `is_call_like_expression`")
					},
				};
	...
	fn is_simple_update_expression(&self, depth:u8) -> SyntaxResult<bool> {
		// Both PreUpdate and PostUpdate expressions have Increment and
		// Decrement operators, but they are typed separately, so must be
		// handled that way. These arms should be equivalent.
		match self {
...
impl From<AnyJsExpression> for SimpleArgument {
	fn from(expr:AnyJsExpression) -> Self { Self::Expression(expr) }
}

impl From<AnyJsAssignment> for SimpleArgument {
	fn from(assignment:AnyJsAssignment) -> Self { Self::Assignment(assignment) }
}

impl From<AnyJsName> for SimpleArgument {
	fn from(name:AnyJsName) -> Self { Self::Name(name) }
}

impl From<JsSpread> for SimpleArgument {
	fn from(_:JsSpread) -> Self { Self::Spread }
}
...
/// - the expressions contained in the template are considered as
///   `is_simple_call_argument`. Check [is_simple_call_argument].
pub fn is_simple_template_literal(template:&JsTemplateExpression, depth:u8) -> SyntaxResult<bool> {
