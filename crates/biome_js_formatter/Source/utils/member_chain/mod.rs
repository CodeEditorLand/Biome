...
use crate::utils::member_chain::groups::{MemberChainGroup, MemberChainGroupsBuilder, TailChainGroups};
pub use crate::utils::member_chain::simple_argument::SimpleArgument;
use biome_formatter::{Buffer, write};
use biome_js_syntax::{
	AnyJsCallArgument, AnyJsExpression, AnyJsLiteralExpression, JsCallExpression, JsIdentifierExpression, JsSyntaxKind,
	JsSyntaxNode, JsSyntaxToken, JsThisExpression,
};
...
impl MemberChain {
	pub(crate) fn from_call_expression(
		...
		let mut chain_members = ChainMembersIterator::new(call_expression.clone().into(), comments).collect::<Vec<_>>();
		chain_members.reverse();

		// as explained before, the first group is particular, so we calculate it
		let (head_group, remaining_members) = split_members_into_head_and_remaining_groups(chain_members);
		...
		let mut member_chain = MemberChain { head: head_group, tail: tail_groups, root: call_expression };
	...
	fn should_merge_tail_with_head(
		...
		let has_computed_property = first_group.members().first().is_some_and(|item| item.is_computed_expression());
	...
	fn groups_should_break(&self, f: &mut JsFormatter) -> FormatResult<bool> {
		...
		let node_has_comments = self.head.has_comments(comments) || self.tail.has_comments(comments);
		...
		let has_empty_line_inside_tail = self.tail.iter().skip(1).any(|group| group.needs_empty_line_before());
	...
	fn has_comments(&self, comments: &JsComments) -> bool {
		...
		for member in members {
			if comments.has_leading_comments(member.syntax()) || comments.has_trailing_comments(member.syntax()) {
				return true;
			}
		}
	...
	fn fmt(&self, f: &mut Formatter<JsFormatContext>) -> FormatResult<()> {
		...
		let format_content = format_with(|f| {
			if self.groups_should_break(f)? {
				...
				let has_empty_line_before_tail = self.tail.first().is_some_and(|group| group.needs_empty_line_before());
...
fn split_members_into_head_and_remaining_groups(mut members: Vec<ChainMember>) -> (MemberChainGroup, Vec<ChainMember>) {
	...
	let first_group_end_index = if !members.first().is_some_and(|member| member.is_call_expression()) {
...
fn has_simple_arguments(call: &JsCallExpression) -> bool {
	call.arguments().is_ok_and(|arguments| {
		arguments
			.args()
			.iter()
			.all(|argument| argument.is_ok_and(|argument| SimpleArgument::new(argument).is_simple()))
	})
}
