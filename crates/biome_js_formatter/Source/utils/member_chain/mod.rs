//! Utility function that applies some heuristic to format chain member
...
//!
//! ### Flattening the AST
//! We flatten the AST. See, the code above is actually nested, where the first
//! member expression (`something`) that we see is actually the last one. This
//! is a oversimplified version of the AST:
//!
//! ```block
//! [
//!     .catch() [
...
//! the first one. In a sense, we have to revert the chain of children. We will
//! do that using a recursion.
//!
//! While we navigate the AST and we found particular nodes that we want to
//! track, we also format them. The format of these nodes is different from the
//! standard version.
//!
//! Our formatter makes sure that we don't format twice the same nodes. Let's
//! say for example that we find a `something().execute()`, its AST is like
//! this:
//!
//! ```block
//! JsCallExpression {
...
//! ```
//!
//! When we track the first [biome_js_syntax::JsCallExpression], we hold
//! basically all the children, that applies for the rest of the nodes. If we
//! decided to format all the children of each node, we will risk to format the
//! last node, the `Reference`, four times.
//!
//! To avoid this, when we encounter particular nodes, we don't format all of
//! its children, but defer the formatting to the child itself.
//!
//! The end result of the flattening, will create an array of something like
//! this:
//!
//! ```block
...
//! ### Grouping
//!
//! After the flattening, we start the grouping. We want to group nodes in a way
//! that will help us to apply a deterministic formatting.
//! - first group will be the identifier
//! - the rest of the groups will be  will start StaticMemberExpression followed
//!   by the rest of the nodes, right before the end of the next
//!   StaticMemberExpression
//!
//! The first group is special, because it holds the reference; it has its own
//! heuristic. Inside the first group we store the first element of the
//! flattened array, then:
//!
//! 1. as many as [biome_js_syntax::JsCallExpression] we can find, this cover
//!    cases like `something()()().then()`;
//! 2. as many as [biome_js_syntax::JsComputedMemberExpression] we can find,
//!    this cover cases like `something()()[1][3].then()`;
//! 3. as many as consecutive [biome_js_syntax::JsStaticMemberExpression] or
//!    [biome_js_syntax::JsComputedMemberExpression], this cover cases like
//!    `this.items[0].then()`
//!
//! The rest of the groups are essentially a sequence of
//! `[StaticMemberExpression , CallExpression]`. In order to achieve that, we
//! simply start looping through the rest of the flatten items that we haven't
//! seen.
//!
...
impl MemberChain {
	pub(crate) fn from_call_expression(
		...
		// as explained before, the first group is particular, so we calculate
		// it
		let (head_group, remaining_members) =
			split_members_into_head_and_remaining_groups(chain_members);

		// `flattened_items` now contains only the nodes that should have a
		// sequence of `[ StaticMemberExpression -> AnyNode + JsCallExpression
		// ]`
		let tail_groups = compute_remaining_groups(remaining_members, comments);

		let mut member_chain =
			MemberChain { head:head_group, tail:tail_groups, root:call_expression };

		// Here we check if the first element of Groups::groups can be moved
		// inside the head. If so, then we extract it and concatenate it
	...
	/// This function checks if the current grouping should be merged with the
	/// first group.
	fn should_merge_tail_with_head(
		...
		if self.head.members().len() == 1 {
			...
			let in_expression_statement = parent
				.map_or(false, |parent| parent.kind() == JsSyntaxKind::JS_EXPRESSION_STATEMENT);
	...
	fn last_group(&self) -> &MemberChainGroup { self.tail.last().unwrap_or(&self.head) }
	...
	fn has_comments(&self, comments:&JsComments) -> bool {
		...
		// Ignore the root member because comments are printed before/after the
		// member chain.
		members.next_back();
...
/// * The head group that contains all notes that are not a sequence of: `[
...
fn split_members_into_head_and_remaining_groups(
	...
	// This where we apply the first two points explained in the description of
	// the main public function. We want to keep iterating over the items until
	// we have call expressions
	// - `something()()()()`
	// - `something[1][2][4]`
	// - `something[1]()[3]()`
	...
	let first_group_end_index =
		if !members.first().map_or(false, |member| member.is_call_expression()) {
...
fn compute_remaining_groups(members:Vec<ChainMember>, comments:&JsComments) -> TailChainGroups {
	...
	for member in members {
		...
		match member {
			...
			ChainMember::StaticMember { .. } | ChainMember::ComputedMember { .. } => {
				// if we have seen a JsCallExpression, we want to close the
				// group. The resultant group will be something like: [ . ,
				// then, () ]; `.` and `then` belong to the previous
				// StaticMemberExpression, and `()` belong to the call
				// expression we just encountered
...
/// Here we check if the length of the groups exceeds the cutoff or there are
/// comments This function is the inverse of the prettier function
...
fn has_short_name(identifier:&JsIdentifierExpression, tab_width:TabWidth) -> bool {
	identifier
		.name()
		.and_then(|name| name.value_token())
		.map_or(false, |name| name.text_trimmed().len() <= u8::from(tab_width) as usize)
}
...
impl<'a> ChainMembersIterator<'a> {
	fn new(root:AnyJsExpression, comments:&'a JsComments) -> Self {
		Self { next:Some(root), comments, root:true }
	}
}
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		let member = match expression {
			JsCallExpression(call_expression) => {
				...
				ChainMember::CallExpression { expression:call_expression, position }
			},

			JsStaticMemberExpression(static_member) => {
				self.next = static_member.object().ok();
				ChainMember::StaticMember { expression:static_member }
			},

			JsComputedMemberExpression(computed_expression) => {
				self.next = computed_expression.object().ok();

				ChainMember::ComputedMember { expression:computed_expression }
			},
