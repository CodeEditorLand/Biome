...
impl MemberChain {
	pub(crate) fn from_call_expression(
		...
		// Here we check if the first element of Groups::groups can be moved inside the
		// head. If so, then we extract it and concatenate it together with the head.
	...
	fn has_comments(&self, comments:&JsComments) -> bool {
		...
		// Ignore the root member because comments are printed before/after the member
		// chain.
		members.next_back();
...
fn split_members_into_head_and_remaining_groups(
	...
	// This where we apply the first two points explained in the description of the
	// main public function. We want to keep iterating over the items until we have
	// call expressions
	// - `something()()()()`
	// - `something[1][2][4]`
	// - `something[1]()[3]()`
