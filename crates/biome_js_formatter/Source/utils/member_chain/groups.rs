...
impl TailChainGroups {
	...
	pub fn has_comments(&self, comments: &JsComments) -> bool {
		...
		let has_comments = members
			.any(|item| comments.has_trailing_comments(item.syntax()) || comments.has_leading_comments(item.syntax()));

		let cutoff_has_leading_comments = if !self.groups.is_empty() {
			let group = self.groups.get(1);
			if let Some(group) = group {
				let first_item = group.members.first();
				first_item.is_some_and(|first_item| comments.has_leading_comments(first_item.syntax()))
			} else {
				false
			}
		} else {
	...
	pub(super) fn has_comments(&self, comments: &JsComments) -> bool {
		self.members.iter().enumerate().any(|(index, member)| {
			if index == 0 {
				...
				comments.has_leading_comments(member.syntax()) || comments.has_trailing_comments(member.syntax())
			} else {
				false
			}
		})
	...
	pub(super) fn needs_empty_line_before(&self) -> bool {
		...
		first.is_some_and(|first| match first {
			...
			ChainMember::ComputedMember { expression } => {
				...
				match l_brack_token {
					Ok(l_brack_token) => {
						get_lines_before_token(&expression.optional_chain_token().unwrap_or(l_brack_token)) > 1
					},
					_ => false,
				}
			},
