...
use crate::{prelude::*, utils::member_chain::chain_member::ChainMember};
...
impl TailChainGroups {
	...
	pub(crate) fn first(&self) -> Option<&MemberChainGroup> { self.groups.first() }

	/// Returns the last group
	pub(crate) fn last(&self) -> Option<&MemberChainGroup> { self.groups.last() }
	...
	pub fn has_comments(&self, comments:&JsComments) -> bool {
		...
		let cutoff_has_leading_comments = if !self.groups.is_empty() {
			...
			if let Some(group) = group {
				...
				first_item
					.map_or(false, |first_item| comments.has_leading_comments(first_item.syntax()))
			} else {
				false
			}
		} else {
	...
	/// Here we check if the length of the groups exceeds the cutoff or there
	/// are comments This function is the inverse of the prettier function
	...
	/// Test if any group except the last group
	/// [break](FormatElements::will_break).
...
pub(super) struct MemberChainGroup {
	...
	///
	/// Manual implementation of `Memoized` to only memorizing the formatted
	/// result if [MemberChainGroup::will_break] is called but not otherwise.
	formatted:RefCell<Option<FormatElement>>,
}

impl MemberChainGroup {
	pub(super) fn into_members(self) -> Vec<ChainMember> { self.members }

	/// Returns the chain members of the group.
	pub(super) fn members(&self) -> &[ChainMember] { &self.members }
	...
	/// Tests if the formatted result of this group results in a
	/// [break](FormatElements::will_break).
...
impl Format<JsFormatContext> for FormatMemberChainGroup<'_> {
	fn fmt(&self, f:&mut Formatter<JsFormatContext>) -> FormatResult<()> {
		...
		let needs_parens = last.map_or(false, |last| {
			match last {
