...
use crate::{js::expressions::computed_member_expression::FormatComputedMemberLookup, prelude::*};
...
pub(crate) enum ChainMember {
	...
	/// [biome_js_syntax::JsStaticMemberExpression] Are tracked using this
	/// variant
	Node(JsSyntaxNode),
}

impl ChainMember {
	/// checks if the current node is a [biome_js_syntax::JsCallExpression], or
	/// a [biome_js_syntax::JsImportExpression]
...
impl Format<JsFormatContext> for ChainMember {
	fn fmt(&self, f:&mut JsFormatter) -> FormatResult<()> {
		match self {
			...
			ChainMember::CallExpression { expression, position } => {
