...
use biome_formatter::{token::string::normalize_string, QuoteStyle};
use biome_js_syntax::{
	JsFileSource,
	JsSyntaxKind::{JSX_STRING_LITERAL, JS_STRING_LITERAL},
	JsSyntaxToken,
};
...
use crate::{
	context::{JsFormatOptions, QuoteProperties},
	prelude::*,
};
...
pub(crate) enum StringLiteralParentKind {
	...
	ImportAttribute,
	/// Variant used when the string literal is inside a directive. This will
	/// apply a simplified logic of normalisation
	Directive,
}
...
impl<'token> FormatLiteralStringToken<'token> {
	...
	fn token(&self) -> &'token JsSyntaxToken { self.token }
...
struct StringInformation {
	...
	/// string. It could be different from the one inside the formatter
	/// options
	preferred_quote:QuoteStyle,
	/// It flags if the raw content has quotes (single or double). The raw
	/// content is the content of a string literal without the quotes
...
struct LiteralStringNormaliser<'token> {
	...
	/// When properties in objects are quoted that was set inside the
	/// configuration
...
impl From<JsFileSource> for SourceFileKind {
	fn from(st:JsFileSource) -> Self {
		if st.language().is_typescript() { Self::TypeScript } else { Self::JavaScript }
	}
}
...
impl<'token> LiteralStringNormaliser<'token> {
	...
	fn get_token(&self) -> &'token JsSyntaxToken { self.token.token() }
	...
	fn normalise_import_attribute(
		...
		if can_remove_quotes { Cow::Owned(quoteless.to_string()) } else { normalised }
	}
	...
	fn normalise_directive(&mut self, string_information:&StringInformation) -> Cow<'token, str> {
		...
		//
		// Note that we could change the quotes if the preferred quote is
		// escaped. However, Prettier doesn't go that far.
	...
	fn can_remove_number_quotes_by_file_type(&self, file_source:SourceFileKind) -> bool {
		...
		if text_to_check.bytes().next().map_or(false, |b| b.is_ascii_digit()) {
			if let Ok(parsed) = text_to_check.parse::<f64>() {
				// In TypeScript, numbers like members have different meaning
				// from numbers. Hence, if we see a number, we bail
				// straightaway
...
mod tests {
	...
	use biome_js_syntax::{
		...
		JsSyntaxKind::{JS_STRING_LITERAL, JS_STRING_LITERAL_EXPRESSION},
		JsSyntaxToken,
	};
	...
	fn generate_syntax_token(input:&str) -> JsSyntaxToken {
		...
		JsStringLiteralExpression::cast(root).unwrap().value_token().unwrap()
	}
