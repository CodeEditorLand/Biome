...
use crate::{
	context::{trailing_commas::FormatTrailingCommas, Semicolons},
	prelude::*,
};
...
/// Utility function to format the separators of the nodes that belong to the
/// unions of [biome_js_syntax::TsAnyTypeMember].
///
/// We can have two kind of separators: `,`, `;` or ASI.
/// Because of how the grammar crafts the nodes, the parent will add the
/// separator to the node. So here, we create - on purpose - an empty node.
pub(crate) struct FormatTypeMemberSeparator<'a> {
	token:Option<&'a JsSyntaxToken>,
}

impl<'a> FormatTypeMemberSeparator<'a> {
	pub fn new(token:Option<&'a JsSyntaxToken>) -> Self { Self { token } }
}
...
impl<'a> FormatInitializerClause<'a> {
	pub fn new(initializer:Option<&'a JsInitializerClause>) -> Self { Self { initializer } }
}
...
impl Format<JsFormatContext> for FormatInterpreterToken<'_> {
	fn fmt(&self, f:&mut JsFormatter) -> FormatResult<()> {
		if let Some(interpreter) = self.token {
			...
			let trimmed_range = interpreter.text_trimmed_range().sub_end(TextSize::from(diff));

			// We replace the interpreter token by its trimmed version
			write!(
				f,
				[format_replaced(interpreter, &located_token_text(interpreter, trimmed_range))]
			)?;
...
/// Formats the body of a statement where it can either be a single statement,
/// an empty statement, or a block statement.
...
impl<'a> FormatStatementBody<'a> {
	pub fn new(body:&'a AnyJsStatement) -> Self { Self { body, force_space:false } }

	/// Prevents that the consequent is formatted on its own line and indented
	/// by one level and instead gets separated by a space.
...
impl Format<JsFormatContext> for FormatStatementBody<'_> {
	fn fmt(&self, f:&mut Formatter<JsFormatContext>) -> FormatResult<()> {
		...
		if let JsEmptyStatement(empty) = &self.body {
			...
			write!(f, [indent(&format_args![soft_line_break_or_space(), self.body.format()])])
		}
	}
}
...
/// Formats a semicolon in a position where it is optional (not needed to
/// maintain syntactical correctness).
///
/// * Inserts a new semicolon if it is absent and [JsFormatOptions::semicolons]
///   is [Semicolons::Always].
/// * Removes the semicolon if it is present and [JsFormatOptions::semicolons]
///   is [Semicolons::AsNeeded].
pub(crate) struct FormatOptionalSemicolon<'a> {
	semicolon:Option<&'a JsSyntaxToken>,
}

impl<'a> FormatOptionalSemicolon<'a> {
	pub(crate) fn new(semicolon:Option<&'a JsSyntaxToken>) -> Self { Self { semicolon } }
}
...
/// Performs semicolon insertion if it is missing in the input source, the
/// [semicolons option](crate::JsFormatOptions::semicolons) is
/// [Semicolons::Always], and the preceding element isn't an bogus node
pub(crate) struct FormatSemicolon<'a> {
	semicolon:Option<&'a JsSyntaxToken>,
}

impl<'a> FormatSemicolon<'a> {
	pub fn new(semicolon:Option<&'a JsSyntaxToken>) -> Self { Self { semicolon } }
}
...
impl Format<JsFormatContext> for FormatSemicolon<'_> {
	fn fmt(&self, f:&mut JsFormatter) -> FormatResult<()> {
		match self.semicolon {
			...
			None => {
				let is_after_bogus =
					f.elements().start_tag(TagKind::Verbatim).map_or(false, |signal| {
						match signal {
