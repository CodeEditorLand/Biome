	...
	fn entry(&mut self, element:EachTemplateElement) {
		match &element {
			EachTemplateElement::Column(column) => {
				...
				// if there was no column with a line break, then add width of the current
				// column to the buffer
...
impl EachTemplateTable {
	pub(crate) fn from(list:&JsTemplateElementList, f:&mut JsFormatter) -> FormatResult<Self> {
		...
		let header = match iter.next() {
			...
			// we check this case in `is_test_each_pattern` and `is_test_each_pattern_elements`
			// functions
			_ => return Err(FormatError::SyntaxError),
		};

		// It's safe to mark the header as checked here because we check that node
		// doesn't have any trivia when we call `is_test_each_pattern`
		...
		while let Some(element) = iter.next() {
			match element {
				AnyJsTemplateElement::JsTemplateChunkElement(element) => {
					// It's safe to mark the element as checked here because we check that node
					// doesn't have any trivia when we call `is_test_each_pattern`
				...
				AnyJsTemplateElement::JsTemplateElement(element) => {
					...
					// The softline buffer replaces all softline breaks with a space or removes it
					// entirely to "mimic" an infinite print width
