...
use crate::{
	js::{
		auxiliary::template_element::TemplateElementOptions,
		lists::template_element_list::TemplateElementIndention,
	},
	prelude::*,
};
...
impl EachTemplateColumn {
	fn new(text:String, range:TextRange, will_break:bool) -> Self {
		...
		EachTemplateColumn { text, width, range, will_break }
	}
}
...
impl EachTemplateTableBuilder {
	...
	fn entry(&mut self, element:EachTemplateElement) {
		match &element {
			EachTemplateElement::Column(column) => {
				...
				// if there was no column with a line break, then add width of
				// the current column to the buffer
	...
	/// Merge the current row columns width with the table ones if row doesn't
	/// contain a line break column.
	fn next_row(&mut self) {
		...
		self.rows
			.push(EachTemplateRow { has_line_break_column:self.current_row.has_line_break_column });
...
impl EachTemplateCurrentRow {
	fn new() -> Self {
		EachTemplateCurrentRow { column_widths:Vec::new(), has_line_break_column:false }
	}
...
impl Format<JsFormatContext> for EachTemplateSeparator {
	fn fmt(&self, f:&mut Formatter<JsFormatContext>) -> FormatResult<()> { write!(f, [text("|")]) }
}
...
impl EachTemplateTable {
	pub(crate) fn from(list:&JsTemplateElementList, f:&mut JsFormatter) -> FormatResult<Self> {
		...
		let header = match iter.next() {
			...
			// we check this case in `is_test_each_pattern` and
			// `is_test_each_pattern_elements` functions
			_ => return Err(FormatError::SyntaxError),
		};

		// It's safe to mark the header as checked here because we check that
		// node doesn't have any trivia when we call `is_test_each_pattern`
		f.context().comments().mark_suppression_checked(header.syntax());
		...
		while let Some(element) = iter.next() {
			match element {
				AnyJsTemplateElement::JsTemplateChunkElement(element) => {
					// It's safe to mark the element as checked here because we
					// check that node doesn't have any trivia when we call
					// `is_test_each_pattern`
					f.context().comments().mark_suppression_checked(element.syntax());

					write!(f, [format_removed(&element.template_chunk_token()?)])?;

					let has_line_break =
						element.template_chunk_token()?.text_trimmed().contains('\n');
					let is_last = iter.peek().is_none();

					// go to the next line if the current element contains a
					// line break
				...
				AnyJsTemplateElement::JsTemplateElement(element) => {
					...
					// The softline buffer replaces all softline breaks with a
					// space or removes it entirely to "mimic" an infinite
					// print width
			...
			while let Some(element) = iter.next() {
				...
				match element {
					EachTemplateElement::Column(column) => {
						...
						// align the column based on the maximum column width in
						// the table
						if !is_last_in_row {
