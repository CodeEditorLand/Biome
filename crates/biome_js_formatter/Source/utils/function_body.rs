		...
		match self.mode {
			...
			FunctionBodyCacheMode::Cached => {
				match f.context().get_cached_function_body(self.body) {
					...
					None => {
						// This can happen in the unlikely event where a
						// function has a parameter with an initializer
						// that contains a call expression with a first or last
						// function/arrow ```javascript
						// test((
						//   problematic = test(() => body)
						// ) => {});
						// ```
						// This case should be rare as it requires very specific
						// syntax (and is rather messy to write)
						// which is why it's fine to just fallback to formatting
						// the body again in this case.
						write!(f, [self.body.format()])
					},
				}
			},
