...
impl Format<JsFormatContext> for AnyJsBinaryLikeExpression {
	fn fmt(&self, f:&mut Formatter<JsFormatContext>) -> FormatResult<()> {
		...
		// Don't indent inside of conditions because conditions add their own indent and
		// grouping.
		...
		if let Some(parent) = parent.as_ref() {
			// Add a group with a soft block indent in cases where it is necessary to
			// parenthesize the binary expression. For example, `(a+b)(call)`, `!(a +
			// b)`, `(a + b).test`.
...
fn split_into_left_and_right_sides(
	...
	// Stores the left and right parts of the binary expression in sequence (rather
	// than nested as they appear in the tree).
...
enum BinaryLeftOrRightSide {
	...
	Right {
		...
		/// Is the parent the condition of a `if` / `while` / `do-while` / `for`
		/// statement?
		inside_condition:bool,
	...
	fn fmt(&self, f:&mut Formatter<JsFormatContext>) -> FormatResult<()> {
		match self {
			...
			Self::Right {
				...
				// formatting OR the formatting of the right hand side value but not of a
				// nested binary expression. This aligns with Prettier's behaviour.
			...
			match next {
				VisitEvent::Enter(binary) => {
					if binary.syntax() == &self.start {
						...
						// SAFETY: Calling `unwrap` here is safe because the iterator only enters
						// (traverses into) a node if it is a valid binary like expression and
						// it is guaranteed to have a parent.
		...
		match &next {
			VisitEvent::Enter(binary) => {
				...
				if let Some(binary) = next {
					...
					// If left is missing or it isn't a binary like expression, then format it as
					// part of the parent binary like expression
			...
			VisitEvent::Exit(node) => {
				if node.syntax() != &self.start {
					self.next = node.syntax().parent().map(|parent| {
						// SAFETY: Calling `unwrap` here is safe because the iterator only enters
						// (traverses into) a node if it is a valid binary like expression.
...
mod tests {
	...
	use biome_js_syntax::{JsLogicalExpression, binary_like_expression::AnyJsBinaryLikeExpression};
