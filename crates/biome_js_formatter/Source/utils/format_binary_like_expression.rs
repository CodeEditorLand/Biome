...
//!
//! The challenge of formatting binary like expressions is that we want to
//! format binary expression chains, when possible, together but they are
//! represented as a deep structured tree in the CST.
//!
//! For example,
//!
//! ```js
...
//! ```
//!
//! The goal is to format all the left and right sides together that don't
//! require parentheses (mainly comes down to whether the parent and its left
//! side's operator have the same precedence).
//!
//! This is achieved by traversing down the left side of a binary expression
//! until it reaches the first expression that can't be flattened. For `some &&
//! thing && elsewhere || happy`, the implementation checks if the first
//! left-side `some && thing && elsewhere` can be grouped. This isn't the case
//! because the left side operator `&&` differs from the parent's `||` operator.
//!
//! That means, we found the end of the first `group` and the left-side of the
//! group is `some && thing && elsewhere`. The algorithm traverses upwards and
//! adds all right-sides of the parent binary like expressions to the group
//! until it reaches the root. In the example, this only is the `|| happy`.
//!
//! Thus, the first group is: `[Left(some && thing && elsewhere), Right(||
//! happy)]`. The formatting formats the left side as is (the call will recurse
//! into the [AnyJsBinaryLikeExpression] formatting again) but formats the
//! operator with the right side.
//!
//! Now, let's see how the implementation groups the `some && thing &&
//! elsewhere`. It first traverses to the left most binary like expression,
//! which is `some && thing`. It then adds this as a `Left` side to the group.
//! From here, the algorithm traverses upwards and adds all right sides
//! of the binary expression. These are: `&& thing` and `&& elsewhere`.
//! The complete group is: `[Left(some), Right(&& thing), Right(&& elsewhere)]`.
//!
//! Each side in the group gets formatted in order, starting with the left, then
//! formatting the operator and right side of each Right side.
...
use crate::{js::expressions::static_member_expression::AnyJsStaticMemberLike, prelude::*};
...
impl Format<JsFormatContext> for AnyJsBinaryLikeExpression {
	fn fmt(&self, f:&mut Formatter<JsFormatContext>) -> FormatResult<()> {
		...
		// Don't indent inside of conditions because conditions add their own
		// indent and grouping.
		...
		if let Some(parent) = parent.as_ref() {
			// Add a group with a soft block indent in cases where it is
			// necessary to parenthesize the binary expression. For example,
			// `(a+b)(call)`, `!(a + b)`, `(a + b).test`.
...
/// * is a [JsBinaryLikeExpression] but it should be formatted as its own group
///   (see [AnyJsBinaryLikeExpression::can_flatten]).
///
/// It then traverses upwards from the left most node and creates
/// [BinaryLikeLeftOrRightSide::Right]s for every [JsBinaryLikeExpression] until
/// it reaches the root again.
fn split_into_left_and_right_sides(
	...
	// Stores the left and right parts of the binary expression in sequence
	// (rather than nested as they appear in the tree).
		...
		match event {
			VisitEvent::Enter(binary) => {
				if !binary.can_flatten()? {
					// Stop at this expression. This is either not a binary
					// expression OR it has different precedence and needs
					// to be grouped separately. Calling skip_subtree
					// prevents the exit event being triggered for this event.
...
fn should_indent_if_parent_inlines(parent:Option<&JsSyntaxNode>) -> bool {
	parent.map_or(false, |parent| {
		match parent.kind() {
			JsSyntaxKind::JS_ASSIGNMENT_EXPRESSION | JsSyntaxKind::JS_PROPERTY_OBJECT_MEMBER => {
				true
			},

			JsSyntaxKind::JS_INITIALIZER_CLAUSE => {
				parent.parent().map_or(false, |grand_parent| {
					matches!(
						grand_parent.kind(),
...
enum BinaryLeftOrRightSide {
	...
	Right {
		...
		/// Is the parent the condition of a `if` / `while` / `do-while` /
		/// `for` statement?
		inside_condition:bool,

		/// Indicates if the comments of the parent should be printed or not.
		/// Must be true if `parent` isn't the root `JsAnyBinaryLike` for which
		/// `format` is called.
	...
	fn fmt(&self, f:&mut Formatter<JsFormatContext>) -> FormatResult<()> {
		match self {
			...
			Self::Right {
				...
				// It's only possible to suppress the formatting of the whole
				// binary expression formatting OR the formatting of the
				// right hand side value but not of a nested binary expression.
				...
				// Doesn't match prettier that only distinguishes between
				// logical and binary
				...
				let left_has_same_kind =
					binary_like_expression.left()?.into_expression().map_or(false, |left| {
				...
				if *print_parent_comments {
					write!(f, [format_leading_comments(binary_like_expression.syntax())])?;
				}
...
fn is_same_binary_expression_kind(binary:&AnyJsBinaryLikeExpression, other:&JsSyntaxNode) -> bool {
	match binary {
		AnyJsBinaryLikeExpression::JsLogicalExpression(_) => {
...
/// The [BinaryLikePreorder] visits every node twice. First on the way down to
/// find the left most binary like expression, then on the way back up. This
/// enum encodes the information whatever the iterator is on its way down
/// (`Enter`) or traversing upwards (`Exit`).
...
/// This is similar to [JsSyntaxNode::descendants] but it only traverses into
/// [AnyJsBinaryLikeExpression] and their left side (the right side is never
/// visited).
///
/// # Examples
///
...
/// * Exit(`a && b && c && d && e`)
///
/// Notice how the iterator doesn't yield events for the terminal identifiers
/// `a`, `b`, `c`, `d`, and `e`, nor for the right hand side expression `d &&
/// e`. This is because the visitor only traverses into
/// [AnyJsBinaryLikeExpression]s and of those, only along the left side.
struct BinaryLikePreorder {
	/// The next node to visit or [None] if the iterator passed the start node
	/// (is at its end).
...
impl BinaryLikePreorder {
	...
	fn skip_subtree(&mut self) {
		self.next = self.next.take().and_then(|next| {
			match next {
				VisitEvent::Enter(binary) => {
					if binary.syntax() == &self.start {
						None
					} else {
						// SAFETY: Calling `unwrap` here is safe because the
						// iterator only enters (traverses into) a node if
						// it is a valid binary like expression and it is
...
impl Iterator for BinaryLikePreorder {
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		match &next {
			VisitEvent::Enter(binary) => {
				let next = binary.left().ok().and_then(|left| left.into_expression()).and_then(
					|expression| AnyJsBinaryLikeExpression::cast(expression.into_syntax()),
				);

				if let Some(binary) = next {
					self.next = Some(VisitEvent::Enter(binary));
				} else {
					// If left is missing or it isn't a binary like expression,
					// then format it as part of the parent binary like
					// expression
			...
			VisitEvent::Exit(node) => {
				if node.syntax() != &self.start {
					self.next = node.syntax().parent().map(|parent| {
						// SAFETY: Calling `unwrap` here is safe because the
						// iterator only enters (traverses into) a node if
						// it is a valid binary like expression.
...
mod tests {
	...
	use biome_js_syntax::{binary_like_expression::AnyJsBinaryLikeExpression, JsLogicalExpression};
	...
	fn in_order_visits_every_binary_like_expression() {
		...
		let root = parse.syntax().descendants().find_map(JsLogicalExpression::cast).unwrap();
		...
		assert_eq!(
			iterator.next(),
			Some(VisitEvent::Enter(AnyJsBinaryLikeExpression::from(root.clone())))
		);
		assert_eq!(
			iterator.next(),
			Some(VisitEvent::Enter(AnyJsBinaryLikeExpression::from(a_and_b_and_c.clone())))
		);
		assert_eq!(
			iterator.next(),
			Some(VisitEvent::Enter(AnyJsBinaryLikeExpression::from(a_and_b.clone())))
		);
		...
		assert_eq!(
			iterator.next(),
			Some(VisitEvent::Exit(AnyJsBinaryLikeExpression::from(a_and_b_and_c)))
		);
		assert_eq!(iterator.next(), Some(VisitEvent::Exit(AnyJsBinaryLikeExpression::from(root))));
	}
