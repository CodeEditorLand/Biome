	...
	fn is_complex(&self) -> bool {
		match self {
			AnyObjectPattern::JsObjectAssignmentPattern(assignment_pattern) => {
				...
				assignment_pattern.properties().iter().flatten().any(|property| match property {
			...
			AnyObjectPattern::JsObjectBindingPattern(binding_pattern) => {
				...
				binding_pattern.properties().iter().flatten().any(|property| match property {
...
fn is_complex_type_arguments(type_arguments: TsTypeArguments) -> SyntaxResult<bool> {
	...
	let is_first_argument_complex =
		ts_type_argument_list.iter().next().transpose()?.map_or(false, |first_argument| {
			matches!(
				first_argument,
