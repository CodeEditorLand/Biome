...
impl AnyJsAssignmentLike {
	fn right(&self) -> SyntaxResult<RightAssignmentLike> {
		let right = match self {
			...
			AnyJsAssignmentLike::JsVariableDeclarator(variable_declarator) => {
				...
				// `has_only_left_hand_side` variant at the beginning of the `layout` function
	...
	fn write_left(&self, f:&mut JsFormatter) -> FormatResult<bool> {
		match self {
			AnyJsAssignmentLike::JsPropertyObjectMember(property) => {
				...
				// It's safe to mark the name as checked here because it is at the beginning of
				// the property and any suppression comment that would apply to the name
				// applies to the property too and is, thus, handled on the property level.
	...
	fn layout(
		...
		// Before checking `BreakAfterOperator` layout, we need to unwrap the right
		// expression from `JsUnaryExpression` or `TsNonNullAssertionExpression` [Prettier applies]: https://github.com/prettier/prettier/blob/a043ac0d733c4d53f980aa73807a63fc914f23bd/src/language-js/print/assignment.js#L199-L211
		// Example:
		//  !"123" -> "123"
		//  void "123" -> "123"
		//  !!"string"! -> "string"
	...
	fn chain_formatting_layout(
		...
		// The chain goes up two levels, by checking up to the great parent if all the
		// conditions are correctly met.
	...
	fn should_break_after_operator(
		...
		let result = match right {
			...
			RightAssignmentLike::AnyTsType(AnyTsType::TsUnionType(ty)) => {
				// Recursively checks if the union type is nested and identifies the innermost
				// union type. If a leading comment is found while navigating to the inner
				// union type, it is considered as having leading comments.
...
impl Format<JsFormatContext> for AnyJsAssignmentLike {
	fn fmt(&self, f:&mut JsFormatter) -> FormatResult<()> {
		let format_content = format_with(|f| {
			// We create a temporary buffer because the left hand side has to conditionally
			// add a group based on the layout, but the layout can only be computed by
			// knowing the width of the left hand side. The left hand side can be a
			// member, and that has a width can can be known only when it's formatted (it
			// can incur in some transformation, like removing some escapes, etc.).
			//
			...
			// If not (for example, left is not an identifier), then let's fallback to
			// false, so we can continue the chain of checks
	...
	if let AnyJsCallArgument::AnyJsExpression(expression) = argument {
		let is_short_argument = match expression {
			...
			AnyJsExpression::JsTemplateExpression(template) => {
				...
				// Besides checking length exceed we also need to check that the template
				// doesn't have any expressions. It means that the elements of the template
				// are empty or have only one `JsTemplateChunkElement` element Prettier: https://github.com/prettier/prettier/blob/a043ac0d733c4d53f980aa73807a63fc914f23bd/src/language-js/print/assignment.js#L402-L405
				match elements.len() {
					0 => true,
					1 => {
