	...
	match child.expression() {
		Some(AnyJsExpression::AnyJsLiteralExpression(
			...
			(Ok(_), Ok(value_token), Ok(r_curly_token)) => {
				...
				let has_comments =
					comments.has_skipped(&r_curly_token) || comments.has_comments(literal.syntax());
...
mod tests {
	...
	fn assert_jsx_text_chunks(text: &str, expected_chunks: Vec<(TextSize, JsxTextChunk)>) {
		...
		assert!(!parse.has_errors(), "Source should not have any errors {:?}", parse.diagnostics());
	...
	fn parse_jsx_children(children: &str) -> JsxChildList {
		...
		parse.syntax().descendants().find_map(JsxChildList::cast).expect("Expect a JsxChildList")
	}
