...
use crate::{JsCommentStyle, prelude::*};
						...
						Some((_, JsxTextChunk::Whitespace(whitespace))) => {
							if whitespace.contains('\n') {
								if chunks.peek().is_none() {
									// A text only consisting of whitespace that also contains a new
									// line isn't considered meaningful text.
									// It can be entirely removed from the content without changing
									// the semantics.
									let newlines =
				...
				while let Some(chunk) = chunks.next() {
					match chunk {
						(_, JsxTextChunk::Whitespace(whitespace)) => {
							// Only handle trailing whitespace. Words must always be joined by new
							// lines
							if chunks.peek().is_none() {
								if whitespace.contains('\n') {
									builder.entry(JsxChild::Newline);
