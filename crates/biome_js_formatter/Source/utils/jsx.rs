...
use crate::{prelude::*, JsCommentStyle};
...
///
/// Suppression for [JsxAnyTag] differs from regular nodes if they are inside of
/// a [JsxChildList] because they can then not be preceded by a comment.
///
/// A [JsxAnyTag] inside of a [JsxChildList] is suppressed if its first
/// preceding sibling (that contains meaningful text) is a [JsxExpressionChild],
/// not containing any expression, with a dangling suppression comment.
///
/// ```javascript
/// <div>
	...
	match tag.parent::<JsxChildList>() {
		Some(_) => {
			let prev_non_empty_text_sibling =
				tag.syntax().siblings(Direction::Prev).skip(1).find(|sibling| {
			...
			match prev_non_empty_text_sibling.and_then(JsxExpressionChild::cast) {
				Some(child) if child.expression().is_none() => {
					comments
...
/// Indicates that an element should always be wrapped in parentheses, should be
/// wrapped only when it's line broken, or should not be wrapped at all.
...
pub(crate) enum WrapState {
	...
	/// For instance, a JSX element nested in a let binding is wrapped on line
	/// break: ```jsx
	///  let component = <div> La Haine dir. Mathieu Kassovitz </div>;
	///
	///  let component = (
	///   <div> Uncle Boonmee Who Can Recall His Past Lives dir. Apichatpong
	/// Weerasethakul </div>  );
	/// ```
	WrapOnBreak,
}

/// Checks if a JSX Element should be wrapped in parentheses. Returns a
/// [WrapState] which indicates when the element should be wrapped in
/// parentheses.
pub(crate) fn get_wrap_state(node:&JsxTagExpression) -> WrapState {
	// We skip the first item because the first item in ancestors is the node
	// itself, i.e. the JSX Element in this case.
	let parent = node.syntax().parent();

	parent.map_or(WrapState::NoWrap, |parent| {
		match parent.kind() {
...
pub(crate) fn is_whitespace_jsx_expression(
	...
	match child.expression() {
		Some(AnyJsExpression::AnyJsLiteralExpression(
			...
			match (child.l_curly_token(), literal.value_token(), child.r_curly_token()) {
	...
	for child in children {
		match child {
			AnyJsxChild::JsxText(text) => {
				...
				// Keep track if there's any leading/trailing empty line, new
				// line or whitespace
						...
						Some((_, JsxTextChunk::Whitespace(whitespace))) => {
							if whitespace.contains('\n') {
								if chunks.peek().is_none() {
									// A text only consisting of whitespace that
									// also contains a new line isn't considered
									// meaningful text.
									// It can be entirely removed from the
									// content without changing the semantics.
									let newlines =
										whitespace.bytes().filter(|b| *b == b'\n').count();

									// Keep up to one blank line between
									// tags/expressions and text.
									// ```javascript
									// <div>
									//
									//   <MyElement />
				...
				while let Some(chunk) = chunks.next() {
					match chunk {
						(_, JsxTextChunk::Whitespace(whitespace)) => {
							// Only handle trailing whitespace. Words must
							// always be joined by new lines
							if chunks.peek().is_none() {
								if whitespace.contains('\n') {
									builder.entry(JsxChild::Newline);
								} else {
...
/// 1. Remove [JsxChild::EmptyLine], [JsxChild::Newline], [JsxChild::Whitespace]
///    if a next element is [JsxChild::Whitespace]
/// 2. Don't push a new element [JsxChild::EmptyLine], [JsxChild::Newline],
///    [JsxChild::Whitespace] if previous one is [JsxChild::EmptyLine],
///    [JsxChild::Newline], [JsxChild::Whitespace]
...
impl JsxSplitChildrenBuilder {
	fn new() -> Self { JsxSplitChildrenBuilder { buffer:vec![] } }
	...
	fn finish(self) -> Vec<JsxChild> { self.buffer }
}
...
pub(crate) enum JsxChild {
	...
	/// A new line at the start or end of a [JsxText] with meaningful content.
	/// (that isn't all whitespace and contains a new line).
	///
	/// ```javascript
	/// <div>
	///     a
	...
	/// A [JsxText] that only consists of whitespace and has at least two line
	/// breaks;
	///
	/// ```javascript
	/// <div>
...
/// A word in a Jsx Text. A word is string sequence that isn't separated by any
/// JSX whitespace.
#[derive(Debug, Clone, Eq, PartialEq)]
...
impl JsxWord {
	fn new(text:TokenText, source_position:TextSize) -> Self { JsxWord { text, source_position } }

	pub(crate) fn is_single_character(&self) -> bool { self.text.chars().count() == 1 }
}
...
impl<'a> JsxSplitChunksIterator<'a> {
	fn new(text:&'a str) -> Self {
		Self { position:TextSize::default(), text, chars:text.chars().peekable() }
	}
}
...
mod tests {
	...
	fn assert_jsx_text_chunks(text:&str, expected_chunks:Vec<(TextSize, JsxTextChunk)>) {
		let parse =
			parse(&std::format!("<>{text}</>"), JsFileSource::jsx(), JsParserOptions::default());
		assert!(
			!parse.has_errors(),
