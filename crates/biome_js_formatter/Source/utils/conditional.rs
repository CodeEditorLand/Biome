...
use crate::{AsFormat, IntoFormat, prelude::*};
...
pub struct FormatJsAnyConditionalRule {
	...
	/// Gets passed through from the root to the consequent and alternate of
	/// [JsConditionalExpression]s.
	///
...
impl FormatRule<AnyJsConditional> for FormatJsAnyConditionalRule {
	...
	fn fmt(
		...
		let format_consequent_and_alternate = format_with(|f| {
			write!(
				f,
				[soft_line_break_or_space(), conditional.question_mark_token().format(), space()]
			)?;
			...
			if is_consequent_nested {
				// Add parentheses around the consequent if it is a conditional expression and
				// fits on the same line so that it's easier to identify the parts that
				// belong to a conditional expression. `a ? b ? c: d : e` -> `a ? (b ? c: d)
				// : e
				write!(
					f,
					[
			...
			write!(f, [soft_line_break_or_space(), conditional.colon_token().format(), space()])?;
			...
			match conditional {
				...
				_ => {
					...
					if layout.is_nested_consequent() {
						// This may look silly but the `dedent` is to remove the outer `align` added
						// by the parent's formatting of the consequent. The `indent` is
						// necessary to indent the content by one level with a tab.
						// Adding an `indent` without the `dedent` would result in the `outer` align
						// being converted into a `indent` + the `indent` added here,
						// ultimately resulting in a two-level indention.
		...
		let format_inner = format_with(|f| {
			write!(f, [FormatConditionalTest { conditional, layout:&layout }])?;

			// Indent the `consequent` and `alternate` **only if** this is the root
			// conditional expression OR this is the `test` of a conditional expression.
			...
			// Add a soft line break in front of the closing `)` in case the parent is a
		...
		let grouped = format_with(|f| {
			if layout.is_root() { group(&format_inner).fmt(f) } else { format_inner.fmt(f) }
		});
		...
		if layout.is_nested_test() || should_extra_indent {
			group(&soft_block_indent(&grouped)).should_expand(has_multiline_comment).fmt(f)
		} else {
...
impl FormatJsAnyConditionalRule {
	fn layout(&self, conditional:&AnyJsConditional, source_type:JsFileSource) -> ConditionalLayout {
		match conditional.syntax().parent() {
			Some(parent) if parent.kind() == conditional.syntax().kind() => {
				...
				if conditional_parent.is_test(conditional.syntax()) {
					ConditionalLayout::NestedTest { parent:conditional_parent }
				} else if conditional_parent.is_alternate(conditional.syntax()) {
					ConditionalLayout::NestedAlternate { parent:conditional_parent }
				} else {
					ConditionalLayout::NestedConsequent { parent:conditional_parent }
				}
			},
			...
			parent => {
				...
				ConditionalLayout::Root { parent, jsx_chain:is_jsx_chain.into() }
			},
		}
	}

	/// It is desired to add an extra indent if this conditional is a
	/// [JsConditionalExpression] and is directly inside of a member chain:
	///
	/// ```javascript
	/// // Input
	/// return (a ? b : c).member
	...
	fn should_extra_indent(
		...
		let ancestors = layout.parent().into_iter().flat_map(|parent| parent.ancestors());

		let mut parent = None;

		let mut expression = AnyJsExpression::from(conditional.clone());

		// This tries to find the start of a member chain by iterating over all
		// ancestors of the conditional. The iteration "breaks" as soon as a
		// non-member-chain node is found.
		...
		// e.g. don't indent for `return a ? b : c`, only for `return (a ? b :
		// c).member`
			...
			Some(parent) => {
				let argument = match parent.kind() {
					...
					JsSyntaxKind::JS_ASSIGNMENT_EXPRESSION => {
						...
						assignment_expression.right().ok().map(AnyJsExpression::from)
					},
	...
	/// Returns `true` if this is the root conditional expression and the parent
	/// is a [JsStaticMemberExpression].
	fn is_parent_static_member_expression(
		...
		match layout {
			ConditionalLayout::Root { parent: Some(parent), .. } => {
...
/// A [JsConditionalExpression] that itself or any of its parent's
/// [JsConditionalExpression] have a a [JsxTagExpression]
/// as its [`test`](JsConditionalExpression::test),
/// [`consequent`](JsConditionalExpression::consequent) or
/// [`alternate`](JsConditionalExpression::alternate).
///
/// Parenthesizes the `consequent` and `alternate` if it the group breaks except
...
impl ConditionalJsxChain {
	pub const fn is_chain(&self) -> bool { matches!(self, ConditionalJsxChain::Chain) }

	pub const fn is_no_chain(&self) -> bool { matches!(self, ConditionalJsxChain::NoChain) }
}
...
impl ConditionalLayout {
	...
	const fn is_root(&self) -> bool { matches!(self, ConditionalLayout::Root { .. }) }
	...
	const fn is_nested_test(&self) -> bool { matches!(self, ConditionalLayout::NestedTest { .. }) }
...
impl AnyJsConditional {
	...
	fn is_test(&self, node:&JsSyntaxNode) -> bool {
		match self {
			AnyJsConditional::JsConditionalExpression(conditional) => {
				conditional.test().ok().map_or(false, |resolved| resolved.syntax() == node)
			},
			AnyJsConditional::TsConditionalType(conditional) => {
				conditional.check_type().map(AstNode::into_syntax).as_ref() == Ok(node)
					|| conditional.extends_type().map(AstNode::into_syntax).as_ref() == Ok(node)
			},
		}
	}
	...
	/// Returns `true` if the passed node is the `alternate` of this conditional
	/// expression.
...
fn format_jsx_chain_alternate(alternate:&AnyJsExpression) -> FormatJsxChainExpression {
	FormatJsxChainExpression { expression:alternate, alternate:true }
}
		...
		let format_expression = format_with(|f| {
			match self.expression {
				JsConditionalExpression(conditional) => {
					write!(f, [conditional.format().with_options(ConditionalJsxChain::Chain)])
				},
