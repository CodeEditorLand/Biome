...
use crate::{prelude::*, AsFormat, IntoFormat};
	...
	fn fmt(
		...
		let format_consequent_and_alternate = format_with(|f| {
			...
			if is_consequent_nested {
				// Add parentheses around the consequent if it is a conditional
				// expression and fits on the same line so that it's easier
				// to identify the parts that belong to a conditional
			...
			match conditional {
				...
				_ => {
					...
					if layout.is_nested_consequent() {
						// This may look silly but the `dedent` is to remove the
						// outer `align` added by the parent's formatting of the
						// consequent. The `indent` is necessary to indent
						// the content by one level with a tab.
						// Adding an `indent` without the `dedent` would result
						// in the `outer` align being converted
						// into a `indent` + the `indent` added here, ultimately
						// resulting in a two-level indention.
		...
		let format_inner = format_with(|f| {
			...
			// Indent the `consequent` and `alternate` **only if** this is the
			// root conditional expression OR this is the `test` of a
			...
			// Add a soft line break in front of the closing `)` in case the
			// parent is a static member expression ```
			// (veryLongCondition
			//      ? a
