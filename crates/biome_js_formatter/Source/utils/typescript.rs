...
/// Utility function that checks if the current type is can categorized as
/// "simple"
...
pub(crate) fn should_hug_type(ty:&AnyTsType, f:&Formatter<JsFormatContext>) -> bool {
	...
	// Checking for unions where all types but one are "void types", so things
	// like `TypeName | null | void`
	if let AnyTsType::TsUnionType(union_type) = ty {
		let mut iter = union_type.types().iter();

		let has_object_type = iter
			.any(|ty| matches!(ty, Ok(AnyTsType::TsObjectType(_) | AnyTsType::TsReferenceType(_))));
		...
		let void_count = union_type
			...
			.iter()
			.filter(|node| {
				matches!(node, Ok(AnyTsType::TsVoidType(_) | AnyTsType::TsNullLiteralType(_)))
			})
			.count();
