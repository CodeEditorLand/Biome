...
type JsFormatSeparatedIter<Node> =
	FormatSeparatedIter<AstSeparatedListElementsIterator<JsLanguage, Node>, Node, JsFormatSeparatedElementRule<Node>>;

/// AST Separated list formatting extension methods
pub(crate) trait FormatAstSeparatedListExtension: AstSeparatedList<Language = JsLanguage> {
	...
	fn format_separated(&self, separator: &'static str) -> JsFormatSeparatedIter<Self::Node> {
		JsFormatSeparatedIter::new(self.elements(), separator, JsFormatSeparatedElementRule { node: PhantomData })
	}
}
