...
use std::{fmt, fmt::Debug, rc::Rc, str::FromStr};
...
pub struct JsFormatContext {
	...
	///
	/// Used during formatting of call arguments where function expressions and
	/// arrow function expressions are formatted a second time if they are the
	/// first or last call argument.
	///
	/// Caching the body in the call arguments formatting is important. It
	/// minimises the cases where the algorithm is quadratic, in case the
	/// function or arrow expression contains another call expression with a
	/// function or call expression as first or last argument.
	///
	/// It's sufficient to only store a single cached body to cover the vast
	/// majority of cases (there's no exception in any of our tests nor
	/// benchmark tests). The only case not covered is when a parameter has an
	/// initializer that contains a call expression:
	///
	/// ```javascript
	///  test((
...
impl JsFormatContext {
	pub fn new(options:JsFormatOptions, comments:JsComments) -> Self {
		Self { options, comments:Rc::new(comments), cached_function_body:None, source_map:None }
	}

	/// Returns the formatted content for the passed function body if it is
	/// cached or `None` if the currently cached content belongs to another
	/// function body or the cache is empty.
	///
	/// See [JsFormatContext::cached_function_body] for more in depth
	/// documentation.
	pub(crate) fn get_cached_function_body(
		&self,
		body:&AnyJsFunctionBody,
	) -> Option<FormatElement> {
		self.cached_function_body.as_ref().and_then(|(expected_body, formatted)| {
			if expected_body == body { Some(formatted.clone()) } else { None }
		})
	}
...
impl From<u8> for TabWidth {
	fn from(value:u8) -> Self { TabWidth(value) }
}

impl From<TabWidth> for u8 {
	fn from(width:TabWidth) -> Self { width.0 }
}
...
impl CstFormatContext for JsFormatContext {
	...
	fn comments(&self) -> &JsComments { &self.comments }
}
...
pub struct JsFormatOptions {
	...
	/// Whether to add non-necessary parentheses to arrow functions. Defaults to
	/// "always".
	...
	/// Whether to hug the closing bracket of multiline HTML/JSX tags to the end
	/// of the last line, rather than being alone on the following line.
	/// Defaults to false.
...
impl JsFormatOptions {
	...
	pub fn set_semicolons(&mut self, semicolons:Semicolons) { self.semicolons = semicolons; }

	pub fn arrow_parentheses(&self) -> ArrowParentheses { self.arrow_parentheses }

	pub fn bracket_spacing(&self) -> BracketSpacing { self.bracket_spacing }

	pub fn bracket_same_line(&self) -> BracketSameLine { self.bracket_same_line }

	pub fn quote_style(&self) -> QuoteStyle { self.quote_style }

	pub fn jsx_quote_style(&self) -> QuoteStyle { self.jsx_quote_style }

	pub fn quote_properties(&self) -> QuoteProperties { self.quote_properties }

	pub fn source_type(&self) -> JsFileSource { self.source_type }

	pub fn trailing_commas(&self) -> TrailingCommas { self.trailing_commas }

	pub fn semicolons(&self) -> Semicolons { self.semicolons }

	pub fn tab_width(&self) -> TabWidth { self.indent_width.value().into() }
...
impl Semicolons {
	pub const fn is_as_needed(&self) -> bool { matches!(self, Self::AsNeeded) }

	pub const fn is_always(&self) -> bool { matches!(self, Self::Always) }
}
...
impl FromStr for Semicolons {
	...
	fn from_str(s:&str) -> Result<Self, Self::Err> {
		match s {
			...
			_ => {
				Err("Value not supported for Semicolons. Supported values are 'as-needed' and \
				     'always'.")
			},
		}
	}
...
impl FromStr for ArrowParentheses {
	...
	fn from_str(s:&str) -> Result<Self, Self::Err> {
		match s {
			...
			"always" | "Always" => Ok(Self::Always),
			_ => {
				Err("Value not supported for Arrow parentheses. Supported values are 'as-needed' \
				     and 'always'.")
			},
		}
	}
