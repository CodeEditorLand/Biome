...
/// server daemon
fn get_pipe_name() -> String { format!(r"\\.\pipe\biome-service-{}", biome_configuration::VERSION) }
...
/// Try to connect to the global pipe and wait for the connection to become
/// ready
...
pub(crate) async fn open_socket() -> io::Result<Option<(ClientReadHalf, ClientWriteHalf)>> {
	match try_connect().await {
		Ok(socket) => {
			...
			Ok(Some((ClientReadHalf { inner:inner.clone() }, ClientWriteHalf { inner })))
		},
...
impl AsyncWrite for ClientWriteHalf {
	...
	fn poll_shutdown(self: Pin<&mut Self>, cx:&mut Context<'_>) -> Poll<Result<(), io::Error>> { self.poll_flush(cx) }
}
...
pub(crate) async fn run_daemon(factory:ServerFactory, config_path:Option<PathBuf>) -> io::Result<Infallible> {
	let mut prev_server = ServerOptions::new().first_pipe_instance(true).create(get_pipe_name())?;
...
async fn run_server(connection:ServerConnection, stream:NamedPipeServer) {
	...
	let read = ServerReadHalf { inner:inner.clone() };
