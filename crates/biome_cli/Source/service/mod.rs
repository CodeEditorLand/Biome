...
use tokio::{
	io::{AsyncBufRead, AsyncBufReadExt, AsyncRead, AsyncReadExt, AsyncWrite, AsyncWriteExt, BufReader, BufWriter},
	runtime::Runtime,
	sync::{
		Notify,
...
mod windows;
#[cfg(windows)]
pub(crate) use self::windows::{ensure_daemon, enumerate_pipes, open_socket, print_socket, run_daemon};

#[cfg(unix)]
mod unix;
#[cfg(unix)]
pub(crate) use self::unix::{ensure_daemon, enumerate_pipes, open_socket, print_socket, run_daemon};
...
/// used concurrently
///
/// This concurrent handling of I/O is implemented useing two "background
/// tasks":
...
impl Deref for PendingRequests {
	...
	fn deref(&self) -> &Self::Target { self.inner.as_ref() }
}
...
impl Drop for PendingRequests {
	fn drop(&mut self) { self.inner.clear(); }
}
...
impl SocketTransport {
	...
	where
		...
		runtime.spawn(write_task(broadcast_shutdown.clone(), write_recv, socket_write));
		...
		Self { runtime, write_send, pending_requests:pending_requests_2 }
	}
}
...
impl WorkspaceTransport for SocketTransport {
	...
	where
		...
		let request = to_vec(&request).map_err(|err| {
			TransportError::SerdeError(format!("failed to serialize {} into byte buffer: {err}", type_name::<P>()))
		})?;
		...
		let result = from_str(response).map_err(|err| {
			TransportError::SerdeError(format!("failed to deserialize {} from {response:?}: {err}", type_name::<R>()))
		})?;
...
where
	...
	loop {
		...
		let (message, response):(_, JsonRpcResponse) = match message {
			Ok(message) => message,
			Err(err) => {
				eprintln!("{:?}", err.context("remote connection read task exited with an error"));
			...
			.context("failed to read header line from the socket")?
		{
			...
			_ => {
				let header:TransportHeader =
					line.parse().context("failed to parse header from the remote workspace")?;
...
async fn write_task<W>(
	...
	while let Some((message, is_shutdown)) = write_recv.recv().await {
		...
		if let Err(err) = write_message(&mut socket_write, message).await {
			eprintln!("{:?}", err.context("remote connection write task exited with an error"));
	...
	fn from_str(line:&str) -> Result<Self, Self::Err> {
		...
		match name {
			"Content-Length" => {
				let value = value
					.parse()
					.with_context(|| format!("could not parse Content-Length header value {value:?}"))?;
