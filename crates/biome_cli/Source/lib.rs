...
impl<'app> CliSession<'app> {
	pub fn new(workspace: &'app dyn Workspace, console: &'app mut dyn Console) -> Result<Self, CliDiagnostic> {
		Ok(Self { app: App::new(console, WorkspaceRef::Borrowed(workspace)) })
	}
	...
	pub fn run(self, command: BiomeCommand) -> Result<(), CliDiagnostic> {
		match command {
			...
			BiomeCommand::Start { log_path, log_prefix_name } => {
			...
			BiomeCommand::LspProxy { log_path, log_prefix_name, .. } => {
				commands::daemon::lsp_proxy(Some(log_path), Some(log_prefix_name))
			},
			BiomeCommand::Migrate { cli_options, write, fix, sub_command } => run_command(
				self,
				&cli_options,
			...
			BiomeCommand::RunServer { stop_on_disconnect, log_path, log_prefix_name } => {
				commands::daemon::run_server(stop_on_disconnect, Some(log_path), Some(log_prefix_name))
			},
