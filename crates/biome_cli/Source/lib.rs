...
pub use reporter::{
	DiagnosticsPayload, Reporter, ReporterVisitor, TraversalSummary,
};
...
impl<'app> CliSession<'app> {
	...
	pub fn run(self, command: BiomeCommand) -> Result<(), CliDiagnostic> {
		...
		let result = match command {
			...
			BiomeCommand::Explain { doc } => {
				commands::explain::explain(self, doc)
			},
			BiomeCommand::Init(emit_jsonc) => {
				commands::init::init(self, emit_jsonc)
			},
