...
pub struct CliOptions {
	...
	/// Skip over files containing syntax errors instead of emitting an error
	/// diagnostic.
	#[bpaf(long("skip-errors"), switch)]
	pub skip_errors:bool,

	/// Silence errors that would be emitted in case no files were processed
	/// during the execution of the command.
	#[bpaf(long("no-errors-on-unmatched"), switch)]
	pub no_errors_on_unmatched:bool,

	/// Tell Biome to exit with an error code if some diagnostics emit
	/// warnings.
	...
	/// The level of diagnostics to show. In order, from the lowest to the most
	/// important: info, warn, error. Passing `--diagnostic-level=error` will
	/// cause Biome to print only diagnostics that contain only errors.
	pub diagnostic_level:Severity,
}

impl CliOptions {
	/// Computes the [ConfigurationPathHint] based on the options passed by the
	/// user
...
impl FromStr for ColorsArg {
	...
	fn from_str(s:&str) -> Result<Self, Self::Err> {
		match s {
			"off" => Ok(Self::Off),
			"force" => Ok(Self::Force),
			_ => Err(format!("value {s:?} is not valid for the --colors argument")),
		}
	}
}
...
pub enum CliReporter {
	...
	Junit,
	/// Reports linter diagnostics grouped by category and number of hits.
	/// Reports formatter diagnostics grouped by file.
	Summary,
...
impl CliReporter {
	pub(crate) const fn is_default(&self) -> bool { matches!(self, Self::Default) }
}
...
impl FromStr for CliReporter {
	...
	fn from_str(s:&str) -> Result<Self, Self::Err> {
		match s {
			...
			_ => Err(format!("value {s:?} is not valid for the --reporter argument")),
		}
	}
}
