...
use crate::{
	...
	commands::{CommandRunner, LoadEditorConfig, get_files_to_process_with_cli_options},
	diagnostics::DeprecatedArgument,
};
...
impl CommandRunner for FormatCommandPayload {
	...
	fn merge_configuration(
		...
		if self.javascript_formatter.is_some() {
			...
			javascript.formatter.merge_with(self.javascript_formatter.clone());
		}
		...
		configuration.files.merge_with(self.files_configuration.clone());
	...
	fn get_files_to_process(
		...
		let paths =
			get_files_to_process_with_cli_options(self.since.as_deref(), self.changed, self.staged, fs, configuration)?
	...
	fn should_write(&self) -> bool { self.write || self.fix }
