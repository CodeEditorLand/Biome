...
use tracing::{debug_span, metadata::LevelFilter, subscriber::Interest, Instrument, Metadata};
...
pub(crate) fn start(
	...
	let rt = Runtime::new()?;
	let did_spawn =
		rt.block_on(ensure_daemon(false, config_path, log_path, log_file_name_prefix))?;
...
impl LoggingFilter {
	fn is_enabled(&self, meta:&Metadata<'_>) -> bool {
		let filter =
			if meta.target().starts_with("biome") { SELF_FILTER } else { LevelFilter::INFO };
