...
use biome_service::{TransportError, WorkspaceError, workspace::WorkspaceClient};
use tokio::{io, runtime::Runtime};
use tracing::{Instrument, Metadata, debug_span, metadata::LevelFilter, subscriber::Interest};
...
pub(crate) fn start(
	...
	let did_spawn = rt.block_on(ensure_daemon(false, config_path, log_path, log_file_name_prefix))?;
...
pub(crate) fn lsp_proxy(
	...
	rt.block_on(start_lsp_proxy(&rt, config_path, log_path, log_file_name_prefix))?;
...
pub(crate) fn read_most_recent_log_file(
	...
	let most_recent = fs::read_dir(biome_log_path)?
		...
		.filter_map(|file| {
			match file.file_name().to_str()?.split_once(log_file_name_prefix.as_str()) {
...
/// - All spans and events at level debug in crates whose name starts with
///   `biome`
...
impl LoggingFilter {
	fn is_enabled(&self, meta:&Metadata<'_>) -> bool {
		let filter = if meta.target().starts_with("biome") { SELF_FILTER } else { LevelFilter::INFO };
...
impl<S> Filter<S> for LoggingFilter {
	fn enabled(&self, meta:&Metadata<'_>, _cx:&Context<'_, S>) -> bool { self.is_enabled(meta) }

	fn callsite_enabled(&self, meta:&'static Metadata<'static>) -> Interest {
		if self.is_enabled(meta) { Interest::always() } else { Interest::never() }
	}

	fn max_level_hint(&self) -> Option<LevelFilter> { Some(SELF_FILTER) }
}
