...
use biome_configuration::{
	...
	PartialGraphqlFormatter,
	PartialGraphqlLinter,
	PartialJavascriptFormatter,
	PartialJsonFormatter,
	PartialLinterConfiguration,
	analyzer::RuleSelector,
	css::{PartialCssLinter, partial_css_formatter, partial_css_linter},
	graphql::{partial_graphql_formatter, partial_graphql_linter},
	javascript::{PartialJavascriptLinter, partial_javascript_formatter, partial_javascript_linter},
	json::{PartialJsonLinter, partial_json_formatter, partial_json_linter},
	partial_configuration,
	partial_files_configuration,
	partial_formatter_configuration,
	partial_linter_configuration,
	vcs::{PartialVcsConfiguration, partial_vcs_configuration},
};
...
use biome_service::{
	...
	Workspace,
	WorkspaceError,
	configuration::{LoadedConfiguration, PartialConfigurationExt, load_configuration, load_editorconfig},
	documentation::Doc,
	workspace::{FixFileMode, RegisterProjectFolderParams, UpdateSettingsParams},
};
use bpaf::Bpaf;
...
use crate::{
	...
	LoggingLevel,
	VERSION,
	changed::{get_changed_files, get_staged_files},
	cli_options::{CliOptions, CliReporter, ColorsArg, cli_options},
	diagnostics::{DeprecatedArgument, DeprecatedConfigurationFile},
	execute::Stdin,
	execute_mode,
	logging::LoggingKind,
...
/// Biome official CLI. Use it to check the health of your project or run it to
/// check single files.
pub enum BiomeCommand {
	...
	Check {
		...
		/// Alias for `--write`, writes safe fixes, formatting and import
		/// sorting
		#[bpaf(long("fix"), switch, hide_usage)]
		fix:bool,

		/// Alias for `--write`, writes safe fixes, formatting and import
		/// sorting (deprecated, use `--write`)
		#[bpaf(long("apply"), switch, hide_usage)]
		apply:bool,

		/// Alias for `--write --unsafe`, writes safe and unsafe fixes,
		/// formatting and import sorting (deprecated, use `--write --unsafe`)
		#[bpaf(long("apply-unsafe"), switch, hide_usage)]
		apply_unsafe:bool,

		/// Allow to enable or disable the formatter check.
		#[bpaf(long("formatter-enabled"), argument("true|false"), optional, hide_usage)]
		...
		#[bpaf(long("organize-imports-enabled"), argument("true|false"), optional, hide_usage)]
		...
		/// When set to true, only the files that have been staged (the ones
		/// prepared to be committed) will be linted. This option should be
		/// used when working locally.
		#[bpaf(long("staged"), switch)]
		staged:bool,

		/// When set to true, only the files that have been changed compared to
		/// your `defaultBranch` configuration will be linted. This option
		/// should be used in CI environments.
		#[bpaf(long("changed"), switch)]
		changed:bool,

		/// Use this to specify the base branch to compare against when you're
		/// using the --changed flag and the `defaultBranch` is not set in
		/// your `biome.json`
	...
	Lint {
		...
		/// Fixes lint rule violations with a comment a suppression instead of
		/// using a rule code action (fix)
		#[bpaf(long("suppress"))]
		suppress:bool,
		...
		/// then the severity level of the rule is set to `error` if it is a
		...
		/// Skip the given rule or group of rules by setting the severity level
		/// of the rules to `off`. This option takes precedence over `--only`.
		///
	...
	/// Command to use in CI environments. Runs formatter, linter and import
	/// sorting to the requested files.
	///
	...
	/// Bootstraps a new biome project. Creates a configuration file with some
	/// defaults.
	#[bpaf(command)]
	Init(
	...
	Search {
		...
		/// parse the code.
		///
		/// Example: `echo 'let a;' | biome search '`let $var`'
...
pub enum MigrateSubCommand {
	/// It attempts to find the files `.prettierrc`/`prettier.json` and
	/// `.prettierignore`, and map the Prettier's configuration into Biome's
	/// configuration file.
	#[bpaf(command)]
	Prettier,
	/// It attempts to find the ESLint configuration file in the working
...
impl MigrateSubCommand {
	pub const fn is_prettier(&self) -> bool { matches!(self, MigrateSubCommand::Prettier) }
}
...
impl BiomeCommand {
	...
	pub const fn has_metrics(&self) -> bool { false }

	pub fn is_verbose(&self) -> bool { self.cli_options().map_or(false, |cli_options| cli_options.verbose) }
...
/// It accepts a [LoadedPartialConfiguration] and it prints the diagnostics
/// emitted during parsing and deserialization.
///
...
fn resolve_manifest(fs:&DynRef<'_, dyn FileSystem>) -> Result<Option<(BiomePath, String)>, WorkspaceError> {
	let result = fs.auto_search(&fs.working_directory().unwrap_or_default(), &["package.json"], false)?;
...
pub(crate) fn determine_fix_file_mode(
	...
	let FixFileModeOptions { apply, apply_unsafe, write, fix, suppress, suppression_reason: _, unsafe_ } = options;
...
fn check_fix_incompatible_arguments(options:FixFileModeOptions) -> Result<(), CliDiagnostic> {
	let FixFileModeOptions { apply, apply_unsafe, write, suppress, suppression_reason, fix, unsafe_ } = options;

	if apply && apply_unsafe {
		return Err(CliDiagnostic::incompatible_arguments("--apply", "--apply-unsafe"));
	} else if apply_unsafe && unsafe_ {
		return Err(CliDiagnostic::incompatible_arguments("--apply-unsafe", "--unsafe"));
		...
		return Err(CliDiagnostic::incompatible_arguments("--suppress", "--write"));
...
pub(crate) trait CommandRunner: Sized {
	...
	/// - Computes the paths to traverse/handle. This changes based on the VCS
	...
	fn configure_workspace(
		...
		let loaded_configuration = load_configuration(fs, cli_options.as_configuration_path_hint())?;
	...
	///
	/// ## Errors
	/// - If the user didn't provide anything via `stdin` but the option
	///   `--stdin-file-path` is passed.
	...
	/// Implements this method if you need to merge CLI arguments to the loaded
	/// configuration.
	///
	/// The CLI arguments take precedence over the option configured in the
	/// configuration file.
	...
	/// Optional method that can be implemented to check if some CLI arguments
	/// aren't compatible.
	///
	/// The method is called before loading the configuration from disk.
	fn check_incompatible_arguments(&self) -> Result<(), CliDiagnostic> { Ok(()) }

	/// Checks whether the configuration has errors.
	fn should_validate_configuration_diagnostics(&self) -> bool { true }
}
...
pub trait LoadEditorConfig: CommandRunner {
	...
	/// It loads the `.editorconfig` from the file system, parses it and
	/// deserialize it into a [PartialConfiguration]
...
mod tests {
		...
		for (apply, apply_unsafe, write, suppress, suppression_reason, fix, unsafe_) in [
			...
			assert_eq!(
				determine_fix_file_mode(
					FixFileModeOptions { apply, apply_unsafe, write, suppress, suppression_reason, fix, unsafe_ },
					&mut console
				)
				.unwrap(),
	...
	#[test]
	fn check_options() { biome_command().check_invariants(false); }
}
