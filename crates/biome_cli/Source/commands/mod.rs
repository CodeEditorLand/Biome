...
use biome_configuration::{
	...
	css::{partial_css_formatter, partial_css_linter, PartialCssLinter},
	graphql::{partial_graphql_formatter, partial_graphql_linter},
	javascript::{
		partial_javascript_formatter,
		partial_javascript_linter,
		PartialJavascriptLinter,
	},
	json::{partial_json_formatter, partial_json_linter, PartialJsonLinter},
	partial_configuration,
	partial_files_configuration,
	partial_formatter_configuration,
	partial_linter_configuration,
	vcs::{partial_vcs_configuration, PartialVcsConfiguration},
	BiomeDiagnostic,
...
use crate::{
	...
	cli_options::{cli_options, CliOptions, CliReporter, ColorsArg},
...
pub enum BiomeCommand {
	...
	Lint {
		...
		/// of the rules to `off`. This option takes precedence over
		/// `--only`.
		///
...
impl BiomeCommand {
	...
	pub const fn get_color(&self) -> Option<&ColorsArg> {
		match self.cli_options() {
			Some(cli_options) => {
				// To properly display GitHub annotations we need to disable
				// colors
	...
	pub fn is_verbose(&self) -> bool {
		self.cli_options().map_or(false, |cli_options| cli_options.verbose)
	}
...
fn resolve_manifest(
	...
	let result =
		fs.auto_search(&fs.working_directory().unwrap_or_default(), &["package.json"], false)?;
...
pub(crate) fn determine_fix_file_mode(
	...
	let FixFileModeOptions { apply, apply_unsafe, write, fix, unsafe_ } = options;
		...
		for (apply, apply_unsafe, write, fix, unsafe_) in [
			...
			assert_eq!(
				determine_fix_file_mode(
					FixFileModeOptions { apply, apply_unsafe, write, fix, unsafe_ },
					&mut console
				)
				.unwrap(),
