...
impl BiomeCommand {
	...
	pub fn log_level(&self) -> LoggingLevel {
		self.cli_options().map_or(LoggingLevel::default(), |cli_options| cli_options.log_level)
	}

	pub fn log_kind(&self) -> LoggingKind {
		self.cli_options().map_or(LoggingKind::default(), |cli_options| cli_options.log_kind)
	}
}
...
pub(crate) fn validate_configuration_diagnostics(
	...
	if let Some(file_path) =
		loaded_configuration.file_path.as_ref().and_then(|f| f.file_name()).and_then(|f| f.to_str())
	{
...
pub(crate) fn determine_fix_file_mode(
	...
	if apply || apply_unsafe {
		let (deprecated, alternative) =
			if apply { ("--apply", "--write") } else { ("--apply-unsafe", "--write --unsafe") };
