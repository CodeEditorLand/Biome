...
use biome_service::{
	DynRef,
	Workspace,
	configuration::{LoadedConfiguration, load_configuration},
	workspace::{RageEntry, RageParams, client},
};
...
pub(crate) fn rage(session:CliSession, daemon_logs:bool, formatter:bool, linter:bool) -> Result<(), CliDiagnostic> {
	let terminal_supports_colors = termcolor::BufferWriter::stdout(ColorChoice::Auto).buffer().supports_color();
		...
		match load_configuration(self.fs, ConfigurationPathHint::default()) {
			Ok(loaded_configuration) => {
				if loaded_configuration.directory_path.is_none() {
					...
					let LoadedConfiguration { configuration, diagnostics, .. } = loaded_configuration;
					...
					if self.formatter {
						...
						let css_formatter_configuration = configuration.get_css_formatter_configuration();
						...
						let graphql_formatter_configuration = configuration.get_graphql_formatter_configuration();
