...
use biome_configuration::{
	...
	css::PartialCssLinter,
	javascript::PartialJavascriptLinter,
	json::PartialJsonLinter,
...
use crate::{
	...
	commands::{CommandRunner, get_files_to_process_with_cli_options},
};
...
impl CommandRunner for LintCommandPayload {
	...
	fn merge_configuration(
		...
		let LoadedConfiguration { configuration: mut fs_configuration, .. } = loaded_configuration;
		...
		if self.graphql_linter.is_some() {
			let graphql = fs_configuration.graphql.get_or_insert_with(Default::default);

			graphql.linter.merge_with(self.graphql_linter.clone());
		}

		if self.javascript_linter.is_some() {
			let javascript = fs_configuration.javascript.get_or_insert_with(Default::default);
