...
use super::{CommandRunner, FixFileModeOptions, MigrateSubCommand, check_fix_incompatible_arguments};
use crate::{
	CliDiagnostic,
...
impl CommandRunner for MigrateCommandPayload {
	...
	fn get_execution(
		...
		if let (Some(path), Some(directory_path)) =
			(self.configuration_file_path.clone(), self.configuration_directory_path.clone())
		{
	...
	fn should_validate_configuration_diagnostics(&self) -> bool { false }
}
