...
pub(crate) fn ci(session: CliSession, payload: CiCommandPayload) -> Result<(), CliDiagnostic> {
	...
	let formatter =
		fs_configuration.formatter.get_or_insert_with(PartialFormatterConfiguration::default);
	...
	let organize_imports =
		fs_configuration.organize_imports.get_or_insert_with(PartialOrganizeImports::default);

	if organize_imports_enabled.is_some() {
		organize_imports.enabled = organize_imports_enabled;
	}

	let assists = fs_configuration.assists.get_or_insert_with(PartialAssistsConfiguration::default);
