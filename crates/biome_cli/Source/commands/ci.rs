...
use biome_configuration::{
	...
	analyzer::assists::PartialAssistsConfiguration,
	organize_imports::PartialOrganizeImports,
};
use biome_console::Console;
use biome_deserialize::Merge;
use biome_fs::FileSystem;
use biome_service::{DynRef, Workspace, WorkspaceError, configuration::LoadedConfiguration};
...
use crate::{
	...
	commands::{CommandRunner, LoadEditorConfig},
};
...
impl CommandRunner for CiCommandPayload {
	...
	fn merge_configuration(
		...
		let LoadedConfiguration { configuration: biome_configuration, directory_path: configuration_path, .. } =
			loaded_configuration;

		let mut fs_configuration = self.load_editor_config(configuration_path, &biome_configuration, fs, console)?;
		// this makes biome configuration take precedence over editorconfig
		// configuration
		...
		let linter = fs_configuration.linter.get_or_insert_with(PartialLinterConfiguration::default);
	...
	fn get_stdin_file_path(&self) -> Option<&str> { None }

	fn should_write(&self) -> bool { false }
	...
	fn check_incompatible_arguments(&self) -> Result<(), CliDiagnostic> {
			...
			&& matches!(self.organize_imports_enabled, Some(false))
		{
			return Err(CliDiagnostic::incompatible_end_configuration(
				"Formatter, linter and organize imports are disabled, can't perform the command. At least one feature \
				 needs to be enabled. This is probably and error.",
			));
		}
