...
use biome_service::{
	...
	workspace::{RegisterProjectFolderParams, UpdateSettingsParams},
};

use crate::{
	changed::get_changed_files,
	cli_options::CliOptions,
	commands::{resolve_manifest, validate_configuration_diagnostics},
	execute::VcsTargeted,
	execute_mode,
...
pub(crate) fn ci(session:CliSession, payload:CiCommandPayload) -> Result<(), CliDiagnostic> {
		...
		&& fs_configuration.is_organize_imports_disabled()
	{
		return Err(CliDiagnostic::incompatible_end_configuration(
			"Formatter, linter and organize imports are disabled, can't perform the command. This \
			 is probably and error.",
		));
	}
	...
	execute_mode(
		Execution::new_ci(VcsTargeted { staged:false, changed }).set_report(&cli_options),
		session,
		&cli_options,
		paths,
	)
