...
pub(crate) fn process_file(ctx:&TraversalOptions, biome_path:&BiomePath) -> FileResult {
	tracing::trace_span!("process_file", path = ?biome_path).in_scope(move || {
		...
		// first we stop if there are some files that don't have ALL
		// features enabled, e.g. images, fonts, etc.
		...
		let unsupported_reason = match ctx.execution.traversal_mode() {
			TraversalMode::Check { .. } | TraversalMode::CI { .. } => {
				file_features
					...
					.and(file_features.support_kind_for(&FeatureKind::Format).and_then(
						|support_kind| {
							if support_kind.is_not_enabled() { Some(support_kind) } else { None }
						},
					))
					.and(file_features.support_kind_for(&FeatureKind::OrganizeImports).and_then(
						|support_kind| {
							if support_kind.is_not_enabled() { Some(support_kind) } else { None }
						},
					))
		...
		match ctx.execution.traversal_mode {
			TraversalMode::Lint { .. } => {
				// the unsupported case should be handled already at this
				// point
