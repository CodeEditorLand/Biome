...
use crate::execute::{
	TraversalMode,
	diagnostics::{ResultExt, UnhandledDiagnostic},
...
impl Message {
	pub(crate) const fn is_failure(&self) -> bool { matches!(self, Message::Failure) }
}
...
where
	...
	D: std::fmt::Debug,
{
	fn from(err:D) -> Self { Self::Error(Error::from(err)) }
}

/// The return type for [process_file], with the following semantics:
/// - `Ok(Success)` means the operation was successful (the file is added to the
///   `processed` counter)
...
/// bypassing the compiler constraints set by the lifetimes of the
/// [TraversalOptions]
...
impl<'ctx, 'app> SharedTraversalOptions<'ctx, 'app> {
	fn new(t:&'app TraversalOptions<'ctx, 'app>) -> Self { Self { _p:PhantomData, inner:t } }
}
...
pub(crate) fn process_file(ctx:&TraversalOptions, biome_path:&BiomePath) -> FileResult {
	tracing::trace_span!("process_file", path = ?biome_path).in_scope(move || {
		let file_features = ctx
			.workspace
			.file_features(SupportsFeatureParams { path:biome_path.clone(), features:ctx.execution.to_feature() })
		...
		// first we stop if there are some files that don't have ALL features enabled,
		...
		let unsupported_reason = match ctx.execution.traversal_mode() {
			TraversalMode::Check { .. } | TraversalMode::CI { .. } => {
				file_features
					...
					.and_then(
						|support_kind| {
							if support_kind.is_not_enabled() { Some(support_kind) } else { None }
						},
					)
					.and(file_features.support_kind_for(&FeatureKind::Format).and_then(|support_kind| {
						if support_kind.is_not_enabled() { Some(support_kind) } else { None }
					}))
					.and(
						file_features
		...
		match ctx.execution.traversal_mode {
			TraversalMode::Lint { ref suppression_reason, suppress, .. } => {
				...
				lint(shared_context, biome_path, suppress, suppression_reason.as_deref())
			},
