...
use biome_diagnostics::{Category, adapters::SerdeJsonError, category};
...
use crate::{
	...
	DiagnosticsPayload,
	Reporter,
	cli_options::{CliOptions, CliReporter},
	commands::MigrateSubCommand,
	...
	reporter::{
		github::{GithubReporter, GithubReporterVisitor},
		gitlab::{GitLabReporter, GitLabReporterVisitor},
		json::{JsonReporter, JsonReporterVisitor},
		junit::{JunitReporter, JunitReporterVisitor},
		summary::{SummaryReporter, SummaryReporterVisitor},
		terminal::{ConsoleReporter, ConsoleReporterVisitor},
	},
};
...
impl Execution {
	pub fn new_format(vcs_targeted:VcsTargeted) -> Self {
		Self {
			traversal_mode:TraversalMode::Format { ignore_errors:false, write:false, stdin:None, vcs_targeted },
			report_mode:ReportMode::default(),
			max_diagnostics:0,
		}
	}

	pub fn report_mode(&self) -> &ReportMode { &self.report_mode }
}
...
impl Stdin {
	fn as_path(&self) -> &Path { self.0.as_path() }

	fn as_content(&self) -> &str { self.1.as_str() }
}

impl From<(PathBuf, String)> for Stdin {
	fn from((path, content):(PathBuf, String)) -> Self { Self(path, content) }
}
...
impl From<(bool, bool)> for VcsTargeted {
	fn from((staged, changed):(bool, bool)) -> Self { Self { staged, changed } }
}
...
pub enum TraversalMode {
	...
	Check {
		...
		///
		/// It's [None] if the `check` command is called without `--apply` or
		/// `--apply-suggested` arguments.
		fix_file_mode:Option<FixFileMode>,
		/// An optional tuple.
		/// 1. The virtual path to the file
		/// 2. The content of the file
		stdin:Option<Stdin>,
		/// A flag to know vcs integrated options such as `--staged` or
		/// `--changed` are enabled
	...
	Lint {
		...
		suppress:bool,
		/// Explanation for suppressing diagnostics with `--suppress` and
		/// `--reason`
		suppression_reason:Option<String>,
	},
	/// This mode is enabled when running the command `biome ci`
	CI {
		/// Whether the CI is running in a specific environment, e.g. GitHub,
		/// GitLab, etc.
...
/// Tells to the execution of the traversal how the information should be
/// reported
...
impl Default for ReportMode {
	fn default() -> Self { Self::Terminal { with_summary:false } }
}

impl From<CliReporter> for ReportMode {
	fn from(value:CliReporter) -> Self {
		match value {
			CliReporter::Default => Self::Terminal { with_summary:false },
...
impl Execution {
	pub(crate) fn new(mode:TraversalMode) -> Self {
		Self { report_mode:ReportMode::default(), traversal_mode:mode, max_diagnostics:20 }
	}
	...
	pub(crate) fn new_ci(vcs_targeted:VcsTargeted) -> Self {
		...
		let is_github = std::env::var("GITHUB_ACTIONS").ok().map_or(false, |value| value == "true");

		Self {
			report_mode:ReportMode::default(),
			traversal_mode:TraversalMode::CI {
				environment:if is_github { Some(ExecutionEnvironment::GitHub) } else { None },
				vcs_targeted,
			},
			max_diagnostics:20,
		}
	...
	pub(crate) fn traversal_mode(&self) -> &TraversalMode { &self.traversal_mode }

	pub(crate) fn get_max_diagnostics(&self) -> u32 { self.max_diagnostics }

	/// `true` only when running the traversal in [TraversalMode::Check] and
	/// `should_fix` is `true`
	pub(crate) fn as_fix_file_mode(&self) -> Option<&FixFileMode> {
		match &self.traversal_mode {
			TraversalMode::Check { fix_file_mode, .. } | TraversalMode::Lint { fix_file_mode, .. } => {
	...
	pub(crate) const fn is_ci(&self) -> bool { matches!(self.traversal_mode, TraversalMode::CI { .. }) }

	pub(crate) const fn is_search(&self) -> bool { matches!(self.traversal_mode, TraversalMode::Search { .. }) }

	pub(crate) const fn is_check(&self) -> bool { matches!(self.traversal_mode, TraversalMode::Check { .. }) }

	pub(crate) const fn is_lint(&self) -> bool { matches!(self.traversal_mode, TraversalMode::Lint { .. }) }

	pub(crate) const fn is_check_apply(&self) -> bool {
		matches!(
			self.traversal_mode,
			TraversalMode::Check { fix_file_mode:Some(FixFileMode::SafeFixes), .. }
		)
	}
	...
	pub(crate) const fn is_check_apply_unsafe(&self) -> bool {
		matches!(
			self.traversal_mode,
			TraversalMode::Check { fix_file_mode:Some(FixFileMode::SafeAndUnsafeFixes), .. }
		)
	}

	pub(crate) const fn is_format(&self) -> bool { matches!(self.traversal_mode, TraversalMode::Format { .. }) }
...
/// Based on the [mode](TraversalMode), the function might launch a traversal of
/// the file system or handles the stdin file.
pub fn execute_mode(
	...
	// If a custom reporter was provided, let's lift the limit so users can see all
	// of them
	...
	if let Some(stdin) = execution.as_stdin_file() {
		...
		std_in::run(session, &execution, biome_path, stdin.as_content(), cli_options.verbose)
	} else if let TraversalMode::Migrate { write, configuration_file_path, configuration_directory_path, sub_command } =
		execution.traversal_mode
	{
		...
		let TraverseResult { summary, evaluated_paths, diagnostics } =
			...
			ReportMode::Json { pretty } => {
				...
				if pretty {
					let content = serde_json::to_string(&buffer).map_err(|error| {
						CliDiagnostic::Report(ReportDiagnostic::Serialization(SerdeJsonError::from(error)))
					})?;
					...
					let code = session
						.app
						.workspace
						.format_file(FormatFileParams { path:report_file.clone() })?;
