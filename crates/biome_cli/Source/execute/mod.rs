...
use biome_diagnostics::{adapters::SerdeJsonError, category, Category};
...
use crate::{
	...
	execute::{
		...
		traverse::{traverse, TraverseResult},
	},
	reporter::{
...
pub fn execute_mode(
	...
	// If a custom reporter was provided, let's lift the limit so users can see
	// all of them
	execution.max_diagnostics = if cli_options.reporter.is_default() {
		cli_options.max_diagnostics.into()
	} else {
		info!(
			"Removing the limit of --max-diagnostics, because of a reporter different from the \
			 default one: {}",
