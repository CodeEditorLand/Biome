...
use crate::{CliDiagnostic, CliSession, TraversalMode, execute::Execution};
...
pub(crate) fn run<'a>(
	...
	if mode.is_format() {
		...
		if file_features.is_protected() {
			let protected_diagnostic = WorkspaceError::protected_file(biome_path.display().to_string());
		...
		if let Some(fix_file_mode) = mode.as_fix_file_mode() {
			if file_features.supports_lint() {
				let fix_file_result = workspace.fix_file(FixFileParams {
					...
					rule_categories:RuleCategoriesBuilder::default().with_syntax().with_lint().build(),
				})?;
			...
			if file_features.supports_organize_imports() && mode.is_check() {
				let result = workspace.organize_imports(OrganizeImportsParams { path:biome_path.clone() })?;
