...
use biome_diagnostics::{DiagnosticExt, DiagnosticTags, Error, Resource, Severity, category};
...
use biome_service::{
	...
	dome::Dome,
	extension_error,
	workspace::{DropPatternParams, IsPathIgnoredParams, SupportsFeatureParams},
};
...
use super::{
	Execution,
	TraversalMode,
	process_file::{DiffKind, FileStatus, Message, process_file},
};
use crate::{
	CliDiagnostic,
	CliSession,
	...
	execute::diagnostics::{
		AssistsDiffDiagnostic,
		CIAssistsDiffDiagnostic,
		CIFormatDiffDiagnostic,
		CIOrganizeImportsDiffDiagnostic,
		ContentDiffAdvice,
		FormatDiffDiagnostic,
		OrganizeImportsDiffDiagnostic,
		PanicDiagnostic,
	},
	reporter::TraversalSummary,
};
...
pub(crate) fn traverse(
	...
	if inputs.is_empty() {
		match &execution.traversal_mode {
			...
			| TraversalMode::Search { .. } => {
				// If `--staged` or `--changed` is specified, it's acceptable for them to be
				// empty, so ignore it.
	...
	if let TraversalMode::Search { pattern, .. } = execution.traversal_mode() {
		let _ = session
			.app
			.workspace
			.drop_pattern(DropPatternParams { pattern:pattern.clone() });
	}
...
/// This function will setup the global Rayon thread pool the first time it's
/// called
///
/// This is currently only used to assign friendly debug names to the threads of
/// the pool
fn init_thread_pool() {
...
/// run it to completion, returning the duration of the process and the
/// evaluated paths
fn traverse_inputs(fs:&dyn FileSystem, inputs:Vec<OsString>, ctx:&TraversalOptions) -> (Duration, BTreeSet<BiomePath>) {
...
impl<'ctx> DiagnosticsPrinter<'ctx> {
	...
	fn errors(&self) -> u32 { self.errors.load(Ordering::Relaxed) }

	fn warnings(&self) -> u32 { self.warnings.load(Ordering::Relaxed) }

	fn not_printed_diagnostics(&self) -> u32 { self.not_printed_diagnostics.load(Ordering::Relaxed) }

	fn skipped_fixes(&self) -> u32 { self.total_skipped_suggested_fixes.load(Ordering::Relaxed) }

	/// Checks if the diagnostic we received from the thread should be
	/// considered or not. Logic:
	/// - it should not be considered if its severity level is lower than the
	///   one provided via CLI;
	/// - it should not be considered if it's a verbose diagnostic and the CLI
	///   **didn't** request a `--verbose` option.
	...
	/// Count the diagnostic, and then returns a boolean that tells if it should
	/// be printed
	fn should_print(&self) -> bool {
		...
		if should_print {
			...
			self.remaining_diagnostics
				.store(self.max_diagnostics.saturating_sub(printed_diagnostics), Ordering::Relaxed);
		} else {
	...
	fn run(&self, receiver:Receiver<Message>, interner:Receiver<PathBuf>) -> Vec<Error> {
		...
		while let Ok(msg) = receiver.recv() {
			match msg {
				Message::SkippedFixes { skipped_suggested_fixes } => {
				...
				Message::Diagnostics { name, content, diagnostics, skipped_diagnostics } => {
					self.not_printed_diagnostics.fetch_add(skipped_diagnostics, Ordering::Relaxed);
				...
				Message::Diff { file_name, old, new, diff_kind } => {
							...
							match diff_kind {
								DiffKind::Format => {
									let diag = CIFormatDiffDiagnostic {
										...
										diff:ContentDiffAdvice { old:old.clone(), new:new.clone() },
									};

									diagnostics_to_print
										.push(diag.with_severity(severity).with_file_source_code(old.clone()));
								},
...
impl<'ctx, 'app> TraversalOptions<'ctx, 'app> {
	pub(crate) fn increment_changed(&self, path:&BiomePath) {
		...
		self.evaluated_paths.write().unwrap().replace(path.to_written());
	}

	pub(crate) fn increment_unchanged(&self) { self.unchanged.fetch_add(1, Ordering::Relaxed); }
	...
	pub(crate) fn push_message(&self, msg:impl Into<Message>) { self.messages.send(msg.into()).ok(); }
	...
	pub(crate) fn protected_file(&self, biome_path:&BiomePath) {
		self.push_diagnostic(WorkspaceError::protected_file(biome_path.display().to_string()).into())
	}
}

impl<'ctx, 'app> TraversalContext for TraversalOptions<'ctx, 'app> {
	fn interner(&self) -> &PathInterner { &self.interner }

	fn evaluated_paths(&self) -> BTreeSet<BiomePath> { self.evaluated_paths.read().unwrap().clone() }

	fn push_diagnostic(&self, error:Error) { self.push_message(error); }
	...
	fn can_handle(&self, biome_path:&BiomePath) -> bool {
		...
		if self.fs.path_is_dir(path) || self.fs.path_is_symlink(path) {
			...
			// - directories
			// - symlinks
			// - unresolved symlinks e.g `symlink/subdir` where symlink points to a
			//   directory that includes `subdir`. Note that `symlink/subdir` is not an
			//   existing file.
			let can_handle = !self
				.workspace
		...
		let file_features = self
			.workspace
			.file_features(SupportsFeatureParams { path:biome_path.clone(), features:self.execution.to_feature() });
	...
	fn handle_path(&self, path:BiomePath) { handle_file(self, &path) }

	fn store_path(&self, path:BiomePath) {
		self.evaluated_paths.write().unwrap().insert(BiomePath::new(path.as_path()));
	}
}
...
fn handle_file(ctx:&TraversalOptions, path:&BiomePath) {
	match catch_unwind(move || process_file(ctx, path)) {
		...
		Err(err) => {
			...
			ctx.push_message(PanicDiagnostic { message }.with_file_path(path.display().to_string()));
		},
	}
}
