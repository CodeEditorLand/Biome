...
use biome_diagnostics::{category, DiagnosticExt, DiagnosticTags, Error, Resource, Severity};
...
use super::{
	process_file::{process_file, DiffKind, FileStatus, Message},
	Execution,
	TraversalMode,
};
use crate::{
	...
	if inputs.is_empty() {
		match &execution.traversal_mode {
			...
			| TraversalMode::Search { .. } => {
				// If `--staged` or `--changed` is specified, it's acceptable
				// for them to be empty, so ignore it.
...
struct DiagnosticsPrinter<'ctx> {
	...
	/// The maximum number of diagnostics the console thread is allowed to
	/// print
	max_diagnostics:u32,
			...
			match msg {
				...
				Message::Error(mut err) => {
					...
					if let Some(Resource::File(file_path)) = location.resource.as_ref() {
						// Retrieves the file name from the file ID cache, if
						// it's a miss flush entries from the interner
						// channel until it's found
							...
							None => {
								loop {
									match interner.recv() {
										...
										// In case the channel disconnected
										// without sending the path we
										// need, print the error without a file
										// name (normally this should never
										// happen)
										Err(_) => break None,
									}
								}
		...
		let file_features = match file_features {
			Ok(file_features) => {
				...
				if file_features.is_not_supported() && !file_features.is_ignored() {
					// we should throw a diagnostic if we can't handle a file
					// that isn't ignored
