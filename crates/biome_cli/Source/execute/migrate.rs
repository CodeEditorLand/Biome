...
use biome_deserialize::{Merge, json::deserialize_from_json_ast};
use biome_diagnostics::{Diagnostic, PrintDiagnostic, category};
...
use crate::{
	...
	execute::diagnostics::{ContentDiffAdvice, MigrateDiffDiagnostic},
};
...
pub(crate) fn run(migrate_payload:MigratePayload) -> Result<(), CliDiagnostic> {
	...
	let mut biome_config_file = fs.open_with_options(configuration_file_path.as_path(), open_options)?;
	...
	let parsed = parse_json_with_cache(&biome_config_content, &mut cache, JsonParserOptions::default());

	match sub_command {
		Some(MigrateSubCommand::Prettier) => {
			let prettier::Config { path: prettier_path, data: prettier_config } =
				prettier::read_config_file(fs, console)?;

			let biome_config =
				deserialize_from_json_ast::<PartialConfiguration>(&parsed.tree(), "").into_deserialized();
			...
			let prettier_biome_config = prettier_config.try_into().map_err(|err:ParseFormatNumberError| {
				CliDiagnostic::MigrateError(MigrationDiagnostic { reason:err.to_string() })
			})?;
			...
			if biome_config == old_biome_config {
				...
				let new_content = serde_json::to_string(&biome_config)
					.map_err(|err| CliDiagnostic::MigrateError(MigrationDiagnostic { reason:err.to_string() }))?;

				workspace.change_file(ChangeFileParams { path:biome_path.clone(), content:new_content, version:1 })?;
				...
				if write {
					...
					let diagnostic = MigrateDiffDiagnostic {
						file_name,
						diff:ContentDiffAdvice { old:biome_config_content, new:printed.as_code().to_string() },
					};
		...
		Some(MigrateSubCommand::Eslint { include_inspired, include_nursery }) => {
			let eslint::Config { path: eslint_path, data: eslint_config } = eslint::read_eslint_config(fs, console)?;
			...
			let (biome_eslint_config, results) = eslint_config
				.into_biome_config(&eslint_to_biome::MigrationOptions { include_inspired, include_nursery });
		...
		None => {
			let has_deprecated_configuration = configuration_file_path.file_name() == Some(OsStr::new("rome.json"));
			...
			loop {
				...
				match action {
					Some(action) => {
						if let (root, Some((range, _))) = action.mutation.commit_with_text_range_and_edit(true) {
				...
				if write {
					...
					let diagnostic = if has_deprecated_configuration {
						MigrateDiffDiagnostic {
							file_name,
							diff:ContentDiffAdvice { old:"rome.json".to_string(), new:"biome.json".to_string() },
						}
					} else {
						MigrateDiffDiagnostic {
							file_name,
							diff:ContentDiffAdvice { old:biome_config_content, new:new_configuration_content },
						}
					};
