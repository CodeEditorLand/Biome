...
use biome_deserialize::{json::deserialize_from_json_ast, Merge};
use biome_diagnostics::{category, Diagnostic, PrintDiagnostic};
...
pub(crate) fn run(migrate_payload:MigratePayload) -> Result<(), CliDiagnostic> {
	...
	let parsed =
		parse_json_with_cache(&biome_config_content, &mut cache, JsonParserOptions::default());
	...
	match sub_command {
		Some(MigrateSubCommand::Prettier) => {
			...
			let prettier_biome_config =
				prettier_config.try_into().map_err(|err:ParseFormatNumberError| {
		...
		Some(MigrateSubCommand::Eslint { include_inspired, include_nursery }) => {
			let eslint::Config { path: eslint_path, data: eslint_config } =
