...
/// This modules includes implementations for converting an ESLint config to a
/// Biome config.
///
/// The conversion relies on:
/// - the generated [super::eslint_any_rule_to_biome::migrate_eslint_any_rule]
///   module that relies on Biome's rule metadata to determine the equivalent
///   Biome's rule of an Eslint rule
/// - hand-written handling of Biome rules that have options in the current
///   module.
...
impl eslint_eslint::FlatConfigData {
	pub(crate) fn into_biome_config(
		...
		let global_config_object = if self.0.len() == 1 {
			...
			for flat_config_object in self.0 {
				if flat_config_object.is_global_ignores() {
					global_config_object.ignores.extend(flat_config_object.ignores);
					...
					if !flat_config_object.ignores.is_empty() {
						override_pat.ignore = Some(flat_config_object.ignores.into_iter().collect());
					}
		...
		if let Some(language_options) = global_config_object.language_options {
			...
			let js_config =
				biome_config::PartialJavascriptConfiguration { globals:Some(globals), ..Default::default() };
...
impl eslint_eslint::LegacyConfigData {
	pub(crate) fn into_biome_config(
		...
		if !self.ignore_patterns.is_empty() {
			let ignore = self.ignore_patterns.into_iter().map(|p| p.0).collect::<StringSet>();
		...
		if !self.overrides.is_empty() {
			...
			for override_elt in self.overrides {
				...
				if !override_elt.excluded_files.is_empty() {
					override_pattern.ignore = Some(override_elt.excluded_files.into_iter().collect());
				}
	...
	match rule {
		...
		eslint_eslint::Rule::NoRestrictedGlobals(conf) => {
			if migrate_eslint_any_rule(rules, &name, conf.severity(), opts, results) {
				...
				let globals = conf.into_vec().into_iter().map(|g| g.into_name().into_boxed_str());

				let group = rules.style.get_or_insert_with(Default::default);

				group.no_restricted_globals =
					Some(biome_config::RuleConfiguration::WithOptions(biome_config::RuleWithOptions {
						level:severity.into(),
		...
		eslint_eslint::Rule::Jsxa11yArioaRoles(conf) => {
			if migrate_eslint_any_rule(rules, &name, conf.severity(), opts, results) {
				if let eslint_eslint::RuleConf::Option(severity, rule_options) = conf {
					...
					group.use_valid_aria_role = Some(biome_config::RuleFixConfiguration::WithOptions(
						biome_config::RuleWithFixOptions {
							level:severity.into(),
							fix:None,
							options:Box::new((*rule_options).into()),
		...
		eslint_eslint::Rule::TypeScriptArrayType(conf) => {
			if migrate_eslint_any_rule(rules, &name, conf.severity(), opts, results) {
				if let eslint_eslint::RuleConf::Option(severity, rule_options) = conf {
					...
					group.use_consistent_array_type = Some(biome_config::RuleFixConfiguration::WithOptions(
						biome_config::RuleWithFixOptions {
							level:severity.into(),
							fix:None,
		...
		eslint_eslint::Rule::TypeScriptNamingConvention(conf) => {
			if migrate_eslint_any_rule(rules, &name, conf.severity(), opts, results) {
				...
				let options = eslint_typescript::NamingConventionOptions::new(conf.into_vec().into_iter().map(|v| *v));

				let group = rules.style.get_or_insert_with(Default::default);

				group.use_naming_convention = Some(biome_config::RuleFixConfiguration::WithOptions(
					biome_config::RuleWithFixOptions { level:severity.into(), fix:None, options:options.into() },
				));
			}
		},
...
mod tests {
	...
	fn flat_config_single_config_object() {
		let flat_config = FlatConfigData(vec![FlatConfigObject {
			...
			rules:Some(Rules(
				[Rule::Any(Cow::Borrowed("eqeqeq"), Severity::Error)].into_iter().collect(),
			)),
		}]);
		...
		assert_eq!(linter.include, Some(["*.js".to_string()].into_iter().collect()));

		assert_eq!(linter.ignore, Some(["*.test.js".to_string()].into_iter().collect()));
	...
	fn flat_config_multiple_config_object() {
		let flat_config = FlatConfigData(vec![
			...
			FlatConfigObject {
				...
				ignores:vec![],
				language_options:None,
				rules:Some(Rules([Rule::Any(Cow::Borrowed("eqeqeq"), Severity::Off)].into_iter().collect())),
			},
		]);
		...
		assert_eq!(
			linter.ignore,
			Some(["*.test.js".to_string(), "*.spec.js".to_string()].into_iter().collect())
		);
		...
		assert_eq!(override0.include, Some(["*.ts".to_string()].into_iter().collect()));

		assert!(override0.ignore.is_none());

		assert_eq!(
			override0.linter.unwrap().rules.unwrap().suspicious.unwrap().no_double_equals,
			Some(biome_config::RuleFixConfiguration::Plain(
