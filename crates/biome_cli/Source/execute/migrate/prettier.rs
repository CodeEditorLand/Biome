...
impl TryFrom<PrettierConfiguration> for biome_configuration::PartialConfiguration {
	...
	fn try_from(value:PrettierConfiguration) -> Result<Self, Self::Error> {
		...
		let formatter = biome_configuration::PartialFormatterConfiguration {
			...
			include:None,
			enabled:Some(true),
			// editorconfig support is intentionally set to true, because
			// prettier always reads the editorconfig file see: https://github.com/prettier/prettier/issues/15255
		...
		let jsx_quote_style =
			if value.jsx_single_quote { QuoteStyle::Single } else { QuoteStyle::Double };
