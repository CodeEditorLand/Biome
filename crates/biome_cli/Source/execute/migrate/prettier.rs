...
use biome_deserialize::{StringSet, json::deserialize_from_json_str};
...
impl TryFrom<PrettierConfiguration> for biome_configuration::PartialConfiguration {
	...
	fn try_from(value:PrettierConfiguration) -> Result<Self, Self::Error> {
		...
		let formatter = biome_configuration::PartialFormatterConfiguration {
			...
			include:None,
			enabled:Some(true),
			// editorconfig support is intentionally set to true, because prettier always reads the
			// editorconfig file see: https://github.com/prettier/prettier/issues/15255
		...
		let semicolons = if value.semi { Semicolons::Always } else { Semicolons::AsNeeded };

		let quote_style = if value.single_quote { QuoteStyle::Single } else { QuoteStyle::Double };

		let jsx_quote_style = if value.jsx_single_quote { QuoteStyle::Single } else { QuoteStyle::Double };
		...
		let js_config =
			biome_configuration::PartialJavascriptConfiguration { formatter:Some(js_formatter), ..Default::default() };
...
impl TryFrom<Override> for biome_configuration::OverridePattern {
	...
	fn try_from(Override { files, options }:Override) -> Result<Self, Self::Error> {
		...
		// js config
		let semicolons = options
			.semi
			.map(|semi| if semi { Semicolons::Always } else { Semicolons::AsNeeded });

		let quote_style = options.single_quote.map(
			|single_quote| {
		...
		let js_formatter = biome_configuration::PartialJavascriptFormatter {
			...
			semicolons,
			trailing_commas:options.trailing_comma.map(|trailing_comma| trailing_comma.into()),
			quote_style,
...
/// This function is in charge of reading prettier files, deserialize its
/// contents
pub(crate) fn read_config_file(
	fs:&DynRef<'_, dyn FileSystem>,
	console:&mut dyn Console,
) -> Result<Config, CliDiagnostic> {
	// We don't report an error if Prettier config is not embedded in
	// `PACKAGE_JSON`.
	if let Ok(data) = load_config(fs, Path::new(PACKAGE_JSON), console) {
		return Ok(Config { path:PACKAGE_JSON, data });
	}
	...
	for config_name in CONFIG_FILES {
		...
		if fs.path_exists(path) {
			return Ok(Config { path:config_name, data:load_config(fs, path, console)? });
		}
	}
	...
	let (deserialized, diagnostics) = match path.extension().and_then(OsStr::to_str) {
		None | Some("json") => {
			...
			if path.file_name().is_some_and(|name| name == PACKAGE_JSON) {
				...
				.consume();
				(deserialized.and_then(|packagejson| packagejson.prettier), vec![])
			} else {
		...
		Some("js" | "mjs" | "cjs") => {
			...
			deserialize_from_json_str::<PrettierConfiguration>(&content, JsonParserOptions::default(), "").consume()
		},

		Some(ext) => {
			return Err(CliDiagnostic::MigrateError(MigrationDiagnostic {
				reason:format!("Prettier configuration ending with the extension `{ext}` are not supported."),
			}));
		},
	};
...
mod tests {
	...
	fn ok() {
		...
		assert!(matches!(configuration, PrettierConfiguration { use_tabs:true, .. }))
	}
