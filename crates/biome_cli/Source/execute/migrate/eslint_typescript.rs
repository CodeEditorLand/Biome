...
///
/// Also, the module includes implementation to convert rule options to Biome's
/// rule options.
...
use biome_js_analyze::{
	lint::{
		...
		style::{use_consistent_array_type, use_naming_convention},
	},
...
impl From<ArrayTypeOptions> for use_consistent_array_type::ConsistentArrayTypeOptions {
	fn from(val:ArrayTypeOptions) -> Self {
		use_consistent_array_type::ConsistentArrayTypeOptions { syntax:val.default.into() }
	}
}
#[derive(Debug, Default, Deserializable)]
...
impl From<ArrayType> for use_consistent_array_type::ConsistentArrayType {
	fn from(val:ArrayType) -> Self {
		match val {
			...
			ArrayType::Array | ArrayType::ArraySimple => use_consistent_array_type::ConsistentArrayType::Shorthand,
...
impl From<NamingConventionOptions> for use_naming_convention::NamingConventionOptions {
	fn from(val:NamingConventionOptions) -> Self {
		...
		for selection in val.0 {
			...
			let matching = if selection.leading_underscore.is_some() || selection.trailing_underscore.is_some() {
				let leading_underscore =
					selection.leading_underscore.map_or("", |underscore| underscore.as_regex_part());
			...
			let prefix = selection.prefix.iter().map(|p| regex::escape(p)).collect::<Vec<_>>().join("|");

			let suffix = selection.suffix.iter().map(|p| regex::escape(p)).collect::<Vec<_>>().join("|");

			let prefix = if prefix.is_empty() { prefix } else { format!("(?:{prefix})") };

			let suffix = if suffix.is_empty() { suffix } else { format!("(?:{suffix})") };
			...
			let formats = if let Some(format) = selection.format {
				format.into_iter().map(use_naming_convention::Format::from).collect()
			} else {
				use_naming_convention::Formats::default()
			};

			for selector in selectors {
				conventions.push(use_naming_convention::Convention { selector, matching:matching.clone(), formats });
			}
		}
...
impl NamingConventionSelection {
	...
	fn selectors(&self) -> Vec<use_naming_convention::Selector> {
		...
		let modifiers:use_naming_convention::Modifiers =
			self.modifiers.iter().flatten().filter_map(|m| m.as_modifier()).collect();

		let has_class_modifier = modifiers.contains(use_naming_convention::RestrictedModifier::Abstract);

		let has_class_member_modifier = modifiers.contains(use_naming_convention::RestrictedModifier::Private)
			|| modifiers.contains(use_naming_convention::RestrictedModifier::Protected);

		let has_property_modifier = modifiers.contains(use_naming_convention::RestrictedModifier::Readonly);

		modifiers.contains(use_naming_convention::RestrictedModifier::Private);

		let scope = self.modifiers.iter().flatten().find_map(|m| m.as_scope()).unwrap_or_default();
