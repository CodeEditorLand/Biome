...
///
/// Also, the module includes implementation to convert rule options to
/// Biome's rule options.
...
impl From<NamingConventionOptions> for use_naming_convention::NamingConventionOptions {
	fn from(val:NamingConventionOptions) -> Self {
		...
		for selection in val.0 {
			...
			let prefix =
				selection.prefix.iter().map(|p| regex::escape(p)).collect::<Vec<_>>().join("|");
			let suffix =
				selection.suffix.iter().map(|p| regex::escape(p)).collect::<Vec<_>>().join("|");
...
pub(crate) enum Selector {
	// Order is important, it reflects the precedence relation between
	// selectors Individual selectors
	ClassicAccessor,
	AutoAccessor,
	Class,
	ClassMethod,
