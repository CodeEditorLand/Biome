...
pub(crate) struct FlatConfigObject {
	...
	// using `Option` is important to distinguish a global ignores from a
	// config objerct
...
pub(crate) enum RuleConf<T = (), U = ()> {
	...
	// `{ rule: <severity> }` and `{ rule: [<severity>, <option1.1>,
	// <option1.2>, ...] }`
...
impl<T> RuleConf<T, ()> {
	pub(crate) fn into_vec(self) -> Vec<T> {
		match self {
			...
			RuleConf::Option(_, value) | RuleConf::Options(_, value, _) => {
				vec![value]
			},
