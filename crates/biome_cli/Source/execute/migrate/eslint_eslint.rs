...
/// This modules includes implementations for deserializing an eslint
/// configuration.
///
...
///
/// See [super::eslint_to_biome] for converting an ESLint config to a Biome
/// config.
...
impl From<FlatConfigData> for AnyConfigData {
	fn from(value:FlatConfigData) -> Self { AnyConfigData::Flat(value) }
}
impl From<LegacyConfigData> for AnyConfigData {
	fn from(value:LegacyConfigData) -> Self { AnyConfigData::Legacy(value) }
}
...
impl FlatConfigObject {
	...
	pub(crate) fn is_global_ignores(&self) -> bool {
		!self.ignores.is_empty() && self.files.is_empty() && self.language_options.is_none() && self.rules.is_none()
	}

	/// Rteurns `true` if this config doesn't specify `files` or `ignores`.
	pub(crate) fn is_global_config(&self) -> bool { self.ignores.is_empty() && self.files.is_empty() }
}
...
impl Merge for FlatLanguageOptions {
	fn merge_with(&mut self, other:Self) { self.globals.merge_with(other.globals); }
}
...
impl Globals {
	pub(crate) fn enabled(self) -> impl Iterator<Item = String> {
		self.0
			.into_iter()
			.filter_map(|(global_name, global_conf)| global_conf.is_enabled().then_some(global_name))
	}
}
...
impl Merge for Globals {
	fn merge_with(&mut self, other:Self) { self.0.extend(other.0); }
}
...
impl<T> Merge for ShorthandVec<T> {
	fn merge_with(&mut self, mut other:Self) { self.0.append(&mut other.0); }
}
impl<T> From<T> for ShorthandVec<T> {
	fn from(value:T) -> Self { Self(vec![value]) }
}
...
impl<T:Deserializable> Deserializable for ShorthandVec<T> {
	fn deserialize(
		...
		Some(ShorthandVec(if value.visitable_type()? == DeserializableType::Array {
...
impl<T, U> RuleConf<T, U> {
	pub(crate) fn severity(&self) -> Severity {
		match self {
			...
			Self::Options(severity, ..) => *severity,
...
impl<T:Default, U:Default> RuleConf<T, U> {
	pub(crate) fn option_or_default(self) -> T {
		match self {
			RuleConf::Severity(_) | RuleConf::Options(..) | RuleConf::Spread(..) => T::default(),
...
impl<T:Deserializable + 'static, U:Deserializable + 'static> Deserializable for RuleConf<T, U> {
	fn deserialize(
		...
		impl<T:Deserializable + 'static, U:Deserializable + 'static> DeserializationVisitor for Visitor<T, U> {
			...
			fn visit_array(
				...
				let Some(first_value) = values.next() else {
					diagnostics.push(DeserializationDiagnostic::new("A severity is expected.").with_range(range));
		...
		if matches!(value.visitable_type()?, DeserializableType::Number | DeserializableType::Str) {
			...
			fn visit_map(
				...
				for (key, value) in members.flatten() {
					...
					match rule_name.text() {
						...
						rule_name => {
							if let Some(conf) = RuleConf::<()>::deserialize(&value, name, diagnostics) {
								result.insert(Rule::Any(Cow::Owned(rule_name.to_string()), conf.severity()));
							}
						},
					}
				}
...
impl Deserializable for NoRestrictedGlobal {
	fn deserialize(
		...
		if value.visitable_type()? == DeserializableType::Str {
			...
			Deserializable::deserialize(value, name, diagnostics).map(NoRestrictedGlobal::WithMessage)
		}
	}
}
#[derive(Debug, Default, Deserializable)]
...
pub(crate) enum Rule {
	...
	TypeScriptExplicitMemberAccessibility(RuleConf<eslint_typescript::ExplicitMemberAccessibilityOptions>),
...
impl Rule {
	pub(crate) fn name(&self) -> Cow<'static, str> {
		match self {
			...
			Rule::TypeScriptNamingConvention(_) => Cow::Borrowed("@typescript-eslint/naming-convention"),
