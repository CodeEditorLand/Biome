...
pub(crate) fn read_ignore_file(fs:&DynRef<'_, dyn FileSystem>, ignore_filename:&str) -> io::Result<IgnorePatterns> {
	let mut file = fs.open_with_options(Path::new(ignore_filename), OpenOptions::default().read(true))?;
...
impl IgnorePatterns {
	pub(crate) fn from(content:&str) -> Self {
		...
		IgnorePatterns { patterns, has_negated_patterns }
	}
}
...
pub(crate) fn convert_pattern(line:&str) -> Result<String, &'static str> {
	...
	} else if line.find('/').is_some_and(|index| index < (line.len() - 1)) || line == "**" || line == "**/" {
...
"#;
    ...
    # This is not a comment because there is some leading spaces
		...
		assert_eq!(
			result.patterns,
			["**/    # This is not a comment because there is some leading spaces".to_string()].into()
		);
	}
}
