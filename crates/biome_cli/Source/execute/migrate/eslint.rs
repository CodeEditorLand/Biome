...
use biome_deserialize::{Merge, json::deserialize_from_json_str};
use biome_diagnostics::{DiagnosticExt, PrintDiagnostic};
use biome_fs::{FileSystem, OpenOptions};
use biome_json_parser::JsonParserOptions;
use biome_service::DynRef;

use super::{eslint_eslint, node};
use crate::{CliDiagnostic, diagnostics::MigrationDiagnostic};

/// This modules includes implementations for loading and deserializing an
/// eslint configuration.
///
/// See [super::eslint_eslint] for the data representation of an ESLint
/// configuration.
///
/// Each ESLint plugin has its own module in which rule options are defined.
/// For example, the ESLint TypeScript plugin is defined in
/// [super::eslint_typescript]. Note that we don't need to deserialise every
/// existing rule option. We only need to deserialise options that have
/// equivalent biome options. This greatly reduces the amount of work involved.
...
/// For example, ESLint looks for `./.eslintrc.js` before looking for
/// `./.eslintrc.json`.
...
/// Returns the ESLint configuration file in the working directory with the
/// highest priority.
///
/// This function respects the priority between ESLint configuration files.
/// For example, it looks for `./.eslintrc.js` before looking for
/// `./.eslintrc.json`. It first ries to load a flat configuration file.
///
/// Unlike ESLint, it doesn't look for a configuration file in parent
/// directories when no configuration file is found in the working directory.
///
/// Deserialization errors are reported using `console`.
/// Other errors (File Not found, unsupported config format, ...) are directly
/// returned.
///
/// We extract the ESLint configuration from a JavaScript file, by invoking
/// `node`.
///
...
pub(crate) fn read_eslint_config(
	...
	for config_path_str in FLAT_CONFIG_FILES {
		...
		if fs.path_exists(path) {
			return load_flat_config_data(path, console).map(|data| Config { path:config_path_str, data:data.into() });
		}
	}
	...
	for config_path_str in LEGACY_CONFIG_FILES {
		...
		if fs.path_exists(path) {
			return load_legacy_config_data(fs, path, console)
				.map(|data| Config { path:config_path_str, data:data.into() });
		}
	}
	...
	if let Ok(data) = load_legacy_config_data(fs, Path::new(PACKAGE_JSON), console) {
		return Ok(Config { path:PACKAGE_JSON, data:data.into() });
	}

	Err(CliDiagnostic::MigrateError(MigrationDiagnostic {
		reason:"The default ESLint configuration file `.eslintrc[.*]` was not found in the working directory."
			.to_string(),
	}))
}
...
/// See https://eslint.org/docs/latest/use/configure/configuration-files-new
fn load_flat_config_data(path:&Path, console:&mut dyn Console) -> Result<eslint_eslint::FlatConfigData, CliDiagnostic> {
	let node::Resolution { content, .. } = node::load_config(&path.to_string_lossy())?;

	let (deserialized, diagnostics) =
		deserialize_from_json_str::<eslint_eslint::FlatConfigData>(&content, JsonParserOptions::default(), "")
			.consume();
...
fn load_legacy_config_data(
	...
	let (deserialized, diagnostics) = match path.extension().and_then(OsStr::to_str) {
		None | Some("json") => {
			...
			if path.file_name().is_some_and(|name| name == PACKAGE_JSON) {
				let (deserialized, diagnostics) = deserialize_from_json_str::<eslint_eslint::EslintPackageJson>(
					&content,
					JsonParserOptions::default().with_allow_trailing_commas().with_allow_comments(),
					"",
				)
				.consume();
				(
		...
		Some("js" | "cjs") => {
			...
			deserialize_from_json_str::<eslint_eslint::LegacyConfigData>(&content, JsonParserOptions::default(), "")
				.consume()
		},

		Some(ext) => {
			return Err(CliDiagnostic::MigrateError(MigrationDiagnostic {
				reason:format!("ESLint configuration ending with the extension `{ext}` are not supported."),
			}));
		},
	};
...
/// Returns the configuration from a preset or an error if the resolution
/// failed.
///
/// This handles:
...
fn load_eslint_extends_config(name:&str) -> Result<eslint_eslint::LegacyConfigData, CliDiagnostic> {
	let (specifier, resolved_path, deserialized) = if let Some((protocol, rest)) = name.split_once(':') {
		let (module_name, config_name) = match protocol {
			...
			"plugin" => {
				let Some(config_name) = rest.split('/').last() else {
					return Err(CliDiagnostic::MigrateError(MigrationDiagnostic {
						reason:format!(
							"The configuration {rest} cannot be resolved. Make sure that your ESLint configuration \
							 file is valid."
						),
					}));
				};
			...
			name => {
				return Err(CliDiagnostic::MigrateError(MigrationDiagnostic {
					reason:format!("The module {name} cannot be resolved. This is likely an internal error."),
				}));
			},
		};
		// load ESLint preset
		let node::Resolution { content, resolved_path } = node::load_config(&module_name)?;

		let deserialized =
			deserialize_from_json_str::<eslint_eslint::PluginExport>(&content, JsonParserOptions::default(), "")
				.into_deserialized();
		...
		if let Some(mut deserialized) = deserialized {
			...
			if deserialized.is_none() {
				return Err(CliDiagnostic::MigrateError(MigrationDiagnostic {
					reason:format!(
						"The ESLint configuration '{config_name}' cannot be extracted from the module \
						 '{module_name}'. Make sure that '{config_name}' is a valid configuration name."
					),
				}));
			}
		...
		let node::Resolution { content, resolved_path } = node::load_config(&module_name)
			.or_else(|err| if module_name != name { node::load_config(name) } else { Err(err) })?;
	...
	let Some(mut deserialized) = deserialized else {
		return Err(CliDiagnostic::MigrateError(MigrationDiagnostic {
			reason:format!(
				"The ESLint configuration of the module '{specifier}' cannot be extracted. This is likely an internal \
				 error."
			),
		}));
	};
...
fn resolve_extends(config:&mut eslint_eslint::LegacyConfigData, console:&mut dyn Console) {
	let extensions:Vec<_> = config
		...
		.iter()
		.filter_map(|preset| {
			match load_eslint_extends_config(preset) {
...
mod tests {
	...
	fn eslint_package_resolve_name() {
		...
		assert_eq!(EslintPackage::Config.resolve_name("package"), "eslint-config-package");
