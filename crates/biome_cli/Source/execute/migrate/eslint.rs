...
use biome_deserialize::{json::deserialize_from_json_str, Merge};
...
const FLAT_CONFIG_FILES:[&str; 3] = [
	// Prefixed with `./` to ensure that it is loadable via Node.js's
	// `import()`
	"./eslint.config.js",
	"./eslint.config.mjs",
	"./eslint.config.cjs",
...
pub(crate) fn read_eslint_config(
	...
	for config_path_str in FLAT_CONFIG_FILES {
		...
		if fs.path_exists(path) {
			return load_flat_config_data(path, console)
	...
	// We don't report an error if ESLint config is not embedded in
	// `PACKAGE_JSON`.
	...
	Err(CliDiagnostic::MigrateError(MigrationDiagnostic {
		reason:"The default ESLint configuration file `.eslintrc[.*]` was not found in the \
		        working directory."
			.to_string(),
	}))
}
		...
		None | Some("json") => {
			...
			if path.file_name().is_some_and(|name| name == PACKAGE_JSON) {
				let (deserialized, diagnostics) = deserialize_from_json_str::<
					eslint_eslint::EslintPackageJson,
				>(
					&content,
...
fn load_eslint_extends_config(name:&str) -> Result<eslint_eslint::LegacyConfigData, CliDiagnostic> {
		...
		name.split_once(':')
	{
		let (module_name, config_name) = match protocol {
			...
			"plugin" => {
				let Some(config_name) = rest.split('/').last() else {
					return Err(CliDiagnostic::MigrateError(MigrationDiagnostic {
						reason:format!(
							"The configuration {rest} cannot be resolved. Make sure that your \
							 ESLint configuration file is valid."
						),
					}));
				};
		...
		if let Some(mut deserialized) = deserialized {
			...
			if deserialized.is_none() {
				return Err(CliDiagnostic::MigrateError(MigrationDiagnostic {
					reason:format!(
						"The ESLint configuration '{config_name}' cannot be extracted from the \
						 module '{module_name}'. Make sure that '{config_name}' is a valid \
						 configuration name."
					),
				}));
			}
	...
	let Some(mut deserialized) = deserialized else {
		return Err(CliDiagnostic::MigrateError(MigrationDiagnostic {
			reason:format!(
				"The ESLint configuration of the module '{specifier}' cannot be extracted. This \
