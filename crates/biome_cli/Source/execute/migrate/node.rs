	...
	match content_output {
		Err(_) => {
			Err(CliDiagnostic::MigrateError(MigrationDiagnostic {
				reason:"The `node` program doesn't exist or cannot be invoked by Biome.\n`node` is invoked to resolve \
				        ESLint configurations written in JavaScript.\nThis includes shared configurations and plugin \
				        configurations imported with ESLint's `extends`."
					.to_string(),
			}))
		},
		Ok(output) => {
			...
			Ok(Resolution { content:String::from_utf8_lossy(&output.stdout).to_string(), resolved_path })
		},
	}
}
