...
pub(crate) fn load_config(specifier:&str) -> Result<Resolution, CliDiagnostic> {
	let content_output = Command::new("node")
		...
		.arg("--eval")
		.arg(format!(
			"{UNCYCLE_FUNCTION} import('{specifier}').then((c) => \
			 console.log(JSON.stringify(uncycle(c.default))))"
		))
		.output();
	match content_output {
		Err(_) => {
			Err(CliDiagnostic::MigrateError(MigrationDiagnostic {
				reason:"The `node` program doesn't exist or cannot be invoked by Biome.\n`node` \
				        is invoked to resolve ESLint configurations written in JavaScript.\nThis \
				        includes shared configurations and plugin configurations imported with \
				        ESLint's `extends`."
					.to_string(),
			}))
		},
		Ok(output) => {
			let path_output = Command::new("node")
				...
				.output();
			let resolved_path = path_output.ok().map_or(String::new(), |path_output| {
				String::from_utf8_lossy(&path_output.stdout).trim().to_string()
			});
			if !output.stderr.is_empty() {
				...
				let output2 = Command::new("node")
					...
					.arg(format!(
						"{UNCYCLE_FUNCTION} \
						 console.log(JSON.stringify(uncycle(require('{specifier}'))))"
					))
					.output();
				...
				return Err(CliDiagnostic::MigrateError(MigrationDiagnostic {
					reason:format!(
						"`node` was invoked to resolve '{specifier}'. This invocation failed with \
						 the following error:\n{stderr}"
					),
				}));
			}
