...
use crate::{
	...
	execute::{
		...
		process_file::{FileResult, FileStatus, Message, SharedTraversalOptions, workspace_file::WorkspaceFile},
	},
};
...
pub(crate) fn lint_with_guard<'ctx>(
	...
	tracing::info_span!("Processes linting", path =? workspace_file.path.display()).in_scope(move || {
		let mut input = workspace_file.input()?;

		let mut changed = false;

		let (only, skip) = if let TraversalMode::Lint { only, skip, .. } = ctx.execution.traversal_mode() {
		...
		if let Some(fix_mode) = ctx.execution.as_fix_file_mode() {
			...
			ctx.push_message(Message::SkippedFixes { skipped_suggested_fixes:fix_result.skipped_suggested_fixes });
		...
		let no_diagnostics =
			pull_diagnostics_result.diagnostics.is_empty() && pull_diagnostics_result.skipped_diagnostics == 0;
		...
		if changed { Ok(FileStatus::Changed) } else { Ok(FileStatus::Unchanged) }
	})
}
