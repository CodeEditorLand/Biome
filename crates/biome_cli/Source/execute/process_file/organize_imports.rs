...
pub(crate) fn organize_imports_with_guard<'ctx>(
	...
	tracing::info_span!("Processes import sorting", path =? workspace_file.path.display()).in_scope(move || {
		let sorted = workspace_file
			.guard()
			.organize_imports()
			.with_file_path_and_code(workspace_file.path.display().to_string(), category!("organizeImports"))?;
