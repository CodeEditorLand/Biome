...
use crate::execute::{
	TraversalMode,
	diagnostics::{ResultExt, SkippedDiagnostic},
	process_file::{DiffKind, FileResult, FileStatus, Message, SharedTraversalOptions, workspace_file::WorkspaceFile},
};
...
pub(crate) fn format_with_guard<'ctx>(
	...
	tracing::info_span!("Processes formatting", path =? workspace_file.path.display()).in_scope(move || {
		...
		let diagnostics_result = workspace_file
			...
			.with_file_path_and_code(workspace_file.path.display().to_string(), category!("format"))?;

		let input = workspace_file.input()?;

		let (should_write, ignore_errors) = match ctx.execution.traversal_mode {
			TraversalMode::Format { write, ignore_errors, .. } => (write, ignore_errors),

			_ => (ctx.execution.is_check_apply() || ctx.execution.is_check_apply_unsafe(), false),
		};
