...
pub fn print_metrics() {
	...
	let mut histograms: Vec<_> = write_guard
		.drain()
		.flat_map(|(key, entry)| {
			entry.into_inner().unwrap().into_histograms(key.0.name())
		})
		.collect();
...
mod tests {
	...
	use super::{
		CallsiteEntry, CallsiteKey, MetricsLayer, Timepoint, Timings, METRICS,
	};
	...
	fn test_timing() {
		...
		for (name, histogram) in histograms {
			...
			for (bucket, expected) in
				histogram.iter_linear(scale as u64).zip(&expected_buckets)
			{
	...
	fn test_layer() {
		let _guard =
			tracing_subscriber::registry().with(MetricsLayer).set_default();
