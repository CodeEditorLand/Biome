...
/// Implementation of a tracing [Layer] that collects timing information for
...
impl CallsiteEntry {
	fn from_level(level:&Level) -> Self {
		...
		match level {
			&Level::TRACE => {
				Self::Trace {
					// SAFETY: Histogram::new only returns an error if the value
					// of SIGNIFICANT_FIGURES is invalid, 3 is statically
					// known to work
...
/// Extension data attached to tracing spans to keep track of their idle and
/// busy time
///
/// Most of the associated code is based on the similar logic found in
	...
	S: Subscriber + for<'a> LookupSpan<'a>,
{
	...
	fn register_callsite(&self, metadata:&'static Metadata<'static>) -> Interest {
		...
		METRICS.write().unwrap().insert(CallsiteKey(metadata), Mutex::new(entry));
	...
	fn on_close(&self, id:span::Id, ctx:Context<'_, S>) {
		...
		if let Some(timing) = extensions.remove::<Timings>() {
			...
			// Acquire a read lock on the metrics storage, access the metrics
			// entry associated with this call site and acquire a write lock
			// on it
...
mod tests {
	...
	#[test]
	fn test_layer() {
		let _guard = tracing_subscriber::registry().with(MetricsLayer).set_default();
