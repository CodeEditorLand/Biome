...
use biome_console::{fmt, fmt::Formatter, markup, Console, ConsoleExt};
use biome_diagnostics::{advice::ListAdvice, Diagnostic, PrintDiagnostic};
...
impl<'a> ReporterVisitor for ConsoleReporterVisitor<'a> {
	...
	fn report_handled_paths(&mut self, evaluated_paths:BTreeSet<BiomePath>) -> io::Result<()> {
		let evaluated_paths_diagnostic = EvaluatedPathsDiagnostic {
			advice:ListAdvice {
				list:evaluated_paths.iter().map(|p| p.display().to_string()).collect(),
			},
		};
