...
use biome_console::{Console, ConsoleExt, fmt, fmt::Formatter, markup};
use biome_diagnostics::{Diagnostic, PrintDiagnostic, advice::ListAdvice};
use biome_fs::BiomePath;

use crate::{
	Reporter,
	execute::{Execution, TraversalMode},
	reporter::{DiagnosticsPayload, ReporterVisitor, TraversalSummary},
};
...
impl<'a> ReporterVisitor for ConsoleReporterVisitor<'a> {
	...
	fn report_handled_paths(&mut self, evaluated_paths:BTreeSet<BiomePath>) -> io::Result<()> {
		let evaluated_paths_diagnostic = EvaluatedPathsDiagnostic {
			advice:ListAdvice { list:evaluated_paths.iter().map(|p| p.display().to_string()).collect() },
		};
		...
		for diagnostic in &diagnostics_payload.diagnostics {
			...
			if diagnostic.severity() >= diagnostics_payload.diagnostic_level {
				if diagnostic.tags().is_verbose() && diagnostics_payload.verbose {
					self.0.error(markup! {{PrintDiagnostic::verbose(diagnostic)}});
				} else {
					self.0.error(markup! {{PrintDiagnostic::simple(diagnostic)}});
				}
			}
		}
...
impl fmt::Display for Files {
	fn fmt(&self, fmt:&mut Formatter) -> io::Result<()> {
		...
		if self.0 == 1 { fmt.write_str("file") } else { fmt.write_str("files") }
	}
}
...
pub(crate) struct ConsoleTraversalSummary<'a>(pub(crate) &'a TraversalMode, pub(crate) &'a TraversalSummary);
