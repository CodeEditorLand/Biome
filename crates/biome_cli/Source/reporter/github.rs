...
impl<'a> ReporterVisitor for GithubReporterVisitor<'a> {
	fn report_summary(&mut self, _execution:&Execution, _summary:TraversalSummary) -> io::Result<()> { Ok(()) }

	fn report_diagnostics(&mut self, _execution:&Execution, diagnostics_payload:DiagnosticsPayload) -> io::Result<()> {
