...
/// When using this trait, the type that implements this trait is the one that
/// holds the read-only information to pass around
...
/// When using this trait, the type that implements this trait is the one that
/// will **write** the data, ideally inside a buffer
pub trait ReporterVisitor {
	...
	fn report_diagnostics(&mut self, execution:&Execution, payload:DiagnosticsPayload) -> io::Result<()>;
}
