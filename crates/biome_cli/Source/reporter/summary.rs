...
use crate::{
	...
	ReporterVisitor,
	TraversalSummary,
	reporter::terminal::ConsoleTraversalSummary,
};
...
impl<'a> ReporterVisitor for SummaryReporterVisitor<'a> {
	fn report_summary(&mut self, execution:&Execution, summary:TraversalSummary) -> io::Result<()> {
	...
	fn report_diagnostics(&mut self, execution:&Execution, diagnostics_payload:DiagnosticsPayload) -> io::Result<()> {
		let mut files_to_diagnostics = FileToDiagnostics::default();

		for diagnostic in &diagnostics_payload.diagnostics {
			let location = diagnostic.location().resource.and_then(|r| {
				match r {
			...
			if diagnostic.severity() >= diagnostics_payload.diagnostic_level {
				...
				if execution.is_check() || execution.is_lint() || execution.is_ci() {
					if let Some(category) = category {
						if category.name().starts_with("lint/") || category.name().starts_with("suppressions/") {
							files_to_diagnostics.insert_lint(category.name(), severity);
						}
					}
				}
...
impl FileToDiagnostics {
	...
	fn insert_format(&mut self, location:&str) { self.formats.insert(location.into()); }

	fn insert_organize_imports(&mut self, location:&str) { self.organize_imports.insert(location.into()); }

	fn insert_parse(&mut self, location:&str) { self.parse.insert(location.into()); }
}
...
impl Display for FileToDiagnostics {
	fn fmt(&self, fmt:&mut Formatter) -> io::Result<()> {
		...
		if !self.lints.0.is_empty() {
			let diagnostic = SummaryTableDiagnostic { tables:&self.lints };
...
impl<'a> Advices for &'a LintsByCategory {
	fn record(&self, visitor:&mut dyn Visit) -> io::Result<()> {
		let headers = &[markup!("Rule Name").to_owned(), markup!("Diagnostics").to_owned()];

		let (first, second):(Vec<_>, Vec<_>) = self
			.0
			.iter()
			.rev()
			.map(|(rule_name, diagnostic)| (markup! {{rule_name}}.to_owned(), markup! {{diagnostic}}.to_owned()))
			.unzip();
...
impl From<&'static str> for RuleName {
	fn from(value:&'static str) -> Self { Self(value) }
}
...
impl Ord for RuleName {
	fn cmp(&self, other:&Self) -> Ordering { self.0.len().cmp(&other.0.len()) }
}
