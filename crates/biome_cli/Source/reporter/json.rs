...
impl JsonReporterVisitor {
	pub(crate) fn new(summary:TraversalSummary) -> Self { Self { summary, diagnostics:vec![], command:String::new() } }
}
...
impl ReporterVisitor for JsonReporterVisitor {
	fn report_summary(&mut self, execution:&Execution, summary:TraversalSummary) -> std::io::Result<()> {
		...
		for diagnostic in payload.diagnostics {
			if diagnostic.severity() >= payload.diagnostic_level {
				if diagnostic.tags().is_verbose() {
					if payload.verbose {
						self.diagnostics.push(biome_diagnostics::serde::Diagnostic::new(diagnostic))
					}
				} else {
					self.diagnostics.push(biome_diagnostics::serde::Diagnostic::new(diagnostic))
				}
