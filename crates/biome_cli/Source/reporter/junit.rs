...
use biome_diagnostics::{Error, Resource, display::SourceFile};
...
impl<'a> ReporterVisitor for JunitReporterVisitor<'a> {
	fn report_summary(&mut self, _execution:&Execution, summary:TraversalSummary) -> io::Result<()> {
	...
	fn report_diagnostics(&mut self, _execution:&Execution, payload:DiagnosticsPayload) -> io::Result<()> {
		let diagnostics = payload
			.diagnostics
			.iter()
			.filter(|diagnostic| if diagnostic.tags().is_verbose() { payload.verbose } else { true });
		...
		for diagnostic in diagnostics {
				...
				(location.span, location.source_code, location.resource)
			{
				...
				let mut case = TestCase::new(
					format!(
						"org.biome.{}",
						diagnostic.category().map(|c| c.name()).unwrap_or_default().replace('/', ".")
					),
					status,
				);
				...
				if let Resource::File(path) = resource {
					...
					case.extra.insert("line".into(), start.line_number.get().to_string().into());

					case.extra.insert("column".into(), start.column_number.get().to_string().into());

					test_suite.extra.insert("package".into(), "org.biome".into());
