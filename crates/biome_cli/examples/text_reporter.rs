...
impl Reporter for TextReport {
	fn write(self, visitor: &mut dyn ReporterVisitor) -> std::io::Result<()> {
		let execution = Execution::new_format(VcsTargeted { staged: false, changed: false });
...
pub fn main() {
	let summary = TraversalSummary { changed: 32, unchanged: 28, ..TraversalSummary::default() };
