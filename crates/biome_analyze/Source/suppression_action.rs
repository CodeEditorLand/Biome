...
pub trait SuppressionAction {
	...
	fn apply_suppression_comment(&self, payload:SuppressionCommentEmitterPayload<Self::Language>) {
		...
		// retrieve the most suited, most left token where the diagnostics was
		// emitted
		let original_token = self.get_token_from_offset(token_offset, diagnostic_text_range);

		// considering that our suppression system works via lines, we need to
		// look for the first newline, so we can place the comment there
	...
	/// Finds the first token, starting with the current token and traversing
	/// backwards, until it find one that has a leading newline trivia.
	///
	/// Sometimes, the offset is between tokens, we need to decide which one to
	/// take.
	///
	/// For example:
	/// ```jsx
	...
	/// ```
	///
	/// In these case it's best to peek the right token, because it belongs to
	/// the node where error actually occurred, and becomes easier to add the
	/// suppression comment.
