...
use biome_rowan::{
	BatchMutation, Language, SyntaxToken, TextRange, TokenAtOffset,
};
...
pub trait SuppressionAction {
	...
	fn apply_suppression_comment(
		&self,
		payload: SuppressionCommentEmitterPayload<Self::Language>,
	) {
		...
		let original_token =
			self.get_token_from_offset(token_offset, diagnostic_text_range);
		...
		let apply_suppression =
			original_token.as_ref().and_then(|original_token| {
	...
	fn get_token_from_offset(
		...
		match token_at_offset {
			...
			TokenAtOffset::Between(left_token, right_token) => {
				let chosen_token = if right_token.text_range().start()
					== diagnostic_text_range.start()
				{
					right_token
				} else {
					left_token
