...
use biome_rowan::{
	BatchMutation, Language, SyntaxToken, TextLen, TextRange, TokenAtOffset, TriviaPiece, TriviaPieceKind,
};
...
pub trait SuppressionAction {
	...
	fn inline_suppression(&self, payload: SuppressionCommentEmitterPayload<Self::Language>) {
		...
		let apply_suppression = original_token
			.as_ref()
			.and_then(|original_token| self.find_token_for_inline_suppression(original_token.clone()));

		if let Some(apply_suppression) = apply_suppression {
			self.apply_inline_suppression(mutation, apply_suppression, suppression_text, suppression_reason);
		}
	}
	...
	fn apply_top_level_suppression(
		...
		let has_comments = token.leading_trivia().pieces().any(|trivia| trivia.is_comments());
...
fn new_trivia_for_top_suppression<L: Language>(
	...
	let mut new_trivia = vec![
		TriviaPiece::new(TriviaPieceKind::SingleLineComment, suppression_text.text_len()),
