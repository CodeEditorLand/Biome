...
pub use crate::categories::{
	ActionCategory, OtherActionCategory, RefactorKind, RuleCategories, RuleCategoriesBuilder, RuleCategory,
	SUPPRESSION_INLINE_ACTION_CATEGORY, SUPPRESSION_TOP_LEVEL_ACTION_CATEGORY, SourceActionKind,
};
...
pub use crate::registry::{
	LanguageRoot, MetadataRegistry, Phase, Phases, RegistryRuleMetadata, RegistryVisitor, RuleRegistry,
	RuleRegistryBuilder, RuleSuppressions,
};
pub use crate::rule::{
	CategoryLanguage, FixKind, GroupCategory, GroupLanguage, Rule, RuleAction, RuleDiagnostic, RuleDomain, RuleGroup,
	RuleMeta, RuleMetadata, RuleSource, RuleSourceKind, SuppressAction,
};
pub use crate::services::{FromServices, MissingServicesDiagnostic, ServiceBag};
pub use crate::signals::{AnalyzerAction, AnalyzerSignal, AnalyzerTransformation, DiagnosticSignal};
...
use biome_rowan::{
	AstNode, BatchMutation, Direction, Language, SyntaxElement, SyntaxToken, TextRange, TextSize, TokenAtOffset,
	...
	pub fn run(self, mut ctx: AnalyzerContext<L>) -> Option<Break> {
		...
		for plugin in plugins {
			...
			for diagnostic in plugin.evaluate(root, ctx.options.file_path.clone()) {
				...
				let suppression = {
					let index = suppressions.line_suppressions.binary_search_by(|suppression| {
					...
					index.ok().map(|index| &mut suppressions.line_suppressions[index])
				};
		...
		for range_suppression in suppressions.range_suppressions.suppressions {
			...
			if let Some(range) = range_suppression.already_suppressed {
				let signal = DiagnosticSignal::new(|| {
					...
					.note(markup! {"This is the suppression comment that was used."}.to_owned(), range)
				});
				if let ControlFlow::Break(br) = (emit_signal)(&signal) {
					return Some(br);
				}
	...
	Diag: Diagnostic + Clone + Send + Sync + 'static,
{
	...
	fn flush_matches(&mut self, cutoff: Option<TextSize>) -> ControlFlow<Break> {
		while let Some(entry) = self.signal_queue.peek() {
			...
			if self.suppressions.top_level_suppression.suppressed_rule(&entry.rule)
			...
			let suppression = self.suppressions.line_suppressions.last_mut().filter(|suppression| {
			...
			let suppression = match suppression {
				...
				None => {
					let index = self
						.suppressions
						.line_suppressions
						.partition_point(|suppression| suppression.text_range.end() < entry.text_range.start());
			...
			let suppression = suppression.filter(|suppression| {
				...
				if suppression.suppressed_instances.is_empty() {
					suppression.suppressed_rules.iter().any(|filter| *filter == entry.rule)
				} else {
	...
	fn handle_comment(
		...
		for result in (self.parse_suppression_comment)(text, range) {
			...
			if let Err(diagnostic) = self.suppressions.push_suppression(&suppression, range, token.text_range()) {
				let signal = DiagnosticSignal::new(|| diagnostic.clone());
				(self.emit_signal)(&signal)?;
				continue;
			}
		...
		if has_suppressions {
			...
			self.suppressions.overlap_last_suppression(line_index, range);
		}
	...
	fn bump_line_index(&mut self, text: &str, range: TextRange) {
		...
		for (index, _) in text.match_indices(['\n']) {
			let index =
				TextSize::try_from(index).expect("integer overflow while converting a suppression line to `TextSize`");
...
type SuppressionParser<D> = for<'a> fn(&'a str, TextRange) -> Vec<Result<AnalyzerSuppression<'a>, D>>;
...
pub fn to_analyzer_suppressions(suppression: Suppression, piece_range: TextRange) -> Vec<AnalyzerSuppression> {
	...
	for (key, subcategory, value) in suppression.categories {
		if key == category!("lint") || key == category!("assist") {
			result.push(AnalyzerSuppression::everything(key.name()).with_variant(&suppression.kind));
			...
			if let Some(rule) = category.strip_prefix("lint/") {
				let suppression = if let Some(instance) = value {
					AnalyzerSuppression::rule_instance(rule, instance).with_ignore_range(ignore_range)
				} else {
...
impl<'analysis> AnalysisFilter<'analysis> {
	...
	pub fn match_group<G: RuleGroup>(&self) -> bool {
		self.match_category::<G::Category>()
			&& self
				.enabled_rules
				.is_none_or(|enabled_rules| enabled_rules.iter().any(|filter| filter.match_group::<G>()))
			&& !self
				.disabled_rules
				.iter()
	...
	pub fn match_rule<R: Rule>(&self) -> bool {
		self.match_category::<<R::Group as RuleGroup>::Category>()
			&& self
				.enabled_rules
				.is_none_or(|enabled_rules| enabled_rules.iter().any(|filter| filter.match_rule::<R>()))
