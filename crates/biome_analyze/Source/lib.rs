	...
	Diag: Diagnostic + Clone + Send + Sync + 'static,
{
	...
	pub fn run(self, mut ctx: AnalyzerContext<L>) -> Option<Break> {
		...
		for (index, (phase, mut visitors)) in phases.into_iter().enumerate() {
			...
			let result =
				if index == 0 { runner.run_first_phase() } else { runner.run_remaining_phases() };
			...
			for visitor in visitors {
				visitor
					.finish(VisitorFinishContext { root: &ctx.root, services: &mut ctx.services });
			}
		}
...
fn update_suppression<L: Language>(
	...
	let old_trivia =
		if is_leading { old_token.leading_trivia() } else { old_token.trailing_trivia() };
