...
/// A convenient new type data structure to store the options that belong to a
/// rule
#[derive(Debug)]
...
impl RuleOptions {
	...
	pub fn value<O:'static>(&self) -> &O {
		...
		// If the code throws an error here, it means that the developer didn't
		// test the rule with the options
		value.downcast_ref::<O>().unwrap()
	}

	pub fn fix_kind(&self) -> Option<FixKind> { self.2 }
}
...
pub struct AnalyzerConfiguration {
	...
	/// A collections of bindings that the analyzers should consider as
	/// "external".
	///
	/// For example, lint rules should ignore them.
	pub globals:Vec<String>,

	/// Allows to choose a different quote when applying fixes inside the lint
	/// rules
	pub preferred_quote:PreferredQuote,

	/// Indicates the type of runtime or transformation used for interpreting
	/// JSX.
...
impl AnalyzerOptions {
	...
	where
		R: Rule<Options: Clone> + 'static, {
		self.configuration.rules.get_rule_fix_kind(&RuleKey::rule::<R>())
	}

	pub fn preferred_quote(&self) -> &PreferredQuote { &self.configuration.preferred_quote }
}
