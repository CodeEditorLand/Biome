use crate::{
	AnalyzerOptions, AnalyzerSignal, Phases, QueryMatch, Rule, RuleFilter,
	RuleGroup, ServiceBag, SuppressionAction,
};
	...
	fn eq(&self, other: &RuleKey) -> bool {
		match *self {
			...
			RuleFilter::Rule(group, rule) => {
				group == other.group && rule == other.rule
			},
		}
	}
}
...
mod tests {
	...
	use crate::{
		signals::DiagnosticSignal, Analyzer, AnalyzerContext, AnalyzerSignal,
		ApplySuppression, ControlFlow, MetadataRegistry, Never, Phases,
		QueryMatcher, RuleKey, ServiceBag, SignalEntry, SuppressionAction,
		SyntaxVisitor,
	};
	...
	impl QueryMatcher<RawLanguage> for SuppressionMatcher {
		...
		fn match_query(&mut self, params: MatchQueryParams<RawLanguage>) {
			let node =
				params.query.downcast::<SyntaxNode<RawLanguage>>().unwrap();
