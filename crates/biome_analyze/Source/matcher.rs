...
/// Parameters provided to [QueryMatcher::match_query] and require to run lint
/// rules
...
impl Query {
	...
	pub fn downcast<T:QueryMatch>(self) -> Option<T> { Some(*self.data.downcast::<T>().ok()?) }

	/// Attempt to downcast the query to a reference type.
	pub fn downcast_ref<T:QueryMatch>(&self) -> Option<&T> { self.data.downcast_ref::<T>() }

	/// Returns the [TypeId] of this query, equivalent to calling
	/// [Any::type_id].
	pub(crate) fn type_id(&self) -> TypeId { self.data.as_ref().type_id() }

	/// Returns the [TextRange] of this query, equivalent to calling
	/// [QueryMatch::text_range].
	pub(crate) fn text_range(&self) -> TextRange { (self.read_text_range)(self.data.as_ref()) }
}
...
impl GroupKey {
	pub(crate) fn new(group:&'static str) -> Self { Self { group } }

	pub fn group<G:RuleGroup>() -> Self { Self::new(G::NAME) }
}

impl From<GroupKey> for RuleFilter<'static> {
	fn from(key:GroupKey) -> Self { RuleFilter::Group(key.group) }
}
...
impl RuleKey {
	pub fn new(group:&'static str, rule:&'static str) -> Self { Self { group, rule } }

	pub fn rule<R:Rule>() -> Self { Self::new(<R::Group as RuleGroup>::NAME, R::METADATA.name) }

	pub fn group(&self) -> &'static str { self.group }

	pub fn rule_name(&self) -> &'static str { self.rule }
}

impl From<RuleKey> for RuleFilter<'static> {
	fn from(key:RuleKey) -> Self { RuleFilter::Rule(key.group, key.rule) }
}
...
impl<'phase, L:Language> PartialEq for SignalEntry<'phase, L> {
	fn eq(&self, other:&Self) -> bool { self.text_range.start() == other.text_range.start() }
}
...
impl<F, I> InspectMatcher<F, I> {
	///  Create a new instance of [InspectMatcher] from an existing
	/// [QueryMatcher] object and an inspection function
	...
	fn suppressions() {
		let root = {
			...
			builder.token_with_trivia(
				RawLanguageKind::STRING_TOKEN,
				"//group/rule\n\"warn_here\"",
				&[TriviaPiece::single_line_comment(12), TriviaPiece::newline(1)],
				&[],
			);
			builder.finish_node();
			...
			builder.token_with_trivia(
				RawLanguageKind::STRING_TOKEN,
				"//unknown_group\n\"warn_here\"",
				&[TriviaPiece::single_line_comment(15), TriviaPiece::newline(1)],
				&[],
			);
			builder.finish_node();
			...
			builder.token_with_trivia(
				RawLanguageKind::STRING_TOKEN,
				"//group/unknown_rule\n\"warn_here\"",
				&[TriviaPiece::single_line_comment(20), TriviaPiece::newline(1)],
				&[],
			);
			builder.finish_node();
