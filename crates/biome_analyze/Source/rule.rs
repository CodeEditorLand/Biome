...
use crate::{Phase, Phases, Queryable, SourceActionKind, SuppressionAction, SuppressionCommentEmitterPayload};
		...
		for domain in domains {
			...
			if !dependencies.is_empty() {
				...
				for (index, (dep, range)) in dependencies.iter().enumerate() {
					fmt.write_markup(markup! { {padding}"- "<Emphasis>{dep}"@"{range}</Emphasis> })?;
...
#[cfg_attr(
	feature = "serde",
	derive(biome_deserialize_macros::Deserializable, serde::Deserialize, serde::Serialize)
)]
#[cfg_attr(feature = "serde", serde(rename_all = "camelCase"))]
...
impl RuleSource {
	...
	pub fn to_rule_url(&self) -> String {
		match self {
			...
			Self::EslintGraphqlSchemaLinter(rule_name) => {
				format!("https://github.com/cjoudrey/graphql-schema-linter?tab=readme-ov-file#{rule_name}")
			},
			Self::EslintImport(rule_name) => {
				format!("https://github.com/import-js/eslint-plugin-import/blob/main/docs/rules/{rule_name}.md")
			},
			Self::EslintImportAccess(_) => "https://github.com/uhyo/eslint-plugin-import-access".to_string(),
			Self::EslintJest(rule_name) => {
				format!("https://github.com/jest-community/eslint-plugin-jest/blob/main/docs/rules/{rule_name}.md")
			},
			Self::EslintJsxA11y(rule_name) => {
				format!("https://github.com/jsx-eslint/eslint-plugin-jsx-a11y/blob/main/docs/rules/{rule_name}.md")
			},
			Self::EslintReact(rule_name) => {
				format!("https://github.com/jsx-eslint/eslint-plugin-react/blob/master/docs/rules/{rule_name}.md")
			},
			Self::EslintReactHooks(_) => {
				"https://github.com/facebook/react/blob/main/packages/eslint-plugin-react-hooks/README.md".to_string()
			},
			...
			Self::EslintSolid(rule_name) => format!(
				"https://github.com/solidjs-community/eslint-plugin-solid/blob/main/packages/eslint-plugin-solid/docs/{rule_name}.md"
			),
			Self::EslintSonarJs(rule_name) => {
				format!("https://github.com/SonarSource/eslint-plugin-sonarjs/blob/HEAD/docs/rules/{rule_name}.md")
			},
			Self::EslintStylistic(rule_name) => format!("https://eslint.style/rules/default/{rule_name}"),
			Self::EslintUnicorn(rule_name) => {
				format!("https://github.com/sindresorhus/eslint-plugin-unicorn/blob/main/docs/rules/{rule_name}.md")
			},
			Self::EslintUnusedImports(rule_name) => format!(
				"https://github.com/sweepline/eslint-plugin-unused-imports/blob/master/docs/rules/{rule_name}.md"
			),
			Self::EslintMysticatea(rule_name) => {
				format!("https://github.com/mysticatea/eslint-plugin/blob/master/docs/rules/{rule_name}.md")
			},
			Self::EslintBarrelFiles(rule_name) => {
				format!("https://github.com/thepassle/eslint-plugin-barrel-files/blob/main/docs/rules/{rule_name}.md")
			},
			Self::EslintN(rule_name) => {
				format!("https://github.com/eslint-community/eslint-plugin-n/blob/master/docs/rules/{rule_name}.md")
			},
			Self::EslintNext(rule_name) => format!("https://nextjs.org/docs/messages/{rule_name}"),
			Self::Stylelint(rule_name) => {
				format!("https://github.com/stylelint/stylelint/blob/main/lib/rules/{rule_name}/README.md")
			},
			Self::EslintNoSecrets(_) => {
				"https://github.com/nickdeis/eslint-plugin-no-secrets/blob/master/README.md".to_string()
			},
...
#[cfg_attr(
	feature = "serde",
	derive(serde::Deserialize, serde::Serialize, biome_deserialize_macros::Deserializable)
)]
#[cfg_attr(feature = "serde", serde(rename_all = "camelCase"))]
#[cfg_attr(feature = "schema", derive(schemars::JsonSchema))]
pub enum RuleDomain {
...
pub trait Rule: RuleMeta + Sized {
		...
		Self: 'static,
	{
		...
		if matches!(category, RuleCategory::Lint | RuleCategory::Action) {
			...
			if let Some(first_token) = root.syntax().first_token() {
				...
				let message = if category == RuleCategory::Action { "action" } else { "rule" };
				return Some(SuppressAction {
					mutation,
					message: markup! { "Suppress " {message} " " {rule_category} " for the whole file."}.to_owned(),
				});
			}
		}
		None
		...
		Self: 'static,
	{
		...
		if matches!(category, RuleCategory::Lint | RuleCategory::Action) {
			...
			Some(SuppressAction {
				mutation,
				message: markup! { "Suppress " {message} " " {rule_category} " for this line."}.to_owned(),
			})
		} else {
			None
		}
	...
	fn transform(_ctx: &RuleContext<Self>, _state: &Self::State) -> Option<BatchMutation<RuleLanguage<Self>>> {
		None
	}
}
