use crate::{
	...
	signals::RuleSignal,
	AddVisitor, AnalysisFilter, GroupCategory, QueryMatcher, Rule, RuleGroup,
...
impl<L: Language + Default + 'static> RegistryVisitor<L>
	for RuleRegistryBuilder<'_, L>
{
	fn record_category<C: GroupCategory<Language = L>>(&mut self) {
	...
	where
		R: Rule<
				Options: Default,
				Query: Queryable<Language = L, Output: Clone>,
			> + 'static,
	{
		...
		match <R::Query as Queryable>::key() {
			QueryKey::Syntax(key) => {
				let TypeRules::SyntaxRules { rules } = phase
					...
					.or_insert_with(|| TypeRules::SyntaxRules {
						rules: Vec::new(),
					})
				else {
			...
			QueryKey::TypeId(key) => {
				let TypeRules::TypeRules { rules } =
					phase.type_rules.entry(key).or_insert_with(|| {
						TypeRules::TypeRules { rules: Vec::new() }
					})
				else {
...
impl<L: Language> AddVisitor<L>
	for BTreeMap<(Phases, TypeId), Box<dyn Visitor<Language = L>>>
{
	...
	fn match_query(&mut self, mut params: MatchQueryParams<L>) {
		...
		let rules = match rules {
			TypeRules::SyntaxRules { rules } => {
				let node =
					params.query.downcast_ref::<SyntaxNode<L>>().unwrap();
...
type RuleExecutor<L> =
	fn(&mut MatchQueryParams<L>, &mut RuleState<L>) -> Result<(), Error>;
...
impl<L: Language + Default> RegistryRule<L> {
			...
			> + 'static,
	{
		...
		where
			R: Rule<Options: Default, Query: Queryable<Output: Clone>>
				+ 'static,
		{
			if let Some(node) =
				params.query.downcast_ref::<SyntaxNode<RuleLanguage<R>>>()
			{
			...
			let query_result = <R::Query as Queryable>::unwrap_match(
				params.services,
				query_result,
			);
			...
			let options =
				params.options.rule_options::<R>().unwrap_or_default();
			...
			for result in R::run(&ctx) {
				let text_range = R::text_range(&ctx, &result)
					.unwrap_or_else(|| params.query.text_range());
