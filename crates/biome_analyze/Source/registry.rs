...
pub trait RegistryVisitor<L:Language> {
	...
	fn record_category<C:GroupCategory<Language = L>>(&mut self) { C::record_groups(self); }

	/// Record the group `G` to this visitor
	fn record_group<G:RuleGroup<Language = L>>(&mut self) { G::record_rules(self); }
...
impl MetadataRegistry {
	/// Return a unique identifier for a rule group if it's known by this
	/// registry
	...
	pub(crate) fn insert_rule(&mut self, group:&'static str, rule:&'static str) {
		self.inner.insert(MetadataKey { inner:(group, rule) });
	}
}
...
/// we have:
/// - Syntax Phase: No services are offered, thus its rules can be run
///   immediately;
/// - Semantic Phase: Offers the semantic model, thus these rules can only run
///   after the "SemanticModel" is ready, which demands a whole transverse of
///   the parsed tree.
...
impl<L:Language + Default> RuleRegistry<L> {
	pub fn builder<'a>(
		...
		RuleRegistryBuilder {
			filter,
			root,
			registry:RuleRegistry { phase_rules:Default::default() },
...
struct PhaseRules<L:Language> {
	/// Maps the [TypeId] of known query matches types to the corresponding
	/// list of rules
		...
		match <R::Query as Queryable>::key() {
			QueryKey::Syntax(key) => {
				...
				for kind in key.iter() {
					...
					// Insert a handle to the rule `R` into the
					// `SyntaxKindRules` entry corresponding to the
					// SyntaxKind index
	...
	fn match_query(&mut self, mut params:MatchQueryParams<L>) {
		...
		let rules = match rules {
			TypeRules::SyntaxRules { rules } => {
				...
				// Convert the numerical value of the SyntaxKind to an index in
				// the `syntax` vector
...
/// [SyntaxKindRules] holds a collection of [Rule]s that match a specific
/// [SyntaxKind] value
...
impl<'a> borrow::Borrow<(&'a str, &'a str)> for MetadataKey {
	fn borrow(&self) -> &(&'a str, &'a str) { &self.inner }
}

impl borrow::Borrow<str> for MetadataKey {
	fn borrow(&self) -> &str { self.inner.0 }
}
...
impl RegistryRuleMetadata {
	pub fn to_rule_key(&self) -> RuleKey { RuleKey::new(self.group, self.rule.name) }
}
...
impl<L:Language> RuleSuppressions<L> {
	...
	pub fn suppress_node(&mut self, node:SyntaxNode<L>) { self.inner.insert(node); }
}
