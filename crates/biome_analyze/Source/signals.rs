use crate::categories::{SUPPRESSION_INLINE_ACTION_CATEGORY, SUPPRESSION_TOP_LEVEL_ACTION_CATEGORY};
use crate::{
	...
	R: Rule<Options: Default> + 'static,
{
	...
	fn actions(&self) -> AnalyzerActionIter<RuleLanguage<R>> {
		...
		if let Some(ctx) = ctx {
			...
			if let Some(suppression_action) = R::top_level_suppression(&ctx, self.suppression_action) {
				let action = AnalyzerAction {
					rule_name: Some((<R::Group as RuleGroup>::NAME, R::METADATA.name)),
