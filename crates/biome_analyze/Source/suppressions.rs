use crate::{
	AnalyzerSuppression, AnalyzerSuppressionDiagnostic, AnalyzerSuppressionKind, AnalyzerSuppressionVariant,
	MetadataRegistry, RuleFilter, RuleKey,
};
...
impl RangeSuppressions {
	...
	pub(crate) fn expand_range(&mut self, text_range: TextRange) {
		for range_suppression in self.suppressions.iter_mut() {
			if !range_suppression.filters.is_empty() {
				range_suppression.suppression_range = range_suppression.suppression_range.cover(text_range);
			}
		}
	}
	...
	pub(crate) fn suppressed_rule(&mut self, filter: &RuleKey, position: &TextRange) -> bool {
		let range_suppression = self
			...
			.iter_mut()
			.rev()
			.find(|range_suppression| range_suppression.suppression_range.contains_range(*position));
		let range_suppression =
			range_suppression.filter(|range_suppression| range_suppression.filters.iter().any(|f| f == filter));
	...
	pub(crate) fn matches_filter_in_range(&self, filter: &RuleFilter, position: &TextRange) -> Option<TextRange> {
		for range_suppression in self.suppressions.iter().rev() {
			if range_suppression.suppression_range.contains_range(*position)
...
impl<'analyzer> Suppressions<'analyzer> {
	...
	fn map_to_rule_filter(
		...
		match filter {
			None => Err(match group_rule {
				Some((group, rule)) => {
					if is_action {
						AnalyzerSuppressionDiagnostic::new_unknown_assist_action(group, rule, text_range)
					} else {
						AnalyzerSuppressionDiagnostic::new_unknown_lint_rule(group, rule, text_range)
					}
				},
	...
	pub(crate) fn push_suppression(
		...
		match suppression.variant {
			AnalyzerSuppressionVariant::Line => {
				self.push_line_suppression(filter, plugin_name, instances, comment_range, already_suppressed)
			},
			AnalyzerSuppressionVariant::TopLevel => {
				self.top_level_suppression
					.push_suppression(suppression, filter, token_range_not_trimmed, comment_range)
			},
	...
	pub(crate) fn overlap_last_suppression(&mut self, next_line_index: usize, text_range: TextRange) {
		if let Some(variant) = &self.last_suppression {
			match variant {
				...
				AnalyzerSuppressionVariant::RangeStart => self.range_suppressions.expand_range(text_range),
				AnalyzerSuppressionVariant::RangeEnd => self.range_suppressions.expand_range(text_range),
			}
		}
	}
	...
	fn already_suppressed(&self, filter: Option<&RuleFilter>, range: &TextRange) -> Option<TextRange> {
		filter.and_then(|filter| {
			self.top_level_suppression
				.has_filter(filter)
				.then_some(self.top_level_suppression.comment_range)
				.or(self.range_suppressions.matches_filter_in_range(filter, range))
		})
	}
}
