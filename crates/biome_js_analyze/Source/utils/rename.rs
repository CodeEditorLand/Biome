...
impl RenamableNode for JsIdentifierBinding {
	fn binding(&self, _:&SemanticModel) -> Option<JsSyntaxNode> { Some(self.syntax().clone()) }
}
...
pub enum RenameError {
	...
	CannotBeRenamed { original_name:String, original_range:TextRange, new_name:String },
}
	...
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			RenameError::CannotBeRenamed { original_name, new_name, .. } => {
				write!(
					f,
					"encountered an error while renaming the symbol \"{original_name}\" to \
					 \"{new_name}\""
				)
			},
			RenameError::CannotFindDeclaration(_) => {
				write!(f, "encountered an error finding a declaration at the specified position")
			},
		}
	}
}
...
impl RenameSymbolExtensions for BatchMutation<JsLanguage> {
	/// Rename the binding and all its references to "new_name".
	/// If we can´t rename the binding, the [BatchMutation] is never changes and
	/// it is left intact.
	fn rename_node_declaration(
		&mut self,
		model:&SemanticModel,
		...
		// We can rename a binding if there is no conflicts in the current
		// scope. We can shadow parent scopes, so we don´t check them.
		let syntax = prev_binding.syntax();
		let scope = model.scope_hoisted_to(syntax).unwrap_or_else(|| model.scope(syntax));
		...
		for reference in all_references {
			...
			if reference.scope().ancestors().any(|scope| scope.get_binding(new_name).is_some()) {
				return false;
			}
...
mod tests {
	...
	use crate::{assert_rename_nok, assert_rename_ok, utils::rename::RenameError};
