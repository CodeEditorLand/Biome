...
///
/// A path segment is delimited by path separator `/` or the start/end of the
/// path.
...
impl From<RestrictedGlob> for String {
	fn from(value:RestrictedGlob) -> Self { value.to_string() }
}
impl std::str::FromStr for RestrictedGlob {
	...
	fn from_str(value:&str) -> Result<Self, Self::Err> {
		...
		match glob_builder.build() {
			Ok(glob) => Ok(RestrictedGlob { is_negated, glob:glob.compile_matcher() }),
			Err(error) => {
				Err(RestrictedGlobError::Generic(error.kind().to_string().into_boxed_str()))
			},
		}
	}
}
...
impl<'a> CandidatePath<'a> {
	...
	pub fn matches(&self, glob:&RestrictedGlob) -> bool { glob.is_match_candidate(self) }
	...
	/// 	&["*".parse().unwrap(), "!a*".parse().unwrap(), "a".parse().unwrap()];
	///
	...
	/// use biome_js_analyze::utils::restricted_glob::{CandidatePath, RestrictedGlob};
	///
	/// let globs:&[RestrictedGlob] = &["a/path".parse().unwrap(), "!b".parse().unwrap()];
	///
...
mod tests {
	...
	fn test_restricted_regex() {
		assert!(!"*.js".parse::<RestrictedGlob>().unwrap().is_match("file/path.js"));
	...
	#[test]
	fn test_to_string() {
		assert_eq!(RestrictedGlob::from_str("**/*.js").unwrap().to_string(), "**/*.js");
