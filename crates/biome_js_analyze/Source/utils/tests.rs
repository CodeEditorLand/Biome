...
/// Search and renames alls bindings where the name contains "a" replacing it to
/// "b". Asserts the renaming worked.
pub fn assert_rename_binding_a_to_b_ok(before:&str, expected:&str) {
	let r = biome_js_parser::parse(before, JsFileSource::js_module(), JsParserOptions::default());
	...
	for binding in bindings {
		let new_name = binding.name_token().unwrap().text_trimmed().replace('a', "b");
...
/// Search an identifier named "a" and remove the entire node of type Anc around
/// it. Asserts the removal worked.
	...
	let node_to_remove = match identifiers_a.as_slice() {
		[identifier_a] => {
			identifier_a
				...
				.unwrap_or_else(|| {
					panic!(
						"Trying to remove the {} ancestor of identifier a, but it has no such \
						 ancestor",
						type_name::<Anc>()
					)
				})
		...
		_ => {
			panic!("Expected exactly one identifier named a, but got {identifiers_a:?}")
		},
	};
