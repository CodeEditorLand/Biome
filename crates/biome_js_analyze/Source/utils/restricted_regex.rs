...
///
/// A restricted regular expression is implicitly delimited by the anchors `^`
/// and `$`.
...
impl From<RestrictedRegex> for String {
	fn from(value:RestrictedRegex) -> Self { value.to_string() }
}
...
impl RestrictedRegexError {
	fn new(error:regex::Error, index:usize) -> Self { Self { error, index:Some(index as u32) } }

	/// Returns the index in the glob where the error is located.
	pub fn index(&self) -> Option<u32> { self.index }
}
...
/// Returns an error if `pattern` doesn't follow the restricted regular
/// expression syntax.
