use biome_js_syntax::{
	inner_string_text, AnyJsExpression, JsBinaryExpression, JsSyntaxNode,
};
...
pub(crate) fn is_node_equal(
	...
	for (a_event, b_event) in iter::zip(a_tree, b_tree) {
		let (WalkEvent::Enter(a_child), WalkEvent::Enter(b_child)) =
			(a_event, b_event)
		else {
			continue;
		};
...
pub(crate) fn find_variable_position(
	...
	let operator_range =
		binary_expression.operator_token().ok()?.text_trimmed_range();
	...
	binary_expression
		...
		.map(|child| {
			if child.syntax().text_trimmed_range().end()
				<= operator_range.start()
			{
				return VariablePosition::Left;
			} else if operator_range.end()
				<= child.syntax().text_trimmed_range().start()
			{
...
mod test {
	...
	fn find_variable_position_matches_on_left() {
		...
		let binary_expression =
			parsed.syntax().descendants().find_map(JsBinaryExpression::cast);
