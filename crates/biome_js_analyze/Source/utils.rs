...
/// Verifies that both nodes are equal by checking their descendants (nodes
/// included) kinds and tokens (same kind and inner token text).
...
///
/// This function takes a reference to a JsBinaryExpression and the name of a
/// variable as input. It determines whether the variable appears to the left or
/// right of the binary operator within the expression and returns the result as
/// an `Option<VariablePosition>`.
///
...
pub(crate) fn find_variable_position(
	...
	let operator_range = binary_expression.operator_token().ok()?.text_trimmed_range();
...
mod test {
	...
	fn find_variable_position_matches_on_left() {
		...
		let binary_expression = parsed.syntax().descendants().find_map(JsBinaryExpression::cast);

		let variable = "a";
		let position =
			find_variable_position(&binary_expression.expect("valid binary expression"), variable);
