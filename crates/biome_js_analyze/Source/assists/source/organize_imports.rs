	...
	fn run(ctx:&RuleContext<Self>) -> Option<Self::State> {
		...
		for item in root.items() {
			let AnyJsModuleItem::JsImport(import) = item else {
				// If we have pending nodes and encounter a non-import node,
				// append the nodes to a new group
				if let Some(first_node) = first_node.take() {
					groups.push(ImportGroup { first_node, nodes:take(&mut nodes) });
				}
				continue;
			};
			...
			// If this is not the first import in the group, check for a group
			// break
	...
	fn action(ctx:&RuleContext<Self>, groups:&Self::State) -> Option<JsRuleAction> {
		...
		while let Some((item_slot, item)) = iter.next() {
			...
			let mut group_leading_trivia:Vec<_> = group_leading_trivia
				...
				.map_or_else(
					|| group_leading_trivia.pieces().take_while(is_ascii_whitespace).collect(),
			...
			for (node_index, import_node) in nodes_iter {
				...
				if node_index > 0 {
					iter.next().unwrap_or_else(|| panic!("mising node {item_slot} {node_index}"));
				}
				...
				if node_index == 0 && group_first_token != first_token {
					...
					// reused by the import take the place of this node in the
					// list
...
impl ImportGroup {
	...
	fn is_sorted(&self) -> bool {
		// The imports are sorted if the text position of each node in the
		// `BTreeMap` (sorted in natural order) is higher than the previous
...
impl ImportNode {
	/// Returns `true` if the named import specifiers of this import node are
	/// sorted
	fn is_sorted(&self) -> bool {
		let mut iter = self.specifiers.values().map(|(node, _)| node.syntax().text_range().start());
	...
	/// Build a clone of the original node this import node was created from
	/// with its import specifiers sorted
	fn build_sorted_node(&self) -> JsImport {
		...
		let items = self.specifiers.values().enumerate().map(|(index, (node, sep))| {
			...
			if let Some(sep) = sep {
				if is_last && separator_count == last_element {
					// If this is the last item and we are removing its
					// trailing separator, move the trailing trivia
					// from the separator to the node
					let next_token =
				...
				// If the node has no separator and this is not the last
				// item, remove the trailing trivia since it will get
				// cloned on the inserted separator
			...
			// Check if the last separator we emitted ended with a
			// single-line comment
			if let Some(newline_source) = needs_newline.take() {
				if let Some(first_token) = node.syntax().first_token() {
					if let Some(new_token) = prepend_leading_newline(&first_token, newline_source) {
						node = node
		...
		let separators = self.specifiers.values().take(separator_count).map(|(node, sep)| {
			// If this entry has an associated separator, reuse it
			let (token, will_need_newline) = if let Some(sep) = sep {
				// If the last trivia piece for the separator token is a
				// single-line comment, signal to the items iterator it
				// will need to prepend a newline to the leading trivia
				// of the next node
				...
				// If the node we're attaching this separator to has no
				// trailing trivia, just create a simple comma token
				...
				// Otherwise we need to clone the trailing trivia from the
				// node to the separator (the items iterator should
				// have already filtered this trivia when it previously
				// emitted the node)
				...
				for piece in last_trailing_trivia.pieces() {
					...
					will_need_newline = matches!(piece.kind(), TriviaPieceKind::SingleLineComment);
				}
			...
			// If the last trivia piece was a single-line comment, signal to
			// the items iterator it will need to prepend a newline to
			// the leading trivia of the next node, and provide
			// it the token that followed this separator in the original
			// source so the newline trivia can be cloned from there
			let newline_source =
		...
		// If the separators iterator has a pending newline, prepend it to
		// closing curly token
...
fn prepend_leading_newline(
	...
	let has_leading_newline = leading_trivia.first().map_or(false, |piece| piece.is_newline());
	...
	// Prepend a newline trivia piece to the node, either by copying the leading
	// newline and whitespace from `newline_source`, or falling back to the
	// "\n" character
...
enum ImportCategory {
	...
	/// Deno.
	Npm,
	/// Modules that contains the column `:` are usually considered "virtual
	/// modules". E.g. `astro:middleware`
	///
	/// This modules are usually injected by the environment of the application,
	/// and usually present before any relative module.
	VirtualModule,
