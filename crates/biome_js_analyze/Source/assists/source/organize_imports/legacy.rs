...
pub(crate) fn action(
	...
	while let Some((item_slot, item)) = iter.next() {
		...
		for (node_index, import_node) in nodes_iter {
			...
			if node_index > 0 {
				iter.next().unwrap_or_else(|| panic!("missing node {item_slot} {node_index}"));
			}
	...
	fn build_sorted_node(&self) -> JsImport {
		...
		let separators = self.specifiers.values().take(separator_count).map(|(node, sep)| {
			...
			let (token, will_need_newline) = if let Some(sep) = sep {
				...
				// If the node we're attaching this separator to has no trailing trivia, just
				// create a simple comma token
				...
				// (the items iterator should have already filtered this trivia when it
				// previously emitted the node)
			...
			// If the last trivia piece was a single-line comment, signal to the items
		...
		// If the separators iterator has a pending newline, prepend it to closing curly
		// token
