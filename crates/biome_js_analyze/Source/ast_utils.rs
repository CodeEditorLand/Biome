...
use biome_js_syntax::{
	AnyJsArrayElement, AnyJsExpression, AnyJsLiteralExpression, AnyJsTemplateElement, JsAssignmentOperator, JsLanguage,
	JsLogicalOperator, JsSyntaxNode, JsSyntaxToken, JsUnaryOperator,
};
...
pub fn is_constant_condition(test: AnyJsExpression, in_boolean_position: bool, model: &SemanticModel) -> Option<()> {
	use AnyJsExpression::*;
	...
	match test.omit_parentheses() {
		...
		JsLogicalExpression(node) => {
			...
			let op = node.operator().ok()?;
			let is_left_constant = is_constant_condition(left.clone(), in_boolean_position, model).is_some();
			let is_right_constant = is_constant_condition(right.clone(), in_boolean_position, model).is_some();

			let is_left_short_circuit = is_left_constant && is_logical_identity(left, op);
			let is_right_short_circuit = in_boolean_position && is_right_constant && is_logical_identity(right, op);

			if (is_left_constant && is_right_constant) || is_left_short_circuit || is_right_short_circuit {
				Some(())
			} else {
				None
			}
		},
		JsSequenceExpression(node) => is_constant_condition(node.right().ok()?, in_boolean_position, model),
						...
						if let Ok(element) = js_statement {
							match element {
								...
								_ => element
									...
									.and_then(|node| is_constant_condition(node.clone(), false, model))
									.is_some(),
							}
						} else {
							false
		...
		JsCallExpression(node) => {
			if node.has_callee("Boolean") {
				...
				return is_constant_condition(args.first()?.ok()?.as_any_js_expression()?.clone(), true, model);
			}
	...
	match node.omit_parentheses() {
		...
		JsLogicalExpression(node) => {
			if let Ok(node_operator) = node.operator() {
				...
				let is_left_logical_identify = node.left().ok().is_some_and(|left| is_logical_identity(left, operator));
				if is_left_logical_identify {
					return true;
				}

				node.right().ok().is_some_and(|right| is_logical_identity(right, operator))
			} else {
				false
			}
		},
			...
			if let Ok(node_operator) = node.operator() {
				if let Ok(right) = node.right() {
					let is_valid_logical_assignment = match node_operator {
						JsAssignmentOperator::LogicalAndAssign if operator == JsLogicalOperator::LogicalAnd => true,
						JsAssignmentOperator::LogicalOrAssign if operator == JsLogicalOperator::LogicalOr => true,
						_ => false,
					};
...
fn get_boolean_value(node: &AnyJsLiteralExpression) -> bool {
	...
	match node {
		...
		_ => node.as_static_value().is_some_and(|value| !value.is_falsy()),
	}
}
