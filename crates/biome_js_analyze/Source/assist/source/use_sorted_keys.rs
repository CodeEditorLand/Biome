...
use biome_js_syntax::{AnyJsObjectMember, AnyJsObjectMemberName, JsObjectExpression, JsObjectMemberList};
			...
			if let Ok(element) = element.node() {
				match element {
					...
					AnyJsObjectMember::JsShorthandPropertyObjectMember(member) => {
						let name = member.name().ok().map(|name| name.name().ok()).unwrap_or_default();
