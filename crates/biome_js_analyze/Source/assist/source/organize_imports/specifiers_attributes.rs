...
use biome_rowan::{
	AstNode, AstSeparatedElement, AstSeparatedList, Language, SyntaxToken, TriviaPieceKind, chain_trivia_pieces,
};
...
impl JsNamedSpecifiers {
	pub fn are_sorted(&self) -> bool {
		match self {
			JsNamedSpecifiers::JsNamedImportSpecifiers(specifeirs) => are_import_specifiers_sorted(specifeirs),
			JsNamedSpecifiers::JsExportNamedFromSpecifierList(specifeirs) => are_export_specifiers_sorted(specifeirs),
		}
		.unwrap_or_default()
	}
}
...
pub fn are_import_specifiers_sorted(named_specifiers: &JsNamedImportSpecifiers) -> Option<bool> {
	...
	if specifiers.len() > 1 {
		...
		for AstSeparatedElement { node, trailing_separator } in specifiers.elements() {
...
pub fn sort_import_specifiers(named_specifiers: JsNamedImportSpecifiers) -> Option<JsNamedImportSpecifiers> {
	let list = named_specifiers.specifiers();
	let mut last_has_separator = false;
	let mut sorted = Vec::with_capacity(list.len());
	for AstSeparatedElement { node, trailing_separator } in list.elements() {
	...
	handle_trvia(sorted.iter_mut().map(|(_, a, b)| (a, b)), last_has_separator, || {
		make::token(T![,])
	});
	let separators: Vec<_> = sorted.iter_mut().filter_map(|(_, _, sep)| sep.take()).collect();
...
pub fn sort_export_specifiers(
	...
	for AstSeparatedElement { node, trailing_separator } in named_specifiers.elements() {
		let node = node.ok()?;
		let separator = trailing_separator.ok()?;
...
pub fn are_import_attributes_sorted(attributes: &JsImportAssertion) -> Option<bool> {
	...
	if attributes_list.len() > 1 {
		...
		for AstSeparatedElement { node, trailing_separator } in attributes_list.elements() {
...
fn handle_trvia<'a, L: Language + 'a, N: AstNode<Language = L> + 'a>(
	...
	for (i, (node, optional_separator)) in iter.enumerate() {
		if let Some(separator) = optional_separator {
			...
			if i == last_index
				&& !(keep_last_separator || separator.has_leading_comments() || separator.has_trailing_comments())
			{
			...
			*optional_separator = Some(make_separator().with_trailing_trivia([(TriviaPieceKind::Whitespace, " ")]));
		}
	}
}
