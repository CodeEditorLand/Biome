use biome_js_syntax::{
	AnyJsCombinedSpecifier, AnyJsExportClause, AnyJsImportClause, AnyJsModuleItem, AnyJsModuleSource, JsExport,
	JsImport, JsImportAssertion,
};
use biome_rowan::AstNode;

use super::{
	comparable_token::ComparableToken, import_groups, import_source, specifiers_attributes::JsNamedSpecifiers,
};
...
impl ImportStatementKind {
	pub fn has_type_token(self) -> bool {
		(ImportStatementKind::DefaultType | ImportStatementKind::NamespaceType | ImportStatementKind::NamedType)
			.contains(self)
	}
...
impl ImportInfo {
	...
	fn from_import(value: &JsImport) -> Option<(Self, Option<JsNamedSpecifiers>, Option<JsImportAssertion>)> {
	...
	fn from_export(value: &JsExport) -> Option<(Self, Option<JsNamedSpecifiers>, Option<JsImportAssertion>)> {
		let (kind, _first_local_name, named_specifiers, source, attributes) = match value.export_clause().ok()? {
			AnyJsExportClause::JsExportFromClause(clause) => (
				...
				clause.export_as().and_then(|export_as| export_as.exported_name().ok()),
				None,
				clause.source(),
				clause.assertion(),
			),
