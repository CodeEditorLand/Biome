...
pub fn leading_newlines(syntax: &JsSyntaxNode) -> impl Iterator<Item = SyntaxTriviaPiece<JsLanguage>> {
	syntax
...
fn find_last_blank_line_index(trivia: &JsSyntaxTrivia) -> Option<usize> {
	...
	while let Some((index, piece)) = reversed_pieces.next() {
		if piece.is_newline() && reversed_pieces.next().is_some_and(|(_, piece)| piece.is_newline()) {
			return Some(index);
		}
	}
	None
