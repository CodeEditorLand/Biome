...
impl ImportGroups {
	...
	pub fn separated_by_blank_line(&self, first_group: u16, second_group: u16) -> bool {
		self.0
			...
			.is_some_and(|groups| groups.iter().any(|group| matches!(group, ImportGroup::BlankLine,)))
	}
}
...
impl ImportGroup {
	pub fn contains(&self, candidate: &ImportSourceCandidate) -> bool {
		match self {
			...
			ImportGroup::MatcherList(matchers) => candidate.matches_with_exceptions(matchers.iter()),
		}
	}
}
...
impl<'a> ImportSourceCandidate<'a> {
	...
	pub fn new(source: &'a ImportSource<ComparableToken>) -> Self {
		Self { source, path_candidate: CandidatePath::new(source.inner().0.text()) }
	}
