	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		impl ChunkBuilder {
			fn new(key: ImportKey) -> Self {
				Self { slot_indexes: key.slot_index..key.slot_index, max_key: key }
			}
		}
		...
		fn report_unsorted_chunk(chunk: Option<ChunkBuilder>, result: &mut Vec<Issue>) {
			if let Some(chunk) = chunk {
				if !chunk.slot_indexes.is_empty() {
					result.push(Issue::UnsortedChunkPrefix { slot_indexes: chunk.slot_indexes });
				}
			}
		}
		...
		for item in root.items() {
			if let Some((info, specifiers, attributes)) = ImportInfo::from_module_item(&item) {
				...
				let blank_line_separated_groups = options.groups.separated_by_blank_line(prev_group, key.group);
				let starts_chunk = chunk.is_none();
				let leading_newline_count = leading_newlines(item.syntax()).count();
				let are_specifiers_unsorted = specifiers.is_some_and(|specifiers| !specifiers.are_sorted());
				let are_attributes_unsorted = attributes
					.is_some_and(|attributes| !(are_import_attributes_sorted(&attributes).unwrap_or_default()));
				...
				if are_specifiers_unsorted || are_attributes_unsorted || !matches!(newline_issue, NewLineIssue::None) {
				...
				if let AnyJsModuleItem::AnyJsStatement(statement) = &item {
					if leading_newlines(statement.syntax()).count() == 1 {
						result.push(Issue::AddLeadingNewline { slot_index: statement.syntax().index() as u32 });
					}
				}
			}
		...
		report_unsorted_chunk(chunk.take(), &mut result);
		if result.is_empty() { None } else { Some(result.into()) }
	}
		...
		for issue in state {
			match issue {
				Issue::AddLeadingNewline { slot_index } => {
					let item = root.items().into_iter().nth(*slot_index as usize)?.into_syntax();
				...
				Issue::UnorganizedItem {
						...
						AnyJsModuleItem::JsExport(export) => {
							...
							if *are_specifiers_unsorted {
								...
								if let AnyJsExportClause::JsExportNamedFromClause(cast) = &clause {
									if let Some(sorted_specifiers) = sort_export_specifiers(&cast.specifiers()) {
					...
					match newline_issue {
						...
						NewLineIssue::ExtraNewLine => {
							...
							let leading_trivia =
								item.first_leading_trivia()?.pieces().skip(leading_newlines(&item).count() - 1);
				...
				Issue::UnsortedChunkPrefix { slot_indexes } => {
					...
					for (index, key) in (slot_indexes.start..).zip(import_keys.drain(..)) {
						...
						let mut new_item = if let Some(item) = organized_items.remove(&key.slot_index) {
							item
						} else {
						...
						if index == slot_indexes.start {
							if let Some(detached) = detached_trivia(&old_item) {
								if leading_newlines(&old_item).count() == 1 {
									...
									new_item =
										new_item.prepend_trivia_pieces(chain_trivia_pieces(newline, detached))?;
								} else {
								...
								if header_trivia.is_empty() {
									...
									new_item = new_item.prepend_trivia_pieces(header_trivia.pieces())?;
								}
							}
							...
							let new_first_token =
								first_token.clone().with_leading_trivia([(TriviaPieceKind::Newline, "\n")]);
							new_item = new_item.replace_child(first_token.into(), new_first_token.into())?;
						}
						// Add newline for group separation
						if index != 0 && blank_line_separated_groups && leading_newlines(&new_item).count() == 1 {
