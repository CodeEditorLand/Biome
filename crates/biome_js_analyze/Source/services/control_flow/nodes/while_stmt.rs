...
impl NodeVisitor for WhileVisitor {
	...
	fn enter(node: Self::Node, builder: &mut FunctionBuilder, _: StatementStack) -> SyntaxResult<Self> {
		...
		Ok(Self { label, continue_block, break_block, loop_block })
	}

	fn exit(self, node: Self::Node, builder: &mut FunctionBuilder, _: StatementStack) -> SyntaxResult<()> {
		let Self { continue_block, break_block, loop_block, .. } = self;
		...
		builder.append_jump(true, loop_block).with_node(node.test()?.into_syntax());
