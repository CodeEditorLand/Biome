...
impl NodeVisitor for ForVisitor {
	...
	fn enter(node: Self::Node, builder: &mut FunctionBuilder, _: StatementStack) -> SyntaxResult<Self> {
		...
		if let Some(initializer) = node.initializer() {
			builder.append_statement().with_node(initializer.into_syntax());
		}
		...
		let label = node.parent::<JsLabeledStatement>().and_then(|label| label.label_token().ok());

		Ok(Self { label, continue_block, break_block, cond_block, loop_block })
	}

	fn exit(self, node: Self::Node, builder: &mut FunctionBuilder, _: StatementStack) -> SyntaxResult<()> {
		let Self { continue_block, break_block, cond_block, loop_block, .. } = self;
		...
		if let Some(test) = node.test() {
			builder.append_jump(true, loop_block).with_node(test.syntax().clone());
		} else {
