...
impl NodeVisitor for IfVisitor {
	...
	fn enter(_: Self::Node, builder: &mut FunctionBuilder, _: StatementStack) -> SyntaxResult<Self> {
		let entry_block = builder.cursor();

		let consequent_start = builder.append_block();
		builder.set_cursor(consequent_start);

		Ok(Self { entry_block, consequent_start, consequent_end: None, alt_block: None })
	}
...
impl NodeVisitor for ElseVisitor {
	...
	fn enter(_: Self::Node, builder: &mut FunctionBuilder, _: StatementStack) -> SyntaxResult<Self> {
		...
		Ok(Self { consequent_block, alt_block })
	}
