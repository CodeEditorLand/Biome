...
impl NodeVisitor for DoWhileVisitor {
	...
	fn enter(node: Self::Node, builder: &mut FunctionBuilder, _: StatementStack) -> SyntaxResult<Self> {
		...
		Ok(Self { label, continue_block, break_block, body_block })
	}

	fn exit(self, node: Self::Node, builder: &mut FunctionBuilder, _: StatementStack) -> SyntaxResult<()> {
		let Self { continue_block, break_block, body_block, .. } = self;
		...
		builder.append_jump(true, body_block).with_node(node.test()?.into_syntax());
