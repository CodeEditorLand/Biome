...
use crate::services::control_flow::{
	FunctionBuilder,
	nodes::{BlockVisitor, DoWhileVisitor, ForInVisitor, ForOfVisitor, ForVisitor, SwitchVisitor, WhileVisitor},
...
impl NodeVisitor for BreakVisitor {
	...
	fn enter(node: Self::Node, builder: &mut FunctionBuilder, state: StatementStack) -> SyntaxResult<Self> {
		...
		let break_block = state
			...
			.find_map(|(type_id, index)| {
				let (block_label, block) = if let Some(visitor) = state.try_downcast::<ForVisitor>(*type_id, *index) {
