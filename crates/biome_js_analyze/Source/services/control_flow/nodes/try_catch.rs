...
impl NodeVisitor for TryVisitor {
	...
	fn enter(node: Self::Node, builder: &mut FunctionBuilder, _: StatementStack) -> SyntaxResult<Self> {
		...
		Ok(Self { catch_block, finally_block, next_block })
	}
}
...
impl NodeVisitor for CatchVisitor {
	...
	fn enter(_: Self::Node, builder: &mut FunctionBuilder, stack: StatementStack) -> SyntaxResult<Self> {
	...
	fn exit(self, _: Self::Node, builder: &mut FunctionBuilder, stack: StatementStack) -> SyntaxResult<()> {
		let try_stmt = stack.read_top::<TryVisitor>()?;
