...
impl NodeVisitor for SwitchVisitor {
	...
	fn enter(node: Self::Node, builder: &mut FunctionBuilder, _: StatementStack) -> SyntaxResult<Self> {
		...
		Ok(Self { entry_block, label, break_block, is_first_case_clause: true, default_block: None })
	}

	fn exit(self, _: Self::Node, builder: &mut FunctionBuilder, _: StatementStack) -> SyntaxResult<()> {
		let Self { entry_block, break_block, is_first_case_clause, .. } = self;
...
impl NodeVisitor for CaseVisitor {
	...
	fn enter(node: Self::Node, builder: &mut FunctionBuilder, stack: StatementStack) -> SyntaxResult<Self> {
