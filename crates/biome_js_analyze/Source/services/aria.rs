...
use biome_aria::{
	iso::{countries, is_valid_country, is_valid_language, languages},
	AriaProperties,
	AriaRoles,
};
...
impl AriaServices {
	...
	pub fn aria_properties(&self) -> &AriaProperties { &self.properties }

	pub fn is_valid_iso_language(&self, language:&str) -> bool { is_valid_language(language) }

	pub fn is_valid_iso_country(&self, country:&str) -> bool { is_valid_country(country) }

	pub fn iso_country_list(&self) -> &'static [&'static str] { countries() }

	pub fn iso_language_list(&self) -> &'static [&'static str] { languages() }

	/// Parses a [JsxAttributeList] and extracts the names and values of each
	/// [JsxAttribute], returning them as a [FxHashMap]. Attributes with no
	/// specified value are given a value of "true". If an attribute has
	/// multiple values, each value is stored as a separate item in the
	/// [FxHashMap] under the same attribute name. Returns [None] if the parsing
	/// fails.
...
mod tests {
	...
	fn test_extract_attributes() {
		// Assume attributes of `<div class="wrapper document"
		// role="article"></div>`
		...
		let services =
			AriaServices { roles:Arc::new(AriaRoles {}), properties:Arc::new(AriaProperties {}) };
