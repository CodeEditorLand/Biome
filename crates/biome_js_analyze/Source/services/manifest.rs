...
impl ManifestServices {
	pub(crate) fn name(&self) -> Option<&str> {
		self.manifest.as_ref().as_ref().and_then(|pkg| pkg.name.as_deref())
	}

	pub(crate) fn is_dependency(&self, specifier: &str) -> bool {
		self.manifest.as_ref().as_ref().is_some_and(|pkg| pkg.dependencies.contains(specifier))
	}

	pub(crate) fn is_dev_dependency(&self, specifier: &str) -> bool {
		self.manifest.as_ref().as_ref().is_some_and(|pkg| pkg.dev_dependencies.contains(specifier))
	}

	pub(crate) fn is_peer_dependency(&self, specifier: &str) -> bool {
		self.manifest.as_ref().as_ref().is_some_and(|pkg| pkg.peer_dependencies.contains(specifier))
	}
