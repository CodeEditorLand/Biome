...
impl ReactCallWithDependencyResult {
	/// Returns all [Reference] captured by the closure argument of the React
	/// hook. See [react_hook_with_dependency].
...
impl From<(u8, u8, bool)> for ReactHookConfiguration {
	fn from((closure_index, dependencies_index, builtin):(u8, u8, bool)) -> Self {
		Self { closure_index, dependencies_index, builtin }
	}
}
...
impl StableReactHookConfiguration {
	pub fn new(hook_name:&str, result:StableHookResult, builtin:bool) -> Self {
		Self { hook_name:hook_name.into(), result, builtin }
	}
}
...
impl JsonSchema for StableHookResult {
	fn schema_name() -> String { "StableHookResult".to_owned() }
			...
			subschemas:Some(Box::new(SubschemaValidation {
				one_of:Some(vec![
					...
					Schema::Object(SchemaObject {
						...
						metadata:Some(Box::new(Metadata {
							description:Some(
								"Used to indicate the hook returns an array and some of its \
								 indices have stable identities."
									.to_owned(),
							),
							..Default::default()
						})),
...
impl DeserializationVisitor for StableResultIndexVisitor {
	...
	fn visit_number(
		...
		match value.parse::<u8>() {
			...
			Err(_) => {
				diagnostics
					.push(DeserializationDiagnostic::new_out_of_bound_integer(0, 255, range));
				None
			},
		}
	}
...
mod test {
	...
	pub fn ok_react_stable_captures() {
		...
		let node = r.syntax().descendants().filter(|x| x.text_trimmed() == "ref").last().unwrap();
