...
impl SuppressionAction for JsSuppressionAction {
	...
	fn find_token_to_apply_suppression(
		...
		// If the flag has been set to `true`, it means we are at the beginning of the
		// file.
		if !should_insert_leading_newline {
			// Still, if there's a a multiline comment, we want to try to attach the
			// suppression comment to the existing multiline comment without newlines.
	...
	fn apply_suppression(
		...
		// When inside a JSX element, we have to apply different logics when applying
		// suppression comments. Newlines are inside JsxText.
		if let Some(current_jsx_element) = current_jsx_element {
			...
			if !should_insert_leading_newline {
				...
				// Trim trailing trivia to prevent double insertion of trailing whitespaces in
