...
use biome_js_syntax::{
	AnyJsxChild, JsLanguage, JsSyntaxKind, JsSyntaxToken, JsxChildList, JsxElement, JsxOpeningElement,
	JsxSelfClosingElement, JsxText, T,
};
...
impl SuppressionAction for JsSuppressionAction {
	...
	fn find_token_for_inline_suppression(&self, token: JsSyntaxToken) -> Option<ApplySuppression<Self::Language>> {
		...
		let mut should_insert_leading_newline = loop {
			...
			let nodes_that_might_contain_newlines = current_token.parent().is_some_and(|node| {
				matches!(
					node.kind(),
					JsSyntaxKind::JSX_TEXT | JsSyntaxKind::JS_STRING_LITERAL | JsSyntaxKind::TEMPLATE_CHUNK
				)
			});
			if current_token
				.trailing_trivia()
				...
				.any(|trivia| trivia.kind().is_multiline_comment())
			{
				...
				|| (nodes_that_might_contain_newlines && current_token.text_trimmed().contains(['\n', '\r']))
			{
				break false;
	...
	fn apply_inline_suppression(
		...
		if let Some(current_jsx_element) = current_jsx_element {
			...
			if current_jsx_element.parent().is_some_and(|p| JsxChildList::can_cast(p.kind())) {
				...
				} else if let Some(current_element) = JsxSelfClosingElement::cast_ref(&current_jsx_element) {
					mutation.add_jsx_elements_before_element(
