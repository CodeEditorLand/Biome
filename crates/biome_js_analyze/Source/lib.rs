...
use biome_analyze::{
	AnalysisFilter, Analyzer, AnalyzerContext, AnalyzerOptions, AnalyzerPluginSlice, AnalyzerSignal,
	AnalyzerSuppression, ControlFlow, InspectMatcher, LanguageRoot, MatchQueryParams, MetadataRegistry, RuleAction,
...
impl From<(Arc<DependencyGraph>, Arc<ProjectLayout>, JsFileSource)> for JsAnalyzerServices {
	fn from(
		(dependency_graph, project_layout, source_type): (Arc<DependencyGraph>, Arc<ProjectLayout>, JsFileSource),
	) -> Self {
		Self { dependency_graph, project_layout, source_type }
	}
}
	...
	B: 'a,
{
	...
	let JsAnalyzerServices { dependency_graph, project_layout, source_type } = services;
...
where
	...
	B: 'a,
{
	analyze_with_inspect_matcher(root, filter, |_| {}, options, plugins, services, emit_signal)
}
...
mod tests {
	...
	fn suppression_syntax() {
		...
		let options = AnalyzerOptions::default();
		analyze(&parsed.tree(), filter, &options, &[], Default::default(), |signal| {
			if let Some(diag) = signal.diagnostic() {
...
const foo0 = function (bar: string) {
  ...
  bar = 'baz';
		...
		let filter = AnalysisFilter { enabled_rules: Some(enabled_rules.as_slice()), ..AnalysisFilter::default() };
		let options = AnalyzerOptions::default();
		let root = parsed.tree();

		let services = JsAnalyzerServices::from((Default::default(), Default::default(), JsFileSource::ts()));
