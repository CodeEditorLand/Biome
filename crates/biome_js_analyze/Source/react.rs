...
use biome_js_syntax::{
	...
	JsCallExpression, JsIdentifierBinding, JsImport, JsObjectExpression, JsPropertyObjectMember, JsxMemberName,
	JsxReferenceIdentifier, binding_ext::AnyJsBindingDeclaration,
};
...
impl ReactCreateElementCall {
	...
	pub(crate) fn from_call_expression(call_expression: &JsCallExpression, model: &SemanticModel) -> Option<Self> {
		let callee = call_expression.callee().ok()?.omit_parentheses();
		let is_react_create_element = is_react_call_api(&callee, model, ReactLibrary::React, "createElement");
		...
		if is_react_create_element {
			...
			if !arguments.is_empty() {
				...
				let second_argument = iter
					...
					.and_then(|argument| argument.as_any_js_expression()?.as_js_object_expression().cloned());
...
/// `"react"` library
pub(crate) fn jsx_member_name_is_react_fragment(member_name: &JsxMemberName, model: &SemanticModel) -> Option<bool> {
...
fn is_named_react_export(binding: &Binding, lib: ReactLibrary, name: &str) -> Option<bool> {
	...
	let name_token = match &import_specifier {
		AnyJsNamedImportSpecifier::JsNamedImportSpecifier(named_import) => named_import.name().ok()?.value().ok()?,
