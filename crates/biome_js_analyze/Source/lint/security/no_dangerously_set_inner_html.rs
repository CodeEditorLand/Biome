...
use crate::{react::ReactCreateElementCall, services::semantic::Semantic};
		...
		match node {
			...
			AnyJsCreateElement::JsCallExpression(call_expression) => {
					...
					ReactCreateElementCall::from_call_expression(call_expression, model)
				{
					...
					// if we are inside a create element call, we inspect the
					// second argument, which should be an object
					// expression. We look for a member that has as name
