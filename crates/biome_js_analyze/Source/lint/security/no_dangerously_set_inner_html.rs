	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		match node {
				...
				match name {
					AnyJsxAttributeName::JsxName(jsx_name) => {
						if jsx_name.syntax().text_trimmed() == "dangerouslySetInnerHTML" {
							return Some(NoDangerState::Attribute(jsx_attribute.name().ok()?.range()));
						}
					},
			...
			AnyJsCreateElement::JsCallExpression(call_expression) => {
				if let Some(react_create_element) = ReactCreateElementCall::from_call_expression(call_expression, model)
				{
					...
					if let Some(props) = props {
						...
						for member in members {
							...
							let property_member = member.as_js_property_object_member()?.name().ok()?;
