...
use biome_js_syntax::{
	JsCallExpression, JsPropertyObjectMember, JsSyntaxNode, JsxAttribute, JsxElement, JsxSelfClosingElement,
};
	...
	fn has_children(&self, model: &SemanticModel) -> Option<JsSyntaxNode> {
		match self {
			...
			AnyJsCreateElement::JsCallExpression(expression) => {
				let react_create_element = ReactCreateElementCall::from_call_expression(expression, model)?;

				react_create_element.children.map(|children| children.syntax().clone())
			},
		}
	}
	...
	fn find_children_prop(&self, model: &SemanticModel) -> Option<DangerousProp> {
		match self {
			AnyJsCreateElement::JsxElement(element) => {
				...
				opening_element.find_attribute_by_name("children").map(DangerousProp::from)
			},
			AnyJsCreateElement::JsxSelfClosingElement(element) => {
				element.find_attribute_by_name("children").map(DangerousProp::from)
			},
			AnyJsCreateElement::JsCallExpression(call_expression) => {
				let react_create_element = ReactCreateElementCall::from_call_expression(call_expression, model)?;

				react_create_element.find_prop_by_name("children").map(DangerousProp::from)
			},
		}
	}
}
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		if let Some(dangerous_prop) = node.find_dangerous_prop(model) {
			...
			if let Some(children_node) = node.has_children(model) {
				...
				return Some(RuleState { children_kind: ChildrenKind::Prop(children_prop.range()), dangerous_prop });
			}
		}
		None
	}
