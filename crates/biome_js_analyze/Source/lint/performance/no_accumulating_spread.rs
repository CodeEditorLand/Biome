...
fn is_known_accumulator(node:&JsSpread, model:&SemanticModel) -> Option<bool> {
	let reference = node.argument().ok()?.as_js_identifier_expression()?.name().ok()?;
	...
	// Known accumulators need at least 2 arguments and no more than 4.
	// (accumulator, value, index, array)
	let param_count = function.parameters().ok()?.as_js_parameters()?.items().iter().count();
	...
	// The accumulator function should be a part of a call expression. This call
	// expression should have no more than 2 arguments. (callback, initialValue)
