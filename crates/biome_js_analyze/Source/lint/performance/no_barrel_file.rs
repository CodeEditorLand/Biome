		...
		for item in items {
			if let Some(export) = JsExport::cast(item.into()) {
				if let Ok(export_from_clause) = export.export_clause() {
					if let Some(export_from_clause) = JsExportFromClause::cast_ref(export_from_clause.syntax()) {
						if export_from_clause.type_token().is_none() {
							return Some(export);
						}
					}

					if let Some(export_from_clause) = JsExportNamedFromClause::cast(export_from_clause.into_syntax()) {
