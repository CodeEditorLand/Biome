...
fn has_type_image_attribute(element: &AnyJsxElement) -> bool {
	element.find_attribute_by_name("type").map_or(false, |attribute| {
		attribute.as_static_value().map_or(false, |value| value.text() == "image")
	})
}

fn has_valid_alt_text(element: &AnyJsxElement) -> bool {
	element.find_attribute_by_name("alt").map_or(false, |attribute| {
		if attribute.initializer().is_none() {
			return false;
		}

		attribute.as_static_value().map_or(true, |value| !value.is_null_or_undefined())
			&& !element.has_trailing_spread_prop(&attribute)
	})
}

fn has_valid_label(element: &AnyJsxElement, name_to_lookup: &str) -> bool {
	element.find_attribute_by_name(name_to_lookup).map_or(false, |attribute| {
		if attribute.initializer().is_none() {
			return false;
		}
		attribute
			.as_static_value()
			.map_or(true, |value| !value.is_null_or_undefined() && value.is_not_string_constant(""))
			&& !element.has_trailing_spread_prop(&attribute)
	})
}

fn is_aria_hidden(element: &AnyJsxElement) -> bool {
	element.find_attribute_by_name("aria-hidden").map_or(false, |attribute| {
		attribute.as_static_value().map_or(true, |value| match value {
