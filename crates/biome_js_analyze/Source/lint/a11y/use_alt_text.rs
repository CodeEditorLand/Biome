...
fn has_type_image_attribute(element: &AnyJsxElement) -> bool {
	element
		...
		.is_some_and(|attribute| attribute.as_static_value().is_some_and(|value| value.text() == "image"))
}

fn has_valid_alt_text(element: &AnyJsxElement) -> bool {
	element.find_attribute_by_name("alt").is_some_and(|attribute| {
		if attribute.initializer().is_none() {
			return false;
		}

		attribute.as_static_value().is_none_or(|value| !value.is_null_or_undefined())
	})
}

fn has_valid_label(element: &AnyJsxElement, name_to_lookup: &str) -> bool {
	element.find_attribute_by_name(name_to_lookup).is_some_and(|attribute| {
		if attribute.initializer().is_none() {
			return false;
		}
		attribute
			.as_static_value()
			.is_none_or(|value| !value.is_null_or_undefined() && value.is_not_string_constant(""))
	})
}

fn is_aria_hidden(element: &AnyJsxElement) -> bool {
	element.find_attribute_by_name("aria-hidden").is_some_and(|attribute| {
