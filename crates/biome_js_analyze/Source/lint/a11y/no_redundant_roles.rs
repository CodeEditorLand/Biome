...
fn get_explicit_role(
	...
	// If a role attribute has multiple values, the first valid value (specified
	// role) will be used. Check: https://www.w3.org/TR/2014/REC-wai-aria-implementation-20140320/#mapping_role
	let explicit_role =
		static_value.text().split(' ').find_map(|role| aria_roles.get_role(role))?;
	Some(explicit_role)
}
