...
use biome_js_syntax::{AnyJsExpression, AnyJsLiteralExpression, AnyJsTemplateElement, AnyJsxAttributeValue};
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		if let Some(aria_hidden) = aria_hidden_attribute {
			let is_false = match aria_hidden.initializer()?.value().ok()? {
				...
				AnyJsxAttributeValue::JsxString(aria_hidden) => aria_hidden.inner_string_text().ok()?.text() == "false",
			};
		...
		let alt = node.find_attribute_by_name("alt")?.initializer()?.value().ok()?;

		match alt {
			AnyJsxAttributeValue::AnyJsxTag(_) => None,
			AnyJsxAttributeValue::JsxExpressionAttributeValue(ref value) => match value.expression().ok()? {
				AnyJsExpression::AnyJsLiteralExpression(AnyJsLiteralExpression::JsStringLiteralExpression(expr)) => {
				...
				AnyJsExpression::JsTemplateExpression(expr) => {
					let contain_redundant_alt =
						expr.elements().into_iter().any(|template_element| match template_element {
...
fn is_redundant_alt(alt: &str) -> bool {
	REDUNDANT_WORDS
		.into_iter()
		.any(|word| alt.split_whitespace().any(|x| x.to_ascii_lowercase_cow() == word))
}
