...
impl Rule for NoBlankTarget {
		...
		if target_attribute.as_static_value()?.text() == "_blank" {
			if !ctx.options().allow_domains.is_empty() {
				...
				if let Some(href_value) = href_attribute.as_static_value() {
					...
					let allow_domains: Vec<&str> =
						ctx.options().allow_domains.iter().map(AsRef::as_ref).collect();
	...
	fn action(
		...
		let message = if let Some(rel_attribute) = rel_attribute {
			...
			let new_text =
				format!("noreferrer {}", prev_jsx_string.inner_string_text().ok()?.text());
			...
			let old_attribute_list =
				target_attribute.syntax().ancestors().find_map(JsxAttributeList::cast)?;
...
fn is_allowed_domain(href: &str, allow_domains: &[&str]) -> bool {
	allow_domains.iter().any(|allowed| href.starts_with(allowed) || href.contains(allowed))
}
