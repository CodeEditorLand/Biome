...
use biome_js_factory::make::{
	jsx_attribute, jsx_attribute_initializer_clause, jsx_attribute_list, jsx_ident, jsx_name, jsx_string,
	jsx_string_literal, token,
};
use biome_js_syntax::jsx_ext::AnyJsxElement;
use biome_js_syntax::{AnyJsxAttribute, AnyJsxAttributeName, AnyJsxAttributeValue, JsxAttribute, JsxAttributeList, T};
...
impl Rule for NoBlankTarget {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		let node = ctx.query();
		if node.name_value_token().ok()?.text_trimmed() != "a" || node.find_attribute_by_name("href").is_none() {
			return None;
		}
		...
		if target_attribute.as_static_value()?.text() == "_blank" {
			if !ctx.options().allow_domains.is_empty() {
				...
				if let Some(href_value) = href_attribute.as_static_value() {
					...
					let allow_domains: Vec<&str> = ctx.options().allow_domains.iter().map(AsRef::as_ref).collect();
	...
	fn action(ctx: &RuleContext<Self>, (target_attribute, rel_attribute): &Self::State) -> Option<JsRuleAction> {
		...
		let message = if let Some(rel_attribute) = rel_attribute {
			...
			let new_text = format!("noreferrer {}", prev_jsx_string.inner_string_text().ok()?.text());
			mutation.replace_node(prev_jsx_string.clone(), jsx_string(jsx_string_literal(new_text.trim_end())));
			...
			let old_attribute_list = target_attribute.syntax().ancestors().find_map(JsxAttributeList::cast)?;
	...
	fn diagnostic(_ctx: &RuleContext<Self>, (target_attribute, _): &Self::State) -> Option<RuleDiagnostic> {
