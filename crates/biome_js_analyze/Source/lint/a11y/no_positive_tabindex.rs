...
use biome_js_syntax::{
	AnyJsLiteralExpression, AnyJsxAttributeValue, JsCallExpression, JsNumberLiteralExpression, JsPropertyObjectMember,
...
impl NoPositiveTabindexQuery {
	fn find_tabindex_attribute(&self, model: &SemanticModel) -> Option<TabindexProp> {
		match self {
			NoPositiveTabindexQuery::AnyJsxElement(jsx) => {
				jsx.find_attribute_by_name("tabIndex").map(TabindexProp::from)
			},
			NoPositiveTabindexQuery::JsCallExpression(expression) => {
				...
				react_create_element.find_prop_by_name("tabIndex").map(TabindexProp::from)
			},
		}
	}
}
...
impl Rule for NoPositiveTabindex {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		match tabindex_attribute {
			...
			TabindexProp::JsPropertyObjectMember(js_object_member) => {
				...
				let expression_value = AnyNumberLikeExpression::cast(expression.into_syntax())?.value()?;
	...
	fn action(ctx: &RuleContext<Self>, _: &Self::State) -> Option<JsRuleAction> {
		...
		Some(JsRuleAction::new(
			...
			markup! { "Replace the "<Emphasis>"tableIndex"</Emphasis>" prop value with 0." }.to_owned(),
			mutation,
		))
	}
}
