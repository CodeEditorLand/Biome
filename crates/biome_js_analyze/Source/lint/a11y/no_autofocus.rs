...
use biome_analyze::{
	FixKind, Phases, QueryMatch, Queryable, Rule, RuleDiagnostic, RuleSource, Visitor, context::RuleContext,
	declare_lint_rule,
};
...
impl Rule for NoAutofocus {
	...
	fn action(ctx: &RuleContext<Self>, attr: &Self::State) -> Option<JsRuleAction> {
		...
		if attr.syntax().has_trailing_comments() {
			...
			let new_token = prev_token.append_trivia_pieces(attr.syntax().last_trailing_trivia()?.pieces());
