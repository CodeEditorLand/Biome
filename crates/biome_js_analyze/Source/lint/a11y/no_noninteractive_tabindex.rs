...
use biome_js_syntax::{AnyJsxAttributeValue, AnyNumberLikeExpression, TextRange, jsx_ext::AnyJsxElement};
...
impl Rule for NoNoninteractiveTabindex {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		if ctx.aria_roles().is_not_interactive_element(node) {
			...
			let element_name = node.name().ok()?.as_jsx_name()?.value_token().ok()?.token_text_trimmed();

			let role_attribute = node.find_attribute_by_name("role");
			let Some(role_attribute) = role_attribute else {
				return Some(RuleState { attribute_range: tabindex_attribute.range(), element_name });
			};
...
fn attribute_has_negative_tabindex(tabindex_attribute_value: &AnyJsxAttributeValue) -> Option<bool> {
...
fn attribute_has_interactive_role(role_attribute_value: &AnyJsxAttributeValue) -> Option<bool> {
	Some(AriaRole::from_roles(role_attribute_value.as_static_value()?.text()).is_some_and(|role| role.is_interactive()))
}
