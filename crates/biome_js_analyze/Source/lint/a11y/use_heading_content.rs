...
impl Rule for UseHeadingContent {
	...
	fn diagnostic(ctx: &RuleContext<Self>, _: &Self::State) -> Option<RuleDiagnostic> {
		let range = match ctx.query() {
			AnyJsxElement::JsxOpeningElement(node) => node.parent::<JsxElement>()?.syntax().text_range_with_trivia(),
...
fn has_valid_heading_content(node: &AnyJsxElement) -> bool {
	node.find_attribute_by_name("dangerouslySetInnerHTML").is_some()
		|| node.find_attribute_by_name("children").is_some_and(|attribute| {
			if attribute.initializer().is_none() {
				return false;
			}
			attribute.as_static_value().is_none_or(|attribute| !attribute.is_falsy())
