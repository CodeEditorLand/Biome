...
impl NoLabelWithoutControlOptions {
	/// Returns `true` whether the passed `attribute` meets one of the following
	/// conditions:
	/// - Has a label attribute that corresponds to the `label_attributes`
	///   parameter
	...
	fn has_label_attribute(&self, attribute:&JsxAttribute) -> bool {
		...
		if !DEFAULT_LABEL_ATTRIBUTES.contains(&attribute_name)
			&& !self.label_attributes.iter().any(|name| name == attribute_name)
		{
			return false;
		}
		attribute
	...
	/// Returns `true` whether the passed `jsx_tag` meets one of the following
	/// conditions:
	/// - Has a label attribute that corresponds to the `label_attributes`
	///   parameter
	/// - Has a label among `DEFAULT_LABEL_ATTRIBUTES`
	...
	/// Returns whether the passed `AnyJsxTag` have a child that is considered
	/// an input component according to the passed `input_components` parameter
					...
					match child.kind() {
						...
						_ => {
							...
							if DEFAULT_INPUT_COMPONENTS.contains(&element_name)
								|| self.input_components.iter().any(|name| name == element_name)
							{
								return true;
							}
						},
