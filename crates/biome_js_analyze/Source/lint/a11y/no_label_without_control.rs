...
use biome_js_syntax::{
	AnyJsxAttribute, AnyJsxAttributeName, AnyJsxAttributeValue, AnyJsxElementName, AnyJsxTag, JsSyntaxKind,
	JsxAttribute,
};
...
impl Rule for NoLabelWithoutControl {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let is_allowed_element =
			options.has_element_name(element_name) || DEFAULT_LABEL_COMPONENTS.contains(&element_name);
	...
	fn diagnostic(ctx: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
		...
		if !state.has_text_content {
			diagnostic =
				diagnostic.note(markup! { "Consider adding an accessible text content to the label element." });
		}
...
impl NoLabelWithoutControlOptions {
	...
	fn has_label_attribute(&self, attribute: &JsxAttribute) -> bool {
		...
		if !DEFAULT_LABEL_ATTRIBUTES.contains(&attribute_name)
			&& !self.label_attributes.iter().any(|name| name.as_ref() == attribute_name)
		{
			return false;
		}
		attribute
		...
		while let Some(event) = child_iter.next() {
			match event {
				WalkEvent::Enter(child) => match child.kind() {
					JsSyntaxKind::JSX_EXPRESSION_CHILD | JsSyntaxKind::JSX_SPREAD_CHILD | JsSyntaxKind::JSX_TEXT => {
						return true;
					},
				...
				WalkEvent::Enter(child) => match child.kind() {
					...
					_ => {
						...
						if DEFAULT_INPUT_COMPONENTS.contains(&element_name)
							|| self.input_components.iter().any(|name| name.as_ref() == element_name)
						{
							return true;
						}
					},
...
const DEFAULT_INPUT_COMPONENTS: [&str; 7] = ["input", "meter", "output", "progress", "select", "textarea", "button"];
...
fn has_label_attribute_value(jsx_attribute_value: &AnyJsxAttributeValue) -> bool {
	match jsx_attribute_value {
		...
		AnyJsxAttributeValue::JsxString(jsx_string) => {
			!jsx_string.inner_string_text().is_ok_and(|text| text.text().trim().is_empty())
		},
	}
}
