...
impl Rule for UseKeyWithMouseEvents {
	...
	fn diagnostic(ctx: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
		...
		Some(
			RuleDiagnostic::new(rule_category!(), node.syntax().text_trimmed_range(), state.message())
	...
	if let Some(on_mouse_over_attribute) = elem.find_attribute_by_name("onMouseOver") {
		if !elem.has_trailing_spread_prop(&on_mouse_over_attribute) {
			return elem
				...
				.is_some_and(|it| !it.as_static_value().is_some_and(|value| value.is_null_or_undefined()));
		}
	}
	true
}
