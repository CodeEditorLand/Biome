		...
		if is_inside_element {
			...
			let missing_aria_props: Vec<_> = role
				...
				.filter_map(|attribute| {
					...
					node.find_attribute_by_name(attribute_name).is_none().then_some(attribute_name)
				})
				.collect();
	...
	fn diagnostic(_ctx: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
		...
		state.attribute.as_ref().map(|(attribute, role_name)| {
			...
			let description = format!(
				"The element with the {role_name} ARIA role does not have the required ARIA attributes: {joined_attributes}."
			);
			RuleDiagnostic::new(
				rule_category!(),
				attribute.range(),
