...
impl Rule for NoInteractiveElementToNoninteractiveRole {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		if node.is_element() {
			...
			{
				// <div> and <span> are considered neither interactive nor
				// non-interactive, depending on the presence or absence of the
				// role attribute. We don't report <div> and <span> here,
				// because we cannot determine whether they are interactive or
				// non-interactive.
				...
				// A <svg> element can be given an "img" to make it
				// non-interactive for a11y reasons.
				if element_name.text_trimmed() == "svg" && role_attribute_value == "img" {
					return None;
				}

				// A <canvas> element can be given an "img" to make it
				// non-interactive for a11y reasons.
