...
impl Rule for NoSvgWithoutTitle {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		// Checks if a `svg` element has a valid `title` element is in a
		// childlist
		...
		// title/aria-label/aria-labelledby attribute
		let Some(role_attribute) = node.find_attribute_by_name("role") else {
			return Some(());
		};

		let role_attribute_value = role_attribute.initializer()?.value().ok()?;
		let Some(role_attribute_text) =
			role_attribute_value.as_jsx_string()?.inner_string_text().ok()
		else {
			return Some(());
		};
		...
		match role_attribute_text.to_lowercase_cow() {
			Cow::Borrowed("img") => {
				let [aria_label, aria_labelledby] =
					node.attributes().find_by_names(["aria-label", "aria-labelledby"]);
			...
			// if role attribute is empty, the svg element should have title
			// element
			Cow::Borrowed("") => Some(()),
			_ => None,
		}
...
/// Checks if the given attribute is attached to the `svg` element and the
/// attribute value is used by the `id` of the childs element.
