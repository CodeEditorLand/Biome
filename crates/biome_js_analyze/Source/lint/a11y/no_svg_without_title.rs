...
impl Rule for NoSvgWithoutTitle {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let Some(role_attribute_text) = role_attribute_value.as_jsx_string()?.inner_string_text().ok() else {
			return Some(());
		};

		match role_attribute_text.to_ascii_lowercase_cow().as_ref() {
			"img" => {
				let [aria_label, aria_labelledby] = node.attributes().find_by_names(["aria-label", "aria-labelledby"]);
				let is_valid_a11y_attribute = aria_label.is_some()
					|| is_valid_attribute_value(aria_labelledby, &jsx_element.children()).unwrap_or(false);
...
fn is_valid_attribute_value(attribute: Option<JsxAttribute>, jsx_child_list: &JsxChildList) -> Option<bool> {
	...
	let is_used_attribute = jsx_child_list
		...
		.filter_map(|child| {
			...
			let is_valid = attribute_value.as_static_value()?.text() == child_attribute_value.as_static_value()?.text();
			Some(is_valid)
		})
		.any(|x| x);
