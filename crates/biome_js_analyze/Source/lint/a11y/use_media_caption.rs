...
impl Rule for UseMediaCaption {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let has_audio_or_video = matches!(node.name_value_token().ok()?.text_trimmed(), "video" | "audio");
				...
				let has_track = jsx_element
					...
					.filter_map(|child| {
						let any_jsx = match child {
							...
							AnyJsxChild::JsxSelfClosingElement(element) => Some(AnyJsxElement::from(element)),
							_ => None,
						}?;
