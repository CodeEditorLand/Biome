...
use biome_js_syntax::{
	AnyJsxElementName, JsCallExpression, JsxAttribute, JsxOpeningElement, JsxSelfClosingElement, TextRange,
};
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		match node {
			UseButtonTypeQuery::JsxSelfClosingElement(element) => {
				...
				let Some(attribute) = type_attribute else {
					...
					if has_spread_prop {
						...
						return Some(UseButtonTypeState { range: element.range(), missing_prop: true });
					}
				};
			...
			UseButtonTypeQuery::JsCallExpression(call_expression) => {
				...
				let first_argument = react_create_element.element_type.as_any_js_expression()?.as_static_value()?;
				if first_argument.text() != "button" {
					return None;
				}
				let Some(props) = react_create_element.props.as_ref() else {
					return Some(UseButtonTypeState { range: first_argument.range(), missing_prop: true });
				};
				let Some(member) = react_create_element.find_prop_by_name("type") else {
					// We haven't found the property `type`
					return Some(UseButtonTypeState { range: props.range(), missing_prop: false });
				};
				let property_value = member.value().ok()?.as_static_value()?;
				if ALLOWED_BUTTON_TYPES.contains(&property_value.text()) {
					None
				} else {
					Some(UseButtonTypeState { range: property_value.range(), missing_prop: false })
				}
			},
		}
	}
...
fn inspect_jsx_type_attribute(attribute: &JsxAttribute) -> Option<UseButtonTypeState> {
	let Some(initializer) = attribute.initializer() else {
		return Some(UseButtonTypeState { range: attribute.range(), missing_prop: false });
	};
	...
	Some(UseButtonTypeState { range: value.range(), missing_prop: false })
}
