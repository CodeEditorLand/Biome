...
use biome_js_syntax::{
	AnyJsxAttribute, AnyJsxAttributeName, AnyJsxAttributeValue, JsxAttributeList, T, jsx_ext::AnyJsxElement,
};
...
impl Rule for UseAriaActivedescendantWithTabindex {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		if node.is_element()
			...
			&& node.find_attribute_by_name("aria-activedescendant").is_some()
	...
	fn action(ctx: &RuleContext<Self>, _: &Self::State) -> Option<JsRuleAction> {
		...
		let old_attribute_list = descendant_attribute.syntax().ancestors().find_map(JsxAttributeList::cast)?;
