...
impl Rule for UseAriaActivedescendantWithTabindex {
		...
		Some(
			...
			.note(
				"aria-activedescendant is used to manage focus within a composite widget.\nThe \
				 element with the attribute aria-activedescendant retains the active document \
				 focus.",
			)
			.note(
				"Add the tabIndex attribute to the element with a value greater than or equal to \
				 -1.",
			),
		)
	}
	...
	fn action(ctx:&RuleContext<Self>, _:&Self::State) -> Option<JsRuleAction> {
		...
		let old_attribute_list =
			descendant_attribute.syntax().ancestors().find_map(JsxAttributeList::cast)?;
