...
impl Rule for NoImplicitBoolean {
	...
	fn action(ctx: &RuleContext<Self>, _: &Self::State) -> Option<JsRuleAction> {
		...
		let JsxAttributeFields { name, initializer: _ } = n.as_fields();
		...
		let next_name = name.replace_token_discard_trivia(last_token_of_name_syntax, next_last_token_of_name_syntax)?;
		let attr_value = make::jsx_expression_attribute_value(
			...
			biome_js_syntax::AnyJsExpression::AnyJsLiteralExpression(
				AnyJsLiteralExpression::JsBooleanLiteralExpression(make::js_boolean_literal_expression(make::token(
					T![true],
				))),
			),
			make::token(JsSyntaxKind::R_CURLY),
		);
		let next_attr = make::jsx_attribute(next_name).with_initializer(make::jsx_attribute_initializer_clause(
			make::token(T![=]),
