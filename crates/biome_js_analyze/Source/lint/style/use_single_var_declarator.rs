...
use biome_js_syntax::{
	JsSyntaxKind, JsSyntaxToken, JsVariableDeclarationFields, JsVariableStatement, JsVariableStatementFields, T,
	TextSize, TriviaPieceKind,
};
use biome_rowan::{AstNode, AstSeparatedList, BatchMutationExt, TriviaPiece, trim_leading_trivia_pieces};
...
impl Rule for UseSingleVarDeclarator {
	...
	fn action(ctx: &RuleContext<Self>, _state: &Self::State) -> Option<JsRuleAction> {
		...
		let JsVariableDeclarationFields { await_token, kind, declarators } = declaration.ok()?.as_fields();
		let kind = kind.ok()?;
		let index = prev_parent.children().position(|slot| &slot == node.syntax())?;
		...
		let next_parent = prev_parent.clone().splice_slots(
			index..=index,
			declarators.iter().enumerate().filter_map(|(index, declarator)| {
				...
				let kind = if index == 0 {
					...
					if let Some(last_piece) = kind.trailing_trivia().last() {
						if last_piece.kind().is_single_line_comment() {
							kind.prepend_trivia_pieces(trim_leading_trivia_pieces(kind.trailing_trivia().pieces()))
					...
					JsSyntaxToken::new_detached(
						...
						&token_text,
						trivia_pieces,
						[TriviaPiece::new(TriviaPieceKind::Whitespace, TextSize::from(1))],
					)
				};

				let mut variable_declaration =
					make::js_variable_declaration(kind, make::js_variable_declarator_list([declarator], []));
				...
				if let Some(Ok(separator)) = separators.next() {
					if separator.has_trailing_comments() {
						result = result.append_trivia_pieces(separator.trailing_trivia().pieces())?;
					}
				}
