...
use crate::{JsRuleAction, use_block_statements_diagnostic, use_block_statements_replace_body};
	...
	fn action(ctx:&RuleContext<Self>, nodes_need_to_replaced:&Self::State) -> Option<JsRuleAction> {
		...
		match nodes_need_to_replaced {
			UseBlockStatementsOperationType::Wrap(stmt) => {
				...
				// Ensure the opening curly token is separated from the previous token by at
				// least one space
				...
				let r_curly_token = if !leading_trivia.is_empty() {
					...
					// if the node we have to enclose has some trailing comments, then we add a new
					// line to the leading trivia of the right curly brace
