...
use crate::{use_block_statements_diagnostic, use_block_statements_replace_body, JsRuleAction};
	...
	fn action(ctx:&RuleContext<Self>, nodes_need_to_replaced:&Self::State) -> Option<JsRuleAction> {
		...
		match nodes_need_to_replaced {
			UseBlockStatementsOperationType::Wrap(stmt) => {
				...
				// Ensure the opening curly token is separated from the previous
				// token by at least one space
				...
				let r_curly_token = if !leading_trivia.is_empty() {
					let leading_trivia =
						leading_trivia.iter().rev().map(|piece| (piece.kind(), piece.text()));

					r_curly_token.with_leading_trivia(leading_trivia)
				} else {
					let has_trailing_single_line_comments =
						stmt.syntax().last_trailing_trivia().map_or(false, |trivia| {
							trivia
								.pieces()
								.any(|trivia| trivia.kind() == TriviaPieceKind::SingleLineComment)
						});
					// if the node we have to enclose has some trailing
					// comments, then we add a new line to the leading
					// trivia of the right curly brace
			...
			UseBlockStatementsOperationType::ReplaceBody => {
				match node {
					AnyJsBlockStatement::JsIfStatement(stmt) => {
...
/// Collect newline and comment trivia pieces in reverse order up to the first
/// newline included
