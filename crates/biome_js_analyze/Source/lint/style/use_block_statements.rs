...
use biome_js_syntax::{
	AnyJsStatement, JsDoWhileStatement, JsElseClause, JsForInStatement, JsForOfStatement, JsForStatement,
	JsIfStatement, JsLanguage, JsSyntaxTrivia, JsWhileStatement, JsWithStatement, T, TriviaPieceKind,
};
...
impl Rule for UseBlockStatements {
	...
	fn run(ctx: &RuleContext<Self>) -> Option<Self::State> {
		...
		match node {
			...
			AnyJsBlockStatement::JsElseClause(stmt) => {
				...
				let is_block = matches!(body, AnyJsStatement::JsBlockStatement(_) | AnyJsStatement::JsIfStatement(_));
				if !is_block {
	...
	fn action(ctx: &RuleContext<Self>, nodes_need_to_replaced: &Self::State) -> Option<JsRuleAction> {
		let node = ctx.query();
		...
		match nodes_need_to_replaced {
			UseBlockStatementsOperationType::Wrap(stmt) => {
				...
				if !has_previous_space {
					l_curly_token = l_curly_token.with_leading_trivia([(TriviaPieceKind::Whitespace, " ")]);
				}
				...
				if leading_trivia.is_empty() {
					l_curly_token = l_curly_token.with_trailing_trivia([(TriviaPieceKind::Whitespace, " ")]);
				}
				...
				let r_curly_token = if !leading_trivia.is_empty() {
					let leading_trivia = leading_trivia.iter().rev().map(|piece| (piece.kind(), piece.text()));
			...
			UseBlockStatementsOperationType::ReplaceBody => match node {
				AnyJsBlockStatement::JsIfStatement(stmt) => {
					use_block_statements_replace_body!(JsIfStatement, with_consequent, mutation, node, stmt)
				},
				AnyJsBlockStatement::JsElseClause(stmt) => {
					use_block_statements_replace_body!(JsElseClause, with_alternate, mutation, node, stmt)
				},
				AnyJsBlockStatement::JsDoWhileStatement(stmt) => {
...
macro_rules! use_block_statements_replace_body {
	($stmt_type:ident, $builder_method:ident, $mutation:ident, $node:ident, $stmt:ident) => {
		$mutation.replace_node(
			$node.clone(),
			AnyJsBlockStatement::$stmt_type($stmt.clone().$builder_method(AnyJsStatement::JsBlockStatement(
