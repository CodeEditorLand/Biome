...
impl Rule for UseEnumInitializers {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		if enum_declaration.is_ambient() {
			// In ambient declarations, enum members without initializers are
			// opaque types. They generally represent an enum with complex
			// initializers.
			return None;
		}
	...
	fn diagnostic(ctx:&RuleContext<Self>, _:&Self::State) -> Option<RuleDiagnostic> {
		...
		Some(diagnostic.note(
			"Allowing implicit initializations for enum members can cause bugs if enum \
		...
		for enum_member in enum_declaration.members() {
			...
			if let Some(initializer) = enum_member.initializer() {
				...
				if let Some(x) = x {
					...
					// When creating the replacement node we first need to
					// remove the trailing trivia. Otherwise nodes without
					// a trailing comma will add [JsSyntacKind::EQ] and
					// [EnumInitializer] after it.
					...
					// Replace current node and attach trivia from it to the new
					// one.
