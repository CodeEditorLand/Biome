		...
		for enum_member in enum_declaration.members() {
			...
			if let Some(initializer) = enum_member.initializer() {
				...
				if let Some(x) = x {
					...
					// When creating the replacement node we first need to remove the trailing
					// trivia. Otherwise nodes without a trailing comma will add
					// [JsSyntacKind::EQ] and [EnumInitializer] after it.
