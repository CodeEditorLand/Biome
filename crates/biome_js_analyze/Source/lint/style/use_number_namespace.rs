...
use biome_js_syntax::{
	AnyJsExpression, JsUnaryExpression, JsUnaryOperator, T, global_identifier, static_value::StaticValue,
};
...
impl Rule for UseNumberNamespace {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		ctx.model().binding(&reference).is_none().then_some(global_ident)
	}
						...
						if let Some(parent) = node.parent::<JsUnaryExpression>() {
							match parent.operator().ok()? {
								JsUnaryOperator::Minus => {
									(AnyJsExpression::JsUnaryExpression(parent), "NEGATIVE_INFINITY")
								},
								JsUnaryOperator::Plus => {
									(AnyJsExpression::JsUnaryExpression(parent), "POSITIVE_INFINITY")
								},
								_ => return None,
							}
						} else {
