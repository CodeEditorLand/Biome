...
impl Rule for NoRestrictedGlobals {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let unresolved_reference_nodes = model.all_unresolved_references().map(|reference| reference.syntax().clone());
		let global_references_nodes = model.all_global_references().map(|reference| reference.syntax().clone());
		...
		unresolved_reference_nodes
			...
			.filter_map(|node| {
				...
				let (token, binding) = match node {
					AnyJsIdentifierUsage::JsReferenceIdentifier(node) => (node.value_token(), node.binding(model)),
					AnyJsIdentifierUsage::JsxReferenceIdentifier(node) => (node.value_token(), node.binding(model)),
					AnyJsIdentifierUsage::JsIdentifierAssignment(node) => (node.name_token(), node.binding(model)),
				};
				let token = token.ok()?;
				let text = token.text_trimmed();
				let denied_globals: Vec<_> = options.denied_globals.iter().map(AsRef::as_ref).collect();
