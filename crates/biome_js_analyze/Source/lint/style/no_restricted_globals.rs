...
impl Rule for NoRestrictedGlobals {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let unresolved_reference_nodes =
			model.all_unresolved_references().map(|reference| reference.syntax().clone());
		let global_references_nodes =
			model.all_global_references().map(|reference| reference.syntax().clone());
