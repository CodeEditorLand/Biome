...
use biome_js_factory::make::{jsx_child_list, jsx_closing_fragment, jsx_fragment, jsx_opening_fragment};
...
impl Rule for UseFragmentSyntax {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let maybe_invalid = match name {
			AnyJsxElementName::JsxMemberName(member_name) => jsx_member_name_is_react_fragment(&member_name, model)?,
	...
	fn action(ctx: &RuleContext<Self>, _state: &Self::State) -> Option<JsRuleAction> {
		...
		let fragment = jsx_fragment(
			jsx_opening_fragment(opening_element.l_angle_token().ok()?, opening_element.r_angle_token().ok()?),
			list,
		...
		mutation.replace_element(node.clone().into_syntax().into(), fragment.into_syntax().into());

		Some(JsRuleAction::new(
			ctx.metadata().action_category(ctx.category(), ctx.group()),
			ctx.metadata().applicability(),
			(markup! { "Replace "<Emphasis>"<Fragment>"</Emphasis>" with the fragment syntax" }).to_owned(),
			mutation,
		))
	}
