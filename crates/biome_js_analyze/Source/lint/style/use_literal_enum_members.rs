...
impl Rule for UseLiteralEnumMembers {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		let Some(enum_name) =
			enum_name.as_js_identifier_binding().and_then(|x| x.name_token().ok())
		else {
			return result;
		};
	...
	fn diagnostic(_:&RuleContext<Self>, initializer_range:&Self::State) -> Option<RuleDiagnostic> {
...
/// A constant enum expression can contain numbers, string literals, and
/// reference to one of the enum member of `enum_member_names` of the enum name
/// `enum_name`. These values can be combined thanks to numeric, bitwise, and
...
// Return true if `expr` is a reference to one of the enum member
// `enum_member_names` of the enum named `enum_name`.
