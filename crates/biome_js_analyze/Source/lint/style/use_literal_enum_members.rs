...
use biome_js_syntax::{
	AnyJsExpression, AnyJsLiteralExpression, AnyJsMemberExpression, JsUnaryOperator, TsEnumDeclaration,
};
...
impl Rule for UseLiteralEnumMembers {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let Some(enum_name) = enum_name.as_js_identifier_binding().and_then(|x| x.name_token().ok()) else {
			return result.into_boxed_slice();
		};
...
fn is_constant_enum_expression(expr: AnyJsExpression, enum_name: &str, enum_member_names: &FxHashSet<String>) -> bool {
	(move || {
		...
		while let Some(expr) = stack.pop() {
			match expr.omit_parentheses() {
				...
				AnyJsExpression::JsUnaryExpression(expr) => {
					if !matches!(
						expr.operator(),
						Ok(JsUnaryOperator::BitwiseNot | JsUnaryOperator::Minus | JsUnaryOperator::Plus)
					) {
						return Some(false);
					}
