		...
		let inner_content = node.elements().iter().fold(String::new(), |mut acc, cur| {
			match cur {
				AnyJsTemplateElement::JsTemplateChunkElement(ele) => {
					// Safety: if `ele.template_chunk_token()` is `Err` variant,
					// [can_convert_to_string_lit] should return false, thus `run` will return
					// None
				...
				AnyJsTemplateElement::JsTemplateElement(_) => {
					// Because we know if TemplateLit has any `JsTemplateElement` will return `None`
					// in `run` function
					unreachable!()
				},
			}
...
fn can_convert_to_string_literal(node:&JsTemplateExpression) -> bool {
	!node.elements().iter().any(|element| {
		// We want to test if any templateElement has violated rule that can convert to
		// string literal, rules are listed below
			...
			AnyJsTemplateElement::JsTemplateChunkElement(chunk) => {
				match chunk.template_chunk_token() {
					Ok(token) => {
						...
						token.text().bytes().any(|byte| matches!(byte, b'\n' | b'\'' | b'"'))
					},

					Err(_) => {
						// if we found an error, then just return `true`, which means that this
						// template literal can't be converted to a string literal
						true
					},
				}
			},
