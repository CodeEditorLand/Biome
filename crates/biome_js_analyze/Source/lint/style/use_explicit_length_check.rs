...
use biome_js_syntax::{
	AnyJsExpression, AnyJsLiteralExpression, JsBinaryExpression, JsBinaryOperator, JsCallArgumentList, JsCallArguments,
	JsCallExpression, JsLogicalExpression, JsLogicalOperator, JsStaticMemberExpression, JsSyntaxKind, JsSyntaxNode,
...
impl Rule for UseExplicitLengthCheck {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		if let Some((binary_expr, mut len_check, is_possibly_valid)) = is_binary_expr_length_check(&parent_syntax) {
			return get_boolean_ancestor(binary_expr.syntax())
				.map(|(expr, is_negative)| {
					...
					UseExplicitLengthCheckState { check: len_check, node: expr, member_name: member_name.clone() }
				})
				.or_else(|| {
		...
		if let Some(logical_expr) = is_logical_expr(parent_syntax) {
			...
			if logical_expr.operator().ok()? == JsLogicalOperator::LogicalOr
				&& logical_expr.right().ok()?.syntax().kind() == JsSyntaxKind::JS_NUMBER_LITERAL_EXPRESSION
			{
				return None;
			}
	...
	fn action(ctx: &RuleContext<Self>, state: &Self::State) -> Option<JsRuleAction> {
		...
		if does_node_needs_space_before_child(&parent) {
			...
			new_node = new_node.trim_leading_trivia()?.prepend_trivia_pieces(leading_trivia)?;
		}

		mutation.replace_element_discard_trivia(state.node.clone().into_syntax().into(), new_node.into());
...
fn extract_binary_position_and_literal(node: &JsBinaryExpression) -> Option<(MemberPosition, AnyJsLiteralExpression)> {
	match (node.left().ok()?, node.right().ok()?) {
		(AnyJsExpression::JsStaticMemberExpression(_), AnyJsExpression::AnyJsLiteralExpression(literal)) => {
			Some((MemberPosition::Right, literal))
		},
		(AnyJsExpression::AnyJsLiteralExpression(literal), AnyJsExpression::JsStaticMemberExpression(_)) => {
...
fn is_binary_expr_length_check(node: &JsSyntaxNode) -> Option<(JsBinaryExpression, LengthCheck, bool)> {
