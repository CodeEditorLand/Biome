...
impl Rule for UseExplicitLengthCheck {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		if let Some(logical_expr) = is_logical_expr(parent_syntax) {
			...
			// TODO. This handles simple cases. To know if right side is a number, we need
			// type inference.
	...
	fn action(ctx:&RuleContext<Self>, state:&Self::State) -> Option<JsRuleAction> {
		...
		// In cases like `export default!foo.length` -> `export default foo.length ===
		// 0` we need to add a space between keyword and expression
