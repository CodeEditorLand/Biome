...
impl Rule for UseFilenamingConvention {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		let (name, mut extensions) = if let Some(matching) = &options.matching {
			...
			let file_name =
				if matches!(first_char, b'.' | b'+') { &file_name[1..] } else { file_name };
			...
			ctx.model()
				...
				.all(|exported_binding| {
					exported_binding
						.exports()
						.filter_map(|export| {
							match AnyJsIdentifierBinding::try_cast(export) {
								Ok(id) => id.name_token().ok(),
								Err(export) => {
									match JsExportNamedSpecifier::cast(export.parent()?) {
										Some(specifier) => {
											specifier.exported_name().ok()?.value().ok()
										},
										None => {
											AnyJsIdentifierUsage::cast(export)?.value_token().ok()
										},
									}
								},
							}
...
impl From<SmallVec<[FilenameCase; 5]>> for FilenameCases {
	fn from(values:SmallVec<[FilenameCase; 5]>) -> Self { Self::from_iter(values) }
}
...
impl schemars::JsonSchema for FilenameCases {
	fn schema_name() -> String { "FilenameCases".to_string() }
