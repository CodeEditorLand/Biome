...
use biome_js_syntax::{AnyJsIdentifierUsage, JsExportNamedSpecifier, binding_ext::AnyJsIdentifierBinding};
...
impl Rule for UseFilenamingConvention {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let (name, mut extensions) = if let Some(matching) = &options.matching {
			...
			let file_name = if matches!(first_char, b'.' | b'+') { &file_name[1..] } else { file_name };
...
#[derive(Clone, Copy, Debug, Deserializable, Eq, Hash, PartialEq, serde::Deserialize, serde::Serialize)]
#[serde(from = "SmallVec<[FilenameCase; 5]>", into = "SmallVec<[FilenameCase; 5]>")]
...
impl FromIterator<FilenameCase> for FilenameCases {
	fn from_iter<T: IntoIterator<Item = FilenameCase>>(values: T) -> Self {
		let mut result = Self { cases: Cases::empty(), allow_export: false };
...
impl From<FilenameCases> for SmallVec<[FilenameCase; 5]> {
	fn from(value: FilenameCases) -> Self {
		let maybe_export = if value.allow_export { &[FilenameCase::Export][..] } else { &[] };
		value
			.cases
			.into_iter()
...
impl Default for FilenameCases {
	fn default() -> Self {
		Self { cases: Case::Camel | Case::Kebab | Case::Snake, allow_export: true }
	}
}

impl biome_deserialize::DeserializableValidator for FilenameCases {
	fn validate(&mut self, ctx: &mut impl DeserializationContext, name: &str, range: TextRange) -> bool {
...
#[derive(Clone, Copy, Debug, serde::Deserialize, Deserializable, Eq, Hash, PartialEq, serde::Serialize)]
#[cfg_attr(feature = "schemars", derive(schemars::JsonSchema))]
...
impl FilenameCase {
	pub const ALLOWED_VARIANTS: &'static [&'static str] =
		&["camelCase", "export", "kebab-case", "PascalCase", "snake_case"];
}
