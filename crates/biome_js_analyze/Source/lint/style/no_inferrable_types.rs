...
impl Rule for NoInferrableTypes {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		if has_trivially_inferrable_type(&init_expr).is_some() {
			// `is_const` signals a const context (const declarations, readonly
			// properties) non const contexts are other situations (let/var
			// declarations, mutable properties, formal parameters)
			...
			if let Some(type_annotation) = type_annotation {
				...
				// because in unsafe null mode, TypeScript widen an unannotated
				// variable to `any`.
				//
				...
				// because in unsafe null mode, it is possible to assign `null`
				// and `undefined` to any type.
