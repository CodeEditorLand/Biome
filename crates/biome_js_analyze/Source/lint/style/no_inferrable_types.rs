...
use biome_js_syntax::{
	AnyJsExpression, AnyTsPropertyAnnotation, AnyTsVariableAnnotation, JsFormalParameter, JsInitializerClause,
	JsPropertyClassMember, JsSyntaxKind, JsVariableDeclaration, JsVariableDeclarator, JsVariableDeclaratorList,
	TsPropertyParameter, TsReadonlyModifier, TsTypeAnnotation,
};
...
impl Rule for NoInferrableTypes {
	...
	fn action(ctx: &RuleContext<Self>, annotation: &Self::State) -> Option<JsRuleAction> {
		...
		let new_next_token = next_token.prepend_trivia_pieces(last_token.trailing_trivia().pieces());
...
fn has_trivially_inferrable_type(expr: &AnyJsExpression) -> Option<()> {
	match expr {
		...
		AnyJsExpression::JsUnaryExpression(unary_exp) => match unary_exp.operator_token().ok()?.kind() {
			JsSyntaxKind::BANG | JsSyntaxKind::MINUS | JsSyntaxKind::PLUS | JsSyntaxKind::VOID_KW => Some(()),
			_ => None,
		},
		_ => None,
	}
