...
impl Rule for UseSelfClosingElements {
	...
	fn action(ctx: &RuleContext<Self>, _: &Self::State) -> Option<JsRuleAction> {
		...
		let JsxOpeningElementFields { l_angle_token, name, type_arguments, attributes, r_angle_token } =
		...
		if let Some(type_arguments) = type_arguments {
			self_closing_element_builder = self_closing_element_builder.with_type_arguments(type_arguments);
		}
