...
use biome_js_syntax::{
	AnyJsCombinedSpecifier, AnyJsIdentifierUsage, AnyJsImportClause, AnyJsModuleItem, AnyJsNamedImportSpecifier,
	JsFileSource, JsIdentifierBinding, JsImport, JsImportCombinedClause, JsImportDefaultClause, JsLanguage,
	JsModuleItemList, JsNamedImportSpecifierList, JsNamedImportSpecifiers, JsSyntaxNode, JsSyntaxToken, T,
};
use biome_rowan::{
	AstNode, AstSeparatedList, BatchMutation, BatchMutationExt, SyntaxElement, SyntaxResult, TriviaPieceKind,
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		match import_clause {
				...
				match clause.specifier().ok()? {
					AnyJsCombinedSpecifier::JsNamedImportSpecifiers(named_specifiers) => {
						match named_import_type_fix(model, &named_specifiers, false) {
							...
							None => is_default_used_as_type.then_some(ImportTypeFix::ExtractDefaultImportType(vec![])),
						}
					},
					AnyJsCombinedSpecifier::JsNamespaceImportSpecifier(namespace_specifier) => {
						...
						match (is_default_used_as_type, is_only_used_as_type(model, namespace_binding)) {
			...
			AnyJsImportClause::JsImportNamedClause(clause) => {
				match named_import_type_fix(model, &clause.named_specifiers().ok()?, clause.type_token().is_some())? {
			...
			AnyJsImportClause::JsImportNamespaceClause(clause) => {
				...
				is_only_used_as_type(model, namespace_binding).then_some(ImportTypeFix::UseImportType)
			},
		}
	}
			...
			ImportTypeFix::ExtractDefaultImportType(named_specifiers) => {
				if named_specifiers.is_empty() {
					...
					for specifier in named_specifiers {
						diagnostic = diagnostic.detail(specifier.range(), "This import is only used as a type.")
					}
					diagnostic
				}
			},
			ImportTypeFix::ExtractCombinedImportType => {
				let AnyJsImportClause::JsImportCombinedClause(import_combined_clause) = import_clause else {
					unreachable!();
				};
		...
		match state {
			ImportTypeFix::UseImportType => match import_clause {
				...
				AnyJsImportClause::JsImportCombinedClause(import_combined_clause) => {
					...
					let default_clause =
						extract_into_default_import_clause(&import_combined_clause, type_token.clone()).ok()?;
					...
					let extra_import = extract_combined_specifier_in_new_import(
						...
						type_token,
						import.semicolon_token().is_some().then_some(make::token(T![;])),
					)?;
					add_module_items(&mut mutation, import.syntax(), [new_import.into(), extra_import.into()]);
				},
				AnyJsImportClause::JsImportDefaultClause(import_clause) => {
					let specifier = import_clause.default_specifier().ok()?;
					let new_import_clause = import_clause.clone().with_default_specifier(specifier).with_type_token(
						Some(make::token(T![type]).with_trailing_trivia([(TriviaPieceKind::Whitespace, " ")])),
					);
				...
				AnyJsImportClause::JsImportNamedClause(import_clause) => {
					...
					let new_specifiers = make::js_named_import_specifier_list(new_specifiers, new_separators);
				...
				AnyJsImportClause::JsImportNamespaceClause(import_clause) => {
					...
					let new_import_clause = import_clause.clone().with_namespace_specifier(specifier).with_type_token(
						Some(make::token(T![type]).with_trailing_trivia([(TriviaPieceKind::Whitespace, " ")])),
					);
					mutation.replace_node(import_clause, new_import_clause);
				},
			...
			ImportTypeFix::ExtractDefaultImportType(specifiers_requiring_type_marker) => {
				...
				.ok()?;
				let new_import = import.clone().with_import_clause(default_import_clause.into());
				let extra_import = if let Some(AnyJsCombinedSpecifier::JsNamedImportSpecifiers(named_specifiers)) =
					import_combined_clause
						.specifier()
						.ok()
			...
			ImportTypeFix::ExtractCombinedImportType => {
				...
				let default_import_clause = extract_into_default_import_clause(&import_combined_clause, None).ok()?;
				let new_import = import.clone().with_import_clause(default_import_clause.into());
				let extra_import = extract_combined_specifier_in_new_import(
					&import_combined_clause,
					Some(make::token(T![type]).with_trailing_trivia([(TriviaPieceKind::Whitespace, " ")])),
			...
			ImportTypeFix::AddInlineTypeQualifiers(specifiers) => {
				for specifier in specifiers {
					let new_specifier = specifier.clone().with_leading_trivia_pieces([])?.with_type_token(Some(
						make::token(T![type])
...
fn named_import_type_fix(
	...
	if has_type_token {
		...
		if useless_type_tokens.is_empty() {
			...
			Some(NamedImportTypeFix::RemoveInlineTypeQualifiers(useless_type_tokens))
		}
	} else {
		...
		for specifier in specifiers.iter() {
			...
			if specifier.type_token().is_none() {
				if specifier
					.local_name()
					.and_then(|local_name| Some(is_only_used_as_type(model, local_name.as_js_identifier_binding()?)))
					.unwrap_or(false)
				{
		...
		if imports_only_types {
			Some(NamedImportTypeFix::UseImportType(specifiers_requiring_type_marker))
		} else if specifiers_requiring_type_marker.is_empty() {
			None
		} else {
			Some(NamedImportTypeFix::AddInlineTypeQualifiers(specifiers_requiring_type_marker))
		}
	}
}
...
fn add_module_items(
	...
	let new_module_item_list = module_item_list.clone().splice_slots(
		slot..(slot + 1),
		new_items.into_iter().map(|item| Some(SyntaxElement::Node(item.into_syntax()))),
	);
...
fn extract_into_default_import_clause(
	...
	let result =
		make::js_import_default_clause(import_clause.default_specifier()?, from_token, import_clause.source()?)
			.build()
...
fn extract_combined_specifier_in_new_import(
	...
	let comma_leading_trivia = trim_trailing_trivia_pieces(import_clause.comma_token().ok()?.leading_trivia().pieces());
	let from_token = make::token(T![from]).with_trailing_trivia([(TriviaPieceKind::Whitespace, " ")]);
