					...
					AnyJsCombinedSpecifier::JsNamedImportSpecifiers(named_specifiers) => {
						match named_import_type_fix(model, &named_specifiers, false) {
							...
							Some(NamedImportTypeFix::RemoveInlineTypeQualifiers(_)) => {
								// Should not be reached because we pass `false` to
								// `named_import_type_fix`.
								None
							},
	...
	fn diagnostic(ctx:&RuleContext<Self>, state:&Self::State) -> Option<RuleDiagnostic> {
		...
		let diagnostic = match state {
			...
			ImportTypeFix::ExtractCombinedImportType => {
				...
				match specifier {
					AnyJsCombinedSpecifier::JsNamedImportSpecifiers(_) => {
						RuleDiagnostic::new(
							rule_category!(),
							specifier.range(),
		...
		for specifier in specifiers.iter() {
			...
			if specifier.type_token().is_none() {
				if specifier
					.local_name()
					.and_then(|local_name| {
						Some(is_only_used_as_type(model, local_name.as_js_identifier_binding()?))
					})
					.unwrap_or(false)
				{
