...
impl AnyFormalParameter {
	...
	pub(crate) fn initializer(&self) -> Option<JsInitializerClause> {
		match self {
			...
			AnyFormalParameter::TsPropertyParameter(x) => {
				x.formal_parameter().ok()?.as_js_formal_parameter()?.initializer()
			},
		}
	}
	...
	pub(crate) fn question_mark_token(&self) -> Option<JsSyntaxToken> {
		match self {
			...
			AnyFormalParameter::TsPropertyParameter(x) => {
				x.formal_parameter().ok()?.as_js_formal_parameter()?.question_mark_token()
			},
		}
	}
}
...
impl Rule for UseDefaultParameterLast {
	...
	fn diagnostic(ctx: &RuleContext<Self>, last_required_param: &Self::State) -> Option<RuleDiagnostic> {
		let formal_param = ctx.query();
	...
	fn action(ctx: &RuleContext<Self>, _: &Self::State) -> Option<JsRuleAction> {
		...
		if opt_param.question_mark_token().is_some() {
			...
			let new_token = prev_token.append_trivia_pieces(question_mark.trailing_trivia().pieces());
		...
		Some(JsRuleAction::new(
			...
			markup! {"Turn the parameter into a "<Emphasis>"required parameter"</Emphasis>"."}.to_owned(),
			mutation,
		))
	}
}
