...
use biome_rowan::{
	AstNode, AstNodeList, BatchMutationExt, SyntaxNodeOptionExt, chain_trivia_pieces, trim_leading_trivia_pieces,
};
...
impl Rule for NoUselessElse {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		while let (Ok(if_consequent), Some(else_clause)) = (if_stmt.consequent(), if_stmt.else_clause()) {
	...
	fn action(ctx: &RuleContext<Self>, if_stmt: &Self::State) -> Option<JsRuleAction> {
		...
		if let Some(stmts_list) = if_stmt.parent::<JsStatementList>() {
			...
			let if_pos = stmts_list.iter().position(|x| x.syntax() == if_stmt.syntax())?;
			...
			let new_stmts: Vec<_> = if let AnyJsStatement::JsBlockStatement(else_block_stmts) = else_alternative {
				prev_stmts
					.chain(else_block_stmts.statements().iter())
					.chain(next_stmts)
					.collect()
			} else {
				prev_stmts.chain([else_alternative]).chain(next_stmts).collect()
			};
