...
use biome_js_syntax::{
	AnyJsExportNamedSpecifier, JsExportNamedClause, JsExportNamedFromClause, JsFileSource, JsSyntaxToken, T,
};
use biome_rowan::{
	AstNode, AstSeparatedList, BatchMutationExt, TriviaPieceKind, chain_trivia_pieces, declare_node_union,
		...
		match export_named_clause {
			AnyJsExportNamedClause::JsExportNamedClause(clause) => {
				...
				if specifiers.is_empty() {
					...
					// `export type { ... }`
					let useless_type_tokens: Vec<_> =
						specifiers.iter().filter_map(|specifier| specifier.ok()?.type_token()).collect();
					if useless_type_tokens.is_empty() {
						None
					} else {
						Some(ExportTypeFix::RemoveInlineTypeQualifiers(useless_type_tokens))
					}
				} else {
					...
					for specifier in specifiers {
						...
						let is_type_only = model
							.global_scope()
							.bindings()
							.filter(|binding| binding.syntax().text_trimmed() == ref_name.syntax().text_trimmed())
					...
					if exports_only_types {
						...
						Some(ExportTypeFix::AddInlineTypeQualifiers(specifiers_requiring_type_marker))
					}
				}
			},
			AnyJsExportNamedClause::JsExportNamedFromClause(clause) => {
				...
				} else if specifiers.iter().all(|x| x.is_ok_and(|x| x.type_token().is_some())) {
					Some(ExportTypeFix::UseExportType)
				} else {
					None
				}
	...
	fn diagnostic(ctx: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
		...
		let diagnostic = match state {
			ExportTypeFix::UseExportType => {
				RuleDiagnostic::new(rule_category!(), named_export_clause.range(), "All exports are only types.")
			},
			ExportTypeFix::AddInlineTypeQualifiers(specifiers) => {
				let mut diagnostic =
					RuleDiagnostic::new(rule_category!(), named_export_clause.range(), "Some exports are only types.");
				for specifier in specifiers {
		...
		let diagnostic = match state {
				...
				match export_named_clause {
					AnyJsExportNamedClause::JsExportNamedClause(clause) => {
						...
						let new_specifier_list = make::js_export_named_specifier_list(
							new_specifiers,
							specifier_list.separators().filter_map(|sep| sep.ok()).collect::<Vec<_>>(),
						);
			...
			ExportTypeFix::AddInlineTypeQualifiers(specifiers) => {
				for specifier in specifiers {
					mutation.replace_node(
						specifier.clone(),
						specifier.clone().with_leading_trivia_pieces([])?.with_type_token(Some(
							make::token(T![type])
								.with_leading_trivia_pieces(specifier.syntax().first_leading_trivia()?.pieces())
			...
			ExportTypeFix::RemoveInlineTypeQualifiers(type_tokens) => {
				...
				JsRuleAction::new(
					ctx.metadata().action_category(ctx.category(), ctx.group()),
					ctx.metadata().applicability(),
					markup! { "Remove useless inline "<Emphasis>"type"</Emphasis>" keywords." }.to_owned(),
					mutation,
				)
			},
		};
