...
use biome_js_syntax::{AnyJsExpression, T, TsNonNullAssertionAssignment, TsNonNullAssertionExpression};
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		match ctx.query() {
			AnyTsNonNullAssertion::TsNonNullAssertionExpression(node) => {
				node.parent::<TsNonNullAssertionExpression>().map_or(Some(()), |_| None)
			},
			AnyTsNonNullAssertion::TsNonNullAssertionAssignment(node) => {
				node.parent::<TsNonNullAssertionAssignment>().map_or(Some(()), |_| None)
			},
		}
	}
