...
use biome_js_syntax::{
	AnyJsExpression, AnyJsLiteralExpression, AnyJsObjectMemberName, JsIdentifierBinding, JsIdentifierExpression,
	JsReferenceIdentifier, JsShorthandPropertyObjectMember, JsStringLiteralExpression, JsSyntaxKind,
...
fn is_id_and_string_literal_inner_text_equal(
	...
	let literal = expression.as_any_js_literal_expression()?.as_js_string_literal_expression()?;
...
impl Rule for NoShoutyConstants {
	...
	fn run(ctx: &RuleContext<Self>) -> Option<Self::State> {
		...
		if declaration.is_const() {
			if let Some((binding, literal)) = is_id_and_string_literal_inner_text_equal(declarator) {
				...
				return Some(State { literal, reference: binding.all_references(model).next()? });
			}
		}
	...
	fn diagnostic(ctx: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
		...
		let diag = diag.note(markup! {"You should avoid declaring constants with a string that's the same
	...
	fn action(ctx: &RuleContext<Self>, state: &Self::State) -> Option<JsRuleAction> {
		...
		if let Some(node) = state.reference.syntax().parent()?.cast::<JsIdentifierExpression>() {
			batch.replace_node(
				AnyJsExpression::JsIdentifierExpression(node),
				AnyJsExpression::AnyJsLiteralExpression(literal),
			);
		} else if let Some(node) = state.reference.syntax().parent()?.cast::<JsShorthandPropertyObjectMember>() {
			// for replacing JsShorthandPropertyObjectMember
			let new_element = js_property_object_member(
				AnyJsObjectMemberName::JsLiteralMemberName(js_literal_member_name(SyntaxToken::new_detached(
