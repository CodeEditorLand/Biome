use crate::{
	JsRuleAction, lint::correctness::no_invalid_builtin_instantiation::convert_new_expression_to_call_expression,
...
use biome_js_syntax::{AnyJsExpression, JsNewOrCallExpression, global_identifier, static_value::StaticValue};
...
impl Rule for UseConsistentBuiltinInstantiation {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		if creation_rule.forbidden_builtins_list().binary_search(&name.text()).is_ok() {
			return ctx
				.model()
				.binding(&reference)
				.is_none()
				.then_some(UseConsistentBuiltinInstantiationState { name, creation_rule });
		}
...
fn extract_callee_and_rule(node: &JsNewOrCallExpression) -> Option<(AnyJsExpression, BuiltinCreationRule)> {
	let rule = match node {
