...
use biome_js_syntax::{
	AnyJsExpression, AnyJsLiteralExpression, AnyJsMemberExpression, JsCallExpression, JsSyntaxToken, global_identifier,
};
...
impl CallInfo {
	fn try_from_expr(expr: &JsCallExpression, model: &SemanticModel) -> Option<CallInfo> {
		...
		let [Some(text), Some(radix)] = expr.arguments().ok()?.get_arguments_by_index([0, 1]) else {
			return None;
		};
		let text = text
		...
		Some(CallInfo { callee, text, radix: Radix::from_f64(radix)? })
	}
...
impl Radix {
	...
	const fn article(self) -> &'static str {
		if matches!(self, Self::Octal) { "an" } else { "a" }
	}
}
