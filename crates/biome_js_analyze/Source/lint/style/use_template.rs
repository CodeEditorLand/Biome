...
use biome_js_syntax::{
	AnyJsExpression, AnyJsLiteralExpression, JsBinaryExpression, JsBinaryOperator, JsParenthesizedExpression,
	JsStringLiteralExpression, JsSyntaxKind, JsSyntaxToken, JsTemplateElementList, JsTemplateExpression, T,
};
...
fn can_be_template_literal(node: &JsBinaryExpression) -> Option<bool> {
	...
	while let Some(walk) = iter.next() {
		...
		match &expression {
			...
			AnyJsExpression::JsBinaryExpression(binary) if binary.operator() == Ok(JsBinaryOperator::Plus) => {
...
fn template_expression_from_binary_expression(node: &JsBinaryExpression) -> Option<JsTemplateExpression> {
	...
	while let Some(walk) = iter.next() {
		match walk {
			WalkEvent::Enter(node) => match AnyJsExpression::cast(node)? {
				...
				AnyJsExpression::JsBinaryExpression(ref binary) if binary.operator() == Ok(JsBinaryOperator::Plus) => {
				...
				match &expression {
					...
					AnyJsExpression::JsBinaryExpression(binary) if binary.operator() == Ok(JsBinaryOperator::Plus) => {
						...
						if binary_evaluates_to_string_stack.pop()? {
							if let Some(parent_evaluates_to_string) = binary_evaluates_to_string_stack.last_mut() {
					...
					AnyJsExpression::JsTemplateExpression(template) if template.tag().is_none() => {
						*binary_evaluates_to_string_stack.last_mut()? = true;
						...
						for left_expression in left_expressions_stack.iter_mut().flat_map(|v| v.drain(..)) {
					...
					_expression => {
						if !template_elements.is_empty() && *binary_evaluates_to_string_stack.last()? {
							template_elements.push(template_element_from(expression)?)
						} else {
