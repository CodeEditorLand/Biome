...
impl Rule for UseTemplate {
	...
	fn run(ctx:&RuleContext<Self>) -> Option<Self::State> {
		let node = ctx.query();
		// Do not handle binary operations contained in a binary operation with
		// operator `+`
		if node
            .syntax()
            .ancestors()
...
///   or one of them and an interpolated template literal
///
...
fn template_expression_from_binary_expression(
	...
	// For each nested binary expression (with operator `+`), we keep track of
	// the last left expressions and whether it evaluates to a string.
	// Once we see a string/template literal, we insert all of
	// `left_expressions_stack` in `template_elements` and the seen
	// string/template literal. Any subsequent expression is directly inserted
	// in `template_elements` if its parent evaluates to a string.
	...
	while let Some(walk) = iter.next() {
		match walk {
			...
			WalkEvent::Leave(node) => {
				let expression = AnyJsExpression::cast(node)?;
				match &expression {
					// Skip parenthesized expressions, because they would be
					// added twice to `left_expressions_stack` or
					// `template_elements` (see the last match arm):
					// First the contained expression and then the parenthesized
					// expression itself.
					AnyJsExpression::JsParenthesizedExpression(_) => continue,
					AnyJsExpression::JsBinaryExpression(binary)
						if binary.operator() == Ok(JsBinaryOperator::Plus) =>
...
fn template_element_from(expr:AnyJsExpression) -> Option<AnyJsTemplateElement> {
	Some(AnyJsTemplateElement::from(make::js_template_element(
		...
		expr.with_leading_trivia_pieces([])?.with_trailing_trivia_pieces([])?,
		make::token(T!['}']),
	)))
}

/// Flatten a [JsTemplateElementList] of [JsTemplate] which could possibly be
/// recursive, into a `Vec<JsAnyTemplateElement>` ## Example
/// flatten
/// ```js
/// `${1 + 2 + `${a}test` }bar`
/// ```
