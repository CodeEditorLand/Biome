...
fn template_expression_from_binary_expression(
	...
	// For each nested binary expression (with operator `+`), we keep track of the
	// last left expressions and whether it evaluates to a string.
	// Once we see a string/template literal, we insert all of
	// `left_expressions_stack` in `template_elements` and the seen string/template
	// literal. Any subsequent expression is directly inserted in
	// `template_elements` if its parent evaluates to a string.
