...
impl Rule for UseConst {
	...
	fn action(ctx: &RuleContext<Self>, state: &Self::State) -> Option<JsRuleAction> {
		...
		if state.can_fix {
			...
			batch.replace_token(declaration.kind_token().ok()?, make::token(JsSyntaxKind::CONST_KW));
...
impl ConstBindings {
	pub fn new(declaration: &AnyJsVariableDeclaration, model: &SemanticModel) -> Option<Self> {
		let mut state = Self { can_be_const: Vec::new(), can_fix: true };
		let in_for_in_or_of_loop = matches!(declaration, AnyJsVariableDeclaration::JsForVariableDeclaration(..));
		let mut bindings = 0;
...
fn check_binding_can_be_const(
	...
	host.can_become_variable_declaration()?.then_some(ConstCheckResult::Report)
}

fn for_each_declarator_of(decl: &AnyJsVariableDeclaration, f: impl FnMut(JsVariableDeclarator)) {
	match decl {
		AnyJsVariableDeclaration::JsVariableDeclaration(x) => {
			x.declarators().into_iter().filter_map(Result::ok).for_each(f)
		},
		AnyJsVariableDeclaration::JsForVariableDeclaration(x) => x.declarator().into_iter().for_each(f),
	}
}

fn for_each_binding_of(decl: &AnyJsVariableDeclaration, mut f: impl FnMut(JsIdentifierBinding, &JsVariableDeclarator)) {
...
fn with_object_binding_pat_identifiers(
	...
	pat.properties().into_iter().filter_map(Result::ok).any(|it| {
		use AnyJsObjectBindingPatternMember as P;
		match it {
			P::JsObjectBindingPatternProperty(p) => p.pattern().is_ok_and(|it| with_binding_pat_identifiers(it, f)),
			P::JsObjectBindingPatternRest(p) => p.binding().is_ok_and(|it| with_binding_identifier(it, f)),
			P::JsObjectBindingPatternShorthandProperty(p) => {
				p.identifier().is_ok_and(|it| with_binding_identifier(it, f))
			},
...
fn with_array_binding_pat_identifiers(
	...
	pat.elements().into_iter().filter_map(Result::ok).any(|it| {
		...
		match it {
			P::JsArrayBindingPatternRestElement(p) => p.pattern().is_ok_and(|it| with_binding_pat_identifiers(it, f)),
			P::JsArrayHole(_) => false,
			P::JsArrayBindingPatternElement(p) => p.pattern().is_ok_and(|it| with_binding_pat_identifiers(it, f)),
		}
	})
}

fn with_binding_identifier(binding: AnyJsBinding, f: &mut impl FnMut(JsIdentifierBinding) -> bool) -> bool {
	match binding {
...
impl DestructuringHost {
	...
	fn has_outer_variables(&self, scope: &Scope) -> bool {
		match self {
			Self::JsVariableDeclarator(it) => it.id().is_ok_and(|pat| has_outer_variables_in_binding_pat(pat, scope)),
			Self::JsAssignmentExpression(it) => {
				it.left().is_ok_and(|pat| has_outer_variables_in_assign_pat(&pat, scope))
			},
		}
	}
}
...
fn is_outer_variable_in_binding(binding: &JsIdentifierBinding, scope: &Scope) -> bool {
	binding.name_token().is_ok_and(|name| is_binding_in_outer_scopes(scope, &name))
}
...
fn has_member_expr_in_object_assign_pat(pat: &JsObjectAssignmentPattern) -> bool {
	pat.properties().into_iter().filter_map(Result::ok).any(|it| {
		...
		match it {
			P::JsObjectAssignmentPatternProperty(p) => p.pattern().is_ok_and(has_member_expr_in_assign_pat),
			P::JsObjectAssignmentPatternRest(p) => p.target().is_ok_and(is_member_expr_assignment),
...
fn is_member_expr_assignment(mut assignment: AnyJsAssignment) -> bool {
	...
	matches!(assignment, JsComputedMemberAssignment(_) | JsStaticMemberAssignment(_))
}
	...
	pat.properties().into_iter().filter_map(Result::ok).any(|it| {
		...
		match it {
			P::JsObjectAssignmentPatternProperty(p) => {
				p.pattern().is_ok_and(|it| has_outer_variables_in_assign_pat(&it, scope))
			},
			P::JsObjectAssignmentPatternRest(p) => {
				p.target().is_ok_and(|it| is_outer_variable_in_assignment(&it, scope))
			},
			P::JsObjectAssignmentPatternShorthandProperty(p) => {
				p.identifier().is_ok_and(|it| is_outer_ident_in_assignment(&it, scope))
			},
...
fn has_outer_variables_in_object_assign_pat(pat: &JsArrayAssignmentPattern, scope: &Scope) -> bool {
	pat.elements()
		.into_iter()
		.filter_map(Result::ok)
		.any(|it| it.pattern().is_some_and(|p| has_outer_variables_in_assign_pat(&p, scope)))
}
