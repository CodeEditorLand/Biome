...
impl Rule for UseConst {
	...
	fn action(ctx: &RuleContext<Self>, state: &Self::State) -> Option<JsRuleAction> {
		...
		if state.can_fix {
			...
			batch
				.replace_token(declaration.kind_token().ok()?, make::token(JsSyntaxKind::CONST_KW));
...
impl ConstBindings {
	pub fn new(declaration: &AnyJsVariableDeclaration, model: &SemanticModel) -> Option<Self> {
		...
		let in_for_in_or_of_loop =
			matches!(declaration, AnyJsVariableDeclaration::JsForVariableDeclaration(..));
		let mut bindings = 0;
...
fn with_object_binding_pat_identifiers(
	...
	pat.properties().into_iter().filter_map(Result::ok).any(|it| {
		...
		match it {
			P::JsObjectBindingPatternProperty(p) => {
				p.pattern().map_or(false, |it| with_binding_pat_identifiers(it, f))
			}
			P::JsObjectBindingPatternRest(p) => {
				p.binding().map_or(false, |it| with_binding_identifier(it, f))
			}
			P::JsObjectBindingPatternShorthandProperty(p) => {
				p.identifier().map_or(false, |it| with_binding_identifier(it, f))
			}
...
fn with_array_binding_pat_identifiers(
	...
	pat.elements().into_iter().filter_map(Result::ok).any(|it| {
		...
		match it {
			P::JsArrayBindingPatternRestElement(p) => {
				p.pattern().map_or(false, |it| with_binding_pat_identifiers(it, f))
			}
			P::JsArrayHole(_) => false,
			P::JsArrayBindingPatternElement(p) => {
				p.pattern().map_or(false, |it| with_binding_pat_identifiers(it, f))
			}
		}
	})
...
impl DestructuringHost {
	...
	fn has_outer_variables(&self, scope: &Scope) -> bool {
		match self {
			Self::JsVariableDeclarator(it) => {
				it.id().map_or(false, |pat| has_outer_variables_in_binding_pat(pat, scope))
			}
			Self::JsAssignmentExpression(it) => {
				it.left().map_or(false, |pat| has_outer_variables_in_assign_pat(&pat, scope))
			}
		}
	}
}
...
fn is_outer_variable_in_binding(binding: &JsIdentifierBinding, scope: &Scope) -> bool {
	binding.name_token().map_or(false, |name| is_binding_in_outer_scopes(scope, &name))
}
...
fn has_outer_variables_in_array_assign_pat(pat: &JsObjectAssignmentPattern, scope: &Scope) -> bool {
	pat.properties().into_iter().filter_map(Result::ok).any(|it| {
		...
		match it {
			P::JsObjectAssignmentPatternProperty(p) => {
				p.pattern().map_or(false, |it| has_outer_variables_in_assign_pat(&it, scope))
			}
			P::JsObjectAssignmentPatternRest(p) => {
				p.target().map_or(false, |it| is_outer_variable_in_assignment(&it, scope))
			}
			P::JsObjectAssignmentPatternShorthandProperty(p) => {
				p.identifier().map_or(false, |it| is_outer_ident_in_assignment(&it, scope))
			}
...
fn has_outer_variables_in_object_assign_pat(pat: &JsArrayAssignmentPattern, scope: &Scope) -> bool {
	pat.elements()
		.into_iter()
		.filter_map(Result::ok)
		.any(|it| it.pattern().map_or(false, |p| has_outer_variables_in_assign_pat(&p, scope)))
}
...
fn is_outer_ident_in_assignment(assignment: &JsIdentifierAssignment, scope: &Scope) -> bool {
	assignment.name_token().map_or(false, |name| is_binding_in_outer_scopes(scope, &name))
}
