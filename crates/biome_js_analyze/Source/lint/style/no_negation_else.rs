...
use biome_js_syntax::{AnyJsExpression, AnyJsStatement, JsConditionalExpression, JsIfStatement, JsUnaryOperator, T};
...
impl Rule for NoNegationElse {
	...
	fn run(ctx: &RuleContext<Self>) -> Option<Self::State> {
		let node = ctx.query();
		match node {
			AnyJsCondition::JsConditionalExpression(expr) => is_negation(&expr.test().ok()?).then_some(()),
			AnyJsCondition::JsIfStatement(stmt) => {
				(!matches!(stmt.else_clause()?.alternate().ok()?, AnyJsStatement::JsIfStatement(_))
