...
use biome_js_syntax::{
	AnyJsExpression, AnyJsLiteralExpression, AnyTsName, AnyTsType, JsInitializerClause, JsPropertyClassMember,
	JsSyntaxKind, JsVariableDeclarator, TsAsExpression, TsTypeAssertionExpression,
};
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		match query {
			...
			Query::JsPropertyClassMember(member) => {
				...
				let literal = literal.as_any_js_literal_expression()?;
				let property_annotation = member.property_annotation()?.as_ts_type_annotation()?.ty().ok()?;
...
fn check_literal_match(literal: &AnyJsLiteralExpression, specified_literal: &AnyTsType) -> Option<TextRange> {
	match (literal, specified_literal) {
		...
		(
			...
			if literal.value_token().ok()?.text_trimmed() == specified_literal.literal_token().ok()?.text_trimmed() {
				return Some(specified_literal.range());
			}
		},
		_ => return None,
