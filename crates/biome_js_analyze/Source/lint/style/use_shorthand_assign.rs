...
use biome_js_syntax::{
	AnyJsAssignment, AnyJsExpression, JsAssignmentExpression, JsAssignmentOperator, JsBinaryExpression,
...
impl Rule for UseShorthandAssign {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let variable_position_in_expression = find_variable_position(&binary_expression, &left_var_name)?;
		...
		if !operator.is_commutative() && matches!(variable_position_in_expression, VariablePosition::Right) {
			return None;
		}

		Some(RuleState { shorthand_operator, replacement_expression })
	}
	...
	fn action(ctx: &RuleContext<Self>, state: &Self::State) -> Option<JsRuleAction> {
		...
		Some(JsRuleAction::new(
			...
			markup! { "Use "<Emphasis>""{shorthand_operator.to_string()?}""</Emphasis>" instead." }.to_owned(),
			mutation,
		))
	}
}
