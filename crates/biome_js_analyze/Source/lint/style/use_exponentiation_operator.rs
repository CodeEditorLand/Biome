...
/// Determines whether the given parent node needs parens if used as the
/// exponent in an exponentiation binary expression.
...
/// Determines whether the given expression needs parens when used in an
/// exponentiation binary expression.
fn does_expression_need_parens(
	...
	let needs_parentheses = match &expression {
		...
		AnyJsExpression::JsNewExpression(new_expr) => {
			new_expr
				.arguments()?
				.args()
				.iter()
...
fn does_base_need_parens(base:&AnyJsExpression) -> SyntaxResult<bool> {
	// '**' is right-associative, parens are needed when Math.pow(a ** b, c) is
	// converted to (a ** b) ** c
