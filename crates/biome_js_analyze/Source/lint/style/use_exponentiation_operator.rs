...
use biome_js_syntax::{
	AnyJsCallArgument, AnyJsExpression, AnyJsMemberExpression, JsBinaryOperator, JsCallExpression, JsClassDeclaration,
	JsClassExpression, JsExtendsClause, JsInExpression, OperatorPrecedence, global_identifier,
};
use biome_rowan::{
	AstNode, AstSeparatedList, BatchMutationExt, SyntaxResult, chain_trivia_pieces, trim_leading_trivia_pieces,
};
...
impl Rule for UseExponentiationOperator {
	...
	fn action(ctx: &RuleContext<Self>, _: &Self::State) -> Option<JsRuleAction> {
		...
		let exponent = exponent
			.prepend_trivia_pieces(trim_leading_trivia_pieces(separator.trailing_trivia().pieces()))?
		...
		let new_node = if let Some((needs_parens, parent)) = does_exponentiation_expression_need_parens(node) {
...
fn does_exponentiation_expression_need_parens(node: &JsCallExpression) -> Option<(bool, Option<AnyJsExpression>)> {
...
fn does_expression_need_parens(node: &JsCallExpression, expression: &AnyJsExpression) -> Option<bool> {
	let needs_parentheses = match &expression {
		...
		AnyJsExpression::JsBinaryExpression(bin_expr) => {
			...
			bin_expr.operator().ok()? != JsBinaryOperator::Exponent || call_expr.is_none() || call_expr? != node
		},
		AnyJsExpression::JsCallExpression(call_expr) => call_expr
			...
			.args()
			.iter()
			.find_map(|arg| Some(arg.ok()?.as_any_js_expression()?.as_js_call_expression()? == node))
			.is_none(),
