...
use biome_js_syntax::{
	AnyJsExpression, AnyJsForInitializer, AnyJsObjectMember, AnyJsStatement, JsAssignmentExpression,
	JsAssignmentOperator, JsBinaryExpression, JsBinaryOperator, JsForStatement, JsIdentifierBinding,
	JsIdentifierExpression, JsPostUpdateExpression, JsPostUpdateOperator, JsPreUpdateExpression, JsPreUpdateOperator,
	JsShorthandPropertyObjectMember, JsSyntaxKind, JsSyntaxToken, JsUnaryOperator, JsVariableDeclarator,
};
...
impl Rule for UseForOf {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let test = node.test()?;
		let binding = initializer_id.as_any_js_binding()?.as_js_identifier_binding()?;
		...
		let array_used_in_for = array_right.as_js_static_member_expression()?.object().ok()?;
		let index_only_used_with_array = |reference| {
			let array_in_use =
				reference_being_used_by_array(reference, &array_used_in_for).is_some_and(|array_in_use| array_in_use);
			let is_delete = is_delete(reference).is_some_and(|is_delete| is_delete);

			array_in_use && !is_delete
		};
		if references.iter().all(index_only_used_with_array) { Some(()) } else { None }
	}
...
fn list_initializer_references(
	...
	binding
		...
		.filter_map(|reference| {
			...
			AnyBindingExpression::try_from(AnyJsExpression::cast(reference.syntax().parent()?)?).ok()
		})
		.collect()
}
...
///
fn is_for_test_valid(initializer_binding: &JsIdentifierBinding, test: &AnyJsExpression) -> Option<bool> {
	...
	if initializer_binding.name_token().ok()?.text_trimmed()
		!= identifier_expression.name().ok()?.value_token().ok()?.text_trimmed()
	{
		return None;
	}
...
///
fn is_for_update_valid(initializer_binding: &JsIdentifierBinding, update: AnyJsExpression) -> Option<bool> {
	let incrementable_like = AnyIncrementableLike::try_from(update).ok()?;

	if initializer_binding.name_token().ok()?.text_trimmed() != incrementable_like.get_name_token()?.text_trimmed() {
		return None;
	}
...
fn is_delete(reference: &AnyBindingExpression) -> Option<bool> {
	...
	match js_expression {
		AnyJsExpression::JsUnaryExpression(unary_expression) => {
			Some(matches!(unary_expression.operator().ok()?, JsUnaryOperator::Delete))
		},
		_ => Some(false),
	}
}
...
fn is_zero_initialized(variable_declarator: &JsVariableDeclarator) -> Option<bool> {
	...
	let number_literal = expression.as_any_js_literal_expression()?.as_js_number_literal_expression()?;
...
impl AnyIncrementableLike {
	...
	fn is_increment_by_one(&self) -> Option<bool> {
		match self {
			AnyIncrementableLike::JsPostUpdateExpression(expression) => {
				Some(matches!(expression.operator().ok()?, JsPostUpdateOperator::Increment))
			},
			AnyIncrementableLike::JsPreUpdateExpression(expression) => {
				Some(matches!(expression.operator().ok()?, JsPreUpdateOperator::Increment))
			},
			AnyIncrementableLike::JsAssignmentExpression(expression) => {
				...
				if let Some(binary_expression) = JsBinaryExpression::cast_ref(right.syntax()) {
					...
					let number_literal =
						binary_right.as_any_js_literal_expression()?.as_js_number_literal_expression()?;

					let binary_value = number_literal.value_token().ok()?;
					let binary_value = binary_value.text_trimmed();

					if matches!(binary_expression.operator().ok()?, JsBinaryOperator::Plus) && binary_value == "1" {
						return Some(true);
					}
				}

				let number_literal = right.as_any_js_literal_expression()?.as_js_number_literal_expression()?;
	...
	fn get_name_token(&self) -> Option<JsSyntaxToken> {
		match self {
			AnyIncrementableLike::JsPostUpdateExpression(expression) => {
				expression.operand().ok()?.as_js_identifier_assignment()?.name_token().ok()
			},
			AnyIncrementableLike::JsPreUpdateExpression(expression) => {
				expression.operand().ok()?.as_js_identifier_assignment()?.name_token().ok()
			},
