...
///
/// If the for loop has a `block` this functions only returns references
...
///
/// The initializer must be declared with 0 and can't have multiple
/// declarations.
///
/// # Returns
///
/// - `Some(true)` if the initializer is valid.
/// - `None` if the initializer is invalid (multiple declarations or not
///   initialized with 0).
...
///
/// The test expression must be declared using less than length of array (eg: i
/// < array.length) and reference to same variable declared in for initializer
///
/// # Returns
///
/// - `Some(true)` if the test expression is valid.
/// - `None` if the test expression is invalid (not using less than length or
///   not the same variable name as the initializer).
...
///
/// The update/final must increment the variable by 1 and reference to same
/// variable declared in for initializer
///
/// # Returns
///
/// - `Some(true)` if the  update/final expression is valid.
/// - `None` if the  update/final expression is invalid (not a increment or not
///   the same variable name as the initializer).
...
fn reference_being_used_by_array(
	...
	match reference.parent::<AnyJsExpression>()? {
		AnyJsExpression::JsComputedMemberExpression(computed_member) => {
			Some(is_node_equal(
...
/// Check whether array with index access is part of a delete operator (e.g:
/// delete array[i])
///
/// # Returns
///
/// - `Some(true)` if reference's grandparent is a unary expression with
///   operator DELETE.
/// - `Some(false)` if the grandparent is not a unary expression or unary
///   operator is not a Delete.
...
fn is_zero_initialized(variable_declarator:&JsVariableDeclarator) -> Option<bool> {
	...
	let number_literal =
		expression.as_any_js_literal_expression()?.as_js_number_literal_expression()?;
	...
	fn is_increment_by_one(&self) -> Option<bool> {
		match self {
			...
			AnyIncrementableLike::JsAssignmentExpression(expression) => {
				...
				let number_literal =
					right.as_any_js_literal_expression()?.as_js_number_literal_expression()?;
