...
impl Rule for UseCollapsedElseIf {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		if let AnyJsStatement::JsIfStatement(if_statement) = statements.first()? {
			Some(RuleState { block_statement, if_statement })
		} else {
			None
		}
	}
	...
	fn action(ctx: &RuleContext<Self>, state: &Self::State) -> Option<JsRuleAction> {
		let RuleState { block_statement, if_statement } = state;

		let has_comments = block_statement.l_curly_token().ok()?.has_trailing_comments()
		...
		mutation.replace_node(AnyJsStatement::from(block_statement.clone()), if_statement.clone().into());
