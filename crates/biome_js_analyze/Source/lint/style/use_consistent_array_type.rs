...
fn transform_array_type(ty:AnyTsType, array_kind:TsArrayKind) -> Option<AnyTsType> {
	if let AnyTsType::TsArrayType(_) = ty {
		let array_types =
			transform_array_element_type(ty, array_kind).into_iter().collect::<Vec<_>>();
...
fn get_array_type(array_types:Vec<AnyTsType>) -> Option<AnyTsType> {
	match array_types.len() {
		0 => None,
		1 => {
			// SAFETY: We know that `length` of `array_types` is 1, so unwrap
			// the first element should be safe.
	...
	element_type.map(|element_type| {
		match array_kind {
			...
			TsArrayKind::ReadonlyGenericArray => {
				...
				// Modify `ReadonlyArray<ReadonlyArray<T>>` to `readonly
				// (readonly T[])[]`
				...
				let element_type = if let AnyTsType::TsParenthesizedType(paren_type) = element_type
				{
					match paren_type.ty() {
						Ok(AnyTsType::TsTypeOperatorType(opt_ty)) => {
							let ele_type =
								if let Ok(AnyTsType::TsArrayType(array_type)) = opt_ty.ty() {
									array_type.element_type().ok()
								} else {
									None
								};
