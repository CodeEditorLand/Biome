...
use biome_js_syntax::{
	AnyTsName, AnyTsType, JsSyntaxKind, JsSyntaxToken, T, TriviaPieceKind, TsReferenceType, TsTypeArguments,
};
		...
		match query {
			...
			AnyTsType::TsReferenceType(ty) => {
				...
				if let Some(kind) = get_array_kind_by_reference(ty) {
					return Some(RuleDiagnostic::new(rule_category!(), ty.range(), get_diagnostic_title(kind)));
				}
...
fn transform_array_type(ty: AnyTsType, array_kind: TsArrayKind) -> Option<AnyTsType> {
	if let AnyTsType::TsArrayType(_) = ty {
		let array_types = transform_array_element_type(ty, array_kind).into_iter().collect::<Vec<_>>();
...
fn convert_to_array_type(type_arguments: &TsTypeArguments, array_kind: TsArrayKind) -> Option<AnyTsType> {
...
fn transform_array_element_type(param: AnyTsType, array_kind: TsArrayKind) -> Option<AnyTsType> {
	...
	element_type.map(|element_type| match array_kind {
		TsArrayKind::GenericArray => {
			AnyTsType::TsArrayType(make::ts_array_type(element_type, make::token(T!['[']), make::token(T![']'])))
		},
...
fn get_action_message(array_kind: TsArrayKind) -> MarkupBuf {
	match array_kind {
		...
		TsArrayKind::ReadonlyGenericArray => {
			markup! { "Use "<Emphasis>"shorthand readonly T[] syntax"</Emphasis>" to replace" }.to_owned()
		},
		TsArrayKind::Shorthand => markup! { "Use "<Emphasis>"Array<T> syntax"</Emphasis>" to replace"}.to_owned(),
		TsArrayKind::Readonly => {
...
{
	let ident = if is_readonly { make::ident("ReadonlyArray") } else { make::ident("Array") };

	AnyTsType::TsReferenceType(
		make::ts_reference_type(AnyTsName::JsReferenceIdentifier(make::js_reference_identifier(ident)))
