...
use biome_js_syntax::{
	AnyJsDeclarationClause, AnyTsReturnType, AnyTsType, JsSyntaxKind, T, TsCallSignatureTypeMember, TsFunctionType,
	TsInterfaceDeclaration, TsObjectType, TsTypeMemberList,
};
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		if let Some(ts_type_member_list) = query.parent::<TsTypeMemberList>() {
			...
			if let Some(interface_decl) = ts_type_member_list.parent::<TsInterfaceDeclaration>() {
				...
				if let AnyTsReturnType::AnyTsType(TsThisType(_)) = query.return_type_annotation()?.ty().ok()? {
					return None;
				}
			}
			return Some(());
	...
	fn action(ctx: &RuleContext<Self>, _: &Self::State) -> Option<JsRuleAction> {
		...
		if let Some(interface_decl) = ts_type_member_list.parent::<TsInterfaceDeclaration>() {
			let type_alias_declaration = ts_type_alias_declaration(
				...
				AnyTsType::from(convert_ts_call_signature_type_member_to_function_type(node)?),
			)
			.build()
		...
		if let Some(ts_object_type) = ts_type_member_list.parent::<TsObjectType>() {
			...
			return Some(JsRuleAction::new(
				ctx.metadata().action_category(ctx.category(), ctx.group()),
				ctx.metadata().applicability(),
				markup! { "Use a function type instead of an object type with a call signature." }.to_owned(),
				mutation,
			));
		}
...
fn convert_ts_call_signature_type_member_to_function_type(node: &TsCallSignatureTypeMember) -> Option<TsFunctionType> {
