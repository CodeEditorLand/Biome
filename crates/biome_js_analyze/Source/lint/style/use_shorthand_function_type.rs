...
use biome_js_factory::{make, make::ts_type_alias_declaration};
use biome_js_syntax::{
...
impl Rule for UseShorthandFunctionType {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		if let Some(ts_type_member_list) = query.parent::<TsTypeMemberList>() {
			// If there is more than one member, it's not a single call
			// signature.
			if ts_type_member_list.len() > 1 {
				return None;
			}
			// If the parent is an interface with an extends clause, it's not a
	...
	fn diagnostic(ctx:&RuleContext<Self>, _:&Self::State) -> Option<RuleDiagnostic> {
		Some(
			...
			.note(
				markup! { "Types containing only a call signature can be shortened to a function type." },
			),
		)
	}
