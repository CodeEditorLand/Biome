...
use biome_js_syntax::{
	AnyJsExpression, AnyJsLiteralExpression, JsLogicalExpression, JsObjectExpression, JsSyntaxKind, JsThrowStatement,
};
...
fn is_invalid_throw_value(any_expr: &AnyJsExpression) -> Option<bool> {
	...
	if AnyJsLiteralExpression::can_cast(kind)
		...
		|| matches!(kind, JsSyntaxKind::JS_BINARY_EXPRESSION | JsSyntaxKind::JS_TEMPLATE_EXPRESSION)
	{
		return Some(true);
	}
