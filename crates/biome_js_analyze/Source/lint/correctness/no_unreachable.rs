...
/// higher than [COMPLEXITY_THRESHOLD]. This score is an arbitrary value (the
...
fn analyze_simple(cfg:&JsControlFlowGraph, signals:&mut UnreachableRanges) {
	...
	while let Some((block_id, handlers)) = queue.pop_front() {
		...
		for inst in &block.instructions {
			// If this block is terminated, mark this instruction as unreachable and
			// continue
			if has_terminator {
...
fn traverse_cfg(
	...
	while let Some(mut path) = queue.pop_front() {
		...
		for inst in &block.instructions {
			...
			// If this block has already ended, immediately mark this instruction as
			// unreachable
