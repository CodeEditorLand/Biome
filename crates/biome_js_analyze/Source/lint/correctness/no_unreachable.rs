...
use biome_js_syntax::{
	JsBlockStatement, JsCaseClause, JsDefaultClause, JsDoWhileStatement, JsForInStatement, JsForOfStatement,
	JsForStatement, JsFunctionBody, JsIfStatement, JsLabeledStatement, JsLanguage, JsReturnStatement,
	JsSwitchStatement, JsSyntaxElement, JsSyntaxKind, JsSyntaxNode, JsTryFinallyStatement, JsTryStatement,
		...
		match state.terminators.as_slice() {
			...
			[node_a, node_b] => {
				if node_a.kind == node_b.kind {
					diagnostic = diagnostic.detail(node_a.range, "... because either this statement ...").detail(
						node_b.range,
						format_args!("... or this statement will {} beforehand", node_b.reason()),
					);
				} else {
					diagnostic = diagnostic
						.detail(
							node_a.range,
							format_args!("... because either this statement will {} ...", node_a.reason()),
						)
						.detail(
							node_b.range,
							format_args!("... or this statement will {} beforehand", node_b.reason()),
			...
			terminators => {
				...
				if has_homogeneous_kind {
					for (index, node) in terminators.iter().enumerate() {
						if index == 0 {
							diagnostic = diagnostic.detail(node.range, "... because either this statement, ...");
						} else if index < last {
							diagnostic = diagnostic.detail(node.range, "... this statement, ...");
						} else {
							diagnostic = diagnostic.detail(
								node.range,
								format_args!("... or this statement will {} beforehand", node.reason()),
							);
						}
					}
				} else {
					for (index, node) in terminators.iter().enumerate() {
						if index == 0 {
							diagnostic = diagnostic.detail(
								node.range,
								format_args!("... because either this statement will {}, ...", node.reason()),
							);
						} else if index < last {
							diagnostic = diagnostic
								.detail(node.range, format_args!("... this statement will {}, ...", node.reason()));
						} else {
							diagnostic = diagnostic.detail(
								node.range,
								format_args!("... or this statement will {} beforehand", node.reason()),
			...
			match inst.kind {
				...
				InstructionKind::Jump { conditional, block, finally_fallthrough } => {
					...
					if path.terminator.is_none() && !conditional {
						path.terminator = Some(
							inst.node
								.as_ref()
								.map(|node| PathTerminator { kind: node.kind(), range: node.text_trimmed_range() }),
						);
	...
	fn push(&mut self, node: &JsSyntaxElement, terminator: Option<PathTerminator>) {
		...
		let index = match insertion {
			...
			Err(index) => {
				self.ranges.insert(
					index,
					UnreachableRange { text_range, text_trimmed_range, terminators: terminator.into_iter().collect() },
				);
		...
		while let Some(parent) = node.ancestors().find_map(JsControlFlowNode::cast) {
			...
			let fields = match &parent {
				...
				JsControlFlowNode::JsTryFinallyStatement(stmt) => {
					...
					res.push(stmt.finally_clause().ok()?.body().ok()?.syntax().text_range_with_trivia());
			...
			entry.text_trimmed_range = entry.text_trimmed_range.cover(parent.syntax().text_trimmed_range());
	...
	fn merge_adjacent_ranges(&mut self) {
		...
		while index < self.ranges.len().saturating_sub(1) {
			...
			if self.ranges[index + 1].text_range.start() <= text_range.end() {
				...
				entry.text_trimmed_range = entry.text_trimmed_range.cover(prev_entry.text_trimmed_range);
