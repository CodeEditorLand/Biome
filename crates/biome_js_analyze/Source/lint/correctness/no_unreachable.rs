		...
		match state.terminators.as_slice() {
			...
			[node] => {
				diagnostic = diagnostic.detail(
					node.range,
					format_args!("... because this statement will {} beforehand", node.reason()),
				);
			},
			...
			terminators => {
				...
				// Do not repeat the reason for each terminator if they all have
				// the same kind
...
/// Returns true if the "complexity score" for the [JsControlFlowGraph] is
/// higher than [COMPLEXITY_THRESHOLD]. This score is an arbritrary value (the
/// formula is similar to the cyclomatic complexity of the function but this is
/// only approximative) used to determine whether the NoDeadCode rule should
/// perform a fine reachability analysis or fall back to a simpler algorithm to
/// avoid spending too much time analyzing exceedingly complex functions
...
fn analyze_simple(cfg:&JsControlFlowGraph, signals:&mut UnreachableRanges) {
	...
	while let Some((block_id, handlers)) = queue.pop_front() {
		...
		for inst in &block.instructions {
			// If this block is terminated, mark this instruction as unreachable
			// and continue
			if has_terminator {
			...
			// Do not create exception edges for instructions with no side
			// effects
			if has_side_effects(inst) {
			...
			match inst.kind {
				InstructionKind::Statement => {},
				InstructionKind::Jump { conditional, block, finally_fallthrough } => {
					if finally_fallthrough && handlers.is_some() {
						// Jump towards the corresponding block if there are
						// pending exception handlers, otherwise return
						// from the function
...
fn traverse_cfg(
	...
	while let Some(mut path) = queue.pop_front() {
		...
		for inst in &block.instructions {
			...
			// If this block has already ended, immediately mark this
			// instruction as unreachable
			...
			match inst.kind {
				...
				InstructionKind::Jump { conditional, block, finally_fallthrough } => {
					...
					if path.terminator.is_none() && !conditional {
						path.terminator = Some(inst.node.as_ref().map(|node| {
							PathTerminator { kind:node.kind(), range:node.text_trimmed_range() }
						}));
...
/// Create an additional visitor path from a jump instruction and push it to the
/// queue
fn handle_jump<'cfg>(
...
impl UnreachableRanges {
	fn new() -> Self { UnreachableRanges { ranges:Vec::new() } }
	...
	fn push(&mut self, node:&JsSyntaxElement, terminator:Option<PathTerminator>) {
		...
		// appropriate position for either merging or inserting the incoming
		// range
	...
	fn propagate_ranges(&mut self, mut node:JsSyntaxNode, mut index:usize) -> Option<()> {
		while let Some(parent) = node.ancestors().find_map(JsControlFlowNode::cast) {
			...
			let fields = match &parent {
				...
				JsControlFlowNode::JsVariableStatement(stmt) => {
					...
					declaration
						...
						.filter_map(|declarator| {
							match declarator {
								Ok(declarator) => {
									match declarator.initializer()?.expression() {
										Ok(expression) => {
											Some(Ok(expression.syntax().text_range()))
										},
				...
				JsControlFlowNode::JsDoWhileStatement(stmt) => {
					vec![
						stmt.body().ok()?.syntax().text_range(),
						stmt.test().ok()?.syntax().text_range(),
					]
				},
				JsControlFlowNode::JsForInStatement(stmt) => {
					vec![
						stmt.initializer().ok()?.syntax().text_range(),
						stmt.body().ok()?.syntax().text_range(),
					]
				},
				JsControlFlowNode::JsForOfStatement(stmt) => {
					vec![
						stmt.initializer().ok()?.syntax().text_range(),
						stmt.body().ok()?.syntax().text_range(),
					]
				...
				JsControlFlowNode::JsTryStatement(stmt) => {
					vec![
						stmt.body().ok()?.syntax().text_range(),
				...
				JsControlFlowNode::JsTryFinallyStatement(stmt) => {
					...
					res.push(stmt.finally_clause().ok()?.body().ok()?.syntax().text_range());

					res
				},
				JsControlFlowNode::JsWhileStatement(stmt) => {
					vec![
						stmt.test().ok()?.syntax().text_range(),
						stmt.body().ok()?.syntax().text_range(),
					]
			...
			// Extend the range at the specific index to cover the whole parent
			// node
			...
			entry.text_trimmed_range =
				entry.text_trimmed_range.cover(parent.syntax().text_trimmed_range());
	...
	fn merge_adjacent_ranges(&mut self) {
		...
		while index < self.ranges.len().saturating_sub(1) {
			...
			if self.ranges[index + 1].text_range.start() <= text_range.end() {
				...
				entry.text_trimmed_range =
					entry.text_trimmed_range.cover(prev_entry.text_trimmed_range);
...
fn check_neighbors(ranges:&[UnreachableRange], index:usize, fields:&[TextRange]) -> Option<usize> {
...
impl IntoIterator for UnreachableRanges {
	...
	fn into_iter(self) -> Self::IntoIter { self.ranges.into_iter() }
}
