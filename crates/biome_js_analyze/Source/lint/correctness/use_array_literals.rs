...
use biome_js_syntax::{AnyJsCallArgument, AnyJsExpression, JsNewOrCallExpression, JsSyntaxKind, T, global_identifier};
...
impl Rule for UseArrayLiterals {
	...
	fn action(ctx: &RuleContext<Self>, _: &Self::State) -> Option<JsRuleAction> {
		...
		let new_node = if let Some(args) = node.arguments() {
			...
			// `new Array` -> `[]`
			make::js_array_expression(make::token(T!['[']), make::js_array_element_list([], []), make::token(T![']']))
		};
