...
use biome_js_syntax::{
	AnyJsClassMember, AnyJsClassMemberName, AnyJsFormalParameter, AnyJsName, JsAssignmentExpression,
	JsAssignmentOperator, JsClassDeclaration, JsSyntaxKind, JsSyntaxNode, TsAccessibilityModifier, TsPropertyParameter,
};
use biome_rowan::{
	AstNode, AstNodeList, AstSeparatedList, BatchMutationExt, SyntaxNodeOptionExt, TextRange, declare_node_union,
};
...
fn traverse_members_usage(syntax: &JsSyntaxNode, mut private_members: FxHashSet<AnyMember>) -> Vec<AnyMember> {
			...
			biome_rowan::WalkEvent::Enter(node) => {
				if let Some(js_name) = AnyJsName::cast(node) {
					private_members.retain(|private_member| {
						...
						let is_write_only = is_write_only(&js_name) == Some(true) && !private_member.is_accessor();
...
fn get_all_declared_private_members(class_declaration: &JsClassDeclaration) -> FxHashSet<AnyMember> {
	class_declaration
		.members()
		.iter()
...
fn get_constructor_params(class_declaration: &JsClassDeclaration) -> FxHashSet<AnyMember> {
	let constructor_member = class_declaration.members().iter().find_map(|member| match member {
...
fn is_write_only(js_name: &AnyJsName) -> Option<bool> {
	...
	if !matches!(assignment_expression.operator(), Ok(JsAssignmentOperator::Assign)) {
		let kind = assignment_expression.syntax().parent().kind();
		return Some(kind.is_some_and(|kind| matches!(kind, JsSyntaxKind::JS_EXPRESSION_STATEMENT)));
	}
...
impl AnyMember {
	...
	fn is_private(&self) -> Option<bool> {
		match self {
			AnyMember::AnyJsClassMember(member) => {
				let is_es_private = matches!(member.name().ok()??, AnyJsClassMemberName::JsPrivateClassMemberName(_));
	...
	fn property_range(&self) -> Option<TextRange> {
		match self {
			AnyMember::AnyJsClassMember(member) => match member {
				...
				AnyJsClassMember::JsPropertyClassMember(member) => Some(member.name().ok()?.range()),
				AnyJsClassMember::JsSetterClassMember(member) => Some(member.name().ok()?.range()),
				_ => None,
			},
			AnyMember::TsPropertyParameter(ts_property) => match ts_property.formal_parameter().ok()? {
				AnyJsFormalParameter::JsBogusParameter(_) | AnyJsFormalParameter::JsMetavariable(_) => None,
	...
	fn match_js_name(&self, js_name: &AnyJsName) -> Option<bool> {
		...
		match self {
			AnyMember::AnyJsClassMember(member) => match member {
				AnyJsClassMember::JsGetterClassMember(member) => Some(member.name().ok()?.name()?.text() == token),
				AnyJsClassMember::JsMethodClassMember(member) => Some(member.name().ok()?.name()?.text() == token),
				AnyJsClassMember::JsPropertyClassMember(member) => Some(member.name().ok()?.name()?.text() == token),
				AnyJsClassMember::JsSetterClassMember(member) => Some(member.name().ok()?.name()?.text() == token),
				_ => None,
			},
			AnyMember::TsPropertyParameter(ts_property) => match ts_property.formal_parameter().ok()? {
				AnyJsFormalParameter::JsBogusParameter(_) | AnyJsFormalParameter::JsMetavariable(_) => None,
