...
fn get_constructor_params(class_declaration:&JsClassDeclaration) -> FxHashSet<AnyMember> {
	let constructor_member = class_declaration.members().iter().find_map(|member| {
		match member {
...
/// Check whether the provided `AnyJsName` is part of a potentially write-only
/// assignment expression. This function inspects the syntax tree around the
/// given `AnyJsName` to check whether it is involved in an assignment operation
/// and whether that assignment can be write-only.
///
/// # Returns
///
/// - `Some(true)`: If the `js_name` is in a write-only assignment.
/// - `Some(false)`: If the `js_name` is in a assignments that also reads like
///   shorthand operators
/// - `None`: If the parent is not present or grand parent is not a
///   JsAssignmentExpression
///
...
impl AnyMember {
	...
	fn is_private(&self) -> Option<bool> {
		match self {
			...
			AnyMember::TsPropertyParameter(param) => {
				Some(
					param
						.modifiers()
						.iter()
