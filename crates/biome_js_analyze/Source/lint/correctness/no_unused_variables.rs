...
use biome_js_syntax::{
	binding_ext::{AnyJsBindingDeclaration, AnyJsIdentifierBinding, AnyJsParameterParentFunction},
...
fn suggested_fix_if_unused(binding:&AnyJsIdentifierBinding) -> Option<SuggestedFix> {
	...
	{
		if node.syntax().siblings(Direction::Next).last().is_some_and(|last_sibling| {
			matches!(last_sibling.kind(), JsSyntaxKind::JS_OBJECT_BINDING_PATTERN_REST)
		}) {
			return None;
		}
	}
...
impl Rule for NoUnusedVariables {
	...
	fn run(ctx:&RuleContext<Self>) -> Option<Self::State> {
		...
		// project. Thus, it is ok if types are not used locally.
		//
		...
		// We need to check if all uses of this binding are somehow recursive or
		// unused
		...
		binding
			...
			.filter_map(|reference| {
				...
				if reference.is_write() {
					...
					// Ensure that the assignment is not used in an used
					// expression.
			...
			.all(|ref_parent| {
				if matches!(
					...
					// Expression in an return position inside an arrow function
					// expression are used. Type parameters declared in
					// mapped types are only used in the mapped type.
					return false;
				}
	...
	for parent in expr.ancestors().skip(1) {
		match parent.kind() {
			...
			JsSyntaxKind::JS_SEQUENCE_EXPRESSION => {
				...
				// If the expression is not the rightmost node in a comma
				// sequence
