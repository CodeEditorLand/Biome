...
use biome_js_syntax::{
	AnyJsExpression, JsClassExpression, JsFileSource, JsForStatement, JsFunctionExpression, JsIdentifierExpression,
	JsModuleItemList, JsSequenceExpression, JsSyntaxKind, JsSyntaxNode, TsConditionalType, TsDeclarationModule,
	TsInferType,
};
use biome_rowan::{AstNode, BatchMutationExt, Direction, SyntaxResult};
use serde::{Deserialize, Serialize};

#[derive(Clone, Debug, Deserialize, biome_deserialize_macros::Deserializable, Eq, PartialEq, Serialize, Default)]
...
fn is_rest_spread_sibling(decl: &AnyJsBindingDeclaration) -> bool {
	...
	| AnyJsBindingDeclaration::JsObjectBindingPatternProperty(_)) = decl
	{
		node.syntax()
			...
			.last()
			.is_some_and(|last_sibling| matches!(last_sibling.kind(), JsSyntaxKind::JS_OBJECT_BINDING_PATTERN_REST))
	} else {
		false
	}
}
...
impl Rule for NoUnusedVariables {
	...
	fn run(ctx: &RuleContext<Self>) -> Option<Self::State> {
		...
		if is_declaration_file {
			if let Some(items) = binding.syntax().ancestors().skip(1).find_map(JsModuleItemList::cast) {
		...
		// Ignore expressions
		if binding.parent::<JsFunctionExpression>().is_some() || binding.parent::<JsClassExpression>().is_some() {
			return None;
		}
