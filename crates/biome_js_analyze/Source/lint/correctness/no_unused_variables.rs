...
impl Rule for NoUnusedVariables {
	...
	fn run(ctx:&RuleContext<Self>) -> Option<Self::State> {
		...
		if is_declaration_file {
			if let Some(items) =
				binding.syntax().ancestors().skip(1).find_map(JsModuleItemList::cast)
			{
				// A declaration file without top-level exports and imports is a global
				// declaration file. All top-level types and variiables are available in
				// every files of the project. Thus, it is ok if top-level types are not
				// used locally.
		...
		binding
			...
			.all(|ref_parent| {
				if matches!(
					...
					// Expression in an return position inside an arrow function expression are
					// used. Type parameters declared in mapped types are only used in the
					// mapped type.
					return false;
				}
