...
use biome_js_syntax::{
	...
	JsFunctionExportDefaultDeclaration, JsFunctionExpression, JsGetterClassMember, JsGetterObjectMember,
	JsMethodClassMember, JsMethodObjectMember, JsReturnStatement, JsSyntaxKind,
};
...
impl AnyJsFunctionMethodWithReturnType {
	pub fn return_type(&self) -> Option<AnyTsReturnType> {
		match self {
			...
			Self::JsFunctionExportDefaultDeclaration(func) => func.return_type_annotation()?.ty().ok(),
			Self::JsFunctionExpression(func) => func.return_type_annotation()?.ty().ok(),
			Self::JsGetterClassMember(func) => Some(AnyTsReturnType::AnyTsType(func.return_type()?.ty().ok()?)),
			Self::JsGetterObjectMember(func) => Some(AnyTsReturnType::AnyTsType(func.return_type()?.ty().ok()?)),
