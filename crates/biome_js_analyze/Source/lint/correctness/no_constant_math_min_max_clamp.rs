...
use biome_js_syntax::{
	AnyJsExpression, AnyJsLiteralExpression, AnyJsMemberExpression, JsCallExpression, JsNumberLiteralExpression,
	global_identifier,
};
...
impl Rule for NoConstantMathMinMaxClamp {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let inner_call =
			get_math_min_or_max_call(outer_call.other_expression_argument.as_js_call_expression()?, model)?;
		...
		match (
			...
			(MinMaxKind::Min, Some(Ordering::Less)) | (MinMaxKind::Max, Some(Ordering::Greater)) => {
...
fn get_math_min_or_max_call(call_expression: &JsCallExpression, model: &SemanticModel) -> Option<MathMinOrMaxCall> {
	...
	match (first_argument, second_argument) {
		(
			any_expression,
			AnyJsExpression::AnyJsLiteralExpression(AnyJsLiteralExpression::JsNumberLiteralExpression(constant_value)),
		)
		| (
			AnyJsExpression::AnyJsLiteralExpression(AnyJsLiteralExpression::JsNumberLiteralExpression(constant_value)),
			any_expression,
