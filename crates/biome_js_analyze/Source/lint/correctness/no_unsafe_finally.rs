...
impl Rule for NoUnsafeFinally {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		let query = ctx.query();
		if query.in_finally_block()? { Some(()) } else { None }
	}
	...
	fn in_finally_block(&self) -> Option<bool> {
		...
		loop {
			...
			let should_stop = match self {
				Self::JsBreakStatement(it) if it.label_token().is_none() => sentinel_for_break(kind),
...
fn sentinel_for_continue(kind: JsSyntaxKind) -> bool {
	...
	sentinel_for_throw_or_return(kind)
		|| matches!(
			kind,
			JS_DO_WHILE_STATEMENT | JS_WHILE_STATEMENT | JS_FOR_OF_STATEMENT | JS_FOR_IN_STATEMENT | JS_FOR_STATEMENT
		)
}
