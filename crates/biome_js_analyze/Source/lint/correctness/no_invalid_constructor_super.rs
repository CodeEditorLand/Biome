...
use biome_js_syntax::{AnyJsClass, AnyJsExpression, JsAssignmentOperator, JsConstructorClassMember, JsLogicalOperator};
...
pub enum NoInvalidConstructorSuperState {
	...
	BadExtends { extends_range: TextRange, super_range: TextRange },
}
		...
		match (super_range, extends_clause) {
			(Some(super_range), Some(extends_clause)) => {
				...
				if let Some(is_valid) = is_valid_constructor(super_class) {
					if !is_valid {
						return Some(NoInvalidConstructorSuperState::BadExtends { super_range, extends_range });
					}
				}
				None
			},
			(Some(super_range), None) => Some(NoInvalidConstructorSuperState::UnexpectedSuper(super_range)),
			_ => None,
		}
	}
...
fn is_valid_constructor(expression: AnyJsExpression) -> Option<bool> {
	match expression.omit_parentheses() {
		AnyJsExpression::JsAwaitExpression(await_expression) => is_valid_constructor(await_expression.argument().ok()?),
		...
		AnyJsExpression::JsIdentifierExpression(identifier) => Some(!identifier.name().ok()?.is_undefined()),
		...
		AnyJsExpression::JsLogicalExpression(expression) => {
			...
			is_valid_constructor(expression.left().ok()?).or_else(|| is_valid_constructor(expression.right().ok()?))
		},
		...
		AnyJsExpression::TsAsExpression(type_assertion) => is_valid_constructor(type_assertion.expression().ok()?),
