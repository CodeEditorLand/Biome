...
use biome_js_syntax::{AnyJsFunction, JsFileSource, Language, TextRange, TsAsExpression, TsReferenceType};
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		ctx.query()
			...
			.filter_map(|reference| {
				...
				if text == "arguments" {
					let is_in_non_arrow_function = identifier.syntax().ancestors().any(|ancestor| {
						!matches!(
