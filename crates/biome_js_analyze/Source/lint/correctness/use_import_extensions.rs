...
impl Rule for UseImportExtensions {
	...
	fn action(ctx: &RuleContext<Self>, state: &Self::State) -> Option<JsRuleAction> {
		...
		mutation.replace_element(state.module_name_token.clone().into(), new_module_name.into());
...
fn get_extensionless_import(
	...
	if !matches!(first_component, Utf8Component::CurDir | Utf8Component::ParentDir) || path.extension().is_some() {
		return None;
	}

	let last_component = path_components.last().unwrap_or(first_component);

	let has_query_or_hash = last_component.as_str().contains('?') || last_component.as_str().contains('#');
	if has_query_or_hash {
		return Some(UseImportExtensionsState { module_name_token, suggestion: None });
	}

	let extension = if force_js_extensions { "js" } else { resolved_path.extension()? };

	let is_index_file = resolved_path.file_stem().is_some_and(|stem| stem == "index");
