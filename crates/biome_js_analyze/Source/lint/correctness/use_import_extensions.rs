...
pub struct UseImportExtensionsOptions {
	...
	/// inspected file extension, and the value is a pair of `module` extension
	/// and `component` import extension
...
fn get_extensionless_import(
	...
	match last_component {
		...
		// Rust Path does not expose `./` path segment at very end, likely because it does not do
		// anything. To provide proper fix, we need to remove it as well.
...
fn resolve_import_extension<'a>(
	...
	let (potential_ext, potential_component_ext):(&str, &str) =
		if let Some(custom_mapping) = custom_suggested_imports.get(file_ext) {
			...
			// TODO. This is not very accurate. We should use file system access to
