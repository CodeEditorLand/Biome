...
use biome_js_syntax::{
	AnyJsClass, AnyJsExpression, JsCallExpression, JsConstructorClassMember, JsSyntaxKind, JsThrowStatement, TextRange,
	WalkEvent,
};
...
impl Rule for NoUnreachableSuper {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let mut block_context_stack = vec![BlockContext { block_id: ROOT_BLOCK_ID, super_call: None }];
		...
		while let Some(BlockContext { block_id, super_call: mut super_ }) = block_context_stack.pop() {
					...
					while let Some(event) = iter.next() {
						...
						match node.kind() {
							...
							JsSyntaxKind::JS_THIS_EXPRESSION => {
								if super_.is_none() {
									return Some(RuleState::ThisWithoutSuper { this: node.text_trimmed_range() });
								}
							},
				...
				match instruction.kind {
					...
					InstructionKind::Jump { block: jump_block_id, conditional, .. } => {
						...
						if visited_block_contexts.insert((jump_block_id, super_.is_some())) {
							block_context_stack.push(BlockContext { block_id: jump_block_id, super_call: super_ });
						}
						if !conditional {
					...
					InstructionKind::Return => {
						if super_.is_none() {
							...
							return Some(RuleState::ReturnWithoutSuper { return_statement: return_ });
						}
						// The next instructions are unreachable.
						break;
					},
				...
				if matches!(exception_handler.kind, ExceptionHandlerKind::Catch) {
					...
					if !had_super && super_.is_some() {
						...
						if visited_block_contexts.insert((exception_handler.target, had_super)) {
							block_context_stack
								.push(BlockContext { block_id: exception_handler.target, super_call: None });
						}
					}
					...
					if visited_block_contexts.insert((exception_handler.target, super_.is_some())) {
						block_context_stack
							.push(BlockContext { block_id: exception_handler.target, super_call: super_ });
					}
				}
			}
		}
