...
/// This allows traversing a control flow graph and retaining contextual
/// information.
...
impl Rule for NoUnreachableSuper {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		// Do not run the rule if the class has no extends clause or is
		// extending a literal expression
		...
		// We traverse the control flow graph and register in a context if we
		// saw `super`. The context follows the traversal of every path (we
		...
		let mut block_context_stack =
			vec![BlockContext { block_id:ROOT_BLOCK_ID, super_call:None }];
		...
		while let Some(BlockContext { block_id, super_call: mut super_ }) =
			block_context_stack.pop()
		{
			...
			for instruction in block.instructions.iter() {
						...
						match node.kind() {
							JsSyntaxKind::JS_SUPER_EXPRESSION => {
								...
								if let Some(first) = super_ {
									return Some(RuleState::DuplicateSuper { first, second:range });
								}
								super_ = Some(range);
							},
				...
				match instruction.kind {
					...
					InstructionKind::Jump { block: jump_block_id, conditional, .. } => {
						// Avoid cycles and redundant checks.
						if visited_block_contexts.insert((jump_block_id, super_.is_some())) {
							block_context_stack
								.push(BlockContext { block_id:jump_block_id, super_call:super_ });
						}
						if !conditional {
			...
			for exception_handler in block.exception_handlers.iter() {
				// Ignore finally handler: they are already in the Control Flow
				// Graph.
				if matches!(exception_handler.kind, ExceptionHandlerKind::Catch) {
					// First, assume that an exception was raised at the start
					// of the block
					...
					// Now, assume that an exception was raised at the end of
					// the block Avoid cycles and redundant checks.
