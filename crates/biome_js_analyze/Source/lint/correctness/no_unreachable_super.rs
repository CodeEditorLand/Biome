...
impl Rule for NoUnreachableSuper {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		// Do not run the rule if the class has no extends clause or is extending a
		// literal expression
		...
		// We traverse the control flow graph and register in a context if we saw
		// `super`. The context follows the traversal of every path (we process
		// [BlockContext]).
		//
