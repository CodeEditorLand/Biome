...
use biome_js_syntax::{
	AnyJsAssignmentPattern, AnyJsBindingPattern, AnyJsOptionalChainExpression, JsArrayAssignmentPatternElement,
	...
	JsLogicalExpression, JsLogicalOperator, JsNewExpression, JsObjectAssignmentPatternProperty, JsObjectMemberList,
	JsParenthesizedExpression, JsSequenceExpression, JsSpread, JsStaticMemberExpression, JsTemplateExpression,
	JsVariableDeclarator, JsWithStatement,
};
		...
		while let Some(current_parent) = parent.take() {
			match &current_parent {
				...
				RuleNode::JsInitializerClause(initializer) => {
					...
					} else if let Some(parent) = initializer.parent::<JsObjectAssignmentPatternProperty>() {
						if matches!(
							parent.pattern(),
					...
					} else if let Some(parent) = initializer.parent::<JsArrayAssignmentPatternElement>() {
						if matches!(
							parent.pattern(),
							Ok(AnyJsAssignmentPattern::JsObjectAssignmentPattern(_)
								| AnyJsAssignmentPattern::JsArrayAssignmentPattern(_))
...
impl From<AnyJsOptionalChainExpression> for RuleNode {
	fn from(node: AnyJsOptionalChainExpression) -> RuleNode {
		match node {
			AnyJsOptionalChainExpression::JsCallExpression(expression) => RuleNode::JsCallExpression(expression),
