...
use biome_js_syntax::{
	AnyJsExpression, AnyJsFunctionBody, AnyJsMemberExpression, AnyJsName, AnyJsStatement, JsCallExpression,
	JsSyntaxKind,
};
...
impl Rule for NoFlatMapIdentity {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let flat_map_expression = AnyJsMemberExpression::cast(flat_map_call.callee().ok()?.into_syntax())?;
			...
			let (function_param, function_body) = match arg.as_any_js_expression()? {
				AnyJsExpression::JsArrowFunctionExpression(arg) => {
					...
					let function_body: String = match arg.body().ok()? {
						AnyJsFunctionBody::AnyJsExpression(body) => body.omit_parentheses().to_trimmed_string(),
				...
				AnyJsExpression::JsFunctionExpression(arg) => {
					...
					if let Some(AnyJsStatement::JsReturnStatement(body)) = statement.next() {
						let Some(AnyJsExpression::JsIdentifierExpression(return_statement)) = body.argument() else {
							return None;
						};
						(function_parameter, return_statement.name().ok()?.to_trimmed_string())
					} else {
						return None;
					}
				},
