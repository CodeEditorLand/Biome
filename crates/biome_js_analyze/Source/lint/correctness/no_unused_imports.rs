		...
		match ctx.query() {
			...
			AnyJsImportClause::JsImportCombinedClause(clause) => {
				...
				match (is_default_import_unused, is_combined_unused) {
					(true, true) => {
						Some(Unused::AllImports(TextRange::new(
			...
			Unused::EmptyStatement(_) | Unused::AllImports(_) => {
				...
				if let Some(blank_line_pos) = blank_line_pos {
					...
					if let Some(prev_sibling) = parent.prev_sibling() {
						let new_prev_sibling =
							prev_sibling.clone().append_trivia_pieces(leading_trivia_pieces)?;
						...
						let new_next_sibling =
							next_sibling.clone().prepend_trivia_pieces(leading_trivia_pieces)?;
...
pub enum Unused {
	...
	/// The imports of the second specifier of the combined clause are unused.
	/// e.g.:
	/// - `import Default, * as UnusedNs from "mod"`
	/// - `import Default, { UnusedA }from "mod"`
	CombinedImport(TextRange),
	/// The default and some named imports in a combined clause are unused.
	/// e.g.:
