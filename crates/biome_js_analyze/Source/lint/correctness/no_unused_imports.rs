...
use biome_analyze::{
	FixKind, Rule, RuleDiagnostic, RuleSource, context::RuleContext, declare_lint_rule, options::JsxRuntime,
};
...
use biome_js_syntax::{
	AnyJsBinding, AnyJsCombinedSpecifier, AnyJsImportClause, AnyJsNamedImportSpecifier, JsNamedImportSpecifiers, T,
};
use biome_rowan::{AstNode, AstSeparatedElement, AstSeparatedList, BatchMutationExt, NodeOrToken, TextRange};
...
impl Rule for NoUnusedImports {
				...
				let (is_combined_unused, named_import_range) = match clause.specifier().ok()? {
					AnyJsCombinedSpecifier::JsNamedImportSpecifiers(specifiers) => {
						match unused_named_specifiers(ctx, &specifiers) {
							Some(Unused::AllImports(range) | Unused::EmptyStatement(range)) => (true, range),
							Some(Unused::NamedImports(unused_named_specifers)) => {
								return Some(if is_default_import_unused {
									Unused::DefaultNamedImport(default_local_name.range(), unused_named_specifers)
								} else {
	...
	fn diagnostic(ctx: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
		let diagnostic = match state {
			...
			Unused::AllImports(range) | Unused::DefaultImport(range) | Unused::CombinedImport(range) => {
			...
			Unused::DefaultNamedImport(default_import_range, unused_named_imports) => {
				let range = TextRange::new(default_import_range.start(), unused_named_imports.last()?.range().end());
				RuleDiagnostic::new(
					rule_category!(),
					range,
					markup! {
	...
	fn action(ctx: &RuleContext<Self>, state: &Self::State) -> Option<JsRuleAction> {
		...
		match state {
			Unused::EmptyStatement(_) | Unused::AllImports(_) => {
				...
				if let Some(blank_line_pos) = blank_line_pos {
					...
					if let Some(prev_sibling) = parent.prev_sibling() {
						let new_prev_sibling = prev_sibling.clone().append_trivia_pieces(leading_trivia_pieces)?;
						mutation.replace_element_discard_trivia(prev_sibling.into(), new_prev_sibling.into());
					} else if let Some(next_sibling) = parent.next_sibling() {
						let new_next_sibling = next_sibling.clone().prepend_trivia_pieces(leading_trivia_pieces)?;
						mutation.replace_element_discard_trivia(next_sibling.into(), new_next_sibling.into());
					} else if let Some(root) = root.as_js_module() {
						mutation.remove_element(parent.clone().into());
						let mut new_root =
							js_module(root.directives(), js_module_item_list(vec![]), root.eof_token().ok()?);
						...
						let new_root = new_root.build().prepend_trivia_pieces(leading_trivia_pieces)?;
			...
			Unused::DefaultNamedImport(_, unused_named_specifiers) => {
				let prev_clause = node.as_js_import_combined_clause()?.clone();
				let Ok(AnyJsCombinedSpecifier::JsNamedImportSpecifiers(named_specifiers)) = prev_clause.specifier()
				else {
					return None;
				};
				let (specifiers, separators): (Vec<_>, Vec<_>) = named_specifiers
					.specifiers()
					.elements()
					.filter_map(|AstSeparatedElement { node, trailing_separator }| {
			...
			Unused::NamedImports(unused_named_specifiers) => {
				for unused_specifier in unused_named_specifiers {
					if let Some(NodeOrToken::Token(next_token)) = unused_specifier.syntax().next_sibling_or_token() {
...
fn is_unused(ctx: &RuleContext<NoUnusedImports>, local_name: &AnyJsBinding) -> bool {
	...
	if ctx.jsx_runtime() == JsxRuntime::ReactClassic && is_global_react_import(binding, ReactLibrary::React) {
		return false;
	}
	let model = ctx.model();
