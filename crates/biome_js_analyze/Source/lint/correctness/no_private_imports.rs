...
use biome_js_syntax::{AnyJsImportClause, AnyJsImportLike, AnyJsNamedImportSpecifier, JsModuleSource, JsSyntaxToken};
...
impl Rule for NoPrivateImports {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let result = match node {
			AnyJsImportLike::JsModuleSource(node) => get_restricted_imports_from_module_source(node, &options),
	...
	let results = match node.syntax().parent().and_then(AnyJsImportClause::cast) {
		Some(AnyJsImportClause::JsImportCombinedClause(node)) => {
			...
			get_restricted_import_visibility(&Text::Static("default"), options)
				.map(|visibility| NoPrivateImportsState { range, path: path.clone(), visibility })
				.into_iter()
				.chain(
					node.specifier()?
						...
						.filter_map(|name| {
							get_restricted_import_visibility(&Text::Borrowed(name.token_text_trimmed()), options).map(
		...
		Some(AnyJsImportClause::JsImportDefaultClause(node)) => {
			let range = node.default_specifier()?.range();
			get_restricted_import_visibility(&Text::Static("default"), options)
				.map(|visibility| NoPrivateImportsState { range, path, visibility })
				.into_iter()
				.collect()
		},
		Some(AnyJsImportClause::JsImportNamedClause(node)) => node
			...
			.collect(),
		Some(AnyJsImportClause::JsImportBareClause(_) | AnyJsImportClause::JsImportNamespaceClause(_)) | None => {
			Vec::new()
		},
	};
...
fn get_restricted_import_visibility(import_name: &Text, options: &GetRestrictedImportOptions) -> Option<Visibility> {
...
#[inline]
fn target_path_is_in_same_module_as_self_path(target_path: &Utf8Path, self_path: &Utf8Path) -> bool {
	if !target_path.file_stem().is_some_and(|stem| INDEX_BASENAMES.contains(&stem)) {
		return false;
	}
	...
	self_path.ancestors().any(|ancestor| ancestor == target_parent)
}

/// Returns whether `target_path` is within the same package as `self_path`.
#[inline]
fn target_path_is_in_same_package_as_self_path(target_path: &Utf8Path, self_path: &Utf8Path) -> bool {
	let target_path = if target_path.file_stem().is_some_and(|stem| INDEX_BASENAMES.contains(&stem)) {
