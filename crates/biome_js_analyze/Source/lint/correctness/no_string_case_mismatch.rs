...
impl Rule for NoStringCaseMismatch {
	...
	fn run(ctx:&RuleContext<Self>) -> Vec<Self::State> {
		let query = ctx.query();
		match query {
			QueryCandidate::JsBinaryExpression(expr) => {
				CaseMismatchInfo::from_binary_expr(expr).into_iter().collect()
			},
	...
	fn diagnostic(ctx:&RuleContext<Self>, state:&Self::State) -> Option<RuleDiagnostic> {
		...
		let mut diagnostic = match query {
			...
			QueryCandidate::JsSwitchStatement(..) => {
				RuleDiagnostic::new(
					rule_category!(),
					state.literal.range(),
		...
		diagnostic = diagnostic
			.description(
				"This expression always returns false, because the string is converted and will \
				 never match",
			)
			.detail(
				state.call.range(),
...
impl<'a> CharCaseIterator<'a> {
	fn from(s:&'a str) -> Self { CharCaseIterator { iter:s.chars() } }
}
