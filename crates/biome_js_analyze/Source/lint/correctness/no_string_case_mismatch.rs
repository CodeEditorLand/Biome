...
impl Rule for NoStringCaseMismatch {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		let query = ctx.query();
		match query {
			QueryCandidate::JsBinaryExpression(expr) => CaseMismatchInfo::from_binary_expr(expr).into_iter().collect(),
...
impl CaseMismatchInfo {
	fn from_binary_expr(expr: &JsBinaryExpression) -> Option<Self> {
		let (left, right) = match expr.as_fields() {
			JsBinaryExpressionFields { left: Ok(left), right: Ok(right), operator_token: Ok(op) }
		...
		let (call, literal) = match (left, right) {
			(AnyJsExpression::JsCallExpression(call), other) | (other, AnyJsExpression::JsCallExpression(call)) => {
				(call, other)
			},
			_ => return None,
		};
	...
	fn from_switch_stmt(stmt: &JsSwitchStatement) -> Vec<Self> {
		match stmt.as_fields() {
			JsSwitchStatementFields { discriminant: Ok(AnyJsExpression::JsCallExpression(call)), cases, .. } => cases
				.into_iter()
	...
	fn compare_call_with_literal(call: JsCallExpression, literal: AnyJsExpression) -> Option<Self> {
		...
		is_mismatch.then_some(Self { expected_case, call, literal })
	}
}
