...
use biome_analyze::{
	AddVisitor, FixKind, Phases, QueryMatch, Queryable, Rule, RuleDiagnostic, RuleSource, ServiceBag, Visitor,
	VisitorContext, declare_lint_rule,
};
use biome_console::markup;
use biome_diagnostics::Severity;
use biome_js_syntax::{
	AnyJsStatement, JsBreakStatement, JsContinueStatement, JsFileSource, JsLabeledStatement, JsLanguage, TextRange,
	WalkEvent,
};
...
impl Rule for NoUnusedLabels {
	...
	fn run(ctx: &RuleContext<Self>) -> Option<Self::State> {
		...
		if label == "$" && ctx.source_type::<JsFileSource>().as_embedding_kind().is_svelte() {
			return None;
		}
		Some(())
	}
