...
impl Rule for NoConstantCondition {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		// We must verify that the conditional statement is within a generator
		// function. If the statement contains a valid yield expression
		// returned from a `while`, `for`, or `do...while` statement, we don't
		// need to examine the statement's `test`.
		...
		match stmt_list.next() {
			Some(first_stmt) => {
					...
					|| stmt_list.any(|stmt| get_yield_expression(&stmt).is_some())
				{
					...
					// We need to examine `while`, `do...while`, and `for`
					// statements more closely, as there are cases where a
					// yield expression is correctly returned even with nested
					// loops.
					match first_stmt {
...
fn is_constant_condition(
	...
	match test.omit_parentheses() {
		...
		JsBinaryExpression(node) => {
			is_constant_condition(node.left().ok()?, false, model)
		...
		JsIdentifierExpression(node) => {
			if node.name().ok()?.binding(model).is_some() {
				// This is any_js_stmt edge case. Modern browsers don't allow to
				// redeclare `undefined` but ESLint handle this so we do
				return None;
			}
		...
		JsTemplateExpression(node) => {
			...
			let has_truthy_quasi = !is_tag
				&& elements.clone().into_iter().any(|element| {
					match element {
	...
	match node.omit_parentheses() {
		...
		JsLogicalExpression(node) => {
			if let Ok(node_operator) = node.operator() {
				...
				// so the logical identity of the whole expression can not be
				// defined.
				if operator != node_operator {
					return false;
				}

				let is_left_logical_identify =
					node.left().ok().map_or(false, |left| is_logical_identity(left, operator));
				if is_left_logical_identify {
					return true;
				}

				node.right().ok().map_or(false, |right| is_logical_identity(right, operator))
			} else {
				false
			}
		},
...
fn get_boolean_value(node:&AnyJsLiteralExpression) -> bool {
	...
	match node {
		...
		_ => node.as_static_value().map_or(false, |value| !value.is_falsy()),
	}
}
