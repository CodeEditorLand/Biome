...
use biome_js_syntax::{
	...
	JsFunctionDeclaration, JsFunctionExpression, JsIfStatement, JsStatementList, JsSyntaxKind, JsWhileStatement,
	JsYieldExpression, TextRange,
};
...
impl Rule for NoConstantCondition {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		if let Some(any_js_stmt) = conditional_stmt.body() {
			if conditional_stmt.is_in_generator_function() && has_valid_yield_expression(&any_js_stmt).unwrap_or(false)
			{
				return None;
			}
		}
	...
	fn is_in_generator_function(&self) -> bool {
		self.syntax().ancestors().any(|node| {
			match JsFunctionDeclaration::try_cast(node) {
				...
				Err(node) => JsFunctionExpression::cast(node).and_then(|func_expr| func_expr.star_token()),
			}
			.is_some()
		})
	}
