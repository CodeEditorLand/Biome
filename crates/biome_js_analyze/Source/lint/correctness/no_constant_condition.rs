...
impl Rule for NoConstantCondition {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		// If the statement contains a valid yield expression returned from a `while`,
		// `for`, or `do...while` statement, we don't need to examine the statement's
		// `test`.
		...
		match stmt_list.next() {
			Some(first_stmt) => {
					...
					|| stmt_list.any(|stmt| get_yield_expression(&stmt).is_some())
				{
					...
					// as there are cases where a yield expression is correctly returned even with
					// nested loops.
					match first_stmt {
	...
	match test.omit_parentheses() {
		...
		JsIdentifierExpression(node) => {
			if node.name().ok()?.binding(model).is_some() {
				// This is any_js_stmt edge case. Modern browsers don't allow to redeclare
				// `undefined` but ESLint handle this so we do
				return None;
			}
