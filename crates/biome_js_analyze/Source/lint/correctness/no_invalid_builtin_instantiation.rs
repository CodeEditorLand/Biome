	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		if creation_rule.forbidden_builtins_list().binary_search(&name.text()).is_ok() {
			return ctx
				...
				.binding(&reference)
				.is_none()
				.then_some(NoInvalidBuiltinInstantiationState { name, creation_rule });
		}
...
pub(crate) fn convert_new_expression_to_call_expression(expr: &JsNewExpression) -> Option<JsCallExpression> {
