...
use biome_js_syntax::{JsLiteralMemberName, JsStringLiteralExpression, JsSyntaxKind, JsSyntaxToken};
...
impl Rule for NoNonoctalDecimalEscape {
			...
			if let Some(previous_escape) = previous_escape {
				if *previous_escape == "\\0" {
					if let Some(unicode_escape) = get_unicode_escape('\0') {
						...
						let replace_string_range = previous_escape_range_start..*decimal_escape_string_end;
					...
					let Some(decimal_char_unicode_escaped) = get_unicode_escape(decimal_char) else {
						continue;
					};
					// \8 -> \u0038
					result.push(RuleState {
	...
	fn diagnostic(
		_: &RuleContext<Self>,
		RuleState { diagnostics_text_range, .. }: &Self::State,
	...
	fn action(
		...
		RuleState { kind, replace_from, replace_to, replace_string_range, .. }: &Self::State,
	) -> Option<JsRuleAction> {
		let mut mutation = ctx.root().begin();
		let node = ctx.query();
		let prev_token = node.string_literal_token()?;
		let replaced =
			safe_replace_by_range(prev_token.text_trimmed().to_string(), replace_string_range.clone(), replace_to)?;
...
fn safe_replace_by_range(mut target: String, range: Range<usize>, replace_with: &str) -> Option<String> {
