...
impl Rule for NoInnerDeclarations {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		if matches!(
			parent.kind(),
			JsSyntaxKind::JS_EXPORT | JsSyntaxKind::TS_EXPORT_DECLARE_CLAUSE | JsSyntaxKind::JS_MODULE_ITEM_LIST
		) {
			return None;
		}
	...
	fn diagnostic(ctx: &RuleContext<Self>, _: &Self::State) -> Option<RuleDiagnostic> {
		...
		let nearest_root = decl.syntax().ancestors().skip(1).find_map(AnyJsControlFlowRoot::cast)?;
