...
fn is_continue_un_necessary(node: &JsContinueStatement) -> Option<bool> {
	...
	let ancestors: Vec<_> = syntax
		...
		.collect();
	let in_switch = ancestors.iter().any(|ancestor| ancestor.kind() == JS_SWITCH_STATEMENT);
...
fn is_continue_inside_last_ancestors(ancestors: &[JsSyntaxNode], syntax: &JsSyntaxNode) -> Option<bool> {
