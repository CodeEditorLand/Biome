...
impl Rule for UseValidForDirection {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		let is_less_than =
			matches!(operator, JsBinaryOperator::LessThan | JsBinaryOperator::LessThanOrEqual);
		...
		match node.update()? {
			...
			AnyJsExpression::JsAssignmentExpression(assignment_expr) => {
				...
				let update_ident =
					assignment_expr_left.as_any_js_assignment()?.as_js_identifier_assignment()?;
...
fn is_identifier_same(
	...
	Some(
		counter_ident.name().ok()?.value_token().ok()?.text_trimmed()
