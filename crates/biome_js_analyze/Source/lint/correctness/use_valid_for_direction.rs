...
use biome_js_syntax::{
	AnyJsExpression, JsAssignmentOperator, JsBinaryOperator, JsForStatement, JsIdentifierAssignment,
	JsIdentifierExpression, JsPostUpdateOperator, JsUnaryOperator,
};
...
impl Rule for UseValidForDirection {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let is_less_than = matches!(operator, JsBinaryOperator::LessThan | JsBinaryOperator::LessThanOrEqual);
		let is_greater_than = matches!(operator, JsBinaryOperator::GreaterThan | JsBinaryOperator::GreaterThanOrEqual);
		...
		match node.update()? {
			AnyJsExpression::JsPostUpdateExpression(update_expr) => {
				...
				if update_expr.operator().ok()? == JsPostUpdateOperator::Increment && is_greater_than {
					return Some(());
				}
			...
			AnyJsExpression::JsAssignmentExpression(assignment_expr) => {
				...
				let update_ident = assignment_expr_left.as_any_js_assignment()?.as_js_identifier_assignment()?;

				if !is_identifier_same(counter_ident, update_ident)? {
					return None;
				}

				if assignment_expr.right().ok()?.as_js_identifier_expression().is_some() {
					return None;
				}
...
fn is_identifier_same(counter_ident: &JsIdentifierExpression, update_ident: &JsIdentifierAssignment) -> Option<bool> {
	Some(
		counter_ident.name().ok()?.value_token().ok()?.text_trimmed() == update_ident.name_token().ok()?.text_trimmed(),
	)
}
