...
/// It traverses an [AnyAssignmentLike] and tracks the identifiers that have the
/// same name
...
struct SameIdentifiers {
	...
	///
	/// The queue is used to "save" the current traversal when it's needed to
	/// start a new one.
	///
	...
	/// ```
	///
	/// After `a`, we find a new assignment-like pattern that requires a new
	/// traversal, so we save the current traversal in the queue and we start
	/// a new one. When the inner traversal is finished, we resume the
	/// previous one.
	assignment_queue:VecDeque<AnyAssignmentLike>,
}

impl SameIdentifiers {
	/// Any assignment-like has a left arm and a right arm. Both arms needs to
	/// be "similar" in order to be compared. If during the traversal part of
	/// each arm differ, they are then ignored
	///
	/// The iterator logic makes sure to return the next eligible
	/// assignment-like.
	fn next_assignment_like(&mut self) -> Option<AnyAssignmentLike> {
		...
		match current_assignment_like {
			AnyAssignmentLike::Arrays { left, right } => {
				...
				// In case we have nested array/object structures, we save the
				// current pair and we restore it once this iterator is
				// consumed
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		loop {
			...
			match new_assignment_like {
				// if we are here, it's plausible that we consumed the current
				// iterator and we have to resume the previous one
				AnyAssignmentLike::None => {
					// we still have assignments-like to complete, so we
					// continue the loop
				...
				// we have a sub structure, which means we queue the current
				// assignment, and inspect the sub structure
...
/// A convenient iterator that continues to return the nested
/// [JsStaticMemberExpression]
#[derive(Debug, Clone)]
...
impl AnyJsAssignmentExpressionLikeIterator {
	...
	fn from_computed_member_assignment(source:&JsComputedMemberAssignment) -> SyntaxResult<Self> {
		Ok(Self {
			source_member:source.member().and_then(|expression| {
				match expression {
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		let reference = match object {
			...
			AnyJsExpression::JsIdentifierExpression(identifier) => {
				// the left side of the static member expression is an
				// identifier, which means that we can't go any further and
				// we should mark the iterator and drained
				self.drained = true;
...
enum AnyAssignmentLike {
	/// No assignments. This variant is used to signal that there aren't any
	/// more assignments to inspect
	None,
	/// To track identifiers that will be compared and check if they are the
	/// same.
	...
	/// ```
	///
	/// It stores a left iterator and a right iterator. Using iterators is
	/// useful to signal when there aren't any more elements to inspect.
	Arrays {
	...
	fn try_from(
		...
		Ok(match (left, right) {
			...
			(
				...
				AnyAssignmentLike::StaticExpression {
					left:AnyJsAssignmentExpressionLikeIterator::from_static_member_assignment(
						&left,
					)?,
...
/// Convenient type that pair possible combination of "identifiers" like that we
/// can find.
///
