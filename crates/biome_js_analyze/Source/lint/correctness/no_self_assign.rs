...
struct SameIdentifiers {
	...
	/// traversal, so we save the current traversal in the queue and we start a
	/// new one. When the inner traversal is finished, we resume the previous
	/// one.
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		loop {
			...
			match new_assignment_like {
				// if we are here, it's plausible that we consumed the current iterator and we have
				// to resume the previous one
				AnyAssignmentLike::None => {
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		let reference = match object {
			...
			AnyJsExpression::JsIdentifierExpression(identifier) => {
				// the left side of the static member expression is an identifier, which means
				// that we can't go any further and we should mark the iterator and drained
				self.drained = true;
