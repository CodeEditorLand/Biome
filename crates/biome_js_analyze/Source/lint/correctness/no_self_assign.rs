...
use biome_js_syntax::{
	AnyJsArrayAssignmentPatternElement, AnyJsArrayElement, AnyJsAssignment, AnyJsAssignmentPattern, AnyJsExpression,
	AnyJsLiteralExpression, AnyJsName, AnyJsObjectAssignmentPatternMember, AnyJsObjectMember, JsAssignmentExpression,
	JsAssignmentOperator, JsComputedMemberAssignment, JsComputedMemberExpression, JsIdentifierAssignment, JsLanguage,
	JsName, JsPrivateName, JsReferenceIdentifier, JsStaticMemberAssignment, JsStaticMemberExpression, JsSyntaxToken,
	inner_string_text,
};
use biome_rowan::{
	AstNode, AstSeparatedList, AstSeparatedListNodesIterator, SyntaxError, SyntaxResult, TextRange, declare_node_union,
};
...
fn compare_assignment_like(any_assignment_like: AnyAssignmentLike, incorrect_identifiers: &mut Vec<IdentifiersLike>) {
	let same_identifiers =
		SameIdentifiers { current_assignment_like: any_assignment_like, assignment_queue: VecDeque::new() };
...
impl SameIdentifiers {
	...
	fn next_assignment_like(&mut self) -> Option<AnyAssignmentLike> {
		...
		match current_assignment_like {
			...
			AnyAssignmentLike::StaticExpression { left, right } => Self::next_static_expression(left, right),
	...
	fn next_array_assignment(
		...
		if let (Some(left_element), Some(right_element)) = (left.next(), right.next()) {
			...
			) = (left_element, right_element)
			{
				...
				let new_assignment_like = AnyAssignmentLike::try_from((left.pattern().ok()?, right)).ok()?;
			...
			match (left_element, right_element) {
				...
				(
					...
					match (left, right) {
						// matches {a: b} = {a: b}
						(
							AnyJsAssignmentPattern::AnyJsAssignment(AnyJsAssignment::JsIdentifierAssignment(left)),
						...
						) => AnyAssignmentLike::Arrays { left: left.elements().iter(), right: right.elements().iter() },
						// matches {a: {b}} = {a: {b}}
						(
							AnyJsAssignmentPattern::JsObjectAssignmentPattern(left),
							AnyJsExpression::JsObjectExpression(right),
						) => AnyAssignmentLike::Object { left: left.properties().iter(), right: right.members().iter() },
			...
			if let Ok(identifier_like) = IdentifiersLike::try_from((left_name, right_name)) {
				if with_same_identifiers(&identifier_like).is_some() {
					if let (Some(left_reference), Some(right_reference)) = (left_reference, right_reference) {
						if with_same_identifiers(&IdentifiersLike::References(left_reference, right_reference))
							.is_some()
						{
							let source_identifier =
								IdentifiersLike::try_from((left.source_member.clone(), right.source_member.clone()))
									.ok()?;
...
impl Iterator for AnyJsAssignmentExpressionLikeIterator {
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		let (name, object) = if let Some(current_member_expression) = self.current_member_expression.as_ref() {
			(current_member_expression.member()?, current_member_expression.object()?)
		} else {
		...
		let reference = match object {
			AnyJsExpression::JsStaticMemberExpression(expression) => {
				self.current_member_expression = Some(AnyAssignmentExpressionLike::from(expression));
				None
			},
			...
			AnyJsExpression::JsComputedMemberExpression(computed_expression) => {
				self.current_member_expression =
					Some(AnyAssignmentExpressionLike::JsComputedMemberExpression(computed_expression));
				None
			},
			_ => return None,
		};
...
impl AnyAssignmentExpressionLike {
	fn member(&self) -> Option<AnyNameLike> {
		match self {
			AnyAssignmentExpressionLike::JsStaticMemberExpression(node) => node.member().ok().map(AnyNameLike::from),
			AnyAssignmentExpressionLike::JsComputedMemberExpression(node) => node.member().ok().and_then(|node| {
				Some(match node {
...
impl AnyAssignmentLike {
	const fn has_sub_structures(&self) -> bool {
		matches!(self, AnyAssignmentLike::Arrays { .. } | AnyAssignmentLike::Object { .. })
	}
}
...
impl TryFrom<(AnyJsAssignmentPattern, AnyJsExpression)> for AnyAssignmentLike {
	...
	fn try_from((left, right): (AnyJsAssignmentPattern, AnyJsExpression)) -> Result<Self, Self::Error> {
		Ok(match (left, right) {
			(AnyJsAssignmentPattern::JsArrayAssignmentPattern(left), AnyJsExpression::JsArrayExpression(right)) => {
				AnyAssignmentLike::Arrays { left: left.elements().iter(), right: right.elements().iter() }
			},

			(AnyJsAssignmentPattern::JsObjectAssignmentPattern(left), AnyJsExpression::JsObjectExpression(right)) => {
				AnyAssignmentLike::Object { left: left.properties().iter(), right: right.members().iter() }
			},
			...
			(
				AnyJsAssignmentPattern::AnyJsAssignment(AnyJsAssignment::JsIdentifierAssignment(left)),
				AnyJsExpression::JsIdentifierExpression(right),
			) => AnyAssignmentLike::Identifiers(IdentifiersLike::IdentifierAndReference(left, right.name()?)),
			(
				AnyJsAssignmentPattern::AnyJsAssignment(AnyJsAssignment::JsStaticMemberAssignment(left)),
			...
			(
				...
				right: AnyJsAssignmentExpressionLikeIterator::from_computed_member_expression(&right)?,
			},
			_ => AnyAssignmentLike::None,
		})
	}
...
impl TryFrom<(AnyNameLike, AnyNameLike)> for IdentifiersLike {
	...
	fn try_from((left, right): (AnyNameLike, AnyNameLike)) -> Result<Self, Self::Error> {
		match (left, right) {
			(AnyNameLike::AnyJsName(AnyJsName::JsName(left)), AnyNameLike::AnyJsName(AnyJsName::JsName(right))) => {
			...
			(AnyNameLike::JsReferenceIdentifier(left), AnyNameLike::JsReferenceIdentifier(right)) => {
				Ok(Self::References(left, right))
			},

			(AnyNameLike::AnyJsLiteralExpression(left), AnyNameLike::AnyJsLiteralExpression(right)) => {
