...
impl NoVoidElementsWithChildrenState {
	...
	fn has_dangerous_prop_cause(&self) -> bool {
		match &self.cause {
			NoVoidElementsWithChildrenCause::Jsx { dangerous_prop_cause, .. } => {
		...
		match node {
			NoVoidElementsWithChildrenQuery::JsxElement(element) => {
				...
				if is_void_dom_element(name) {
					let dangerous_prop =
						opening_element.find_attribute_by_name("dangerouslySetInnerHTML").ok()?;
			...
			NoVoidElementsWithChildrenQuery::JsxSelfClosingElement(element) => {
				...
				if is_void_dom_element(name) {
					let dangerous_prop =
						element.find_attribute_by_name("dangerouslySetInnerHTML").ok()?;
		...
		match node {
			NoVoidElementsWithChildrenQuery::JsxElement(element) => {
				...
				} = &state.cause
				{
					...
					// here we create a new list of attributes, ignoring the
					// ones that needs to be removed
			...
			// self closing elements don't have inner children so we can safely
			// remove the props that we don't need
