...
use biome_js_syntax::{
	AnyJsxAttribute, JsCallExpression, JsPropertyObjectMember, JsxAttribute, JsxElement, JsxSelfClosingElement,
};
...
impl NoVoidElementsWithChildrenState {
	fn new(element_name: impl Into<String>, cause: NoVoidElementsWithChildrenCause) -> Self {
		Self { element_name: element_name.into(), cause }
	}

	fn has_children_cause(&self) -> bool {
		match &self.cause {
			NoVoidElementsWithChildrenCause::Jsx { children_prop, children_cause, .. } => {
				*children_cause || children_prop.is_some()
			},
			NoVoidElementsWithChildrenCause::ReactCreateElement { children_prop, children_cause, .. } => {
				*children_cause || children_prop.is_some()
			},
		}
	}
	...
	fn has_dangerous_prop_cause(&self) -> bool {
		match &self.cause {
			NoVoidElementsWithChildrenCause::Jsx { dangerous_prop_cause, .. } => dangerous_prop_cause.is_some(),
			NoVoidElementsWithChildrenCause::ReactCreateElement { dangerous_prop_cause, .. } => {
...
impl Rule for NoVoidElementsWithChildren {
		...
		match node {
			NoVoidElementsWithChildrenQuery::JsxElement(element) => {
				...
				if is_void_dom_element(name) {
					let dangerous_prop = opening_element.find_attribute_by_name("dangerouslySetInnerHTML");
			...
			NoVoidElementsWithChildrenQuery::JsCallExpression(call_expression) => {
				let react_create_element = ReactCreateElementCall::from_call_expression(call_expression, model)?;
				...
				if is_void_dom_element(element_name) {
					...
					let dangerous_prop = react_create_element.find_prop_by_name("dangerouslySetInnerHTML");
	...
	fn diagnostic(ctx: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
		...
		let range = match node {
			NoVoidElementsWithChildrenQuery::JsxElement(element) => element.syntax().text_trimmed_range(),
			NoVoidElementsWithChildrenQuery::JsCallExpression(expression) => expression.syntax().text_trimmed_range(),
			NoVoidElementsWithChildrenQuery::JsxSelfClosingElement(element) => element.syntax().text_trimmed_range(),
		};
		Some(RuleDiagnostic::new(rule_category!(), range, state.diagnostic_message()))
	}
	...
	fn action(ctx: &RuleContext<Self>, state: &Self::State) -> Option<JsRuleAction> {
		...
		match node {
			NoVoidElementsWithChildrenQuery::JsxElement(element) => {
				if let NoVoidElementsWithChildrenCause::Jsx { children_prop, dangerous_prop_cause, .. } = &state.cause {
					let opening_element = element.opening_element().ok()?;
					...
					.build();
					mutation.replace_element(element.clone().into_syntax().into(), new_node.into_syntax().into());
				}
			},
