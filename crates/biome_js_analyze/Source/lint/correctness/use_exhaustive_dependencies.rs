...
use crate::{react::hooks::*, services::semantic::Semantic};
...
pub struct UseExhaustiveDependenciesOptions {
	/// Whether to report an error when a dependency is listed in the
	/// dependencies array but isn't used. Defaults to true.
...
fn report_unnecessary_dependencies_default() -> bool { true }
...
pub struct Hook {
	...
	///
	/// For example, for React's `useEffect()` hook, the dependencies index is
	/// 1.
	pub dependencies_index:Option<u8>,

	/// Whether the result of the hook is stable.
	///
	/// Set to `true` to mark the identity of the hook's return value as
	/// stable, or use a number/an array of numbers to mark the "positions" in
	/// the return array as stable.
	///
	...
	match decl.parent_binding_pattern_declaration().unwrap_or(decl) {
		...
		AnyJsBindingDeclaration::JsVariableDeclarator(declarator) => {
			let Some(declaration) =
				declarator.syntax().ancestors().find_map(JsVariableDeclaration::cast)
			else {
				return false;
			};
			let declaration_range = declaration.syntax().text_range();
		...
		// This should be unreachable because we call
		...
		// This should be unreachable because of the test if the capture is
		// imported
...
fn determine_unstable_dependency(
	...
	match declaration {
		AnyJsBindingDeclaration::JsArrowFunctionExpression(_)
		| AnyJsBindingDeclaration::JsFunctionDeclaration(_) => Some(UnstableDependencyKind::Function),
		AnyJsBindingDeclaration::JsArrayBindingPatternRestElement(_)
		| AnyJsBindingDeclaration::JsObjectBindingPatternRest(_) => {
		...
		AnyJsBindingDeclaration::JsVariableDeclarator(declaration) => {
			...
			match initializer.expression().ok()? {
				...
				| AnyJsExpression::JsFunctionExpression(_) => Some(UnstableDependencyKind::Function),
...
impl Rule for UseExhaustiveDependencies {
			...
			for (capture_text, capture_range, capture_path) in captures.iter() {
				...
				for dep in deps.iter() {
					...
					match (capture_contains_dep, dep_contains_capture) {
						...
						// capture: a.b.c
						// dependency: a
						// this is ok, but we may suggest performance
						// improvements in the future
						(true, false) => {
							// We need to continue, because it may still have a
							// perfect match in the dependency list
							is_captured_covered = true;
						},
						// example
						// capture: a.b
						// dependency: a.b.c
						// This can be valid in some cases. We will flag an
						// error nonetheless.
						(false, true) => {
							// We need to continue, because it may still have a
							// perfect match in the dependency list
	...
	fn instances_for_signal(signal:&Self::State) -> Vec<String> {
		match signal {
			Fix::MissingDependenciesArray { function_name_range: _ } => vec![],
			Fix::AddDependency { captures, .. } => vec![captures.0.clone()],
			Fix::RemoveDependency { dependencies, .. } => {
				dependencies.iter().map(|dep| dep.syntax().text_trimmed().to_string()).collect()
			},
			Fix::DependencyTooUnstable { dependency_name, .. } => {
				vec![dependency_name.clone()]
			},
			Fix::DependencyTooDeep { dependency_text, .. } => {
	...
	fn diagnostic(ctx:&RuleContext<Self>, dep:&Self::State) -> Option<RuleDiagnostic> {
		match dep {
			...
			Fix::AddDependency { function_name_range, captures, dependencies_len } => {
				...
				for dep in dependencies {
					if is_out_of_function_scope(dep, component_function, model).unwrap_or(false) {
						diag = diag.detail(
							...
							"Outer scope values aren't valid dependencies because mutating them \
