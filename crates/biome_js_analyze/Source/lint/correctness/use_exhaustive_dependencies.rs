...
use biome_deserialize::{DeserializableValidator, DeserializationContext, DeserializationDiagnostic, non_empty};
use biome_deserialize_macros::Deserializable;
use biome_diagnostics::Severity;
use biome_js_factory::make;
use biome_js_semantic::{Capture, SemanticModel};
use biome_js_syntax::{AnyJsArrayElement, AnyJsExpression, AnyJsMemberExpression, JsArrayExpression, T, TsTypeofType};
use biome_js_syntax::{
...
impl Default for HookConfigMaps {
	fn default() -> Self {
		...
		let stable_config = FxHashSet::from_iter([
			...
			StableReactHookConfiguration::new("useReducer", StableHookResult::Indices(vec![1]), true),
			StableReactHookConfiguration::new("useTransition", StableHookResult::Indices(vec![1]), true),
...
impl DeserializableValidator for Hook {
	fn validate(&mut self, ctx: &mut impl DeserializationContext, _name: &str, range: TextRange) -> bool {
		match (self.closure_index, self.dependencies_index) {
			(Some(closure_index), Some(dependencies_index)) if closure_index == dependencies_index => {
				ctx.report(
...
impl HookConfigMaps {
	pub fn new(hooks: &UseExhaustiveDependenciesOptions) -> Self {
		...
		for hook in &hooks.hooks {
			...
			if let (Some(closure_index), Some(dependencies_index)) = (hook.closure_index, hook.dependencies_index) {
				result.hooks_config.insert(
					hook.name.clone(),
					ReactHookConfiguration { closure_index, dependencies_index, builtin: false },
				);
			}
		}
...
fn capture_needs_to_be_in_the_dependency_list(
	...
	if capture.node().ancestors().any(|a| TsTypeofType::can_cast(a.kind())) {
		return false;
	}
	...
	match decl.parent_binding_pattern_declaration().unwrap_or(decl) {
		...
		AnyJsBindingDeclaration::JsFunctionDeclaration(declaration) => {
			...
			// }
			//
			if capture.node().ancestors().any(|ancestor| &ancestor == declaration.syntax()) {
				return false;
			}
		...
		AnyJsBindingDeclaration::JsVariableDeclarator(declarator) => {
			let Some(declaration) = declarator.syntax().ancestors().find_map(JsVariableDeclaration::cast) else {
				return false;
			};
			let declaration_range = declaration.syntax().text_range_with_trivia();
...
fn is_out_of_function_scope(
	...
	Some(model.scope(declaration.syntax()).is_ancestor_of(&model.scope(function)))
}
...
fn determine_unstable_dependency(
	...
	let declaration = model.binding(&identifier_name)?.tree().declaration()?;
	match declaration {
		AnyJsBindingDeclaration::JsArrowFunctionExpression(_) | AnyJsBindingDeclaration::JsFunctionDeclaration(_) => {
			Some(UnstableDependencyKind::Function)
		},
		AnyJsBindingDeclaration::JsArrayBindingPatternRestElement(_)
		| AnyJsBindingDeclaration::JsObjectBindingPatternRest(_) => Some(UnstableDependencyKind::ObjectLiteral),
...
impl Rule for UseExhaustiveDependencies {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		if let Some(result) = react_hook_with_dependency(call, &hook_config_maps.hooks_config, model) {
			...
			let dependencies_array = match &result.dependencies_node {
				...
				Some(expr) => {
					return vec![Fix::NonLiteralDependenciesArray { expr: expr.clone() }].into_boxed_slice();
				},
				None => {
					return if options.report_missing_dependencies_array {
						vec![Fix::MissingDependenciesArray { function_name_range: result.function_name_range }]
							.into_boxed_slice()
					} else {
			...
			for capture in &captures {
				...
				for dep in &deps {
					let (capture_contains_dep, dep_contains_capture) = compare_member_depth(capture, dep.syntax());
				...
				if !is_captured_covered {
					let captures = add_deps.entry(capture.text_trimmed().to_string().into()).or_default();
	...
	fn instances_for_signal(signal: &Self::State) -> Box<[Box<str>]> {
		match signal {
			...
			Fix::RemoveDependency { dependencies, .. } => dependencies
				...
				.collect::<Vec<_>>()
				.into_boxed_slice(),
			Fix::DependencyTooUnstable { dependency_name, .. } => vec![dependency_name.clone()].into(),
			Fix::DependencyTooDeep { dependency_text, .. } => vec![dependency_text.clone()].into(),
		}
	}

	fn diagnostic(ctx: &RuleContext<Self>, dep: &Self::State) -> Option<RuleDiagnostic> {
		match dep {
			Fix::MissingDependenciesArray { function_name_range } => Some(RuleDiagnostic::new(
				rule_category!(),
				function_name_range,
			...
			Fix::AddDependency { function_name_range, captures, dependencies_array, .. } => {
			...
			Fix::RemoveDependency { function_name_range, dependencies, component_function, .. } => {
			...
			Fix::DependencyTooUnstable { dependency_name, dependency_range, kind } => {
			...
			Fix::DependencyTooDeep { function_name_range, capture_range, dependency_range, dependency_text } => {
				let diag = RuleDiagnostic::new(
					rule_category!(),
					function_name_range,
					markup! {
	...
	fn action(ctx: &RuleContext<Self>, state: &Self::State) -> Option<JsRuleAction> {
		...
		let message = match state {
			Fix::AddDependency { captures: (_, nodes), dependencies_array, .. } => {
				...
				mutation.replace_node(dependencies_array.clone(), recreate_array(dependencies_array, elements));

				markup! { "Add the missing dependencies to the list." }
			},
			Fix::RemoveDependency { dependencies, dependencies_array, .. } => {
				let elements = dependencies_array.elements();
				let elements = elements
					.elements()
					.flat_map(|element| element.into_node())
					.filter(
						|node| matches!(node, AnyJsArrayElement::AnyJsExpression(expr) if !dependencies.contains(expr)),
					)
					.collect::<Vec<_>>();
	...
	I: ExactSizeIterator<Item = AnyJsArrayElement>,
{
	...
	current.clone().with_elements(make::js_array_element_list(elements, separators))
}
