...
use biome_deserialize::{DeserializableValidator, DeserializationDiagnostic, non_empty};
...
impl Rule for UseExhaustiveDependencies {
	...
	fn instances_for_signal(signal:&Self::State) -> Box<[Box<str>]> {
		match signal {
			Fix::MissingDependenciesArray { function_name_range: _ } => vec![].into_boxed_slice(),
