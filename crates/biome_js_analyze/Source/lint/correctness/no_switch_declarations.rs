...
use biome_js_syntax::{AnyJsDeclaration, AnyJsStatement, AnyJsSwitchClause, JsVariableStatement, T, TriviaPieceKind};
...
impl Rule for NoSwitchDeclarations {
	...
	fn action(ctx: &RuleContext<Self>, _: &Self::State) -> Option<JsRuleAction> {
		...
		let new_consequent = make::js_statement_list(Some(AnyJsStatement::JsBlockStatement(make::js_block_statement(
			make::token(T!['{'])
				.with_leading_trivia(Some((TriviaPieceKind::Whitespace, " ")))
				.with_trailing_trivia_pieces(colon_token.trailing_trivia().pieces()),
			consequent.clone(),
			make::token(T!['}']).with_leading_trivia_pieces(clause_token.indentation_trivia_pieces()),
		))));
