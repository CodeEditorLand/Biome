...
use crate::services::{control_flow::AnyJsControlFlowRoot, semantic::SemanticServices};
		...
		for binding in model.all_bindings() {
			...
			for reference in binding.all_references() {
				if reference.range_start() < declaration_end {
					...
					// References that are exports, such as `export { a }` are
					// always valid, even when they appear before the
