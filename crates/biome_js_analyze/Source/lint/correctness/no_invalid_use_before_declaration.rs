...
impl Rule for NoInvalidUseBeforeDeclaration {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		for binding in model.all_bindings() {
			...
			if matches!(
				id,
				AnyJsIdentifierBinding::TsIdentifierBinding(_) | AnyJsIdentifierBinding::TsTypeParameterName(_)
			) {
			...
			let declaration_control_flow_root = if let AnyJsBindingDeclaration::JsVariableDeclarator(declarator) =
				declaration.parent_binding_pattern_declaration().unwrap_or(declaration)
			{
				declarator
					.syntax()
					.ancestors()
	...
	fn diagnostic(_: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
		let InvalidUseBeforeDeclaration { declaration_kind, reference_range, binding_range: declaration_range } = state;
		...
		Some(
			...
			.detail(declaration_range, markup! { "The "{declaration_kind_text}" is declared here:" }),
		)
	}
}
