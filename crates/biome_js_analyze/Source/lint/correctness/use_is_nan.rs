...
use biome_js_syntax::{
	AnyJsCallArgument, AnyJsExpression, AnyJsMemberExpression, JsBinaryExpression, JsBinaryOperator, JsCaseClause,
	JsSwitchStatement, T, TextRange, global_identifier,
};
...
impl Rule for UseIsNan {
		...
		match node {
			UseIsNanQuery::JsBinaryExpression(bin_expr) => {
				if bin_expr.is_comparison_operator()
					&& (has_nan(bin_expr.left().ok()?, model) || has_nan(bin_expr.right().ok()?, model))
				{
					return Some(RuleState { message_id: Message::BinaryExpression, range: bin_expr.range() });
				}
			},
			UseIsNanQuery::JsCaseClause(case_clause) => {
				...
				if has_nan(test, model) {
					return Some(RuleState { message_id: Message::CaseClause, range });
				}
			},
			UseIsNanQuery::JsSwitchStatement(switch_stmt) => {
				...
				if has_nan(discriminant, model) {
					return Some(RuleState { message_id: Message::SwitchCase, range });
				}
			},
		}
		None
	...
	fn diagnostic(_: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
		Some(RuleDiagnostic::new(rule_category!(), state.range, state.message_id.as_str()))
	}
	...
	fn action(ctx: &RuleContext<Self>, _: &Self::State) -> Option<JsRuleAction> {
		...
		match query {
			UseIsNanQuery::JsBinaryExpression(binary_expression) => {
				...
				mutation.replace_node(AnyJsExpression::JsBinaryExpression(binary_expression.clone()), call.into());
...
fn create_unary_expression(with_inequality: bool, nan_expression: AnyJsExpression) -> Option<AnyJsExpression> {
	if with_inequality {
...
fn create_is_nan_expression(nan: AnyJsExpression) -> Option<AnyJsExpression> {
	match nan {
		AnyJsExpression::JsIdentifierExpression(_) | AnyJsExpression::JsComputedMemberExpression(_) => {
		...
		AnyJsExpression::JsStaticMemberExpression(member_expression) => {
			let is_nan_expression = member_expression.with_member(make::js_name(make::ident("isNaN")).into());
			...
			if !reference.is_global_this() && !reference.has_name("window") || number_identifier_exists {
			...
			Some(is_nan_expression.with_object(member_expression.into()).into())
		},
		_ => None,
	}
}
...
fn get_literal(bin_expr: &JsBinaryExpression, model: &SemanticModel) -> Option<(AnyJsExpression, AnyJsExpression)> {
	...
	if !is_nan_on_left { Some((left, right)) } else { Some((right, left)) }
}
