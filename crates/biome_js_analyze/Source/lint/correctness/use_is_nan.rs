...
fn get_literal(
	...
	let left = left_expression.with_leading_trivia_pieces([])?.with_trailing_trivia_pieces([])?;
	let right = right_expression.with_leading_trivia_pieces([])?.with_trailing_trivia_pieces([])?;
