...
use crate::{
	react::hooks::{is_react_component, is_react_hook, is_react_hook_call},
	services::semantic::{SemanticModelBuilderVisitor, SemanticServices},
};
...
impl FunctionCallServices {
	fn early_returns_model(&self) -> &EarlyReturnsModel { &self.early_returns }

	fn semantic_model(&self) -> &SemanticModel { self.semantic_services.model() }
}
...
impl Phase for FunctionCallServices {
	fn phase() -> Phases { Phases::Semantic }
}
...
impl QueryMatch for FunctionCall {
	fn text_range(&self) -> TextRange { self.0.range() }
}
...
impl Queryable for FunctionCall {
	...
	fn unwrap_match(_services:&ServiceBag, query:&Self::Input) -> Self::Output { query.clone() }
}
