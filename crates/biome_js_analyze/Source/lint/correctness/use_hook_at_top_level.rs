...
use biome_analyze::{
	AddVisitor, FromServices, MissingServicesDiagnostic, Phase, Phases, QueryMatch, Queryable, Rule, RuleDiagnostic,
	RuleKey, ServiceBag, Visitor, VisitorContext, VisitorFinishContext, context::RuleContext, declare_lint_rule,
};
use biome_analyze::{RuleDomain, RuleSource};
use biome_console::markup;
use biome_deserialize::{
	Deserializable, DeserializableTypes, DeserializableValue, DeserializationContext, DeserializationDiagnostic,
...
use biome_js_syntax::{
	...
	JsArrayAssignmentPatternElement, JsArrayBindingPatternElement, JsCallExpression, JsConditionalExpression,
	...
	fn name(&self) -> Option<String> {
		match self {
			AnyJsFunctionOrMethod::AnyJsFunction(function) => {
				function.binding().as_ref().map(AnyJsBinding::to_trimmed_string)
			},
			AnyJsFunctionOrMethod::JsMethodObjectMember(method) => {
				method.name().ok().as_ref().map(AnyJsObjectMemberName::to_trimmed_string)
			},
		}
	}
}
...
fn enclosing_function_if_call_is_at_top_level(call: &JsCallExpression) -> Option<AnyJsFunctionOrMethod> {
...
fn is_conditional_expression(parent_node: &JsSyntaxNode, node: &JsSyntaxNode) -> bool {
	if let Some(assignment_with_default) = JsArrayAssignmentPatternElement::cast_ref(parent_node) {
		return assignment_with_default.init().is_some_and(|default| default.syntax() == node);
	}
	...
	if let Some(conditional_expression) = JsConditionalExpression::cast_ref(parent_node) {
		return conditional_expression.test().is_ok_and(|test| test.syntax() != node);
	}
	...
	if let Some(logical_expression) = JsLogicalExpression::cast_ref(parent_node) {
		return logical_expression.right().is_ok_and(|right| right.syntax() == node);
	}
...
impl Visitor for EarlyReturnDetectionVisitor {
	...
	fn visit(&mut self, event: &WalkEvent<SyntaxNode<Self::Language>>, _ctx: VisitorContext<Self::Language>) {
		match event {
			WalkEvent::Enter(node) => {
				if AnyFunctionLike::can_cast(node.kind()) {
					self.stack.push(EarlyReturnDetectionVisitorStackEntry::default());
				}
			},
...
impl Visitor for FunctionCallVisitor {
	...
	fn visit(&mut self, event: &WalkEvent<SyntaxNode<Self::Language>>, mut ctx: VisitorContext<Self::Language>) {
		match event {
			WalkEvent::Enter(_) => {},
			WalkEvent::Leave(node) => {
...
impl FromServices for FunctionCallServices {
	fn from_services(rule_key: &RuleKey, services: &ServiceBag) -> Result<Self, MissingServicesDiagnostic> {
		let early_returns: &EarlyReturnsModel = services
			.get_service()
			.ok_or_else(|| MissingServicesDiagnostic::new(rule_key.rule_name(), &["EarlyReturnsModel"]))?;
		Ok(Self {
...
impl Rule for UseHookAtTopLevel {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let root = CallPath { call: call.clone(), path: vec![] };
				...
				if let AnyJsFunctionOrMethod::AnyJsFunction(function) = enclosing_function {
					if let Some(calls_iter) = function.all_calls(model) {
						for call in calls_iter {
							calls.push(CallPath { call: call.tree(), path: path.clone() });
						}
					}
				}
			} else {
	...
	fn diagnostic(_: &RuleContext<Self>, suggestion: &Self::State) -> Option<RuleDiagnostic> {
		match suggestion {
			Suggestion::None { hook_name_range, path, early_return, is_nested } => {
				...
				if let Some(range) = early_return {
					diag = diag.detail(range, markup! { "Hooks should not be called after an early return." })
				}
	...
	fn visit_map(
		...
		for (key, value) in members.flatten() {
			...
			match key_text.text() {
				"hooks" => {
					ctx.report(
						DeserializationDiagnostic::new_deprecated(key_text.text(), value.range()).with_note(markup! {
							<Emphasis>"useHookAtTopLevel"</Emphasis>" now uses the React hook naming convention to determine hook calls."
						}),
					);
				},
				text => ctx.report(DeserializationDiagnostic::new_unknown_key(text, key.range(), ALLOWED_KEYS)),
			}
		}
