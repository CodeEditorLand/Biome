	...
	fn new(num: &str) -> NormalizedNumber<'_> {
		...
		match mantissa.split_once(['.']) {
			...
			Some((integer, fraction)) => {
				...
				let digits = if fraction.is_empty() { remove_trailing_zeros(integer) } else { integer };
				NormalizedNumber { digits, digits_rest: fraction, exponent: exponent + integer.len() as isize }
			},
		}
	}
