use std::{num::IntErrorKind, ops::RangeInclusive};
...
use biome_js_syntax::{numbers::split_into_radix_and_number, JsNumberLiteralExpression};
...
impl Rule for NoPrecisionLoss {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		if is_precision_lost(node)? { Some(()) } else { None }
	}
	...
	fn diagnostic(ctx:&RuleContext<Self>, _:&Self::State) -> Option<RuleDiagnostic> {
		...
		Some(
			...
			.note(
				markup! { "The value at runtime will be "<Emphasis>{ value.to_string() }</Emphasis> },
			),
		)
	}
}
...
fn is_precision_lost_in_base_other(num:&str, radix:u8) -> bool {
	let parsed = match i64::from_str_radix(num, radix as u32) {
		Ok(x) => x,
		Err(e) => {
			return matches!(e.kind(), IntErrorKind::PosOverflow | IntErrorKind::NegOverflow);
		},
	};
...
fn remove_leading_zeros(num:&str) -> &str { num.trim_start_matches('0') }

fn remove_trailing_zeros(num:&str) -> &str { num.trim_end_matches('0') }
...
impl NormalizedNumber<'_> {
	fn new(num:&str) -> NormalizedNumber<'_> {
		...
		match mantissa.split_once(['.']) {
			...
			Some((integer, fraction)) => {
				...
				let digits =
					if fraction.is_empty() { remove_trailing_zeros(integer) } else { integer };
				NormalizedNumber {
					digits,
	...
	fn to_scientific(&self) -> String {
		...
		if fraction.is_empty() && self.digits_rest.is_empty() {
			...
			format!("{}.{}{}e{}", &self.digits[..1], fraction, self.digits_rest, self.exponent - 1)
		}
	}

	fn precision(&self) -> usize { self.digits.len() + self.digits_rest.len() }
}
