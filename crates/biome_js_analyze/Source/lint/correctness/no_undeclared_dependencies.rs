...
impl DependencyAvailability {
	fn is_available(&self, path: &Utf8Path) -> bool {
		match self {
			Self::Bool(b) => *b,
			Self::Patterns(globs) => biome_glob::CandidatePath::new(&path).matches_with_exceptions(globs),
		}
	}
}
...
impl Rule for NoUndeclaredDependencies {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let is_optional_dependency_available = ctx.options().optional_dependencies.is_available(path);
...
#[test]
fn test() {
	assert_eq!(parse_package_name("@scope/package-name"), Some("@scope/package-name"));
	assert_eq!(parse_package_name("@scope/package-name/path"), Some("@scope/package-name"));
