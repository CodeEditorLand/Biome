...
impl Default for DependencyAvailability {
	fn default() -> Self { Self::Bool(true) }
}
...
impl schemars::JsonSchema for DependencyAvailability {
	fn schema_name() -> String { "DependencyAvailability".to_owned() }
			...
			subschemas:Some(Box::new(SubschemaValidation {
				one_of:Some(vec![
					...
					Schema::Object(SchemaObject {
						...
						metadata:Some(Box::new(Metadata {
							description:Some(
								"This type of dependency will be available only if the linted \
								 file matches any of the globs."
									.to_owned(),
							),
							..Default::default()
						})),
...
pub struct NoUndeclaredDependenciesOptions {
	/// If set to `false`, then the rule will show an error when
