...
fn handle_collections(node:&JsArrayExpression, model:&SemanticModel) -> Vec<TextRange> {
	...
	node.elements()
		...
		.filter_map(|node| {
			let node = node.ok()?;
			// no need to handle spread case, if the spread argument is itself a
			// list it will be handled during list declaration
...
/// Inspects each statement for variable declarations and return statements to
/// find potential React components.
fn handle_function_body(
	node:&JsFunctionBody,
	model:&SemanticModel,
	is_inside_jsx:bool,
) -> Vec<TextRange> {
	// if the return statement definitely has a key prop, don't need to check
	...
	while let Some(current) = stack.pop() {
		match current {
			...
			AnyJsxChild::JsxFragment(node) => {
				let has_any_tags = node.children().iter().any(|child| {
					match &child {
						AnyJsxChild::JsxElement(_) | AnyJsxChild::JsxSelfClosingElement(_) => true,
						// HACK: don't flag the entire fragment if there's a
...
fn has_key_attribute(attributes:&JsxAttributeList) -> bool {
	attributes.into_iter().any(|attr| {
		...
		if let AnyJsxAttribute::JsxAttribute(attr) = attr {
			if let Ok(name) = attr.name() { name.text() == "key" } else { false }
		} else {
			false
		}
	})
