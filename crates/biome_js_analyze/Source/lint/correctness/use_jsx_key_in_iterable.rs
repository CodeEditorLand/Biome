...
use biome_js_syntax::{
	AnyJsExpression, AnyJsFunctionBody, AnyJsMemberExpression, AnyJsObjectMember, AnyJsxAttribute, AnyJsxChild,
	JsArrayExpression, JsCallExpression, JsFunctionBody, JsObjectExpression, JsxAttributeList, JsxExpressionChild,
	JsxTagExpression,
};
...
impl Rule for UseJsxKeyInIterable {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		match node {
			...
			UseJsxKeyInIterableQuery::JsCallExpression(node) => handle_iterators(node, model).unwrap_or_default(),
		}
		.into_boxed_slice()
	}
...
fn handle_iterators(node: &JsCallExpression, model: &SemanticModel) -> Option<Vec<TextRange>> {
	...
	let callback_index = if caller_name.is_some_and(|name| name == "Array") { 1 } else { 0 };

	let callback_arguments = arguments.get_arguments_by_index([callback_index]);

	let callback_argument = callback_arguments.first()?.as_ref()?.as_any_js_expression()?;
	...
	match callback_argument {
		...
		AnyJsExpression::JsArrowFunctionExpression(callback) => {
			...
			match body {
				...
				AnyJsFunctionBody::JsFunctionBody(body) => Some(handle_function_body(&body, model, is_inside_jsx)),
			}
		},
		_ => None,
	}
...
fn handle_function_body(node: &JsFunctionBody, model: &SemanticModel, is_inside_jsx: bool) -> Vec<TextRange> {
	...
	let is_return_component = return_statement
		...
		.and_then(|ret| {
			...
			Some(ReactComponentExpression::can_cast(returned_value.syntax().kind()))
		})
		.unwrap_or_default();
...
fn handle_potential_react_component(
	...
	if is_inside_jsx {
		...
		let range = handle_react_component(ReactComponentExpression::cast(node.into_syntax())?, model)?;
		Some(range)
	}
}

fn handle_react_component(node: ReactComponentExpression, model: &SemanticModel) -> Option<Vec<TextRange>> {
	match node {
		...
		match current {
			...
			AnyJsxChild::JsxFragment(node) => {
				let has_any_tags = node.children().iter().any(|child| match &child {
					...
					AnyJsxChild::JsxExpressionChild(node) => {
						node.expression().is_some_and(|n| n.as_js_conditional_expression().is_some())
					},
					_ => false,
				});
