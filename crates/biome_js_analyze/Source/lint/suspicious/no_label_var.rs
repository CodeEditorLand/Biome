...
impl Rule for NoLabelVar {
	...
	/// The first element of the tuple is the name of the binding, the second
	/// element of the tuple is the label name
	...
	fn run(ctx:&RuleContext<Self>) -> Option<Self::State> {
		...
		// if we find a binding that has its name equal to label name, then we
		// found a  `LabelVar` issue.
	...
	fn diagnostic(_:&RuleContext<Self>, state:&Self::State) -> Option<RuleDiagnostic> {
		...
		Some(
			...
			.note(
				markup! {"Creating a label with the same name as an in-scope variable leads to confusion."},
			),
		)
	}
}
