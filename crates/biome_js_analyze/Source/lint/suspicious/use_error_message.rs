...
impl Rule for UseErrorMessage {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		if BUILTIN_ERRORS.binary_search(&name_text).is_err() || ctx.model().binding(&reference).is_some() {
			return None;
		}
		...
		let Some(arg) = arguments.args().into_iter().nth(argument_position).and_then(|a| a.ok()) else {
