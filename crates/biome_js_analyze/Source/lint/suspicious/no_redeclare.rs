...
use biome_js_syntax::{binding_ext::AnyJsBindingDeclaration, JsSyntaxKind, TextRange};
...
impl Rule for NoRedeclare {
	...
	fn diagnostic(_ctx:&RuleContext<Self>, state:&Self::State) -> Option<RuleDiagnostic> {
		let Redeclaration { name, declaration, redeclaration } = state;
...
fn check_redeclarations_in_single_scope(scope:&Scope, redeclarations:&mut Vec<Redeclaration>) {
	...
	if scope.syntax().kind() == JsSyntaxKind::JS_FUNCTION_BODY {
		// Handle cases where a variable/type redeclares a parameter or type
		...
		// to directly add declarations of the function body in the function
		// scope. Unfortunately, this is not a good idea because variables and
		// types outside the function can be referenced in the parameters and
		// type parameters of the function, then shadowed in the function
		// body. Thus, using a distinct scope for the function body and the
		// function makes sense. For example:
		//
		// ```js
		// type U = string;
		// function g() {}
	...
	for binding in scope.bindings() {
		...
		if let Some(decl) = id_binding.declaration() {
			...
			if let Some((first_text_range, first_decl)) = declarations.get(&name) {
				// Do not report:
				// - mergeable declarations. e.g. a `function` and a `namespace`
				// - when both are parameter-like. A parameter can override a previous
				//   parameter.
				// - when both are type parameter in different declarations. A type parameter
				//   can be redeclared if they are in different declarations.
