...
use biome_js_syntax::{JsSyntaxKind, TextRange, binding_ext::AnyJsBindingDeclaration};
...
fn check_redeclarations_in_single_scope(scope:&Scope, redeclarations:&mut Vec<Redeclaration>) {
	...
	if scope.syntax().kind() == JsSyntaxKind::JS_FUNCTION_BODY {
		...
		// Unfortunately, this is not a good idea because variables and types outside
		// the function can be referenced in the parameters and type parameters of the
		// function, then shadowed in the function body.
		// Thus, using a distinct scope for the function body and the function makes
		// sense. For example:
		//
		// ```js
		// type U = string;
