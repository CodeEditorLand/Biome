...
use biome_js_syntax::{AnyJsObjectMember, JsGetterObjectMember, JsObjectExpression, JsSetterObjectMember};
use biome_js_syntax::{JsMethodObjectMember, JsPropertyObjectMember, JsShorthandPropertyObjectMember, TextRange};
...
impl MemberDefinition {
	fn name(&self) -> Option<TokenText> {
		match self {
			MemberDefinition::Getter(getter) => getter.name().ok()?.as_js_literal_member_name()?.name().ok(),
			MemberDefinition::Setter(setter) => setter.name().ok()?.as_js_literal_member_name()?.name().ok(),
			MemberDefinition::Method(method) => method.name().ok()?.as_js_literal_member_name()?.name().ok(),
			MemberDefinition::Property(property) => property.name().ok()?.as_js_literal_member_name()?.name().ok(),
			MemberDefinition::ShorthandProperty(shorthand_property) => {
				Some(shorthand_property.name().ok()?.value_token().ok()?.token_text_trimmed())
			},
		}
	}
	...
	fn node(&self) -> AnyJsObjectMember {
		match self {
			...
			MemberDefinition::ShorthandProperty(shorthand_property) => shorthand_property.clone().into(),
		}
	}
}
...
impl TryFrom<AnyJsObjectMember> for MemberDefinition {
	...
	fn try_from(member: AnyJsObjectMember) -> Result<Self, Self::Error> {
		match member {
			...
			AnyJsObjectMember::JsPropertyObjectMember(member) => Ok(MemberDefinition::Property(member)),
...
impl DefinedProperty {
	fn extend_with(&self, member_definition: MemberDefinition) -> Result<DefinedProperty, PropertyConflict> {
		match (self, member_definition) {
			...
			// Else conflict
			(defined_property, member_definition) => Err(PropertyConflict(defined_property.clone(), member_definition)),
		}
	}
}
...
impl Rule for NoDuplicateObjectKeys {
            ...
            .rev()
		{
			if let Some(member_name) = member_definition.name() {
				match defined_properties.remove(&member_name) {
					None => {
						defined_properties.insert(member_name, DefinedProperty::from(member_definition));
					},
					Some(defined_property) => match defined_property.extend_with(member_definition) {
	...
	fn diagnostic(
		...
		let mut diagnostic = RuleDiagnostic::new(
			...
			format!("This {member_definition} is later overwritten by an object member with the same name."),
		);
		diagnostic = match defined_property {
			DefinedProperty::Get(range) => diagnostic.detail(range, "Overwritten with this getter."),
			DefinedProperty::Set(range) => diagnostic.detail(range, "Overwritten with this setter."),
			DefinedProperty::Value(range) => diagnostic.detail(range, "Overwritten with this value."),
			DefinedProperty::GetSet(get_range, set_range) => match member_definition {
				MemberDefinition::Getter(_) => diagnostic.detail(get_range, "Overwritten with this getter."),
				MemberDefinition::Setter(_) => diagnostic.detail(set_range, "Overwritten with this setter."),
				...
				| MemberDefinition::ShorthandProperty(_) => match get_range.ordering(*set_range) {
					...
					Ordering::Greater => diagnostic.detail(get_range, "Overwritten with this getter."),
					Ordering::Equal => {
	...
	fn action(ctx: &RuleContext<Self>, PropertyConflict(_, member_definition): &Self::State) -> Option<JsRuleAction> {
