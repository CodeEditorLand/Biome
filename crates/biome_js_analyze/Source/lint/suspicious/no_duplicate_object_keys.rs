...
impl MemberDefinition {
	fn name(&self) -> Option<TokenText> {
		match self {
			...
			MemberDefinition::Property(property) => {
				property.name().ok()?.as_js_literal_member_name()?.name().ok()
			},
...
/// A descriptor for a property that is, as far as we can tell from statically
/// analyzing the object expression, not overwritten by another object member
/// and will make it into the object.
#[derive(Clone)]
...
impl DefinedProperty {
	fn extend_with(
		...
		match (self, member_definition) {
			...
			// Else conflict
			(defined_property, member_definition) => {
				Err(PropertyConflict(defined_property.clone(), member_definition))
			},
		}
	}
}
...
impl Rule for NoDuplicateObjectKeys {
	...
	fn diagnostic(
		...
		let mut diagnostic = RuleDiagnostic::new(
			...
			format!(
				"This {member_definition} is later overwritten by an object member with the same \
				 name."
			),
		);
		diagnostic = match defined_property {
			...
			DefinedProperty::GetSet(get_range, set_range) => {
				match member_definition {
					...
					| MemberDefinition::ShorthandProperty(_) => {
						match get_range.ordering(*set_range) {
							Ordering::Less => {
								diagnostic.detail(set_range, "Overwritten with this setter.")
							},
							Ordering::Greater => {
							...
							Ordering::Equal => {
								panic!(
									"The ranges of the property getter and property setter cannot \
									 overlap."
								)
							},
						}
		...
		diagnostic = diagnostic.note(
			"If an object property with the same name is defined multiple times (except when \
			 combining a getter with a setter), only the last definition makes it into the object \
			 and previous definitions are ignored.",
		);
