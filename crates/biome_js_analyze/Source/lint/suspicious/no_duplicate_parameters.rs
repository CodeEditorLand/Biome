...
use biome_js_syntax::{
	...
	JsIdentifierBinding,
	parameter_ext::{AnyJsParameterList, AnyJsParameters, AnyParameter},
};
...
impl Rule for NoDuplicateParameters {
	...
	fn run(ctx:&RuleContext<Self>) -> Option<Self::State> {
		...
		// Traversing the parameters of the function in preorder and checking for
		// duplicates,
...
/// Traverse a [JsAnyBindingPattern] in preorder and check if the name of
/// [JsIdentifierBinding] has seem before. If true then add the
/// [JsIdentifierBinding] to the [duplicated_arguments]. If false then add the
/// [JsIdentifierBinding] to the [tracked_bindings], mark it name as seen. If it
/// is not a [JsIdentifierBinding] then recursively call [traverse_binding] on
/// its children.
fn traverse_binding(
	...
	match binding {
		AnyJsBindingPattern::AnyJsBinding(inner_binding) => {
			match inner_binding {
...
#[inline]
/// If the name of binding has been seen in set, then we push the
/// `JsIdentifierBinding` into `identifier_vec`. Else we mark the name of
/// binding as seen.
fn track_binding(id_binding:&JsIdentifierBinding, tracked_bindings:&mut FxHashSet<String>) -> bool {
