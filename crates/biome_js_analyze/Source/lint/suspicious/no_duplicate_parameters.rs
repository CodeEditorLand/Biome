...
impl Rule for NoDuplicateParameters {
	...
	fn run(ctx: &RuleContext<Self>) -> Option<Self::State> {
		...
		let list = match parameters {
			AnyJsParameters::JsParameters(parameters) => AnyJsParameterList::from(parameters.items()),
			AnyJsParameters::JsConstructorParameters(parameters) => AnyJsParameterList::from(parameters.parameters()),
		};
	...
	match binding {
		...
		AnyJsBindingPattern::JsArrayBindingPattern(inner_binding) => {
			return inner_binding.elements().into_iter().find_map(|element| {
				...
				match element {
					AnyJsArrayBindingPatternElement::JsArrayBindingPatternRestElement(binding_rest) => {
					...
					AnyJsArrayBindingPatternElement::JsArrayBindingPatternElement(binding_with_default) => {
		...
		AnyJsBindingPattern::JsObjectBindingPattern(pattern) => {
			return pattern.properties().into_iter().find_map(|prop| {
				...
				match prop {
					...
					AnyJsObjectBindingPatternMember::JsObjectBindingPatternShorthandProperty(shorthand_binding) => {
