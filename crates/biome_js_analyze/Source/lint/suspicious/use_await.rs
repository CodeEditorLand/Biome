...
use biome_js_syntax::{AnyFunctionLike, JsAwaitExpression, JsForOfStatement, JsLanguage, TextRange, WalkEvent};
			...
			WalkEvent::Leave(node) => {
				if let Some(node) = AnyFunctionLike::cast_ref(node) {
					if let Some((function_start_range, has_await)) = self.stack.pop() {
						if function_start_range == node.range().start() && !has_await && node.is_async() {
