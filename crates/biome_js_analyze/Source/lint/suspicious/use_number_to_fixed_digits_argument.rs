...
use biome_js_syntax::{
	AnyJsCallArgument, AnyJsExpression, AnyJsLiteralExpression, JsCallArgumentList, JsCallExpression, JsSyntaxKind,
};
...
impl Rule for UseNumberToFixedDigitsArgument {
	...
	fn action(ctx: &RuleContext<Self>, _: &Self::State) -> Option<JsRuleAction> {
		...
		let zero_literal = AnyJsLiteralExpression::JsNumberLiteralExpression(make::js_number_literal_expression(
			make::js_number_literal('0'),
		));

		let arg = AnyJsCallArgument::AnyJsExpression(AnyJsExpression::AnyJsLiteralExpression(zero_literal));
