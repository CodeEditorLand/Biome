...
use biome_js_syntax::{
	AnyJsExpression, AnyJsLiteralExpression, JsNumberLiteralExpression, T, numbers::split_into_radix_and_number,
};
...
impl Rule for NoApproximativeNumericConstant {
	...
	fn action(ctx: &RuleContext<Self>, constant_name: &Self::State) -> Option<JsRuleAction> {
		...
		let new_node = make::js_static_member_expression(
			make::js_identifier_expression(make::js_reference_identifier(make::ident("Math"))).into(),
			make::token(T![.]),
