use biome_analyze::{
	AddVisitor, Phases, QueryMatch, Queryable, Rule, RuleDiagnostic, RuleDomain, RuleSource, RuleSourceKind,
					...
					identifier.name().and_then(|name| name.value_token()).map_or((), |name| {
						if let Some(hooks_context) = self.stack.last_mut() {
							match name.text_trimmed() {
								"beforeEach" | "beforeAll" | "afterEach" | "afterAll" | "after" | "before" => {
									let counter = HooksContext::add(hooks_context, name.text_trimmed());
									if counter > 1 {
