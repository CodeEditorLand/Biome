		...
		match event {
			WalkEvent::Enter(node) => {
				...
				if let Ok(AnyJsExpression::JsIdentifierExpression(identifier)) = node.callee() {
					identifier.name().and_then(|name| name.value_token()).map_or((), |name| {
						if let Some(hooks_context) = self.stack.last_mut() {
							match name.text_trimmed() {
								"beforeEach" | "beforeAll" | "afterEach" | "afterAll" | "after"
								| "before" => {
									let counter =
			...
			WalkEvent::Leave(node) => {
				// When the visitor exits a function, if it matches the node of
				// the top-most entry of the stack and the `has_yield` flag
				// is `false`, emit a query match
...
impl Queryable for DuplicateHooks {
	// `Input` is the type that `ctx.match_query()` is called with in the
	// visitor
	type Input = Self;
	type Language = JsLanguage;
...
impl Rule for NoDuplicateTestHooks {
	...
	fn run(_:&RuleContext<Self>) -> Self::Signals { Some(()) }
