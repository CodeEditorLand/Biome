...
use biome_js_syntax::{
	AnyJsClassMemberName, JsClassMemberList, JsGetterClassMember, JsMethodClassMember, JsPropertyClassMember,
	JsSetterClassMember, JsStaticModifier, JsSyntaxList, TextRange,
};
...
impl AnyClassMemberDefinition {
	...
	fn modifiers_list(&self) -> JsSyntaxList {
		match self {
			AnyClassMemberDefinition::JsGetterClassMember(node) => node.modifiers().into_syntax_list(),
			AnyClassMemberDefinition::JsMethodClassMember(node) => node.modifiers().into_syntax_list(),
			AnyClassMemberDefinition::JsPropertyClassMember(node) => node.modifiers().into_syntax_list(),
			AnyClassMemberDefinition::JsSetterClassMember(node) => node.modifiers().into_syntax_list(),
		}
	}
...
impl Rule for NoDuplicateClassMembers {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		let mut defined_members: FxHashMap<MemberState, FxHashSet<MemberType>> = FxHashMap::default();
		...
		node.into_iter()
			.filter_map(|member| {
				...
				if let Some(stored_members) = defined_members.get_mut(&member_state) {
					...
					defined_members.entry(member_state).or_default().insert(member_type);
				}
	...
	fn diagnostic(_: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
		let diagnostic = RuleDiagnostic::new(
			rule_category!(),
			state.range(),
			format!("Duplicate class member name {:?}", get_member_name(&state.name()?)?.text()),
		);
