...
impl Rule for NoImportAssign {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let local_name_binding = match label_statement {
			...
			// `import {x} from 'y'`
			//          ^
			AnyJsImportSpecifier::JsShorthandNamedImportSpecifier(specifier) => specifier.local_name().ok(),
			// `import * as xxx from 'y'`
			//         ^^^^^^^^
			// `import a, * as b from 'y'`
			//            ^^^^^^
			AnyJsImportSpecifier::JsNamespaceImportSpecifier(specifier) => specifier.local_name().ok(),
			// `import xx from 'y'`
			//         ^^
			// `import a, * as b from 'y'`
			//         ^
			AnyJsImportSpecifier::JsDefaultImportSpecifier(specifier) => specifier.local_name().ok(),
		};
		local_name_binding
			.and_then(|binding| {
				...
				for reference in ident_binding.all_writes(model) {
					invalid_assign_list
						.push((JsIdentifierAssignment::cast_ref(reference.syntax())?, ident_binding.clone()));
				}
