...
use biome_js_syntax::{
	AnyJsArrayElement, AnyJsAssignment, AnyJsAssignmentPattern, AnyJsCallArgument, AnyJsDeclarationClause,
	AnyJsExportClause, AnyJsExportNamedSpecifier, AnyJsExpression, AnyJsObjectMemberName, AnyJsTemplateElement,
	ClassMemberName, JsMethodObjectMember,
};
use biome_js_syntax::{
	AnyJsClassMember, AnyJsObjectMember, JsAssignmentExpression, JsCallExpression, JsComputedMemberName, JsExport,
};
...
impl RuleState {
	fn diagnostic_message(&self) -> MarkupBuf {
		match self.message {
			...
			NoThenPropertyMessage::Export => markup! { "Do not export "<Emphasis>"then"</Emphasis>"."}.to_owned(),
...
impl Rule for NoThenProperty {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		match binding {
			...
			NoThenPropertyQuery::JsComputedMemberName(node) => process_js_computed_member_name(node),
	...
	fn diagnostic(_: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
		Some(RuleDiagnostic::new(rule_category!(), state.range, state.diagnostic_message()))
	}
}
	...
	match node {
		AnyJsObjectMember::JsGetterObjectMember(node) => {
			if node.name().ok()?.name()? == "then" {
				return Some(RuleState { range: node.name().ok()?.range(), message: NoThenPropertyMessage::Object });
			}
		},
...
fn process_js_class_member(node: &AnyJsClassMember) -> Option<RuleState> {
	...
	if let Some(ClassMemberName::Public(name)) = any_class_member_name.name() {
		if name == "then" {
			return Some(RuleState { range: any_class_member_name.range(), message: NoThenPropertyMessage::Class });
		}
	}
	None
}
	...
	match node.expression().ok()? {
		AnyJsExpression::AnyJsLiteralExpression(expr) => {
			if expr.value_token().ok()?.text_trimmed() == "\"then\"" {
				return Some(RuleState { range: expr.range(), message: NoThenPropertyMessage::Object });
			}
		},
		AnyJsExpression::JsTemplateExpression(lit) => {
			for l in lit.elements() {
				if let AnyJsTemplateElement::JsTemplateChunkElement(chunk) = l {
					if chunk.template_chunk_token().ok()?.text_trimmed() == "then" {
						return Some(RuleState { range: chunk.range(), message: NoThenPropertyMessage::Object });
					}
				}
			}
		},
		...
		AnyJsExpression::JsStaticMemberExpression(m) => {
				...
				if let AnyJsCallArgument::AnyJsExpression(expr) = &first {
					if let AnyJsExpression::JsArrayExpression(array) = expr {
						for arr in array.elements().iter() {
							match arr.ok()? {
								AnyJsArrayElement::AnyJsExpression(AnyJsExpression::JsArrayExpression(arg)) => {
			...
			if (callee == "Object" || callee == "Reflect") && member == "defineProperty" {
				...
				if second.to_trimmed_string() == "\"then\"" || second.to_trimmed_string() == "`then`" {
					return Some(RuleState { range: second.range(), message: NoThenPropertyMessage::Object });
				}
			}
		},
		_ => return None,
...
fn process_js_export_named_clause(node: &JsExport) -> Option<RuleState> {
	match node.export_clause().ok()? {
		...
		AnyJsExportClause::AnyJsDeclarationClause(AnyJsDeclarationClause::JsVariableDeclarationClause(node)) => {
			let decls = node.declaration().ok()?;
			for d in decls.declarators().iter() {
				let id = d.ok()?.id().ok()?;
				if id.to_trimmed_string() == "then" {
					return Some(RuleState { range: id.range(), message: NoThenPropertyMessage::Object });
				}
			}
		},
		_ => return None,
