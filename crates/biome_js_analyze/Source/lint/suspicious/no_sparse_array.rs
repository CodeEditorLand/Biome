...
impl Rule for NoSparseArray {
	...
	fn action(ctx: &RuleContext<Self>, _state: &Self::State) -> Option<JsRuleAction> {
		...
		for (i, item) in final_array_element_list.iter().enumerate() {
			if matches!(item, Ok(AnyJsArrayElement::JsArrayHole(_))) {
				let undefine_indent = if i == 0 {
					...
					make::ident("undefined").with_leading_trivia([(TriviaPieceKind::Whitespace, " ")])
				};
				let ident_expr = make::js_identifier_expression(make::js_reference_identifier(undefine_indent));
				...
				final_array_element_list = final_array_element_list.replace_node(
					n_element,
					AnyJsArrayElement::AnyJsExpression(AnyJsExpression::JsIdentifierExpression(ident_expr)),
				)?;
			}
		}
		...
		mutation.replace_node(
			node.clone(),
			make::js_array_expression(node.l_brack_token().ok()?, final_array_element_list, node.r_brack_token().ok()?),
		);
