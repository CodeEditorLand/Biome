	...
	fn action(ctx:&RuleContext<Self>, _state:&Self::State) -> Option<JsRuleAction> {
		...
		for (i, item) in final_array_element_list.iter().enumerate() {
			if matches!(item, Ok(AnyJsArrayElement::JsArrayHole(_))) {
				...
				// Why we need to use `final_array_element_list.iter().nth(i)`
				// instead of `item`, because every time we
				// call `replace_node` the previous iteration `item` is not the
				// descent child of current `final_array_element_list` any more.
