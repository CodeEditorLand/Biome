...
use crate::{
	react::{is_react_call_api, ReactLibrary},
...
impl Rule for NoArrayIndexKey {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		// find the declaration and ensure it resolves to the parameter of a
		// function, and navigate up to the closest call expression
		let parameter = model
			.binding(&reference)
		...
		// Check if the caller is an array method and the parameter is the array
		// index of that method
		...
		if node.is_property_object_member() {
			...
			// Check if the object expression is passed to a
			// `React.cloneElement` call
...
/// Given a parameter and a call expression, it navigates the `callee` of the
/// call and check if the method called by this function belongs to an array
/// method and if the parameter is an array index
///
/// ```jsx
/// Array.map((_, index) => {
...
/// ```
///
/// Given this example, the input node is the `index` and `Array.map(...)` call
/// and we navigate to retrieve the name `map` and we check if it belongs to an
