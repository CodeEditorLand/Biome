...
use biome_js_syntax::{
	AnyJsExpression, AnyJsFunction, AnyJsMemberExpression, AnyJsTemplateElement, JsBinaryExpression,
	JsCallArgumentList, JsCallArguments, JsCallExpression, JsFormalParameter, JsObjectExpression, JsObjectMemberList,
	...
	fn as_js_expression(&self) -> Option<AnyJsExpression> {
		match self {
			NoArrayIndexKeyQuery::JsxAttribute(attribute) => attribute
				...
				.expression()
				.ok(),
			NoArrayIndexKeyQuery::JsPropertyObjectMember(object_member) => object_member.value().ok(),
		}
	}
}
...
impl Rule for NoArrayIndexKey {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
			...
			AnyJsExpression::JsTemplateExpression(template_expression) => {
				...
				for element in template_elements {
					if let AnyJsTemplateElement::JsTemplateElement(template_element) = element {
						let cap_index_value =
							template_element.expression().ok()?.as_js_identifier_expression()?.name().ok();
		...
		if node.is_property_object_member() {
			...
			if is_react_call_api(&callee, model, ReactLibrary::React, "cloneElement") {
				...
				Some(NoArrayIndexKeyState { binding_origin: binding_origin.range(), incorrect_prop: reference.range() })
			} else {
				None
			}
		} else {
	...
	fn diagnostic(_ctx: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
		let NoArrayIndexKeyState { binding_origin: incorrect_key, incorrect_prop } = state;
		let diagnostic = RuleDiagnostic::new(
			rule_category!(),
			incorrect_prop,
			markup! {"Avoid using the index of an array as key property in an element."},
		)
		.detail(incorrect_key, markup! {"This is the source of the key value."})
		.note(markup! {"The order of the items may change, and this also affects performances and component state."})
		.note(markup! {
...
fn is_array_method_index(parameter: &JsFormalParameter, call_expression: &JsCallExpression) -> Option<bool> {
	let member_expression = AnyJsMemberExpression::cast(call_expression.callee().ok()?.into_syntax())?;
