...
use biome_js_syntax::{AnyJsCallArgument, AnyJsExpression, AnyJsFunction, JsNewExpression, JsNewExpressionFields};
...
impl Rule for NoAsyncPromiseExecutor {
	...
	fn run(ctx: &RuleContext<Self>) -> Option<Self::State> {
		let node = ctx.query();
		let JsNewExpressionFields { new_token: _, callee, type_arguments: _, arguments } = node.as_fields();
...
fn get_async_function_expression_like(expr: AnyJsExpression) -> Option<AnyJsFunction> {
	match expr.omit_parentheses() {
		AnyJsExpression::JsFunctionExpression(func) => {
			func.async_token().map(|_| AnyJsFunction::JsFunctionExpression(func))
		},
		AnyJsExpression::JsArrowFunctionExpression(func) => {
			func.async_token().map(|_| AnyJsFunction::JsArrowFunctionExpression(func))
		},
		_ => None,
	}
}
