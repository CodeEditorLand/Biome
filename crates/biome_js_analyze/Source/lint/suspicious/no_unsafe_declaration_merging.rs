...
impl Rule for NoUnsafeDeclarationMerging {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let interface_name = interface_binding.as_ts_identifier_binding()?.name_token().ok()?;
		let scope = model.scope(ts_interface.syntax()).parent()?;
		for binding in scope.bindings() {
			if let Some(AnyJsBindingDeclaration::JsClassDeclaration(class)) = binding.tree().declaration() {
