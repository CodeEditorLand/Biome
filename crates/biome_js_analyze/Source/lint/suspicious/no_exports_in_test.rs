						...
						object.is_some_and(|object| match object {
							AnyJsExpression::JsIdentifierExpression(ident) => match member_expr {
								...
								AnyJsMemberAssignment::JsStaticMemberAssignment(static_member) => {
									...
									let member_text = static_member.member().map(|member| member.to_trimmed_string());
									indent_text == "module"
							...
							AnyJsExpression::JsStaticMemberExpression(member_expr) => {
								...
								let object_text = member_expr.object().map(|object| object.to_trimmed_string());
								let member_text = member_expr.member().map(|member| member.to_trimmed_string());
