...
use biome_js_syntax::{
	AnyJsMemberExpression, JsArrowFunctionExpression, JsCallExpression, JsExpressionStatement, T, global_identifier,
};
...
impl Rule for NoConsole {
	...
	fn diagnostic(ctx: &RuleContext<Self>, _: &Self::State) -> Option<RuleDiagnostic> {
		...
		let range =
			JsExpressionStatement::cast_ref(&parent).map_or(node.range(), |node| node.syntax().text_trimmed_range());
		Some(
			RuleDiagnostic::new(
				rule_category!(),
				range,
