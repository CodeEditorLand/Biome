...
impl Rule for NoFallthroughSwitchClause {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		while let Some(block_id) = block_stack.pop() {
			...
			// Traverse the instructions of the block searching for a switch
			// statement. A switch statements is followed by conditional
			// jumps (the cases), and a last unconditional jump (maybe the
			// default clause)
			...
			for instruction in block.instructions.iter() {
				match instruction.kind {
					InstructionKind::Statement => {
						if let Some(node) = &instruction.node {
								...
								node.parent().and_then(JsSwitchStatement::cast)
							{
								...
								// If this assertion fails, then it is likely a
								...
								// Register in-order the switch cases, but the
								// default clause which is inserted at
								// the end. This mimics the order of the
								// jumps in a blocks. The default
								// clause, if any is the last (unconditional)
								// jump.
					...
					InstructionKind::Jump { conditional, block: jump_block_id, .. } => {
						// If the block was already visited, skip it.
						// This avoid cycles and checking twice the same block.
						...
						// then any conditional jump is a case and an
						// unconditional jump is a default clause if the
						// switch has a default clause.
						if is_switch && (conditional || has_default_clause) {
							// Take the unconditional jump into account only if
							// a default clause is present.
...
fn register_fallthrough_switch_clauses(
	...
	while let Some(block_id) = block_stack.pop() {
		current_switch_clause =
			block_to_switch_clause_range.get(&block_id).or(current_switch_clause);
		let block = cfg.get(block_id);
		// Register exception handlers as blocks to process
