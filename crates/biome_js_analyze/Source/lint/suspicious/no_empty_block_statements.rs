...
impl Rule for NoEmptyBlockStatements {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let is_constructor_with_ts_param_props_or_private = is_constructor_with_ts_param_props_or_private(query);
...
fn is_constructor_with_ts_param_props_or_private(query: &Query) -> bool {
	...
	let Some(constructor) = body.syntax().parent().and_then(|node| node.cast::<JsConstructorClassMember>()) else {
		return false;
	};
