	...
	fn run(ctx:&RuleContext<Self>) -> Vec<Self::State> {
		...
		catch_clause
			...
			.and_then(|decl| {
				...
				// Only [JsIdentifierBinding] is allowed to use
				// `model.all_references` now, so there's need to make sure
				// this is a [JsIdentifierBinding].
				let identifier_binding =
					catch_binding.as_any_js_binding()?.as_js_identifier_binding()?;
