...
use biome_js_syntax::{AnyJsExpression, AnyJsLiteralExpression, JsBinaryExpression, JsSyntaxKind, JsUnaryOperator};
...
fn is_neg_zero(node: &AnyJsExpression) -> Option<bool> {
	match node {
		AnyJsExpression::JsUnaryExpression(expr) => {
			...
			if let AnyJsExpression::AnyJsLiteralExpression(AnyJsLiteralExpression::JsNumberLiteralExpression(expr)) =
				argument
			{
