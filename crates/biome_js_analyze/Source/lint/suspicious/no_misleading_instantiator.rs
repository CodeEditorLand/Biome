...
use biome_js_syntax::{
	AnyJsClassMember, AnyTsType, AnyTsTypeMember, ClassMemberName, JsClassDeclaration, JsSyntaxToken,
...
impl Rule for NoMisleadingInstantiator {
	...
	fn diagnostic(_ctx: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
		let diagnostic = RuleDiagnostic::new(rule_category!(), state.range(), state.message()).note(state.note());
		Some(diagnostic)
	}
}
...
fn check_interface_methods(decl: &TsInterfaceDeclaration) -> Option<RuleState> {
	let interface_ident = decl.id().ok()?.as_ts_identifier_binding()?.name_token().ok()?;
	for member in decl.members() {
		match member {
			AnyTsTypeMember::TsConstructSignatureTypeMember(construct) if construct.new_token().ok().is_some() => {
...
fn check_class_methods(js_class_decl: &JsClassDeclaration) -> Option<RuleState> {
	let class_ident = js_class_decl.id().ok()?.as_js_identifier_binding()?.name_token().ok()?;
...
fn extract_return_type_ident(reference_type: &TsReferenceType) -> Option<JsSyntaxToken> {
	reference_type.name().ok()?.as_js_reference_identifier()?.value_token().ok()
}
