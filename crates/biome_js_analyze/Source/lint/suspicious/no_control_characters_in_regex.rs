...
/// Collecting control characters for regex. The following characters in regular
/// expression patterns are considered as control characters:
/// - Hexadecimal character escapes from `\x00` to `\x1F`.
/// - Unicode character escapes from `\u0000` to `\u001F`.
			...
			b'\\' => {
				...
				match c {
					b'x' if (hex_index + 2) <= bytes.len() => {
						(decode_hex(&bytes[hex_index..(hex_index + 2)]), hex_index + 2)
					},
					b'u' if is_str_escape_seq || is_unicode_flag_set => {
						if matches!(iter.next(), Some((_, b'{'))) {
							...
							(decode_hex(&bytes[hex_index..(hex_index + 4)]), hex_index + 4)
						}
					},
...
fn collect_control_characters_from_expression(
	...
	if callee.as_js_reference_identifier().is_some_and(|name| name.has_name("RegExp")) {
		...
		let flags = args.next().and_then(|arg| arg.ok()?.as_any_js_expression()?.as_static_value());
		let flags = if let Some(StaticValue::String(flags)) = &flags { flags.text() } else { "" };
