...
fn decode_hex(digits: &[u8]) -> Option<u32> {
	str::from_utf8(digits).ok().and_then(|digits| u32::from_str_radix(digits, 16).ok())
}
	...
	while let Some((index, c)) = iter.next() {
		let decoded = match c {
			b'\\' => {
				...
				match c {
					b'x' => (decode_hex(&bytes[hex_index..(hex_index + 2)]), hex_index + 2),
					...
					b'u' => (decode_hex(&bytes[hex_index..(hex_index + 4)]), hex_index + 4),
