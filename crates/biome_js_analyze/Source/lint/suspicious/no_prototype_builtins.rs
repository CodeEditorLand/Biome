...
use biome_js_syntax::{
	AnyJsCallArgument, AnyJsExpression, AnyJsMemberExpression, JsCallExpression, JsSyntaxKind, T, TextRange,
};
...
impl Rule for NoPrototypeBuiltins {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		if member_name_text == "call" {
			...
			if obj_name_text == "hasOwnProperty" && has_left_hand_object(&obj_expr)? && is_global_object(ctx.model()) {
				return Some(RuleState {
...
fn build_has_own_expr(node: &JsCallExpression, is_callee_parenthesized: bool) -> AnyJsExpression {
	...
	AnyJsExpression::from(make::js_static_member_expression(
		make::js_identifier_expression(make::js_reference_identifier(make::ident(&object_identifier))).into(),
		make::token(T![.]),
