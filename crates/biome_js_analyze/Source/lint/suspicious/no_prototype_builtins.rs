...
fn is_prototype_builtins(token_text:&str) -> bool {
	matches!(token_text, "hasOwnProperty" | "isPrototypeOf" | "propertyIsEnumerable")
}
...
/// Checks if the given node is considered to be an access to a property of
/// `Object.prototype`. Returns `true` if `expression.object` is `Object`,
/// `Object.prototype`, or `{}` (empty `JsObjectExpression`).
...
/// Checks if previous token of the given node can be adjacent when applying the
/// action. Some tokens are not valid such as `return`, `in` and `of`.
///
/// # Example
/// ```js
