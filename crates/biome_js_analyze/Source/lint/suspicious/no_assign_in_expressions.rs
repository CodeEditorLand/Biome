...
use biome_js_syntax::{
	JsAssignmentExpression, JsExpressionStatement, JsForStatement, JsParenthesizedExpression, JsSequenceExpression,
};
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let mut ancestor = assign
			...
			.take_while(|x| {
				...
				JsAssignmentExpression::can_cast(x.kind()) || JsParenthesizedExpression::can_cast(x.kind())
			})
			.last()?;
