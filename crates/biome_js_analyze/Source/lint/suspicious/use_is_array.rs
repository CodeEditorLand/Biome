...
use biome_js_syntax::{AnyJsCallArgument, AnyJsExpression, JsInstanceofExpression, T, global_identifier};
...
impl Rule for UseIsArray {
	...
	fn action(ctx: &RuleContext<Self>, _: &Self::State) -> Option<JsRuleAction> {
		...
		let args = make::js_call_arguments(
			make::token(T!['(']).with_trailing_trivia_pieces(trim_leading_trivia_pieces(instanceof_trailing_trivia)),
		...
		mutation.replace_node_discard_trivia(AnyJsExpression::JsInstanceofExpression(node.clone()), call.into());
