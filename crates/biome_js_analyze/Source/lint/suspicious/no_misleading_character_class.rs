...
impl Message {
	fn diagnostic(&self) -> &str {
		match self {
			...
			Self::SurrogatePairWithoutUFlag => {
				"A character class cannot match a surrogate pair. Add the 'u' unicode flag to \
				 match against them."
			},
	...
	fn note(&self) -> &str {
		match self {
			Self::CombiningClassOrVs16 => {
				"A character and a combining character forms a new character. Replace the \
				 character class with an alternation."
			},
			Self::SurrogatePairWithoutUFlag => {
				"A surrogate pair forms a single codepoint, but is encoded as a pair of two \
				 characters. Without the unicode flag, the regex matches a single surrogate \
				 character."
			},

			Self::EmojiModifier => "Replace the character class with an alternation.",
			Self::RegionalIndicatorSymbol => {
				"A pair of regional indicator symbols encodes a country code. Replace the \
				 character class with an alternation."
			},

			Self::JoinedCharSequence => {
				"A zero width joiner composes several emojis into a new one. Replace the character \
				 class with an alternation."
			},
		}
	}
}
...
impl Rule for NoMisleadingCharacterClass {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		if is_regex_expr(callee)? {
			...
			let pattern = args.next()?.ok()?.as_any_js_expression()?.as_static_value()?;
			...
			let flags =
				args.next().and_then(|arg| arg.ok()?.as_any_js_expression()?.as_static_value());

			let flags =
				if let Some(StaticValue::String(flags)) = &flags { flags.text() } else { "" };
...
/// - surrogate pair: \uXXXX\uXXXX If the unicode escape sequence is not valid,
///   it will be treated as a simple string.
///
/// ```example
/// \uD83D\uDC4D -> 👍
/// \u0041\u0301 -> Á
...
fn decode_next_codepoint(char_class:&str, is_in_string:bool) -> Option<(u32, usize)> {
	...
	// `\u{XXX}`
	// `\uXXXX`
	let Some(UnicodeEscape { kind, codepoint, len }) =
	...
	if kind != UnicodeEscapeKind::RegexBraced
		...
		&& len <= char_class.len()
	{
		if let Some(UnicodeEscape { kind: low_kind, codepoint: low_codepoint, len: low_len }) =
...
fn decode_unicode_escape_sequence(s:&str, is_in_string:bool) -> Option<UnicodeEscape> {
	...
	let (offset, is_regex_escape) =
		if is_in_string && bytes[1] == b'\\' { (1, true) } else { (0, !is_in_string) };
	...
	if bytes[offset + 2] == b'{' {
		let (end, _) = bytes.iter().enumerate().skip(offset + 3).find(|(_, &c)| c == b'}')?;
		...
		Some(UnicodeEscape {
			// We use `get` for slicing to handle malformed escape sequence that end with a
			// multi-byte char.
...
fn make_suggestion(
	...
	let suggestion = match flag {
		None => {
			Some(AnyJsCallArgument::AnyJsExpression(AnyJsExpression::AnyJsLiteralExpression(
				...
				AnyJsCallArgument::AnyJsExpression(expr) => {
					match expr {
									...
									AnyJsExpression::AnyJsLiteralExpression(
										AnyJsLiteralExpression::JsStringLiteralExpression(
											make::js_string_literal_expression(
												make::js_string_literal(&format!("'{text}u'")),
											),
										),
									),
								))
						...
						AnyJsExpression::JsTemplateExpression(expr) => {
							let mut elements =
								expr.elements().iter().collect::<Vec<AnyJsTemplateElement>>();

							let uflag = AnyJsTemplateElement::from(
								make::js_template_chunk_element(make::js_template_chunk("u")),
							);
...
mod tests {
	...
	fn test_decode_unicode_escape_sequence() {
		...
		assert_eq!(
			...
			Some(UnicodeEscape { codepoint:0x31, kind:UnicodeEscapeKind::RegexPlain, len:6 })
		);

		assert_eq!(
			decode_unicode_escape_sequence(r"\u0031 test", true),
			Some(UnicodeEscape { codepoint:0x31, kind:UnicodeEscapeKind::String, len:6 })
		);

		assert_eq!(
			decode_unicode_escape_sequence(r"\\u0031 test", true),
			Some(UnicodeEscape { codepoint:0x31, kind:UnicodeEscapeKind::RegexPlain, len:7 })
		);

		assert_eq!(
			decode_unicode_escape_sequence(r"\u{31} test", false),
			Some(UnicodeEscape { codepoint:0x31, kind:UnicodeEscapeKind::RegexBraced, len:6 })
		);
		...
		assert_eq!(
			decode_unicode_escape_sequence(r"\\u{31} test", true),
			Some(UnicodeEscape { codepoint:0x31, kind:UnicodeEscapeKind::RegexBraced, len:7 })
		);

		assert_eq!(
			decode_unicode_escape_sequence(r"\u{1} test", false),
			Some(UnicodeEscape { codepoint:1, kind:UnicodeEscapeKind::RegexBraced, len:5 })
		);
	}
	...
	fn test_decode_next_codepoint() {
		...
		assert_eq!(decode_next_codepoint(r"\u0031\u0031", false), Some((0x31, 6)));

		assert_eq!(decode_next_codepoint(r"\u0031 test", true), Some((0x31, 6)));

		assert_eq!(decode_next_codepoint(r"\\u0031 test", true), Some((0x31, 7)));

		assert_eq!(decode_next_codepoint(r"\u{31}\u{31}", false), Some((0x31, 6)));

		assert_eq!(decode_next_codepoint(r"\u{31} test", true), Some((0x31, 6)));

		assert_eq!(decode_next_codepoint(r"\\u{31} test", true), Some((0x31, 7)));

		// Surrogate pairs
		assert_eq!(decode_next_codepoint(r"\uD83D\uDC4D", false), Some(('👍' as u32, 12)));

		assert_eq!(decode_next_codepoint(r"\uD83D\uDC4D", true), Some(('👍' as u32, 12)));

		assert_eq!(decode_next_codepoint(r"\\uD83D\\uDC4D", true), Some(('👍' as u32, 14)));

		assert_eq!(decode_next_codepoint(r"\u{D83D}\u{DC4D}", true), Some(('👍' as u32, 16)));

		assert_eq!(decode_next_codepoint(r"\uD83D\u{DC4D}", true), Some(('👍' as u32, 14)));

		assert_eq!(decode_next_codepoint(r"\u{D83D}\uDC4D", true), Some(('👍' as u32, 14)));

		// Lone high surrogate
		assert_eq!(decode_next_codepoint(r"\u{D83D}\u{DC4D}", false), Some((0xD83D, 8)));

		assert_eq!(decode_next_codepoint(r"\\u{D83D}\\u{DC4D}", true), Some((0xD83D, 9)));

		assert_eq!(decode_next_codepoint(r"\\uD83D\\u{DC4D}", true), Some((0xD83D, 7)));

		assert_eq!(decode_next_codepoint(r"\\u{D83D}\\uDC4D", true), Some((0xD83D, 9)));

		assert_eq!(decode_next_codepoint(r"\u{D83D}\\uDC4D", true), Some((0xD83D, 8)));

		assert_eq!(decode_next_codepoint(r"\uD83D\\uDC4D", true), Some((0xD83D, 6)));
	}
}
