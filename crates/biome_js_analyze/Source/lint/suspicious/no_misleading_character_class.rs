...
impl Rule for NoMisleadingCharacterClass {
	...
	fn diagnostic(_ctx: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
		Some(RuleDiagnostic::new(rule_category!(), state.range, state.message.as_str()))
	}
		...
		match ch {
			...
			_ if is_in_character_class && i < regex_bytes_len => {
				if !has_u_flag && has_surrogate_pair(&regex_pattern[i..]) {
					return Some(RuleState { range, message: Message::SurrogatePairWithoutUFlag });
				}

				if has_combining_class_or_vs16(&regex_pattern[i..]) {
					return Some(RuleState { range, message: Message::CombiningClassOrVs16 });
				}

				if has_regional_indicator_symbol(&regex_pattern[i..]) {
					return Some(RuleState { range, message: Message::RegionalIndicatorSymbol });
				}

				if has_emoji_modifier(&regex_pattern[i..]) {
					return Some(RuleState { range, message: Message::EmojiModifier });
				}

				if zwj(&regex_pattern[i..]) {
					return Some(RuleState { range, message: Message::JoinedCharSequence });
				}
			}
			_ => {}
		}
...
fn make_suggestion(
	...
	let suggestion = match flag {
		None => Some(AnyJsCallArgument::AnyJsExpression(AnyJsExpression::AnyJsLiteralExpression(
			AnyJsLiteralExpression::JsStringLiteralExpression(make::js_string_literal_expression(
				make::js_string_literal("u"),
			)),
		))),
		Some(f) => match f {
			AnyJsCallArgument::AnyJsExpression(expr) => match expr {
				...
				AnyJsExpression::JsTemplateExpression(expr) => {
					...
					elements.push(uflag);
					Some(AnyJsCallArgument::AnyJsExpression(AnyJsExpression::JsTemplateExpression(
...
fn handle_braced_escape_sequence(
	...
	u32::from_str_radix(&codepoint_str, 16).ok().and_then(char::from_u32).map(|c| c.to_string())
}
...
mod tests {
	...
	fn test_replace_escaped_unicode() {
		...
		assert_eq!(replace_escaped_unicode(r#"/[\u899\uD83D\uDC4D]/"#), r#"/[\u899👍]/"#);
		assert_eq!(
