...
impl Message {
	fn diagnostic(&self) -> &str {
		match self {
			...
			Self::RegionalIndicatorSymbol => "A character class cannot match a pair of regional indicator symbols.",
...
impl Rule for NoMisleadingCharacterClass {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let RuleState { range, message } = diagnostic_regex_pattern(pattern.text(), flags.text(), false)?;
		Some(RuleState { range: range.checked_add(node.range().start().checked_add(1.into())?)?, message })
	}

	fn diagnostic(_ctx: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
		Some(RuleDiagnostic::new(rule_category!(), state.range, state.message.diagnostic()).note(state.message.note()))
	}
	...
	fn action(ctx: &RuleContext<Self>, state: &Self::State) -> Option<JsRuleAction> {
		...
		let next_token = JsSyntaxToken::new_detached(JsSyntaxKind::JS_REGEX_LITERAL, &format!("{text}u"), [], []);
...
fn diagnostic_regex_pattern(regex_pattern: &str, flags: &str, is_in_string: bool) -> Option<RuleState> {
	if flags.contains('v') {
		return None;
	}
				...
				while let Some((j, byte)) = bytes_iter.next() {
					match byte {
						...
						b']' => {
							...
							{
								return Some(RuleState { range: range.checked_add(((i + 1) as u32).into())?, message });
							}
							break;
						},
						_ => {},
...
fn diagnostic_regex_class(char_class: &str, has_u_flag: bool, is_in_string: bool) -> Option<RuleState> {
	...
	while let Some((i, c)) = iter.next() {
		...
		let (codepoint, end) = if c == '\\' {
			...
			let Some((codepoint, len)) = decode_next_codepoint(&char_class[i..], is_in_string) else {
				prev_char_index = i;
				continue;
			};
...
fn decode_next_codepoint(char_class: &str, is_in_string: bool) -> Option<(u32, usize)> {
	...
	// `\u{XXX}`
	// `\uXXXX`
	let Some(UnicodeEscape { kind, codepoint, len }) = decode_unicode_escape_sequence(char_class, is_in_string) else {
		// Ignore the escape sequence
		return Some((c as u32, c.len_utf8()));
	};
	if kind != UnicodeEscapeKind::RegexBraced && matches!(codepoint, 0xD800..=0xDBFF) && len <= char_class.len() {
		...
		{
			let (final_codepoint, final_len) = if kind == low_kind && matches!(low_codepoint, 0xDC00..=0xDFFF) {
...
fn decode_unicode_escape_sequence(s: &str, is_in_string: bool) -> Option<UnicodeEscape> {
	...
	let (offset, is_regex_escape) = if is_in_string && bytes[1] == b'\\' { (1, true) } else { (0, !is_in_string) };
