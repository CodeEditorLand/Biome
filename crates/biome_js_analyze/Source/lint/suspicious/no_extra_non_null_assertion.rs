...
impl Rule for NoExtraNonNullAssertion {
	...
	fn diagnostic(ctx: &RuleContext<Self>, _: &Self::State) -> Option<RuleDiagnostic> {
		let diagnostic =
			RuleDiagnostic::new(rule_category!(), ctx.query().range(), "Forbidden extra non-null assertion.");
	...
	fn action(ctx: &RuleContext<Self>, _: &Self::State) -> Option<JsRuleAction> {
		...
		let excl_token = match node {
			AnyTsNonNullAssertion::TsNonNullAssertionAssignment(assignment) => assignment.excl_token().ok()?,
			AnyTsNonNullAssertion::TsNonNullAssertionExpression(expression) => expression.excl_token().ok()?,
		};
