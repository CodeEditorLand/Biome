...
use biome_js_syntax::{
	AnyJsExpression, AnyJsLiteralExpression, JsBinaryExpression, JsBinaryOperator, JsLogicalOperator, JsUnaryOperator,
};
...
impl Rule for UseValidTypeof {
	...
	fn run(ctx: &RuleContext<Self>) -> Option<Self::State> {
		...
		let other = match (left, right) {
			(AnyJsExpression::JsUnaryExpression(unary), other) if unary.operator().ok()? == JsUnaryOperator::Typeof => {
				other
			},
			(other, AnyJsExpression::JsUnaryExpression(unary)) if unary.operator().ok()? == JsUnaryOperator::Typeof => {
				other
			},
			_ => {
				return None;
		...
		match other {
			...
			AnyJsExpression::JsUnaryExpression(unary) if unary.operator() == Ok(JsUnaryOperator::Typeof) => None,
			// `typeof foo == f() ?? g()`
	...
	fn action(ctx: &RuleContext<Self>, other: &Self::State) -> Option<JsRuleAction> {
		...
		mutation.replace_node(
			other.clone(),
			AnyJsExpression::AnyJsLiteralExpression(AnyJsLiteralExpression::from(make::js_string_literal_expression(
