...
impl Rule for UseValidTypeof {
	...
	fn run(ctx:&RuleContext<Self>) -> Option<Self::State> {
		...
		let JsBinaryExpressionFields { left, operator_token: _, right } = n.as_fields();
		...
		let range = match (&left, &right) {
			...
			| (
				...
				if let AnyJsLiteralExpression::JsStringLiteralExpression(literal) = literal {
					...
					// Try to fix the casing of the literal eg. "String" ->
					// "string"
			...
			| (
				...
				// Try to convert the identifier to a string literal eg. String
				// -> "string"
