use biome_analyze::{
	Ast, FixKind, QueryMatch, Rule, RuleDiagnostic, RuleSource, context::RuleContext, declare_lint_rule,
};
...
impl Rule for UseCollapsedIf {
	...
	fn action(ctx: &RuleContext<Self>, state: &Self::State) -> Option<JsRuleAction> {
		...
		let mut expr = make::js_logical_expression(parent_test.clone(), operator, child_test.clone());
