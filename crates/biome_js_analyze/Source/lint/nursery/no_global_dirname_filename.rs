...
use biome_js_syntax::{
	AnyJsExpression, AnyJsName, AnyJsObjectMember, JsFileSource, JsObjectExpression, JsPropertyObjectMember,
	JsStaticMemberExpression, JsSyntaxKind, JsSyntaxToken, JsVariableDeclarator, global_identifier,
};
		...
		match node {
			AnyGlobalDirnameFileName::JsVariableDeclarator(declarator) => {
				mutation.replace_node(
					...
					AnyJsExpression::JsStaticMemberExpression(make_import_meta(dirname_or_filename)),
				);
			},
			AnyGlobalDirnameFileName::JsObjectExpression(object_expr) => {
				for member in object_expr.members().iter().flatten() {
					match member {
						AnyJsObjectMember::JsPropertyObjectMember(member) => {
							...
							if &id == syntax_token {
								...
								let property_member =
									make_property_object_member(&key.value().ok()?, dirname_or_filename);
						...
						AnyJsObjectMember::JsShorthandPropertyObjectMember(member) => {
							...
							if &key == syntax_token {
								let property_member = make_property_object_member(&key, dirname_or_filename);
								mutation.replace_node(
									AnyJsObjectMember::JsShorthandPropertyObjectMember(member.clone()),
...
fn validate_dirname_filename(expr: &AnyJsExpression, model: &SemanticModel) -> Option<(JsSyntaxToken, String)> {
...
fn make_property_object_member(key: &JsSyntaxToken, import_meta_property: &str) -> JsPropertyObjectMember {
	make::js_property_object_member(
		biome_js_syntax::AnyJsObjectMemberName::JsLiteralMemberName(make::js_literal_member_name(make::ident(
			key.text_trimmed(),
		))),
