...
impl Rule for UseTrimStartEnd {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		if !args.is_empty() {
			// If arguments are present, it suggests this function call may not
			// be intended for `String.trimStart()` or `String.trimEnd()`, as
			// these methods do not accept parameters according to the
			// specification:
			...
			AnyJsExpression::AnyJsLiteralExpression(
				AnyJsLiteralExpression::JsStringLiteralExpression(
					make::js_string_literal_expression(JsSyntaxToken::new_detached(
						...
						// make::js_string_literal() call the function that
