...
impl Rule for UseTrimStartEnd {
	...
	fn diagnostic(_: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
		...
		Some(RuleDiagnostic::new(rule_category!(), state.span, diagnostic_message).note(note_message))
	}
	...
	fn action(ctx: &RuleContext<Self>, state: &Self::State) -> Option<JsRuleAction> {
		...
		let template_elements = AnyJsTemplateElement::from(make::js_template_chunk_element(make::js_template_chunk(
			replaced_member_name.as_str(),
		)));
		...
		let call_expression = if is_computed_member {
			...
			AnyJsExpression::JsStaticMemberExpression(make::js_static_member_expression(
				callee_object?,
				callee.as_js_static_member_expression()?.operator_token().ok()?,
...
fn extract_token_from_expression(callee: AnyJsExpression) -> Option<SyntaxToken<JsLanguage>> {
	let token = if let AnyJsExpression::JsComputedMemberExpression(expression) = callee {
		...
		match member {
			...
			AnyJsExpression::JsTemplateExpression(element) => element.elements().into_iter().find_map(|x| {
