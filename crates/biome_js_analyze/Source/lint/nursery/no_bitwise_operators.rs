...
use biome_js_syntax::{
	JsAssignmentExpression, JsAssignmentOperator, JsBinaryExpression, JsBinaryOperator, JsSyntaxToken,
...
impl AnyExpressionWithBitwise {
	...
	fn exist_bitwise_op(&self) -> bool {
		match self {
			...
			AnyExpressionWithBitwise::JsUnaryExpression(unary_expr) => {
				unary_expr.operator().ok().is_some_and(|op| op == JsUnaryOperator::BitwiseNot)
			},
...
impl Rule for NoBitwiseOperators {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		if expr.exist_bitwise_op() {
			...
			if ctx.options().allow.iter().any(|op_str| op_str.as_ref() == op.text_trimmed()) {
				return None;
			}
			return Some(op);
		}
