...
impl Rule for NoTsIgnore {
				...
				.filter_map(|trivia| {
					if let Some(comment) = trivia.as_comments() {
						if let Some((index, _)) = comment.text().match_indices("@ts-ignore").next() {
							return Some((token.clone(), comment.text_range().add_start(TextSize::from(index as u32))));
						}
					}
					None
				})
	...
	fn action(ctx: &RuleContext<Self>, state: &Self::State) -> Option<JsRuleAction> {
		...
		Some(JsRuleAction::new(
			...
			markup! { "Use the "<Emphasis>"@ts-expect-error"</Emphasis>" directive instead." }.to_owned(),
			mutation,
		))
	}
}
