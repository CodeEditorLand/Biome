...
use biome_js_syntax::{
	AnyJsExpression, AnyJsFunctionBody, JsArrayBindingPatternElement, JsArrowFunctionExpression, JsFormalParameter,
	JsObjectBindingPatternShorthandProperty, JsReturnStatement, JsVariableStatement, JsYieldArgument, T,
};
...
fn find_undefined_range(expr: Option<&AnyJsExpression>) -> Option<TextRange> {
	...
	if ident.is_undefined() { Some(ident.range()) } else { None }
}
...
impl Rule for NoUselessUndefined {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		match node {
					...
					if let Some(undefined_range) = find_undefined_range(expr.as_ref()) {
						...
						if let Some(binding) = binding_text.as_any_js_binding() {
							if let Some(ident_binding) = binding.as_js_identifier_binding() {
								let binding_text = ident_binding.name_token().map(|t| t.token_text_trimmed()).ok();
								signals.push(RuleState { binding_text, diagnostic_range: undefined_range });
							}
						}
					}
				}
			...
			// { a: undefined }
			AnyUndefinedNode::JsObjectBindingPatternShorthandProperty(js_object_binding_pattern_shorthand_property) => {
				if let Some(init) = js_object_binding_pattern_shorthand_property.init() {
					...
					if let Some(range) = find_undefined_range(expr.as_ref()) {
						signals.push(RuleState { binding_text: None, diagnostic_range: range });
					}
				}
			},
			// function foo([bar = undefined]) {}
	...
	fn action(ctx: &RuleContext<Self>, state: &Self::State) -> Option<JsRuleAction> {
		...
		match node {
			AnyUndefinedNode::JsVariableStatement(js_variable_statement) => {
				...
				let current_declaration = declarators.into_iter().filter_map(Result::ok).find(|decl| {
					decl.id()
						.is_ok_and(|id| id.syntax().text_trimmed() == state.binding_text.as_deref().unwrap_or(""))
				})?;
			...
			AnyUndefinedNode::JsArrowFunctionExpression(js_arrow_function_expression) => {
				...
				mutation.replace_node_discard_trivia(undefined_body, AnyJsFunctionBody::JsFunctionBody(next_node));
			},
			AnyUndefinedNode::JsFormalParameter(js_formal_parameter) => {
