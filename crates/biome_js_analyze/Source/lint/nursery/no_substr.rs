...
impl Rule for NoSubstr {
	...
	fn action(ctx:&RuleContext<Self>, _:&Self::State) -> Option<JsRuleAction> {
		...
		if !args.is_empty() {
			// If the function has arguments, we cannot replace it with slice()
			// as it has different behavior.
...
impl NoSubstrState {
	fn member_name(&self) -> TokenText { self.value_token.token_text_trimmed() }

	fn span(&self) -> TextRange { self.value_token.text_range() }
}

// Helper union type to handle both JsExpressionStatement and
// JsVariableStatement. To handle arguments, we need to know the type of the
// statement.
declare_node_union! {
...
impl AnyJsStatement {
	pub fn value_token(&self) -> Option<JsSyntaxToken> {
		match self {
			AnyJsStatement::JsExpressionStatement(node) => {
				let callee = node.expression().ok()?.as_js_call_expression()?.callee().ok()?;
				callee.as_js_static_member_expression()?.member().ok()?.value_token().ok()
			},
	...
	pub fn member(&self) -> Option<AnyJsName> {
		match self {
			...
			AnyJsStatement::JsVariableStatement(node) => {
				...
				declarators.into_iter().find_map(|declarator| {
					let init = declarator.ok()?.initializer()?;
					init.expression().ok()?.as_js_static_member_expression()?.member().ok()
				})
			},
		}
	}
	...
	pub fn arguments(&self) -> Option<JsCallArguments> {
		match self {
			AnyJsStatement::JsExpressionStatement(node) => {
				node.expression().ok()?.as_js_call_expression()?.arguments().ok()
			},
			_ => None,
		}
	}
