...
fn get_restricted_import(range: TextRange, module_path: &TokenText) -> Option<NoPackagePrivateImportsState> {
	...
	let is_restricted = path_parts.iter().filter(|&&part| part != "." && part != "..").count() > 1;
	if !is_restricted {
		return None;
	}
	...
	Some(NoPackagePrivateImportsState { range, path: path_parts.join("/"), suggestion: suggestion_parts.join("/") })
}
