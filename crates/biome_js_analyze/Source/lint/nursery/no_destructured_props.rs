...
use biome_analyze::{
	Rule, RuleDiagnostic, RuleDomain, RuleSource, RuleSourceKind, context::RuleContext, declare_lint_rule,
};
use biome_console::markup;
use biome_js_semantic::SemanticModel;
use biome_js_syntax::{
	AnyJsArrayBindingPatternElement, AnyJsBinding, AnyJsBindingPattern, AnyJsObjectBindingPatternMember,
	JsArrowFunctionExpression, JsLanguage, JsObjectBindingPattern, JsParameters, JsVariableDeclarator,
...
impl Rule for NoDestructuredProps {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let model = ctx.model();
		let Some(parameters) = binding_pattern.syntax().ancestors().find_map(JsParameters::cast) else {
			return vec![];
		};
		...
		if is_inside_jsx_component(&parameters).unwrap_or_default() {
			...
			if properties.len() == 0 {
				...
				let iter =
					BindingPatterIterator::new(BindingPatternLikeList::Object(binding_pattern.properties().iter()));
				for binding in iter {
...
fn is_inside_jsx_component(parameters: &JsParameters) -> Option<bool> {
	let arrow_function_expression = parameters.syntax().parent().and_then(JsArrowFunctionExpression::cast)?;
...
fn is_binding_a_jsx_prop(binding: &AnyJsBinding, model: &SemanticModel) -> Option<TextRange> {
	if let Some(binding) = binding.as_js_identifier_binding().map(|b| model.as_binding(b)) {
						...
						AnyJsArrayBindingPatternElement::JsArrayBindingPatternElement(node) => {
							...
							match pattern {
								...
								AnyJsBindingPattern::JsObjectBindingPattern(pattern) => {
									self.queue
										.push_back(BindingPatternLikeList::Object(pattern.properties().iter()));
									None
								},
							}
						},
