...
impl Rule for UseAriaPropsSupportedByRole {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let role_prohibited_attributes = role.map_or(Default::default(), |role| role.prohibited_attributes());

		for attribute in node.attributes() {
			let AnyJsxAttribute::JsxAttribute(attribute) = attribute else {
				continue;
			};
			let aria_attribute = attribute
				.name()
				.ok()
				.and_then(|x| AriaAttribute::from_str(x.as_jsx_name()?.value_token().ok()?.text_trimmed()).ok());
