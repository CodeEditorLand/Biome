...
//! They are a starting point that can be extended (e.g. by adding custom
...
pub fn get_empty_preset() -> ConfigPreset {
	ConfigPreset { utilities:[].as_slice(), variants:[].as_slice() }
}

pub fn get_tailwind_css_preset() -> ConfigPreset {
	ConfigPreset { utilities:TAILWIND_LAYERS.as_slice(), variants:VARIANT_CLASSES.as_slice() }
}
