...
/// `None` is returned.
fn get_utility_info(utility_config: &[UtilityLayer], utility_data: &ClassSegmentStructure) -> Option<UtilityInfo> {
...
mod get_utility_info_tests {
	...
	fn test_arbitrary_layer() {
		let utility_config = vec![UtilityLayer { name: "layer", classes: &["border-t-", "border-"] }];
		let utility_data = ClassSegmentStructure { text: "[arbitrary:css]".to_string(), arbitrary: true };
		assert_eq!(
			get_utility_info(utility_config.as_slice(), &utility_data),
...
pub fn get_class_info(class_name: &str, sort_config: &SortConfig) -> Option<ClassInfo> {
	...
	let (arbitrary_variants, current_variants): (Vec<&ClassSegmentStructure>, Vec<&ClassSegmentStructure>) =
		utility_data.variants.iter().partition(|el| el.arbitrary);

	let arbitrary_variants: Vec<String> = arbitrary_variants.iter().map(|&variant| variant.text.clone()).collect();
	...
	if let Some(utility_info) = utility_info {
		return Some(ClassInfo {
			...
			arbitrary_variants: if arbitrary_variants.is_empty() { None } else { Some(arbitrary_variants) },
		});
	}
...
mod get_class_info_tests {
	...
	use super::*;
	use crate::lint::nursery::use_sorted_classes::{presets::ConfigPreset, sort_config::UtilityLayer};
