//! Each CSS class needs to be processed to determine the information that will
//! be used to sort it. This information includes:
//! - The layer it belongs to (e.g. `components` or `utilities`).
//! - The index of the utility within the layer.
//! - The total variants weight that results from the combination of all the
//!   variants.
//! - The text of the class itself.
//! - The arbitrary variants of the class. It is generated according to the
//!   information contained in a `SortConfig`, which includes:
//! - The list of layers, in order.
//! - The list of utilities, in order, for each layer.
//! - The list of variants, in order of importance (which is used to compute the
//!   variants weight).
...
impl From<(&str, &str)> for UtilityMatch {
	...
	fn from((target, utility_text):(&str, &str)) -> UtilityMatch {
		...
		if target.ends_with('$') {
			// Check if the utility matches the target (without the final `$`)
			// exactly.
...
mod utility_match_tests {
	...
	fn test_exact_match() {
		...
		// assert_eq!(UtilityMatch::from(("px-2$", "-px-2")),
		// UtilityMatch::Exact);
	...
	fn test_partial_match() {
		...
		// TODO: support negative values
		// assert_eq!(UtilityMatch::from(("px-", "-px-2")),
		// UtilityMatch::Partial);
		assert_eq!(UtilityMatch::from(("px-", "px-2.5")), UtilityMatch::Partial);
		assert_eq!(UtilityMatch::from(("px-", "px-anything")), UtilityMatch::Partial);
		assert_eq!(UtilityMatch::from(("px-", "px-%$>?+=-")), UtilityMatch::Partial);
...
/// Computes sort-related information about a CSS utility. If the utility is not
...
fn get_utility_info(
	...
	// This layer is always at the end, and the order of the utilities in it is
	// not determined at this point, so they all have the same index.
	if utility_data.arbitrary {
		return Some(UtilityInfo { layer:"arbitrary", index:0 });
	}
		...
		for (index, &target) in layer_data.classes.iter().enumerate() {
			match UtilityMatch::from((target, utility_text)) {
				UtilityMatch::Exact => {
					...
					return Some(UtilityInfo { layer:layer_data.name, index });
				},
				UtilityMatch::Partial => {
					// Multiple partial matches can occur, so we need to keep
					// looking to find the longest target that matches. For
					// example, if the utility text is `gap-x-4`, and there
					// are targets like `gap-` and `gap-x-`, we want to
					// make sure that the `gap-x-` target is matched as it is
					// more specific, regardless of the order in which the
					// targets are defined.
	...
	if let Some(layer_match) = layer {
		return Some(UtilityInfo { layer:layer_match, index:match_index });
	}
	None
}
...
mod get_utility_info_tests {
	...
	#[test]
	fn test_exact_match() {
		let utility_config = vec![UtilityLayer { name:"layer", classes:&["px-2$"] }];
		let utility_data = ClassSegmentStructure { text:"px-2".to_string(), arbitrary:false };
		assert_eq!(
			get_utility_info(utility_config.as_slice(), &utility_data),
			Some(UtilityInfo { layer:"layer", index:0 })
		);
		let utility_data = ClassSegmentStructure { text:"px-4".to_string(), arbitrary:false };
		assert_eq!(get_utility_info(utility_config.as_slice(), &utility_data), None);
	}

	#[test]
	fn test_partial_match() {
		let utility_config = vec![UtilityLayer { name:"layer", classes:&["px-"] }];
		let utility_data = ClassSegmentStructure { text:"px-2".to_string(), arbitrary:false };
		assert_eq!(
			get_utility_info(utility_config.as_slice(), &utility_data),
			Some(UtilityInfo { layer:"layer", index:0 })
		);
		let utility_data = ClassSegmentStructure { text:"not-px-2".to_string(), arbitrary:false };
		assert_eq!(get_utility_info(utility_config.as_slice(), &utility_data), None);
	}

	#[test]
	fn test_partial_match_longest() {
		let utility_config = vec![UtilityLayer { name:"layer", classes:&["border-", "border-t-"] }];
		let utility_data = ClassSegmentStructure { text:"border-t-2".to_string(), arbitrary:false };
		assert_eq!(
			get_utility_info(utility_config.as_slice(), &utility_data),
			Some(UtilityInfo { layer:"layer", index:1 })
		);
	}

	#[test]
	fn test_partial_match_longest_first() {
		let utility_config = vec![UtilityLayer { name:"layer", classes:&["border-t-", "border-"] }];
		let utility_data = ClassSegmentStructure { text:"border-t-2".to_string(), arbitrary:false };
		assert_eq!(
			get_utility_info(utility_config.as_slice(), &utility_data),
			Some(UtilityInfo { layer:"layer", index:0 })
	...
	fn test_arbitrary_layer() {
		let utility_config = vec![UtilityLayer { name:"layer", classes:&["border-t-", "border-"] }];
		let utility_data =
			ClassSegmentStructure { text:"[arbitrary:css]".to_string(), arbitrary:true };
		assert_eq!(
			get_utility_info(utility_config.as_slice(), &utility_data),
			Some(UtilityInfo { layer:"arbitrary", index:0 })
		);
	}
}
...
impl From<(&str, &str)> for VariantMatch {
	...
	fn from((target, variant_text):(&str, &str)) -> VariantMatch {
		...
		// Checks if variant text has a custom value thus it starts with the
		// target and it's followed by "-["
...
mod variant_match_tests {
	...
	fn test_exact_match() {
		...
		assert_eq!(VariantMatch::from(("group", "group-[.is-published]")), VariantMatch::Exact);
		assert_eq!(VariantMatch::from(("has", "has-[:checked]")), VariantMatch::Exact);
		assert_eq!(
	...
	fn test_partial_match() {
		...
		assert_eq!(VariantMatch::from(("peer", "peer-has-[:checked]")), VariantMatch::Partial);
	}
	...
	for (index, &target) in config_variants.iter().enumerate() {
		match VariantMatch::from((target, variant_text)) {
			...
			VariantMatch::Partial => {
				// Multiple partial matches can occur, so we need to keep
				// looking to find the longest target that matches. For
				// example, if the variant text is `group-aria-[.
				// custom-class]`, and there are targets like `group` and
				// `group-aria`, we want to make sure that the `group-aria`
				// target is matched as it is more specific, so when the
				// target is `group` a Partial match will occur.
				let target_size = target.len();
				if target_size > last_size {
					variant = Some(target);
					match_index = index;
...
pub fn compute_variants_weight(
	...
	// If it is then compute weights for each variant on the fly by using index
	// as size TODO: Cache the weights for next run?
	...
	// Compute Variants Weight as the BitWise XOR of all the recognized
	// variants' weights
	let variants_weight = variants_map.iter().fold(BitVec::<u8, Lsb0>::new(), |acc, (_, val)| {
...
/// Computes sort-related information about a CSS class. If the class is not
/// recognized as a utility, it is considered a custom class instead and `None`
/// is returned.
pub fn get_class_info(class_name:&str, sort_config:&SortConfig) -> Option<ClassInfo> {
	...
	let arbitrary_variants:Vec<String> =
		arbitrary_variants.iter().map(|&variant| variant.text.clone()).collect();
...
mod get_class_info_tests {
	...
	use crate::lint::nursery::use_sorted_classes::{
		...
		sort_config::UtilityLayer,
	};

	#[test]
	fn test_get_class_info() {
		const UTILITIES_CONFIG:[UtilityLayer; 2] = [
			UtilityLayer { name:"layer0", classes:&["px-", "py-", "block$", "bg-"] },
			UtilityLayer { name:"layer1", classes:&["mx-", "my-", "inline$"] },
		];
		let variants:&'static [&'static str; 4] = &["hover", "focus", "focus-visible", "active"];

		let sort_config = SortConfig::new(&ConfigPreset { utilities:&UTILITIES_CONFIG, variants });
