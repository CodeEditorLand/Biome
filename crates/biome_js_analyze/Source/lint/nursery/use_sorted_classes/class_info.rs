...
impl From<(&str, &str)> for VariantMatch {
	...
	fn from((target, variant_text):(&str, &str)) -> VariantMatch {
		...
		// Checks if variant text has a custom value thus it starts with the target and
		// it's followed by "-["
	...
	for (index, &target) in config_variants.iter().enumerate() {
		match VariantMatch::from((target, variant_text)) {
			...
			VariantMatch::Partial => {
				...
				// `group-aria-[.custom-class]`, and there are targets like `group` and
				// `group-aria`, we want to make sure that the `group-aria` target is
				// matched as it is more specific, so when the target is `group` a Partial
				// match will occur.
				let target_size = target.len();
...
pub fn compute_variants_weight(
	...
	// If it is then compute weights for each variant on the fly by using index as
	// size TODO: Cache the weights for next run?
	...
	// Compute Variants Weight as the BitWise XOR of all the recognized variants'
	// weights
