...
mod get_utility_info_tests {
	...
	fn test_partial_match_longest() {
		let utility_config =
			vec![UtilityLayer { name: "layer", classes: &["border-", "border-t-"] }];
		let utility_data =
			ClassSegmentStructure { text: "border-t-2".to_string(), arbitrary: false };
		assert_eq!(
			get_utility_info(utility_config.as_slice(), &utility_data),
	...
	fn test_partial_match_longest_first() {
		let utility_config =
			vec![UtilityLayer { name: "layer", classes: &["border-t-", "border-"] }];
		let utility_data =
			ClassSegmentStructure { text: "border-t-2".to_string(), arbitrary: false };
		assert_eq!(
			get_utility_info(utility_config.as_slice(), &utility_data),
