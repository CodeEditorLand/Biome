...
use biome_js_syntax::{
	AnyJsExpression, JsCallArguments, JsCallExpression, JsLiteralMemberName, JsStringLiteralExpression, JsSyntaxNode,
...
fn get_attribute_name(attribute: &JsxAttribute) -> Option<TokenText> {
	Some(attribute.name().ok()?.as_jsx_name()?.value_token().ok()?.token_text_trimmed())
}
...
fn inspect_string_literal(node: &JsSyntaxNode, options: &UtilityClassSortingOptions) -> Option<bool> {
	...
	pub(crate) fn should_visit(&self, options: &UtilityClassSortingOptions) -> Option<bool> {
		match self {
			...
			AnyClassStringLike::JsxString(jsx_string) => {
				let jsx_attribute = jsx_string.syntax().ancestors().skip(1).find_map(JsxAttribute::cast)?;
			...
			AnyClassStringLike::JsTemplateChunkElement(template) => {
				for ancestor in template.syntax().ancestors().skip(1) {
					if let Some(template_expression) = JsTemplateExpression::cast_ref(&ancestor) {
						if let Some(AnyJsExpression::JsIdentifierExpression(tag)) = template_expression.tag() {
							let name = tag.name().ok()?.name().ok()?;
							if options.has_function(name.text()) {
								return Some(true);
							}
						}
						if let Some(AnyJsExpression::JsStaticMemberExpression(tag)) = template_expression.tag() {
