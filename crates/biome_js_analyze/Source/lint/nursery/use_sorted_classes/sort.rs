...
pub fn sort_class_name(
	...
	// Separate custom classes from recognized classes, and compute the
	// recognized classes' info. Custom classes always go first, in the order
	// that they appear in.
	...
	// Sort recognized classes using the compare function. Needs to be a stable
	// sort to preserve the lexico-graphical order as a fallback.
...
pub fn should_ignore_prefix(node:&AnyClassStringLike) -> bool {
	if let Some(value) = node.value() {
		// For example, for <div class={`${variable}mx-2 m-5`} />, we should
		// ignore "${variable}mx-2" as a sorting item because it is an
		// indivisible whole.
...
pub fn should_ignore_postfix(node:&AnyClassStringLike) -> bool {
	if let Some(value) = node.value() {
		// For example, for <div class={`mx-2 m-5${variable}`} />, we should
		// ignore "m-5${variable}" as a sorting item because it is an
		// indivisible whole.
