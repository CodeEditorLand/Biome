...
impl ClassInfo {
	/// Compare based on the existence of variants. Classes with variants go
	/// last. Returns `None` if both or none of the classes has variants.
	...
	/// First compare variants weight length. Only if their equal compare their
	/// actual weight. Returns `None` if they have the same weight.
	...
	/// Compare based on the existence of arbitrary variants. Classes with
	/// arbitrary variants go last. Returns `None` if both or none of the
	/// classes has arbitrary variants.
	...
	/// Compare arbitrary variants based on their length and then
	/// lexicographically
...
// This comparison function follows a very similar logic to the one in Tailwind
// CSS, with some simplifications and necessary differences.
...
/// ignore_prefix and ignore_postfix are used to ignore the first and last class
/// respectively.
pub fn sort_class_name(
	...
	let class_str_prefix = if ignore_prefix { classes_iter.next() } else { None };

	let class_str_postfix = if ignore_postfix { classes_iter.next_back() } else { None };
	...
	// Separate custom classes from recognized classes, and compute the recognized
	// classes' info. Custom classes always go first, in the order that they appear
	// in.
	...
	// Sort recognized classes using the compare function. Needs to be a stable sort
	// to preserve the lexico-graphical order as a fallback.
	classes_info.sort_by(compare_classes);

	// Add the sorted classes to the result.
	sorted_classes.extend(classes_info.iter().map(|class_info| class_info.text.as_str()));
...
pub fn should_ignore_prefix(node:&AnyClassStringLike) -> bool {
	if let Some(value) = node.value() {
		// For example, for <div class={`${variable}mx-2 m-5`} />, we should ignore
		// "${variable}mx-2" as a sorting item because it is an indivisible whole.
...
pub fn should_ignore_postfix(node:&AnyClassStringLike) -> bool {
	if let Some(value) = node.value() {
		// For example, for <div class={`mx-2 m-5${variable}`} />, we should ignore
		// "m-5${variable}" as a sorting item because it is an indivisible whole.
