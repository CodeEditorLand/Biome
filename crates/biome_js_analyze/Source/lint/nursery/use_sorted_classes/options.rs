...
impl UtilityClassSortingOptions {
	...
	pub(crate) fn match_function(&self, name: &str) -> bool {
		self.functions.iter().flatten().any(|matcher| {
			...
			let all_parts_match = matcher_parts.by_ref().zip(name_parts.by_ref()).all(|(m, p)| m == "*" || m == p);
	...
	pub(crate) fn has_attribute(&self, name: &str) -> bool {
		CLASS_ATTRIBUTES.contains(&name) || self.attributes.iter().flatten().any(|v| v.as_ref() == name)
	}
}
...
impl DeserializationVisitor for UtilityClassSortingOptionsVisitor {
	...
	fn visit_map(
		...
		for (key, value) in members.flatten() {
			...
			match key_text.text() {
				"attributes" => {
					if let Some(attributes_option) = Deserializable::deserialize(ctx, &value, &key_text) {
						attributes.extend::<Vec<Box<str>>>(attributes_option);
					}
				},
				"functions" => result.functions = Deserializable::deserialize(ctx, &value, &key_text),
		...
		result.attributes = if attributes.is_empty() { None } else { Some(attributes.into_boxed_slice()) };
		Some(result)
	}
}
