//! CSS utility classes need to be lexed into segments, which represent the
//! variants and the utility, and whether they are arbitrary or not. Some
//! examples:
//! - `px-2`: utility `px-2`.
//! - `hover:px-2`: variant `hover`, utility `px-2`.
//! - `sm:hover:px-2`: variant `sm`, variant `hover`, utility `px-2`.
//! - `hover:[mask:circle]`: variant `hover`, utility `[mask:circle]`
//!   (arbitrary).
//! - `[&:nth-child(3)]:px-2`: variant `[&:nth-child(3)]` (arbitrary), utility
//!   `px-2`. The results of the lexer are then used to process classes into
//!   `ClassInfo` structs, which are, in turn, used to sort the classes.
...
mod tests {
	...
	fn test_split_at_indexes() {
		assert_eq!(split_at_indexes("foo:bar:baz", &[3, 7]), vec!["foo", "bar", "baz"]);
		assert_eq!(split_at_indexes("foobar:baz", &[6]), vec!["foobar", "baz"]);
		assert_eq!(split_at_indexes("foobarbaz", &[]), vec!["foobarbaz"]);
		assert_eq!(split_at_indexes("foo_bar_baz", &[3, 7]), vec!["foo", "bar", "baz"]);
...
/// Information about the structure of a segment of a CSS class (variant or
/// utility).
						...
						Some(quote_type) => {
							...
							if let CharKind::Quote(last_quote) = &last_char {
								...
								if quote_type == last_quote {
									// then we are no longer in an arbitrary
									// block.
