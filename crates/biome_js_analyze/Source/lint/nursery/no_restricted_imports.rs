...
enum ImportRestrictionCause {
	...
	ImportSource,
	/// Reason: A set of forbidden import names has been defined via
	/// `importNames`.
	ImportNames,
	/// Reason: A set of allowed import names has been defined via
	/// `allowImportNames`.
	AllowImportNames,
}

/// Specifies whether a specific import is (dis)allowed, and why it is
/// allowed/disallowed.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
struct ImportRestrictionStatus(bool, ImportRestrictionCause);

impl ImportRestrictionStatus {
	pub fn is_allowed(&self) -> bool { self.0 }

	pub fn is_forbidden(&self) -> bool { !self.0 }

	pub fn reason(&self) -> ImportRestrictionCause { self.1 }
}
...
pub enum CustomRestrictedImport {
	/// The message to display when this module is imported.
	Plain(Box<str>),
	/// Additional options to configure the message and allowed/disallowed
	/// import names.
...
impl<'a> RestrictedImportVisitor<'a> {
	...
	pub fn visit_import_call(&mut self, import_call:&JsImportCallExpression) -> Option<()> {
		...
		// ... */ })     import("imported-module").then(function({ import1, import2:
		// localName2 }) { /* ... */ })     const namespaceImport = await
		// import("imported-module")     const { default: localName1, import1,
		// import2: localName2, "import3": localName3 } = await
		// import("imported-module")
		//
		...
		if let Some(bindings) = Self::get_context_for_import_call(import_call) {
			match bindings {
				AnyJsBindingPattern::AnyJsBinding(namespace_binding) => {
					match namespace_binding {
						// const ... = import(...)
				...
				AnyJsBindingPattern::JsArrayBindingPattern(_) => {
					// const [ ... ] = await import(...)
					//
					// Array binding patterns do not really make sense for an
					// import, so discard the additonal information and use
					// fallback instead.
				},
			}
		};
		...
		if current.kind() == JsSyntaxKind::JS_AWAIT_EXPRESSION {
			...
			if current.kind() == JsSyntaxKind::JS_CALL_EXPRESSION {
				...
				return match then_call_arg {
					...
					// then(function(...) { ... })
					AnyJsExpression::JsFunctionExpression(function_expr) => {
						Some(
							function_expr
								.parameters()
								.ok()?
	...
	/// Analyze a static `import ... from ...` or `export ... from
	/// ...`declaration (including all the different variants of `import` and
	/// `export`) to find the names that are being imported, then validate that
	/// each of the names is allowed to be imported.
	pub fn visit_import(&mut self, module_source_node:&JsModuleSource) -> Option<()> {
		// Only certain imports are allowed/disallowed, add diagnostic to each
		// disallowed import
	...
	/// Checks whether this bare import of the form `import from 'source'` is
	/// allowed.
	...
	/// Checks whether this import of the form `local_name` (as in `import
	/// local_name from 'source'`) is allowed.
	...
	/// Checks whether this namespace reexport of the form `export * from ...`
	/// is allowed.
	...
	/// Checks whether this import of the form `const local_name = import(...)`
	/// is allowed.
	...
	/// Checks whether this import of the form `{ imported_name }` or `{
	/// imported_name as exported_name }` (including `{ default as
	/// exported_name }`) is allowed.
	...
	/// (including `{ default: local_name }` and `{ "imported name": local_name
	/// `) is allowed.
	fn visit_named_binding(&mut self, named_import:&JsObjectBindingPatternProperty) -> Option<()> {
		self.visit_imported_identifier(
			&named_import.member().ok()?.as_js_literal_member_name()?.value().ok()?,
		)
	}
	...
	fn visit_imported_identifier(&mut self, name_token:&SyntaxToken<JsLanguage>) -> Option<()> {
		// TODO: inner_string_text removes quotes but does not e.g. decode escape
		// sequences.       If the imported name uses e.g. Unicode escape sequences,
		// this may cause       problems because
		// restricted_import.(allow_)import_names contains decoded       strings,
		// while inner_string_text(name_token) returns encoded strings.
