...
use biome_deserialize::{Deserializable, DeserializableType, DeserializableValue, DeserializationContext};
use biome_js_syntax::{
	AnyJsArrowFunctionParameters, AnyJsBindingPattern, AnyJsCombinedSpecifier, AnyJsExpression, AnyJsImportLike,
	AnyJsNamedImportSpecifier, AnyJsObjectBindingPatternMember, JsCallExpression, JsDefaultImportSpecifier,
	JsExportFromClause, JsExportNamedFromClause, JsExportNamedFromSpecifier, JsExportNamedFromSpecifierList,
	JsIdentifierBinding, JsImportBareClause, JsImportCallExpression, JsImportCombinedClause, JsImportDefaultClause,
	JsImportNamedClause, JsImportNamespaceClause, JsLanguage, JsModuleSource, JsNamedImportSpecifier,
	JsNamedImportSpecifiers, JsNamespaceImportSpecifier, JsObjectBindingPattern, JsObjectBindingPatternProperty,
	JsObjectBindingPatternShorthandProperty, JsShorthandNamedImportSpecifier, JsStaticMemberExpression, JsSyntaxKind,
	JsVariableDeclarator, inner_string_text,
};
...
#[derive(Clone, Debug, Default, Deserialize, biome_deserialize_macros::Deserializable, Eq, PartialEq, Serialize)]
...
impl CustomRestrictedImportOptions {
	...
	fn is_import_allowed(&self, imported_name: &str) -> ImportRestrictionStatus {
		if !self.allow_import_names.is_empty() {
			...
			let is_allowed = self.allow_import_names.iter().any(|name| &**name == imported_name);
			...
			let is_forbidden = self.import_names.iter().any(|name| &**name == imported_name);
...
impl From<CustomRestrictedImport> for CustomRestrictedImportOptions {
	fn from(options: CustomRestrictedImport) -> Self {
		match options {
			CustomRestrictedImport::Plain(message) => {
				CustomRestrictedImportOptions { message, import_names: [].into(), allow_import_names: [].into() }
			},
...
impl RestrictedImportVisitor<'_> {
	...
	fn get_context_for_import_call(import_call: &JsImportCallExpression) -> Option<AnyJsBindingPattern> {
	...
	fn visit_combined_specifier(&mut self, combined_specifier: &AnyJsCombinedSpecifier) -> Option<()> {
		match combined_specifier {
			AnyJsCombinedSpecifier::JsNamedImportSpecifiers(named_imports) => self.visit_named_imports(named_imports),
	...
	fn visit_named_reexports(&mut self, named_reexports: &JsExportNamedFromSpecifierList) -> Option<()> {
	...
	fn visit_named_or_shorthand_import(&mut self, import_specifier: &AnyJsNamedImportSpecifier) -> Option<()> {
		match import_specifier {
			...
			AnyJsNamedImportSpecifier::JsNamedImportSpecifier(named_import) => self.visit_named_import(named_import),
			AnyJsNamedImportSpecifier::JsBogusNamedImportSpecifier(_) => None,
		}
	}

	fn visit_named_or_shorthand_binding(&mut self, import_binding: &AnyJsObjectBindingPatternMember) -> Option<()> {
		match import_binding {
			AnyJsObjectBindingPatternMember::JsObjectBindingPatternShorthandProperty(shorthand_import) => {
	...
	fn visit_side_effect_import(&mut self, bare_import: &JsImportBareClause) -> Option<()> {
		let source_token = bare_import.source().ok()?.as_js_module_source()?.value_token().ok()?;
	...
	fn visit_namespace_import(&mut self, namespace_import: &JsNamespaceImportSpecifier) -> Option<()> {
		self.visit_special_import_token(&namespace_import.star_token().ok()?, Self::NAMESPACE_IMPORT_ALIAS)
	}
	...
	fn visit_namespace_reexport(&mut self, namespace_reexport: &JsExportFromClause) -> Option<()> {
		self.visit_special_import_token(&namespace_reexport.star_token().ok()?, Self::NAMESPACE_IMPORT_ALIAS)
	}
	...
	fn visit_shorthand_import(&mut self, shorthand_import: &JsShorthandNamedImportSpecifier) -> Option<()> {
	...
	/// Checks whether this import of the form `{ imported_name }` is allowed.
	fn visit_shorthand_binding(&mut self, shorthand_import: &JsObjectBindingPatternShorthandProperty) -> Option<()> {
		self.visit_imported_identifier(
			&shorthand_import
				.identifier()
				.ok()?
	...
	fn visit_named_or_shorthand_reexport(&mut self, named_reexport: &JsExportNamedFromSpecifier) -> Option<()> {
	...
	fn visit_named_binding(&mut self, named_import: &JsObjectBindingPatternProperty) -> Option<()> {
		self.visit_imported_identifier(&named_import.member().ok()?.as_js_literal_member_name()?.value().ok()?)
	}
	...
	fn visit_special_import_node(&mut self, import_node: &SyntaxNode<JsLanguage>, name_or_alias: &str) -> Option<()> {
...
impl Rule for NoRestrictedImports {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let restricted_import: CustomRestrictedImportOptions = restricted_import_settings.clone().into();
		...
		match node {
			AnyJsImportLike::JsModuleSource(module_source_node) => {
				if !restricted_import.has_import_name_patterns() {
					...
					let mut visitor = RestrictedImportVisitor { import_source, restricted_import, results: Vec::new() };
			...
			AnyJsImportLike::JsCallExpression(_expression) => {
				let status = restricted_import.is_import_allowed(RestrictedImportVisitor::DEFAULT_IMPORT_ALIAS);
	...
	fn diagnostic(_ctx: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
		let RestrictedImportMessage { import_source, allowed_import_names, location, message } = state;
