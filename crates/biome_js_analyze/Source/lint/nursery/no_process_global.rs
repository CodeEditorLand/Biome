...
impl Rule for NoProcessGlobal {
	...
	fn action(ctx: &RuleContext<Self>, _state: &Self::State) -> Option<JsRuleAction> {
		let top_level_statement = ctx.query().syntax().ancestors().find(is_top_level_statement)?;
		...
		let module_item_list = top_level_statement.parent().and_then(JsModuleItemList::cast)?.into_syntax();
		let mut slot = 0;
		...
		if let Some(import) = most_recent_import {
			...
			new_items = smallvec![
				...
				module_item_list.first_child()?.with_leading_trivia_pieces([])?,
			];
		}

		let new_module_item_list = module_item_list.clone().splice_slots(
			slot..(slot + 1),
			new_items.into_iter().map(|item| Some(SyntaxElement::Node(item))),
		);
		...
		Some(JsRuleAction::new(
			...
			markup! { "Add `import process from \"node:process\";` to this file's imports." }.to_owned(),
			mutation,
		))
	}
}
...
fn is_top_level_statement(node: &SyntaxNode<JsLanguage>) -> bool {
	node.parent().is_some_and(|node| JsModuleItemList::can_cast(node.kind()))
}
...
fn create_porcess_import(with_trailing_new_line: bool) -> JsImport {
	...
	let binding = make::js_identifier_binding(make::ident("process").with_trailing_trivia(whitespace));
