...
use biome_js_syntax::{
	AnyJsBinding, AnyJsExpression, AnyJsFunctionBody, AnyJsStatement, AnyTsType, JsArrowFunctionExpression,
	JsCallExpression, JsFileSource, JsFormalParameter, JsFunctionDeclaration, JsInitializerClause, JsLanguage,
	JsObjectExpression, JsParenthesizedExpression, JsPropertyClassMember, JsPropertyObjectMember, JsReturnStatement,
...
use biome_js_syntax::{
	...
	TsCallSignatureTypeMember, TsDeclareFunctionDeclaration, TsDeclareFunctionExportDefaultDeclaration,
		...
		match node {
			AnyCallableWithReturn::AnyJsFunction(func) => {
				...
				if let Ok(Some(AnyJsBinding::JsIdentifierBinding(id))) = func.id() {
					return Some(TextRange::new(func_range.start(), id.syntax().text_trimmed_range().end()));
				}
...
fn is_type_assertion(syntax: &SyntaxNode<JsLanguage>) -> bool {
	...
	syntax.parent().is_some_and(|parent| {
		if parent.kind() == JsSyntaxKind::JS_PARENTHESIZED_EXPRESSION {
			parent.parent().is_some_and(|grandparent| is_attribute_kind(grandparent.kind()))
		} else {
