...
use biome_js_syntax::{JsCallExpression, JsReferenceIdentifier, JsStaticMemberAssignment, global_identifier};
...
impl Rule for NoCommonJs {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let (reference, is_export) = match expression {
			CommonJsImportExport::JsCallExpression(node) => Some((is_require_call_expression(node)?, false)),
			CommonJsImportExport::JsStaticMemberAssignment(node) => Some((is_common_js_exports(node)?, true)),
		}?;
