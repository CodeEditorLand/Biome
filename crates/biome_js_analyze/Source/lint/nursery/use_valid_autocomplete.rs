...
impl Rule for UseValidAutocomplete {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		if (autocompletes.len() == 1 && autocompletes[0] == "none") || is_valid_autocomplete(&autocompletes) {
			return None;
		}
...
fn is_valid_autocomplete(autocomplete_values: &[&str]) -> bool {
	match autocomplete_values.len() {
		...
		1 => {
			...
			first.is_empty() || first.starts_with("section-") || VALID_AUTOCOMPLETE_VALUES.binary_search(&first).is_ok()
		},
		2.. => {
