...
use biome_js_syntax::{
	AnyJsTemplateElement, JsLiteralMemberName, JsStringLiteralExpression, JsSyntaxKind, JsSyntaxToken,
...
impl Rule for NoUselessEscapeInString {
	...
	fn diagnostic(_: &RuleContext<Self>, (token, index): &Self::State) -> Option<RuleDiagnostic> {
		let escape_start = token.text_trimmed_range().start().checked_add((*index as u32 + 1).into())?;
		...
		if c == b'\\' {
			if let Some((_, c)) = it.next() {
				match c {
					...
					0xE2 => {
						if !(matches!(it.next(), Some((_, 0x80))) && matches!(it.next(), Some((_, 0xA8 | 0xA9)))) {
							return Some(i);
						}
					},
					_ => {
