...
use biome_js_syntax::{
	AnyJsModuleItem, JsClassDeclaration, JsFunctionDeclaration, JsModule, JsModuleItemList, TsDeclarationModule,
...
impl Rule for UseAdjacentOverloadSignatures {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		let methods = match ctx.query() {
			...
			DeclarationOrModuleNode::TsInterfaceDeclaration(node) => collect_type_member_list(&node.members()),
			// Handle type Foo = {}
			DeclarationOrModuleNode::TsTypeAliasDeclaration(node) => {
				let members = node.ty().ok().and_then(|ty| ty.as_ts_object_type().cloned())?.members();
		...
		if !methods.is_empty() { Some(methods.into_boxed_slice()) } else { None }
	}
		...
		if let Some(ts_method_signature) = member.as_ts_method_signature_type_member() {
			if let Ok(method_member) = ts_method_signature.name() {
				if let Some(text) = method_member.name() {
					...
					check_method(text, range, &mut methods, &mut seen_methods, &mut last_method);
				}
			}
		}
	}
...
fn collect_function(node: &JsFunctionDeclaration) -> Vec<(TokenText, TextRange)> {
	...
	if let Some(return_type_annotation) = node.return_type_annotation() {
		if let Some(ty) = return_type_annotation.ty().ok().and_then(|ty| ty.as_any_ts_type().cloned()) {
			if let Some(ts_object) = ty.as_ts_object_type() {
				let members = ts_object.members();
				for member in members {
					if let Some(method_member) = member.as_ts_method_signature_type_member().and_then(|m| m.name().ok())
					{
						if let Some(text) = method_member.name() {
							let range = method_member.range();
							check_method(text, range, &mut methods, &mut seen_methods, &mut last_method);
		...
		if let AnyJsModuleItem::JsExport(node) = item {
			if let Ok(export) = node.export_clause() {
				if let Some(declaration_clause) = export.as_any_js_declaration_clause() {
					if let Some(ts_declare) = declaration_clause.as_ts_declare_function_declaration() {
						if let Some(name_token) = ts_declare
							.id()
							.ok()
							.and_then(|id| id.as_js_identifier_binding().and_then(|id| id.name_token().ok()))
						{
