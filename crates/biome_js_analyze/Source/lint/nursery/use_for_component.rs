use biome_analyze::{
	Ast, Rule, RuleDiagnostic, RuleDomain, RuleSource, RuleSourceKind, context::RuleContext, declare_lint_rule,
};
...
impl Rule for UseForComponent {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		if let Some(parent) = node.parent::<JsxExpressionChild>() {
			...
			if !matches!(parent.syntax().parent().kind(), Some(JsSyntaxKind::JSX_CHILD_LIST)) {
				return None;
			}
