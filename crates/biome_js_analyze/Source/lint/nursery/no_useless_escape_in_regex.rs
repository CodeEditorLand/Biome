...
impl Rule for NoUselessEscapeInRegex {
	...
	fn diagnostic(ctx: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
		...
		let escaped_char = &node.value_token().ok()?.text_trimmed()[(adjusted_backslash_index as usize + 1)..]
			.chars()
			.next()?;
		let diag = RuleDiagnostic::new(
			rule_category!(),
			TextRange::at(backslash_position, (1 + escaped_char.len_utf8() as u32).into()),
		...
		Some(if matches!(escaped, b'p' | b'P' | b'k') {
			...
			match escaped {
				b'^' => diag.note("The character should only be escaped if it is the first character of the class."),
				b'B' => diag.note("The escape sequence only has meaning outside a character class."),
				b'(' | b')' | b'[' | b'{' | b'}' | b'/' | b'|' => diag.note(
					"The character should only be escaped if it is outside a character class or under the `v` flag.",
				),
				b'.' | b'$' | b'*' | b'+' | b'?' => {
					diag.note("The character should only be escaped if it is outside a character class.")
				},
				b'-' => diag.note(
					"The character should only be escaped if it appears in the middle of the character class or under the `v` flag.",
				),
				_ => diag,
			}
		} else {
