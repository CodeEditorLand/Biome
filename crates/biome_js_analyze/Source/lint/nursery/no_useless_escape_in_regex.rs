...
impl Rule for NoUselessEscapeInRegex {
	...
	fn diagnostic(ctx:&RuleContext<Self>, state:&Self::State) -> Option<RuleDiagnostic> {
		...
		// Add 1 because the index was computed in the pattern (it doesn't take
		// `/` into account).
		...
		// To compute the correct text range, we need the byte length of the
		// escaped character. To get that, we take a string slice from the
		// escaped character and iterate until thenext character. The index of
		// the next character corresponds to the byte length of the escaped
		// character.
