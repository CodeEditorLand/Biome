...
impl Rule for NoUselessEscapeInRegex {
	...
	fn diagnostic(ctx:&RuleContext<Self>, state:&Self::State) -> Option<RuleDiagnostic> {
		let State { backslash_index, escaped, in_char_class } = state;
		// Add 1 because the index was computed in the pattern (it doesn't take `/` into
		// account).
		...
		// To compute the correct text range, we need the byte length of the escaped
		// character. To get that, we take a string slice from the escaped character
		// and iterate until thenext character. The index of the next character
		// corresponds to the byte length of the escaped character.
		...
		Some(if matches!(escaped, b'p' | b'P' | b'k') {
			diag.note(
				"The escape sequence is only useful if the regular expression is unicode-aware. \
				 To be unicode-aware, the `u` or `v` flag should be used.",
			)
		} else if *in_char_class {
			match escaped {
				b'^' => {
					diag.note(
						"The character should only be escaped if it is the first character of the \
						 class.",
					)
				},
				...
				b'(' | b')' | b'[' | b'{' | b'}' | b'/' | b'|' => {
					diag.note(
						"The character should only be escaped if it is outside a character class \
						 or under the `v` flag.",
					)
				},
				...
				b'-' => {
					diag.note(
						"The character should only be escaped if it appears in the middle of the \
						 character class or under the `v` flag.",
					)
				},
	...
	fn action(ctx:&RuleContext<Self>, state:&Self::State) -> Option<JsRuleAction> {
		let State { backslash_index, .. } = state;
		// Add 1 because the index was computed in the pattern (it doesn't take `/` into
		// account).
