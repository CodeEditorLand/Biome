...
impl Rule for NoSecrets {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		let entropy_threshold =
			ctx.options().entropy_threshold.unwrap_or(DEFAULT_HIGH_ENTROPY_THRESHOLD);
...
// Uses Shannon Entropy as a base algorithm, then adds "boosts" for special
// patterns/occurrences. For example, Continuous mixed cases (lIkE tHiS) are
// more likely to contribute to a higher score than single cases. Symbols also
// contribute highly to secrets.
//
// TODO: This needs work. False positives/negatives are highlighted in valid.js
// and invalid.js.
//
// References:
...
fn calculate_entropy_with_case_and_classes(
	...
	let symbol_entropy_boost =
		if symbol_count > 0 { symbol_count as f64 / len as f64 } else { 0.0 };

	let digit_entropy_boost = if digit_count > 0 { digit_count as f64 / len as f64 } else { 0.0 };
	...
	// Apply exponential scaling to avoid excessive boosting for long, structured
	// tokens
	apply_exponential_entropy_scaling(adjusted_entropy, len, base_threshold, scaling_factor)
}

// A simple mechanism to scale entropy as the string length increases, the
// reason being that large length strings are likely to be secrets.
// TODO: However, at some point there should definitely be a cutoff i.e. 100
// characters, because it's probably base64 data or something similar at that
// point. This was taken from GPT, and I sadly couldn't find references for it.
fn apply_exponential_entropy_scaling(
	...
	// We will apply a logarithmic dampening to prevent excessive scaling for long
	// tokens
...
mod tests {
	...
	fn test_min_pattern_len() {
		let actual_min_pattern_len =
			SENSITIVE_PATTERNS.iter().map(|pattern| pattern.min_len).min().unwrap_or(0);

		let initialized_min_pattern_len = MIN_PATTERN_LEN;

		assert_eq!(
			initialized_min_pattern_len, actual_min_pattern_len,
			"The initialized MIN_PATTERN_LEN value is not correct. Please ensure it's the \
			 smallest possible number from the SENSITIVE_PATTERNS."
		);
	}
}
