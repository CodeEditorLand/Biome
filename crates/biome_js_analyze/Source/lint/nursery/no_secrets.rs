...
static PASSWORD_IN_URL_REGEX: LazyLock<Regex> =
	LazyLock::new(|| Regex::new(r#"[a-zA-Z]{3,10}://[^/\s:@]{3,20}:[^/\s:@]{3,20}@.{1,100}['"\s]"#).unwrap());

static GOOGLE_SERVICE_ACCOUNT_REGEX: LazyLock<Regex> = LazyLock::new(|| {
	Regex::new(r#"(?:^|[,\s])"type"\s*:\s*(?:['"]service_account['"']|service_account)(?:[,\s]|$)"#).unwrap()
});

static TWILIO_API_KEY_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r#"SK[a-z0-9]{32}"#).unwrap());

static GOOGLE_OAUTH_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r#"ya29\\.[0-9A-Za-z\\-_]+"#).unwrap());

static AWS_API_KEY_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"AKIA[0-9A-Z]{16}").unwrap());
...
static BASE64_REGEX: LazyLock<Regex> =
	LazyLock::new(|| Regex::new(r"^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$").unwrap());
...
static WINDOWS_PATH_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^[a-zA-Z]:\\(?:[^\\]+\\?)*$").unwrap());
...
fn detect_secret(data: &str, entropy_threshold: &u16) -> Option<&'static str> {
	...
	for token in tokens {
		if token.len() >= MIN_PATTERN_LEN {
			...
			let entropy = calculate_entropy_with_case_and_classes(token, *entropy_threshold as f64, 15.0);
...
*/
fn calculate_entropy_with_case_and_classes(data: &str, base_threshold: f64, scaling_factor: f64) -> f64 {
	...
	let symbol_entropy_boost = if symbol_count > 0 { symbol_count as f64 / len as f64 } else { 0.0 };

	let digit_entropy_boost = if digit_count > 0 { digit_count as f64 / len as f64 } else { 0.0 };

	let adjusted_entropy =
		shannon_entropy + (case_entropy_boost * 2.5) + (symbol_entropy_boost * 1.5) + digit_entropy_boost;
...
mod tests {
	...
	fn test_min_pattern_len() {
		let actual_min_pattern_len = SENSITIVE_PATTERNS.iter().map(|pattern| pattern.min_len).min().unwrap_or(0);
