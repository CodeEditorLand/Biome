use biome_analyze::{Rule, RuleDiagnostic, RuleSource, RuleSourceKind, context::RuleContext, declare_lint_rule};
use biome_console::fmt::{Display, Formatter};
use biome_console::markup;
use biome_js_semantic::{BindingExtensions, SemanticModel};
use biome_js_syntax::{
	AnyJsArrayElement, AnyJsCallArgument, AnyJsExpression, AnyJsLiteralExpression, AnyJsTemplateElement,
	JsAssignmentOperator, JsBinaryExpression, JsBinaryOperator, JsCallExpression, JsIdentifierExpression,
	JsLogicalExpression, JsLogicalOperator, JsReferenceIdentifier, JsUnaryOperator,
};
...
impl Rule for NoConstantBinaryExpression {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		match ctx.query() {
				...
				match operator {
					JsLogicalOperator::LogicalAnd | JsLogicalOperator::LogicalOr => {
						if is_constant_condition(left.clone(), true, model).is_some() {
							return Some(Issue::ConstantShortCircuit { left, right, property: Property::Truthiness });
						}
					},
					JsLogicalOperator::NullishCoalescing => {
						if let Ok(true) = has_constant_nullishness(model, &left, false) {
							return Some(Issue::ConstantShortCircuit { left, right, property: Property::Nullishness });
						}
					},
				}
			},
			Query::JsBinaryExpression(expr) => {
				...
				if let Some(operand) = find_binary_expression_constant_operand(model, operator, &left, &right) {
					return Some(Issue::ConstantBinaryOperand { operand: operand.clone() });
				}

				if let Some(operand) = find_binary_expression_constant_operand(model, operator, &right, &left) {
					return Some(Issue::ConstantBinaryOperand { operand: operand.clone() });
				}
				...
				match operator {
					...
					JsBinaryOperator::Equality | JsBinaryOperator::Inequality => {
						if let (Ok(true), Ok(true)) = (is_always_new(model, &left), is_always_new(model, &right)) {
	...
	fn diagnostic(ctx: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
		...
		Some(match state {
			...
			.detail(operand.range(), markup! { "This expression always constructs a new object."}),
...
fn find_binary_expression_constant_operand<'a>(
	...
	match operator {
		JsBinaryOperator::Equality | JsBinaryOperator::Inequality => {
			if (is_null_or_undefined(model, a).ok()? && has_constant_nullishness(model, b, false).ok()?)
				|| (is_static_boolean(model, a).ok()? && has_constant_loose_boolean_comparison(model, b).ok()?)
			{
				return Some(b);
			}
		},
		JsBinaryOperator::StrictEquality | JsBinaryOperator::StrictInequality => {
			if (is_null_or_undefined(model, a).ok()? && has_constant_nullishness(model, b, false).ok()?)
				|| (is_static_boolean(model, a).ok()? && has_constant_strict_boolean_comparison(model, b).ok()?)
			{
				return Some(b);
			}
		},
...
fn has_constant_loose_boolean_comparison(model: &SemanticModel, node: &AnyJsExpression) -> SyntaxResult<bool> {
	Ok(match node.clone().omit_parentheses() {
		...
		AnyJsExpression::JsUnaryExpression(expr) => match expr.operator()? {
			...
			JsUnaryOperator::LogicalNot => is_constant_condition(expr.argument()?, true, model).is_some(),
			_ => false,
		},
		...
		AnyJsExpression::JsAssignmentExpression(expr) => match expr.operator()? {
			JsAssignmentOperator::Assign => has_constant_loose_boolean_comparison(model, &expr.right()?)?,
			_ => false,
		},

		AnyJsExpression::JsSequenceExpression(expr) => has_constant_loose_boolean_comparison(model, &expr.right()?)?,
...
fn has_constant_strict_boolean_comparison(model: &SemanticModel, node: &AnyJsExpression) -> SyntaxResult<bool> {
	Ok(match node.clone().omit_parentheses() {
		// not a boolean
		AnyJsExpression::JsObjectExpression(_)
		...
		AnyJsExpression::JsSequenceExpression(expr) => has_constant_strict_boolean_comparison(model, &expr.right()?)?,

		AnyJsExpression::JsIdentifierExpression(expr) => is_undefined(model, &expr)?,

		AnyJsExpression::JsAssignmentExpression(expr) => match expr.operator()? {
			JsAssignmentOperator::Assign => has_constant_strict_boolean_comparison(model, &expr.right()?)?,
		...
		AnyJsExpression::JsCallExpression(expr) => match expr.callee()? {
			AnyJsExpression::JsIdentifierExpression(ident) => {
				...
				((name.has_name("String") || name.has_name("Number")) && is_global_reference(model, &name))
	...
	Ok(
		...
		name.has_name("Boolean") && is_global_reference(model, &name) && {
			match node.arguments()?.args().first() {
				Some(arg) => match arg? {
					AnyJsCallArgument::AnyJsExpression(expr) => is_constant_condition(expr, true, model).is_some(),
					_ => false,
				},
				_ => true,
			}
...
fn has_constant_nullishness(model: &SemanticModel, node: &AnyJsExpression, non_nullish: bool) -> SyntaxResult<bool> {
	...
	Ok(match node.clone().omit_parentheses() {
		...
		AnyJsExpression::JsAssignmentExpression(expr) => match expr.operator()? {
			JsAssignmentOperator::Assign => has_constant_nullishness(model, &expr.right()?, non_nullish)?,
		...
		AnyJsExpression::JsSequenceExpression(expr) => has_constant_nullishness(model, &expr.right()?, non_nullish)?,
...
fn is_global_reference(model: &SemanticModel, node: &JsReferenceIdentifier) -> bool {
	node.binding(model).is_none_or(|b| b.scope().is_global_scope())
}
