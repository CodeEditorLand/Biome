use biome_analyze::{
	Ast, FixKind, Rule, RuleDiagnostic, RuleDomain, RuleSource, RuleSourceKind, context::RuleContext, declare_lint_rule,
};
use biome_console::markup;
use biome_js_factory::make;
use biome_js_syntax::{AnyJsxAttribute, AnyJsxAttributeName, AnyJsxAttributeValue, T, jsx_ext::AnyJsxElement};
...
impl Rule for UseGoogleFontPreconnect {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		if href.starts_with("https://fonts.gstatic.com") && !matches!(rel.as_deref(), Some("preconnect")) {
		...
		let new_attribute = AnyJsxAttribute::from(
			make::jsx_attribute(AnyJsxAttributeName::JsxName(make::jsx_name(rel)))
				.with_initializer(make::jsx_attribute_initializer_clause(
					make::token(T![=]),
					AnyJsxAttributeValue::JsxString(make::jsx_string(make::jsx_string_literal("preconnect"))),
				))
				.build(),
		);
