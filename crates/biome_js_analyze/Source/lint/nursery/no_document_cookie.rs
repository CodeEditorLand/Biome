...
fn identifier_is_global_document(reference: &JsReferenceIdentifier, name: &StaticValue, model: &SemanticModel) -> bool {
...
fn is_cookie(assignment: &AnyJsAssignment) -> Option<()> {
	...
	match assignment {
		...
		AnyJsAssignment::JsComputedMemberAssignment(computed_assignment) => {
			...
			let string_literal = any_expr.as_any_js_literal_expression()?.as_js_string_literal_expression()?;
