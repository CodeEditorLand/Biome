...
use biome_js_syntax::{
	AnyJsBindingPattern, AnyJsCallArgument, AnyJsExpression, AnyJsModuleItem, AnyJsStatement, JsModule,
		...
		for item in root.items() {
			if let AnyJsModuleItem::AnyJsStatement(stmt) = item {
				...
				for exported_item in export.get_exported_items() {
					...
					if let Some(exported_item_id) = exported_item.identifier.as_ref().and_then(|x| x.name_token()) {
						if ctx
							.options()
							.allow_export_names
							.iter()
							.any(|export_name| export_name.as_ref() == exported_item_id.text_trimmed())
						{
							continue;
						}
					}
					...
					if ctx.options().allow_constant_export
						&& exported_item.exported.clone().is_some_and(|partof| match partof {
							AnyJsExported::AnyJsExpression(expr) => expr.is_literal_expression(),
							_ => false,
						}) {
						continue;
					}
...
fn is_exported_react_component(any_exported_item: &ExportedItem) -> bool {
		...
		any_exported_item.exported.clone()
	{
		if let Ok(AnyJsExpression::JsIdentifierExpression(fn_name)) = f.callee() {
			...
			let itr = args.args().into_iter().filter_map(Result::ok).collect::<Vec<_>>();
			if itr.len() != 1 {
				return false;
			}
			let AnyJsCallArgument::AnyJsExpression(AnyJsExpression::JsIdentifierExpression(arg)) = &itr[0] else {
				return false;
			};
