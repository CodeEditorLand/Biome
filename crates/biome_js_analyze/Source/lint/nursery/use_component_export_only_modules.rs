		...
		for item in root.items() {
			if let AnyJsModuleItem::AnyJsStatement(stmt) = item {
				...
				for exported_item in export.get_exported_items() {
					...
					if let Some(exported_item_id) =
						exported_item.identifier.as_ref().and_then(|x| x.name_token())
					{
					...
					if ctx.options().allow_constant_export
						&& exported_item.exported.clone().is_some_and(|partof| {
							match partof {
		...
		if !exported_component_ids.is_empty() {
			return exported_non_component_ids
				...
				.filter_map(|id| {
					...
					range.map(|range| {
						UseComponentExportOnlyModulesState {
