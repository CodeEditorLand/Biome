use biome_analyze::{
	Ast, QueryMatch, Rule, RuleDiagnostic, RuleSource, RuleSourceKind, context::RuleContext, declare_lint_rule,
};
use biome_console::markup;
use biome_js_syntax::{
	JsAwaitExpression, JsDoWhileStatement, JsForInStatement, JsForOfStatement, JsForStatement,
	JsForVariableDeclaration, JsSyntaxKind, JsSyntaxNode, JsVariableDeclaration, JsWhileStatement, JsWithStatement,
};
			...
			match event {
				WalkEvent::Enter(node) => {
					...
					if JsAwaitExpression::can_cast(node.kind()) {
						...
						if let Some(parent) = node.parent() {
							if JsForOfStatement::can_cast(parent.kind()) || JsForInStatement::can_cast(parent.kind()) {
								continue;
							}
						}
						return Some(node);
