...
impl Rule for NoOctalEscape {
			...
			if byte == b'\\' {
				if let Some((_, byte)) = it.next() {
					if matches!(byte, b'0'..=b'7') {
						let len = 2 + it.clone().take(5).take_while(|(_, byte)| matches!(byte, b'0'..=b'7')).count();
	...
	fn diagnostic(ctx: &RuleContext<Self>, RuleState { index, len }: &Self::State) -> Option<RuleDiagnostic> {
		let token = ctx.query().string_literal_token()?;
		let escape_start = token.text_trimmed_range().start().checked_add((*index as u32).into())?;
	...
	fn action(ctx: &RuleContext<Self>, RuleState { index, len }: &Self::State) -> Option<JsRuleAction> {
		let token = ctx.query().string_literal_token()?;
		...
		let (new_text, unicode_or_hexa) = if codepoint <= 0xff {
			(format!("{before_octal}x{codepoint:02x}{after_octal}"), "hexadecimal")
		} else {
			(format!("{before_octal}u{codepoint:04x}{after_octal}"), "unicode")
		};
