			...
			AnyJsStatement::JsBlockStatement(block) => {
				...
				match statements.len() {
					...
					1 => {
						...
						if first_statement.as_js_if_statement().is_none() { Some(()) } else { None }
					},
					_ => {
						let first_statement = statements.first()?;
						if let Some(first_if_statement) = first_statement.as_js_if_statement() {
							match first_if_statement.consequent().ok()? {
								AnyJsStatement::JsBlockStatement(block) if block.statements().len() == 1 => {
									if block.statements().first()?.as_js_continue_statement().is_none() {
										Some(())
									} else {
										None
									}
