...
/// Checks whether the given function is a higher-order function, i.e., a
/// function that returns another function either directly in its body or as an
/// expression.
///
/// # Arguments
///
/// * `func` - A reference to an `AnyJsFunction` that represents the JavaScript
///   function to inspect.
///
/// # Returns
///
/// * `true` if the function returns another function (either a regular function
///   or an arrow function).
/// * `false` if it does not return a function or if the body is not a valid
///   returnable function expression.
///
/// # Note
///
/// This function currently **does not support** detecting a return of a
...
/// Checks whether the first statement in the given list of JavaScript
/// statements is a return statement that returns a function expression (either
/// a regular function or an arrow function).
///
/// # Arguments
///
/// * `statements` - A list of JavaScript statements (`JsStatementList`) to
///   inspect.
///
/// # Returns
///
...
fn is_type_assertion(syntax:&SyntaxNode<JsLanguage>) -> bool {
	...
	syntax.parent().map_or(false, |parent| {
		if parent.kind() == JsSyntaxKind::JS_PARENTHESIZED_EXPRESSION {
			parent.parent().is_some_and(|grandparent| is_assertion_kind(grandparent.kind()))
		} else {
