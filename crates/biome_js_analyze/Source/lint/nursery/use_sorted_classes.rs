...
use biome_js_factory::make::{
	js_literal_member_name, js_string_literal, js_string_literal_expression, js_string_literal_single_quotes,
...
use self::{
	any_class_string_like::AnyClassStringLike, presets::UseSortedClassesPreset, sort::get_sort_class_name_range,
	sort::should_ignore_postfix, sort::should_ignore_prefix, sort::sort_class_name, sort_config::SortConfig,
};
...
impl Rule for UseSortedClasses {
	...
	fn run(ctx: &RuleContext<Self>) -> Option<Self::State> {
		...
		if node.should_visit(options)? {
			if let Some(value) = node.value() {
				...
				let sorted_value = sort_class_name(&value, &SORT_CONFIG, ignore_prefix, ignore_postfix);
	...
	fn diagnostic(ctx: &RuleContext<Self>, _: &Self::State) -> Option<RuleDiagnostic> {
		...
		let sort_range = if let Some(value) = node.value() {
			...
			let ignore_prefix = should_ignore_prefix(node);
			let ignore_postfix = should_ignore_postfix(node);
			let real_sort_range = get_sort_class_name_range(&value, &range, ignore_prefix, ignore_postfix);
	...
	fn action(ctx: &RuleContext<Self>, state: &Self::State) -> Option<JsRuleAction> {
		...
		match ctx.query() {
			AnyClassStringLike::JsStringLiteralExpression(string_literal) => {
				let replacement = js_string_literal_expression(if ctx.as_preferred_quote().is_double() {
