...
impl Rule for UseStrictMode {
	...
	fn action(ctx:&RuleContext<Self>, _state:&Self::State) -> Option<JsRuleAction> {
		...
		let use_strict_diretcive =
			make::js_directive(value).with_semicolon_token(make::token(T![;])).build();
		let directives = make::js_directive_list(
			node.directives().into_iter().chain([use_strict_diretcive]).collect::<Vec<_>>(),
		);
