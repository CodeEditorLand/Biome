...
use biome_analyze::{
	Ast, FixKind, Rule, RuleDiagnostic, context::RuleContext, declare_lint_rule, options::PreferredQuote,
};
...
impl Rule for UseStrictMode {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		if node
			.directives()
			.iter()
			.any(|directive| directive.inner_string_text().is_ok_and(|text| text.text() == "use strict"))
		{
			return None;
		}
	...
	fn action(ctx: &RuleContext<Self>, _state: &Self::State) -> Option<JsRuleAction> {
		...
		let is_statement_first_trivia_newline = node
			...
			.is_some_and(|f| f.pieces().next().is_some_and(|piece| piece.kind() == TriviaPieceKind::Newline));
		...
		let mut strict_directive_token = JsSyntaxToken::new_detached(JsSyntaxKind::JSX_STRING_LITERAL, value, [], []);
		...
		if !trailing_trivia.is_empty() {
			strict_directive = strict_directive.with_semicolon_token(token(T![;]).with_trailing_trivia(trailing_trivia))
		} else {
