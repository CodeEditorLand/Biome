...
use biome_js_syntax::{AnyJsExpression, AnyJsObjectMember, AnyJsObjectMemberName, inner_string_text};
...
impl Rule for UseConsistentObjectDefinition {
				...
				match name_token {
					...
					AnyJsObjectMemberName::JsComputedMemberName(_computed_token) => {
						...
						match reference_token {
							AnyJsExpression::JsFunctionExpression(_function_token) => match options.syntax {
	...
	fn diagnostic(ctx: &RuleContext<Self>, _state: &Self::State) -> Option<RuleDiagnostic> {
		...
		Some(RuleDiagnostic::new(rule_category!(), node.range(), markup! {{title}}).note(markup! {{note}}))
	}
}
