...
impl AnyJsMemberWithAccessibility {
	fn accessibility_modifier(&self) -> Option<TsAccessibilityModifier> {
		match self {
			...
			Self::TsPropertyParameter(member) => {
				member
					.modifiers()
					.into_iter()
					.map(AstNode::into_syntax)
			...
			Self::TsConstructorSignatureClassMember(member) => {
				member
					.modifiers()
					.into_iter()
					.map(AstNode::into_syntax)
			...
			Self::TsPropertySignatureClassMember(member) => {
				member
					.modifiers()
					.into_iter()
					.map(AstNode::into_syntax)
			...
			Self::TsGetterSignatureClassMember(member) => {
				member
					.modifiers()
					.into_iter()
					.map(AstNode::into_syntax)
			...
			Self::TsSetterSignatureClassMember(member) => {
				member
					.modifiers()
					.into_iter()
					.map(AstNode::into_syntax)
