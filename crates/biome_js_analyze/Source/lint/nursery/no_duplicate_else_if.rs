		...
		if let Some(logical_expression) = expr.as_js_logical_expression() {
			if let Ok(operator_token) = logical_expression.operator() {
				if operator_token == JsLogicalOperator::LogicalAnd {
					conditions_to_check.append(&mut split_by_logical_operator_wrapper(operator_token, expr));
				}
			}
		}
...
fn split_by_logical_operator_wrapper(operator: JsLogicalOperator, node: AnyJsExpression) -> Vec<AnyJsExpression> {
	let mut result: Vec<AnyJsExpression> = vec![];
	split_by_logical_operator(operator, node, &mut result);
	result
}

fn split_by_logical_operator(operator: JsLogicalOperator, node: AnyJsExpression, result: &mut Vec<AnyJsExpression>) {
	...
	match &node {
		AnyJsExpression::JsLogicalExpression(logic_expression) => {
			...
			if let (Ok(left_node), Ok(right_node)) = (logic_expression.left(), logic_expression.right()) {
