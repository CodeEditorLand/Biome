...
impl Rule for NoUnwantedPolyfillio {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let src_attribute_str = src_attribute_value.as_jsx_string()?.inner_string_text().ok()?;
...
fn unwanted_features_message(features: &[&str]) -> Box<str> {
	...
	let duplicate_polyfills = format!("{} {}", joined_features, if features.len() > 1 { "are" } else { "is" });
...
fn check_unwanted_polyfill<'a>(src: &'a str, next_polyfilled_features: &[&str]) -> Option<Vec<&'a str>> {
	...
	if !unwanted_sources.iter().any(|prefix| src.starts_with(prefix)) {
		return None;
	}
	...
	if unwanted_features.is_empty() { None } else { Some(unwanted_features) }
}
