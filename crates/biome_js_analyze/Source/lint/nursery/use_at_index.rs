...
/// When using this expression in other operations, enclose it in parentheses as
/// needed.
fn overwrap_parentheses_expression(node:&AnyJsExpression) -> Option<AnyJsExpression> {
	match node {
		...
		_ => {
			Some(AnyJsExpression::JsParenthesizedExpression(make::js_parenthesized_expression(
				make::token(T!['(']),
				node.clone(),
				make::token(T![')']),
			)))
...
/// AnyJsExpressiion -> Some(i64) if the expression is an integer literal,
/// otherwise None.
...
fn analyze_slice_element_access(node:&AnyJsExpression) -> Option<UseAtIndexState> {
	...
	match (extract_type.clone(), optional_arg1) {
		(SliceExtractType::ZeroMember | SliceExtractType::Shift, None) => {
...
fn check_call_expression_char_at(
	...
	match arg0.clone() {
		...
		AnyJsExpression::JsBinaryExpression(_) => {
			...
			at_number_exp.map(|at_number_exp| {
				UseAtIndexState {
					at_number_exp,
