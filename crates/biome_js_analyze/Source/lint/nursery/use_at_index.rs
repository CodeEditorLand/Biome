...
use biome_analyze::{
	Ast, FixKind, Rule, RuleDiagnostic, RuleSource, RuleSourceKind, context::RuleContext, declare_lint_rule,
};
...
use biome_js_syntax::{
	...
	JsComputedMemberExpression, JsParenthesizedExpression, JsStaticMemberExpression, JsUnaryExpression, T,
};
...
#[derive(Clone)]
pub enum ErrorType {
	Index { is_negative: bool },
	StringCharAt { is_negative: bool },
	Slice { arg_type: SliceArgType, extract_type: SliceExtractType },
}
...
impl Rule for UseAtIndex {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let result: Option<UseAtIndexState> = match exp {
			// foo[a]
			AnyJsArrayAccess::JsComputedMemberExpression(exp) => check_computed_member_expression(exp),
			// foo.bar()
	...
	fn diagnostic(ctx: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
		let node = ctx.query();
		Some(
			RuleDiagnostic::new(rule_category!(), node.range(), state.error_type.get_error_message()).note(markup! {
	...
	fn action(ctx: &RuleContext<Self>, state: &Self::State) -> Option<JsRuleAction> {
		...
		let prev_node = match node {
			...
			AnyJsArrayAccess::JsCallExpression(node) => AnyJsExpression::JsCallExpression(node.clone()),
		};
		let UseAtIndexState { at_number_exp, error_type: _, object } = state;
		let object = overwrap_parentheses_expression(object)?;

		mutation.replace_node(
			prev_node,
			AnyJsExpression::JsCallExpression(make_at_method(object, at_number_exp.clone().trim_trivia()?)),
		);
	...
	fn get_error_message(self: &ErrorType) -> MarkupBuf {
		match self {
			ErrorType::Index { is_negative } | ErrorType::StringCharAt { is_negative } => {
				...
				markup! { "Prefer "<Emphasis>{method}</Emphasis>" over "<Emphasis>{old_method}</Emphasis>"." }
					.to_owned()
			},
			ErrorType::Slice { arg_type, extract_type } => {
				...
				let (method, old_method) = match (arg_type, extract_type) {
					...
					_ => (
						...
						format!(
							"X.slice({}){}",
							if matches!(arg_type, SliceArgType::OneArg) { "Y" } else { "Y, a" },
							extract_string
						),
					),
				};
...
fn is_same_reference(left: AnyJsExpression, right: AnyJsExpression) -> Option<bool> {
	...
	match (left, right) {
		// x[0]
		(AnyJsExpression::JsComputedMemberExpression(left), AnyJsExpression::JsComputedMemberExpression(right)) => {
			let AnyJsExpression::AnyJsLiteralExpression(left_member) = left.member().ok()?.omit_parentheses() else {
				return Some(false);
			};
			let AnyJsExpression::AnyJsLiteralExpression(right_member) = right.member().ok()?.omit_parentheses() else {
				return Some(false);
			};
		...
		// x.y
		(AnyJsExpression::JsStaticMemberExpression(left), AnyJsExpression::JsStaticMemberExpression(right)) => {
...
fn overwrap_parentheses_expression(node: &AnyJsExpression) -> Option<AnyJsExpression> {
	match node {
		AnyJsExpression::JsArrayExpression(exp) => Some(AnyJsExpression::JsArrayExpression(exp.clone())),
		...
		AnyJsExpression::JsIdentifierExpression(exp) => {
			Some(AnyJsExpression::JsIdentifierExpression(exp.clone().trim_trivia()?))
		},
		AnyJsExpression::JsParenthesizedExpression(exp) => {
			Some(AnyJsExpression::JsParenthesizedExpression(exp.clone()))
		},
		AnyJsExpression::JsStaticMemberExpression(exp) => Some(AnyJsExpression::JsStaticMemberExpression(exp.clone())),
		_ => Some(AnyJsExpression::JsParenthesizedExpression(make::js_parenthesized_expression(
			make::token(T!['(']),
			node.clone(),
			make::token(T![')']),
		))),
...
fn get_integer_from_literal(node: &AnyJsExpression) -> Option<i64> {
	if let AnyJsExpression::JsUnaryExpression(unary) = node {
		...
		return get_integer_from_literal(&unary.argument().ok()?.omit_parentheses()).map(|num| -num);
	}
	let AnyJsExpression::AnyJsLiteralExpression(AnyJsLiteralExpression::JsNumberLiteralExpression(number)) = node
	else {
		return None;
	};
...
///    a - b - c // => Some((a, [b, c]))
/// ```
fn split_minus_binary_expressions(mut expression: AnyJsExpression) -> Option<(AnyJsExpression, Vec<AnyJsExpression>)> {
...
/// ```
fn extract_negative_index_expression(member: AnyJsExpression, object: AnyJsExpression) -> Option<AnyJsExpression> {
	...
	if right_list.len() == 1 {
		...
		if let Some(number) = get_integer_from_literal(&right_list[0].clone().omit_parentheses()) {
			if number > 0 {
				Some(AnyJsExpression::JsUnaryExpression(make::js_unary_expression(
					make::token(T![-]),
					right_list[0].clone(),
				)))
			} else {
...
fn analyze_slice_element_access(node: &AnyJsExpression) -> Option<UseAtIndexState> {
	...
	let [Some(arg0), optional_arg1, None] = call_exp.arguments().ok()?.get_arguments_by_index([0, 1, 2]) else {
		return None;
	};
	...
	match (extract_type.clone(), optional_arg1) {
		(SliceExtractType::ZeroMember | SliceExtractType::Shift, None) => Some(UseAtIndexState {
			...
			error_type: ErrorType::Slice { arg_type: SliceArgType::OneArg, extract_type },
			object: sliced_exp,
		}),
		(SliceExtractType::Pop, None) if get_integer_from_literal(&start_exp)? < 0 => Some(UseAtIndexState {
			at_number_exp: make_number_literal(-1),
			error_type: ErrorType::Slice { arg_type: SliceArgType::OneArg, extract_type: SliceExtractType::Pop },
			object: sliced_exp,
		}),
		(SliceExtractType::ZeroMember | SliceExtractType::Shift, Some(arg1)) => {
			let start_index = get_integer_from_literal(&start_exp)?;
			let end_index = get_integer_from_literal(&arg1.as_any_js_expression()?.clone().omit_parentheses())?;
			(start_index * end_index >= 0 && start_index < end_index).then_some(UseAtIndexState {
				at_number_exp: start_exp,
				error_type: ErrorType::Slice { arg_type: SliceArgType::TwoArg, extract_type },
				object: sliced_exp,
			})
		},
		(SliceExtractType::Pop, Some(arg1)) => {
			...
			(start_index * end_index >= 0 && start_index < end_index).then_some(UseAtIndexState {
				...
				error_type: ErrorType::Slice { arg_type: SliceArgType::TwoArg, extract_type: SliceExtractType::Pop },
				object: sliced_exp,
			})
		},
		_ => None,
...
fn check_binary_expression_member(member: JsBinaryExpression, object: AnyJsExpression) -> Option<UseAtIndexState> {
	let member = AnyJsExpression::JsBinaryExpression(member);
	let negative_index_exp = extract_negative_index_expression(member, object.clone().omit_parentheses());
...
fn check_computed_member_expression(exp: &JsComputedMemberExpression) -> Option<UseAtIndexState> {
	// check slice
	if let Some(slice_err) = analyze_slice_element_access(&AnyJsExpression::JsComputedMemberExpression(exp.clone())) {
		return Some(slice_err);
	}
	// invalid optional chain, mutable case
	if exp.is_optional_chain()
		|| is_within_delete_expression(&AnyJsExpression::JsComputedMemberExpression(exp.clone())).unwrap_or(false)
	{
		return None;
	}
	// check member
	...
	match member.clone() {
		// foo[foo.length - 1]
		AnyJsExpression::JsBinaryExpression(binary) => check_binary_expression_member(binary, object),
		_ => None,
	}
}
...
fn check_call_expression(call_exp: &JsCallExpression) -> Option<UseAtIndexState> {
	// check slice
	if let Some(slice_err) = analyze_slice_element_access(&AnyJsExpression::JsCallExpression(call_exp.clone())) {
		return Some(slice_err);
	}
...
fn make_at_method(object: AnyJsExpression, arg: AnyJsExpression) -> JsCallExpression {
	let at_member =
		make::js_static_member_expression(object, make::token(T![.]), make::js_name(make::ident("at")).into());
