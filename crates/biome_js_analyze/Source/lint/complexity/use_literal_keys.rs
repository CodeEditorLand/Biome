...
use biome_js_syntax::{
	AnyJsAssignment, AnyJsComputedMember, AnyJsMemberExpression, AnyJsName, AnyJsObjectMemberName, AnyTsEnumMemberName,
	JsComputedMemberName, JsSyntaxKind, T,
};
...
impl Rule for UseLiteralKeys {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		if (is_computed_member_name && !has_unescaped_new_line(value)) || is_js_ident(value) {
			return Some((inner_expression.range(), value.to_string(), is_computed_member_name));
		}
		None
	}
	...
	fn action(ctx: &RuleContext<Self>, (_, identifier, _): &Self::State) -> Option<JsRuleAction> {
		...
		match node {
			AnyJsMember::AnyJsComputedMember(node) => {
				...
				let dot_token = node.optional_chain_token().unwrap_or_else(|| make::token(T![.]));

				match node {
					AnyJsComputedMember::JsComputedMemberExpression(node) => {
						let static_expression =
							make::js_static_member_expression(object, dot_token, AnyJsName::JsName(member));
						mutation.replace_node(AnyJsMemberExpression::from(node.clone()), static_expression.into());
					},
					AnyJsComputedMember::JsComputedMemberAssignment(node) => {
						let static_member =
							make::js_static_member_assignment(object, dot_token, AnyJsName::JsName(member));
						mutation.replace_node(AnyJsAssignment::from(node.clone()), static_member.into());
					},
				}
			},
			AnyJsMember::JsComputedMemberName(member) => {
				...
				if member.syntax().parent().kind() == Some(JsSyntaxKind::TS_ENUM_MEMBER) {
					...
					mutation.replace_node(AnyTsEnumMemberName::from(member.clone()), literal_enum_member_name.into());
				} else {
					let literal_member_name = make::js_literal_member_name(name_token);
					mutation.replace_node(AnyJsObjectMemberName::from(member.clone()), literal_member_name.into());
				}
			},
		}
