...
impl Rule for UseLiteralKeys {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		// `{["__proto__"]: null }` and `{"__proto__": null}`/`{"__proto__":
		// null}` have different semantic.
		...
		// A computed property `["something"]` can always be simplified to a
		// string literal "something", unless it is a template literal inside
		// that contains unescaped new line characters:
		//
		// const a = {
		//   [`line1
		//   line2`]: true
