...
impl Default for NestedTestVisitor {
	fn default() -> Self { Self { max_count:5, curr_count:0 } }
}
...
/// Determines whether or not the call expression is for a function that is a
...
fn is_member(call:&JsCallExpression) -> bool {
	call.syntax().parent().kind().map_or(false, JsStaticMemberExpression::can_cast)
		|| call
			.callee()
