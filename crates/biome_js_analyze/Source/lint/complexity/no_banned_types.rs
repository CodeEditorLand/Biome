...
impl Rule for NoBannedTypes {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		match query {
			TsBannedType::TsObjectType(ts_object_type) => {
				// Allow empty object type for type constraint and
				// intersections. ```js
				...
				if ts_object_type.members().is_empty()
					&& (ts_object_type.parent::<TsTypeConstraintClause>().is_none()
						&& ts_object_type.parent::<TsIntersectionTypeElementList>().is_none())
				{
					return Some(State {
	...
	fn diagnostic(
		...
		State { banned_type, banned_type_range, .. }:&Self::State,
	...
	fn action(
		...
		State { banned_type, reference_identifier, .. }:&Self::State,
	) -> Option<JsRuleAction> {
...
impl BannedType {
	...
	fn message(&self) -> &str {
		match *self {
			...
			Self::Function => {
				"Prefer explicitly define the function shape. This type accepts any function-like \
				 value, which can be a common source of bugs."
			},
			Self::Object => {
				"Prefer explicitly define the object shape. This type means \"any non-nullable \
				 value\", which is slightly better than 'unknown', but it's still a broad type."
			},
			Self::EmptyObject => {
	...
	/// Retrieves a [JsReferenceIdentifier] from a [BannedType] that will be
	/// used to replace it on the rule action
