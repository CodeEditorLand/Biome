...
use biome_js_syntax::{
	AnyJsExpression, AnyJsLiteralExpression, JsConditionalExpression, JsSyntaxKind, OperatorPrecedence, T,
};
	...
	fn action(ctx: &RuleContext<Self>, _: &Self::State) -> Option<JsRuleAction> {
		...
		if alternate.text_trimmed() == consequent.text_trimmed() {
			if node_expression_kind == JsSyntaxKind::JS_IDENTIFIER_EXPRESSION {
				let res_boolean_value = if consequent.text_trimmed() == "true" { T![true] } else { T![false] };

				new_node = biome_js_syntax::AnyJsExpression::AnyJsLiteralExpression(
					AnyJsLiteralExpression::JsBooleanLiteralExpression(make::js_boolean_literal_expression(
						make::token(res_boolean_value),
					)),
				);
			} else {
				return None;
			...
			match node_expression_kind {
				JsSyntaxKind::JS_BINARY_EXPRESSION => {
					...
					let operator = node.test().ok()?.as_js_binary_expression()?.operator_token().ok()?.kind();
				...
				JsSyntaxKind::JS_INSTANCEOF_EXPRESSION => {
					let left = node.test().ok()?.as_js_instanceof_expression()?.left().ok()?;
					let right = node.test().ok()?.as_js_instanceof_expression()?.right().ok()?;
					new_node =
						make::js_instanceof_expression(left, make::token_decorated_with_space(T![instanceof]), right)
							.into();
				},
				JsSyntaxKind::JS_IN_EXPRESSION => {
					...
					new_node =
						make::js_in_expression(property, make::token_decorated_with_space(T![in]), object).into();
				},
				JsSyntaxKind::JS_UNARY_EXPRESSION => {
					let argument = node.test().ok()?.as_js_unary_expression()?.argument().ok()?;
...
fn is_boolean_expression(expression: &AnyJsExpression) -> Option<bool> {
	match expression.syntax().kind() {
		JsSyntaxKind::JS_BINARY_EXPRESSION => {
			if expression.as_js_binary_expression()?.is_comparison_operator() {
				return Some(true);
			}
		},
		JsSyntaxKind::JS_UNARY_EXPRESSION => {
...
fn invert_expression(expression: &AnyJsExpression) -> Option<AnyJsExpression> {
	if expression.syntax().kind() == JsSyntaxKind::JS_BINARY_EXPRESSION {
		let operator = expression.as_js_binary_expression()?.operator_token().ok()?;
		...
		if let Some(operator) = suggested_operator {
			...
			let new_node = AnyJsExpression::from(make::js_binary_expression(left, make::token(operator), right));
