...
use biome_js_syntax::{
	AnyJsExpression, AnyJsMemberExpression, AnyJsName, JsLogicalExpression, JsLogicalOperator, OperatorPrecedence, T,
};
	...
	fn run(ctx: &RuleContext<Self>) -> Option<Self::State> {
		...
		match operator {
			JsLogicalOperator::LogicalAnd => {
				...
				Some(UseOptionalChainState::LogicalAnd(optional_chain_expression_nodes))
			},
		...
		match state {
				...
				for subject in optional_chain_expression_nodes {
					...
					let replacement = match updated_subject {
						AnyJsExpression::JsCallExpression(call_expression) => {
							let optional_chain_token =
								call_expression.optional_chain_token().unwrap_or_else(|| make::token(T![?.]));
							call_expression.with_optional_chain_token(Some(optional_chain_token)).into()
						},
						...
						AnyJsExpression::JsComputedMemberExpression(member_expression) => {
							let optional_chain_token =
								member_expression.optional_chain_token().unwrap_or_else(|| make::token(T![?.]));
							member_expression.with_optional_chain_token(Some(optional_chain_token)).into()
						},
						_ => return None,
					};
			...
			UseOptionalChainState::LogicalOrLike(chain) => {
				...
				for (mut left, member) in chain {
					...
					let need_parenthesis = left.precedence().ok()? < OperatorPrecedence::LeftHandSide;
					if need_parenthesis {
					...
					let next_member = match member.clone() {
						AnyJsMemberExpression::JsStaticMemberExpression(expression) => {
							let static_member_expression =
								make::js_static_member_expression(left, make::token(T![?.]), expression.member().ok()?);
...
impl LogicalAndChain {
			...
			if let Some(grand_parent) = parent.parent::<JsLogicalExpression>() {
				...
				if grand_parent_logical_left.as_js_logical_expression() == Some(&parent) {
					let grand_parent_right_chain =
						LogicalAndChain::from_expression(normalized_optional_chain_like(grand_parent.right()?)?)?;
	...
	fn cmp_chain(&self, other: &LogicalAndChain) -> SyntaxResult<LogicalAndChainOrdering> {
		...
		for (main_expression, branch_expression) in self.buf.iter().zip(&other.buf) {
			let (main_expression, branch_expression, main_call_expression_args, branch_call_expression_args) =
				match (&main_expression, &branch_expression) {
					...
					_ => (main_expression.clone(), branch_expression.clone(), None, None),
				};
			let (main_value_token, branch_value_token) = match (main_expression, branch_expression) {
				(
					...
					) => (main_identifier.name()?.value_token()?, branch_identifier.name()?.value_token()?),
					(
						AnyJsExpression::AnyJsLiteralExpression(main_expression),
						AnyJsExpression::AnyJsLiteralExpression(branch_expression),
					) => (main_expression.value_token()?, branch_expression.value_token()?),
				...
				(
					...
					(AnyJsName::JsPrivateName(main_name), AnyJsName::JsPrivateName(branch_name)) => {
						(main_name.value_token()?, branch_name.value_token()?)
					},
					_ => return Ok(LogicalAndChainOrdering::Different),
				},
				...
				) => (main_expression.name()?.value_token()?, branch_expression.name()?.value_token()?),
				_ => return Ok(LogicalAndChainOrdering::Different),
			};
	...
	fn optional_chain_expression_nodes(mut self) -> Option<VecDeque<AnyJsExpression>> {
		...
		while let Some(expression) = next_chain_head.take() {
			let expression = match expression {
				...
				AnyJsExpression::JsBinaryExpression(expression) => {
					expression.is_optional_chain_like().ok()?.then_some(expression.left().ok()?)?
				},
			...
			let branch = LogicalAndChain::from_expression(normalized_optional_chain_like(head).ok()?).ok()?;
								...
								(prev_branch.buf.pop_back(), self.buf.pop_back())
							{
								match left_part {
									AnyJsExpression::JsStaticMemberExpression(ref expr)
										if expr.operator_token().is_ok_and(|token| token.kind() == T![?.]) =>
									{
		...
		if let Some(mut prev_branch) = prev_branch {
			while let (Some(left_part), Some(right_part)) = (prev_branch.buf.pop_back(), self.buf.pop_back()) {
				match left_part {
					AnyJsExpression::JsStaticMemberExpression(ref expr)
						if expr.operator_token().is_ok_and(|token| token.kind() == T![?.]) =>
					{
					...
					AnyJsExpression::JsComputedMemberExpression(ref expr) if expr.optional_chain_token().is_some() => {
						optional_chain_expression_nodes.push_front(right_part);
					},
					AnyJsExpression::JsCallExpression(ref expr) if expr.optional_chain_token().is_some() => {
						optional_chain_expression_nodes.push_front(right_part);
					},
					_ => {},
				}
...
impl LogicalOrLikeChain {
	...
	fn optional_chain_expression_nodes(&self) -> VecDeque<(AnyJsExpression, AnyJsMemberExpression)> {
		...
		while let Some(member) = next_member_chain.take() {
			...
			if let AnyJsExpression::JsLogicalExpression(logical) = object {
				let is_valid_operator = logical.operator().is_ok_and(|operator| {
					matches!(operator, JsLogicalOperator::NullishCoalescing | JsLogicalOperator::LogicalOr)
				});
	...
	fn get_chain_parent_member(logical: &JsLogicalExpression) -> Option<AnyJsMemberExpression> {
		...
		.and_then(|parent| {
			let member = match parent {
				AnyJsExpression::JsComputedMemberExpression(expression) => AnyJsMemberExpression::from(expression),
				AnyJsExpression::JsStaticMemberExpression(expression) => AnyJsMemberExpression::from(expression),
				_ => return None,
			};
			Some(member)
		})
	...
	fn get_member(expression: AnyJsExpression) -> Option<AnyJsMemberExpression> {
		let expression = iter::successors(Some(expression), |expression| match expression {
			...
			AnyJsExpression::TsNonNullAssertionExpression(expression) => expression.expression().ok(),
