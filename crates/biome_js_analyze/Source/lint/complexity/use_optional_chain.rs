	...
	fn action(ctx:&RuleContext<Self>, state:&Self::State) -> Option<JsRuleAction> {
		match state {
			UseOptionalChainState::LogicalAnd(optional_chain_expression_nodes) => {
				...
				// still need to transform the logical expression into the
				// chain.
				let logical = ctx.query();
				let replacement = {
...
impl LogicalAndChain {
	...
	fn is_inside_another_chain(&self) -> SyntaxResult<bool> {
		...
		// The grand-parent of the head is logical expression `foo && foo.bar &&
		// foo.bar.baz`
		if let Some(parent) = self.head.parent::<JsLogicalExpression>() {
			if let Some(grand_parent) = parent.parent::<JsLogicalExpression>() {
				...
				// Here we check that we came from the left side of the logical
				// expression. Because only the left-hand parts can be
				// sub-chains.
	...
	fn cmp_chain(&self, other:&LogicalAndChain) -> SyntaxResult<LogicalAndChainOrdering> {
		...
		for (main_expression, branch_expression) in self.buf.iter().zip(&other.buf) {
				...
				(main_call_expression_args, branch_call_expression_args)
			{
				...
				// they should be considered different even if
				// `main_value_token` and `branch_value_token` are the same.
	...
	fn optional_chain_expression_nodes(mut self) -> Option<VecDeque<AnyJsExpression>> {
		...
		// The parent of the head is a logical expression `foo && foo.bar &&
		// foo.bar.baz`. The next chain head is a left part of the logical
		...
		while let Some(expression) = next_chain_head.take() {
			...
			let head = match expression {
				AnyJsExpression::JsLogicalExpression(logical) => {
					if matches!(logical.operator().ok()?, JsLogicalOperator::LogicalAnd) {
						// Here we move our sub-chain head over the chains of
						// logical expression
			...
			match self.cmp_chain(&branch).ok()? {
				LogicalAndChainOrdering::SubChain => {
					...
					// After splitting the main buffer will be `[foo]` and the
					// tail will be `[bar, baz]`. It means that we need to
					// transform `bar` (first tail expression) into the optional
					// one.
...
/// Buffer is `[(foo, (foo ?? {}).bar), ((foo ?? {}).bar, ((foo ?? {}).bar ||
/// {}).baz)]`
///
...
impl LogicalOrLikeChain {
	...
	fn optional_chain_expression_nodes(
		...
		while let Some(member) = next_member_chain.take() {
			...
			if let AnyJsExpression::JsLogicalExpression(logical) = object {
				...
				// E.g. `((foo || {}).baz() || {}).bar`
				// If current member chain is `bar` the next member chain is
				// baz. Need to downward traversal to find first
				// `JsAnyExpression` which we can't include in chain
	...
	/// Need to downward traversal to find first `JsAnyExpression` which we
	/// can't include in chain.
	fn get_member(expression:AnyJsExpression) -> Option<AnyJsMemberExpression> {
		let expression = iter::successors(Some(expression), |expression| {
			match expression {
