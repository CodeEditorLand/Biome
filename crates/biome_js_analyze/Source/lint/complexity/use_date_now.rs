...
use biome_js_syntax::{
	...
	JsSyntaxKind, JsSyntaxNode, JsUnaryOperator, T,
};
...
impl Rule for UseDateNow {
	...
	fn action(ctx: &RuleContext<Self>, (node, _): &Self::State) -> Option<JsRuleAction> {
		...
		let args =
			make::js_call_arguments(make::token(T!['(']), make::js_call_argument_list([], []), make::token(T![')']));

		let mut date_now_expr = make::js_static_member_expression(
			make::js_identifier_expression(make::js_reference_identifier(make::ident("Date"))).into(),
			make::token(T![.]),
		...
		if does_node_needs_space_before_child(&node.syntax().parent()?) {
			...
			date_now_expr = date_now_expr.trim_leading_trivia()?.prepend_trivia_pieces(leading_trivia)?;
		}
...
fn get_date_method_issue(call_expr: &JsCallExpression) -> Option<(AnyJsExpression, UseDateNowIssueKind)> {
	...
	let object = callee.as_js_static_member_expression()?.object().ok()?.omit_parentheses();
...
fn get_new_date_issue(expr: &JsNewExpression) -> Option<(AnyJsExpression, UseDateNowIssueKind)> {
	...
	if callee.get_callee_member_name()?.token_text_trimmed() != "Date" || expr.arguments()?.args().len() > 0 {
		return None;
	}
...
fn get_parent_without_parenthesis(node: &JsSyntaxNode) -> Option<AnyJsExpression> {
	node.ancestors()
		.skip(1)
		.find(|ancestor| {
			ancestor.kind() != JsSyntaxKind::JS_PARENTHESIZED_EXPRESSION && AnyJsExpression::can_cast(ancestor.kind())
		})
