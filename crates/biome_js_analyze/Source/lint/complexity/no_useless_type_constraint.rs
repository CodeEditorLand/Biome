...
use biome_js_syntax::{
	AnyTsType, JsFileSource, JsSyntaxKind, T, TsTypeConstraintClause, TsTypeParameter, TsTypeParameterList,
};
use biome_rowan::{AstNode, AstSeparatedList, BatchMutationExt, SyntaxNodeOptionExt, trim_leading_trivia_pieces};
	...
	fn action(ctx: &RuleContext<Self>, _state: &Self::State) -> Option<JsRuleAction> {
			...
			.filter(|type_param| type_param.default().is_none())
		{
			...
			if type_params.len() == 1
				...
				&& type_params.syntax().grand_parent().kind() == Some(JsSyntaxKind::JS_ARROW_FUNCTION_EXPRESSION)
			{
				let name = type_param.name().ok()?;
				let trailing_pieces = name.syntax().last_trailing_trivia()?.pieces();
				let new_name = name.with_trailing_trivia_pieces(trim_leading_trivia_pieces(trailing_pieces))?;
				...
				let new_type_params = make::ts_type_parameter_list([new_type_param], [make::token(T![,])]);
				mutation.replace_node(type_params, new_type_params);
			} else {
				let prev = node.syntax().prev_sibling()?;
				// Remove the extra space
				mutation.replace_element_discard_trivia(prev.clone().into(), prev.trim_trailing_trivia()?.into());
