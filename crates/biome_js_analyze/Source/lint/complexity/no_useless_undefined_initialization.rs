...
use biome_rowan::{AstNode, BatchMutationExt, SyntaxTriviaPiece, TextRange, chain_trivia_pieces};
...
impl Rule for NoUselessUndefinedInitialization {
	...
	fn action(ctx:&RuleContext<Self>, state:&Self::State) -> Option<JsRuleAction> {
		...
		let current_declaration =
			declarators.clone().into_iter().filter_map(|declarator| declarator.ok()).find(
				|decl| {
					decl.id()
						.ok()
						.and_then(|id| {
							id.as_any_js_binding()?.as_js_identifier_binding()?.name_token().ok()
						})
		...
		let new_declarators:Vec<JsVariableDeclarator> = declarators
			...
			.map(|decl| if decl == current_declaration { new_declaration.clone() } else { decl })
			.collect();
		...
		// Create the whole statement using updated subtree and append comments to the
		// statement
