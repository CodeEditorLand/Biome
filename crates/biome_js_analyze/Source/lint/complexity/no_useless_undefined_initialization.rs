...
impl Rule for NoUselessUndefinedInitialization {
	...
	fn action(ctx: &RuleContext<Self>, state: &Self::State) -> Option<JsRuleAction> {
		...
		let current_declaration =
			declarators
				...
				.find(|decl| {
					decl.id()
						.ok()
						.and_then(|id| id.as_any_js_binding()?.as_js_identifier_binding()?.name_token().ok())
		...
		let new_declaration_statement =
			current_declaration_statement.with_declarators(js_variable_declarator_list(new_declarators, separators));
