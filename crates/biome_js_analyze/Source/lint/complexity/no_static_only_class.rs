...
impl Rule for NoStaticOnlyClass {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		// If the class is empty, we can't be sure that the user can use a
		// module-based approach instead Also, the user might be in the
		// process of writing a class, so we won't warn them yet
		if class_declaration.members().is_empty() {
			return None;
		}

		// If the class has decorators, we can't be sure that the user can use a
		// module-based approach instead
