use crate::{JsRuleAction, services::control_flow::AnyJsControlFlowRoot, services::semantic::Semantic};
use biome_analyze::{
...
use biome_js_syntax::{
	AnyJsBinding, AnyJsBindingPattern, AnyJsExpression, JsArrowFunctionExpression, JsAssignmentExpression,
	JsExpressionStatement, JsIdentifierBinding, JsIdentifierExpression, JsThisExpression, JsVariableDeclaration,
	JsVariableDeclarator, JsVariableStatement, T,
};
...
impl Rule for NoUselessThisAlias {
	...
	fn action(ctx: &RuleContext<Self>, id: &Self::State) -> Option<JsRuleAction> {
		...
		if var_declarator_list.len() == 1 {
			...
			for (current_declarator, current_comma) in var_declarator_list.iter().zip(var_declarator_list.separators())
			{
