...
use biome_js_factory::make::{js_binary_expression, js_string_literal, js_string_literal_expression};
use biome_js_syntax::{AnyJsExpression, AnyJsLiteralExpression, JsBinaryExpression, JsBinaryOperator};
...
impl Rule for NoUselessStringConcat {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		if parent_binary_expression.is_some() && get_useless_concat(&parent_binary_expression?).is_some() {
			return None;
		}
	...
	fn action(ctx: &RuleContext<Self>, _state: &Self::State) -> Option<JsRuleAction> {
		...
		let fix_result = match (left, left_string, right_string) {
			...
			(Some(AnyJsExpression::JsBinaryExpression(left_binary_expression)), None, Some(right_string_value)) => {
				let binary_expression = concat_binary_expression(&left_binary_expression, right_string_value.as_str())?;
			...
			(
				...
				Ok(AnyJsExpression::JsBinaryExpression(left_binary_expression)) => {
					let binary_expression = concat_binary_expression(&left_binary_expression, right_string.as_str())?;
...
fn is_string_expression(expression: &Option<AnyJsExpression>) -> bool {
	expression.as_ref().is_some_and(|node| {
		match (node.as_any_js_literal_expression(), node.as_js_template_expression()) {
			(Some(literal_expression), _) => literal_expression.as_js_string_literal_expression().is_some(),
...
fn is_binary_expression_with_literal_string(expression: &Option<AnyJsExpression>) -> bool {
	if let Some(AnyJsExpression::JsBinaryExpression(binary_expression)) = expression {
		...
		let has_left_identifier_expression =
			matches!(&binary_expression.left().ok(), Some(AnyJsExpression::JsIdentifierExpression(_)));
...
fn is_concatenation(binary_expression: &JsBinaryExpression) -> Option<TextRange> {
	...
	let has_right_numeric_expression = is_numeric_expression(&right) || is_numeric_calculation(&right);
...
fn is_parenthesized_concatenation(expression: &Option<AnyJsExpression>) -> bool {
	if let Some(AnyJsExpression::JsParenthesizedExpression(parenthesized_expression)) = expression {
		return is_binary_expression_with_literal_string(&parenthesized_expression.expression().ok());
	}
...
fn extract_string_value(expression: &Option<AnyJsExpression>) -> Option<String> {
	match expression {
		Some(AnyJsExpression::AnyJsLiteralExpression(AnyJsLiteralExpression::JsStringLiteralExpression(
			string_literal_expression,
		))) => string_literal_expression
			.inner_string_text()
		...
		Some(AnyJsExpression::AnyJsLiteralExpression(AnyJsLiteralExpression::JsNumberLiteralExpression(
		...
		Some(AnyJsExpression::JsBinaryExpression(binary_expression)) => {
			match (
				...
				(Some(left_string), Some(right_string)) => Some(left_string + right_string.as_str()),
				_ => None,
			}
		},
		...
		Some(AnyJsExpression::JsTemplateExpression(template_expression)) => {
			...
			if is_useless_template_literal {
				let concatenated_string =
					template_expression.elements().into_iter().fold(String::new(), |acc, element| {
...
fn concat_binary_expression(
	...
	if is_string_expression(&current_right) {
		...
		let string_literal_expression = AnyJsExpression::AnyJsLiteralExpression(AnyJsLiteralExpression::from(
		...
		return match left {
			AnyJsExpression::JsBinaryExpression(binary_expression) => {
				...
				if is_binary_expression_with_literal_string(&Some(expression.clone())) {
					...
					Some(js_binary_expression(expression.clone(), operator, string_literal_expression))
				}
			},
			_ => Some(js_binary_expression(left.clone(), operator, string_literal_expression)),
		};
	}
