...
use biome_js_syntax::{
	AnyJsExportNamedSpecifier, AnyJsNamedImportSpecifier, AnyJsObjectBindingPatternMember, JsExportNamedFromSpecifier,
	JsExportNamedSpecifier, JsNamedImportSpecifier, JsObjectBindingPatternProperty,
};
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let (old_name, new_name) = match renaming {
			...
			JsRenaming::JsExportNamedSpecifier(x) => {
				(x.local_name().ok()?.value_token().ok()?, x.exported_name().ok()?.value().ok()?)
			},
			JsRenaming::JsNamedImportSpecifier(x) => (
				x.name().ok()?.value().ok()?,
				x.local_name().ok()?.as_js_identifier_binding()?.name_token().ok()?,
			),
	...
	fn action(ctx: &RuleContext<Self>, _: &Self::State) -> Option<JsRuleAction> {
		...
		match renaming {
			JsRenaming::JsExportNamedFromSpecifier(x) => {
				...
				let replacing_token = last_token
					.append_trivia_pieces(trim_leading_trivia_pieces(export_as_last_token.trailing_trivia().pieces()));
				mutation.remove_node(export_as);
				mutation.replace_token_discard_trivia(last_token, replacing_token);
			},
			JsRenaming::JsExportNamedSpecifier(x) => {
				let replacing = make::js_export_named_shorthand_specifier(x.local_name().ok()?).build();
				mutation.replace_node(AnyJsExportNamedSpecifier::from(x.clone()), replacing.into());
			},
			JsRenaming::JsNamedImportSpecifier(x) => {
				let replacing = make::js_shorthand_named_import_specifier(x.local_name().ok()?).build();
				mutation.replace_node(AnyJsNamedImportSpecifier::from(x.clone()), replacing.into());
			},
			JsRenaming::JsObjectBindingPatternProperty(x) => {
				let mut replacing_builder =
					make::js_object_binding_pattern_shorthand_property(x.pattern().ok()?.as_any_js_binding()?.clone());
