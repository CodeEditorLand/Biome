...
use crate::{
	react::{jsx_member_name_is_react_fragment, jsx_reference_identifier_is_fragment},
...
impl Rule for NoUselessFragments {
		...
		match node {
			NoUselessFragmentsQuery::JsxFragment(fragment) => {
				let parents_where_fragments_must_be_preserved =
					node.syntax().parent().map_or(false, |parent| {
						match JsxTagExpression::try_cast(parent) {
							Ok(parent) => {
								parent
									.syntax()
									.parent()
			...
			NoUselessFragmentsQuery::JsxElement(element) => {
				...
				if is_valid_react_fragment {
					...
					// The `Fragment` component supports only the "key" prop and
					// react emits a warning for not supported props.
					// We assume that the user knows - and fixed - that and only
					// care about the prop that is actually supported.
					let attribute_key =
						opening_element.attributes().into_iter().find_map(|attribute| {
	...
	fn action(ctx:&RuleContext<Self>, state:&Self::State) -> Option<JsRuleAction> {
		...
		let in_jsx_attr = node
			.syntax()
			.grand_parent()
			.map_or(false, |parent| JsxExpressionAttributeValue::can_cast(parent.kind()));
		...
		if is_in_list {
			...
			if let Some(child) = child {
				let new_node = match child {
					...
					// can't apply a code action because it will create invalid
					// syntax for example `<>{...foo}</>` would become
					// `{...foo}` which would produce a syntax error
				...
				// can't apply a code action when there is no children because
				// it will create invalid syntax for example `<div
				// x-some-prop={<></>}` would become `<div x-some-prop=` which
