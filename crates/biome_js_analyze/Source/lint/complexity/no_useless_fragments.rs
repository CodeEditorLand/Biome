...
use biome_js_factory::make::{
	JsxExpressionChildBuilder, js_string_literal_expression, jsx_expression_attribute_value, jsx_expression_child,
	jsx_string, jsx_string_literal, jsx_tag_expression, token,
};
use biome_js_syntax::{
	AnyJsExpression, AnyJsxChild, AnyJsxElementName, AnyJsxTag, JsLanguage, JsLogicalExpression,
	JsParenthesizedExpression, JsSyntaxKind, JsxAttributeInitializerClause, JsxChildList, JsxElement,
	JsxExpressionAttributeValue, JsxExpressionChild, JsxFragment, JsxTagExpression, JsxText, T,
};
		...
		match node {
			NoUselessFragmentsQuery::JsxFragment(fragment) => {
				let parents_where_fragments_must_be_preserved =
					node.syntax()
						.parent()
						.is_some_and(|parent| match JsxTagExpression::try_cast(parent.clone()) {
							Ok(parent) => parent
								.syntax()
								.parent()
								.and_then(|parent| {
									...
									match JsParenthesizedExpression::try_cast(parent) {
										Ok(parenthesized_expression) => parenthesized_expression.syntax().parent(),
					...
					for child in child_list.iter() {
						match child.syntax().kind() {
							...
							JsSyntaxKind::JSX_TEXT => {
								...
								if (in_jsx_expr || in_js_logical_expr) && contains_html_character_references(child_text)
								{
									children_where_fragments_must_preserved = true;
									break;
								}
			...
			NoUselessFragmentsQuery::JsxElement(element) => {
				...
				if is_valid_react_fragment {
					...
					let attribute_key = opening_element.attributes().into_iter().find_map(|attribute| {
					...
					if attribute_key.is_none() {
						return match child_list.first() {
							Some(first) if child_list.len() == 1 => Some(NoUselessFragmentsState::Child(first)),
		...
		if is_in_list {
			match state {
				...
				NoUselessFragmentsState::Children(children) => {
					if let Some(old_children) =
						node.syntax().parent().and_then(|parent| JsxChildList::cast(parent.clone()))
					{
			...
			let child = node.children().iter().find(|child| match child.syntax().kind() {
				JsSyntaxKind::JSX_SELF_CLOSING_ELEMENT
				| JsSyntaxKind::JSX_ELEMENT
				| JsSyntaxKind::JSX_EXPRESSION_CHILD
			...
			if let Some(child) = child {
				let new_node = match child {
					...
					AnyJsxChild::JsxSelfClosingElement(node) => {
						Some(jsx_tag_expression(AnyJsxTag::JsxSelfClosingElement(node)).into_syntax())
					},
					AnyJsxChild::JsxText(text) => {
						...
						if parent.kind() == JsSyntaxKind::JSX_EXPRESSION_ATTRIBUTE_VALUE {
							...
							Some(js_string_literal_expression(jsx_string_literal(new_value)).into_syntax())
						}
					},
					AnyJsxChild::JsxExpressionChild(child) => {
						if is_in_jsx_attr || !JsxTagExpression::can_cast(node.syntax().parent()?.kind()) {
							child.expression().map(|expression| {
								let jsx_expr_child = jsx_expression_child(token(T!['{']), token(T!['}']));
								JsxExpressionChildBuilder::with_expression(jsx_expr_child, expression)
									.build()
									.into_syntax()
							})
						} else {
							child.expression().map(|expression| expression.into_syntax())
						}
					},
