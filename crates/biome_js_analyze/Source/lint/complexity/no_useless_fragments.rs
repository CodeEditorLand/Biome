		...
		match node {
					...
					for child in child_list.iter() {
						match child.syntax().kind() {
							...
							JsSyntaxKind::JSX_TEXT => {
								...
								// Since in the JSX newlines aren't trivia, we require to allocate a
								// string to trim from those characters.
			...
			NoUselessFragmentsQuery::JsxElement(element) => {
				...
				if is_valid_react_fragment {
					...
					// The `Fragment` component supports only the "key" prop and react emits a
					// warning for not supported props. We assume that the user knows - and
					// fixed - that and only care about the prop that is actually supported.
					let attribute_key =
	...
	fn action(ctx:&RuleContext<Self>, state:&Self::State) -> Option<JsRuleAction> {
		...
		if is_in_list {
			...
			if let Some(child) = child {
				...
				// can't apply a code action when there is no children because it will create
				// invalid syntax for example `<div x-some-prop={<></>}` would become `<div
				// x-some-prop=` which would produce a syntax error
				return None;
			}
		}
