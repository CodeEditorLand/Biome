...
use biome_js_syntax::{
	AnyJsCallArgument, AnyJsClass, AnyJsConstructorParameter, AnyJsFormalParameter, JsCallExpression,
...
impl Rule for NoUselessConstructor {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let is_not_public = constructor.modifiers().iter().any(|modifier| !modifier.is_public());
		if is_not_public {
			return None;
		}
		for parameter in constructor.parameters().ok()?.parameters() {
			let decorators = match parameter.ok()? {
				...
				AnyJsConstructorParameter::AnyJsFormalParameter(AnyJsFormalParameter::JsFormalParameter(parameter)) => {
...
fn is_delegating_initialization(constructor: &JsConstructorClassMember, super_call: &JsCallExpression) -> bool {
	let result = || {
		...
		for (param, arg) in zipped {
			...
			match (param, arg) {
				(
					AnyJsConstructorParameter::AnyJsFormalParameter(AnyJsFormalParameter::JsFormalParameter(param)),
					_,
				) if param.initializer().is_some() => {
					return Some(false);
				},
				(AnyJsConstructorParameter::JsRestParameter(param), AnyJsCallArgument::JsSpread(arg)) => {
				...
				(AnyJsConstructorParameter::AnyJsFormalParameter(param), AnyJsCallArgument::AnyJsExpression(expr)) => {
					let param_name = param
						...
						.name_token()
						.ok()?;
					let arg_name = expr.as_js_identifier_expression()?.name().ok()?.value_token().ok()?;
					if param_name.text_trimmed() != arg_name.text_trimmed() {
						return Some(false);
					}
				},
