		...
		for parameter in constructor.parameters().ok()?.parameters() {
			let decorators = match parameter.ok()? {
				...
				| AnyJsConstructorParameter::TsPropertyParameter(_) => {
					// Ignore constructors with Bogus parameters or parameter
					// properties
					return None;
				},
				AnyJsConstructorParameter::AnyJsFormalParameter(
...
///
/// This checks that constructors' **all** parameters are passed to the
/// super-call in the same order.
			...
			match (param, arg) {
				...
				(
					...
					let param_name = param
						...
						.ok()?;
					let arg_name =
						expr.as_js_identifier_expression()?.name().ok()?.value_token().ok()?;
					if param_name.text_trimmed() != arg_name.text_trimmed() {
						return Some(false);
					}
				},
