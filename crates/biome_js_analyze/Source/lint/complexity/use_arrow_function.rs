...
impl Rule for UseArrowFunction {
	...
	fn action(ctx:&RuleContext<Self>, _:&Self::State) -> Option<JsRuleAction> {
		...
		if let Some(type_parameters) = function_expression.type_parameters() {
			...
			// Keep a trailing comma when there is a single type parameter in arrow
			// functions and JSX is enabled Or the parser will treat it as a JSX tag and
			// fail to parse it.
...
/// Returns `true` if `function_expression` needs parenthesis when turned into
/// an arrow function.
