...
use biome_analyze::{
	AddVisitor, FixKind, Phases, QueryMatch, Queryable, Rule, RuleDiagnostic, RuleSource, RuleSourceKind, ServiceBag,
...
use biome_js_syntax::{
	AnyJsExpression, AnyJsFunctionBody, AnyJsStatement, JsConstructorClassMember, JsFileSource, JsFunctionBody,
	JsFunctionDeclaration, JsFunctionExportDefaultDeclaration, JsFunctionExpression, JsGetterClassMember,
	JsGetterObjectMember, JsLanguage, JsMethodClassMember, JsMethodObjectMember, JsModule, JsScript,
	JsSetterClassMember, JsSetterObjectMember, JsStaticInitializationBlockClassMember, JsSyntaxKind, T,
};
use biome_rowan::{
	AstNode, AstNodeList, AstSeparatedList, BatchMutationExt, Language, SyntaxNode, TextRange, TriviaPieceKind,
...
impl Rule for UseArrowFunction {
	...
	fn action(ctx: &RuleContext<Self>, _: &Self::State) -> Option<JsRuleAction> {
		...
		if let Some(type_parameters) = function_expression.type_parameters() {
			let mut type_parameters_iter = type_parameters.items().iter().filter_map(|item| item.ok());
		...
		if let Some(return_type_annotation) = function_expression.return_type_annotation() {
			arrow_function_builder = arrow_function_builder.with_return_type_annotation(return_type_annotation);
		}
		...
		mutation.replace_node(AnyJsExpression::from(function_expression.clone()), arrow_function);
		...
		match event {
			WalkEvent::Enter(node) => {
				...
				if let Some(scope) = AnyThisScope::cast_ref(node) {
					self.stack.push(AnyThisScopeMetadata { scope, has_this: false });
				} else if matches!(
					node.kind(),
...
fn to_arrow_body(body: JsFunctionBody) -> AnyJsFunctionBody {
	...
	if matches!(return_arg, AnyJsExpression::JsSequenceExpression(_))
		|| return_arg.syntax().first_token().is_some_and(|token| token.kind() == T!['{'])
	{
		// () => (first, second)
		// () => ({ ... })
