...
impl Rule for UseArrowFunction {
	...
	fn action(ctx:&RuleContext<Self>, _:&Self::State) -> Option<JsRuleAction> {
		...
		if let Some(type_parameters) = function_expression.type_parameters() {
			...
			// Keep a trailing comma when there is a single type parameter in
			// arrow functions and JSX is enabled Or the parser will treat it
			// as a JSX tag and fail to parse it.
...
/// Returns `true` if `function_expr` needs parenthesis when turned into an
/// arrow function.
fn needs_parentheses(function_expression:&JsFunctionExpression) -> bool {
	function_expression.syntax().parent().is_some_and(|parent| {
		// Copied from the implementation of `NeedsParentheses` for
		// `JsArrowFunctionExpression` in the `biome_js_formatter` crate.
...
impl QueryMatch for ActualThisScope {
	fn text_range(&self) -> TextRange { self.0.scope.range() }
}
...
impl Queryable for ActualThisScope {
	...
	fn unwrap_match(_:&ServiceBag, query:&Self::Input) -> Self::Output { query.0.clone() }
}

#[derive(Default)]
struct AnyThisScopeVisitor {
	/// Vector to hold a function or block where `this` is scoped.
	/// The function or block is associated to a boolean indicating whether it
	/// contains `this`.
	...
	fn visit(
		...
		match event {
			WalkEvent::Enter(node) => {
				// When the visitor enters a function node, push a new entry on
				// the stack
				...
				if matches!(
					...
					// When the visitor enters a `this` expression or
					// `new.target`, set the `has_this` flag for the top
					// entry on the stack to `true`.
