...
use biome_js_syntax::{
	AnyJsClass, AnyJsClassMember, AnyJsExpression, JsArrowFunctionExpression, JsSuperExpression, JsSyntaxToken,
	JsThisExpression,
};
...
impl Rule for NoThisInStatic {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let static_method = this_super_expression
			.syntax()
			.ancestors()
			.find(|x| AnyJsControlFlowRoot::can_cast(x.kind()) && !JsArrowFunctionExpression::can_cast(x.kind()))
	...
	fn action(ctx: &RuleContext<Self>, _: &Self::State) -> Option<JsRuleAction> {
		let this_super_expression = ctx.query();
		let class = this_super_expression.syntax().ancestors().find_map(AnyJsClass::cast)?;
		let suggested_class_name = if let JsThisSuperExpression::JsSuperExpression(_) = this_super_expression {
