...
impl Rule for NoUselessSwitchCase {
	...
	fn action(ctx: &RuleContext<Self>, useless_case: &Self::State) -> Option<JsRuleAction> {
		...
		if consequent.len() > 0 {
			...
			let new_default_clause = default_clause.clone().with_consequent(consequent).with_colon_token(
				default_clause_colon_token
					.append_trivia_pieces(useless_case.colon_token().ok()?.trailing_trivia().pieces()),
			);
			mutation.remove_node(default_clause.clone());
			mutation.replace_node(AnyJsSwitchClause::from(useless_case), new_default_clause.into());
		} else {
