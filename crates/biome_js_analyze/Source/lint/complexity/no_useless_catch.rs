...
impl Rule for NoUselessCatch {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		// We need guarantees that body_statements has only one `throw`
		// statement.
		...
		let throw_ident = js_throw_statement.argument().ok()?.as_js_identifier_expression()?.text();
	...
	fn action(ctx:&RuleContext<Self>, _:&Self::State) -> Option<JsRuleAction> {
		...
		let note = if node.finally_clause().is_some() {
			...
			let new_stmts = prev_stmts.chain(try_stmts).chain(next_stmts).collect::<Vec<_>>();
