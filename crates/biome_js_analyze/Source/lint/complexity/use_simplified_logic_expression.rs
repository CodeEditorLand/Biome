...
use biome_js_syntax::{
	AnyJsExpression, AnyJsLiteralExpression, JsBooleanLiteralExpression, JsLogicalExpression, JsUnaryExpression,
	JsUnaryOperator, T,
};
...
impl Rule for UseSimplifiedLogicExpression {
	...
	fn run(ctx: &RuleContext<Self>) -> Option<Self::State> {
		...
		match node.operator().ok()? {
			...
			biome_js_syntax::JsLogicalOperator::LogicalOr => {
				if let AnyJsExpression::AnyJsLiteralExpression(AnyJsLiteralExpression::JsBooleanLiteralExpression(
					literal,
				)) = left
				{
				...
				if could_apply_de_morgan(node).unwrap_or(false) {
					return simplify_de_morgan(node).map(|expr| (true, AnyJsExpression::JsUnaryExpression(expr)));
				}
			},
	...
	fn action(ctx: &RuleContext<Self>, state: &Self::State) -> Option<JsRuleAction> {
		...
		mutation.replace_node(AnyJsExpression::JsLogicalExpression(node.clone()), expr.clone());
...
fn could_apply_de_morgan(node: &JsLogicalExpression) -> Option<bool> {
	...
	let right = node.right().ok()?;
	match (left, right) {
		(AnyJsExpression::JsUnaryExpression(left), AnyJsExpression::JsUnaryExpression(right)) => Some(
			matches!(left.operator().ok()?, JsUnaryOperator::LogicalNot)
				...
				&& !matches!(right.argument().ok()?, AnyJsExpression::JsUnaryExpression(_)),
		),
		_ => Some(false),
	}
}
...
fn simplify_or_expression(literal: JsBooleanLiteralExpression, expression: AnyJsExpression) -> Option<AnyJsExpression> {
	...
	match (left, right) {
		(AnyJsExpression::JsUnaryExpression(left), AnyJsExpression::JsUnaryExpression(right)) => {
			let mut next_logic_expression = match operator_token.kind() {
				T![||] => node.clone().replace_token(operator_token, make::token(T![&&])),
				T![&&] => node.clone().replace_token(operator_token, make::token(T![||])),
				_ => return None,
			}?;
			...
			Some(make::js_unary_expression(
				make::token(T![!]),
				AnyJsExpression::JsParenthesizedExpression(make::parenthesized(AnyJsExpression::JsLogicalExpression(
