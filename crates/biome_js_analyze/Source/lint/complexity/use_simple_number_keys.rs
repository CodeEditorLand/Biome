...
use biome_js_syntax::{
	AnyJsObjectMember, JsLiteralMemberName, JsObjectExpression, JsSyntaxKind, JsSyntaxToken, TextRange,
};
...
pub enum NumberLiteral {
	Binary { node: JsLiteralMemberName, value: Box<str>, big_int: bool },
	Decimal { node: JsLiteralMemberName, value: Box<str>, big_int: bool, underscore: bool },
	Octal { node: JsLiteralMemberName, value: Box<str>, big_int: bool },
	Hexadecimal { node: JsLiteralMemberName, value: Box<str>, big_int: bool },
	FloatingPoint { node: JsLiteralMemberName, value: Box<str>, exponent: bool, underscore: bool },
}
...
impl TryFrom<AnyJsObjectMember> for NumberLiteral {
	...
	fn try_from(any_member: AnyJsObjectMember) -> Result<Self, Self::Error> {
		let Some(literal_member_name) = any_member.syntax().children().find_map(JsLiteralMemberName::cast) else {
		...
		match token.kind() {
			JsSyntaxKind::JS_NUMBER_LITERAL | JsSyntaxKind::JS_BIGINT_LITERAL => {
				...
				match is_second_char_a_letter {
					Some(b'b' | b'B') => {
						return Ok(Self::Binary { node: literal_member_name, value: value.into_boxed_str(), big_int });
					},
					Some(b'o' | b'O') => {
						return Ok(Self::Octal { node: literal_member_name, value: value.into_boxed_str(), big_int });
					},
					Some(b'x' | b'X') => {
				...
				Ok(Self::Decimal { node: literal_member_name, value: value.into_boxed_str(), big_int, underscore })
			},
...
impl NumberLiteral {
	fn to_base_ten(&self) -> Option<f64> {
		match self {
			...
			Self::Decimal { value, .. } | Self::FloatingPoint { value, .. } => f64::from_str(value).ok(),
			Self::Octal { value, .. } => i64::from_str_radix(value, 7).map(|num| num as f64).ok(),
			Self::Hexadecimal { value, .. } => i64::from_str_radix(value, 16).map(|num| num as f64).ok(),
		}
	}
}
...
impl Rule for UseSimpleNumberKeys {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
			...
			.filter_map(|member| NumberLiteral::try_from(member).ok())
		{
			match number_literal {
				...
				NumberLiteral::FloatingPoint { underscore: true, .. }
				| NumberLiteral::Decimal { underscore: true, .. } => {
					result.push(RuleState(WrongNumberLiteralName::WithUnderscore, number_literal))
				},
				NumberLiteral::Binary { .. } => result.push(RuleState(WrongNumberLiteralName::Binary, number_literal)),
				NumberLiteral::Hexadecimal { .. } => {
					result.push(RuleState(WrongNumberLiteralName::Hexadecimal, number_literal))
				},
				NumberLiteral::Octal { .. } => result.push(RuleState(WrongNumberLiteralName::Octal, number_literal)),
				_ => (),
			}
		}
	...
	fn diagnostic(_ctx: &RuleContext<Self>, RuleState(reason, literal): &Self::State) -> Option<RuleDiagnostic> {
		let title = match reason {
			WrongNumberLiteralName::BigInt => "Bigint is not allowed here.",
			WrongNumberLiteralName::WithUnderscore => "Number literal with underscore is not allowed here.",
			WrongNumberLiteralName::Binary => "Binary number literal in is not allowed here.",
			WrongNumberLiteralName::Hexadecimal => "Hexadecimal number literal is not allowed here.",
	...
	fn action(ctx: &RuleContext<Self>, RuleState(reason, literal): &Self::State) -> Option<JsRuleAction> {
		let mut mutation = ctx.root().begin();
		let token = literal.token().ok()?;
		let token_text = token.text_trimmed().to_string();

		let message = match reason {
			WrongNumberLiteralName::Binary | WrongNumberLiteralName::Octal | WrongNumberLiteralName::Hexadecimal => {
