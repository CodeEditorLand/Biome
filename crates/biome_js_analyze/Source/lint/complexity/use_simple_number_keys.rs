...
pub enum NumberLiteral {
	Binary { node: JsLiteralMemberName, value: String, big_int: bool },
	Decimal { node: JsLiteralMemberName, value: String, big_int: bool, underscore: bool },
	Octal { node: JsLiteralMemberName, value: String, big_int: bool },
	Hexadecimal { node: JsLiteralMemberName, value: String, big_int: bool },
	FloatingPoint { node: JsLiteralMemberName, value: String, exponent: bool, underscore: bool },
}
...
impl TryFrom<AnyJsObjectMember> for NumberLiteral {
	...
	fn try_from(any_member: AnyJsObjectMember) -> Result<Self, Self::Error> {
		let Some(literal_member_name_syntax) =
			any_member.syntax().children().find(|x| JsLiteralMemberName::can_cast(x.kind()))
		else {
		...
		match token.kind() {
			JsSyntaxKind::JS_NUMBER_LITERAL | JsSyntaxKind::JS_BIGINT_LITERAL => {
				...
				match is_second_char_a_letter {
					Some('b' | 'B') => {
						return Ok(Self::Binary { node: literal_member_name, value, big_int })
					}
					Some('o' | 'O') => {
						return Ok(Self::Octal { node: literal_member_name, value, big_int })
					}
					Some('x' | 'X') => {
						return Ok(Self::Hexadecimal { node: literal_member_name, value, big_int })
					}
					_ => (),
				}
				...
				Ok(Self::Decimal { node: literal_member_name, value, big_int, underscore })
			}
			...
			.filter_map(|member| NumberLiteral::try_from(member).ok())
		{
			match number_literal {
				...
				| NumberLiteral::Decimal { underscore: true, .. } => {
					signals.push(RuleState(WrongNumberLiteralName::WithUnderscore, number_literal))
				}
				NumberLiteral::Binary { .. } => {
					signals.push(RuleState(WrongNumberLiteralName::Binary, number_literal))
				}
				NumberLiteral::Hexadecimal { .. } => {
					signals.push(RuleState(WrongNumberLiteralName::Hexadecimal, number_literal))
				}
				NumberLiteral::Octal { .. } => {
