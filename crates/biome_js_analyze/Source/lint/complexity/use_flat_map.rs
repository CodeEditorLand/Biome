...
impl Rule for UseFlatMap {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		...
		let flat_member_expression = AnyJsMemberExpression::cast(flat_call.callee().ok()?.into_syntax())?;
		if flat_member_expression.member_name()?.text() == "flat" {
			let Ok(AnyJsExpression::JsCallExpression(map_call)) = flat_member_expression.object() else {
				return None;
			};
			let map_call_arguments = map_call.arguments().ok()?.args();
			let map_member_expression = AnyJsMemberExpression::cast(map_call.callee().ok()?.into_syntax())?;
	...
	fn action(ctx: &RuleContext<Self>, flat_call: &Self::State) -> Option<JsRuleAction> {
		...
		let Ok(AnyJsExpression::JsStaticMemberExpression(old_static_member_expression)) = flat_call.callee() else {
			return None;
		};
		let member = js_name(ident("flatMap"));

		let flat_map_member_expression = old_static_member_expression.with_member(AnyJsName::JsName(member));

		let flat_map_call = flat_call
			.clone()
			.with_callee(AnyJsExpression::JsStaticMemberExpression(flat_map_member_expression));
