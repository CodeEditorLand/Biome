...
impl Rule for NoExcessiveCognitiveComplexity {
	...
	fn diagnostic(ctx:&RuleContext<Self>, _:&Self::State) -> Option<RuleDiagnostic> {
		let CognitiveComplexity { function_like, score: ComplexityScore { calculated_score } } =
			ctx.query();

		let ComplexityOptions { max_allowed_complexity } = ctx.options();

		let range = function_like
			.name_range()
			.or_else(|| function_like.function_token().map(|token| token.text_range()))
			.or_else(|| function_like.fat_arrow_token().map(|token| token.text_range()))
		...
		Some(
			RuleDiagnostic::new(
				...
				markup!({
					format!(
						"Excessive complexity of {calculated_score} detected (max: \
						 {max_allowed_complexity})."
					)
				}),
			)
			.note(if calculated_score == &MAX_SCORE {
				"Please refactor this function to reduce its complexity. It's currently too \
				 complex or too deeply nested to calculate an accurate score."
					.to_owned()
			} else {
				format!(
...
impl QueryMatch for CognitiveComplexity {
	fn text_range(&self) -> TextRange { self.function_like.range() }
}
...
impl CognitiveComplexityVisitor {
	fn on_enter(&mut self, node:&JsSyntaxNode) {
		...
		// When the visitor enters a function node, push a new entry on the
		// stack
		if let Some(function_like) = AnyFunctionLike::cast_ref(node) {
			if self.stack.len() < MAX_FUNCTION_DEPTH {
				self.stack.push(CognitiveComplexityFunctionState {
					function_like,
					score:0,
					nesting_level:parent.map(|parent| parent.nesting_level + 1).unwrap_or_default(),
			...
			if let Some(function_state) = self.stack.pop() {
				if function_state.function_like == exit_node {
					ctx.match_query(CognitiveComplexity {
						...
						score:ComplexityScore { calculated_score:function_state.score },
					});
				} else {
...
///
/// Note: This is a strict subset of the nodes that receive a structural
/// penalty.
...
pub struct ComplexityOptions {
	/// The maximum complexity score that we allow. Anything higher is
	/// considered excessive.
	pub max_allowed_complexity:NonZeroU8,
}

impl Default for ComplexityOptions {
	fn default() -> Self { Self { max_allowed_complexity:NonZeroU8::new(15).unwrap() } }
}
