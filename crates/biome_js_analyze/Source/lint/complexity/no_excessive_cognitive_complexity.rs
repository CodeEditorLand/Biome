...
impl Rule for NoExcessiveCognitiveComplexity {
	...
	fn diagnostic(ctx: &RuleContext<Self>, _: &Self::State) -> Option<RuleDiagnostic> {
		...
		Some(
			RuleDiagnostic::new(rule_category!(), range, markup!("Excessive complexity detected."))
...
fn receives_structural_penalty(node: &JsSyntaxNode) -> bool {
	...
	receives_nesting_penalty(node)
		...
		|| JsBreakStatement::cast_ref(node).and_then(|js_break| js_break.label_token()).is_some()
