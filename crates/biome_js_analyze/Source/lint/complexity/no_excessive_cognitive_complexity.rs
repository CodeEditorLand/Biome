...
use biome_js_syntax::{
	AnyFunctionLike, JsBreakStatement, JsContinueStatement, JsElseClause, JsLanguage, JsLogicalExpression,
	JsLogicalOperator, JsSyntaxNode,
};
...
impl Rule for NoExcessiveCognitiveComplexity {
	...
	fn diagnostic(ctx: &RuleContext<Self>, _: &Self::State) -> Option<RuleDiagnostic> {
		let CognitiveComplexity { function_like, score: ComplexityScore { calculated_score } } = ctx.query();
	...
	fn on_enter(&mut self, node: &JsSyntaxNode) {
		...
		if let Some(state) = self.stack.last_mut() {
			...
			if increases_nesting(node) {
				...
				JsLogicalExpression::cast_ref(node).and_then(|expression| expression.operator().ok())
			{
				if state.last_seen_operator != Some(operator) {
					state.score = state.score.saturating_add(1);
					state.last_seen_operator = Some(operator);
				}
			} else if let Some(alternate) = JsElseClause::cast_ref(node).and_then(|js_else| js_else.alternate().ok()) {
...
fn is_loop_node(node: &JsSyntaxNode) -> bool {
	...
	matches!(
		node.kind(),
		JS_DO_WHILE_STATEMENT | JS_FOR_OF_STATEMENT | JS_FOR_IN_STATEMENT | JS_FOR_STATEMENT | JS_WHILE_STATEMENT
	)
}
...
impl Default for ComplexityOptions {
	fn default() -> Self {
		Self { max_allowed_complexity: NonZeroU8::new(15).unwrap() }
	}
}
