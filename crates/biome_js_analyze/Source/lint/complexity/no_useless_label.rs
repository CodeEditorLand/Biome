...
impl Rule for NoUselessLabel {
	...
	fn action(ctx:&RuleContext<Self>, _:&Self::State) -> Option<JsRuleAction> {
		...
		// We want to remove trailing spaces and keep all comments that follows
		// `stmt_token` e.g. `break /* a comment */  ` to `break /* a comment
		// */`. This requires to traverse the trailing trivia in reverse
		// order. We keep trailing trivia of `label_stmt`
		// e.g. `break label // a comment` -> `break // a comment`
		// We do not keep leading trivia of `label_stmt` because we assume that
		// they are associated to the label.
