...
use biome_js_syntax::{
	AnyJsStatement, AnyJsSwitchClause, JsBlockStatement, JsFileSource, JsLabeledStatement, JsStatementList,
...
impl Rule for NoUselessLoneBlockStatements {
	...
	fn action(ctx: &RuleContext<Self>, _state: &Self::State) -> Option<JsRuleAction> {
		...
		let block_pos = stmts_list.iter().position(|x| x.syntax() == block.syntax())?;
		...
		let new_stmts: Vec<_> = prev_stmts.chain(inner_statements).chain(next_stmts).collect();
...
fn in_control_structure(block: &JsBlockStatement) -> bool {
	if let Some(node) = block.syntax().parent() {
		...
		if let JsSyntaxKind::JS_ELSE_CLAUSE | JsSyntaxKind::JS_CATCH_CLAUSE | JsSyntaxKind::JS_FINALLY_CLAUSE =
			syntax_kind
		{
			return true;
		}
...
fn is_not_var_declaration(variable: &JsVariableStatement) -> bool {
	variable.declaration().ok().is_some_and(|decl| !decl.is_var())
}
