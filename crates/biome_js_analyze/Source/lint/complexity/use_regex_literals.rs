...
use biome_js_syntax::{
	AnyJsCallArgument, AnyJsExpression, AnyJsLiteralExpression, JsCallArguments, JsComputedMemberExpression,
	JsNewOrCallExpression, JsSyntaxKind, JsSyntaxToken, global_identifier, static_value::StaticValue,
};
...
impl Rule for UseRegexLiterals {
	...
	fn action(ctx: &RuleContext<Self>, state: &Self::State) -> Option<JsRuleAction> {
		...
		let token = JsSyntaxToken::new_detached(
			...
			&format!("/{}/{}", state.pattern, state.flags.as_deref().unwrap_or_default()),
			[],
			[],
		);
		let next =
			AnyJsExpression::AnyJsLiteralExpression(AnyJsLiteralExpression::from(js_regex_literal_expression(token)));
...
fn create_pattern(pattern: Result<AnyJsCallArgument, SyntaxError>, model: &SemanticModel) -> Option<String> {
...
fn extract_literal_string(from: AnyJsCallArgument) -> Option<String> {
	...
	expr.omit_parentheses().as_static_value().and_then(|value| match value {
