...
impl Rule for NoExtraBooleanCast {
	...
	fn run(ctx:&RuleContext<Self>) -> Option<Self::State> {
		...
		// Convert `!!x` -> `x` if parent `SyntaxNode` in any boolean `Type
		// Coercion` context
		if parent_node_in_boolean_cast_context {
			...
			// Convert `Boolean(x)` -> `x` if parent `SyntaxNode` in any boolean
			// `Type Coercion` context Only if `Boolean` Call Expression have
			// one `JsAnyExpression` argument
			...
			// Convert `new Boolean(x)` -> `x` if parent `SyntaxNode` in any
			// boolean `Type Coercion` context Only if `Boolean` Call
			// Expression have one `JsAnyExpression` argument
		...
		let (title, note) = match extra_boolean_cast_type {
			ExtraBooleanCastType::DoubleNegation => {
				(
					...
					"It is not necessary to use double-negation when a value will already be \
			...
			ExtraBooleanCastType::BooleanCall => {
				(
					...
					"It is not necessary to use `Boolean` call when a value will already be \
					 coerced to a boolean.",
				)
			},
		};
...
/// !(!x)
/// ```
/// Return [Rule::State] `(JsAnyExpression, ExtraBooleanCastType)` if it is a
/// DoubleNegation Expression
			...
			AnyJsExpression::JsParenthesizedExpression(expr) => {
				expr.expression().ok().and_then(|expr| {
					is_negation(expr.syntax()).and_then(|negation| {
						Some((negation.argument().ok()?, ExtraBooleanCastType::DoubleNegation))
					})
				})
			},
			_ => None,
