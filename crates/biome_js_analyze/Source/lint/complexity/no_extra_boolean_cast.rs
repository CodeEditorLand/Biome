...
impl Rule for NoExtraBooleanCast {
	...
	fn run(ctx:&RuleContext<Self>) -> Option<Self::State> {
		...
		// Convert `!!x` -> `x` if parent `SyntaxNode` in any boolean `Type Coercion`
		// context
		if parent_node_in_boolean_cast_context {
			...
			// Convert `Boolean(x)` -> `x` if parent `SyntaxNode` in any boolean `Type
			// Coercion` context Only if `Boolean` Call Expression have one
			// `JsAnyExpression` argument
			...
			// Convert `new Boolean(x)` -> `x` if parent `SyntaxNode` in any boolean `Type
			// Coercion` context Only if `Boolean` Call Expression have one
			// `JsAnyExpression` argument
