...
use biome_js_syntax::{
	AnyJsExpression, JsCallArgumentList, JsCallArguments, JsCallExpression, JsNewExpression, JsSyntaxNode,
	JsUnaryOperator, is_in_boolean_context, is_negation,
};
...
impl Rule for NoExtraBooleanCast {
	...
	fn run(ctx: &RuleContext<Self>) -> Option<Self::State> {
		...
		let parent_node_in_boolean_cast_context = is_in_boolean_context(n.syntax()).unwrap_or(false)
...
fn is_double_negation_ignore_parenthesis(
	...
	if let Some(negation_expr) = is_negation(syntax) {
		...
		match argument {
			AnyJsExpression::JsUnaryExpression(expr) if expr.operator().ok()? == JsUnaryOperator::LogicalNot => expr
				.argument()
				.ok()
				.map(|argument| (argument, ExtraBooleanCastType::DoubleNegation)),
			// Check edge case `!(!xxx)`
			AnyJsExpression::JsParenthesizedExpression(expr) => expr.expression().ok().and_then(|expr| {
				is_negation(expr.syntax())
					.and_then(|negation| Some((negation.argument().ok()?, ExtraBooleanCastType::DoubleNegation)))
			}),
			_ => None,
		}
	} else {
