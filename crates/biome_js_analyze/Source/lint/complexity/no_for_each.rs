...
impl Rule for NoForEach {
	...
	fn run(ctx: &RuleContext<Self>) -> Self::Signals {
		let node = ctx.query();
		let member_expression = AnyJsMemberExpression::cast(node.callee().ok()?.omit_parentheses().into_syntax())?;
		...
		if !options.allowed_identifiers.is_empty() {
			...
			if let Some(reference) = object.as_js_reference_identifier() {
				...
				if options.allowed_identifiers.iter().any(|identifier| identifier.as_ref() == name) {
					return None;
				}
			}
		}
		...
		let [Some(first), _, None] = node.arguments().ok()?.get_arguments_by_index([0, 1, 2]) else {
			return None;
		};
		// Report calls that use a callbacks with 0 or 1 parameter.
		let parameter_count = match first.as_any_js_expression()? {
			AnyJsExpression::JsArrowFunctionExpression(function) => function.parameters().ok()?.len(),
			AnyJsExpression::JsFunctionExpression(function) => function.parameters().ok()?.items().len(),
			_ => return None,
		};
