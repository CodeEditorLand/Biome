...
use biome_js_syntax::{AnyJsExportClause, AnyJsImportClause, AnyJsModuleItem, JsNamedImportSpecifiers, JsSyntaxToken};
...
impl Rule for NoTypeOnlyImportAttributes {
		...
		match module_item {
			...
			AnyJsModuleItem::JsExport(export) => match export.export_clause().ok()? {
				...
				AnyJsExportClause::JsExportNamedFromClause(clause) => {
					...
					Some(RuleState { assertion_range, type_token_range: type_token.text_trimmed_range() })
				},
			},
			AnyJsModuleItem::JsImport(import) => match import.import_clause().ok()? {
				...
				AnyJsImportClause::JsImportCombinedClause(clause) => {
					let assertion_range = clause.assertion()?.range();
					let type_token = find_first_type_token(clause.specifier().ok()?.as_js_named_import_specifiers()?)?;
					Some(RuleState { assertion_range, type_token_range: type_token.text_trimmed_range() })
				},
	...
	fn diagnostic(ctx: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
		let node = ctx.query();
		let import_or_export = if matches!(node, AnyJsModuleItem::JsImport(_)) { "import" } else { "export" };
		Some(
			RuleDiagnostic::new(
				rule_category!(),
