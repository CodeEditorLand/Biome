	...
	W: WriteColor,
{
	fn write_str(&mut self, elements:&MarkupElements, content:&str) -> io::Result<()> {
		with_format(&mut self.0, elements, |writer| {
			let mut adapter = SanitizeAdapter { writer, error:Ok(()) };
			...
			match adapter.write_fmt(content) {
				...
				Err(..) => {
					if adapter.error.is_err() {
						...
						Err(io::Error::new(io::ErrorKind::Other, "a Display formatter returned an error"))
					}
				},
			}
		})
...
fn with_format<W>(writer:&mut W, state:&MarkupElements, func:impl FnOnce(&mut W) -> io::Result<()>) -> io::Result<()>
where
	W: WriteColor, {
...
fn grapheme_is_whitespace(grapheme:&str) -> bool { grapheme.chars().all(|c| c.is_whitespace()) }
