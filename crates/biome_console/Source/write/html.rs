...
impl<W> HTML<W> {
	pub fn new(writer:W) -> Self { Self(writer, false) }
	...
	elements.for_each(&mut |styles| {
		for style in styles {
			match style {
				...
				MarkupElement::Inverse => write!(fmt, "<span style=\"color: #000; background-color: #ddd;\">")?,
...
mod test {
	...
	fn does_not_escape_curly_braces() {
		...
		assert_eq!(String::from_utf8(buf).unwrap(), "New rules that are {still} under development.");
	}
