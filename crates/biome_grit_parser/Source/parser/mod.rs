...
use crate::{constants::*, token_source::GritTokenSource};
...
impl<'source> GritParser<'source> {
	pub fn new(source:&'source str) -> Self {
		Self { context:ParserContext::default(), source:GritTokenSource::from_str(source) }
	}

	pub fn lookahead(&mut self) -> GritSyntaxKind { self.source.lookahead() }

	pub fn finish(self) -> (Vec<Event<GritSyntaxKind>>, Vec<ParseDiagnostic>, Vec<Trivia>) {
...
fn parse_version(p:&mut GritParser) -> ParsedSyntax {
	...
	if p.eat(T![biome]) {
		if p.eat(T!['(']) {
			...
			p.error(expected_engine_version(p, TextRange::new(engine_end, engine_end)));
	...
	Present(m.complete(p, if is_supported { GRIT_VERSION } else { GRIT_BOGUS_VERSION }))
}
...
#[inline]
fn parse_variable_list(p:&mut GritParser) { VariableList.parse_list(p); }
...
impl ParseSeparatedList for VariableList {
	...
	fn parse_element(&mut self, p:&mut Self::Parser<'_>) -> ParsedSyntax { parse_variable(p) }
