...
use super::{
	parse_error::{expected_list_pattern, expected_map_element, expected_pattern},
	parse_name,
	patterns::{parse_maybe_curly_pattern, parse_pattern},
	GritParser,
};
use crate::constants::*;
...
impl ParseSeparatedList for ListPatternList {
	...
	fn parse_element(&mut self, p:&mut Self::Parser<'_>) -> ParsedSyntax { parse_list_pattern(p) }
...
fn parse_list_pattern(p:&mut GritParser) -> ParsedSyntax {
	if p.at(DOT3) { parse_dotdotdot(p) } else { parse_pattern(p) }
}
...
impl ParseSeparatedList for MapElementList {
	...
	fn parse_element(&mut self, p:&mut Self::Parser<'_>) -> ParsedSyntax { parse_map_element(p) }
