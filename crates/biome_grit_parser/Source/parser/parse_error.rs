use biome_parser::{diagnostic::expected_node, prelude::ParseDiagnostic, Parser};
...
pub(crate) fn expected_predicate_call_arg(p:&GritParser, range:TextRange) -> ParseDiagnostic {
	p.err_builder("Unexpected predicate call argument.", range).with_hint(
		"Predicate call arguments must be patterns, optionally preceeded by a name and an equal \
		 sign.",
	)
}

pub(crate) fn expected_predicate_infix_operator(p:&GritParser, range:TextRange) -> ParseDiagnostic {
...
pub(crate) fn too_many_patterns(p:&GritParser, range:TextRange) -> ParseDiagnostic {
	p.err_builder("Too many patterns.", range).with_hint(
		"Grit files may only contain a single pattern. Use `sequential` if you would like to \
