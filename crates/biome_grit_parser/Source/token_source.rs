...
impl<'source> GritTokenSource<'source> {
	pub fn from_str(source: &'source str) -> Self {
		...
		let mut source = Self { lexer, trivia: Vec::new(), current: NonTriviaToken::default(), next: None };
		...
		loop {
			...
			match trivia_kind {
				...
				Ok(trivia_kind) => {
					...
					self.trivia.push(Trivia::new(trivia_kind, self.lexer.current_range(), trailing));
				},
			}
		}
