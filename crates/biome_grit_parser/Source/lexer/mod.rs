//! An extremely fast, lookup table based, GritQL lexer which yields SyntaxKind
//! tokens used by the biome Grit parser.
...
impl<'src> Lexer<'src> for GritLexer<'src> {
	...
	fn next_token(&mut self, _context:Self::LexContext) -> Self::Kind {
		...
		let kind = match self.current_byte() {
			Some(current) => {
				...
				debug_assert!(self.current_start < self.text_position(), "Lexer did not progress");
				kind
			},
			None => EOF,
		};
	...
	fn has_preceding_line_break(&self) -> bool { self.after_newline }
					...
					match byte {
						b' ' | b'\t' | b'\n' | b'\r' => {
							...
							self.diagnostics.push(
								ParseDiagnostic::new(
									"GritQL only allows tabs, whitespace, carriage return and \
			...
			LexNumberState::Invalid { position, reason } => {
				let diagnostic = match reason {
					...
					InvalidNumberReason::Octal => {
						ParseDiagnostic::new(
							"GritQL doesn't allow octal number notation (numbers starting with \
							 zero)",
							position..position + TextSize::from(1),
						)
					},
			...
			match chr {
				...
				b'\\' => {
					...
					match self.current_byte() {
						...
						Some(b'u') => {
							match (self.consume_unicode_escape(), state) {
								(Ok(_), _) => {},
	...
	fn consume_name(&mut self, first:u8) -> GritSyntaxKind {
		...
		while let Some(byte) = self.current_byte() {
			if is_identifier_byte(byte) {
				...
				return match &buffer[..len] {
					...
					_ => {
						self.diagnostics.push(
							...
							.with_hint(
								"Use a language annotation to create a language-specific snippet \
								 or use the `r` prefix to create a regex literal.",
							)
							.with_alternatives(
								"Supported language annotations are:",
								SUPPORTED_LANGUAGE_SET_STR,
							),
						);
				...
				return match &buffer[..len] {
					b"r" => {
						match self.consume_backtick_snippet() {
...
fn is_leading_identifier_byte(byte:u8) -> bool { byte.is_ascii_alphabetic() || byte == b'_' }
