...
use biome_parser::{diagnostic::ParseDiagnostic, tree_sink::LosslessTreeSink, AnyParse};
...
/// Parses the provided string as a GritQL pattern using the provided node
/// cache.
...
impl GritParse {
	...
	/// # }
	/// ```
	pub fn syntax(&self) -> GritSyntaxNode { self.root.clone() }
	...
	pub fn tree(&self) -> GritRoot { GritRoot::unwrap_cast(self.syntax()) }
}
