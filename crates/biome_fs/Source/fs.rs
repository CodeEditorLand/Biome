...
impl From<PathKind> for oxc_resolver::FileMetadata {
	fn from(kind: PathKind) -> Self {
		match kind {
			PathKind::File { is_symlink } => oxc_resolver::FileMetadata::new(true, false, is_symlink),
			PathKind::Directory { is_symlink } => oxc_resolver::FileMetadata::new(false, true, is_symlink),
		}
	}
}
...
pub trait FileSystem: Send + Sync + RefUnwindSafe {
	...
	fn auto_search_files(&self, search_dir: &Utf8Path, search_files: &[&str]) -> Option<AutoSearchResult> {
			...
			for file_name in search_files {
				...
				match self.read_file_from_path(&file_path) {
					Ok(content) => {
						...
						return Some(AutoSearchResult { content, file_path, directory_path: current_search_dir });
					},
					_ => continue,
				}
			}
	...
	fn resolve_configuration(&self, specifier: &str, path: &Utf8Path) -> Result<FsResolution, ResolveError>;
}
...
impl Advices for FsErrorKind {
	fn record(&self, visitor: &mut dyn Visit) -> io::Result<()> {
		match self {
			...
			Self::NonUtf8Path => {
				visitor.record_log(LogCategory::Error, &"Biome encountered a path with invalid UTF-8 characters.")
			},
		}
	}
}
