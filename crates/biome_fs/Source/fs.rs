...
pub trait FileSystem: Send + Sync + RefUnwindSafe {
	...
	fn read_file_from_path(
		&self,
		file_path: &PathBuf,
	) -> Result<String, FileSystemDiagnostic> {
		match self
...
pub trait FileSystemExt: FileSystem {
	...
	fn create(&self, path: &Path) -> io::Result<Box<dyn File>> {
		self.open_with_options(
			path,
			OpenOptions::default().write(true).create(true).truncate(true),
		)
	}
	...
	fn create_new(&self, path: &Path) -> io::Result<Box<dyn File>> {
		self.open_with_options(
			path,
			OpenOptions::default().read(true).write(true).create_new(true),
		)
	}
}
...
type BoxedTraversal<'fs, 'scope> =
	Box<dyn FnOnce(&dyn TraversalScope<'scope>) + Send + 'fs>;
	...
	fn fmt(&self, fmt: &mut console::fmt::Formatter) -> io::Result<()> {
		match self {
			...
			ErrorKind::DereferencedSymlink(_) => {
				fmt.write_str("Dereferenced symlink")
			},
