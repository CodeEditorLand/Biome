...
use biome_diagnostics::{console, Advices, Diagnostic, Error, LogCategory, Severity, Visit};
...
impl ConfigName {
	...
	pub const fn biome_json() -> &'static str { Self::BIOME_JSON[0] }

	pub const fn biome_jsonc() -> &'static str { Self::BIOME_JSON[1] }

	pub const fn file_names() -> [&'static str; 2] { Self::BIOME_JSON }
}
...
pub trait FileSystem: Send + Sync + RefUnwindSafe {
	...
	fn deprecated_config_name(&self) -> &str { ROME_JSON }
	...
	/// This method accepts a directory path (`search_dir`) and a list of
	/// filenames (`file_names`), It looks for the files in the specified
	/// directory in the order they appear in the list. If a file is not found
	/// in the initial directory, the search may continue into the parent
	/// directories based on the `should_error_if_file_not_found` flag.
	///
	/// Behavior if files are not found in `search_dir`:
	///
	/// - If `should_error_if_file_not_found` is set to `true`, the method will
	///   return an error.
	/// - If `should_error_if_file_not_found` is set to `false`, the method will
	///   search for the files in the parent directories of `search_dir`
	///   recursively until:
	///     - It finds a file, reads it, and returns its contents along with its
	///       path.
	///     - It confirms that the file doesn't exist in any of the checked
	///       directories.
	///
	/// ## Errors
	///
	/// The method returns an error if `should_error_if_file_not_found` is
	/// `true`, and the file is not found or cannot be opened or read.
	fn auto_search(
		&self,
		search_dir:&Path,
		file_names:&[&str],
		...
		loop {
			...
			if !is_searching_in_parent_dir && should_error_if_file_not_found {
				if let Some(diagnostic) = errors.into_iter().next() {
					// We can only return one Err, so we return the first
					// diagnostic.
	...
	///
	/// This method attempts to open and read the entire content of a file at
	/// the given path.
	///
	/// ## Errors
	/// This method logs an error message and returns a `FileSystemDiagnostic`
	/// error in two scenarios:
	/// - If the file cannot be opened, possibly due to incorrect path or
	///   permission issues.
	/// - If the file is opened but its content cannot be read, potentially due
	///   to the file being damaged.
		...
		match self.open_with_options(file_path, OpenOptions::default().read(true)) {
			Ok(mut file) => {
				...
				match file.read_to_string(&mut content) {
					Ok(_) => Ok(content),
					Err(err) => {
						error!("Biome couldn't read the file {:?}, reason:\n{:?}", file_path, err);
			...
			Err(err) => {
				error!("Biome couldn't open the file {:?}, reason:\n{:?}", file_path, err);
				Err(FileSystemDiagnostic {
					path:file_path.display().to_string(),
					severity:Severity::Error,
					error_kind:ErrorKind::CantReadFile(file_path.display().to_string()),
...
pub trait FileSystemExt: FileSystem {
	...
	fn create(&self, path:&Path) -> io::Result<Box<dyn File>> {
		self.open_with_options(path, OpenOptions::default().write(true).create(true).truncate(true))
	}
	...
	fn create_new(&self, path:&Path) -> io::Result<Box<dyn File>> {
		self.open_with_options(path, OpenOptions::default().read(true).write(true).create_new(true))
	}
}
...
pub trait TraversalScope<'scope> {
	...
	///
	/// If the provided path exists and is a file, then the
	/// [`handle_file`](TraversalContext::handle_path) method of the provided
	/// [TraversalContext] will be called. If it's a directory, it will be
	/// recursively traversed and all the files the
	...
	/// Spawn a new filesystem read task.
	///
	/// It's assumed that the provided already exist and was already evaluated
...
pub trait TraversalContext: Sync {
	...
	/// to emit diagnostics for IO errors that may happen in the traversal
	/// process
	...
	T: FileSystem + Send,
{
	...
	fn traversal<'scope>(&'scope self, func:BoxedTraversal<'_, 'scope>) { T::traversal(self, func) }

	fn working_directory(&self) -> Option<PathBuf> { T::working_directory(self) }

	fn path_exists(&self, path:&Path) -> bool { T::path_exists(self, path) }

	fn path_is_file(&self, path:&Path) -> bool { T::path_is_file(self, path) }

	fn path_is_dir(&self, path:&Path) -> bool { T::path_is_dir(self, path) }
...
impl std::fmt::Display for ErrorKind {
	fn fmt(&self, fmt:&mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		match self {
			...
			ErrorKind::DereferencedSymlink(_) => {
				write!(fmt, "Dereferenced symlink")
			},
			ErrorKind::DeeplyNestedSymlinkExpansion(_) => {
