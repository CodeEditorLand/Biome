...
// NOTE: The order of the variants is important, the one on the top has the
// highest priority
pub enum FileKind {
	/// A configuration file has the highest priority. It's usually `biome.json`
	/// and `biome.jsonc`
	///
	...
	Inspectable,
	/// A file to handle has the lowest priority. It's usually a traversed file,
	/// or a file opened by the LSP
	#[default]
	Handleable,
}
...
impl From<SmallVec<[FileKind; 5]>> for FileKinds {
	fn from(value:SmallVec<[FileKind; 5]>) -> Self {
		value.into_iter().fold(FileKinds::default(), |mut acc, kind| {
			acc.insert(kind);
...
impl From<FileKinds> for SmallVec<[FileKind; 5]> {
	fn from(value:FileKinds) -> Self { value.iter().collect() }
}
...
impl From<FileKind> for FileKinds {
	fn from(flag:FileKind) -> Self { Self(BitFlags::from(flag)) }
}
...
pub struct BiomePath {
	...
	path:PathBuf,
	/// Determines the kind of the file inside Biome. Some files are considered
	/// as configuration files, others as manifest files, and others as files to
	/// handle
	kind:FileKinds,
	/// Whether this path (usually a file) was fixed as a result of a
	/// format/lint/check command with the `--write` filag.
	was_written:bool,
}
...
impl BiomePath {
	pub fn new(path_to_file:impl Into<PathBuf>) -> Self {
		...
		Self { path, kind, was_written:false }
	}
	...
	pub fn new_written(path_to_file:impl Into<PathBuf>) -> Self {
		...
		Self { path, kind, was_written:true }
	}
	...
	/// - `package.json` and `tsconfig.json`/`jsconfig.json` have the
	///   second-highest priority, and they are considered as manifest files
	...
	pub fn is_config(&self) -> bool { self.kind.contains(FileKind::Config) }

	pub fn is_manifest(&self) -> bool { self.kind.contains(FileKind::Manifest) }

	pub fn is_ignore(&self) -> bool { self.kind.contains(FileKind::Ignore) }

	pub fn is_to_inspect(&self) -> bool { self.kind.contains(FileKind::Inspectable) }
}

#[cfg(feature = "serde")]
impl schemars::JsonSchema for FileKinds {
	fn schema_name() -> String { String::from("FileKind") }
...
impl AliasPath {
	#[allow(dead_code)]
	pub fn new(name:impl Into<String>) -> Self { Self { name:name.into(), prefix:vec![] } }

	#[allow(dead_code)]
	pub fn add_prefix(&mut self, prefix:impl Into<PathBuf>) { self.prefix.push(prefix.into()); }
}
...
mod test {
	...
	fn test_biome_paths() {
		...
		assert_eq!(BiomePath::priority(OsStr::new("package.json")), FileKind::Manifest.into());

		assert_eq!(BiomePath::priority(OsStr::new("biome.json")), FileKind::Config.into());

		assert_eq!(BiomePath::priority(OsStr::new("biome.jsonc")), FileKind::Config.into());
	}
	...
	fn test_biome_file_names_order() {
		...
		assert_eq!(iter.next().unwrap().file_name(), Some(OsStr::new("biome.json")));

		assert_eq!(iter.next().unwrap().file_name(), Some(OsStr::new("biome.jsonc")));

		assert_eq!(iter.next().unwrap().file_name(), Some(OsStr::new("biome.jsonc")));

		assert_eq!(iter.next().unwrap().file_name(), Some(OsStr::new("package.json")));

		assert_eq!(iter.next().unwrap().file_name(), Some(OsStr::new("tsconfig.json")));

		assert_eq!(iter.next().unwrap().file_name(), Some(OsStr::new("README.md")));
	}
	...
	fn test_biome_paths_order() {
		...
		assert_eq!(iter.next().unwrap().display().to_string(), "src/biome.jsonc");

		assert_eq!(iter.next().unwrap().display().to_string(), "src/frontend/biome.jsonc");

		assert_eq!(iter.next().unwrap().display().to_string(), "src/frontend/package.json");

		assert_eq!(iter.next().unwrap().display().to_string(), "src/package.json");

		assert_eq!(iter.next().unwrap().display().to_string(), "src/tsconfig.json");
