...
#[cfg_attr(
	...
	serde(from = "smallvec::SmallVec<[FileKind; 5]>", into = "smallvec::SmallVec<[FileKind; 5]>")
)]
...
impl AliasPath {
	...
	pub fn new(name: impl Into<String>) -> Self {
		Self { name: name.into(), prefix: vec![] }
	}
...
impl Aliases {
	...
	pub fn get_paths_by_name(&self, name: &str) -> Option<&[PathBuf]> {
		self.aliases.iter().find(|alias| alias.name == name).map(|alias| alias.prefix.as_slice())
	}
}
