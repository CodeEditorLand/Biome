...
impl<'de> serde::Deserialize<'de> for BiomePath {
		...
		D: serde::Deserializer<'de>,
	{
		...
		let path = Utf8PathBuf::from_path_buf(path)
			.map_err(|e| serde::de::Error::custom(format!("Unable to deserialize path {}", e.display())))?;
...
impl BiomePath {
	...
	fn priority(file_name: &str) -> FileKinds {
		...
		} else if matches!(file_name, "package.json" | "tsconfig.json" | "jsconfig.json") {
...
mod test {
	...
	fn test_biome_paths() {
		...
		assert_eq!(BiomePath::priority("package.json"), FileKind::Manifest.into());
	...
	fn test_biome_paths_order() {
		...
		assert_eq!(iter.next().unwrap().to_string(), "src/frontend/package.json");
