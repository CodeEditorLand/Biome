//! Implementation of the [FileSystem] and related traits for the underlying OS
//! filesystem
use std::{
	env,
	fs,
...
use biome_diagnostics::{DiagnosticExt, Error, Severity, adapters::IoError};
...
use crate::{
	BiomePath,
	FileSystem,
	fs::{OpenOptions, TraversalContext, TraversalScope},
};

const MAX_SYMLINK_DEPTH:u8 = 3;

/// Implementation of [FileSystem] that directly calls through to the underlying
/// OS
...
impl FileSystem for OsFileSystem {
	fn open_with_options(&self, path:&Path, options:OpenOptions) -> io::Result<Box<dyn File>> {
		tracing::debug_span!("OsFileSystem::open_with_options", path = ?path, options = ?options).in_scope(
	...
	fn working_directory(&self) -> Option<PathBuf> { self.working_directory.clone() }

	fn path_exists(&self, path:&Path) -> bool { path.exists() }

	fn path_is_file(&self, path:&Path) -> bool { path.is_file() }

	fn path_is_dir(&self, path:&Path) -> bool { path.is_dir() }

	fn path_is_symlink(&self, path:&Path) -> bool { path.is_symlink() }
...
/// Traverse a single directory entry, scheduling any file to execute the
/// context handler and sub-directories for subsequent traversal
...
fn follow_symlink(path:&Path, ctx:&dyn TraversalContext) -> Result<(PathBuf, FileType), SymlinkExpansionError> {
...
impl From<FileType> for ErrorKind {
	fn from(_:FileType) -> Self { Self::UnknownFileType }
}
