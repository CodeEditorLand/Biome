...
fn handle_dir<'scope>(
	...
	path:&Path,
	// The unresolved origin path in case the directory is behind a symbolic
	// link
...
fn handle_any_file<'scope>(
	...
	if file_type.is_symlink() {
		...
		if !ctx.interner().intern_path(target_path.clone()) {
			// If the path was already inserted, it could have been pointed at
			// by multiple symlinks. No need to traverse again.
			return;
		}
