...
impl MemoryFileSystem {
	...
	pub fn new_read_only() -> Self {
		Self { allow_write: false, ..Self::default() }
	}
...
mod tests {
	...
	fn file_read_write() {
		...
		file.set_content(content_2.as_bytes()).expect("the file should be written without error");
	...
	fn file_create() {
		...
		file.set_content(b"content".as_slice()).expect("the file should be written without error");
	}
	...
	fn file_create_new() {
		...
		file.set_content(content.as_bytes()).expect("the file should be written without error");
