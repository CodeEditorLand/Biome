...
use crate::{BiomePath, FileSystem, TraversalContext, TraversalScope, fs::OpenOptions};

type OnGetChangedFiles =
	Option<Arc<AssertUnwindSafe<Mutex<Option<Box<dyn FnOnce() -> Vec<String> + Send + 'static + RefUnwindSafe>>>>>>;

/// Fully in-memory file system, stores the content of all known files in a
/// hashmap
...
impl Default for MemoryFileSystem {
	fn default() -> Self {
		Self {
			...
			allow_write:true,
			on_get_staged_files:Some(Arc::new(AssertUnwindSafe(Mutex::new(Some(Box::new(Vec::new)))))),
			on_get_changed_files:Some(Arc::new(AssertUnwindSafe(Mutex::new(Some(Box::new(Vec::new)))))),
		}
	}
}
...
/// - `AssertUnwindSafe` tells the type system this value can safely be accessed
///   again after being recovered from a panic (using `catch_unwind`), which
///   means the filesystem guarantees a file will never get into an inconsistent
///   state if a thread panics while having a handle open (a read or write
///   either happens or not, but will never panic halfway through)
...
impl MemoryFileSystem {
	...
	pub fn new_read_only() -> Self { Self { allow_write:false, ..Self::default() } }
	...
	pub fn insert_error(&mut self, path:PathBuf, kind:ErrorEntry) { self.errors.insert(path, kind); }

	/// Remove a file from the filesystem
	pub fn remove(&mut self, path:&Path) { self.files.0.write().remove(path); }
	...
	pub fn set_on_get_changed_files(&mut self, cfn:Box<dyn FnOnce() -> Vec<String> + Send + RefUnwindSafe + 'static>) {
		self.on_get_changed_files = Some(Arc::new(AssertUnwindSafe(Mutex::new(Some(cfn)))));
	}

	pub fn set_on_get_staged_files(&mut self, cfn:Box<dyn FnOnce() -> Vec<String> + Send + RefUnwindSafe + 'static>) {
...
impl FileSystem for MemoryFileSystem {
	fn open_with_options(&self, path:&Path, options:OpenOptions) -> io::Result<Box<dyn File>> {
		if !self.allow_write && (options.create || options.create_new || options.truncate || options.write) {
	...
	fn traversal<'scope>(&'scope self, func:BoxedTraversal<'_, 'scope>) { func(&MemoryTraversalScope { fs:self }) }

	fn working_directory(&self) -> Option<PathBuf> { None }

	fn path_exists(&self, path:&Path) -> bool { self.path_is_file(path) }
	...
	fn path_is_dir(&self, path:&Path) -> bool { !self.path_is_file(path) }

	fn path_is_symlink(&self, _path:&Path) -> bool { false }
	...
	fn resolve_configuration(&self, _specifier:&str, _path:&Path) -> Result<Resolution, ResolveError> { todo!() }
}
...
impl File for MemoryFile {
	fn read_to_string(&mut self, buffer:&mut String) -> io::Result<()> {
		...
		let content = str::from_utf8(&self.inner).map_err(|err| io::Error::new(io::ErrorKind::InvalidData, err))?;
	...
	fn file_version(&self) -> i32 { self.version }
}
...
impl<'scope> TraversalScope<'scope> for MemoryTraversalScope<'scope> {
		...
		// those that are prefixed with the provided `base` path
		{
			...
			for path in files.keys() {
				let should_process_file = if base.starts_with(".") || base.starts_with("./") {
					// we simulate absolute paths, so we can correctly strips out the base path from
					// the path
				...
				ctx.push_diagnostic(Error::from(FileSystemDiagnostic {
					...
					error_kind:match entry {
						...
						ErrorEntry::DeeplyNestedSymlinkExpansion(path) => {
							ErrorKind::DeeplyNestedSymlinkExpansion(path.to_string_lossy().to_string())
						},
					},
	...
	fn handle(&self, context:&'scope dyn TraversalContext, path:PathBuf) { context.handle_path(BiomePath::new(path)); }
}
...
mod tests {
	...
	fn fs_read_only() {
		...
		match fs.open_with_options(path, OpenOptions::default().read(true).write(true)) {
			Ok(_) => {
				panic!("fs.open_with_options(read + write) for a read-only filesystem should return an error")
			},
			Err(error) => {
				assert_eq!(error.kind(), io::ErrorKind::PermissionDenied);
			},
	...
	fn file_read_write() {
		...
		file.read_to_string(&mut buffer).expect("the file should be read without error");
	...
	fn file_create_truncate() {
		...
		assert!(buffer.is_empty(), "fs.create() should truncate the file content");
	}
	...
	fn file_create_new() {
		...
		let mut file = fs.create_new(path).expect("the file should not fail to create");
	...
	fn traversal() {
		...
		impl TraversalContext for TestContext {
			...
			fn push_diagnostic(&self, err:Error) { panic!("unexpected error {err:?}") }

			fn can_handle(&self, _:&BiomePath) -> bool { true }

			fn handle_path(&self, path:BiomePath) { self.visited.lock().insert(path.to_written()); }

			fn store_path(&self, path:BiomePath) { self.visited.lock().insert(path); }
		...
		let mut ctx = TestContext { interner, visited:Mutex::default() };
