...
use crate::{fs::OpenOptions, BiomePath, FileSystem, TraversalContext, TraversalScope};
...
impl FileSystem for MemoryFileSystem {
	fn open_with_options(&self, path:&Path, options:OpenOptions) -> io::Result<Box<dyn File>> {
		...
		let mut inner = if options.create || options.create_new {
			// Acquire write access to the files map if the file may need to be
			// created
				...
				Entry::Occupied(entry) => {
					if options.create {
						...
						*entry = Arc::new(Mutex::new(vec![]));
						...
						// This branch can only be reached if `create_new` was
						// true, we should return an error if the file
						// already exists
						return Err(io::Error::new(
		...
		// those that are prefixed with the provided `base` path
		{
			...
			for path in files.keys() {
				let should_process_file = if base.starts_with(".") || base.starts_with("./") {
					// we simulate absolute paths, so we can correctly
					// strips out the base path from the path
...
mod tests {
	...
	fn fs_read_only() {
		...
		match fs.create(path) {
			Ok(_) => {
				panic!("fs.create() for a read-only filesystem should return an error")
			},
			Err(error) => {
		...
		match fs.open_with_options(path, OpenOptions::default().read(true).write(true)) {
			Ok(_) => {
				panic!(
					"fs.open_with_options(read + write) for a read-only filesystem should return \
					 an error"
				)
			},
			Err(error) => {
	...
	fn file_create_new_exists() {
		...
		match result {
			Ok(_) => {
				panic!("fs.create_new() for an existing file should return an error")
			},
			Err(error) => {
	...
	fn missing_file() {
		...
		match result {
			Ok(_) => {
				panic!("opening a non-existing file should return an error")
			},
			Err(error) => {
