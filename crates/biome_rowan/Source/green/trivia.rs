...
pub(crate) fn has_live() -> bool { countme::get::<GreenTrivia>().live > 0 }
...
impl GreenTriviaData {
	#[allow(unused)]
	#[inline]
	pub fn header(&self) -> &GreenTriviaHead { &self.data.header }

	#[inline]
	pub fn pieces(&self) -> &[TriviaPiece] { self.data.slice() }
...
impl PartialEq for GreenTriviaData {
	fn eq(&self, other:&Self) -> bool { self.pieces() == other.pieces() }
}
...
/// List of trivia. Used to store either the leading or trailing trivia of a
/// token. The identity of a trivia is defined by the kinds and lengths of its
/// items but not by the texts of an individual piece. That means, that `\r` and
/// `\n` can both be represented by the same trivia, a trivia with a single
/// `LINEBREAK` piece with the length 1. This is safe because the text is stored
/// on the token to which the trivia belongs and `a\n` and `a\r` never resolve
/// to the same tokens. Thus, they only share the trivia but are otherwise two
/// different tokens.
...
impl fmt::Debug for GreenTrivia {
	fn fmt(&self, f:&mut Formatter<'_>) -> fmt::Result { fmt::Debug::fmt(self.pieces(), f) }
}
...
impl GreenTrivia {
	...
	pub fn empty() -> Self { GreenTrivia { ptr:None } }
	...
	pub fn get_piece(&self, index:usize) -> Option<&TriviaPiece> { self.pieces().get(index) }

	pub(crate) fn into_raw(self) -> *mut GreenTriviaData {
		self.ptr
			.map_or_else(ptr::null_mut, |ptr| Arc::from_thin(ptr).into_raw().cast().as_ptr())
	}
