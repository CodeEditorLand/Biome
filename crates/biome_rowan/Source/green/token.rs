...
use crate::{
	...
	green::{trivia::GreenTrivia, RawSyntaxKind},
	TextSize,
};
...
pub(crate) fn has_live() -> bool { countme::get::<GreenToken>().live > 0 }
...
impl PartialEq for GreenTokenData {
	fn eq(&self, other:&Self) -> bool { self.kind() == other.kind() && self.text() == other.text() }
}
...
impl Borrow<GreenTokenData> for GreenToken {
	#[inline]
	fn borrow(&self) -> &GreenTokenData { self }
}
...
impl GreenTokenData {
	/// Kind of this Token.
	#[inline]
	pub fn kind(&self) -> RawSyntaxKind { self.data.header.kind }

	/// Whole text of this Token, including all trivia.
	#[inline]
	pub fn text(&self) -> &str { unsafe { std::str::from_utf8_unchecked(self.data.slice()) } }
	...
	#[inline]
	pub fn text_len(&self) -> TextSize { TextSize::of(self.text()) }

	#[inline]
	pub fn leading_trivia(&self) -> &GreenTrivia { &self.data.header.leading }

	#[inline]
	pub fn trailing_trivia(&self) -> &GreenTrivia { &self.data.header.trailing }
}
...
impl GreenToken {
	...
	pub fn with_trivia(
		...
		let head = GreenTokenHead { kind, leading, trailing, _c:Count::new() };
