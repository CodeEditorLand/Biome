...
pub(crate) fn has_live() -> bool { countme::get::<GreenNode>().live > 0 }
...
impl Borrow<GreenNodeData> for GreenNode {
	#[inline]
	fn borrow(&self) -> &GreenNodeData { self }
}

impl From<Cow<'_, GreenNodeData>> for GreenNode {
	#[inline]
	fn from(cow:Cow<'_, GreenNodeData>) -> Self { cow.into_owned() }
}

impl From<&'_ GreenNodeData> for GreenNode {
	#[inline]
	fn from(borrow:&'_ GreenNodeData) -> Self { borrow.to_owned() }
}
...
impl GreenNodeData {
	#[inline]
	fn header(&self) -> &GreenNodeHead { &self.data.header }

	#[inline]
	pub(crate) fn slice(&self) -> &[Slot] { self.data.slice() }

	/// Kind of this node.
	#[inline]
	pub fn kind(&self) -> RawSyntaxKind { self.header().kind }

	/// Returns the length of the text covered by this node.
	#[inline]
	pub fn text_len(&self) -> TextSize { self.header().text_len }

	/// Children of this node.
	#[inline]
	pub fn children(&self) -> Children<'_> { Children::new(self.slots().enumerate()) }

	/// Returns the slots of this node. Every node of a specific kind has the
	/// same number of slots to allow using fixed offsets to retrieve a
	/// specific child even if some other child is missing.
	#[inline]
	pub fn slots(&self) -> Slots<'_> { Slots { raw:self.slice().iter() } }

	pub(crate) fn slot_at_range(&self, rel_range:TextRange) -> Option<(usize, TextSize, &'_ Slot)> {
		let idx = self
            .slice()
            .binary_search_by(|it| {
		...
		let slot = &self.slice().get(idx).filter(|it| it.rel_range().contains_range(rel_range))?;
...
impl GreenNode {
	...
	pub(crate) fn into_raw(self) -> ptr::NonNull<GreenNodeData> {
		// SAFETY: casting from `HeaderSlice<GreenNodeHead,
		// [green::node::Slot]>` to `GreenNodeData` if safe since
		// `GreenNodeData` is marked as `repr(transparent)`
...
impl<'a> Iterator for Slots<'a> {
	...
	#[inline]
	fn next(&mut self) -> Option<&'a Slot> { self.raw.next() }
	...
	#[inline]
	fn nth(&mut self, n:usize) -> Option<Self::Item> { self.raw.nth(n) }
...
impl<'a> DoubleEndedIterator for Slots<'a> {
	#[inline]
	fn next_back(&mut self) -> Option<Self::Item> { self.raw.next_back() }

	#[inline]
	fn nth_back(&mut self, n:usize) -> Option<Self::Item> { self.raw.nth_back(n) }
...
impl<'a> Child<'a> {
	...
	pub fn rel_offset(&self) -> TextSize { self.rel_offset }

	pub fn element(&self) -> GreenElementRef<'a> { self.element }
}
...
impl<'a> Children<'a> {
	pub fn new(slots:Enumerate<Slots<'a>>) -> Self { Self { slots } }
}

impl<'a> Iterator for Children<'a> {
	type Item = Child<'a>;

	fn next(&mut self) -> Option<Self::Item> { self.slots.find_map(|it| Child::try_from(it).ok()) }
...
mod tests {
	...
	fn children() {
		...
		assert_eq!(
			root.children().map(|child| child.element.to_string()).collect::<Vec<_>>(),
			vec!["a", "b"]
		);

		// Slot 2 (index 1) is empty
		assert_eq!(root.children().map(|child| child.slot).collect::<Vec<_>>(), vec![0, 2]);

		// Same when reverse
		assert_eq!(root.children().rev().map(|child| child.slot).collect::<Vec<_>>(), vec![2, 0]);
	}
