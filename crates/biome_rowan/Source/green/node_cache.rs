...
impl<'a> VacantNodeEntry<'a> {
	...
	pub fn cache(self, node:GreenNode) -> u64 {
		if self.original_kind != node.kind() {
			// The kind has changed since it has been queried. For example, the node has
			// been converted to an unknown node. Never cache these nodes because cache
			// lookups will never match.
