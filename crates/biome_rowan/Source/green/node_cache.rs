...
/// Does intentionally not implement `Hash` to have compile-time guarantees that
/// the `NodeCache` uses the correct hash.
#[derive(Debug)]
...
impl IntoRawPointer for GreenToken {
	...
	fn into_raw(self) -> *mut Self::Pointee { GreenToken::into_raw(self).as_ptr() }
...
impl IntoRawPointer for GreenNode {
	...
	fn into_raw(self) -> *mut Self::Pointee { GreenNode::into_raw(self).as_ptr() }
...
// efficient.
//
// To fix that, we additionally wrap the data in `Cached*` wrappers, to make
// sure we don't accidentally use the wrong hash!
...
fn token_hash(token:&GreenTokenData) -> u64 { token_hash_of(token.kind(), token.text()) }
...
impl NodeCache {
	/// Hash used for nodes that haven't been cached because it has too many
	/// slots or one of its children wasn't cached.
	const UNCACHED_NODE_HASH:u64 = 0;

	/// Tries to retrieve a node with the given `kind` and `children` from the
	/// cache.
	///
	...
	pub(crate) fn node(
		...
		// For `libsyntax/parse/parser.rs`, measurements show that deduping
		// saves 17% of the memory for green nodes!
		let entry = self.nodes.raw_entry_mut().from_hash(hash, |no_hash| {
			no_hash.node.value().kind() == kind && {
				let lhs = no_hash.node.value().slots().filter_map(|slot| {
					match slot {
						// Ignore empty slots. The queried node only has the
						// present children
						Slot::Empty { .. } => None,
				...
				let rhs = children.iter().map(|(_, element)| element_id(element.as_deref()));
		...
		match entry {
			RawEntryMut::Occupied(mut entry) => {
				...
				NodeCacheNodeEntryMut::Cached(CachedNodeEntry { hash, raw_entry:entry })
			},
	...
	pub(crate) fn increment_generation(&mut self) {
		debug_assert!(
			self.nodes.keys().all(|entry| entry.node.generation() == self.generation)
				&& self.tokens.keys().all(|token| token.0.generation() == self.generation)
				&& self.trivia.cache.keys().all(|trivia| trivia.0.generation() == self.generation)
		);
	...
	pub(crate) fn retain_cache(&mut self) {
		...
		self.tokens.retain(|token, _| token.0.generation() == self.generation);

		self.trivia.cache.retain(|trivia, _| trivia.0.generation() == self.generation);
	}
}
...
/// The `insert` method allows to place a node inside of the vacant entry. The
/// inserted node may have a different representation (kind or children) than
/// the originally queried node. For example, a node may change its kind to
/// bogus or add empty slots. The only importance is that these changes apply
/// for all nodes that have the same shape as the originally queried node.
...
impl<'a> CachedNodeEntry<'a> {
	pub fn node(&self) -> &GreenNodeData { self.raw_entry.key().node.value() }

	pub fn hash(&self) -> u64 { self.hash }
}

impl<'a> VacantNodeEntry<'a> {
	/// Inserts the `node` into the cache so that future queries for the same
	/// kind and children resolve to the passed `node`.
	///
	/// Returns the hash of the node.
	///
	/// The cache does not cache the `node` if the kind doesn't match the `kind`
	/// of the queried node because cache lookups wouldn't be successful
	/// because the hash collision prevention check compares the kinds of the
	/// cached and queried node.
	pub fn cache(self, node:GreenNode) -> u64 {
		if self.original_kind != node.kind() {
			// The kind has changed since it has been queried. For example, the
			// node has been converted to an unknown node. Never cache these
			// nodes because cache lookups will never match.
			NodeCache::UNCACHED_NODE_HASH
		} else {
			self.raw_entry.insert_with_hasher(
				self.hash,
				CachedNode { node:GenerationalPointer::new(node, self.generation), hash:self.hash },
				(),
				|n| n.hash,
			);
			self.hash
...
impl IntoRawPointer for GreenTrivia {
	...
	fn into_raw(self) -> *mut Self::Pointee { GreenTrivia::into_raw(self) }

	unsafe fn from_raw(ptr:*mut Self::Pointee) -> Self { GreenTrivia::from_raw(ptr) }
}
...
impl TriviaCache {
	/// Tries to retrieve a [GreenTrivia] with the given pieces from the cache
	/// or creates a new one and caches it for further calls.
...
mod tests {
	...
	use crate::{
		green::{
			node_cache::{token_hash, CachedNode, CachedToken, CachedTrivia},
			trivia::GreenTrivia,
		},
		GreenToken,
		RawSyntaxKind,
