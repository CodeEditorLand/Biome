...
pub use trivia::{
	...
	SyntaxTriviaPieceNewline,
	SyntaxTriviaPieceSkipped,
	SyntaxTriviaPieceWhitespace,
...
pub trait SyntaxKind: fmt::Debug + PartialEq + Copy {
	...
	/// Returns a string for keywords and punctuation tokens or `None`
	/// otherwise.
...
impl<L:Language> SyntaxList<L> {
	...
	fn new(node:SyntaxNode<L>) -> Self { Self { list:node } }

	/// Iterates over the elements in the list.
	pub fn iter(&self) -> SyntaxSlots<L> { self.list.slots() }

	/// Returns the number of items in this list
	pub fn len(&self) -> usize { self.list.slots().len() }

	pub fn is_empty(&self) -> bool { self.len() == 0 }

	pub fn first(&self) -> Option<SyntaxSlot<L>> { self.list.slots().next() }

	pub fn last(&self) -> Option<SyntaxSlot<L>> { self.list.slots().last() }

	pub fn node(&self) -> &SyntaxNode<L> { &self.list }

	pub fn into_node(self) -> SyntaxNode<L> { self.list }
}
...
mod tests {
	...
	fn node_list() {
		...
		let node_texts:Vec<_> =
			list.iter().map(|e| e.into_node().map(|n| n.text().to_string())).collect();

		assert_eq!(node_texts, vec![Some(String::from("1")), Some(String::from("2"))])
	}
	...
	fn siblings() {
		...
		assert_eq!(first.next_sibling().map(|e| e.text().to_string()), Some(String::from("b")));

		let second = root.children().nth(1).unwrap();
		assert_eq!(second.text().to_string(), "b");

		// Skips the missing element
		assert_eq!(second.next_sibling().map(|e| e.text().to_string()), Some(String::from("c")));

		assert_eq!(second.prev_sibling().map(|e| e.text().to_string()), Some(String::from("a")));

		let last = root.children().last().unwrap();
		assert_eq!(last.text(), "c");
		assert_eq!(last.next_sibling(), None);
		assert_eq!(last.prev_sibling().map(|e| e.text().to_string()), Some(String::from("b")));
		...
		assert_eq!(
			last.siblings(Direction::Prev).map(|s| s.text().to_string()).collect::<Vec<_>>(),
			vec!["c", "b", "a"]
		);
	}
	...
	fn siblings_with_tokens() {
		...
		let first_semicolon =
			root.children_with_tokens().nth(2).and_then(|e| e.into_token()).unwrap();
	...
	pub fn syntax_range() {
		...
		assert_eq!(TextRange::new(3.into(), 16.into()), node.text_trimmed_range());
		assert_eq!(
		...
		assert_eq!(TextRange::new(12.into(), 13.into()), eq_token.text_trimmed_range());
		assert_eq!(
		...
		assert_eq!(TextRange::new(11.into(), 14.into()), eq_token.text_range());
		assert_eq!(TextRange::new(12.into(), 13.into()), eq_token.text_trimmed_range());
		assert_eq!(TextRange::new(11.into(), 12.into()), eq_token.leading_trivia().text_range());
		assert_eq!(TextRange::new(13.into(), 14.into()), eq_token.trailing_trivia().text_range());
	}
	...
	pub fn syntax_trivia_pieces() {
		...
		let pieces_rev:Vec<_> = node.first_leading_trivia().unwrap().pieces().rev().collect();
