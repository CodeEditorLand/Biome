...
///
/// Allows third-party crates to access limited information about a `GreenNode`
/// or construct a `GreenNode` in a limited places.
#[derive(Debug)]
...
impl<K:SyntaxKind> RawSyntaxNode<K> {
	...
	where
		...
		Self {
			raw:GreenNode::new(
				kind.to_raw(),
				slots.into_iter().map(|slot| slot.map(|element| element.into_green())),
			),
			ph:PhantomData,
		}
	}
	...
	#[inline]
	pub fn kind(&self) -> K { K::from_raw(self.raw.kind()) }

	/// Unwraps this raw syntax into it's underlying green node.
	#[inline]
	pub(crate) fn into_green(self) -> GreenNode { self.raw }
}

impl<K:SyntaxKind> From<GreenNode> for RawSyntaxNode<K> {
	#[inline]
	fn from(node:GreenNode) -> Self { Self { raw:node, ph:PhantomData } }
}

/// New-type wrapper around a `GreenToken`. Allows third-party crates to access
/// limited information on not yet fully constructed nodes.
#[derive(Debug)]
...
impl<K:SyntaxKind> From<GreenToken> for RawSyntaxToken<K> {
	fn from(token:GreenToken) -> Self { Self { raw:token, ph:PhantomData } }
}
...
impl<'a, K:SyntaxKind> From<&'a GreenNode> for RawSyntaxNodeRef<'a, K> {
	#[inline]
	fn from(node:&'a GreenNode) -> Self { Self { raw:node, ph:PhantomData } }
}
...
impl<'a, K:SyntaxKind> From<&'a GreenToken> for RawSyntaxTokenRef<'a, K> {
	#[inline]
	fn from(token:&'a GreenToken) -> Self { Self { raw:token, ph:PhantomData } }
}
