...
/// The parsed children of a node, not accounting for any missing children
/// (required or optional)
#[derive(Debug)]
pub struct ParsedChildren<'a, K> {
	/// Reference to an array containing all children of this node or any of its
	/// parents
...
impl<'a, K:SyntaxKind> ParsedChildren<'a, K> {
	pub(crate) fn new(all_children:&'a mut Vec<(u64, GreenElement)>, first_child:usize) -> Self {
		Self { all_children, first_child, ph:PhantomData }
	}

	pub fn len(&self) -> usize { (self.first_child..self.all_children.len()).len() }
...
impl<'a, K:SyntaxKind> Iterator for ParsedChildrenIntoIterator<'a, K> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		self.inner.next().map(|(_, raw)| RawSyntaxElement::from(raw))
	}
...
impl<'a, K:SyntaxKind> DoubleEndedIterator for ParsedChildrenIntoIterator<'a, K> {
	fn next_back(&mut self) -> Option<Self::Item> {
		self.inner.next_back().map(|(_, raw)| RawSyntaxElement::from(raw))
	}
}
...
impl<'a, K:SyntaxKind> Iterator for ParsedChildrenIterator<'a, K> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		self.inner.next().map(|(_, raw)| RawSyntaxElementRef::from(raw))
	}
...
impl<'a, K:SyntaxKind> DoubleEndedIterator for ParsedChildrenIterator<'a, K> {
	fn next_back(&mut self) -> Option<Self::Item> {
		self.inner.next_back().map(|(_, raw)| RawSyntaxElementRef::from(raw))
	}
}
