...
pub use self::raw_syntax::{
	RawSyntaxElement, RawSyntaxElementRef, RawSyntaxNode, RawSyntaxNodeRef,
	RawSyntaxToken, RawSyntaxTokenRef,
};
...
pub trait SyntaxFactory: fmt::Debug {
	...
	{
		let valid =
			(&children).into_iter().all(|element| can_cast(element.kind()));
...
impl<'a, K: SyntaxKind> Iterator for RawNodeSlotIterator<'a, K> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		match slot {
			SlotContent::Present => Some(Some(self.children.next().expect(
