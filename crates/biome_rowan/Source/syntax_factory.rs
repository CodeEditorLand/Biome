...
pub use self::parsed_children::{ParsedChildren, ParsedChildrenIntoIterator, ParsedChildrenIterator};
pub use self::raw_syntax::{
	RawSyntaxElement, RawSyntaxElementRef, RawSyntaxNode, RawSyntaxNodeRef, RawSyntaxToken, RawSyntaxTokenRef,
};
...
impl<K: SyntaxKind> ExactSizeIterator for SeparatedListWithMissingNodesOrSeparatorSlotsIterator<'_, K> {
	fn len(&self) -> usize {
...
impl<const COUNT: usize> Default for RawNodeSlots<COUNT> {
	fn default() -> Self {
		Self { slots: [SlotContent::Absent; COUNT], current_slot: 0 }
	}
}
...
impl<const COUNT: usize> RawNodeSlots<COUNT> {
	...
	pub fn into_node<K: SyntaxKind>(self, kind: K, children: ParsedChildren<K>) -> RawSyntaxNode<K> {
		...
		RawSyntaxNode::new(
			kind,
			RawNodeSlotIterator { children: children.into_iter(), slots: self.slots.as_slice().iter() },
		)
	}
}
