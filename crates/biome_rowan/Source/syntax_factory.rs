...
pub trait SyntaxFactory: fmt::Debug {
	...
	///
	/// The `children` contains the parsed direct children of the node. There
	/// may be fewer children in case there's a syntax error and a required
	/// child or an optional child isn't present in the source code.
	/// The `make_syntax` implementation must then fill in empty slots to match
	/// the slots as they're defined in the grammar.
	///
	/// The implementation is free to change the `kind` of the node but that has
	/// the consequence that such a node will not be cached. The reason for not
	/// caching these nodes is that the cache lookup is performed
	/// before calling `make_syntax`, thus querying the cache with the old kind.
	///
	/// It's important that the factory function is idempotent, meaning, calling
	/// the function multiple times with the same `kind` and `children` returns
	/// syntax nodes with the same structure. This is important because the
	/// returned nodes may be cached by `kind` and what `children` are present.
	fn make_syntax(
		kind:Self::Kind,
		children:ParsedChildren<Self::Kind>,
	) -> RawSyntaxNode<Self::Kind>;

	/// Crates a *node list* syntax node. Validates if all elements are valid
	/// and changes the node's kind to [SyntaxKind::to_bogus] if that's not the
	/// case.
	...
	where
		F: Fn(Self::Kind) -> bool, {
		let valid = (&children).into_iter().all(|element| can_cast(element.kind()));
	...
	/// Creates a *separated list* syntax node. Validates if the elements are
	/// valid, are correctly separated by the specified separator token.
	///
	/// It changes the kind of the node to [SyntaxKind::to_bogus] if an element
	/// isn't a valid list-node nor separator.
	///
...
/// Iterator that "fixes up" a separated list by inserting empty slots for any
/// missing separator or element.
...
impl<'a, K:SyntaxKind> ExactSizeIterator
	for SeparatedListWithMissingNodesOrSeparatorSlotsIterator<'a, K>
{
	fn len(&self) -> usize { self.inner.len() + self.missing_count }
}
...
/// It stores for each slot if the node is present in [ParsedChildren] or not,
/// allowing to generate a node with the right number of empty slots.
#[derive(Debug)]
...
impl<const COUNT: usize> Default for RawNodeSlots<COUNT> {
	fn default() -> Self { Self { slots:[SlotContent::Absent; COUNT], current_slot:0 } }
}
...
impl<const COUNT: usize> RawNodeSlots<COUNT> {
	...
	/// Marks that the node for the current slot is *present* in the source
	/// code.
	...
	/// Creates a node with the kind `kind`, filling in the nodes from the
	/// `children`.
	pub fn into_node<K:SyntaxKind>(self, kind:K, children:ParsedChildren<K>) -> RawSyntaxNode<K> {
...
impl<'a, K:SyntaxKind> Iterator for RawNodeSlotIterator<'a, K> {
		...
		match slot {
			SlotContent::Present => {
				Some(Some(
					self.children
						.next()
						.expect("Expected a present node according to the slot description"),
				))
			},
	...
	fn size_hint(&self) -> (usize, Option<usize>) { (self.slots.len(), Some(self.slots.len())) }
}
