...
pub trait AstNodeExt: AstNode {
	/// Return a new version of this node with the node `prev_node` replaced
	/// with `next_node`
	///
	/// `prev_node` can be a direct child of this node, or an indirect child
	...
	/// Return a new version of this node with the node `prev_node` replaced
	/// with `next_node`, transfering the leading and trailing trivia of
	/// `prev_node` to `next_node`
	///
	/// `prev_node` can be a direct child of this node, or an indirect child
	/// through any descendant node
	///
	...
	/// Return a new version of this node with the token `prev_token` replaced
	/// with `next_token`
	///
	/// `prev_token` can be a direct child of this node, or an indirect child
	/// through any descendant node
	///
	...
	/// Return a new version of this node with the token `prev_token` replaced
	/// with `next_token`, transfering the leading and trailing trivia of
	/// `prev_token` to `next_token`
	///
	/// `prev_token` can be a direct child of this node, or an indirect child
	/// through any descendant node
	///
	...
	T: AstNode,
{
	...
	where
		...
		Some(Self::unwrap_cast(
			self.into_syntax()
				.replace_child(prev_node.into_syntax().into(), next_node.into_syntax().into())?,
		))
	}
	...
	where
		N: AstNode<Language = Self::Language>,
		Self: Sized, {
		// Lookup the first token of `prev_node` and `next_node`, and transfer
		// the leading trivia of the former to the later
		...
		// Lookup the last token of `prev_node` and `next_node`, and transfer
		// the trailing trivia of the former to the later
	...
	where
		Self: Sized, {
		Some(Self::unwrap_cast(
			self.into_syntax().replace_child(prev_token.into(), next_token.into())?,
		))
	}
	...
	fn detach(self) -> Self { Self::unwrap_cast(self.into_syntax().detach()) }
}

pub trait AstNodeListExt: AstNodeList {
	/// Replace a range of the children of this list with the content of an
	/// iterator
	...
	T: AstNodeList,
{
	...
	where
		...
		Self::unwrap_cast(self.into_syntax_list().into_node().splice_slots(
			range,
			replace_with.into_iter().map(|node| Some(node.into_syntax().into())),
		))
	}
}
...
pub trait AstSeparatedListExt: AstSeparatedList {
	...
	where
		...
		R: ops::RangeBounds<usize>,
		I: IntoIterator<
			Item = (Self::Node, Option<SyntaxToken<<Self as AstSeparatedList>::Language>>),
		>;
}
	...
	where
		...
		Self::unwrap_cast(self.into_syntax_list().into_node().splice_slots(
			...
			replace_with.into_iter().flat_map(|(node, separator)| {
				[Some(node.into_syntax().into()), separator.map(|token| token.into())]
			}),
		))
	}
}
