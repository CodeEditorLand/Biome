...
//!
//! Every field of every AST node is optional, this is to allow the parser to
//! recover from any error and produce an ast from any source code. If you don't
//! want to account for optionals for everything, you can use ...
	...
	L: Language,
{
	...
	pub fn of(kind:L::Kind) -> Self { Self::from_raw(kind.to_raw()) }

	/// Create a new [SyntaxKindSet] containing only the provided
	/// [RawSyntaxKind]
	///
...
/// of the AstNode to be mapped to any slot of the SyntaxNode using an
/// additional `slot_map`. This must get built every time the untyped syntax
/// node is converted into the typed ast node, and is determined by the order of
/// fields in the original grammar. Even still, this cost is relatively low and
/// should not be considered prohibitive, as the only work done is checking
...
pub trait AstNode: Clone {
	...
	/// Returns `true` if a node with the given kind can be cased to this AST
	/// node.
	...
	/// Takes a reference of a syntax node and tries to cast it to this AST
	/// node.
	///
	...
	where
		Self: Sized, {
		if Self::can_cast(syntax.kind()) { Self::cast(syntax.clone()) } else { None }
	}
	...
	/// Returns the string representation of this node without the leading and
	/// trailing trivia
	fn text(&self) -> std::string::String { self.syntax().text_trimmed().to_string() }

	fn range(&self) -> TextRange { self.syntax().text_trimmed_range() }
	...
	/// Return a new version of this node with the leading trivia of its first
	/// token replaced with `trivia`.
	...
	/// Return a new version of this node with the trailing trivia of its last
	/// token replaced with `trivia`.
	...
	// Return a new version of this node with `trivia` prepended to the leading
	// trivia of the first token.
	...
	// Return a new version of this node with `trivia` appended to the trailing
	// trivia of the last token.
	...
	/// Return a new version of this node without leading and trailing newlines
	/// and whitespaces.
	fn trim_trivia(self) -> Option<Self> {
		Self::cast(self.into_syntax().trim_leading_trivia()?.trim_trailing_trivia()?)
	}

	/// Return a new version of this node without leading newlines and
	/// whitespaces.
	fn trim_leading_trivia(self) -> Option<Self> {
		Self::cast(self.into_syntax().trim_leading_trivia()?)
	}

	/// Return a new version of this node without trailing newlines and
	/// whitespaces.
...
impl<L:Language> SyntaxNodeCast<L> for SyntaxNode<L> {
	fn cast<T:AstNode<Language = L>>(self) -> Option<T> { T::cast(self) }
}
...
pub trait AstNodeList {
	...
	fn iter(&self) -> AstNodeListIterator<Self::Language, Self::Node> {
		AstNodeListIterator { inner:self.syntax_list().iter(), ph:PhantomData }
	}

	#[inline]
	fn len(&self) -> usize { self.syntax_list().len() }

	/// Returns the first node from this list or None
	#[inline]
	fn first(&self) -> Option<Self::Node> { self.iter().next() }

	/// Returns the last node from this list or None
	fn last(&self) -> Option<Self::Node> { self.iter().last() }

	#[inline]
	fn is_empty(&self) -> bool { self.syntax_list().is_empty() }
}
...
impl<L:Language, N:AstNode<Language = L>> AstNodeListIterator<L, N> {
	fn slot_to_node(slot:&SyntaxSlot<L>) -> N {
		match slot {
			SyntaxSlot::Empty { .. } => {
				panic!("Node isn't permitted to contain empty slots")
			},
...
impl<L:Language, N:AstNode<Language = L>> Iterator for AstNodeListIterator<L, N> {
	...
	fn next(&mut self) -> Option<Self::Item> { Some(Self::slot_to_node(&self.inner.next()?)) }
...
impl<L:Language, N:AstNode<Language = L>> AstSeparatedElement<L, N> {
	...
	pub fn into_node(self) -> SyntaxResult<N> { self.node }
...
/// For example, the elements of an array where every two elements are separated
/// by a comma token. The list expects that the underlying syntax node has a
/// slot for every node and separator even if they are missing from the source
/// code. For example, a list for `a b` where the `,` separator is missing
/// contains the slots `Node(a), Empty, Node(b)`. This also applies for missing
/// nodes: the list for `, b,` must have the slots `Empty, Token(,), Node(b),
/// Token(,)`.
pub trait AstSeparatedList {
	...
	fn separators(&self) -> AstSeparatorIterator<Self::Language, Self::Node> {
		AstSeparatorIterator { inner:self.elements() }
	}

	/// Returns an iterator over all nodes
	fn iter(&self) -> AstSeparatedListNodesIterator<Self::Language, Self::Node> {
		AstSeparatedListNodesIterator { inner:self.elements() }
	}

	/// Returns the first node
	fn first(&self) -> Option<SyntaxResult<Self::Node>> { self.iter().next() }

	/// Returns the last node
	fn last(&self) -> Option<SyntaxResult<Self::Node>> { self.iter().next_back() }

	#[inline]
	fn is_empty(&self) -> bool { self.len() == 0 }

	fn len(&self) -> usize { (self.syntax_list().len() + 1) / 2 }
...
impl<L:Language, N:AstNode<Language = L>> AstSeparatedListElementsIterator<L, N> {
	fn new(list:&SyntaxList<L>) -> Self { Self { slots:list.iter(), ph:PhantomData } }
}
...
impl<L:Language, N:AstNode<Language = L>> Iterator for AstSeparatedListElementsIterator<L, N> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		let node = match slot {
			// The node for this element is missing if the next child is a token
			// instead of a node.
			SyntaxSlot::Token(token) => {
				panic!(
					"Malformed list, node expected but found token {token:?} instead. You must \
					 add missing markers for missing elements."
				)
			},
			// Missing element
		...
		let separator = match self.slots.next() {
			...
			Some(SyntaxSlot::Node(node)) => {
				panic!(
					"Malformed separated list, separator expected but found node {node:?} \
					 instead. You must add missing markers for missing separators."
				)
			},
		};

		Some(AstSeparatedElement { node, trailing_separator:separator })
	}
}

impl<L:Language, N:AstNode<Language = L>> FusedIterator for AstSeparatedListElementsIterator<L, N> {}
	...
	fn next_back(&mut self) -> Option<Self::Item> {
		...
		let separator = match first_slot {
			SyntaxSlot::Node(node) => {
				// if we fallback here, it means that we are at the end of the
				// iterator which means that we don't have the optional
				// separator and we have only a node, we bail early.
		...
		let node = match self.slots.next_back() {
			None => {
				panic!("Malformed separated list, expected a node but found none")
			},
			Some(SyntaxSlot::Empty { .. }) => Err(SyntaxError::MissingRequiredChild),
...
impl<L:Language, N:AstNode<Language = L>> Iterator for AstSeparatedListNodesIterator<L, N> {
	...
	fn next(&mut self) -> Option<Self::Item> { self.inner.next().map(|element| element.node) }
}
...
pub mod support {
	...
	use crate::{syntax::SyntaxSlot, SyntaxElementChildren};
	...
	pub fn list<L:Language, N:AstNode<Language = L>>(parent:&SyntaxNode<L>, slot_index:usize) -> N {
	...
	/// New-type wrapper to flatten the debug output of syntax result fields
	/// when printing [AstNode]s. Omits the [Ok] if the node is present and
	/// prints `missing (required)` if the child is missing
	...
	/// New-type wrapper to flatten the debug output of optional children when
	/// printing [AstNode]s. Omits the [Some] if the node is present and prints
	/// `missing (optional)` if the child is missing
...
mod tests {
	...
	fn map_elements<'a>(
		...
		let actual:Vec<_> = if revert { actual.rev().collect() } else { actual.collect() };
		let actual = actual
			.into_iter()
			.map(|element| {
				(
	...
	fn separated_with_two_successive_separators() {
		...
		assert_elements(list.elements(), vec![(Some(1.), Some(",")), (None, Some(","))]);

		assert_rev_elements(list.elements(), vec![(None, Some(",")), (Some(1.), Some(","))]);
	}
	...
	fn separated_with_two_successive_nodes() {
		...
		assert_elements(list.elements(), vec![(Some(1.), None), (Some(2.), Some(","))]);

		assert_rev_elements(list.elements(), vec![(Some(2.), Some(",")), (Some(1.), None)]);
	}
	...
	fn ok_typed_parent_navigation() {
		...
		impl AstNode for RawRoot {
			...
			fn can_cast(_:<Self::Language as Language>::Kind) -> bool { todo!() }
			...
			fn syntax(&self) -> &SyntaxNode<Self::Language> { &self.0 }

			fn into_syntax(self) -> SyntaxNode<Self::Language> { todo!() }
		}
	}
}
