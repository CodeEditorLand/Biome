...
where
	...
	#[must_use = "This method consumes the node and return the BatchMutation api that returns the \
	              new SyntaxNode on commit"]
	...
	T: AstNode<Language = L>,
{
	#[must_use = "This method consumes the node and return the BatchMutation api that returns the \
	              new SyntaxNode on commit"]
	fn begin(self) -> BatchMutation<L> { BatchMutation::new(self.into_syntax()) }
}
...
///
/// This is necesasry so we can aggregate all changes to the same node using
/// "peek".
#[derive(Debug, Clone)]
...
where
	L: Language,
{
	pub fn new(root:SyntaxNode<L>) -> Self { Self { root, changes:BinaryHeap::new() } }
	...
	where
		T: AstNode<Language = L>, {
		self.replace_element(prev_node.into_syntax().into(), next_node.into_syntax().into())
	}
	...
	/// please use `commit_with_text_range_and_edit`
	pub fn commit(self) -> SyntaxNode<L> { self.commit_with_text_range_and_edit(false).0 }

	/// The core of the batch mutation algorithm can be summarized as:
	///
	/// 1. Iterate all requested changes;
	/// 2. Insert them into a heap (priority queue) by depth. Deeper changes are
	///    done first;
	/// 3. Loop popping requested changes from the heap, taking the deepest
	///    change we have for the moment;
	/// 4. Each requested change has a "parent", an "index" and the "new node"
	///    (or None);
	/// 5. Clone the current parent's "parent", the "grandparent";
	/// 6. Detach the current "parent" from the tree;
	/// 7. Replace the old node at "index" at the current "parent" with the
	///    current "new node";
	/// 8. Insert into the heap the grandparent as the parent and the current
	///    "parent" as the "new node";
	///
	/// This is the simple case. The algorithm also has a more complex case when
	/// to changes have a common ancestor, which can actually be one of the
	/// changed nodes.
	///
	/// To address this case at step 3, when we pop a new change to apply it, we
	/// actually aggregate all changes to the current parent together. This is
	/// done by the heap because we also sort by node and it's range.
	///
	/// Text range and text edit can be collected simultaneously while
	/// committing if "with_text_range_and_edit" is true. They're directly
	/// calculated from the commit changes. So you can commit and get text range
	/// and text edit in one pass.
	///
	/// The calculation of text range and text edit can be summarized as:
	///
	/// While we popping requested changes from the heap, collect the
	/// "deleted_text_range" and "optional_inserted_text" into an ordered
	/// vector "text_mutation_list" sorted by the "deleted_text_range". The
	/// reason behind it is that changes on the heap are first ordered by
	/// parent depth, but we need to construct the TextEdit from start to end.
	/// So we use binary search and insertion to populate the
	/// "text_mutation_list". Reaching the root node means all changes have
	/// been visted. So we start to construct the TextEdit with the help of
	/// "text_edit_builder" by iterating the collected "text_mutation_list".
	pub fn commit_with_text_range_and_edit(
		...
		}) = changes.pop()
		{
			if let Some(curr_parent) = curr_parent {
				...
				// This works because of the Ord we defined in the
				// [CommitChange] struct
				...
				// Collect text mutations, this has to be done before the detach
				// below, or we'll lose the "deleted_text_range" info
				if with_text_range_and_edit {
					for (new_node_slot, new_node, is_from_action) in &modifications {
						...
						// We use binary search to keep the text mutations in
						// order
						match text_mutation_list
							.binary_search_by(|(range, _)| range.ordering(deleted_text_range))
						{
							// Overwrite the text mutation with an overlapping
							// text range
							Ok(pos) => {
				...
				// Now we detach the current parent, commit all the
				// modifications and push a pending change to its parent
				...
				let optional_text_range_and_edit = if with_text_range_and_edit {
					...
					if curr_is_from_action {
						text_mutation_list = vec![(
							...
							Some(curr_new_node.as_ref().map_or(String::new(), |n| n.to_string())),
						)];
					}

					// Build text range and text edit from the text mutation
					// list
	...
	pub fn root(&self) -> &SyntaxNode<L> { &self.root }
}
