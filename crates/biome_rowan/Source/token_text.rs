...
/// Reference to the text of a SyntaxToken without having to worry about the
/// lifetime of `&str`.
#[derive(Eq, Clone)]
...
impl std::hash::Hash for TokenText {
	fn hash<H:std::hash::Hasher>(&self, state:&mut H) { self.text().hash(state); }
}
...
impl TokenText {
	...
	pub fn text(&self) -> &str { &self.token.text()[self.range] }
}
...
impl PartialEq<&'_ str> for TokenText {
	fn eq(&self, rhs:&&'_ str) -> bool { **self == **rhs }
}

impl PartialEq<TokenText> for &'_ str {
	fn eq(&self, other:&TokenText) -> bool { **self == **other }
}

impl Borrow<str> for TokenText {
	fn borrow(&self) -> &str { self.text() }
}
