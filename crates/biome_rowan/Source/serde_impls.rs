...
impl<L:Language> Serialize for SyntaxToken<L> {
	...
	where
		...
		// To implement this, SyntaxTrivia will need to expose the kind and the
		// length of each trivia state.serialize_entry("leading",
		// &self.leading())?; state.serialize_entry("trailing",
		// &self.trailing())?;
		state.end()
	}
}
