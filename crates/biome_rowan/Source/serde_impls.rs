...
impl<L: Language> Serialize for SyntaxNode<L> {
		...
		S: Serializer,
	{
		...
		state
			.serialize_entry("kind", &SerDisplay(DisplayDebug(self.kind())))?;
...
impl<L: Language> Serialize for Children<&'_ SyntaxNode<L>> {
		...
		S: Serializer,
	{
		...
		self.0.children_with_tokens().try_for_each(
...
mod test {
	...
	pub fn serialization() {
		let mut builder: crate::TreeBuilder<
			RawLanguage,
			RawLanguageSyntaxFactory,
