...
use crate::{
	AstNode, AstSeparatedList, Language, ParsedChildren, RawNodeSlots, RawSyntaxKind, RawSyntaxNode, SyntaxFactory,
	SyntaxKind, SyntaxKindSet, SyntaxList, SyntaxNode, TreeBuilder,
};
...
impl AstNode for LiteralExpression {
	...
	const KIND_SET: SyntaxKindSet<RawLanguage> = SyntaxKindSet::from_raw(RawSyntaxKind(LITERAL_EXPRESSION as u16));
...
impl SyntaxFactory for RawLanguageSyntaxFactory {
	...
	fn make_syntax(kind: Self::Kind, children: ParsedChildren<Self::Kind>) -> RawSyntaxNode<Self::Kind> {
		match kind {
			RawLanguageKind::BOGUS | RawLanguageKind::ROOT => RawSyntaxNode::new(kind, children.into_iter().map(Some)),
			...
			RawLanguageKind::SEPARATED_EXPRESSION_LIST => {
				Self::make_separated_list_syntax(kind, children, |kind| kind == LITERAL_EXPRESSION, COMMA_TOKEN, true)
			},
			RawLanguageKind::LITERAL_EXPRESSION => {
				...
				if let Some(element) = &current_element {
					if !matches!(element.kind(), RawLanguageKind::STRING_TOKEN | RawLanguageKind::NUMBER_TOKEN) {
						return RawSyntaxNode::new(kind.to_bogus(), std::iter::once(current_element));
					}
				} else {
