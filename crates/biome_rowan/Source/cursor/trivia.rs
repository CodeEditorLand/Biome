...
use crate::{cursor::SyntaxToken, green::GreenTrivia, TriviaPiece};
...
impl SyntaxTrivia {
	pub(super) fn leading(token:SyntaxToken) -> Self { Self { token, is_leading:true } }

	pub(super) fn trailing(token:SyntaxToken) -> Self { Self { token, is_leading:false } }

	pub(crate) fn text(&self) -> &str {
		let trivia_range = self.text_range();

		let relative_range =
			TextRange::at(trivia_range.start() - self.token.data().offset, trivia_range.len());

		&self.token.text()[relative_range]
	}

	pub(crate) fn token(&self) -> &SyntaxToken { &self.token }
	...
	pub(crate) fn len(&self) -> usize { self.green_trivia().len() }

	/// Gets index-th trivia piece when the token associated with this trivia
	/// was created. See [SyntaxTriviaPiece].
	...
	pub(crate) fn last(&self) -> Option<&TriviaPiece> { self.green_trivia().pieces().last() }

	/// Returns the first trivia piece element
	pub(crate) fn first(&self) -> Option<&TriviaPiece> { self.green_trivia().pieces().first() }

	/// Iterate over all pieces of the trivia. The iterator returns the offset
	/// of the trivia as [TextSize] and its data as [Trivia], which contains its
	/// length. See [SyntaxTriviaPiece].
