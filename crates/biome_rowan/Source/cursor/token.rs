...
impl SyntaxToken {
	...
	pub(crate) fn new_detached(green:GreenToken) -> SyntaxToken {
		SyntaxToken {
			ptr:NodeData::new(
				NodeKind::Root { green:GreenElement::Token(green) },
				0,
				TextSize::from(0),
			),
		}
	...
	#[inline]
	pub(super) fn into_green(self) -> green::GreenElement { self.ptr.into_green() }
	...
	#[inline]
	pub fn token_text(&self) -> TokenText { TokenText::new(self.green().to_owned()) }
	...
	#[inline]
	pub fn text_trimmed(&self) -> &str { self.green().text_trimmed() }
	...
	pub fn next_token(&self) -> Option<SyntaxToken> { self.next_token_impl(Direction::Next) }

	pub fn prev_token(&self) -> Option<SyntaxToken> { self.next_token_impl(Direction::Prev) }

	/// Returns the token preceding or following this token depending on the
	/// passed `direction`.
		...
		loop {
			current = match current {
				...
				WalkEvent::Leave(element) => {
					...
					loop {
						// Only traverse the left (pref) / right (next) siblings
						// of the parent to avoid traversing into the same
						// children again.
	...
	#[inline]
	pub fn leading_trivia(&self) -> SyntaxTrivia { SyntaxTrivia::leading(self.clone()) }

	#[inline]
	pub fn trailing_trivia(&self) -> SyntaxTrivia { SyntaxTrivia::trailing(self.clone()) }
}

// Identity semantics for hash & eq
impl PartialEq for SyntaxToken {
	#[inline]
	fn eq(&self, other:&SyntaxToken) -> bool { self.data().key() == other.data().key() }
}
...
impl fmt::Display for SyntaxToken {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result { fmt::Display::fmt(self.text(), f) }
}
