...
impl SyntaxNode {
	pub(crate) fn new_root(green:GreenNode) -> SyntaxNode {
		SyntaxNode {
			ptr:NodeData::new(NodeKind::Root { green:GreenElement::Node(green) }, 0, 0.into()),
		}
	}
	...
	pub fn clone_subtree(&self) -> SyntaxNode { SyntaxNode::new_root(self.green().into()) }

	#[inline]
	pub(super) fn data(&self) -> &NodeData { self.ptr.as_ref() }

	#[inline]
	pub fn kind(&self) -> RawSyntaxKind { self.data().kind() }

	#[inline]
	pub(super) fn offset(&self) -> TextSize { self.data().offset() }

	pub(crate) fn element_in_slot(&self, slot_index:u32) -> Option<SyntaxElement> {
		let slot = self.slots().nth(slot_index as usize).expect("Slot index out of bounds");

		slot.map(|element| element)
	}

	#[inline]
	pub(crate) fn slots(&self) -> SyntaxSlots { SyntaxSlots::new(self.clone()) }

	#[inline]
	pub fn text_range(&self) -> TextRange { self.data().text_range() }
	...
	#[inline]
	pub fn index(&self) -> usize { self.data().slot() as usize }

	#[inline]
	pub fn text(&self) -> SyntaxNodeText { SyntaxNodeText::new(self.clone()) }
	...
	#[inline]
	pub(crate) fn key(&self) -> (NonNull<()>, TextSize) { self.data().key() }

	#[inline]
	pub(crate) fn green(&self) -> &GreenNodeData { self.data().green().into_node().unwrap() }

	#[inline]
	pub fn parent(&self) -> Option<SyntaxNode> { self.data().parent_node() }
	...
	#[inline]
	pub fn children(&self) -> SyntaxNodeChildren { SyntaxNodeChildren::new(self.clone()) }
	...
	pub fn next_sibling(&self) -> Option<SyntaxNode> { self.data().next_sibling() }

	pub fn prev_sibling(&self) -> Option<SyntaxNode> { self.data().prev_sibling() }
	...
	pub fn first_token(&self) -> Option<SyntaxToken> {
		self.descendants_with_tokens(Direction::Next).find_map(|x| x.into_token())
	}
	...
	#[inline]
	pub fn preorder(&self) -> Preorder { Preorder::new(self.clone()) }
	...
	pub(crate) fn preorder_slots(&self) -> SlotsPreorder { SlotsPreorder::new(self.clone()) }
	...
	pub fn child_or_token_at_range(&self, range:TextRange) -> Option<SyntaxElement> {
		...
		self.green().slot_at_range(rel_range).and_then(|(index, rel_offset, slot)| {
	...
	#[must_use = "syntax elements are immutable, the result of update methods must be propagated \
	              to have any effect"]
	pub fn detach(self) -> Self { Self { ptr:self.ptr.detach() } }
	...
	pub fn replace_child(self, prev_elem:SyntaxElement, next_elem:SyntaxElement) -> Option<Self> {
		Some(Self { ptr:self.ptr.replace_child(prev_elem, next_elem)? })
	}
}
...
impl PartialEq for SyntaxNode {
	#[inline]
	fn eq(&self, other:&SyntaxNode) -> bool { self.data().key() == other.data().key() }
}
...
impl Hash for SyntaxNode {
	#[inline]
	fn hash<H:Hasher>(&self, state:&mut H) { self.data().key().hash(state); }
}
...
impl Preorder {
	...
	pub fn skip_subtree(&mut self) { self.skip_subtree = true; }
...
impl PreorderWithTokens {
	fn new(start:SyntaxNode, direction:Direction) -> PreorderWithTokens {
		...
		PreorderWithTokens { start:start.into(), next, direction, skip_subtree:false }
	}
...
/// Represents a cursor to a green node slot. A slot either contains an element
/// or is empty if the child isn't present in the source.
#[derive(Debug, Clone)]
...
impl SyntaxSlots {
	...
	fn new(parent:SyntaxNode) -> Self {
		Self { pos:0, back_pos:parent.green().slice().len() as u32, parent }
	}
	...
	fn map_slot(&self, slot:&Slot, slot_index:u32) -> SyntaxSlot {
		match slot {
			Slot::Empty { .. } => {
				SyntaxSlot::Empty { parent:self.parent.clone(), index:slot_index }
			},
...
impl<'a> Siblings<'a> {
	...
	pub fn following(&self) -> Children<'a> {
		...
		// Navigate to the start slot so that calling `next` returns the first following
		// sibling
	...
	/// Creates an iterator over the siblings preceding the start node in
	/// reverse order. For example, the preceding siblings of the if
	/// statement's condition are:
	/// * opening parentheses: (
	/// * if keyword: if
	pub fn previous(&self) -> impl Iterator<Item = Child<'a>> {
		let mut slots = self.parent.slots().enumerate();

		// Navigate to the start slot from the back so that calling `next_back` (or
		// rev().next()) returns the first slot preceding the start node
