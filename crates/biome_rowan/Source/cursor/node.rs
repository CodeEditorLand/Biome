...
impl<'a> Siblings<'a> {
	...
	pub fn following(&self) -> Children<'a> {
		...
		// Navigate to the start slot so that calling `next` returns the first
		// following sibling
	...
	pub fn previous(&self) -> impl Iterator<Item = Child<'a>> {
		...
		// Navigate to the start slot from the back so that calling `next_back`
		// (or rev().next()) returns the first slot preceding the start node
