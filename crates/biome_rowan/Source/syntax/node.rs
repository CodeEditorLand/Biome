...
impl<L:Language> SyntaxNode<L> {
	...
	/// Create a new detached (root) node from a syntax kind and an iterator of
	/// slots
	///
	...
	fn green_node(&self) -> GreenNode { self.raw.green().to_owned() }
	...
	/// Returns the element stored in the slot with the given index. Returns
	...
	pub fn kind(&self) -> L::Kind { L::Kind::from_raw(self.raw.kind()) }

	/// Returns the text of all descendants tokens combined, including all
	/// trivia.
	///
	/// ```
	/// use biome_rowan::{
	...
	/// assert_eq!("\n\t let \t\ta; \t\t", node.text());
	/// ```
	pub fn text(&self) -> SyntaxNodeText { self.raw.text() }

	/// Returns the text of all descendants tokens combined,
	/// excluding the first token leading trivia, and the last token trailing
	/// trivia. All other trivia is included.
	///
	/// ```
	/// use biome_rowan::{
	/// 	raw_language::{RawLanguage, RawLanguageKind, RawSyntaxTreeBuilder},
	...
	/// ```
	pub fn text_trimmed(&self) -> SyntaxNodeText { self.raw.text_trimmed() }

	/// Returns the range corresponding for the text of all descendants tokens
	/// combined, including all trivia.
	///
	/// ```
	/// use biome_rowan::{
	/// 	raw_language::{RawLanguage, RawLanguageKind, RawSyntaxTreeBuilder},
	...
	/// ```
	pub fn text_range(&self) -> TextRange { self.raw.text_range() }

	/// Returns the range corresponding for the text of all descendants tokens
	/// combined, excluding the first token leading  trivia, and the last token
	/// trailing trivia. All other trivia is included.
	///
	/// ```
	/// use biome_rowan::{
	/// 	raw_language::{RawLanguage, RawLanguageKind, RawSyntaxTreeBuilder},
	...
	/// ```
	pub fn text_trimmed_range(&self) -> TextRange { self.raw.text_trimmed_range() }

	/// Returns the leading trivia of the
	/// [first_token](SyntaxNode::first_token), or [None] if the node does not
	/// have any descendant tokens.
	///
	/// ```
	/// use biome_rowan::{
	/// 	raw_language::{RawLanguage, RawLanguageKind, RawSyntaxTreeBuilder},
	...
	/// Returns the trailing trivia of the
	/// [last_token](SyntaxNode::last_token), or [None] if the node does not
	/// have any descendant tokens.
	///
	/// ## Examples
	///
	...
	pub fn parent(&self) -> Option<SyntaxNode<L>> { self.raw.parent().map(Self::from) }
	...
	#[inline]
	pub fn index(&self) -> usize { self.raw.index() }
	...
	pub fn slots(&self) -> SyntaxSlots<L> { SyntaxSlots { raw:self.raw.slots(), _p:PhantomData } }
	...
	pub fn first_child(&self) -> Option<SyntaxNode<L>> { self.raw.first_child().map(Self::from) }

	pub fn last_child(&self) -> Option<SyntaxNode<L>> { self.raw.last_child().map(Self::from) }
	...
	pub fn next_sibling(&self) -> Option<SyntaxNode<L>> { self.raw.next_sibling().map(Self::from) }

	pub fn prev_sibling(&self) -> Option<SyntaxNode<L>> { self.raw.prev_sibling().map(Self::from) }
	...
	pub fn descendants_tokens(&self, direction:Direction) -> impl Iterator<Item = SyntaxToken<L>> {
		self.descendants_with_tokens(direction).filter_map(|x| x.as_token().cloned())
	}
	...
	pub fn descendants_with_tokens(
		...
		self.raw.descendants_with_tokens(direction).map(NodeOrToken::from)
	}

	/// Traverse the subtree rooted at the current node (including the current
	/// node) in preorder, excluding tokens.
	pub fn preorder(&self) -> Preorder<L> { Preorder { raw:self.raw.preorder(), _p:PhantomData } }

	/// Traverse the subtree rooted at the current node (including the current
	/// node) in preorder, including tokens.
	pub fn preorder_with_tokens(&self, direction:Direction) -> PreorderWithTokens<L> {
		PreorderWithTokens { raw:self.raw.preorder_with_tokens(direction), _p:PhantomData }
	}
	...
	pub fn clone_subtree(&self) -> SyntaxNode<L> { SyntaxNode::from(self.raw.clone_subtree()) }

	/// Return a new version of this node detached from its parent node
	#[must_use = "syntax elements are immutable, the result of update methods must be propagated \
	              to have any effect"]
	pub fn detach(self) -> Self { Self { raw:self.raw.detach(), _p:PhantomData } }
	...
	where
		...
		Self {
			raw:self.raw.splice_slots(
				range,
				replace_with.into_iter().map(|element| element.map(cursor::SyntaxElement::from)),
			),
			_p:PhantomData,
		}
	}

	/// Return a new version of this node with the element `prev_elem` replaced
	/// with `next_elem`
	///
	/// `prev_elem` can be a direct child of this node, or an indirect child
	/// through any descendant node
	///
	...
	#[must_use = "syntax elements are immutable, the result of update methods must be propagated \
	              to have any effect"]
	pub fn trim_trivia(self) -> Option<Self> { self.trim_leading_trivia()?.trim_trailing_trivia() }
	...
	pub fn into_list(self) -> SyntaxList<L> { SyntaxList::new(self) }
	...
	pub fn has_trailing_comments(&self) -> bool {
		self.last_token().map_or(false, |tok| tok.has_trailing_comments())
	}

	/// Whether the last token of a node has comments (leading or trailing)
	pub fn last_token_has_comments(&self) -> bool {
		self.last_token()
			.map_or(false, |tok| tok.has_trailing_comments() || tok.has_leading_comments())
	}
	...
	pub fn has_leading_comments(&self) -> bool {
		self.first_token().map_or(false, |tok| tok.has_leading_comments())
	}

	/// Whether the node contains leading newlines.
	pub fn has_leading_newline(&self) -> bool {
		self.first_token().map_or(false, |tok| tok.has_leading_newline())
	}
}
	...
	L: Language + 'static,
{
	...
	pub fn as_send(&self) -> Option<SendNode> {
		if self.parent().is_none() {
			Some(SendNode { language:TypeId::of::<L>(), green:self.green_node() })
		} else {
			None
		}
	}
...
impl<L:Language> fmt::Display for SyntaxNode<L> {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result { fmt::Display::fmt(&self.raw, f) }
}

impl<L:Language> From<SyntaxNode<L>> for cursor::SyntaxNode {
	fn from(node:SyntaxNode<L>) -> cursor::SyntaxNode { node.raw }
}

impl<L:Language> From<cursor::SyntaxNode> for SyntaxNode<L> {
	fn from(raw:cursor::SyntaxNode) -> SyntaxNode<L> { SyntaxNode { raw, _p:PhantomData } }
}
...
impl<L:Language> Iterator for SyntaxNodeChildren<L> {
	...
	fn next(&mut self) -> Option<Self::Item> { self.raw.next().map(SyntaxNode::from) }
}
...
impl<L:Language> Default for SyntaxElementChildren<L> {
	fn default() -> Self {
		SyntaxElementChildren { raw:cursor::SyntaxElementChildren::default(), _p:PhantomData }
	}
}
...
impl<L:Language> Iterator for SyntaxElementChildren<L> {
	...
	fn next(&mut self) -> Option<Self::Item> { self.raw.next().map(NodeOrToken::from) }
}
...
impl<L:Language> Preorder<L> {
	pub fn skip_subtree(&mut self) { self.raw.skip_subtree() }
}

impl<L:Language> Iterator for Preorder<L> {
	type Item = WalkEvent<SyntaxNode<L>>;

	fn next(&mut self) -> Option<Self::Item> { self.raw.next().map(|it| it.map(SyntaxNode::from)) }
}
...
/// Each node has a slot for each of its children regardless if the child is
/// present or not. A child that isn't present either because it's optional or
/// because of a syntax error is stored in an [SyntaxSlot::Empty] to preserve
/// the index of each child.
...
impl<L:Language> Iterator for SyntaxSlots<L> {
	...
	#[inline]
	fn next(&mut self) -> Option<Self::Item> { self.raw.next().map(SyntaxSlot::from) }

	#[inline(always)]
	fn size_hint(&self) -> (usize, Option<usize>) { self.raw.size_hint() }
	...
	#[inline]
	fn nth(&mut self, n:usize) -> Option<Self::Item> { self.raw.nth(n).map(SyntaxSlot::from) }
}
...
impl<L:Language> DoubleEndedIterator for SyntaxSlots<L> {
	#[inline]
	fn next_back(&mut self) -> Option<Self::Item> { self.raw.next_back().map(SyntaxSlot::from) }
...
impl<L:Language> SyntaxNodeOptionExt<L> for Option<&SyntaxNode<L>> {
	fn kind(&self) -> Option<L::Kind> { self.map(|node| node.kind()) }
}

impl<L:Language> SyntaxNodeOptionExt<L> for Option<SyntaxNode<L>> {
	fn kind(&self) -> Option<L::Kind> { self.as_ref().kind() }
}
