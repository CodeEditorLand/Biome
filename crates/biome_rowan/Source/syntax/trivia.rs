...
impl TriviaPieceKind {
	pub const fn is_newline(&self) -> bool { matches!(self, TriviaPieceKind::Newline) }

	pub const fn is_whitespace(&self) -> bool { matches!(self, TriviaPieceKind::Whitespace) }
	...
	pub const fn is_skipped(&self) -> bool { matches!(self, TriviaPieceKind::Skipped) }
}
...
impl TriviaPiece {
	...
	pub fn newline<L:Into<TextSize>>(len:L) -> Self { Self::new(TriviaPieceKind::Newline, len) }

	/// Creates a new comment trivia piece that does not contain any line
	/// breaks. For example, JavaScript's `//` comments are guaranteed to not
	/// spawn multiple lines. However, this can also be a `/* ... */` comment
	/// if it doesn't contain any line break characters.
	pub fn single_line_comment<L:Into<TextSize>>(len:L) -> Self {
		Self::new(TriviaPieceKind::SingleLineComment, len)
	}

	/// Creates a new comment trivia piece that contains at least one line
	/// breaks. For example, a JavaScript `/* ... */` comment that spawns at
	/// least two lines (contains at least one line break character).
	...
	pub fn new<L:Into<TextSize>>(kind:TriviaPieceKind, length:L) -> Self {
		Self { kind, length:length.into() }
	}

	/// Returns the trivia's length
	pub fn text_len(&self) -> TextSize { self.length }

	/// Returns the trivia's kind
	pub fn kind(&self) -> TriviaPieceKind { self.kind }
}
...
impl<L:Language> SyntaxTriviaPieceNewline<L> {
	...
	pub fn text_len(&self) -> TextSize { self.0.text_len() }

	pub fn text_range(&self) -> TextRange { self.0.text_range() }

	/// Returns a reference to its [SyntaxTriviaPiece]
	pub fn as_piece(&self) -> &SyntaxTriviaPiece<L> { &self.0 }

	/// Returns its [SyntaxTriviaPiece]
	pub fn into_piece(self) -> SyntaxTriviaPiece<L> { self.0 }
}
...
impl<L:Language> SyntaxTriviaPieceComments<L> {
	...
	pub fn has_newline(&self) -> bool { self.0.trivia.kind.is_multiline_comment() }
...
/// [SyntaxTriviaPiece] gives access to the most granular information about the
/// trivia that was specified by the lexer at the token creation time.
///
/// For example:
///
/// ```no_test
...
/// });
/// ```
/// This token has two pieces in the leading trivia, and one piece at the
/// trailing trivia. Each piece is defined by the [TriviaPiece]; its content is
/// irrelevant.
#[derive(Clone)]
...
impl<L:Language> SyntaxTriviaPiece<L> {
	pub(crate) fn into_raw_piece(self) -> TriviaPiece { self.trivia }

	/// Returns the internal kind of this trivia piece
	pub fn kind(&self) -> TriviaPieceKind { self.trivia.kind() }

	/// Returns the associated text just for this trivia piece. This is
	/// different from [SyntaxTrivia::text()], which returns the text of the
	/// whole trivia.
	///
	/// ```
	...
	/// Returns the associated text length just for this trivia piece. This is
	/// different from `SyntaxTrivia::len()`, which returns the text length of
	/// the whole trivia.
	///
	/// ```
	/// use std::iter::Iterator;
	...
	/// ```
	pub fn text_len(&self) -> TextSize { self.trivia.text_len() }

	/// Returns the associated text range just for this trivia piece. This is
	/// different from [SyntaxTrivia::text_range()], which returns the text
	/// range of the whole trivia.
	///
	/// ```
	/// use std::iter::Iterator;
	///
	...
	/// ```
	pub fn text_range(&self) -> TextRange { TextRange::at(self.offset, self.text_len()) }
	...
	/// ```
	pub fn is_newline(&self) -> bool { self.trivia.kind.is_newline() }
	...
	/// ```
	pub fn is_whitespace(&self) -> bool { self.trivia.kind.is_whitespace() }
	...
	pub fn is_skipped(&self) -> bool { self.trivia.kind.is_skipped() }
	...
	pub fn token(&self) -> SyntaxToken<L> { SyntaxToken::from(self.raw.token().clone()) }
}
...
impl<L:Language> SyntaxTrivia<L> {
	pub(super) fn new(raw:cursor::SyntaxTrivia) -> Self { Self { raw, _p:PhantomData } }
	...
	pub fn has_skipped(&self) -> bool { self.pieces().any(|piece| piece.is_skipped()) }
}
...
pub fn trim_leading_trivia_pieces<L:Language>(
	...
	// We cannot use `skip_while` because `SkipWhile` doesn't implement
	// `ExactSizeIterator`. Eager version of `skip_while` which eagerly consume
	// trivia.
...
pub fn trim_trailing_trivia_pieces<L:Language>(
	...
	// We cannot use `take_while` because `TakeWhile` doesn't implement
	// `ExactSizeIterator`.
	while trivia.next_if(|x| x.is_whitespace() || x.is_newline()).is_some() {
		take_count -= 1;
	}
	// We have to use `take` to avoid panicking on `ExactSizeIterator under-reported
	// length`.
...
///
/// This is the same as Rust's [std::iter::Chain] iterator but implements
/// [ExactSizeIterator]. Rust doesn't implement [ExactSizeIterator] because
/// adding the sizes of both pieces may overflow.
///
/// Implementing [ExactSizeIterator] in our case is safe because this may only
/// overflow if a source document has more than 2^32 trivia which isn't possible
/// because our source documents are limited to 2^32 length.
...
impl<F, S> ChainTriviaPiecesIterator<F, S> {
	fn new(first:F, second:S) -> Self { Self { first:Some(first), second } }
}
	...
	fn len(&self) -> usize {
		match &self.first {
			Some(first) => {
				...
				// SAFETY: Should be safe because a program can never contain more than u32
				// pieces because the text ranges are represented as u32 (and each piece
				// must at least contain a single character).
