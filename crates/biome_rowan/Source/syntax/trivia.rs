...
pub fn trim_trailing_trivia_pieces<L:Language>(
	...
	// We have to use `take` to avoid panicking on `ExactSizeIterator
	// under-reported length`.
	...
	fn len(&self) -> usize {
		match &self.first {
			Some(first) => {
				...
				// SAFETY: Should be safe because a program can never contain
				// more than u32 pieces because the text ranges are
				// represented as u32 (and each piece must at least contain a
				// single character).
