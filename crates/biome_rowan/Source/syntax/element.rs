...
use crate::{cursor, syntax::SyntaxTrivia, Language, NodeOrToken, SyntaxNode, SyntaxToken};
...
impl<L:Language> From<SyntaxToken<L>> for SyntaxElement<L> {
	fn from(token:SyntaxToken<L>) -> SyntaxElement<L> { NodeOrToken::Token(token) }
}

impl<L:Language> From<SyntaxNode<L>> for SyntaxElement<L> {
	fn from(node:SyntaxNode<L>) -> SyntaxElement<L> { NodeOrToken::Node(node) }
}
...
impl SyntaxElementKey {
	pub(crate) fn new(node_data:NonNull<()>, offset:TextSize) -> Self { Self { node_data, offset } }
}
