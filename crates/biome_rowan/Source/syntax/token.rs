...
use crate::{
	...
	syntax::{element::SyntaxElementKey, SyntaxTrivia},
...
impl<L:Language> SyntaxToken<L> {
	...
	pub(super) fn green_token(&self) -> GreenToken { self.raw.green().to_owned() }
	...
	/// assert_eq!(format!("{:?}", "\n\t let \t\t"), format!("{:?}", token.token_text()));
	/// ```
	pub fn token_text(&self) -> TokenText { self.raw.token_text() }

	pub fn token_text_trimmed(&self) -> TokenText { self.raw.token_text_trimmed() }
	...
	/// ```
	pub fn text_trimmed(&self) -> &str { self.raw.text_trimmed() }

	pub fn parent(&self) -> Option<SyntaxNode<L>> { self.raw.parent().map(SyntaxNode::from) }
	...
	/// Return a new version of this token with its leading trivia replaced with
	/// `trivia`
	#[must_use = "syntax elements are immutable, the result of update methods must be propagated \
	              to have any effect"]
	...
	/// Return a new version of this token with its trailing trivia replaced
	/// with `trivia`
	#[must_use = "syntax elements are immutable, the result of update methods must be propagated \
	              to have any effect"]
	...
	// Return a new version of this token with `trivia` prepended to its leading
	// trivia.
	/// # Examples
	///
	/// ```
	...
	/// assert_eq!(
	/// 	format!("{:?}", new_token),
	/// 	"LET_TOKEN@0..13 \"let\" [Whitespace(\" \\t\"), Comments(\"/*c*/\"), Whitespace(\" \")] \
	/// 	 [Whitespace(\" \\t\")]"
	/// );
	/// ```
	#[must_use = "syntax elements are immutable, the result of update methods must be propagated \
	...
	// Return a new version of this token with `trivia` appended to its trailing
	// trivia.
	/// # Examples
	///
	/// ```
	...
	/// assert_eq!(
	/// 	format!("{:?}", new_token),
	/// 	"LET_TOKEN@0..13 \"let\" [Whitespace(\"\\t \")] [Whitespace(\" \"), Comments(\"/*c*/\"), \
	/// 	 Whitespace(\"\\t \")]"
	/// );
	/// ```
	#[must_use = "syntax elements are immutable, the result of update methods must be propagated \
	...
	/// Return a new version of this token without leading newlines and
	/// whitespaces.
	///
	/// # Examples
	///
	...
	/// Return whitespace that juxtapose the token until the first
	/// non-whitespace item.
	...
	///
	/// Looking backward in the text, a token owns all of its preceding trivia
	...
	/// ```
	#[inline]
	pub fn leading_trivia(&self) -> SyntaxTrivia<L> { SyntaxTrivia::new(self.raw.leading_trivia()) }

	/// Returns the token's trailing trivia.
	///
	/// A token owns all of its following trivia up to, but not including, the
	...
	pub fn has_trailing_whitespace(&self) -> bool {
		self.trailing_trivia().pieces().any(|piece| piece.is_whitespace())
	}
	...
	/// Checks if the token has any leading trivia that is a whitespace or a
	/// line break
	...
	pub fn has_leading_newline(&self) -> bool {
		self.leading_trivia().pieces().any(|piece| piece.is_newline())
	}
}
...
impl<L:Language> From<SyntaxToken<L>> for cursor::SyntaxToken {
	fn from(token:SyntaxToken<L>) -> cursor::SyntaxToken { token.raw }
}

impl<L:Language> From<cursor::SyntaxToken> for SyntaxToken<L> {
	fn from(raw:cursor::SyntaxToken) -> SyntaxToken<L> { SyntaxToken { raw, _p:PhantomData } }
}
