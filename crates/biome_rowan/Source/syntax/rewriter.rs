...
/// Meaning, it first visits the `root`, and then visits the children of the
/// root from left to right, recursively traversing into child nodes and calling
/// [`visit_node`](SyntaxRewriter) for every node.
///
...
/// # Examples
///
/// Implementation of a rewritten that replaces all literal expression nodes
/// that contain a number token with a bogus node.
///
/// ```
/// # use std::iter;
...
pub trait SyntaxRewriter {
	...
	/// Recursively transforms the subtree of `node` by calling
	/// [`visit_node`](SyntaxRewriter::visit_node) for every token and
	/// [`visit_token`](SyntaxRewriter::visit_token) for every token in the
	/// subtree.
	///
	/// Returns a new syntax tree reflecting the changes by the rewriter if it
	/// replaced any node and returns `node` if no changes were made.
	...
	/// Called for every node in the tree. The method should return a signal
	/// specifying what should be done with the node
	///
	/// * [VisitNodeSignal::Traverse]: Recourse into `node` so that
	///   [`visit_node`](SyntaxRewriter::visit_node) gets called for all
	///   children of `node`. The `node` will only be replaced if any node in
	///   its subtree changes.
	/// * [VisitNodeSignal::Replace]: Replaces `node` with the node specified in
	///   the [`Replace`](VisitNodeSignal::Replace) variant. It's your
	///   responsibility to call [`traverse`](SyntaxRewriter::transform) for any
	///   child of `node` for which you want the rewritter to recurse into its
	///   content.
	fn visit_node(&mut self, node:SyntaxNode<Self::Language>) -> VisitNodeSignal<Self::Language> {
		VisitNodeSignal::Traverse(node)
	}

	/// Called for every token in the tree. Returning a new token changes the
	/// token in the parent node.
...
pub enum VisitNodeSignal<L:Language> {
	/// Signals the [SyntaxRewriter] to replace the current node with the
	/// specified node.
	Replace(SyntaxNode<L>),

	/// Signals the [SyntaxRewriter] to traverse into the children of the
	/// specified node.
