...
pub(crate) fn has_live() -> bool { countme::get::<_SyntaxElement>().live > 0 }
...
#[derive(Debug)]
enum NodeKind {
	Root { green:GreenElement },
	Child { green:WeakGreenElement, parent:Rc<NodeData> },
}
...
impl WeakGreenElement {
	fn new(green:GreenElementRef) -> Self {
		match green {
			NodeOrToken::Node(ptr) => Self::Node { ptr:ptr::NonNull::from(ptr) },
			NodeOrToken::Token(ptr) => Self::Token { ptr:ptr::NonNull::from(ptr) },
		}
	}
...
impl NodeData {
	...
	fn new(kind:NodeKind, slot:u32, offset:TextSize) -> Rc<NodeData> {
		let res = NodeData { _c:Count::new(), kind, slot, offset };
	...
	fn parent_node(&self) -> Option<SyntaxNode> {
		debug_assert!(matches!(self.parent()?.green(), GreenElementRef::Node { .. }));
		match &self.kind {
			NodeKind::Child { parent, .. } => Some(SyntaxNode { ptr:parent.clone() }),
			NodeKind::Root { .. } => None,
		}
	}
	...
	/// Returns an iterator over the siblings of this node. The iterator is
	/// positioned at the current node.
	#[inline]
	...
	#[inline]
	fn slot(&self) -> u32 { self.slot }

	#[inline]
	fn offset(&self) -> TextSize { self.offset }
	...
	#[inline]
	fn kind(&self) -> RawSyntaxKind { self.green().kind() }
	...
	fn next_sibling_or_token(&self) -> Option<SyntaxElement> {
		...
		siblings.following().next().and_then(|child| {
			let parent = self.parent_node()?;
			let offset = parent.offset() + child.rel_offset();
			Some(SyntaxElement::new(child.element(), parent, child.slot(), offset))
		})
	}
	...
	fn detach(self: Rc<Self>) -> Rc<Self> {
		match &self.kind {
			NodeKind::Child { green, .. } => {
				Self::new(NodeKind::Root { green:green.to_owned() }, 0, 0.into())
			},
			// If this node is already detached, increment the reference count
			// and return a clone
	...
	where
		...
		let green = match self.green() {
			...
			NodeOrToken::Token(_) => {
				panic!("called splice_slots on a token node")
			},
		};
