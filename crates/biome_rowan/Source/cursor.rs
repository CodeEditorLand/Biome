...
impl WeakGreenElement {
	...
	fn to_owned(&self) -> GreenElement {
		match self {
			WeakGreenElement::Node { ptr } => GreenElement::Node(unsafe { ptr.as_ref().to_owned() }),
			WeakGreenElement::Token { ptr } => GreenElement::Token(unsafe { ptr.as_ref().to_owned() }),
		}
	}
}
...
impl NodeData {
	...
	fn detach(self: Rc<Self>) -> Rc<Self> {
		match &self.kind {
			NodeKind::Child { green, .. } => Self::new(NodeKind::Root { green: green.to_owned() }, 0, 0.into()),
	...
	#[must_use = "syntax elements are immutable, the result of update methods must be propagated to have any effect"]
	fn replace_child(mut self: Rc<Self>, prev_elem: SyntaxElement, next_elem: SyntaxElement) -> Option<Rc<Self>> {
