...
pub(crate) use node::{
	Preorder, PreorderWithTokens, SyntaxElementChildren, SyntaxNode,
	...
	fn new(green: GreenElementRef) -> Self {
		match green {
			NodeOrToken::Node(ptr) => {
				Self::Node { ptr: ptr::NonNull::from(ptr) }
			},
			NodeOrToken::Token(ptr) => {
				Self::Token { ptr: ptr::NonNull::from(ptr) }
			},
		}
	}
	...
	fn as_deref(&self) -> GreenElementRef {
		match self {
			WeakGreenElement::Node { ptr } => {
				GreenElementRef::Node(unsafe { ptr.as_ref() })
			},
	...
	fn parent_node(&self) -> Option<SyntaxNode> {
		...
		match &self.kind {
			NodeKind::Child { parent, .. } => {
				Some(SyntaxNode { ptr: parent.clone() })
			},
			NodeKind::Root { .. } => None,
		}
	}
	...
	fn detach(self: Rc<Self>) -> Rc<Self> {
		match &self.kind {
			NodeKind::Child { green, .. } => Self::new(
				NodeKind::Root { green: green.to_owned() },
				0,
				0.into(),
			),
		...
		I: Iterator<Item = Option<green::GreenElement>>,
	{
		let green = match self.green() {
			NodeOrToken::Node(green) => {
				green.splice_slots(range, replace_with).into()
			},
			NodeOrToken::Token(_) => {
