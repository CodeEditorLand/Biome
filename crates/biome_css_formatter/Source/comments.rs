...
use biome_css_syntax::{
	AnyCssDeclarationName, CssFunction, CssIdentifier, CssLanguage, TextLen,
};
use biome_diagnostics::category;
use biome_formatter::comments::{
	is_doc_comment, CommentKind, CommentPlacement, CommentStyle,
	CommentTextPosition, Comments, DecoratedComment, SourceComment,
};
...
fn handle_declaration_name_comment(
	...
	match comment.preceding_node() {
		Some(following_node)
			if AnyCssDeclarationName::can_cast(following_node.kind()) =>
		{
...
fn handle_function_comment(
	...
	let is_inside_function =
		CssFunction::can_cast(comment.enclosing_node().kind());
