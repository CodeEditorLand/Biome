...
impl<'token> FormatLiteralStringToken<'token> {
	pub fn new(token:&'token CssSyntaxToken) -> Self { Self { token } }
...
impl<'token> LiteralStringNormaliser<'token> {
	...
	fn normalise_text(&mut self) -> Cow<'token, str> {
		...
		//
		// Add the chosen quotes to any non-string tokensto normalize them into
		// strings.
		//
		// CSS has various places where "string-like" tokens can be used without
		// quotes, but the semantics aren't affected by whether they are
		// present or not. This function lets those tokens become string
		// literals by safely adding quotes around them.
