...
use biome_css_syntax::{
	CssLanguage,
	CssSyntaxKind::{CSS_STRING_LITERAL, CSS_URL_VALUE_RAW_LITERAL, IDENT},
	CssSyntaxToken,
};
use biome_formatter::{
	Format,
...
use crate::{
	AsFormat,
	CssFormatter,
	context::CssFormatOptions,
	prelude::{CssFormatContext, *},
};
...
impl From<SyntaxToken<CssLanguage>> for FormatTokenAsLowercase {
	fn from(value:SyntaxToken<CssLanguage>) -> Self { Self { token:value } }
}
...
impl<'token> FormatLiteralStringToken<'token> {
	...
	fn token(&self) -> &'token CssSyntaxToken { self.token }

	pub fn clean_text(&self, options:&CssFormatOptions) -> CleanedStringLiteralText {
		let token = self.token();

		debug_assert!(
			matches!(token.kind(), CSS_STRING_LITERAL | CSS_URL_VALUE_RAW_LITERAL | IDENT),
			"Found kind {:?}",
			token.kind()
		);
		...
		CleanedStringLiteralText { text:content, token }
	}
}
...
impl Format<CssFormatContext> for CleanedStringLiteralText<'_> {
	fn fmt(&self, f:&mut Formatter<CssFormatContext>) -> FormatResult<()> {
		format_replaced(
			self.token,
			&syntax_token_cow_slice(self.text.clone(), self.token, self.token.text_trimmed_range().start()),
		)
		.fmt(f)
	}
}
...
struct StringInformation {
	/// This is the quote that the is calculated and eventually used inside the
	/// string. It could be different from the one inside the formatter options
	preferred_quote:QuoteStyle,
}

impl FormatLiteralStringToken<'_> {
	/// This function determines which quotes should be used inside to enclose
	/// the string. The function take as a input the string **without quotes**.
	///
	/// # How it works
	///
	/// The function determines the preferred quote and alternate quote.
	/// The preferred quote is the one that comes from the formatter options.
	/// The alternate quote is the other one.
	///
	/// We check how many preferred quotes we have inside the content. If this
	/// number is greater then the number alternate quotes that we have inside
	/// the content, then we swap them, so we can reduce the number of escaped
	/// quotes.
	///
	/// For example, let's suppose that the preferred quote is double, and we
	/// have a string like this: ```js
	/// (" content \"\"\" don't ")
	/// ```
	/// Excluding the quotes at the start and beginning, we have three double
	/// quote and one single quote. If we decided to keep them like this, we
	/// would have three escaped quotes.
	///
	/// But then, we choose the single quote as preferred quote and we would
	/// have only one quote that is escaped, resulting into a string like this:
	/// ```js
	/// (' content """ dont\'t ')
	/// ```
	...
	fn compute_string_information(&self, chosen_quote:QuoteStyle) -> StringInformation {
		...
		if !matches!(self.token().kind(), CSS_STRING_LITERAL) {
			return StringInformation { preferred_quote:chosen_quote };
		}
...
/// Struct of convenience used to manipulate the string. It saves some state in
/// order to apply the normalise process.
...
impl<'token> LiteralStringNormaliser<'token> {
	pub fn new(token:&'token FormatLiteralStringToken<'_>, chosen_quote_style:QuoteStyle) -> Self {
		Self { token, chosen_quote_style }
	}

	fn normalise_text(&mut self) -> Cow<'token, str> {
		match self.token.parent_kind {
			StringLiteralParentKind::CharsetAtRule => {
				let string_information = StringInformation { preferred_quote:QuoteStyle::Double };

				self.normalise_tokens(string_information)
			},

			StringLiteralParentKind::Others => {
				let string_information = self.token.compute_string_information(self.chosen_quote_style);

				// Normalize string token and non-string token.
				//
				// Add the chosen quotes to any non-string tokensto normalize them into strings.
				//
				// CSS has various places where "string-like" tokens can be used without quotes,
				// but the semantics aren't affected by whether they are present or not.
				// This function lets those tokens become string literals by safely adding
				// quotes around them.
				self.normalise_tokens(string_information)
			},
		}
	...
	fn get_token(&self) -> &'token CssSyntaxToken { self.token.token() }
	...
	fn swap_quotes(&self, content_to_use:&'token str, string_information:&StringInformation) -> Cow<'token, str> {
