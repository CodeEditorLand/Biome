...
use biome_formatter::{write, CstFormatContext, Format, FormatResult};

use crate::{prelude::*, CssFormatter};
...
impl<'a> FormatCssBlockLike<'a> {
	pub(crate) fn new(block:&'a CssBlockLike) -> Self { Self { block } }
	...
	fn fmt(&self, f:&mut Formatter<CssFormatContext>) -> FormatResult<()> {
		...
		if self.block.is_empty() {
			...
			if has_dangling_comments {
				write!(f, [format_dangling_comments(self.block.syntax()).with_block_indent()])?;
			} else {
