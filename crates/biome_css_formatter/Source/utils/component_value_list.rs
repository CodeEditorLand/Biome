...
use biome_formatter::{CstFormatContext, FormatOptions, FormatResult, write};
use biome_rowan::{AstNode, AstNodeList, TextSize};
use biome_string_case::StrLikeExtension;

use crate::{CssFormatter, comments::CssComments, prelude::*};
...
where
	...
	let has_newline = match layout {
		ValueListLayout::PreserveInline => node.iter().skip(1).any(|element| element.syntax().has_leading_newline()),
		_ => false,
	};
		...
		for (element, formatted) in node.iter().zip(node.iter().formatted()) {
			fill.entry(
				&format_once(|f| {
					...
					// A separator should not be added before the comma because the comma acts as a
					// `CssGenericDelimiter`.
					...
					if !is_comma {
						if matches!(layout, ValueListLayout::PreserveInline | ValueListLayout::OnePerLine) {
...
/// printed compactly.
pub(crate) fn get_value_list_layout<N, I>(list:&N, _:&CssComments, f:&CssFormatter) -> ValueListLayout
where
	N: AstNodeList<Language = CssLanguage, Node = I> + AstNode<Language = CssLanguage>,
	I: AstNode<Language = CssLanguage> + IntoFormat<CssFormatContext>, {
	...
	let is_comma_separated = list.iter().any(|x| CssGenericDelimiter::cast_ref(x.syntax()).is_some());
