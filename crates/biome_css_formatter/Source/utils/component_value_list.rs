...
use biome_formatter::{write, CstFormatContext, FormatOptions, FormatResult};
use biome_rowan::{AstNode, AstNodeList, TextSize};
use biome_string_case::StrOnlyExtension;

use crate::{comments::CssComments, prelude::*, CssFormatter};
...
where
	...
	// We skip the first element because it is the first element in the list and
	// should not be considered. div {
	//     grid-template-columns:
	//                          1fr 100px 3em;
	// }
	let has_newline = match layout {
		ValueListLayout::PreserveInline => {
			node.iter().skip(1).any(|element| element.syntax().has_leading_newline())
		},
		_ => false,
	};
	...
	let values = format_with(|f:&mut Formatter<'_, CssFormatContext>| {
		...
		for (element, formatted) in node.iter().zip(node.iter().formatted()) {
			fill.entry(
				&format_once(|f| {
					// If the current element is not a comma, insert a soft line
					// break or a space. Consider the CSS example: `font:
					// first , second;` The desired format is: `font:
					// first, second;` A separator should not be added
					// before the comma because the comma acts as a
					// `CssGenericDelimiter`.
	...
	match layout {
		ValueListLayout::PreserveInline => {
			let content = format_once(|f| {
				if has_newline {
					// Add line break before the first element if we have more
					// than two lines.
					write!(f, [hard_line_break()])?;
				}
				write!(f, [values])
...
pub(crate) enum ValueListLayout {
	...
	/// Keeps elements on the same line if they're on the same line in the
	/// source file.
	///
	/// For example, this layout option is commonly used for CSS grid
	/// properties. It ensures that properties remain on the same line in the
	/// formatted output if they were on the same line in the source file. If a
	/// new line is encountered in the source file, a corresponding new line is
	/// added in the formatted output at the beginning of the property.
	///
	/// # Example
	///
	/// ```css
...
where
	...
	let text_size:TextSize = list
		...
		.map(|x| x.range().len())
		.sum();
	let value_count = list.iter().filter(|x| x.range().len() > TextSize::from(1)).count();

	let is_comma_separated =
		list.iter().any(|x| CssGenericDelimiter::cast_ref(x.syntax()).is_some());

	// TODO: Check for comments, check for the types of elements in the list,
	// etc.
	if is_grid_property {
		ValueListLayout::PreserveInline
	} else if list.len() == 1 {
