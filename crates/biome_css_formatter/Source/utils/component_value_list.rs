	...
	I: AstNode<Language = CssLanguage> + IntoFormat<CssFormatContext>,
{
	let is_grid_property = list
		...
		.and_then(|name| name.as_css_identifier().map(|name| name.text().to_lowercase()))
		.map_or(false, |name| name.starts_with("grid-template") || name == "grid");

	let text_size: TextSize =
		list.iter().filter(|x| x.range().len() > TextSize::from(1)).map(|x| x.range().len()).sum();
