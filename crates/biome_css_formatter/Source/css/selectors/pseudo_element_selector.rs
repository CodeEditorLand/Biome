...
impl FormatNodeRule<CssPseudoElementSelector> for FormatCssPseudoElementSelector {
	fn fmt_fields(&self, node:&CssPseudoElementSelector, f:&mut CssFormatter) -> FormatResult<()> {
		let CssPseudoElementSelectorFields { double_colon_token, element } = node.as_fields();
