...
impl FormatNodeRule<CssPseudoClassOfNthSelector> for FormatCssPseudoClassOfNthSelector {
	fn fmt_fields(&self, node: &CssPseudoClassOfNthSelector, f: &mut CssFormatter) -> FormatResult<()> {
		let CssPseudoClassOfNthSelectorFields { of_token, selectors } = node.as_fields();
