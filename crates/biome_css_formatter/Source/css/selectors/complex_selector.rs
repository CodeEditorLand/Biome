...
impl FormatNodeRule<CssComplexSelector> for FormatCssComplexSelector {
	fn fmt_fields(&self, node:&CssComplexSelector, f:&mut CssFormatter) -> FormatResult<()> {
		let CssComplexSelectorFields { left, combinator, right } = node.as_fields();
		...
		if let Some(computed_selector) = left.clone()?.as_css_compound_selector() {
			...
			has_leading_comments = simple_selector_has_leading_comments || sub_selector_has_leading_comments;
		}

		// The selector list first child like the case:
		// .a b, .a c {}
		// Then the complex selector `.a b` should be the first child of the selector
		// list.
		let is_selector_list_first_child = node.syntax().parent().is_some_and(|parent| {
			matches!(parent.kind(), CssSyntaxKind::CSS_SELECTOR_LIST) && node.syntax().prev_sibling().is_none()
		});

		// If the complex selector has leading comments and it's not the first child of
		// the selector list, don't insert `soft_line_break_or_space` before it,
		// because the comments will cause the selector to break. Otherwise the case
		// like: .a b,
		...
		if has_leading_comments && !is_selector_list_first_child {
			write!(f, [left.format(), formatted_combinator, space(), right.format()])
		} else {
			write!(
				f,
				[
