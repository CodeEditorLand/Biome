...
impl FormatNodeRule<CssValueAtRuleGenericProperty> for FormatCssValueAtRuleGenericProperty {
	fn fmt_fields(&self, node:&CssValueAtRuleGenericProperty, f:&mut CssFormatter) -> FormatResult<()> {
		let CssValueAtRuleGenericPropertyFields { name, colon_token, value } = node.as_fields();
