...
impl FormatRule<AnyCssPseudoClass> for FormatAnyCssPseudoClass {
	...
	fn fmt(&self, node: &AnyCssPseudoClass, f: &mut CssFormatter) -> FormatResult<()> {
		match node {
			...
			AnyCssPseudoClass::CssPseudoClassFunctionCompoundSelectorList(node) => node.format().fmt(f),
			...
			AnyCssPseudoClass::CssPseudoClassFunctionRelativeSelectorList(node) => node.format().fmt(f),
