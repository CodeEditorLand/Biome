...
impl FormatNodeRule<CssImportAtRule> for FormatCssImportAtRule {
	fn fmt_fields(&self, node:&CssImportAtRule, f:&mut CssFormatter) -> FormatResult<()> {
		let CssImportAtRuleFields { import_token, url, layer, supports, media, semicolon_token } =
			node.as_fields();
		...
		if has_any_modifiers {
			// If there are, we need to group them together and try to fill
			// them.
			...
			// If there are no modifiers, simply write the formatted `url` to
			// the formatter `f`.
			write!(f, [url.format()])?;
		}
