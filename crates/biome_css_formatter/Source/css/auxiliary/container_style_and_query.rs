...
impl FormatNodeRule<CssContainerStyleAndQuery> for FormatCssContainerStyleAndQuery {
	fn fmt_fields(&self, node:&CssContainerStyleAndQuery, f:&mut CssFormatter) -> FormatResult<()> {
		let CssContainerStyleAndQueryFields { left, and_token, right } = node.as_fields();
