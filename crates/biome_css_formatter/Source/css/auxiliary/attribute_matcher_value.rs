			...
			AnyCssAttributeMatcherValue::CssIdentifier(ident) => {
				...
				write!(
					...
					[
						...
						// Unlike almost all other usages of regular
						// identifiers, attribute values are
						// case-sensitive, so the identifier here
						// does not get converted to lowercase. Once it's
						// quoted, it will be parsed as a CssString on
						// the next pass, at which point casing is
