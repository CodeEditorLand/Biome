...
impl FormatRule<CssKeyframesSelectorList> for FormatCssKeyframesSelectorList {
	...
	fn fmt(&self, node:&CssKeyframesSelectorList, f:&mut CssFormatter) -> FormatResult<()> {
		// Using `join_with` and a manual separator instead of
		// `join_nodes_with_soft_line` here allows ensures that the list won't
		// try to read the input source to preserve line breaks and empty
		// lines. That way, an input like:     h1,
		//
		//     h2 {}
		// gets formatted to
		//     h1, h2 {}
		...
		//
		// For top-level selector lists, like as the `prelude` of a `CssRule`,
		// the parent node can wrap this list in a group and set
		// `should_expand(true)` to have the separator expand into full line
		// breaks instead of trying to fit on a single line.
		...
		for formatted in node.format_separated(",") {
			...
			// first and only expands when the single selector can't fit the
			// line.
			//
