...
impl FormatRule<CssSelectorList> for FormatCssSelectorList {
	...
	fn fmt(&self, node:&CssSelectorList, f:&mut CssFormatter) -> FormatResult<()> {
		...
		// `join_nodes_with_soft_line` here allows ensures that the list won't try to
		// read the input source to preserve line breaks and empty lines. That way, an
		// input like:     h1,
		//
		//     h2 {}
		// gets formatted to
		...
		for formatted in node.format_separated(",") {
			...
			let computed_selector = formatted
				.node()?
				.as_css_complex_selector()
				.and_then(|complex_selector| complex_selector.left().ok()?.as_css_compound_selector().cloned());
