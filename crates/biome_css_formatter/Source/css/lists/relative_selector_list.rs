	...
	fn fmt(&self, node:&CssRelativeSelectorList, f:&mut CssFormatter) -> FormatResult<()> {
		...
		for formatted in node.format_separated(",") {
			let has_leading_comments = formatted
				...
				.and_then(|relative_selector| relative_selector.selector().ok()?.as_css_compound_selector().cloned())
			...
			if has_leading_comments {
				// Computed Selector which contains a leading comments should be formatted
				// without indent.
