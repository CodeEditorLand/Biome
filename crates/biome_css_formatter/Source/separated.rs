	...
	AstSeparatedList<Language = CssLanguage>
{
	...
	) -> CssFormatSeparatedIter<Self::Node> {
...
impl<N, O> CssFormatSeparatedElementRuleWithOptions<N, O> {
	pub(crate) fn new(options: O) -> Self {
		Self { node: PhantomData, options }
	}
}

impl<N, O, R> FormatSeparatedElementRule<N>
	for CssFormatSeparatedElementRuleWithOptions<N, O>
where
	O: Clone + Copy,
	R: FormatNodeRule<N>
		+ FormatRuleWithOptions<N, Context = CssFormatContext, Options = O>,
	N: AstNode<Language = CssLanguage>
		+ for<'a> AsFormat<
			CssFormatContext,
			Format<'a> = FormatRefWithRule<'a, N, R>,
		> + 'static,
{
	type Context = CssFormatContext;
