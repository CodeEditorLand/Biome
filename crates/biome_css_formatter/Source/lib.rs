...
use biome_css_syntax::{
	AnyCssDeclarationBlock, AnyCssRule, AnyCssRuleBlock, AnyCssValue, CssLanguage, CssSyntaxKind, CssSyntaxNode,
	CssSyntaxToken,
};
...
use biome_formatter::{
	CstFormatContext, FormatContext, FormatLanguage, FormatOwnedWithRule, FormatRefWithRule, TransformSourceMap, write,
};
	...
	fn fmt(&self, token: &CssSyntaxToken, f: &mut Formatter<Self::Context>) -> FormatResult<()> {
		...
		if token.kind().is_contextual_keyword() {
			...
			match original.to_ascii_lowercase_cow() {
				...
				Cow::Owned(lowercase) => write!(f, [dynamic_text(&lowercase, token.text_trimmed_range().start())]),
			}
		} else {
