...
impl JsonFileSource {
	...
	// This list should be SORTED!
	const WELL_KNOWN_JSON_ALLOW_COMMENTS_DIRECTORIES:
		&'static [&'static [u8]] = &[b".vscode", b".zed"];

	// Well-known JSON-like files that support comments and trailing commas
	// This list should be SORTED!
	// Note: we shouldn't include machine generated files
	const WELL_KNOWN_JSON_ALLOW_COMMENTS_AND_TRAILING_COMMAS_FILES:
		&'static [&'static [u8]] = &[
	...
	pub fn is_well_known_json_allow_comments_directory(
		dirname: &OsStr,
	) -> bool {
		// Note: we don't use a binary search because the slice has only a few elements.
		Self::WELL_KNOWN_JSON_ALLOW_COMMENTS_DIRECTORIES
			.contains(&dirname.as_encoded_bytes())
	}

	pub fn is_well_known_json_allow_comments_and_trailing_commas_file(
		filename: &OsStr,
	) -> bool {
	...
	pub fn try_from_well_known(path: &Path) -> Result<Self, FileSourceError> {
		let file_name =
			path.file_name().ok_or(FileSourceError::MissingFileName)?;
		if Self::is_well_known_json_allow_comments_and_trailing_commas_file(
			file_name,
		) {
		...
		if let Some(Component::Normal(parent_dir)) =
	...
	pub fn try_from_extension(
		...
		match extension.as_encoded_bytes() {
			...
			b"json" | b"jsonld" | b"webapp" | b"webmanifest" => {
				Ok(Self::json())
			},
	...
	pub fn try_from_language_id(
		...
		match language_id {
			"json" => Ok(Self::json()),
			"jsonc" | "snippets" => {
				Ok(Self::json_allow_comments_and_trailing_commas())
			},
...
fn test_order() {
	for items in
		JsonFileSource::WELL_KNOWN_JSON_ALLOW_COMMENTS_AND_TRAILING_COMMAS_FILES
			.windows(2)
	{
		assert!(
			items[0] < items[1],
	...
	for items in
		JsonFileSource::WELL_KNOWN_JSON_ALLOW_COMMENTS_DIRECTORIES.windows(2)
	{
		assert!(
			items[0] < items[1],
			"{} < {}",
