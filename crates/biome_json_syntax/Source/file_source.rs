...
impl JsonFileSource {
	...
	const WELL_KNOWN_JSON_ALLOW_COMMENTS_FILES:&'static [&'static [u8]] = &[
		...
		b".jscsrc",
		// `.jsfmtrc` can be either an `ini` file or a `json` file (which will be parsed after
		// `strip-with-comments`), so we shouldn't include it here. https://github.com/rdio/jsfmt?tab=readme-ov-file#jsfmtrc
