...
use biome_configuration::diagnostics::{
	CantLoadExtendFile, EditorConfigDiagnostic,
};
use biome_configuration::VERSION;
use biome_configuration::{
	push_to_analyzer_rules, BiomeDiagnostic, ConfigurationPathHint,
	ConfigurationPayload, PartialConfiguration,
};
			...
			diagnostics: diagnostics
				...
				.map(|diagnostic| {
					diagnostic.with_file_path(
						configuration_file_path.display().to_string(),
					)
				})
				.collect(),
...
fn load_config(
	...
	if let ConfigurationPathHint::FromUser(ref config_file_path) = base_path {
		if file_system.path_is_file(config_file_path) {
			...
			let parser_options =
				match config_file_path.extension().map(OsStr::as_encoded_bytes)
				{
	...
	if let Some(auto_search_result) = match file_system.auto_search(
		...
		let parser_options =
			match file_path.extension().map(OsStr::as_encoded_bytes) {
				Some(b"json") => JsonParserOptions::default(),
				_ => JsonParserOptions::default()
					.with_allow_comments()
					.with_allow_trailing_commas(),
...
pub fn load_editorconfig(
	...
	(Option<PartialConfiguration>, Vec<EditorConfigDiagnostic>),
	WorkspaceError,
> {
...
pub fn create_config(
	...
	let mut config_file =
		fs.open_with_options(&path, options).map_err(|err| {
	...
	if VERSION == "0.0.0" {
		...
		configuration.schema =
			Some(format!("https://biomejs.dev/schemas/{VERSION}/schema.json"));
	}
	...
	config_file.set_content(formatted.as_code().as_bytes()).map_err(|_| {
		WorkspaceError::cant_read_file(format!("{}", path.display()))
	})?;
...
pub fn to_analyzer_rules(settings: &Settings, path: &Path) -> AnalyzerRules {
	...
	if let Some(rules) = linter_settings.rules.as_ref() {
		push_to_analyzer_rules(
			rules,
			js_lint_metadata.deref(),
			&mut analyzer_rules,
		);
		push_to_analyzer_rules(
			rules,
			css_lint_metadata.deref(),
			&mut analyzer_rules,
		);
		push_to_analyzer_rules(
			rules,
			json_lint_metadata.deref(),
			&mut analyzer_rules,
		);
		push_to_analyzer_rules(
			rules,
			graphql_lint_metadata.deref(),
			&mut analyzer_rules,
		);
	}
...
impl PartialConfigurationExt for PartialConfiguration {
	...
	fn apply_extends(
		...
		let (configurations, errors): (Vec<_>, Vec<_>) = deserialized
			...
			.map(|(config, diagnostics)| {
				(config.unwrap_or_default(), diagnostics)
			})
			.unzip();
	...
	fn migrate_deprecated_fields(&mut self) {
		...
		if let Some(formatter) = self.formatter.as_mut() {
			if formatter.indent_size.is_some()
				&& formatter.indent_width.is_none()
			{
		...
		if let Some(formatter) =
			self.javascript.as_mut().and_then(|js| js.formatter.as_mut())
		{
			if formatter.indent_size.is_some()
				&& formatter.indent_width.is_none()
			{
			...
			if formatter.trailing_comma.is_some()
				&& formatter.trailing_commas.is_none()
			{
		...
		// TODO: remove in biome 2.0
		if let Some(formatter) =
			self.json.as_mut().and_then(|json| json.formatter.as_mut())
		{
			if formatter.indent_size.is_some()
				&& formatter.indent_width.is_none()
			{
