...
use biome_configuration::{
	diagnostics::{CantLoadExtendFile, EditorConfigDiagnostic},
...
use biome_deserialize::{json::deserialize_from_json_str, Deserialized, Merge};
...
use crate::{matcher::Pattern, settings::Settings, DynRef, WorkspaceError};
...
impl LoadedConfiguration {
	...
	pub fn directory_path(&self) -> Option<&Path> { self.directory_path.as_deref() }

	/// Return the path of the **file** where the configuration is
	pub fn file_path(&self) -> Option<&Path> { self.file_path.as_deref() }
	...
	/// It return an iterator over the diagnostics emitted during the resolution
...
impl<'a> ConfigurationDiagnosticsIter<'a> {
	fn new(errors:&'a [Error]) -> Self { Self { len:errors.len(), index:0, errors } }
}
...
/// - [Option]: sometimes not having a configuration file should not be an
///   error, so we need this type.
...
///
/// The configuration file will be read from the `file_system`. A [path
/// hint](ConfigurationPathHint) should be provided.
///
/// - If the path hint is a path to a file that is provided by the user, the
///   function will try to load that file or error. The name doesn't have to be
///   `biome.json` or `biome.jsonc`. And if it doesn't end with `.json`, Biome
///   will try to deserialize it as a `.jsonc` file.
///
...
///
/// - Otherwise, the function will try to traverse upwards the file system until
///   it finds a `biome.json` or `biome.jsonc` file, or there aren't directories
///   anymore. In this case, the function will not error but return an
///   `Ok(None)`, which means Biome will use the default configuration.
fn load_config(
	...
	if let ConfigurationPathHint::FromUser(ref config_file_path) = base_path {
		if file_system.path_is_file(config_file_path) {
			...
			let parser_options = match config_file_path.extension().map(OsStr::as_encoded_bytes) {
				...
				_ => {
					JsonParserOptions::default().with_allow_comments().with_allow_trailing_commas()
				},
			};
			let deserialized =
	...
	if let Some(auto_search_result) = match file_system.auto_search(
		...
		let parser_options = match file_path.extension().map(OsStr::as_encoded_bytes) {
			Some(b"json") => JsonParserOptions::default(),
			_ => JsonParserOptions::default().with_allow_comments().with_allow_trailing_commas(),
		};
...
pub fn load_editorconfig(
	...
	// We currently don't support the `root` property, so we just search for the
	// file like we do for biome.json
	if let Some(auto_search_result) =
		...
		let AutoSearchResult { content, file_path: path } = auto_search_result;
		let editorconfig = biome_configuration::editorconfig::parse_str(&content)?;
		let config = editorconfig.to_biome();

		// test the patterns to see if they are parsable so we can emit a better
		// diagnostic
...
impl PartialConfigurationExt for PartialConfiguration {
	...
	fn apply_extends(
		...
		if let Some(mut extended_configuration) = extended_configuration {
			// We swap them to avoid having to clone `self.configuration` to
			// merge it.
	...
	/// It attempts to deserialize all the configuration files that were
	...
	fn deserialize_extends(
		...
		for extend_entry in extends.iter() {
			...
			let deserialized = deserialize_from_json_str::<PartialConfiguration>(
				content.as_str(),
				match extend_configuration_file_path.extension().map(OsStr::as_encoded_bytes) {
					Some(b"json") => JsonParserOptions::default(),
					_ => {
	...
	fn migrate_deprecated_fields(&mut self) {
		...
		// TODO: remove in biome 2.0
		if let Some(formatter) = self.javascript.as_mut().and_then(|js| js.formatter.as_mut()) {
			if formatter.indent_size.is_some() && formatter.indent_width.is_none() {
				formatter.indent_width = formatter.indent_size;
			}
	...
	/// This function checks if the VCS integration is enabled, and if so, it
	/// will attempts to resolve the VCS root directory and the `.gitignore`
	/// file.
	///
	/// ## Returns
	///
				...
				if !vcs.ignore_file_disabled() {
					...
					if let Some(result) = result {
						return Ok((
							...
							result.content.lines().map(String::from).collect::<Vec<String>>(),
						));
					}
				}
			}
