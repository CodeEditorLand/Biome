...
use biome_configuration::{BiomeDiagnostic, ConfigurationPathHint, ConfigurationPayload, push_to_analyzer_rules};
...
use biome_fs::{AutoSearchResult, ConfigName, FileSystem, FileSystemDiagnostic, FsErrorKind, OpenOptions};
...
impl<'a> ConfigurationDiagnosticsIter<'a> {
	fn new(errors: &'a [Error]) -> Self {
		Self { len: errors.len(), index: 0, errors }
	}
}
...
impl LoadedConfiguration {
	fn try_from_payload(value: Option<ConfigurationPayload>, fs: &dyn FileSystem) -> Result<Self, WorkspaceError> {
		let Some(value) = value else {
			return Ok(LoadedConfiguration::default());
		};

		let ConfigurationPayload { external_resolution_base_path, configuration_file_path, deserialized } = value;
...
fn load_config(fs: &dyn FileSystem, base_path: ConfigurationPathHint) -> LoadConfig {
	...
	let Some(auto_search_result) =
		fs.auto_search_files(&configuration_directory, &[ConfigName::biome_json(), ConfigName::biome_jsonc()])
	else {
		return Ok(None);
	};
...
fn load_user_config(
	...
	if fs.path_is_file(config_file_path) {
		...
		let parser_options = match config_file_path.extension() {
			Some("json") => JsonParserOptions::default(),
			Some("jsonc") => JsonParserOptions::default().with_allow_comments().with_allow_trailing_commas(),
...
pub fn load_editorconfig(
	...
	if let Some(auto_search_result) = fs.auto_search_files(&workspace_root, &[".editorconfig"]) {
		let AutoSearchResult { content, directory_path, .. } = auto_search_result;
...
pub fn create_config(
	...
	if VERSION == "0.0.0" {
		...
		configuration.schema = Some(format!("https://biomejs.dev/schemas/{VERSION}/schema.json").into());
	}

	let contents =
		serde_json::to_string_pretty(&configuration).map_err(|_| BiomeDiagnostic::new_serialization_error())?;

	let parsed = parse_json(&contents, JsonParserOptions::default());
	let formatted = biome_json_formatter::format_node(JsonFormatOptions::default(), &parsed.syntax())?
		.print()
...
impl ConfigurationExt for Configuration {
	...
	fn deserialize_extends(
		...
		for extend_entry in extends.iter() {
			...
			let extend_configuration_file_path = if extend_entry_as_path.starts_with(".")
				...
				Utf8PathBuf::try_from(
					fs.resolve_configuration(extend_entry.as_ref(), external_resolution_base_path)
						.map_err(|error| {
							BiomeDiagnostic::cant_resolve(external_resolution_base_path.to_string(), error)
						})?
						.into_path_buf(),
				)
			...
			let mut file = fs
				.open_with_options(extend_configuration_file_path.as_path(), OpenOptions::default().read(true))
				.map_err(|err| {
					CantLoadExtendFile::new(extend_configuration_file_path.to_string(), err.to_string())
