...
impl Dome {
	...
	pub fn new(paths:BTreeSet<BiomePath>) -> Self { Self { paths } }

	pub fn iter(&self) -> DomeIterator { DomeIterator { iter:self.paths.iter().peekable() } }

	pub fn to_paths(self) -> BTreeSet<BiomePath> { self.paths }
}
...
impl<'a> DomeIterator<'a> {
	pub fn next_config(&mut self) -> Option<&'a BiomePath> {
		return if let Some(path) = self.iter.peek() {
			if path.is_config() { self.iter.next() } else { None }
		} else {
			None
		};
	}
	...
	pub fn next_ignore(&mut self) -> Option<&'a BiomePath> {
		return if let Some(path) = self.iter.peek() {
			if path.is_ignore() { self.iter.next() } else { None }
		} else {
			None
		};
	}
	...
	pub fn next_manifest(&mut self) -> Option<&'a BiomePath> {
		return if let Some(path) = self.iter.peek() {
			if path.is_manifest() { self.iter.next() } else { None }
		} else {
			None
		};
	}
