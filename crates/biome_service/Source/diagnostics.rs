...
use biome_configuration::diagnostics::{
	ConfigurationDiagnostic, EditorConfigDiagnostic,
};
...
use biome_diagnostics::{
	category, Advices, Category, Diagnostic, DiagnosticTags, Location,
	LogCategory, MessageAndDescription, Severity, Visit,
};
...
impl From<CantLoadExtendFile> for WorkspaceError {
	fn from(value: CantLoadExtendFile) -> Self {
		WorkspaceError::Configuration(
			BiomeDiagnostic::CantLoadExtendFile(value).into(),
		)
	}
}
...
impl From<CompileError> for WorkspaceError {
	fn from(value: CompileError) -> Self {
		match value {
			...
			_ => Self::SearchError(SearchError::QueryError(QueryDiagnostic(
				format!("{value:?}"),
			))),
		}
	}
...
mod test {
	use crate::diagnostics::{
		CantReadDirectory, CantReadFile, DirtyWorkspace, FileIgnored,
		FileTooLarge, NotFound, SourceFileNotSupported,
	};
	...
	fn transport_serde_error() {
		snap_diagnostic(
			...
			TransportError::SerdeError(
				"Some serialization/deserialization error".to_string(),
			)
			.into(),
		)
	}
	...
	fn formatter_syntax_error() {
		snap_diagnostic(
			...
			WorkspaceError::FormatError(FormatError::SyntaxError)
				.with_file_path("example.js"),
		)
	}
}
