...
use biome_diagnostics::{
	Advices, Category, Diagnostic, DiagnosticTags, Location, LogCategory, MessageAndDescription, Severity, Visit,
	category,
};
...
impl WorkspaceError {
	...
	pub fn source_file_not_supported(language: DocumentFileSource, path: String, extension: Option<String>) -> Self {
		Self::SourceFileNotSupported(SourceFileNotSupported { file_source: language, path, extension })
	}
	...
	pub fn protected_file(file_path: impl Into<String>) -> Self {
		Self::ProtectedFile(ProtectedFile { file_path: file_path.into(), verbose_advice: ProtectedFileAdvice })
	}

	pub fn is_editor_config_error(&self) -> bool {
		matches!(self, WorkspaceError::Configuration(ConfigurationDiagnostic::EditorConfig(_)))
	}
}
...
impl From<CheckFileSizeResult> for FileTooLarge {
	fn from(value: CheckFileSizeResult) -> Self {
		Self { size: value.file_size, limit: value.limit }
	}
}
	...
	fn description(&self, fmt: &mut Formatter<'_>) -> fmt::Result {
		match self {
			...
			TransportError::ChannelClosed => {
				fmt.write_str("a request to the remote workspace failed because the connection was interrupted")
			},
			TransportError::Timeout => fmt.write_str("the request to the remote workspace timed out"),
	...
	fn from(value: CompileError) -> Self {
		match value {
			CompileError::ParsePatternError(diagnostic) => {
				Self::SearchError(SearchError::PatternCompilationError(PatternCompilationError::from(diagnostic)))
			},
...
mod test {
	...
	fn file_ignored() {
		snap_diagnostic(
			"file_ignored",
			WorkspaceError::FileIgnored(FileIgnored { path: "example.js".to_string() }).with_file_path("example.js"),
		)
	}
	...
	fn cant_read_file() {
		snap_diagnostic(
			"cant_read_file",
			WorkspaceError::CantReadFile(CantReadFile { path: "example.js".to_string() }).with_file_path("example.js"),
		)
	}

	#[test]
	fn not_found() {
		snap_diagnostic("not_found", WorkspaceError::NotFound(NotFound).with_file_path("not_found.js"))
	}
	...
	fn transport_serde_error() {
		snap_diagnostic(
			...
			TransportError::SerdeError("Some serialization/deserialization error".to_string()).into(),
		)
	}
