...
use biome_configuration::{
	diagnostics::{ConfigurationDiagnostic, EditorConfigDiagnostic},
	BiomeDiagnostic,
	CantLoadExtendFile,
};
use biome_console::{fmt::Bytes, markup};
...
pub enum WorkspaceError {
	...
	/// Emitted when a file could not be parsed because it's larger than the
	/// size limit
...
impl WorkspaceError {
	...
	pub fn cant_read_file(path:String) -> Self { Self::CantReadFile(CantReadFile { path }) }

	pub fn not_found() -> Self { Self::NotFound(NotFound) }

	pub fn file_too_large(path:String, size:usize, limit:usize) -> Self {
		Self::FileTooLarge(FileTooLarge { path, size, limit })
	}

	pub fn file_ignored(path:String) -> Self { Self::FileIgnored(FileIgnored { path }) }
	...
	pub fn vcs_disabled() -> Self { Self::Vcs(VcsDiagnostic::DisabledVcs(DisabledVcs {})) }
...
impl Debug for WorkspaceError {
	fn fmt(&self, f:&mut Formatter<'_>) -> fmt::Result { std::fmt::Display::fmt(self, f) }
}

impl Display for WorkspaceError {
	fn fmt(&self, f:&mut Formatter<'_>) -> fmt::Result { Diagnostic::description(self, f) }
}
...
impl From<FormatError> for WorkspaceError {
	fn from(err:FormatError) -> Self { Self::FormatError(err) }
}
...
impl From<PrintError> for WorkspaceError {
	fn from(err:PrintError) -> Self { Self::PrintError(err) }
}

impl From<FileSystemDiagnostic> for WorkspaceError {
	fn from(err:FileSystemDiagnostic) -> Self { Self::FileSystem(err) }
}

impl From<BiomeDiagnostic> for WorkspaceError {
	fn from(value:BiomeDiagnostic) -> Self { Self::Configuration(value.into()) }
}

impl From<EditorConfigDiagnostic> for WorkspaceError {
	fn from(value:EditorConfigDiagnostic) -> Self { Self::Configuration(value.into()) }
}
...
#[diagnostic(category = "internalError/fs", message = "Uncommitted changes in repository")]
...
#[diagnostic(
	category = "format",
	message = "Code formatting aborted due to parsing errors. To format code with errors, enable \
	           the 'formatter.formatWithErrors' option."
)]
...
impl Diagnostic for FileTooLarge {
	fn category(&self) -> Option<&'static Category> { Some(category!("internalError/fs")) }
	...
	fn description(&self, fmt:&mut Formatter<'_>) -> fmt::Result {
		write!(
			...
			"Size of {} is {} which exceeds configured maximum of {} for this project.\nThe file \
			 size limit exists to prevent us inadvertently slowing down and loading large files \
			 that we shouldn't.\nUse the `files.maxSize` configuration to change the maximum size \
			 of files processed.",
			self.path,
			Bytes(self.size),
			Bytes(self.limit)
...
impl Diagnostic for SourceFileNotSupported {
	fn category(&self) -> Option<&'static Category> { Some(category!("files/missingHandler")) }

	fn severity(&self) -> Severity { Severity::Warning }

	fn location(&self) -> Location<'_> { Location::builder().resource(&self.path).build() }
...
impl From<ParseDiagnostic> for PatternCompilationError {
	fn from(diagnostic:ParseDiagnostic) -> Self { Self { message:diagnostic.message } }
}
...
impl Diagnostic for QueryDiagnostic {
	fn category(&self) -> Option<&'static Category> { Some(category!("search")) }
...
pub fn extension_error(path:&BiomePath) -> WorkspaceError {
	...
	WorkspaceError::source_file_not_supported(
		...
		path.clone().extension().and_then(OsStr::to_str).map(str::to_string),
	)
}
...
pub enum TransportError {
	/// Error emitted by the transport layer if the connection was lost due to
	/// an I/O error
	ChannelClosed,
	...
	SerdeError(String),
	/// Generic error type for RPC errors that can't be deserialized into
	/// RomeError
	RPCError(String),
}

impl Display for TransportError {
	fn fmt(&self, fmt:&mut Formatter) -> fmt::Result { self.description(fmt) }
}
	...
	fn description(&self, fmt:&mut Formatter<'_>) -> fmt::Result {
		match self {
			TransportError::SerdeError(err) => {
				write!(fmt, "serialization error: {err}")
			},
			TransportError::ChannelClosed => {
				fmt.write_str(
					"a request to the remote workspace failed because the connection was \
					 interrupted",
				)
			},
...
impl From<VcsDiagnostic> for WorkspaceError {
	fn from(value:VcsDiagnostic) -> Self { Self::Vcs(value) }
}
		...
		match value {
			CompileError::ParsePatternError(diagnostic) => {
				Self::SearchError(SearchError::PatternCompilationError(
					PatternCompilationError::from(diagnostic),
				))
			},
			// FIXME: This really needs proper diagnostics
			_ => Self::SearchError(SearchError::QueryError(QueryDiagnostic(format!("{value:?}")))),
		}
	}
}
...
mod test {
	...
	use crate::{
		...
		file_handlers::DocumentFileSource,
		TransportError,
		WorkspaceError,
	};
	...
	#[test]
	fn diagnostic_size() { assert_eq!(std::mem::size_of::<WorkspaceError>(), 96) }

	#[test]
	fn dirty_workspace() {
		snap_diagnostic("dirty_workspace", WorkspaceError::DirtyWorkspace(DirtyWorkspace).into())
	}
	...
	fn file_ignored() {
		snap_diagnostic(
			"file_ignored",
			WorkspaceError::FileIgnored(FileIgnored { path:"example.js".to_string() })
	...
	fn cant_read_directory() {
		snap_diagnostic(
			"cant_read_directory",
			WorkspaceError::CantReadDirectory(CantReadDirectory { path:"example/".to_string() })
	...
	fn cant_read_file() {
		snap_diagnostic(
			"cant_read_file",
			WorkspaceError::CantReadFile(CantReadFile { path:"example.js".to_string() })
				.with_file_path("example.js"),
		)
	}
	...
	fn transport_channel_closed() {
		snap_diagnostic("transport_channel_closed", TransportError::ChannelClosed.into())
	}

	#[test]
	fn transport_timeout() { snap_diagnostic("transport_timeout", TransportError::Timeout.into()) }
