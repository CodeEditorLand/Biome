...
use super::{
	CloseFileParams, CloseProjectParams, FileContent, FileFeaturesResult, FileGuard, GetFileContentParams,
	GetSyntaxTreeParams, OpenFileParams, OpenProjectParams, PullDiagnosticsParams, ScanProjectFolderParams,
...
fn create_server() -> (Box<dyn Workspace>, ProjectKey) {
	...
	let project_key = workspace
		.open_project(OpenProjectParams { path: Default::default(), open_uninitialized: true })
		.unwrap();
...
fn correctly_handle_json_files() {
	...
	.unwrap();
	assert!(well_known_json_with_comments_file_with_trailing_commas.format_file().is_err());
	...
	.unwrap();
	assert!(well_known_json_with_comments_and_trailing_commas_file.format_file().is_ok());
}
...
fn correctly_pulls_lint_diagnostics() {
  ...
  member @deprecated(abc: 123)
	...
	let result = graphql_file.pull_diagnostics(
		RuleCategories::all(),
		10,
		vec![RuleSelector::Rule(RuleGroup::Nursery.as_str(), "useDeprecatedReason")],
		vec![],
	);
	assert!(result.is_ok());
...
fn files_loaded_by_the_scanner_are_only_unloaded_when_the_project_is_unregistered() {
	...
	let project_key = workspace
		.open_project(OpenProjectParams { path: Utf8PathBuf::from("/project").into(), open_uninitialized: true })
		.unwrap();

	workspace
		.scan_project_folder(ScanProjectFolderParams { project_key, path: None, watch: false, force: false })
		.unwrap();
	...
	macro_rules! assert_file_a_content {
		() => {
			assert_eq!(
				workspace
					.get_file_content(GetFileContentParams { project_key, path: BiomePath::new("/project/a.ts") })
					.unwrap(),
	...
	workspace
		.close_file(CloseFileParams { project_key, path: BiomePath::new("/project/a.ts") })
		.unwrap();

	assert_file_a_content!();

	workspace.close_project(CloseProjectParams { project_key }).unwrap();
...
fn too_large_files_are_tracked_but_not_parsed() {
	...
	assert!(
		workspace
			.get_syntax_tree(GetSyntaxTreeParams { project_key, path: BiomePath::new("/project/a.ts") })
...
"#;
	...
	workspace
		.update_settings(UpdateSettingsParams {
			project_key,
			configuration: Configuration {
				plugins: Some(Plugins(vec![PluginConfiguration::Path("./plugin.grit".to_string())])),
				..Default::default()
			},
