...
fn instance_type<'a>(
	...
	match ty {
		...
		InstanceType::Object => {
			...
			let additional_properties = object.additional_properties.as_deref().and_then(|schema| match schema {
			...
			let additional_properties_type = additional_properties.map(|schema| {
				...
				if let Some(key_type) = key_type {
					...
					.with_optional_modifier(make::ts_mapped_type_optional_modifier_clause(make::token(T![?])).build())
		...
		InstanceType::Array => {
			...
			match items {
				SingleOrVec::Single(schema) => {
					...
					AnyTsType::from(make::ts_array_type(ts_type, make::token(T!['[']), make::token(T![']'])))
				},
		...
		InstanceType::Number | InstanceType::Integer => AnyTsType::from(make::ts_number_type(make::token(T![number]))),
	}
}
...
fn value_type(value: &Value) -> AnyTsType {
	match value {
		...
		Value::Bool(false) => AnyTsType::from(make::ts_boolean_literal_type(make::token(T![false]))),
		Value::Number(value) => {
			AnyTsType::from(make::ts_number_literal_type(make::js_number_literal(value.as_f64().unwrap())).build())
		},
		Value::String(value) => AnyTsType::from(make::ts_string_literal_type(make::js_string_literal(value))),
...
fn make_union_type(items: impl IntoIterator<Item = AnyTsType>) -> AnyTsType {
	...
	let separators = (0..result.len().saturating_sub(1)).map(|_| make::token(T![|]));
	AnyTsType::from(make::ts_union_type(make::ts_union_type_variant_list(result, separators)).build())
}
...
fn schema_object_type<'a>(
	...
	let has_defaults = schema.metadata.as_ref().is_some_and(|metadata| metadata.default.is_some());
...
fn schema_type<'a>(
	...
	match schema {
		...
		Schema::Bool(true) => (AnyTsType::from(make::ts_any_type(make::token(T![any]))), true, None),
		// Types defined as `false` in the schema never pass validation,
		// map them to the `never` type
		Schema::Bool(false) => (AnyTsType::from(make::ts_never_type(make::token(T![never]))), false, None),
...
pub fn generate_type<'a>(
	...
	while let Some((name, schema)) = queue.pop_front() {
		...
		let is_interface = schema.object.as_deref().is_some_and(|object| {
			object
				...
				.as_deref()
				.is_none_or(|additional_properties| matches!(additional_properties, Schema::Bool(false)))
		...
		if is_interface {
			...
			for (property_str, schema) in &object.properties {
				...
				let type_annotation = if let Some((container_type, key_type, value_type)) = match property_str.as_str()
				{
					...
					let full_type =
						make::ts_reference_type(make::js_reference_identifier(make::ident(container_type)).into())
							.with_type_arguments(make::ts_type_arguments(
								make::token(T![<]),
								make::ts_type_argument_list(
									[
				...
				let mut builder = make::ts_property_signature_type_member(AnyJsObjectMemberName::from(
					make::js_literal_member_name(property),
				))
	...
	AnyTsType::TsReferenceType(
		make::ts_reference_type(AnyTsName::JsReferenceIdentifier(make::js_reference_identifier(make::ident(
			root_name,
		))))
		.build(),
	)
