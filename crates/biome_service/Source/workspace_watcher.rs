...
use notify::{
	Error as NotifyError, Event as NotifyEvent, EventKind, RecursiveMode, Result as NotifyResult, Watcher,
...
impl WorkspaceWatcher {
	...
	pub fn new() -> Result<(Self, WatcherInstructionChannel), WorkspaceError> {
		...
		let instruction_channel = WatcherInstructionChannel { sender: instruction_tx };
		let watcher = Self { watcher: Box::new(watcher), notify_rx: rx, instruction_rx };
...
impl From<NotifyError> for WorkspaceError {
	fn from(error: NotifyError) -> Self {
		Self::WatchError(WatchError { reason: error.to_string() })
	}
}
