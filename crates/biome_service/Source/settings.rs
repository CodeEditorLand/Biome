...
impl Settings {
	...
	pub fn merge_with_configuration(
		...
		// NOTE: keep this last. Computing the overrides require reading the settings
		// computed by the parent settings.
...
impl From<PartialGraphqlConfiguration> for LanguageSettings<GraphqlLanguage> {
	fn from(graphql:PartialGraphqlConfiguration) -> Self {
		...
		if let Some(formatter) = graphql.formatter {
			// TODO: change RHS to `formatter.enabled` when graphql formatting is enabled by
			// default
		...
		if let Some(linter) = graphql.linter {
			// TODO: change RHS to `linter.enabled` when graphql linting is enabled by
			// default
...
impl OverrideSettingPattern {
	...
	fn apply_overrides_to_json_parser_options(&self, options:&mut JsonParserOptions) {
		// these options are no longer cached because it was causing incorrect override
		// behavior, see #3260
	...
	#[allow(dead_code)]
	// NOTE: Currently not used because the rule options are typed using TypeId and
	// Any, which isn't thread safe. TODO: Find a way to cache this
