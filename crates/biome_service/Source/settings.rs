...
use biome_configuration::{
	analyzer::assists::AssistsConfiguration,
...
use crate::{
	workspace::{DocumentFileSource, ProjectKey, WorkspaceData},
	Matcher,
	WorkspaceError,
};
...
impl WorkspaceSettings {
	pub fn get_current_project_key(&self) -> ProjectKey { self.current_project }
	...
	pub fn get_current_settings(&self) -> Option<&Settings> {
		...
		if let Some(data) = data { Some(&data.settings) } else { None }
	}
	...
	pub fn register_current_project(&mut self, key:ProjectKey) { self.current_project = key; }

	/// Insert a new project using its folder. Use
	/// [WorkspaceSettings::get_current_settings_mut] to retrieve
	...
	pub fn insert_project(&mut self, workspace_path:impl Into<PathBuf>) -> ProjectKey {
		...
		self.data
			.insert(ProjectData { path, settings:Settings::default(), project:None })
	}
	...
	///
	/// If there's a match, and the match **isn't** the current project, it
	/// returns the new key.
	pub fn path_belongs_to_current_workspace(&self, path:&BiomePath) -> Option<ProjectKey> {
		...
		let iter = self.data.iter();
		for (key, path_to_settings) in iter {
			trace!("Workspace path {:?}, file path {:?}", path_to_settings.path, path);
			trace!("Iter key: {:?}", key);
	...
	/// Checks if the current path belongs to a registered project.
	///
	/// If there's a match, and the match **isn't** the current project, the
	/// function will mark the match as the current project.
	pub fn set_current_project(&mut self, new_key:ProjectKey) { self.current_project = new_key; }
}
...
impl Settings {
	...
	pub fn merge_with_configuration(
		...
		// NOTE: keep this last. Computing the overrides require reading the
		// settings computed by the parent settings.
	...
	pub fn formatter(&self) -> &FormatSettings { &self.formatter }
	...
	pub fn linter(&self) -> &LinterSettings { &self.linter }

	/// Retrieves the settings of the organize imports
	pub fn organize_imports(&self) -> &OrganizeImportsSettings { &self.organize_imports }

	/// Retrieves the settings of the organize imports
	pub fn assists(&self) -> &AssistsSettings { &self.assists }

	/// It retrieves the severity based on the `code` of the rule and the
	...
	pub fn get_severity_from_rule_code(
		...
		if let Some(rules) = rules { rules.get_severity_from_code(code) } else { None }
	}
...
impl Default for OrganizeImportsSettings {
	fn default() -> Self {
		Self { enabled:true, ignored_files:Matcher::empty(), included_files:Matcher::empty() }
	}
}
...
impl From<PartialGraphqlConfiguration> for LanguageSettings<GraphqlLanguage> {
	fn from(graphql:PartialGraphqlConfiguration) -> Self {
		...
		if let Some(formatter) = graphql.formatter {
			// TODO: change RHS to `formatter.enabled` when graphql formatting
		...
		if let Some(linter) = graphql.linter {
			// TODO: change RHS to `linter.enabled` when graphql linting is
			// enabled by default
...
pub trait ServiceLanguage: biome_rowan::Language {
	...
	/// Settings related to the environment/runtime in which the language is
	/// used.
	type EnvironmentSettings: Default;

	/// Read the settings type for this language from the [LanguageListSettings]
	/// map
...
impl<'a> WorkspaceSettingsHandle<'a> {
	...
	pub(crate) fn settings(&self) -> Option<&Settings> { self.inner.get_current_settings() }
}

impl<'a> AsRef<WorkspaceSettings> for WorkspaceSettingsHandle<'a> {
	fn as_ref(&self) -> &WorkspaceSettings { &self.inner }
}
...
impl<'a> WorkspaceSettingsHandle<'a> {
	...
	where
		...
		let overrides = settings.map(|s| &s.override_settings);
		let editor_settings =
			settings.map(|s| L::lookup_settings(&s.languages)).map(|result| &result.linter);
		L::resolve_analyzer_options(settings, linter, overrides, editor_settings, path, file_source)
	}
}
...
impl<'a> AsMut<WorkspaceSettings> for WorkspaceSettingsHandleMut<'a> {
	fn as_mut(&mut self) -> &mut WorkspaceSettings { &mut self.inner }
}
...
impl OverrideSettings {
	...
	/// It scans the current override rules and return the formatting options
	/// that of the first override is matched
	...
	/// It scans the current override rules and return the json format that of
	/// the first override is matched
	...
	/// It scans the current override rules and return the parser options that
	/// of the first override is matched
	...
	/// Scans the overrides and checks if there's an override that disable the
	/// formatter for `path`
...
pub struct OverrideSettingPattern {
	...
	// the file source params will affect how tokens are treated during
	// formatting. So we cannot reuse the same format options for all
	// js-family files.
...
impl OverrideSettingPattern {
	...
	fn apply_overrides_to_json_parser_options(&self, options:&mut JsonParserOptions) {
		// these options are no longer cached because it was causing incorrect
		// override behavior, see #3260
	...
	#[allow(dead_code)]
	// NOTE: Currently not used because the rule options are typed using TypeId
	// and Any, which isn't thread safe. TODO: Find a way to cache this
...
fn to_git_ignore(path:PathBuf, matches:&[String]) -> Result<Gitignore, WorkspaceError> {
	...
	for the_match in matches {
		gitignore_builder.add_line(Some(path.clone()), the_match).map_err(|err| {
...
pub fn to_override_settings(
	...
	for mut pattern in overrides.0 {
		...
		let linter = pattern
			.linter
			.map(|linter| OverrideLinterSettings { enabled:linter.enabled, rules:linter.rules })
			.unwrap_or_default();
		let organize_imports = OverrideOrganizeImportsSettings {
			enabled:pattern.organize_imports.and_then(|organize_imports| organize_imports.enabled),
		};
...
fn to_javascript_language_settings(
	...
	language_setting.formatter.indent_width =
		formatter.indent_width.map(Into::into).or(formatter.indent_size.map(Into::into));
	language_setting.formatter.indent_style = formatter.indent_style.map(Into::into);
...
fn to_css_language_settings(
	...
	language_setting.parser.allow_wrong_line_comments =
		parser.allow_wrong_line_comments.or(parent_parser.allow_wrong_line_comments);
