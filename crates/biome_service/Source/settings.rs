...
use biome_configuration::{
	push_to_analyzer_rules, BiomeDiagnostic, FilesConfiguration,
	FormatterConfiguration, JavascriptConfiguration, LinterConfiguration,
	OverrideAssistsConfiguration, OverrideFormatterConfiguration,
	OverrideLinterConfiguration, OverrideOrganizeImportsConfiguration,
	Overrides, PartialConfiguration, PartialCssConfiguration,
	PartialGraphqlConfiguration, PartialJavascriptConfiguration,
	PartialJsonConfiguration,
};
...
use biome_formatter::{
	AttributePosition, BracketSpacing, IndentStyle, IndentWidth, LineEnding,
	LineWidth,
};
...
impl WorkspaceSettings {
	...
	pub fn path_belongs_to_current_workspace(
		&self,
		path: &BiomePath,
	) -> Option<ProjectKey> {
	...
	pub fn merge_with_configuration(
		...
		if let Some(javascript) = configuration.javascript {
			self.languages.javascript =
				JavascriptConfiguration::from(javascript).into();
		}
		// json settings
...
impl From<JavascriptConfiguration> for LanguageSettings<JsLanguage> {
	fn from(javascript: JavascriptConfiguration) -> Self {
		...
		language_setting.formatter.attribute_position =
			formatter.attribute_position;
		language_setting.formatter.indent_width =
			formatter.indent_width.map(Into::into);
		language_setting.formatter.indent_style =
...
impl From<PartialJsonConfiguration> for LanguageSettings<JsonLanguage> {
	fn from(json: PartialJsonConfiguration) -> Self {
		let mut language_setting: LanguageSettings<JsonLanguage> =
			LanguageSettings::default();
...
impl From<PartialCssConfiguration> for LanguageSettings<CssLanguage> {
	fn from(css: PartialCssConfiguration) -> Self {
		let mut language_setting: LanguageSettings<CssLanguage> =
			LanguageSettings::default();

		if let Some(parser) = css.parser {
			language_setting.parser.allow_wrong_line_comments =
				parser.allow_wrong_line_comments;
...
impl From<PartialGraphqlConfiguration> for LanguageSettings<GraphqlLanguage> {
	fn from(graphql: PartialGraphqlConfiguration) -> Self {
		let mut language_setting: LanguageSettings<GraphqlLanguage> =
			LanguageSettings::default();
		...
		if let Some(formatter) = graphql.formatter {
			...
			language_setting.formatter.enabled =
				Some(formatter.enabled.unwrap_or_default());
			language_setting.formatter.indent_width = formatter.indent_width;
			language_setting.formatter.indent_style =
				formatter.indent_style.map(Into::into);
			language_setting.formatter.line_width = formatter.line_width;
			language_setting.formatter.line_ending = formatter.line_ending;
			language_setting.formatter.quote_style = formatter.quote_style;
			language_setting.formatter.bracket_spacing =
				formatter.bracket_spacing;
		}

		if let Some(linter) = graphql.linter {
			// TODO: change RHS to `linter.enabled` when graphql linting is enabled by default
			language_setting.linter.enabled =
				Some(linter.enabled.unwrap_or_default());
		}
...
pub trait ServiceLanguage: biome_rowan::Language {
	...
	fn lookup_settings(
		languages: &LanguageListSettings,
	) -> &LanguageSettings<Self>;
...
pub struct OverrideSettingPattern {
	...
	pub(crate) cached_js_format_options:
		RwLock<FxHashMap<JsFileSource, JsFormatOptions>>,
	...
	pub(crate) cached_graphql_format_options:
		RwLock<Option<GraphqlFormatOptions>>,
...
impl OverrideSettingPattern {
	fn apply_overrides_to_js_format_options(
		&self,
		options: &mut JsFormatOptions,
	) {
		if let Ok(readonly_cache) = self.cached_js_format_options.read() {
			if let Some(cached_options) =
				readonly_cache.get(&options.source_type())
			{
		...
		if let Some(indent_style) =
			js_formatter.indent_style.or(formatter.indent_style)
		{
			options.set_indent_style(indent_style);
		}
		if let Some(indent_width) =
			js_formatter.indent_width.or(formatter.indent_width)
		{
			options.set_indent_width(indent_width);
		}
		if let Some(line_ending) =
			js_formatter.line_ending.or(formatter.line_ending)
		{
			options.set_line_ending(line_ending);
		}
		if let Some(line_width) =
			js_formatter.line_width.or(formatter.line_width)
		{
		...
		if let Some(bracket_spacing) =
			js_formatter.bracket_spacing.or(formatter.bracket_spacing)
		{
		...
		if let Some(attribute_position) =
			js_formatter.attribute_position.or(formatter.attribute_position)
		{
	...
	fn apply_overrides_to_json_format_options(
		&self,
		...
		if let Some(indent_style) =
			json_formatter.indent_style.or(formatter.indent_style)
		{
			options.set_indent_style(indent_style);
		}
		if let Some(indent_width) =
			json_formatter.indent_width.or(formatter.indent_width)
		{
			options.set_indent_width(indent_width)
		}
		if let Some(line_ending) =
			json_formatter.line_ending.or(formatter.line_ending)
		{
			options.set_line_ending(line_ending);
		}
		if let Some(line_width) =
			json_formatter.line_width.or(formatter.line_width)
		{
			options.set_line_width(line_width);
		}
	...
	fn apply_overrides_to_css_format_options(
		&self,
		...
		if let Some(indent_style) =
			css_formatter.indent_style.or(formatter.indent_style)
		{
			options.set_indent_style(indent_style);
		}
		if let Some(indent_width) =
			css_formatter.indent_width.or(formatter.indent_width)
		{
			options.set_indent_width(indent_width)
		}
		if let Some(line_ending) =
			css_formatter.line_ending.or(formatter.line_ending)
		{
			options.set_line_ending(line_ending);
		}
		if let Some(line_width) =
			css_formatter.line_width.or(formatter.line_width)
		{
			options.set_line_width(line_width);
		}
	...
	fn apply_overrides_to_graphql_format_options(
		...
		if let Some(indent_style) =
			graphql_formatter.indent_style.or(formatter.indent_style)
		{
			options.set_indent_style(indent_style);
		}
		if let Some(indent_width) =
			graphql_formatter.indent_width.or(formatter.indent_width)
		{
			options.set_indent_width(indent_width)
		}
		if let Some(line_ending) =
			graphql_formatter.line_ending.or(formatter.line_ending)
		{
			options.set_line_ending(line_ending);
		}
		if let Some(line_width) =
			graphql_formatter.line_width.or(formatter.line_width)
		{
			options.set_line_width(line_width);
		}
		if let Some(bracket_spacing) =
			graphql_formatter.bracket_spacing.or(formatter.bracket_spacing)
		{
			options.set_bracket_spacing(bracket_spacing);
		}
		...
		if let Ok(mut writeonly_cache) =
			self.cached_graphql_format_options.write()
		{
			let options = options.clone();
			let _ = writeonly_cache.insert(options);
		}
	...
	fn apply_overrides_to_html_format_options(
		&self,
		...
		if let Some(indent_style) =
			html_formatter.indent_style.or(formatter.indent_style)
		{
			options.set_indent_style(indent_style);
		}
		if let Some(indent_width) =
			html_formatter.indent_width.or(formatter.indent_width)
		{
			options.set_indent_width(indent_width)
		}
		if let Some(line_ending) =
			html_formatter.line_ending.or(formatter.line_ending)
		{
			options.set_line_ending(line_ending);
		}
		if let Some(line_width) =
			html_formatter.line_width.or(formatter.line_width)
		{
			options.set_line_width(line_width);
		}
	...
	fn apply_overrides_to_js_parser_options(
		&self,
		options: &mut JsParserOptions,
	) {
	...
	fn apply_overrides_to_json_parser_options(
		&self,
	...
	fn apply_overrides_to_css_parser_options(
		&self,
		...
		if let Some(allow_wrong_line_comments) =
			css_parser.allow_wrong_line_comments
		{
...
fn to_git_ignore(
	...
	for the_match in matches {
		gitignore_builder.add_line(Some(path.clone()), the_match).map_err(
			|err| {
...
pub fn to_override_settings(
	...
	for mut pattern in overrides.0 {
		...
		languages.javascript = to_javascript_language_settings(
			javascript,
			&current_settings.languages.javascript,
		);

		languages.json =
			to_json_language_settings(json, &current_settings.languages.json);
		languages.css =
			to_css_language_settings(css, &current_settings.languages.css);
		languages.graphql = to_graphql_language_settings(
			graphql,
			&current_settings.languages.graphql,
		);
...
fn to_javascript_language_settings(
	...
	language_setting.environment.jsx_runtime =
		conf.jsx_runtime.unwrap_or(parent_settings.environment.jsx_runtime);
...
fn to_json_language_settings(
	...
	let parser = conf.parser.take().unwrap_or_default();
	let parent_parser = &parent_settings.parser;
	language_setting.parser.allow_comments =
		parser.allow_comments.or(parent_parser.allow_comments);

	language_setting.parser.allow_trailing_commas =
		parser.allow_trailing_commas.or(parent_parser.allow_trailing_commas);
...
fn to_css_language_settings(
	...
	language_setting.parser.css_modules =
		parser.css_modules.or(parent_parser.css_modules);
...
impl TryFrom<OverrideAssistsConfiguration> for AssistsSettings {
	...
	fn try_from(
		conf: OverrideAssistsConfiguration,
	) -> Result<Self, Self::Error> {
		Ok(Self {
			enabled: conf.enabled.unwrap_or_default(),
			actions: conf.rules,
