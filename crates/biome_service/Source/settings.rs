...
use biome_configuration::{
	BiomeDiagnostic, Configuration, CssConfiguration, FilesConfiguration, FilesIgnoreUnknownEnabled,
	FormatterConfiguration, GraphqlConfiguration, GritConfiguration, JsConfiguration, JsonConfiguration,
	LinterConfiguration, OverrideAssistConfiguration, OverrideFormatterConfiguration, OverrideGlobs,
...
use biome_formatter::{
	AttributePosition, BracketSameLine, BracketSpacing, Expand, IndentStyle, IndentWidth, LineEnding, LineWidth,
};
...
impl Settings {
	...
	pub fn merge_with_configuration(
		...
		if let Some(overrides) = configuration.overrides {
			self.override_settings = to_override_settings(working_directory.clone(), overrides, self)?;
		}
...
impl VcsSettings {
	...
	pub fn store_ignore_patterns(&mut self, path: &Utf8Path, patterns: &[&str]) -> Result<(), WorkspaceError> {
		match self.client_kind {
			Some(VcsClientKind::Git) => {
				...
				if let Some(ignore_matches) = self.ignore_matches.as_mut() {
					...
					self.ignore_matches = Some(VcsIgnoredPatterns::Git { root: git_ignore, nested: vec![] });
				}
			},
			None => {},
		};
...
impl WorkspaceSettingsHandle {
	...
	pub fn format_options<L>(&self, path: &BiomePath, file_source: &DocumentFileSource) -> L::FormatOptions
	where
		L: ServiceLanguage,
	{
	...
	{
		let settings = self.settings();
		let editor_settings = settings.map(|s| L::lookup_settings(&s.languages)).map(|result| &result.linter);

		let environment = L::resolve_environment(settings);
		L::resolve_analyzer_options(settings, editor_settings, environment, path, file_source, suppression_reason)
	}
...
impl OverrideSettings {
	...
	pub fn override_js_format_options(&self, path: &Utf8Path, mut options: JsFormatOptions) -> JsFormatOptions {
	...
	pub fn to_override_js_parser_options(&self, path: &Utf8Path, mut options: JsParserOptions) -> JsParserOptions {
		for pattern in self.patterns.iter() {
			if pattern.is_file_included(path) {
	...
	pub fn to_override_css_parser_options(&self, path: &Utf8Path, mut options: CssParserOptions) -> CssParserOptions {
		for pattern in self.patterns.iter() {
			if pattern.is_file_included(path) {
	...
	pub fn to_override_css_format_options(&self, path: &Utf8Path, mut options: CssFormatOptions) -> CssFormatOptions {
		for pattern in self.patterns.iter() {
			if pattern.is_file_included(path) {
	...
	pub fn to_override_json_parse_options(&self, path: &Utf8Path, mut options: JsonParserOptions) -> JsonParserOptions {
		for pattern in self.patterns.iter() {
			if pattern.is_file_included(path) {
				pattern.apply_overrides_to_json_parser_options(&mut options);
			}
	...
	pub fn override_analyzer_rules(&self, path: &Utf8Path, mut analyzer_rules: AnalyzerRules) -> AnalyzerRules {
		for pattern in self.patterns.iter() {
			if pattern.is_file_included(path) {
				if let Some(rules) = pattern.linter.rules.as_ref() {
					push_to_analyzer_rules(rules, biome_js_analyze::METADATA.deref(), &mut analyzer_rules);
					push_to_analyzer_rules(rules, biome_json_analyze::METADATA.deref(), &mut analyzer_rules);
					push_to_analyzer_rules(rules, biome_css_analyze::METADATA.deref(), &mut analyzer_rules);
					push_to_analyzer_rules(rules, biome_graphql_analyze::METADATA.deref(), &mut analyzer_rules);
				}

				if let Some(actions) = pattern.assist.actions.as_ref() {
					push_to_analyzer_assist(actions, biome_js_analyze::METADATA.deref(), &mut analyzer_rules);
					push_to_analyzer_assist(actions, biome_json_analyze::METADATA.deref(), &mut analyzer_rules);
					push_to_analyzer_assist(actions, biome_css_analyze::METADATA.deref(), &mut analyzer_rules);
					push_to_analyzer_assist(actions, biome_graphql_analyze::METADATA.deref(), &mut analyzer_rules);
				}
			}
		}
		analyzer_rules
...
impl OverrideSettingPattern {
	...
	fn apply_overrides_to_js_format_options(&self, options: &mut JsFormatOptions) {
		...
		if let Some(attribute_position) = js_formatter.attribute_position.or(formatter.attribute_position) {
	...
	fn apply_overrides_to_graphql_format_options(&self, options: &mut GraphqlFormatOptions) {
		...
		if let Some(bracket_spacing) = graphql_formatter.bracket_spacing.or(formatter.bracket_spacing) {
			options.set_bracket_spacing(bracket_spacing);
		}
	...
	fn apply_overrides_to_js_parser_options(&self, options: &mut JsParserOptions) {
		...
		options.parse_class_parameter_decorators =
			js_parser.parse_class_parameter_decorators.unwrap_or_default().into();
	}
	...
	for mut pattern in overrides.0 {
		let formatter = pattern
			...
			.map(|formatter| OverrideFormatSettings {
				...
				format_with_errors: formatter.format_with_errors.or(current_settings.formatter.format_with_errors),
		...
		let assist = pattern
			.assist
			.map(|assist| OverrideAssistSettings { enabled: assist.enabled, actions: assist.actions })
			.unwrap_or_default();
		...
		languages.javascript = to_javascript_language_settings(javascript, &current_settings.languages.javascript);
		...
		languages.graphql = to_graphql_language_settings(graphql, &current_settings.languages.graphql);
...
fn to_javascript_language_settings(
	...
	language_setting.environment.jsx_runtime = conf.jsx_runtime.or(parent_settings.environment.jsx_runtime);
...
fn to_css_language_settings(
	...
	language_setting.parser.css_modules_enabled = parser.css_modules.or(parent_parser.css_modules_enabled);
...
impl TryFrom<OverrideAssistConfiguration> for AssistSettings {
	...
	fn try_from(conf: OverrideAssistConfiguration) -> Result<Self, Self::Error> {
		Ok(Self { enabled: conf.enabled, actions: conf.actions, includes: Default::default() })
	}
}
