...
impl WorkspaceSettings {
	...
	pub fn get_current_project_data_mut(&mut self) -> &mut ProjectData {
		self.data.get_mut(self.current_project).expect("You must have at least one workspace.")
	}
	...
	pub fn insert_project(&mut self, workspace_path: impl Into<PathBuf>) -> ProjectKey {
		...
		self.data.insert(ProjectData { path, settings: Settings::default(), project: None })
	}
...
impl<'a> WorkspaceSettingsHandle<'a> {
		...
		L: ServiceLanguage,
	{
		...
		let editor_settings =
			settings.map(|s| L::lookup_settings(&s.languages)).map(|result| &result.formatter);
