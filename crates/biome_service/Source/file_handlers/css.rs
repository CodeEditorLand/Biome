...
impl ServiceLanguage for CssLanguage {
	...
	fn lookup_settings(language:&LanguageListSettings) -> &LanguageSettings<Self> { &language.css }
	...
	fn resolve_format_options(
		...
		let options =
			CssFormatOptions::new(document_file_source.to_css_file_source().unwrap_or_default())
	...
	fn resolve_analyzer_options(
		...
		let preferred_quote = global
			.and_then(|global| {
				global.languages.css.formatter.quote_style.map(|quote_style:QuoteStyle| {
		...
		let configuration = AnalyzerConfiguration {
			rules:global.map(|g| to_analyzer_rules(g, file_path.as_path())).unwrap_or_default(),
			globals:Vec::new(),
			preferred_quote,
			jsx_runtime:None,
		};

		AnalyzerOptions { configuration, file_path:file_path.to_path_buf() }
	}
}
...
fn parse(
	...
	let mut options = CssParserOptions {
		allow_wrong_line_comments:settings
			...
			.unwrap_or_default(),
		css_modules:settings.and_then(|s| s.languages.css.parser.css_modules).unwrap_or_default(),
		grit_metavariables:false,
	};
	if let Some(settings) = settings {
		options = settings.override_settings.to_override_css_parser_options(biome_path, options);
	}
		...
		move || {
			...
			let (_, analyze_diagnostics) = analyze(&tree, filter, &analyzer_options, |signal| {
				if let Some(mut diagnostic) = signal.diagnostic() {
					// Do not report unused suppression comment diagnostics if this is a syntax-only
					// analyzer pass
					...
					// The configuration allows to change the severity of the diagnostics emitted by
					// rules.
...
fn organize_imports(parse:AnyParse) -> Result<OrganizeImportsResult, WorkspaceError> {
	Ok(OrganizeImportsResult { code:parse.syntax::<CssLanguage>().to_string() })
}

#[tracing::instrument(level = "debug", skip(params))]
pub(crate) fn code_actions(params:CodeActionsParams) -> PullActionsResult {
	let CodeActionsParams { parse, range, workspace, path, manifest: _, language, only, skip } =
		params;
...
pub(crate) fn fix_all(params:FixAllParams) -> Result<FixFileResult, WorkspaceError> {
	...
	loop {
		let (action, _) = analyze(&tree, filter, &analyzer_options, |signal| {
			...
			for action in signal.actions() {
				// suppression actions should not be part of the fixes (safe or
				// suggested)
				if action.is_suppression() {
					continue;
				}
