...
impl SvelteFileHandler {
	/// It extracts the JavaScript/TypeScript code contained in the script block
	/// of a Svelte file
	///
	...
	/// It takes the original content of a Svelte file, and new output of an
	/// Svelte file. The output is only the content contained inside the Svelte
	/// `<script>` tag. The function replaces `output` inside that `<script>`.
	pub fn output(input:&str, output:&str) -> String {
		if let Some(script) = Self::matches_script(input) {
			format!("{}{}{}", &input[..script.start()], output, &input[script.end()..])
		} else {
			input.to_string()
		}
	}
	...
	fn matches_script(input:&str) -> Option<Match> {
		SVELTE_FENCE.captures(input).and_then(|captures| captures.name("script"))
	}
...
fn parse(
	...
	ParseResult { any_parse:parse.into(), language:Some(file_source.into()) }
}
