...
use biome_graphql_formatter::{context::GraphqlFormatOptions, format_node};
...
impl Default for GraphqlLinterSettings {
	fn default() -> Self { Self { enabled:Some(false) } }
}
...
impl ServiceLanguage for GraphqlLanguage {
	...
	fn resolve_format_options(
		...
		let options = GraphqlFormatOptions::new(
			document_file_source.to_graphql_file_source().unwrap_or_default(),
		)
	...
	loop {
		...
		match action {
			...
			None => {
				...
				//     format_node(
				//         workspace.
				// format_options::<GraphqlLanguage>(biome_path,
				// &document_file_source),         tree.syntax(),
				//     )?
				//         .print()?
				//         .into_code()
				// } else {
