...
use biome_js_analyze::{
	...
	utils::rename::{RenameError, RenameSymbolExtensions},
	ControlFlowGraph,
};
...
use crate::{
	...
	file_handlers::{is_diagnostic_error, FixAllParams},
	settings::{
		FormatSettings,
		LanguageListSettings,
		LanguageSettings,
		LinterSettings,
		OverrideSettings,
		ServiceLanguage,
...
impl From<JsxRuntime> for JsEnvironmentSettings {
	fn from(jsx_runtime:JsxRuntime) -> Self { Self { jsx_runtime } }
}
...
impl ServiceLanguage for JsLanguage {
	...
	fn resolve_format_options(
		...
		.with_quote_properties(language.and_then(|l| l.quote_properties).unwrap_or_default())
		...
		.with_arrow_parentheses(language.and_then(|l| l.arrow_parentheses).unwrap_or_default())
		...
		.with_bracket_same_line(language.and_then(|l| l.bracket_same_line).unwrap_or_default())
	...
	fn resolve_analyzer_options(
		...
		let preferred_quote = global
			.and_then(|global| {
				global.languages.javascript.formatter.quote_style.map(|quote_style:QuoteStyle| {
		...
		if let (Some(overrides), Some(global)) = (overrides, global) {
			jsx_runtime = Some(
				...
				{
					// In the future, we may wish to map an `Auto` variant to a
					// concrete analyzer value for easy access by the
					// analyzer.
		...
		let configuration = AnalyzerConfiguration {
			rules:global.map(|g| to_analyzer_rules(g, path.as_path())).unwrap_or_default(),
			globals,
			preferred_quote,
			jsx_runtime,
		};

		AnalyzerOptions { configuration, file_path:path.to_path_buf() }
	}
}
...
fn parse(
	...
	let mut options = JsParserOptions {
		...
		parse_class_parameter_decorators:settings.is_some_and(|settings| {
			settings.languages.javascript.parser.parse_class_parameter_decorators
		}),
	};
	if let Some(settings) = settings {
		options = settings.override_settings.to_override_js_parser_options(biome_path, options);
	}
	...
	ParseResult { any_parse:parse.into(), language:None }
}
...
fn debug_syntax_tree(_rome_path:&BiomePath, parse:AnyParse) -> GetSyntaxTreeResult {
	...
	GetSyntaxTreeResult { cst:format!("{syntax:#?}"), ast:format!("{tree:#?}") }
}
	...
	debug_span!("Linting JavaScript file", path =? params.path, language =? params.language)
		.in_scope(move || {
			let Some(file_source) = params
				...
				return LintResults { errors:0, diagnostics:Vec::new(), skipped_diagnostics:0 };
			};
			let tree = params.parse.tree();
			let analyzer_options =
				&params.workspace.analyzer_options::<JsLanguage>(params.path, &params.language);
			...
			let mut errors =
				diagnostics.iter().filter(|diag| diag.severity() <= Severity::Error).count();

			info!("Analyze file {}", params.path.display());
			let (_, analyze_diagnostics) =
				analyze(&tree, filter, analyzer_options, file_source, params.manifest, |signal| {
					if let Some(mut diagnostic) = signal.diagnostic() {
						...
						// The configuration allows to change the severity of the diagnostics
						// emitted by rules.
			...
			LintResults { diagnostics, errors, skipped_diagnostics }
		})
}

#[tracing::instrument(level = "debug", skip(params))]
pub(crate) fn code_actions(params:CodeActionsParams) -> PullActionsResult {
	let CodeActionsParams { parse, range, workspace, path, manifest, language, only, skip } =
		params;
	debug_span!("Code actions JavaScript", range =? range, path =? path).in_scope(move || {
		...
		trace_span!("Parsed file", tree =? tree).in_scope(move || {
			...
			let Some(source_type) = language.to_js_file_source() else {
				...
				return PullActionsResult { actions:Vec::new() };
			};

			trace!("Javascript runs the analyzer");
			analyze(&tree, filter, &analyzer_options, source_type, manifest, |signal| {
	...
	loop {
		let (action, _) = analyze(
			...
			|signal| {
				...
				for action in signal.actions() {
					// suppression actions should not be part of the fixes (safe
					// or suggested)
					if action.is_suppression() {
						continue;
					}
...
pub(crate) fn format_on_type(
	...
	let token = match tree.token_at_offset(offset) {
		...
		// The cursor should be right after the closing character that was just
		// typed, select the previous token as the correct one
...
fn rename(
		...
		.and_then(|token| token.parent())
	{
		...
		Err(WorkspaceError::RenameError(RenameError::CannotFindDeclaration(new_name)))
	}
}
...
pub(crate) fn organize_imports(parse:AnyParse) -> Result<OrganizeImportsResult, WorkspaceError> {
	...
	if let Some(action) = action {
		tree = match AnyJsRoot::cast(action.mutation.commit()) {
			Some(tree) => tree,
			None => {
				return Err(WorkspaceError::RuleError(RuleError::ReplacedRootWithNonRootError {
					rule_name:action
						.rule_name
						.map(|(group, rule)| (Cow::Borrowed(group), Cow::Borrowed(rule))),
				}));
		...
		Ok(OrganizeImportsResult { code:tree.syntax().to_string() })
	} else {
		Ok(OrganizeImportsResult { code:tree.syntax().to_string() })
	}
}
