...
impl VueFileHandler {
	...
	/// It takes the original content of a Vue file, and new output of an Vue
	/// file. The output is only the content contained inside the
	/// Vue `<script>` tag. The function replaces `output` inside that
	/// `<script>`.
	...
	fn matches_script(input:&str) -> Option<Match> {
		VUE_FENCE.captures(input).and_then(|captures| captures.name("script"))
	}
