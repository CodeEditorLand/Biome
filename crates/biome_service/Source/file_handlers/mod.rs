...
pub(crate) fn is_diagnostic_error(
	...
	let severity =
		diagnostic.category().filter(|category| category.name().starts_with("lint/")).map_or_else(
			|| diagnostic.severity(),
			|category| {
				rules
...
pub(crate) fn search(
	...
	let (query_result, _logs) =
		query.execute(GritTargetFile { path: path.to_path_buf(), parse }).map_err(|err| {
...
fn test_svelte_script_lang() {
	...
	assert!(parse_lang_from_script_opening_tag(SVELTE_JS_SCRIPT_OPENING_TAG).0.is_javascript());
	assert!(parse_lang_from_script_opening_tag(SVELTE_TS_SCRIPT_OPENING_TAG).0.is_typescript());
	assert!(parse_lang_from_script_opening_tag(SVELTE_CONTEXT_MODULE_JS_SCRIPT_OPENING_TAG)
		.0
		.is_javascript());
	assert!(parse_lang_from_script_opening_tag(SVELTE_CONTEXT_MODULE_TS_SCRIPT_OPENING_TAG)
		.0
		.is_typescript());
}
...
impl<'a> RegistryVisitor<JsLanguage> for SyntaxVisitor<'a> {
	...
	{
		self.enabled_rules.push(RuleFilter::Rule(<R::Group as RuleGroup>::NAME, R::METADATA.name))
	}
}
...
fn test_vue_script_lang() {
	...
	assert!(parse_lang_from_script_opening_tag(VUE_SETUP_JS_SCRIPT_OPENING_TAG).0.is_javascript());
	assert!(parse_lang_from_script_opening_tag(VUE_SETUP_TS_SCRIPT_OPENING_TAG).0.is_typescript());
}
