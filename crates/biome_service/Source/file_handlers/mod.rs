...
use biome_configuration::{analyzer::RuleSelector, Rules};
...
use self::{
	css::CssFileHandler,
	javascript::JsFileHandler,
	json::JsonFileHandler,
	unknown::UnknownFileHandler,
};
pub use crate::file_handlers::{
	astro::{AstroFileHandler, ASTRO_FENCE},
	svelte::{SvelteFileHandler, SVELTE_FENCE},
	vue::{VueFileHandler, VUE_FENCE},
};
use crate::{
	diagnostics::{QueryDiagnostic, SearchError},
	file_handlers::graphql::GraphqlFileHandler,
	settings::{Settings, WorkspaceSettingsHandle},
	workspace::{
		FixFileMode,
		FixFileResult,
		GetSyntaxTreeResult,
...
impl From<JsFileSource> for DocumentFileSource {
	fn from(value:JsFileSource) -> Self { Self::Js(value) }
}

impl From<JsonFileSource> for DocumentFileSource {
	fn from(value:JsonFileSource) -> Self { Self::Json(value) }
}

impl From<CssFileSource> for DocumentFileSource {
	fn from(value:CssFileSource) -> Self { Self::Css(value) }
}

impl From<GraphqlFileSource> for DocumentFileSource {
	fn from(value:GraphqlFileSource) -> Self { Self::Graphql(value) }
}

impl From<HtmlFileSource> for DocumentFileSource {
	fn from(value:HtmlFileSource) -> Self { Self::Html(value) }
}

impl From<GritFileSource> for DocumentFileSource {
	fn from(value:GritFileSource) -> Self { Self::Grit(value) }
}

impl From<&Path> for DocumentFileSource {
	fn from(path:&Path) -> Self { Self::from_path(path) }
}
...
impl DocumentFileSource {
	...
	/// Returns the document file source corresponding to this file name from
	/// well-known files
	...
	fn try_from_path(path:&Path) -> Result<Self, FileSourceError> {
		...
		// TODO: This should be extracted to a dedicated function, maybe in
		// biome_fs because the same logic is also used in
		// JsFileSource::try_from and we may support more and more extensions
	...
	/// Returns the document file source if it's not unknown, otherwise returns
	/// `other`.
	///
	/// # Examples
	///
	...
	pub fn or(self, other:DocumentFileSource) -> DocumentFileSource {
		if self != DocumentFileSource::Unknown { self } else { other }
	}

	pub const fn is_javascript_like(&self) -> bool { matches!(self, DocumentFileSource::Js(_)) }

	pub const fn is_json_like(&self) -> bool { matches!(self, DocumentFileSource::Json(_)) }

	pub const fn is_css_like(&self) -> bool { matches!(self, DocumentFileSource::Css(_)) }
...
/// Checks whether a diagnostic coming from the analyzer is an
...
/// Parse the "lang" attribute from the opening tag of the "\<script\>" block in
/// Svelte or Vue files. This function will return the language based on the
/// existence or the value of the "lang" attribute. We use the JSX parser at the
/// moment to parse the opening tag. So the opening tag should be first
...
pub(crate) fn parse_lang_from_script_opening_tag(
	...
	parse(script_opening_tag, JsFileSource::jsx(), JsParserOptions::default())
		.try_tree()
		.and_then(|tree| {
			tree.as_js_module()?.items().into_iter().find_map(|item| {
				let expression =
					item.as_any_js_statement()?.as_js_expression_statement()?.expression().ok()?;
				...
				match attribute_inner_string.text() {
					"ts" => {
						Some((
							Language::TypeScript { definition_file:false },
							LanguageVariant::Standard,
						))
					},
					"tsx" => {
						Some((Language::TypeScript { definition_file:false }, LanguageVariant::Jsx))
					},
		...
		.map_or((Language::JavaScript, LanguageVariant::Standard), |lang| lang)
}
...
pub(crate) fn search(
	...
	let (query_result, _logs) = query
		.execute(GritTargetFile { path:path.to_path_buf(), parse })
		.map_err(|err| {
...
/// Type meant to register all the syntax rules for each language supported by
/// Biome
///
/// When a new language is introduced, it must be implemented it. Syntax rules
/// aren't negotiable via configuration, so it's safe to pull all of them.
...
impl<'a> RegistryVisitor<JsLanguage> for SyntaxVisitor<'a> {
	...
	where
		...
		self.enabled_rules
			.push(RuleFilter::Rule(<R::Group as RuleGroup>::NAME, R::METADATA.name))
	}
}
...
/// Type meant to register all the lint rules for each language supported by
/// Biome
#[derive(Debug)]
...
impl<'a, 'b> LintVisitor<'a, 'b> {
	pub(crate) fn new(
		...
		Self { enabled_rules:vec![], disabled_rules:vec![], only, skip, settings, path }
	}
...
impl<'a, 'b> AssistsVisitor<'a, 'b> {
	...
	where
		...
		let organize_imports_enabled =
			self.settings.is_some_and(|settings| settings.organize_imports.enabled);
...
impl<'a, 'b> AnalyzerVisitorBuilder<'a, 'b> {
	pub(crate) fn new(settings:Option<&'b Settings>) -> Self {
		Self { settings, syntax:None, lint:None, assists:None }
	}
...
fn test_vue_script_lang() {
	...
	assert!(parse_lang_from_script_opening_tag(VUE_JS_SCRIPT_OPENING_TAG).0.is_javascript());
	assert!(parse_lang_from_script_opening_tag(VUE_JS_SCRIPT_OPENING_TAG).1.is_standard());
	assert!(parse_lang_from_script_opening_tag(VUE_TS_SCRIPT_OPENING_TAG).0.is_typescript());
	assert!(parse_lang_from_script_opening_tag(VUE_TS_SCRIPT_OPENING_TAG).1.is_standard());
	assert!(parse_lang_from_script_opening_tag(VUE_JSX_SCRIPT_OPENING_TAG).0.is_javascript());
	assert!(parse_lang_from_script_opening_tag(VUE_JSX_SCRIPT_OPENING_TAG).1.is_jsx());
	assert!(parse_lang_from_script_opening_tag(VUE_TSX_SCRIPT_OPENING_TAG).0.is_typescript());
	assert!(
