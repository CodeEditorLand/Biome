...
use crate::{
	...
	workspace::{DocumentFileSource, FixFileResult, OrganizeImportsResult, PullActionsResult},
	WorkspaceError,
};
...
pub static ASTRO_FENCE:LazyLock<Regex> =
	LazyLock::new(|| RegexBuilder::new(r#"^---\s*$"#).multi_line(true).build().unwrap());

impl AstroFileHandler {
	/// It extracts the JavaScript code contained in the frontmatter of an Astro
	/// file
	///
	...
	fn matches(input:&str) -> Matches { ASTRO_FENCE.find_iter(input) }

	/// It takes the original content of an Astro file, and new output of an
	/// Astro file. The output is only the content contained inside the
	...
	pub fn output(input:&str, output:&str) -> String {
		...
		if let (Some(start), Some(end)) = (matches.next(), matches.next()) {
			format!("{}{}{}", &input[..start.end() + 1], output.trim_start(), &input[end.start()..])
		} else {
			input.to_string()
		}
	}
...
fn parse(
	...
	let parse = parse_js_with_cache(
		frontmatter,
		file_source.to_js_file_source().unwrap_or(JsFileSource::ts()),
		JsParserOptions::default(),
		cache,
	);

	ParseResult { any_parse:parse.into(), language:Some(JsFileSource::astro().into()) }
}
...
pub(crate) fn lint(params:LintParams) -> LintResults { javascript::lint(params) }
