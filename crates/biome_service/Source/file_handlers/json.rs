...
use biome_json_formatter::{
	context::{JsonFormatOptions, TrailingCommas},
	format_node,
};
...
impl ServiceLanguage for JsonLanguage {
	...
	fn lookup_settings(language:&LanguageListSettings) -> &LanguageSettings<Self> { &language.json }
	...
	fn resolve_format_options(
		...
		let trailing_commas =
			if matches!(path.file_name().map(OsStr::as_encoded_bytes), Some(b"biome.json")) {
				TrailingCommas::None
			} else {
...
fn parse(
	...
	ParseResult { any_parse:parse.into(), language:Some(file_source) }
}
	...
	tracing::debug_span!("Linting JSON file", path =? params.path, language =? params.language)
		.in_scope(move || {
			let Some(file_source) = params
				...
				return LintResults { errors:0, diagnostics:vec![], skipped_diagnostics:0 };
			};
			let root:JsonRoot = params.parse.tree();

			let analyzer_options =
				&params.workspace.analyzer_options::<JsonLanguage>(params.path, &params.language);
			...
			// if we're parsing the `biome.json` file, we deserialize it, so we can emit
			// diagnostics for malformed configuration
...
fn code_actions(params:CodeActionsParams) -> PullActionsResult {
	let CodeActionsParams { parse, range, workspace, path, manifest: _, language, skip, only } =
		params;
...
fn fix_all(params:FixAllParams) -> Result<FixFileResult, WorkspaceError> {
	...
	let filter = AnalysisFilter {
		categories:RuleCategoriesBuilder::default().with_syntax().with_lint().with_action().build(),
		enabled_rules:Some(enabled_rules.as_slice()),
		disabled_rules:&disabled_rules,
		range:None,
	};
...
fn organize_imports(parse:AnyParse) -> Result<OrganizeImportsResult, WorkspaceError> {
	Ok(OrganizeImportsResult { code:parse.syntax::<JsonLanguage>().to_string() })
}
