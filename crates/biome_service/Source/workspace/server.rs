	...
	fn is_ignored_by_top_level_config(&self, path:&Path) -> bool {
		...
		!is_included
			...
			|| settings.files.git_ignore.as_ref().is_some_and(|ignore| {
				// `matched_path_or_any_parents` panics if `source` is not under the gitignore
				// root. This checks excludes absolute paths that are not a prefix of the
				// base root.
...
impl Workspace for WorkspaceServer {
	fn file_features(
		...
		// If the file is not ignored by at least one feature, then check that the file
		// is not protected.
		//
