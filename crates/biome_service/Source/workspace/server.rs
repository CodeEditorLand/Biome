...
use biome_diagnostics::{
	serde::Diagnostic as SerdeDiagnostic,
	Diagnostic,
	DiagnosticExt,
	Severity,
};
...
use super::{
	...
	FormatOnTypeParams,
	FormatRangeParams,
	GetControlFlowGraphParams,
	GetFormatterIRParams,
	GetSyntaxTreeParams,
	GetSyntaxTreeResult,
	OpenFileParams,
	ParsePatternParams,
...
use crate::{
	diagnostics::{InvalidPattern, SearchError},
	file_handlers::{
		Capabilities,
		CodeActionsParams,
		DocumentFileSource,
...
pub(super) struct WorkspaceServer {
	...
	/// Stores the document (text content + version number) associated with a
	/// URL
	documents:DashMap<BiomePath, Document>,
	/// Stores the result of the parser (syntax tree + diagnostics) for a given
	/// URL
...
pub(crate) struct Document {
	...
	/// Use `WorkspaceServer#file_sources` to retrieve the file source that
...
impl WorkspaceServer {
	...
	fn workspace(&self) -> WorkspaceSettingsHandle { WorkspaceSettingsHandle::new(&self.settings) }
	...
	/// Return an error factory function for unsupported features at a given
	/// path
	fn build_capability_error<'a>(
		...
		move || {
			...
			WorkspaceError::source_file_not_supported(
				...
				path.extension().and_then(OsStr::to_str).map(|s| s.to_string()),
			)
		}
	}

	/// Returns the current project. The information of this project depend on
	/// path set by [WorkspaceServer::set_current_project]
	///
	/// ## Errors
	///
	...
	/// Updates the manifest for the current project. Given the manifest path,
	/// the function will try to parse the manifest and update the current
	/// project.
	...
	fn get_parse(&self, biome_path:BiomePath) -> Result<AnyParse, WorkspaceError> {
		match self.syntax.entry(biome_path) {
			...
			Entry::Vacant(entry) => {
				...
				let mut document =
					self.documents.get_mut(biome_path).ok_or_else(WorkspaceError::not_found)?;
	...
	/// Check whether a file is ignored in the top-level config
	/// `files.ignore`/`files.include` or in the feature `ignore`/`include`
	...
	fn is_ignored_by_top_level_config(&self, path:&Path) -> bool {
		...
		!is_included
			...
			|| settings.files.git_ignore.as_ref().is_some_and(|ignore| {
				// `matched_path_or_any_parents` panics if `source` is not under
				// the gitignore root. This checks excludes absolute paths
				// that are not a prefix of the base root.
				if !path.has_root() || path.starts_with(ignore.path()) {
					// Because Biome passes a list of paths,
					// we use `matched_path_or_any_parents` instead of
					// `matched`.
	...
	fn is_ignored_by_feature_config(&self, path:&Path, feature:FeatureKind) -> bool {
		...
		let (feature_included_files, feature_ignored_files) = match feature {
			...
			FeatureKind::OrganizeImports => {
				...
				(&organize_imports.included_files, &organize_imports.ignored_files)
			},
			FeatureKind::Assists => {
			...
			FeatureKind::Search => return false, // There is no
...
impl Workspace for WorkspaceServer {
	...
	fn update_settings(&self, params:UpdateSettingsParams) -> Result<(), WorkspaceError> {
		let mut workspace = self.workspaces_mut();
		workspace.as_mut().get_current_settings_mut().merge_with_configuration(
	...
	fn get_file_content(&self, params:GetFileContentParams) -> Result<String, WorkspaceError> {
		let document = self.documents.get(&params.path).ok_or(WorkspaceError::not_found())?;
	...
	fn change_file(&self, params:ChangeFileParams) -> Result<(), WorkspaceError> {
		{
			let mut document =
				self.documents.get_mut(&params.path).ok_or_else(WorkspaceError::not_found)?;
	...
	fn close_file(&self, params:CloseFileParams) -> Result<(), WorkspaceError> {
		self.documents.remove(&params.path).ok_or_else(WorkspaceError::not_found)?;
		...
		let (diagnostics, errors, skipped_diagnostics) =
			if let Some(lint) = self.get_file_capabilities(&params.path).analyzer.lint {
				info_span!("Pulling diagnostics", categories =? params.categories).in_scope(|| {
					...
					(results.diagnostics, results.errors, results.skipped_diagnostics)
				})
			} else {
	...
	fn format_range(&self, params:FormatRangeParams) -> Result<Printed, WorkspaceError> {
		...
		let document_file_source = self.get_file_source(&params.path);
		format_range(&params.path, &document_file_source, parse, workspace, params.range)
	}
	...
	fn format_on_type(&self, params:FormatOnTypeParams) -> Result<Printed, WorkspaceError> {
		...
		format_on_type(&params.path, &document_file_source, parse, workspace, params.offset)
	}
	...
	fn search_pattern(&self, params:SearchPatternParams) -> Result<SearchResults, WorkspaceError> {
		let Some(query) = self.patterns.get(&params.pattern) else {
			return Err(WorkspaceError::SearchError(SearchError::InvalidPattern(InvalidPattern)));
		};
		...
		let document_file_source = self.get_file_source(&params.path);
		let matches = search(&params.path, &document_file_source, parse, &query, workspace)?;

		Ok(SearchResults { file:params.path, matches })
	}
