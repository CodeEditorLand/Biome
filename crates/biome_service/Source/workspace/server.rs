...
impl Workspace for WorkspaceServer {
	...
	fn open_file(&self, params: OpenFileParams) -> Result<(), WorkspaceError> {
		let mut source =
			params.document_file_source.unwrap_or(DocumentFileSource::from_path(&params.path));
	...
	fn pull_diagnostics(
			...
			self.get_file_capabilities(&params.path).analyzer.lint
		{
			...
			let errors =
				parse_diagnostics.iter().filter(|diag| diag.severity() <= Severity::Error).count();
	...
	fn format_file(&self, params: FormatFileParams) -> Result<Printed, WorkspaceError> {
		let capabilities = self.get_file_capabilities(&params.path);
		let format =
			capabilities.formatter.format.ok_or_else(self.build_capability_error(&params.path))?;
		let workspace = self.workspace();
		let settings = workspace.settings();
		let parse = self.get_parse(params.path.clone())?;
	...
	fn fix_file(&self, params: super::FixFileParams) -> Result<FixFileResult, WorkspaceError> {
		...
		let fix_all =
			capabilities.analyzer.fix_all.ok_or_else(self.build_capability_error(&params.path))?;
		let parse = self.get_parse(params.path.clone())?;
	...
	fn rename(&self, params: super::RenameParams) -> Result<RenameResult, WorkspaceError> {
		let capabilities = self.get_file_capabilities(&params.path);
		let rename =
			capabilities.analyzer.rename.ok_or_else(self.build_capability_error(&params.path))?;
	...
	fn search_pattern(&self, params: SearchPatternParams) -> Result<SearchResults, WorkspaceError> {
		...
		let capabilities = self.get_file_capabilities(&params.path);
		let search =
			capabilities.search.search.ok_or_else(self.build_capability_error(&params.path))?;
		let workspace = self.workspace();
		let parse = self.get_parse(params.path.clone())?;
