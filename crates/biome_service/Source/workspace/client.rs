	...
	T: WorkspaceTransport + RefUnwindSafe + Send + Sync,
{
	pub fn new(transport:T) -> Result<Self, WorkspaceError> {
		let mut client = Self { transport, request_id:AtomicU64::new(0), server_info:None };
	...
	pub fn shutdown(self) -> Result<(), WorkspaceError> { self.request("biome/shutdown", ()) }
}
	...
	T: WorkspaceTransport + RefUnwindSafe + Send + Sync,
{
	...
	fn server_info(&self) -> Option<&ServerInfo> { self.server_info.as_ref() }
