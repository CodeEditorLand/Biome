...
impl Projects {
	...
	pub fn insert_project(&self, path: Utf8PathBuf) -> ProjectKey {
		...
		self.0.pin().insert(key, ProjectData { path, settings: Settings::default() });
		key
	}
	...
	pub fn get_settings(&self, project_key: ProjectKey) -> Option<Settings> {
		self.0.pin().get(&project_key).map(|data| data.settings.clone())
	}
	...
	pub fn get_files_settings(&self, project_key: ProjectKey) -> Option<FilesSettings> {
		self.0.pin().get(&project_key).map(|data| data.settings.files.clone())
	}
	...
	pub fn set_settings(&self, project_key: ProjectKey, settings: Settings) {
		...
		let project_data = ProjectData { path: project_data.path.clone(), settings };
	...
	pub fn path_belongs_only_to_project_with_path(&self, path: &Utf8Path, project_path: &Utf8Path) -> bool {
	...
	pub fn is_ignored_by_feature_config(&self, project_key: ProjectKey, path: &Utf8Path, feature: FeatureKind) -> bool {
		let data = self.0.pin();
		let Some(project_data) = data.get(&project_key) else {
			return false;
		};
