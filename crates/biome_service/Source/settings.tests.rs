...
use biome_configuration::{
	Configuration, JsConfiguration, LinterConfiguration, OverrideGlobs, OverrideLinterConfiguration, OverridePattern,
...
fn correctly_passes_jsx_runtime() {
	let js_conf = JsConfiguration { jsx_runtime: Some(JsxRuntime::ReactClassic), ..Default::default() };

	let settings = LanguageSettings::<JsLanguage>::from(js_conf);

	assert_eq!(settings.environment.jsx_runtime, Some(JsxRuntime::ReactClassic));
}

#[test]
fn correctly_lookups_environment_settings() {
	let js_conf = JsConfiguration { jsx_runtime: Some(JsxRuntime::ReactClassic), ..Default::default() };
	let configuration = Configuration { javascript: Some(js_conf), ..Default::default() };
	...
	assert_eq!(environment.unwrap().jsx_runtime, Some(JsxRuntime::ReactClassic));
}
...
fn correctly_computes_analyzer_options() {
	...
	assert_eq!(options.jsx_runtime(), Some(biome_analyze::options::JsxRuntime::ReactClassic));
}
...
fn merge_override_linter_group_rule() {
	let configuration = Configuration {
		...
		overrides: Some(Overrides(vec![OverridePattern {
			includes: Some(OverrideGlobs::Globs(Box::new([Glob::from_str("**/*").unwrap()]))),
			linter: Some(OverrideLinterConfiguration {
				rules: Some(Rules {
					nursery: Some(SeverityOrGroup::Group(Nursery {
						use_explicit_type: Some(RuleConfiguration::Plain(RulePlainConfiguration::Off)),
						..Nursery::default()
					})),
					..Rules::default()
				}),
