...
/// A data structure to use when there's need to match a string or a path a
/// against a unix shell style patterns
	...
	fn run_match(&self, source:&Path) -> bool {
		for pattern in &self.patterns {
			let matches = if pattern.matches_path_with(source, self.options) {
				...
				// Here we cover cases where the user specifies single files
				// inside the patterns. The pattern library doesn't support
				// single files, we here we just do a check on contains
				//
				...
				source.ancestors().any(|ancestor| ancestor.ends_with(pattern.as_str()))
			};
...
mod test {
	...
	use crate::matcher::{pattern::MatchOptions, Matcher};
