	...
	fn run_match(&self, source:&Path) -> bool {
		for pattern in &self.patterns {
			let matches = if pattern.matches_path_with(source, self.options) {
				...
				// Here we cover cases where the user specifies single files inside the
				// patterns. The pattern library doesn't support single files, we here we
				// just do a check on contains
				//
...
mod test {
	...
	use crate::matcher::{Matcher, pattern::MatchOptions};
