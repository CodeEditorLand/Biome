use std::{error::Error, fmt, path, path::Path, str::FromStr};
...
impl Pattern {
	...
	pub fn new(pattern:&str) -> Result<Self, PatternError> { Self::parse(pattern, false) }
	...
	pub fn parse(pattern:&str, is_editorconfig:bool) -> Result<Self, PatternError> {
		...
		if is_relative {
			...
			// A pattern is absolute if it starts with a path separator, eg.
			// "/home" or "\\?\C:\Users"
			...
			// drive letter, a colon and a separator, eg. "c:/Users" or
			// "G:\Users"
		...
		while i < chars.len() {
			match chars[i] {
				...
				'*' => {
					...
					match count {
						count if count > 2 => {
							return Err(PatternError { pos:old + 2, msg:ERROR_WILDCARDS });
						},
						count if count == 2 => {
							...
							let is_valid = if i == 2 || path::is_separator(chars[i - count - 1]) {
								...
								if i < chars.len() && path::is_separator(chars[i]) {
									...
									// this enables the existing globbing
									// mechanism
								} else if i == chars.len() {
									true
							...
							if is_valid {
								// collapse consecutive AnyRecursiveSequence to
								// a single one
				...
				'[' => {
					...
					return Err(PatternError { pos:i, msg:ERROR_INVALID_RANGE });
				},
				'{' if is_editorconfig => {
					let mut depth = 1;
					let mut j = i + 1;
					...
					if depth != 0 {
						return Err(PatternError { pos:i, msg:"unmatched '{'" });
					}
		...
		Ok(Self { tokens, original:pattern.to_string(), is_recursive, is_editorconfig })
	}

	fn from_tokens(tokens:Vec<PatternToken>, original:String, is_recursive:bool) -> Self {
		Self { tokens, original, is_recursive, is_editorconfig:false }
	}
			...
			match token {
				AnySequence | AnyRecursiveSequence => {
					...
					while let Some(c) = file.next() {
						...
						match *token {
							AnyRecursiveSequence if !follows_separator => {
								continue;
							},
							AnySequence
				...
				_ => {
					...
					if !match *token {
						...
						AnySequence | AnyRecursiveSequence | AnyPattern(_) => {
							unreachable!()
						},
					} {
						return SubPatternDoesntMatch;
...
mod test {
	...
	fn test_pattern_matches_require_literal_leading_dot() {
		...
		let f =
			|options| Pattern::new("aaa/bbb/*").unwrap().matches_with("aaa/bbb/c.c.c.", options);
		assert!(f(options_not_require_literal_leading_dot));
		assert!(f(options_require_literal_leading_dot));
	...
	fn test_pattern_absolute() {
		...
		if cfg!(windows) {
			assert!(Pattern::new("c:/a/b").unwrap().matches_path(Path::new("c:/a/b")));
			assert!(Pattern::new("C:\\a\\b").unwrap().matches_path(Path::new("C:\\a\\b")));
			...
			assert!(
				Pattern::new("\\\\?\\C:/a/b").unwrap().matches_path(Path::new("\\\\?\\C:/a/b"))
			);
		}
	}
	...
	fn test_pattern_glob() {
		assert!(Pattern::new("*.js").unwrap().matches_path(Path::new("b/c.js")));

		assert!(Pattern::new("**/*.js").unwrap().matches_path(Path::new("b/c.js")));

		assert!(Pattern::new("*.js").unwrap().matches_path(Path::new("/a/b/c.js")));

		assert!(Pattern::new("**/*.js").unwrap().matches_path(Path::new("/a/b/c.js")));

		if cfg!(windows) {
			assert!(Pattern::new("*.js").unwrap().matches_path(Path::new("C:\\a\\b\\c.js")));
	...
	fn test_pattern_glob_brackets_not_available_by_default() {
		// RODO: Remove this test when we make brackets available by default in
		// Biome 2.0
