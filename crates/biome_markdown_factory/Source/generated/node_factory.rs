...
pub fn markdown_bullet_list_item(
	...
	MarkdownBulletListItem::unwrap_cast(SyntaxNode::new_detached(
		...
		[Some(SyntaxElement::Node(markdown_bullet_list.into_syntax()))],
	))
}
pub fn markdown_document(
	...
	MarkdownDocumentBuilder { value, eof_token, bom_token: None }
}
...
pub fn markdown_header(before: MarkdownHashList, after: MarkdownHashList) -> MarkdownHeaderBuilder {
	MarkdownHeaderBuilder { before, after, markdown_paragraph: None }
}
	...
	pub fn build(self) -> MarkdownHeader {
		MarkdownHeader::unwrap_cast(SyntaxNode::new_detached(
			...
			[
				...
				self.markdown_paragraph.map(|token| SyntaxElement::Node(token.into_syntax())),
...
pub fn markdown_inline_image(
	...
	MarkdownInlineImageBuilder { alt, src, title: None }
}
	...
	pub fn build(self) -> MarkdownInlineImage {
		MarkdownInlineImage::unwrap_cast(SyntaxNode::new_detached(
			...
			[
				...
				self.title.map(|token| SyntaxElement::Node(token.into_syntax())),
			],
		))
	}
}
pub fn markdown_inline_link(
	...
	MarkdownInlineLinkBuilder { label, url, title: None }
}
...
pub fn markdown_paragraph(
	...
	MarkdownParagraph::unwrap_cast(SyntaxNode::new_detached(
		...
		[Some(SyntaxElement::Node(markdown_paragraph_item_list.into_syntax()))],
	))
}
...
where
	...
	I::IntoIter: ExactSizeIterator,
{
	MarkdownBogus::unwrap_cast(SyntaxNode::new_detached(MarkdownSyntaxKind::MARKDOWN_BOGUS, slots))
}
