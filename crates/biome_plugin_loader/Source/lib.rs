...
impl BiomePlugin {
	...
	pub fn load(fs: &dyn FileSystem, plugin_path: &str, base_path: &Utf8Path) -> Result<Self, PluginDiagnostic> {
		...
		if plugin_path.extension().is_some_and(|extension| extension == "grit") {
			...
			return Ok(Self { analyzer_plugins: vec![Arc::new(Box::new(plugin) as Box<dyn AnalyzerPlugin>)] });
		}

		let manifest_path = plugin_path.join("biome-manifest.jsonc");
		if !fs.path_is_file(&manifest_path) {
			return Err(PluginDiagnostic::cant_resolve(manifest_path.to_string(), None));
		}
...
mod test {
    ...
    "rules": ["rules/1.grit"]
		...
		let plugin = BiomePlugin::load(&fs, "./my-plugin", Utf8Path::new("/")).expect("Couldn't load plugin");
	...
	fn load_plugin_without_manifest() {
		...
		let error =
			BiomePlugin::load(&fs, "./my-plugin", Utf8Path::new("/")).expect_err("Plugin loading should've failed");
