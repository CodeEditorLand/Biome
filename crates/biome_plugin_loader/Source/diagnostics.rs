...
impl From<CompileError> for PluginDiagnostic {
	fn from(value: CompileError) -> Self {
		...
		PluginDiagnostic::Compile(CompileDiagnostic { source: Some(Error::from(value)) })
	}
}
...
mod test {
	...
	fn deserialization_error() {
		let content = "{}";
		let result = deserialize_from_json_str::<PluginManifest>(content, JsonParserOptions::default(), "");
    ...
    "rules": [
		...
		let _result = deserialize_from_json_str::<PluginManifest>(content, JsonParserOptions::default(), "")
			.into_deserialized()
			.unwrap_or_default();
	}
}
