...
use biome_grit_patterns::{
	BuiltInFunction, CompilePatternOptions, GritBinding, GritExecContext, GritPattern, GritQuery, GritQueryContext,
	GritQueryState, GritResolvedPattern, GritTargetFile, compile_pattern_with_options,
};
...
fn register_diagnostic<'a>(
	...
	context.add_diagnostic(RuleDiagnostic::new(category!("plugin"), span, message).with_severity(severity));
