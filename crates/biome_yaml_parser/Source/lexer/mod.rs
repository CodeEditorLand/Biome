...
impl Token {
	#[allow(dead_code)]
	pub fn kind(&self) -> YamlSyntaxKind { self.kind }
...
impl<'source> YamlLexer<'source> {
	...
	fn current_char(&self) -> Option<u8> { self.source.as_bytes().get(self.position).copied() }
	...
	fn consume_token(&mut self) -> Option<Token> {
		...
		Some(Token { kind, range:TextRange::new(start, end) })
	}
...
fn interpret_value(value:&str) -> YamlSyntaxKind {
	match value {
		...
		_ => {
			value
				.parse::<f64>()
				.map_or(YamlSyntaxKind::YAML_STRING_VALUE, |_| YamlSyntaxKind::YAML_NUMBER_VALUE)
		},
	}
}
...
impl<'src> Lexer<'src> for YamlLexer<'src> {
	...
	fn rewind(&mut self, _checkpoint:biome_parser::lexer::LexerCheckpoint<Self::Kind>) { todo!() }
...
impl Iterator for YamlLexer<'_> {
	...
	fn next(&mut self) -> Option<Self::Item> { self.consume_token() }
}
...
pub enum YamlLexContext {
	#[default]
	Regular,
	/// The lexer has just lexed an identifier and is expecting a value to come
	/// next.
	///
	/// After an identifier, a `: ` is expected to come next, and then the
	/// value. If the next token is another identifier, it must come after a
	/// newline. Otherwise, values can remain on the same line. However, it is
	/// possible to have a nested mapping on the same line, but the nested
	/// mapping must be enclosed in `{}`.
	///
	/// Valid:
	/// ```yaml
	/// foo:
	...
	/// ```
	AfterIdent,
	/// The lexer has lexed an inline array and is expecting a value or the end
	/// of the array to come next.
	///
	/// In this context, commas are allowed to separate values. Normally, commas
	/// are simply treated as part of a value. A newline is not allowed in this
	/// context. The newline must come after the array has been closed with `]`
	/// ```yaml
	/// foo: [1, 2, 3]
	/// ```
	AfterInlineArray,
...
impl LexContext for YamlLexContext {
	...
	fn is_regular(&self) -> bool { matches!(self, YamlLexContext::Regular) }
}
