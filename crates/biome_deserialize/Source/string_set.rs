...
impl StringSet {
	pub fn new(index_set:IndexSet<String>) -> Self { Self(index_set) }

	pub fn into_index_set(self) -> IndexSet<String> { self.0 }
...
impl FromStr for StringSet {
	...
	fn from_str(_s:&str) -> Result<Self, Self::Err> { Ok(StringSet::default()) }
}
