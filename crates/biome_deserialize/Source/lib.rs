...
//! themselves along with data formats that know how to deserialize data.
...
//! 0ne of the main difference is the fault-tolerant behavior of
//! `biome_deserialize`. _Serde_ uses a fast-fail strategy, while
//! `biome_deserialize` deserialize as much as possible and report several
//! diagnostics (errors, warning, deprecation messages, ...).
...
//!
//! It currently supports the JSON data format. See [biome_deserialize::json]
//! for more details.
//!
//! The two most important traits are [Deserializable] and
//! [DeserializableValue].
//!
...
/// Implemented by data structures that can deserialize any
/// [DeserializableValue].
///
/// `biome_deserialize` provides [Deserializable] implementations for common
/// Rust types. To implement [Deserializable], you can reuse a type that
/// implements [Deserializable] and turn the obtained value into what you want.
///
/// When deserializing more complex types, such as a `struct` or `enum`, you can
/// use the [Deserializable] derive macro.
///
/// ## Example
///
/// ```
...
/// let source = r#""a""#;
/// let deserialized =
/// 	deserialize_from_json_str::<Variant>(&source, JsonParserOptions::default(), "");
...
pub trait Deserializable: Sized {
	...
	/// Any diagnostics emitted during deserialization are appended to
	/// `diagnostics`. `name` corresponds to the name used in a diagnostic to
	/// designate the deserialized value.
	fn deserialize(
...
///
/// This trait should only be implemented when adding the support for a new data
/// format. See [biome_deserialize::json] for an example of an implementation.
...
/// [DeserializableValue::deserialize] calls the `viist_` method that matches
/// the type of the value.
///
/// Most of the time you should implement [Deserializable] and rely on existing
/// deserializable types. You should use a visitor only when you deserialize a
/// `struct` or a union of several types.
///
/// ## Examples
///
/// ```
...
/// let deserialized =
/// 	deserialize_from_json_str::<Person>(&source, JsonParserOptions::default(), "");
/// assert!(!deserialized.has_errors());
/// assert_eq!(
/// 	deserialized.into_deserialized(),
/// 	Some(Person { name:"Isaac Asimov".to_string() })
/// );
/// ```
///
/// ```
...
/// let source = r#" "string" "#;
/// let deserialized =
/// 	deserialize_from_json_str::<Union>(&source, JsonParserOptions::default(), "");
/// assert!(!deserialized.has_errors());
...
pub trait DeserializationVisitor: Sized {
	...
	///
	/// The default implementation appends an incorrect type diagnostic to
	/// `diagnostics`. The expected type is retrieved from
	/// [Self::EXPECTED_TYPE].
	fn visit_null(
		self,
		range:TextRange,
...
impl<T> Deserialized<T> {
	...
	pub fn into_diagnostics(self) -> Vec<Error> { self.diagnostics }

	pub fn diagnostics(&self) -> &[Error] { self.diagnostics.as_slice() }
