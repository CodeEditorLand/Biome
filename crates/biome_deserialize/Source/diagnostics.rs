use biome_console::{MarkupBuf, fmt::Display, markup};
...
impl DeserializationDiagnostic {
	...
	/// Emitted when there's a deprecated property and you can suggest an
	...
	pub fn with_note(mut self, message:impl Display) -> Self {
		self.deserialization_advice.notes.push((markup! {{message}}.to_owned(), vec![]));
	...
	pub fn note_with_list(mut self, message:impl Display, list:&[impl Display]) -> Self {
		self.deserialization_advice.notes.push((
			...
			list.iter().map(|message| markup! {{message}}.to_owned()).collect::<Vec<_>>(),
		));
...
impl From<SyntaxError> for DeserializationDiagnostic {
	fn from(_:SyntaxError) -> Self { DeserializationDiagnostic::new("Syntax error") }
}
...
impl DeserializationAdvice {
	pub fn note(mut self, message:impl Display) -> Self {
		self.notes.push((markup! {{message}}.to_owned(), Vec::new()));
...
mod test {
	...
	fn test_visitable_type_fmt() {
		...
		assert_eq!(
			DeserializableTypes::NULL.union(DeserializableTypes::BOOL).to_string(),
			"null, or a boolean"
		);
