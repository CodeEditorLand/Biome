...
use biome_diagnostics::{
	Advices, Diagnostic, DiagnosticTags, LogCategory, MessageAndDescription,
	Severity, Visit,
};
...
impl DeserializationDiagnostic {
	...
	pub fn new_missing_key(
		key_name: &str,
		range: impl AsSpan,
		required_keys: &[&str],
	) -> Self {
		let diagnostic = Self::new(
			markup!("The key `"<Emphasis>{key_name}</Emphasis>"` is missing." ),
		)
		.with_range(range);
	...
	pub fn new_unknown_key(
		...
		Self::new(
			markup!("Found an unknown key `"<Emphasis>{key_name}</Emphasis>"`." ),
		)
		.with_range(range)
	...
	pub fn new_unknown_value(
		...
		Self::new(
			markup! {"Found an unknown value `"<Emphasis>{variant_name}</Emphasis>"`."},
		)
		.with_range(range)
	...
	pub fn new_deprecated_use_instead(
		key_name: &str,
		range: impl AsSpan,
		instead: &str,
	) -> Self {
	...
	pub fn new_deprecated(key_name: &str, range: impl AsSpan) -> Self {
		Self::new(
			markup! { "The property "<Emphasis>{key_name}</Emphasis>" is deprecated." },
		)
		.with_range(range)
