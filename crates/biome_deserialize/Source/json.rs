...
/// It attempts to parse and deserialize a source file in JSON. Diagnostics from
/// the parse phase are consumed and joined with the diagnostics emitted during
/// the deserialization.
///
/// The data structures that need to be deserialized have to implement the
/// [Deserializable] trait. For most data structures, this can be achieved using
/// the [biome_deserialize_macros::Deserializable] derive.
///
...
/// let deserialized =
/// 	deserialize_from_json_str::<NewConfiguration>(&source, JsonParserOptions::default(), "");
/// assert!(!deserialized.has_errors());
/// assert_eq!(
/// 	deserialized.into_deserialized().unwrap(),
/// 	NewConfiguration { lorem:"ipsum".to_string() }
/// );
/// ```
...
impl DeserializableValue for AnyJsonValue {
	fn range(&self) -> biome_rowan::TextRange { AstNode::range(self) }
...
impl DeserializableValue for JsonMemberName {
	...
	fn visitable_type(&self) -> Option<DeserializableType> { Some(DeserializableType::Str) }
}
...
fn unescape_json(s:TokenText) -> Text {
	if s.text().bytes().any(|c| c == b'\\') {
		...
		// However, strings are expected to be small and escapees are expected to be
		// rare.
