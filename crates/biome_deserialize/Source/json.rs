...
use crate::{
	DefaultDeserializationContext, Deserializable, DeserializableValue, DeserializationContext, DeserializationVisitor,
	Deserialized, TextNumber, diagnostics::DeserializableType,
};
...
pub fn deserialize_from_json_str<Output: Deserializable>(
	...
	let Deserialized { diagnostics, deserialized } = deserialize_from_json_ast::<Output>(&parse.tree(), id);
	let errors = parse
		.into_diagnostics()
		.into_iter()
		.map(Error::from)
...
pub fn deserialize_from_json_ast<Output: Deserializable>(parse: &JsonRoot, id: &str) -> Deserialized<Output> {
	let mut ctx = DefaultDeserializationContext::new(id);
	let deserialized = parse.value().ok().and_then(|value| Output::deserialize(&mut ctx, &value, ""));
	Deserialized { diagnostics: ctx.diagnostics, deserialized }
}
...
mod tests {
	...
	fn test_name() {
		...
		impl Deserializable for Name {
			fn deserialize(
				...
				Some(Name { name: ctx.id().unwrap_or(name).to_string() })
			}
		}
		let source = "0";
	...
	fn test_index_map() {
		let source = r#"{ "a": 0, "b": 1 }"#;
		let Deserialized { deserialized, diagnostics } =
			deserialize_from_json_str::<indexmap::IndexMap<String, u8>>(source, JsonParserOptions::default(), "");
		assert!(diagnostics.is_empty());
		assert_eq!(
			deserialized.unwrap(),
