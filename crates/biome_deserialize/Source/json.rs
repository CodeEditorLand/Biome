...
pub fn deserialize_from_json_str<Output: Deserializable>(
	...
	let errors = parse
		...
		.collect::<Vec<_>>();
	Deserialized { diagnostics: errors, deserialized }
}

pub fn deserialize_from_str<Output: Deserializable>(
	source: &str,
) -> Deserialized<Output> {
	...
	fn visitable_type(&self) -> Option<DeserializableType> {
		match self {
			...
			AnyJsonValue::JsonNumberValue(_) => {
				Some(DeserializableType::Number)
			},
...
mod tests {
	...
	fn test_name() {
		...
		let source = "0";
		let Deserialized { deserialized, .. } = deserialize_from_json_str::<Name>(
			source,
			JsonParserOptions::default(),
			"root",
		);
		assert_eq!(deserialized, Some(Name { name: "root".to_string() }))
	}
	...
	fn test_bool() {
		let source = "true";
		let Deserialized { deserialized, diagnostics } =
			deserialize_from_json_str::<bool>(
				source,
				JsonParserOptions::default(),
				"",
			);
	...
	fn test_isize() {
		let source = "-1";
		let Deserialized { deserialized, diagnostics } =
			deserialize_from_json_str::<isize>(
				source,
				JsonParserOptions::default(),
				"",
			);
	...
	fn test_usize() {
		let source = "0";
		let Deserialized { deserialized, diagnostics } =
			deserialize_from_json_str::<usize>(
				source,
				JsonParserOptions::default(),
				"",
			);
	...
	fn test_non_zero_u8() {
		let source = "1";
		let Deserialized { deserialized, diagnostics } =
			deserialize_from_json_str::<NonZeroU8>(
				source,
				JsonParserOptions::default(),
				"",
			);
	...
	fn test_non_zero_u16() {
		let source = "1";
		let Deserialized { deserialized, diagnostics } =
			deserialize_from_json_str::<NonZeroU16>(
				source,
				JsonParserOptions::default(),
				"",
			);
	...
	fn test_non_zero_u32() {
		let source = "1";
		let Deserialized { deserialized, diagnostics } =
			deserialize_from_json_str::<NonZeroU32>(
				source,
				JsonParserOptions::default(),
				"",
			);
	...
	fn test_non_zero_u64() {
		let source = "1";
		let Deserialized { deserialized, diagnostics } =
			deserialize_from_json_str::<NonZeroU64>(
				source,
				JsonParserOptions::default(),
				"",
			);
	...
	fn test_non_zero_usize() {
		let source = "1";
		let Deserialized { deserialized, diagnostics } =
			deserialize_from_json_str::<NonZeroUsize>(
				source,
				JsonParserOptions::default(),
				"",
			);
	...
	fn test_number() {
		...
		let Deserialized { deserialized, diagnostics } =
			deserialize_from_json_str::<TextNumber>(
				&source,
				JsonParserOptions::default(),
				"",
			);
	...
	fn test_string() {
		let source = r#""string""#;
		let Deserialized { deserialized, diagnostics } =
			deserialize_from_json_str::<String>(
				source,
				JsonParserOptions::default(),
				"",
			);
	...
	fn test_vec() {
		let source = r#"[0, 1]"#;
		let Deserialized { deserialized, diagnostics } =
			deserialize_from_json_str::<Vec<u8>>(
				source,
				JsonParserOptions::default(),
				"",
			);
	...
	fn test_hash_set() {
		let source = r#"[0, 1]"#;
		let Deserialized { deserialized, diagnostics } =
			deserialize_from_json_str::<HashSet<u8>>(
				source,
				JsonParserOptions::default(),
				"",
			);
	...
	fn test_index_set() {
		let source = r#"[0, 1]"#;
		let Deserialized { deserialized, diagnostics } =
			deserialize_from_json_str::<IndexSet<u8>>(
				source,
				JsonParserOptions::default(),
				"",
			);
