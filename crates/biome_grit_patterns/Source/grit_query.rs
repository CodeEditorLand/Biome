...
use crate::grit_definitions::{Definitions, ScannedDefinitionInfo, compile_definitions, scan_definitions};
...
use crate::pattern_compiler::{compilation_context::CompilationContext, compilation_context::NodeCompilationContext};
...
use camino::Utf8Path;
use grit_pattern_matcher::constants::{ABSOLUTE_PATH_INDEX, FILENAME_INDEX, NEW_FILES_INDEX, PROGRAM_INDEX};
use grit_pattern_matcher::file_owners::{FileOwner, FileOwners};
use grit_pattern_matcher::pattern::{FilePtr, FileRegistry, Matcher, Pattern, ResolvedPattern, State, VariableSource};
...
impl GritQuery {
	pub fn execute(&self, file: GritTargetFile) -> GritResult<GritQueryResult> {
		...
		if self.pattern.execute(&binding.into(), &mut state, &context, &mut logs)? {
		...
		Ok(GritQueryResult { effects, diagnostics: context.into_diagnostics(), logs })
	}
	...
	pub fn from_node(
		...
		let ScannedDefinitionInfo { pattern_definition_info, predicate_definition_info, function_definition_info } =
		...
		let mut node_context =
			NodeCompilationContext::new(&context, &mut vars, &mut vars_array, &mut global_vars, &mut diagnostics);

		let mut definitions = compile_definitions(root.definitions(), &mut node_context)?;

		let pattern =
			PatternCompiler::from_node(&root.pattern().ok_or(CompileError::MissingPattern)?, &mut node_context)?;

		let pattern = auto_wrap_pattern(pattern, &mut definitions.patterns, true, None, &mut node_context, None)?;

		let name = source_path.and_then(Utf8Path::file_stem).map(|stem| stem.to_string());
		let language = context.lang;
		let variable_locations = VariableLocations::new(vars_array);

		Ok(Self { pattern, definitions, name, built_ins, language, diagnostics, variable_locations })
	}
...
impl GritQueryEffect {
	pub fn from_file(file: &[&FileOwner<GritTargetTree>]) -> GritResult<Option<Self>> {
		...
		let result = if file.len() == 1 {
			...
			if file.new {
				Some(GritQueryEffect::CreateFile(CreateFile::new(&file.name, &file.tree.source())))
			} else if let Some(ranges) = &file.matches.borrow().input_matches {
				if ranges.suppressed {
					None
				} else {
					Some(GritQueryEffect::Match(Match::from_file_ranges(ranges, &file.name)))
				}
			} else {
				None
			}
	...
	fn from(val: FilePattern) -> Self {
		match val {
			...
			FilePattern::Many(files) => {
				Self::from_files(Self::from_list_parts(files.into_iter().map(Self::from_file_pointer)))
			},
		}
	}
}
...
impl Rewrite {
	fn new(original: Match, rewritten: OutputFile) -> Self {
		Self { original, rewritten }
	}

	fn from_file(initial: &FileOwner<GritTargetTree>, rewritten_file: &FileOwner<GritTargetTree>) -> GritResult<Self> {
...
impl CreateFile {
	fn new(path: &Path, body: &str) -> Self {
		CreateFile { rewritten: OutputFile::new(path, body, None), ranges: None }
	}
}
...
impl OutputFile {
	...
	fn from_file(file: &FileOwner<GritTargetTree>) -> Self {
		Self::new(&file.name, &file.tree.source(), file.matches.borrow().byte_ranges.as_deref())
	}
}
