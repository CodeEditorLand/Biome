...
pub use grit_query::{CreateFile, GritQuery, GritQueryEffect, GritQueryResult, Message, OutputFile};
...
pub fn compile_pattern_with_options(source: &str, options: CompilePatternOptions) -> Result<GritQuery, CompileError> {
	...
	GritQuery::from_node(parsed.tree(), options.path, language, options.extra_built_ins)
}
