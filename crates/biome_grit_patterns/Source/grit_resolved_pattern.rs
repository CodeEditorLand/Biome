...
use grit_pattern_matcher::pattern::{
	Accessor, DynamicPattern, DynamicSnippet, DynamicSnippetPart, File, FilePtr, FileRegistry, GritCall, ListIndex,
	Pattern, PatternName, PatternOrResolved, ResolvedFile, ResolvedPattern, ResolvedSnippet, State, to_unsigned,
};
	...
	fn to_snippets(&self) -> GritResult<Vec<ResolvedSnippet<'a, GritQueryContext>>> {
		match self {
			...
			Self::Binding(bindings) => Ok(vec![ResolvedSnippet::from_binding(
				bindings
					.last()
					.ok_or_else(|| GritPatternError::new("cannot create resolved snippet from unresolved binding"))?
					.to_owned(),
			)]),
			Self::List(elements) => {
			...
			Self::File(_) => Err(GritPatternError::new("cannot convert ResolvedPattern::File to ResolvedSnippet")),
			Self::Files(_) => Err(GritPatternError::new(
				"cannot convert ResolvedPattern::Files to ResolvedSnippet",
			)),
			Self::Constant(constant) => Ok(vec![ResolvedSnippet::Text(constant.to_string().into())]),
		}
	}
}
...
impl<'a> ResolvedPattern<'a, GritQueryContext> for GritResolvedPattern<'a> {
		...
		for part in &snippet.parts {
			match part {
				...
				DynamicSnippetPart::Variable(var) => {
					let content = &state.bindings[var.try_scope().unwrap() as usize].last().unwrap()
						[var.try_index().unwrap() as usize];
	...
	fn from_dynamic_pattern(
		...
		match pattern {
			...
			DynamicPattern::Accessor(accessor) => Self::from_accessor(accessor, state, context, logs),
	...
	fn from_accessor(
		...
		match accessor.get(state, context.language())? {
			Some(PatternOrResolved::Pattern(pattern)) => Self::from_pattern(pattern, state, context, logs),
	...
	fn from_pattern(
		...
		match pattern {
			...
			Pattern::CodeSnippet(GritCodeSnippet { dynamic_snippet: Some(pattern), .. }) => {
			...
			Pattern::StringConstant(string) => Ok(Self::Snippets(vec![ResolvedSnippet::Text((&string.text).into())])),
			...
			Pattern::Map(map) => map
				.elements
				.iter()
				.map(|(key, value)| Ok((key.clone(), Self::from_pattern(value, state, context, logs)?)))
			...
			Pattern::File(file_pattern) => {
				...
				Ok(Self::File(GritFile::Resolved(Box::new(ResolvedFile { name, body }))))
			},
	...
	fn float(&self, state: &FileRegistry<'a, GritQueryContext>, language: &GritTargetLanguage) -> GritResult<f64> {
		match self {
			...
			Self::List(_) | Self::Map(_) | Self::File(_) | Self::Files(_) => Err(GritPatternError::new(
	...
	fn get_files(&self) -> Option<&Self> {
		if let Self::Files(files) = self { Some(files) } else { None }
	}

	fn get_last_binding(&self) -> Option<&GritBinding<'a>> {
		if let Self::Binding(bindings) = self { bindings.last() } else { None }
	}
	...
	fn get_list_items(&self) -> Option<impl Iterator<Item = &Self>> {
		if let Self::List(items) = self { Some(items.iter()) } else { None }
	}
	...
	fn is_truthy(&self, state: &mut State<'a, GritQueryContext>, language: &GritTargetLanguage) -> GritResult<bool> {
	...
	fn matches_undefined(&self) -> bool {
		match self {
			Self::Binding(b) => b.last().and_then(Binding::as_constant).is_some_and(Constant::is_undefined),
			Self::Constant(Constant::Undefined) => true,
			Self::Constant(_) | Self::Snippets(_) | Self::List(_) | Self::Map(_) | Self::File(_) | Self::Files(_) => {
				false
			},
		}
	}
	...
	fn text(
		...
		match self {
			...
			Self::Map(map) => Ok(format!(
				...
				map.iter()
					.map(|(key, value)| {
						let value = value.text(state, language).expect("failed to get text of map value");
						format!("\"{key}\": {value}")
					})
