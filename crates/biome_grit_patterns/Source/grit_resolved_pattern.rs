	...
	fn to_snippets(&self) -> GritResult<Vec<ResolvedSnippet<'a, GritQueryContext>>> {
		match self {
			...
			Self::Binding(bindings) => {
				Ok(vec![ResolvedSnippet::from_binding(
					bindings
						.last()
						.ok_or_else(|| {
...
impl<'a> ResolvedPattern<'a, GritQueryContext> for GritResolvedPattern<'a> {
	fn from_binding(binding:GritBinding<'a>) -> Self { Self::Binding(vec![binding]) }
	...
	fn from_pattern(
		...
		match pattern {
			...
			Pattern::StringConstant(string) => {
				Ok(Self::Snippets(vec![ResolvedSnippet::Text((&string.text).into())]))
			},
