...
impl<'a> ExecContext<'a, GritQueryContext> for GritExecContext<'a> {
	...
	fn predicate_definitions(
		&self,
	) -> &[PredicateDefinition<GritQueryContext>] {
		self.predicates
	}

	fn function_definitions(
		&self,
	) -> &[GritFunctionDefinition<GritQueryContext>] {
		self.functions
	}
	...
	fn exec_step(
		...
		let new_files_binding = &mut state.bindings[GLOBAL_VARS_SCOPE_INDEX]
			.back_mut()
			.unwrap()[NEW_FILES_INDEX];
		if new_files_binding.value.is_none() {
			new_files_binding.value =
				Some(GritResolvedPattern::from_list_parts([].into_iter()));
		}
		...
		for f in new_files {
			...
			let name: PathBuf = file
				...
				.into();
			let body =
				file.body(&state.files).text(&state.files, &self.lang)?;
			let owned_file =
				new_file_owner(name.clone(), &body, &self.lang, logs)?
					.ok_or_else(|| {
						anyhow!(
		...
		new_files_binding.value =
			Some(ResolvedPattern::from_list_parts([].into_iter()));
		Ok(true)
	}
...
fn file_owner_from_matches(
	...
	let Some(tree) = language.get_parser().from_cached_parse_result(
		parse,
		Some(&name),
		logs,
	) else {
...
fn new_file_owner(
	...
	let Some(tree) = language.get_parser().parse_file(
		source,
		Some(&name),
		logs,
		FileOrigin::New,
