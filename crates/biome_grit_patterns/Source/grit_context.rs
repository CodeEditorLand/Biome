...
use grit_pattern_matcher::pattern::{
	CallBuiltIn, File, FilePtr, GritFunctionDefinition, Matcher, Pattern, PatternDefinition, PredicateDefinition,
...
impl<'a> ExecContext<'a, GritQueryContext> for GritExecContext<'a> {
	...
	fn exec_step(
		...
		let binding = if files.len() == 1 {
			...
			files.retain(|file_ptr| self.load_file(&GritFile::Ptr(*file_ptr), state, logs).unwrap_or(false));
		...
		let (variables, ranges, suppressed) = state.bindings_history_to_ranges(&self.lang, self.name());
		...
		let new_files_binding =
			&mut state.bindings[GLOBAL_VARS_SCOPE_INDEX as usize].last_mut().unwrap()[NEW_FILES_INDEX];
		if new_files_binding.value.is_none() {
			new_files_binding.value = Some(GritResolvedPattern::from_list_parts([].into_iter()));
		}

		let Some(new_files) = new_files_binding.value.as_ref().and_then(ResolvedPattern::get_list_items) else {
		...
		for f in new_files {
			...
			let name: PathBuf = file.name(&state.files).text(&state.files, &self.lang)?.as_ref().into();
			let body = file.body(&state.files).text(&state.files, &self.lang)?;
			let owned_file = new_file_owner(name.clone(), &body, &self.lang, logs)?.ok_or_else(|| {
				GritPatternError::Builder(format!("failed to construct new file for file {}", name.to_string_lossy()))
			})?;
	...
	fn load_file(
		...
		match file {
			...
			GritFile::Ptr(ptr) => {
				...
				let file = file_owner_from_matches(&file.path, &file.parse, None, FileOrigin::Fresh, &self.lang, logs)?;
...
fn file_owner_from_matches(
	...
	let Some(tree) = language.get_parser().from_cached_parse_result(parse, Some(&name), logs) else {
		return Ok(None);
	};
...
fn new_file_owner(
	...
	let Some(tree) = language.get_parser().parse_file(source, Some(&name), logs, FileOrigin::New) else {
		return Ok(None);
	};
