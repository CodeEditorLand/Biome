...
use crate::{
	...
	grit_file::GritFile,
	grit_node_patterns::{GritLeafNodePattern, GritNodePattern},
...
impl<'a> ExecContext<'a, GritQueryContext> for GritExecContext<'a> {
	fn pattern_definitions(&self) -> &[PatternDefinition<GritQueryContext>] { self.patterns }

	fn predicate_definitions(&self) -> &[PredicateDefinition<GritQueryContext>] { self.predicates }

	fn function_definitions(&self) -> &[GritFunctionDefinition<GritQueryContext>] { self.functions }

	fn ignore_limit_pattern(&self) -> bool { false }
	...
	fn files(&self) -> &FileOwners<GritTargetTree> { self.files }

	fn language(&self) -> &GritTargetLanguage { &self.lang }
	...
	fn exec_step(
		...
		let mut files = if let Some(files) = binding.get_file_pointers() {
			files.iter().map(|f| state.files.latest_revision(f)).collect::<Vec<FilePtr>>()
		} else {
			return Ok(false);
		};
		...
		let binding = if files.len() == 1 {
			...
			files.retain(|file_ptr| {
				self.load_file(&GritFile::Ptr(*file_ptr), state, logs).unwrap_or(false)
			});
		...
		let input_ranges = InputRanges { ranges, variables, suppressed };
		...
		let Some(new_files) =
			new_files_binding.value.as_ref().and_then(ResolvedPattern::get_list_items)
		else {
		...
		for f in new_files {
			...
			let name:PathBuf =
				file.name(&state.files).text(&state.files, &self.lang)?.as_ref().into();
			let body = file.body(&state.files).text(&state.files, &self.lang)?;
			let owned_file =
				new_file_owner(name.clone(), &body, &self.lang, logs)?.ok_or_else(|| {
					anyhow!("failed to construct new file for file {}", name.to_string_lossy())
				})?;
			self.files().push(owned_file);
			// SAFETY: We just pushed to the list of files, so there must be
			// one.
...
fn file_owner_from_matches(
	...
	let Some(tree) = language.get_parser().from_cached_parse_result(parse, Some(&name), logs)
	else {
		return Ok(None);
	};
...
fn new_file_owner(
	...
	let Some(tree) = language.get_parser().parse_file(source, Some(&name), logs, FileOrigin::New)
	else {
		return Ok(None);
	};
