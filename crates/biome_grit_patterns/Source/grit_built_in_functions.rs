...
use grit_pattern_matcher::{
	...
	context::ExecContext,
	pattern::{
		CallBuiltIn, CallbackPattern, JoinFn, LazyBuiltIn, Pattern, ResolvedPattern, ResolvedSnippet, State,
		get_absolute_file_name, get_file_name,
	},
};
use grit_util::{
...
impl BuiltInFunction {
	...
	pub fn new(name: &'static str, params: &'static [&'static str], func: Box<CallableFn>) -> Self {
		Self { name, params, func, position: BuiltInFunctionPosition::Pattern }
	}
...
impl Debug for BuiltIns {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		f.write_fmt(format_args!("BuiltIns {{ built_ins: {:?} }}", self.built_ins))
	}
}
...
impl Default for BuiltIns {
	fn default() -> Self {
		vec![
			...
			BuiltInFunction::new("log", &["message", "variable"], Box::new(log_fn)).as_predicate_or_pattern(),
		]
		.into()
	}
}
...
impl BuiltIns {
	pub(crate) fn add(&mut self, built_in: BuiltInFunction) {
		debug_assert!(self.built_ins.iter().all(|existing| existing.name != built_in.name));
	...
	pub(crate) fn get_with_index(&self, name: &str) -> Option<(usize, &BuiltInFunction)> {
		self.built_ins.iter().enumerate().find(|(_, built_in)| built_in.name == name)
	}
}

impl From<Vec<BuiltInFunction>> for BuiltIns {
	fn from(built_ins: Vec<BuiltInFunction>) -> Self {
		Self { built_ins, callbacks: Vec::new() }
	}
}
...
fn capitalize_fn<'a>(
	...
	let Some(Some(arg1)) = args.into_iter().next() else {
		return Err(GritPatternError::new("capitalize() takes 1 argument: string"));
	};

	let string = arg1.text(&state.files, context.language())?;
	Ok(ResolvedPattern::from_string(capitalize(&string).to_string()))
}
...
fn distinct_fn<'a>(
	...
	match arg1 {
		...
		GritResolvedPattern::Binding(binding) => match binding.last() {
			Some(binding) => {
				let Some(list_items) = binding.list_items() else {
					return Err(GritPatternError::new("distinct() requires a list as the first argument"));
				};
		...
		_ => Err(GritPatternError::new("distinct() requires a list as the first argument")),
	}
}
...
fn join_fn<'a>(
	...
	let (Some(Some(arg1)), Some(Some(arg2))) = (args.next(), args.next()) else {
		return Err(GritPatternError::new("join() takes 2 arguments: list and separator"));
	};
	...
	let join = if let Some(items) = arg1.get_list_items() {
		...
		return Err(GritPatternError::new("join() requires a list as the first argument"));
	};
...
fn length_fn<'a>(
	...
	let args = GritResolvedPattern::from_patterns(args, state, context, logs)?;
	let Some(Some(arg1)) = args.into_iter().next() else {
		return Err(GritPatternError::new("length() takes 1 argument: list or string"));
	};
...
fn log_fn<'a>(
	...
	if let Some(Some(Pattern::Variable(variable))) = args.get(1) {
		...
		let var_content =
			&state.bindings[var.try_scope().unwrap() as usize].last().unwrap()[var.try_index().unwrap() as usize];
		let value = var_content.value.as_ref();

		let src = value.map_or_else(
			|| Ok("Variable has no source".to_string()),
			|v| v.text(&state.files, context.language()).map(|s| s.to_string()),
		)?;
	...
	logs.push(log_builder.build().map_err(|err| GritPatternError::new(err.to_string()))?);
...
fn lowercase_fn<'a>(
	...
	let args = GritResolvedPattern::from_patterns(args, state, context, logs)?;
	let Some(Some(arg1)) = args.into_iter().next() else {
		return Err(GritPatternError::new("lowercase() takes 1 argument: string"));
	};

	let string = arg1.text(&state.files, context.language())?;
	Ok(ResolvedPattern::from_string(string.to_lowercase_cow().to_string()))
}
...
fn shuffle_fn<'a>(
	...
	let mut list: Vec<_> = if let Some(items) = arg1.get_list_items() {
		...
		return Err(GritPatternError::new("shuffle() requires a list as the first argument"));
	};
...
fn split_fn<'a>(
	...
	let mut args = args.into_iter();
	let (Some(Some(arg1)), Some(Some(arg2))) = (args.next(), args.next()) else {
		return Err(GritPatternError::new("split() takes 2 arguments: string and separator"));
	};
	...
	let parts = string
		.split(separator)
		.map(|s| ResolvedPattern::from_resolved_snippet(ResolvedSnippet::Text(s.to_string().into())));
...
fn trim_fn<'a>(
	...
	let mut args = args.into_iter();
	let (Some(Some(arg1)), Some(Some(arg2))) = (args.next(), args.next()) else {
		return Err(GritPatternError::new("trim() takes 2 arguments: string and trim_chars"));
	};
...
fn uppercase_fn<'a>(
	...
	let args = GritResolvedPattern::from_patterns(args, state, context, logs)?;
	let Some(Some(arg1)) = args.into_iter().next() else {
		return Err(GritPatternError::new("uppercase() takes 1 argument: string"));
	};
...
fn resolve<'a>(target_path: Cow<'a, str>, from_file: Cow<'a, str>) -> GritResult<String> {
	...
	Ok(absolutized
		.to_str()
		.ok_or_else(|| GritPatternError::new("could not build absolute path from file name {target_path}"))?
		.to_owned())
}
