use std::{borrow::Cow, fmt::Debug, num::TryFromIntError, path::Path};
...
impl BuiltIns {
	...
	pub(crate) fn get_built_ins(&self) -> &[BuiltInFunction] { &self.built_ins }
}

impl From<Vec<BuiltInFunction>> for BuiltIns {
	fn from(built_ins:Vec<BuiltInFunction>) -> Self { Self { built_ins, callbacks:Vec::new() } }
}
