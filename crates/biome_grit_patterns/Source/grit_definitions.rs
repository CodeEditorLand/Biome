...
use grit_pattern_matcher::pattern::{GritFunctionDefinition, PatternDefinition, PredicateDefinition};
...
pub fn compile_definitions(
	...
	for definition in definitions {
		match definition? {
			...
			AnyGritDefinition::GritJavascriptFunctionDefinition(_) | AnyGritDefinition::GritBogusDefinition(_) => {
				unreachable!(); // Should be handled in `scan_definitions()`.
			},
		}
	}

	Ok(Definitions { patterns, predicates, functions })
}
...
pub fn scan_definitions(definitions: GritDefinitionList) -> Result<ScannedDefinitionInfo, CompileError> {
		...
		match definition? {
			...
			AnyGritDefinition::GritPatternDefinition(node) => {
				...
				pattern_definition_info.insert(
					name.to_owned(),
					DefinitionInfo { index: pattern_index, parameters: collect_variables(node.args())? },
				);
			...
			AnyGritDefinition::GritPredicateDefinition(node) => {
				...
				predicate_definition_info.insert(
					name.to_owned(),
					DefinitionInfo { index: predicate_index, parameters: collect_variables(node.args())? },
				);
			...
			AnyGritDefinition::GritFunctionDefinition(node) => {
				...
				function_definition_info.insert(
					name.to_owned(),
					DefinitionInfo { index: function_index, parameters: collect_variables(node.args())? },
				);
	...
	Ok(ScannedDefinitionInfo { pattern_definition_info, predicate_definition_info, function_definition_info })
}
