...
use self::{
	...
	map_accessor_compiler::MapAccessorCompiler,
	...
	fn from_node_with_rhs(
		...
		match node {
			...
			AnyGritPattern::GritEvery(node) => {
				Ok(Pattern::Every(Box::new(EveryCompiler::from_node(node, context)?)))
			},
			...
			AnyGritPattern::GritLike(node) => {
				Ok(Pattern::Like(Box::new(LikeCompiler::from_node(node, context)?)))
			},
			...
			AnyGritPattern::GritPatternAnd(node) => {
				Ok(Pattern::And(Box::new(AndCompiler::from_node(node, context)?)))
			},
			AnyGritPattern::GritPatternAny(node) => {
				Ok(Pattern::Any(Box::new(AnyCompiler::from_node(node, context)?)))
			},
			AnyGritPattern::GritPatternAs(node) => {
				Ok(Pattern::Where(Box::new(AsCompiler::from_node(node, context)?)))
			},
			...
			AnyGritPattern::GritPatternNot(node) => {
				Ok(Pattern::Not(Box::new(NotCompiler::from_node(node, context)?)))
			},
			...
			AnyGritPattern::GritSome(node) => {
				Ok(Pattern::Some(Box::new(SomeCompiler::from_node(node, context)?)))
			},
			...
			AnyGritPattern::GritVariable(node) => {
				Ok(Pattern::Variable(VariableCompiler::from_node(node, context)))
			},
