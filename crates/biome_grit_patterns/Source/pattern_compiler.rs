...
use self::{
	...
	after_compiler::AfterCompiler, and_compiler::AndCompiler,
	...
	literal_compiler::LiteralCompiler,
	...
	not_compiler::NotCompiler, or_compiler::OrCompiler,
...
use biome_grit_syntax::{
	AnyGritMaybeCurlyPattern, AnyGritPattern, GritSyntaxKind,
};
use biome_rowan::AstNode as _;
use grit_pattern_matcher::pattern::{
	DynamicPattern, DynamicSnippet, DynamicSnippetPart, Pattern,
};
...
impl PatternCompiler {
	...
	pub(crate) fn from_maybe_curly_node(
		...
		match node {
			AnyGritMaybeCurlyPattern::AnyGritPattern(pattern) => {
				Self::from_node(pattern, context)
			},
	...
	fn from_node_with_rhs(
		...
		match node {
			...
			AnyGritPattern::GritAddOperation(node) => Ok(Pattern::Add(
				Box::new(AddCompiler::from_node(node, context)?),
			)),
			...
			AnyGritPattern::GritDivOperation(node) => Ok(Pattern::Divide(
				Box::new(DivideCompiler::from_node(node, context)?),
			)),
			...
			AnyGritPattern::GritLike(node) => Ok(Pattern::Like(Box::new(
				LikeCompiler::from_node(node, context)?,
			))),
			AnyGritPattern::GritListAccessor(node) => Ok(Pattern::ListIndex(
				Box::new(ListIndexCompiler::from_node(node, context)?),
			)),
			AnyGritPattern::GritMapAccessor(node) => Ok(Pattern::Accessor(
				Box::new(MapAccessorCompiler::from_node(node, context)?),
			)),
			AnyGritPattern::GritModOperation(node) => Ok(Pattern::Modulo(
				Box::new(ModuloCompiler::from_node(node, context)?),
			)),
			AnyGritPattern::GritMulOperation(node) => Ok(Pattern::Multiply(
				Box::new(MultiplyCompiler::from_node(node, context)?),
			)),
			...
			AnyGritPattern::GritPatternAccumulate(node) => {
				Ok(Pattern::Accumulate(Box::new(
					AccumulateCompiler::from_node(node, context)?,
				)))
			},
			AnyGritPattern::GritPatternAfter(node) => Ok(Pattern::After(
				Box::new(AfterCompiler::from_node(node, context)?),
			)),
			...
			AnyGritPattern::GritPatternAs(node) => Ok(Pattern::Where(
				Box::new(AsCompiler::from_node(node, context)?),
			)),
			AnyGritPattern::GritPatternBefore(node) => Ok(Pattern::Before(
				Box::new(BeforeCompiler::from_node(node, context)?),
			)),
			AnyGritPattern::GritPatternContains(node) => Ok(Pattern::Contains(
				Box::new(ContainsCompiler::from_node(node, context)?),
			)),
			AnyGritPattern::GritPatternIfElse(node) => {
				Ok(Pattern::If(Box::new(IfCompiler::from_node(node, context)?)))
			},
			AnyGritPattern::GritPatternIncludes(node) => Ok(Pattern::Includes(
				Box::new(IncludesCompiler::from_node(node, context)?),
			)),
			AnyGritPattern::GritPatternLimit(node) => Ok(Pattern::Limit(
				Box::new(LimitCompiler::from_node(node, context)?),
			)),
			AnyGritPattern::GritPatternMaybe(node) => Ok(Pattern::Maybe(
				Box::new(MaybeCompiler::from_node(node, context)?),
			)),
			...
			AnyGritPattern::GritPatternWhere(node) => Ok(Pattern::Where(
				Box::new(WhereCompiler::from_node(node, context)?),
			)),
			AnyGritPattern::GritRegexPattern(node) => Ok(Pattern::Regex(
				Box::new(RegexCompiler::from_node(node, context, is_rhs)?),
			)),
			AnyGritPattern::GritRewrite(node) => Ok(Pattern::Rewrite(
				Box::new(RewriteCompiler::from_node(node, context)?),
			)),
			AnyGritPattern::GritSequential(node) => Ok(Pattern::Sequential(
				SequentialCompiler::from_node(node, context)?,
			)),
			AnyGritPattern::GritSome(node) => Ok(Pattern::Some(Box::new(
				SomeCompiler::from_node(node, context)?,
			))),
			AnyGritPattern::GritSubOperation(node) => Ok(Pattern::Subtract(
				Box::new(SubtractCompiler::from_node(node, context)?),
			)),
			...
			AnyGritPattern::GritBogusPattern(_) => {
				Err(CompileError::UnexpectedKind(
