use crate::{
	grit_context::GritQueryContext, grit_target_language::GritTargetLanguage, grit_target_node::GritTargetNode,
	source_location_ext::SourceFileExt, util::TextRangeGritExt,
};
use biome_diagnostics::{SourceCode, display::SourceFile};
use biome_rowan::TextRange;
use grit_pattern_matcher::{binding::Binding, constant::Constant, effects::Effect, pattern::FileRegistry};
use grit_util::{
...
impl<'a> Binding<'a, GritQueryContext> for GritBinding<'a> {
	...
	fn get_insertion_padding(&self, _text: &str, _is_first: bool, _language: &GritTargetLanguage) -> Option<String> {
	...
	fn text(&self, _language: &GritTargetLanguage) -> GritResult<Cow<'a, str>> {
		match self {
			...
			Self::Range(range, source) => Ok((&source[range.start().into()..range.end().into()]).into()),
