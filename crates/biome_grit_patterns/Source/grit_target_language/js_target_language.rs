...
use super::{
	...
	DisregardedSlotCondition,
	GritTargetLanguageImpl,
...
const COMMENT_KINDS:SyntaxKindSet<JsLanguage> =
	SyntaxKindSet::from_raw(RawSyntaxKind(JsSyntaxKind::COMMENT as u16))
		.union(SyntaxKindSet::from_raw(RawSyntaxKind(JsSyntaxKind::MULTILINE_COMMENT as u16)));
...
impl GritTargetLanguageImpl for JsTargetLanguage {
	...
	fn is_comment_kind(kind:GritTargetSyntaxKind) -> bool {
		kind.as_js_kind().map_or(false, |kind| COMMENT_KINDS.matches(kind))
	}

	fn metavariable_kind() -> Self::Kind { JsSyntaxKind::JS_METAVARIABLE }
		...
		DISREGARDED_SNIPPET_SLOTS.iter().any(
			|(disregarded_kind, disregarded_slot_index, condition)| {
				...
				match condition {
					...
					DisregardedSlotCondition::OnlyIf(node_texts) => {
						node_texts.iter().any(|text| {
