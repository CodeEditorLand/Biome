...
impl Diagnostic for CompileError {
	...
	fn message(&self, fmt: &mut Formatter<'_>) -> std::io::Result<()> {
		match self {
			...
			CompileError::DuplicateParameters => fmt.write_markup(markup! { "Duplicate parameters" }),
			...
			CompileError::InvalidMetavariableRange(_) => fmt.write_markup(markup! { "Invalid range for metavariable" }),
			CompileError::InvalidRawSnippetPosition => fmt.write_markup(markup! { "Invalid range for metavariable" }),
			CompileError::InvalidRegexPosition => {
				fmt.write_markup(markup! { "Regular expressions are not allowed on the right-hand side of a rule" })
			},
			CompileError::MetavariableNotFound(var) => fmt.write_markup(markup! { "Metavariable not found: "{{var}} }),
			CompileError::ReservedMetavariable(var) => fmt.write_markup(markup! { "Reserved metavariable: "{{var}} }),
			CompileError::UnsupportedKind(kind) => fmt.write_markup(markup! { "Unsupported syntax kind ("{{kind}}")" }),
			CompileError::UnexpectedKind(kind) => fmt.write_markup(markup! { "Unexpected syntax kind ("{{kind}}")" }),
			...
			CompileError::FunctionArgument(_) => fmt.write_markup(markup! { "Invalid function argument" }),
			...
			CompileError::UnknownVariable(var) => fmt.write_markup(markup! { "Unknown variable: "{{var}} }),
	...
	fn advices(&self, visitor: &mut dyn biome_diagnostics::Visit) -> std::io::Result<()> {
		match self {
			CompileError::UnexpectedBuiltinCall(name) => visitor.record_log(
				LogCategory::Info,
				&markup! { "Built-in "{{name}}" can only be used on the right-hand side of a rewrite" }.to_owned(),
			),
			CompileError::ReservedMetavariable(_) => {
				visitor.record_log(LogCategory::Info, &markup! { "Try using a different variable name" }.to_owned())
			},
			_ => Ok(()),
		}
	}
