...
impl GritTargetLanguage {
	...
	pub fn from_declaration(language_decl: &GritLanguageDeclaration) -> Option<Self> {
		match language_decl.name().ok()?.as_grit_language_name()?.language_kind().ok()?.kind() {
	...
	pub fn from_extension(extension: &str) -> Option<Self> {
		match extension {
			...
			"cjs" | "js" | "jsx" | "mjs" | "ts" | "tsx" => Some(Self::JsTargetLanguage(JsTargetLanguage)),
			_ => None,
		}
	}
...
impl LeafEquivalenceClass {
	pub fn are_equivalent(&self, kind: GritTargetSyntaxKind, text: &str) -> bool {
		self.class
			...
			.is_some_and(|normalizer| normalizer.normalize(text).is_some_and(|s| s == self.representative))
	}
	...
	pub(crate) fn new(
		...
		if let Some(normalizer) = members.iter().find(|norm| norm.kind == kind) {
			let rep = normalizer.normalize(representative).ok_or(CompileError::NormalizationError)?;
...
impl LeafNormalizer {
	...
	pub(crate) const fn new(kind: GritTargetSyntaxKind, normalizer: fn(&str) -> Option<&str>) -> Self {
