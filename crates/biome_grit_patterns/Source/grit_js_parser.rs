use crate::{grit_analysis_ext::GritAnalysisExt, grit_target_language::GritTargetParser, grit_tree::GritTargetTree};
...
impl Parser for GritJsParser {
	...
	fn parse_file(
		...
		let parse_result = parse(body, JsFileSource::tsx(), JsParserOptions::default().with_metavariables());
	...
	fn parse_snippet(
		...
		let parse_result = parse(&context, JsFileSource::tsx(), JsParserOptions::default().with_metavariables());
