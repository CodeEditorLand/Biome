...
use grit_pattern_matcher::pattern::{
	CodeSnippet, DynamicPattern, Matcher, Pattern, PatternName,
...
impl Matcher<GritQueryContext> for GritCodeSnippet {
	fn execute<'a>(
		...
		let Some(binding) = resolved.get_last_binding() else {
			return Ok(resolved.text(&state.files, context.language())?.trim()
				== self.source);
		};
		...
		if let Some((_, pattern)) =
			self.patterns.iter().find(|(kind, _)| *kind == node.kind())
		{
