...
use super::{
	...
	snippet_compiler::split_snippet,
...
impl AsCompiler {
	pub(crate) fn from_node(
		...
		// could possible lead to some false positives, but more precise
		// solutions require much greater changes.
		...
		Ok(Where::new(
			...
			Predicate::Match(Box::new(Match::new(Container::Variable(variable), Some(pattern)))),
		))
	}
}
