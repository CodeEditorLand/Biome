...
pub fn auto_wrap_pattern<Q:QueryContext>(
	...
	let pattern = if is_not_multifile {
		...
		let second_wrap =
			if should_wrap_in_file { wrap_pattern_in_file(first_wrap)? } else { first_wrap };
...
fn is_sequential<Q:QueryContext>(
	...
	match pattern {
		...
		Pattern::Call(call) => {
			is_sequential(&pattern_definitions[call.index].pattern, pattern_definitions)
		},
		Pattern::AstNode(_)
		| Pattern::List(_)
		| Pattern::ListIndex(_)
...
pub(crate) fn should_autowrap<Q:QueryContext>(
	...
	match pattern {
		...
		Pattern::Call(call) => {
			should_autowrap(&pattern_definitions[call.index].pattern, pattern_definitions)
		},
		Pattern::AstNode(_)
		| Pattern::List(_)
		| Pattern::ListIndex(_)
...
fn should_wrap_in_file<Q:QueryContext>(
	...
	match pattern {
		...
		Pattern::Call(call) => {
			should_wrap_in_file(&pattern_definitions[call.index].pattern, pattern_definitions)
		},
		Pattern::AstNode(_)
		| Pattern::Contains(_)
		| Pattern::List(_)
...
fn wrap_pattern_in_range<Q:QueryContext>(
	...
	for file_range in ranges {
		...
		let file_match = Predicate::Match(Box::new(Match::new(
			...
			Some(Pattern::Includes(Box::new(Includes::new(Pattern::StringConstant(
				StringConstant::new(file_range.file_path.to_string_lossy().to_string()),
			))))),
		)));
		predicates.push(Predicate::And(Box::new(PrAnd::new(vec![file_match, range_match]))));
	}
	...
	let pattern = Pattern::Where(Box::new(Where::new(
		...
		Predicate::Match(Box::new(Match::new(Container::Variable(var), Some(pattern)))),
	)));
	Ok(pattern)
}
...
pub(crate) fn wrap_pattern_in_before_and_after_each_file<Q:QueryContext>(
	...
	if let Some(DefinitionInfo { index, parameters: _ }) =
