...
use crate::{
	...
	pattern_compiler::snippet_compiler::parse_snippet_content,
	...
	pub(crate) fn from_node(
		...
		let regex = match node.regex()? {
			...
			AnyGritRegex::GritSnippetRegexLiteral(regex_node) => {
				...
				if !context.compilation.lang.metavariable_regex().is_match(regex) {
					let alternative = format!("r\"{}\"", &regex[2..regex.len() - 1]);
					context.log(CompilerDiagnostic::new_warning(
						format!(
							"Unnecessary use of metavariable snippet syntax without \
							 metavariables. Replace {regex} with {alternative}"
						),
