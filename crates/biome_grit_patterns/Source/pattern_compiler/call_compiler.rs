...
pub(super) fn call_pattern_from_node_with_name(
		...
		for arg in &args {
			if !VALID_FILE_ARGS.contains(&arg.0.as_str()) {
				return Err(NodeLikeArgumentError::UnknownVariable {
					...
					valid_vars: VALID_FILE_ARGS.iter().map(|arg| (*arg).to_string()).collect(),
				}
				.into());
			}
		}

		let name = args.remove_entry("$name").map_or(Pattern::Underscore, |p| p.1);
		...
		.find(|(_, built_in)| built_in.name == name)
	{
		...
		Ok(Pattern::CallBuiltIn(Box::new(call_built_in_from_args(args, params, index, lang)?)))
		...
		Ok(Pattern::CallFunction(Box::new(CallFunction::new(info.index, args))))
...
pub(super) fn named_args_from_node(
	...
	let expected_params = if let Some(built_in) =
		context.compilation.built_ins.get_built_ins().iter().find(|built_in| built_in.name == name)
	{
		Some(built_in.params.iter().map(|param| (*param).to_string()).collect())
	} else if let Some(info) = context.compilation.function_definition_info.get(name) {
	...
	node_to_args_pairs(name, node.named_args(), &context.compilation.lang, &expected_params)
}
		...
		.map(|(i, node)| match node {
			Ok(AnyGritMaybeNamedArg::AnyGritPattern(pattern)) => {
				let var = match pattern {
					...
					_ => {
						Err(NodeLikeArgumentError::ExpectedVariable { name: fn_name.to_string() })?
					}
				};
			...
			Ok(AnyGritMaybeNamedArg::GritBogusNamedArg(_)) => {
				Err(CompileError::UnexpectedKind(GritSyntaxKind::GRIT_BOGUS_NAMED_ARG.into()))
			}
