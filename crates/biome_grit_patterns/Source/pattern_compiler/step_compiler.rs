...
const SEQUENTIAL_WARNING:&str = "`sequential` matches at the top of the file. If a pattern \
                                 matched outside of a sequential, but no longer matches, it is \
                                 likely because naked patterns are automatically wrapped with \
			...
			Pattern::Sequential(ref s) => {
				for step in s.iter() {
					if !matches!(
						step.pattern,
						Pattern::File(_)
							| Pattern::Files(_) | Pattern::Contains(_)
							| Pattern::Includes(_)
							| Pattern::Maybe(_) | Pattern::Call(_)
							| Pattern::Where(_)
					) {
						context.log(CompilerDiagnostic::new_warning(
							SEQUENTIAL_WARNING,
