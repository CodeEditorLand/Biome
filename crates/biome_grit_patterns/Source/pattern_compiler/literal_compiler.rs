	...
	pub(crate) fn from_node_with_rhs(
		...
		match node {
			AnyGritLiteral::GritBooleanLiteral(node) => {
				Ok(Pattern::BooleanConstant(BooleanConstant::new(
					node.value()?.text_trimmed() == "true",
				)))
			},
			...
			AnyGritLiteral::GritDoubleLiteral(node) => {
				Ok(Pattern::FloatConstant(FloatConstant::new(
			...
			AnyGritLiteral::GritIntLiteral(node) => {
				Ok(Pattern::IntConstant(IntConstant::new(
					node.value_token()?.text_trimmed().parse().map_err(|err| {
			...
			AnyGritLiteral::GritList(node) => {
				Ok(Pattern::List(Box::new(ListCompiler::from_node_with_rhs(
					node, context, is_rhs,
				)?)))
			},
			AnyGritLiteral::GritMap(node) => {
				Ok(Pattern::Map(Box::new(MapCompiler::from_node_with_rhs(node, context, is_rhs)?)))
			},
			AnyGritLiteral::GritStringLiteral(node) => {
				let token = node.value_token()?;
				let text = token.text_trimmed();
				debug_assert!(text.len() >= 2, "Strings must have quotes");
				Ok(Pattern::StringConstant(StringConstant::new(unescape(&text[1..text.len() - 1]))))
			},
			AnyGritLiteral::GritUndefinedLiteral(_) => Ok(Pattern::Undefined),
			AnyGritLiteral::GritBogusLiteral(_) => {
				Err(CompileError::UnexpectedKind(GritSyntaxKind::GRIT_BOGUS_LITERAL.into()))
			},
		}
	}
}
