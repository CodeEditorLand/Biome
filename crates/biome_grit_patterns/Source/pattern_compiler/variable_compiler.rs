...
impl<'a> NodeCompilationContext<'a> {
	...
	pub fn register_variable(&mut self, name:String, range:ByteRange) -> Variable {
		self.register_variable_with_optional_range(
			name,
			Some(FileLocation { range, path:self.compilation.source_path }),
		)
	}
	...
	pub fn register_variable_with_optional_range(
		...
		let Self { vars, vars_array, global_vars, scope_index, .. } = self;
		...
		if let Some(i) = global_vars.get(&name) {
			if let Some(FileLocation { path, range }) = location {
				if path.is_none() {
					vars_array[GLOBAL_VARS_SCOPE_INDEX][*i].locations.insert(range);
				}
			}
		...
		let (locations, path) = if let Some(FileLocation { path, range }) = location {
			...
			// this currently only comes up with the $match variable which
			// we autowrap, and is not usually used by the user, but
			// feels like this could potentially be a source of bugs
			(BTreeSet::new(), None)
		};
