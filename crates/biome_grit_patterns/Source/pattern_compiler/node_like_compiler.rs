...
use super::{call_compiler::*, compilation_context::NodeCompilationContext, PatternCompiler};
use crate::{
	grit_context::GritQueryContext,
	grit_node_patterns::{GritNodePattern, GritNodePatternArg},
...
fn node_pattern_from_node_with_name_and_kind(
	...
	if context.compilation.lang.is_comment_kind(kind) {
		let args = match named_args.len().cmp(&1) {
			...
			Ordering::Greater => {
				Err(NodeLikeArgumentError::TooManyArguments {
	...
	for (arg_name, node) in named_args {
		...
		let Some((_, slot_index)) = node_slots.iter().find(|(slot_name, _)| *slot_name == arg_name)
		else {
			return Err(NodeLikeArgumentError::UnknownArgument {
				name,
				argument:arg_name,
				valid_args:node_slots.iter().map(|(_, slot_name)| slot_name.to_string()).collect(),
			})?;
		};
