...
pub(crate) fn parse_snippet_content(
	...
	// identifier.
	if context.compilation.lang.matches_bracket_metavariable(source) {
		return if is_rhs {
			Ok(Pattern::Dynamic(
	...
	if context.compilation.lang.matches_exact_metavariable(source.trim()) {
...
pub(crate) fn dynamic_snippet_from_source(
	...
	for (byte_range, var) in metavariables.into_iter().rev() {
		parts.push(DynamicSnippetPart::String(source[last..byte_range.start].to_string()));
		...
		if let Some(var) = context.vars.get(var.as_ref()) {
			context.vars_array[context.scope_index][*var].locations.insert(range);
			parts.push(DynamicSnippetPart::Variable(Variable::new(context.scope_index, *var)));
		} else if let Some(var) = context.global_vars.get(var.as_ref()) {
			parts.push(DynamicSnippetPart::Variable(Variable::new(GLOBAL_VARS_SCOPE_INDEX, *var)));
...
fn pattern_arg_from_slot(
	...
	if slot.contains_list() {
		...
		Ok(GritNodePatternArg::new(
			slot.index(),
			if nodes_list.len() == 1
				&& matches!(nodes_list.first(), Some(Pattern::Variable(_) | Pattern::Underscore))
			{
...
fn text_to_var(
	...
	match meta_value {
		...
		GritMetaValue::Variable(name) => {
			let range =
				*range_map.get(&range).ok_or(CompileError::InvalidMetavariableRange(range))?;
