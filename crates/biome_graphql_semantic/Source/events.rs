...
pub enum SemanticEvent {
	...
	/// Tracks where a symbol is referenced regardless of its declaration
	/// position. Generated for:
	/// - All reference identifiers
	Reference { range:TextRange, declared_at:TextRange },
	...
	UnresolvedVariableReference { range:TextRange, referenced_operation:Option<TextRange> },
}
...
struct Scope {
	...
	references:FxHashMap<BindingName, Vec<ReferenceInfo>>,
	/// Every implicit variables references in this scope, which might be
	/// referenced directly in this scope, or by any referenced fragments
...
impl SemanticEventExtractor {
	...
	fn enter_identifier_binding(&mut self, node:&GraphqlNameBinding) {
		...
		let Some(parent) = node.syntax().parent() else {
			// every node aside from the root should have a parent, so this should never
			// happen
			return;
		};
		...
		if AnyGraphqlTypeDefinition::can_cast(parent.kind()) {
			self.push_binding(
				...
				BindingInfo { range, scope_id:self.get_current_scope_id() },
			);
		} else {
			self.push_binding(
	...
	fn enter_identifier_usage(&mut self, node:&GraphqlNameReference) {
		...
		if let Some(scope) = &mut self.current_scope {
			scope.references.entry(binding_name).or_default().push(binding_info);
		}
	}
	...
	fn enter_variable_usage(&mut self, node:&GraphqlVariableReference) {
		...
		scope
			...
			.or_default()
			.insert(VariableReferenceInfo { range, name:name_token });
	}
	...
	fn resolve_references(&mut self) {
		for (name, references) in self.references.clone() {
			if let Some(&BindingInfo { range: declared_at, .. }) = self.bindings.get(&name) {
				// We know the declaration of these reference.
				for reference in references {
					let event = SemanticEvent::Reference { range:reference.range, declared_at };
				...
				for reference in references {
					self.stash
						.push_back(SemanticEvent::UnresolvedReference { range:reference.range });
				}
			}
		}
	}
		...
		for scope in &self.scopes {
			...
			for (name, variables) in &scope.implicit_variables_references {
					...
					variables_definitions.get(name)
				{
					...
					for variable in variables {
						self.stash.push_back(SemanticEvent::UnresolvedVariableReference {
							range:variable.range,
	...
	/// Recursively resolve implicit variables references from referenced
	/// fragments
	fn resolve_scope_implicit_variables_references(
		...
		for (name, _) in references {
			let Some(&BindingInfo { scope_id: Some(binding_scope_id), .. }) =
			...
			if self.scopes[binding_scope_id].variables_definitions.is_some() {
				continue;
			}

			// Bring every implicit variable reference from fragment definition's scope to
			// the current scope Cycle detection is handled by a lint rule
			self.resolve_scope_implicit_variables_references(binding_scope_id, processed_scopes);

			let binding_scope_variables =
				self.scopes[binding_scope_id].implicit_variables_references.clone();
