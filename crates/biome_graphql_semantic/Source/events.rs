...
use biome_graphql_syntax::{
	AnyGraphqlTypeDefinition, AnyGraphqlTypeExtension,
	GraphqlFragmentDefinition, GraphqlNameBinding, GraphqlNameReference,
	GraphqlOperationDefinition, GraphqlSyntaxKind, GraphqlSyntaxNode,
	GraphqlVariableBinding, GraphqlVariableReference, TextRange,
};
...
struct Scope {
	...
	implicit_variables_references:
		FxHashMap<TokenText, HashSet<VariableReferenceInfo>>,
...
impl SemanticEventExtractor {
	...
	pub fn enter(&mut self, node: &GraphqlSyntaxNode) {
		match node.kind() {
			GRAPHQL_NAME_BINDING => {
				self.enter_identifier_binding(
					&GraphqlNameBinding::unwrap_cast(node.clone()),
				);
			},

			GRAPHQL_NAME_REFERENCE => {
				self.enter_identifier_usage(
					&GraphqlNameReference::unwrap_cast(node.clone()),
				);
			},

			GRAPHQL_VARIABLE_BINDING => {
				self.enter_variable_binding(
					&GraphqlVariableBinding::unwrap_cast(node.clone()),
				);
			},

			GRAPHQL_VARIABLE_REFERENCE => {
				self.enter_variable_usage(
					&GraphqlVariableReference::unwrap_cast(node.clone()),
				);
			},

			GRAPHQL_OPERATION_DEFINITION => {
				self.push_operation_scope(
					&GraphqlOperationDefinition::unwrap_cast(node.clone()),
				);
			},

			GRAPHQL_FRAGMENT_DEFINITION => {
				self.push_fragment_scope(
					&GraphqlFragmentDefinition::unwrap_cast(node.clone()),
				);
			},
	...
	fn enter_identifier_usage(&mut self, node: &GraphqlNameReference) {
		...
		let binding_name = match parent.kind() {
			...
			_ if AnyGraphqlTypeExtension::can_cast(parent.kind()) => {
				BindingName::Type(name)
			},
	...
	fn enter_variable_binding(&mut self, node: &GraphqlVariableBinding) {
		...
		let Some(variables_definitions) = &mut scope.variables_definitions
		else {
			return;
		};
	...
	fn resolve_references(&mut self) {
		for (name, references) in self.references.clone() {
			if let Some(&BindingInfo { range: declared_at, .. }) =
	...
	fn resolve_variables_references(&mut self) {
		...
		for scope_id in 0..self.scopes.len() {
			self.resolve_scope_implicit_variables_references(
				scope_id,
				&mut processed_scopes,
			);
		}
		...
		for scope in &self.scopes {
			...
			let Some(variables_definitions) = &scope.variables_definitions
			else {
				continue;
			};
			...
			for (name, variables) in &scope.implicit_variables_references {
					...
					variables_definitions.get(name)
				{
					...
					for variable in variables {
						self.stash.push_back(
							SemanticEvent::UnresolvedVariableReference {
								range: variable.range,
