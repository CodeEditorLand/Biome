...
use biome_graphql_syntax::{
	AnyGraphqlTypeDefinition, AnyGraphqlTypeExtension, GraphqlFragmentDefinition, GraphqlNameBinding,
	GraphqlNameReference, GraphqlOperationDefinition, GraphqlSyntaxKind, GraphqlSyntaxNode, GraphqlVariableBinding,
	GraphqlVariableReference, TextRange,
};
...
impl SemanticEventExtractor {
	...
	fn resolve_variables_references(&mut self) {
		...
		for scope in &self.scopes {
			...
			for (name, variables) in &scope.implicit_variables_references {
				if let Some(&VariableBindingInfo { range: declared_at }) = variables_definitions.get(name) {
					// Bind those variables to its declaration
					for variable in variables {
						self.stash
							.push_back(SemanticEvent::Reference { range: variable.range, declared_at });
	...
	fn resolve_scope_implicit_variables_references(
		...
		for (name, _) in references {
			let Some(&BindingInfo { scope_id: Some(binding_scope_id), .. }) = self.bindings.get(&name) else {
				continue;
			};
			...
			let binding_scope_variables = self.scopes[binding_scope_id].implicit_variables_references.clone();
