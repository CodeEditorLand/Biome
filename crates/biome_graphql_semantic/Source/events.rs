...
impl SemanticEventExtractor {
	...
	fn enter_identifier_binding(&mut self, node:&GraphqlNameBinding) {
		...
		let Some(parent) = node.syntax().parent() else {
			// every node aside from the root should have a parent, so this
			// should never happen
			return;
		};
	...
	fn resolve_scope_implicit_variables_references(
		...
		for (name, _) in references {
			...
			// Bring every implicit variable reference from fragment
			// definition's scope to the current scope Cycle detection is
			// handled by a lint rule
