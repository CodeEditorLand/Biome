...
/// Builds the [SemanticModel] consuming [SemanticEvent] and
/// [GraphqlSyntaxNode]. For a good example on how to use it see
/// [semantic_model].
///
...
pub struct SemanticModelBuilder {
	...
	/// maps a reference range start to its index inside
	/// SemanticModelBuilder::references vec
	...
	pub fn push_event(&mut self, e:SemanticEvent) {
		...
		match e {
			Declaration { range } => {
				...
				self.bindings.push(SemanticModelBinding { index:binding_id.into(), range });
				self.bindings_by_start.insert(range.start(), binding_id);
				self.bindings_to_references.push(Vec::new());
			},
			Reference { range, declared_at: declaration_at } => {
				...
				let reference_id =
					if let Entry::Vacant(e) = self.references_by_start.entry(range.start()) {
						...
						self.references
							.push(SemanticModelReference { index:reference_id.into(), range });
			...
			UnresolvedVariableReference { range, referenced_operation } => {
				self.unresolved_variable_references
					.push(SemanticModelUnresolvedVariableReference { range, referenced_operation })
			},
		}
	}
