...
use crate::{
	...
	SemanticModelUnresolvedVariableReference,
	UnresolvedVariableReference,
	semantic_model::reference::UnresolvedReference,
};
...
impl From<usize> for SemanticIndex {
	fn from(v:usize) -> Self { SemanticIndex(v) }
}
...
impl SemanticModel {
	...
	pub fn all_bindings(&self) -> impl Iterator<Item = Binding> + '_ {
		self.data
			.bindings
			.iter()
			.map(|x| Binding { data:self.data.clone(), index:x.index })
	}
	...
	pub fn all_references(&self) -> impl Iterator<Item = Reference> + '_ {
		self.data
			.references
			.iter()
			.map(|x| Reference { data:self.data.clone(), index:x.index })
	}
	...
	/// use biome_graphql_semantic::{BindingExtensions, semantic_model};
	...
	pub fn binding(&self, reference:&GraphqlNameReference) -> Option<Binding> {
		...
		self.data.references_to_bindings[*reference_id]
			.iter()
			.map(|&x| Binding { data:self.data.clone(), index:x.into() })
			.next()
	}

	/// Returns the [Binding] of a variable reference.
	/// Since a variable can be referenced in a fragment, which in turn can be
	/// referenced by multiple operations, and then defined in those
	/// operations, this method returns a list of bindings.
	///
	/// ```rust
	/// use biome_graphql_parser::parse_graphql;
	...
	pub fn all_unresolved_references(&self) -> impl Iterator<Item = UnresolvedReference> + '_ {
		(0..self.data.unresolved_references.len())
			.map(move |id| UnresolvedReference { data:self.data.clone(), id })
	}

	/// Returns an iterator of all the unresolved variable references in the
	/// program
	...
	pub fn as_binding(&self, binding:&GraphqlNameBinding) -> Binding {
		...
		Binding { data:self.data.clone(), index:id.into() }
	}
}
