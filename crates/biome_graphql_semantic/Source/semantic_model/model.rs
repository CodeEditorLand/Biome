...
impl SemanticModel {
	...
	pub fn all_bindings(&self) -> impl Iterator<Item = Binding> + '_ {
		self.data.bindings.iter().map(|x| Binding { data: self.data.clone(), index: x.index })
	}

	pub fn all_references(&self) -> impl Iterator<Item = Reference> + '_ {
		self.data.references.iter().map(|x| Reference { data: self.data.clone(), index: x.index })
	}
	...
	pub fn all_unresolved_variable_references(
		...
		self.data.unresolved_variable_references.iter().enumerate().map(move |(id, reference)| {
