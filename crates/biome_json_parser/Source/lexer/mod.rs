		...
		match dispatched {
			...
			UNI => {
				...
				if is_js_id_start(chr) {
					...
					// haven't advanced at all yet, perform the check. At
					// any other position, the BOM is just considered plain
					// whitespace.
					UNICODE_BOM
				} else {
			...
			match dispatch {
				...
				BSL => {
					...
					match self.current_byte() {
						...
						// Handle escaped `'` but only if this is a single quote
						// string. The whole string will be marked as
						// erroneous
						Some(b'\'') if quote == b'\'' => {
							self.advance(1);
						},
				...
				// From the spec:
				// All code points may be placed within the quotation marks
				// except for the code points that must be escaped:
	...
	fn lex_unicode_escape(&mut self) -> Result<(), ParseDiagnostic> {
		...
		for _ in 0..4 {
			match self.current_byte() {
				...
				None => {
					// Reached the end of the file before processing 4 hex
					// digits
					return Err(ParseDiagnostic::new(
