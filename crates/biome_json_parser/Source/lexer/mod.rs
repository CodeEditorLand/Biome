//! An extremely fast, lookup table based, JSON lexer which yields SyntaxKind
//! tokens used by the rome-json parser.
...
impl Token {
	pub fn kind(&self) -> JsonSyntaxKind { self.kind }
...
impl<'src> Lexer<'src> {
	...
	pub fn finish(self) -> Vec<ParseDiagnostic> { self.diagnostics }
	...
	pub(crate) fn next_token(&mut self) -> Option<Token> {
		...
		match self.current_byte() {
			Some(current) => {
				...
				Some(Token { kind, range:TextRange::new(start, self.text_position()) })
			},

			None if self.position == self.source.len() => {
				self.advance(1);

				Some(Token { kind:EOF, range:TextRange::new(start, start) })
			},
					...
					match byte {
						...
						_ => {
							...
							self.diagnostics.push(
								ParseDiagnostic::new(
									"The JSON standard only allows tabs, whitespace, carriage \
	...
	fn current_byte(&self) -> Option<u8> {
		if self.is_eof() { None } else { Some(self.source.as_bytes()[self.position]) }
	}
			...
			state = match self.current_byte() {
				Some(b'0') => {
					...
					match state {
						...
						{
							LexNumberState::Invalid { position, reason:InvalidNumberReason::Octal }
						},
		...
		match state {
			...
			LexNumberState::Invalid { position, reason } => {
				let diagnostic = match reason {
					InvalidNumberReason::Fraction => {
						ParseDiagnostic::new(
							"Invalid fraction part",
							position..position + TextSize::from(1),
						)
					},
					InvalidNumberReason::Exponent => {
						ParseDiagnostic::new(
							"Invalid exponent part",
							position..position + TextSize::from(1),
						)
					},
					InvalidNumberReason::Octal => {
						ParseDiagnostic::new(
							"The JSON standard doesn't allow octal number notation (numbers \
							 starting with zero)",
							position..position + TextSize::from(1),
						)
					},
			...
			match dispatch {
				...
				BSL => {
					...
					match self.current_byte() {
						...
						Some(b'u') => {
							match (self.lex_unicode_escape(), state) {
								(Ok(_), _) => {},
						...
						// Handle escaped `'` but only if this is a single quote string. The whole
						// string will be marked as erroneous
						Some(b'\'') if quote == b'\'' => {
							self.advance(1);
						},
						...
						None => {
							if matches!(state, LexStringState::InString) {
								self.diagnostics.push(
									ParseDiagnostic::new(
										"Expected an escape sequence following a backslash, but \
										 found none",
				...
				ERR | WHS if matches!(state, LexStringState::InString) && chr <= 0x1F => {
					self.diagnostics.push(
						ParseDiagnostic::new(
							format!(
								"Control character '\\u{chr:04x}' is not allowed in string \
								 literals."
							),
	...
	/// Lexes a `\u0000` escape sequence. Assumes that the lexer is positioned
	/// at the `u` token.
	///
		...
		for _ in 0..4 {
			match self.current_byte() {
				...
				Some(_) => {
					...
					.with_detail(
						self.text_position()..self.text_position().add(char.text_len()),
						"Non hexadecimal number",
					)
					.with_hint(
						"A unicode escape sequence must consist of 4 hexadecimal numbers: \
						 `\\uXXXX`, e.g. `\\u002F' for '/'.",
					));
				},
	...
	/// Implements basic lexing of identifiers without support for escape
	/// sequences. This is merely for improved error recovery as identifiers
	...
	/// Lexes a comment. Comments are not supported in JSON but it should yield
...
enum LexNumberState {
	...
	Invalid { reason:InvalidNumberReason, position:TextSize },
}
...
impl KeywordMatcher {
	...
	fn next_character(self, next:u8) -> KeywordMatcher {
		match self {
			KeywordMatcher::MaybeNull(position) => {
				match (next, position) {
			...
			KeywordMatcher::MaybeTrue(position) => {
				match (next, position) {
