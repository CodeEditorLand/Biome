...
pub fn create_analyzer_options(input_file: &Utf8Path, diagnostics: &mut Vec<String>) -> AnalyzerOptions {
	...
	if deserialized.has_errors() {
		diagnostics.extend(
			deserialized
				.into_diagnostics()
				.into_iter()
				.map(|diagnostic| diagnostic_to_string(options_file.file_stem().unwrap(), &json, diagnostic))
				.collect::<Vec<_>>(),
		);
	} else {
		...
		settings.merge_with_configuration(configuration, None).unwrap();

		analyzer_configuration = analyzer_configuration.with_rules(to_analyzer_rules(&settings, input_file));
	}
	options.with_configuration(analyzer_configuration)
}

pub fn create_formatting_options<L>(input_file: &Utf8Path, diagnostics: &mut Vec<String>) -> L::FormatOptions
where
	L: ServiceLanguage,
{
...
pub fn dependency_graph_for_test_file(input_file: &Utf8Path, project_layout: &ProjectLayout) -> Arc<DependencyGraph> {
...
pub fn project_layout_with_node_manifest(input_file: &Utf8Path, diagnostics: &mut Vec<String>) -> Arc<ProjectLayout> {
	...
	if let Ok(json) = std::fs::read_to_string(options_file.clone()) {
		...
		if deserialized.has_errors() {
			...
			project_layout
				.insert_node_manifest(Utf8PathBuf::new(), deserialized.into_deserialized().unwrap_or_default());
...
fn markup_to_string(markup: biome_console::Markup) -> String {
	...
	let mut write = biome_console::fmt::Termcolor(biome_diagnostics::termcolor::NoColor::new(&mut buffer));
...
pub fn register_leak_checker() {
	...
	unsafe extern {
		fn atexit(f: unsafe extern fn()) -> c_int;
	}
