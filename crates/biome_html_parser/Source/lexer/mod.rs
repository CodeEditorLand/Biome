...
impl<'src> HtmlLexer<'src> {
	...
	fn consume_token_embedded_language(
		...
		if self.text_position() != start {
			...
			// we HAVE to consume something, so we start consuming the closing
			// tag.
			self.consume_byte(T![<])
		}
	}
	...
	fn consume_unquoted_string_literal(&mut self) -> HtmlSyntaxKind {
		...
		while let Some(current) = self.current_byte() {
			match current {
				...
				b'\n' | b'\r' | b'\t' | b' ' | b'>' => break,
				// these characters are absolutely invalid in an unquoted
				// attribute value
				b'?' | b'\'' | b'"' | b'=' | b'<' | b'`' => {
	...
	///
	/// This includes text and single spaces between words. If newline or a
	/// second consecutive space is found, this will stop consuming and to
	/// allow the lexer to switch to `consume_whitespace`.
	///
