...
use crate::{
	parser::HtmlParser,
	syntax::parse_error::*,
	token_source::{HtmlEmbededLanguage, HtmlLexContext},
};

const RECOVER_ATTRIBUTE_LIST:TokenSet<HtmlSyntaxKind> = token_set!(T![>], T![<], T![/]);

/// These elements are effectively always self-closing. They should not have a
/// closing tag (if they do, it should be a parsing error). They might not
/// contain a `/` like in `<img />`.
static VOID_ELEMENTS:&[&str] = &[
	"area", "base", "br", "col", "embed", "hr", "img", "input", "link", "meta", "param", "source",
	"track", "wbr",
];

/// For these elements, the content is treated as raw text and no parsing is
/// done inside them. This is so that the contents of these tags can be parsed
...
fn parse_closing_element(p:&mut HtmlParser) -> ParsedSyntax {
	...
	p.bump(T![<]);
	p.bump(T![/]);
	let should_be_self_closing =
		VOID_ELEMENTS.iter().any(|tag| tag.eq_ignore_ascii_case(p.cur_text()));
	if should_be_self_closing {
