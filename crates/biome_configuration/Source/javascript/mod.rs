...
use bpaf::Bpaf;
pub use formatter::{JavascriptFormatter, PartialJavascriptFormatter, partial_javascript_formatter};
...
pub struct JavascriptParser {
	...
	///
	/// These decorators belong to an old proposal, and they are subject to
	/// change.
	#[partial(bpaf(hide))]
...
#[derive(Bpaf, Clone, Copy, Debug, Default, Deserialize, Deserializable, Eq, Merge, PartialEq, Serialize)]
...
impl PartialJavascriptLinter {
	pub fn get_linter_configuration(&self) -> JavascriptLinter {
		JavascriptLinter { enabled:self.enabled.unwrap_or_default() }
	}
}
...
impl PartialJavascriptAssists {
	pub fn get_linter_configuration(&self) -> JavascriptAssists {
		JavascriptAssists { enabled:self.enabled.unwrap_or_default() }
	}
}
