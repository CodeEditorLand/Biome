...
use crate::{
	Configuration, FormatterConfiguration, OverrideFormatterConfiguration, OverrideGlobs, OverridePattern, Overrides,
...
impl EditorConfig {
	pub fn to_biome(mut self) -> (Option<Configuration>, Vec<EditorConfigDiagnostic>) {
		...
		let mut config =
			Configuration { formatter: self.options.remove("*").map(|o| o.to_biome()), ..Default::default() };
		let overrides: Vec<_> = self
			.options
			.into_iter()
			.filter_map(|(k, v)| {
				// Ignore glob patterns that cannot be parsed.
				Some((biome_glob::editorconfig::EditorconfigGlob::try_from(k).ok()?, v))
			})
