...
impl A11y {
	...
	pub(crate) fn get_rule_configuration(
		...
		match rule_name {
			...
			"noDistractingElements" => {
				self.no_distracting_elements.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noHeaderScope" => {
			...
			"noNoninteractiveTabindex" => self
				...
				.map(|conf| (conf.level(), conf.get_options())),
			"noPositiveTabindex" => {
				self.no_positive_tabindex.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noRedundantAlt" => {
			...
			"noSvgWithoutTitle" => {
				self.no_svg_without_title.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"useAltText" => {
			...
			"useAriaActivedescendantWithTabindex" => self
				...
				.map(|conf| (conf.level(), conf.get_options())),
			"useAriaPropsForRole" => {
				self.use_aria_props_for_role.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"useButtonType" => {
			...
			"useFocusableInteractive" => self
				...
				.map(|conf| (conf.level(), conf.get_options())),
			"useGenericFontNames" => {
				self.use_generic_font_names.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"useHeadingContent" => {
			...
			"useSemanticElements" => {
				self.use_semantic_elements.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"useValidAnchor" => {
				self.use_valid_anchor.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"useValidAriaProps" => {
				self.use_valid_aria_props.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"useValidAriaRole" => {
				self.use_valid_aria_role.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"useValidAriaValues" => {
				self.use_valid_aria_values.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"useValidLang" => {
	...
	pub(crate) fn get_rule_configuration(
		...
		match rule_name {
			...
			"noExcessiveNestedTestSuites" => self
				...
				.map(|conf| (conf.level(), conf.get_options())),
			"noExtraBooleanCast" => {
				self.no_extra_boolean_cast.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			...
			"noMultipleSpacesInRegularExpressionLiterals" => self
				...
				.map(|conf| (conf.level(), conf.get_options())),
			"noStaticOnlyClass" => {
				self.no_static_only_class.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noThisInStatic" => {
			...
			"noUselessConstructor" => {
				self.no_useless_constructor.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noUselessEmptyExport" => {
				self.no_useless_empty_export.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noUselessFragments" => {
				self.no_useless_fragments.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noUselessLabel" => {
			...
			"noUselessStringConcat" => self
				...
				.map(|conf| (conf.level(), conf.get_options())),
			"noUselessSwitchCase" => {
				self.no_useless_switch_case.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noUselessTernary" => {
				self.no_useless_ternary.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noUselessThisAlias" => {
				self.no_useless_this_alias.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			...
			"useSimpleNumberKeys" => {
				self.use_simple_number_keys.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
...
impl Correctness {
	...
	pub(crate) fn get_rule_configuration(
		...
		match rule_name {
			...
			"noConstantCondition" => {
				self.no_constant_condition.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noConstantMathMinMaxClamp" => self
				...
				.map(|conf| (conf.level(), conf.get_options())),
			"noConstructorReturn" => {
				self.no_constructor_return.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			...
			"noFlatMapIdentity" => {
				self.no_flat_map_identity.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noGlobalObjectCalls" => {
				self.no_global_object_calls.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noInnerDeclarations" => {
				self.no_inner_declarations.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			...
			"noInvalidDirectionInLinearGradient" => self
				...
				.map(|conf| (conf.level(), conf.get_options())),
			"noInvalidGridAreas" => {
				self.no_invalid_grid_areas.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noInvalidNewBuiltin" => {
				self.no_invalid_new_builtin.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			...
			"noRenderReturnValue" => {
				self.no_render_return_value.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noSelfAssign" => {
			...
			"noStringCaseMismatch" => {
				self.no_string_case_mismatch.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noSwitchDeclarations" => {
				self.no_switch_declarations.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noUndeclaredDependencies" => self
				...
				.map(|conf| (conf.level(), conf.get_options())),
			"noUndeclaredVariables" => {
				self.no_undeclared_variables.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noUnknownFunction" => {
			...
			"noUnmatchableAnbSelector" => self
				...
				.map(|conf| (conf.level(), conf.get_options())),
			"noUnnecessaryContinue" => {
				self.no_unnecessary_continue.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noUnreachable" => {
				self.no_unreachable.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noUnreachableSuper" => {
				self.no_unreachable_super.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noUnsafeFinally" => {
			...
			"useExhaustiveDependencies" => self
				...
				.map(|conf| (conf.level(), conf.get_options())),
			"useHookAtTopLevel" => {
				self.use_hook_at_top_level.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"useImportExtensions" => {
				self.use_import_extensions.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"useIsNan" => self.use_is_nan.as_ref().map(|conf| (conf.level(), conf.get_options())),
			"useJsxKeyInIterable" => {
				self.use_jsx_key_in_iterable.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"useValidForDirection" => {
				self.use_valid_for_direction.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
	...
	pub(crate) fn get_rule_configuration(
		...
		match rule_name {
			...
			"noDuplicateCustomProperties" => self
				...
				.map(|conf| (conf.level(), conf.get_options())),
			"noDuplicateElseIf" => {
				self.no_duplicate_else_if.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noDuplicatedFields" => {
				self.no_duplicated_fields.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			...
			"noIrregularWhitespace" => {
				self.no_irregular_whitespace.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noMissingVarFunction" => {
				self.no_missing_var_function.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noOctalEscape" => {
			...
			"noRestrictedImports" => {
				self.no_restricted_imports.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noRestrictedTypes" => {
			...
			"noUnknownPseudoClass" => {
				self.no_unknown_pseudo_class.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			...
			"useConsistentMemberAccessibility" => self
				...
				.map(|conf| (conf.level(), conf.get_options())),
			"useDeprecatedReason" => {
				self.use_deprecated_reason.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"useImportRestrictions" => {
				self.use_import_restrictions.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"useSortedClasses" => {
			...
			"useValidAutocomplete" => {
				self.use_valid_autocomplete.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			_ => None,
		}
	}
...
impl Performance {
	...
	pub(crate) const GROUP_RULES: &'static [&'static str] =
		&["noAccumulatingSpread", "noBarrelFile", "noDelete", "noReExportAll", "useTopLevelRegex"];
	...
	pub(crate) fn get_rule_configuration(
		...
		match rule_name {
			"noAccumulatingSpread" => {
				self.no_accumulating_spread.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noBarrelFile" => {
...
impl Style {
	...
	pub(crate) fn get_rule_configuration(
		...
		match rule_name {
			...
			"noNonNullAssertion" => {
				self.no_non_null_assertion.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noParameterAssign" => {
				self.no_parameter_assign.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noParameterProperties" => {
				self.no_parameter_properties.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noRestrictedGlobals" => {
				self.no_restricted_globals.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noShoutyConstants" => {
			...
			"useAsConstAssertion" => {
				self.use_as_const_assertion.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"useBlockStatements" => {
				self.use_block_statements.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"useCollapsedElseIf" => {
				self.use_collapsed_else_if.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			...
			"useDefaultSwitchClause" => self
				...
				.map(|conf| (conf.level(), conf.get_options())),
			"useEnumInitializers" => {
				self.use_enum_initializers.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			...
			"useLiteralEnumMembers" => self
				...
				.map(|conf| (conf.level(), conf.get_options())),
			"useNamingConvention" => {
				self.use_naming_convention.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"useNodeAssertStrict" => {
				self.use_node_assert_strict.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"useNodejsImportProtocol" => self
				...
				.map(|conf| (conf.level(), conf.get_options())),
			"useNumberNamespace" => {
				self.use_number_namespace.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"useNumericLiterals" => {
				self.use_numeric_literals.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			...
			"useShorthandArrayType" => self
				...
				.map(|conf| (conf.level(), conf.get_options())),
			"useShorthandAssign" => {
				self.use_shorthand_assign.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			...
			"useThrowOnlyError" => {
				self.use_throw_only_error.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
...
impl Suspicious {
	...
	pub(crate) fn get_rule_configuration(
		...
		match rule_name {
			...
			"noConfusingVoidType" => {
				self.no_confusing_void_type.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			...
			"noDuplicateClassMembers" => self
				...
				.map(|conf| (conf.level(), conf.get_options())),
			"noDuplicateFontNames" => {
				self.no_duplicate_font_names.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noDuplicateJsxProps" => {
				self.no_duplicate_jsx_props.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noDuplicateObjectKeys" => self
				...
				.map(|conf| (conf.level(), conf.get_options())),
			"noDuplicateParameters" => {
				self.no_duplicate_parameters.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noDuplicateSelectorsKeyframeBlock" => self
				...
				.map(|conf| (conf.level(), conf.get_options())),
			"noDuplicateTestHooks" => {
				self.no_duplicate_test_hooks.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noEmptyBlock" => {
			...
			"noMisleadingInstantiator" => self
				...
				.map(|conf| (conf.level(), conf.get_options())),
			"noMisplacedAssertion" => {
				self.no_misplaced_assertion.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noMisrefactoredShorthandAssign" => self
				...
				.map(|conf| (conf.level(), conf.get_options())),
			"noPrototypeBuiltins" => {
				self.no_prototype_builtins.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noReactSpecificProps" => {
				self.no_react_specific_props.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noRedeclare" => {
				self.no_redeclare.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noRedundantUseStrict" => {
				self.no_redundant_use_strict.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
			"noSelfCompare" => {
			...
			"useNamespaceKeyword" => {
				self.use_namespace_keyword.as_ref().map(|conf| (conf.level(), conf.get_options()))
			}
