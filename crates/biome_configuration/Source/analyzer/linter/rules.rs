...
pub struct A11y {
	...
	/// content is accessible to screen readers. Accessible means that it is
	/// not hidden using the aria-hidden prop.
	#[serde(skip_serializing_if = "Option::is_none")]
	...
	/// It detects the use of role attributes in JSX elements and suggests
	/// using semantic elements instead.
	#[serde(skip_serializing_if = "Option::is_none")]
...
impl A11y {
	...
	pub(crate) fn get_rule_configuration(
		...
		match rule_name {
			"noAccessKey" => {
				self.no_access_key.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noAriaHiddenOnFocusable" => {
			...
			"noAutofocus" => {
				self.no_autofocus.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noBlankTarget" => {
				self.no_blank_target.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noDistractingElements" => {
			...
			"noHeaderScope" => {
				self.no_header_scope.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noInteractiveElementToNoninteractiveRole" => {
			...
			"noRedundantAlt" => {
				self.no_redundant_alt.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noRedundantRoles" => {
				self.no_redundant_roles.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noSvgWithoutTitle" => {
			...
			"useAltText" => {
				self.use_alt_text.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useAnchorContent" => {
				self.use_anchor_content.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useAriaActivedescendantWithTabindex" => {
			...
			"useButtonType" => {
				self.use_button_type.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useFocusableInteractive" => {
			...
			"useHeadingContent" => {
				self.use_heading_content.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useHtmlLang" => {
				self.use_html_lang.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useIframeTitle" => {
				self.use_iframe_title.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useKeyWithClickEvents" => {
			...
			"useMediaCaption" => {
				self.use_media_caption.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useSemanticElements" => {
			...
			"useValidAnchor" => {
				self.use_valid_anchor.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useValidAriaProps" => {
			...
			"useValidAriaRole" => {
				self.use_valid_aria_role.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useValidAriaValues" => {
			...
			"useValidLang" => {
				self.use_valid_lang.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			_ => None,
		}
	}
...
impl Complexity {
	...
	pub(crate) fn get_rule_configuration(
		...
		match rule_name {
			"noBannedTypes" => {
				self.no_banned_types.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noEmptyTypeParameters" => {
			...
			"noThisInStatic" => {
				self.no_this_in_static.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noUselessCatch" => {
				self.no_useless_catch.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noUselessConstructor" => {
			...
			"noUselessLabel" => {
				self.no_useless_label.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noUselessLoneBlockStatements" => {
			...
			"noUselessRename" => {
				self.no_useless_rename.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noUselessStringConcat" => {
			...
			"noUselessTernary" => {
				self.no_useless_ternary.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noUselessThisAlias" => {
			...
			"useArrowFunction" => {
				self.use_arrow_function.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useDateNow" => {
				self.use_date_now.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useFlatMap" => {
				self.use_flat_map.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useLiteralKeys" => {
				self.use_literal_keys.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useOptionalChain" => {
				self.use_optional_chain.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useRegexLiterals" => {
				self.use_regex_literals.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useSimpleNumberKeys" => {
...
pub struct Correctness {
	...
	/// Disallow function and var declarations that are accessible outside
	/// their block.
	#[serde(skip_serializing_if = "Option::is_none")]
...
impl Correctness {
	...
	pub(crate) fn get_rule_configuration(
		...
		match rule_name {
			"noChildrenProp" => {
				self.no_children_prop.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noConstAssign" => {
				self.no_const_assign.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noConstantCondition" => {
			...
			"noEmptyPattern" => {
				self.no_empty_pattern.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noFlatMapIdentity" => {
			...
			"noNewSymbol" => {
				self.no_new_symbol.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noNodejsModules" => {
				self.no_nodejs_modules.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noNonoctalDecimalEscape" => {
			...
			"noPrecisionLoss" => {
				self.no_precision_loss.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noRenderReturnValue" => {
			...
			"noSelfAssign" => {
				self.no_self_assign.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noSetterReturn" => {
				self.no_setter_return.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noStringCaseMismatch" => {
			...
			"noUnknownFunction" => {
				self.no_unknown_function.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noUnknownMediaFeatureName" => {
			...
			"noUnknownProperty" => {
				self.no_unknown_property.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noUnknownUnit" => {
				self.no_unknown_unit.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noUnmatchableAnbSelector" => {
			...
			"noUnreachable" => {
				self.no_unreachable.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noUnreachableSuper" => {
			...
			"noUnsafeFinally" => {
				self.no_unsafe_finally.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noUnsafeOptionalChaining" => {
			...
			"noUnusedImports" => {
				self.no_unused_imports.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noUnusedLabels" => {
				self.no_unused_labels.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noUnusedPrivateClassMembers" => {
			...
			"noUnusedVariables" => {
				self.no_unused_variables.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noVoidElementsWithChildren" => {
			...
			"noVoidTypeReturn" => {
				self.no_void_type_return.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useArrayLiterals" => {
				self.use_array_literals.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useExhaustiveDependencies" => {
...
pub struct Nursery {
	...
	/// Disallow the use of overload signatures that are not next to each
	/// other.
	#[serde(skip_serializing_if = "Option::is_none")]
...
impl Nursery {
	...
	pub(crate) fn get_rule_configuration(
		...
		match rule_name {
			"noCommonJs" => {
				self.no_common_js.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noDescendingSpecificity" => {
			...
			"noExportedImports" => {
				self.no_exported_imports.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noIrregularWhitespace" => {
			...
			"noNestedTernary" => {
				self.no_nested_ternary.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noOctalEscape" => {
				self.no_octal_escape.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noProcessEnv" => {
				self.no_process_env.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noRestrictedImports" => {
			...
			"noRestrictedTypes" => {
				self.no_restricted_types.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			...
			"noValueAtRule" => {
				self.no_value_at_rule.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useAdjacentOverloadSignatures" => {
			...
			"useExplicitFunctionReturnType" => {
				self.use_explicit_function_return_type
					.as_ref()
					.map(|conf| (conf.level(), conf.get_options()))
			},
			"useImportRestrictions" => {
			...
			"useSortedClasses" => {
				self.use_sorted_classes.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useStrictMode" => {
				self.use_strict_mode.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useTrimStartEnd" => {
				self.use_trim_start_end.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useValidAutocomplete" => {
...
impl Performance {
	...
	pub(crate) fn get_rule_configuration(
		...
		match rule_name {
			...
			"noBarrelFile" => {
				self.no_barrel_file.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noDelete" => self.no_delete.as_ref().map(|conf| (conf.level(), conf.get_options())),
			"noReExportAll" => {
				self.no_re_export_all.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useTopLevelRegex" => {
				self.use_top_level_regex.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			_ => None,
		}
	}
...
impl Security {
	...
	pub(crate) fn get_rule_configuration(
		...
		match rule_name {
			...
			"noGlobalEval" => {
				self.no_global_eval.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			_ => None,
		}
	}
...
pub struct Style {
	...
	/// This rule recommends a for-of loop when in a for loop, the index used
	/// to extract an item from the iterated array.
	#[serde(skip_serializing_if = "Option::is_none")]
...
impl Style {
	...
	pub(crate) fn get_rule_configuration(
		...
		match rule_name {
			"noArguments" => {
				self.no_arguments.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noCommaOperator" => {
				self.no_comma_operator.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noDefaultExport" => {
				self.no_default_export.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noDoneCallback" => {
				self.no_done_callback.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noImplicitBoolean" => {
				self.no_implicit_boolean.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noInferrableTypes" => {
				self.no_inferrable_types.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noNamespace" => {
				self.no_namespace.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noNamespaceImport" => {
				self.no_namespace_import.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noNegationElse" => {
				self.no_negation_else.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noNonNullAssertion" => {
			...
			"noParameterAssign" => {
				self.no_parameter_assign.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noParameterProperties" => {
			...
			"noShoutyConstants" => {
				self.no_shouty_constants.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noUnusedTemplateLiteral" => {
			...
			"noUselessElse" => {
				self.no_useless_else.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noVar" => self.no_var.as_ref().map(|conf| (conf.level(), conf.get_options())),
			"noYodaExpression" => {
				self.no_yoda_expression.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useAsConstAssertion" => {
			...
			"useExportType" => {
				self.use_export_type.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useFilenamingConvention" => {
			...
			"useFragmentSyntax" => {
				self.use_fragment_syntax.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useImportType" => {
				self.use_import_type.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useLiteralEnumMembers" => {
			...
			"useTemplate" => {
				self.use_template.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useThrowNewError" => {
				self.use_throw_new_error.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useThrowOnlyError" => {
...
pub struct Suspicious {
	...
	/// Checks that the assertion function, for example expect, is placed
	/// inside an it() function call.
	#[serde(skip_serializing_if = "Option::is_none")]
	...
	/// This rule verifies the result of typeof $expr unary expressions is
	/// being compared to valid values, either string literals containing valid
	/// type names or other typeof expressions
	#[serde(skip_serializing_if = "Option::is_none")]
...
impl Suspicious {
	...
	pub(crate) fn get_rule_configuration(
		...
		match rule_name {
			...
			"noArrayIndexKey" => {
				self.no_array_index_key.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noAssignInExpressions" => {
			...
			"noCatchAssign" => {
				self.no_catch_assign.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noClassAssign" => {
				self.no_class_assign.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noCommentText" => {
				self.no_comment_text.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noCompareNegZero" => {
				self.no_compare_neg_zero.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noConfusingLabels" => {
				self.no_confusing_labels.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noConfusingVoidType" => {
			...
			"noConsoleLog" => {
				self.no_console_log.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noConstEnum" => {
				self.no_const_enum.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noControlCharactersInRegex" => {
			...
			"noDebugger" => {
				self.no_debugger.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noDoubleEquals" => {
				self.no_double_equals.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noDuplicateAtImportRules" => {
			...
			"noDuplicateCase" => {
				self.no_duplicate_case.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noDuplicateClassMembers" => {
			...
			"noEmptyBlock" => {
				self.no_empty_block.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noEmptyBlockStatements" => {
			...
			"noEmptyInterface" => {
				self.no_empty_interface.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noEvolvingTypes" => {
				self.no_evolving_types.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noExplicitAny" => {
				self.no_explicit_any.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noExportsInTest" => {
				self.no_exports_in_test.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noExtraNonNullAssertion" => {
			...
			"noFocusedTests" => {
				self.no_focused_tests.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noFunctionAssign" => {
				self.no_function_assign.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noGlobalAssign" => {
				self.no_global_assign.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noGlobalIsFinite" => {
				self.no_global_is_finite.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noGlobalIsNan" => {
				self.no_global_is_nan.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noImplicitAnyLet" => {
				self.no_implicit_any_let.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noImportAssign" => {
				self.no_import_assign.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noImportantInKeyframe" => {
			...
			"noLabelVar" => {
				self.no_label_var.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noMisleadingCharacterClass" => {
			...
			"noRedeclare" => {
				self.no_redeclare.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noRedundantUseStrict" => {
			...
			"noSelfCompare" => {
				self.no_self_compare.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noShadowRestrictedNames" => {
			...
			"noSkippedTests" => {
				self.no_skipped_tests.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noSparseArray" => {
				self.no_sparse_array.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noSuspiciousSemicolonInJsx" => {
			...
			"noThenProperty" => {
				self.no_then_property.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"noUnsafeDeclarationMerging" => {
			...
			"noUnsafeNegation" => {
				self.no_unsafe_negation.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			...
			"useErrorMessage" => {
				self.use_error_message.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useGetterReturn" => {
				self.use_getter_return.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useIsArray" => {
				self.use_is_array.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			"useNamespaceKeyword" => {
			...
			"useValidTypeof" => {
				self.use_valid_typeof.as_ref().map(|conf| (conf.level(), conf.get_options()))
			},
			_ => None,
		}
	}
