...
use biome_analyze::{FixKind, RuleFilter, options::RuleOptions};
use biome_deserialize::{Deserializable, DeserializableType, DeserializableValue, DeserializationDiagnostic, Merge};
...
impl<T:Default + Deserializable> Deserializable for RuleConfiguration<T> {
	fn deserialize(
		...
		if value.visitable_type()? == DeserializableType::Str {
			...
			Deserializable::deserialize(value, rule_name, diagnostics).map(|rule| Self::WithOptions(rule))
		}
	}
}
impl<T:Default> RuleConfiguration<T> {
	pub fn is_disabled(&self) -> bool { matches!(self.level(), RulePlainConfiguration::Off) }
...
impl<T:Default> Default for RuleConfiguration<T> {
	fn default() -> Self { Self::Plain(RulePlainConfiguration::Error) }
}
...
impl<T:Clone + Default + 'static> RuleFixConfiguration<T> {
	pub fn get_options(&self) -> Option<RuleOptions> {
		match self {
			Self::Plain(_) => None,
			Self::WithOptions(options) => Some(RuleOptions::new(options.options.clone(), options.fix)),
		}
	}
}
...
impl<T:Default> RuleAssistConfiguration<T> {
	pub fn is_disabled(&self) -> bool { matches!(self.level(), RuleAssistPlainConfiguration::Off) }
...
impl<T:Default> Default for RuleAssistConfiguration<T> {
	fn default() -> Self { Self::Plain(RuleAssistPlainConfiguration::Off) }
}
...
impl RuleAssistPlainConfiguration {
	pub const fn is_enabled(&self) -> bool { matches!(self, Self::On) }

	pub const fn is_disabled(&self) -> bool { matches!(self, Self::Off) }
}
	...
	fn from_str(selector:&str) -> Result<Self, Self::Err> {
		...
		if let Some((group_name, rule_name)) = selector.split_once('/') {
			...
			match linter::RuleGroup::from_str(selector) {
				...
				Err(_) => Err("This group doesn't exist. Use the syntax `<group>/<rule>` to specify a rule."),
			}
		}
	}
}
...
impl schemars::JsonSchema for RuleSelector {
	fn schema_name() -> String { "RuleCode".to_string() }
