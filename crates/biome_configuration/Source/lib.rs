...
pub use analyzer::{
	LinterConfiguration, RuleConfiguration, RuleFixConfiguration, RulePlainConfiguration, RuleWithFixOptions,
	RuleWithOptions, Rules, linter_configuration,
};
...
use biome_deserialize::{
	...
	DeserializationVisitor, Deserialized, Text, TextRange,
};
...
pub use overrides::{
	OverrideAssistConfiguration, OverrideFormatterConfiguration, OverrideGlobs, OverrideLinterConfiguration,
...
#[derive(Clone, Debug, Default, Deserialize, Eq, PartialEq, Serialize, Bpaf, Deserializable, Merge)]
...
impl Configuration {
	...
	pub fn init() -> Self {
		Self {
			...
			files: Some(FilesConfiguration { ignore_unknown: Some(false.into()), ..Default::default() }),
	...
	pub fn get_assist_actions(&self) -> Actions {
		self.assist.as_ref().map(|f| f.get_actions()).unwrap_or_default()
	}
	...
	pub fn use_editorconfig(&self) -> bool {
		self.formatter.as_ref().is_some_and(|c| c.use_editorconfig_resolved())
	}

	pub fn get_json_linter_configuration(&self) -> JsonLinterConfiguration {
		self.json.as_ref().and_then(|lang| lang.linter.clone()).unwrap_or_default()
	}
				...
				if let Some(captures) = SCHEMA_REGEX.captures(value.text()) {
					if let Some(config_version_match) = captures.get(1) {
						...
						match config_version_str.cmp(&cli_version) {
							Ordering::Less | Ordering::Greater => ctx.report(
								...
								.with_note(
									markup!("Run the command "<Emphasis>"biome migrate"</Emphasis>" to migrate the configuration file."),
								),
							),
							_ => {},
						}
...
impl<'a> Version<'a> {
	...
	fn parse_version(&self) -> Vec<u32> {
		self.0.split('.').filter_map(|part| part.parse::<u32>().ok()).collect()
	}
}
