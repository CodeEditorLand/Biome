...
//!
//! The configuration is divided by "tool", and then it's possible to further
//! customise it by language. The language might further options divided by
//! tool.
pub mod analyzer;
pub mod css;
pub mod diagnostics;
...
pub use javascript::{
	...
	partial_javascript_configuration,
};
pub use json::{
	JsonConfiguration,
	JsonFormatter,
...
use crate::{
	...
	json::JsonLinter,
	organize_imports::{OrganizeImports, PartialOrganizeImports, partial_organize_imports},
	vcs::{PartialVcsConfiguration, VcsConfiguration, partial_vcs_configuration},
};
pub use crate::{
	diagnostics::{BiomeDiagnostic, CantLoadExtendFile},
	generated::{push_to_analyzer_assists, push_to_analyzer_rules},
};
...
pub struct Configuration {
	...
	/// A list of paths to other JSON files, used to extends the current
	/// configuration.
	#[partial(bpaf(hide))]
	...
	#[partial(type, bpaf(external(partial_files_configuration), optional, hide_usage))]
	...
	/// A list of granular patterns that should be applied only to a sub set of
	/// files
	#[partial(bpaf(hide))]
...
impl PartialConfiguration {
	...
	pub fn get_javascript_linter_configuration(&self) -> JavascriptLinter {
		self.javascript
			.as_ref()
			.map(|f| f.linter.as_ref().map(|f| f.get_linter_configuration()).unwrap_or_default())
			.unwrap_or_default()
	}
	...
	pub fn is_vcs_disabled(&self) -> bool { self.vcs.as_ref().map_or(true, |f| f.is_disabled()) }
...
pub struct FilesConfiguration {
	...
	/// Tells Biome to not emit diagnostics when handling files that doesn't
	/// know
	#[partial(bpaf(long("files-ignore-unknown"), argument("true|false"), optional))]
	pub ignore_unknown:bool,

	/// A list of Unix shell style patterns. Biome will ignore files/folders
	/// that will match these patterns.
	#[partial(bpaf(hide))]
	pub ignore:StringSet,

	/// A list of Unix shell style patterns. Biome will handle only those
...
pub struct ConfigurationPayload {
	...
	/// The path of where the `biome.json` or `biome.jsonc` file was found. This
	/// contains the file name.
	pub configuration_file_path:PathBuf,
	/// The base path where the external configuration in a package should be
	/// resolved from
...
pub enum ConfigurationPathHint {
	...
	/// The path will be filled with the working directory if it is not filled
	/// at the time of usage.
	#[default]
	None,

	/// Very similar to [ConfigurationPathHint::None]. However, the path
	/// provided by this variant will be used as **working directory**, which
	/// means that all globs defined in the configuration will use **this
	/// path** as base path.
	FromWorkspace(PathBuf),

	/// The configuration path provided by the LSP, not having a configuration
	/// file is not an error. The path will always be a directory path.
	FromLsp(PathBuf),
	/// The configuration path provided by the user, not having a configuration
	/// file is an error. The path can either be a directory path or a file
	/// path. Throws any kind of I/O errors.
	FromUser(PathBuf),
}

impl ConfigurationPathHint {
	pub const fn is_from_user(&self) -> bool { matches!(self, Self::FromUser(_)) }

	pub const fn is_from_lsp(&self) -> bool { matches!(self, Self::FromLsp(_)) }
}
...
mod test {
	...
	fn resolver_test() {
		...
		let result = resolver
			.resolve(
				env::current_dir().unwrap().canonicalize().unwrap().display().to_string(),
				"example/biome",
			)
			.unwrap();
