...
pub struct VcsConfiguration {
	...
	/// Whether Biome should use the VCS ignore file. When [true], Biome will
	/// ignore the files specified in the ignore file.
	#[partial(bpaf(long("vcs-use-ignore-file"), argument("true|false")))]
	pub use_ignore_file:bool,

	/// The folder where Biome should check for VCS files. By default, Biome
	...
	/// found, Biome won't use the VCS integration, and a diagnostic
	/// will be emitted
...
impl PartialVcsConfiguration {
	pub const fn is_enabled(&self) -> bool { matches!(self.enabled, Some(true)) }

	pub const fn is_disabled(&self) -> bool { !self.is_enabled() }

	pub const fn ignore_file_disabled(&self) -> bool { matches!(self.use_ignore_file, Some(false)) }
}
