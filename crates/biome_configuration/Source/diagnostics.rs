...
impl BiomeDiagnostic {
	...
	pub fn new_invalid_ignore_pattern_with_path(
		...
		Self::InvalidIgnorePattern(InvalidIgnorePattern {
			message: format!("Couldn't parse the pattern \"{}\". Reason: {}", pattern, reason,),
	...
	pub fn invalid_configuration_file(path: &Utf8Path) -> Self {
		Self::InvalidConfigurationFile(InvalidConfigurationFile { path: path.to_string() })
	}

	pub fn no_configuration_file_found(path: &Utf8Path) -> Self {
		Self::NoConfigurationFileFound(NoConfigurationFileFound { path: path.to_string() })
	}
...
mod test {
	...
	fn deserialization_error() {
		let content = "{ \n\n\"formatter\" }";
		let result = deserialize_from_json_str::<Configuration>(content, JsonParserOptions::default(), "");
  ...
  "linter": {
    "rules": {
		...
		let _result = deserialize_from_json_str::<Configuration>(content, JsonParserOptions::default(), "")
			.into_deserialized()
			.unwrap_or_default();
	}
}
