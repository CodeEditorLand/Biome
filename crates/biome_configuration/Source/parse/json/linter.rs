...
use crate::{
	LinterConfiguration,
	RuleConfiguration,
	linter::{RulePlainConfiguration, RuleWithOptions},
};
...
impl<T:Deserializable + Default> Deserializable for RuleConfiguration<T> {
	fn deserialize(
		...
		value.deserialize(RuleConfigurationVisitor(PhantomData), rule_name, diagnostics)
	}
}
...
impl<T:Deserializable + Default> DeserializationVisitor for RuleConfigurationVisitor<T> {
	...
	const EXPECTED_TYPE:DeserializableTypes = DeserializableTypes::STR.union(DeserializableTypes::MAP);
	...
	fn visit_str(
		...
		RulePlainConfiguration::deserialize_from_str(value, range, diagnostics).map(RuleConfiguration::Plain)
	}
	...
	fn visit_map(
		...
		for (key, value) in members.flatten() {
			...
			match key_text.text() {
				...
				"options" => {
					if let Some(options) = Deserializable::deserialize(&value, rule_name, diagnostics) {
