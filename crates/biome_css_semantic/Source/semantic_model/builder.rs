		...
		match event {
			...
			SemanticEvent::RuleEnd => {
				if let Some(completed_rule) = self.current_rule_stack.pop() {
					...
					if has_parent {
						self.range_to_rule.insert(completed_rule.range, completed_rule.clone());
					} else {
						self.range_to_rule.insert(completed_rule.range, completed_rule.clone());
			...
			SemanticEvent::SelectorDeclaration { name, range, original, specificity } => {
			...
			SemanticEvent::PropertyDeclaration { property, value, range } => {
				...
				if let Some(current_rule) = self.current_rule_stack.last_mut() {
					...
					current_rule.declarations.push(CssDeclaration { property, value, range });
				}
			},
			...
			SemanticEvent::AtProperty { property, initial_value, syntax, inherits, range } => {
				self.global_custom_variables.insert(
					property.name.to_string(),
					CssGlobalCustomVariable::AtProperty { property, initial_value, syntax, inherits, range },
				);
			},
		}
	}
