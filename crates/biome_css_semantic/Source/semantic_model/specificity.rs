...
fn evaluate_any_pseudo_class(class:&AnyCssPseudoClass) -> Specificity {
	...
	match class {
				...
				.and_then(|name| evaluate_pseudo_function_selector(name.text()))
			{
				let list_max = selector_list
					...
					.map(|relative_selector| {
						relative_selector.map_or(ZERO_SPECIFICITY, |s| evaluate_any_relative_selector(&s))
					})
					.reduce(|acc, e| acc.max(e))
					.unwrap_or(ZERO_SPECIFICITY);
		...
		AnyCssPseudoClass::CssPseudoClassFunctionSelector(s) => {
			if let Some(base) = s.name().ok().and_then(|name| evaluate_pseudo_function_selector(name.text())) {
				base + s
					.selector()
					.map_or(ZERO_SPECIFICITY, |selector| evaluate_any_selector(&selector))
			} else {
				ZERO_SPECIFICITY
			}
		},
		...
		AnyCssPseudoClass::CssPseudoClassFunctionSelectorList(selector_list) => {
				...
				.and_then(|name| evaluate_pseudo_function_selector(name.text()))
			{
				let list_max = selector_list
					.selectors()
					.iter()
					.map(|selector| selector.map_or(ZERO_SPECIFICITY, |s| evaluate_any_selector(&s)))
					.reduce(|acc, e| acc.max(e))
					.unwrap_or(ZERO_SPECIFICITY);
...
pub fn evaluate_complex_selector(selector:&CssComplexSelector) -> Specificity {
	let left_specificity = selector.left().map_or(ZERO_SPECIFICITY, |s| evaluate_any_selector(&s));

	let right_specificity = selector.right().map_or(ZERO_SPECIFICITY, |s| evaluate_any_selector(&s));
...
fn evaluate_any_relative_selector(selector:&AnyCssRelativeSelector) -> Specificity {
	match selector {
		...
		AnyCssRelativeSelector::CssRelativeSelector(s) => {
			s.selector().map_or(ZERO_SPECIFICITY, |s| evaluate_any_selector(&s))
		},
	}
}
