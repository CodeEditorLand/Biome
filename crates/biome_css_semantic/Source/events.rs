use biome_css_syntax::{
	AnyCssDeclarationName, AnyCssProperty, AnyCssSelector, CssDeclaration, CssPropertyAtRule, CssRelativeSelector,
	CssSyntaxKind::*, CssSyntaxNode,
};
...
impl SemanticEventExtractor {
					...
					match property {
						...
						AnyCssProperty::CssGenericProperty(generic) => {
							...
							let property = match name {
								AnyCssDeclarationName::CssDashedIdentifier(name) => CssProperty::from(name),
								AnyCssDeclarationName::CssIdentifier(name) => CssProperty::from(name),
							};
	...
	fn process_at_property(&mut self, node: CssPropertyAtRule) {
		...
		let Some(decls) = node.block().ok().and_then(|block| block.as_css_declaration_block().cloned()) else {
			return;
		};
	...
	fn add_selector_event(&mut self, node: CssSyntaxNode, specificity: Specificity) {
		self.stash.push_back(SemanticEvent::SelectorDeclaration { node, specificity });
	}
