...
impl Rule for NoDuplicatedFields {
		...
		match operation {
			AnyGraphqlOperationDefinition::GraphqlOperationDefinition(operation) => {
				if let Some(variable_definitions) = operation.variables() {
					duplicated_fields.extend(check_duplicated_variable_definitions(&variable_definitions))
				}
				// We should not check for duplicated selection fields in
				// operation definition, because it is handled in the
	...
	fn diagnostic(_ctx:&RuleContext<Self>, state:&Self::State) -> Option<RuleDiagnostic> {
		let Self::State { text_range, field_type, name } = state;
...
fn check_duplicated_variable_definitions(variable_definitions:&GraphqlVariableDefinitions) -> Vec<DuplicatedField> {
	let mut duplicated_fields = vec![];
