...
impl SuppressionAction for GraphqlSuppressionAction {
	...
	fn find_token_for_inline_suppression(&self, token: GraphqlSyntaxToken) -> Option<ApplySuppression<Self::Language>> {
	...
	fn apply_inline_suppression(
		...
		let ApplySuppression { token_to_apply_suppression, .. } = apply_suppression;
