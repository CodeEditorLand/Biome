...
macro_rules! define_property {
	($id:ident { PROPERTY_TYPE: $property_type:literal,VALUES: $values:expr, }) => {
		#[derive(Debug)]
		struct $id;
		...
		impl AriaPropertyDefinition for $id {
			fn values(&self) -> Iter<&'static str> { $id::VALUES.iter() }

			fn property_type(&self) -> $crate::AriaPropertyTypeEnum {
				// SAFETY: PROPERTY_TYPE is internal and should not contain
