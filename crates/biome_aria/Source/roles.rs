...
pub trait AriaRoleDefinition: Debug {
	...
	/// Given a [aria property](ARIA_PROPERTIES) as input, it checks if it's
	...
	fn is_interactive(&self) -> bool { self.roles().any(|role| *role == "widget") }
...
impl<'a> AriaRoles {
	...
	/// Given a element and attributes, it returns the metadata of the element's
	/// implicit role.
	///
	...
	pub fn get_implicit_role(
		&self,
		element:&str,
		// To generate `attributes`, you can use
		// `biome_js_analyze::services::aria::AriaServices::extract_defined_attributes`
		...
		let result = match element {
			...
			// but this crate does not support checking a descendant of an
			// element.
			//
			...
			"select" => {
				let size = match attributes.get("size") {
					Some(size) => size.first().unwrap_or(&"0".to_string()).parse::<i32>().ok()?,
					None => 0,
				};
			...
			"header" | "footer" => {
				// This crate does not support checking a descendant of an
				// element. header (maybe BannerRole): https://www.w3.org/WAI/ARIA/apg/patterns/landmarks/examples/banner.html
	...
	pub fn is_role_interactive(&self, role:&str) -> bool {
		...
		if let Some(role) = role { role.is_interactive() } else { false }
	}
	...
	pub fn is_not_interactive_element(
		...
		// They are primarily used for graphics and visual rendering. While they
		// can be made interactive with additional attributes and JavaScript,
		// inherently they don't provide user interaction capabilities. Hence,
		// we classify them as non-interactive elements similar to other
		// non-interactive elements like <img> or <progress>.
		//
	...
	/// Given a role, it returns the corresponding elements and attributes
	...
	/// Given an element name and attributes, it returns the role associated
	/// with that element. If no explicit role attribute is present, an
	/// implicit role is returned.
	pub fn get_role_by_element_name(
		...
		attributes
			...
			.map_or_else(|| self.get_implicit_role(element_name, attributes), |r| self.get_role(r))
	}
...
pub trait AriaRoleDefinitionWithConcepts: AriaRoleDefinition {
	fn concepts_by_element_name<'a>(&self, _element_name:&str) -> ElementsAndAttributes<'a> { None }

	fn concepts_by_role<'a>(&self) -> ElementsAndAttributes<'a> { None }
}
...
mod test {
	...
	fn test_get_implicit_role() {
		...
		// No attributes
		let implicit_role = aria_roles.get_implicit_role("button", &FxHashMap::default()).unwrap();
		...
		assert_eq!(implicit_role.type_name(), "biome_aria::roles::SearchboxRole");
