	...
	fn is_property_required(&self, property_to_check: &str) -> bool {
		if is_aria_property_valid(property_to_check) {
			let property_to_check =
				AriaPropertiesEnum::from_str(property_to_check);
			if let Ok(property_to_check) = property_to_check {
				for (property, required) in self.properties() {
					let property =
						AriaPropertiesEnum::from_str(property).unwrap();
...
impl<'a> AriaRoles {
	...
	pub fn get_role(
		...
		let result = match role {
			...
			"graphics-document" => {
				&GraphicsDocumentRole as &dyn AriaRoleDefinition
			},
			...
			"menuitemcheckbox" => {
				&MenuItemCheckboxRole as &dyn AriaRoleDefinition
			},
	...
	pub fn get_implicit_role(
		...
		let result = match element {
			...
			"address" | "details" | "fieldset" => {
				&GroupRole as &dyn AriaRoleDefinition
			},
			"h1" | "h2" | "h3" | "h4" | "h5" | "h6" => {
				&HeadingRole as &dyn AriaRoleDefinition
			},
			"tbody" | "tfoot" | "thead" => {
				&RowGroupRole as &dyn AriaRoleDefinition
			},
			"input" => {
				let type_values = attributes.get("type")?;
				match type_values.first()?.as_str() {
			...
			"b" | "bdi" | "bdo" | "body" | "data" | "div" | "hgroup" | "i"
			| "q" | "samp" | "small" | "span" | "u" | "pre" => {
	...
	pub fn is_not_interactive_element(
		...
		for element in Self::ROLE_WITH_CONCEPTS {
			let role = match *element {
				"checkbox" => {
					&CheckboxRole as &dyn AriaRoleDefinitionWithConcepts
				},
				"radio" => &RadioRole as &dyn AriaRoleDefinitionWithConcepts,
				"option" => &OptionRole as &dyn AriaRoleDefinitionWithConcepts,
				"combobox" => {
					&ComboBoxRole as &dyn AriaRoleDefinitionWithConcepts
				},
				"heading" => {
					&HeadingRole as &dyn AriaRoleDefinitionWithConcepts
				},
				"separator" => {
					&SeparatorRole as &dyn AriaRoleDefinitionWithConcepts
				},
				"button" => &ButtonRole as &dyn AriaRoleDefinitionWithConcepts,
				"article" => {
					&ArticleRole as &dyn AriaRoleDefinitionWithConcepts
				},
				"dialog" => &DialogRole as &dyn AriaRoleDefinitionWithConcepts,
				"cell" => &CellRole as &dyn AriaRoleDefinitionWithConcepts,
				"columnheader" => {
					&ColumnHeaderRole as &dyn AriaRoleDefinitionWithConcepts
				},
				"definition" => {
					&DefinitionRole as &dyn AriaRoleDefinitionWithConcepts
				},
				"figure" => &FigureRole as &dyn AriaRoleDefinitionWithConcepts,
				"form" => &FormRole as &dyn AriaRoleDefinitionWithConcepts,
				"graphics-document" => {
					&GraphicsDocumentRole as &dyn AriaRoleDefinitionWithConcepts
				},
				"graphics-object" => {
					&GraphicsObjectRole as &dyn AriaRoleDefinitionWithConcepts
				},
				"grid" => &GridRole as &dyn AriaRoleDefinitionWithConcepts,
				"gridcell" => {
					&GridCellRole as &dyn AriaRoleDefinitionWithConcepts
				},
				...
				"listbox" => {
					&ListBoxRole as &dyn AriaRoleDefinitionWithConcepts
				},
				"listitem" => {
					&ListItemRole as &dyn AriaRoleDefinitionWithConcepts
				},
				"navigation" => {
					&NavigationRole as &dyn AriaRoleDefinitionWithConcepts
				},
				"row" => &RowRole as &dyn AriaRoleDefinitionWithConcepts,
				"rowgroup" => {
					&RowGroupRole as &dyn AriaRoleDefinitionWithConcepts
				},
				"rowheader" => {
					&RowHeaderRole as &dyn AriaRoleDefinitionWithConcepts
				},
				"search" => {
					&SearchboxRole as &dyn AriaRoleDefinitionWithConcepts
				},
				"searchbox" => {
					&SearchboxRole as &dyn AriaRoleDefinitionWithConcepts
				},
				...
				"textbox" => {
					&TextboxRole as &dyn AriaRoleDefinitionWithConcepts
				},
				"generic" => {
					&GenericRole as &dyn AriaRoleDefinitionWithConcepts
				},
				"caption" => {
					&CaptionRole as &dyn AriaRoleDefinitionWithConcepts
				},
				...
				"aside" => {
					&ComplementaryRole as &dyn AriaRoleDefinitionWithConcepts
				},
				"blockquote" => {
					&BlockQuoteRole as &dyn AriaRoleDefinitionWithConcepts
				},
				"associationlist" => {
					&AssociationListRole as &dyn AriaRoleDefinitionWithConcepts
				},
				"status" => &StatusRole as &dyn AriaRoleDefinitionWithConcepts,
				"contentinfo" => {
					&ContentInfoRole as &dyn AriaRoleDefinitionWithConcepts
				},
				"region" => &RegionRole as &dyn AriaRoleDefinitionWithConcepts,
				_ => return false,
			};
			if let Some(mut concepts) =
				role.concepts_by_element_name(element_name)
			{
	...
	pub fn get_role_by_element_name(
		...
		attributes.get("role").and_then(|role| role.first()).map_or_else(
	...
	pub fn is_not_static_element(
		...
		match role_name.map(|role| role.type_name()) {
			Some(
				"biome_aria::roles::PresentationRole"
				| "biome_aria::roles::GenericRole",
			) => false,
			Some(_) => true,
			None => false,
		}
...
type ElementsAndAttributes<'a> =
	Option<Iter<'a, (&'a str, &'a [(&'a str, &'a str)])>>;
...
mod test {
	...
	fn test_get_implicit_role() {
		...
		attributes.insert("type".to_string(), vec!["search".to_string()]);
		let implicit_role =
			aria_roles.get_implicit_role("input", &attributes).unwrap();
		assert_eq!(
		...
		let implicit_role =
			aria_roles.get_implicit_role("select", &attributes).unwrap();
