...
impl AriaRoles {
		...
		Some(match elt.name()?.as_ref() {
			...
			"input" => {
					...
					.map(|v| v.as_ref())
				{
					...
					Some("search") => {
						match elt.find_attribute_by_name(|n| n == "list") {
			...
			// Should we remove it?
			"a" | "area" | "link" => {
				match elt.find_attribute_by_name(|n| n == "href") {
	...
	pub fn is_not_interactive_element(&self, element:&impl Element) -> bool {
		match element.name().as_ref().map(|name| name.as_ref()) {
			...
			// They are primarily used for graphics and visual rendering. While they can be made
			// interactive with additional attributes and JavaScript, inherently they don't
			// provide user interaction capabilities. Hence, we classify them as non-interactive
			// elements similar to other non-interactive elements like <img> or <progress>.
			//
			...
			_ => {
				self.get_implicit_role(element)
	...
	pub fn is_not_static_element(&self, element:&impl Element) -> bool {
		match element.name().as_ref().map(|name| name.as_ref()) {
			...
			_ => {
				match self.get_role_by_element_name(element) {
