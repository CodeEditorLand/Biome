...
pub fn matches_path(optional_node: Option<&JsonMemberName>, path: &[&str]) -> bool {
	...
	if !node.inner_string_text().is_ok_and(|text| Some(&text.text()) == path.last()) {
		return false;
	}
	...
	let optional_parent_node = node
		.syntax()
		.parent()
		.and_then(|p| if JsonMember::can_cast(p.kind()) { p.parent() } else { None })
		.and_then(|p| if JsonMemberList::can_cast(p.kind()) { p.parent() } else { None })
		.and_then(|p| if JsonObjectValue::can_cast(p.kind()) { p.parent() } else { None })
