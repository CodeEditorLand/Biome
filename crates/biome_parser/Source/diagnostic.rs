		...
		for advice_kind in &self.advice_list {
			match advice_kind {
				...
				ParserAdviceKind::List(message, list) => {
					...
					let list: Vec<_> = list.iter().map(|suggestion| suggestion as &dyn Display).collect();
...
pub fn expect_one_of(names: &[&str], range: TextRange) -> ParseDiagnostic {
	ParseDiagnostic::new("Unexpected value or character.", range).with_alternatives("Expected one of:", names)
}
...
pub fn merge_diagnostics(first: Vec<ParseDiagnostic>, second: Vec<ParseDiagnostic>) -> Vec<ParseDiagnostic> {
	if first.is_empty() {
		return second;
	}
	...
	loop {
		match (current_first, current_second) {
			(Some(first_item), Some(second_item)) => {
				let (first, second) = match (first_item.diagnostic_range(), second_item.diagnostic_range()) {
