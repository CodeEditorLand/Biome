...
use biome_diagnostics::{
	console::{fmt::Display, markup, MarkupBuf},
	location::AsSpan,
	Advices,
	Diagnostic,
	Location,
...
use crate::{token_source::TokenSource, Parser};
...
/// 2. a list of details, useful to give more information and context around the
///    error
...
/// The structure of the advice. A message that gives details, a possible range
/// so the diagnostic is able to highlight the part of the code we want to
/// explain.
#[derive(Clone, Debug)]
...
impl ParserAdvice {
	fn add_detail(&mut self, message:impl Display, range:impl AsSpan) {
		self.advice_list.push(ParserAdviceKind::Detail(ParserAdviceDetail {
	...
	fn add_hint_with_alternatives(&mut self, message:impl Display, alternatives:&[impl Display]) {
		self.advice_list.push(ParserAdviceKind::List(
			...
			alternatives.iter().map(|msg| markup! {{msg}}.to_owned()).collect(),
		))
	}
}
		...
		for advice_kind in &self.advice_list {
			match advice_kind {
				...
				ParserAdviceKind::List(message, list) => {
					...
					let list:Vec<_> =
						list.iter().map(|suggestion| suggestion as &dyn Display).collect();
...
impl ParseDiagnostic {
	...
	pub fn new_with_any(names:&[&str], range:TextRange, p:&impl Parser) -> Self {
		...
		let msg = if p.source().text().text_len() <= range.start() {
			...
			format!("Expected {} but instead found '{}'.", joined_names, p.text(range))
		};
	...
	/// Use this API if you want to highlight more code frame, to help to
	/// explain where's the error.
	///
	...
	/// A message that also allows to list of alternatives in case a fixed range
	/// of values/characters are expected.
	///
	/// ## Examples
	///
	/// ```
	...
	/// ```
	pub fn with_alternatives(mut self, message:impl Display, alternatives:&[impl Display]) -> Self {
		self.advice.add_hint_with_alternatives(message, alternatives);
		self
	}

	/// Retrieves the range that belongs to the diagnostic
	pub(crate) fn diagnostic_range(&self) -> Option<&TextRange> { self.span.as_ref() }
}
...
impl<P:Parser> ToDiagnostic<P> for ParseDiagnostic {
	fn into_diagnostic(self, _:&P) -> ParseDiagnostic { self }
}
...
where
	K: SyntaxKind, {
	ExpectedToken(token.to_string().expect("Expected token to be a punctuation or keyword."))
}
...
pub fn expected_token_any<K:SyntaxKind>(tokens:&[K]) -> ExpectedTokens {
	...
	for (index, token) in tokens.iter().enumerate() {
		...
		let _ = write!(
			&mut expected,
			"'{}'",
			token.to_string().expect("Expected token to be a punctuation or keyword.")
		);
	}
	...
	P: Parser,
{
	fn into_diagnostic(self, p:&P) -> ParseDiagnostic {
		if p.cur() == P::Kind::EOF {
			p.err_builder(format!("expected `{}` but instead the file ends", self.0), p.cur_range())
...
/// Creates a diagnostic saying that any of the nodes in `names` was expected at
/// range
pub fn expected_any(names:&[&str], range:TextRange, p:&impl Parser) -> ParseDiagnostic {
	ParseDiagnostic::new_with_any(names, range, p)
}

/// Creates a diagnostic with message "Unexpected value." and then it lists the
/// values that should be expected.
...
/// Merges two lists of parser diagnostics. Only keeps the error from the first
/// collection if two start at the same range.
///
	...
	loop {
		match (current_first, current_second) {
			(Some(first_item), Some(second_item)) => {
				let (first, second) =
					match (first_item.diagnostic_range(), second_item.diagnostic_range()) {
						(Some(first_range), Some(second_range)) => {
							match first_range.start().cmp(&second_range.start()) {
								...
								Ordering::Equal => {
									// Only keep one error, skip the one from the
									// second list.
