...
pub trait Lexer<'src> {
	...
	fn current_char_unchecked(&self) -> char {
		...
		unsafe {
			let Some(chr) = self.source().get_unchecked(self.position()..self.source().len()).chars().next() else {
	...
	Lex: LexerWithCheckpoint<'l>,
{
	...
	pub fn nth_non_trivia(&mut self, n: usize) -> Option<LookaheadToken<Lex::Kind>> {
		...
		if let Some(lookahead_token) = self.lookahead.get_non_trivia_checkpoint(n - 1).map(LookaheadToken::from) {
