...
/// Structure for converting events to a syntax tree representation, while
...
where
	...
	/// Signal that the sink must generate an EOF token when its finishing. See
	/// [LosslessTreeSink::finish] for more details.
	needs_eof:bool,
	...
	Factory: SyntaxFactory<Kind = L::Kind>,
{
	...
	fn token(&mut self, kind:L::Kind, end:TextSize) { self.do_token(kind, end); }
	...
	fn errors(&mut self, errors:Vec<ParseDiagnostic>) { self.errors = errors; }
}
	...
	Factory: SyntaxFactory<Kind = L::Kind>,
{
	...
	///
	/// If tree is finished without a [biome_rowan::SyntaxKind::EOF], one will
	/// be generated and all pending trivia will be appended to its leading
	/// trivia.
	...
	fn do_token(&mut self, kind:L::Kind, token_end:TextSize) {
		...
		// Every trivia up to the token (including line breaks) will be the leading
		// trivia
