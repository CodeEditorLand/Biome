...
impl Marker {
	pub fn new(pos:u32, start:TextSize) -> Marker {
		Marker {
			...
			bomb:DebugDropBomb::new(
				"Marker must either be `completed` or `abandoned` to avoid that children are \
				 implicitly attached to a marker's parent.",
			),
		}
	}
	...
	where
		...
		if idx == p.context().events.len() - 1 {
			if let Some(Event::Start { forward_parent: None, kind }) = p.context_mut().events.pop()
			{
...
impl CompletedMarker {
	pub fn new(start_pos:u32, finish_pos:u32, offset:TextSize) -> Self {
		CompletedMarker { start_pos, offset, old_start:start_pos, finish_pos }
	}
	...
	where
		...
		match p.context_mut().events[idx] {
			Event::Start { ref mut forward_parent, .. } => {
				// Safety: The new marker is always inserted after the start marker of this
				// node, thus subtracting the two positions can never be 0.
	...
	where
		...
		match events[start_idx] {
			Event::Start { ref mut kind, forward_parent: None } => *kind = P::Kind::TOMBSTONE,
			_ => unreachable!(),
		}
