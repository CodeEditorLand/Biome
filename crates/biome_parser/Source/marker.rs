...
impl Marker {
		...
		P: Parser,
	{
		...
		if idx == p.context().events.len() - 1 {
			if let Some(Event::Start { forward_parent: None, kind }) =
...
impl CompletedMarker {
	...
	where
		P: Parser,
	{
		let end = p.context().events
			[self.old_start as usize..self.finish_pos as usize]
			.iter()
			.rev()
		...
		P: Parser,
	{
		...
		match p.context_mut().events[idx] {
			Event::Start { ref mut forward_parent, .. } => {
				...
				*forward_parent = Some(
					NonZeroU32::try_from(new_pos.pos - self.start_pos).unwrap(),
				);
			},
			_ => unreachable!(),
		}
		...
		P: Parser,
	{
		...
		match events[start_idx] {
			Event::Start { ref mut kind, forward_parent: None } => {
