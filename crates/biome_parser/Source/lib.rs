...
impl<K: SyntaxKind> ParserContext<K> {
	...
	pub fn checkpoint(&self) -> ParserContextCheckpoint {
		ParserContextCheckpoint { event_pos: self.cur_event_pos(), errors_pos: self.diagnostics.len() as u32 }
	}
	...
	fn error(&mut self, err: impl ToDiagnostic<Self>) {
		...
		if let Some(previous) = self.context().diagnostics.last() {
			match (&err.diagnostic_range(), &previous.diagnostic_range()) {
				(Some(err_range), Some(previous_range)) if err_range.start() == previous_range.start() => {
					return;
				},
				_ => {},
			}
...
impl AnyParse {
		...
		L: Language + 'static,
	{
		self.root
			.clone()
			.into_node()
			.unwrap_or_else(|| panic!("could not downcast root node to language {}", type_name::<L>()))
	}
