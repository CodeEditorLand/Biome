...
impl Trivia {
	...
	pub fn offset(&self) -> TextSize { self.range.start() }

	/// Returns `true` if this is the trailing trivia of a non-trivia token or
	/// false otherwise.
	pub fn trailing(&self) -> bool { self.trailing }

	/// Returns the text range of this trivia
	pub fn text_range(&self) -> TextRange { self.range }
}
...
pub trait TokenSource {
	...
	/// Returns the byte offset of the current token from the start of the
	/// source document
...
where
	...
	/// The worst case is a document where every character is its own token.
	/// This would result in `u32::MAX` tokens
	pub trivia_len:u32,
}
	...
	K: SyntaxKind,
{
	...
	pub fn current_start(&self) -> TextSize { self.lexer_checkpoint.current_start() }

	pub fn trivia_position(&self) -> usize { self.trivia_len as usize }
}
