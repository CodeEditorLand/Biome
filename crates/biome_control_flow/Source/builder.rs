...
impl<L:Language> FunctionBuilder<L> {
	...
	pub fn append_block(&mut self) -> BlockId {
		let index = self.result.blocks.len().try_into().expect("BlockId overflow");
	...
	pub fn cursor(&self) -> BlockId { self.block_cursor }
	...
	pub fn pop_exception_target(&mut self) { self.exception_target.pop(); }
