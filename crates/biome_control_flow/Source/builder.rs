...
use crate::{
	BasicBlock, ControlFlowGraph, ExceptionHandler, ExceptionHandlerKind,
...
impl<L: Language> FunctionBuilder<L> {
	...
	pub fn append_block(&mut self) -> BlockId {
		let index =
			self.result.blocks.len().try_into().expect("BlockId overflow");
	...
	pub fn push_exception_target(
		&mut self,
		kind: ExceptionHandlerKind,
		target: BlockId,
	) {
		self.exception_target.push(ExceptionHandler { kind, target });
	}
	...
	pub fn append_finally_fallthrough(
		&mut self,
		block: BlockId,
	) -> InstructionBuilder<L> {
		self.append_instruction(InstructionKind::Jump {
