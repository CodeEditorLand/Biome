...
use biome_service::workspace::{
	DocumentFileSource, FeaturesBuilder, OpenProjectParams, SupportsFeatureParams, UpdateSettingsParams,
};
...
impl<'a> SpecTestFile<'a> {
	pub fn try_from_file(
		...
		let project_key = app
			.workspace
			.open_project(OpenProjectParams { path: BiomePath::new(""), open_uninitialized: true })
			.unwrap();
