...
impl<'a> SpecTestFile<'a> {
	...
	pub fn input_code(&self) -> &str { &self.input_code }

	pub fn file_name(&self) -> &str { self.input_file.file_name().unwrap().to_str().unwrap() }

	pub fn input_file(&self) -> &BiomePath { &self.input_file }
	...
	L: TestFormatLanguage,
{
	...
	fn formatted(&self, parsed:&AnyParse, format_language:L::FormatLanguage) -> (String, Printed) {
	...
	pub fn test(self) {
		...
		if options_path.exists() {
			...
			// SAFETY: we checked its existence already, we assume we have
			// rights to read it
			...
			// There are some logs that print different line endings, and we
			// can't snapshot those otherwise we risk automatically having
			// them replaced with LF by git.
			//
	...
	fn test_file(&self) -> &SpecTestFile { &self.test_file }
}
