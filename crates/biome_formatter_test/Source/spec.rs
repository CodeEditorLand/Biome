...
use biome_service::workspace::{
	DocumentFileSource, FeaturesBuilder, RegisterProjectFolderParams,
	...
	L: TestFormatLanguage,
{
	...
	fn formatted(
		...
		let output_code = match range {
			(Some(_), Some(_)) => {
				...
				output_code.replace_range(
					Range::<usize>::from(range),
					formatted.as_code(),
				);
	...
	pub fn test(self) {
		...
		let (output_code, printed) =
			self.formatted(&parsed, self.format_language.clone());
		...
		if options_path.exists() {
			...
			let (test_options, diagnostics) =
				deserialize_from_str::<PartialConfiguration>(
			...
			let (mut output_code, printed) =
				self.formatted(&parsed, format_language.clone());

			let max_width =
				format_language.options().line_width().value() as usize;
