...
async function traverseDir(dir, input_config) {
	...
	for (const entry of await fs.readdir(dir, { withFileTypes: true })) {
		...
		if (entry.isFile()) {
			...
			const snapOriginalFile =
				path.basename(file) + ".prettier-snap-original";
			...
			if (key in snapshot) {
				...
				const optionsStart = snapshotContent.match(
					new RegExp(OPTIONS + "\\n"),
				);
				const optionsEnd = snapshotContent.match(
					new RegExp("\\n" + INPUT),
				);
				const optionsStartOffset =
					optionsStart.index + optionsStart[0].length;
				const optionsEndOffset = optionsEnd.index;
				const optionsContent = snapshotContent.substring(
					optionsStartOffset,
					optionsEndOffset,
				);
				...
				const rangeOptions = {
					rangeStart: Number(
						optionsContent.match(
							new RegExp(/rangeStart: (\d+)/),
						)?.[1] ?? 0,
					),
					rangeEnd: Number(
						optionsContent.match(
							new RegExp(/rangeEnd: (\d+)/),
						)?.[1] ?? Number.POSITIVE_INFINITY,
					),
				};

				// extract output string
				const outputStart = snapshotContent.match(
					new RegExp(OUTPUT + "\\n"),
				);
				const outputEnd = snapshotContent.match(
					new RegExp("\\n" + FOOTER),
				);
				...
				snapshotContent = snapshotContent.substring(
					outputStartOffset,
					outputEndOffset,
				);
				...
				try {
					...
					snapshotContent = await prettier.format(
						snapshotContent,
						updatedOptions,
					);
				} catch (error) {
					console.error(
						`Prettier format error in ${filePath}: ${error}`,
					);
				}
				...
				if (originalSnapshot !== snapshotContent) {
					...
					await fs.writeFile(
						path.resolve(outDir, snapOriginalFile),
						originalSnapshot,
					);
				}
