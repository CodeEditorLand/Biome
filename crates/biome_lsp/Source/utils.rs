	...
	for op in diff.iter() {
		match op {
			...
			CompressedOp::DiffOp(DiffOp::Insert { range }) => {
				...
				let last_edit = result
					.last_mut()
					.filter(|text_edit| text_edit.range.end == start && text_edit.new_text.is_empty());
			...
			CompressedOp::DiffOp(DiffOp::Delete { range }) => {
				...
				result.push(lsp::TextEdit { range: lsp::Range::new(start, end), new_text: String::new() });
			},
...
pub(crate) fn diagnostic_to_lsp<D: Diagnostic>(
	...
	let span = if let Some(offset) = offset {
		TextRange::new(span.start().add(TextSize::from(offset)), span.end().add(TextSize::from(offset)))
	} else {
		span
	};
	...
	let code_description = diagnostic.category().and_then(|category| category.link()).and_then(|link| {
