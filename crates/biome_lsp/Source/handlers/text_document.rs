...
pub(crate) async fn did_change(
	...
	let old_text =
		session.workspace.get_file_content(GetFileContentParams { path: biome_path.clone() })?;
	tracing::trace!("old document: {:?}", old_text);
	tracing::trace!("content changes: {:?}", params.content_changes);

	let text =
		apply_document_changes(session.position_encoding(), old_text, params.content_changes);

	tracing::trace!("new document: {:?}", text);

	session.insert_document(url.clone(), Document::new(version, &text));

	session.workspace.change_file(ChangeFileParams { path: biome_path, version, content: text })?;
...
pub(crate) async fn did_close(
	...
	session.workspace.close_file(CloseFileParams { path: biome_path })?;

	session.remove_document(&url);

	let diagnostics = vec![];
	let version = None;
	session.client.publish_diagnostics(url, diagnostics, version).await;
