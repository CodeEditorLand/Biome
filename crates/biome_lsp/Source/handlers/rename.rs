...
use crate::{converters::from_proto, diagnostics::LspError, session::Session, utils};
...
pub(crate) fn rename(
	...
	let result = session.workspace.rename(biome_service::workspace::RenameParams {
		path:biome_path,
	...
	changes.insert(url, utils::text_edit(&doc.line_index, result.indels, position_encoding, None)?);
