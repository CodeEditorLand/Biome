...
use crate::{
	converters::{from_proto, to_proto},
...
pub(crate) fn format(
	...
	if file_features.supports_format() {
		debug!("Formatting...");
		let printed =
			session.workspace.format_file(FormatFileParams { path:biome_path.clone() })?;
		...
		let range =
			Range { start:Position::default(), end:Position { line:num_lines, character:0 } };

		let edits = vec![TextEdit { range, new_text:output }];
...
pub(crate) fn format_range(
	...
	if file_features.supports_format() {
		...
		let format_range = from_proto::text_range(&doc.line_index, params.range, position_encoding)
			.with_context(|| {
				format!("failed to convert range {:?} in document {url}", params.range.end)
			})?;
		let content = session
			.workspace
			.get_file_content(GetFileContentParams { path:biome_path.clone() })?;
		...
		let formatted = session
			.workspace
			.format_range(FormatRangeParams { path:biome_path, range:format_range })?;

		// Recalculate the actual range that was reformatted from the formatter
		// result
		let formatted_range = match formatted.range() {
			...
			None => {
				Range {
					...
					end:Position { line:doc.line_index.len(), character:0 },
				}
			},
		};

		Ok(Some(vec![TextEdit { range:formatted_range, new_text:formatted.into_code() }]))
	} else {
		notify_user(file_features, biome_path)
	}
}
...
pub(crate) fn format_on_type(
	...
	if file_features.supports_format() {
		...
		let formatted = session
			.workspace
			.format_on_type(FormatOnTypeParams { path:biome_path, offset })?;
		...
		let formatted_range = match formatted.range() {
			Some(range) => {
				...
				Range { start:start_loc, end:end_loc }
			},
			None => {
				Range {
					start:Position::default(),
