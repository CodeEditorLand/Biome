...
use crate::{diagnostics::LspError, session::Session, utils::text_edit};
...
pub(crate) fn format_range(
	...
	if file_features.supports_format() {
		...
		let formatted = session
			.workspace
			.format_range(FormatRangeParams { path:biome_path.clone(), range:format_range })?;
...
pub(crate) fn format_on_type(
	...
	if file_features.supports_format() {
		...
		let formatted = session
			.workspace
			.format_on_type(FormatOnTypeParams { path:biome_path.clone(), offset })?;
		...
		let edits = text_edit(&doc.line_index, indels, position_encoding, Some(offset.into()))?;
