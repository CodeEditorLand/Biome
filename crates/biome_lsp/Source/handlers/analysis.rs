...
use crate::{
	converters::{from_proto, line_index::LineIndex},
...
pub(crate) fn code_actions(
	...
	let mut actions:Vec<_> = result
		...
		.filter_map(|action| {
			...
			// Don't apply unsafe fixes when the code action is on-save
			// quick-fixes
			if has_quick_fix && action.suggestion.applicability == Applicability::MaybeIncorrect {
				return None;
			}
			// Filter out source.organizeImports.biome action when organize
