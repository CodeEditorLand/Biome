use std::{borrow::Cow, collections::HashMap, ffi::OsStr, ops::Sub};

use anyhow::{Context, Result};
use biome_analyze::{ActionCategory, RuleCategoriesBuilder, SourceActionKind};
use biome_diagnostics::Applicability;
use biome_fs::BiomePath;
use biome_lsp_converters::{from_proto, line_index::LineIndex};
...
use crate::{diagnostics::LspError, session::Session, utils};
...
/// Queries the [`AnalysisServer`] for code actions of the file matching its
/// path
///
...
pub(crate) fn code_actions(
	...
	let fix_all = if has_fix_all {
		fix_all(session, &url, biome_path.clone(), &doc.line_index, &diagnostics, offset)?
	} else {
		None
	};
	...
	let mut actions:Vec<_> = result
		...
		.filter_map(|action| {
			...
			// Filter out source.organizeImports.biome action when organize imports is not
			// supported.
			...
			let matches_filters = filters.iter().any(|filter| {
				trace!("Filter {:?}, category {:?}", filter, action.category.to_str());
...
fn fix_all(
	...
	let diagnostics = diagnostics
		...
		.filter_map(|d| {
			...
			if has_matching_rule { Some(d.clone()) } else { None }
		})
		.collect();
