...
impl From<WorkspaceError> for LspError {
	fn from(value:WorkspaceError) -> Self { Self::WorkspaceError(value) }
}

impl From<biome_diagnostics::Error> for LspError {
	fn from(value:biome_diagnostics::Error) -> Self { Self::Error(value) }
}

impl From<anyhow::Error> for LspError {
	fn from(value:Error) -> Self { Self::Anyhow(value) }
}
...
/// Receives an error coming from a LSP query, and converts it into a JSON-RPC
/// error.
///
...
pub(crate) async fn handle_lsp_error<T>(
	...
	match err {
		LspError::WorkspaceError(err) => {
			match err {
				// diagnostics that shouldn't raise an hard error, but send a
				// message to the user
