...
use crate::{
	...
	diagnostics::{LspError, handle_lsp_error},
	handlers,
	requests,
	requests::syntax_tree::{SYNTAX_TREE_REQUEST, SyntaxTreePayload},
	session::{
		CapabilitySet,
		CapabilityStatus,
		ClientInformation,
...
impl LanguageServer for LSPServer {
	// The `root_path` field is deprecated, but we still read it so we can print a
	// warning about it
	#[allow(deprecated)]
	#[tracing::instrument(
        level = "trace",
	...
	async fn initialized(&self, params:InitializedParams) {
		...
		// Diagnostics are disabled by default, so update them after fetching workspace
		// config
