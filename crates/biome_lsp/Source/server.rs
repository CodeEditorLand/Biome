...
use crate::session::{CapabilitySet, CapabilityStatus, ClientInformation, Session, SessionHandle, SessionKey};
...
use biome_service::workspace::{
	CloseProjectParams, OpenProjectParams, RageEntry, RageParams, RageResult, ServiceDataNotification,
};
...
impl LSPServer {
	fn new(
		...
		Self { session, sessions, stop_on_disconnect, is_initialized }
	}
	...
	async fn rage(&self, params: RageParams) -> LspResult<RageResult> {
		...
		let RageResult { entries: workspace_entries } = self.session.failsafe_rage(params);
		...
		if let Ok(sessions) = self.sessions.lock() {
			if sessions.len() > 1 {
				...
				for (key, session) in sessions.iter() {
					...
					let RageResult { entries: workspace_entries } = session.failsafe_rage(params);
	...
	async fn setup_capabilities(&self) {
		...
		capabilities.add_capability(
			...
			if let Some(base_path) = self.session.base_path() {
				CapabilityStatus::Enable(Some(json!(DidChangeWatchedFilesRegistrationOptions {
					watchers: vec![
						FileSystemWatcher {
							glob_pattern: GlobPattern::String(format!("{}/biome.json", base_path.as_str())),
							kind: Some(WatchKind::all()),
						},
						FileSystemWatcher {
							glob_pattern: GlobPattern::String(format!("{}/biome.jsonc", base_path.as_str())),
							kind: Some(WatchKind::all()),
						},
						FileSystemWatcher {
							glob_pattern: GlobPattern::String(format!("{}/.editorconfig", base_path.as_str())),
							kind: Some(WatchKind::all()),
						}
					],
				})))
		...
		capabilities.add_capability(
			"biome_rename",
			"textDocument/rename",
			if rename { CapabilityStatus::Enable(None) } else { CapabilityStatus::Disable },
		);

		self.session.register_capabilities(capabilities).await;
	}

	async fn map_op_error<T>(&self, result: Result<Result<Option<T>, LspError>, PanicError>) -> LspResult<Option<T>> {
		match result {
...
impl LanguageServer for LSPServer {
	...
	async fn initialize(&self, params: InitializeParams) -> LspResult<InitializeResult> {
		...
		self.session.initialize(
			...
			params
				.client_info
				.map(|client_info| ClientInformation { name: client_info.name, version: client_info.version }),
			params.root_uri,
	...
	async fn initialized(&self, params: InitializedParams) {
		...
		self.session.client.log_message(MessageType::INFO, msg).await;
	...
	async fn did_change_watched_files(&self, params: DidChangeWatchedFilesParams) {
		let file_paths = params.changes.iter().map(|change| change.uri.to_file_path());
	...
	async fn did_open(&self, params: DidOpenTextDocumentParams) {
		handlers::text_document::did_open(&self.session, params).await.ok();
	}

	async fn did_change(&self, params: DidChangeTextDocumentParams) {
		handlers::text_document::did_change(&self.session, params).await.ok();
	}

	async fn did_close(&self, params: DidCloseTextDocumentParams) {
		handlers::text_document::did_close(&self.session, params).await.ok();
	}
		...
		for removed in &params.event.removed {
				...
				.and_then(|project_path| self.session.project_for_path(&project_path))
			{
				...
				if let Err(err) = result {
					...
					self.session.client.log_message(MessageType::ERROR, err).await;
				}
			}
		}
		...
		for added in &params.event.added {
			if let Ok(project_path) = self.session.file_path(&added.uri) {
				let result = self
					.session
					.workspace
					.open_project(OpenProjectParams { path: project_path.clone(), open_uninitialized: true })
					.map_err(into_lsp_error);

				match result {
					Ok(project_key) => {
						self.session.insert_and_scan_project(project_key, project_path.clone());
	...
	async fn formatting(&self, params: DocumentFormattingParams) -> LspResult<Option<Vec<TextEdit>>> {
		let result =
			biome_diagnostics::panic::catch_unwind(move || handlers::formatting::format(&self.session, params));
	...
	async fn range_formatting(&self, params: DocumentRangeFormattingParams) -> LspResult<Option<Vec<TextEdit>>> {
		let result =
			biome_diagnostics::panic::catch_unwind(move || handlers::formatting::format_range(&self.session, params));
		self.map_op_error(result).await
	}

	async fn on_type_formatting(&self, params: DocumentOnTypeFormattingParams) -> LspResult<Option<Vec<TextEdit>>> {
		let result =
			biome_diagnostics::panic::catch_unwind(move || handlers::formatting::format_on_type(&self.session, params));
...
impl Drop for LSPServer {
	fn drop(&mut self) {
		if let Ok(mut sessions) = self.sessions.lock() {
			...
			if self.stop_on_disconnect && sessions.is_empty() && self.is_initialized.load(Ordering::Relaxed) {
