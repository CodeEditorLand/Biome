...
impl LineIndex {
	pub fn new(text:&str) -> LineIndex {
		...
		for (offset, char) in text.char_indices() {
			...
			if char == '\n' {
				// SAFETY: the conversion from `usize` to `TextSize` can fail if
				// `offset` is larger than 2^32. We don't support such large
				// files.
	...
	pub(crate) fn len(&self) -> u32 { self.newlines.len().try_into().unwrap_or(u32::MAX) }
		...
		if let Some(wide_chars) = self.line_wide_chars.get(&line) {
			for c in wide_chars {
				if c.end <= col {
					...
					// From here on, all utf16 characters come *after* the
					// character we are mapping, so we don't need to take
					// them into account
					break;
				}
			}
