...
use crate::{
	...
	documents::Document,
	extension_settings::{CONFIGURATION_SECTION, ExtensionSettings},
	utils,
};
...
pub(crate) struct Session {
	...
	/// A flag to notify a message to the user when the configuration is broken,
	/// and the LSP attempts to update the diagnostics
								...
								if let ConfigurationPathHint::FromWorkspace(path) = &base_path {
									...
									self.workspace
										.register_project_folder(RegisterProjectFolderParams {
											...
											// This is naive, but we don't know if the user has a
											// file already open or not, so we register every
											// project as the current one. The correct
											// one is actually set when the LSP calls
	...
	pub(crate) fn is_linting_and_formatting_disabled(&self) -> bool {
		match self.configuration_status() {
			...
			ConfigurationStatus::Missing => {
				self.extension_settings.read().unwrap().requires_configuration()
			},
