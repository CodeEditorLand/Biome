...
use crate::{
	lexer::CssReLexContext,
	state::CssParserState,
	token_source::{CssTokenSource, CssTokenSourceCheckpoint},
};
...
impl CssParserOptions {
	...
	pub fn is_css_modules_disabled(&self) -> bool { !self.css_modules }

	/// Checks if parsing of Grit metavariables is enabled.
	pub fn is_metavariable_enabled(&self) -> bool { self.grit_metavariables }
}
...
impl<'source> CssParser<'source> {
	...
	pub fn options(&self) -> &CssParserOptions { &self.options }
	...
	pub fn re_lex(&mut self, context:CssReLexContext) -> CssSyntaxKind { self.source_mut().re_lex(context) }

	pub(crate) fn state(&self) -> &CssParserState { &self.state }

	pub(crate) fn state_mut(&mut self) -> &mut CssParserState { &mut self.state }
