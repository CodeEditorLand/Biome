//! An extremely fast, lookup table based, СSS lexer which yields SyntaxKind
//! tokens used by the rome-css parser.
#[rustfmt::skip]
mod tests;
...
impl LexContext for CssLexContext {
	...
	fn is_regular(&self) -> bool { matches!(self, CssLexContext::Regular) }
}
			...
			Some(current) => {
				match context {
					...
					CssLexContext::PseudoNthSelector => {
						self.consume_pseudo_nth_selector_token(current)
					},
...
impl<'src> CssLexer<'src> {
	...
	pub(crate) fn with_options(self, options:CssParserOptions) -> Self { Self { options, ..self } }
	...
	fn current_char_unchecked(&self) -> char { self.char_unchecked_at(0) }

	/// Peek the UTF8 char which starts at the current byte
	///
	/// ## Safety
	/// Must be called at a valid UT8 char boundary
	fn peek_char_unchecked(&self) -> char { self.char_unchecked_at(1) }
	...
	fn char_unchecked_at(&self, offset:usize) -> char {
		...
		let string = unsafe {
			std::str::from_utf8_unchecked(
				self.source.as_bytes().get_unchecked((self.position + offset)..),
			)
		};
		let chr = if let Some(chr) = string.chars().next() {
			chr
		} else {
			// Safety: we always call this when we are at a valid char, so this
			// branch is completely unreachable
			unsafe {
	...
	fn consume_token(&mut self, current:u8) -> CssSyntaxKind {
		// The speed difference comes from the difference in table size, a 2kb
		// table is easily fit into cpu cache While a 16kb table will be
		// ejected from cache very often leading to slowdowns, this also allows
		// LLVM to do more aggressive optimizations on the match regarding how
		// to map it to instructions
	...
	fn consume_color(&mut self) -> CssSyntaxKind {
		...
		let mut length = 0;
		while matches!(self.current_byte(), Some(b'0'..=b'9' | b'a'..=b'f' | b'A'..=b'F')) {
			self.advance(1);
			length += 1;
		}
	...
	/// Consumes a Unicode range token and returns its corresponding syntax
	/// kind.
	...
	///
	/// This method reads consecutive bytes representing valid hexadecimal
	/// characters ('0'-'9', 'a'-'f', 'A'-'F') or the wildcard character '?'.
	...
	fn consume_unicode_range(&mut self) -> CssSyntaxKind {
		...
		while matches!(self.current_byte(), Some(b'0'..=b'9' | b'a'..=b'f' | b'A'..=b'F' | b'?')) {
			// If the current byte is a wildcard character, set the wildcard
			// flag to true.
		...
		if length == 0 || length > 6 {
			let diagnostic =
				ParseDiagnostic::new("Invalid unicode range", start..self.text_position())
					.with_hint(
		...
		while let Some(chr) = self.current_byte() {
			...
			match dispatch {
				...
				BSL if self.is_valid_escape_at(1) => {
					// We can escape any character, so we just skip over the
					// escape sequence Even a closing parenthesis (PNC
					// token): url(https://example.com/ima\)ge.png);
			...
			match dispatch {
				...
				BSL => {
					...
					match self.current_byte() {
						...
						// Handle escaped `'` but only if this is a end quote
						// string.
						Some(b'\'') if quote == b'\'' => {
							self.advance(1);
						},
	...
	fn consume_escape_sequence(&mut self, current:u8) -> char {
		...
		for _ in 0..5 {
			let Some(digit) = self
				.current_byte()
				.and_then(|c| if c.is_ascii_hexdigit() { (c as char).to_digit(16) } else { None })
			else {
				break;
			};
			self.advance(1);
		...
		// zero, or is for a surrogate, or is greater than the maximum allowed
	...
	fn consume_number(&mut self, current:u8) -> CssSyntaxKind {
		...
		// According to the spec if the next 2 input code points are U+002E FULL
		// STOP (.) followed by a digit we need to consume them. However we
		// want to parse numbers like `1.` and `1.e10` where we don't have a
		// number after (.) If the next input code points are U+002E FULL STOP
		// (.)...
		...
		// If the next 2 or 3 input code points are U+0045 LATIN CAPITAL LETTER
		// E (E) or U+0065 LATIN SMALL LETTER E (e), optionally followed by
		// U+002D HYPHEN-MINUS (-) or U+002B PLUS SIGN (+), followed by a
		// digit, then:
	...
	/// Consumes a sequence of identifier characters from a byte stream,
	/// appending them to the provided buffer in lowercase ASCII form.
	///
	/// This function iteratively processes bytes from the stream, which are
	/// part of an identifier, and appends their lowercase ASCII representation
	/// to the buffer. It stops processing either when the buffer is full or
	/// when a non-identifier character is encountered.
	///
	/// # Arguments
	///
	/// * `buf` - A mutable reference to a byte array where the identifier
	///   characters will be appended. This buffer should be pre-allocated and
	///   have enough space to hold the expected identifier.
	///
	/// # Returns
	///
	/// A tuple containing:
	...
	/// * A boolean indicating whether only ASCII characters were used (`true`
	///   if so).
	///
	/// # Panics
	///
	/// This function will panic if the first character to be consumed is not a
	/// valid start of an identifier, as determined by `self.is_ident_start()`.
		...
		while let Some(current) = self.current_byte() {
			if let Some(part) = self.consume_ident_part(current) {
				...
				if only_ascii_used {
					...
					// Since we're only dealing with ASCII, we need at most 1
					// byte.
	...
	///
	/// Before calling this function, you should make sure that there is a valid
	/// identifier start using [Self::is_ident_start].
	///
	/// Also handles CSS escape sequences in identifiers and attach appropriate
	/// diagnostics for invalid cases.
	///
	/// Returns the consumed character wrapped in `Some` if it is part of an
	/// identifier, and `None` if it is not.
	fn consume_ident_part(&mut self, current:u8) -> Option<char> {
		let chr = match lookup_byte(current) {
			IDT | MIN | DIG | ZER => {
				self.advance(1);
				// SAFETY: We know that the current byte is a hyphen or a
				// number.
				current as char
			},
			// name code point
			UNI => {
				// SAFETY: We know that the current byte is a valid unicode code
				// point
			...
			// If the first and second code points are a valid escape, continue
			// consume. Otherwise, break.
			// BSL if self.is_valid_escape_at(1) => '\\',
			BSL if self.is_valid_escape_at(1) => {
				let escape_start = self.text_position();
				self.advance(1);
	...
	fn consume_slash(&mut self) -> CssSyntaxKind {
		...
		match self.peek_byte() {
			Some(b'*') => {
				...
				if has_newline { MULTILINE_COMMENT } else { COMMENT }
			},
		...
		match self.current_byte() {
			Some(b'+' | b'-') => {
				match self.peek_byte() {
					...
					// (.) and the third code point is a digit, return
					// true.
					Some(b'.') if self.byte_at(2).map_or(false, |byte| byte.is_ascii_digit()) => {
						true
					},
					_ => false,
				}
				...
				match lookup_byte(next) {
					MIN => {
						...
						match lookup_byte(next) {
							...
							// If the third code point is a name-start code
							// point return true.
							UNI => is_css_non_ascii(self.char_unchecked_at(2)),
							// or the third and fourth code points are a valid
							// escape return true.
