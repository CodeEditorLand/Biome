...
impl<'src> CssLexer<'src> {
	...
	fn char_unchecked_at(&self, offset:usize) -> char {
		// Precautionary measure for making sure the unsafe code below does not read
		// over memory boundary
		debug_assert!(!self.is_eof());

		self.assert_at_char_boundary(offset);

		// Safety: We know this is safe because we require the input to the lexer to be
		// valid utf8 and we always call this when we are at a char
		let string =
			unsafe { std::str::from_utf8_unchecked(self.source.as_bytes().get_unchecked((self.position + offset)..)) };

		let chr = if let Some(chr) = string.chars().next() {
			chr
		} else {
			// Safety: we always call this when we are at a valid char, so this branch is
	...
	fn consume_token(&mut self, current:u8) -> CssSyntaxKind {
		// The speed difference comes from the difference in table size, a 2kb table is
		// easily fit into cpu cache While a 16kb table will be ejected from cache
		// very often leading to slowdowns, this also allows LLVM to do more
		// aggressive optimizations on the match regarding how to map it to instructions
	...
	fn consume_unicode_range(&mut self) -> CssSyntaxKind {
		...
		if length == 0 || length > 6 {
			let diagnostic = ParseDiagnostic::new("Invalid unicode range", start..self.text_position())
				.with_hint("Valid length (minimum 1 or maximum 6 hex digits) in the start of unicode range.");
	...
	fn consume_string_literal(&mut self, quote:u8) -> CssSyntaxKind {
		...
		while let Some(chr) = self.current_byte() {
			...
			match dispatch {
					...
					match self.current_byte() {
						...
						Some(c) if c.is_ascii_hexdigit() => {
							...
							if hex == REPLACEMENT_CHARACTER {
								...
								let diagnostic =
									ParseDiagnostic::new("Invalid escape sequence", escape_start..self.text_position());
				...
				WHS if matches!(chr, b'\n' | b'\r') => {
					let unterminated = ParseDiagnostic::new("Missing closing quote", start..self.text_position())
		...
		match state {
			...
			LexStringState::InString => {
				let unterminated = ParseDiagnostic::new("Missing closing quote", start..self.text_position())
					.with_detail(self.source.text_len()..self.source.text_len(), "file ends here");
	...
	fn consume_number(&mut self, current:u8) -> CssSyntaxKind {
		...
		// According to the spec if the next 2 input code points are U+002E FULL STOP
		// (.) followed by a digit we need to consume them. However we want to parse
		// numbers like `1.` and `1.e10` where we don't have a number after (.) If the
		// next input code points are U+002E FULL STOP (.)...
	...
	fn consume_slash(&mut self) -> CssSyntaxKind {
		...
		match self.peek_byte() {
			Some(b'*') => {
				...
				let err = ParseDiagnostic::new("Unterminated block comment", start..self.text_position())
	...
	fn consume_lss(&mut self) -> CssSyntaxKind {
		...
		if self.peek_byte() == Some(b'!') && self.byte_at(2) == Some(b'-') && self.byte_at(3) == Some(b'-') {
			self.advance(4);
