...
use crate::{
	...
	parser::CssParser,
	syntax::{
		css_modules::{composes_not_allowed, expected_classes_list, expected_composes_import_source},
		is_at_any_value,
		is_at_dashed_identifier,
		is_at_identifier,
		is_at_string,
		parse_any_value,
...
#[inline]
pub(crate) fn is_at_any_property(p:&mut CssParser) -> bool { is_at_generic_property(p) }
...
///
/// This function determines if the parser is currently positioned at a
/// `composes` property, which is indicated by the presence of the `composes`
/// keyword followed by a colon (`:`).
#[inline]
fn is_at_composes_property(p:&mut CssParser) -> bool { p.at(T![composes]) && p.nth_at(1, T![:]) }

/// Parses a `composes` property in CSS Modules.
///
/// This function parses a `composes` property, which is used in CSS Modules to
/// compose classes from other modules. If the current parser position is not at
/// a `composes` property, it returns `Absent`. If CSS Modules are disabled,
/// it generates a diagnostic error and falls back to parsing a generic
/// property.
///
/// Basic usage in CSS:
/// ```css
...
impl ParseNodeList for ComposesClassList {
	...
	///
	/// This function parses an identifier as a custom identifier because it is
	/// a selector, which is case-sensitive. For more information, see:
	...
	fn is_at_list_end(&self, p:&mut Self::Parser<'_>) -> bool { p.at_ts(END_OF_COMPOSES_CLASS_TOKEN_SET) }
...
#[inline]
fn is_at_generic_property(p:&mut CssParser) -> bool { is_at_identifier(p) && p.nth_at(1, T![:]) }
...
impl ParseNodeList for GenericComponentValueList {
	...
	fn parse_element(&mut self, p:&mut Self::Parser<'_>) -> ParsedSyntax { parse_generic_component_value(p) }
...
#[inline]
fn is_at_generic_component_value(p:&mut CssParser) -> bool { is_at_any_value(p) || is_at_generic_delimiter(p) }
...
#[inline]
fn is_at_generic_delimiter(p:&mut CssParser) -> bool { p.at_ts(GENERIC_DELIMITER_SET) }
