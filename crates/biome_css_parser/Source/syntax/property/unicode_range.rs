...
use crate::{
	lexer::{CssLexContext, CssReLexContext},
	parser::CssParser,
};
...
/// Checks if the parser is positioned to potentially start parsing a Unicode
/// range in CSS, identified by "U" or "u".
...
///
/// This function first performs lexical analysis to confirm the presence of a
/// Unicode range indicator before proceeding to parse the range or fallback to
/// a bogus value.
///
/// Specification:
...
/// ```
///
/// This function is integral in parsing and interpreting Unicode range
/// descriptors in CSS.
pub(crate) fn parse_unicode_range(p:&mut CssParser) -> ParsedSyntax {
	...
	// Perform lexical analysis to confirm the presence of a Unicode range
	// indicator. This is necessary to ensure that the parser is positioned to
	// parse a Unicode range.
	let kind = p.re_lex(CssReLexContext::UnicodeRange);

	// If the parser is not positioned to parse a Unicode range, return an absent
	// value.
	if kind != T![U+] {
		return Absent;
	}
	...
	// A wildcard cannot be combined with a range interval. For example,
	// `U+????-U+????` is invalid.
	...
	// A range interval is identified by a hyphen (`-`) followed by another Unicode
	// codepoint.
	if p.at(T![-]) {
		...
		// Abandon the range if the parser is not positioned to parse a Unicode
		// codepoint.

		if parse_unicode_codepoint(p).is_absent() {
			// If the parser is positioned to parse a Unicode range wildcard add a
			// diagnostic.
			if parse_unicode_range_wildcard(p)
				.add_diagnostic_if_present(p, wildcard_not_allowed)
				.is_none()
			{
				// If the parser is not positioned to parse a Unicode codepoint, add a
				// diagnostic.
...
fn is_at_unicode_codepoint(p:&mut CssParser) -> bool { p.at(CSS_UNICODE_CODEPOINT_LITERAL) }
...
fn is_at_unicode_range_wildcard(p:&mut CssParser) -> bool { p.at(CSS_UNICODE_RANGE_WILDCARD_LITERAL) }
...
fn parse_unicode_range_wildcard(p:&mut CssParser) -> ParsedSyntax {
	...
	p.bump_with_context(CSS_UNICODE_RANGE_WILDCARD_LITERAL, CssLexContext::UnicodeRange);
...
/// Wildcards (`U+????`) are only valid at the beginning of a Unicode range
/// descriptor. When specifying a range interval (`U+XXXX-YYYY`), wildcards
/// cannot be used in the second position.
pub(crate) fn wildcard_not_allowed(p:&CssParser, range:TextRange) -> ParseDiagnostic {
	p.err_builder("Codepoint range wildcard is not valid here.", range).with_hint(
		"Wildcards (`U+????`) are only allowed at the beginning of a Unicode range descriptor. When specifying a \
		 range interval (`U+XXXX-YYYY`), wildcards cannot be used in the second position.",
	)
}

/// Generates a parse diagnostic for an expected "codepoint" error message at
/// the given range.
pub(crate) fn expected_codepoint(p:&CssParser, range:TextRange) -> ParseDiagnostic {
	expected_node("codepoint", range, p).with_hint("Expected a valid Unicode codepoint (e.g., U+1234).")
}

/// Generates a parse diagnostic for an expected "codepoint or wildcard" error
/// message at the given range.
