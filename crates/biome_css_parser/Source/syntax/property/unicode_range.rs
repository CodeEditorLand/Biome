...
pub(crate) fn parse_unicode_range(p:&mut CssParser) -> ParsedSyntax {
	...
	// If the parser is not positioned to parse a Unicode range, return an
	// absent value.
	if kind != T![U+] {
		return Absent;
	}
	...
	// A range interval is identified by a hyphen (`-`) followed by another
	// Unicode codepoint.
	if p.at(T![-]) {
		...
		if parse_unicode_codepoint(p).is_absent() {
			// If the parser is positioned to parse a Unicode range wildcard add
			// a diagnostic.
			if parse_unicode_range_wildcard(p)
				.add_diagnostic_if_present(p, wildcard_not_allowed)
				.is_none()
			{
				// If the parser is not positioned to parse a Unicode codepoint,
				// add a diagnostic.
...
pub(crate) fn wildcard_not_allowed(p:&CssParser, range:TextRange) -> ParseDiagnostic {
	p.err_builder("Codepoint range wildcard is not valid here.", range).with_hint(
		"Wildcards (`U+????`) are only allowed at the beginning of a Unicode range descriptor. \
		 When specifying a range interval (`U+XXXX-YYYY`), wildcards cannot be used in the second \
		 position.",
	)
}
...
pub(crate) fn expected_codepoint_value(p:&CssParser, range:TextRange) -> ParseDiagnostic {
	expected_any(&["codepoint", "codepoint range wildcard"], range, p).with_hint(
		"Expected a valid Unicode codepoint (e.g., U+1234) or a codepoint range wildcard (e.g., \
		 U+????).",
	)
}
