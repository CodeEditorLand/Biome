...
use crate::{
	...
	syntax::{
		...
		selector::{SelectorList, eat_or_recover_selector_function_close_token},
	},
};
...
pub(crate) fn parse_pseudo_class_function_selector_list(p:&mut CssParser) -> ParsedSyntax {
	...
	let kind = if eat_or_recover_selector_function_close_token(p, list, expected_selector) && !list_range.is_empty() {
