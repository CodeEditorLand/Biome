...
use crate::{
	...
	parser::CssParser,
	syntax::{
		parse_error::{expected_any_pseudo_class_nth, expected_number},
		parse_number,
		parse_regular_number,
		selector::{SelectorList, eat_or_recover_selector_function_close_token, recover_selector_function_parameter},
	},
};
...
const PSEUDO_CLASS_FUNCTION_NTH_CLASS_IDENTIFIER_SET:TokenSet<CssSyntaxKind> = token_set![T![odd], T![even]];

const PSEUDO_CLASS_FUNCTION_NTH_CLASS_SIGN_SET:TokenSet<CssSyntaxKind> = token_set![T![+], T![-]];

const PSEUDO_CLASS_FUNCTION_NTH_CLASS_SET:TokenSet<CssSyntaxKind> = PSEUDO_CLASS_FUNCTION_NTH_CLASS_IDENTIFIER_SET
	.union(PSEUDO_CLASS_FUNCTION_NTH_CLASS_SIGN_SET)
	.union(token_set![T![n], CSS_DIMENSION_VALUE, CSS_NUMBER_LITERAL]);

#[inline]
fn is_at_pseudo_class_nth_selector(p:&mut CssParser) -> bool { p.at_ts(PSEUDO_CLASS_FUNCTION_NTH_CLASS_SET) }
