...
use crate::{
	parser::CssParser,
	syntax::{
		css_modules::{CSS_MODULES_SCOPE_SET, local_or_global_not_allowed},
		parse_error::expected_selector,
		selector::{eat_or_recover_selector_function_close_token, parse_selector, recover_selector_function_parameter},
	},
};
...
/// of a `:local` or `:global` pseudo-class function selector, which is part of
/// the CSS Modules syntax.
#[inline]
...
///
/// This function parses a pseudo-class function selector, specifically `:local`
/// or `:global`, in CSS Modules. If the `css.parser.cssModules` option is not
/// enabled, it generates a diagnostic error and skips the selector. ```css
/// :local(.className) {
///     color: red;
/// }
