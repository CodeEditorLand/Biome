...
use biome_parser::{
	Parser,
	TokenSet,
	diagnostic::{ParseDiagnostic, expected_any},
	parsed_syntax::{
		ParsedSyntax,
...
use crate::{
	...
	syntax::{
		...
		selector::{eat_or_recover_selector_function_close_token, recover_selector_function_parameter},
	},
};
...
pub(crate) fn parse_pseudo_class_function_identifier(p:&mut CssParser) -> ParsedSyntax {
	...
	let kind = if is_at_dir_parameter_identifier(p) {
		...
		if eat_or_recover_selector_function_close_token(p, identifier, expected_dir_parameter_identifier) {
...
#[inline]
fn is_at_dir_parameter_identifier(p:&mut CssParser) -> bool { p.at_ts(DIR_PARAMETER_IDENTIFIER_SET) }
