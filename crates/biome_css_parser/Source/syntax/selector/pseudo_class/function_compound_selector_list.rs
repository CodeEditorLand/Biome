use biome_css_syntax::{
	CssSyntaxKind,
	CssSyntaxKind::{CSS_COMPOUND_SELECTOR_LIST, CSS_PSEUDO_CLASS_FUNCTION_COMPOUND_SELECTOR_LIST, *},
	T,
};
use biome_parser::{
	Parser,
	...
	parse_recovery::{RecoveryError, RecoveryResult},
	parsed_syntax::{
		ParsedSyntax,
...
use crate::{
	...
	syntax::{
		...
		selector::{eat_or_recover_selector_function_close_token, parse_compound_selector},
	},
};
...
#[inline]
pub(crate) fn parse_pseudo_class_function_compound_selector_list(p:&mut CssParser) -> ParsedSyntax {
...
impl ParseSeparatedList for CompoundSelectorList {
	...
	fn parse_element(&mut self, p:&mut CssParser) -> ParsedSyntax { parse_compound_selector(p) }

	fn is_at_list_end(&self, p:&mut CssParser) -> bool { p.at(T![')']) }
	...
	fn recover(&mut self, p:&mut CssParser, parsed_element:ParsedSyntax) -> RecoveryResult {
		match parsed_element.or_add_diagnostic(p, expected_compound_selector) {
			Some(m) => Ok(m),
			// we don't need to recover here, because we have a better diagnostic message in a close
			// token
