...
pub(crate) fn parse_selector(p:&mut CssParser) -> ParsedSyntax {
	...
	if is_nth_at_metavariable(p, 0) {
		...
		// while complex selectors are compound selectors separated by
		// combinators. After parsing the compound selector, it then checks if
		// this compound selector is a part of a complex selector.
...
fn parse_namespace_prefix(p:&mut CssParser) -> ParsedSyntax {
	...
	let kind = if p.eat(T![*]) {
		...
		// we don't need to check if the identifier is valid, because we already
		// did that
