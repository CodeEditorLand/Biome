...
use biome_css_syntax::{CssSyntaxKind, CssSyntaxKind::*, T, TextRange};
use biome_parser::{
	CompletedMarker,
	Parser,
	ParserProgress,
...
use crate::{
	...
	syntax::{
		...
		parse_error::{expected_any_sub_selector, expected_compound_selector, expected_identifier, expected_selector},
		parse_identifier,
		parse_regular_identifier,
		selector::{
			attribute::parse_attribute_selector,
			nested_selector::NestedSelectorList,
			pseudo_class::parse_pseudo_class_selector,
			pseudo_element::parse_pseudo_element_selector,
		},
	},
};
...
/// current context should be treated as a regular context or a
/// selector-specific context. The distinction is important for handling
/// whitespaces, especially around combinators in CSS selectors.
...
impl SelectorList {
	/// Configures the `SelectorList` with a specified token set to indicate the
	/// end of the selector list.
	///
	/// This method allows setting a custom `TokenSet<CssSyntaxKind>` that
	/// determines the tokens which mark the end of a selector list. It can be
	/// used to extend or modify the default behavior of the selector list
	/// parsing.
	...
	/// Configures the `SelectorList` with a specified set of tokens for error
	/// recovery.
	///
	/// This method allows setting a custom `TokenSet<CssSyntaxKind>` which is
	/// used for identifying points at which the parser can attempt to recover
	/// from errors during the parsing process. The specified token set
	/// represents tokens that the parser will recognize as potential recovery
	/// points.
	...
	///
	/// By default, the parser might attempt to recover from errors encountered
	/// while parsing the selector list. This method disables such recovery,
	/// which can be useful in scenarios where we want to implement a custom
	/// recovery.
...
impl ParseSeparatedList for SelectorList {
	...
	fn parse_element(&mut self, p:&mut Self::Parser<'_>) -> ParsedSyntax { parse_selector(p) }

	fn is_at_list_end(&self, p:&mut Self::Parser<'_>) -> bool { p.at_ts(self.end_kind_ts) }
	...
	fn recover(&mut self, p:&mut Self::Parser<'_>, parsed_element:ParsedSyntax) -> RecoveryResult {
		if parsed_element.is_absent() && self.is_recovery_disabled {
			...
			parsed_element.or_recover(p, &SelectorListParseRecovery::new(self.recovery_ts), expected_selector)
		}
	}
...
impl SelectorListParseRecovery {
	fn new(recovery_ts:TokenSet<CssSyntaxKind>) -> Self { SelectorListParseRecovery { recovery_ts } }
}
...
impl ParseRecovery for SelectorListParseRecovery {
	...
	/// Determines if the parser is at a point where it can recover from an
	/// error while parsing a selector list.
	///
	/// This method checks if the parser is currently positioned at a token that
	/// indicates a safe point to resume parsing after encountering an error in
	/// a selector list. The recovery points are identified by `recovery_ts`, a
	/// comma, the start of a new selector, or a preceding line break.
	/// # CSS Examples
	///
	/// - Recovery at `recovery_ts` (e.g., `{`): ```css .class1, { } /*       ^
	///   */ /*       is a recovery point */ ```
	///
	/// - Recovery at Comma (`,`): ```css .class1, "string", div {} /* ^ */ /*
	///   is a recovery point */ ```
	///
	/// - Recovery at New Selector (`is_nth_at_selector`): ```css .class1,
	///   "string" div {} /*                ^                   */ /* is a
	///   recovery point */ ```
	///
	/// - Recovery at Line Break (`has_nth_preceding_line_break`): ```css
	///   .class1 123123 /*new line is a recovery point*/  color: red; } ```
	fn is_at_recovered(&self, p:&mut Self::Parser<'_>) -> bool {
		p.at_ts(self.recovery_ts) || p.at(T![,]) || is_nth_at_selector(p, 0) || p.has_nth_preceding_line_break(1)
	}
}

/// Determines if the current or nth token in the parser is at the start of a
/// selector.
///
/// This function checks whether the specified position in the CSS parser aligns
/// with the beginning of a compound selector. In CSS, selectors are patterns
/// used to select the elements to which a set of CSS rules apply.
#[inline]
...
///
/// This function attempts to parse a CSS selector, which may be either a
/// compound selector or a complex selector. Compound selectors are simple,
/// unseparated chains of simple selectors, whereas complex selectors are
/// compound selectors separated by combinators.
///
/// Initially, the function tries to parse a compound selector. If successful,
/// it then checks if this compound selector forms part of a complex selector
/// and continues parsing accordingly.
#[inline]
pub(crate) fn parse_selector(p:&mut CssParser) -> ParsedSyntax {
	...
	if is_nth_at_metavariable(p, 0) {
		...
		// After parsing the compound selector, it then checks if this compound selector
		// is a part of a complex selector.
...
/// Checks if the current or nth token in the parser is a complex selector
/// combinator.
///
/// This function determines whether the specified position in the CSS parser is
/// at a complex selector combinator. Complex selector combinators include
/// characters like '>', '+', ' ', and '~', used to define relationships between
/// different elements in CSS.
#[inline]
...
///
/// This function attempts to parse a complex selector, which combines compound
/// selectors through combinators like '>', '+', ' ', and '~'. These combinators
/// express relationships between elements in the document tree.
///
/// The function iterates over the tokens, parsing each complex selector
/// combinator and its associated compound selector. Parsing continues until no
/// more complex combinators are found, at which point it returns the completed
/// complex selector.
#[inline]
...
/// compound selector.
///
/// This function checks if the specified position in the CSS parser corresponds
/// to the start of a compound selector. A compound selector combines multiple
/// simple selectors, optionally including a reference combinator ('&').
#[inline]
fn is_nth_at_compound_selector(p:&mut CssParser, n:usize) -> bool {
	p.nth_at(n, T![&]) || is_nth_at_simple_selector(p, n) || p.nth_at_ts(n, SubSelectorList::START_SET)
}

/// Parses a compound selector in CSS.
///
/// This function attempts to parse a compound selector from the current
/// position in the CSS parser. Compound selectors are combinations of simple
/// selectors and optionally a reference combinator ('&'). They match elements
...
///
/// This function determines if the specified token position in the CSS parser
/// is at the start of a simple selector. Simple selectors are the basic
/// building blocks of CSS selectors, including type selectors, universal
...
///
/// This function attempts to parse a simple selector from the current position
/// in the CSS parser. Simple selectors are the most basic component of CSS
/// selectors, including type selectors, universal selectors, and attribute
/// selectors.
#[inline]
...
/// Determines if the current or nth token in the parser is at a namespace or
/// namespace prefix.
///
/// This function checks if the specified token position in the CSS parser is at
/// a namespace delimiter ('|') or at a namespace prefix. A namespace in CSS is
/// used to differentiate between different types of elements and attributes.
#[inline]
...
///
/// This function attempts to parse a namespace declaration from the current
/// position in the CSS parser. Namespaces in CSS are used for qualifying
/// element and attribute names by associating them with a namespace, defined by
/// a URI.
#[inline]
...
///
/// This function determines whether the parser is currently positioned at a
/// namespace prefix, either a universal namespace prefix ('*') or a specific
/// identifier. Namespace prefixes in CSS are used in conjunction with namespace
/// URIs to apply styles to elements in that namespace.
#[inline]
fn is_nth_at_namespace_prefix(p:&mut CssParser, n:usize) -> bool { p.nth_at(n, T![*]) || is_nth_at_identifier(p, n) }

/// Parses a namespace prefix in CSS.
///
/// This function attempts to parse a namespace prefix, which can be a universal
/// namespace prefix ('*') or a named namespace prefix (an identifier). The
/// namespace prefix is used in CSS selectors to apply styles to elements within
...
fn parse_namespace_prefix(p:&mut CssParser) -> ParsedSyntax {
	...
	let kind = if p.eat(T![*]) {
		...
		// we don't need to check if the identifier is valid, because we already did
		// that
...
impl SubSelectorList {
	pub(crate) const START_SET:TokenSet<CssSyntaxKind> = token_set![T![#], T![.], T![:], T![::], T!['[']];
}
impl ParseNodeList for SubSelectorList {
	...
	fn parse_element(&mut self, p:&mut CssParser) -> ParsedSyntax { parse_sub_selector(p) }

	fn is_at_list_end(&self, p:&mut CssParser) -> bool { !p.at_ts(Self::START_SET) }
...
///
/// This function is responsible for identifying and parsing different types of
/// sub-selectors based on the current token in the CSS parser. It dispatches to
/// specific parsing functions for class selectors, ID selectors, attribute
...
/// ('.') followed by an identifier. Class selectors are used to apply styles to
...
/// ('#') followed by an identifier. ID selectors are used to apply styles to a
/// single element with the specified ID attribute.
#[inline]
...
///
/// This function attempts to parse a universal selector from the current
/// position in the CSS parser. The universal selector matches elements of any
/// type.
#[inline]
...
///
/// This function attempts to parse a type selector, which matches elements of a
/// specific type, from the current position in the CSS parser. The type
/// selector is typically an identifier representing an HTML element type.
#[inline]
...
///
/// This function parses an identifier, which is a fundamental part of various
/// CSS selectors, considering the specific lexical context of selectors. The
/// context affects how the parser interprets tokens, particularly with respect
/// to whitespace handling and combinator parsing.
#[inline]
...
where
	...
	if p.eat_with_context(T![')'], context) {
		...
		if let Ok(m) = ParseRecoveryTokenSet::new(CSS_BOGUS, SELECTOR_FUNCTION_RECOVERY_SET)
			...
			.recover(p)
		{
			let diagnostic = error_builder(p, TextRange::new(parameter.range(p).start(), m.range(p).end()));
