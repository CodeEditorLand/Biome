...
use crate::{
	...
	syntax::{
		is_at_identifier,
		parse_error::{expected_any_attribute_matcher_name, expected_any_attribute_modifier, expected_identifier},
		parse_regular_identifier,
		parse_string,
		selector::{is_nth_at_namespace, parse_namespace, selector_lex_context},
	},
};
...
pub(crate) fn parse_attribute_selector(p:&mut CssParser) -> ParsedSyntax {
	...
	// which turns an `Err` into an `Ok` variant, because attribute matcher in a CSS
	// attribute selector is optional. If it isn't present or fails to parse
	// correctly, parsing can continue.
...
fn is_at_attribute_name(p:&mut CssParser) -> bool { is_at_identifier(p) || is_nth_at_namespace(p, 0) }
...
const ATTRIBUTE_MATCHER_SET:TokenSet<CssSyntaxKind> = token_set![T![~=], T![|=], T![^=], T!["$="], T![*=], T![=]];
#[inline]
fn is_at_attribute_matcher(p:&mut CssParser) -> bool { p.at_ts(ATTRIBUTE_MATCHER_SET) }
...
#[inline]
fn is_at_attribute_matcher_value(p:&mut CssParser) -> bool { is_at_identifier(p) || p.at(CSS_STRING_LITERAL) }
#[inline]
