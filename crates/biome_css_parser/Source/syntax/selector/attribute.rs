...
use crate::{
	...
	syntax::{
		is_at_identifier,
		parse_error::{
			expected_any_attribute_matcher_name,
			expected_any_attribute_modifier,
			expected_identifier,
		},
...
pub(crate) fn parse_attribute_selector(p:&mut CssParser) -> ParsedSyntax {
	...
	// which turns an `Err` into an `Ok` variant, because attribute matcher in a
	// CSS attribute selector is optional. If it isn't present or fails to
	// parse correctly, parsing can continue.
...
fn parse_attribute_matcher(p:&mut CssParser) -> ParsedSyntax {
	...
	if modifier.is_attribute_modifier_keyword() {
		...
		// if we have an invalid modifier, we should add a diagnostic and bump
		// it
