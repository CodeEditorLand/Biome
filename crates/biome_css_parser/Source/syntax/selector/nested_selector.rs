use biome_css_syntax::{
	CssSyntaxKind,
	CssSyntaxKind::{CSS_NESTED_SELECTOR, CSS_NESTED_SELECTOR_LIST},
	T,
};
use biome_parser::{
...
use crate::{parser::CssParser, syntax::selector::selector_lex_context};
...
impl ParseNodeList for NestedSelectorList {
	...
	fn parse_element(&mut self, p:&mut CssParser) -> ParsedSyntax { parse_nested_selector(p) }
...
/// Checks if the parser is currently positioned at the start of a nested
/// selector.
///
/// In CSS, nested selectors often start with an `&` character, which refers to
/// the parent selector. This function checks if the current token in the parser
/// matches `&`.
#[inline]
fn is_at_nested_selector(p:&mut CssParser) -> bool { p.at(T![&]) }
