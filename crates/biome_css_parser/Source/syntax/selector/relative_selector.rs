...
use biome_parser::{
	...
	parse_recovery::{ParseRecovery, RecoveryError, RecoveryResult},
	parsed_syntax::{
		ParsedSyntax,
...
use crate::{
	parser::CssParser,
	syntax::{
		parse_error::expected_relative_selector,
		selector::{is_nth_at_compound_selector, parse_selector},
	},
};
...
/// ## Example
///
/// A relative selector might look like `"E > F"` where `E` is an element that
/// is a parent of an element `F`. In the context of this structure, a list
/// might be parsed from a string like `"div > p, ul > li:first-child"`.
...
impl RelativeSelectorList {
	...
	///
	/// Initializes the list with a specified end kind, with error recovery
	/// enabled by default.
	///
	/// # Arguments
	///
	/// * `end_kind` - A `CssSyntaxKind` that indicates the end of the selector
	///   list.
	pub(crate) fn new(end_kind:CssSyntaxKind) -> Self { RelativeSelectorList { end_kind, is_recovery_disabled:false } }

	/// Disables error recovery for this selector list.
	///
	/// This method modifies the instance to disable the error recovery
	/// mechanism. It is useful when strict parsing is required without
	/// fallbacks.
...
impl RelativeSelectorListParseRecovery {
	...
	pub(crate) fn new(end_kind:CssSyntaxKind) -> Self { RelativeSelectorListParseRecovery { end_kind } }
}
...
impl ParseRecovery for RelativeSelectorListParseRecovery {
	...
	/// specific tokens or conditions that indicate a safe point for
	/// continuation.
	///
	/// # CSS Examples
	///
	...
	///
	/// - Recovery at Comma (`,`) between selectors: ```css .class1 , 12312312 ,
	///   .class3 { /*        ^^^^^^^^ invalid selector   ^ recovery point at
	///   comma */ ```
	///
	/// - Recovery at Relative Selector:
	///   - When the parser identifies a combinator indicating the start of a
	///     new relative selector: ```css .class1 , 12312312
	///     > .class2 { /*        ^^^^^^^^ invalid selector     ^ recovery point
	///     at new relative selector */ ```
...
impl ParseSeparatedList for RelativeSelectorList {
	...
	fn parse_element(&mut self, p:&mut CssParser) -> ParsedSyntax { parse_relative_selector(p) }

	fn is_at_list_end(&self, p:&mut CssParser) -> bool { p.at(self.end_kind) }
	...
	fn separating_element_kind(&mut self) -> CssSyntaxKind { T![,] }
}

/// A set of CSS syntax kinds representing combinator types for relative
/// selectors.
///
/// This constant defines a set of combinators used in CSS to establish
/// relationships between selectors. It includes combinators like `>`, `+`, `~`,
/// and `||`, which are used to specify parent-child, adjacent siblings, general
/// siblings, and column combinator selectors, respectively.
const RELATIVE_SELECTOR_COMBINATOR_SET:TokenSet<CssSyntaxKind> = token_set![T![>], T![+], T![~], T![||]];

/// Checks if the current position in the parser is at a relative selector
/// combinator.
///
/// This function uses `RELATIVE_SELECTOR_COMBINATOR_SET` to determine if the
/// current position in the CSS parser is at a combinator that can be used in a
/// relative selector.
#[inline]
fn is_at_relative_selector_combinator(p:&mut CssParser) -> bool { p.at_ts(RELATIVE_SELECTOR_COMBINATOR_SET) }

/// Determines if the parser is positioned at a relative selector.
///
/// This function checks if the current position in the CSS parser is at a
/// relative selector by checking for the presence of a combinator or if it's at
/// the start of a compound selector.
#[inline]
...
///
/// This function attempts to parse a relative selector starting from the
/// current position. If the position does not indicate a relative selector, it
/// returns `Absent`. Otherwise, it parses the selector, adding diagnostics if
/// necessary, and returns `Present` with the parsed syntax.
#[inline]
