...
pub(crate) fn expected_non_css_wide_keyword_identifier(
	...
	if CssSyntaxKind::from_keyword(text).is_some_and(|keyword| keyword.is_css_wide_keyword()) {
		// Trying to use `expected_node` here with the additional hint results
		// in two details being added, but since we're adding the hint as
		// well, we only want to show one code frame.
