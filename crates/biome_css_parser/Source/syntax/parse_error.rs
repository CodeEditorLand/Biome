...
use biome_parser::{
	Parser,
	diagnostic::{expect_one_of, expected_any, expected_node},
	prelude::{ParseDiagnostic, ToDiagnostic},
};
...
pub(crate) fn expected_non_css_wide_keyword_identifier(p:&CssParser, range:TextRange) -> ParseDiagnostic {
	...
	if CssSyntaxKind::from_keyword(text).is_some_and(|keyword| keyword.is_css_wide_keyword()) {
		...
		ParseDiagnostic::new(format!("Expected an identifier but instead found '{text}'"), range)
			.with_hint(format!("'{text}' is a CSS-wide keyword that cannot be used here"))
	} else {
		expected_identifier(p, range)
	}
}

pub(crate) fn expected_number(p:&CssParser, range:TextRange) -> ParseDiagnostic { expected_node("number", range, p) }

pub(crate) fn expected_string(p:&CssParser, range:TextRange) -> ParseDiagnostic { expected_node("string", range, p) }
...
pub(crate) fn expected_any_declaration_or_at_rule(p:&CssParser, range:TextRange) -> ParseDiagnostic {
...
pub(crate) fn expected_any_attribute_matcher_name(p:&CssParser, range:TextRange) -> ParseDiagnostic {
