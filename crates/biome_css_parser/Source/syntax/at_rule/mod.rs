...
use crate::{
	...
	syntax::{
		at_rule::{
			charset::parse_charset_at_rule,
			color_profile::parse_color_profile_at_rule,
			container::parse_container_at_rule,
			counter_style::parse_counter_style_at_rule,
			document::parse_document_at_rule,
			font_face::parse_font_face_at_rule,
			font_feature_values::parse_font_feature_values_at_rule,
			font_palette_values::parse_font_palette_values_at_rule,
			import::parse_import_at_rule,
			keyframes::parse_keyframes_at_rule,
			layer::parse_layer_at_rule,
			media::parse_media_at_rule,
			namespace::parse_namespace_at_rule,
			page::parse_page_at_rule,
			property::parse_property_at_rule,
			scope::parse_scope_at_rule,
			starting_style::parse_starting_style_at_rule,
			supports::parse_supports_at_rule,
			unknown::{is_at_unknown_at_rule, parse_unknown_at_rule},
			value::parse_value_at_rule,
		},
		parse_error::expected_any_at_rule,
	},
};

#[inline]
pub(crate) fn is_at_at_rule(p:&mut CssParser) -> bool { p.at(T![@]) }
