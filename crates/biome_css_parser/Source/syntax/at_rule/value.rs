...
use crate::{
	...
	syntax::{
		...
		parse_error::{expected_component_value, expected_identifier},
...
#[inline]
pub(crate) fn is_at_value_at_rule(p:&mut CssParser) -> bool { p.at(T![value]) }
...
/// ```
/// This function identifies and parses these `@value` rules within CSS
/// stylesheets.
#[inline]
...
fn is_at_value_at_rule_import_clause(p:&mut CssParser) -> bool { is_at_identifier(p) }
...
impl ParseSeparatedList for ValueAtRuleImportSpecifierList {
	...
	fn parse_element(&mut self, p:&mut Self::Parser<'_>) -> ParsedSyntax { parse_value_at_rule_import_specifier(p) }

	fn is_at_list_end(&self, p:&mut Self::Parser<'_>) -> bool { p.at(T![from]) }

	fn recover(&mut self, p:&mut Self::Parser<'_>, parsed_element:ParsedSyntax) -> RecoveryResult {
		parsed_element.or_recover(p, &ValueAtRuleImportSpecifierListParseRecovery, expected_import_specifier)
	}
...
fn is_at_value_at_rule_import_source(p:&mut CssParser) -> bool { is_at_identifier(p) || is_at_string(p) }
...
fn parse_value_at_rule_import_source(p:&mut CssParser) -> ParsedSyntax {
	...
	if is_at_identifier(p) { parse_regular_identifier(p) } else { parse_string(p) }
}

/// Checks if the current parser position is at a value at-rule declaration
/// clause.
fn is_at_value_at_rule_declaration_clause(p:&mut CssParser) -> bool { is_nth_at_value_at_rule_generic_property(p, 0) }
...
impl ParseSeparatedList for ValueAtRulePropertyList {
	...
	fn parse_element(&mut self, p:&mut Self::Parser<'_>) -> ParsedSyntax { parse_value_at_rule_generic_property(p) }

	fn is_at_list_end(&self, p:&mut Self::Parser<'_>) -> bool { p.at(T![;]) }

	fn recover(&mut self, p:&mut Self::Parser<'_>, parsed_element:ParsedSyntax) -> RecoveryResult {
		parsed_element.or_recover(p, &ValueAtRulePropertyListParseRecovery, expected_component_value)
	}

	fn separating_element_kind(&mut self) -> Self::Kind { T![,] }
}

/// Checks if the parser is at a generic property of a value at-rule at the nth
/// position.
...
/// A generic property can be any sequence of tokens, which is why we parse it
/// as an unformed tree. This approach allows for flexibility, as the exact
/// structure of generic properties may not be well-defined. By parsing the
/// property in this manner, we ensure that we can handle a wide variety of
/// cases without requiring specific grammar rules for each possible property
/// structure.
///
/// If we want to extend the parsing with a specific grammar in the future, we
/// can implement speculative parsing. Speculative parsing would attempt to
/// parse the property according to the new grammar rules. If an error occurs
/// during this process, we can fall back to the current implementation, which
/// treats the property as an unformed tree. This approach ensures backward
/// compatibility and allows for incremental improvements to the parser without
...
/// Structure for recovering from parsing errors in a value at-rule property
/// list.
...
/// This function returns a diagnostic error indicating that an at-rule
/// declaration clause or import clause was expected at the given range in the
/// CSS parser.
fn expected_at_rule_declaration_clause(p:&CssParser, range:TextRange) -> ParseDiagnostic {
	expect_one_of(&["declaration at rule clause", "import at rule clause"], range).into_diagnostic(p)
}
...
/// parsing of @value at-rules by setting the `css.parser.cssModules` option to
/// `true` in the configuration file.
pub(crate) fn value_at_rule_not_allowed(p:&CssParser, range:TextRange) -> ParseDiagnostic {
	p.err_builder("@value at-rule is not a standard CSS feature.", range).with_hint(
		"You can enable @value at-rule parsing by setting the `css.parser.cssModules` option to `true` in your \
		 configuration file.",
	)
}
