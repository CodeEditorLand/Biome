...
use biome_parser::{
	diagnostic::{expect_one_of, ParseDiagnostic},
...
fn parse_value_at_rule_generic_property(p:&mut CssParser) -> ParsedSyntax {
...

	{
		...
		// Skip all tokens until the end of the property value or the next
		// property. EOF indicates the end of the file.
...
fn expected_at_rule_declaration_clause(p:&CssParser, range:TextRange) -> ParseDiagnostic {
	expect_one_of(&["declaration at rule clause", "import at rule clause"], range)
		.into_diagnostic(p)
}
...
pub(crate) fn value_at_rule_not_allowed(p:&CssParser, range:TextRange) -> ParseDiagnostic {
	p.err_builder("@value at-rule is not a standard CSS feature.", range).with_hint(
		"You can enable @value at-rule parsing by setting the `css.parser.cssModules` option to \
