...
use crate::{
	...
	syntax::{
		at_rule::{
			...
			supports::parse_any_supports_condition,
		},
		is_at_declaration,
		is_at_string,
		parse_declaration,
...
/// Checks if the current token in the parser is a URL or a string, indicating
/// the start of a URL import.
///
/// This function is utilized within the context of parsing an `@import` rule in
/// CSS. It determines whether the current token is either a URL function or a
...
///
/// This function checks if the current token is a valid URL or string format
...
pub(crate) fn parse_import_url(p:&mut CssParser) -> ParsedSyntax {
	...
	if is_at_url_function(p) { parse_url_function(p) } else { parse_string(p) }
}

/// Determines if the current parsing position is at an anonymous layer within
/// an `@import` rule.
///
/// This function checks whether the current token in the `CssParser` matches
/// the `layer` token, indicating the start of an anonymous layer declaration in
...
/// Checks if the current token in the parser is the start of a named layer in
/// an `@import` rule.
///
/// This function verifies if the current token is `layer` and is immediately
...
/// Checks if the current token in the parser is the start of a `supports`
/// condition in an `@import` rule.
///
