use biome_parser::{
	diagnostic::{ToDiagnostic, expect_one_of, expected_node},
	prelude::ParseDiagnostic,
};
...
pub(crate) fn expected_any_scope_range(p:&CssParser, range:TextRange) -> ParseDiagnostic {
	expect_one_of(
		&["(<scope-start>)", "to (<scope-end>)", "(scope-start) to (<scope-end>)"],
		range,
	)
	.into_diagnostic(p)
}
...
pub(crate) fn expected_font_feature_values_item(p:&CssParser, range:TextRange) -> ParseDiagnostic {
	expected_node("font-feature-values item", range, p)
}

pub(crate) fn expected_any_font_feature_value_item(p:&CssParser, range:TextRange) -> ParseDiagnostic {
	expect_one_of(
		&[
			"stylistic",
			"historical_forms",
