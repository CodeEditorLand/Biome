...
#[inline]
fn is_at_query_feature_boolean(p:&mut CssParser) -> bool { is_at_identifier(p) }
...
#[inline]
fn is_at_query_feature_reverse_or_interval_range(p:&mut CssParser) -> bool { is_at_any_query_feature_value(p) }
...
#[inline]
fn is_at_query_feature_plain(p:&mut CssParser) -> bool { is_at_identifier(p) && p.nth_at(1, T![:]) }
...
#[inline]
pub(crate) fn is_at_any_query_feature_value(p:&mut CssParser) -> bool { is_at_any_value(p) }
