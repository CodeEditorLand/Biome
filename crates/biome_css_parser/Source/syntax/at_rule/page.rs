...
use crate::{
	...
	syntax::{
		at_rule::{
			is_at_at_rule,
			parse_at_rule,
			parse_error::{expected_any_page_at_rule_item, expected_page_selector, expected_page_selector_pseudo},
		},
		block::{ParseBlockBody, parse_declaration_or_at_rule_list_block},
		is_at_declaration,
		is_at_identifier,
...
#[inline]
pub(crate) fn is_at_page_at_rule(p:&mut CssParser) -> bool { p.at(T![page]) }
...
const PAGE_SELECTOR_LIST_RECOVERY_SET:TokenSet<CssSyntaxKind> = PAGE_SELECTOR_SET.union(token_set!(T!['{']));
...
impl ParseSeparatedList for PageSelectorList {
	...
	fn parse_element(&mut self, p:&mut Self::Parser<'_>) -> ParsedSyntax { parse_page_selector(p) }
...
#[inline]
fn is_at_page_selector(p:&mut CssParser) -> bool { is_at_identifier(p) || p.at(T![:]) }
...
impl ParseNodeList for PageSelectorPseudoList {
	...
	fn parse_element(&mut self, p:&mut Self::Parser<'_>) -> ParsedSyntax { parse_page_selector_pseudo(p) }

	fn is_at_list_end(&self, p:&mut Self::Parser<'_>) -> bool { p.at_ts(PAGE_SELECTOR_PSEUDO_LIST_END_SET) }

	fn recover(&mut self, p:&mut Self::Parser<'_>, parsed_element:ParsedSyntax) -> RecoveryResult {
		parsed_element.or_recover_with_token_set(
			p,
			&ParseRecoveryTokenSet::new(CSS_BOGUS_PSEUDO_CLASS, PAGE_SELECTOR_PSEUDO_LIST_RECOVERY_SET),
			expected_page_selector_pseudo,
		)
	}
}

const PAGE_SELECTOR_PSEUDO_SET:TokenSet<CssSyntaxKind> = token_set!(T![left], T![right], T![first], T![blank]);

#[inline]
fn is_at_page_selector_pseudo(p:&mut CssParser) -> bool { p.at(T![:]) }
...
impl ParseBlockBody for PageBlock {
	...
	fn is_at_element(&self, p:&mut CssParser) -> bool { is_at_at_rule(p) || is_at_declaration(p) || at_margin_rule(p) }

	fn parse_list(&mut self, p:&mut CssParser) { PageAtRuleItemList.parse_list(p); }
}

const CSS_PAGE_AT_RULE_ITEM_LIST_RECOVERY_SET:TokenSet<CssSyntaxKind> = token_set!(T![@], T![ident]);
...
#[inline]
pub(crate) fn at_margin_rule(p:&mut CssParser) -> bool { p.at(T![@]) && p.nth_at_ts(1, PAGE_MARGIN_AT_RULE_NAME_SET) }
#[inline]
