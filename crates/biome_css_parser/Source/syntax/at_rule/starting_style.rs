...
use crate::{
	parser::CssParser,
	syntax::block::{parse_declaration_block, parse_rule_block},
};
...
///
/// This function handles the parsing of a `@starting-style` at-rule, which is
/// defined in the CSS Transitions Level 2 specification. It starts by
/// confirming the presence of such a rule and then processes the content
/// depending on whether the parser is currently inside a nesting block or at
/// the root level. It employs different parsing strategies for declarations or
/// rules based on the parser state `is_nesting_block`.
///
