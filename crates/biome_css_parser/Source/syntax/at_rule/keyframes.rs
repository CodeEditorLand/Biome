...
use crate::{
	...
	parser::CssParser,
	syntax::{
		at_rule::parse_error::{expected_keyframes_item, expected_keyframes_item_selector},
		block::{ParseBlockBody, parse_declaration_block},
		css_modules::{CSS_MODULES_SCOPE_SET, expected_any_css_module_scope, local_or_global_not_allowed},
		is_at_declaration,
		is_at_identifier,
		is_at_string,
		...
		parse_string,
		value::dimension::{is_at_percentage_dimension, parse_percentage_dimension},
	},
};

/// Checks if the current parser position is at a `@keyframes` at-rule.
///
/// This function determines if the parser is currently positioned at the start
/// of a `@keyframes` rule, which is part of the CSS syntax.
#[inline]
pub(crate) fn is_at_keyframes_at_rule(p:&mut CssParser) -> bool { p.at(T![keyframes]) }
/// Parses a `@keyframes` at-rule in CSS.
///
/// This function parses a `@keyframes` rule, which can be scoped locally or
/// globally using the `:local` and `:global` pseudo-classes specific to CSS
/// Modules.
///
...
pub(crate) fn parse_keyframes_at_rule(p:&mut CssParser) -> ParsedSyntax {
	...
	if is_at_keyframes_scoped_name(p) {
		// is_at_keyframes_scoped_name guaranties that it will parse a keyframes scoped
		// name
		parse_keyframes_scoped_name(p).ok();
	} else {
		parse_keyframes_identifier(p).or_add_diagnostic(p, expected_non_css_wide_keyword_identifier);
	};
...
///
/// This function determines if the parser is currently positioned at the start
/// of a keyframes scoped name, which is indicated by the presence of a `:`
/// character.
fn is_at_keyframes_scoped_name(p:&mut CssParser) -> bool { p.at(T![:]) }

/// Parses a keyframes scoped name in CSS Modules.
///
/// This function parses a keyframes scoped name, which can be either `:local`
/// or `:global`, specific to CSS Modules. If CSS Modules are not enabled, it
/// generates a diagnostic error and skips the scoped name.
fn parse_keyframes_scoped_name(p:&mut CssParser) -> ParsedSyntax {
	...
	let kind = {
		...
		let name = parse_keyframes_identifier(p).or_add_diagnostic(p, expected_non_css_wide_keyword_identifier);
...
///
/// This function determines if the parser is currently positioned at the start
/// of a keyframes identifier, which can be either a standard identifier or a
/// string.
fn is_at_keyframes_identifier(p:&mut CssParser) -> bool { is_at_identifier(p) || is_at_string(p) }

/// Parses a keyframes identifier in CSS.
/// This function parses a keyframes identifier, which can be either a standard
...
impl ParseBlockBody for KeyframesBlock {
	...
	fn is_at_element(&self, p:&mut CssParser) -> bool { is_at_keyframes_item_selector(p) }

	fn parse_list(&mut self, p:&mut CssParser) { KeyframesItemList.parse_list(p); }
}
...
impl ParseRecovery for KeyframesItemListParseRecovery {
	...
	fn is_at_recovered(&self, p:&mut Self::Parser<'_>) -> bool { p.at(T!['}']) || is_at_keyframes_item_selector(p) }
}
...
impl ParseNodeList for KeyframesItemList {
	...
	fn parse_element(&mut self, p:&mut Self::Parser<'_>) -> ParsedSyntax { parse_keyframes_item(p) }
...
impl ParseRecovery for KeyframesSelectorListParseRecovery {
	...
	fn is_at_recovered(&self, p:&mut Self::Parser<'_>) -> bool {
		// If we have an invalid selector, we need to skip tokens until we find a
		// recover point: @keyframes name1 {
...
impl ParseSeparatedList for KeyframesSelectorList {
	...
	fn parse_element(&mut self, p:&mut Self::Parser<'_>) -> ParsedSyntax { parse_keyframes_item_selector(p) }

	fn is_at_list_end(&self, p:&mut Self::Parser<'_>) -> bool { p.at(T!['{']) }

	fn recover(&mut self, p:&mut Self::Parser<'_>, parsed_element:ParsedSyntax) -> RecoveryResult {
		parsed_element.or_recover(p, &KeyframesSelectorListParseRecovery, expected_keyframes_item_selector)
	}
...
fn is_at_keyframes_selector_list_end(p:&mut CssParser) -> bool {
	...
	// @keyframes name1 {
	// 	from   <----- here we miss a '{', but we can try to assume that we can parse
	// a declaration block. 		color: red;
	// 	}
	// 	from   <----- here we miss a '{', but we can try to assume that we can parse
	// a declaration block. 	}
	// }
...
///
/// This function determines if the parser is currently positioned at the start
/// of a keyframes item selector, which can be either `from`, `to`, or a
...
///
/// This function parses a keyframes item selector, which can be either `from`,
/// `to`, or a percentage dimension.
#[inline]
