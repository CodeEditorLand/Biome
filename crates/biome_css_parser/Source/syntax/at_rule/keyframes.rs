...
use crate::{
	...
	syntax::{
		...
		css_modules::{
			expected_any_css_module_scope,
			local_or_global_not_allowed,
			CSS_MODULES_SCOPE_SET,
		},
		is_at_declaration,
		is_at_identifier,
		is_at_string,
...
pub(crate) fn parse_keyframes_at_rule(p:&mut CssParser) -> ParsedSyntax {
	...
	if is_at_keyframes_scoped_name(p) {
		// is_at_keyframes_scoped_name guaranties that it will parse a keyframes
		// scoped name
...
impl ParseRecovery for KeyframesItemBlockParseRecovery {
	...
	fn is_at_recovered(&self, p:&mut Self::Parser<'_>) -> bool {
		// We need to recover the contents of an invalid block for the following
		// cases: We have a declaration block, but it lacks a '{' token:
		//    @keyframes name {
		//      from
		//          color: red;
...
impl ParseRecovery for KeyframesSelectorListParseRecovery {
	...
	fn is_at_recovered(&self, p:&mut Self::Parser<'_>) -> bool {
		// If we have an invalid selector, we need to skip tokens until we find
		// a recover point: @keyframes name1 {
...
fn is_at_keyframes_selector_list_end(p:&mut CssParser) -> bool {
	...
	// @keyframes name1 {
	// 	from   <----- here we miss a '{', but we can try to assume that we can
	// parse a declaration block. 		color: red;
	// 	}
	// 	from   <----- here we miss a '{', but we can try to assume that we can
