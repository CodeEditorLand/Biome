...
use crate::{
	parser::CssParser,
	syntax::{block::parse_declaration_block, parse_dashed_identifier, parse_error::expected_dashed_identifier},
};

#[inline]
pub(crate) fn is_at_font_palette_values_at_rule(p:&mut CssParser) -> bool { p.at(T![font_palette_values]) }
...
pub(crate) fn parse_font_palette_values_at_rule(p:&mut CssParser) -> ParsedSyntax {
	...
	let kind = if parse_dashed_identifier(p)
		.or_recover_with_token_set(
			p,
			&ParseRecoveryTokenSet::new(CSS_BOGUS, FONT_PALETTE_VALUES_RECOVERY_SET).enable_recovery_on_line_break(),
