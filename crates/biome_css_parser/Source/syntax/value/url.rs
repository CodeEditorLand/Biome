...
use crate::{
	...
	syntax::{
		...
		parse_string,
		value::{
			function::{is_at_function, is_nth_at_function, parse_function},
			parse_error::expected_url_modifier,
		},
	},
};
...
/// Determines if the current position of the parser is at the beginning of a
/// URL function.
pub(crate) fn is_at_url_function(p:&mut CssParser) -> bool { p.at_ts(URL_SET) && p.nth_at(1, T!['(']) }

/// Parses a URL function from the current position of the CSS parser.
///
/// This function is designed to parse URL functions as specified in the CSS
/// Values and Units Module. The URL function can be of various forms and may
/// include different types of content, like a standard URL, a URL with quotes,
/// a URL with variables, or a URL with special characters. This parser handles
/// these variations according to the syntax rules defined in the CSS
/// specification.
///
...
///
/// - Standard URL: `url(image.jpg)` Represents a basic URL function without any
///   additional modifiers or quotes.
///
/// - URL with quotes: `url("image.jpg")` A URL function where the URL is
///   enclosed within quotes.
///
/// - URL with a variable: `url(var(--image-path))` Demonstrates a URL function
///   that uses a CSS variable as its value.
///
/// - URL with special characters: `url(http://example.com/image.jpg?size=large)`
///   Shows a URL function with a full HTTP URL, including special characters
///   like query parameters.
///
/// # Grammar
///
/// The grammar for the URL function according to the CSS specification is as
/// follows:
///
/// `<url>` = `<url()>` | `<src()>`
///
...
///
/// Here, `<url-modifier>` represents any modifiers that might be applied to the
/// URL, such as resolution-based adjustments or format hints.
...
///
/// This function checks if the parser's current position is at the beginning of
/// either a raw URL value or a string.
#[inline]
pub(crate) fn is_at_url_value(p:&mut CssParser) -> bool { is_at_url_value_raw(p) || is_at_string(p) }

/// Parses a URL value from the current position of the CSS parser.
///
/// This function attempts to parse a URL value starting from the parser's
/// current position. If the current position is at a string, it parses using
/// `parse_string`; otherwise, it uses `parse_url_value_raw` for a raw URL.
#[inline]
pub(crate) fn parse_url_value(p:&mut CssParser) -> ParsedSyntax {
	...
	if is_at_string(p) { parse_string(p) } else { parse_url_value_raw(p) }
}

/// Determines if the current position of the parser is at a raw URL value.
#[inline]
pub(crate) fn is_at_url_value_raw(p:&mut CssParser) -> bool { p.at(CSS_URL_VALUE_RAW_LITERAL) }
...
impl ParseNodeList for UrlModifierList {
	...
	fn parse_element(&mut self, p:&mut Self::Parser<'_>) -> ParsedSyntax { parse_url_modifier(p) }
...
/// This function determines if the current token is either an identifier or any
/// function, indicating a potential modifier for a URL in CSS.
#[inline]
pub(crate) fn is_at_url_modifier(p:&mut CssParser) -> bool { is_at_identifier(p) || is_at_function(p) }

/// Parses a URL modifier, which can be either a simple function or a regular
/// identifier.
#[inline]
pub(crate) fn parse_url_modifier(p:&mut CssParser) -> ParsedSyntax {
	...
	if is_at_function(p) { parse_function(p) } else { parse_regular_identifier(p) }
}
