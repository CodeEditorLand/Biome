...
pub(crate) trait ParseBlockBody {
	...
	///  - Try to check if the next item is an item of a list. If it is, we can
	///    parse the list.
	///  - If the next item is not a list item, we return bogus block and skip
	///    the list parsing.
	...
	///
	/// This function handles the parsing of a block's content, delimited by
	/// curly braces `{}`. It temporarily sets the parser's state to indicate
	/// it is within a nesting block and then processes the content of the
	/// block using the provided callback function.
...
pub(crate) fn expected_block(p:&CssParser, range:TextRange) -> ParseDiagnostic { expected_node("body", range, p) }
