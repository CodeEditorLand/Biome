...
use biome_parser::{
	...
	prelude::{ParsedSyntax, ParsedSyntax::Absent},
};
...
fn is_at_declaration_or_rule_item(p:&mut CssParser) -> bool {
	is_at_at_rule(p) || is_at_nested_qualified_rule(p) || is_at_declaration(p) || is_at_metavariable(p)
}
...
impl ParseRecovery for DeclarationOrRuleListParseRecovery {
	...
	fn is_at_recovered(&self, p:&mut Self::Parser<'_>) -> bool { p.at(T!['}']) || is_at_declaration_or_rule_item(p) }
}
...
impl ParseNodeList for DeclarationOrRuleList {
	...
	fn recover(&mut self, p:&mut Self::Parser<'_>, parsed_element:ParsedSyntax) -> RecoveryResult {
		parsed_element.or_recover(p, &DeclarationOrRuleListParseRecovery, expected_any_declaration_or_at_rule)
	}
}
