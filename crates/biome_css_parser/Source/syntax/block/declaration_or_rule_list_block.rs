...
impl ParseBlockBody for DeclarationOrRuleListBlock {
	...
	fn is_at_element(&self, p:&mut CssParser) -> bool { is_at_declaration_or_rule_item(p) }

	fn parse_list(&mut self, p:&mut CssParser) { DeclarationOrRuleList.parse_list(p); }
}
	...
	fn parse_element(&mut self, p:&mut Self::Parser<'_>) -> ParsedSyntax {
		if is_at_at_rule(p) {
			...
			let declaration = try_parse(p, |p| {
				...
				// Indicates the end of the last declaration because `;` is
				// optional. E.g
				// .class {
				//     color: red; <----
				...
				// If either condition is true, the declaration is considered
				// valid.
				if matches!(p.last(), Some(T![;])) || p.at(T!['}']) {
					Ok(declaration)
				} else {
					// If neither condition is met, return an error to indicate
					// parsing failure. And rewind the parser to the start
					// of the declaration.
					Err(())
				}
			});

			// If parsing as a declaration was successful, return the parsed
			// declaration.
			...
			let rule = try_parse(p, |p| {
				...
				if matches!(p.last(), Some(T!['}'])) {
					...
					// If the condition is not met, return an error to indicate
					// parsing failure.
					Err(())
				}
			});

			// If parsing as a nested qualified rule was successful, return the
			// parsed rule.
			if let Ok(rule) = rule {
				return rule;
			}
