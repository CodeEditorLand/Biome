...
use crate::{
	...
	syntax::{
		...
		selector::{
			is_nth_at_selector,
			relative_selector::{is_at_relative_selector, RelativeSelectorList},
			SelectorList,
		},
...
pub(crate) fn parse_declaration_with_semicolon(p:&mut CssParser) -> ParsedSyntax {
	...
	// Otherwise, a semicolon is expected and the parser will enforce its
	// presence. div { color: red; }
	// div { color: red }
	if !p.at(T!['}']) {
		if p.nth_at(1, T!['}']) {
			p.eat(T![;]);
...
impl ParseRecovery for BracketedValueListRecovery {
	...
	fn is_at_recovered(&self, p:&mut Self::Parser<'_>) -> bool {
		// If the next token is the end of the list or the next element, we're
		// at a recovery point.
...
#[must_use = "The result of try_parse contains information about whether the parse succeeded and \
...
mod tests {
	...
	fn try_parse_preserves_position_on_success() {
		...
		let result = try_parse(&mut p, |p| {
			...
			// attempt to parse a number, and succeed because the input has
			// `100`.
			match parse_regular_number(p) {
				Present(marker) => Ok(Present(marker)),
				Absent => Err(()),
