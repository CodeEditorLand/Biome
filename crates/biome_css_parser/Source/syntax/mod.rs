...
use value::{
	dimension::{is_at_any_dimension, parse_any_dimension},
	function::{is_at_any_function, parse_any_function},
};
...
use crate::{
	...
	syntax::{
		...
		block::parse_declaration_or_rule_list_block,
		parse_error::{expected_any_rule, expected_non_css_wide_keyword_identifier},
		property::{
			color::{is_at_color, parse_color},
			is_at_any_property,
			parse_any_property,
			unicode_range::{is_at_unicode_range, parse_unicode_range},
		},
		selector::{
			SelectorList,
			is_nth_at_selector,
			relative_selector::{RelativeSelectorList, is_at_relative_selector},
		},
		value::function::BINARY_OPERATION_TOKEN,
	},
};
...
impl RuleList {
	fn new(end_kind:CssSyntaxKind) -> Self { Self { end_kind } }
}

#[inline]
pub(crate) fn is_at_rule_list_element(p:&mut CssParser) -> bool { is_at_at_rule(p) || is_at_qualified_rule(p) }
...
impl ParseRecovery for RuleListParseRecovery {
	...
	fn is_at_recovered(&self, p:&mut Self::Parser<'_>) -> bool { p.at(self.end_kind) || is_at_rule_list_element(p) }
}
...
impl ParseNodeList for RuleList {
	...
	fn recover(&mut self, p:&mut Self::Parser<'_>, parsed_element:ParsedSyntax) -> RecoveryResult {
		parsed_element.or_recover(p, &RuleListParseRecovery::new(self.end_kind), expected_any_rule)
	}
}

#[inline]
pub(crate) fn is_at_qualified_rule(p:&mut CssParser) -> bool { is_nth_at_selector(p, 0) }
...
/// Checks if the current position in the CSS parser is at the start of a nested
/// qualified rule. Nested qualified rules are determined by the presence of a
/// relative selector, indicating the start of a rule that is nested within
/// another rule.
#[inline]
pub(crate) fn is_at_nested_qualified_rule(p:&mut CssParser) -> bool { is_at_relative_selector(p) }

/// Parses a nested qualified rule from the current position in the CSS parser.
/// If the current position is identified as the start of a nested qualified
/// rule, it proceeds to parse the rule. This involves parsing the list of
/// relative selectors and then parsing or recovering the declaration
/// or rule list block. The kind of rule parsed (nested qualified or bogus) is
/// determined based on the success of parsing the block.
#[inline]
...
impl ParseNodeList for DeclarationList {
	...
	fn parse_element(&mut self, p:&mut Self::Parser<'_>) -> ParsedSyntax { parse_declaration_with_semicolon(p) }
...
pub(crate) fn is_at_declaration(p:&mut CssParser) -> bool { is_at_any_property(p) }
#[inline]
...
///
/// This function attempts to parse a single CSS declaration from the current
/// position of the parser. It handles the optional semicolon (';') at the end
/// of the declaration, adhering to CSS syntax rules where the semicolon is
/// mandatory for all declarations except the last one in a block. In the case
/// of the last declaration before a closing brace ('}'), the semicolon is
/// optional. If the semicolon is omitted for declarations that are not at the
/// end, the parser will raise an error.
#[inline]
...
#[inline]
fn is_at_declaration_important(p:&mut CssParser) -> bool { p.at(T![!]) && p.nth_at(1, T![important]) }
...
#[inline]
fn is_at_metavariable(p:&mut CssParser) -> bool { p.at(GRIT_METAVARIABLE) }

#[inline]
fn is_nth_at_metavariable(p:&mut CssParser, n:usize) -> bool { p.nth_at(n, GRIT_METAVARIABLE) }
...
impl ParseNodeList for CssComponentValueList {
	...
	fn parse_element(&mut self, p:&mut Self::Parser<'_>) -> ParsedSyntax { parse_any_value(p) }
...
#[inline]
pub(crate) fn is_at_css_wide_keyword(p:&mut CssParser) -> bool { p.cur().is_css_wide_keyword() }

#[inline]
pub(crate) fn is_at_identifier(p:&mut CssParser) -> bool { is_nth_at_identifier(p, 0) }
...
#[inline]
pub(crate) fn parse_regular_identifier(p:&mut CssParser) -> ParsedSyntax { parse_identifier(p, CssLexContext::Regular) }
...
#[inline]
pub(crate) fn parse_regular_number(p:&mut CssParser) -> ParsedSyntax { parse_number(p, CssLexContext::Regular) }
#[inline]
...
#[inline]
pub(crate) fn is_at_string(p:&mut CssParser) -> bool { p.at(CSS_STRING_LITERAL) }

/// Checks if the parser is currently at the start of a bracketed value.
#[inline]
pub(crate) fn is_at_bracketed_value(p:&mut CssParser) -> bool { p.at(T!['[']) }

/// Parses a bracketed value from the current position in the CSS parser.
///
/// This function parses a list of values enclosed in square brackets, commonly
/// used in CSS properties like `grid-template-areas` where the value is a list
/// of identifiers representing grid areas. For details on the syntax of
/// bracketed values, see the [CSS Syntax specification](https://drafts.csswg.org/css-grid/#named-lines)
#[inline]
...
///
/// This parser is responsible for parsing a list of identifiers inside a
/// bracketed value.
...
impl ParseNodeList for BracketedValueList {
	...
	fn recover(&mut self, p:&mut Self::Parser<'_>, parsed_element:ParsedSyntax) -> RecoveryResult {
		parsed_element.or_recover(p, &BracketedValueListRecovery, expected_non_css_wide_keyword_identifier)
	}
}

/// Recovery strategy for bracketed value lists.
///
/// This recovery strategy handles the recovery process when parsing bracketed
/// value lists.
...
impl ParseRecovery for BracketedValueListRecovery {
	...
	fn is_at_recovered(&self, p:&mut Self::Parser<'_>) -> bool {
		// If the next token is the end of the list or the next element, we're at a
		// recovery point.
...
#[allow(dead_code)]
pub(crate) fn try_parse<T, E>(p:&mut CssParser, func:impl FnOnce(&mut CssParser) -> Result<T, E>) -> Result<T, E> {
...
mod tests {
	...
	use biome_parser::{
		Parser,
		prelude::ParsedSyntax::{Absent, Present},
	};
