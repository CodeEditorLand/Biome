...
pub fn run(test_case: &str, _snapshot_name: &str, test_directory: &str, outcome_str: &str) {
	...
	let content =
		fs::read_to_string(test_case_path).expect("Expected test path to be a readable file in UTF8 encoding");
	...
	if options_path.exists() {
		...
		let (test_options, diagnostics) =
			deserialize_from_str::<Configuration>(options_path.get_buffer_from_file().as_str()).consume();
...
"#,
	...
	if !diagnostics.is_empty() {
		...
		for diagnostic in diagnostics {
			let error = diagnostic.clone().with_file_path(file_name).with_file_source_code(&content);
	...
	match outcome {
		ExpectedOutcome::Pass => {
			...
			if missing_required || parsed.syntax().descendants().any(|node| node.kind().is_bogus()) {
				panic!(
