...
fn get_fixtures_path() -> Utf8PathBuf {
	...
	while !path.join("Cargo.lock").exists() {
		path = path.parent().expect("couldn't find Cargo.lock").to_path_buf();
	}
...
fn test_resolve_relative_import() {
	...
	let added_paths = vec![BiomePath::new("/src/index.ts"), BiomePath::new("/src/bar.ts")];
	...
	dependency_graph.update_graph_for_js_paths(&fs, &project_layout, &added_paths, &[], |path| {
		fs.read_file_from_path(path).ok().and_then(|content| {
			let parsed = biome_js_parser::parse(&content, JsFileSource::tsx(), JsParserOptions::default());
	...
	assert_eq!(
		...
		Some(&Import { resolved_path: Ok(Utf8PathBuf::from("/src/bar.ts")) })
	);
}
...
fn test_resolve_package_import() {
	...
	assert_eq!(
		...
		Some(&Import { resolved_path: Ok(Utf8PathBuf::from("/node_modules/shared/dist/index.js")) })
	);
}
...
fn test_resolve_package_import_in_monorepo_fixtures() {
	...
	project_layout.insert_node_manifest(format!("{fixtures_path}/frontend").into(), {
		...
		deserialize_from_json_str::<PackageJson>(
			&fs.read_file_from_path(&path).expect("package.json must be readable"),
	...
	project_layout.insert_node_manifest(format!("{fixtures_path}/frontend/node_modules/shared").into(), {
		let path = Utf8PathBuf::from(format!("{fixtures_path}/frontend/node_modules/shared/package.json"));
		deserialize_from_json_str::<PackageJson>(
			&fs.read_file_from_path(&path).expect("package.json must be readable"),
			JsonParserOptions::default(),
			"package.json",
	...
	let added_paths = vec![
		...
		BiomePath::new(format!("{fixtures_path}/frontend/node_modules/shared/dist/index.js")),
	...
	let file_imports = imports
		.get(Utf8Path::new(&format!("{fixtures_path}/frontend/src/index.ts")))
		.unwrap();
	...
	assert_eq!(
		file_imports.static_imports.get("shared"),
		Some(&Import {
			resolved_path: Ok(Utf8PathBuf::from(format!("{fixtures_path}/shared/dist/index.js")))
		})
	);
	assert_eq!(
		file_imports.static_imports.get("./bar"),
		Some(&Import {
			resolved_path: Ok(Utf8PathBuf::from(format!("{fixtures_path}/frontend/src/bar.ts")))
		})
	);
}
...
fn test_resolve_exports() {
	...
	assert_eq!(
		...
		Some(&Export::Own(OwnExport { jsdoc_comment: Some("@package".to_string()) }))
	);
	assert_eq!(
	...
	for export in destructured_exports {
		assert_eq!(
			...
			Some(&Export::Own(OwnExport { jsdoc_comment: None }))
		);
	}
	...
	assert_eq!(
		...
		vec![ReexportAll { import: Import { resolved_path: Ok(Utf8PathBuf::from("/src/reexports.ts")) } }]
	);

	let data = dependency_data.get(Utf8Path::new("/src/reexports.ts")).unwrap();
	assert_eq!(data.exports.len(), 1);
	assert_eq!(
		data.exports.get(&Text::Static("renamed")),
		Some(&Export::ReexportAll(ReexportAll {
			import: Import { resolved_path: Ok(Utf8PathBuf::from("/src/renamed-reexports.ts")) }
		}))
	);
}
