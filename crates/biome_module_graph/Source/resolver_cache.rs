...
use oxc_resolver::{Cache, CachedPath as _, ResolveError, ResolveOptions, TsConfig, context::ResolveContext as Ctx};
					...
					|parent| {
						self.canonicalize_impl(parent).and_then(|parent_canonical| {
							...
							if self.fs.path_is_symlink(&utf8_path) {
								...
								if link.is_absolute() {
									return self.canonicalize_impl(&self.value(normalize(&link).as_std_path()));
...
impl Cache for ResolverCache<'_> {
		...
		let result = path
			...
			.get_or_try_init(|| {
				let utf8_path = path
					.path()
					.try_into()
					.map_err(|_| ResolveError::NotFound(path.path().to_string_lossy().to_string()))?;
	...
	fn get_tsconfig<F: FnOnce(&mut TsConfigJson) -> Result<(), ResolveError>>(
		...
		let (mut tsconfig, diagnostics) = TsConfigJson::parse(root, &tsconfig_path, &mut tsconfig_string);
...
impl CachedPathImpl {
	const fn new(hash: u64, path: Box<Utf8Path>, parent: Option<CachedPath>, meta: Option<PathKind>) -> Self {
		Self {
			hash,
			path,
			parent,
