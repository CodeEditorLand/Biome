...
impl DependencyGraph {
	...
	fn find_exported_symbol(&self, module: &ModuleDependencyData, symbol_name: &str) -> Option<OwnExport> {
		...
		fn find_exported_symbol_with_seen_paths<'a>(
			...
			match module.exports.get(symbol_name) {
				...
				Some(Export::Reexport(import)) => match &import.resolved_path {
					Ok(path) if seen_paths.insert(path) => data
						.get(path)
						.and_then(|module| find_exported_symbol_with_seen_paths(data, module, symbol_name, seen_paths)),
					_ => None,
				},
				...
				Some(Export::ReexportAll(_)) => Some(OwnExport { jsdoc_comment: None }),
				None => module
					.blanket_reexports
					.iter()
					.find_map(|reexport| match &reexport.import.resolved_path {
...
impl ModuleDependencyData {
	...
	#[inline]
	pub fn find_exported_symbol(&self, dependency_graph: &DependencyGraph, name: &str) -> Option<OwnExport> {
