...
impl FormatNodeRule<GritPatternDefinitionBody> for FormatGritPatternDefinitionBody {
	fn fmt_fields(&self, node: &GritPatternDefinitionBody, f: &mut GritFormatter) -> FormatResult<()> {
		let GritPatternDefinitionBodyFields { l_curly_token, r_curly_token, patterns } = node.as_fields();
