...
impl FormatNodeRule<GritSequential> for FormatGritSequential {
	fn fmt_fields(&self, node: &GritSequential, f: &mut GritFormatter) -> FormatResult<()> {
		let GritSequentialFields { l_curly_token, sequential_token, sequential, r_curly_token } = node.as_fields();

		write!(f, [sequential_token.format(), space(), l_curly_token.format(),])?;
