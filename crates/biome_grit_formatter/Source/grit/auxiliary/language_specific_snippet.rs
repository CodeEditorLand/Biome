...
impl FormatNodeRule<GritLanguageSpecificSnippet> for FormatGritLanguageSpecificSnippet {
	fn fmt_fields(&self, node: &GritLanguageSpecificSnippet, f: &mut GritFormatter) -> FormatResult<()> {
		let GritLanguageSpecificSnippetFields { language, snippet_token } = node.as_fields();
