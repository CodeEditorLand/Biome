...
impl FormatNodeRule<GritLanguageDeclaration> for FormatGritLanguageDeclaration {
	fn fmt_fields(&self, node: &GritLanguageDeclaration, f: &mut GritFormatter) -> FormatResult<()> {
		let GritLanguageDeclarationFields { language_token, name, flavor, semicolon_token } = node.as_fields();
		write!(f, [language_token.format(), space(), name.format(), flavor.format(),])?;
