...
pub fn format_node(options: GritFormatOptions, root: &GritSyntaxNode) -> FormatResult<Formatted<GritFormatContext>> {
...
pub fn format_range(options: GritFormatOptions, root: &GritSyntaxNode, range: TextRange) -> FormatResult<Printed> {
...
impl FormatLanguage for GritFormatLanguage {
	...
	fn is_range_formatting_node(&self, _node: &biome_rowan::SyntaxNode<Self::SyntaxLanguage>) -> bool {
		true
	}
	...
	fn create_context(
		...
		let comments: Comments<GritLanguage> = Comments::from_node(root, &GritCommentStyle, source_map.as_ref());
