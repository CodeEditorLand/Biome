...
pub(crate) trait FormatAstSeparatedListExtension: AstSeparatedList<Language = GritLanguage> {
	...
	fn format_separated(&self, separator: &'static str) -> GritFormatSeparatedIter<Self::Node> {
		GritFormatSeparatedIter::new(self.elements(), separator, GritFormatSeparatedElementRule { node: PhantomData })
	}
}
