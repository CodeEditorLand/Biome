...
impl Rule for NoUnknownPseudoElement {
	...
	fn run(ctx:&RuleContext<Self>) -> Option<Self::State> {
		...
		let pseudo_element_name = match &pseudo_element {
			...
			AnyCssPseudoElement::CssPseudoElementFunctionIdentifier(ident) => ident.name().ok()?.text().to_string(),

			AnyCssPseudoElement::CssPseudoElementFunctionSelector(selector) => selector.text(),
			AnyCssPseudoElement::CssPseudoElementIdentifier(ident) => ident.name().ok()?.text().to_string(),
		};
