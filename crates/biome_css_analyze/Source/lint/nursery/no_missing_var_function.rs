...
impl Rule for NoMissingVarFunction {
	...
	fn run(ctx:&RuleContext<Self>) -> Option<Self::State> {
		...
		if rule.declarations.iter().any(|decl| decl.property.name == custom_variable_name) {
		...
		if model.global_custom_variables().contains_key(&custom_variable_name) {
			return Some(node.clone());
		}
	...
	while let Some(parent) = current_node {
		match parent.kind() {
			...
			CssSyntaxKind::CSS_FUNCTION => {
				return parent.text().starts_with("var");
			},
			_ => {},
		}
