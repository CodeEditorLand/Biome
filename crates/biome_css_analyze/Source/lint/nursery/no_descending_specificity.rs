...
fn find_tail_selector(selector:&AnyCssSelector) -> Option<String> {
	match selector {
		...
		AnyCssSelector::CssComplexSelector(s) => s.right().as_ref().ok().and_then(find_tail_selector),

		_ => None,
	}
}

/// This function traverses the CSS rules starting from the given rule and
/// checks for selectors that have the same tail selector. For each selector, it
/// compares its specificity with the previously encountered specificity of the
/// same tail selector. If a lower specificity selector is found after a higher
/// specificity selector with the same tail selector, it records this as a
/// descending selector.
fn find_descending_selector(
	...
	for selector in &rule.selectors {
		...
		if let Some((last_textrange, last_specificity)) = visited_selectors.get(&tail_selector) {
			...
			visited_selectors.insert(tail_selector, (selector.range, selector.specificity.clone()));
		}
	}

	for child_id in &rule.child_ids {
		if let Some(child_rule) = model.get_rule_by_id(*child_id) {
			find_descending_selector(child_rule, model, visited_rules, visited_selectors, descending_selectors);
		}
	}
}
