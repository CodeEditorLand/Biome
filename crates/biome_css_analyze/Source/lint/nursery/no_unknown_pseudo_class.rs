...
impl Rule for NoUnknownPseudoClass {
	...
	fn run(ctx:&RuleContext<Self>) -> Option<Self::State> {
		...
		let is_valid_class = match pseudo_type {
			...
			PseudoClassType::Other => {
				is_custom_selector(lower_name) || vendor_prefixed(lower_name) || is_known_pseudo_class(lower_name)
			},
		};
		...
		if is_valid_class {
			...
			Some(NoUnknownPseudoClassSelectorState { class_name:name, span, class_type:pseudo_type })
		}
	}
