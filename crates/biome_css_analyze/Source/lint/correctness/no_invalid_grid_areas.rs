...
impl Rule for NoInvalidGridAreas {
	...
	fn diagnostic(_:&RuleContext<Self>, state:&Self::State) -> Option<RuleDiagnostic> {
		match state.reason {
			GridAreaValidationError::EmptyGridArea => {
				Some(
					RuleDiagnostic::new(
						rule_category!(),
						state.span,
			...
			GridAreaValidationError::InconsistentCellCount => {
				Some(
					RuleDiagnostic::new(
						rule_category!(),
						state.span,
			...
			GridAreaValidationError::DuplicateGridToken => {
				Some(
					RuleDiagnostic::new(
						rule_category!(),
						state.span,
...
fn has_partial_match(grid_areas_props:&GridAreasProps) -> Option<GridAreasProp> {
	...
	for (text, range) in grid_areas_props {
		...
		let parts:FxHashSet<String> = prop.split_whitespace().map(|part| part.to_string()).collect();
