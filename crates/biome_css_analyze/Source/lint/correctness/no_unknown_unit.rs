...
use biome_css_syntax::{AnyCssDimension, CssFunction, CssGenericProperty, CssQueryFeaturePlain, CssSyntaxKind};
	...
	fn run(ctx:&RuleContext<Self>) -> Option<Self::State> {
		...
		match node {
			...
			AnyCssDimension::CssRegularDimension(dimension) => {
				...
				// The `x` unit is parsed as `CssRegularDimension`, but it is used for
				// describing resolutions. This check is to disallow the use of the `x` unit
				...
				if unit == "x" {
						...
						match ancestor.kind() {
							CssSyntaxKind::CSS_FUNCTION => {
								let function_name_token =
									ancestor.cast::<CssFunction>()?.name().ok()?.value_token().ok()?;

								let function_name = function_name_token.text_trimmed().to_ascii_lowercase_cow();
							...
							CssSyntaxKind::CSS_GENERIC_PROPERTY => {
								...
								let property_name = property_name_token.text_trimmed().to_ascii_lowercase_cow();
							...
							CssSyntaxKind::CSS_QUERY_FEATURE_PLAIN => {
								let feature_name_token =
									ancestor.cast::<CssQueryFeaturePlain>()?.name().ok()?.value_token().ok()?;

								let feature_name = feature_name_token.text_trimmed().to_ascii_lowercase_cow();
					...
					if !allow_x {
						return Some(NoUnknownUnitState { unit, span:unit_token.text_trimmed_range() });
					}
				}
