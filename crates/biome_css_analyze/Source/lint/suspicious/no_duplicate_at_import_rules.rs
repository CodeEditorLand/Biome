					...
					match item.rule().ok()? {
						AnyCssAtRule::CssImportAtRule(import_rule) => {
							...
							if let Some(media_query_set) = import_url_map.get_mut(&import_url) {
								// if the current import_rule has no media queries or there are no
								// queries saved in the media_query_set, this is always a
								// duplicate
								...
								for media in import_rule.media() {
									match media {
										Ok(media) => {
											if !media_query_set.insert(media.text().to_lowercase_cow().into()) {
												return Some(import_rule);
											}
										},
