...
use biome_css_syntax::{AnyCssDeclarationBlock, AnyCssKeyframesItem, CssDeclarationImportant, CssKeyframesBlock};
...
impl Rule for NoImportantInKeyframe {
	...
	fn run(ctx:&RuleContext<Self>) -> Option<Self::State> {
		...
		for item in node.items() {
			...
			let AnyCssDeclarationBlock::CssDeclarationBlock(block_declaration) = keyframe_item.block().ok()? else {
				return None;
			};
