		...
		if let WalkEvent::Enter(node) = event {
			match node.kind() {
				...
				CssSyntaxKind::CSS_GENERIC_PROPERTY => {
					if let Some(prop_node) =
						CssGenericProperty::cast_ref(node).and_then(|property_node| property_node.name().ok())
					{
						...
						self.prior_props_in_block.iter().for_each(|prior_prop| {
							...
							let unprefixed_prior_prop = remove_vendor_prefix(&prior_prop.lowercase, prior_prop_prefix);
...
impl Queryable for NoDeclarationBlockShorthandPropertyOverridesQuery {
	...
	fn build_visitor(analyzer:&mut impl AddVisitor<Self::Language>, _:&<Self::Language as Language>::Root) {
		analyzer.add_visitor(Phases::Syntax, NoDeclarationBlockShorthandPropertyOverridesVisitor::default);
	}
