use crate::keywords::{
	...
	A_NPLUS_BOF_SNOTATION_PSEUDO_CLASSES, BASIC_KEYWORDS, FONT_FAMILY_KEYWORDS,
	FONT_SIZE_KEYWORDS, FONT_STRETCH_KEYWORDS, FONT_STYLE_KEYWORDS,
	FONT_VARIANTS_KEYWORDS, FONT_WEIGHT_ABSOLUTE_KEYWORDS,
	FONT_WEIGHT_NUMERIC_KEYWORDS, FUNCTION_KEYWORDS, KNOWN_CHROME_PROPERTIES,
	KNOWN_EDGE_PROPERTIES, KNOWN_EXPLORER_PROPERTIES, KNOWN_FIREFOX_PROPERTIES,
	KNOWN_PROPERTIES, KNOWN_SAFARI_PROPERTIES,
	KNOWN_SAMSUNG_INTERNET_PROPERTIES, KNOWN_US_BROWSER_PROPERTIES,
	LEVEL_ONE_AND_TWO_PSEUDO_ELEMENTS, LINE_HEIGHT_KEYWORDS,
	LINGUISTIC_PSEUDO_CLASSES, LOGICAL_COMBINATIONS_PSEUDO_CLASSES,
	LONGHAND_SUB_PROPERTIES_OF_SHORTHAND_PROPERTIES, MEDIA_FEATURE_NAMES,
	OTHER_PSEUDO_CLASSES, OTHER_PSEUDO_ELEMENTS,
	RESET_TO_INITIAL_PROPERTIES_BY_BORDER, RESET_TO_INITIAL_PROPERTIES_BY_FONT,
	RESOURCE_STATE_PSEUDO_CLASSES, SHADOW_TREE_PSEUDO_ELEMENTS,
	SHORTHAND_PROPERTIES, SYSTEM_FAMILY_NAME_KEYWORDS, VENDOR_PREFIXES,
	VENDOR_SPECIFIC_PSEUDO_ELEMENTS,
};
use biome_css_syntax::{
	AnyCssGenericComponentValue, AnyCssValue, CssGenericComponentValueList,
};
...
pub fn is_system_family_name_keyword(value: &str) -> bool {
	BASIC_KEYWORDS.contains(&value)
		|| SYSTEM_FAMILY_NAME_KEYWORDS.contains(&value)
}
...
pub fn find_font_family(
	...
	for v in value {
		...
		if matches!(
			v,
			AnyCssGenericComponentValue::AnyCssValue(
				AnyCssValue::AnyCssDimension(_)
			)
		) {
			continue;
		}
		...
		match v {
			...
			AnyCssGenericComponentValue::AnyCssValue(css_value) => {
				match css_value {
					AnyCssValue::CssIdentifier(_)
					| AnyCssValue::CssString(_) => font_families.push(css_value),
					_ => continue,
				}
			},
		}
...
pub fn is_function_keyword(value: &str) -> bool {
	FUNCTION_KEYWORDS.binary_search(&value.to_lowercase_cow().as_ref()).is_ok()
}
...
pub fn get_longhand_sub_properties(
...
pub fn get_reset_to_initial_properties(
	shorthand_property: &str,
) -> &'static [&'static str] {
