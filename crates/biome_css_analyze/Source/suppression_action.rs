...
impl SuppressionAction for CssSuppressionAction {
	...
	fn find_token_for_inline_suppression(&self, token: CssSyntaxToken) -> Option<ApplySuppression<Self::Language>> {
	...
	fn apply_inline_suppression(
		...
		if token_has_trailing_comments {
			...
			let mut trivia = vec![
				(TriviaPieceKind::SingleLineComment, suppression_comment.as_str()),
				(TriviaPieceKind::Newline, "\n"),
			];
