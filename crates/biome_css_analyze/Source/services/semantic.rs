...
use biome_css_semantic::{SemanticEventExtractor, builder::SemanticModelBuilder, model::SemanticModel};
...
impl SemanticServices {
	pub fn model(&self) -> &SemanticModel { &self.model }
}
...
impl FromServices for SemanticServices {
	fn from_services(rule_key:&RuleKey, services:&ServiceBag) -> Result<Self, MissingServicesDiagnostic> {
		let model:&SemanticModel = services
			.get_service()
			.ok_or_else(|| MissingServicesDiagnostic::new(rule_key.rule_name(), &["SemanticModel"]))?;
...
impl QueryMatch for SemanticModelEvent {
	fn text_range(&self) -> TextRange { self.0 }
}

/// The [Semantic] type usable by lint rules **that uses the semantic model** to
/// match on specific [AstNode] types
///
/// ```ignore
/// impl Rule for SampleCssLintRule {
