...
impl Glob {
	...
	pub fn negated(self) -> Self {
		Self { is_negated: !self.is_negated, glob: self.glob }
	}
	...
	fn deserialize(
		...
		match glob.parse() {
			...
			Err(error) => {
				...
				ctx.report(
					biome_deserialize::DeserializationDiagnostic::new(format_args!("{error}")).with_range(range),
				);
				None
			},
		}
...
mod tests {
	...
	fn test_validate_glob() {
		...
		assert_eq!(
			...
			Err(GlobError::Regular { kind: GlobErrorKind::UnsupportedNestedAlternates, index: 6 })
		);
	...
	fn test_is_match() {
		...
		assert!("file.{js,jsx}".parse::<Glob>().unwrap().is_match("file.jsx"));
	}
	...
	fn test_to_string() {
		...
		assert_eq!(Glob::from_str("!**/*.rs").unwrap().to_string(), "!**/*.rs");
		assert_eq!(Glob::from_str("file.{js,jsx}").unwrap().to_string(), "file.{js,jsx}");
	}
}
