//! `biome_glob` provides globbing functionality. When listing the globs to
//! match, it also possible to provide globs that function as "exceptions" by
//! prefixing the globs with `!`.
//!
//! A glob is primarlly used to select or filter a set of file paths by matching
//! every file paths against the glob. A file path either matches or doesn't
//! match a glob. For example, the path `lib.rs` matches the glob `*.rs`.
//!
//! Biome globs are case-sensitive. This means that `lib.RS` doesn't match
//! `*.rs`.
//!
//! You have to understand the structure of a path to understand which path
//! match a glob. A path is divided in path segments.
//! Every path segment is delimited by the path separator `/` or the start/end
//! of the path. For instance `src/lib.rs` cosnists of two path segments: `src`
//! and `lib.rs`. A Biome glob supports the following patterns:
//!
...
//!
//! - globstar `**` that matches zero or more path segments `**` must be
//!   enclosed by path separators `/` or the start/end of the glob. For example,
//!   `**a` is not a valid glob. Also, `**` must not be followed by another
//!   globstar. For example, `**/**` is not a valid glob.
//!
//!   `lib.rs` and `src/lib.rs` match `**` and `**/*.rs`
//!   Conversely, `README.txt` doesn't match `**/*.rs` because the pat hends
//! with `.txt`.
//!
//! - Use `\*` to escape `*`
//!
//!   the path `*` matches `\*`.
//!
//! - `?`, `[`, `]`, `{`, and `}` must be escaped using `\`. These characters
...
//! Use [Glob::is_match] to match against anything that can be turned into a
//! [std::path::Path], such as a string.
//!
//! In the following example we parse the string `"*.rs"` into a glob and we
//! match against two strings. `lib.rs` matches the glob because the path has a
//! single path segment that ends with `.rs`. Conversely, `src/lib.rs` doesn't
//! match because it has two path segments (`src` and `lib.rs`).
//!
//! ```
//! use biome_glob::Glob;
//!
...
//! you should compile the path into a [CandidatePath] using
//! [CandidatePath::new]. [CandidatePath] may speed up matching against several
//! globs. To get adavantage of the speed-up, you have to use the
//! [CandidatePath::matches] method instead of [Glob::is_match].
//!
//! In the following example, we create a list of two globs and we match them
//! against a path compiled into a candidate path. The path matches the second
//! glob of the list.
//!
//! ```
...
//!
//! Negated globs are particularly useful to denote exceptions in a list of
//! globs. To interpret negated globs as exceptions, use
//! [CandidatePath::matches_with_exceptions]. This semantic is taken from the [.gitignore](https://git-scm.com/docs/gitignore#_pattern_format) format.
//!
//! In the following example we accept all files in the `src` directory, except
//! the ones ending with the `txt` extension.
//!
//! ```
//! use biome_glob::{CandidatePath, Glob};
//!
...
//!
//! Taking the previous example, the directory path `a/path` doesn't match
//! `**/*.rs` the list of glob, because the path doesn't end with the `.rs`
//! extension. This behavior is porblematic when you write a file crawler that
//! traverse the file hierarchy and ignore directories with files that never
//! match the list of globs. Biome provides a deidcated method
//! [CandidatePath::matches_directory_with_exceptions] for this purpose.
//! The method only check if the directory is not excluded by an exception.
//!
//! In the following example, `dir1` matches the list of globs, while `dir2`
//! doesn't.
//!
//! ```
//! use biome_glob::{CandidatePath, Glob};
//!
//! let globs:&[Glob] =
//! 	&["**/*.rs".parse().expect("correct glob"), "!test".parse().expect("correct glob")];
//!
...
impl Glob {
	...
	/// negation.
	fn is_raw_match(&self, path:impl AsRef<std::path::Path>) -> bool { self.glob.is_match(path) }
...
impl From<Glob> for String {
	fn from(value:Glob) -> Self { value.to_string() }
}
impl std::str::FromStr for Glob {
	...
	fn from_str(value:&str) -> Result<Self, Self::Err> {
		...
		match glob_builder.build() {
			Ok(glob) => Ok(Glob { is_negated, glob:glob.compile_matcher() }),
			Err(error) => Err(GlobError::Generic(error.kind().to_string().into_boxed_str())),
		}
	}
}
...
impl<'a> CandidatePath<'a> {
	...
	pub fn new(path:&'a impl AsRef<std::path::Path>) -> Self { Self(globset::Candidate::new(path)) }

	/// Tests whether the current path matches `glob`.
	pub fn matches(&self, glob:&Glob) -> bool { glob.is_match_candidate(self) }

	/// Match against a list of globs where negated globs are handled as
	/// exceptions.
	///
	...
	/// use biome_glob::{CandidatePath, Glob};
	///
	/// let globs:&[Glob] = &["*".parse().unwrap(), "!a*".parse().unwrap(), "a".parse().unwrap()];
	///
	...
	/// ```
	///
	/// - `b` matches `*` and is not excluded by the exception `!a*`. Thus, `b`
	///   matches the list of globs.
	/// - `abc` matches the first glob `*`, however it is excluded by the
	///   exception `!a*`. Thus `abc` doesn't match the list of globs.
	/// - `a` matches the first glob `*` and is excluded by the exception `!a*`.
	///   However, it is included again by the last glob `a`. Thus `a` matches
	///   the list of globs.
	...
	/// Match against a list of globs where negated globs are handled as
	/// exceptions handling the path as a directory.
	///
	/// This method is useful for implementing a file crawler that avoids
	/// traversing directories that should be ignored.
	///
	/// The current implementation returns `false` only if at least one
	/// exception (negated glob) matches the path.
	///
	///
	/// ```
	/// use biome_glob::{CandidatePath, Glob};
	///
	/// let globs:&[Glob] = &["a/path".parse().unwrap(), "!b".parse().unwrap()];
	///
	...
	/// Match against a list of globs where negated globs are handled as
	/// exceptions. Returns `default` if there is no globs that match.
...
pub enum GlobErrorKind {
	...
	InvalidEscape,
	/// Occurs when `**` isn't enclosed by the path separator `/` or the
	/// start/end of the glob.
	InvalidGlobStar,
	/// `{}` is not supported.
...
mod tests {
	...
	fn test_validate_glob() {
		assert_eq!(
			validate_glob("*.[jt]s"),
			Err(GlobError::Regular { kind:GlobErrorKind::UnsupportedCharacterClass, index:2 })
		);
		assert_eq!(
			validate_glob("?*.js"),
			Err(GlobError::Regular { kind:GlobErrorKind::UnsupportedAnyCharacter, index:0 })
		);
		assert_eq!(
			validate_glob(r"\"),
			Err(GlobError::Regular { kind:GlobErrorKind::DanglingEscape, index:0 })
		);
		assert_eq!(
			validate_glob(r"\n"),
			Err(GlobError::Regular { kind:GlobErrorKind::InvalidEscape, index:0 })
		);
		assert_eq!(
			validate_glob(r"\😀"),
			Err(GlobError::Regular { kind:GlobErrorKind::InvalidEscape, index:0 })
		...
		assert_eq!(
			validate_glob(r"***"),
			Err(GlobError::Regular { kind:GlobErrorKind::InvalidGlobStar, index:0 })
		);
		assert_eq!(
			validate_glob(r"a**"),
			Err(GlobError::Regular { kind:GlobErrorKind::InvalidGlobStar, index:1 })
		);
		assert_eq!(
			validate_glob(r"**a"),
			Err(GlobError::Regular { kind:GlobErrorKind::InvalidGlobStar, index:0 })
		...
		assert_eq!(
			validate_glob(r"**/**"),
			Err(GlobError::Regular { kind:GlobErrorKind::InvalidGlobStar, index:3 })
		);
