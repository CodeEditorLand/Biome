...
impl ProjectLayout {
	...
	pub fn get_node_manifest_for_path(&self, path: &Utf8Path) -> Option<(Utf8PathBuf, PackageJson)> {
		...
		result.map(|(package_path, package_json)| (package_path.join("package.json"), package_json.clone()))
	}
	...
	pub fn insert_node_manifest(&self, path: Utf8PathBuf, manifest: PackageJson) {
		self.0.pin().update_or_insert_with(
			...
			|data| {
				...
				PackageData { node_package: Some(node_js_package) }
			},
			|| {
				let node_js_package = NodeJsPackage { manifest: Some(manifest.clone()), ..Default::default() };
