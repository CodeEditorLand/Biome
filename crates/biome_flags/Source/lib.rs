...
pub fn is_unstable() -> bool { BIOME_VERSION.deref().is_none() }
...
impl BiomeEnvVariable {
	fn new(name:&'static str, description:&'static str) -> Self { Self { name, description } }

	/// It attempts to read the value of the variable
	pub fn value(&self) -> Option<String> { env::var(self.name).ok() }
...
pub fn biome_env() -> &'static BiomeEnv { BIOME_ENV.get_or_init(BiomeEnv::new) }
