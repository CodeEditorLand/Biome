...
pub trait LanguageSrc {
	...
	fn language(&self) -> TokenStream {
		let ident = format_ident!("{}{}", Case::Pascal.convert(self.language_prefix()), "Language");
		quote! { #ident }
	}

	fn string_literal(&self) -> Ident {
		format_ident!("{}{}", Case::Upper.convert(self.language_prefix()), "_STRING_LITERAL")
	}

	/// Special characters of the language. Usually these are parenthesis, dots,
	/// commas, etc.
	fn punct(&self) -> &[(&str, &str)];
	/// Reserved keywords of the language
	fn keywords(&self) -> &[&str];
	/// Literals are special nodes that holds some **values** inside the
	/// language, for example: strings, numbers, etc.
	fn literals(&self) -> &[&str];
	/// Whitespaces, comments, identifiers, etc.
	fn tokens(&self) -> &[&str];

	/// Nodes of the CST. Usually you want to map these names from the `.ungram`
	/// file. For example:
	///
	...
	///
	/// For example, returning "foo", will generate types like
	/// `FooSyntaxFactory`
	fn language_prefix(&self) -> &str;

	/// Maps a token or multiple tokens defined in the grammar to a "name" that
	/// can be used as field of a struct.
	///
	/// For example, you could map the tokens `"||"` to `"or"`.
	///
	/// Another example, you might want to map the start of an HTML comment to
	/// specific token:
	/// - `"<!--"` -> `"comment_start"`
	/// - `"-->"` -> `"comment_end"`
	///
	/// For tokens that might conflict with Rust language like parenthesis, you
