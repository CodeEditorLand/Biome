...
pub fn generate_node_factory<K>(ast: &AstSrc, syntax_crate_name: &str, language_kind: &K) -> Result<String>
where
	K: LanguageSrc,
{
	...
	let nodes = ast.nodes.iter().map(|node| {
		...
		let (optional, required): (Vec<_>, Vec<_>) = node.fields.iter().partition(|field| field.is_optional());
