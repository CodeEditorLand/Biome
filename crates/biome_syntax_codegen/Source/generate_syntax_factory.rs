...
pub fn generate_syntax_factory<K>(ast: &AstSrc, syntax_crate_name: &str, language_kind: &K) -> Result<String>
where
	K: LanguageSrc,
{
	...
	let bogus_kinds = ast.bogus.iter().map(|node| format_ident!("{}", Case::Constant.convert(node)));
