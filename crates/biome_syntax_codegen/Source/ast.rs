...
pub(crate) fn load_ungrammar_file(path: &Path, should_check_unions: bool) -> Result<AstSrc> {
	let grammar_src =
		fs::read_to_string(path).with_context(|| format!("Tried to load the path {}", { path.display() }))?;
...
pub(crate) fn generate_syntax<K>(language_src: K, ast: AstSrc, options: GrammarOptions) -> Result<()>
where
	K: LanguageSrc,
{
	let GrammarOptions {
...
fn write_and_format(contents: String, path: &Path) -> Result<()> {
	...
	fs::write(path, contents).with_context(|| format!("Tried to load the path {}", { path.display() }))?;
	rustfmt.args([path.as_os_str()]).output().expect("format the file");
	...
	for node in grammar.iter() {
		...
		match classify_node_rule(grammar, rule, &name) {
			NodeRuleClassification::Union(variants) => {
				...
				let variants = if name.trim().starts_with("AnyCss") && name.trim().ends_with("PropertyValue") {
			...
			NodeRuleClassification::Node => {
				...
				ast.nodes
					.push(AstNodeSrc { documentation: vec![], name, fields, dynamic: is_dynamic })
			},
			NodeRuleClassification::DynamicNode => {
				let mut fields = vec![];
				handle_rule(&mut fields, grammar, rule, None, false, true);
				ast.nodes
					.push(AstNodeSrc { documentation: vec![], name, fields, dynamic: true })
			},
	...
	match rule {
		...
		Rule::Rep(rule) => {
			...
			if element_type == SYNTAX_ELEMENT_TYPE {
				...
				NodeRuleClassification::List { separator: None, element_name: element_type.to_string() }
			}
		},
		Rule::Seq(rules) => {
			// (T (',' T)* ','?)
...
fn handle_tokens_in_unions(
	...
	let field = Field::Token { name: label.to_string(), kind: TokenKind::Many(token_kinds), optional, unordered };
	fields.push(field);
	true
}
		...
		K: LanguageSrc,
	{
		match self {
			...
			Field::Node { name, .. } => {
				...
				let final_name = if kind_source.prefixes().contains(&prefix) { tail } else { name.as_str() };
