...
use biome_service::workspace::{
	self, ChangeFileParams, CloseFileParams, FixFileParams, FormatFileParams,
	FormatOnTypeParams, FormatRangeParams, GetControlFlowGraphParams,
	GetFileContentParams, GetFormatterIRParams, GetSyntaxTreeParams,
	OrganizeImportsParams, PullActionsParams, PullDiagnosticsParams,
...
impl Workspace {
	...
	pub fn new() -> Workspace {
		Workspace { inner: workspace::server() }
	}
	...
	pub fn file_features(
		...
		let params: SupportsFeatureParams =
			serde_wasm_bindgen::from_value(params.into())
				.map_err(into_error)?;
	...
	pub fn register_workspace_folder(
		...
		let params: RegisterProjectFolderParams =
			serde_wasm_bindgen::from_value(params.into())
				.map_err(into_error)?;
		let result =
			self.inner.register_project_folder(params).map_err(into_error)?;
	...
	pub fn get_formatter_ir(
		&self,
		params: IGetFormatterIRParams,
	) -> Result<String, Error> {
	...
	pub fn fix_file(
		&self,
		params: IFixFileParams,
	) -> Result<IFixFileResult, Error> {
	...
	pub fn organize_imports(
		...
		to_value(&result).map(IOrganizeImportsResult::from).map_err(into_error)
	}

	pub fn rename(
		&self,
		params: IRenameParams,
	) -> Result<IRenameResult, Error> {
