...
impl Workspace {
	...
	pub fn new() -> Workspace { Workspace { inner:workspace::server() } }
	...
	pub fn register_workspace_folder(
		...
		let params:RegisterProjectFolderParams =
			serde_wasm_bindgen::from_value(params.into()).map_err(into_error)?;
		let result = self.inner.register_project_folder(params).map_err(into_error)?;
