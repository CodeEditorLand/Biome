...
impl Reference {
	pub(crate) fn find_next(&self) -> Option<Reference> {
		...
		Some(Reference { data:self.data.clone(), id })
	}
	...
	pub fn range_start(&self) -> TextSize { self.data.reference(self.id).range_start }
	...
	pub fn syntax(&self) -> &JsSyntaxNode { &self.data.binding_node_by_start[&self.range_start()] }

	/// Returns the binding of this reference
	pub fn binding(&self) -> Option<Binding> {
		Some(Binding { data:self.data.clone(), id:self.id.binding_id() })
	}
	...
	pub fn as_call(&self) -> Option<FunctionCall> {
		...
		match call {
			Some(node) if node.kind() == JsSyntaxKind::JS_CALL_EXPRESSION => {
				Some(FunctionCall { data:self.data.clone(), id:self.id })
			},
			_ => None,
		}
	}
...
impl UnresolvedReference {
	...
	pub fn range(&self) -> TextRange { self.data.unresolved_reference(self.id).range }
}
...
impl CallsExtensions for AnyJsFunction {
	fn all_calls(&self, model:&SemanticModel) -> Option<AllCallsIter> { model.all_calls(self) }
}
