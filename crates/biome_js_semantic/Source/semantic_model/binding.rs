...
impl SemanticModelReference {
	#[inline(always)]
	pub fn is_read(&self) -> bool { matches!(self.ty, SemanticModelReferenceType::Read { .. }) }

	#[inline(always)]
	pub fn is_write(&self) -> bool { matches!(self.ty, SemanticModelReferenceType::Write { .. }) }
}
...
impl Binding {
	...
	pub fn scope(&self) -> Scope {
		...
		Scope { data:self.data.clone(), id:self.data.scope(binding.range) }
	}
	...
	pub fn all_references(&self) -> AllBindingReferencesIter {
		...
		let first = if binding.references.is_empty() {
			...
			Some(Reference { data:self.data.clone(), id:ReferenceId::new(self.id, 0) })
		};
	...
	pub fn all_reads(&self) -> AllBindingReadReferencesIter {
		...
		let first =
			binding
				...
				.find(|(_, x)| x.is_read())
				.map(|(index, _)| {
					Reference { data:self.data.clone(), id:ReferenceId::new(self.id, index) }
				});
	...
	/// The node kind is either an identifier binding (tehd eclaration is self
	/// exported) or an identifier usage.
	...
	pub fn is_imported(&self) -> bool { super::is_imported(self.syntax()) }
}
...
pub trait BindingExtensions {
	/// Returns the [Binding] that declared the symbol this reference
	/// references.
