...
impl PartialEq for Scope {
	fn eq(&self, other:&Self) -> bool { self.id == other.id && self.data == other.data }
}

impl Eq for Scope {}

impl Scope {
	pub fn is_global_scope(&self) -> bool { self.id.index() == 0 }
	...
	pub fn descendents(&self) -> impl Iterator<Item = Scope> {
		...
		ScopeDescendentsIter { data:self.data.clone(), q }
	}
	...
	pub fn parent(&self) -> Option<Scope> {
		...
		Some(Scope { data:self.data.clone(), id:parent })
	}
	...
	pub fn bindings(&self) -> ScopeBindingsIter {
		ScopeBindingsIter { data:self.data.clone(), scope_id:self.id, binding_index:0 }
	}
	...
	/// ```
	pub fn is_ancestor_of(&self, other:&Scope) -> bool { other.ancestors().any(|s| s == *self) }

	pub fn range(&self) -> TextRange { self.data.scopes[self.id.index()].range }

	pub fn syntax(&self) -> &JsSyntaxNode { &self.data.scope_node_by_range[&self.range()] }
	...
	pub fn closure(&self) -> Option<Closure> { Closure::from_scope(self.data.clone(), self.id) }
}

/// Iterate all descendents scopes of the specified scope in breadth-first
/// order.
...
impl<T:AstNode<Language = JsLanguage>> SemanticScopeExtensions for T {
	fn scope(&self, model:&SemanticModel) -> Scope { model.scope(self.syntax()) }
