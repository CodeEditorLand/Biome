...
impl Capture {
	...
	pub fn ty(&self) -> &CaptureType { &self.ty }

	/// Returns the reference node of the capture
	pub fn node(&self) -> &JsSyntaxNode { &self.node }

	/// Returns the binding of this capture
	pub fn binding(&self) -> Binding { Binding { data:self.data.clone(), id:self.binding_id } }
	...
	/// ```
	pub fn declaration_range(&self) -> TextRange { self.data.binding(self.binding_id).range }
}
	...
	fn next(&mut self) -> Option<Self::Item> {
		'references: loop {
			...
			'scopes: while let Some(scope_id) = self.scopes.pop() {
				...
				self.references.extend(scope.write_references.iter().copied());
...
impl Iterator for ChildrenIter {
	...
	fn next(&mut self) -> Option<Self::Item> {
		while let Some(scope_id) = self.scopes.pop() {
			let scope = &self.data.scopes[scope_id.index()];
			if scope.is_closure {
				return Some(Closure { data:self.data.clone(), scope_id });
			} else {
				self.scopes.extend(scope.children.iter());
			}
		}
...
impl Closure {
	...
	pub fn closure_range(&self) -> TextRange { self.data.scopes[self.scope_id.index()].range }
	...
	pub fn children(&self) -> impl Iterator<Item = Closure> {
		let scope = &self.data.scopes[self.scope_id.index()];
		ChildrenIter { data:self.data.clone(), scopes:scope.children.clone() }
	}

	/// Returns all descendents of this closure in breadth-first order. Starting
	...
	pub fn descendents(&self) -> impl Iterator<Item = Closure> {
		...
		DescendentsIter { data:self.data.clone(), scopes }
	}
}
...
mod test {
	...
	fn assert_closure(code:&str, name:&str, captures:&[&str]) {
		...
		let closure = if name != "ARROWFUNCTION" {
			let node =
				r.syntax().descendants().filter(|x| x.text_trimmed() == name).last().unwrap();
			let node = node.parent().and_then(|node| AnyHasClosureNode::from_node(&node)).unwrap();
			model.closure(&node)
		} else {
			let node = r
				.syntax()
		...
		let all_captures:BTreeSet<String> =
			closure.all_captures().map(|x| x.node().text_trimmed().to_string()).collect();
