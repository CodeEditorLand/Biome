...
pub struct SemanticModelBuilder {
	...
	/// maps a binding range start to its index inside
	/// [SemanticModelBuilder::bindings] vec
	bindings_by_start:FxHashMap<TextSize, BindingId>,
	/// maps a reference range start to its binding index inside
	/// [SemanticModelBuilder::bindings] vec
...
impl SemanticModelBuilder {
	...
	pub fn push_node(&mut self, node:&JsSyntaxNode) {
		...
		match node.kind() {
			...
			| TS_FUNCTION_TYPE
			| TS_MAPPED_TYPE => {
				self.scope_node_by_range.insert(node.text_trimmed_range(), node.clone());
			},
			...
			_ => {
				if let Some(conditional_type) = TsConditionalType::cast_ref(node) {
					if let Ok(conditional_true_type) = conditional_type.true_type() {
						...
						self.scope_node_by_range.insert(syntax.text_trimmed_range(), syntax);
					}
				}
			},
		}
	...
	pub fn push_event(&mut self, e:SemanticEvent) {
		...
		match e {
			ScopeStarted { range, parent_scope_id, is_closure } => {
				...
				self.scope_range_by_start.entry(start).or_default().insert(Interval {
					start:start.into(),
					stop:range.end().into(),
					val:scope_id,
				});
			...
			DeclarationFound { range, scope_id, hoisted_scope_id } => {
				let binding_scope_id = hoisted_scope_id.unwrap_or(scope_id);

				// SAFETY: this scope id is guaranteed to exist because they were generated by
				// the event extractor
			...
			HoistedRead { range, declaration_at, scope_id } => {
				let binding_id = self.bindings_by_start[&declaration_at];
			...
			HoistedWrite { range, declaration_at, scope_id } => {
				let binding_id = self.bindings_by_start[&declaration_at];
			...
			UnresolvedReference { is_read, range } => {
				...
				match self.globals_by_name.entry(name) {
					...
					Entry::Vacant(_) => {
						self.unresolved_references.push(SemanticModelUnresolvedReference { range })
					},
				}
			},

			Export { declaration_at, range } => {
