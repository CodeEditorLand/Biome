...
impl ReferenceId {
	...
	pub(crate) fn binding_id(&self) -> BindingId { self.0 }

	pub(crate) fn index(self) -> usize { self.1 as usize }
}
...
impl ScopeId {
	...
	pub(crate) fn index(self) -> usize {
		// SAFETY: The internal representation ensures that the value is never equal to
		// 0. Thus, it is safe to substract 1.
...
impl SemanticModelData {
	...
	/// Returns the [ScopeId] which the specified syntax node was hoisted to, if
	/// any.
	fn scope_hoisted_to(&self, range:TextRange) -> Option<ScopeId> {
		self.scope_hoisted_to_by_range.get(&range.start()).copied()
	}

	pub fn is_exported(&self, range:TextRange) -> bool { self.exported.contains(&range.start()) }

	pub fn has_exports(&self) -> bool { !self.exported.is_empty() }
}
...
impl SemanticModel {
	pub(crate) fn new(data:SemanticModelData) -> Self { Self { data:Rc::new(data) } }
	...
	pub fn scopes(&self) -> impl Iterator<Item = Scope> + '_ {
		self.data
			...
			.iter()
			.enumerate()
			.map(|(id, _)| Scope { data:self.data.clone(), id:ScopeId::new(id) })
	}

	/// Returns the global scope of the model
	pub fn global_scope(&self) -> Scope { Scope { data:self.data.clone(), id:ScopeId::new(0) } }
	...
	/// use biome_js_semantic::{SemanticModelOptions, SemanticScopeExtensions, semantic_model};
	...
	pub fn scope(&self, node:&JsSyntaxNode) -> Scope {
		...
		Scope { data:self.data.clone(), id }
	}

	/// Returns the [Scope] which the specified syntax node was hoisted to, if
	/// any. Can also be called from [AstNode]::scope_hoisted_to extension
	/// method.
	pub fn scope_hoisted_to(&self, node:&JsSyntaxNode) -> Option<Scope> {
		...
		Some(Scope { data:self.data.clone(), id })
	}
	...
	pub fn all_bindings(&self) -> impl Iterator<Item = Binding> + '_ {
		self.data
			...
			.iter()
			.enumerate()
			.map(|(index, _)| Binding { data:self.data.clone(), id:BindingId::new(index) })
	}
	...
	pub fn all_exported_bindings(&self) -> impl Iterator<Item = Binding> + '_ {
		self.data
			...
			.map(|id| Binding { data:self.data.clone(), id })
	}
	...
	pub fn binding(&self, reference:&impl HasDeclarationAstNode) -> Option<Binding> {
		...
		Some(Binding { data:self.data.clone(), id })
	}
	...
	pub fn all_global_references(
		...
		let first = self
			...
			.map(|_| GlobalReference { data:self.data.clone(), global_id:0, id:0 });
		...
		fn succ(current:&GlobalReference) -> Option<GlobalReference> {
			...
			while (global_id as usize) < current.data.globals.len() {
				let reference = current
					...
					.map(|_| GlobalReference { data:current.data.clone(), global_id, id });
	...
	> {
		let first = self
			...
			.first()
			.map(|_| UnresolvedReference { data:self.data.clone(), id:0 });
		...
		fn succ(current:&UnresolvedReference) -> Option<UnresolvedReference> {
			...
			current
				...
				.unresolved_references
				.get(id as usize)
				.map(|_| UnresolvedReference { data:current.data.clone(), id })
		}
	...
	pub fn has_exports(&self) -> bool { self.data.has_exports() }
	...
	pub fn is_constant(&self, expr:&AnyJsExpression) -> bool { is_constant::is_constant(expr) }
	...
	pub fn as_binding(&self, binding:&impl IsBindingAstNode) -> Binding {
		...
		Binding { data:self.data.clone(), id }
	}
	...
	/// ```rust
	/// use biome_js_parser::JsParserOptions;
	/// use biome_js_semantic::{CallsExtensions, SemanticModelOptions, semantic_model};
	/// use biome_js_syntax::{AnyJsFunction, JsFileSource};
	/// use biome_rowan::{AstNode, SyntaxNodeCast};
	///
	/// let r = biome_js_parser::parse(
	/// 	"function f(){} f() f()",
	...
	/// let model = semantic_model(&r.tree(), SemanticModelOptions::default());
	///
	/// let f_declaration = r.syntax().descendants().filter_map(AnyJsFunction::cast).next().unwrap();
	///
	...
	pub fn all_calls(&self, function:&AnyJsFunction) -> Option<AllCallsIter> {
		Some(AllCallsIter {
			references:function.binding()?.as_js_identifier_binding()?.all_reads(self),
		})
	}
}
