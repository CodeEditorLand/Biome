...
use biome_js_syntax::binding_ext::{
	AnyJsBindingDeclaration, AnyJsIdentifierBinding,
};
use biome_js_syntax::{
	inner_string_text, AnyJsIdentifierUsage, JsDirective, JsLanguage,
	JsSyntaxKind, JsSyntaxNode, TextRange, TsTypeParameterName,
};
use biome_js_syntax::{
	AnyJsImportClause, AnyJsNamedImportSpecifier, AnyTsType,
};
...
impl SemanticEventExtractor {
		...
		match node.kind() {
			...
			| TS_LITERAL_ENUM_MEMBER_NAME => {
				self.enter_identifier_binding(
					&AnyJsIdentifierBinding::unwrap_cast(node.clone()),
				);
			},
			...
			| JS_IDENTIFIER_ASSIGNMENT => {
				self.enter_identifier_usage(AnyJsIdentifierUsage::unwrap_cast(
					node.clone(),
				));
			},
			...
			JS_DIRECTIVE => {
					...
					.is_ok_and(|diretcive| diretcive == "use strict")
				{
					...
					if node.grand_parent().is_some_and(|grand_parent| {
						grand_parent.kind() == JS_FUNCTION_BODY
					}) {
	...
	fn leave_any_type(&mut self, node: &AnyTsType) {
		...
		if matches!(node.kind(), JsSyntaxKind::TS_CONDITIONAL_TYPE)
			&& !self.infers.is_empty()
		{
			self.push_infers_in_scope()
		}
	}
	...
	fn push_infers_in_scope(&mut self) {
		...
		for infer in infers {
			if let Ok(name_token) = infer.ident_token() {
				...
				let binding_info =
					BindingInfo::new(name_range, JsSyntaxKind::TS_INFER_TYPE);
	...
	fn pop_scope(&mut self, scope_range: TextRange) {
		...
		for (name, mut references) in scope.references {
			...
			}) = self.bindings.get(&name)
			{
				...
				.iter()
				.all(|r| matches!(r, Reference::Export(_)))
				&& self.bindings.contains_key(&name.clone().dual())
			{
				...
				let parent_references =
					parent.references.entry(name).or_default();
	...
	/// the `f` scope index.
	fn scope_index_to_hoist_declarations(
		&mut self,
		skip: u32,
	) -> Option<ScopeId> {
		...
		self.scopes
			...
			.skip(skip as usize)
			.find(|scope| {
				scope.hoisting == ScopeHoisting::DontHoistDeclarationsToParent
			})
			.map(|x| x.scope_id)
...
pub fn semantic_events(
	root: JsSyntaxNode,
) -> impl IntoIterator<Item = SemanticEvent> {
