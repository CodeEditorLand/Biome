...
use biome_rowan::{AstNode, SyntaxNodeOptionExt, TextSize, TokenText, syntax::Preorder};
...
struct Scope {
	...
	/// References that still needs to be bound and will be solved at the end of
	/// the scope.
...
impl SemanticEventExtractor {
		...
		match node.kind() {
			...
			JS_MODULE => {
				self.push_scope(
					...
					ScopeOptions { is_closure:false, implies_strict_mode:true },
				);
			},
			...
			JS_SCRIPT => {
				self.push_scope(
					node.text_trimmed_range(),
					ScopeHoisting::DontHoistDeclarationsToParent,
					ScopeOptions { is_closure:false, implies_strict_mode:false },
				);
			},
			...
			| JS_SETTER_OBJECT_MEMBER => {
				self.push_scope(
					node.text_trimmed_range(),
					ScopeHoisting::DontHoistDeclarationsToParent,
					ScopeOptions { is_closure:true, implies_strict_mode:false },
				);
			},
	...
	fn enter_identifier_binding(&mut self, node:&AnyJsIdentifierBinding) {
		...
		let is_exported = if let Ok(name_token) = node.name_token() {
			...
			if let Some(declaration) = node.declaration() {
				...
				let info =
					BindingInfo::new(name_token.text_trimmed_range().start(), node.syntax().kind());
					...
					_ => {
						...
						match parent.ancestors().find(|x| x.kind() != TS_QUALIFIED_NAME).kind() {
							...
							Some(TS_TYPEOF_TYPE) => {
								...
								// We handle this particular case in `pop_scope` (unresolved
								// reference)
								self.push_reference(
									BindingName::Value(name.clone()),
									Reference::AmbientRead(range),
	...
	fn pop_scope(&mut self, scope_range:TextRange) {
		...
		for (name, mut references) in scope.references {
			if let Some(&BindingInfo { range_start: declaration_range_start, declaration_kind }) =
				...
				for reference in references {
					match reference {
						Reference::Export(_) => {
							...
							// If a dual binding exists, then it exports the
							// dual binding.
						},
