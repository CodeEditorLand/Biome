...
use biome_js_syntax::{
	AnyJsIdentifierUsage, JsDirective, JsLanguage, JsSyntaxKind, JsSyntaxNode, TextRange, TsTypeParameterName,
	inner_string_text,
};
...
impl Reference {
	...
	const fn range(&self) -> TextRange {
		match self {
			Self::Export(range) | Self::Read(range) | Self::AmbientRead(range) | Self::Write(range) => *range,
		}
	}
}
...
impl SemanticEventExtractor {
	...
	pub fn enter(&mut self, node: &JsSyntaxNode) {
		...
		match node.kind() {
			...
			JS_IDENTIFIER_BINDING | TS_IDENTIFIER_BINDING | TS_TYPE_PARAMETER_NAME | TS_LITERAL_ENUM_MEMBER_NAME => {
			...
			JS_CONSTRUCTOR_CLASS_MEMBER | JS_METHOD_CLASS_MEMBER | JS_GETTER_CLASS_MEMBER | JS_SETTER_CLASS_MEMBER => {
				self.push_scope(
					node.text_trimmed_range(),
					ScopeHoisting::DontHoistDeclarationsToParent,
					ScopeOptions {
			...
			JS_BLOCK_STATEMENT | JS_FOR_STATEMENT | JS_FOR_OF_STATEMENT | JS_FOR_IN_STATEMENT | JS_SWITCH_STATEMENT
			| JS_CATCH_CLAUSE => {
				self.push_scope(
					node.text_trimmed_range(),
	...
	fn enter_any_type(&mut self, node: &AnyTsType) {
		...
		if matches!(
			node.kind(),
			JsSyntaxKind::TS_CONSTRUCTOR_TYPE | JsSyntaxKind::TS_FUNCTION_TYPE | JsSyntaxKind::TS_MAPPED_TYPE
		) {
			self.push_scope(
				node.text_trimmed_range(),
				ScopeHoisting::DontHoistDeclarationsToParent,
	...
	fn enter_identifier_binding(&mut self, node: &AnyJsIdentifierBinding) {
		...
		let is_exported = if let Ok(name_token) = node.name_token() {
			...
			if let Some(declaration) = node.declaration() {
				let info = BindingInfo::new(name_token.text_trimmed_range().start(), declaration.syntax().kind());
				...
				match declaration {
					...
					AnyJsBindingDeclaration::JsFunctionDeclaration(_) => {
						let is_in_strict_mode = self.scopes.last().is_some_and(|scope| scope.is_in_strict_mode);
					...
					| AnyJsBindingDeclaration::TsEnumDeclaration(_) => {
						...
						hoisted_scope_id = self.scopes.get(self.scopes.len() - 2).map(|scope| scope.scope_id);
						self.push_binding(hoisted_scope_id, BindingName::Value(name.clone()), info.clone());
					...
					AnyJsBindingDeclaration::TsMappedType(_) | AnyJsBindingDeclaration::TsTypeParameter(_) => {
						self.push_binding(None, BindingName::Type(name), info);
					},
					...
					AnyJsBindingDeclaration::JsDefaultImportSpecifier(_) => {
						let type_token =
							declaration.parent::<AnyJsImportClause>().and_then(|clause| clause.type_token());
				...
				let info = BindingInfo::new(name_token.text_trimmed_range().start(), node.syntax().kind());
				self.push_binding(None, BindingName::Value(name), info);
				false
			}
		} else {
						...
						match parent.ancestors().find(|x| x.kind() != TS_QUALIFIED_NAME).kind() {
							Some(TS_REFERENCE_TYPE) => {
								if matches!(parent.kind(), TS_QUALIFIED_NAME) {
									self.push_reference(BindingName::Value(name), Reference::AmbientRead(range))
								} else {
							...
							Some(TS_TYPEOF_TYPE) => {
								...
								self.push_reference(BindingName::Value(name.clone()), Reference::AmbientRead(range));
							},
							_ => {
	...
	fn push_infers_in_scope(&mut self) {
		...
		for infer in infers {
			if let Ok(name_token) = infer.ident_token() {
				...
				let binding_info = BindingInfo::new(name_range.start(), JsSyntaxKind::TS_INFER_TYPE);
	...
	fn scope_index_to_hoist_declarations(&mut self, skip: u32) -> Option<ScopeId> {
		...
		// We should at least have the global scope
		// that do not hoist
		debug_assert!(matches!(self.scopes[0].hoisting, ScopeHoisting::DontHoistDeclarationsToParent));
		self.scopes
			.iter()
			.rev()
			.skip(skip as usize)
