...
impl SemanticEventExtractor {
	...
	fn enter_identifier_binding(&mut self, node: &AnyJsIdentifierBinding) {
		...
		self.stash.push_back(SemanticEvent::DeclarationFound { scope_id, hoisted_scope_id, range });
		if is_exported {
			self.stash.push_back(SemanticEvent::Export { range, declaration_at: range.start() });
		}
	}
	...
	fn push_reference(&mut self, binding_name: BindingName, reference: Reference) {
		self.current_scope_mut().references.entry(binding_name).or_default().push(reference);
	}
}
