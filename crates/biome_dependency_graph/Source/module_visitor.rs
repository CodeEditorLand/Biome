use biome_js_syntax::{
	AnyJsArrayBindingPatternElement, AnyJsBinding, AnyJsBindingPattern, AnyJsDeclarationClause, AnyJsExportClause,
	AnyJsImportLike, AnyJsModuleSource, AnyJsObjectBindingPatternMember, AnyJsRoot, AnyTsIdentifierBinding,
	AnyTsModuleName, JsExport, JsExportFromClause, JsExportNamedFromClause, JsExportNamedSpecifierList,
	JsIdentifierBinding, JsVariableDeclaratorList, unescape_js_string,
};
...
impl<'a> ModuleVisitor<'a> {
	pub fn new(root: AnyJsRoot, directory: &'a Utf8Path, resolver: &'a ResolverGeneric<ResolverCache<'a>>) -> Self {
		Self { root, directory, resolver, data: Default::default() }
	}
	...
	fn visit_import(&mut self, node: AnyJsImportLike) {
		...
		match node {
			AnyJsImportLike::JsModuleSource(_) => {
				self.data.static_imports.insert(specifier.to_string(), import);
			},
			AnyJsImportLike::JsCallExpression(_) | AnyJsImportLike::JsImportCallExpression(_) => {
				self.data.dynamic_imports.insert(specifier.to_string(), import);
			},
		}
	}
	...
	fn visit_export(&mut self, node: JsExport) -> Option<()> {
		let jsdoc_comment = node.export_token().ok().and_then(|token| {
			token.leading_trivia().pieces().rev().find_map(|trivia| match trivia.kind() {
		...
		match node.export_clause().ok()? {
			...
			AnyJsExportClause::JsExportNamedFromClause(node) => self.visit_export_named_from_clause(node),
	...
	fn visit_export_declaration_clause(
		...
		let name = match node {
			AnyJsDeclarationClause::JsClassDeclaration(node) => node.id().ok().and_then(get_name)?,
			AnyJsDeclarationClause::JsFunctionDeclaration(node) => node.id().ok().and_then(get_name)?,
			AnyJsDeclarationClause::JsVariableDeclarationClause(node) => {
				return self.visit_export_variable_declarations(node.declaration().ok()?.declarators(), jsdoc_comment);
			},
			AnyJsDeclarationClause::TsDeclareFunctionDeclaration(node) => node.id().ok().and_then(get_name)?,
			...
			AnyJsDeclarationClause::TsInterfaceDeclaration(node) => node.id().ok().and_then(get_ts_name)?,
	...
	fn visit_export_from_clause(&mut self, node: JsExportFromClause) -> Option<()> {
		...
		if let Some(export_as) = node.export_as() {
			let name = export_as
				...
				.ok()?;
			self.data.exports.insert(name, Export::ReexportAll(ReexportAll { import }));
		} else {
	...
	fn visit_export_named_from_clause(&mut self, node: JsExportNamedFromClause) -> Option<()> {
		...
		for specifier in node.specifiers() {
			...
			let name = specifier
				...
				.map(unescape_js_string)?;
			self.data.exports.insert(name, Export::Reexport(import.clone()));
		}
	...
	fn visit_binding_pattern(&mut self, binding: AnyJsBindingPattern, jsdoc_comment: Option<String>) -> Option<()> {
		match binding {
	...
	fn visit_identifier_binding(&mut self, binding: &JsIdentifierBinding, jsdoc_comment: Option<String>) -> Option<()> {
	...
	fn register_export_with_name_and_jsdoc_comment(
		...
		self.data.exports.insert(name.into(), Export::Own(OwnExport { jsdoc_comment }));

		Some(())
	}

	fn import_from_module_source(&self, module_source: AnyJsModuleSource) -> Option<Import> {
		let specifier = module_source.as_js_module_source()?.inner_string_text().ok()?;
		Some(self.import_from_specifier(specifier.text()))
	}

	fn import_from_specifier(&self, specifier: &str) -> Import {
		let resolved_path = self.resolver.resolve(self.directory, specifier).and_then(|resolution| {
