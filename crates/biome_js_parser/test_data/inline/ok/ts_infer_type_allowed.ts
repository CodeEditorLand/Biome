...
type MatchPair<S extends string> = S extends `[${infer A},${infer B}]`
	? [A, B]
	: unknown;
type FirstTwoAndRest<S extends string> =
	S extends `${infer A}${infer B}${infer R}` ? [`${A}${B}`, R] : unknown;
type Trim<S extends string> = S extends `${infer T} ` ? Trim<T> : S;
...
type ArgumentType<T extends (x: any) => any> = T extends (a: infer A) => any
	? A
	: any;
type X3<T> = T extends { a: (x: infer U) => void; b: (x: infer U) => void }
	? U
	: never;
type X1<T extends { x: any; y: any }> = T extends { x: infer X; y: infer Y }
	? [X, Y]
	: any;
type T62<T> = U extends (infer U)[] ? U : U;
type T63<T> = T extends (infer A extends infer B ? infer C : infer D)
	? string
	: number;
type T75<T> = T extends T74<infer U, infer U>
	? T70<U> | T72<U> | T74<U, U>
	: never;
