...
use biome_js_syntax::{
	JsSyntaxKind::{self, EOF, JS_NUMBER_LITERAL, NEWLINE, WHITESPACE},
	T,
};
...
// and make sure the tokens yielded are fully lossless and the source can be
// reconstructed from only the tokens
...
// It parses random strings and puts them back together with the produced tokens
// and compares
#[quickcheck]
fn losslessness(string:String) -> bool {
	// using an mpsc channel allows us to spawn a thread and spawn the lexer there,
	// then if it takes more than 2 seconds we panic because it is 100% infinite
	// recursion
...
fn without_lookahead() {
	...
	assert_eq!(buffered.current_range(), TextRange::at(TextSize::from(0), TextSize::from(3)));
	...
	assert_eq!(buffered.next_token(JsLexContext::default()), JS_NUMBER_LITERAL);
...
fn lookahead() {
	...
	{
		let lookahead = buffered.lookahead_iter().map(|l| l.kind()).collect::<Vec<_>>();
