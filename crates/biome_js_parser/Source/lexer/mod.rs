//! An extremely fast, lookup table based, ECMAScript lexer which yields
//! SyntaxKind tokens used by the rome-js parser. For the purposes of error
//! recovery, tokens may have an error attached to them, which is reflected in
//! the Iterator Item. The lexer will also yield `COMMENT` and `WHITESPACE`
//! tokens.
//!
//! The lexer operates on raw bytes to take full advantage of lookup table
//! optimizations, these bytes **must** be valid utf8, therefore making a lexer
//! from a `&[u8]` is unsafe since you must make sure the bytes are valid utf8.
//! Do not use this to learn how to lex JavaScript, this is just needlessly fast
//! and demonic because i can't control myself :)
//!
...
//! # Warning ⚠️
//!
//! `>>` and `>>>` are not emitted as single tokens, they are emitted as
//! multiple `>` tokens. This is because of TypeScript parsing and productions
//! such as `T<U<N>>`
...
// The first utf8 byte of every valid unicode whitespace char, used for short
// circuiting whitespace checks
...
pub enum JsLexContext {
	...
	/// Lexes a JSX Attribute value. Calls into normal lex token if positioned
	/// at anything that isn't `'` or `"`.
	JsxAttributeValue,
}

impl LexContext for JsLexContext {
	/// Returns true if this is [JsLexContext::Regular]
	fn is_regular(&self) -> bool { matches!(self, JsLexContext::Regular) }
}
...
pub enum JsReLexContext {
	...
	/// * `> > > =` as `>>>=`
	BinaryOperator,
	/// Re-lexes `'<', '<'` as `<<` in places where a type argument is expected
	/// to support `B<<A>()>`
	TypeArgumentLessThan,
	/// Re-lexes an identifier or keyword as a JSX identifier (that allows `-`
	/// tokens)
	JsxIdentifier,
...
pub(crate) struct JsLexer<'src> {
	...
	/// If the source starts with a Unicode BOM, this is the number of bytes
	/// for that token.
...
impl<'src> Lexer<'src> for JsLexer<'src> {
	...
	fn source(&self) -> &'src str { self.source }

	fn current(&self) -> Self::Kind { self.current_kind }
	...
	#[inline]
	fn current_start(&self) -> TextSize { self.current_start }
	...
	fn next_token(&mut self, context:Self::LexContext) -> Self::Kind {
		...
		self.current_flags.set(TokenFlags::PRECEDING_LINE_BREAK, self.after_newline);
	...
	fn has_preceding_line_break(&self) -> bool { self.current_flags.has_preceding_line_break() }

	fn has_unicode_escape(&self) -> bool { self.current_flags.has_unicode_escape() }
	...
	fn finish(self) -> Vec<ParseDiagnostic> { self.diagnostics }

	fn current_flags(&self) -> TokenFlags { self.current_flags }

	fn push_diagnostic(&mut self, diagnostic:ParseDiagnostic) { self.diagnostics.push(diagnostic); }

	fn position(&self) -> usize { self.position }

	fn advance(&mut self, n:usize) { self.position += n; }
...
impl<'src> JsLexer<'src> {
	...
	pub(crate) fn with_options(self, options:JsParserOptions) -> Self { Self { options, ..self } }
			...
			while let Some(current_byte) = self.current_byte() {
				match current_byte {
					...
					_ => {
						...
						// consume ident advances by one position, so move back
						// by one
						self.position -= 1;
						self.consume_ident();
	...
	fn lex_jsx_child_token(&mut self) -> JsSyntaxKind {
		...
		match chr {
			// `<`: empty jsx text, directly followed by another element or
			// closing element
			b'<' => self.eat_byte(T![<]),
			...
			_ => {
				while let Some(chr) = self.current_byte() {
					...
					match chr {
						// Start of a new element, the closing tag, or an
						// expression
						b'<' | b'{' => break,
						b'>' => {
	...
	/// Returns the current byte without checking if the lexer is at the end of
	/// the file.
	///
	/// ## Safety
	/// Calling this function if the lexer is at or passed the end of file is
	/// undefined behaviour.
	#[inline]
	...
	fn read_codepoint_escape_char(&mut self) -> Result<char, ()> {
		...
		if current_byte != Some(b'}') {
			// We should not yield diagnostics on a unicode char boundary. That
			// wont make codespan panic but it may cause a panic for other
			// crates which just consume the diagnostics
			let invalid = self.current_char_unchecked();
			let err = ParseDiagnostic::new(
				"expected hex digits for a unicode code point escape, but encountered an invalid \
				 character",
		...
		// Safety: We know for a fact this is in bounds because we must be on
		// the possible char after the } at this point which means its
		// impossible for the range of the digits to be out of bounds. We also
		// know we cant possibly be indexing a unicode char boundary because a
		// unicode char (which cant be a hexdigit) would have triggered the if
		// statement above. We also know this must be valid utf8, both because
		// of read_hexnumber's behavior and because input to the lexer must be
		// valid utf8
	...
	fn read_unicode_escape(&mut self) -> Result<u32, ()> {
		...
		let digits_str = unsafe {
			std::str::from_utf8_unchecked(
				self.source.as_bytes().get_unchecked((self.position - 3)..(self.position + 1)),
			)
		};
	...
	// Validate a `\x00 escape sequence, this expects the current char to be the
	// `x`, it also does not skip over the escape sequence The pos after this
	// method is the last hex digit
	...
	/// Consumes the identifier at the current position, and fills the given buf
	/// with the UTF-8 encoded identifier that got consumed.
	///
	/// Returns the number of bytes written into the buffer, and if any char was
	/// escaped. This method will stop writing into the buffer if the buffer is
	/// too small to fit the whole identifier.
	#[inline]
	...
	/// Consume a string literal and advance the lexer, and returning a list of
	/// errors that occurred when reading the string This could include
	/// unterminated string and invalid escape sequences
	///
	/// ## Safety
	/// Must be called at a valid UT8 char boundary
	...
	/// Returns `Some(x)` if the current position is an identifier, with the
	/// character at the position.
	///
	/// Boolean states if there are escaped characters.
	///
	/// The character may be a char that was generated from a unicode escape
	/// sequence, e.g. `t` is returned, the actual source code is `\u{74}`
	#[inline]
	...
	// check if the current char is an identifier start, this implicitly
	// advances if the char being matched is a `\uxxxx` sequence which is an
	// identifier start, or if the char is a unicode char which is an identifier
	// start
	#[inline]
	...
	/// Returns the identifier token at the current position, or the keyword
	/// token if the identifier is a keyword.
	///
	...
	fn read_number(&mut self, leading_zero:bool) {
		...
		loop {
			match self.next_byte_bounded() {
				...
				// TODO: merge this, and read_float's implementation into one so
				// we dont duplicate exponent code
				Some(b'e' | b'E') => {
					// At least one digit is required
					match self.peek_byte() {
						Some(b'-' | b'+') => {
	...
	fn read_float(&mut self) {
		loop {
			match self.next_byte_bounded() {
				...
				// LLVM has a hard time optimizing inclusive patterns, perhaps
				// we should check if it makes llvm sad, and optimize this
				// into a lookup table
				Some(b'0'..=b'9') => {},
				Some(b'e' | b'E') => {
					// At least one digit is required
	...
	fn read_slash(&mut self) -> JsSyntaxKind {
		...
		match self.peek_byte() {
			Some(b'*') => {
				...
				.with_detail(self.position..self.position + 1, "... but the file ends here")
	...
	fn flag_err(&self, flag:char) -> ParseDiagnostic {
		ParseDiagnostic::new(format!("Duplicate flag `{flag}`."), self.position..self.position + 1)
	...
	fn read_regex(&mut self) -> JsSyntaxKind {
		...
		impl BitOr for RegexFlags {
			...
			fn bitor(self, rhs:Self) -> Self::Output { RegexFlags(self.0 | rhs.0) }
		}
			...
			match chr {
				...
				b'\\' => {
					...
					match self.current_byte() {
						...
						// eat the next ascii or unicode char followed by the
						// escape char.
	...
	// Dont ask it to resolve the question of life's meaning because you'll be
	// disappointed
	#[inline]
	...
	fn lex_token(&mut self) -> JsSyntaxKind {
		...
		// A lookup table of `byte -> fn(l: &mut Lexer) -> Token` is
		// exponentially slower than this approach The speed difference comes
		// from the difference in table size, a 2kb table is easily fit into cpu
		// cache While a 16kb table will be ejected from cache very often
		// leading to slowdowns, this also allows LLVM to do more aggressive
		// optimizations on the match regarding how to map it to instructions
		...
		match dispatched {
					...
					match res {
						Ok(chr) => {
							if is_js_id_start(chr) {
								...
								let err = ParseDiagnostic::new(
									"unexpected unicode escape",
									start..self.position,
								)
								.with_hint(
									"this escape is unexpected, as it does not designate the \
			...
			UNI => {
				// A BOM can only appear at the start of a file, so if we
				// haven't advanced at all yet, perform the check. At any
				// other position, the BOM is just considered plain whitespace.
				if self.position == 0 {
...
fn is_linebreak(chr:char) -> bool { matches!(chr, '\n' | '\r' | '\u{2028}' | '\u{2029}') }
