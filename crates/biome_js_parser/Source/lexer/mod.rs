...
pub(crate) struct JsLexer<'src> {
	...
	/// If the source starts with a Unicode BOM, this is the number of bytes for
	/// that token.
...
impl<'src> JsLexer<'src> {
	...
	fn read_codepoint_escape_char(&mut self) -> Result<char, ()> {
		...
		if current_byte != Some(b'}') {
			// We should not yield diagnostics on a unicode char boundary. That wont make
			// codespan panic but it may cause a panic for other crates which just
			// consume the diagnostics
		...
		// Safety: We know for a fact this is in bounds because we must be on the
		// possible char after the } at this point which means its impossible for the
		// range of the digits to be out of bounds. We also know we cant possibly be
		// indexing a unicode char boundary because a unicode char (which cant be a
		// hexdigit) would have triggered the if statement above. We also know this
		// must be valid utf8, both because of read_hexnumber's behavior and because
		// input to the lexer must be valid utf8
	...
	// check if the current char is an identifier start, this implicitly advances if
	// the char being matched is a `\uxxxx` sequence which is an identifier start,
	// or if the char is a unicode char which is an identifier start
	#[inline]
	...
	fn read_number(&mut self, leading_zero:bool) {
		...
		loop {
			match self.next_byte_bounded() {
				...
				// TODO: merge this, and read_float's implementation into one so we dont duplicate
				// exponent code
				Some(b'e' | b'E') => {
					// At least one digit is required
					match self.peek_byte() {
	...
	fn read_float(&mut self) {
		loop {
			match self.next_byte_bounded() {
				...
				// LLVM has a hard time optimizing inclusive patterns, perhaps we should check if it
				// makes llvm sad, and optimize this into a lookup table
				Some(b'0'..=b'9') => {},
	...
	fn lex_token(&mut self) -> JsSyntaxKind {
		...
		// A lookup table of `byte -> fn(l: &mut Lexer) -> Token` is exponentially
		// slower than this approach The speed difference comes from the difference in
		// table size, a 2kb table is easily fit into cpu cache While a 16kb table
		// will be ejected from cache very often leading to slowdowns, this also allows
		// LLVM to do more aggressive optimizations on the match regarding how to map
		// it to instructions
		...
		match dispatched {
			...
			UNI => {
				// A BOM can only appear at the start of a file, so if we haven't advanced at
				// all yet, perform the check. At any other position, the BOM is just
				// considered plain whitespace.
				if self.position == 0 {
