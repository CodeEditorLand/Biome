...
///
/// This is essentially a hack to allow us to use SyntaxElement, SyntaxNode, etc
/// directly
pub trait Span {
...
impl<T:Span> Span for &T {
	fn as_range(&self) -> TextRange { (*self).as_range() }
}
...
impl<T:Language> Span for SyntaxNode<T> {
	fn as_range(&self) -> TextRange { self.text_range() }
}
...
impl Span for TextRange {
	fn as_range(&self) -> TextRange { *self }
}
