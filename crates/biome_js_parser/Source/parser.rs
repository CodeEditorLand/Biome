...
use crate::{
	lexer::JsReLexContext,
	prelude::*,
	state::{ChangeParserState, JsParserStateCheckpoint, ParserStateGuard},
	token_source::{JsTokenSource, JsTokenSourceCheckpoint},
	*,
};
...
/// These events are then processed into a syntax tree through a [`TreeSink`]
/// implementation.
...
impl<'source> JsParser<'source> {
	...
	pub(crate) fn state(&self) -> &JsParserState { &self.state }

	pub(crate) fn options(&self) -> &JsParserOptions { &self.options }

	pub(crate) fn state_mut(&mut self) -> &mut JsParserState { &mut self.state }

	pub fn source_type(&self) -> JsFileSource { self.source_type }

	/// Whether the code we are parsing is a module
	pub const fn is_module(&self) -> bool { self.source_type.module_kind().is_module() }

	/// Re-lexes the current token in the specified context. Returns the kind
	/// of the re-lexed token (can be the same as before if the context doesn't
	/// make a difference for the current token)
	pub fn re_lex(&mut self, context:JsReLexContext) -> JsSyntaxKind {
		self.source_mut().re_lex(context)
	}

	/// Stores the parser state and position before calling the function and
	/// restores the state and position before returning.
	///
	/// Useful in situation where the parser must advance a few tokens to
	/// determine whatever a syntax is of one or the other kind.
	#[inline]
	...
	/// Applies the passed in change to the parser state before applying the
	/// passed `func` and restores the state to before the change before
...
impl<'source> Parser for JsParser<'source> {
	...
	fn context(&self) -> &ParserContext<Self::Kind> { &self.context }

	fn context_mut(&mut self) -> &mut ParserContext<Self::Kind> { &mut self.context }

	fn source(&self) -> &Self::Source { &self.source }

	fn source_mut(&mut self) -> &mut Self::Source { &mut self.source }

	fn is_speculative_parsing(&self) -> bool { self.state.speculative_parsing }
...
mod tests {
	...
	#[test]
	#[should_panic(expected = "Marker must either be `completed` or `abandoned` to avoid that \
	                           children are implicitly attached to a marker's parent.")]
	fn uncompleted_markers_panic() {
		let mut parser =
			JsParser::new("'use strict'", JsFileSource::default(), JsParserOptions::default());
