...
use biome_js_syntax::{AnyJsRoot, JsCallArguments, JsFileSource, JsLogicalExpression, JsSyntaxKind, JsSyntaxToken};
use biome_rowan::{AstNode, Direction, TextSize};
use expect_test::expect_file;

use crate::{
	JsParserOptions, Parse, parse, parse_module,
...
fn try_parse_with_printed_ast(path: &str, text: &str, options: JsParserOptions) -> (Parse<AnyJsRoot>, String) {
	catch_unwind(|| {
	...
	.unwrap_or_else(|err| {
		...
		// `SyntaxNode`s aren't unwind safe. That's why the same `ParseResult` can't
		// be reused here. This should be fine because this code is only executed for
		// local tests. No checked-in test should ever hit this line.
