...
pub fn jsroot_ranges() {
	...
	let eq = syntax.descendants_tokens(Direction::Next).find(|x| x.text_trimmed() == "=").unwrap();
...
pub fn node_contains_trailing_comments() {
	...
	let syntax = root.syntax();
	let node =
		syntax.descendants().find(|n| n.kind() == JsSyntaxKind::JS_LOGICAL_EXPRESSION).unwrap();
...
fn diagnostics_print_correctly() {
	...
	for diagnostic in root.diagnostics() {
		let mut write = biome_diagnostics::termcolor::Buffer::no_color();
		let error =
			diagnostic.clone().with_file_path("example.js").with_file_source_code(text.to_string());
