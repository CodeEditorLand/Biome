...
use crate::{
	...
	prelude::*,
	state::{EnterParameters, SignatureFlags},
	syntax::{
		...
		js_parse_error,
		js_parse_error::decorators_not_allowed,
		typescript::{
...
impl ParseSeparatedList for ObjectMembersList {
	...
	fn parse_element(&mut self, p:&mut JsParser) -> ParsedSyntax { parse_object_member(p) }
	...
	match p.cur() {
		...
		_ => {
				...
				&& !token_set![T!['('], T![<], T![:]].contains(p.nth(1))
			{
				...
				// There are multiple places where it's first needed to parse an
				// expression to determine if it is an assignment target or
				// not. This requires that parse expression is valid for any
				// assignment expression. Thus, it's needed that the parser
				// silently parses over a "{ arrow = test }" property
				if p.at(T![=]) {
			...
			if p.at(T!['(']) || p.at(T![<]) {
				...
				if p.eat(T![:]) {
					...
					// It turns out that this isn't a valid member after all.
					// Make sure to throw away everything that has been
					// parsed so far so that the caller can do its error
					// recovery
					p.rewind(checkpoint);
					m.abandon(p);
					Absent
...
fn parse_setter_object_member(p:&mut JsParser) -> ParsedSyntax {
	...
	p.with_state(EnterParameters(SignatureFlags::empty()), |p| {
		// test_err ts ts_decorator_object { "parse_class_parameter_decorators":
		// true } ({
...
pub(crate) fn is_nth_at_type_member_name(p:&mut JsParser, offset:usize) -> bool {
	...
	let start_names =
		token_set![JS_STRING_LITERAL, JS_NUMBER_LITERAL, T![ident], T![await], T![yield], T!['[']];

	nth.is_keyword() || start_names.contains(nth)
}

pub(crate) fn is_at_object_member_name(p:&mut JsParser) -> bool { is_nth_at_type_member_name(p, 0) }
...
pub(super) fn is_at_literal_member_name(p:&mut JsParser, offset:usize) -> bool {
	matches!(p.nth(offset), JS_STRING_LITERAL | JS_NUMBER_LITERAL | T![ident])
...
//     async *id<R>(param: Promise<R>): AsyncIterableIterator<R> { yield await
// param }, })

// test_err ts ts_method_object_member_body_error
// ({
//     x<>(maybeA: any): maybeA is A { return true },
//     async *id<>(param: Promise<R>): AsyncIterableIterator<R> { yield await
// param }, })

/// Parses the body of a method object member starting right after the member
/// name.
fn parse_method_object_member_body(p:&mut JsParser, flags:SignatureFlags) {
	...
	parse_parameter_list(p, ParameterContext::Implementation, TypeContext::default(), flags)
