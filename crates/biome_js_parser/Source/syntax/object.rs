	...
	match p.cur() {
		...
		_ => {
				...
				&& !token_set![T!['('], T![<], T![:]].contains(p.nth(1))
			{
				...
				// There are multiple places where it's first needed to parse an expression to
				// determine if it is an assignment target or not. This requires that parse
				// expression is valid for any assignment expression. Thus, it's needed that
				// the parser silently parses over a "{ arrow = test }" property
				if p.at(T![=]) {
