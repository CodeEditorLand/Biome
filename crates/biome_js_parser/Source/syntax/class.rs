...
use biome_js_syntax::{JsSyntaxKind, JsSyntaxKind::*, TextSize, T};
use biome_parser::{
...
use super::{
	function::LineBreak,
	js_parse_error::unexpected_body_inside_ambient_context,
	metavariable::{is_at_metavariable, parse_metavariable},
	typescript::{
		expect_ts_index_signature_member,
		is_at_ts_index_signature_member,
		ts_parse_error::{self, unexpected_abstract_member_with_body},
		MemberParent,
	},
};
use crate::{
	parser::{ParsedSyntax, RecoveryResult},
	prelude::*,
	state::{
		EnableStrictMode,
		EnterClassPropertyInitializer,
	...
	syntax::{
		...
		js_parse_error,
		js_parse_error::{
			decorator_must_precede_modifier,
			decorators_not_allowed,
			expected_binding,
			expected_expression,
			invalid_decorator_error,
			modifier_already_seen,
			modifier_cannot_be_used_with_modifier,
			modifier_must_precede_modifier,
			parameter_decorators_not_allowed,
		},
		object::{
			is_at_literal_member_name,
			parse_computed_member_name,
			parse_literal_member_name,
		},
		stmt::{optional_semi, parse_statements, StatementContext},
		typescript::{
			is_reserved_type_name,
			parse_ts_implements_clause,
			parse_ts_return_type_annotation,
			parse_ts_type_annotation,
			parse_ts_type_arguments,
			parse_ts_type_parameters,
			ts_parse_error::{
				ts_accessibility_modifier_already_seen,
				ts_accessor_type_parameters_error,
				ts_constructor_type_parameters_error,
...
pub(super) fn parse_class_expression(p:&mut JsParser, decorator_list:ParsedSyntax) -> ParsedSyntax {
	if !p.at(T![class]) {
		return Absent;
	}
...
fn parse_class(p:&mut JsParser, kind:ClassKind, decorator_list:ParsedSyntax) -> CompletedMarker {
		...
		Present(id) => {
			...
			if TypeScript.is_supported(p) && is_reserved_type_name(text) {
				...
				let err = p.err_builder(
					format!(
						"`{text}` cannot be used as a class name because it is already reserved \
						 as a type"
					),
					id.range(p),
				);
	...
	p.expect(T!['{']);
	ClassMembersList { inside_abstract_class:is_abstract }.parse_list(p);
	p.expect(T!['}']);
...
// class E extends {}
// class F extends E, {}
/// Eats a class's 'implements' and 'extends' clauses, attaching them to the
/// current active node. Implements error recovery in case a class has multiple
/// extends/implements clauses or if they appear out of order
fn eat_class_heritage_clause(p:&mut JsParser) {
	...
	loop {
		match p.cur() {
			...
			T![implements] => {
				let mut current = parse_ts_implements_clause(p).expect(
					"expected 'implements' clause because parser is positioned at 'implements' \
					 keyword.",
				);
...
fn parse_extends_clause(p:&mut JsParser) -> ParsedSyntax {
	...
	while p.at(T![,]) {
		...
		p.error(p.err_builder("Classes can only extend a single class.", extra_class.range(p)));
	}
...
fn parse_extends_expression(p:&mut JsParser) -> ParsedSyntax {
	if p.at(T!['{']) && p.nth_at(1, T!['}']) {
		// Don't eat the body of the class as an object expression except if we
		// have
		// * extends {} {
		// * extends {} implements
		// * extends {},
...
fn parse_class_member(p:&mut JsParser, inside_abstract_class:bool) -> ParsedSyntax {
	...
	match member {
		...
		Absent => {
			// If the modifier list contains a modifier other than a decorator,
			// such modifiers can also be valid member names.
			debug_assert!(!modifiers.flags.contains(ModifierFlags::ALL_MODIFIERS_EXCEPT_DECORATOR));
...
fn parse_class_member_impl(
	...
	if matches!(p.cur(), T![get] | T![set]) && is_at_class_member_name(p, 1) {
		...
		let completed = if is_getter {
			...
			{
				p.error(ts_set_accessor_return_type_error(p, &return_type_annotation));
			}
	...
	if is_at_method_class_member(p, 0) {
		...
		return if is_constructor {
			Present(parse_constructor_class_member_body(p, member_marker, modifiers))
		} else {
...
/// If the current member is abstract, the [ParsedSyntax] will have kind
/// TS_PROPERTY_SIGNATURE_CLASS_MEMBER, otehrwise will be
/// JS_PROPERTY_CLASS_MEMBER.
///
///  # Arguments
///
/// * `p` - Parser being used
/// * `member_marker` - Marker that will be completed at the end of this
///   function
/// * `modifiers` - All the member modifiers parsed previously. This will be
///   used for validation and for the [ParsedSyntax::kind]
	...
	if let Present(initializer) = &initializer_syntax {
		if modifiers.has(ModifierKind::Abstract) {
			...
			if !modifiers.has(ModifierKind::Readonly) {
				...
				p.error(
					p.err_builder(
						"In ambient contexts, properties cannot have both a type annotation and \
						 an initializer.",
						initializer.range(p),
					)
...
//   c!: string;
// }
fn parse_ts_property_annotation(p:&mut JsParser, modifiers:&ClassMemberModifiers) -> ParsedSyntax {
	if !p.at(T![?]) && !p.at(T![!]) {
	...
	let mut annotation = match (optional_range, definite_range) {
		...
		(None, Some(_)) => {
			parse_ts_type_annotation(p, TypeContext::default()).or_add_diagnostic(p, |p, range| {
				p.err_builder(
					"Properties with definite assignment assertions must also have type \
					 annotations.",
					range,
				)
			});
		...
		// handled by the test at the beginning of the function that returns if
		// the parser isn't at a ! or ? token.
...
/// Eats the '?' token for optional member. Emits an error if this isn't
/// typescript
...
// class B { lorem = ; }
pub(crate) fn parse_initializer_clause(p:&mut JsParser, context:ExpressionContext) -> ParsedSyntax {
	if p.at(T![=]) {
		let m = p.start();
		p.bump(T![=]);
...
/// Parses the body (everything after the identifier name) of a method class
/// member that includes: parameters and its types, return type and method body
fn parse_method_class_member_rest(
	...
	let parameter_context = if modifiers.is_signature() {
		...
		// Not perfect. It may turn out that this is a method overload without a
		// body in which case this isn't an implementation.
	...
	if modifiers.has(ModifierKind::Abstract) && is_async {
		...
		// }
		p.error(
			p.err_builder("A method signature cannot be declared as a generator.", member.range(p)),
		);
...
impl MemberKind {
	const fn is_signature(&self) -> bool { matches!(self, MemberKind::Signature) }
...
impl ClassMethodMemberKind {
	...
	const fn is_body_optional(&self) -> bool {
		matches!(self, ClassMethodMemberKind::Method(_) | ClassMethodMemberKind::Constructor)
	}

	const fn is_constructor(&self) -> bool { matches!(self, ClassMethodMemberKind::Constructor) }
...
/// Parses the body of a method (constructor, getter, setter, or regular
/// method).
///
/// TypeScript supports method signatures. These are methods without a body (and
/// are terminated by a semicolon or ASI). A method is a signature if one of the
/// following applies
/// * The member has an `abstract` modifier (not supported by constructors)
/// * It's a declaration in an ambient context (`declare class { ... }` or
///   `declare namespace { class { ... } }`).
/// * It's a method overload (doesn't apply to getters/setters)
///
/// The method determines which case applies to the current member and emits a
/// diagnostic if:
...
///
/// The method returns the inferred kind (signature or declaration) of the
/// parsed method body
	...
	// }
		...
		match body {
			...
			Absent => {
				...
				// abstract class A { abstract constructor() abstract method()
				// abstract get getter() abstract set setter(value) abstract
				// prop }
				expect_member_semi(p, member_marker, "method declaration")
			},
		}
...
fn parse_constructor_class_member_body(
	...
	let constructor_kind =
		expect_method_body(p, &member_marker, modifiers, ClassMethodMemberKind::Constructor);
...
fn parse_constructor_parameter_list(p:&mut JsParser) -> ParsedSyntax {
	...
	parse_parameters_list(p, flags, parse_constructor_parameter, JS_CONSTRUCTOR_PARAMETER_LIST);
...
fn parse_constructor_parameter(p:&mut JsParser, context:ExpressionContext) -> ParsedSyntax {
	...
	// test ts ts_decorator_constructor { "parse_class_parameter_decorators":
	// true } class C {
	...
	if is_nth_at_modifier(p, 0, true) {
		...
		// class B { constructor(readonly w, private readonly x, protected
		// readonly y, public readonly z) {} } class C { constructor(private
		// x: string, readonly y?, z = "default", ...rest) {} }
		//
		...
		// we pass decorator list as Absent because TsPropertyParameter has its
		// own decorator list
		parse_formal_parameter(
			p,
			Absent,
...
pub(crate) fn is_nth_at_modifier(p:&mut JsParser, n:usize, constructor_parameter:bool) -> bool {
	// Test if this modifier is followed by another modifier, member name or any
	// other token that starts a new member. If that's the case, then this is
	// fairly likely a modifier. If not, then this is probably not a modifier,
	// but the name of the member. For example, all these are valid
	// members: `static() {}, private() {}, protected() {}`... but are modifiers
	// if followed by another modifier or a name: `static x() {} private
	// static() {}`...
	if !matches!(
		p.nth(n),
		T![declare]
...
/// Parses all possible modifiers regardless of what the current member is. It's
/// up to the caller to create diagnostics for not allowed modifiers.
...
fn parse_modifier(p:&mut JsParser, constructor_parameter:bool) -> Option<ClassMemberModifier> {
	...
	if !is_nth_at_modifier(p, 0, constructor_parameter) && !is_at_decorator_modifier {
		// all modifiers can also be valid member names. That's why we shouldn't
		// parse a modifier if it isn't followed by a valid member name or
		// another modifier
		return None;
	}
...
/// Useful to cheaply track all already seen modifiers of a member (instead of
/// using a HashSet<ModifierKind>).
#[derive(Debug, Copy, Clone, Eq, PartialEq)]
...
impl BitOr for ModifierFlags {
	...
	fn bitor(self, rhs:Self) -> Self::Output { ModifierFlags(self.0 | rhs.0) }
}
...
impl ModifierKind {
	const fn is_ts_modifier(&self) -> bool {
		!matches!(self, ModifierKind::Static | ModifierKind::Accessor | ModifierKind::Decorator)
	}
...
impl ClassMemberModifier {
	fn as_text_range(&self) -> TextRange { TextRange::new(self.start, self.start.add(self.length)) }
}

// 4 is sufficient to store all valid modifiers without requiring any heap
// allocations.
...
impl ClassMemberModifierList {
	...
	fn add_modifier(&mut self, modifier:ClassMemberModifier) { self.0.push(modifier); }

	fn iter(&self) -> Iter<'_, ClassMemberModifier> { self.0.iter() }
...
struct ClassMemberModifiers {
	...
	// Stores all added flags. Useful to determine with `O(1)` if a modifier
...
impl ClassMemberModifiers {
	fn new(
		...
		Self {
			...
			bomb:DebugDropBomb::new(
				"list must either be 'completed' or 'abandoned' by calling 'complete' or \
				 'abandon'.",
			),
		}
	}
	...
	fn has(&self, kind:ModifierKind) -> bool { self.flags.contains(kind.as_flags()) }

	fn is_signature(&self) -> bool {
		self.has(ModifierKind::Abstract) || self.has(ModifierKind::Declare)
	}

	/// Returns the range for the passed in modifier or [None] if the modifier
	/// isn't set or is a missing marker
	fn get_first_range(&self, kind:ModifierKind) -> Option<TextRange> {
		if self.flags.contains(kind.as_flags()) {
			self.modifiers.iter().find(|m| m.kind == kind).map(|m| m.as_text_range())
		} else {
			None
		}
	}
	...
	///
	/// ## Safety
	/// Expected that the caller first checked that such a modifier is present
	/// (Either by using `has` or by iterating over all modifiers and keeping
	/// track of the modifier it has seen).
	...
	fn is_empty(&self) -> bool { self.modifiers.is_empty() }
	...
	/// Validates if these modifiers are valid for a member of the given kind
	/// and completes the modifier list.
	///
	...
	fn validate_and_complete(mut self, p:&mut JsParser, member_kind:JsSyntaxKind) -> bool {
		...
		let list_kind = match member_kind {
			...
			JS_BOGUS_MEMBER | JS_STATIC_INITIALIZATION_BLOCK_CLASS_MEMBER => {
				// Error recovery kicked in. There's no "right" list to pick in
				// this case, let's just remove it
				self.list_marker.undo_completion(p).abandon(p);
				return false;
			},
	...
	fn check_class_member_modifier(
		...
		// test_err ts
		// ts_index_signature_class_member_cannot_have_visibility_modifiers
		// class A {
		...
		//  constructor(private priv: string, protected prot: string, public
		// pub: string, override name: string, readonly read: string, protected
		// override readonly other: string) {      super();
		//  }
		// }
		member_kind == TS_PROPERTY_PARAMETER
			...
			return Some(ts_modifier_cannot_appear_on_a_parameter(p, modifier.as_text_range()));
		}
		...
		match modifier.kind {
			...
			ModifierKind::Readonly => {
				if preceding_modifiers.contains(ModifierFlags::READONLY) {
					...
					JS_PROPERTY_CLASS_MEMBER
						...
						| JS_BOGUS_MEMBER | TS_PROPERTY_PARAMETER
				) {
			...
			ModifierKind::Abstract => {
				if preceding_modifiers.contains(ModifierFlags::ABSTRACT) {
					...
					// test_err ts
					// typescript_abstract_classes_abstract_accessor_precedence
			...
			ModifierKind::Private | ModifierKind::Protected | ModifierKind::Public => {
					...
					&& modifier.kind == ModifierKind::Private
				{
					...
					// test_err ts
					// typescript_classes_invalid_accessibility_modifier_private_member
