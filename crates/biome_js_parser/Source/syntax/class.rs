...
use biome_js_syntax::{JsSyntaxKind, JsSyntaxKind::*, T, TextSize};
use biome_parser::{
	ParserProgress,
...
use crate::{
	...
	syntax::{
		...
		stmt::{StatementContext, optional_semi, parse_statements},
		typescript::{
			TypeContext,
...
fn parse_class_member(p:&mut JsParser, inside_abstract_class:bool) -> ParsedSyntax {
	...
	match member {
		...
		Absent => {
			// If the modifier list contains a modifier other than a decorator, such
			// modifiers can also be valid member names.
	...
	if matches!(p.cur(), T![get] | T![set]) && is_at_class_member_name(p, 1) {
		...
		let completed = if is_getter {
			...
			p.with_state(EnterParameters(SignatureFlags::empty()), |p| {
				...
				// test ts ts_decorator_on_class_setter { "parse_class_parameter_decorators":
				// true } class A {
				//     set val(@dec x) {}
				//     set val(@dec.fn() x) {}
				//     set val(@dec() x) {}
...
fn parse_ts_property_annotation(p:&mut JsParser, modifiers:&ClassMemberModifiers) -> ParsedSyntax {
	...
	let mut annotation = match (optional_range, definite_range) {
		...
		// handled by the test at the beginning of the function that returns if the parser isn't at
		// a ! or ? token.
...
fn parse_method_class_member_rest(
	...
	let parameter_context = if modifiers.is_signature() {
		...
		// Not perfect. It may turn out that this is a method overload without a body in
		// which case this isn't an implementation.
	...
	// }
		...
		match body {
			...
			Absent => {
				...
				// abstract class A { abstract constructor() abstract method() abstract get
				// getter() abstract set setter(value) abstract prop }
...
fn parse_constructor_parameter(p:&mut JsParser, context:ExpressionContext) -> ParsedSyntax {
	...
	if is_nth_at_modifier(p, 0, true) {
		...
		// class B { constructor(readonly w, private readonly x, protected readonly y,
		// public readonly z) {} } class C { constructor(private x: string, readonly
		// y?, z = "default", ...rest) {} }
		//
		...
		// we pass decorator list as Absent because TsPropertyParameter has its own
		// decorator list
		parse_formal_parameter(
			p,
			Absent,
...
pub(crate) fn is_nth_at_modifier(p:&mut JsParser, n:usize, constructor_parameter:bool) -> bool {
	...
	// fairly likely a modifier. If not, then this is probably not a modifier, but
	// the name of the member. For example, all these are valid members: `static()
	// {}, private() {}, protected() {}`... but are modifiers if followed by another
	// modifier or a name: `static x() {} private static() {}`...
	if !matches!(
		p.nth(n),
		T![declare]
			| T![public]
...
fn parse_modifier(p:&mut JsParser, constructor_parameter:bool) -> Option<ClassMemberModifier> {
	...
	if !is_nth_at_modifier(p, 0, constructor_parameter) && !is_at_decorator_modifier {
		// all modifiers can also be valid member names. That's why we shouldn't parse a
		// modifier if it isn't followed by a valid member name or another modifier
		return None;
	}
...
struct ClassMemberModifiers {
	...
	// Stores all added flags. Useful to determine with `O(1)` if a modifier exists in the list or
	// not
	flags:ModifierFlags,
...
impl ClassMemberModifiers {
	...
	fn validate_and_complete(mut self, p:&mut JsParser, member_kind:JsSyntaxKind) -> bool {
		...
		let list_kind = match member_kind {
			...
			JS_BOGUS_MEMBER | JS_STATIC_INITIALIZATION_BLOCK_CLASS_MEMBER => {
				// Error recovery kicked in. There's no "right" list to pick in this case, let's
				// just remove it
				self.list_marker.undo_completion(p).abandon(p);
	...
	fn check_class_member_modifier(
		...
		//  constructor(private priv: string, protected prot: string, public pub: string, override
		// name: string, readonly read: string, protected override readonly other: string) {
		//      super();
		//  }
		// }
