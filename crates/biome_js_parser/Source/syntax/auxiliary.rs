use biome_js_syntax::{
	JsSyntaxKind::{JS_BOGUS_STATEMENT, JS_VARIABLE_DECLARATION_CLAUSE},
	T,
};
...
use crate::{
	...
	prelude::*,
	syntax::{
		class::{parse_class_declaration, parse_decorators},
		...
		stmt::{
			...
			is_nth_at_variable_declarations,
			parse_variable_declaration,
			semi,
		},
		typescript::{
			is_nth_at_any_ts_namespace_declaration,
			is_nth_at_ts_interface_declaration,
			parse_any_ts_namespace_declaration_clause,
...
pub(crate) fn is_nth_at_declaration_clause(p:&mut JsParser, n:usize) -> bool {
	if matches!(p.nth(n), T![function] | T![const] | T![enum] | T![class] | T![import] | T![@]) {
		return true;
	}
