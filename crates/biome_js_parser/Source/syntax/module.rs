...
use crate::{
	...
	syntax::{
		...
		function::{LineBreak, parse_function_export_default_declaration},
		js_parse_error::{
		...
		stmt::{STMT_RECOVERY_SET, StatementContext, parse_statement, semi},
		typescript::{
...
fn parse_import_clause(p:&mut JsParser) -> ParsedSyntax {
	...
	let clause = match p.cur() {
		...
		_ => {
			// SAFETY: Safe because the parser only eats the "type" keyword if it's followed
			// by either a *, {, or binding
...
where
	...
	// `as`) { type as as as }    // name: `as`      type-export: `true`, aliased
	// to `as` ```
	if p.at(T![type]) {
		// `{ type identifier }`
