...
use biome_parser::{
	...
	parse_recovery::RecoveryResult,
	ParserProgress,
};
use rustc_hash::FxHashMap;

use super::{
	auxiliary::{is_nth_at_declaration_clause, parse_declaration_clause},
	js_parse_error::{expected_named_import, expected_namespace_import},
	metavariable::{is_at_metavariable, parse_metavariable},
};
use crate::{
	lexer::TextSize,
	prelude::*,
	state::{EnterAmbientContext, ExportDefaultItem, ExportDefaultItemKind},
	syntax::{
		binding::{
			is_at_identifier_binding,
			is_nth_at_identifier_binding,
			parse_binding,
			parse_identifier_binding,
		},
		class::{
			empty_decorator_list,
			is_at_export_class_declaration,
			is_at_export_default_class_declaration,
			is_at_ts_abstract_class_declaration,
			parse_class_declaration,
			parse_class_export_default_declaration,
			parse_decorators,
		},
		expr::{
			is_at_expression,
		...
		function::{parse_function_export_default_declaration, LineBreak},
		js_parse_error::{
			decorators_not_allowed,
			duplicate_assertion_keys_error,
			expected_binding,
			expected_declaration,
			expected_export_clause,
			expected_export_default_declaration,
			...
			expected_namespace_or_named_import,
			expected_statement,
		},
		stmt::{parse_statement, semi, StatementContext, STMT_RECOVERY_SET},
		typescript::{
			parse_ts_enum_declaration,
			parse_ts_import_equals_declaration_rest,
...
impl ModuleItemListParent {
	fn is_module(&self) -> bool { matches!(self, ModuleItemListParent::Module) }
...
pub(crate) fn parse_import_or_import_equals_declaration(p:&mut JsParser) -> ParsedSyntax {
	...
	let statement = if is_at_identifier_binding(p) && (p.nth_at(1, T![=]) || p.nth_at(2, T![=])) {
		...
		parse_import_clause(p).or_add_diagnostic(p, |p, range| {
			expected_any(&["default import", "namespace import", "named import"], range, p)
		});
...
fn parse_import_clause(p:&mut JsParser) -> ParsedSyntax {
	...
	let clause = match p.cur() {
		...
		_ => {
			// SAFETY: Safe because the parser only eats the "type" keyword if
			// it's followed by either a *, {, or binding
...
fn parse_any_named_import_specifier(p:&mut JsParser) -> ParsedSyntax {
	...
	let metadata =
		specifier_metadata(p, is_nth_at_literal_export_name, is_nth_at_identifier_binding);
...
// import "lorem"
// assert { type: "json" }
// import foo2 from "foo.json" assert { "type": "json", type: "html", "type":
// "js" }; import "x" assert;
// import ipsum from "ipsum.json" assert { type: "json", lazy: true,
// startAtLine: 1 }; import { a } from "a.json" assert
...
// }; import "x" with;
// import ipsum from "ipsum.json" with { type: "json", lazy: true, startAtLine:
...
fn parse_import_assertion_entry(
	...
	match p.cur() {
		...
		T![:] => {
			p.error(expected_any(&["identifier", "string literal"], p.cur_range(), p));
		},
		_ => {
			m.abandon(p);
			return Absent;
...
// // SCRIPT
// export function A () { return "Kinsmen hot Moria tea serves. Sticky camp
// spell covering forged they're Oakenshield vines. Admirable relatives march
// regained wheel Ere eternally on rest parts unhappy? Leave hundreds market's
// Argonath answered avail grieve doing goodness! Wrong miserable well-wishers
// wander stood immediately neither Agreed goat poison holes fire? Nobody tosses
// a Dwarf. Brigands Bilbo Baggins prisoner stinker birthday injuries. Kili's
// loosened shy spiders till. Gandalf's death was not in vain. Nor would he have
// you give up hope. Bread kindly ghost Beorn's jelly. Andûril two-faced
// bitterness biding seemed says drinking splendor feed light unnoticed one!
// Carven nearest Eärendil fireworks former. Mattress smelling wandering
// teaching appear taste wise Mithril uprooted winter forebearers wheel. Let's
// beside Proudfoots succumbed! Excuse Anárion stolen helpless nudge study shown
// holding form? Changes point Snowbourn material side outer highest eaves
// flash-flame relic descendant lurking. Thousand death Agreed oppose whole?
// Glóin head's hurts feasting fight shiny legacy. Thror's broken odds suffice
// believe well-protected? Rightfully manners begged Maggot's fairer. Unheard-of
// grog shields sad wondering gardener killed gone Galadriel! Pan Frodo fingers
// spreads magic parting amount interest idly naked. It's some form of Elvish. I
// can't read it. Silverwork Wraiths riddled enchantment apple anywhere."; }
pub(super) fn parse_export(p:&mut JsParser, decorators_list:ParsedSyntax) -> ParsedSyntax {
	...
	let clause = if is_nth_at_declaration_clause(p, 0) {
		...
		match p.cur() {
			...
			T![as] if p.nth_at(1, T![namespace]) => {
				TypeScript.parse_exclusive_syntax(
					p,
			...
			T![declare] if !p.has_nth_preceding_line_break(1) => {
				TypeScript.parse_exclusive_syntax(
					p,
...
fn parse_any_export_named_specifier(p:&mut JsParser) -> ParsedSyntax {
	...
	let metadata =
		specifier_metadata(p, is_nth_at_reference_identifier, is_nth_at_literal_export_name);
	...
	if metadata.is_local_name_missing {
		...
		// We need to parse "default" or any string literal here so the "export
		// ... from..." rewind later works.
...
struct SpecifierMetadata {
	// Is this a type export (`export { type test }`) or a regular value export
	// (`export { test }`)
	is_type:bool,
...
where
	...
	// This may be a typed import/export, but it could also be the name of the
	// import/export: ```ts
	// { type}              // name: `type`
	// { type type }        // name: `type`    type-export: `true`
	// { type as }          // name: `as`      type-export: `true`
	// { type as as }       // name: `type`    type-export: `false` (aliased to
	// `as`) { type as as as }    // name: `as`      type-export: `true`,
	// aliased to `as` ```
	if p.at(T![type]) {
		// `{ type identifier }`
		...
		if p.nth_at(1, T![as]) {
			...
			if p.nth_at(2, T![as]) {
				...
				// `{ type as as }`: Type can either be an identifier or the
				// type keyword
		...
		// error recovery case in case someone typed "as x" but forgot the local
		// name. `{ as x }`
		...
		T![@] => {
			...
			match p.cur() {
				...
				_ => {
					...
					match p.cur() {
						// test_err ts
						// decorator_function_export_default_declaration_clause
						// @decorator
						...
						// test_err ts
						// decorator_interface_export_default_declaration_clause
						// @decorator
						...
						// test_err ts
						// decorator_enum_export_default_declaration_clause
						// @decorator
	...
	clause.map(|mut clause| {
			...
			p.state().default_item.as_ref().filter(|_| p.is_module())
		{
			if existing_default_item.kind.is_mergeable(&default_item_kind) {
				// It's ok to have multiple overload declarations and an
				// implementation. This check won't catch if there are
				// multiple implementations for the same overload
				// or if the overloads define different functions.
			} else {
				let err = p
					.err_builder("Illegal duplicate default export declarations", clause.range(p))
		...
		// TypeScript supports multiple `export default interface` They all get
		// merged together
		...
		// export default function test() {}
			...
			p.state_mut().default_item =
				Some(ExportDefaultItem { range:clause.range(p).into(), kind:default_item_kind });
		}
...
fn parse_function_export_default_declaration_clause(
	...
	(Present(m.complete(p, JS_EXPORT_DEFAULT_DECLARATION_CLAUSE)), item_kind)
}
...
fn parse_ts_enum_export_default_declaration_clause(
	...
	let declaration = parse_ts_enum_declaration(p).map(|enum_declaration| {
		p.error(p.err_builder(
			"'export default' isn't allowed for 'enum's. Move the 'enum' declaration in its own \
			 statement and then export the enum's name.",
