//! Provides factory function to create common diagnostics for the JavaScript
//! syntax

use biome_js_syntax::TextRange;
use biome_parser::diagnostic::{expected_any, expected_node};

use crate::{prelude::*, span::Span, JsParser, JsSyntaxFeature::TypeScript};
...
pub(crate) fn expected_object_member(p:&JsParser, range:TextRange) -> ParseDiagnostic {
	expected_any(&["property", "shorthand property", "getter", "setter", "method"], range, p)
}
...
pub(crate) fn expected_object_member_name(p:&JsParser, range:TextRange) -> ParseDiagnostic {
	expected_any(
		&["identifier", "string literal", "number literal", "computed property"],
		range,
		p,
	)
}
...
pub(crate) fn expected_namespace_or_named_import(p:&JsParser, range:TextRange) -> ParseDiagnostic {
...
pub(crate) fn expected_declaration(p:&JsParser, range:TextRange) -> ParseDiagnostic {
	expected_any(
		&["function", "class", "variable declaration", "interface", "enum", "type alias"],
		range,
		p,
	)
}

pub(crate) fn expected_export_default_declaration(p:&JsParser, range:TextRange) -> ParseDiagnostic {
	if TypeScript.is_supported(p) {
		expected_any(
			&["class declaration", "function declaration", "interface declaration"],
			range,
			p,
		)
	} else {
...
pub(crate) fn unexpected_body_inside_ambient_context(
	...
	p.err_builder("members inside ambient contexts should not have a body", range)
}
...
pub(crate) fn invalid_assignment_error(p:&JsParser, range:TextRange) -> ParseDiagnostic {
	p.err_builder(format!("Invalid assignment to `{}`", p.text(range.as_range()),), range)
...
pub(crate) fn modifier_cannot_be_used_with_modifier(
	...
	p.err_builder(format!("'{modifier}' cannot be used with '{other_modifier}' modifier."), range)
...
pub(crate) fn modifier_must_precede_modifier(
	...
	p.err_builder(format!("'{modifier_name}' must precede '{to_precede_name}'",), range)
...
pub(crate) fn invalid_decorator_error(p:&JsParser, range:TextRange) -> ParseDiagnostic {
	p.err_builder(format!("Invalid decorator `{}`", p.text(range.as_range()),), range)
}

pub(crate) fn parameter_decorators_not_allowed(p:&JsParser, range:TextRange) -> ParseDiagnostic {
	decorators_not_allowed(p, range).with_hint(
		"You can enable parameter decorators by setting the `unsafeParameterDecoratorsEnabled` \
		 option to `true` in your configuration file.",
	)
}

pub(crate) fn decorators_not_allowed(p:&JsParser, range:TextRange) -> ParseDiagnostic {
	p.err_builder("Decorators are not valid here.", range).with_hint(
