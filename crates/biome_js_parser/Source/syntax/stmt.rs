...
use super::{
	binding::*,
	class::is_at_ts_abstract_class_declaration,
	expr::parse_expression,
	metavariable::{is_at_metavariable, is_nth_at_metavariable, parse_metavariable},
	module::parse_export,
	typescript::*,
};
use crate::{
	...
	syntax::{
		assignment::expression_to_assignment_pattern,
		class::{parse_class_declaration, parse_decorators, parse_initializer_clause},
		expr::{
			is_at_expression,
			is_at_identifier,
			is_nth_at_identifier,
			parse_assignment_expression_or_higher,
			parse_expression_or_recover_to_next_statement,
			parse_identifier,
			ExpressionContext,
		},
		function::{is_at_async_function, parse_function_declaration, LineBreak},
		js_parse_error,
		js_parse_error::{decorators_not_allowed, expected_binding, expected_statement},
		module::parse_import_or_import_equals_declaration,
		typescript::ts_parse_error::{expected_ts_type, ts_only_syntax_error},
	},
	JsParser,
	JsSyntaxFeature,
	JsSyntaxFeature::{StrictMode, TypeScript},
...
/// Eats a semicolon if present but doesn't add an error none is present and the
/// automatic semicolon insertion rule does not apply.
///
/// Returns false if neither a semicolon was present and the current position
/// doesn't allow an automatic semicolon insertion.
...
pub(crate) fn parse_statement(p:&mut JsParser, context:StatementContext) -> ParsedSyntax {
	match p.cur() {
				...
				&& (is_nth_at_identifier(p, 1) || is_nth_at_metavariable(p, 1)) =>
		{
			...
			// let type;
			// type = getFlowTypeInConstructor(symbol,
	...
	let x = parse_identifier(p, JS_LABEL).map(|identifier| {
		fn parse_body(p:&mut JsParser, context:StatementContext) -> ParsedSyntax {
			if is_at_identifier(p) && p.nth_at(1, T![:]) && StrictMode.is_unsupported(p) {
				// Re-use the parent context to catch `if (true) label1: label2:
				// function A() {}
		...
		match body.or_add_diagnostic(p, expected_statement) {
			Some(mut body)
				if context.is_single_statement() && body.kind(p) == JS_FUNCTION_DECLARATION =>
			{
				// test_err js
				// labelled_function_decl_in_single_statement_context
				// if (true) label1: label2: function a() {}
				p.error(
					p.err_builder(
						"Labelled function declarations are only allowed at top-level or inside a \
						 block",
						body.range(p),
					)
					.with_hint("Wrap the labelled statement in a block statement"),
				);
	...
	let error = if !p.has_preceding_line_break() && is_at_identifier(p) {
		...
		let error = match p.state().get_labelled_item(label_name) {
			...
			Some(LabelledItem::Other(range)) => {
				Some(
					p.err_builder(
						"A `continue` statement can only jump to a label of an enclosing `for`, \
...
fn parse_return_statement(p:&mut JsParser) -> ParsedSyntax {
	...
	// The frontmatter of Astro files is executed inside a function during the
	// compilation, so it's safe to have illegal returns
...
/// A block wrapped in curly brackets. Can either be a function body or a block
/// statement.
...
/// * The marker for the following statement list. May already contain a parsed
...
/// Top level items or items inside of a block statement, this also handles
/// module items so we can easily recover from erroneous module declarations in
/// scripts
...
fn parse_with_statement(p:&mut JsParser) -> ParsedSyntax {
	...
	// or SloppyMode.exclusive_syntax(...) but this reads better with the error
	// message, saying that it's only forbidden in strict mode
...
/// A var, const, using or let declaration statement such as `var a = 5, b;` or
/// `let {a, b} = foo;`
// test js var_decl
// var a = 5;
// let { foo, bar } = 5;
...
// await let s;
// await const t = 1;
pub(crate) fn parse_variable_statement(p:&mut JsParser, context:StatementContext) -> ParsedSyntax {
	...
	parse_variable_declaration(p, VariableDeclarationParent::VariableStatement).map(|declaration| {
		...
		if is_await_using && !is_top_level_module_or_async_fn {
			// test_err js
			// await_using_declaration_only_allowed_inside_an_async_function
			// function foo() { await using x = y };
			// foo = function() { await using x = y };
			// foo = () => { await using x = y };
			p.error(
				p.err_builder(
					"`await using` declarations are only allowed at top-level or inside an async \
					 function",
					statement.range(p),
				)
...
pub(super) enum VariableDeclarationParent {
	...
	/// Declaration as part of a variable statement (`let a`, `const b`, or `var
	/// c`).
	VariableStatement,

	/// Declaration as part of another statement, like `export let ...` or
	/// `declare let a`
	Clause,
}

/// Parses and consume variable declarations like
/// `var`/`let`/`const`/`using`/`await using`. Returns a tuple where
/// * the first element is the marker to the not yet completed list
/// * the second element is the range of all variable declarations except the
///   first one. Is [None] if there's only one declaration.
...
impl ParseSeparatedList for VariableDeclaratorList {
	...
	fn is_at_list_end(&self, p:&mut JsParser) -> bool {
		if self.declarator_context.is_first { false } else { !p.at(T![,]) }
	}
...
struct VariableDeclaratorContext {
	/// What kind of variable declaration is this (`var`, `let`, `const`,
	/// 'using')
	kind_name:Option<&'static str>,
	/// Is this the first declaration in the declaration list (a first, b
	/// second in `let a, b`)
	is_first:bool,
...
impl VariableDeclaratorContext {
	...
	fn is_var(&self) -> bool { self.kind_name.is_none() }

	fn is_const(&self) -> bool { matches!(self.kind_name, Some("const")) }

	fn is_using(&self) -> bool { matches!(self.kind_name, Some("using")) }
}
...
fn parse_variable_declarator(p:&mut JsParser, context:&VariableDeclaratorContext) -> ParsedSyntax {
	...
	id.map(|id| {
		...
		let ts_annotation = TypeScript
			.parse_exclusive_syntax(p, parse_ts_variable_annotation, |p, annotation| {
		...
		// Heuristic to determine if we're in a for of or for in loop. This may
		// be off if the user uses a for of/in with multiple declarations but
		// this isn't allowed anyway.
		...
		if is_in_for_of || is_in_for_in {
			if TypeScript.is_supported(p) {
				if let Some(mut ts_annotation) = ts_annotation {
					let err = p.err_builder(
						"`for` statement declarators cannot have a type annotation",
						ts_annotation.range(p),
					);
			...
			&& matches!(id_kind, JS_ARRAY_BINDING_PATTERN | JS_OBJECT_BINDING_PATTERN)
		{
			let err = p
				.err_builder("Object and Array patterns require initializers.", id_range)
				.with_hint("This pattern is declared, but it is not given an initialized value.");
...
/// Parses the header of a for statement into the current node and returns
/// whatever it is a for in/of or "regular" for statement
fn parse_for_head(p:&mut JsParser, has_l_paren:bool, is_for_await:bool) -> JsSyntaxKind {
	...
	if is_nth_at_variable_declarations(p, 0) {
		...
		if is_in || is_of {
			// remove the intermediate list node created by parse variable
			// declarations that is not needed for a ForInOrOfInitializer
			// where the variable declaration is a direct child.
		...
		if p.at(T![in]) || p.at(T![of]) {
			...
			if let Present(assignment_expr) = init_expr {
				...
				if TypeScript.is_supported(p)
					...
					let err = p.err_builder(
						"the left hand side of a `for..in` statement cannot be a destructuring \
						 pattern",
						assignment.range(p),
					);
					p.error(err);
					...
					// Statement[?Yield, ?Await, ?Return] [+Await] for
					// await ( [lookahead ≠ let] LeftHandSideExpression[?Yield,
...
impl ParseNodeList for SwitchCaseStatementList {
	...
	fn recover(&mut self, p:&mut JsParser, parsed_element:ParsedSyntax) -> parser::RecoveryResult {
		parsed_element.or_recover_with_token_set(
			p,
...
// We return the range in case its a default clause so we can report multiple
// default clauses in a better way
		...
		JsSyntaxFeature::TypeScript
			.parse_exclusive_syntax(
				...
				|p| {
					...
					if let Some(ty) = parse_ts_type(p, TypeContext::default())
...
pub(crate) fn parse_try_statement(p:&mut JsParser) -> ParsedSyntax {
	// TODO: recover from `try catch` and `try finally`. The issue is
	// block_items will cause infinite recursion because parsing a stmt would
	// not consume the catch token and block_items would not exit, and if we
	// exited on any error that would greatly limit block_items error recovery
