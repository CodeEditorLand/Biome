...
use crate::{
	...
	syntax::{
		...
		function::{LineBreak, is_at_async_function, parse_function_declaration},
		js_parse_error,
...
struct VariableDeclaratorContext {
	...
	/// Is this the first declaration in the declaration list (a first, b second
	/// in `let a, b`)
	is_first:bool,
...
fn parse_variable_declarator(p:&mut JsParser, context:&VariableDeclaratorContext) -> ParsedSyntax {
	...
	id.map(|id| {
		...
		// Heuristic to determine if we're in a for of or for in loop. This may be off
		// if the user uses a for of/in with multiple declarations but this isn't
		// allowed anyway.
...
fn parse_for_head(p:&mut JsParser, has_l_paren:bool, is_for_await:bool) -> JsSyntaxKind {
	...
	if is_nth_at_variable_declarations(p, 0) {
		...
		if is_in || is_of {
			// remove the intermediate list node created by parse variable declarations that
			// is not needed for a ForInOrOfInitializer where the variable declaration is
			// a direct child.
		...
		if p.at(T![in]) || p.at(T![of]) {
			...
			if let Present(assignment_expr) = init_expr {
				...
				if TypeScript.is_supported(p)
					...
					//  for ( [lookahead ∉ { let, async of }] LeftHandSideExpression[?Yield, ?Await]
					// of AssignmentExpression[+In, ?Yield, ?Await] ) Statement[?Yield, ?Await,
					// ?Return] [+Await] for await ( [lookahead ≠ let]
					// LeftHandSideExpression[?Yield, ?Await] of AssignmentExpression[+In, ?Yield,
					// ?Await] ) Statement[?Yield, ?Await, ?Return]
...
pub(crate) fn parse_try_statement(p:&mut JsParser) -> ParsedSyntax {
	...
	// will cause infinite recursion because parsing a stmt would not consume the
	// catch token and block_items would not exit, and if we exited on any error
	// that would greatly limit block_items error recovery
