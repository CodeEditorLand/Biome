...
use crate::{
	...
	syntax::{
		...
		expr::{is_at_identifier, parse_conditional_expr, parse_unary_expr, ExpressionContext},
		js_parse_error::{
...
pub(crate) fn expression_to_assignment(
	...
	try_expression_to_assignment(p, target, checkpoint).unwrap_or_else(
		...
		|mut invalid_assignment_target| {
			// Doesn't seem to be a valid assignment target. Recover and create
			// an error.
...
fn try_expression_to_assignment(
	...
	// assignment, but it's not yet guaranteed if it is valid or not (for
	// example, a static member expression is valid, except if it uses optional
	// chaining).
...
struct ReparseAssignment {
	...
	// Index 1: Started marker. A `None` marker means that this node should be
	// dropped          from the re-written tree
...
impl RewriteParseEvents for ReparseAssignment {
	fn start_node(&mut self, kind:JsSyntaxKind, p:&mut RewriteParser) {
		...
		// Make sure to also add the kind to the match in
		// `try_expression_to_assignment`
