...
use crate::syntax::expr::{ExpressionContext, is_nth_at_identifier_or_keyword, parse_expression, parse_name};
use crate::syntax::js_parse_error::{expected_expression, expected_identifier};
use crate::syntax::jsx::jsx_parse_errors::{
	jsx_expected_attribute, jsx_expected_attribute_value, jsx_expected_children, jsx_expected_closing_tag,
};
...
fn expect_closing_fragment(p: &mut JsParser, in_expression: bool, opening_range: TextRange) -> CompletedMarker {
	let m = p.start();
	p.expect(T![<]);
	p.expect(T![/]);
	...
	if let Some(opening_name_marker) = opening_name_marker {
		...
		let error = match name_marker {
			Present(name) if name.text(p) != opening_name => {
				let closing_end = if p.at(T![>]) { p.cur_range().end() } else { name.range(p).end() };
...
impl ParseNodeList for JsxChildrenList {
	...
	fn recover(&mut self, p: &mut JsParser, parsed_element: ParsedSyntax) -> RecoveryResult {
		parsed_element.or_recover_with_token_set(
			p,
			&ParseRecoveryTokenSet::new(JsSyntaxKind::JS_BOGUS, token_set![T![<], T![>], T!['{'], T!['}']]),
...
fn parse_jsx_spread_attribute(p: &mut JsParser) -> ParsedSyntax {
	...
	let argument = parse_expression(p, ExpressionContext::default()).map(|mut expr| {
		if expr.kind(p) == JS_SEQUENCE_EXPRESSION {
			p.error(p.err_builder("Comma operator isn't a valid value for a JSX spread argument.", expr.range(p)));
...
fn parse_jsx_assignment_expression(p: &mut JsParser, is_spread: bool) -> ParsedSyntax {
	...
	expr.map(|mut expr| {
		...
		let err = match expr.kind(p) {
			...
			JsSyntaxKind::JS_SEQUENCE_EXPRESSION if is_spread => Some(p.err_builder(msg, expr.range(p))),
			_ => None,
		};
