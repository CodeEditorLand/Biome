...
use biome_parser::{diagnostic::expected_token, parse_lists::ParseNodeList};
...
use crate::{
	...
	ParsedSyntax,
	Present,
	lexer::{JsLexContext, JsReLexContext, JsSyntaxKind, T},
	parser::RecoveryResult,
	syntax::{
		expr::{ExpressionContext, is_nth_at_identifier_or_keyword, parse_expression, parse_name},
		js_parse_error::{expected_expression, expected_identifier},
		jsx::jsx_parse_errors::{
			jsx_expected_attribute,
			jsx_expected_attribute_value,
...
//     let d = <div>a</div>/; // ambiguous: JSX or "type assertion a less than
// regex /div>/". Probably JSX.     let d = <string>a</string>/;
// }
...
pub(crate) fn parse_jsx_tag_expression(p:&mut JsParser) -> ParsedSyntax {
	...
	// Safety: Safe because `parse_any_jsx_tag only returns Absent if the parser
	// isn't positioned at the `<` token which is tested for at the beginning of
	// the function.
...
///
/// `in_expression` must be `true` if this element is a direct child of the
/// `JsxElementExpression` (root of an expression). It should be false when
/// parsing any child node.
...
enum OpeningElement {
	...
	Element { name:Option<CompletedMarker>, opening:CompletedMarker },
...
fn parse_any_jsx_opening_tag(p:&mut JsParser, in_expression:bool) -> Option<OpeningElement> {
	...
	if p.at(T![>]) {
		...
		return Some(OpeningElement::Fragment(m.complete(p, JSX_OPENING_FRAGMENT)));
	}

	let name = parse_jsx_any_element_name(p).or_add_diagnostic(p, expected_identifier);

	// Don't parse type arguments in JS because it prevents us from doing better
	// error recovery in case the `>` token of the opening element is missing:
	// `<test <inner></test>` The `inner` is it's own element and not the type
	// arguments
...
fn expect_closing_fragment(
	...
	if let Present(name) = parse_jsx_any_element_name(p) {
		p.error(
			p.err_builder("JSX fragment has no corresponding closing tag.", opening_range)
	...
	if let Some(opening_name_marker) = opening_name_marker {
		...
		let error = match name_marker {
			Present(name) if name.text(p) != opening_name => {
				let closing_end =
					if p.at(T![>]) { p.cur_range().end() } else { name.range(p).end() };

				let closing_range = TextRange::new(m.start(), closing_end);

				Some(jsx_expected_closing_tag(p, opening_name, opening_range, closing_range))
			},
...
/// Ensures that the child content is lexed with the [JsLexContext::JsxChild]
/// context.
...
#[inline]
fn parse_jsx_children(p:&mut JsParser) { JsxChildrenList.parse_list(p); }
...
/// Tests if this is an intrinsic element name. Intrinsic elements are such
/// elements that are built in, for example HTML elements. This implementation
/// uses React's semantic and assumes that anything starting with a lower case
/// character is an intrinsic element, and that custom components start with an
...
impl ParseNodeList for JsxAttributeList {
	...
	fn is_at_list_end(&self, p:&mut JsParser) -> bool { matches!(p.cur(), T![>] | T![/] | T![<]) }
...
fn parse_jsx_attribute(p:&mut JsParser) -> ParsedSyntax {
	...
	// SAFETY: Guaranteed to succeed because the parser is at an identifier or
	// keyword
