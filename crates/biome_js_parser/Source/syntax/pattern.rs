...
use biome_js_syntax::{
	JsSyntaxKind,
	JsSyntaxKind::{EOF, JS_ARRAY_HOLE},
	TextRange,
	T,
};
...
/// Trait for parsing a pattern with an optional default of the form `pattern =
/// default`
pub(crate) trait ParseWithDefaultPattern {
	...
	/// Creates a diagnostic for the case where the pattern is missing. For
	/// example, if the code only contains ` = default`
	...
	/// Parses a pattern and wraps it in a pattern with default if a `=` token
	/// follows the pattern
...
pub(crate) trait ParseArrayPattern<P:ParseWithDefaultPattern> {
	/// The kind of a bogus pattern. Used in case the pattern contains elements
	/// that aren't valid patterns
	...
	///  Creates a diagnostic saying that the parser expected an element at the
	/// position passed as an argument.
	fn expected_element_error(p:&JsParser, range:TextRange) -> ParseDiagnostic;
	/// Creates a pattern with default instance. Used to parse the array
	/// elements.
...
pub(crate) trait ParseObjectPattern {
	...
	fn list_kind() -> JsSyntaxKind;
	/// Creates a diagnostic saying that a property is expected at the passed in
	/// range that isn't present.
...
/// Validates if the parsed completed rest marker is a valid rest element inside
/// of a array or object assignment target and converts it to an bogus
/// assignment target if not. A rest element must be:
///
/// * the last element
...
fn validate_rest_pattern(
	...
	if p.at(T![=]) {
		...
		p.error(
			...
			.with_detail(default_start..p.cur_range().start(), "Remove the default value here")
		...
		p.error(p.err_builder("rest element must be the last element", rest.range(p)).with_hint(
			format!(
