...
use crate::{
	...
	syntax::{
		...
		expr::{is_nth_at_identifier, parse_identifier, ExpressionContext},
		js_parse_error::{expected_binding, expected_identifier, expected_object_member_name},
...
#[inline]
pub(crate) fn parse_binding(p:&mut JsParser) -> ParsedSyntax { parse_identifier_binding(p) }
...
// let let = 5;
// const let = 5;
/// Parses an identifier binding or returns an invalid syntax if the identifier
/// isn't valid in this context. An identifier may not be valid if:
/// * it is named "eval" or "arguments" inside of strict mode
/// * it is named "let" inside of a "let" or "const" declaration
/// * the same identifier is bound multiple times inside of a `let` or const`
///   declaration
	...
	parsed.map(|mut identifier| {
		...
		if StrictMode.is_supported(p) && matches!(identifier_name, "eval" | "arguments") {
			let err = p.err_builder(
				format!("Illegal use of `{identifier_name}` as an identifier in strict mode"),
				identifier.range(p),
			);
			p.error(err);
		...
		if let Some(parent) = p.state().duplicate_binding_parent {
			if identifier_name == "let" {
				let err = p
					.err_builder(
						format!(
							"`let` cannot be declared as a variable name inside of a `{parent}` \
							 declaration",
						),
						identifier.range(p),
					)
			...
			if let Some(existing) = p.state().name_map.get(identifier_name) {
				let err = p
					.err_builder(
						format!(
							"Declarations inside of a `{parent}` declaration may not have \
							 duplicates"
						),
						identifier.range(p),
					)
					.with_detail(
						identifier.range(p),
						format!("a second declaration of `{identifier_name}` is not allowed"),
					)
					.with_detail(*existing, format!("`{identifier_name}` is first declared here"));
				p.error(err);
				identifier.change_to_bogus(p);
				return identifier;
			}
			...
			p.state_mut().name_map.insert(identifier_name, identifier_range.as_range());
		}
...
impl ParseWithDefaultPattern for ArrayBindingPatternWithDefault {
	#[inline]
	fn pattern_with_default_kind() -> JsSyntaxKind { JS_ARRAY_BINDING_PATTERN_ELEMENT }
...
impl ParseArrayPattern<ArrayBindingPatternWithDefault> for ArrayBindingPattern {
	#[inline]
	fn bogus_pattern_kind() -> JsSyntaxKind { JS_BOGUS_BINDING }

	#[inline]
	fn array_pattern_kind() -> JsSyntaxKind { JS_ARRAY_BINDING_PATTERN }

	#[inline]
	fn rest_pattern_kind() -> JsSyntaxKind { JS_ARRAY_BINDING_PATTERN_REST_ELEMENT }

	fn list_kind() -> JsSyntaxKind { JS_ARRAY_BINDING_PATTERN_ELEMENT_LIST }

	#[inline]
	fn expected_element_error(p:&JsParser, range:TextRange) -> ParseDiagnostic {
		expected_any(&["identifier", "object pattern", "array pattern", "rest pattern"], range, p)
	}
...
impl ParseObjectPattern for ObjectBindingPattern {
	...
	#[inline]
	fn object_pattern_kind() -> JsSyntaxKind { JS_OBJECT_BINDING_PATTERN }

	fn list_kind() -> JsSyntaxKind { JS_OBJECT_BINDING_PATTERN_PROPERTY_LIST }
			...
			if let Some(mut inner) = inner {
				if inner.kind(p) != JS_IDENTIFIER_BINDING {
					...
					if inner.kind(p) != JS_BOGUS_BINDING {
						p.error(
							p.err_builder("Expected identifier binding", inner_range).with_hint(
								"Object rest patterns must bind to an identifier, other patterns \
								 are not allowed.",
							),
						);
					}
