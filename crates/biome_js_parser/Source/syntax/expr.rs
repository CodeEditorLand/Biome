...
use biome_parser::{diagnostic::expected_token, parse_lists::ParseSeparatedList, ParserProgress};
...
use crate::{
	...
	prelude::*,
	rewrite::{rewrite_events, RewriteParseEvents},
	syntax,
	syntax::{
		assignment::{
		...
		stmt::{is_semi, STMT_RECOVERY_SET},
	...
	if is_assign_token(assign_operator) {
		let target =
			if matches!(target.kind(p), JS_BINARY_EXPRESSION | TS_TYPE_ASSERTION_EXPRESSION) {
				// Special handling for binary expressions and type assertions to
				// avoid having to deal with `a as string = ...` inside of the
				// `ReparseAssignment` implementation because not using parentheses
				// is valid in for heads `for (a as any in []) {}`
...
fn parse_binary_or_logical_expression_recursive(
	...
	// current operator has the same or a lower precedence than the left-hand
	// side expression. Thus, the algorithm goes at most
	// `count(OperatorPrecedence)` levels deep.
	loop {
		...
		// This isn't spec compliant but improves error recovery in case the `}`
		// is missing inside of a JSX attribute expression value or an
		// expression child. Prevents that it parses `</` as less than
		// followed by a RegEx if JSX is enabled and only if there's no
		// whitespace between the two tokens. The downside of this is that `a
		// </test/` will be incorrectly left unparsed. I think this is a worth
		// compromise and compatible with what TypeScript's doing.
		...
		// satisfies;
		// let precedence = "hello" satisfies string + 3 satisfies number
		// satisfies number;
	...
	if let Present(left) = &mut left {
		...
		// or is a standalone expression. Private names aren't allowed as
		// standalone expressions nor on the right-hand side
...
fn parse_member_expression_rest(
	...
	while !p.at(EOF) {
		...
		lhs = match p.cur() {
			...
			// Don't parse out `[` as a member expression because it may as well
			// be the start of a computed class member
			...
			T![<] | T![<<] => {
				...
				// `parse_ts_type_arguments_in_expression` could possibly return
				// a `Present(_)`
		...
		match p.cur() {
			...
			DOLLAR_CURLY => {
				...
				if !p.at(T!['}']) {
					...
					// Seems there's more. For example a `${a a}`. We must eat
					// all tokens away to avoid a panic because of an unexpected
					// token
					...
					if !p.at(T!['}']) {
						...
						// Failed to fully recover, unclear where we are now,
						// exit
						break;
					}
				}
...
fn parse_call_expression_rest(
	...
	loop {
		...
		// Cloning here is necessary because parsing out the type arguments may
		// rewind in which case we want to return the `lhs`.
		let m = match lhs.kind(p) {
		...
		if type_arguments.is_some() || p.at(T!['(']) {
			...
			break if optional_chain_call {
				// If the `?.` is present and what followed was neither a valid
				// type arguments nor valid arguments. In this case, parse
				// this as a static member access with an optional chain
...
pub(super) fn parse_unary_expr(p:&mut JsParser, context:ExpressionContext) -> ParsedSyntax {
	...
	// This is a type assertion expression if the parser is at the `<` token and
	// JSX is disabled JSX elements are parsed in parse_primary_expression.
	...
	if p.at_ts(UNARY_SINGLE) {
		...
		// class TestClass { #member = true; method() { delete
		// func(this.#member) } } class TestClass { #member = true; method() {
		// delete [this.#member] } } class TestClass { #member = true;
		// method() { delete { key: this.#member } } } class TestClass {
		// #member = true; method() { delete (() => { this.#member; }) } }
		// class TestClass { #member = true; method() { delete (param => {
		// this.#member; }) } } class TestClass { #member = true; method() {
		// delete (async () => { this.#member; }) } }
