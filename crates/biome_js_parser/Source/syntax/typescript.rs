...
pub(crate) use self::{statement::*, types::*};
use self::{ts_parse_error::ts_member_cannot_be, types::parse_ts_reference_type};
use super::{
	binding::parse_identifier_binding,
	class::is_nth_at_modifier,
	expr::is_nth_at_identifier,
	js_parse_error::expected_identifier,
	stmt::optional_semi,
};
use crate::{
	syntax::{
		expr::{parse_identifier, parse_unary_expr, ExpressionContext},
		js_parse_error::expected_expression,
		typescript::ts_parse_error::expected_ts_type,
	},
	Absent,
	JsParser,
	ParsedSyntax,
...
pub(crate) fn expect_ts_index_signature_member(
	...
	while is_nth_at_modifier(p, 0, false) {
		...
		p.error(ts_member_cannot_be(p, p.cur_range(), "index signature", p.cur_text()));
		p.bump_any();
	}
