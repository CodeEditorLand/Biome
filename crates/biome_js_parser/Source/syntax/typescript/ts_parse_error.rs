...
pub(crate) fn unexpected_abstract_member_with_body(p: &JsParser, range: TextRange) -> ParseDiagnostic {
...
pub(crate) fn ts_modifier_cannot_appear_on_a_parameter(p: &JsParser, modifier_range: TextRange) -> ParseDiagnostic {
	let modifier = p.text(modifier_range);
	p.err_builder(format!("'{modifier}' cannot appear on a parameter."), modifier_range)
}
...
pub(crate) fn ts_only_syntax_error(p: &JsParser, syntax: &str, range: TextRange) -> ParseDiagnostic {
	p.err_builder(
		format!("{syntax} are a TypeScript only feature. Convert your file to a TypeScript file or remove the syntax."),
		range,
	)
	.with_hint("TypeScript only syntax")
}

pub(crate) fn ts_accessor_type_parameters_error(p: &JsParser, type_parameters: &CompletedMarker) -> ParseDiagnostic {
	p.err_builder("An accessor cannot have type parameters.", type_parameters.range(p))
}

pub(crate) fn ts_constructor_type_parameters_error(p: &JsParser, type_parameters: &CompletedMarker) -> ParseDiagnostic {
	p.err_builder("constructors cannot have type parameters.", type_parameters.range(p))
}

pub(crate) fn ts_set_accessor_return_type_error(p: &JsParser, type_annotation: &CompletedMarker) -> ParseDiagnostic {
	p.err_builder(
...
pub(crate) fn expected_ts_import_type_with_arguments(p: &JsParser, range: TextRange) -> ParseDiagnostic {
	p.err_builder(format!("Expected '(', but got '{}' here", p.cur_text()), range)
}
