...
pub(crate) fn ts_in_out_modifier_cannot_appear_on_a_type_parameter(
	...
	p.err_builder(
		format!(
			"'{modifier}' modifier can only appear on a type parameter of a class, interface or \
			 type alias.",
		),
		modifier_range,
	)
...
pub(crate) fn ts_only_syntax_error(p:&JsParser, syntax:&str, range:TextRange) -> ParseDiagnostic {
	p.err_builder(
		format!(
			"{syntax} are a TypeScript only feature. Convert your file to a TypeScript file or \
			 remove the syntax."
		),
		range,
	)
