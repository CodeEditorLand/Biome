...
use crate::{
	...
	parser::{RecoveryError, RecoveryResult},
	prelude::*,
	span::Span,
	state::{EnterType, SignatureFlags},
	syntax::{
		class::parse_decorators,
		expr::{
			is_at_binary_operator,
			is_at_expression,
			is_at_identifier,
			is_nth_at_identifier,
			is_nth_at_identifier_or_keyword,
			parse_big_int_literal_expression,
			parse_identifier,
			parse_literal_expression,
			parse_name,
			parse_number_literal_expression,
		...
		metavariable::parse_metavariable,
		object::{is_at_object_member_name, is_nth_at_type_member_name, parse_object_member_name},
		stmt::optional_semi,
		typescript::{
			try_parse,
			ts_parse_error::{
...
impl TypeContext {
	...
	/// Whether conditional types `extends string ? string : number` are allowed
	...
	pub fn contains(&self, other:impl Into<TypeContext>) -> bool { self.0.contains(other.into().0) }
	...
	fn and(self, flag:TypeContext, set:bool) -> Self { if set { self | flag } else { self - flag } }
}
...
fn parse_ts_call_signature(p:&mut JsParser, context:TypeContext) {
	...
	parse_parameter_list(p, ParameterContext::Declaration, context, SignatureFlags::empty())
...
impl ClassMemberModifierList {
	fn add_modifier(&mut self, modifier:TypeParameterModifier) { self.0.push(modifier); }

	fn find(&self, modifier_kind:&TypeParameterModifierKind) -> Option<&TypeParameterModifier> {
		self.0.iter().find(|predicate| predicate.kind == *modifier_kind)
	}
}
...
fn parse_ts_type_parameter_modifiers(p:&mut JsParser, context:TypeContext) -> ParsedSyntax {
	...
	while is_nth_at_type_parameter_modifier(p, 0) {
		let modifier_kind = match p.cur() {
			...
			_ => {
				unreachable!("keywords that are not 'in', 'out' and 'const' are checked earlier")
			},
		};
		let m = p.start();
		...
		if matches!(modifier_kind, TypeParameterModifierKind::In | TypeParameterModifierKind::Out,)
			&& !context.is_in_out_modifier_allowed()
		{
			p.error(ts_in_out_modifier_cannot_appear_on_a_type_parameter(p, text_range));
			m.abandon(p);
			continue;
		}
		...
		{
			p.error(ts_const_modifier_cannot_appear_on_a_type_parameter(p, text_range));
			m.abandon(p);
			continue;
		}
		...
		if let Some(existing_modifier) = modifiers.find(&modifier_kind) {
			p.error(modifier_already_seen(p, text_range, existing_modifier.range));
			m.abandon(p);
			continue;
		}
		...
		if let Some(ts_out_modifier) = modifiers.find(&TypeParameterModifierKind::Out) {
			if modifier_kind == TypeParameterModifierKind::In {
				p.error(modifier_must_precede_modifier(p, text_range, ts_out_modifier.range));
				m.abandon(p);
				continue;
			}
		}

		let modifier = TypeParameterModifier { kind:modifier_kind, range:text_range };
...
fn is_nth_at_ts_type_parameters(p:&mut JsParser, n:usize) -> bool { p.nth_at(n, T![<]) }
...
pub(crate) fn parse_ts_type(p:&mut JsParser, context:TypeContext) -> ParsedSyntax {
	p.with_state(EnterType, |p| {
		...
		// type X<V> = V extends (...args: any[]) => any ? (...args:
		// Parameters<V>) => void : Function;
		if context.is_conditional_type_allowed() {
			left.map(|left| {
				// test ts ts_conditional_type
				// type A = number;
				// type B = string extends number ? string : number;
				// type C = A extends (B extends A ? number : string) ? void :
				// number; type D<T> = T extends [infer S extends string,
				// ...unknown[]] ? S : never; type E<U, T> = T extends
				// (infer U extends number ? U : T ) ? U : T type F<T> = T
				// extends { [P in infer U extends keyof T ? 1 : 0]: 1; } ? 1 :
				// 0; type G<T> = T extends [unknown, infer S extends
				// string] ? S : never; type H = A extends () => B extends C
				// ? D : E ? F : G; type J<T> = T extends ((...a: any[]) =>
				// infer R extends string) ? R : never; type Equals = A
				// extends (x: B extends C ? D : E) => 0 ? F : G;
				// type Curry<F extends ((...args: any) => any)> =
				//     <T extends any[]>(...args: Tools.Cast<Tools.Cast<T,
				// Gaps<Parameters<F>>>, any[]>) =>          GapsOf<T,
				// Parameters<F>> extends [any, ...any[]]          ?
				// Curry<(...args: GapsOf<T, Parameters<F>> extends infer G ?
				// Tools.Cast<G, any[]> : never) => ReturnType<F>>
				//          : ReturnType<F>;
				// interface GapsOfWorker<T1 extends any[], T2 extends any[], TN
				// extends any[] = [], I extends any[] = []> {
				//     0: GapsOf<T1, T2, GapOf<T1, T2, TN, I> extends infer G ?
				// Tools.Cast<G, any[]> : never, Tools.Next<I>>;
				//     1: Tools.Concat<TN, Tools.Drop<Tools.Pos<I>, T2> extends
				// infer D ? Tools.Cast<D, any[]> : never>; }
				if !p.has_preceding_line_break() && p.at(T![extends]) {
					...
					parse_ts_type(
						p,
						context.and_allow_conditional_types(false).and_in_conditional_extends(true),
					)
...
fn parse_ts_primary_type(p:&mut JsParser, context:TypeContext) -> ParsedSyntax {
	...
	if p.at(T![infer]) {
		...
		return if context.in_conditional_extends() {
			...
			// type ContentBetweenBrackets<S> = S extends `[${infer T}]` ? T :
			// never; type WithSelectors<S> = S extends { getState: () =>
			// infer T }     ? S & { use: { [K in keyof T]: () => T[K] } }
			//     : never;
			// type A = MyType extends (OtherType extends infer T ? infer U :
			// InnerFalse) ? OuterTrue : OuterFalse type Join<T extends
			// unknown[], D extends string> =      T extends [] ? '' :
			//      T extends [string | number | boolean | bigint] ? `${T[0]}` :
			//      T extends [string | number | boolean | bigint, ...infer U] ?
			// `${T[0]}${D}${Join<U, D>}` :      string;
			// type MatchPair<S extends string> = S extends `[${infer A},${infer
			// B}]` ? [A, B] : unknown; type FirstTwoAndRest<S extends
			// string> = S extends `${infer A}${infer B}${infer R}` ?
			// [`${A}${B}`, R] : unknown; type Trim<S extends string> =
			...
			//     T;
			// type ArgumentType<T extends (x: any) => any> = T extends (a:
			// infer A) => any ? A : any; type X3<T> = T extends { a: (x:
			// infer U) => void, b: (x: infer U) => void } ? U : never;
			// type X1<T extends { x: any, y: any }> = T extends { x: infer X,
			// y: infer Y } ? [X, Y] : any; type T62<T> = U extends (infer
			// U)[] ? U : U; type T63<T> = T extends ((infer A) extends infer
			// B ? infer C : infer D) ? string : number; type T75<T> = T
			// extends T74<infer U, infer U> ? T70<U> | T72<U> | T74<U, U> :
			// never; type Jsonified<T> = T extends string | number | boolean
			// | null ? T     : T extends undefined | Function ? never
			...
			// type T5 = F1 extends (args: [...((infer T))]) => void ? T :
			// never; type T6<T> = T extends IsNumber<((infer N))> ? true :
			// false; type T7 = F1 extends (...args: ((((infer T))))) => void
			// ? T : never; type T8 = F1 extends (args: [...((((infer T))))])
			// => void ? T : never; type T9<T> = T extends IsNumber<((((infer
			// N))))> ? true : false; type Prepend<E, T extends any[]> =
			//     ((head: E, ...args: T) => any) extends ((...args: infer U) =>
			// any)     ? U
			//     : T;
...
fn try_parse_constraint_of_infer_type(p:&mut JsParser, context:TypeContext) -> ParsedSyntax {
	...
	try_parse(p, |p| {
		...
		// Rewind if conditional types are allowed, and the parser is at the `?`
		// token because this should instead be parsed as a conditional type.
...
impl TypeMembers {
	pub fn new(context:TypeContext) -> Self { Self { context } }
}
...
fn parse_ts_construct_signature_type_member(p:&mut JsParser, context:TypeContext) -> ParsedSyntax {
	...
	parse_ts_type_parameters(
		p,
		context.and_allow_const_modifier(true).and_allow_in_out_modifier(true),
	)
	.ok();
	parse_parameter_list(p, ParameterContext::Declaration, context, SignatureFlags::empty())
		.or_add_diagnostic(p, expected_parameters);
...
impl ParseSeparatedList for TsTupleTypeElementList {
	...
	fn recover(&mut self, p:&mut JsParser, parsed_element:ParsedSyntax) -> RecoveryResult {
		parsed_element.or_recover_with_token_set(
			...
			&ParseRecoveryTokenSet::new(
				TS_BOGUS_TYPE,
				token_set![T![']'], T![...], T![ident], T!['['], T!['{'], T![void], T![null]],
			),
			expected_ts_type,
		)
	}
...
fn parse_ts_literal_type(p:&mut JsParser) -> ParsedSyntax {
	...
	if p.at(T![-]) && p.nth_at(1, JS_NUMBER_LITERAL) {
		...
		// Inline the number or big int literal into the number/big int literal
		// type
...
fn parse_ts_template_literal_type(p:&mut JsParser, context:TypeContext) -> ParsedSyntax {
	...
	parse_template_elements(p, TS_TEMPLATE_CHUNK_ELEMENT, TS_TEMPLATE_ELEMENT, false, |p| {
		parse_ts_type(p, context).or_add_diagnostic(p, expected_ts_type)
	});
...
/// Determines if the parser's currently located at a function type. Performs a
/// lookahead of at most a single character.
fn is_at_function_type(p:&mut JsParser) -> bool {
	...
	p.lookahead(|p| {
		...
		if skip_parameter_start(p) {
			// We're passed the start of the parameter. Now we can verify if
			// anything indicates that this is a parameter
...
fn parse_ts_type_predicate(p:&mut JsParser, context:TypeContext) -> ParsedSyntax {
	...
	let kind = if is_asserts { TS_ASSERTS_RETURN_TYPE } else { TS_PREDICATE_RETURN_TYPE };
...
fn can_follow_type_arguments_in_expr(p:&mut JsParser, context:ExpressionContext) -> bool {
	...
	match cur_kind {
		...
		// followed by a line break, a binary operator, or something that
...
impl TypeArgumentsList {
	pub(crate) fn new(context:TypeContext, recover_on_errors:bool) -> Self {
		Self { context, recover_on_errors }
	}
}
...
impl ParseSeparatedList for TypeArgumentsList {
	...
	fn parse_element(&mut self, p:&mut JsParser) -> ParsedSyntax { parse_ts_type(p, self.context) }
	...
	fn recover(&mut self, p:&mut JsParser, parsed_element:ParsedSyntax) -> RecoveryResult {
		if parsed_element.is_absent() && !self.recover_on_errors {
			...
			// Parse conditional expression speculatively tries to parse a list
			// of type arguments The parser shouldn't perform error recovery
			// in that case and simply bail out of parsing
