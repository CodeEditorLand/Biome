...
use crate::syntax::expr::{
	ExpressionContext, is_at_binary_operator, is_at_expression, is_at_identifier, is_nth_at_identifier,
	is_nth_at_identifier_or_keyword, parse_big_int_literal_expression, parse_identifier, parse_literal_expression,
	parse_name, parse_number_literal_expression, parse_reference_identifier, parse_template_elements,
};
use crate::syntax::function::{ParameterContext, parse_formal_parameter, parse_parameter_list, skip_parameter_start};
use crate::syntax::js_parse_error::{
	decorators_not_allowed, expected_identifier, expected_object_member_name, expected_parameter, expected_parameters,
...
use crate::syntax::object::{is_at_object_member_name, is_nth_at_type_member_name, parse_object_member_name};
...
use crate::syntax::typescript::ts_parse_error::{
	...
	ts_const_modifier_cannot_appear_on_a_type_parameter, ts_in_out_modifier_cannot_appear_on_a_type_parameter,
};
...
impl TypeContext {
	...
	pub const DISALLOW_CONDITIONAL_TYPES: Self = Self(make_bitflags!(ContextFlag::{DisallowConditionalTypes}));
	...
	pub const IN_CONDITIONAL_EXTENDS: Self = Self(make_bitflags!(ContextFlag::{InConditionalExtends}));

	/// Whether the current context is within a type or interface declaration
	pub const TYPE_OR_INTERFACE_DECLARATION: Self = Self(make_bitflags!(ContextFlag::{TypeOrInterfaceDeclaration}));
...
pub(crate) fn parse_ts_return_type_annotation(p: &mut JsParser, context: TypeContext) -> ParsedSyntax {
	if !p.at(T![:]) {
		return Absent;
	}
...
impl ParseSeparatedList for TsTypeParameterList {
	...
	fn recover(&mut self, p: &mut JsParser, parsed_element: ParsedSyntax) -> RecoveryResult {
		parsed_element.or_recover_with_token_set(
			p,
			&ParseRecoveryTokenSet::new(TS_BOGUS_TYPE, token_set![T![>], T![,], T![ident], T![yield], T![await]])
...
fn parse_ts_type_parameter_modifiers(p: &mut JsParser, context: TypeContext) -> ParsedSyntax {
	...
	while is_nth_at_type_parameter_modifier(p, 0) {
		...
		if matches!(modifier_kind, TypeParameterModifierKind::Const) && !context.is_const_modifier_allowed() {
		...
		if context.is_conditional_type_allowed() {
			left.map(|left| {
				...
				if !p.has_preceding_line_break() && p.at(T![extends]) {
					...
					parse_ts_type(p, context.and_allow_conditional_types(false).and_in_conditional_extends(true))
...
fn parse_ts_non_array_type(p: &mut JsParser, context: TypeContext) -> ParsedSyntax {
	...
	match p.cur() {
		...
		JS_NUMBER_LITERAL | JS_STRING_LITERAL | TRUE_KW | FALSE_KW | T![null] => parse_ts_literal_type(p),
...
impl ParseNodeList for TypeMembers {
	...
	fn recover(&mut self, p: &mut JsParser, member: ParsedSyntax) -> RecoveryResult {
		member.or_recover_with_token_set(
			p,
			&ParseRecoveryTokenSet::new(JS_BOGUS, token_set![T!['}'], T![,], T![;]]).enable_recovery_on_line_break(),
...
fn parse_ts_type_member(p: &mut JsParser, context: TypeContext) -> ParsedSyntax {
	...
	if is_at_ts_index_signature_member(p) {
		let m = p.start();
		return Present(expect_ts_index_signature_member(p, m, MemberParent::TypeOrInterface, context));
	}

	match p.cur() {
		T!['('] | T![<] => parse_ts_call_signature_type_member(p, context),
		T![new] if is_at_ts_construct_signature_type_member(p) => parse_ts_construct_signature_type_member(p, context),
...
fn parse_ts_property_or_method_signature_type_member(p: &mut JsParser, context: TypeContext) -> ParsedSyntax {
...
fn parse_ts_construct_signature_type_member(p: &mut JsParser, context: TypeContext) -> ParsedSyntax {
	...
	let m = p.start();
	p.expect(T![new]);
	parse_ts_type_parameters(p, context.and_allow_const_modifier(true).and_allow_in_out_modifier(true)).ok();
	parse_parameter_list(p, ParameterContext::Declaration, context, SignatureFlags::empty())
		.or_add_diagnostic(p, expected_parameters);
	parse_ts_type_annotation(p, context).ok();
	parse_ts_type_member_semi(p);
	...
	fn parse_element(&mut self, p: &mut JsParser) -> ParsedSyntax {
		if is_at_named_tuple_type_element(p) {
			...
			if has_ellipsis && has_question_mark {
				let err = p.err_builder("A tuple member cannot be both optional and rest.", syntax.range(p).as_range());
				p.error(err);
...
fn parse_ts_function_type(p: &mut JsParser, context: TypeContext) -> ParsedSyntax {
	...
	.or_add_diagnostic(p, expected_parameters);
	p.expect(T![=>]);
	parse_ts_return_type(p, context.and_allow_conditional_types(true)).or_add_diagnostic(p, expected_ts_type);
...
pub(crate) fn parse_ts_type_arguments_in_expression(p: &mut JsParser, context: ExpressionContext) -> ParsedSyntax {
