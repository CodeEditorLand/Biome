...
pub(crate) fn parse_ts_type(p:&mut JsParser, context:TypeContext) -> ParsedSyntax {
	p.with_state(EnterType, |p| {
		...
		// type X<V> = V extends (...args: any[]) => any ? (...args: Parameters<V>) =>
		// void : Function;
		...
		if context.is_conditional_type_allowed() {
			left.map(|left| {
				...
				//     <T extends any[]>(...args: Tools.Cast<Tools.Cast<T, Gaps<Parameters<F>>>,
				// any[]>) =>          GapsOf<T, Parameters<F>> extends [any, ...any[]]
				//          ? Curry<(...args: GapsOf<T, Parameters<F>> extends infer G ?
				// Tools.Cast<G, any[]> : never) => ReturnType<F>>          : ReturnType<F>;
				// interface GapsOfWorker<T1 extends any[], T2 extends any[], TN extends any[] =
				// [], I extends any[] = []> {     0: GapsOf<T1, T2, GapOf<T1, T2, TN, I>
				// extends infer G ? Tools.Cast<G, any[]> : never, Tools.Next<I>>;
				//     1: Tools.Concat<TN, Tools.Drop<Tools.Pos<I>, T2> extends infer D ?
				// Tools.Cast<D, any[]> : never>; }
...
fn parse_ts_primary_type(p:&mut JsParser, context:TypeContext) -> ParsedSyntax {
	...
	if p.at(T![infer]) {
		...
		return if context.in_conditional_extends() {
			...
			//     : never;
			// type A = MyType extends (OtherType extends infer T ? infer U : InnerFalse) ?
			// OuterTrue : OuterFalse type Join<T extends unknown[], D extends string> =
			//      T extends [] ? '' :
			//      T extends [string | number | boolean | bigint] ? `${T[0]}` :
			//      T extends [string | number | boolean | bigint, ...infer U] ?
			// `${T[0]}${D}${Join<U, D>}` :      string;
			// type MatchPair<S extends string> = S extends `[${infer A},${infer B}]` ? [A,
			// B] : unknown; type FirstTwoAndRest<S extends string> = S extends `${infer
			// A}${infer B}${infer R}` ? [`${A}${B}`, R] : unknown; type Trim<S extends
			// string> =      S extends `${infer T} ` ? Trim<T> :
			//      S;
			...
			//     T;
			// type ArgumentType<T extends (x: any) => any> = T extends (a: infer A) => any
			// ? A : any; type X3<T> = T extends { a: (x: infer U) => void, b: (x: infer
			// U) => void } ? U : never; type X1<T extends { x: any, y: any }> = T
			// extends { x: infer X, y: infer Y } ? [X, Y] : any; type T62<T> = U extends
			// (infer U)[] ? U : U; type T63<T> = T extends ((infer A) extends infer B ?
			// infer C : infer D) ? string : number; type T75<T> = T extends T74<infer U,
			// infer U> ? T70<U> | T72<U> | T74<U, U> : never; type Jsonified<T> = T
			// extends string | number | boolean | null ? T     : T extends undefined |
			// Function ? never     : T extends { toJSON(): infer R } ? R
...
fn try_parse_constraint_of_infer_type(p:&mut JsParser, context:TypeContext) -> ParsedSyntax {
	...
	try_parse(p, |p| {
		...
		// Rewind if conditional types are allowed, and the parser is at the `?` token
		// because this should instead be parsed as a conditional type.
...
// type G = import("test", { with: { "resolution-mode": "import" }
// }).TypeFromImport; type H = import("test", { with: { "resolution-mode":
// "import" } })<string>; type I = import("test", { with: { "resolution-mode":
// "require" } }).C<string>; type J = typeof import("test", { with: {
// "resolution-mode": "require" } }).a.b.c.d.e.f;
...
fn parse_ts_import_type_assertion_block(p:&mut JsParser) -> ParsedSyntax {
	...
	if p.at(T!['{']) {
		...
		if p.at(T!['}']) {
			p.error(
				p.err_builder("Missing import type assertion keyword 'with'", p.cur_range())
...
fn parse_ts_import_type_arguments(p:&mut JsParser, context:TypeContext) -> ParsedSyntax {
	...
	if p.at(T![,]) {
		if p.nth_at(1, T![')']) {
			p.error(
				p.err_builder("ts import type may not have a trailing comma", p.cur_range())
...
fn is_at_function_type(p:&mut JsParser) -> bool {
	...
	p.lookahead(|p| {
		...
		if skip_parameter_start(p) {
			// We're passed the start of the parameter. Now we can verify if anything
...
impl ParseSeparatedList for TypeArgumentsList {
	...
	fn recover(&mut self, p:&mut JsParser, parsed_element:ParsedSyntax) -> RecoveryResult {
		if parsed_element.is_absent() && !self.recover_on_errors {
			...
			// Parse conditional expression speculatively tries to parse a list of type
			// arguments The parser shouldn't perform error recovery in that case and
			// simply bail out of parsing
