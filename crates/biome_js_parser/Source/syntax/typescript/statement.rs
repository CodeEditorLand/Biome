...
use crate::{
	...
	prelude::*,
	state::EnterAmbientContext,
	syntax,
	syntax::{
		auxiliary::{is_nth_at_declaration_clause, parse_declaration_clause},
		binding::{is_nth_at_identifier_binding, parse_binding, parse_identifier_binding},
		class::parse_initializer_clause,
		expr::{ExpressionContext, is_nth_at_identifier, parse_name},
		js_parse_error::{expected_declare_statement, expected_identifier, expected_module_source},
		module::{ModuleItemListParent, parse_module_item_list, parse_module_source},
		stmt::{STMT_RECOVERY_SET, semi},
		typescript::{
			TypeContext,
			TypeMembers,
			expect_ts_type_list,
			...
			ts_parse_error::expected_ts_type,
		},
	},
};
...
impl ParseSeparatedList for TsEnumMembersList {
	...
	fn parse_element(&mut self, p:&mut JsParser) -> ParsedSyntax { parse_ts_enum_member(p) }
...
#[inline(always)]
fn is_reserved_enum_name(name:&str) -> bool { super::is_reserved_type_name(name) }
...
// interface A {}
// interface B { prop: string, method(): string, [index: number]: string, new():
// B }
...
/// Eats an interface's `extends` or an `extends` (not allowed but for better
/// recovery) clauses Attaches the clauses to the currently active node
	...
	if !p.eat(T![namespace]) {
		...
		if p.at(JS_STRING_LITERAL) {
			parse_module_source(p).expect(
				"expected module source to be present because parser is positioned at a string \
				 literal",
			);
