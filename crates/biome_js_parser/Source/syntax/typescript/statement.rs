...
use crate::prelude::*;
use crate::syntax::binding::{is_nth_at_identifier_binding, parse_binding, parse_identifier_binding};
...
use crate::syntax::js_parse_error::{expected_declare_statement, expected_identifier, expected_module_source};
...
use crate::syntax::typescript::{
	TypeContext, TypeMembers, expect_ts_type_list, parse_ts_identifier_binding, parse_ts_implements_clause,
	parse_ts_name, parse_ts_type, parse_ts_type_parameters,
};
		...
		Present(id) => {
			...
			if is_reserved_enum_name(text) {
				let err = p.err_builder(
					format!("`{text}` cannot be used as a enum name because it is already reserved"),
					id.range(p),
				);
...
pub(crate) fn parse_ts_type_alias_declaration(p: &mut JsParser) -> ParsedSyntax {
	...
	let m = p.start();
	p.expect(T![type]);
	parse_ts_identifier_binding(p, super::TsIdentifierContext::Type).or_add_diagnostic(p, expected_identifier);
...
pub(crate) fn is_at_ts_declare_statement(p: &mut JsParser) -> bool {
	...
	if matches!(p.nth(1), T![using]) || (matches!(p.nth(1), T![await]) && matches!(p.nth(2), T![using])) {
		return false;
	}
...
fn eat_interface_heritage_clause(p: &mut JsParser) {
	...
	loop {
		if p.at(T![extends]) {
			let extends = parse_ts_extends_clause(p)
				.expect("expected an extends clause because parser is positioned at the extends keyword");
			...
			let implements = parse_ts_implements_clause(p).expect("positioned at the implements keyword");
			p.error(p.err_builder("Interface declaration cannot have 'implements' clause.", implements.range(p)));
		} else {
			break;
		}
	}
...
pub(crate) fn parse_any_ts_namespace_declaration_clause(p: &mut JsParser, stmt_start_pos: TextSize) -> ParsedSyntax {
	match p.cur() {
		T![global] => parse_ts_global_declaration(p),
		T![namespace] | T![module] => parse_ts_namespace_or_module_declaration_clause(p, stmt_start_pos),
		_ => Absent,
	}
}
...
fn parse_ts_namespace_or_module_declaration_clause(p: &mut JsParser, stmt_start_pos: TextSize) -> ParsedSyntax {
	...
	if !p.eat(T![namespace]) {
		...
		if p.at(JS_STRING_LITERAL) {
			parse_module_source(p)
				.expect("expected module source to be present because parser is positioned at a string literal");
