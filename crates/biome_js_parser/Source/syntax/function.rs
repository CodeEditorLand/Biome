use biome_js_syntax::{JsSyntaxKind, JsSyntaxKind::*, T, TextRange};
...
use crate::{
	...
	syntax::{
		...
		expr::{ExpressionContext, is_nth_at_identifier, parse_assignment_expression_or_higher},
		js_parse_error,
		js_parse_error::{
		...
		stmt::{StatementContext, is_semi, parse_block_impl, semi},
		typescript::{
			TypeContext,
...
fn parse_function(p:&mut JsParser, m:Marker, kind:FunctionKind) -> CompletedMarker {
	...
	let parameter_context = if !kind.is_expression() && TypeScript.is_supported(p) {
		// It isn't known at this point if this is a function overload definition (body
		// is missing) or a regular function implementation.
		// Let's go with the laxer of the two. Ideally, these verifications should be
...
fn try_parse_parenthesized_arrow_function_head(
	...
	// "parse_class_parameter_decorators": true } const method = (@dec x, second,
	// @dec third = 'default') => {}; const method = (@dec.fn() x, second,
	// @dec.fn() third = 'default') => {}; const method = (@dec() x, second, @dec()
	// third = 'default') => {};
...
fn is_parenthesized_arrow_function_expression(
	...
	match p.cur() {
		// These could be the start of a parenthesized arrow function expression but needs further
		// verification
		T!['('] | T![<] => {
...
fn is_parenthesized_arrow_function_expression_impl(
	...
	match p.nth(n) {
		T!['('] => {
			match p.nth(n + 1) {
				T![')'] => {
					// '()' is an arrow expression if followed by an '=>', a type annotation or
					// body. Otherwise, a parenthesized expression with a missing inner
					// expression
					match p.nth(n + 2) {
				...
				T![...] => IsParenthesizedArrowFunctionExpression::True,
				// '([ ...', '({ ... } can either be a parenthesized object or array expression or a
				...
				_ if is_nth_at_identifier_binding(p, n + 1) || p.nth_at(n + 1, T![this]) => {
					match p.nth(n + 2) {
						...
						T![?] => {
							// Disambiguate between an optional parameter and a parenthesized
...
fn arrow_function_parameter_flags(p:&JsParser, mut flags:SignatureFlags) -> SignatureFlags {
	if p.state().in_generator() {
		// Arrow functions inherit whatever yield is a valid identifier name from the
		// parent.
		flags |= SignatureFlags::GENERATOR;
	}
...
pub(crate) fn parse_formal_parameter(
	...
	// test ts ts_formal_parameter_decorator { "parse_class_parameter_decorators":
	// true } class Foo {
	//    constructor(@dec x) {}
	//    method(@dec x) {}
	// }
	...
	if let Present(binding) = parse_binding_pattern(p, expression_context) {
		...
		// declare namespace A {
		//   export class Ajv {
		//     errorsText(errors?: string[] | null | undefined, { separator, dataVar }?:
...
pub(super) fn skip_parameter_start(p:&mut JsParser) -> bool {
	...
	if p.at(T!['[']) || p.at(T!['{']) {
		// Array or object pattern. Try to parse it and return true if there were no
		// parsing errors
	...
	parse_parameters_list(
		...
		|p, expression_context| {
			...
			let decorator_list = if parameter_context.is_any_class_method() {
				...
				// test ts ts_decorator_on_class_method { "parse_class_parameter_decorators":
				// true } class A {
				//     method(@dec x, second, @dec third = 'default') {}
				//     method(@dec.fn() x, second, @dec.fn() third = 'default') {}
				//     method(@dec() x, second, @dec() third = 'default') {}
				...
				// "parse_class_parameter_decorators": true } function method(@dec x,
				// second, @dec third = 'default') {} function method(@dec.fn() x, second,
				// @dec.fn() third = 'default') {} function method(@dec() x, second, @dec()
				// third = 'default') {}

				// test_err ts ts_decorator_on_function_expression {
				// "parse_class_parameter_decorators": true } const expr = function
				// method(@dec x, second, @dec third = 'default') {} const expr = function
				// method(@dec.fn() x, second, @dec.fn() third = 'default') {} const expr =
				// function method(@dec() x, second, @dec() third = 'default') {}

				// test_err ts ts_decorator_on_function_type {
				// "parse_class_parameter_decorators": true } type I = (@dec x, second, @dec
				// third = 'default') => string; type I = (@dec.fn() x, second, @dec.fn()
				// third = 'default') => string; type I = (@dec() x, second, @dec() third =
				// 'default') => string;

				// test_err ts ts_decorator_on_constructor_type {
				// "parse_class_parameter_decorators": true } type I = new(@dec x, second,
				// @dec third = 'default') => string; type I = abstract new(@dec.fn() x,
				// second, @dec.fn() third = 'default') => string; type I = abstract
				// new(@dec() x, second, @dec() third = 'default') => string;

				// test_err ts ts_decorator_on_signature_member {
				// "parse_class_parameter_decorators": true } type A = {new (@dec x, second,
				// @dec third = 'default'): string; } type B = {method(@dec.fn() x, second,
				// @dec.fn() third = 'default'): string; } type C = {
				//  new(@dec() x, second, @dec() third = 'default'): string;
				// 	method(@dec.fn() x, second, @dec.fn() third = 'default'): string;
				// }

				// test_err ts ts_decorator_on_ambient_function {
				// "parse_class_parameter_decorators": true } declare module a {
				// 		function method(@dec x, second, @dec third = 'default') {}
