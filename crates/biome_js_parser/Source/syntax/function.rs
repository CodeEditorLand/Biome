use biome_js_syntax::{JsSyntaxKind, JsSyntaxKind::*, TextRange, T};
...
use crate::{
	parser::ParsedSyntax,
	prelude::*,
	state::{EnterFunction, EnterParameters, SignatureFlags},
	syntax::{
		binding::{
			is_at_identifier_binding,
			is_nth_at_identifier_binding,
			parse_binding,
			parse_binding_pattern,
		},
		class::{empty_decorator_list, parse_initializer_clause, parse_parameter_decorators},
		expr::{is_nth_at_identifier, parse_assignment_expression_or_higher, ExpressionContext},
		js_parse_error,
		js_parse_error::{
		...
		stmt::{is_semi, parse_block_impl, semi, StatementContext},
		typescript::{
			is_nth_at_type_parameter_modifier,
			parse_ts_return_type_annotation,
			parse_ts_type_annotation,
			parse_ts_type_parameters,
			try_parse,
			ts_parse_error::ts_only_syntax_error,
			TypeContext,
		},
	},
	JsParser,
...
/// A function declaration, this could be async and or a generator. This takes a
/// marker because you need to first advance over async or start a marker and
/// feed it in.
...
pub(super) fn parse_function_declaration(
	...
	let mut function = if p.state().in_ambient_context() {
		...
		parse_function(
			p,
			m,
			FunctionKind::Declaration { single_statement_context:context.is_single_statement() },
		)
	};
	...
	if context != StatementContext::StatementList && !function.kind(p).is_bogus() {
		if JsSyntaxFeature::StrictMode.is_supported(p) {
			...
			p.error(
				...
				.with_hint("wrap the function in a block statement"),
			);
			...
			p.error(
				p.err_builder(
					"In non-strict mode code, functions can only be declared at top level, inside \
					 a block, or as the body of an if or labelled statement",
					function.range(p),
				)
				.with_hint("wrap the function in a block statement"),
			);
...
impl AmbientFunctionKind {
	const fn is_export_default(&self) -> bool { matches!(self, AmbientFunctionKind::ExportDefault) }
}
...
impl FunctionKind {
	const fn is_export_default(&self) -> bool { matches!(self, FunctionKind::ExportDefault) }

	fn is_id_optional(&self) -> bool {
		matches!(self, FunctionKind::Expression | FunctionKind::ExportDefault)
	}

	fn is_expression(&self) -> bool { matches!(self, FunctionKind::Expression) }

	fn is_in_single_statement_context(&self) -> bool {
		matches!(self, FunctionKind::Declaration { single_statement_context:true })
	}
}
...
fn parse_function(p:&mut JsParser, m:Marker, kind:FunctionKind) -> CompletedMarker {
	...
	let parameter_context = if !kind.is_expression() && TypeScript.is_supported(p) {
		// It isn't known at this point if this is a function overload
		// definition (body is missing) or a regular function
		// implementation. Let's go with the laxer of the two. Ideally,
		// these verifications should be part of a second compiler pass.
	...
	TypeScript
		.parse_exclusive_syntax(
			...
			|p, marker| {
				p.err_builder("return types can only be used in TypeScript files", marker.range(p))
			},
		)
		.ok();
		...
		&& !kind.is_expression()
	{
		...
		if kind.is_in_single_statement_context() && (in_async || generator_range.is_some()) {
			p.error(p.err_builder(
				"`async` and generator functions can only be declared at top level or inside a \
				 block",
				function.range(p),
			));
			function.change_to_bogus(p);
...
pub(super) fn parse_function_body(p:&mut JsParser, flags:SignatureFlags) -> ParsedSyntax {
	p.with_state(EnterFunction(flags), |p| parse_block_impl(p, JS_FUNCTION_BODY))
}
...
// declare function test<A>(a: A): string { return "ambient function with a
// body"; }
//
...
// }
fn parse_ambient_function(p:&mut JsParser, m:Marker, kind:AmbientFunctionKind) -> CompletedMarker {
	...
	if is_async {
		p.error(
			p.err_builder("'async' modifier cannot be used in an ambient context.", p.cur_range()),
		);
		p.bump(T![async]);
	}
	...
	if is_generator {
		...
		// }
		p.error(p.err_builder("Generators are not allowed in an ambient context.", p.cur_range()));
		p.bump(T![*]);
	}
	...
	if let Present(body) = parse_function_body(p, SignatureFlags::empty()) {
		p.error(
			p.err_builder("A 'declare' function cannot have a function body", body.range(p))
...
pub(crate) fn parse_ts_type_annotation_or_error(p:&mut JsParser) -> ParsedSyntax {
	TypeScript.parse_exclusive_syntax(
		...
		|p, annotation| {
			p.err_builder("return types can only be used in TypeScript files", annotation.range(p))
...
/// There are cases where the parser must speculatively parse a syntax. For
/// example, parsing `<string>(test)` very much looks like an arrow expression
/// *except* that it isn't followed by a `=>`. This enum tells a parse function
/// if ambiguity should be tolerated or if it should stop if it is not.
#[derive(Debug, Copy, Clone)]
pub(crate) enum Ambiguity {
	/// Ambiguity is allowed. A parse method should continue even if an expected
	/// character is missing.
	Allowed,

	/// Ambiguity isn't allowed. A parse method should stop parsing if an
	/// expected character is missing and let the caller decide what to do in
	/// this case.
	Disallowed,
}

impl Ambiguity {
	fn is_disallowed(&self) -> bool { matches!(self, Ambiguity::Disallowed) }
}
...
///
/// The header is everything coming before the (or everything up and including
/// the `=>` token): `async (a) =>`.
///
/// Returns the [Marker] for the parsed arrow function header that must be
/// completed by the caller.
///
/// ## Errors
///
/// Returns `Err` if `ambiguity` is [Ambiguity::Disallowed] and the syntax
/// is ambiguous. The `Err` contains the [Marker] of the syntax parsed to this
/// point. It's up to the caller to abandon or complete the returned marker.
///
/// For example, the parser speculatively tries to parse `<string>(test)` as an
/// arrow function because the start very much looks like one, except that the
/// `=>` token is missing (it's a TypeScript `<string>` cast followed by a
...
fn try_parse_parenthesized_arrow_function_head(
	...
	// \u0061sync () => {}
	let flags = if p.eat(T![async]) { SignatureFlags::ASYNC } else { SignatureFlags::empty() };
	...
	// "parse_class_parameter_decorators": true } const method = (@dec x,
	// second, @dec third = 'default') => {}; const method = (@dec.fn() x,
	// second, @dec.fn() third = 'default') => {}; const method = (@dec() x,
	// second, @dec() third = 'default') => {};
	parse_parameter_list(
		p,
...
fn parse_possible_parenthesized_arrow_function_expression(p:&mut JsParser) -> ParsedSyntax {
	...
	// Test if we already tried to parse this position as an arrow function and
	// failed. If so, bail out immediately.
	if p.state().not_parenthesized_arrow.contains(&start_pos) {
		return Absent;
	}

	match try_parse(p, |p| try_parse_parenthesized_arrow_function_head(p, Ambiguity::Disallowed)) {
		Ok((m, flags)) => {
		...
		Err(m) => {
			// SAFETY: Abandoning the marker here is safe because `try_parse`
			// rewinds if the callback returns `Err` (which is the case that
			// this branch is handling).
			m.abandon(p);
...
fn parse_parenthesized_arrow_function_expression(p:&mut JsParser) -> ParsedSyntax {
	...
	match is_parenthesized {
		IsParenthesizedArrowFunctionExpression::True => {
			let (m, flags) = try_parse_parenthesized_arrow_function_head(p, Ambiguity::Allowed)
				.expect(
					"'CompletedMarker' because function should never return 'Err' if called with \
					 'Ambiguity::Allowed'.",
				);
			parse_arrow_body(p, flags).or_add_diagnostic(p, js_parse_error::expected_arrow_body);
			Present(m.complete(p, JS_ARROW_FUNCTION_EXPRESSION))
...
fn is_parenthesized_arrow_function_expression(
	...
	match p.cur() {
		// These could be the start of a parenthesized arrow function expression
...
fn is_parenthesized_arrow_function_expression_impl(
	...
	match p.nth(n) {
		T!['('] => {
			match p.nth(n + 1) {
				T![')'] => {
					// '()' is an arrow expression if followed by an '=>', a
					// type annotation or body. Otherwise, a parenthesized
					// expression with a missing inner expression
					match p.nth(n + 2) {
						T![=>] | T![:] | T!['{'] => IsParenthesizedArrowFunctionExpression::True,
						_ => IsParenthesizedArrowFunctionExpression::False,
					}
				},
				// Rest parameter '(...a' is certainly not a parenthesized
				// expression
				T![...] => IsParenthesizedArrowFunctionExpression::True,
				...
				_ if is_nth_at_identifier_binding(p, n + 1) || p.nth_at(n + 1, T![this]) => {
					match p.nth(n + 2) {
						...
						// * '(a, ': separator to next parameter or a parenthesized sequence
						//   expression
						// * '(a)': a single parameter OR a parenthesized expression
						T![=] | T![,] | T![')'] => IsParenthesizedArrowFunctionExpression::Unknown,

						T![?] => {
							// Disambiguate between an optional parameter and a
		...
		T![<] => {
			...
			if JsSyntaxFeature::Jsx.is_supported(p) {
				...
				// Type parameters of arrow functions accept only the `const`
				// modifier.
				...
				match p.nth(n + 2) {
					T![extends] => {
						// `<a extends=` OR `<a extends/>` OR `<a extends>` is a
						// JSX element and a `extends` type refinement:
						// `<A extends string>`
					...
					// `<A=` or `<A,` or always type arguments and never JSX
					// tags
...
/// Computes the signature flags for parsing the parameters of an arrow
/// expression. These have different semantics from parsing the body
fn arrow_function_parameter_flags(p:&JsParser, mut flags:SignatureFlags) -> SignatureFlags {
	if p.state().in_generator() {
		// Arrow functions inherit whatever yield is a valid identifier name
		// from the parent.
		flags |= SignatureFlags::GENERATOR;
	}

	// The arrow function is in an async context if the outer function is in an
	// async context or itself is declared async
	if p.state().in_async() {
		flags |= SignatureFlags::ASYNC;
	}
...
fn parse_arrow_function_with_single_parameter(p:&mut JsParser) -> ParsedSyntax {
	...
	p.with_state(EnterParameters(arrow_function_parameter_flags(p, flags)), parse_binding)
		.expect(
			"Expected function parameter to be present as guaranteed by \
			 is_arrow_function_with_simple_parameter",
		);
...
pub(crate) fn parse_any_parameter(
	...
	let parameter = match p.cur() {
		...
		T![this] => {
			// test_err ts ts_decorator_this_parameter_option {
			// "parse_class_parameter_decorators": true } class A {
			//   method(@dec this) {}
			//   method(@dec(val) this) {}
	...
	parameter.map(|mut parameter| {
		if parameter.kind(p) == TS_THIS_PARAMETER {
			if TypeScript.is_unsupported(p) {
				...
				p.error(ts_only_syntax_error(p, "this parameter", parameter.range(p)));
...
pub(crate) fn parse_rest_parameter(
	...
	if let Present(initializer) = parse_initializer_clause(p, ExpressionContext::default()) {
		...
		// for ((...a = "b" in {}) => {};;) {}
		let err =
			p.err_builder("rest elements may not have default initializers", initializer.range(p));
	...
	if p.at(T![,]) {
		let err =
			p.err_builder("rest elements may not have trailing commas", rest_parameter.range(p));
...
impl ParameterContext {
	pub fn is_any_setter(&self) -> bool { self.is_setter() || self.is_class_setter() }

	pub fn is_setter(&self) -> bool { self == &ParameterContext::Setter }

	pub fn is_class_setter(&self) -> bool { self == &ParameterContext::ClassSetter }
	...
	pub fn is_parameter_property(&self) -> bool { self == &ParameterContext::ParameterProperty }

	pub fn is_arrow_function(&self) -> bool { self == &ParameterContext::Arrow }
}
...
pub(crate) fn parse_formal_parameter(
	...
	// "parse_class_parameter_decorators": true } class Foo {
	//    constructor(@dec x) {}
	//    method(@dec x) {}
	// }
	...
	// "parse_class_parameter_decorators": true } function a(@dec x) {}

	// we use a checkpoint to avoid bogus nodes if the binding pattern fails to
	// parse.
	...
	if let Present(binding) = parse_binding_pattern(p, expression_context) {
		...
		let is_optional = if p.at(T![?]) {
			if TypeScript.is_unsupported(p) {
				p.error(ts_only_syntax_error(p, "optional parameters", p.cur_range()));
				valid = false;
		...
		// declare namespace A {
		//   export class Ajv {
		//     errorsText(errors?: string[] | null | undefined, { separator,
		// dataVar }?: ErrorsTextOptions): string;   }
		// }
		if valid
			&& matches!(binding_kind, JS_OBJECT_BINDING_PATTERN | JS_ARRAY_BINDING_PATTERN)
...
/// Skips over the binding token of a parameter. Useful in the context of
/// lookaheads to determine if any typescript specific syntax like `:` is
/// present after the parameter name. Returns `true` if the function skipped
/// over a valid binding, returns false if the parser is not positioned at a
/// binding.
pub(super) fn skip_parameter_start(p:&mut JsParser) -> bool {
	...
	if p.at(T!['[']) || p.at(T!['{']) {
		// Array or object pattern. Try to parse it and return true if there
	...
	parse_parameters_list(
		...
		|p, expression_context| {
			...
			let decorator_list = if parameter_context.is_any_class_method() {
				...
				// {}     method(@dec() x, second, @dec() third = 'default')
				// {}     static method(@dec x, second, @dec third =
				// 'default') {}     static method(@dec.fn() x, second,
				// @dec.fn() third = 'default') {}     static method(@dec()
				// x, second, @dec() third = 'default') {} }
				...
				// function method(@dec.fn() x, second, @dec.fn() third =
				// 'default') {} function method(@dec() x, second, @dec()
				// third = 'default') {}

				// test_err ts ts_decorator_on_function_expression {
				// "parse_class_parameter_decorators": true } const expr =
				// function method(@dec x, second, @dec third = 'default') {}
				// const expr = function method(@dec.fn() x, second, @dec.fn()
				// third = 'default') {} const expr = function method(@dec()
				// x, second, @dec() third = 'default') {}

				// test_err ts ts_decorator_on_function_type {
				// "parse_class_parameter_decorators": true } type I = (@dec
				// x, second, @dec third = 'default') => string; type I = (@
				// dec.fn() x, second, @dec.fn() third = 'default') => string;
				// type I = (@dec() x, second, @dec() third = 'default') =>
				// string;

				// test_err ts ts_decorator_on_constructor_type {
				// "parse_class_parameter_decorators": true } type I = new(@
				// dec x, second, @dec third = 'default') => string;
				// type I = abstract new(@dec.fn() x, second, @dec.fn() third =
				// 'default') => string; type I = abstract new(@dec() x,
				// second, @dec() third = 'default') => string;

				// test_err ts ts_decorator_on_signature_member {
				// "parse_class_parameter_decorators": true } type A = {new
				// (@dec x, second, @dec third = 'default'): string; }
				// type B = {method(@dec.fn() x, second, @dec.fn() third =
				// 'default'): string; } type C = {
				//  new(@dec() x, second, @dec() third = 'default'): string;
				// 	method(@dec.fn() x, second, @dec.fn() third = 'default'):
				// string; }

				// test_err ts ts_decorator_on_ambient_function {
				// "parse_class_parameter_decorators": true } declare module
				// a { 		function method(@dec x, second, @dec third =
				// 'default') {} 		function method(@dec.fn() x, second,
				// @dec.fn() third = 'default') {} 		function method(@dec()
				// x, second, @dec() third = 'default') {} }
				// declare function method(@dec x, second, @dec third =
				// 'default') declare function method(@dec.fn() x, second,
				// @dec.fn() third = 'default') declare function
				// method(@dec() x, second, @dec() third = 'default')
				decorator_list
