...
use biome_parser::{event::Event, token_source::Trivia, AnyParse};
...
impl<T> Parse<T> {
	...
	/// ```
	pub fn syntax(&self) -> JsSyntaxNode { self.root.clone() }

	/// Get the diagnostics which occurred when parsing
	pub fn diagnostics(&self) -> &[ParseDiagnostic] { self.errors.as_slice() }

	/// Get the diagnostics which occurred when parsing
	pub fn into_diagnostics(self) -> Vec<ParseDiagnostic> { self.errors }

	/// Returns [true] if the parser encountered some errors during the parsing.
	pub fn has_errors(&self) -> bool { self.errors.iter().any(|diagnostic| diagnostic.is_error()) }
}
...
impl<T:AstNode<Language = JsLanguage>> Parse<T> {
	...
	pub fn try_tree(&self) -> Option<T> { T::cast(self.syntax()) }
...
/// Parse text into a [`Parse`](Parse) which can then be turned into an untyped
/// root [`JsSyntaxNode`](JsSyntaxNode). Or turned into a typed
/// [`JsScript`](JsScript) with [`tree`](Parse::tree).
///
/// ```
...
pub fn parse_script(text:&str, options:JsParserOptions) -> Parse<JsScript> {
	parse(text, JsFileSource::js_module().with_module_kind(ModuleKind::Script), options)
		.cast::<JsScript>()
		.unwrap()
}

/// Same as [parse_script] but configures the parser to parse an ECMAScript
...
pub fn parse_module(text:&str, options:JsParserOptions) -> Parse<JsModule> {
	parse(text, JsFileSource::js_module(), options).cast::<JsModule>().unwrap()
}

/// Parses the provided string as a EcmaScript program using the provided syntax
/// features.
///
/// ### Examples
///
...
/// use biome_js_syntax::{JsFileSource, LanguageVariant, LanguageVersion, ModuleKind};
