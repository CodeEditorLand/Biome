...
pub(crate) struct JsParserState {
	...
	/// Indicates that the parser is speculatively parsing a syntax.
	/// Speculative parsing means that the parser tries to parse a syntax as
	/// one kind and determines at the end if the assumption was right
	/// by testing if the parser is at a specific token (or has no errors). For
	/// this approach to work, the parser isn't allowed to skip any tokens
	/// while doing error recovery because it may then successfully
	/// skip over all invalid tokens, so that it appears as if it was able to
	/// parse the syntax correctly.
	///
