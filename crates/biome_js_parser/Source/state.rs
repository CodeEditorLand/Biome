...
impl ExportDefaultItemKind {
	pub(crate) fn is_interface(&self) -> bool { matches!(self, ExportDefaultItemKind::Interface) }
...
pub(crate) struct JsParserState {
	...
	/// A list of labels for labelled statements used to report undefined label
	/// errors for break and continue, as well as duplicate labels.
	...
	/// If set, the parser reports bindings with identical names. The option
	/// stores the name of the node that disallows duplicate bindings, for
	/// example `let`, `const` or `import`.
	pub duplicate_binding_parent:Option<&'static str>,
	pub name_map:IndexMap<String, TextRange>,

	/// Indicates that the parser is speculatively parsing a syntax. Speculative
	/// parsing means that the parser tries to parse a syntax as one kind and
	/// determines at the end if the assumption was right by testing if the
	/// parser is at a specific token (or has no errors). For this approach to
	/// work, the parser isn't allowed to skip any tokens while doing error
	/// recovery because it may then successfully skip over all invalid tokens,
	/// so that it appears as if it was able to parse the syntax correctly.
	///
	/// Speculative parsing is useful if a syntax is ambiguous and no amount of
	/// lookahead (except parsing the whole syntax) is sufficient to determine
	/// what syntax it is. For example, the syntax `(a, b) ...` in JavaScript
	...
	/// Stores the token positions of all syntax that looks like an arrow
	/// expressions but aren't one. Optimization to reduce the back-tracking
	/// required when parsing parenthesized and arrow function expressions.
...
impl JsParserState {
	pub fn new(source_type:&JsFileSource) -> Self {
		let mut state = JsParserState {
			...
			strict:source_type.module_kind().is_module().then_some(StrictMode::Module),
			default_item:None,
	...
	pub fn in_function(&self) -> bool {
		self.parsing_context.contains(ParsingContextFlags::IN_FUNCTION)
	}

	pub fn in_generator(&self) -> bool {
		self.parsing_context.contains(ParsingContextFlags::IN_GENERATOR)
	}

	pub fn in_async(&self) -> bool { self.parsing_context.contains(ParsingContextFlags::IN_ASYNC) }

	pub fn in_ambient_context(&self) -> bool {
		self.parsing_context.contains(ParsingContextFlags::AMBIENT_CONTEXT)
	}

	pub fn in_constructor(&self) -> bool {
		self.parsing_context.contains(ParsingContextFlags::IN_CONSTRUCTOR)
	}

	pub fn is_top_level(&self) -> bool {
		self.parsing_context.contains(ParsingContextFlags::TOP_LEVEL)
	}

	pub fn continue_allowed(&self) -> bool {
		self.parsing_context.contains(ParsingContextFlags::CONTINUE_ALLOWED)
	}

	pub fn break_allowed(&self) -> bool {
		self.parsing_context.contains(ParsingContextFlags::BREAK_ALLOWED)
	}

	pub fn strict(&self) -> Option<&StrictMode> { self.strict.as_ref() }
...
///
/// It's important that creating and rewinding a snapshot is cheap. Consider the
/// performance implications before adding new unscoped state.
#[derive(Debug)]
...
impl JsParserStateCheckpoint {
	...
	fn snapshot(state:&JsParserState) -> Self {
		Self { debug_checkpoint:JsDebugParserStateCheckpoint::snapshot(state) }
	}

	#[cfg(not(debug_assertions))]
	fn snapshot(_:&JsParserState) -> Self { Self {} }

	/// Restores the `state values` to the time when this snapshot was created.
	#[cfg(debug_assertions)]
	fn rewind(self, state:&mut JsParserState) { self.debug_checkpoint.rewind(state); }

	#[cfg(not(debug_assertions))]
	fn rewind(self, _:&JsParserState) {}
}

/// Most of the [JsParserState] is scoped state. It should, therefore, not be
/// necessary to rewind that state because that's already taken care of by
/// `with_state` and `with_scoped_state`. But, you can never no and better be
/// safe than sorry. That's why we use some heuristics to verify that non of the
/// scoped state did change and assert for it when rewinding.
#[derive(Debug, Clone)]
#[cfg(debug_assertions)]
...
impl JsDebugParserStateCheckpoint {
	...
	fn rewind(self, state:&mut JsParserState) {
		...
		assert_eq!(state.duplicate_binding_parent, self.duplicate_binding_parent);
...
/// Implements a specific modification to the parser state that can later be
/// reverted.
pub(crate) trait ChangeParserState {
	type Snapshot: Default;

	/// Applies the change to the passed in state and returns snapshot that
	/// allows restoring the previous state.
...
impl ChangeParserState for EnableStrictMode {
	...
	#[inline]
	fn restore(state:&mut JsParserState, value:Self::Snapshot) { state.strict = value.0 }
}
...
impl BitOr for SignatureFlags {
	...
	fn bitor(self, rhs:Self) -> Self::Output { SignatureFlags(self.0 | rhs.0) }
}
...
/// The reasons to use flags instead of individual boolean fields on
/// `ParserState` are:
/// * It's possible to use bit masks to define what state should be inherited.
///   For example, functions inherit whether they're defined inside a parameter
///   but override the `in_async` flag
/// * It's easier to snapshot the previous state. Individual boolean fields
///   would require that a change snapshots each individual boolean field to
///   allow restoring the previous state. With bitflags, all that is needed is
///   to copy away the flags field and restore it after.
...
impl ParsingContextFlags {
	...
	/// Whether the parser is in an iteration statement and `continue` is
	/// allowed.
	const CONTINUE_ALLOWED:Self = Self(make_bitflags!(ParsingContextFlag::{ContinueAllowed}));
	/// Bitmask of all the flags that must be reset (shouldn't be inherited)
	/// when the parser enters a function
	...
	/// Is async allowed in this context. Either because it's an async function
	/// or top level await is supported. Equivalent to the `Async` generator in
	/// the ECMA spec
	...
	/// Whether the parser is parsing a top-level statement (not inside a class,
	/// function, parameter) or not
...
impl BitOr for ParsingContextFlags {
	...
	fn bitor(self, rhs:Self) -> Self::Output { ParsingContextFlags(self.0 | rhs.0) }
}
...
impl<T:ChangeParserStateFlags> ChangeParserState for T {
	...
	fn restore(state:&mut JsParserState, value:Self::Snapshot) { state.parsing_context = value.0 }
}
...
/// Enters the parsing of a function/method. Resets the relevant parser state
/// and sets the state according to the passed [SignatureFlags]
...
impl ChangeParserState for WithLabel {
	...
	#[cfg(not(debug_assertions))]
	fn restore(state:&mut JsParserState, _:Self::Snapshot) { state.label_set.pop(); }
...
/// Sets the state changes needed when parsing a TS type declaration (async and
/// await are not reserved identifiers)
