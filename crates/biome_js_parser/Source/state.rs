...
impl LabelledItem {
	pub(crate) fn range(&self) -> &TextRange {
		match self {
			LabelledItem::Iteration(range) | LabelledItem::Other(range) => {
				range
			},
		}
	}
...
impl ExportDefaultItemKind {
	...
	const fn can_merge(
		a: &ExportDefaultItemKind,
		b: &ExportDefaultItemKind,
	) -> bool {
		match (a, b) {
...
impl<'parser, 't, C: ChangeParserState> Drop
	for ParserStateGuard<'parser, 't, C>
{
	fn drop(&mut self) {
...
impl<'parser, 't, C: ChangeParserState> Deref
	for ParserStateGuard<'parser, 't, C>
{
...
impl<'parser, 't, C: ChangeParserState> DerefMut
	for ParserStateGuard<'parser, 't, C>
{
...
impl ChangeParserState for EnableStrictMode {
	...
	#[inline]
	fn apply(self, state: &mut JsParserState) -> Self::Snapshot {
		EnableStrictModeSnapshot(std::mem::replace(
			&mut state.strict,
			Some(self.0),
		))
	}
...
impl SignatureFlags {
	...
	pub const GENERATOR: Self =
		Self(make_bitflags!(SignatureFlag::{Generator}));
	/// Is the function a constructor (or constructor context)
	pub const CONSTRUCTOR: Self =
		Self(make_bitflags!(SignatureFlag::{Constructor}));
...
impl ParsingContextFlags {
	...
	const IN_GENERATOR: Self =
		Self(make_bitflags!(ParsingContextFlag::{InGenerator}));
	/// Whether the parser is inside a function
	const IN_FUNCTION: Self =
		Self(make_bitflags!(ParsingContextFlag::{InFunction}));
	/// Whatever the parser is inside a constructor
	const IN_CONSTRUCTOR: Self =
		Self(make_bitflags!(ParsingContextFlag::{InConstructor}));
	...
	const TOP_LEVEL: Self =
		Self(make_bitflags!(ParsingContextFlag::{TopLevel}));

	/// Whether the parser is in an iteration or switch statement and
	/// `break` is allowed.
	const BREAK_ALLOWED: Self =
		Self(make_bitflags!(ParsingContextFlag::{BreakAllowed}));

	/// Whether the parser is in an iteration statement and `continue` is allowed.
	const CONTINUE_ALLOWED: Self =
		Self(make_bitflags!(ParsingContextFlag::{ContinueAllowed}));

	/// Whether the parser is in a TypeScript ambient context
	const AMBIENT_CONTEXT: Self =
		Self(make_bitflags!(ParsingContextFlag::{AmbientContext}));
...
pub(crate) trait ChangeParserStateFlags {
	fn compute_new_flags(
		&self,
		existing: ParsingContextFlags,
	) -> ParsingContextFlags;
}
...
impl ChangeParserStateFlags for EnterParameters {
	fn compute_new_flags(
		...
		(existing - ParsingContextFlags::PARAMETER_RESET_MASK)
...
impl ChangeParserState for EnterFunction {
	...
	fn apply(self, state: &mut JsParserState) -> Self::Snapshot {
		...
		EnterFunctionSnapshot {
			parsing_context: std::mem::replace(
				&mut state.parsing_context,
				new_flags,
			),
