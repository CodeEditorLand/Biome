...
use crate::{parser::JsParser, prelude::*, token_source::JsTokenSourceCheckpoint};

/// Simplified parser API for when rewriting the AST structure with
/// `rewrite_events`.
///
/// The difference from the regular [Parser] is that the [TokenSource] must be
/// detached during rewriting to avoid lexing previously lexed tokens in a
/// different context. For example for `a[`test`] = "b"`. Template literal
/// elements get lexed in the [TemplateElement] context. However, if the
/// rewriter rewinds the token source then all tokens are lexed in the
/// [LexContext::Regular] which yields complete different results.
///
/// This is why the [RewriteParser] tracks the source offset without relying on
/// the `TokenSource` and explicitly passes the positions to [Marker] and
/// [CompletedMarker]. This further has the benefit that rewriting the events
/// doesn't require re-lexing all tokens as well.
pub(crate) struct RewriteParser<'parser, 'source> {
	...
	/// Offset to the next not yet processed trivia in
	/// [TokenSource::trivia_list].
	trivia_offset:usize,
}
...
impl<'parser, 'source> RewriteParser<'parser, 'source> {
	...
	pub fn bump(&mut self, token:RewriteToken) {
		...
		// If the parser originally skipped this token as trivia, then make sure
		// to also consume the trivia.
	...
	fn skip_trivia(&mut self, trailing:bool) {
		...
		for trivia in remaining_trivia {
			// Don't skip over any "skipped token trivia". These get consumed
			// when bumping the token.
	...
	///
	/// ## Panics
	/// If not all tokens have been consumed or if they have been consumed out
	/// of order
	...
	pub fn is_strict_mode(&self) -> bool { self.inner.state().strict().is_some() }
...
impl RewriteToken {
	pub fn new(kind:JsSyntaxKind, end:TextSize) -> Self { Self { kind, end } }
}
...
impl RewriteCompletedMarker {
	...
	pub fn range(&self, p:&RewriteParser) -> TextRange { self.0.range(p.inner) }

	/// Returns the source text of the marker
	pub fn text<'a>(&self, p:&'a RewriteParser) -> &'a str { self.0.text(p.inner) }

	pub fn change_to_bogus(&mut self, p:&mut RewriteParser) { self.0.change_to_bogus(p.inner) }
}

impl From<RewriteCompletedMarker> for CompletedMarker {
	fn from(inner:RewriteCompletedMarker) -> Self { inner.0 }
}
