...
pub(crate) fn rewrite_events<T: RewriteParseEvents>(
	...
	let events: Vec<_> = unsafe { p.context_mut().split_off_events(checkpoint.context.event_position() + 1) };

	let mut sink = RewriteParseEventsTreeSink { parser: RewriteParser::new(p, checkpoint.source), reparse: rewriter };
