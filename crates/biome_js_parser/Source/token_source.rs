...
use crate::{
	JsParserOptions,
	lexer::{JsLexContext, JsLexer, JsReLexContext, TextRange},
	prelude::*,
};
...
impl<'l> JsTokenSource<'l> {
	...
	pub(crate) fn new(lexer:BufferedLexer<JsSyntaxKind, JsLexer<'l>>) -> JsTokenSource<'l> {
		JsTokenSource { lexer, trivia_list:vec![] }
	}
	...
	fn next_non_trivia_token(&mut self, context:JsLexContext, first_token:bool) {
		...
		loop {
			...
			match trivia_kind {
				Err(_) => break,
				Ok(trivia_kind) => {
					// Trivia after and including the newline is considered the leading trivia of
					// the next token
	...
	#[inline(always)]
	pub fn has_unicode_escape(&self) -> bool { self.lexer.has_unicode_escape() }

	/// Returns `true` if the next token has any preceding trivia (either
	/// trailing trivia of the current token or leading trivia of the next
	/// token)
	...
	pub fn re_lex(&mut self, mode:JsReLexContext) -> JsSyntaxKind { self.lexer.re_lex(mode) }
...
impl<'source> TokenSource for JsTokenSource<'source> {
	...
	#[inline(always)]
	fn current(&self) -> JsSyntaxKind { self.lexer.current() }
	...
	#[inline(always)]
	fn text(&self) -> &'source str { self.lexer.source() }

	#[inline(always)]
	fn position(&self) -> TextSize { self.current_range().start() }
	...
	#[inline(always)]
	fn bump(&mut self) { self.bump_with_context(JsLexContext::Regular) }

	fn skip_as_trivia(&mut self) { self.skip_as_trivia_with_context(JsLexContext::Regular) }
...
impl<'source> TokenSourceWithBufferedLexer<JsLexer<'source>> for JsTokenSource<'source> {
	fn lexer(&mut self) -> &mut BufferedLexer<JsSyntaxKind, JsLexer<'source>> { &mut self.lexer }
}
