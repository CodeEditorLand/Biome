...
impl<'l> JsTokenSource<'l> {
	...
	pub fn has_next_preceding_trivia(&mut self) -> bool {
		let next_token_trivia =
			self.lexer.lookahead_iter().next().and_then(|lookahead| {
				TriviaPieceKind::try_from(lookahead.kind()).ok()
			});
...
impl<'source> TokenSourceWithBufferedLexer<JsLexer<'source>>
	for JsTokenSource<'source>
{
