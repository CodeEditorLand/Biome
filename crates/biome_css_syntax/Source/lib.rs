...
pub use biome_rowan::{
	SyntaxNodeText, TextLen, TextRange, TextSize, TokenAtOffset,
...
impl CssSyntaxKind {
	...
	pub const fn is_contextual_keyword(self) -> bool {
		(self as u16) >= (MEDIA_KW as u16)
			&& (self as u16) <= (FONT_FACE_KW as u16)
	}

	/// Returns `true` for css-wide keywords
	#[inline]
	pub const fn is_css_wide_keyword(self) -> bool {
		(self as u16) >= (INITIAL_KW as u16)
			&& (self as u16) <= (DEFAULT_KW as u16)
	}
...
impl biome_rowan::SyntaxKind for CssSyntaxKind {
	...
	fn to_bogus(&self) -> Self {
		match self {
			kind if AnyCssSubSelector::can_cast(*kind) => {
				CSS_BOGUS_SUB_SELECTOR
			},
			kind if AnyCssSelector::can_cast(*kind) => CSS_BOGUS_SELECTOR,
			kind if AnyCssRule::can_cast(*kind) => CSS_BOGUS_RULE,
			kind if AnyCssPseudoClass::can_cast(*kind) => {
				CSS_BOGUS_PSEUDO_CLASS
			},
			kind if AnyCssPseudoElement::can_cast(*kind) => {
				CSS_BOGUS_PSEUDO_ELEMENT
			},
			...
			kind if AnyCssKeyframesSelector::can_cast(*kind) => {
				CSS_BOGUS_SELECTOR
			},
			kind if AnyCssPageSelectorPseudo::can_cast(*kind) => {
				CSS_BOGUS_PAGE_SELECTOR_PSEUDO
			},
			kind if AnyCssLayer::can_cast(*kind) => CSS_BOGUS_LAYER,
			kind if AnyCssScopeRange::can_cast(*kind) => CSS_BOGUS_SCOPE_RANGE,
			kind if AnyCssKeyframesItem::can_cast(*kind) => {
				CSS_BOGUS_KEYFRAMES_ITEM
			},
			kind if AnyCssProperty::can_cast(*kind) => CSS_BOGUS_PROPERTY,
			kind if AnyCssDocumentMatcher::can_cast(*kind) => {
				CSS_BOGUS_DOCUMENT_MATCHER
			},
			kind if AnyCssKeyframesName::can_cast(*kind) => {
				CSS_BOGUS_KEYFRAMES_NAME
			},
			kind if AnyCssCustomIdentifier::can_cast(*kind) => {
				CSS_BOGUS_CUSTOM_IDENTIFIER
			},
			kind if AnyCssDeclarationOrAtRuleBlock::can_cast(*kind) => {
				CSS_BOGUS_BLOCK
			},
			kind if AnyCssDeclarationOrRuleBlock::can_cast(*kind) => {
				CSS_BOGUS_BLOCK
			},
			kind if AnyCssConditionalBlock::can_cast(*kind) => CSS_BOGUS_BLOCK,
			kind if AnyCssFontFeatureValuesBlock::can_cast(*kind) => {
				CSS_BOGUS_BLOCK
			},
			kind if AnyCssUnicodeValue::can_cast(*kind) => {
				CSS_BOGUS_UNICODE_RANGE_VALUE
			},
