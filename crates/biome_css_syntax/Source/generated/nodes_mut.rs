...
impl CssAttributeName {
	pub fn with_namespace(self, element:Option<CssNamespace>) -> Self {
		Self::unwrap_cast(
			self.syntax
				.splice_slots(0usize..=0usize, once(element.map(|element| element.into_syntax().into()))),
		)
	}
...
impl CssAttributeSelector {
	...
	pub fn with_matcher(self, element:Option<CssAttributeMatcher>) -> Self {
		Self::unwrap_cast(
			self.syntax
				.splice_slots(2usize..=2usize, once(element.map(|element| element.into_syntax().into()))),
		)
	}
...
impl CssComposesPropertyValue {
	...
	pub fn with_specifier(self, element:Option<CssComposesImportSpecifier>) -> Self {
		Self::unwrap_cast(
			self.syntax
				.splice_slots(1usize..=1usize, once(element.map(|element| element.into_syntax().into()))),
		)
	}
}
...
impl CssImportAtRule {
	...
	pub fn with_supports(self, element:Option<CssImportSupports>) -> Self {
		Self::unwrap_cast(
			self.syntax
				.splice_slots(3usize..=3usize, once(element.map(|element| element.into_syntax().into()))),
		)
	}
