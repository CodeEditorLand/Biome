...
impl CssMediaType {
	...
	pub fn as_fields(&self) -> CssMediaTypeFields {
		CssMediaTypeFields { value: self.value() }
	}
...
impl AstNode for AnyCssDocumentMatcher {
	...
	fn can_cast(kind: SyntaxKind) -> bool {
		matches!(kind, CSS_BOGUS_DOCUMENT_MATCHER | CSS_DOCUMENT_CUSTOM_MATCHER | CSS_URL_FUNCTION)
	}
	fn cast(syntax: SyntaxNode) -> Option<Self> {
		let res = match syntax.kind() {
...
impl AstNode for AnyCssStartingStyleBlock {
	type Language = Language;
	const KIND_SET: SyntaxKindSet<Language> =
		CssBogusBlock::KIND_SET.union(CssDeclarationBlock::KIND_SET).union(CssRuleBlock::KIND_SET);
	fn can_cast(kind: SyntaxKind) -> bool {
...
impl AstNode for AnyCssUrlModifier {
	type Language = Language;
	const KIND_SET: SyntaxKindSet<Language> =
		CssBogusUrlModifier::KIND_SET.union(CssFunction::KIND_SET).union(CssIdentifier::KIND_SET);
	fn can_cast(kind: SyntaxKind) -> bool {
