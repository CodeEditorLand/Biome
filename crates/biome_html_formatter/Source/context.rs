...
use biome_formatter::{
	AttributePosition, BracketSameLine, CstFormatContext, FormatContext, FormatOptions, IndentStyle, IndentWidth,
	LineEnding, LineWidth, TransformSourceMap, printer::PrinterOptions,
};
...
impl HtmlFormatOptions {
	...
	pub fn with_whitespace_sensitivity(mut self, whitespace_sensitivity: WhitespaceSensitivity) -> Self {
		self.whitespace_sensitivity = whitespace_sensitivity;
		self
	}

	pub fn with_indent_script_and_style(mut self, indent_script_and_style: IndentScriptAndStyle) -> Self {
...
impl fmt::Display for HtmlFormatOptions {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		...
		writeln!(f, "Indent script and style: {}", self.indent_script_and_style.value())?;
		Ok(())
	}
}
...
impl FromStr for WhitespaceSensitivity {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		match s {
			...
			_ => {
				Err("Value not supported for WhitespaceSensitivity. Supported values are 'css', 'strict' and 'ignore'.")
			},
		}
	}
}
...
impl FromStr for IndentScriptAndStyle {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		match bool::from_str(s) {
			...
			Err(_) => Err("Value not supported for IndentScriptAndStyle. Supported values are 'true' and 'false'."),
		}
	}
}
