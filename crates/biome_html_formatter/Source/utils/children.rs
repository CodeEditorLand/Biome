...
/// A word in a Html Text. A word is string sequence that isn't separated by any
/// HTML whitespace.
...
impl HtmlWord {
	fn new(text:TokenText, source_position:TextSize) -> Self { HtmlWord { text, source_position } }
...
pub(crate) enum HtmlChild {
	...
	/// A new line at the start or end of a [HtmlText] with meaningful content.
	...
	/// A [HtmlText] that only consists of whitespace and has at least two line
	/// breaks;
	///
	/// ```html
	/// <div>
					...
					match chunks.next().unwrap() {
						(_, HtmlTextChunk::Whitespace(whitespace)) => {
							if whitespace.contains('\n') {
								// A text only consisting of whitespace that
								// also contains a new line isn't considered
								// meaningful text. It can be entirely
								// removed from the content without changing the
								// semantics.
...
/// 1. Remove [HtmlChild::EmptyLine], [HtmlChild::Newline],
///    [HtmlChild::Whitespace] if a next element is [HtmlChild::Whitespace]
/// 2. Don't push a new element [HtmlChild::EmptyLine], [HtmlChild::Newline],
///    [HtmlChild::Whitespace] if previous one is [HtmlChild::EmptyLine],
///    [HtmlChild::Newline], [HtmlChild::Whitespace]
///
...
impl HtmlSplitChildrenBuilder {
	fn new() -> Self { HtmlSplitChildrenBuilder { buffer:vec![] } }
	...
	fn finish(self) -> Vec<HtmlChild> { self.buffer }
}
