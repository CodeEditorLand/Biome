...
impl FormatNodeRule<HtmlElement> for FormatHtmlElement {
	fn fmt_fields(&self, node:&HtmlElement, f:&mut HtmlFormatter) -> FormatResult<()> {
		...
		// We ignore the `script` and `style` tags as well, since embedded language
		// parsing/formatting is not yet implemented.
		let should_be_verbatim =
			["script", "style", "pre"].iter().any(|tag| tag_name.eq_ignore_ascii_case(tag));
