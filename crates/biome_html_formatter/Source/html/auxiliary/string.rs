	...
	fn fmt_fields(&self, node: &HtmlString, f: &mut HtmlFormatter) -> FormatResult<()> {
		...
		if let Ok(value) = value_token.as_ref() {
			...
			if !(value_text.starts_with('"') && value_text.ends_with('"')) {
				...
				let range = if value_text.starts_with('\'') && value_text.ends_with('\'') && !contains_double_quote {
					value.text_trimmed_range().add_start(1.into()).sub_end(1.into())
				} else {
				...
				if !contains_double_quote {
					write!(
						...
						[format_replaced(
							value,
							&group(&format_args![text("\""), located_token_text(value, range), text("\""),])
						)]
					)?;
				} else {
					value.format().fmt(f)?;
