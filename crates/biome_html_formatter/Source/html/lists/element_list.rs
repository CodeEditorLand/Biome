//! This implementation is very heavily inspired by the JSX formatter
//! implementation for JsxChildList.

use std::cell::RefCell;

use biome_formatter::{best_fitting, format_args, prelude::*, write, CstFormatContext, VecBuffer};
...
/// The result of formatting the children of an [HtmlElementList]. This is
/// ultimately determined by [FormatHtmlElementList::layout].
#[derive(Debug)]
...
impl FormatHtmlElementList {
	pub(crate) fn fmt_children(
		...
		let is_root_parent =
			list.syntax().parent().is_some_and(|parent| HtmlRoot::can_cast(parent.kind()));
		...
		while let Some(child) = children_iter.next() {
			...
			match &child {
				...
				// they're separated by HTML Whitespace.
				HtmlChild::Word(word) => {
				...
				HtmlChild::Newline => {
					let is_soft_break = {
						...
						// [HtmlChild::Word, HtmlChild::Newline,
						// HtmlChild::NonText] ```
						// <div>
						//   <div>First</div>,
						//   <div>Second</div>
						...
						if let Some(HtmlChild::Word(word)) = last {
							let is_next_element_self_closing = matches!(
								children_iter.peek(),
								Some(HtmlChild::NonText(AnyHtmlElement::HtmlSelfClosingElement(_)))
							);
						...
						// [HtmlChild::Newline, HtmlChild::Word,
						// HtmlChild::NonText] ```
						// <div>
						//   <div>First</div>
						//   ,<div>Second</div>
	...
	///
	/// This is necessary because the formatting of [HtmlContentList] bypasses
	/// the node formatting for [HtmlContent] and instead, formats the nodes
	/// itself.
...
impl HtmlChildListLayout {
	const fn is_multiline(&self) -> bool { matches!(self, HtmlChildListLayout::Multiline) }
}
...
struct ChildrenMeta {
	...
	/// `true` if any child contains meaningful a [HtmlText] with meaningful
	/// text.
...
enum WordSeparator {
	...
	/// A separator of a word at the end of a [HtmlText] element. Either because
...
impl MultilineBuilder {
	fn new(layout:MultilineLayout, is_root:bool) -> Self {
		Self { layout, is_root, result:Ok(Vec::new()) }
	}
	...
	/// slot
	fn write_separator(&mut self, separator:&dyn Format<HtmlFormatContext>, f:&mut HtmlFormatter) {
...
impl Format<HtmlFormatContext> for FormatMultilineChildren {
	fn fmt(&self, f:&mut Formatter<HtmlFormatContext>) -> FormatResult<()> {
		...
		// We do not need the block ident when the list node is at the html root
		// node
		if self.is_root {
