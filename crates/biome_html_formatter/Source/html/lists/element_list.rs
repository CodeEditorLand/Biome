...
use crate::{
	...
	prelude::*,
	utils::{
		children::{HtmlChild, HtmlChildrenIterator, HtmlSpace, html_split_children, is_meaningful_html_text},
...
use biome_html_syntax::{
	AnyHtmlElement, HtmlClosingElement, HtmlClosingElementFields, HtmlElementList, HtmlRoot, HtmlSyntaxToken,
};
use tag::GroupMode;
...
impl FormatHtmlElementList {
	pub(crate) fn fmt_children(
		...
		let is_root_parent = list.syntax().parent().is_some_and(|parent| HtmlRoot::can_cast(parent.kind()));
			...
			match &child {
				...
				HtmlChild::Word(word) => {
					let separator = match children_iter.peek() {
						...
						Some(HtmlChild::NonText(next_child)) => Some(WordSeparator::EndOfText {
							is_soft_line_break: !matches!(next_child, AnyHtmlElement::HtmlSelfClosingElement(_))
								|| word.is_single_character(),
							is_next_element_whitespace_sensitive: is_element_whitespace_sensitive_from_element(
								f, next_child,
							),
						}),

						Some(HtmlChild::Newline | HtmlChild::Whitespace | HtmlChild::EmptyLine) => None,
				...
				HtmlChild::Newline => {
					let is_soft_break = {
						...
						// ```
							...
							let is_next_next_element_new_line = matches!(next_next_element, Some(HtmlChild::Newline));
				...
				HtmlChild::NonText(non_text) => {
					let line_mode = match children_iter.peek() {
						...
						Some(HtmlChild::NonText(next_non_text)) => {
							...
							if !is_element_whitespace_sensitive_from_element(f, non_text) {
								...
								let has_whitespace_between =
									non_text.syntax().last_token().is_some_and(|tok| tok.has_trailing_whitespace())
										|| next_non_text
											.syntax()
											.first_token()
					...
					let format_separator =
						line_mode.map(|mode| format_with(move |f| f.write_element(FormatElement::Line(mode))));

					if force_multiline {
						if let Some(format_separator) = format_separator {
							multiline.write_with_separator(&non_text.format(), &format_separator, f);
						} else {
							// it's safe to write without a separator because None means that next element is a separator or end of the iterator
			...
			match child {
				...
				HtmlContent(text) => {
					meta.meaningful_text = meta.meaningful_text
						|| text.value_token().is_ok_and(|token| is_meaningful_html_text(token.text()));
				},
				_ => {},
			}
		}
...
enum WordSeparator {
	...
	/// );
	/// ```
	EndOfText { is_soft_line_break: bool, is_next_element_whitespace_sensitive: bool },
}
...
impl WordSeparator {
	...
	fn will_break(&self) -> bool {
		matches!(
			self,
			WordSeparator::EndOfText { is_soft_line_break: false, is_next_element_whitespace_sensitive: _ }
		)
	}
}
...
impl Format<HtmlFormatContext> for WordSeparator {
	fn fmt(&self, f: &mut Formatter<HtmlFormatContext>) -> FormatResult<()> {
		match self {
			...
			WordSeparator::EndOfText { is_soft_line_break, is_next_element_whitespace_sensitive } => {
...
impl MultilineBuilder {
	fn new(layout: MultilineLayout, is_root: bool) -> Self {
		Self { layout, is_root, result: Ok(Vec::new()), prefix: Ok(Vec::new()) }
	}
...
fn format_partial_closing_tag(
	...
	let HtmlClosingElementFields { l_angle_token, name, slash_token, r_angle_token: _ } = closing_tag.as_fields();

	write!(f, [l_angle_token.format(), slash_token.format(), name.format(),])
}
