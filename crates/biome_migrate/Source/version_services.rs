use biome_analyze::{
	AddVisitor, FromServices, MissingServicesDiagnostic, Phase, Phases,
	QueryKey, Queryable, RuleKey, ServiceBag, SyntaxVisitor,
};
...
impl FromServices for VersionServices {
	fn from_services(
		...
		let version: &Arc<TheVersion> =
			services.get_service().ok_or_else(|| {
				MissingServicesDiagnostic::new(
					rule_key.rule_name(),
					&["TheVersion"],
				)
			})?;
	...
	N: AstNode<Language = JsonLanguage> + 'static,
{
	...
	fn build_visitor(
		analyzer: &mut impl AddVisitor<JsonLanguage>,
		_: &JsonRoot,
	) {
