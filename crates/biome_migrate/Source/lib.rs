...
use biome_analyze::{
	AnalysisFilter, Analyzer, AnalyzerContext, AnalyzerOptions, AnalyzerSignal, ApplySuppression, InspectMatcher,
	LanguageRoot, MatchQueryParams, MetadataRegistry, RuleAction, RuleRegistry, SuppressionAction,
};
	...
	B: 'a,
{
	...
	(
		analyzer.run(AnalyzerContext { root: root.clone(), range: filter.range, services, options: &options }),
		diagnostics,
	)
}
...
"#;
		...
		migrate_configuration(&parsed.tree(), AnalysisFilter::default(), Utf8Path::new(""), |signal| {
