...
use biome_analyze::{Ast, FixKind, Rule, RuleDiagnostic, context::RuleContext};
...
impl Rule for NurseryRules {
	...
	fn action(ctx:&RuleContext<Self>, state:&Self::State) -> Option<MigrationAction> {
		...
		// If the target group exists, then we just need to delete the rule from the
		// nursery group, and update the target group by adding a new member with the
		// name of rule we are migrating
		...
		// If we don't have a group, we have to create one. To avoid possible side effects with our
		// mutation logic we recreate the "rules" object by removing the
		// `rules.nursery.<nursery_rule_name>` member (hence we create a new list), and add a new
		// member `rules.<new_group_name>.<new_rule_name>`. This new group is added at the very end.
		else {
