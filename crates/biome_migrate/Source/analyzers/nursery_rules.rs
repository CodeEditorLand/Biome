...
use biome_analyze::{context::RuleContext, ActionCategory, Ast, FixKind, Rule, RuleDiagnostic};
...
impl MigrateRuleState {
	fn as_rule_name_range(&self) -> TextRange { self.nursery_rule.range() }
}
...
impl Rule for NurseryRules {
	...
	fn run(ctx:&RuleContext<Self>) -> Self::Signals {
		...
		if let Some(nursery_group) = find_group_by_name(node, "nursery") {
			...
			let Some(nursery_group_object) =
				nursery_group.value().ok().and_then(|node| node.as_json_object_value().cloned())
			else {
			...
			for nursery_rule in nursery_group_object.json_member_list().iter().flatten() {
				...
				let Ok(nursery_rule_name) =
					nursery_rule.name().and_then(|node| node.inner_string_text())
				else {
					continue;
				};
	...
	fn action(ctx:&RuleContext<Self>, state:&Self::State) -> Option<MigrationAction> {
		...
		// If the target group exists, then we just need to delete the rule from
		// the nursery group, and update the target group by adding a new
		// member with the name of rule we are migrating
		if let Some(target_group) = find_group_by_name(node, target_group_name) {
			...
			let mut current_rule_separators =
				target_group_value_object.json_member_list().separators().flatten();
			...
			// We only add the rule if the rule doesn't already exist in the
			// target group to avoid duplicate rules in the target group
			...
			// Remove the stale nursery rule and the corresponding comma
			// separator
		...
		// If we don't have a group, we have to create one. To avoid possible
		// side effects with our mutation logic we recreate the "rules" object
		// by removing the `rules.nursery.<nursery_rule_name>` member (hence we
		// create a new list), and add a new member
		// `rules.<new_group_name>.<new_rule_name>`. This new group is added at
		// the very end.
		else {
			let rules = nursery_group.syntax().ancestors().find_map(JsonMemberList::cast)?;
			...
			let new_member = json_member(
				...
				AnyJsonValue::JsonObjectValue(json_object_value(
					...
					token(T!['}']).with_leading_trivia_pieces(
						nursery_group.syntax().last_token()?.leading_trivia().pieces(),
					),
				)),
			)
			.with_leading_trivia_pieces(
				nursery_group.syntax().first_token()?.leading_trivia().pieces(),
			)?
			.with_trailing_trivia_pieces(
				nursery_group.syntax().last_token()?.trailing_trivia().pieces(),
			)?;
...
/// It creates a new [JsonMember] by using the old nursery group node, but with
/// the new name
fn make_new_rule_name_member(
	...
	Some(json_member(
		json_member_name(json_string_literal(rule_name))
			.with_leading_trivia_pieces(
				nursery_rule_member.syntax().first_token()?.leading_trivia().pieces(),
			)?
			.with_trailing_trivia_pieces(
				nursery_rule_member.syntax().last_token()?.trailing_trivia().pieces(),
			)?,
