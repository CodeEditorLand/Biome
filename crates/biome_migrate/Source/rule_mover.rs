use biome_json_factory::make::{
	json_member, json_member_list, json_member_name, json_object_value, json_string_literal, json_string_value, token,
};
use biome_json_syntax::{
	AnyJsonValue, JsonLanguage, JsonMember, JsonMemberList, JsonObjectValue, JsonRoot, JsonSyntaxToken, T,
};
...
impl AnalyzerMover {
	...
	pub(crate) fn from_root(root: JsonRoot) -> Self {
		...
		let object = root.value().ok().and_then(|n| n.as_json_object_value().cloned());
		...
		Self { root, groups, queries: vec![], assist_member, linter_member }
	}
	...
	fn track_groups(
		...
		let rules_list = linter_member
			...
			.flatten()
			.find_map(|member| {
				let name = member.name().ok().and_then(|n| n.inner_string_text().ok())?;
				if name.text() == parent_group_name { Some(member) } else { None }
			})
			.and_then(|member| Some(member.value().ok()?.as_json_object_value()?.json_member_list()))?;
	...
	#[expect(unused)]
	pub(crate) fn remove_rule(&mut self, rule_name: impl ToString, rule_member: JsonMember, group: &str) {
	...
	fn remove_rule_from_group(
		...
		if let Some(member) = groups.get_mut(group) {
			let list = member.value().ok()?.as_json_object_value()?.json_member_list();
	...
	pub(crate) fn run_queries(mut self) -> Option<BatchMutation<JsonLanguage>> {
		...
		for query in self.queries {
			let Query { rule_name, rule_member, kind } = query;
		...
		let list = self.root.value().ok()?.as_json_object_value()?.json_member_list();

		let mut separators = vec![];
		let mut top_level_members = vec![];

		// AKAK: linter: {}
		let linter_member = self.linter_member.unwrap_or(create_new_linter_member(vec![], vec![], 4));

		// AKAK: assist: {}
		let assist_memebr = self.assist_member.unwrap_or(create_new_assist_member(vec![], vec![]));

		let mut linter_member_added = false;
		let mut assist_member_added = false;

		for top_level_member in list.iter().flatten() {
			let name = top_level_member.name().ok().and_then(|n| n.inner_string_text().ok());
...
fn group_member(members: Vec<JsonMember>, separators: Vec<JsonSyntaxToken>, group_name: &str) -> JsonMember {
...
fn rules_member(members: Vec<JsonMember>, separators: Vec<JsonSyntaxToken>, white_space: usize) -> JsonMember {
	let list = json_member_list(members, separators);
...
fn linter_member(members: Vec<JsonMember>, separators: Vec<JsonSyntaxToken>, white_space: usize) -> JsonMember {
	let list = json_member_list(members, separators);
	let object = create_object(list, white_space);
	...
	if let Some(list) = list {
		for item in list.iter().flatten() {
			...
			if !matches!(name.as_deref(), Some("rules")) {
				...
				let rules_members = rules_member(members.clone(), separators.clone(), rules_member_indentation);
...
fn create_new_assist_member(members: Vec<JsonMember>, separators: Vec<JsonSyntaxToken>) -> JsonMember {
