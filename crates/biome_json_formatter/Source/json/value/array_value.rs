...
impl FormatNodeRule<JsonArrayValue> for FormatJsonArrayValue {
	fn fmt_fields(&self, node: &JsonArrayValue, f: &mut JsonFormatter) -> FormatResult<()> {
		let JsonArrayValueFields { l_brack_token, elements, r_brack_token } = node.as_fields();

		let should_expand =
			f.comments().has_dangling_comments(node.syntax()) || f.context().options().expand() == Expand::Always;
