	...
	fn fmt(&self, node: &JsonArrayElementList, f: &mut JsonFormatter) -> FormatResult<()> {
		...
		match layout {
			ArrayLayout::Fill => {
				...
				for (element, formatted) in node.iter().zip(node.format_separated(",", trailing_separator)) {
					filler.entry(
						&format_once(|f| {
			...
			ArrayLayout::OnePerLine => {
				...
				for (element, formatted) in node.elements().zip(node.format_separated(",", trailing_separator)) {
