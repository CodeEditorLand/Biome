		...
		match layout {
			ArrayLayout::Fill => {
				...
				for (element, formatted) in
					node.iter().zip(node.format_separated(",", trailing_separator))
				{
					filler.entry(
						&format_once(|f| {
...
enum ArrayLayout {
	...
	/// Prints every element on a single line if the array exceeds the line
...
/// Returns `true` if the array can be "fill-printed" instead of breaking each
...
pub(crate) fn can_concisely_print_array_list(list:&JsonArrayElementList) -> bool {
	...
	list.iter().all(|node| matches!(node, Ok(AnyJsonValue::JsonNumberValue(_))))
}
