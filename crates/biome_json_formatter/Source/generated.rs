...
use crate::{
	AsFormat, FormatBogusNodeRule, FormatNodeRule, IntoFormat,
	JsonFormatContext, JsonFormatter,
};
use biome_formatter::{
	...
	for crate::json::value::boolean_value::FormatJsonBooleanValue
{
	...
	fn fmt(
		...
		FormatNodeRule::<biome_json_syntax::JsonBooleanValue>::fmt(
			self, node, f,
		)
	}
}
	...
	for crate::json::auxiliary::member::FormatJsonMember
{
	...
	fn fmt(
		&self,
		node: &biome_json_syntax::JsonMember,
		f: &mut JsonFormatter,
	) -> FormatResult<()> {
...
impl FormatRule<biome_json_syntax::JsonRoot>
	for crate::json::auxiliary::root::FormatJsonRoot
{
	type Context = JsonFormatContext;
	#[inline(always)]
	fn fmt(
		&self,
		node: &biome_json_syntax::JsonRoot,
		f: &mut JsonFormatter,
	) -> FormatResult<()> {
...
impl FormatRule<biome_json_syntax::JsonBogus>
	for crate::json::bogus::bogus::FormatJsonBogus
{
	type Context = JsonFormatContext;
	#[inline(always)]
	fn fmt(
		&self,
		node: &biome_json_syntax::JsonBogus,
		f: &mut JsonFormatter,
	) -> FormatResult<()> {
	...
	for crate::json::bogus::bogus_value::FormatJsonBogusValue
{
	...
	fn fmt(
		...
		FormatBogusNodeRule::<biome_json_syntax::JsonBogusValue>::fmt(
			self, node, f,
		)
	}
}
