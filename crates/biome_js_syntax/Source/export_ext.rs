...
use crate::{
	...
	AnyJsExportNamedSpecifier, AnyJsExpression, AnyTsIdentifierBinding, AnyTsType, JsExport, JsExportNamedClause,
	JsIdentifierExpression, JsImportAssertion, JsLiteralExportName, JsReferenceIdentifier, JsSyntaxToken,
...
impl AnyIdentifier {
	pub fn name_token(&self) -> Option<JsSyntaxToken> {
		match self {
			Self::AnyJsBindingPattern(id) => id.as_any_js_binding()?.as_js_identifier_binding()?.name_token(),
				...
				AnyJsExportClause::AnyJsDeclarationClause(declaration_clause) => {
					match declaration_clause {
							...
							variable_declaration_clause.declaration().ok().map(|variable_declaration| {
								variable_declaration
									...
									.filter_map(|declarator| {
										...
										let initializer =
											declarator.initializer().and_then(|init| init.expression().ok());
										Some(ExportedItem {
											identifier: Some(AnyIdentifier::AnyJsBindingPattern(identifier)),
											exported: initializer.map(AnyJsExported::AnyJsExpression),
											is_default: false,
										})
									})
									.collect()
						...
						AnyJsDeclarationClause::TsTypeAliasDeclaration(ts_type_alias_declaration) => {
							ts_type_alias_declaration.binding_identifier().ok().map(|type_alias_id| {
								vec![ExportedItem {
									identifier: Some(AnyIdentifier::AnyTsIdentifierBinding(type_alias_id)),
				...
				AnyJsExportClause::JsExportNamedClause(named_clause) => Some(
					named_clause
						...
						.filter_map(|export_specifier| match export_specifier {
							AnyJsExportNamedSpecifier::JsExportNamedShorthandSpecifier(shorthand) => {
									...
									if exported_name.to_trimmed_string() == "default" {
										return ExportedItem {
											identifier: specifier
												.local_name()
												.ok()
												.map(|local_name| AnyIdentifier::JsReferenceIdentifier(local_name)),
											exported: None,
											is_default: true,
										};
									}
									ExportedItem {
										identifier: Some(AnyIdentifier::JsLiteralExportName(exported_name)),
										exported: None,
										is_default: false,
									}
								})
...
impl AnyJsExportNamedSpecifier {
	...
	pub fn with_type_token(self, type_token: Option<JsSyntaxToken>) -> Self {
		match self {
			Self::JsExportNamedShorthandSpecifier(specifier) => specifier.with_type_token(type_token).into(),
