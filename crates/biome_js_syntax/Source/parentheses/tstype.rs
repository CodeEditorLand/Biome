...
impl NeedsParentheses for TsConstructorType {
	#[inline]
	fn needs_parentheses(&self) -> bool { function_like_type_needs_parentheses(self.syntax()) }
}
...
impl NeedsParentheses for TsTypeofType {
	fn needs_parentheses(&self) -> bool {
		...
		match parent.kind() {
			...
			// Typeof operators are parenthesized when used as an object type in
			// an indexed access to avoid ambiguity of precedence, as it's
			// higher than the JS equivalent: ```typescript
			// const array = [1, 2, 3]
			...
			JsSyntaxKind::TS_INDEXED_ACCESS_TYPE => {
				...
				// The typeof operator only needs parens if it's the object of
				// the indexed access. If it's the index_type, then the
				// braces already act as the visual precedence.
...
fn function_like_type_needs_parentheses(node:&JsSyntaxNode) -> bool {
	...
	match parent.kind() {
		...
		JsSyntaxKind::TS_CONDITIONAL_TYPE => {
			let conditional = TsConditionalType::unwrap_cast(parent.clone());
			let is_check_type =
				conditional.check_type().is_ok_and(|check_type| check_type.syntax() == node);
			if is_check_type {
				return true;
			}
