...
impl NeedsParentheses for JsArrayExpression {
	#[inline]
	fn needs_parentheses(&self) -> bool { false }
}
...
impl NeedsParentheses for JsBinaryExpression {
	#[inline]
	fn needs_parentheses(&self) -> bool { binary_like_needs_parens(self.syntax()) }
}
		...
		parent.kind() == JsSyntaxKind::JS_NEW_EXPRESSION
			|| (parent.kind() == JsSyntaxKind::JS_EXPORT_DEFAULT_EXPRESSION_CLAUSE
				&& self.callee().map_or(true, |callee| {
					...
					// when the leftmost expression is not a class expression or
...
impl NeedsParentheses for JsClassExpression {
	fn needs_parentheses(&self) -> bool {
		...
		(parent.kind() == JsSyntaxKind::JS_EXTENDS_CLAUSE && !self.decorators().is_empty())
			...
			) || is_first_in_statement(self.syntax(), FirstInStatementMode::ExpressionOrExportDefault)
	}
}
...
fn is_in_for_initializer(expression:&JsInExpression) -> bool {
	let statement = expression.syntax().ancestors().skip(1).find_map(AnyJsStatement::cast);
	match statement {
		Some(AnyJsStatement::JsForInStatement(for_in_statement)) => {
			for_in_statement
				.initializer()
				.is_ok_and(|initializer| initializer.range().contains(expression.range().start()))
		},
		Some(AnyJsStatement::JsForStatement(for_statement)) => {
			for_statement
				.initializer()
...
impl NeedsParentheses for JsPostUpdateExpression {
	#[inline]
	fn needs_parentheses(&self) -> bool { unary_like_expression_needs_parens(self.syntax()) }
}
...
impl NeedsParentheses for JsStringLiteralExpression {
	fn needs_parentheses(&self) -> bool {
		if let Some(expression_statement) = self.parent::<JsExpressionStatement>() {
			expression_statement.syntax().parent().is_some_and(|grand_parent| {
				matches!(
					grand_parent.kind(),
...
impl NeedsParentheses for JsUnaryExpression {
	fn needs_parentheses(&self) -> bool {
		match self.parent::<AnyJsExpression>() {
			...
			// instanceof Bar)`, so format to `(!foo) instance Bar` to what
			// is really happening
			Some(AnyJsExpression::JsInstanceofExpression(_)) => true,
			// A user typing `!foo in bar` probably intended `!(foo instanceof
			// Bar)`, so format to `(!foo) in bar` to what is really
			// happening
...
impl NeedsParentheses for TsInstantiationExpression {
	#[inline]
	fn needs_parentheses(&self) -> bool {
		self.parent::<AnyJsMemberExpression>().is_some_and(|member_expr| {
			member_expr
				.object()
...
/// Implements the rules when a node needs parentheses that are common across
/// all [AnyJsBinaryLikeExpression] nodes.
...
fn member_chain_callee_needs_parens(node:AnyJsExpression) -> bool {
	let mut object_chain = std::iter::successors(Some(node), |expr| {
		match expr {
...
/// Implements the shared logic for when parentheses are necessary for
/// [JsPreUpdateExpression], [JsPostUpdateExpression], or [JsUnaryExpression]
/// expressions. Each expression may implement node specific rules, which is why
/// calling `needs_parens` on the node is preferred.
...
/// Returns `true` if an expression with lower precedence than an update
/// expression needs parentheses.
///
/// This is generally the case if the expression is used in a left hand side, or
/// primary expression context.
fn update_or_lower_expression_needs_parens(expression:&JsSyntaxNode, parent:JsSyntaxNode) -> bool {
	matches!(
		parent.kind(),
		JsSyntaxKind::JS_EXTENDS_CLAUSE
            | JsSyntaxKind::TS_NON_NULL_ASSERTION_EXPRESSION
...
pub(crate) enum FirstInStatementMode {
	/// Considers [JsExpressionStatement] and the body of
	/// [JsArrowFunctionExpression] as the first statement.
	ExpressionStatementOrArrow,
	/// Considers [JsExpressionStatement] and [JsExportDefaultExpressionClause]
	/// as the first statement.
	ExpressionOrExportDefault,
}

/// Returns `true` if this node is at the start of an expression (depends on the
/// passed `mode`).
///
/// Traverses upwards the tree for as long as the `node` is the left most
/// expression until the node isn't the left most node or reached a statement.
	...
	while let Some(parent) = current.parent() {
		let parent = match parent.kind() {
			...
			JsSyntaxKind::JS_SEQUENCE_EXPRESSION => {
				...
				let is_left = expr.left().is_ok_and(|left| left.syntax() == current.as_ref());
				if is_left {
					expr.into_syntax()
				} else {
					break;
			...
			JsSyntaxKind::JS_COMPUTED_MEMBER_EXPRESSION => {
				...
				let is_object =
					expr.object().is_ok_and(|object| object.syntax() == current.as_ref());
				if is_object {
					expr.into_syntax()
				} else {
					break;
			...
			JsSyntaxKind::JS_COMPUTED_MEMBER_ASSIGNMENT => {
				...
				let is_object =
					assignment.object().is_ok_and(|object| object.syntax() == current.as_ref());
				if is_object {
			...
			JsSyntaxKind::JS_ASSIGNMENT_EXPRESSION => {
				...
				let is_left = assignment.left().is_ok_and(|left| left.syntax() == current.as_ref());
				if is_left {
					assignment.into_syntax()
				} else {
					break;
			...
			JsSyntaxKind::JS_CONDITIONAL_EXPRESSION => {
				...
				let is_test = cond.test().is_ok_and(|test| test.syntax() == current.as_ref());
				if is_test {
					cond.into_syntax()
				} else {
					break;
				...
				if mode == FirstInStatementMode::ExpressionStatementOrArrow =>
			{
				...
				let is_body = arrow.body().is_ok_and(|body| {
					match body {
			...
			_ => {
				...
				let is_left = binary_like.left().is_ok_and(|left| {
					match left {
