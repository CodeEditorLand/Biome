...
use crate::{
	AnyJsExpression, AnyJsInProperty, JsArrowFunctionExpression, JsBinaryExpression, JsBinaryOperator,
	JsDoWhileStatement, JsIfStatement, JsInExpression, JsInstanceofExpression, JsLogicalExpression, JsLogicalOperator,
	JsPrivateName, JsSwitchStatement, JsSyntaxKind, JsSyntaxNode, JsSyntaxToken, JsWhileStatement, OperatorPrecedence,
};
...
impl AnyJsBinaryLikeExpression {
	pub fn left(&self) -> SyntaxResult<AnyJsBinaryLikeLeftExpression> {
		match self {
			Self::JsLogicalExpression(logical) => logical.left().map(AnyJsBinaryLikeLeftExpression::AnyJsExpression),
			Self::JsBinaryExpression(binary) => binary.left().map(AnyJsBinaryLikeLeftExpression::AnyJsExpression),
			Self::JsInstanceofExpression(instanceof) => {
				instanceof.left().map(AnyJsBinaryLikeLeftExpression::AnyJsExpression)
			},
			Self::JsInExpression(in_expression) => in_expression.property().map(AnyJsBinaryLikeLeftExpression::from),
		}
	}
	...
	pub fn operator(&self) -> SyntaxResult<BinaryLikeOperator> {
		match self {
			Self::JsLogicalExpression(logical) => logical.operator().map(BinaryLikeOperator::Logical),
	...
	pub fn is_inside_condition(&self, parent: Option<&JsSyntaxNode>) -> bool {
		parent.is_some_and(|parent| {
			let test = match parent.kind() {
				...
				JsSyntaxKind::JS_DO_WHILE_STATEMENT => JsDoWhileStatement::unwrap_cast(parent.clone()).test(),
				JsSyntaxKind::JS_WHILE_STATEMENT => JsWhileStatement::unwrap_cast(parent.clone()).test(),
				JsSyntaxKind::JS_SWITCH_STATEMENT => JsSwitchStatement::unwrap_cast(parent.clone()).discriminant(),
				_ => return false,
			};
	...
	pub fn should_inline_logical_expression(&self) -> bool {
		match self {
			AnyJsBinaryLikeExpression::JsLogicalExpression(logical) => logical.right().is_ok_and(|right| match right {
	...
	pub fn should_not_indent_if_parent_indents(self: &AnyJsBinaryLikeExpression, parent: Option<JsSyntaxNode>) -> bool {
		parent.is_some_and(|parent| match parent.kind() {
			...
			JsSyntaxKind::JS_ARROW_FUNCTION_EXPRESSION => JsArrowFunctionExpression::unwrap_cast(parent)
				.body()
				.is_ok_and(|body| body.syntax() == self.syntax()),
			JsSyntaxKind::JS_CONDITIONAL_EXPRESSION => parent.parent().is_some_and(|grand_parent| {
				!matches!(
					grand_parent.kind(),
...
pub(crate) fn should_flatten(parent_operator: BinaryLikeOperator, operator: BinaryLikeOperator) -> bool {
