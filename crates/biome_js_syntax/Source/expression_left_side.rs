				...
				let left_expression = match expression {
					...
					AnyJsExpression::JsPostUpdateExpression(expr) => {
						return expr.operand().ok().map(|assignment| {
							Self::from(AnyJsAssignmentPattern::AnyJsAssignment(
								assignment,
							))
						})
					},
			...
			Self::AnyJsAssignmentPattern(pattern) => {
				let left = match pattern {
					AnyJsAssignmentPattern::AnyJsAssignment(assignment) => {
						match assignment {
							AnyJsAssignment::JsComputedMemberAssignment(
								computed,
							) => return computed.object().ok().map(Self::from),
							AnyJsAssignment::JsStaticMemberAssignment(
								member,
							) => return member.object().ok().map(Self::from),
							AnyJsAssignment::TsAsAssignment(parent) => {
								parent.assignment().ok()
							},
							AnyJsAssignment::TsSatisfiesAssignment(parent) => {
								parent.assignment().ok()
							},
							AnyJsAssignment::TsNonNullAssertionAssignment(
								parent,
							) => parent.assignment().ok(),
							AnyJsAssignment::TsTypeAssertionAssignment(
								parent,
							) => parent.assignment().ok(),
