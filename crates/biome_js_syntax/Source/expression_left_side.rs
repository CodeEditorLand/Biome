				...
				let left_expression = match expression {
					...
					AnyJsExpression::JsPostUpdateExpression(expr) => {
						return expr
							.operand()
							.ok()
							.map(|assignment| Self::from(AnyJsAssignmentPattern::AnyJsAssignment(assignment)));
					},
					expr => {
						return AnyJsBinaryLikeExpression::cast_ref(expr.syntax()).and_then(|binary_like| {
							match binary_like.left().ok() {
								Some(AnyJsBinaryLikeLeftExpression::AnyJsExpression(expression)) => {
									Some(Self::from(expression))
								},
								Some(AnyJsBinaryLikeLeftExpression::JsPrivateName(name)) => Some(Self::from(name)),
								None => None,
							}
						});
					},
			...
			Self::AnyJsAssignmentPattern(pattern) => {
				let left = match pattern {
					AnyJsAssignmentPattern::AnyJsAssignment(assignment) => match assignment {
						...
						AnyJsAssignment::TsNonNullAssertionAssignment(parent) => parent.assignment().ok(),
				...
				left.map(|assignment| Self::from(AnyJsAssignmentPattern::AnyJsAssignment(assignment)))
			},
