...
impl AnyJsExpressionLeftSide {
	...
	///
	/// For example, returns `a` for `(a ? b : c) + d` because it first resolves
	/// the left hand expression of the binary expression, then resolves to the
	/// inner expression of the parenthesized expression, and finally resolves
	/// to the test condition of the conditional expression.
	...
	/// Returns the left side of an expression (an expression where the first
	/// child is a `Node` or [None] if the expression has no left side.
				...
				let left = match pattern {
					AnyJsAssignmentPattern::AnyJsAssignment(assignment) => {
						match assignment {
							...
							AnyJsAssignment::TsSatisfiesAssignment(parent) => {
