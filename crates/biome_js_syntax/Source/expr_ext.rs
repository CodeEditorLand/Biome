...
use crate::{
	AnyJsArrayElement, AnyJsArrowFunctionParameters, AnyJsCallArgument, AnyJsClassMemberName, AnyJsExpression,
	AnyJsFunctionBody, AnyJsLiteralExpression, AnyJsName, AnyJsObjectMemberName, AnyJsTemplateElement,
	AnyTsEnumMemberName, JsArrayExpression, JsArrayHole, JsAssignmentExpression, JsBinaryExpression,
	JsCallArgumentList, JsCallArguments, JsCallExpression, JsComputedMemberAssignment, JsComputedMemberExpression,
	JsConditionalExpression, JsDoWhileStatement, JsForStatement, JsIfStatement, JsLiteralMemberName,
	JsLogicalExpression, JsNewExpression, JsNumberLiteralExpression, JsObjectExpression, JsPostUpdateExpression,
	JsPreUpdateExpression, JsReferenceIdentifier, JsRegexLiteralExpression, JsStaticMemberExpression,
	JsStringLiteralExpression, JsSyntaxKind, JsSyntaxNode, JsSyntaxToken, JsTemplateChunkElement, JsTemplateExpression,
	JsUnaryExpression, JsWhileStatement, OperatorPrecedence, T, TsStringLiteralType, inner_string_text,
};
use biome_rowan::{
	AstNode, AstNodeList, AstSeparatedList, NodeOrToken, SyntaxNodeCast, SyntaxResult, TextRange, TextSize, TokenText,
	declare_node_union,
};
use core::iter;
...
impl JsBinaryOperator {
	pub const fn precedence(&self) -> OperatorPrecedence {
		match self {
			...
			JsBinaryOperator::LeftShift | JsBinaryOperator::RightShift | JsBinaryOperator::UnsignedRightShift => {
...
impl JsBinaryExpression {
	...
	pub fn is_optional_chain_like(&self) -> SyntaxResult<bool> {
		if matches!(
			...
			Ok(self.right()?.as_static_value().is_some_and(|x| x.is_null_or_undefined()))
		} else {
			Ok(false)
		}
	}
...
impl JsUnaryExpression {
	...
	pub fn is_signed_numeric_literal(&self) -> SyntaxResult<bool> {
		...
		let is_numeric_literal = matches!(
			argument,
			AnyJsExpression::AnyJsLiteralExpression(AnyJsLiteralExpression::JsNumberLiteralExpression(_))
		);
...
impl AnyJsExpression {
	...
	pub fn precedence(&self) -> SyntaxResult<OperatorPrecedence> {
		let precedence = match self {
			...
			AnyJsExpression::JsPostUpdateExpression(_) | AnyJsExpression::JsPreUpdateExpression(_) => {
			...
			AnyJsExpression::JsBogusExpression(_) | AnyJsExpression::JsMetavariable(_) => OperatorPrecedence::lowest(),
		...
		match self {
			...
			AnyJsExpression::JsTemplateExpression(template) => {
				...
				if element_list.len() == 0 {
					let range = template.l_tick_token().ok()?.text_trimmed_range().add_start(1.into());
	...
	pub fn is_literal_expression(&self) -> bool {
		match self {
			...
			AnyJsExpression::JsUnaryExpression(unary_expression) => {
				let is_minus_operator = matches!(unary_expression.operator(), Ok(JsUnaryOperator::Minus));
			...
			// Parenthesized expression: (1)
			AnyJsExpression::JsParenthesizedExpression(parenthesized_expression) => parenthesized_expression
				.expression()
...
impl AnyJsLiteralExpression {
	pub fn value_token(&self) -> SyntaxResult<JsSyntaxToken> {
		match self {
			AnyJsLiteralExpression::JsBigintLiteralExpression(expression) => expression.value_token(),
			AnyJsLiteralExpression::JsBooleanLiteralExpression(expression) => expression.value_token(),
			AnyJsLiteralExpression::JsNullLiteralExpression(expression) => expression.value_token(),
			AnyJsLiteralExpression::JsNumberLiteralExpression(expression) => expression.value_token(),
			AnyJsLiteralExpression::JsRegexLiteralExpression(expression) => expression.value_token(),
			AnyJsLiteralExpression::JsStringLiteralExpression(expression) => expression.value_token(),
		}
	}
	...
	pub fn as_static_value(&self) -> Option<StaticValue> {
		match self {
			...
			AnyJsLiteralExpression::JsNullLiteralExpression(null) => Some(StaticValue::Null(null.value_token().ok()?)),
...
impl AnyJsComputedMember {
	...
	pub fn l_brack_token(&self) -> SyntaxResult<JsSyntaxToken> {
		match self {
			AnyJsComputedMember::JsComputedMemberExpression(expression) => expression.l_brack_token(),
			AnyJsComputedMember::JsComputedMemberAssignment(assignment) => assignment.l_brack_token(),
		}
	}

	pub fn optional_chain_token(&self) -> Option<JsSyntaxToken> {
		match self {
			AnyJsComputedMember::JsComputedMemberExpression(expression) => expression.optional_chain_token(),
	...
	pub fn r_brack_token(&self) -> SyntaxResult<JsSyntaxToken> {
		match self {
			AnyJsComputedMember::JsComputedMemberExpression(expression) => expression.r_brack_token(),
			AnyJsComputedMember::JsComputedMemberAssignment(assignment) => assignment.r_brack_token(),
		}
	}
}
...
impl AnyJsOptionalChainExpression {
	...
	pub fn optional_chain_token(&self) -> Option<JsSyntaxToken> {
		match self {
			Self::JsStaticMemberExpression(expr) => expr.operator_token().ok().filter(|x| x.kind() == T![?.]),
	...
	pub fn is_optional_chain(&self) -> bool {
		...
		while let Some(member_expr) = current_expression.ok().and_then(|expr| Self::cast(expr.into_syntax())) {
...
impl From<AnyJsOptionalChainExpression> for AnyJsExpression {
	fn from(node: AnyJsOptionalChainExpression) -> AnyJsExpression {
		match node {
			...
			AnyJsOptionalChainExpression::JsComputedMemberExpression(expression) => expression.into(),
...
impl JsCallExpression {
	...
	pub fn has_callee(&self, name: &str) -> bool {
		self.callee()
			.is_ok_and(|it| it.as_js_reference_identifier().is_some_and(|it| it.has_name(name)))
	}
		...
		match (args.next(), args.next(), args.next()) {
			(Some(Ok(argument)), None, None) if arguments.args().len() == 1 => {
						...
						.is_some_and(|parent| parent.is_test_call_expression().unwrap_or(false))
				{
					return Ok(matches!(
						argument,
						AnyJsCallArgument::AnyJsExpression(JsArrowFunctionExpression(_) | JsFunctionExpression(_))
					));
				}

				if is_unit_test_set_up_callee(&callee) {
					return Ok(argument.as_any_js_expression().is_some_and(is_angular_test_wrapper));
				}
				...
				if arguments.args().len() <= 3 && callee.contains_a_test_pattern()? =>
			{
				...
				if second.as_any_js_expression().is_some_and(is_angular_test_wrapper) {
					return Ok(true);
				}
	...
	match expression {
		JsCallExpression(call_expression) => match call_expression.callee() {
			Ok(JsIdentifierExpression(identifier)) => identifier
				...
				.is_ok_and(|name| matches!(name.text_trimmed(), "async" | "inject" | "fakeAsync" | "waitForAsync")),
			_ => false,
		},
		_ => false,
	}
...
fn is_unit_test_set_up_callee(callee: &AnyJsExpression) -> bool {
	match callee {
		AnyJsExpression::JsIdentifierExpression(identifier) => identifier
			.name()
			.and_then(|name| name.value_token())
			.is_ok_and(|name| matches!(name.text_trimmed(), "beforeEach" | "beforeAll" | "afterEach" | "afterAll")),
		_ => false,
	}
}
...
pub fn is_in_boolean_context(node: &JsSyntaxNode) -> Option<bool> {
	...
	match parent.kind() {
		JsSyntaxKind::JS_IF_STATEMENT => Some(parent.cast::<JsIfStatement>()?.test().ok()?.syntax() == node),
		JsSyntaxKind::JS_DO_WHILE_STATEMENT => Some(parent.cast::<JsDoWhileStatement>()?.test().ok()?.syntax() == node),
		JsSyntaxKind::JS_WHILE_STATEMENT => Some(parent.cast::<JsWhileStatement>()?.test().ok()?.syntax() == node),
		JsSyntaxKind::JS_FOR_STATEMENT => Some(parent.cast::<JsForStatement>()?.test()?.syntax() == node),
...
mod test {
	...
	fn matches_test_call_expression() {
		...
		let call_expression = extract_call_expression("test.only(name = name || 'test', () => {});");
		assert_eq!(call_expression.is_test_call_expression(), Ok(true));
		...
		let call_expression = extract_call_expression("describe.only(name = name || 'test', () => {});");
		assert_eq!(call_expression.is_test_call_expression(), Ok(true));
	}
