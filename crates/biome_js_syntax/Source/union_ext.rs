use crate::{
	AnyJsArrowFunctionParameters, AnyJsBinding, AnyJsClass, AnyJsClassMember,
	AnyJsClassMemberName, AnyJsFunction, AnyJsFunctionBody,
	AnyTsPropertyAnnotation, AnyTsVariableAnnotation, JsClassMemberList,
	JsDecoratorList, JsExtendsClause, JsInitializerClause, JsSyntaxToken,
	JsVariableDeclarator, TsImplementsClause, TsReturnTypeAnnotation,
...
impl AnyJsClass {
	pub fn decorators(&self) -> JsDecoratorList {
		match self {
			...
			AnyJsClass::JsClassExpression(expression) => {
	...
	pub fn abstract_token(&self) -> Option<JsSyntaxToken> {
		match self {
			AnyJsClass::JsClassDeclaration(declaration) => {
				declaration.abstract_token()
			},
			AnyJsClass::JsClassExpression(_) => None,
			AnyJsClass::JsClassExportDefaultDeclaration(clause) => {
				clause.abstract_token()
			},
		}
	}
	...
	pub fn l_curly_token(&self) -> SyntaxResult<JsSyntaxToken> {
		match self {
			AnyJsClass::JsClassDeclaration(declaration) => {
				declaration.l_curly_token()
			},
			AnyJsClass::JsClassExpression(expression) => {
				expression.l_curly_token()
			},
			AnyJsClass::JsClassExportDefaultDeclaration(declaration) => {
				declaration.l_curly_token()
			},
	...
	pub fn r_curly_token(&self) -> SyntaxResult<JsSyntaxToken> {
		match self {
			AnyJsClass::JsClassDeclaration(declaration) => {
				declaration.r_curly_token()
			},
			AnyJsClass::JsClassExpression(expression) => {
				expression.r_curly_token()
			},
			AnyJsClass::JsClassExportDefaultDeclaration(declaration) => {
				declaration.r_curly_token()
			},
