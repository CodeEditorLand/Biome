use crate::{
	AnyJsBindingPattern, AnyJsConstructorParameter, AnyJsFormalParameter, AnyJsParameter, JsConstructorParameterList,
	JsConstructorParameters, JsDecoratorList, JsLanguage, JsParameterList, JsParameters, TsTypeAnnotation,
};
use biome_rowan::{AstNodeList, AstSeparatedList, AstSeparatedListNodesIterator, SyntaxResult, declare_node_union};
...
impl AnyJsParameterList {
	...
	pub fn first(&self) -> Option<SyntaxResult<AnyParameter>> {
		Some(match self {
			AnyJsParameterList::JsParameterList(parameters) => parameters.first()?.map(|parameter| parameter.into()),
	...
	pub fn iter(&self) -> AnyJsParameterListNodeIter {
		match self {
			AnyJsParameterList::JsParameterList(list) => AnyJsParameterListNodeIter::JsParameterList(list.iter()),
			AnyJsParameterList::JsConstructorParameterList(list) => {
	...
	pub fn last(&self) -> Option<SyntaxResult<AnyParameter>> {
		Some(match self {
			AnyJsParameterList::JsParameterList(parameters) => parameters.last()?.map(|parameter| parameter.into()),
			AnyJsParameterList::JsConstructorParameterList(parameters) => {
...
pub enum AnyJsParameterListNodeIter {
	...
	JsConstructorParameterList(AstSeparatedListNodesIterator<JsLanguage, AnyJsConstructorParameter>),
}
...
impl Iterator for AnyJsParameterListNodeIter {
	...
	fn next(&mut self) -> Option<Self::Item> {
		Some(match self {
			AnyJsParameterListNodeIter::JsParameterList(inner) => inner.next()?.map(AnyParameter::from),
			AnyJsParameterListNodeIter::JsConstructorParameterList(inner) => inner.next()?.map(AnyParameter::from),
		})
	}
}
	...
	pub fn binding(&self) -> Option<AnyJsBindingPattern> {
		match self {
			...
			AnyParameter::AnyJsParameter(parameter) => match parameter {
				AnyJsParameter::AnyJsFormalParameter(parameter) => parameter.as_js_formal_parameter()?.binding().ok(),
...
impl AnyJsConstructorParameter {
	...
	pub fn decorators(&self) -> Option<JsDecoratorList> {
		match self {
			...
			AnyJsConstructorParameter::TsPropertyParameter(parameter) => Some(parameter.decorators()),
		}
	}

	/// Returns `true` if any parameter in the given list is decorated.
	pub fn has_any_decorator(&self) -> bool {
		self.decorators().is_some_and(|decorators| !decorators.is_empty())
	}

	/// Returns the type annotation of the parameter if any.
	pub fn type_annotation(&self) -> Option<TsTypeAnnotation> {
		match self {
			AnyJsConstructorParameter::AnyJsFormalParameter(parameter) => parameter.type_annotation(),
