...
use crate::{JsParenthesizedAssignment, JsParenthesizedExpression, JsSyntaxNode, JsSyntaxToken, TsParenthesizedType};
...
impl AnyJsParenthesized {
	...
	pub fn inner(&self) -> SyntaxResult<JsSyntaxNode> {
		match self {
			Self::JsParenthesizedExpression(expression) => expression.expression().map(AstNode::into_syntax),
			Self::JsParenthesizedAssignment(assignment) => assignment.assignment().map(AstNode::into_syntax),
