...
use crate::{
	AnyJsxAttribute, AnyJsxAttributeName, AnyJsxAttributeValue, AnyJsxChild, AnyJsxElementName, AnyJsxTag,
	JsSyntaxToken, JsxAttribute, JsxAttributeList, JsxElement, JsxOpeningElement, JsxSelfClosingElement, JsxString,
	...
	pub fn find_by_names<const N: usize>(&self, names_to_lookup: [&str; N]) -> [Option<JsxAttribute>; N] {
		...
		for att in self {
			if let AnyJsxAttribute::JsxAttribute(attribute) = att {
				if let Some(name) = attribute.name().ok().and_then(|x| x.as_jsx_name()?.value_token().ok()) {
...
impl AnyJsxElement {
	...
	pub fn has_trailing_spread_prop(&self, current_attribute: &JsxAttribute) -> bool {
		match self {
			AnyJsxElement::JsxSelfClosingElement(element) => element.has_trailing_spread_prop(current_attribute),
			AnyJsxElement::JsxOpeningElement(element) => element.has_trailing_spread_prop(current_attribute),
		}
	}

	pub fn find_attribute_by_name(&self, name_to_lookup: &str) -> Option<JsxAttribute> {
		match self {
			AnyJsxElement::JsxSelfClosingElement(element) => element.find_attribute_by_name(name_to_lookup),
			AnyJsxElement::JsxOpeningElement(element) => element.find_attribute_by_name(name_to_lookup),
		}
	}
	...
	pub fn has_truthy_attribute(&self, name_to_lookup: &str) -> bool {
		self.find_attribute_by_name(name_to_lookup).is_some_and(|attribute| {
			attribute
				.as_static_value()
...
impl JsxAttribute {
	pub fn is_value_null_or_undefined(&self) -> bool {
		self.as_static_value().is_some_and(|it| it.is_null_or_undefined())
	}
...
impl AnyJsxAttributeValue {
	...
	pub fn as_static_value(&self) -> Option<StaticValue> {
		match self {
			...
			AnyJsxAttributeValue::JsxString(string) => Some(StaticValue::String(string.value_token().ok()?)),
		}
	}
}
	...
	pub fn is_accessible_node(&self) -> Option<bool> {
		Some(match self {
			...
			AnyJsxChild::JsxExpressionChild(expression) => {
				...
				expression.as_static_value().is_none_or(|value| !value.is_falsy())
			},
			AnyJsxChild::JsxElement(element) => {
				...
				jsx_element.is_custom_component() || !jsx_element.has_truthy_attribute("aria-hidden")
			},
