...
impl JsxAttributeList {
	...
	///
	/// Supports a maximum of 16 names to avoid stack overflow. Each attribute
	/// will consume:
	///
...
impl JsxAttribute {
	pub fn is_value_null_or_undefined(&self) -> bool {
		self.as_static_value().map_or(false, |it| it.is_null_or_undefined())
	}
	...
	pub fn is_accessible_node(&self) -> Option<bool> {
		Some(match self {
			...
			AnyJsxChild::JsxElement(element) => {
				...
				// We don't check if a component (e.g. <Text aria-hidden />) is using the
				// `aria-hidden` property, since we don't have enough information about how
