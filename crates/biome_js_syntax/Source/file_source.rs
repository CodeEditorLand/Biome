...
impl LanguageVersion {
	...
	pub const fn latest() -> Self { LanguageVersion::ES2022 }
}
...
impl ModuleKind {
	pub const fn is_script(&self) -> bool { matches!(self, ModuleKind::Script) }

	pub const fn is_module(&self) -> bool { matches!(self, ModuleKind::Module) }
}
...
pub enum LanguageVariant {
	...
	/// Standard JavaScript or TypeScript syntax with some restrictions for
	/// future-proof compatibility with JSX
	StandardRestricted,
...
impl LanguageVariant {
	pub const fn is_standard(&self) -> bool { matches!(self, LanguageVariant::Standard) }

	pub const fn is_standard_restricted(&self) -> bool {
		matches!(self, LanguageVariant::StandardRestricted)
	}

	pub const fn is_jsx(&self) -> bool { matches!(self, LanguageVariant::Jsx) }
}
...
impl Language {
	pub const fn is_javascript(&self) -> bool { matches!(self, Language::JavaScript) }

	pub const fn is_typescript(&self) -> bool { matches!(self, Language::TypeScript { .. }) }

	pub const fn is_definition_file(&self) -> bool {
		matches!(self, Language::TypeScript { definition_file:true })
	}
}
#[cfg_attr(feature = "schema", derive(schemars::JsonSchema))]
#[derive(
...
impl EmbeddingKind {
	pub const fn is_astro(&self) -> bool { matches!(self, EmbeddingKind::Astro) }

	pub const fn is_vue(&self) -> bool { matches!(self, EmbeddingKind::Vue) }

	pub const fn is_svelte(&self) -> bool { matches!(self, EmbeddingKind::Svelte) }
}
...
pub struct JsFileSource {
	...
	/// Used to mark if the source is being used for an Astro, Svelte or Vue
	/// file
...
impl JsFileSource {
	...
	pub fn js_module() -> Self { Self::default() }

	/// language: JS, variant: Standard, module_kind: Script, version: Latest
	pub fn js_script() -> Self { Self::default().with_module_kind(ModuleKind::Script) }

	/// language: JS, variant: JSX, module_kind: Module, version: Latest
	pub fn jsx() -> Self { Self::js_module().with_variant(LanguageVariant::Jsx) }

	/// language: TS, variant: Standard, module_kind: Module, version: Latest
	pub fn ts() -> Self {
		Self { language:Language::TypeScript { definition_file:false }, ..Self::default() }
	}

	/// language: TS, variant: StandardRestricted, module_kind: Module, version:
	/// Latest
	pub fn ts_restricted() -> Self { Self::ts().with_variant(LanguageVariant::StandardRestricted) }

	/// language: TS, variant: JSX, module_kind: Module, version: Latest
	pub fn tsx() -> Self { Self::ts().with_variant(LanguageVariant::Jsx) }

	/// TypeScript definition file
	/// language: TS, ambient, variant: Standard, module_kind: Module, version:
	/// Latest
	pub fn d_ts() -> Self {
		Self { language:Language::TypeScript { definition_file:true }, ..Self::default() }
	}

	/// Astro file definition
	pub fn astro() -> Self { Self::ts().with_embedding_kind(EmbeddingKind::Astro) }

	/// Vue file definition
	pub fn vue() -> Self { Self::js_module().with_embedding_kind(EmbeddingKind::Vue) }

	/// Svelte file definition
	pub fn svelte() -> Self { Self::js_module().with_embedding_kind(EmbeddingKind::Svelte) }
	...
	pub fn set_module_kind(&mut self, kind:ModuleKind) { self.module_kind = kind; }
	...
	pub const fn variant(&self) -> LanguageVariant { self.variant }

	pub const fn version(&self) -> LanguageVersion { self.version }

	pub const fn module_kind(&self) -> ModuleKind { self.module_kind }

	pub const fn is_module(&self) -> bool { self.module_kind.is_module() }

	pub const fn is_script(&self) -> bool { self.module_kind.is_script() }

	pub const fn is_typescript(&self) -> bool { self.language.is_typescript() }

	pub const fn is_jsx(&self) -> bool { self.variant.is_jsx() }

	pub const fn as_embedding_kind(&self) -> &EmbeddingKind { &self.embedding_kind }
	...
	/// Try to return the JS file source corresponding to this file name from
	/// well-known files
	...
	pub fn try_from_language_id(language_id:&str) -> Result<Self, FileSourceError> {
		match language_id {
			...
			// because `.js` files will be associated with the javascript
			// language id and we already use Self::jsx() for `.js` files in
			// try_from_extension(). This will make LSP and CLI work
			// consistently.
			//
			// This also fixes the issue where some plugins like babel plugin
			// will associate `.jsx` files with the javascript language id.
			//
			// TODO: This should be considered as an ad hoc fix.
			// We might want to reconsider the design and variants of the file
			// source.
...
impl TryFrom<&Path> for JsFileSource {
	...
	fn try_from(path:&Path) -> Result<Self, Self::Error> {
		...
		// DocumentFileSource::from_path_optional and we may support more and
		// more extensions with more than one dots.
