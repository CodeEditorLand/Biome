...
impl JsFileSource {
	...
	pub fn ts() -> Self {
		Self {
			language: Language::TypeScript { definition_file: false },
			..Self::default()
		}
	}
	...
	pub fn d_ts() -> Self {
		Self {
			language: Language::TypeScript { definition_file: true },
			..Self::default()
		}
	}
...
impl TryFrom<&Path> for JsFileSource {
	...
	fn try_from(path: &Path) -> Result<Self, Self::Error> {
		...
		let extension = &match filename {
			Some(filename) if filename.ends_with(b".d.ts") => {
				Cow::Borrowed("d.ts".as_ref())
			},
			Some(filename) if filename.ends_with(b".d.mts") => {
				Cow::Borrowed("d.mts".as_ref())
			},
			Some(filename) if filename.ends_with(b".d.cts") => {
