	...
	pub fn is_type_only(&self) -> bool {
		match self {
			Self::JsIdentifierBinding(binding) => {
				...
				if let Some(clause) =
					binding.syntax().grand_parent().and_then(AnyJsImportClause::cast)
				{
...
impl JsIdentifierBinding {
	/// Navigate upward until the declaration of this binding bypassing all
	/// nodes related to pattern binding.
	pub fn declaration(&self) -> Option<AnyJsBindingDeclaration> {
		declaration(self.syntax.parent()?)
	}
