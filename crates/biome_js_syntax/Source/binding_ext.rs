use crate::{
	...
	JsArrayBindingPatternRestElement, JsArrowFunctionExpression,
	JsBogusNamedImportSpecifier, JsBogusParameter, JsCatchDeclaration,
	JsClassDeclaration, JsClassExportDefaultDeclaration, JsClassExpression,
	JsConstructorClassMember, JsConstructorParameterList,
	JsConstructorParameters, JsDefaultImportSpecifier, JsExport,
	JsFormalParameter, JsFunctionDeclaration,
	JsFunctionExportDefaultDeclaration, JsFunctionExpression,
	JsIdentifierBinding, JsMethodClassMember, JsMethodObjectMember,
	JsNamedImportSpecifier, JsNamespaceImportSpecifier,
	JsObjectBindingPatternProperty, JsObjectBindingPatternRest,
	JsObjectBindingPatternShorthandProperty, JsParameterList, JsParameters,
	JsRestParameter, JsSetterClassMember, JsSetterObjectMember,
	JsShorthandNamedImportSpecifier, JsSyntaxKind, JsSyntaxNode, JsSyntaxToken,
	JsVariableDeclarator, TsCallSignatureTypeMember,
	TsConstructSignatureTypeMember, TsConstructorSignatureClassMember,
	TsConstructorType, TsDeclareFunctionDeclaration,
	TsDeclareFunctionExportDefaultDeclaration, TsEnumDeclaration, TsEnumMember,
	TsFunctionType, TsIdentifierBinding, TsImportEqualsDeclaration,
	TsIndexSignatureClassMember, TsIndexSignatureParameter, TsInferType,
	TsInterfaceDeclaration, TsLiteralEnumMemberName, TsMappedType,
	TsMethodSignatureClassMember, TsMethodSignatureTypeMember,
	TsModuleDeclaration, TsPropertyParameter, TsSetterSignatureClassMember,
	TsSetterSignatureTypeMember, TsTypeAliasDeclaration, TsTypeParameter,
	TsTypeParameterName,
};
...
impl AnyJsBindingDeclaration {
	...
	const fn can_merge(
		a: &AnyJsBindingDeclaration,
		b: &AnyJsBindingDeclaration,
	) -> bool {
		match (a, b) {
            (
	...
	pub fn parent_binding_pattern_declaration(
		&self,
	...
	match AnyJsBindingDeclaration::cast(node)? {
		AnyJsBindingDeclaration::JsFormalParameter(parameter) => {
			match parameter.parent::<TsPropertyParameter>() {
				...
				None => {
					Some(AnyJsBindingDeclaration::JsFormalParameter(parameter))
				},
			}
		},
...
fn parent_binding_pattern_declaration(
	node: &JsSyntaxNode,
) -> Option<AnyJsBindingDeclaration> {
...
fn is_under_array_pattern_binding(node: &JsSyntaxNode) -> Option<bool> {
	...
	match parent.kind() {
		...
		JS_ARRAY_BINDING_PATTERN_ELEMENT => {
			is_under_array_pattern_binding(&parent)
		},
		_ => Some(false),
	}
}
...
fn is_under_object_pattern_binding(node: &JsSyntaxNode) -> Option<bool> {
	...
	match parent.kind() {
		...
		JS_ARRAY_BINDING_PATTERN_ELEMENT => {
			is_under_object_pattern_binding(&parent)
		},
		_ => Some(false),
	}
}
		...
		match self {
			Self::JsIdentifierBinding(binding) => {
				if let Some(specifier) =
					binding.parent::<AnyJsNamedImportSpecifier>()
				{
	...
	pub fn with_name_token(self, name_token: JsSyntaxToken) -> Self {
		match self {
			...
			Self::TsLiteralEnumMemberName(binding) => {
				Self::TsLiteralEnumMemberName(
					binding.with_value_token(name_token),
				)
			},
		}
	}
...
fn parent_function(
	node: &JsSyntaxNode,
) -> Option<AnyJsParameterParentFunction> {
	let parent = node.parent()?;

	match parent.kind() {
		JsSyntaxKind::JS_PARAMETER_LIST => {
			// SAFETY: kind check above
			let parameters = JsParameterList::unwrap_cast(parent)
				.parent::<JsParameters>()?;
