use crate::{
	AnyJsImportClause, AnyJsNamedImportSpecifier, JsArrayBindingPatternElement, JsArrayBindingPatternRestElement,
	JsArrowFunctionExpression, JsBogusNamedImportSpecifier, JsBogusParameter, JsCatchDeclaration, JsClassDeclaration,
	JsClassExportDefaultDeclaration, JsClassExpression, JsConstructorClassMember, JsConstructorParameterList,
	JsConstructorParameters, JsDefaultImportSpecifier, JsExport, JsFormalParameter, JsFunctionDeclaration,
	JsFunctionExportDefaultDeclaration, JsFunctionExpression, JsIdentifierBinding, JsMethodClassMember,
	JsMethodObjectMember, JsNamedImportSpecifier, JsNamespaceImportSpecifier, JsObjectBindingPatternProperty,
	JsObjectBindingPatternRest, JsObjectBindingPatternShorthandProperty, JsParameterList, JsParameters,
	JsRestParameter, JsSetterClassMember, JsSetterObjectMember, JsShorthandNamedImportSpecifier, JsSyntaxKind,
	JsSyntaxNode, JsSyntaxToken, JsVariableDeclarator, TsCallSignatureTypeMember, TsConstructSignatureTypeMember,
	TsConstructorSignatureClassMember, TsConstructorType, TsDeclareFunctionDeclaration,
	TsDeclareFunctionExportDefaultDeclaration, TsEnumDeclaration, TsEnumMember, TsFunctionType, TsIdentifierBinding,
	...
	TsMethodSignatureTypeMember, TsModuleDeclaration, TsPropertyParameter, TsSetterSignatureClassMember,
	TsSetterSignatureTypeMember, TsTypeAliasDeclaration, TsTypeParameter, TsTypeParameterName,
};
...
impl AnyJsBindingDeclaration {
	...
	const fn can_merge(a: &AnyJsBindingDeclaration, b: &AnyJsBindingDeclaration) -> bool {
		match (a, b) {
			...
			(AnyJsBindingDeclaration::TsEnumDeclaration(_), AnyJsBindingDeclaration::TsEnumDeclaration(_)) => true,
			(
	...
	pub fn export(&self) -> Option<JsExport> {
		let maybe_export = match self {
			...
			| AnyJsBindingDeclaration::JsObjectBindingPatternRest(_)
			| AnyJsBindingDeclaration::JsObjectBindingPatternShorthandProperty(_) => {
				return parent_binding_pattern_declaration(self.syntax()).and_then(|decl| decl.export());
			},
...
fn declaration(node: JsSyntaxNode) -> Option<AnyJsBindingDeclaration> {
	match AnyJsBindingDeclaration::cast(node)? {
		AnyJsBindingDeclaration::JsFormalParameter(parameter) => match parameter.parent::<TsPropertyParameter>() {
...
fn is_under_array_pattern_binding(node: &JsSyntaxNode) -> Option<bool> {
	...
	match parent.kind() {
		JS_ARRAY_BINDING_PATTERN | JS_ARRAY_BINDING_PATTERN_ELEMENT_LIST | JS_ARRAY_BINDING_PATTERN_REST_ELEMENT => {
			Some(true)
		},
...
impl AnyJsIdentifierBinding {
	...
	pub fn is_type_only(&self) -> bool {
		match self {
			Self::JsIdentifierBinding(binding) => {
				...
				if let Some(clause) = binding.syntax().grand_parent().and_then(AnyJsImportClause::cast) {
	...
	pub fn with_name_token(self, name_token: JsSyntaxToken) -> Self {
		match self {
			Self::JsIdentifierBinding(binding) => Self::JsIdentifierBinding(binding.with_name_token(name_token)),
			Self::TsIdentifierBinding(binding) => Self::TsIdentifierBinding(binding.with_name_token(name_token)),
			Self::TsTypeParameterName(binding) => Self::TsTypeParameterName(binding.with_ident_token(name_token)),
...
fn parent_function(node: &JsSyntaxNode) -> Option<AnyJsParameterParentFunction> {
	...
	match parent.kind() {
		...
		JsSyntaxKind::JS_CONSTRUCTOR_PARAMETER_LIST => {
			// SAFETY: kind check above
			let parameters = JsConstructorParameterList::unwrap_cast(parent).parent::<JsConstructorParameters>()?;
			let parent = parameters.syntax().parent()?;
			AnyJsParameterParentFunction::cast(parent)
		},
