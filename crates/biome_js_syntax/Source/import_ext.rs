use crate::{
	AnyJsBinding, AnyJsCombinedSpecifier, AnyJsImportClause, AnyJsModuleSource, AnyJsNamedImportSpecifier,
	JsCallExpression, JsDefaultImportSpecifier, JsImport, JsImportAssertion, JsImportCallExpression, JsModuleSource,
	JsNamedImportSpecifier, JsNamedImportSpecifiers, JsNamespaceImportSpecifier, JsShorthandNamedImportSpecifier,
	JsSyntaxKind, JsSyntaxToken, inner_string_text,
};
use biome_rowan::{AstNode, SyntaxError, SyntaxNodeOptionExt, SyntaxResult, TokenText, declare_node_union};
...
impl AnyJsImportClause {
	...
	pub fn named_specifiers(&self) -> Option<JsNamedImportSpecifiers> {
		match self {
			...
			Self::JsImportCombinedClause(clause) => {
				if let Ok(AnyJsCombinedSpecifier::JsNamedImportSpecifiers(named_specifiers)) = clause.specifier() {
	...
	pub fn with_named_specifiers(self, named_specifiers: JsNamedImportSpecifiers) -> Self {
		match self {
			...
			Self::JsImportCombinedClause(clause) => {
				if matches!(clause.specifier(), Ok(AnyJsCombinedSpecifier::JsNamedImportSpecifiers(_))) {
			...
			Self::JsImportNamedClause(clause) => clause.with_named_specifiers(named_specifiers).into(),
...
impl AnyJsNamedImportSpecifier {
	...
	pub fn imported_name(&self) -> Option<JsSyntaxToken> {
		match self {
			specifier @ (Self::JsNamedImportSpecifier(_) | Self::JsShorthandNamedImportSpecifier(_)) => {
	...
	pub fn with_type_token(self, type_token: Option<JsSyntaxToken>) -> AnyJsNamedImportSpecifier {
		match self {
			...
			Self::JsShorthandNamedImportSpecifier(specifier) => specifier.with_type_token(type_token).into(),
		}
	}
}
		...
		match self {
			...
			AnyJsImportLike::JsCallExpression(expression) => {
				...
				if name.text_trimmed() == "require" {
					let [Some(argument)] = expression.arguments().ok()?.get_arguments_by_index([0]) else {
						return None;
					};
					argument
			...
			AnyJsImportLike::JsImportCallExpression(import_call) => {
				let [Some(argument)] = import_call.arguments().ok()?.get_arguments_by_index([0]) else {
					return None;
				};
				argument
					.as_any_js_expression()?
