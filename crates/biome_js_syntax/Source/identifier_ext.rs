use crate::{
	AnyJsExportNamedSpecifier, AnyJsName, JsIdentifierAssignment, JsLiteralExportName, JsReferenceIdentifier,
	JsSyntaxKind, JsSyntaxToken, JsxReferenceIdentifier, inner_string_text,
};
...
impl AnyJsIdentifierUsage {
	...
	pub fn is_only_type(&self) -> bool {
		match self {
			...
			AnyJsIdentifierUsage::JsxReferenceIdentifier(_) | AnyJsIdentifierUsage::JsIdentifierAssignment(_) => false,
		}
	}
}
...
impl JsLiteralExportName {
	...
	pub fn is_default(&self) -> bool {
		self.inner_string_text().map(|x| x.text() == "default").unwrap_or(false)
	}
}
