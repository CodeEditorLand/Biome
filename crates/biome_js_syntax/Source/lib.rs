...
pub use biome_rowan::{
	SyntaxNodeText, TextLen, TextRange, TextSize, TokenAtOffset, TokenText, TriviaPieceKind, WalkEvent,
};
pub use expr_ext::*;
pub use file_source::*;
...
impl JsSyntaxKind {
	...
	pub const fn is_keyword(self) -> bool {
		(self as u16) <= (JsSyntaxKind::USING_KW as u16) && (self as u16) >= (JsSyntaxKind::BREAK_KW as u16)
	}
	...
	pub const fn is_contextual_keyword(self) -> bool {
		(self as u16) >= (JsSyntaxKind::ABSTRACT_KW as u16) && (self as u16) <= (JsSyntaxKind::USING_KW as u16)
	}
	...
	pub const fn is_future_reserved_keyword(self) -> bool {
		(self as u16) >= (JsSyntaxKind::IMPLEMENTS_KW as u16) && (self as u16) <= (JsSyntaxKind::YIELD_KW as u16)
	}
}
...
impl biome_rowan::SyntaxKind for JsSyntaxKind {
	...
	fn to_bogus(&self) -> JsSyntaxKind {
		match self {
			...
			kind if AnyJsClassMember::can_cast(*kind) || AnyJsObjectMember::can_cast(*kind) => JS_BOGUS_MEMBER,
	...
	fn is_trivia(self) -> bool {
		matches!(
			self,
			JsSyntaxKind::NEWLINE | JsSyntaxKind::WHITESPACE | JsSyntaxKind::COMMENT | JsSyntaxKind::MULTILINE_COMMENT
		)
	}
...
impl OperatorPrecedence {
	...
	pub const fn is_bitwise(&self) -> bool {
		matches!(
			self,
			OperatorPrecedence::BitwiseAnd | OperatorPrecedence::BitwiseOr | OperatorPrecedence::BitwiseXor
		)
	}
