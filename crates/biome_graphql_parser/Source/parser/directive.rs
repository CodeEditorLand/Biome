...
impl ParseRecovery for DirectiveListParseRecovery {
	...
	fn is_at_recovered(&self, p:&mut Self::Parser<'_>) -> bool { is_at_directive(p) }
}
...
impl ParseNodeList for DirectiveList {
	...
	fn parse_element(&mut self, p:&mut Self::Parser<'_>) -> ParsedSyntax { parse_directive(p) }

	fn is_at_list_end(&self, p:&mut Self::Parser<'_>) -> bool { !is_at_directive(p) }
...
#[inline]
pub(crate) fn is_at_directive(p:&GraphqlParser<'_>) -> bool { p.at(T![@]) }
