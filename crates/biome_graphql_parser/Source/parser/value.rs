...
impl ParseNodeList for ListValueElementList {
	...
	fn parse_element(&mut self, p:&mut Self::Parser<'_>) -> ParsedSyntax { parse_value(p) }

	fn is_at_list_end(&self, p:&mut Self::Parser<'_>) -> bool { is_at_list_end(p) }
...
impl ParseNodeList for ObjectValueMemberList {
	...
	fn parse_element(&mut self, p:&mut Self::Parser<'_>) -> ParsedSyntax { parse_object_field(p) }

	fn is_at_list_end(&self, p:&mut Self::Parser<'_>) -> bool { is_at_object_end(p) }
	...
	fn recover(
		...
		parsed_element.or_recover(p, &ObjectValueMemberListParseRecovery, expected_object_field)
	}
}
...
#[inline]
fn is_at_int(p:&GraphqlParser) -> bool { p.at(GRAPHQL_INT_LITERAL) }

#[inline]
fn is_at_float(p:&GraphqlParser) -> bool { p.at(GRAPHQL_FLOAT_LITERAL) }

#[inline]
pub(crate) fn is_at_string(p:&GraphqlParser) -> bool { p.at(GRAPHQL_STRING_LITERAL) }

#[inline]
fn is_at_boolean(p:&GraphqlParser) -> bool { p.at_ts(BOOLEAN_VALUE_SET) }

#[inline]
fn is_at_null(p:&GraphqlParser) -> bool { p.at(T![null]) }
...
#[inline]
fn is_at_list(p:&GraphqlParser) -> bool { p.at(T!['[']) }
...
#[inline]
fn is_at_object(p:&GraphqlParser) -> bool { p.at(T!['{']) }

#[inline]
fn is_at_object_field(p:&mut GraphqlParser) -> bool { is_nth_at_name(p, 0) }
