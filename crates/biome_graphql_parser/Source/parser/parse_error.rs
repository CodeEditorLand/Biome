...
pub(crate) fn expected_schema_extension(p: &GraphqlParser, range: TextRange) -> ParseDiagnostic {
	p.err_builder("Expected at least one directive or root operation type definition", range)
}
...
pub(crate) fn expected_directive_location(p: &GraphqlParser, range: TextRange) -> ParseDiagnostic {
	p.err_builder("Expected a valid directive location", range).with_alternatives(
		"Must be one of:",
		&[
			"QUERY",
			"MUTATION",
...
pub(crate) fn expected_union_extension(p: &GraphqlParser, range: TextRange) -> ParseDiagnostic {
	p.err_builder("Expected at least one directive or a set union member types", range)
}

pub(crate) fn expected_enum_extension(p: &GraphqlParser, range: TextRange) -> ParseDiagnostic {
	p.err_builder("Expected at least one directive or a set of enum values", range)
}
...
pub(crate) fn expected_input_object_extension(
	...
	p.err_builder("Expected at least one directive or a set of fields definition", range)
}
