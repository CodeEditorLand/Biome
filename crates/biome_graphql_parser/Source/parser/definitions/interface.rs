...
/// Must only be called if the next 2 token is `extend` and `interface`,
...
impl ParseSeparatedList for ImplementsInterfaceList {
	...
	fn parse_element(&mut self, p:&mut Self::Parser<'_>) -> ParsedSyntax { parse_named_type(p) }

	fn is_at_list_end(&self, p:&mut Self::Parser<'_>) -> bool { is_at_implements_interface_end(p) }
	...
	fn recover(
		...
		parsed_element.or_recover(p, &ImplementsInterfaceListParseRecovery, expected_named_type)
	}
	...
	fn allow_empty(&self) -> bool { false }
}
...
#[inline]
fn is_at_implements_interface(p:&mut GraphqlParser<'_>) -> bool { p.at(T![implements]) }
