...
impl ParseNodeList for FieldDefinitionList {
	...
	fn is_at_list_end(&self, p:&mut Self::Parser<'_>) -> bool { is_at_fields_end(p) }
	...
	fn recover(
		...
		parsed_element.or_recover(p, &FieldDefinitionListParseRecovery, expected_field_definition)
	}
}
...
impl ParseNodeList for ArgumentDefinitionList {
	...
	fn is_at_list_end(&self, p:&mut Self::Parser<'_>) -> bool { is_at_arguments_definition_end(p) }
...
/// We must enforce that the arguments definition is always opened with a `(`
/// token. Otherwise, we might end up parsing a field definition as an input
/// value definition. For example:
/// ```graphql
/// name : String = "") : String
/// ```
/// In this case, the opening parenthesis is missing, the name token of an input
/// value definition is also missing. It would be to complex to disambiguate
/// input value definitions from field.
#[inline]
pub(super) fn is_at_arguments_definition(p:&mut GraphqlParser<'_>) -> bool { p.at(T!['(']) }
