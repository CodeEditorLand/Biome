...
/// Must only be called if the next 2 token is `extend` and `union`, otherwise
/// it will panic.
#[inline]
...
impl ParseSeparatedList for UnionMemberTypeList {
	...
	fn parse_element(&mut self, p:&mut Self::Parser<'_>) -> ParsedSyntax { parse_union_member(p) }

	fn is_at_list_end(&self, p:&mut Self::Parser<'_>) -> bool { is_at_union_member_types_end(p) }
...
#[inline]
fn is_at_union_member_types_end(p:&mut GraphqlParser<'_>) -> bool { is_at_definition(p) }
