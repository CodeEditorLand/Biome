...
impl ParseNodeList for SelectionList {
	...
	fn parse_element(&mut self, p:&mut Self::Parser<'_>) -> ParsedSyntax { parse_selection(p) }

	fn is_at_list_end(&self, p:&mut Self::Parser<'_>) -> bool { is_at_selection_set_end(p) }
...
impl ParseNodeList for VariableDefinitionList {
	...
	fn is_at_list_end(&self, p:&mut Self::Parser<'_>) -> bool { is_at_variable_definitions_end(p) }
...
#[inline]
fn is_at_selection_set_end(p:&mut GraphqlParser) -> bool { p.at(T!['}']) }

#[inline]
fn is_at_selection(p:&mut GraphqlParser) -> bool { is_at_field(p) || is_at_fragment(p) }

#[inline]
fn is_at_field(p:&mut GraphqlParser) -> bool { is_nth_at_name(p, 0) || is_at_alias(p) }

#[inline]
fn is_at_fragment(p:&GraphqlParser) -> bool { p.at(DOT3) || p.at(T![on]) }
