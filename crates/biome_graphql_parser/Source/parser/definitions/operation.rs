use crate::parser::{
	...
	parse_error::{
		expected_any_selection, expected_name, expected_type, expected_variable, expected_variable_definition,
	},
...
use biome_parser::{
	Parser, TokenSet, parse_lists::ParseNodeList, parse_recovery::ParseRecovery, parsed_syntax::ParsedSyntax,
	prelude::ParsedSyntax::*, token_set,
};

use super::fragment::{is_at_type_condition, parse_type_condition};

pub(crate) const OPERATION_TYPE: TokenSet<GraphqlSyntaxKind> = token_set![T![query], T![mutation], T![subscription]];
...
impl ParseNodeList for VariableDefinitionList {
	...
	fn recover(
		...
		parsed_element.or_recover(p, &VariableDefinitionListParseRecovery, expected_variable_definition)
	}
}
