//! An extremely fast, lookup table based, GraphQL lexer which yields SyntaxKind
//! tokens used by the biome GraphQL parser.
#[rustfmt::skip]
mod tests;
...
impl<'src> Lexer<'src> for GraphqlLexer<'src> {
	...
	fn consume_newline_or_whitespaces(&mut self) -> Self::Kind {
		if self.consume_newline() {
			self.current_flags.set(TokenFlags::PRECEDING_LINE_BREAK, true);
			NEWLINE
		} else {
	...
	fn next_token(&mut self, _context:Self::LexContext) -> Self::Kind {
		...
		if !kind.is_trivia() {
			self.current_flags.set(TokenFlags::PRECEDING_LINE_BREAK, false);
		}
...
impl<'src> GraphqlLexer<'src> {
	...
	fn consume_exponent(&mut self, chr:u8, start:TextSize, state:LexNumberState) -> LexNumberState {
				...
				match self.current_byte() {
					...
					None => {
						(
							...
							Some(
								ParseDiagnostic::new(
									"Expected an escape sequence following a backslash, but found \
									 none",
...
fn is_name_continue(byte:u8) -> bool { byte.is_ascii_alphanumeric() || byte == b'_' }

fn is_number_start(byte:u8) -> bool { byte.is_ascii_digit() || byte == b'-' }
