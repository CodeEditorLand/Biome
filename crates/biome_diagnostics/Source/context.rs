...
/// [Diagnostic](super::Diagnostic) and the [Error] struct, and exposes various
/// combinator methods to enrich existing diagnostics with additional
/// information.
pub trait DiagnosticExt: internal::Sealed + Sized {
	...
	/// Returns a new diagnostic using the provided `span` instead of the one in
	/// `self`.
...
impl<E:AsDiagnostic> DiagnosticExt for E {
	...
	where
		...
		Error::from(internal::ContextDiagnostic { message, source:self })
	}

	fn with_category(self, category:&'static Category) -> Error
	where
		Error: From<internal::CategoryDiagnostic<Self>>, {
		Error::from(internal::CategoryDiagnostic { category, source:self })
	}
	...
	where
		...
		Error::from(internal::FileSpanDiagnostic { span:span.as_span(), source:self })
	}
	...
	where
		Error: From<internal::SeverityDiagnostic<Self>>, {
		Error::from(internal::SeverityDiagnostic { severity, source:self })
	}
}
...
mod internal {
	...
	impl<M:fmt::Display + 'static, E:AsDiagnostic> Diagnostic for ContextDiagnostic<M, E> {
		fn category(&self) -> Option<&'static Category> { self.source.as_diagnostic().category() }

		fn severity(&self) -> Severity { self.source.as_diagnostic().severity() }
		...
		fn location(&self) -> Location<'_> { self.source.as_diagnostic().location() }

		fn tags(&self) -> DiagnosticTags { self.source.as_diagnostic().tags() }

		fn source(&self) -> Option<&dyn Diagnostic> { Some(self.source.as_dyn()) }
	}
	...
	/// Helper wrapper implementing [Debug] for types implementing
	/// [fmt::Display], prints a debug representation of the markup generated
	/// by printing `T`.
	...
	impl<E:AsDiagnostic> Diagnostic for CategoryDiagnostic<E> {
		fn category(&self) -> Option<&'static Category> {
			Some(self.source.as_diagnostic().category().unwrap_or(self.category))
		}
	...
	/// uses `span` as its location span instead of the one returned by
	/// `source`.
	...
	/// Diagnostic type returned by
	/// [super::DiagnosticExt::with_file_source_code], uses `source_code` as
	/// its location source code if `source` doesn't return one.
	...
	impl<E:AsDiagnostic> Diagnostic for TagsDiagnostic<E> {
		...
		fn tags(&self) -> DiagnosticTags { self.source.as_diagnostic().tags() | self.tags }
	}
