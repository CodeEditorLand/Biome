...
mod internal {
	...
	impl fmt::Write for DisplayMarkup<'_, '_> {
		fn write_str(&mut self, _: &fmt::MarkupElements<'_>, content: &str) -> io::Result<()> {
			self.0.write_str(content).map_err(|error| io::Error::new(io::ErrorKind::Other, error))
		}
		...
		fn write_fmt(
			...
			self.0.write_fmt(content).map_err(|error| io::Error::new(io::ErrorKind::Other, error))
		}
	}
