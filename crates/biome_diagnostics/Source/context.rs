...
mod internal {
	...
	use crate::{
		diagnostic::internal::AsDiagnostic, Advices, Backtrace, Category,
		Diagnostic, DiagnosticTags, LineIndex, LineIndexBuf, Location,
		LogCategory, Resource, Severity, SourceCode, Visit,
	};
	...
	impl<M: fmt::Display + 'static, E: AsDiagnostic> Diagnostic
	...
	impl<E: AsDiagnostic> Diagnostic for CategoryDiagnostic<E> {
		fn category(&self) -> Option<&'static Category> {
			Some(
				self.source.as_diagnostic().category().unwrap_or(self.category),
			)
		}
	...
	impl Visit for FileSourceCodeVisitor<'_> {
		...
		fn record_frame(&mut self, location: Location<'_>) -> io::Result<()> {
			self.visitor.record_frame(Location {
				source_code: Some(
					location.source_code.unwrap_or(self.source_code),
				),
				..location
			})
		}
