...
impl<E:std::error::Error + Send + Sync + 'static> From<E> for StdError {
	fn from(error:E) -> Self { Self { error:Box::new(error) } }
}
...
impl From<bpaf::ParseFailure> for BpafError {
	fn from(error:bpaf::ParseFailure) -> Self { Self { error } }
}

impl Diagnostic for BpafError {
	fn category(&self) -> Option<&'static Category> { Some(category!("flags/invalid")) }
...
impl From<oxc_resolver::ResolveError> for ResolveError {
	fn from(error:oxc_resolver::ResolveError) -> Self { Self { error } }
}
...
impl From<serde_json::Error> for SerdeJsonError {
	fn from(error:serde_json::Error) -> Self { Self { error } }
}
...
impl Diagnostic for IniError {
	fn category(&self) -> Option<&'static Category> { Some(category!("configuration")) }
...
impl From<serde_ini::de::Error> for IniError {
	fn from(error:serde_ini::de::Error) -> Self { Self { error } }
}
