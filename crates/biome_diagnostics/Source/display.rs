...
impl<D:AsDiagnostic + ?Sized> std::fmt::Display for PrintDescription<'_, D> {
	fn fmt(&self, fmt:&mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		self.0.as_diagnostic().description(fmt).map_err(|_| std::fmt::Error)
	}
}
...
impl<'fmt, D:AsDiagnostic + ?Sized> PrintDiagnostic<'fmt, D> {
	pub fn simple(diag:&'fmt D) -> Self { Self { diag, verbose:false, search:false } }

	pub fn verbose(diag:&'fmt D) -> Self { Self { diag, verbose:true, search:false } }

	pub fn search(diag:&'fmt D) -> Self { Self { diag, verbose:false, search:true } }
}
...
impl<'write> CountWidth<'write, dyn fmt::Write + 'write> {
	/// Wrap the writer in an existing [fmt::Formatter] with an instance of
	/// [CountWidth].
...
/// Advice visitor used to detect if the diagnostic contains any frame or
...
where
	...
	// Print a log advice for the message, with a special fallback if the buffer is
	// empty
	if message.is_empty() {
		visitor.record_log(
			LogCategory::None,
...
impl PrintAdvices<'_, '_> {
	fn print_log(
		...
		fmt.write_markup(Markup(&[MarkupNode { elements:&[kind], content:text }]))?;
...
impl Visit for PrintAdvices<'_, '_> {
	...
	fn record_diff(&mut self, diff:&TextEdit) -> io::Result<()> { diff::print_diff(self.0, diff) }
	...
	fn record_table(
		...
		for current_row_index in 0..rows_number {
			for current_column_index in 0..columns_number {
				let cell = columns.get(current_column_index).and_then(|c| c.get(current_row_index));
				...
				if let Some(cell) = cell {
					...
					if columns_number != current_column_index + 1 {
						self.0.write_markup(markup! {{Padding::new(padding + extra_padding)}})?;
					}
				}
			}
...
where
	...
	if diagnostic.severity() == Severity::Fatal {
		visitor.record_log(
			LogCategory::Warn,
			&"Biome exited as this error could not be handled and resulted in a fatal error. \
	...
	if diagnostic.tags().contains(DiagnosticTags::INTERNAL) {
		visitor.record_log(
			LogCategory::Warn,
			&"This diagnostic was derived from an internal Biome error. Potential bug, please \
...
impl<D:Diagnostic + ?Sized> Advices for PrintVerboseAdvices<'_, D> {
	fn record(&self, visitor:&mut dyn Visit) -> io::Result<()> { self.0.verbose_advices(visitor) }
}
...
impl<'write> IndentWriter<'write, dyn fmt::Write + 'write> {
	fn wrap<'slot, 'fmt:'write + 'slot>(
		...
		fmt.wrap_writer(|writer| slot.get_or_insert(Self { writer, pending_indent, ident_text }))
	}
}

impl<W:fmt::Write + ?Sized> fmt::Write for IndentWriter<'_, W> {
	fn write_str(&mut self, elements:&fmt::MarkupElements<'_>, mut content:&str) -> io::Result<()> {
		while !content.is_empty() {
			if self.pending_indent {
				self.writer.write_str(&MarkupElements::Root, self.ident_text)?;
...
mod tests {
	...
	impl<A:Advices + std::fmt::Debug> Diagnostic for TestDiagnostic<A> {
		...
		fn tags(&self) -> DiagnosticTags { DiagnosticTags::FIXABLE }

		fn source(&self) -> Option<&dyn Diagnostic> { self.source.as_deref() }
	}
	...
	impl Advices for FrameAdvice {
		fn record(&self, visitor:&mut dyn Visit) -> io::Result<()> {
			visitor.record_frame(Location {
				...
				source_code:Some(SourceCode { text:"context location context", line_starts:None }),
			})
		}
	}
	...
	#[test]
	fn test_log_advices() {
		let diag = TestDiagnostic { advice:Some(LogAdvices), ..TestDiagnostic::empty() };
	...
	#[test]
	fn test_list_advice() {
		let diag = TestDiagnostic { advice:Some(ListAdvice), ..TestDiagnostic::empty() };
	...
	fn test_frame_advice() {
		let diag = TestDiagnostic { advice:Some(FrameAdvice), ..TestDiagnostic::empty() };
	...
	#[test]
	fn test_diff_advice() {
		let diag = TestDiagnostic { advice:Some(DiffAdvice), ..TestDiagnostic::empty() };
	...
	fn test_backtrace_advice() {
		let diag = TestDiagnostic { advice:Some(BacktraceAdvice), ..TestDiagnostic::empty() };
	...
	fn test_command_advice() {
		let diag = TestDiagnostic { advice:Some(CommandAdvice), ..TestDiagnostic::empty() };
	...
	fn test_group_advice() {
		let diag = TestDiagnostic { advice:Some(GroupAdvice), ..TestDiagnostic::empty() };
