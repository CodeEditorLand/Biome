...
use biome_console::{
	fmt, markup, HorizontalLine, Markup, MarkupBuf, MarkupElement, MarkupNode,
};
...
use crate::{
	diagnostic::internal::AsDiagnostic, Advices, Diagnostic, DiagnosticTags,
	Location, LogCategory, Resource, Severity, Visit,
};
...
impl<D: Diagnostic + ?Sized> fmt::Display for PrintHeader<'_, D> {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> io::Result<()> {
		...
		if let Some(name) = file_name {
			...
			if let (Some(span), Some(source_code)) =
	...
	D: Diagnostic + ?Sized,
{
	...
	let mut frame_visitor =
		FrameVisitor { location: diagnostic.location(), skip_frame: false };
...
impl Visit for PrintAdvices<'_, '_> {
	fn record_log(
		...
		match category {
			...
			LogCategory::Info => {
				self.print_log(MarkupElement::Info, '\u{2139}', text)
			},
			LogCategory::Warn => {
				self.print_log(MarkupElement::Warn, '\u{26a0}', text)
			},
			LogCategory::Error => {
				self.print_log(MarkupElement::Error, '\u{2716}', text)
			},
		}
	}
...
impl<'write> IndentWriter<'write, dyn fmt::Write + 'write> {
	fn wrap<'slot, 'fmt: 'write + 'slot>(
		...
		fmt.wrap_writer(|writer| {
			slot.get_or_insert(Self { writer, pending_indent, ident_text })
		})
	}
}
...
mod tests {
	...
	impl Advices for DiffAdvice {
		fn record(&self, visitor: &mut dyn Visit) -> io::Result<()> {
			let diff = TextEdit::from_unicode_words(
