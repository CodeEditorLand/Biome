...
use biome_console::{HorizontalLine, Markup, MarkupBuf, MarkupElement, MarkupNode, Padding, fmt, markup};
...
impl<'fmt, D: AsDiagnostic + ?Sized> PrintDiagnostic<'fmt, D> {
	pub fn simple(diag: &'fmt D) -> Self {
		Self { diag, verbose: false, search: false }
	}

	pub fn verbose(diag: &'fmt D) -> Self {
		Self { diag, verbose: true, search: false }
	}

	pub fn search(diag: &'fmt D) -> Self {
		Self { diag, verbose: false, search: true }
	}
}
...
impl PrintAdvices<'_, '_> {
	fn print_log(&mut self, kind: MarkupElement<'_>, prefix: char, text: &dyn fmt::Display) -> io::Result<()> {
...
mod tests {
	...
	use crate::{Advices, Diagnostic, Location, LogCategory, PrintDiagnostic, Resource, SourceCode, Visit};
	...
	impl Advices for DiffAdvice {
		fn record(&self, visitor: &mut dyn Visit) -> io::Result<()> {
			let diff = TextEdit::from_unicode_words("context before context", "context after context");
