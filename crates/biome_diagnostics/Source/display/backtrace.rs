	...
	where
		...
		let frames = match &self.inner {
			BacktraceKind::Native(backtrace) => {
				...
				let frames:Vec<_> = backtrace.frames().iter().map(SerializedFrame::from).collect();
	...
	fn frames(&self) -> &'_ [::backtrace::BacktraceFrame] {
		...
		let top_frame = frames.iter().position(|frame| {
			frame
				.symbols()
				.iter()
				.any(|symbol| symbol.addr().map_or(false, |addr| addr as usize == self.top_frame))
		});
		...
		let bottom_frame = frames.iter().position(|frame| {
			frame.symbols().iter().any(|symbol| {
				symbol.addr().map_or(false, |addr| addr as usize == self.bottom_frame)
			})
		});
...
pub(super) fn print_backtrace(fmt:&mut fmt::Formatter<'_>, backtrace:&Backtrace) -> io::Result<()> {
	for (frame_index, frame) in backtrace.frames().iter().enumerate() {
		...
		for symbol in frame.symbols().iter() {
			...
			if let Some(filename) = symbol.filename() {
				...
				// Build up the text of the link from the file path, the line number and column
				// number
