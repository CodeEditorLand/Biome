...
impl Default for Backtrace {
	...
	#[inline(never)]
	fn default() -> Self { Self::capture(Backtrace::default as usize) }
}

impl Backtrace {
	/// Take a snapshot of the current state of the stack and return it as a
	/// [Backtrace].
	pub fn capture(top_frame:usize) -> Self {
		Self { inner:BacktraceKind::Native(NativeBacktrace::new(top_frame)) }
	}
	...
	fn frames(&self) -> BacktraceFrames<'_> {
		match &self.inner {
			BacktraceKind::Native(inner) => {
				BacktraceFrames::Native(inner.frames())
			},
			BacktraceKind::Serialized(inner) => {
				BacktraceFrames::Serialized(inner)
			},
		}
	}
...
impl<'de> serde::Deserialize<'de> for Backtrace {
	...
	where
		...
		Ok(Self {
			inner:BacktraceKind::Serialized(
				<Vec<SerializedFrame>>::deserialize(deserializer)?,
			),
		})
	}
}
...
impl schemars::JsonSchema for Backtrace {
	fn schema_name() -> String { String::from("Backtrace") }
...
impl PartialEq for BacktraceKind {
	fn eq(&self, _other:&Self) -> bool {
		if let (
			BacktraceKind::Serialized(this),
			BacktraceKind::Serialized(other),
		) = (self, _other)
		{
			return this == other;
		}
...
impl NativeBacktrace {
	...
	fn resolve(&mut self) { self.backtrace.resolve(); }
...
fn bottom_frame() -> usize { BOTTOM_FRAME.with(|cell| cell.get().unwrap_or(0)) }
	...
	for (frame_index, frame) in backtrace.frames().iter().enumerate() {
		...
		for symbol in frame.symbols().iter() {
			...
			if let Some(filename) = symbol.filename() {
				let mut slot = None;
				let mut fmt =
					IndentWriter::wrap(&mut fmt, &mut slot, true, "    ");
				...
				// Build up the text of the link from the file path, the line
	...
	fn iter(&self) -> BacktraceFramesIter<'_> {
		match self {
			...
			Self::Serialized(inner) => {
				BacktraceFramesIter::Serialized(inner.iter())
			},
		}
	}
...
impl<'a> Iterator for BacktraceFramesIter<'a> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		match self {
			...
			Self::Serialized(inner) => {
				inner.next().map(BacktraceFrame::Serialized)
			},
		}
	}
}
...
impl BacktraceFrame<'_> {
	...
	fn symbols(&self) -> BacktraceSymbols<'_> {
		match self {
			...
			Self::Serialized(inner) => {
				BacktraceSymbols::Serialized(&inner.symbols)
			},
		}
	}
}
...
impl BacktraceSymbols<'_> {
	fn iter(&self) -> BacktraceSymbolsIter<'_> {
		match self {
			...
			Self::Serialized(inner) => {
				BacktraceSymbolsIter::Serialized(inner.iter())
			},
		}
	}
}
...
impl<'a> Iterator for BacktraceSymbolsIter<'a> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		match self {
			...
			Self::Serialized(inner) => {
				inner.next().map(BacktraceSymbol::Serialized)
			},
		}
	}
}
