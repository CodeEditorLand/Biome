...
/// Convenient type that can be used when message and descriptions match, and
/// they need to be displayed using different formatters
///
/// ## Examples
///
/// ```
...
impl MessageAndDescription {
	...
	pub fn set_message(&mut self, new_message:MarkupBuf) { self.message = new_message; }
...
impl From<String> for MessageAndDescription {
	fn from(description:String) -> Self { Self { message:markup! { {description} }.to_owned(), description } }
}
...
impl biome_console::fmt::Display for MessageAndDescription {
	fn fmt(&self, fmt:&mut Formatter<'_>) -> std::io::Result<()> { fmt.write_markup(markup! {{self.message}}) }
}
...
pub fn markup_to_string(markup:&MarkupBuf) -> String {
	...
	fmt.write_markup(markup! { {markup} }).expect("to have written in the buffer");
