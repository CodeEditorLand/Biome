...
pub(super) fn print_diff(fmt:&mut fmt::Formatter<'_>, diff:&TextEdit) -> io::Result<()> {
	// Before printing, we need to preprocess the list of DiffOps it's made of to
	// classify them by line
	...
	let modified_line = if before_line_count == after_line_count {
		let mut iter = modified_lines.iter().filter_map(|key| {
			...
			let has_edits = line.diffs.is_empty()
				|| line
					.diffs
					.iter()
					.any(|(tag, text)| matches!(tag, ChangeTag::Delete | ChangeTag::Insert) && !text.is_empty());
		...
		iter.next().and_then(|(key, line)| {
			...
			if !line.diffs.is_empty() { Some((key, line)) } else { None }
		})
	} else {
		None
	};
	...
	// Otherwise if multiple lines were modified we need to perform more
	// preprocessing, to merge identical line numbers and calculate how many
	// context lines need to be rendered
	...
	print_full_diff(fmt, &diffs_by_line, &shown_line_indexes, before_line_count, after_line_count)
}
...
/// - `after_line` counts the number of lines in the new revision of the
///   document
/// - `before_line` counts the number of lines in the old revision of the
///   document
		...
		let op = match op {
			...
			CompressedOp::EqualLines { line_count } => {
				...
				for line_index in 0..=line_count.get() {
					// Don't increment the first line if we are the first tuple marking the
...
impl LineKey {
	const fn before(before_line:OneIndexed) -> Self { Self { before_line:Some(before_line), after_line:None } }

	const fn after(after_line:OneIndexed) -> Self { Self { before_line:None, after_line:Some(after_line) } }
}
...
impl<'a> GroupDiffsLine<'a> {
	fn insert(inserted_lines:&mut BTreeMap<LineKey, Self>, key:LineKey, tag:ChangeTag, text:&'a str) {
		inserted_lines
			.entry(key)
			.and_modify(|line| {
				if !text.is_empty() {
			...
			.or_insert_with_key(|key| {
				GroupDiffsLine {
					...
					diffs:if text.is_empty() { Vec::new() } else { vec![(tag, text)] },
				}
			});
	}
}
...
fn push_to_line<'b>(
	...
	let PushToLineState { modified_lines, inserted_lines, before_line_to_after } = state;
...
fn process_diff_lines<'lines, 'diff>(
	...
	for before_line in IntoIter::new(OneIndexed::MIN..=before_line_count) {
		...
		if let (Some(inserted_before_line), Some(inserted_after_line)) = (inserted_before_line, inserted_after_line) {
			if inserted_before_line.diffs == inserted_after_line.diffs {
				let line = inserted_lines.remove(&LineKey::before(before_line)).unwrap();

				inserted_lines.remove(&LineKey::after(after_line)).unwrap();

				inserted_lines.insert(
					LineKey { before_line:Some(before_line), after_line:Some(after_line) },
					GroupDiffsLine { before_line:Some(before_line), after_line:Some(after_line), diffs:line.diffs },
				);
			}
		}
	}
	...
	for before_line in IntoIter::new(OneIndexed::MIN..=before_line_count) {
		...
		if let Some(after_line) = before_line_to_after.get(&before_line) {
			let line = inserted_lines.get(&LineKey { before_line:Some(before_line), after_line:Some(*after_line) });
...
fn print_short_diff(fmt:&mut fmt::Formatter<'_>, key:&LineKey, entry:&GroupDiffsLine<'_>) -> io::Result<()> {
	let index = match (key.before_line, key.after_line) {
		(None, Some(index)) | (Some(index), None) => index,
		(None, None) | (Some(_), Some(_)) => {
			unreachable!("the key of a modified line should have exactly one index in one of the two revisions")
		},
	};
...
fn print_full_diff(
	...
	for (i, line) in diffs_by_line.iter().enumerate() {
		...
		if single_line {
			let line = FormatDiffLine { is_equal:line_type == ChangeTag::Equal, ops:&line.diffs };
...
impl<W:fmt::Write + ?Sized> fmt::Write for ElementWrapper<'_, W> {
	...
	fn write_fmt(&mut self, elements:&fmt::MarkupElements<'_>, content:std::fmt::Arguments<'_>) -> io::Result<()> {
		let elements = fmt::MarkupElements::Node(elements, slice::from_ref(&self.1));
