...
pub(super) fn print_diff(fmt:&mut fmt::Formatter<'_>, diff:&TextEdit) -> io::Result<()> {
	// Before printing, we need to preprocess the list of DiffOps it's made of
	// to classify them by line
		...
		let op = match op {
			...
			CompressedOp::EqualLines { line_count } => {
				...
				for line_index in 0..=line_count.get() {
					// Don't increment the first line if we are the first tuple
...
impl LineKey {
	const fn before(before_line:OneIndexed) -> Self {
		Self { before_line:Some(before_line), after_line:None }
	}

	const fn after(after_line:OneIndexed) -> Self {
		Self { before_line:None, after_line:Some(after_line) }
	}
}
	...
	for before_line in IntoIter::new(OneIndexed::MIN..=before_line_count) {
		...
		if let Some(after_line) = before_line_to_after.get(&before_line) {
			let line = inserted_lines
				.get(&LineKey { before_line:Some(before_line), after_line:Some(*after_line) });
...
fn print_short_diff(
	...
	let index = match (key.before_line, key.after_line) {
		...
		(None, None) | (Some(_), Some(_)) => {
			unreachable!(
				"the key of a modified line should have exactly one index in one of the two \
				 revisions"
			)
		},
	};
