...
pub(super) fn print_frame(fmt:&mut fmt::Formatter<'_>, location:Location<'_>) -> io::Result<()> {
	let source_span = location.source_code.and_then(|source_code| Some((source_code, location.span?)));
	...
	let context_start = start_location.line_number.saturating_sub(CODE_FRAME_CONTEXT_LINES.get());
	...
	for line_index in IntoIter::new(context_start..=context_end) {
		...
		let should_highlight = line_index >= start_location.line_number && line_index <= end_location.line_number;

		let padding_width = max_gutter_len.get().saturating_sub(calculate_print_width(line_index).get());
		...
		if should_highlight {
			...
			let start_index_relative_to_line = start_index.max(line_range.start()) - line_range.start();
			...
			let marker = if is_first_line && is_last_line {
				...
				Some(TextRange::new(start_index_relative_to_line, line_text.text_len()))
...
pub(super) fn print_highlighted_frame(fmt:&mut fmt::Formatter<'_>, location:Location<'_>) -> io::Result<()> {
	...
	for line_index in IntoIter::new(match_line_start..match_line_end) {
		...
		let marker = if is_first_line && is_last_line {
			...
			let safe_start_index = start_index.saturating_sub(current_text.trim_start().text_len().into());
...
pub(super) fn text_width(text:&str) -> usize { text.chars().map(char_width).sum() }
...
pub(super) fn print_invisibles(
	...
	while let Some((i, char)) = iter.next() {
		...
		if char == ' ' && options.ignore_lone_spaces {
			...
			let next_char_is_whitespace = iter.peek().map_or(false, |(_, char)| char.is_ascii_whitespace());
		...
		if options.at_line_start && !had_non_whitespace && char == '\t' && options.ignore_leading_tabs {
			show_invisible = false;
		}
...
impl<'diagnostic> SourceFile<'diagnostic> {
	...
	pub fn new(source_code:BorrowedSourceCode<'diagnostic>) -> Self {
		// Either re-use the existing line index provided by the diagnostic or create
		// one
		Self {
			source:source_code.text,
			line_starts:source_code
				.line_starts
				.map_or_else(|| Cow::Owned(LineIndexBuf::from_source_text(source_code.text)), Cow::Borrowed),
		}
	}

	/// Return the starting byte index of the line with the specified line
	/// index. Convenience method that already generates errors if necessary.
	fn line_start(&self, line_index:usize) -> io::Result<TextSize> {
		...
		match line_index.cmp(&self.line_starts.len()) {
			...
			Ordering::Greater => Err(io::Error::new(io::ErrorKind::InvalidInput, "overflow error")),
		}
	}
...
/// returned. If the byte index is past the end of the line, the column index of
/// the last character `+ 1` is returned.
fn column_index(source:&str, line_range:TextRange, byte_index:TextSize) -> usize {
	let end_index = std::cmp::min(byte_index, std::cmp::min(line_range.end(), source.text_len()));
...
impl OneIndexed {
	...
	pub const fn from_zero_indexed(value:usize) -> Self { Self(ONE.saturating_add(value)) }

	/// Returns the value as a primitive type.
	pub const fn get(self) -> usize { self.0.get() }

	/// Return the zero-indexed primitive value for this [OneIndexed]
	pub const fn to_zero_indexed(self) -> usize { self.0.get() - 1 }
...
impl fmt::Display for OneIndexed {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> io::Result<()> { self.0.get().fmt(f) }
}

impl std::fmt::Display for OneIndexed {
	fn fmt(&self, f:&mut std::fmt::Formatter<'_>) -> std::fmt::Result { self.0.get().fmt(f) }
}
...
impl Iterator for IntoIter {
	...
	fn next(&mut self) -> Option<Self::Item> { self.0.next().map(|index| OneIndexed::new(index).unwrap()) }

	fn size_hint(&self) -> (usize, Option<usize>) { self.0.size_hint() }
}

impl DoubleEndedIterator for IntoIter {
	fn next_back(&mut self) -> Option<Self::Item> { self.0.next_back().map(|index| OneIndexed::new(index).unwrap()) }
}
