...
pub(super) fn print_frame(fmt:&mut fmt::Formatter<'_>, location:Location<'_>) -> io::Result<()> {
	let source_span =
		location.source_code.and_then(|source_code| Some((source_code, location.span?)));
	...
	// If we have too many lines in our selection, then collapse them to an
	// ellipsis
	...
	for line_index in IntoIter::new(context_start..=context_end) {
		...
		let padding_width =
			max_gutter_len.get().saturating_sub(calculate_print_width(line_index).get());
	...
	while let Some((i, char)) = iter.next() {
		...
		if char == ' ' && options.ignore_lone_spaces {
			...
			let next_char_is_whitespace =
				iter.peek().map_or(false, |(_, char)| char.is_ascii_whitespace());
...
impl<'diagnostic> SourceFile<'diagnostic> {
	...
	pub fn new(source_code:BorrowedSourceCode<'diagnostic>) -> Self {
		// Either re-use the existing line index provided by the diagnostic or
		// create one
		Self {
	...
	fn column_number(&self, line_index:usize, byte_index:TextSize) -> io::Result<OneIndexed> {
		...
		// SAFETY: Adding `1` to the value of `column_index` ensures it's
		// non-zero
...
impl DoubleEndedIterator for IntoIter {
	fn next_back(&mut self) -> Option<Self::Item> {
		self.0.next_back().map(|index| OneIndexed::new(index).unwrap())
	}
}
