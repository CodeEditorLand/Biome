...
pub use node_js_package::{Dependencies, NodeJsPackage, PackageJson, PackageType, TsConfigJson, Version};
...
pub(crate) type PackageRoot<P> = <<<P as Package>::Manifest as Manifest>::Language as Language>::Root;
...
impl AnyProject {
	...
	pub fn into_serde_diagnostics(self) -> Vec<Diagnostic> {
		self.parse_diagnostics
			.into_iter()
			.map(Diagnostic::new)
			.chain(self.deserialize_diagnostics.into_iter().map(Diagnostic::new))
			.collect()
	}
}
