...
impl GritDot {
	...
	pub fn as_fields(&self) -> GritDotFields {
		GritDotFields { dot_token: self.dot_token() }
	}
...
impl GritName {
	...
	pub fn as_fields(&self) -> GritNameFields {
		GritNameFields { value_token: self.value_token() }
	}
	pub fn value_token(&self) -> SyntaxResult<SyntaxToken> {
		support::required_token(&self.syntax, 0usize)
	}
...
impl GritNot {
	...
	pub fn as_fields(&self) -> GritNotFields {
		GritNotFields { token: self.token() }
	}
...
impl std::fmt::Debug for GritNot {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		f.debug_struct("GritNot").field("token", &support::DebugSyntaxResult(self.token())).finish()
	}
}
...
impl AstNode for AnyGritMaybeNamedArg {
	type Language = Language;
	const KIND_SET: SyntaxKindSet<Language> =
		AnyGritPattern::KIND_SET.union(GritBogusNamedArg::KIND_SET).union(GritNamedArg::KIND_SET);
	fn can_cast(kind: SyntaxKind) -> bool {
		match kind {
