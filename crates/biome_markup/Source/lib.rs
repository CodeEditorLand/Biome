				...
				match punct.as_char() {
					'<' => {
						...
						while let Some(TokenTree::Ident(_)) = input.peek() {
							// SAFETY: these panics are checked by the above
							// call to peek
						...
						if !is_closing_element {
							...
							// Only verify the coherence of the top element on
							// the stack with a closing element, skip
							// over the check if the stack is empty as
							// that error will be handled when the top
							// element gets popped off the stack later
