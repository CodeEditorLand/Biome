...
impl Modules {
	fn insert<'a>(&mut self, mut path: impl Iterator<Item = &'a str>, test: proc_macro2::TokenStream) {
		match path.next() {
			Some(module) => {
...
impl Arguments {
	fn get_all_files(&self) -> Result<AllFiles, &str> {
		let base =
			std::env::var("CARGO_MANIFEST_DIR").map_err(|_| "Cannot find CARGO_MANIFEST_DIR. Are you using cargo?")?;
