...
use quote::ToTokens;
use syn::{Attribute, Error, Lit, Meta, MetaNameValue, Path, spanned::Spanned};
...
pub(crate) struct ContainerAttrs {
	...
	/// Deserialize the given `try_from` type, then try converting to the
	/// annotated type
	///
		...
		for attr in attrs {
			if attr.path.is_ident("deserializable") {
				parse_meta_list(&attr.parse_meta()?, |meta| {
					match meta {
						Meta::Path(path) if path.is_ident("with_validator") => opts.with_validator = true,

						Meta::NameValue(MetaNameValue { path, lit: Lit::Str(s), .. }) if path.is_ident("from") => {
							opts.from = Some(s.parse()?)
						},
						Meta::NameValue(MetaNameValue { path, lit: Lit::Str(s), .. }) if path.is_ident("try_from") => {
						...
						_ => {
							...
							return Err(Error::new(meta.span(), format_args!("Unexpected attribute: {meta_str}")));
						},
					}

					if opts.from.is_some() && opts.try_from.is_some() {
						return Err(Error::new(meta.span(), "You cannot specify both `from` and `try_from`"));
					}
