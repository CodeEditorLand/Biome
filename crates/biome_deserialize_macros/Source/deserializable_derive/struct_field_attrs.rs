use quote::ToTokens;
use syn::{spanned::Spanned, Attribute, Error, Lit, Meta, MetaNameValue, Path};
...
pub struct StructFieldAttrs {
	...
	/// this field fails.
	///
	/// Note the struct may still be deserialized if the field is not present
	/// in the serialized representation at all. In that case
	/// `Default::default()` will be filled in.
