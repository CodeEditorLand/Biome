...
pub enum DeprecatedField {
	/// A generic message that explains what to do or why the field is
	/// deprecated.
	Message(String),
					...
					match meta {
						Meta::Path(path) => {
							if path.is_ident("required") {
								...
								return Err(Error::new(path.span(), format_args!("Unexpected attribute: {path_str}")));
							}
						},
						...
						Meta::List(_) if meta.path().is_ident("deprecated") => {
							...
							parse_meta_list(meta, |meta| {
								let Meta::NameValue(MetaNameValue { path, lit: Lit::Str(s), .. }) = meta else {
								...
								deprecated = if deprecated.is_some() {
									...
									return Err(Error::new(
										path.span(),
										format_args!("Unexpected attribute inside deprecated(): {path_text}"),
									));
								};
						...
						_ => {
							...
							return Err(Error::new(meta.span(), format_args!("Unexpected attribute: {meta_text}")));
						},
					}
