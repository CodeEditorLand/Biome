...
impl DeriveInput {
	pub fn parse(input: syn::DeriveInput) -> Self {
		let data = match input.data {
			...
			Data::Union(_) => {
				abort!(input, "Merge can only be derived for enums and structs")
			},
		};

		Self { ident: input.ident, data }
	}
}
...
pub(crate) fn generate_merge(input: DeriveInput) -> TokenStream {
	match input.data {
		...
		MergeableData::Struct(fields) => {
			generate_merge_struct(input.ident, fields)
		},
	}
}
