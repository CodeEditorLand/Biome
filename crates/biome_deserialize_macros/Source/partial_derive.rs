...
impl DeriveInput {
	pub fn parse(input: syn::DeriveInput) -> Self {
		...
		let attrs =
			Attrs::try_from(&input.attrs).expect("Could not parse attributes");
		...
		Self { ident, partial_ident, attrs, fields }
	}
}
...
pub(crate) fn generate_partial(input: DeriveInput) -> TokenStream {
	...
	let fields =
		input.fields.iter().map(|FieldData { ident, attrs, ty, .. }| {
	...
	let from_partial_fields = input.fields.iter().map(
		|FieldData { ident, ty, should_wrap, .. }| {
			if *should_wrap {
				quote! {
