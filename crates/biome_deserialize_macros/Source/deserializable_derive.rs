...
impl DeriveInput {
	pub fn parse(input:syn::DeriveInput) -> Self {
		...
		} else if let ContainerAttrs { with_validator, try_from: Some(try_from), .. } = attrs {
			DeserializableData::TryFrom(DeserializableTryFromData { try_from, with_validator })
		} else {
			match input.data {
				Data::Enum(data) => {
					let variants = data
						...
						.map(|variant| {
							...
							let attrs = EnumVariantAttrs::try_from(&variant.attrs)
								.expect("Could not parse enum variant attributes");
						...
						let fields = data
							...
							.filter_map(|(ident, attrs, ty)| {
								...
								if attrs.rest {
									...
									// If rest field, we don't return a
									// field data, because we don't
									// want to deserialize into the field
									// directly.
									return None;
								}
				...
				_ => {
					abort!(input, "Deserializable can only be derived for enums and structs")
				},
			}
		};

		Self { ident:input.ident, generics:input.generics, data }
	}
}
...
fn generate_deserializable_struct(
	...
	let required_fields:Vec<_> = data.fields.iter().filter(|data| data.required).cloned().collect();

	let deserialize_fields:Vec<_> = data
		.fields
		.into_iter()
		.map(|field_data| {
			let DeserializableFieldData { ident: field_ident, key, .. } = field_data;
			let deprecation_notice = field_data.deprecated.map(|deprecated| {
				match deprecated {
					DeprecatedField::Message(message) => {
						quote! {
					...
					DeprecatedField::UseInstead(path) => {
						quote! {
		...
		let params = generics.params.iter().map(|param| {
			match param {
				GenericParam::Type(ty) => {
					let attrs = ty.attrs.iter().fold(quote! {}, |acc, attr| quote! { #acc #attr });
					let ident = &ty.ident;
					quote! { #attrs #ident }
				},
