...
fn generate_deserializable_enum(
	...
	let allowed_variants: Vec<_> =
		data.variants.iter().map(|DeserializableVariantData { key, .. }| quote! { #key }).collect();
...
fn generate_deserializable_struct(
	...
	let required_fields: Vec<_> =
		data.fields.iter().filter(|data| data.required).cloned().collect();
