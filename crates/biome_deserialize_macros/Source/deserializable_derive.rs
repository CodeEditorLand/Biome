...
fn generate_deserializable_struct(
	...
	let deserialize_fields: Vec<_> = data
		...
		.into_iter()
		.map(|field_data| {
			let DeserializableFieldData { ident: field_ident, key, .. } =
				field_data;
			let deprecation_notice =
				field_data.deprecated.map(|deprecated| match deprecated {
			...
			let error_result =
				if field_data.bail_on_error || field_data.required {
					quote! { return None, }
				} else {
					quote! { {} }
				};
	...
	let validator = if required_fields.is_empty() {
		...
		let required_fields = required_fields.iter().map(|field_data| {
			let DeserializableFieldData { ident: field_ident, key, ty, .. } =
				field_data;
			quote! {
	...
	let unknown_key_handler = if let Some(rest_field) = data.rest_field {
		...
		match data.unknown_fields {
			UnknownFields::Warn | UnknownFields::Deny => {
				let with_customseverity =
					if data.unknown_fields == UnknownFields::Warn {
