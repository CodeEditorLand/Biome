...
impl FormatNodeRule<GraphqlStringValue> for FormatGraphqlStringValue {
	fn fmt_fields(&self, node:&GraphqlStringValue, f:&mut GraphqlFormatter) -> FormatResult<()> {
		...
		if node.is_block() {
			...
			// But leading whitespace is significant in block strings to
			// calculate the indentation level SAFETY:
			// We check that the string starts with triple quotes and the parser
			// guarantees that ends with triple quotes.
			...
			let content = format_with(|f| {
				...
				for line in trimmed_content.lines() {
					if line.is_empty() || is_blank(line) {
						...
						// write an empty line because two hardline breaks don't
						// work
						join.entry(&empty_line());
						continue;
					}
					// Write the line with the minimum indentation level removed
					// SAFETY: min_indent is always less than or equal to the
					// length of the line
