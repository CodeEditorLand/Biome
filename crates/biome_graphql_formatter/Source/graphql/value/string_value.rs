...
impl FormatNodeRule<GraphqlStringValue> for FormatGraphqlStringValue {
	fn fmt_fields(&self, node: &GraphqlStringValue, f: &mut GraphqlFormatter) -> FormatResult<()> {
		...
		if node.is_block() {
			...
			// But leading whitespace is significant in block strings to calculate the
			// indentation level SAFETY:
			// We check that the string starts with triple quotes and the parser guarantees
			// that ends with triple quotes.
