...
impl FormatNodeRule<GraphqlUnionTypeExtension> for FormatGraphqlUnionTypeExtension {
	fn fmt_fields(&self, node: &GraphqlUnionTypeExtension, f: &mut GraphqlFormatter) -> FormatResult<()> {
		let GraphqlUnionTypeExtensionFields { extend_token, union_token, name, directives, union_members } =
			node.as_fields();
