...
impl FormatNodeRule<GraphqlFieldDefinition> for FormatGraphqlFieldDefinition {
	fn fmt_fields(&self, node: &GraphqlFieldDefinition, f: &mut GraphqlFormatter) -> FormatResult<()> {
		let GraphqlFieldDefinitionFields { description, name, arguments, colon_token, ty, directives } =
			node.as_fields();
