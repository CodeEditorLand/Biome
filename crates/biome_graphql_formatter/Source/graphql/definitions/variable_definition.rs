...
impl FormatNodeRule<GraphqlVariableDefinition> for FormatGraphqlVariableDefinition {
	fn fmt_fields(&self, node: &GraphqlVariableDefinition, f: &mut GraphqlFormatter) -> FormatResult<()> {
		let GraphqlVariableDefinitionFields { variable, colon_token, ty, default, directives } = node.as_fields();
