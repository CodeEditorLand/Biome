	...
	for FormatGraphqlRootOperationTypeDefinition
{
	fn fmt_fields(
		...
		let GraphqlRootOperationTypeDefinitionFields { operation_type, colon_token, named_type } =
			node.as_fields();

		write!(f, [operation_type.format(), colon_token.format(), space(), named_type.format(),])
	}
}
