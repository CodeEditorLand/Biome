...
impl FormatNodeRule<GraphqlInputValueDefinition> for FormatGraphqlInputValueDefinition {
	fn fmt_fields(&self, node: &GraphqlInputValueDefinition, f: &mut GraphqlFormatter) -> FormatResult<()> {
		let GraphqlInputValueDefinitionFields { description, name, colon_token, ty, default, directives } =
			node.as_fields();
