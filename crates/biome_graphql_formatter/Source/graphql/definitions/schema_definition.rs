...
impl FormatNodeRule<GraphqlSchemaDefinition> for FormatGraphqlSchemaDefinition {
	fn fmt_fields(&self, node: &GraphqlSchemaDefinition, f: &mut GraphqlFormatter) -> FormatResult<()> {
		let GraphqlSchemaDefinitionFields { description, schema_token, directives, root_operation_types } =
			node.as_fields();
