...
use biome_graphql_syntax::{GraphqlSyntaxToken, GraphqlUnionMemberTypes, GraphqlUnionMemberTypesFields};
...
impl Format<GraphqlFormatContext> for FormatTypeLeadingSeparator<'_> {
	fn fmt(&self, f: &mut GraphqlFormatter) -> FormatResult<()> {
		match &self.leading_separator {
			...
			None => {
				let content = format_with(|f| write!(f, [soft_line_break_or_space(), text(self.separator), space()]));
