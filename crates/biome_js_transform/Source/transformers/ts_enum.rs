use biome_analyze::{context::RuleContext, Ast, Rule};
...
fn make_function_caller(node:&TsEnumMembers) -> JsExpressionStatement {
	...
	let arguments =
		js_call_arguments(token(T!['(']), js_call_argument_list([argument], []), token(T![')']));
	let expression =
		js_call_expression(AnyJsExpression::JsParenthesizedExpression(callee), arguments).build();
...
fn make_function(node:&TsEnumMembers) -> JsFunctionExpression {
	...
	let body =
		js_function_body(token(T!['{']), js_directive_list([]), make_members(node), token(T!['}']));
...
fn make_members(ts_enum:&TsEnumMembers) -> JsStatementList {
	...
	for (index, (name, value)) in ts_enum.member_names.iter().enumerate() {
		...
		list.push(AnyJsStatement::JsExpressionStatement(make_high_order_assignment(
			ts_enum.name.as_str(),
			name.as_str(),
			value,
		)));
...
/// Foo["Lorem"]
/// ```
fn make_computed_member_assignment(enum_name:&str, member_name:&str) -> JsComputedMemberAssignment {
